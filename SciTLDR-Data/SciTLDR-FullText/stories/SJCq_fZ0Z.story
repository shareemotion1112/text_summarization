...
t{−8,−7,−6,−5,−4,−3,−2,−1} are evaluated, first by broadcasting the current provisional hidden stateĥ (t) against the macrostate (which, in the presented case of k att = 1, consists of all past hidden states), concatenating, then passing the result to an MLP.
...
In the models we use here, the selection mechanism is chosen to be a 1-hidden-layer Linear-TanhLinear MLP that computes a scalar attention weight a i for each eligible microstate vector m (i) , and a sparsifier that masks out all but the k top greatest attention weights, producing the sparse attention weightsã i .
...
While many options exist for doing so, we choose to simply perform a summation of the microstates, weighted by their sparsified attention weightã i .
...
At time t, the underlying LSTM receives a vector of hidden states h (t−1) , a vector of cell states c (t−1) and an input x (t) , and computes a provisional hidden state vectorĥ (t) that also serves as a provisional output.

We next use an attention mechanism that is similar to BID1 , but modified to produce sparse discrete attention decisions.

First, the provisional hidden state vectorĥ (t) is concatenated to each microstate vector m (i) .
...
Our failure to do so in an early iteration of the work resulted in a catastrophic cancellation in the subsequent sparsification of the weights toã (t) i .
