...
We present a method of robust transfer between visual RL domains, using attention and a β variational autoencoder to automatically learn a state representation sufficient to solve both source and target domains.
...
Specifically, it utilizes a β Variational Autoencoder (β-VAE) to learn a compressed, disentagled state representation (Higgins et al., 2017) .
...
Since the β-VAE is trained to reconstruct input states, the distributions of compressed and input states will match.
...
We assume that both D S and D T share a discount factor γ.
...
The deep network implicitly learns a mapping function F : S U →Ŝ z , for some MDP U ∈ M and a policy function π :Ŝ z → A that maps from latent state factors to actions.

These functions are learned as a result of the feed-forward nature of deep neural networks.

The policy π is a function of the last layer of the network.

The output of the last layer of the neural network is a highly processed feature representation of the input that is optimized for use in generating π.
...
By optimizing for π F overfits to the specific S U from the source domain U .
...
The RL agent is the trained on the (approximate) extracted latent state factorsŜ z , separating the "vision" and "action" of the RL agent.

DARLA uses a β variational autoencoder to learn the mapping F (Higgins et al., 2017) .

The extracted latent state factorsŜ z are (mostly) disentangled in practice, leading to better transfer results than a baseline RL agent.
...
SADALA learns the same mapping function F : S U →Ŝ z as DARLA, also utilizing a β variational autoencoder.

It then learns a policy π :Ŝ z → A, but utilizes a soft attention mechanism to ignore irrelevant features.

Using this mechanism, it implicitly learns two functions: A :Ŝ z → S a that maps from extracted latent state factors to latent state factors relevant to solving the MDP, and π A : S a → A that maps from relevant latent state factors to actions.
...
W is defined as W = σ(h(Z)) where h is a differentiable function parameterized by dense layers of a neural network.
...
At inference time, an input state s ∈ S is fed into a β variational autoencoder.

The bottleneck representation of the β-VAE is then used as extracted latent state factorsŜ z .
...
First, the β variational autoencoder, shown in figure 2 must be trained.

To do so, we use the loss term of a variational autoencoder, modified to fit the β-VAE (Higgins et al., 2017) .

The standard VAE loss is

where β is a weighting term that pressures the encoder q to output a latent space similar to an isotropic unit gaussian.

Increasing the value of β encourages disentanglement of the latent space while sacrificing minor reconstruction details.

We sample states from the environment, following a random policy.

We then train the β-VAE to reconstruct these images and freeze the network weights.

As shown by DARLA, freezing the network weights of the β-VAE is necessary (Higgins et al., 2017) .
...
We make the assumption that the ordering of the extracted latent state variablesŜ z does not change between subsequent frames within an episode.
...
Optimally, the agent should learn to ignore the factors in extracted latent state factorsŜ z that correspond to color, as they do not aid the agent in balancing the pole.
...
To test the results of the SADALA algorithm, we first test the reconstruction and disentanglement properties of the β-VAE used in the state representation stage.
...
Based on the reconstructions, it is apparent that the β-VAE has learned to represent cart position and pole angle.
...
This is due to imperfections in the β-VAE.

Though the β-VAE attempts to reconstruct the input image, it does not do so perfectly, as shown in figure 3 .
...
Additionally, while the β-VAE's goal is to learn a disentangled representation, it does not do so perfectly.
...
In the scenario where the extracted latent state factors from the β-VAE are perfectly disentangled, static attention weights should be sufficient to solve the source task and should transfer better to the target task, as in the Visual Cartpole task.

However, the β-VAE does not output perfectly disentangled factors, especially in more complex visual domains such as the Deepmind lab.

Thus, the amount of attention paid to each feature from the β-VAE may differ across tasks, violating the assumption that attention weights should be zero variance.
...
The β-VAE is trained on a set of MDPs covering all latent state factor combinations, the RL network and attention mechanism is trained on a subset of N and tested on a held-out MDP.

The neural network for the Visual Cartpole task consists of two parts: the β-VAE network and the policy network.

First, the encoder for the β-VAE network is composed of three convolutional layers, each with kernel size 3 and stride 1.
...
The encoder from the β-VAE is then frozen and used as a feature extractor for the policy network.
...
As in DARLA, the reconstruction loss of the β-VAE for Deepmind Lab takes place in the latent space of a denoising autoencoder with 100 latent factors Higgins et al. (2017) .
...
The β-VAE networks are trained on a set of 1, 000, 000 images sampled from the observed state space of source policies.
