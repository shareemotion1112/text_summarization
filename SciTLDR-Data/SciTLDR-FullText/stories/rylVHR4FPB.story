...
We use bold letters such as θ to denote random variables, and non-bold letters such as θ to denote their realizations.

We abbreviate Pr [θ = θ]

of N data points, we aim to learn a neural network with model parameters θ that predict the output y ∈ Y based on the input x ∈ X .

(1) We first solve the learning problem to find an approximate posterior distribution Q(θ; φ) over θ with parameters φ such that Q(θ; φ) ≈ Pr[θ|D].

(2) We then solve the prediction problem to compute the predictive distribution Pr[y|x, D] for arbitrary input x = x given Q(θ; φ).
...
We assume that θ (l) 's are layer-wise independent, i.e. Q(θ; φ) =

Computing the predictive distribution Pr[y|x, D] with a BNN requires marginalizing over the random variable θ.
...
This iterative process to compute the predictive distributions layer-by-layer sequentially is known as probabilistic propagation (Soudry et al., 2014; Hernández-Lobato & Adams, 2015; Ghosh et al., 2016) .

With this approach, we need to explicitly compute and store each intermediate result

is a function of x).

Therefore, probabilistic propagation is a deterministic process that computes ψ (l+1) as a function of ψ (l) and φ (l) , which we denote as
...
A standard approach to finding a good approximation Q(θ; φ) is variational inference, which finds φ such that the KL-divergence KL(Q(θ; φ)||Pr [θ|D] ) from Q(θ; φ) to Pr[θ|D] is minimized.

In Appendix B, we prove that to minimizing the KL-divergence is equivalent to maximizing an objective function known as the evidence lower bound (ELBO), denoted as L(φ).

where

Probabilistic Backpropagation.

Optimization in neural networks heavily relies on the gradientbased methods, where the partial derivatives ∂L(φ)/∂φ of the objective L(φ) w.r.t.

the parameters φ are obtained by backpropagation.

Formally, if the output produced by a neural network is given by a (sub-)differentiable function g(φ), and the objective L(g(φ)) is an explicit function of g(φ) (and not just an explicit function of φ), then the partial derivatives can be computed by chain rule:
...
Notice that (1) For classification, the function g(φ) returns the probabilities after the softmax function, not the categorical label; (2) An additional regularizer R(φ) on the parameters will not cause difficulty in backpropagation, given ∂R(φ)/∂φ is easily computed.

Challenge in Sampling-Free Probabilistic Backpropagation.

Learning BNNs is not amenable to standard backpropagation because the ELBO objective function L(φ) in (4b) is not an explicit (i.e. implicit) function of the predictive distribution g(φ) in (4a):

Although L n (φ) is a function of φ, it is not an explicit function of g n (φ).
...
With L n (φ) in hand, we can (approximately) find φ by maximizing the alternative objective via gradient-based method:

In Appendix C.1, we proved one feasible L n (φ) which only depends on second last output h (L−1) .

) is deterministic given input x and all parameters before the last layer θ

Analytic Forms of L n (φ).

While the lower bound in Theorem 3.1 applies to BNNs with arbitrary distributions P on hidden variables h, Q on model parameters θ, and any problem setting (e.g. classification or regression), in practice sampling-free probabilistic backpropagation requires that L n (φ) can be analytically evaluated (or further lower bounded) in terms of φ (L−1) and θ (L−1) .

This task is nontrivial since it requires redesign of the output layer, i.e. the function of Pr[y|h (L−1) , θ (L−1) ].
...
Since L n (φ) involves the last layer only, we omit the superscripts/subsripts of

, x n , y n , and denote them as h, ψ, φ, x, y .

with K the number of classes) be the pre-activations of a softmax layer (a.k.a.

logits), and φ = s ∈ R + be a scaling factor that adjusts its scale such that
...
To address this, we introduce Bayesian quantized networks (BQNs) -BNNs where both hidden units h (l) 's and model parameters θ (l) 's take discrete values -along with a set of novel algorithms for efficient sampling-free probabilistic propagation in BQNs.
...
where each term of ψ

k parameterizes a single categorical variable.

In our implementation, we store the parameters in their log-space, i.e. Q(θ
...
In the experiments, we consider a class of quantized neural networks, with both binary weights and activations (i.e. Q = {−1, 1}) with sign activations σ(·) = sign(·).

For BQNs, the distribution parameters φ are initialized by Xavier's uniform initializer, and all models are trained by ADAM optimizer (Kingma & Ba, 2014) for 100 epochs (and 300 epochs for CIFAR10) with batch size 100 and initial learning rate 10 −2 , which decays by 0.98 per epoch.
...
To allow for customized level of uncertainty in the learned Bayesian models, we introduce a regularization coefficient λ in the alternative ELBO proposed in Equation (5) (i.e. a lower bound of the likelihood), and train the BQNs by maximizing the following objective:
...
Once a BQN is trained (i.e. an approximate posterior Q(θ) is learned), we consider three modes to evaluate the behavior of the model: (1) analytic inference (AI), (2) Monte Carlo (MC) sampling and (3) Maximum a Posterior (MAP) estimation:

1.

In analytic inference (AI, i.e. our proposed method), we analytically integrate over Q(θ) to obtain the predictive distribution as in the training phase.
...
In MC sampling, S sets of model parameters are drawn independently from the posterior posterior θ s ∼ Q(θ), ∀s ∈ [S], and the forward propagation is performed as in (non-Bayesian) quantized neural network for each set θ s , followed by an average over the model outputs.
...
MAP estimation is similar to MC sampling, except that only one set of model parameters θ is obtained θ = arg max θ Q(θ).
...
To further decouple these two factors of mean-field approximation and relaxation of the original ELBO, we vary the regularization coefficient λ in the learning objective.

(1) For λ = 0 (where the prior term is removed), the models are forced to become deterministic during training.
...
(2) As λ increases, the gaps increases slightly as well, which shows that the mean-field approximation becomes slightly less accurate with higher learned uncertainty in the model.
...
This approach has multiple drawbacks: (a) Repeated sampling suffers from high variance, besides being computationally expensive in both learning and prediction phases; (b) While g n (φ) is differentiable w.r.t.

φ, f n (θ) may not be differentiable w.r.t.
...
General Bayesian model Formally, the graphical model in Figure 3a implies the joint distribution of the model parameters θ, the observed dataset D = {(x n , y n )} N n=1 and any unseen data point (x, y) is factorized as follows: [y|x, θ] .

In other words, we assume that (1) the samples (x n , y n )'s (and unseen data point (x, y)) are are identical and independent distributed according to the same data distribution; and (2) x n (or x) and θ together predict the output y n (or y) according to the same conditional distribution.
...
dθ (17) where we approximate the posterior distribution Pr[θ|D] by a parameterized distribution Q(θ; φ).

const.

( 22) where (1) L n (φ) is the expected log-likelihood, which reflects the predictive performance of the Bayesian model on the data point (x n , y n ); and (2) R(φ) is the KL-divergence between Q(θ; φ) and its prior Pr[θ], which reduces to entropy H(Q) if the prior of θ follows a uniform distribution.
...
Assumption B.1 (Independence of Model Parameters θ (l) ).

The approximate posterior Q(θ; φ) over the model parameters θ are partitioned into L disjoint and statistically independent layers {θ (l) } L−1 l=0 (where each φ (l) parameterizes θ (l) in the l-th layer) such that:

satisfy the Markov property that h (l+1) depends on the input x only through its previous layer h (l) :

where we use short-hand notations h ( : l) and θ ( : l) to represent the sets of previous layers {h (k) } l k=0
...
is not random variable (typical for an output layer), L n (φ) can be simplified as:

where we write Pr[

(L−1) can be obtained by differentiating over Equation (37), while other gradients ∂L n (φ)/φ ( : L−2) further obtained by chain rule:

which requires us to compute ∂L n (φ)/∂ψ (L−1) and ∂ψ

can be derived from Equation (37), ∂ψ (L−1) /∂φ ( : L−2) can be obtained by backpropagating outputs of the (L − 2) th layer obtained from probabilistic propagation in Equation (1).

In other words:

is a function of all parameters from previous layers φ ( : L−2) , and if each step
...
Theorem C.1 (Analytic Form of L n (φ) for Classification).
...
Then L n (φ) can be further upper bound by the following analytic form:
...
Derivatives of L n (φ) in (39) To use probabilistic backpropagation to obtain the gradients w.r.t.
...
First, we expand the function c (h) by Taylor's series at the point µ (up to the second order):
...
where the second term is zero by the symmetry of Pr[h k |x] around µ k (or simply the definition of µ k 's).

Therefore, the first-order approximation results exactly in a (deterministic) softmax function of the mean vector µ. In order to incorporate the variance into the approximation, we will need to derive the exact forms of the derivatives of c (h).
...
Theorem C.2 (Analytic Form of L n (φ) for Regression).

Let h ∈ R I be the output of last hidden layer (with I the number of hidden units), and φ = (w, s) ∈ R I × R + be the parameters that define the predictive distribution over output y as

Suppose the hidden units {h k } K k=1 are pairwise independent (which holds under mean-field approximation), and each h i has mean µ i and variance ν i , then L n (φ) takes an analytic form:

where ( Proof.

The Equation (68) is obtained by plugging Pr[y|h; w, s] into Equation (6).

where the long summation in the first term can be further simplified with notations of µ and ν:

where w •2 denotes element-wise square, i.e.

Derivatives of L n (φ) in Equation (68) It is not difficult to show that the gradient of L n (φ) can be backpropagated through the last layer.

by computing derivatives of L n (φ) w.r.t.
...
Furthermore, if we denote the (normalized) skewness and kurtosis of h i as γ i and κ i :
...
Depth-wise layers include dropout layers (where θ (l) are dropout rates), nonlinear layers (where θ (l) are threshold values) or element-wise product layers (where θ (l) are the weights).
...
Linear Layers Linear layers (followed by a nonlinear transformations σ(·)) are the most important building blocks in neural networks, which include fully-connected and convolutional layers.

A linear layer is parameterized by a set of vectors θ (l) 's, and maps

where u .

Let v j = σ(ṽ j ) = σ( i∈I(j) θ ji u i ) be an activation of a linear layer followed by nonlinearity σ.
