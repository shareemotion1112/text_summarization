...
Over a series of turns, an agent interacts with its environment via a Markov decision process, or MDP, (S, A, T , R, γ).

At each step t, an agent observes a state s ∈ S. The agent then chooses an action a ∈ A according to some policy π.
...
The goal of an agent is to maximize the cumulative discounted return T t=0 γ t r t .
...
In Q-learning with function approximation, an agent alternately collects experiences by acting greedily with respect to Q(s, a; θ Q ) and updates its parameters θ Q .

Updates proceed as follows.

For a given experiences (s t , a t , r t , s t+1 ), we minimize the squared Bellman error: DISPLAYFORM0 (1) for y t = r t + γ · max a Q(s t+1 , a ; θ Q ).

Traditionally, the parameterised Q(s, a; θ) is trained by stochastic approximation, estimating the loss on each experience as it is encountered, yielding the update: DISPLAYFORM1 Q-learning methods also require an exploration strategy for action selection.
...
Suppose that we are given a priori knowledge that acting according to the optimal policy π * , an agent never encounters states s ∈ S for which lie within distance d(s, c) < k τ for any catastrophe state c ∈ C. Then each state s for which ∃c ∈ C s.t.

d(s, c) < k τ is a danger state.
...
Instead of updating Q(s t , a t ; θ Q ) towards r t + max a Q(s t+1 , a ; θ Q ), we introduce the intrinsic fear to the model via the target: With probability select random action a t DISPLAYFORM0

Otherwise, select action a t = argmax a Q(s t , a ; θ Q )
...
For an MDP, M = S, A, T , R, γ , with 0 ≤ γ ≤ 1, the average reward return is as follows: DISPLAYFORM3 The optimal policy π * of the model M is the policy which maximizes the average reward return, π * = max π∈P η(π) where P is a set of stationary polices.

Theorem 1.

For a given MDP, M , with γ ∈ [0, 1] and a catastrophe detector f , let π * denote an optimal policy of M , andπ denote an optimal policy of M equipped with fear model F and λ.

If the probability π * visits the states in the danger zone is at most , and R min ≤ R(s, a) ≤ R max , then DISPLAYFORM4 Proof.

Appendix A.It is worth noting that when at least one of the optimal policies of M , does not visit the fear zone ( = 0), then η * M = η M,F (π) and the fear signal can boost up the process of learning the optimal policy.
...
In general, in practical RL problems, we use discount factors γ < 1 ( BID14 in order to reduce the planing horizon, and computation cost.
...
We show that under modeling errors, if the actual objective function to optimize for Eq. 4 has with discount factor γ eval , it's better to use some γ ≤ γ eval because it reduces the average deviation in the value functions.

For a given environment, with fear model F 1 and discount factor γ 1 , let V π * F 2 ,γ 2 F1,γ1 (s), s ∈ S, denote the state value function under the optimal policy of a environment with fear model F 2 and the discount factor γ 2 .

On the same environment, let ω DISPLAYFORM5 (s) denote the stationary distribution over states.
...
For a given MDP model, the average deviation on the value functions, L(F, F , γ eval , γ), F,F ∈ F, vanishes as the number of samples N increases DISPLAYFORM7 with probability at least 1 − δ.
...
So, in all of our experiments we gradually phase in the fear factor λ from 0 to λ reaching full strength at predetermined time step k λ .

In our Cart-Pole experiments, we phase λ in over 1M steps.
...
state s t , T (s t+1 |s t , a t ) gives successor state s t+1 ← s t + .01 · a t + η where η ∼ N (0, .01 2 ).
...
Formally, at each time, the agent observes a four-dimensional state vector (x, v, θ, ω) consisting respectively of the cart position, cart velocity, pole angle, and the pole's angular velocity.
...
On this problem we set the fear factor λ to 40.For Cart-Pole, we set a wider fear radius of k r = 20.
...
BID10 ) defines a fatality as any return below some threshold τ .
...
The average return of the reward under a policy π is as follows: DISPLAYFORM0 Let us assume that any stationary policy π induces a stationary distribution ω π (s), s ∈ S.

Therefore we can rewrite Eq. 7 in terms of stationary distribution BID24 .

DISPLAYFORM1 In RL, we are interested in a policy π * that maximizes the expected average reward: DISPLAYFORM2 .

In a first place, the optimization in Eq. 7 looks linear in π but actually the policy π derives the stationary distribution ω π (·), which makes the optimization problem harder.

Given the policy π, let's define the joint distribution in (s, a) as follows: DISPLAYFORM3 Then we can rewrite the optimization problem in terms of the joint probability distribution µ π .

DISPLAYFORM4 We can see that this new formalization, turns our optimization objective into a linear function of µ π .Since µ π is a join distribution of (s, a) under the model dynamics T , it can not take any arbitrary value.

Let ∆ denote the set of feasible value for µ π , then DISPLAYFORM5 T (s |s, a)µ(s, a), ∀s ∈ S} .Clearly, ∆ is a polytope on the simplex in R S×A .

Now, we can rewrite the optimization problem Eq. 7 as a constrained linear program on µ π : DISPLAYFORM6 This change of variable allows us to analyze the introduction of intrinsic fear in different situations.

Among all the optimal policies of Eq. 7, consider the one which minimizes := s∈C,a µ π * (s, a).let's assume that the negative reward assigned to the states in the danger zone is λ(R max − R min ) and the optimal policyπ of the environment with the intrinsic fear has return of η M,F (π).Applying policy π * on the environment with intrinsic fear gives a return of η * M − λ (R max − R min ).

Therefore, the return ofπ on the environment with intrinsic fear, η M,F , is lower bounded by η * M − λ (R max − R min ).

Therefore, applyingπ on the environment without intrinsic fear gives a return of η M (π) which is lower bounded by η * DISPLAYFORM7 A.1 DISCOUNTED CUMULATIVE REWARD For the γ-discounted setting, we are interested in DISPLAYFORM8 The above mentioned equations hold in this setting as well, i.e., η * DISPLAYFORM9
...
In general, in practical RL problems, we use discount factors γ eval < 1 ( BID14 in order to reduce the planing horizon, and computation cost.

Moreover, BID12 suggest that when we have an estimation (up to the confidence intervals) of our MDP model, it is better to use γ ≤ γ eval .
...
In this section, we show how to choose the discount factor γ ≤ γ eval such that the learned Value function stays close to the Value function under the perfect classifier F is perfect.

Let, V π * F 1 ,γ 1 F2,γ2 (s), s ∈ S, denote the state value under the optimal policy of model with classifier F 1 under the discount factor γ 1 on the environment equipped with classifier F and discount factor γ 2 .

On the same environment, ω DISPLAYFORM0

(s) denotes the stationary distribution over states.

We are interested in the average deviation on value functions caused by the imperfect classifier: DISPLAYFORM0 This quantity can be upper bounded by DISPLAYFORM1 The goal is to find an γ * which minimizes this loss, i.e. γ * = argmin γ≤γ eval L with high probability.

For simplicity and without loss of generality, let's assume that all the rewards, including the intrinsic fears, are in [0, 1] and call λ , the transformed version of λ 2 .
...
Since γ ≤ γ eval we have V DISPLAYFORM3 The second part of Eq. 13 is the deviation in value function under different policies and different DISPLAYFORM4 where the last inequality is due to the optimality of π * F,γ on the environment of F, γ.

To bound this part we exploit the proof trick used in BID12 .

DISPLAYFORM5 since the middle term is negative we have DISPLAYFORM6 This quantity V π F ,γ DISPLAYFORM7 is the difference between the performance of the same policy on two different environments.

These two values functions should satisfy the following bellman equations: As i tends to infinity, these two dynamics updates converge to V DISPLAYFORM8 As i tends to infinity, we have (1 − γ)
...
with probability δ where N (s) is the number visits to a state s at time step N .
...
In order to have this bound to hold for all the states at once, we need another union bounds over states and all possibly optimal policies Π γ under noisy classifier , which requires to replace δ → δ/SAΠ γ .
...
Let F denote a set of given binary classifiers and F ∈ F. In this case, let's assume that we are given a set of N i.i.d samples from the stationary distribution ω π * F ,γ

.

Given a policy π, the MDP transition process reduces to a Markov chain with transition probability T π .

Now we rewrite the Eq. 19 in a matrix format where V π i , F ∈ R S are vectors of concatenation of V π i (s) and F (s), ∀s ∈ S respectively.

DISPLAYFORM0 as i goes to infinity we have DISPLAYFORM1 Using PAC analysis of binary classification in BID9 a follow up to BID32 , we have DISPLAYFORM2 ≤ 3200 VC(F) + log 1 δ N with probability at least 1 − δ where VC(F) is the VC dimension of the hypothesis class and | · | is entry-wise absolute value.

Since γ < 1, then α max , the maximum eigenvalue of (1 − γT π ) −1 , is bounded above and we have The same analysis, up to a slight modification 3 , holds for the continuous state and action spaces.
