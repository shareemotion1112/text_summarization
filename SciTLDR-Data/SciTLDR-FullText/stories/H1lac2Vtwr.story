...
We restrictÔ h i,j,: to a local scope with a fixed size k centered at the position i and dimension j, where 1 ≤ j ≤ d, and k can be any odd number between 1 and l, expressed as follows:

We then enhance the localness ofÔ h i,j,: through a parameter-free 1D convolution operation with g.

The final attention output is O h , which is the dot product between the Gaussian kernel and the corresponding input array elements at every position ofÔ
