...
We now define our pointer network model (see FIG2 ).Given a program token sequence f = t 1 , t 2 , · · · , t n , we embed the tokens t i using a trainable embedding matrix φ : T → R d , where d is the embedding dimension.
...
We then perform attention over the hidden states using a mechanism similar to that of BID23 as follows: DISPLAYFORM2 where W 1 , W 2 ∈ R h×h are trainable projection matrices and 1 n ∈ R n is a vector of ones used to obtain n copies of the final hidden state h n .We then use another trained projection matrix W ∈ R h×2 to compute the α ∈ R 2×n as follows: DISPLAYFORM3 The attention matrix α corresponds to two probability distributions over the program tokens.

The first distribution α [0] ∈ R n corresponds to location token indices and the second distribution α [1] ∈ R n corresponds to repair token indices.
...
We define two parameters to filter the predictions: 1) τ : a threshold value for probabilities to decide whether to return the predictions, and 2) k: the maximum number of predictions the enumerative approach is allowed to make.

The results for the comparison for different τ and k values are shown in TAB2 .
...
A higher τ value (and lower k value) reduces the number of model predictions compared to lower τ values (and higher k values).

As expected, higher τ and lower k values enable the enumerative approach to achieve a higher true positive rate, but a lower classification accuracy rate.

More importantly for buggy programs, the localization and repair accuracy drop quite sharply.

With lower τ and higher k values, the true positive rate drops dramatically, while the localization and repair accuracy improve significantly.
...
Figure 3a shows that for Any, the model loses significant accuracy, dropping about 4.3 percentage points for τ = 0.5.

The accuracy drop is lower as a higher prediction probability is required by higher τ , but it is already catastrophically low.

Results are even worse for the more adversarial Near.

As shown in Figure 3b , accuracy drops between 8 and 14.6 percentage points for different reporting thresholds τ .These experiments show that a repair prediction performed on an unlikely fault location can significantly impair repair, and hence the overall enumerative approach, since it relies on repair predictions for both localization and repair.
