...
In the case of GNNs (Scarselli et al., 2008) , GATs (Veličković et al., 2018) achieved stateof-the-art performance by using the attention mechanism.
...
The addition of self-loops to the adjacency matrix is A = A + I n , and the column normalized matrix of A isÂ = AD −1 withÂ 0 = I n .
...
The output is then multiplied by a normalized adjacency matrixÂ for each random walk step that is to be considered.
...
where is the concatenation operation, s is the maximum number of steps considered for aggregation, andÂ k is the normalized adjacency matrixÂ multiplied k times.

As can be seen from Equation 3, weights are shared across nodes.

In our method, the adjacency matrixÂ is an asymmetric matrix, which is generated by random walks and flexible to arbitrary graphs.
...
As described in Equation 4, we employ multi-head attention, where H multi is the concatenation of m attention layers and α is the coefficient of attention computed using concatenated features of nodes and its neighboring nodes.
...
We use the protein-protein interaction PPI dataset (Zitnik & Leskovec, 2017) ,which is preprocessed by Veličković et al. (2018) .
...
Our model is compared with baseline models specified in (Veličković et al., 2018) such as label propagation (LP) (Xiaojin & Zoubin, 2002) , graph embeddings via random walk (DeepWalk) (Perozzi et al., 2014) , and Planetoid (Yang et al., 2016) .
...
We trained for a maximum of 300 epochs with L2 regularization λ = 0.003 and learning rate lr = 0.001.
...
L2 regularization and dropout were not used for inductive learning (Veličković et al., 2018) .
...
These results for unseen graphs are in good agreement with results shown by Veličković et al. (2018) , in which reducing the influence of structural information improved generalization.
...
As shown in Figure 4a , GCN (Kipf & Welling, 2017) , SGC (Wu et al., 2019a) , and GAT (Veličković et al., 2018) suffer from oversmoothing.
...
The inference time of GESM is much faster than GAT (Veličković et al., 2018) while providing higher accuracies and stable results (see Appendix A).
...
93.0 ± 0.8 82.4 ± 1.8 91.4 ± 1.3 GAT (Veličković et al., 2018) 90.5 ± 0.6 92.5 ± 0.9 78.0 ± 19.0 85.7 ± 20.3 GSM (our base model) 91.8 ± 0.4 93.3 ± 0.6 79.2 ± 2.0 89.3 ± 1.9 GESM (GSM+attention)
