...
We model the agent-environment interaction with a MDP (S, A, R, P, γ), where S and A are the state and action spaces, R(s, a) is the stochastic reward function, P (s |s, a) is the probability of transitioning from state s to state s after taking action a, and γ ∈ [0, 1) is the discount factor.

A policy π(·|s) ∈ P(A) maps a state s ∈ S to a distribution over actions.

For a fixed policy π, the discounted return of action a in state s is a random variable Z π (s, a) = ∞ t=0 γ t R(s t , a t ), with initial state s = s 0 and action a = a 0 and transition probabilities s t ∼ P (·|s t−1 , a t−1 ), a t ∼ π(·|s t ).

The return distribution Z statisfied the Bellman equation, DISPLAYFORM0 where D = denotes distributional equality.

If we take the expectation of (1), the usual Bellman equation BID5 for the Q-function, DISPLAYFORM1 The objective is to find an optimal policy π * that maximizes the expected total discounted return DISPLAYFORM2
...
First, each head Q k (s, a; θ) is trained on its own independent target head Q k (s, a; θ − ), which is periodically updated BID25 .
...
Intrinsic uncertainty is captured by the return distribution Z π .

While Q-learning (Watkins, 1989) aims to estimate the expected discounted return DISPLAYFORM1 , distributional RL approximates the random return Z π (s, a) directly.
...
Since the parameterization Z θ and the Bellman update T Z θ have disjoint supports, the algorithm requires an additional step Φ that projects the shifted support DISPLAYFORM3
...
This is due to Bellman targets being generated based on an evolving policy π.
...
In Temporal Difference (TD) algorithms BID37 , given a sample transition (s t , a t , r t , s t+1 ), the learning target is generated as y t = r t + γQ π (s t+1 , a ), for some action a .

Similarly to the observation model above, we can describe TD-targets for learning Q * being generated as DISPLAYFORM0 The last term clearly shows the dependence of the noise function π t (s, a) on the policy π, used to generate the Bellman target.

Note additionally that heteroscedastic targets are not limited to TDlearning methods, but also occur in TD(λ) and Monte-Carlo learning BID37 , no matter if the environment is stochastic or not.
...
Intuitively, IDS chooses actions with small regret-information ratioΨ t (a) :=∆ DISPLAYFORM2 It(a) to balance between incurring regret and acquiring new information at each step.

BID20 introduce several information-gain functions and derive a high-probability bound on the cumulative regret, DISPLAYFORM3 Here, γ T is an upper bound on the total information gain T t=1 I t (a t ), which has a sublinear dependence in T for different function classes and the specific information-gain function we use in the following BID35 .
...
One particular choice of the information gain function, that works well empirically and we focus on in the following, is I t (a) = log 1 + σ t (a) 2 /ρ(a) 2 BID20 .

Here σ t (a) 2 is the variance in the parametric estimate of E[R(a)] and ρ(a) 2 = Var[R(a)] is the variance of the observed reward.
...
Importantly, note that ρ(a) 2 may explicitly depend on the selected action a, which allows the policy to account for heteroscedastic noise.
...
, R(a N ) by a multivariate Gaussian, with covariance Cov[R(a i ), R(a j )] = κ(x i , x j ), where κ is a positive definite kernel.

In our toy example, the goal is to maximize R(x) under heteroscedastic observation noise with variance ρ(x) 2 ( FIG0 ).

As UCB and TS do not consider observation noise in the acquisition function, they may sample at points where ρ(x) 2 is large.
...
Since ∆ π t (s, a) depends on the true Qfunction Q π , which is not available in practice and can only be estimated based on finite data, the IDS framework instead uses a conservative estimate.
...
DISPLAYFORM3 Execute action a t = arg min a∈AΨ (s t , a), observe r t and state s t+1 end for end for Given the regret surrogate, the only missing component to use the IDS strategy in eq. FORMULA9 is to compute the information gain function I t .

In particular, we use I t (a) = log 1 + σ t (a) 2 /ρ(a) 2 based on the discussion in BID20 .

In addition to the previously defined predictive parameteric variance estimates for the regret, it depends on the variance of the noise distribution, ρ.

While in the bandit setting we track one-step rewards, in RL we focus on learning from returns from complete trajectories.

Therefore, instantaneous reward observation noise variance ρ(a) 2 in the bandit setting transfers to the variance of the return distribution Var (Z(s, a)) in RL.
...
We also experimentally found this version to give better results compared to the unnormalized variance ρ(s, a) 2 = Var (Z(s, a)).
...
Each head Q k (s, a; θ) is trained against its own target head Q k (s, a; θ − ) and all heads are trained on the exact same data.

We use Double DQN targets BID41 and normalize gradients propagated by each head by 1/K.To estimate Z(s, a), it makes sense to share some of the weights θ from the Bootstrapped DQN.

We propose to use the output of the last convolutional layer φ(s) as input to a separate head that estimates Z(s, a).

The output of this head is the only one used for computing ρ(s, a) 2 and is also not included in the bootstrap estimate.
...
To isolate the effect of noise-sensitive exploration from the advantages of distributional training, we do not propagate distributional loss gradients in the convolutional layers and use the representation φ(s) learned only from the bootstrap branch.
...
Importantly, our method can account for deep exploration, since both the parametric uncertainty σ(s, a) 2 and the intrinsic uncertainty ρ(s, a) 2 estimates in the information gain are extended beyond a single time step and propagate information over sequences of states.
...
Our code can be found at https:// github.com/nikonikolov/rltf/tree/ids-drl.We evaluate two versions of our method: a homoscedastic one, called DQN-IDS, for which we do not estimate Z(s, a) and set ρ(s, a)2 to a constant, and a heteroscedastic one, C51-IDS, for which we estimate Z(s, a) using C51 as previously described.
...
We also use the suggested hyperparameters from C51 and Bootstrapped DQN, and set learning rate α = 0.00005, ADAM = 0.01/32, number of heads K = 10, number of atoms N = 51.
...
One way to get a risk-sensitive IDS policy is by tuning for β in the additive IDS formulationΨ(s, a) =∆(s, a) 2 − βI(s, a), proposed by BID31 .
...
Plots obtained by sampling a random batch of 32 states from the replay buffer every 50000 steps and computing the estimates for ρ 2 (s, a) based on eq. (9).

A histogram over the resulting values is then computed and displayed as a distribution (by interpolation).

From top to bottom, the lines on each plot correspond to standard deviation boundaries of a normal distribution [max, µ + 1.5σ, µ + σ, µ + 0.5σ, µ, µ − 0.5σ, µ − σ, µ − 1.5σ, min].
