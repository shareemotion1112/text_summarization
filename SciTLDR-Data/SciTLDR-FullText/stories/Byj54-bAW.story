...
These features are selected from a given parametric family F = {f θ : R s → R : θ ∈ Θ}, such as Gaussian kernels, wavelet functions, or learned features.
...
, r L−1 ∈ N are the number of channels in the hidden layers, {a γ∈[r0] are the weights in the first hidden convolutions, DISPLAYFORM7 DISPLAYFORM8 are the weights of the hidden layers, and a L,1,y ∈ R r L−1 stores the weights corresponding to the output y in the output layer.
...
Note that, for the special case where the input features belong to an exponential kernel family, such as F = {f θ (x) = e θ T x : R s → R : θ ∈ Θ} or F = {f θ (x) = e θ−x p : R s → R : θ ∈ Θ} where · p denotes the p norm with p ∈ N, the number of polynomial terms is equivalent to the number of exponential basis that the network function can realize.
...
For the sake of comparison, let us assume networks with hidden layer widths r l decaying (or increasing) at an exponential rate of λ ∈ R. Formally, this is r l = λr l−1 ∈ N, thus r l = (λ) l r for all l = 0, 1, . . .

, L − 1, where r r 0 .

To shorten the notation, we denote as (L, r, λ, k) to a ConvAC with of exponential width decay λ ∈ R, length L ∈ N, initial with r ∈ N and growth-rate k ∈ N. A growth-rate of k = 0 refers to a standard ConvAC with no dense connections.

Definition 1 Suppose that the weights of a (L, r, λ, k) ConvAC, with L, k ∈ N and r, λ ∈ R, are randomly drawn according to some continuous non-vanishing distribution.

Then, this (L, r, λ, k) ConvAC is said to have weak dense gain G w ∈ R if, with probability p > 0, we obtain score functions that cannot be realized by a (L, r , λ, 0) ConvAC with r < G w r. When p = 1, this (L, r, λ, k) ConvAC is said to have a strong dense gain G s = G w ∈ R.Using this definition we present a bound for the weak dense gain G w in the following theorem.
...
If a ConvAC has a dense gain G w = (1 + 1 λ ) that is already close to the general bound from Theorem 5.1 it is less encouraging to include broader dense connections, since it would increase the number of parameters of the model while there is no room for a significant expressive gain increase.
...
Proposition 3 Let ∆P dense ∈ N be the additional number of parameters that are added to a (L, r, λ, 0) ConvAC when we introduce dense connections of growth-rate k > 0.

In the same manner, let ∆P stand ∈ N be the number of parameters that are added to a (L, r, λ, 0) ConvAC when we increase its initial width r by a factor G ∈ R. Then the ratio between ∆P dense and ∆P stand is greater than DISPLAYFORM1 Proof See appendix B.4.The factor G from this proposition directly relates to the dense gain of a ConvAC, thus this ratio may be used to decide whether is interesting to add dense connections to a model (we want this ratio to be as large as possible).
...
Proof DISPLAYFORM0 T ∈ R M N , the output of the l-th layer of a (L, r, λ, 0) ConvAC can be stored into the vectors of mappings DISPLAYFORM1 .

Moreover, since the entries of these vectors are the result of l − 1 convolution-pooling layers with product pooling of window size 2, all the mappings δ l,j 1 (x) can be expressed as a sum of polynomial terms on x of degree 2 l .

Now, let the coefficient vectors a DISPLAYFORM2 ], be the weight vectors for the convolution of the l-th layer.

To shorten the notation we use a l,j,γ , DISPLAYFORM3 as shorthand for the convolution between these vectors.
...
In order to compute the convolutions of this layer, an additional vector of coefficients is required as b DISPLAYFORM7 Then, the outputs of the l-th layer of this (L, r, λ, k) ConvAC are the denoted as the vectorsδ DISPLAYFORM8 where DISPLAYFORM9 Note that the entries of ω l,j (x) are assumed to come from preceding layers with an appropiate average pooling.

Since performing avergae pooling does not increase the degree of the polynomial terms involved (only product pooling does) and the jump length Ljump is at least 1, the entries of ω l,j (x) have at most polynomial degree 2 l−1 , which is strictly less than the degree of the entries of δ l,j (x) (i.e., 2 l ).

Therefore, from the obtained expression of ω l+1,j we observe that it has polynomials withb degree no greater than 2 l + 2 l−1 , while the entries of δ l+1,j have a strictly higher degree of DISPLAYFORM10 Moreover, since a l,j,γ , δ l,j + ω l,j can be expressed as DISPLAYFORM11 we can make use of the obtained results in an unductive manner up to the L-th layer, thus leading to DISPLAYFORM12 , where g(x) contains polynomial terms of x of order strictly less than N , thus proving this theorem.
...
Using this assumption, we upper bound the rank of the grid tensor as DISPLAYFORM14 It was shown in that, when the weights are independently generated from some continuous distribution, we have that rank φ 1,j,γ = min{r 0 , M } with probability 1.
...
Finally, by induction up to the L-th layer, we obtain a bound for the grid tensor rank as DISPLAYFORM15 Since we assumed networks with hidden layer widths r l decaying (or increasing) at an exponential rate of λ ∈ R. Formally, this is r l = λr l−1 ∈ N, thus r l = (λ) l r for all l = 0, 1, . . .

, L − 1, where r r 0 .

Therefore, we may simplify the obtained bound to DISPLAYFORM16 We this analysis by proving Theorem 5.1.

To that end let A y dense be the grid tensor of a dense (L, r, λ, k) ConvAC with k > 0, while A y stand is the grid tensor of a (L, r , λ, 0) ConvAC with r ∈ R. As discussed in Section 4, this dense version of the former (L, r, λ, 0) ConvAC is equivalent to virtually increasing the widths of the ConvAC, which translates extra additive terms in the expressions from 12.

Moreover, using corollary 1 we observe that, if the ranks of the tensors φ l,j,γ are additive and multiplicative up to rank(A y dense ) > rank(A y stand ), so they are up to rank(A y stand ).
...
This leads to rank φ 1,j,γ = rank Note that for r l = λr l−1 ∈ N (λ ∈ R), we get virtually increased widthsr l = (1 + λ) l r = λ 1 + As in for the proof of Theorem 5.1, the maximum dense gain G w is obtained when rank(A y dense ) reaches the maximum possible rank.

In this particular case, this corresponds to rank(A By definition, we have that ∆P std = P (L, Gr, λ, 0) − P (L, Gr, λ, 0) and ∆P dense = P (L, Gr, λ, k) − P (L, r, λ, 0), thus yielding Finnaly, we use these expressions to compute the ratio of interest as DISPLAYFORM18 2 ) l + (G 2 − 1) k q=1 λ −q which proves this proposition.
