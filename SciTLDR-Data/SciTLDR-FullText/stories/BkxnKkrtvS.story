...
We call the NER task of predicting tags for tokens inference, and model it with a discriminative distribution q φ (y 1:N |x 1:N ) having parameters φ.
...
We use the same architecture for q φ throughout this work, as follows:
...
Combine local and transition potentials: ψ yi,yi+1 = s yi + T yi,yi+1 , T yi,yi+1 ∈ R 5.

Using special start and end states y 0 = * , y N +1 = with binary potentials ψ * ,y = T * ,y , ψ y, = T y, and the forward algorithm (Lafferty et al., 2001) to compute the the partition function Z, we can compute the joint distribution:

Our tagging CRF has trainable parameters φ = {W 1 , b 1 , V, b 2 , T } 3 and we learn them on a dataset of fully annotated sentences D S = {(x i 1:N i , y i 1:N i )} using stochastic gradient descent (SGD) and maximum likelihood estimation.
...
The purpose of this work is to consider methods for estimation of q φ in semi-supervised data regimes, as in Kingma et al. This marginalization is intractable for models that are not factored among y i , so we resort to optimizing the familiar evidence lower bound (ELBO) (Jordan et al., 1999; Blei et al., 2017) with an approximate variational posterior distribution, which we set to our tagging model q φ .
...
where α is scalar hyper-parameter used to balance the supervised loss L S and the unsupervised loss L U (Kingma et al., 2014) .

β is a scalar hyper-parameter used to balance the reconstruction and KL terms for the unsupervised loss (Bowman et al., 2015; Higgins et al., 2017) .
...
We also explore using this approach simply for regularization of the CRF posterior by omitting the token model p θ (x|y).
...
Optimizing Equations 5 and 6 with respect to θ and φ using backpropagation and SGD is straightforward for every term except for the expectation terms E q φ (y|x) [log p θ (x|y)].

To optimize these expectations, we first make an Monte Carlo approximation using a single sample drawn from q φ .

This discrete sample, however, is not differentiable with respect to φ and blocks gradient computation.

While we may appeal to score function estimation (Miller, 1967; Williams, 1992; Paisley et al., 2012; Ranganath et al., 2014; Mohamed et al., 2019) to work around this, its high-variance gradients make successful optimization difficult. (2019), we can compute approximate samples from q φ that are differentiable with respect to φ using the Relaxed Perturb-and-MAP algorithm (Corro & Titov, 2018; Kim et al., 2019) .
...
The KL between q φ and this distribution can be computed in polynomial time using a modification of the forward recursion derived in Mann & McCallum (2007) , detailed in Appendix B.

We experiment with several variations of architectures for p θ (x 1:N |y 1:N ), presented in order of increasing complexity.
...
where σ X is the softmax function normalized over X .

This model is generalization of the CRF Autoencoder architecture in Appendix C where the tag-token parameters θ x,y are computed with a low-rank factorization W U .
...
Next, we see if we can leverage information from a pretrained language model to provide additional training signal to p θ .
...
MT-GPT2-Residual: Our last variation directly couples GPT2 with p θ by predicting a residual via a two-layer MLP based on the tag encoding and GPT2 state:
...
We then explore the inclusion of the priors for the supervised and MT models with β = 0.01.
...
We find that including the prior with a small weight, β = 0.01, marginally improved performance and interestingly, the simple prior outperforms the empirical prior, most likely because it is slightly smoother and does not emphasize the O tag as heavily.
...
We also find that in this case, the MT models need to be trained with higher prior weights β = 0.1, otherwise they diverge towards using the O tag more uniformly with the other tags to achieve better generative likelihoods.

5 Code and experiments are available online at github.com/<anonymizedforsubmission> 6 In preliminary SSL experiments we found β > 0.01 to have a negative impact on performance, likely due to global/local normalization mismatch of the CRF and the prior.
...
A key challenge for learning VAEs with discrete latent variables is optimization with respect to the inference model parameters φ.
...
Alternatively, obtaining gradients with respect to φ can be achieved using the relaxed Gumbel-max trick (Jang et al., 2016; Maddison et al., 2016) and has been recently extended to latent tree-CRFs by (Corro & Titov, 2018) , which we make use of here for sequence CRFs.
...
Gumbel, 1954) and τ ≥ 0 be the temperature:
...
where q φ (y|x; τ ) is the tempered but unperturbed q φ and "one-hot" is a function that converts elements of Y N to a one-hot vector representation.

Thus we can use the temperature τ to anneal the perturbed joint distributionq φ (y|x; τ ) to a sample from the unperturbed distribution,ỹ ∼ q φ .

When τ > 0,q φ (y|x; τ ) is differentiable and can be used for end-to-end optimization by allowing us to approximate the expectation with a relaxed single-sample Monte Carlo estimate:
...
While this algorithm does provide practical convenience, we note that real implementations should have separate routines for computing the partition function (running only the forward algorithm), and the discrete τ = 0 Viterbi algorithm, since it is more numerically stable and efficient.

We also have included the dynamic program for computing the constrained KL divergence between q φ and a factored p(y) in Algorithm 2.

The idea of using a CRF to reconstruct tokens given tags for SSL has been explored before by Ammar et al. (2014) ; Zhang et al. (2017) , which we consider to be a strong baseline and restate ψ yi,yi+1 + log p θ (x i |y i )} − log Z(ψ) = log Z(ψ + log p θ ) − log Z(ψ)

where log Z(ψ + log p θ ) is a slight abuse of notation intended to illustrate that the first term in Equation 12 is the same computation as the partition function, but with the generative log-likelihoods added to the CRF potentials.

We note that instead of using the Mixed-EM procedure from Zhang et al. (2017) , we model p θ (x i |y i ) using free logit parameters θ x,y for each token-tag pair and normalize using a softmax, which allows for end-to-end optimization via backpropagation and SGD.
...
Additionally, we use gradient clipping of 5 for all models and a temperature of τ = .66 for all relaxed sampling models.
