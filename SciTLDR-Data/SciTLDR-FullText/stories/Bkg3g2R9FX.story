...
In such settings, RMSPROP and ADAM provably do not converge to an optimal solution, and furthermore, however large the initial step size α is, it is impossible for ADAM to fight against the scale-down term.
...
Notations Given a vector θ ∈ R d we denote its i-th coordinate by θ i ; we use θ k to denote elementwise power of k and θ to denote its 2 -norm; for a vector θ t in the t-th iteration, the i-th coordinate of θ t is denoted as θ t,i by adding a subscript i.
...
Note that the algorithm is still abstract since the functions φ t : F t → R d and ψ t : DISPLAYFORM2 + have not been specified.

In this paper, we refer to α as initial step size and α t / √ V t as Algorithm 1 Generic framework of optimization methods Input: x 1 ∈ F, initial step size α, sequence of functions {φ t , ψ t }

1: for t = 1 to T do 2: DISPLAYFORM0 Vt (x t+1 ) 7: end for learning rate of the algorithm.

Note that we employ a design of decreasing step size by α t = α/ √ t for it is required for theoretical proof of convergence.

However such an aggressive decay of step size typically translates into poor empirical performance, while a simple constant step size α t = α usually works well in practice.
...
We can see the scaling term ψ t is I in SGD(M), while adaptive methods introduce different kinds of averaging of the squared values of past gradients.

ADAM and RMSPROP can be seen as variants of ADAGRAD, where the former ones use an exponential moving average as function ψ t instead of the simple average used in ADAGRAD.

In particular, RMSPROP is essentially a special case of ADAM with β 1 = 0.

AMSGRAD is not listed in the table as it does not has a simple expression of ψ t .

It can be defined as ψ t = diag(v t ) wherev t is obtained by the following recursion: DISPLAYFORM1 The definition of φ t is same with that of ADAM.
...
Second, as the learning rate highly depends on the initial step size, can we use a relatively larger initial step size α to get rid of too small learning rates?To answer these questions, we show that undesirable convergence behavior for ADAM and RM-SPROP can be caused by extremely small learning rates, and furthermore, in some cases no matter how large the initial step size α is, ADAM will still fail to find the right path and converge to some highly suboptimal points.

Consider the following sequence of linear functions for F = [−1, 1]: DISPLAYFORM3 where C ∈ N satisfies: 5β DISPLAYFORM4 For this function sequence, it is easy to see that the point x = −1 provides the minimum regret.

Supposing β 1 = 0, we show that ADAM converges to a highly suboptimal solution of x ≥ 0 for this setting.
...
There is an online convex optimization problem where for any initial step size α, ADAM has non-zero average regret i.e., R T /T 0 as T → ∞.We relegate all proofs to the appendix.

Note that the above example also holds for constant step size α t = α.

Also note that vanilla SGD does not suffer from this problem.

There is a wide range of valid choices of initial step size α where the average regret of SGD asymptotically goes to 0, in other words, converges to the optimal solution.
...
One may wonder if using large β 1 helps as we usually use β 1 close to 1 in practice.

However, the following result shows that for any constant β 1 and β 2 with β 1 < √ β 2 , there exists an example where ADAM has non-zero average regret asymptotically regardless of the initial step size α.

Theorem 2.

For any constant β 1 , β 2 ∈ [0, 1) such that β 1 < √ β 2 , there is an online convex optimization problem where for any initial step size α, ADAM has non-zero average regret i.e., DISPLAYFORM5 Furthermore, a stronger result stands in the easier stochastic optimization setting.

Theorem 3.

For any constant β 1 , β 2 ∈ [0, 1) such that β 1 < √ β 2 , there is a stochastic convex optimization problem where for any initial step size α, ADAM does not converge to the optimal solution.

Remark.

The analysis of ADAM in BID7 relies on decreasing β 1 over time, while here we use constant β 1 .

Indeed, since the critical parameter is β 2 rather than β 1 in our analysis, it is quite easy to extend our examples to the case using decreasing scheme of β 1 .As mentioned by BID14 , the condition β 1 < √ β 2 is benign and is typically satisfied in the parameter settings used in practice.
...
Consider applying the following operation in ADAM DISPLAYFORM1 which clips the learning rate element-wisely such that the output is constrained to be in [η l , η u ].

It follows that SGD(M) with α = α * can be considered as the case where η l = η u = α * .

As for ADAM, η l = 0 and η u = ∞. Now we can provide the new strategy with the following steps.

We employ η l and η u as functions of t instead of constant lower and upper bound, where η l (t) is a non-decreasing function that starts from 0 as t = 0 and converges to α * asymptotically; and η u (t) is a non-increasing function that starts from ∞ as t = 0 and also converges to α * asymptotically.
...
Suppose β 1t = β 1 λ t−1 in Theorem 4, we have DISPLAYFORM2 It is easy to see that the regret of ADABOUND is upper bounded by O( et al. (2018) , one can use a much more modest momentum decay of β 1t = β 1 /t and still ensure a regret of O( √ T ).
...
The results are placed in Appendix G for we find that the convergence target α * and convergence speed are not very important to the final results.

For the sake of clarity, we will use η l (t) = 0.1− 0.1 (1−β2)t+1 and η u (t) = 0.1+ 0.1(1−β2)t in the rest of the paper unless otherwise specified.
...
For instance, SGD with α = 10 performs best for language modeling on PTB but for the ResNet-34 architecture on CIFAR-10, a learning rate of 0.1 for SGD is necessary.
...
The initial step sizes are chosen from: {1e-2, 5e-3, 1e-3, 5e-4, 1e-4}. We turn over β 1 values of {0.9, 0.99} and β 2 values of {0.99, 0.999}. We use for the perturbation value = 1e-8.ADABOUND & AMSBOUND We directly apply the default hyperparameters for ADAM (a learning rate of 0.001, β 1 = 0.9 and β 2 = 0.999) in our proposed methods.
...
Suppose m t = β 1 m t−1 + (1 − β 1 )g t with m 0 = 0 and 0 ≤ β 1 < 1.

We have DISPLAYFORM5 Proof.

If β 1 = 0, the equality directly holds due to m t = g t .

Otherwise, 0 < β 1 < 1.

For any θ > 0 we have DISPLAYFORM6 The inequality follows from Cauchy-Schwarz and Young's inequality.

In particular, let θ = 1/β 1 − 1.
...
First, we rewrite the update of ADAM in Algorithm 1 in the following recursion form: DISPLAYFORM0 where m 0,i = 0 and v 0,i = 0 for all i ∈ [d] and ψ t = diag(v t ).

We consider the setting where f t are linear functions and F = [−1, 1].

In particular, we define the following function sequence: DISPLAYFORM1 for t mod C = 1; 2x, for t mod C = 2; 0, otherwise where C ∈ N satisfies the following: DISPLAYFORM2 It is not hard to see that the condition hold for large constant C that depends on β 2 .Since the problem is one-dimensional, we drop indices representing coordinates from all quantities in Algorithm 1.

For this function sequence, it is easy to see that the point x = −1 provides the minimum regret.

Consider the execution of ADAM algorithm for this sequence of functions with β 1 = 0.

Note that since gradients of these functions are bounded, F has bounded D ∞ diameter and β 2 1 /β 2 < 1 as β 1 = 0, the conditions on the parameters required for ADAM are satisfied BID7 .

The gradients have the following form: DISPLAYFORM3 for i mod C = 2; 0, otherwise.

Let τ ∈ N, τ > 1 be such that DISPLAYFORM4 DISPLAYFORM5 for all t ≥ τ .

We start with the following preliminary result.

Lemma 3.

For the parameter settings and conditions assumed in Theorem 1, there is a t ≥ τ such that x Ct +1 ≥ 0.Proof by contradiction.

Assume that x Ct+1 < 0 for all t ≥ τ .

Firstly, for t ≥ τ , we observe the following inequalities: DISPLAYFORM6 DISPLAYFORM7 DISPLAYFORM8 From the (Cτ + 1)-th update of ADAM in Equation (2), we obtain: DISPLAYFORM9 The first inequality follows from x Ct+1 < 0 and Equation (6).

The last inequality follows from Equation (4).

Therefore, we have −1 ≤ x Cτ +1 <x Cτ +2 < 1 and hence x Cτ +2 =x Cτ +2 .

Then after the (Cτ + 2)-th update, we have: DISPLAYFORM10 where κ = αβ 2 (1 − β 2 )/162 √ C is a constant that depends on α, β 2 and C. The first inequality follows from Equation (2).
...
Therefore, we have −1 ≤ x Cτ +1 <x Cτ +3 < x Cτ +2 < 1.

Furthermore, since gradients ∇f i (x) = 0 when i mod C = 1 or 2, we have DISPLAYFORM12 Then, following Equation FORMULA37 we have DISPLAYFORM13 Similarly, we can subsequently obtain DISPLAYFORM14 and generally DISPLAYFORM15 for t ≥ τ .

Let t be such that 2κ( DISPLAYFORM16 This contradicts the assumption that x Ct+1 < 0 for all t ≥ τ .
...
Supposex Ct+2 > 1, then x Ct+2 = Π F (x Ct+2 ) = min{x Ct+2 , 1} = 1 (note that in one-dimension, Π F , √ Vt = Π F is the simple Euclidean projection).
...
Let δ be an arbitrary small positive constant.

Consider the following one dimensional stochastic optimization setting over the domain [−1, 1].

At each time step t, the function f t (x) is chosen as follows: DISPLAYFORM0 where C is a large constant that depends on β 1 , β 2 and δ.

The expected function is F (x) = δx.
...
Note that there is no limitation of the initial step size α by now.
...
We begin with the following observation: DISPLAYFORM0 Using Lemma 1 with u 1 = x t+1 and u 2 = x * , we have the following: DISPLAYFORM1 Rearranging the above inequality, we have DISPLAYFORM2 (10) DISPLAYFORM3 The second inequality use the fact that β 1t ≤ β 1 < 1.

In order to further simplify the bound in Equation FORMULA17 , we need to use telescopic sum.

We observe that, by definition of η t , we have DISPLAYFORM4 t−1,i .
...
Let {x t } and {v t } be the sequences obtained from Algorithm 3, β 1 = β 11 , β 1t ≤ β 1 for all t ∈ [T ] and β 1 / √ β 2 < 1.

Suppose η l (t + 1) ≥ η l (t) > 0, η u (t + 1) ≤ η u (t), η l (t) → α * as t → ∞, η u (t) → α * as t → ∞, L ∞ = η l (1) and R ∞ = η u (1).

Assume that x − y ∞ ≤ D ∞ for all x, y ∈ F and ∇f t (x) ≤ G 2 for all t ∈ [T ] and x ∈ F. For x t generated using the ADABOUND algorithm, we have the following bound on the regret We further directly compare the performance between SGDM and ADABOUND with each α (or α * ).
...
The increasing at the beginning is due to the property of the exponential moving average of φ t of ADAM, while the gradually decreasing indicates the transition from ADAM to SGD.
