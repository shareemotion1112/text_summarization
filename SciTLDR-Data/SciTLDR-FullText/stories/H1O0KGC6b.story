...
Let φ l : X l → X l+1 be the applications which respectively compute the output of the l-th layer of the network, for 1 ≤ l ≤ L, using the output of the l−1-th layer and Φ L = φ L • · · · • φ 1 be the mapping of the full network from X 1 to Y .

Also, for each layer l, we denote W W W l its weights matrix and ψ l its activation function.

The training of our network is done using a convex and continuous loss function : Y × Y → R + .

The objective of the neural network training is to find weights parametrizing Φ L that solves the following problem: DISPLAYFORM0 , drawn from this input distribution.
...
When Φ L−1 is fixed, the problem of finding W W W L is simple for several popular choices of activation function ψ L and loss .

For instance, when the activation function ψ L is the softmax function and the loss is the cross entropy, (2) is a multinomial logistic regression.

In this case, training the last layer is equivalent to a regression of the labels y using the embedding of the data x in X L by the mapping Φ L−1 .

Since the problem is convex in W W W L (see Appendix A), classical optimization techniques can efficiently produce an accurate approximation of the optimal weights W W W L -and this optimization given the mapping Φ L−1 is the idea behind post-training.
...
Any training strategy can be applied to the network, optimizing the empirical loss DISPLAYFORM2 The stochastic gradient descent explores the parameter space and provides a solution for Φ L−1 and W W W L .
...
Post-training: During this step, the first L − 1 layers are fixed and only the last layer of the network, φ L , is trained by minimizing over W W W L the following problem DISPLAYFORM3 where˜ (x, y) := (ψ L (x), y) .

This extra learning step uses the mapping Φ L−1 as an embedding of the data in X L and learn the best linear predictor in this space.
...
It is important to note that Dropout should not be applied on the previous layers of the network during the post-training, as it would lead to changes in the feature function Φ L−1 .

In this section, we show that for the case where DISPLAYFORM0 can be approximated using kernel methods.

We define the kernel k as follows, DISPLAYFORM1 Then k is the kernel associated with the feature function Φ L−1 .
...
As a consequence, there exists α * ∈ R N such that DISPLAYFORM4 Rewriting FORMULA8 with g * of the form (5), we have that g * = g W W W * , with DISPLAYFORM5 We emphasize that W W W * gives the optimal solution for the problem (6) and should not be confused with W W W * L , the optimum of (4).
...
By definition of the RKHS norm, we have DISPLAYFORM6 Consequently, we have that g W H ≤ W 2 , with equality when Vect(Φ L−1 (X 1 )) spans the entire space X L .
...
The regularization parameter λ for post-training is set to 1 × 10 −3 .The results show that while the training cost of the network mildly increases due to the use of post-training, this extra step improves the generalization of the solution.
...
We use dropout for the regularization, and set λ = 1 × 10 −2 .
...
The regularization parameter for posttraining, λ, is set to 1 × 10 −3 .
...
For the posttraining, the value of the curve at iteration q is the error for a network trained for q − 100 iterations with the regular training strategy and then trained for 100 iterations with post-training.the neural network, which learn the embedding Φ L−1 and an estimate W L .
...
The layer weights are all regularized with the 2 -norm and a fixed regularization parameter λ = 10 −3 .

Then, starting from each of the trained networks, 200 iterations of post-training are used with the same regularization parameter λ and the performances are compared to the closed-form solutions computed using (8) for each saved network.
...
The regularization parameter λ plays an important role in post-training.

Setting λ to be very large reduces the explanatory capacity of the networks whereas if λ is too small, the capacity can become too large and lead to overfitting.

Overall, our experiments highlighted that the post-training produces significant results for any choice of λ reasonably small (i.e 10 −5 ≤ λ ≤ 10 −2 ).
...
∀N, M ∈ N, ∀X ∈ R N , ∀j ∈ 1, M , the following function F is convex: DISPLAYFORM0 δ ij log exp(XW i ) .where δ is the Dirac function, and W i denotes the i-th row of a W .

Proof 1.

Let DISPLAYFORM1 .

DISPLAYFORM2 we have DISPLAYFORM3 ∂W m,n ∂W p,q = x n ∂P m ∂W p,q , = x n x q P m (W ) δ m,p − P p (W ) .

where ⊗ is the Kronecker product, and the matrix P(W ) is defined by P m,p = P m (W ) δ m,p − P p (W ) .
