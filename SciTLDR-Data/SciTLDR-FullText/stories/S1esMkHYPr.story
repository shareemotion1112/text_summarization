...
Now considering two key processes of normalizing flows as a generative model: (1) Calculating Data Likelihood: given a datapoint z, the exact density p Z (z) can be calculated by inverting the transformation f , = f −1 θ (z); (2) Sampling: z can be sampled from the distribution p Z (z) by first sample ∼ p E ( ) and then perform the feedforward transformation z = f θ ( ).

To efficiently perform the above mentioned operations, f θ is required to be invertible with an easily computable Jacobian determinant.
...
where g µ and g α are unconstrained and positive scalar functions of z 1:d−1 respectively to compute the mean and deviation.

In practice, these functions can be implemented as neural networks.

The affine transformation of AF can be written as:

The Jacobian matrix in AF is triangular, since ∂zi ∂ j is non-zero only for j ≤ i. Therefore, the determinant can be efficiently computed through D d=1 α d .
...
Graph Convolutional Networks (GCN) (Duvenaud et al., 2015; Gilmer et al., 2017; Kearnes et al., 2016; Kipf & Welling, 2016; Schütt et al., 2017) are a family of neural network architectures for learning representations of graphs.
...
where E i = A [:,:,i] denotes the i th slice of edge-conditioned adjacency tensor,Ẽ i = E i + I, and
...
, where g µ X , g µ A and g α X , g α A are parameterized neural networks for defining the mean and standard deviation of a Gaussian distribution.
...
The policy network itself defines a distribution p θ of molecular graphs G. If there are no edges between the newly generated atom and current sub-graph, the generation process terminates.
...
where r i (θ) = p θ (Xi|Gi) p θ old (Xi|Gi) and r ij (θ) = p θ (Aij |Gij ) p θ old (Aij |Gij ) are ratios of probabilities output by old and new policies, and V (state, action) is the estimated advantage function with a moving average baseline to reduce the variance of estimation.
...
JT-VAE has been shown to outperform other previous VAE-based models (Kusner et al., 2017; Gómez-Bombarelli et al., 2018; Simonovsky & Komodakis, 2018) .
...
The goal of the last task is to modify the given molecule to improve specified property with the constraint that the similarity between the original and modified molecule is above a threshold δ.
