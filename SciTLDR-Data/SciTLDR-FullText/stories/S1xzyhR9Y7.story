...
An RNN encoder f (s; φ) and a linear avg-on-word-vectors g(s; W ) produce a vector representation z f i and z g i for i-th sentence respectively.
...
Another model trained with fixed τ set to the final value performed similarly.4 EFFECT OF POST-PROCESSING STEP Table 2 : Effect of the Post-processing Step. 'WR' refers to the post-processing step BID6 which removes the principal component of a set of learnt vectors.
