...
The intuition is that if a summarization system truly understands the salient entities and events, it would ignore unrelated content.32nd Conference on Neural Information Processing Systems (NIPS 2018), Montréal, Canada.
...
Attention is calculated as in Eqs. 1-5.For both models, the loss becomes the weighted sum of the semantic loss and the summary loss: DISPLAYFORM5 In our experiments, we set α as 0.5 unless otherwise specified.
...
At every other timestep, we rank the hypotheses based on conditional likelihood and a weaker redundancy handler, r , that considers unigram novelty (i.e., percentage of unique content words): score = log (p(y a | x)) + α r .

We use R = 10, α = 0.4, β = 0.1, α = 0.1 in our experiments.
...
The models trained on the NYT dataset use an additional final training stage, where we optimized only on the summary loss (i.e., α = 1 in Eq. 7).
