...
The posterior probabilityp is modeled using a classifier C(. , θ C ) with a set of parameters θ C ; it can operate either on X

is learned using another model M(X i ; θ M ) with a set of parameters θ M .
...
where ω is a constant scalar that ensures that the sigmoid approximately equals to 1 when T ↑ i (r, z) is larger than σ , and approximately equals to 0 otherwise.
...
where exp −t σ encodes the overall trust with respect to the current step of the recursion.

Such trust is expected to decrease with the depth of the recursion (Belharbi et al., 2016) .

σ controls the slop of the trust function.
...
As for the parameters of (Durand et al., 2017) , we consider their α = 0 since most negative evidence is dropped, and use kmax = kmin = 0.09.

u = 0, u = 4, σ = 10, σ = 0.5, ω = 8.
...
To this end, we use the CRF implementation of (Krähenbühl & Koltun, 2011) 6 .

The results are presented in Tab.6.

Following the notation in (Krähenbühl & Koltun, 2011), we set w (1) = w (2) = 1.

We set, over all the methods, θ α = 13, θ β = 3, θ γ = 3 for 2 iterations, over GlaS, and θ α = 19, θ β = 11, θ γ = 5 for 5 iterations, over CUB5, CUB, and OxF. Tab.6 shows a slight improvement in term of F1 + and slight degradation in term of F1 − .
...
et al., 2018 ) is discarded since the produced plans do not form probability over the classes axe at pixel level which is required for the CRF input (Krähenbühl & Koltun, 2011) .
