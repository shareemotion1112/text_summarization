...
We feed the sentence encoding s into a ReLU layer followed by a sigmoid layer with output size 1, representing P (e 1 ∼ e 2 | s), as illustrated in DISPLAYFORM2 where σ is the sigmoid function and W 2 , b 2 , W 3 , b 3 are model parameters.
...
The model loss is a weighted sum of the sentence-level and the bag-level losses: DISPLAYFORM2 where λ is a parameter that controls the contribution of each loss, tuned on a validation set.
...
Although we did not spend much time tuning hyperparameters, we made sure to carefully tune λ (Equation 3) which balances the contribution of the two losses.

Early experiments showed that sentence-level loss is typically smaller than bag-level loss, so we experimented with λ ∈ {0, 0.5, 1, 2, 4, 8, 16, 32, 64} .

FIG4 shows thresholded AUC for different values of λ, where each point is the average of three runs.

It is clear that picking the right value for λ has a big impact on the final result.
