...
We use these properties to learn an approximate group homomorphism Φ ∈ M between motion in the world and in an embedding.
...
To see that a latent motion space respects these properties, first consider a latent structure space S. In this model, an element of the structure space generates images I via a projection operator π :
...
Our framework encompasses flow inference if brightness constancy is viewed as a constraint on the projection operator π.
...
The functions Φ and are implemented as a convolutional neural network (CNN) and a recurrent neural network (RNN), respectively.
...
The CNN Φ processes these images and outputs an intermediate representation [C 1,2 , ..., C t−1,t ].
...
We also explored learning a representation Φ taking single images (and not image pairs) as CNN input.
...
As discussed in BID25 , such a saliency map can be interpreted as a first-order Taylor expansion of the function Φ, evaluated at image I. The saliency map thus gives an indication of how pixel values affect the representation.
