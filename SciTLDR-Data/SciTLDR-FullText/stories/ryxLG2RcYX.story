...
The manager navigates the agent to the fringe of the known set (s 3 ), then randomly explores with π d to discover new transitions near s 3 (dotted box).
...
We further assume we have a simple predefined state abstraction function mapping concrete states x to abstract states s = φ(x).
...
We assume that reward only depends on the abstract states: taking any action transitioning from concrete state x to x leads to reward R(φ(x), φ(x )).Model-based approaches promise better exploration via planning, but struggle in high-dimensional state spaces due to compounding errors and computational intractability.
...
Over time, we monotonically grow the known set, which initially only contains the starting abstract state.• The action set comprises of calling the worker policy π w (a|x, (s, s )) on transitions (s, s ) from the current abstract state s to a nearby abstract state s .
...
The transition dynamics of calling action go(s, s ) at abstract state s are defined by the original MDP: i.e., if the worker takes a concrete trajectory x 0 , a 0 , x 1 , a 1 , · · · , x T , the resulting abstract state is φ(x T ).
...
When a transition (s, s ) becomes reliable (i.e., the dynamics T (s, s ) exceeds the threshold 1 − δ), the manager adds go(s, s ) to the action set of the abstract MDP and adds s to the known set.
...
Concretely, at an abstract state s, the manager finds nearby transitions by following a simple policy π d (a t |x 0:t , a 0:t−1 ) for T d timesteps (Algorithm 3).

The policy π d outputs randomized concrete actions a t conditioned on the past concrete states x 0:t and past concrete actions a 0:t−1 , where φ(x 0 ) is the abstract state s it was initially invoked.

During those T d timesteps, the manager records the transitions and rewards it observes: (φ(x 0 ), r 0 , φ(x 1 )), · · · (φ(x T −1 ), r T −1 , φ(x T )), using the rewards to update its rewards model and the transitions as candidates for the worker to learn.
...
Instead, we use a simple method for π d to commit to an exploration direction.

At each timestep, the π d uniformly samples a concrete action and a number between 1 and T repeat , and repeats the action the sampled number of times.
...
To avoid getting stuck, as candidates for the worker, the manager also considers "long-distance" transitions: (s, s ) pairs for which the manager did not directly transition from s to s , but indirectly did so through a sequence of intermediate states DISPLAYFORM0 Input: a transition (s, s ) to learn, called at concrete state x0 with φ(x0) = s 1: Set worker horizon DISPLAYFORM1 Observe xt 5:Compute worker intrinsic reward rt = R (s,s ) (xt|s ) 6:Update worker on (xt−1, at−1, rt, xt) 7:Choose DISPLAYFORM2 Freeze worker's skill π I(s,s ) DISPLAYFORM3 be the length of the shortest such path, the manager considers all pairs (s, DISPLAYFORM4 Priority scores of the exploration goals.
...
The manager heuristically computes the usefulness of a learning a transition (s, s ) as u(s, s ) = λ 2 I new + R(s 0 , s), where I new is an indicator that is 1 if there is an outgoing transition (s , s ) and no current candidate transitions end in s .
...
For an abstract state s, the manager computes the usefulness just as u(s) = λ 3 + R(s 0 , s).

The λ 3 constant accounts for how much more or less useful discovering new transitions is compared to learning new transitions.
...
If at the end of the skill episode, the success rate of the worker traversing (s, s ) exceeds the reliability threshold 1 − δ, the action go(s, s ) is added to the abstract MDP.

The worker's skill inventory I indexes skills so that the skill at index I(s, s ) reliably traverses transition (s, s ).

Each skill is a goal-conditioned subpolicy π I(s,s ) (a|x, s ), which produces concrete actions a conditioned on the current concrete state x and the goal abstract state s .

When the worker traverses a transition (s, s ), it calls on the corresponding skill until the transition is traversed: i.e., π w (a|x, (s, s )) = π I(s,s ) (a|x, s ).When learning a new transition (s, s ), the worker first tries to reuse its already learned skills from the skill inventory.

For each skill π i in the skill inventory, it measures the success rate of π i on the new transition (s, s ) over N transition attempts.

If the success rate exceeds the reliability threshold 1 − δ for any skill π i , it updates the skill repository to reuse the skill: I(s, s ) ← π i .
...
Each episode of this subtask consists of d(s, s ) × H worker timesteps (longer transitions need more timesteps to traverse), where the reward at each timestep is R (s,s ) (x t ) = 1 if the skill has successfully reached the end of the transition (φ(x t ) = s ) and 0 otherwise.
...
The skills recover a policy π K(s,s ) (a|x, (s, s )) by greedily selecting the action with the highest Q-value at each concrete state x.
...
We use λ = 0.5, which was chosen with no hyperparameter tuning.
...
Under the assumptions, for a given input η and , π t is at most suboptimal, To prove Proposition 1, we require the following three lemmas: DISPLAYFORM0 Lemma 1.

By setting N transition to be O( log 1−(1−η ) 1 |Φ| 2 /2 2( /|Φ|HV * (x0)) 2 ), with probability 1 − η , at each timestep t, π t is near-optimal on the current abstract MDP: i.e., V πt t (s) ≥ V * t (s)− for all abstract states s ∈ Φ.Lemma 2.

If the known set is equal to the set of all abstract states (S = Φ) at timestep T , then for any policy π on the abstract MDP, π achieves the same expected reward on the abstract MDP as on the concrete MDP: i.e., V π T (φ(x 0 )) = V π (x 0 ), where x 0 is the initial concrete state.

In addition, the expected return of the optimal policy on the abstract MDP is equal to the expected return of the optimal policy on the concrete MDP: i.e., V * T (φ(x 0 )) = V * (x 0 ) where x 0 is the initial state.

Lemma 3.

With probability 1 − η, the known set grows to cover all abstract states in O |Φ| 3 (|A| + Given these lemmas, we are ready to prove Proposition 1:Proof of Proposition 1.

For simplicity, we ignore terms due to appropriately setting N transition and 1 − η from Lemma 1, but these terms are all polynomial in the size of the abstract MDP.By Lemma 3 the known set grows to cover all abstract states in T = O |Φ| 3 (|A| + log K|Φ|+log 1 η log 1 p ) + d max × H worker timesteps.

For all timesteps t ≥ T , by Lemma 1, π t is at most suboptimal on the abstract MDP.

On all those timesteps, the known set is equal to all abstract states, so by Lemma 2, π t is at most suboptimal on the concrete MDP.Proofs.
