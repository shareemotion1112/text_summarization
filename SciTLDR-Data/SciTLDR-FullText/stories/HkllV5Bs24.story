...
In this paper, we work with Input: decision set ∆, initial distribution P , learning rate η Initialization: Q 1,0 = P At any time point t = 1, 2, . . . : BID4 run on the level of samples.
...
The predictors produced by MTLAB satisfy with probability 1 − δ (over the random training sets) uniformly over Q ∈ ∆ DISPLAYFORM3 Corollary.

Set η = m n .

Then, with probability 1 − δ, it holds uniformly over DISPLAYFORM4 To put this result into perspective, we compare it to the average regret bounds given in BID0 , where the goal is to find the best possible data representation for tasks.
...
LetQ t be a sequence of predictors produced by MTLAB run with η = m n and let Q n = 1 n n t=1Q t .

Then the following inequality holds with probability 1 − δ, uniformly over Q ∈ ∆ DISPLAYFORM1 DISPLAYFORM2 This bound resembles the guarantees typical in the setting of learning from drifting distributions BID8 .
...
Consequently, we redefine our goal to prove error rates that converge below a given threshold ε.
...
Further notation used are: I n are the indices of the tasks in the subsequence chosen at step n, s n = |I n | is the size of this subsequence,m n is the harmonic average of the sizes of tasks in the chosen subsequence and η n is the learning rate of MTLAB associated with the chosen subsequence.
...
If we run MTLAB.MS with dist(k i , k j ) = disc(k i , k j ), we get with probability 1 − δ, DISPLAYFORM4 This theorem works when the transfer algorithm uses a fixed learning rate η for each subsequence.
...
Input: task distance dist, prior distribution P , threshold ε Initialization: set of representative tasks R = ∅ set of priors P = ∅ At any time point t = 1, 2, . . .

:• receive dataset S t .• set I = {r ∈ R : dist(k r , k t ) ≤ ε} • if I = ∅ then -add t to the set of representatives R -set P(t) = P • choose the closest representatives r = argmin DISPLAYFORM5 • run the transfer algorithm: DISPLAYFORM6 • set P(r ) = P • outputQ t Figure 2 .
