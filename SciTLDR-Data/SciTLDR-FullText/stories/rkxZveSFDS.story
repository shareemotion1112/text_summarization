...
whereÃ = A + I is the adjacency matrix with inserted self-loops,D = jÃ ij is the renormalization factor (degree matrix).
...
The most general Quantum Graph Neural Network ansatz is a parameterized quantum circuit on a network which consists of a sequence of Q different Hamiltonian evolutions, with the whole sequence repeated P times:Û

where the product is time-ordered (Poulin et al., 2011) , the η and θ are variational (trainable) parameters, and the HamiltoniansĤ q (θ) can generally be any parameterized Hamiltonians whose topology of interactions is that of the problem graph:
...
To make compilation easier, we enforce that the terms of a given HamiltonianĤ q commute with one another, but differentĤ q 's need not commute.
...
We define quantum graph recurrent neural networks as ansatze of the form of equation 3 where the temporal parameters are tied between iterations, η pq → η q .
...
As η q acts as a time parameter for Hamiltonian evolution under H q , we can view the QGRNN ansatz as a Trotter-based (Lloyd, 1996; Poulin et al., 2011) quantum simulation of an evolution e −i∆Ĥ eff under the HamiltionianĤ eff = ∆ −1 q η qĤq for a time step of size ∆ = η 1 = q |η q |.
...
The θ parameters thus become tied over indices of the graph: W qrjk →
...
First, for a weighted graph G with edge weights Λ jk , we define the coupling Hamiltonian aŝ

The Λ jk here are the weights of the graph G, and are not trainable parameters.
...
After evolving byĤ C , which we consider to be the message passing step, one applies an exponential of the kinetic Hamiltonian,Ĥ K ≡ 1 2 j∈Vp 2 j .

Herep j denotes the continuous-variable momentum (Fourier conjugate) of the position, obeying the canonical commutation relation [x j ,p j ] = iδ jk .
...
where L jk = δ jk v∈V Λ jv − Λ jk is the Graph Laplacian matrix for the weighted graph G. We can recognize this step as analogous to classical spectral-based graph convolutions.
...
1 The next evolution is thus generated by an anharmonic HamiltonianĤ A = j∈V f (x j ), where f is a nonlinear function of degree greater than 2, e.g., a quartic potential of the form f (x j ) = ((x j − µ) 2 − ω 2 ) 2 for some µ, ω hyperparameters.
...
with variational parameters θ = {α, β, γ, δ}, we then recover a quantum-coherent analogue of the node update prescription of Kipf & Welling (2016) in the original graph convolutional networks paper.
...
We are given copies of a fixed low-energy state |ψ 0 as well as copies of the state |ψ T ≡ U (T ) |ψ 0 = e −iTĤtarget for some known but randomly chosen times T ∈ [0, T max ].

Our goal is to learn the target Hamiltonian parameters {J jk , Q v } j,k,v∈V by comparing the state |ψ T with the state obtained by evolving |ψ 0 according to the QGRNN ansatz for a number of iterations P ≈ T /∆ (where ∆ is a hyperparameter determining the Trotter step size).
...
We use a QGCNN ansatz withĤ 1 = {j,k}∈EẐ jẐk andĤ 2 = j∈VX j .

The loss function is the negative expectation of the sum of stabilizer group generators which stabilize the GHZ state (Tóth & Gühne, 2005) , i.e.,
...
In Fig. 3 we present the results for a QSGCNN for varying multi-qubit precision for the representation of the continuous values, where the loss function that was minimized was the expected value of the anharmonic potential L(η) = Ĥ C +Ĥ A η .

Of particular interest to near-term quantum computing with low numbers if qubits is the single-qubit precision case, where we modify the QSGCNN construction asp 2 j →X j , 3 For this test, one applies a phase rotation j∈V e −iϕẐ j on all the qubits in paralel, then one applies a sequence of CNOT's (quantum adder gates) such as to concentrate the phase shifts onto a single collector node, m ∈ V. Given that one had a GHZ state initially, one should then observe a phase shift e −inϕẐm where n = |V|.
...
I andx j → |1 1| j which transforms the coupling Hamiltonian aŝ
...
Our approach was the following, given two graphs G 1 and G 2 , one applies the single-qubit precision QSGCNN ansatz P j=1 e iηjĤ K e iγjĤ C withĤ K = j∈VX j andĤ C from equation 5 in parallel according to each graph's structure.

One then samples eigenvalues of the coupling HamiltonianĤ C on both graphs via standard basis measurement of the qubits and computation of the eigenvalue at each sample of the wavefunction.

One then obtains a set of samples of "energies" of this Hamiltonian.

By comparing the energetic measurement statistics output by the QSGCNN ansatz applied with identical parameters θ = {η, γ} for two different graphs, one can then infer whether the graphs are isomorphic.
