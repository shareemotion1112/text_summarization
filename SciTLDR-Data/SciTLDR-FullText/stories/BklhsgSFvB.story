...
Note that η here is a scalar parameter used to prevent the vanish gradient problem of two normalization operations.
...
where x i is the output of the lower level layer for task i, α ij is the transfer weight from task j to task i, andx i is the input of the higher level layer for task i.
...
where O ∈ R n×d is the output of the LSTM, H n ∈ R 1×d is the hidden state for the last word, α ∈ R n×1 is attention weight for each word, andÔ ∈ R 1×d is the text-level representation (n is the number of words, d is the feature dimension for each word).
