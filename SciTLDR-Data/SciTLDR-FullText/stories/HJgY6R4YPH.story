...
), where f i , f j are the embedded feature vectors of x i , x j , and σ is a scale parameter.

It is known (Liu et al., 2019 ) that graph-based methods are sensitive to the scale parameter σ.

A large σ results in an uniformly connected graph that disregards the latent structure of the data, while a small σ produces a sparse graph which fails to express all the relationship between the data.
...
In label propagation, the labels of the target domain is assigned to the propagated labelsŷ t by infinite transition, formulated asŷ

tt T ts y s , which converges as follows (Xiaojin & Zoubin, 2002)

In our method,ŷ t is used to obtain the propagated labels of the source data in the same way aŝ y s = (I − T ss ) −1 T stŷ t where T ss and T st are defined analogous to T tt and T ts , so that we can learn the features of which clusters match each other.

We then refer to the property thatŷ s should be the same as the original label y s as cycle consistency.
...
Assume that 1) the source data is ideally clustered, in the sense that T ss has positive values if the row and the column are the same class and zero otherwise, i.e., T ss = diag(T 1 , T 2 , · · · , T C ), the block diagonal where T i is a n i × n i positive matrix for i = 1, 2, · · · , C and 2)ŷ s = y s .
...
th element to [n 1 + n 2 + · · · + n j ] th element) are positive and the other elements are all zero and 2) v j T stŷ t e i = 0 for all 1 ≤ i ≤ C, i = j.
...
1,ŷ t e i refers to the assigned probability as i th class to the target data.
...
1, each row ofŷ t has only one nonzero value, i.e., every target data belongs to only one cluster.
...
Cycle consistency is enforced by minimizing the l 1 loss L cycle betweenŷ s and y s :
...
µ and η are coefficients for the source and the target local manifold consistency loss, respectively.
...
α and β are coefficients for the last two terms and λ is a scheduling parameter described in Appx B.1.
...
1 when the source features are slightly perturbed from the ideally clustered condition and the other assumption y s =ŷ s holds.

We start from representing T ss as follows to indicate the perturbation.

where, δT ss is assumed to be sufficiently small under infinite norm and T

ss is a block diagonal transition matrix when the source features are ideally clustered as stated in Thm.

1.

In the proof above, we showed eigenvalue λ j and its corresponding eigenvector v j of T j .
...
The second term of Eq. 26 holds because T st andŷ t are bounded by 0 and 1.
...
In addition, the more strongly the target data is classified as i th class i.e., the corresponding element ofŷ t becomes greater, the smaller the transitions from source data in the other classes are, indicating the segregation against the other classes.
...
Scheduling the effect of losses To reduce the effect of noisy signal from L dann and L cycle during the early stages of training, a weight balance factor λ = 2 1+exp(−γ·p) − 1 is applied in Eq. 6.

A constant γ determines the rate of increase of λ; p is the progress of training, which proceeds from 0 to 1.
...
Throughout the experiments, γ was set to 10.
...
We searched hyperparameters within α = {0, 0.01, 0.1, 1}, β = {0.01, 0.1, 1}, µ = {0, 0.01} and η = {0, 0.1}. Perturbation to the LGAN generator, i.e. z, is fixed to 0.5 for all experiments.

The best hyperparameters for each task is shown in Table.

3.

Setting an appropriate value for the scale parameter, σ, is important because it has a substantial role in determining the transition matrix, T. Therefore, we conducted several experiments with fixing σ to various values.

For these experiments, we excluded L local to observe the effect of σ. '

Adapt' means that the σ is learned to adapt according to the embedded features.

For four out of five scenarios, fixing σ to 1 performed better than fixing it to 0.1 or 10.

With this observation, we initialized σ to 1 took it as a trainable variable.

The result of adaptively learning σ is reported at the bottom row of the table.

Compared to fixing σ to 1, adaptively learning σ achieved better accuracy and had a lower standard deviation range which means that it is more stable.

We also would like to highlight that our model is robust to the initial value of σ.

We conducted extensive experiments with initializing σ to 0.1, 1 and 10 and taking it as a trainable variable.

Except for SVHN → MNIST transfer task with setting initial σ value to 10, the initial value of σ has a minute influence to the accuracy.

We believe that adaptively learning the scale parameter can be usefully employed in any other graph-based method.

The learned σ values for various scenarios are as follows.
