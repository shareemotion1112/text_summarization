...
There are two sets of hyper-parameters during search, namely, α o i,j and {β i,j }, where 0 i < j and o ∈ O. To determine α o i,j , we only sample a subset, 1/K, of channels and connect them to the next stage, so that the memory consumption is reduced by K times.

To minimize the uncertainty incurred by sampling, we add {β i,j } as extra edge-level parameters.
...
, where x i is the output of the i-th node, and α o i,j is a hyper-parameter for weighting operation o(x i ).
...
This design makes the entire framework differentiable to both layer weights and hyper-parameters α o i,j , so that it is possible to perform architecture search in an end-to-end fashion.

After the search process is finished, on each edge (i, j), the operation o with the largest α o i,j value is preserved, and each node j is connected to two precedents i < j with the largest α o i,j preserved.
...
In other words, for edge (i, j), given an input x i , the difference from using two sets of hyper-parameters α o i,j and α o i,j is largely reduced, because only a small part (1/K) of input channels would go through the operation mixture while the remaining channels are left intact.
...
Consequently, weight-free operations often accumulate larger weights (namely α o i,j ) at the beginning, and this makes it difficult for the weightequipped operations to beat them even after they have been well trained thereafter.
...
To mitigate this problem, we introduce edge normalization that weighs on each edge (i, j) explicitly, denoted by β i,j , so that the computation of x j becomes:

Specifically, after the architecture search is done, the connectivity of edge (i, j) is determined by both α o i,j and β i,j , for which we multiply the normalized coefficients together, i.e., multiplying

.

Then the edges are selected by finding the large edge weights as in DARTS.

Since β i,j are shared through the training process, the learned network architecture is insensitive to the sampled channels across iterations, making the architecture search more stable.
...
Among prior efforts, ProxylessNAS (Cai et al., 2019) binarized the multinomial distribution α o i,j and samples two paths at each time, which significantly reduced memory cost and enabled direct search on ImageNet.
...
In the search stage, the goal is to determine the best sets of hyper-parameters, namely α o i,j and {β i,j } for each edge (i, j).
...
Besides, following , we freeze the hyper-parameters, α o i,j and {β i,j }, and only allow the network parameters to be tuned in the first 15 epochs.
...
We use an Adam optimizer (Kingma & Ba, 2015) for α o i,j and {β i,j }, with a fixed learning rate of 6 × 10 −4 , a momentum of (0.5, 0.999) and a weight decay of 10 −3 .
