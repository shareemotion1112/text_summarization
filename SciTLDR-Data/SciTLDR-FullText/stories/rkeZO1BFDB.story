...
Formally, given an environment E, an initial pose p 1 = (v 1 , φ 1 , θ 1 ) (spatial position, heading, elevation angles) and natural language instructions x 1:N , the agent takes a sequence of actions a 1:T to finally reach the goal G. Thus the VLN dataset D is defined as {(E, p 1 , x 1:N , G)} |D | .
...
Then it takes an action a t and leads to a new pose p t+1 = (v t+1 , φ t+1 , θ t+1 ).
...
The meta-learner is a SoftMax layer which takes the concatenation of two hidden states h S t and h T t as input, and produces a probability α t representing the belief of the source language representation.
...
where ψ denotes a non-linearity activation such as ReLU or tanh, W I , W S and W T are the projection matrices.
...
where y H is the domain label of an instruction indicating whether it is from human annotation or machine translation, andŷ H is the approximation by the domain discriminator.
...
where γ is set to 10 and p is learning steps.

We use 0.2λ p to train the domain discriminator.
