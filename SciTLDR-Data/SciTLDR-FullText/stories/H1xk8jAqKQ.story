...
If P(A) denotes the space of probability distributions over actions, the agent chooses actions by sampling from a stochastic policy π : S → P(A), and receives reward r : S × A → R at every time step.

The agent's goal is to construct a policy which minimizes its discounted expected return R t = E ∞ k=0 γ k r t+k+1 where r t is the reward at time t and γ ∈ [0, 1] is the discount factor, and the expectation is taken with respect to both the policy and the environment.
...
Assume that π is a fixed policy on M, such that the underlying Markov chain is irreducible and aperiodic: DISPLAYFORM1 K π has a single absorbing state at s = s * := v * .

Our goal is to estimate the value function of this policy, equivalent to the expected first-passage time of the Markov chain K π from s 0 = s to s * : DISPLAYFORM2 We consider a special tractable case where the value function can be well approximated by looking at the distance of a state from the goal state.

Formally: DISPLAYFORM3 We wish to analyze the process which is the projection of our Markov policy π only in terms of the distance z t .
...
Repeat the argument m times so that after O( DISPLAYFORM9 In the case where α l = α for all l, we obtain γ −1 km,(k+1)m−j = γ 0,m−j = α −m+j and therefore DISPLAYFORM10 where the important term is the rate DISPLAYFORM11 On the other hand, BID18 shows that the first-passage time τ (0, M ) in a skip-free finite Markov chain of M states with a single absorbing state is a random variable whose momentgenerating function ϕ(s) = Es τ (s,s * ) is given by DISPLAYFORM12 where λ 1 , . . .

, λ M are the non-unit eigenvalues of the transition probability matrix.
...
The probability that a trajectory initialized at one of the uniform samples will reach the absorbing state is lower bounded by DISPLAYFORM14 which is approximately α M when α is small, leading to a sample complexity of O(M 2 α −1 ) to update the value function at the originating state, and O(M 3 α −1 ) at the starting state.
...
Is there an ideal rate for advancing the curriculum window and is there a better approach than a hand-tuned schedule?Stéphane Ross, Geoffrey Gordon, and Drew Bagnell.
