...
The labelŷ (t) is predicted based on the sign of the output value from the model.
...
We maintain and update a relationship matrix τ ∈ R K×K through the learning process.

The k-th row of τ , denoted τ k , is the committee weight vector for task k, also referred to as committee for brevity.

Element τ ij of the relationship matrix indicates the closeness or similarity between task i and task

if P (t) = 1 then 10: Query true label y (t) and set DISPLAYFORM0 12: Update τ : DISPLAYFORM1 13: 20: end function j, and also the importance of task j in task i's committee in predicting.
...
The hyperparameter C decides how much decrease happens on the weight given non-zero loss, and λ = K m=1 l (t) km .
...
2) Our method updates the committee weight vector while keeping m∈ [K] τ km = 1 instead of m∈ [K] ,m =k τ km = 1.
