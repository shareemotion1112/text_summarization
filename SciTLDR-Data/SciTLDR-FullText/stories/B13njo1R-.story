...
Leveraging the framework of reinforcement learning, we frame the problem as a Markov Decision Processes (MDP): at each time step t, the world (including the agent) is in a state s t ∈ S, wherein the agent is able to perform actions a t ∈ A, sampled from a policy π(s t , a t ) = p(a t |s t ) and resulting in state s t+1 ∈ S according to transition probabilities T (s t , a t , s t+1 ).

Performing action a t from state s t produces a reward r t ; the expected cumulative reward earned from following some policy π may then be written as: DISPLAYFORM0 where T is the time horizon, and γ is the discount factor, defining the planning horizon length.

The agent's goal is to learn an optimal policy, π * , maximizing J(π).

If the policy has parameters θ π , then the goal may be reformulated to identify the optimal parameters θ * π : DISPLAYFORM1 Our policy models a Gaussian distribution with a mean state dependent mean, µ θt (s t ).

Thus, our stochastic policy may be formulated as follows: DISPLAYFORM2 where Σ is a diagonal covariance matrix with entries σ 2 i on the diagonal, similar to BID15 .To optimize our policy, we use stochastic policy gradient methods, which are well-established family of techniques for reinforcement learning BID26 .

The gradient of the expected reward with respect to the policy parameters, ∇ θπ J(π(·|θ π )), is given by: DISPLAYFORM3 where DISPLAYFORM4 represents the initial state distribution, and p 0 (s 0 )(s 0 → s | t, π 0 ) models the likelihood of reaching state s by starting at state s 0 and following the policy π(a, s|θ π ) for T steps BID24 .

A π (s, a) represents an advantage function BID22 .

In this work, we use the Positive Temporal Difference (PTD) update proposed by BID30 for A π (s, a): DISPLAYFORM5 DISPLAYFORM6 where V π (s) = E T t=0 γ t r t | s 0 = s is the value function, which gives the expected discounted cumulative reward from following policy π starting in state s. PTD has the benefit of being insensitive to the advantage function scale.
...
Because the true value function is unknown, an approximation V π (· | θ v ) with parameters θ v is learned, which is formulated as the regression problem: DISPLAYFORM7 3.2 POLICY DISTILLATION Given a set of expert agents that have solved/mastered different tasks we may want to combine the skills of these different experts into a single multi-skilled agent.
...
Considering the state distribution expert is skilled at solving, (D ωi the source distribution) it can be advantageous to start learning a new, target task ω i+1 with target distribution D ωi+1 using assistance from the expert.

The agent learning how to solve the target task with domain D ωi+1 is referred to as the student.

When the expert is used to assist the student in learning the target task it can be referred to as the teacher.

The success of these methods are dependent on overlap between the D ωi and D ωi+1 state distributions.
...
In the acquisition (TL) step, we are interested in learning a new task ω i+1 .

Here transfer can be beneficial if the task structure is somewhat similar to previous tasks ω i .
...
In the integration (distillation) step, we are interested in combining all past skills (π 0 , . . .

, π i ) with the newly acquired skill π i+1 .
...
The expert used to generate actions for tasks 0 − i is π i and the expert used to generate action for task i + 1 is π i+1 .
