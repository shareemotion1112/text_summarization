...
Stage I Stage II S ·, · Φ θ3 ( − ) Figure 1 : High-level illustration of our two-stage neighborhood consensus architecture.

Node features are first locally matched based on a graph neural network Ψ θ1 , before their correspondence scores get iteratively refined based on neighborhood consensus.

Here, an injective node coloring of G s is transferred to G t via S, and distributed by Ψ θ2 on both graphs.

Updates on S are performed by a neural network Φ θ3 based on pair-wise color differences.
...
|Vt|×· computed by a shared neural network Ψ θ1 for source graph G s and target graph G t , respectively, we obtain initial soft correspondences as
...
We train Ψ θ1 in a dicriminative, supervised fashion against ground truth correspondences π gt (·) by minimizing the negative log-likelihood of correct correspondence scores L (initial) = − i∈Vs log(S

i,πgt(i) ).

We implement Ψ θ1 as a Graph Neural Network (GNN) to obtain localized, permutation equivariant vectorial node representations Hamilton et al., 2017; Battaglia et al., 2018; Goyal & Ferrara, 2018) .
...
Then, we distribute this coloring in corresponding neighborhoods by performing synchronous message passing on both graphs via a shared graph neural network Ψ θ2 , i.e.,
...
based on an MLP Φ θ3 .

The process can be applied L times to iteratively improve the consensus in

i,πgt(i) ) combines both the feature matching error and neighborhood consensus error.
...
Since an initial matching is needed to test for neighborhood consensus, this task cannot be fulfilled by Ψ θ1 alone, which stresses the importance of our two-stage approach.
...
Let G s and G t be two isomorphic graphs and let Ψ θ2 be a permutation equivariant GNN, i.e., P Ψ θ2 (X, A) = Ψ θ2 (P X, P AP ) for any permutation matrix P ∈ {0, 1} |V|×|V| .

If S ∈ {0, 1} |Vs|×|Vt| encodes an isomorphism between G s and

Theorem 2.

Let G s and G t be two graphs and let Ψ θ2 be a permutation equivariant and T -layered GNN for which both AGGREGATE (t) and UPDATE (t) are injective for all t ∈ {1, . . .

, T }.

Hence, a GNN Ψ θ2 that satisfies both criteria in Theorem 1 and 2 provides equal node embeddings o Note that both requirements, permutation equivariance and injectivity, are easily fulfilled: (1) All common graph neural network architectures following the message passing scheme of Equation (2) are equivariant due to the use of permutation invariant neighborhood aggregators.
...
Moreover, the gradient Q is closely related to our neighborhood consensus scheme for the particular simple, non-trainable GNN instantiation Ψ(X,

based on the similarity between O s and O t obtained from a fixed-function GNN Ψ, we choose to update correspondence scores via trainable neural networks Ψ θ2 and Φ θ3 based on the difference between O s and O t .
...
We experimentally verify the benefits of using trainable neural networks Ψ θ2 instead of Ψ(X, A, E) = AX in Appendix D.
...
Although applying Ψ θ2 on node indicator functions I |Vs| is computationally efficient, it requires a parameter complexity of O(|V s |).
...
Furthermore, it is inherently inefficient to compute and runs the risk of vanishing gradients ∂S (l) /∂Ŝ (l) (Zhang et al., 2019b) .

Here, we propose to relax this constraint by only applying row-wise softmax normalization onŜ (l) , and expect our supervised refinement procedure to naturally resolve violations of i∈Vs S i,j ≤ 1 on its own by re-ranking false correspondences via neighborhood consensus.
...
We use similar architectures for Ψ θ1 and Ψ θ2 except that we omit dropout (Srivastava et al., 2014) in Ψ θ2 .
...
Each pair of graphs consists of an undirected Erdős & Rényi (1959) graph G s with |V s | ∈ {50, 100} nodes and edge probability p ∈ {0.1, 0.2}, and a target graph G t which is constructed from G s by removing edges with probability p s without disconnecting any nodes (Heimann et al., 2018) .
...
We implement the graph neural network operators Ψ θ1 and Ψ θ2 by stacking three layers (T = 3) of the GIN operator (Xu et al., 2019c)
...
show the matching accuracy Hits@1 for different choices of |V s | and p. We observe that the purely local matching approach via softmax(Ŝ (0) ) starts decreasing in performance with the structural noise p s increasing.

This also holds when applying global sinkhorn normalization onŜ (0) .
...
This applies to both variants discussed in the previous sections, i.e., our initial formulation sinkhorn(Ŝ (L) ), and our optimized architecture using random node indicator sampling and row-wise normalization softmax(Ŝ (L) ).
...
whose trainable B-spline based kernel function Φ θ (·) is conditioned on edge features e j,i between node-pairs.
...
For SPLINECNN, we use a kernel size of 5 in each dimension, a hidden dimensionality of 256, and apply ReLU as our non-linearity function σ.
...
We evaluate our complete architecture using isotropic and anisotropic GNNs for L ∈ {0, 10, 20}, and include ablation results obtained from using Ψ θ1 = MLP for the local node matching procedure.
...
The benefits of the second stage are even more crucial when starting from a weaker initial feature matching baseline (Ψ θ1 = MLP), with overall improvements of up to 14 percentage points on PASCALVOC.

However, good initial matchings do help our consensus stage to improve its performance further, as indicated by the usage of task-specific isotropic or anisotropic GNNs for Ψ θ1 .
...
We use ReLU followed by dropout with probability 0.5 as our non-linearity σ, and obtain final node representations via
...
In theory, one might resolve these ambiguities by adding a small amount of noise toŜ (0) .
...
Since Ψ θ2 is permutation equivariant, it holds for any node feature matrix X s ∈ R |Vs|×· that Ψ θ2 (S X s , S A s S) = S Ψ θ2 (X s , A s ).

With X t = S X s and A t = S A s S, it follows that

Then, the T -layered GNN Ψ θ2 maps both T -hop neighborhoods around nodes i ∈ V s and j ∈ V t to the same vectorial representation:

Because Ψ θ2 is as powerful as the WL heuristic in distinguishing graph structures (Xu et al., 2019c; Morris et al., 2019) and is operating on injective node colorings I |V|s , it has the power to distinguish any graph structure from
...
To evaluate the impact of a trainable refinement procedure, we replicated the experiments of Sections 4.2 and 4.4 by implementing Ψ θ2 via a non-trainable, one-layer GNN instantiation Ψ θ2 (X, A, E) = AX.

Tables 5 and 6 show that using trainable neural networks Ψ θ2 consistently improves upon the results of using the fixed-function message passing scheme.
...
We form graph-pairs by treating an Erdős & Rényi graph with |V s | ∈ {50, 100} nodes and edge probability p ∈ {0.1, 0.2} as our source graph G s .
...
Recently, it has been proposed to learn such functions for a specific task, e.g., in form of a cost model for the graph edit distance (Cortés et al., 2019) .
...
Xu et al. (2019b) tackles the problem of graph matching by relating it to the Gromov-Wasserstein discrepancy (Peyré et al., 2016) .
