...
where Z(w) is the partition function summing over all ground predicates and φ f (·) is the potential function defined by a formula f as illustrated in Fig. 2 .

One form of φ f (·) can simply be the truth value of the logic formula f .

For instance, if the formula is f (c, c ) := ¬S(c) ∨ ¬F(c, c ) ∨ S(c ), then φ f (c, c ) can simply take value 1 when f (c, c ) is true and 0 otherwise.

Other more sophisticated φ f can also be designed, which have the potential to take into account complex entities, such as images or texts, but will not be the focus of this paper.
...
where Q θ (H | O) is a variational posterior distribution of the latent variables given the observed ones.

The equality in Eq. 2 holds if the variational posterior Q θ (H|O) equals to the true posterior P w (H|O).

We then use the variational EM algorithm (Ghahramani et al., 2000) to effectively optimize the ELBO.

The variational EM algorithm consists of an expectation step (E-step) and a maximization step (M-step), which will be called in an alternating fashion to train the model: 1) In the E-step (Sec. 4.1), we infer the posterior distribution of the latent variables, where P w is fixed and Q θ is optimized to minimize the KL divergence between Q θ (H|O) and P w (H|O); 2) In the M-step (Sec. 4.2), we learn the weights of the logic formulae in MLN, where Q θ is fixed and P w is optimized to maximize the data log-likelihood.

In the E-step, which is also known as the inference step, we are minimizing the KL divergence between the variational posterior distribution Q θ (H|O) and the true posterior distribution P w (H|O).
...
where each factorized distribution Q θ (r(a r )) follows the Bernoulli distribution.

We parameterize the variational posterior Q θ with deep learning models as our neural inference network.
...
With the mean-field approximation, L ELBO (Q θ , P w ) defined in Eq. 2 can be reorganized as below:

where w f is fixed in the E-step and thus the partition function Z(w) can be treated as a constant.

We notice that the first term E Q θ (H|O) [log P w (O, H)] has the summation over all formulae and all possible assignments to each formula.

Thus this double summation may involve a large number of terms.

The second term E Q θ (H|O) [log Q θ (H|O)] is the sum of entropy of the variational posterior distributions Q θ (r(a r )), which also involves a large number of terms since the summation ranges over all possible latent variables.
...
where λ is a hyperparameter to control the weight.

This overall objective essentially combines the knowledge in logic rules and the supervision from labeled data.

In the M-step, which is also known as the learning step, we are learning the weights of logic formulae in Markov Logic Networks with the variational posterior Q θ (H|O) fixed.
...
where y r(ar) = 0 or 1 if r(a r ) is an observed fact, and y r(ar) = Q θ (r(a r )) otherwise.
...
For each variable in the Markov blanket, we take the truth value if it's observed and draw a sample from the variational posterior Q θ if it's latent.
...
The GNN parameters θ 1 and θ 2 are shared across the entire graph and independent of the number of entities.
...
• Entity A and B have opposite relations with E, i.e., F(A, E) = 1 versus F(B, E) = 0 in the knowledge graph, but running GNN on the knowledge graph will always produce the same embeddings for A and B, i.e., µ A = µ B .
...
Two ordered sequences of nodes (c 1 , . . . , c n ) and (c 1 , . . . , c n ) are isomorphic in a graph G K if there exists an isomorphism π : Implied by the theorem, to obtain an expressive enough representation for the posterior, we need a more powerful GNN variant.
...
For the choice of λ in the combined objective L θ in Eq. 6, we set λ = 0 for the inference experiments, since the query predicates are never seen in the training data and no supervision is available.

For the learning experiments, we set λ = 1.
