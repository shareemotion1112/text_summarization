...
To answer this we can use the VAE encoder to generate a distribution q φ (z|x) representing that image in the latent space (see Sections 2.1 for details on VAEs).
...
We have repeated this for a β-VAE (see section 2.3 for a full description of β-VAEs).

We note that there is very little variation between the different samples drawn from q φ (z|x), particularly for the standard VAE (β = 1), showing that the latent space of the VAE is relatively smooth (there is more variation when β = 20).
...
In some cases it appears that the type of object is being captured, although in the case of the boat with the β-VAE (with β = 20) the wrong object is being rendered.
...
For each input x the encoder outputs a mean vector, µ, and standard deviation vector, σ, that describes an axis aligned normal distribution,
...
For more complex datasets the decoder ususal generates a "reconstruction"x = D θ (z).

The probability of generating a pixel value x i is then usually taken as

2 )) and variance σ 2 that measures the expected size of the errors between the input images, x, and the reconstructions,x.

The loss function for a VAE is equal to the negative evidence lower bound (ELBO)

As explained in Appendix A, log(p θ (x|z)) is chosen to be proportional to the logarithm of the reconstruction error betweenx and the input image x-in our experiments this produced better reconstructions than replacing log(p θ (x|z)) with the mean squared error.
...
One of the most prominent has been the β-VAE introduced by Higgins et al. (2017) , where the KL-divergence term in a normal VAE is weighted by a parameter β

The argument is that by making β 1 we encourage disentanglement.

Contrariwise, by making β 1 we make a VAE closer to an auto-encoder.
...
In Figure 4 we show examples of input-output pairs for different values of β.

We observe that for large β the outputs are quite different from the input images in contrast to small β where many more details of the original input are captured.
...
We show the mean squared error measured on a testing set from CIFAR-10 as a function of β in Figure 5 (a).
...
Interestingly, the mean squared error remains fairly constant as we increase β from a low value, until we reach β = 1 after which it rapidly increases.

One interpretation of this fact is that the VAE with β = 1 is successfully encoding all the useful information (i.e. compressible information) so for reconstructing unseen images it will perform as well as an auto-encoder.

As we increase β above 1, the reconstruction error increases rapidly.
...
We find little variation as we decrease β below 1.

As we increase β above 1 the classification accuracy falls off.

Again we can attribute this to the latent space of the VAE (with β = 1) capturing most useful information.

Interestingly the high-β VAE fails to capture (a) (b) Figure 5 : Performance of the VAE (blue points) and LSR-GAN (red points) versus β.
...
By encoding a message as a random variable z drawn from a distribution q φ (z|x) the VAE is able to find an optimal balance between accuracy to which it transmits the model (determined by the standard deviation vector, σ, generated by the VAE encoder) and the need to reduce the reconstruction error.
...
More often it is assumed that the pixel errors are normally distributed with some variance σ 2 .
...
2 so that we should minimise N log σ 2 /2.
...
Note that since σ 2 is proportional to the mean squared error, E MSE , it suffices to minimise N log(E MSE ) /2.

We note that

which is precisely the gradient of

if we ignored the dependence of σ 2 onx i .

In many publically available implementations of VAEs the algorithm minimises

2 which arbitrarily assumes σ 2 = 1 2 rather than its true value.

This means that these implementations are effectively running a β-VAE with some unknown β (in our experience with β > 1).
...
It introduces a second network F θ to the GAN.

The task of F θ is to map both the real images and synthetic images to a Gaussian distribution which is what we ask the encoder to do.

When the input of F θ is the output of generator, the objective function minimise the distance between the input of generator and the output of F θ .

If the input of F θ is real data, the objective function minimise the cross entropy between Gaussian prior and the output of F θ .
...
In Table 1 we present measurements of the performance of outputs from both VAEs and LSR-GAN for different values of β.

Some of this data is also presented graphically in Figure 5 , but we have included additional measurements.

The performance of the LSR-GAN depends on the hyper-parameter λ.
...
These two objectives are not necessarily contradictory, although we will see that changing λ has benefits and drawbacks.
...
We see that increasing λ improves the classification performance (both relative and absolute).

However, and perhaps surprisingly, increasing λ produces a significant reduction in the reconstruction error.

More intuitively it also causes a reduction in the variance between images sampled independently from q φ (z|x).
...
(a) (b) (c) (d) Figure 6 : Graphs showing the classification performance of images generated by our GAN with different λ values (on test dataset).
...
The x-axis is the log value of different λ, the errors are too small which make error bars look like lines.
...
Both networks are optimized using Adam (Kingma and Ba, 2015) with a learning rate of 2 × 10 −4 and β 1 = 0.5.
...
Figure 8: Random samples generated by LSR-GAN trained on CIFAR10 and ImageNet with β =1 and λ=1.
...
Figure 9: Random samples generated by LSR-GAN trained on MNIST and Celeb-A with β =1 and λ=1.
