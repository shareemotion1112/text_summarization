...
A VAE has an encoder q φ (z|x) and a decoder p θ (x|z) each parameterized by a neural network.

In many cases, a standard Gaussian distribution is used for the prior distribution of the latent vector p(z) and a Gaussian distribution is used for q φ (z|x).

Instead of directly maximizing the intractable marginal probability p(x) = p(z)p θ (x|z)dz, we maximize the evidence lower bound: DISPLAYFORM1 (1) = L ELBO As the model samples from q φ (z|x), the reparameterization trick BID11 can be used to train the model with gradient descent.
...
When the KL term becomes zero, no information from the input text is reflected on latent variables since q φ (z|x) and p(z) are identical.
...
In typical VAEs, a standard normal distribution N (0, 1) is used as the prior distribution p(z) and a normal distribution N (µ, σ 2 ) is used as the posterior distribution q φ (z|x).

Although this model is also used for previous textual VAE models BID1 BID30 , there is a trivial local minimum p(z) = q φ (z|x) which makes KL(q φ (z|x)|p(z)) in Equation 1 zero, manifesting in what is referred to as posterior collapse.

Roughly speaking, we can avoid this if q φ (z|x) cannot be identical to p(z).

One simple way to achieve this is to use a multimodal distribution as the prior distribution p(z) and an unimodal distribution as the posterior distribution q φ (z|x).
...
This objective can be minimized to zero without utilizing latent variables under the assumption that (i) the decoder is sufficiently flexible and (ii) the posterior distribution can be trained so p(z) = q φ (z|x) BID31 .
...
When the prior distribution p(z) is a multimodal distribution and the posterior distribution q φ (z|x) is an unimodal distribution, there is no way to satisfy p(z) = q φ (z|x).

Moreover, there will be multiple minima for KL(q φ (z|x)|p(z)).
...
In practice, it is natural to assume that training the decoder so p D (x) = p θ (x|z) for all z is much harder than to make KL(q φ (z|x)|p(z)) = 0.

Under the assumption, the model will be trained so KL(q φ (z|x)|p(z)) = 0 as the first step and this condition force the decoder to be trained so p D (x) = p θ (x|z) for all z when there is no modification of the model.
...
Therefore, if we modify the model to avoid KL(q φ (z|x)|p(z)) = 0, the decoder will not try to satisfy p D (x) = p θ (x|z) for all z but learn the conditioned distribution for each z. This analysis motivates us to modify textual VAE without weakening the capacity of the decoder.
...
According to our experiments, setting the learning rate to 5 × 10 −4 and β 1 to 0.5 performs the best.
