...
In this paper, we use a von Mises-Fisher (vMF hereafter) distribution, which is often used in directional statistics BID20 , and its concentration parameter κ to characterize the directional balance of minibatch gradients and understand the learning dynamics of SGD from the perspective of directional statistics of minibatch gradients.
...
An angle θ between d-dimensional vectors u and v is defined by θ = Loss functions A loss function of a neural network is written as f (w) = 1 n n i=1 f i (w), where w ∈ R d is a trainable parameter.
...
For example, the negative gradient of f Ii (w) is written aŝ g i (w).
...
100 random samples are drawn from vMF(µ, κ) where µ = (1, 0) and κ = {0, 5, 50}.on the hypersphere DISPLAYFORM0 Here, the concentration parameter κ determines how the samples from this distribution are concentrated on the mean direction µ and C d (κ) is constant determined by d and κ.

If κ is zero, then it is a uniform distribution on the unit hypersphere, and as κ → ∞, it becomes a point mass on the unit hypersphere ( Figure 1 ).

The maximum likelihood estimates for µ and κ arê DISPLAYFORM1 1−r 2 where x i 's are random samples from the vMF distribution andr = n i=1 xi n .

The formula forκ is approximate since the exact computation is intractable BID0 .

It is a usual practice for SGD to use a minibatch gradientĝ(w) = −∇ w f I (w) instead of a full batch gradient g(w) = −∇ w f (w).

The minibatch index set I is drawn from {1,. . .

,n} randomly.ĝ(w) satisfies E[ĝ(w)] = g(w) and Cov(ĝ(w),ĝ(w)) ≈ 1 mn n i=1 g i (w)g i (w) for n m where n is the number of full data points and g i (w) = −∇ w f i (w) BID11 .

As the batch size m increases, the randomness ofĝ(w) decreases.

Hence E ĝ(w) tends to g(w) , and Var( ĝ(w) ), which is the variance of the norm of the minibatch gradient, vanishes.

The convergence rate analysis is as the following: Theorem 1.

Letĝ(w) be a minibatch gradient induced from the minibatch index set I of batch size m from {1, . . .

, n} and suppose γ = max i,j∈{1,...,n} | g i (w), g j (w) |.

Then DISPLAYFORM0 and DISPLAYFORM1 Proof.

See Supplemental A.According to Theorem 1, a large batch size m reduces the variance of ĝ(w) centered at E ĝ(w) with convergence rate O(1/m).
...
This implies that the learning dynamics and convergence of SGD, measured in terms of the vanishing gradient, i.e., n b i=1ĝ i (w) ≈ 0, is not necessarily explained by the vanishing norms of minibatch gradients, but rather by the balance of the directions ofĝ i (w)'s, which motivates our investigation of the directional statistics of minibatch gradients.
...
We draw a density plot θ(u,ĝ j (w)) ĝ j (w)) ) for 3, 000 minibatch gradients (black) at w = w 0 0 (b) and w = w 0 final , with training accuracy of > 99.9%, (c) when u is given.

After SGD iterations, the density of θ(u,ĝ j (w)) converges to an asymptotic density (red).
...
The concentration parameter κ measures how uniformly the directions of unit vectors are distributed.

By Theorem 1, with a large batch size, the norm of minibatch gradient is nearly deterministic, andμ is almost parallel to the direction of full batch gradient.

In other words, κ measures the concentration of the minibatch gradients directions around the full batch gradient.

The following Lemma 1 introduces the relationship between the norm of averaged unit vectors and κ, the approximate estimator of κ.

Lemma 1.

The approximated estimator of κ induced from the d-dimensional unit vectors DISPLAYFORM0 Moreover, h(·) and h (·) are strictly increasing and increasing on [0, n b ), respectively.
...
Since h(·) is an increasing function, we may focus only on DISPLAYFORM2 pi−w pi−w to see howκ behaves with respect to its argument.

Lemma 2 implies that the estimated directional concentrationκ decreases if we move away from w FIG5 ).

In other words,κ(w ) <κ(w 0 0 ).

DISPLAYFORM3 If all p i 's are not on a single ray from the current location w, then there exists positive number η such that DISPLAYFORM4 DISPLAYFORM5 DISPLAYFORM6 Proof.
...
Then we reasonably expect that the direction ofĝ i (w) = −∇ w f Ii (w) is similar to that of p i (w) − w.

Each epoch of SGD with a learning rate η computes a series of w DISPLAYFORM7 . .

, n b } with a large batch size or at the early stage of SGD iterations.

Combining these approximations, ĝ i (w DISPLAYFORM8 For example, suppose that t = 0, n b = 3 and τ = 1, and assume that p i (w DISPLAYFORM9 Hence, we haveκ(w DISPLAYFORM10 DISPLAYFORM11 for a sufficiently small ξ > 0, then there exists positive number η such that DISPLAYFORM12 Proof.

See Supplemental C.3.This Theorem 3 asserts thatκ(·) decreases even with some perturbation along the averaged direction Without the corollary above, we need to solve p i (w 0 t ) = arg min w∈N (w 0 t ;r) f Ii (w) for all i ∈ {1, . . .

, n s }, where n s is the number of samples to estimate κ, in order to computeκ(w 0 t ).

Corollary 3.1 however implies that we can computeκ(w 0 t ) by usingĝ DISPLAYFORM13 In Practice Although the number of all possible minibatches in each epoch is n b = n m , it is often the case to use n b ≈ n/m minibatches at each epoch in practice to go from w 0 t to w 0 t+1 .

Assuming that these n b minibatches were selected uniformly at random, the average of the n b normalized minibatch gradients is the maximum likelihood estimate of µ, just like the average of all n b normalized minibatch gradients.

Thus, we expect with a large n b , DISPLAYFORM14 and that SGD in practice also satisfiesκ(w
...
Althoughκ with the large batch size decreases more smoothly rather than the small batch size, we observe thatκ still decreases well with minibatches of size 64.
...
We useκ to denote the κ estimate.
...
From these plots,κ decreases monotonically regardless of the minibatch size, but the variance over multiple training runs is much smaller with a larger minibatch size.
...
FNN and DFNN We first observe thatκ decreases over training regardless of the network's depth in FIG12 (a,b).

We however also notice thatκ decrease monotonically with the FNN, but less so with its deeper variant (DFNN).
...
This conjecture is supported by for instance BID27 , who demonstrated batch normalization guarantees the boundedness of Hessian, and BID25 , who showed residual connections eliminate some singularities of Hessian.κ near the end of training The minimum averageκ of DFNN+BN, which has 1, 920, 000 parameters, is 71, 009.20, that of FNN+BN, which has 636, 800 parameters, is 23, 059.16, and that of CNN+BN+Res, which has 207, 152 parameters, is 20, 320.43.

These averageκ are within a constant multiple of estimated κ using 3,000 samples from the vMF distribution with true κ = 0 (35, 075.99 with 1, 920, 000 dimensions, 11, 621.63 with 636, 800 dimensions, and 3, 781.04 with 207, 152 dimensions.)
...
This quantity includes both the gradient norm stochasticity (GNS) and the directional uniformity κ, implying that either or both of GNS and κ could drive the gradient stochasticity.
...
and directional uniformityκ.

We normalized each quantity by its maximum value over training for easier comparison on a single plot.

In all the cases, SNR (orange) andκ (red) are almost entirely correlated with each other, while normSNR is less correlated. (Second row) We further verify this by illustrating SNR-κ scatter plots (red) and SNR-normSNR scatter plots (blue) in log-log scales.

These plots suggest that the SNR is largely driven by the directional uniformity.

From FIG13 (First row), it is clear that the proposed metric of directional uniformityκ correlates better with the gradient stochasticity than the gradient norm stochasticity does.
...
Define selector random variables BID11 as below: DISPLAYFORM0 Then we haveĝ DISPLAYFORM1 Lemma A.1.

Letĝ(w) be a minibatch gradient induced from the minibatch index set I with batch size m from {1, . . .

, n}. Then DISPLAYFORM2 where γ = max i,j∈{1,...,n} | g i (w), g j (w) |.
...
Then DISPLAYFORM4 and DISPLAYFORM5 Hence, Var( ĝ(w) ) DISPLAYFORM6
...
Theorem B.1. (Slutsky's theorem, BID3 ) Let {x n }, {y n } be a sequence of random variables that satisfies x n ⇒ x and y n P → ρ when n goes to infinity and ρ is constant.

Then x n y n ⇒ cx Theorem B.2. (Delta method, BID3 ) Let y n be a sequence of random variables that satisfies √ n(y n − µ) ⇒ N (0, σ 2 ).

For a given smooth function f : R → R, suppose that f (µ) exists and is not 0 where f is a derivative.
...
Suppose that µ = 0, σ = 1, and f (·) = DISPLAYFORM8 Moreover, h(·) and h (·) are strict increasing and increasing on [0, n b ), respectively.

(1 −r 2 ) 2 and its numerator is always positive for d > 2.

When d = 2, DISPLAYFORM0 (1 −r 2 ) 2 > 0.

Soκ increases asr increases.
...
Hence the derivative ofκ with respect to u is bounded.

This implies the Lipschitz continuity of h(·).h(·) is strictly increasing sincer = u n b. Further, DISPLAYFORM2 If all p i 's are not on a single ray from the current location w, then there exists positive number η such that DISPLAYFORM3 Proof.
...
DISPLAYFORM0 for sufficiently small ξ > 0, then there exists positive number η such that DISPLAYFORM1 for all ∈ (0, η].Proof.
...
If ) where η is a learning rate.

To prove Corollary 3.1, we need to showκ(w 0 t+1 ) <κ(w 0 t ) which is equivalent to DISPLAYFORM7 DISPLAYFORM8 Since DISPLAYFORM9 is Lipschitz continuous on R BID2 .

If the batch size is sufficiently large and the learning rate η is sufficiently small, ĝ i (w DISPLAYFORM10 If we denote τ η as , we can convert (8) to (9).

DISPLAYFORM11 Since both w DISPLAYFORM12 where σ max (A) and σ min (A) are maximal and minimal singular values of A, respectively.

If A is positive-definite matrix, then DISPLAYFORM13 Here λ max (A) and λ min (A) are maximal and minimal eigenvalues of A, respectively.
...
By the second order Taylor expansion, DISPLAYFORM15 Then, we only need to show DISPLAYFORM16 Since H i is positive definite, we can diagonalize it as H i = P i Λ i P i where P i is an orthonormal transition matrix for H i .

DISPLAYFORM17 for sufficiently small ξ.

This implies (6) since DISPLAYFORM18 .Then we can apply Theorem 3 andκ(w DISPLAYFORM19 where h(·) is increasing and Lipschitz continuous(Lemma 1).

By Lemma D.1, we have DISPLAYFORM20 t ) where rhs is bounded by ξ.
...
Figure 8: We showκ estimated from {1, 000 (black), 2, 000 (blue), 3, 000 (red)} random samples of the vMF distribution with underlying true κ in 10, 000-dimensional space, as the function of κ (in log-log scale except 0).

For large κ, it is well-estimated byκ regardless of sample sizes.

When the true κ approaches 0, we need a larger sample size to more accurately estimate this.

635, 200 20, 111.90 ± 13.04 14, 196.89 ± 14.91 11, 607.39 ± 9.27 FNN+BN 636, 800 20, 157.57 ± 14.06 14, 259.83 ± 16.38 11, 621.63 ± 6.83 DFNN 1, 915, 200 60, 619.02 ± 13.49 42, 849.86 ± 18.90 34, 983.31 ± 15.62 DFNN+BN 1, 920, 000 60, 789.84 ± 17.93 42, 958.71 ± 25.61 35, 075.99 ± 12.39 F SOME NOTES ABOUT THE κ ESTIMATE We point out that, for a small κ, the absolute value ofκ is not a precise indicator of the uniformity due to its dependence on the dimensionality, as was investigated earlier by BID5 .

In order to verify this claim, we run some simulations.

First, we vary the number of samples and the true underlying κ with the fixed dimensionality (Unfortunately, we could not easily go over 10, 000 dimensions due to the difficulty in sampling from the vMF distribution with positive κ.).

We draw {1, 000, 2, 000, 3, 000} random samples from the vMF distribution with the designated κ.

We computeκ from these samples.

As can be seen from Figure 8 , theκ approaches the true κ from above as the number of samples increases.

When the true κ is large, the estimation error rapidly becomes zero as the number of samples approaches 3, 000.

When the true κ is low, however, the gap does not narrow completely even with 3, 000 samples.

While fixing the true κ to 0 and the number of samples to {1, 000, 2, 000, 3, 000}, we vary the dimensionality to empirically investigate theκ.

We choose to use 3, 000 samples to be consistent with our experiments in this paper.

We run five simulations each and report both mean and standard deviation TAB1 .We clearly observe the trend of increasingκ's with respect to the dimensions.

This suggests that we should not compare the absolute values ofκ's across different network architectures due to the differences in the number of parameters.

This agrees well with BID5 which empirically showed that the threshold for rejecting the null hypothesis of κ = p by usingκ where p is a fixed value grows with respect to the dimensions.

We show plots from other four training runs in FIG12 .

For all runs, the curves of GS (inverse of SNR) andκ are strongly correlated while GNS (inverse of normSNR) is less correlated to GS.

Figure 9: (a,c,e) We plot the evolution of the training loss (Train loss), validation loss (Valid loss), inverse of gradient stochasticity (SNR), inverse of gradient norm stochasticity (normSNR) and directional uniformity κ.

We normalized each quantity by its maximum value over training for easier comparison on a single plot.

In all the cases, SNR (orange) andκ (red) are almost entirely correlated with each other, while normSNR is less correlated. (b,d,f) We further verify this by illustrating SNR-κ scatter plots (red) and SNR-normSNR scatter plots (blue) in log-log scales.

These plots suggest that the SNR is largely driven by the directional uniformity.

Figure 10: (a,c,e) We plot the evolution of the training loss (Train loss), validation loss (Valid loss), inverse of gradient stochasticity (SNR), inverse of gradient norm stochasticity (normSNR) and directional uniformity κ.
...
Under review as a conference paper at ICLR 2019 Figure 11: (a,c,e) We plot the evolution of the training loss (Train loss), validation loss (Valid loss), inverse of gradient stochasticity (SNR), inverse of gradient norm stochasticity (normSNR) and directional uniformityκ.

We normalized each quantity by its maximum value over training for easier comparison on a single plot.

In all the cases, SNR (orange) and κ (red) are almost entirely correlated with each other, while normSNR is less correlated. (b,d,f) We further verify this by illustrating SNR-κ scatter plots (red) and SNR-normSNR scatter plots (blue) in log-log scales.

These plots suggest that the SNR is largely driven by the directional uniformity.

Figure 12: (a,c,e) We plot the evolution of the training loss (Train loss), validation loss (Valid loss), inverse of gradient stochasticity (SNR), inverse of gradient norm stochasticity (normSNR) and directional uniformity κ.
