...
Starting from a constant function F 0 and applying steepest descent we can findF by iteratively updating with learning rate γ:
...
The primal form solution of Eq. 7 isv t = (o T t o t + αI) −1 o t r t and plugging it into Eq. 6 yields the following update rule:
...
In the following we will compare the performance of DGB and SGD on a variety of reference data sets and explore the effect of the α parameter using simple dense layer neural networks.
...
Both left and right variants achieve increasingly better test set performance with larger α values culminating with 98.59% average accuracy for the DGB(r) model with α = 1.0.
...
The learning rate was kept constant at 0.01 for SGD and DGB for high α values and was reduced to 0.001 for α ≤ 0.1.
...
The best test set accuracy is obtained for a smaller α value (Table A1) .
...
Table A2 shows the results for experiments run using the SGD optimizer and DGB with α values 1.0, 0.1 and 0.01.
...
In this case, the results are close for all experiments and only marginally better at 78.05% mean accuracy for DGB with α = 0.1.
...
Just as the previous experiments, DGB with larger α values were closer in performance to SGD (Table A4 ).

In this case, relaxing the regularization parameter led to a gradual increase in test set performance with almost half the root mean squared error for DGB with α = 0.001 when compared to SGD.
...
in the case of DGB(r) andô

in the case of DGB(l).

In the above notationÔ t is the normalized version of layer input matrix O t with elementsô ij .
...
After resizing the input tensor O t to (B × w 0 × h 0 , c 0 ) the new normalized inputÔ t becomes:ô
...
It is clear from Eq. 10 and previous paragraphs that DGB's gradient update formulation can also be viewed as input normalization defined as N (X) := X(X T X + αI) −1 or N (X) := (XX T + αI) −1 X followed by a regular SGD update.
...
In contrast to these methods, the normalization procedure described here is applied to the layer input, it is not scale invariant and has only one parameter α which is predefined before training, not learned.
...
If X = U ΣV T is the singular value decomposition of input matrix X, where U and V are orthogonal matrices and Σ is a diagonal matrix, then the condition number of

where σ max and σ min are the largest and respectively smallest singular values of X.

Let N (X) := X(X T X + αI) −1 and arg min w (y − N (X)w) T (y − N (X)w) be the resulting linear regression problem after applying input normalization.

The new Hessian matrix is

After plugging in X = U ΣV T and some linear algebra manipulation the new condition number becomes:
...
The same result can be obtained when using the alternative input normalization N (X) := (XX T + αI) −1 X (see Prop.
...
Let X = U ΣV T be the singular value decomposition of X then:
