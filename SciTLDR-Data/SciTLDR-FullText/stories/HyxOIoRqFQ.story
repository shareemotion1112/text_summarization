...
The evidence lower bound objective (ELBO), takes the form of an expectation computed over the approximate posterior, Q φ (z), DISPLAYFORM0 Optimizing this objective with respect to the generative model parameters, θ, is straightforward, as we can push the derivative inside the expectation, and perform stochastic gradient ascent.

However, we cannot do the same with the recognition parameters, φ, as they control the distribution over which the expectation is taken.
...
In our case, samples from the approximate posterior are the relaxed variables,ẑ, so we need to evaluateQ (ẑ).

However, we are actually using a different expression,Q (ẑ), and while this may in practice be an effective approximation, it cannot give us a valid variational bound.

To obtain a valid variational bound, we must useQ (z), which we can compute but not differentiate.

To obtain a valid variational bound, we need to evaluate the actual probability of the relaxed variable, i.e. we need to computeQ (ẑ), andP (ẑ).

While we could work with theẑ directly, this is known to be numerically unstable so instead we work in terms of the logistic variables, l BID11 .
...
Further, note that we have not specified whether these distributions correspond to the discrete or relaxed model, as we can obtain eitherẑ orz from exactly the same l, by using different inverse-temperatures for the transformation (Eq. 4).
...
One approach to sampling the sequential autoregressive posterior in Eq. FORMULA7 is to use, DISPLAYFORM0 where η t is Logistic (0, 1) noise, l t and η t are length N vectors, and we define f t so as to highlight the temporal dependencies (cf.
...
To construct these iterations, we simply apply the autoregressive updates at all time-steps in parallel, (rather than sequentially, see FIG0 ), DISPLAYFORM1 where all the variables are N × T matrices, and where v x, σ βl k is simply Eq. (8) computed in parallel across time for an externally specified input.
...
First, we can compute relaxed Bernoulli variables usingẑ = σ βl K , and insert them into the probability mass function for the autoregressive discrete approximate posterior (Eq. 8).
...
Normalizing flows exploit the fact that we can compute the probability density of a random variable, l k (η), generated by transforming, via a oneto-one function, a sample η, DISPLAYFORM0 where |∂l k /∂η| is the absolute value of the determinant of the Jacobian of l k (η).

While the determinant of the Jacobian is often very difficult to compute, we can ensure the Jacobian is 1 by using a restricted family of transformations, under which the value of l k t depends only on the current value, η t via simple addition, and on past values of η via an arbitrary function, DISPLAYFORM1 And we know that our fixed-point iterations indeed lie in this family of functions, as Eq. FORMULA0 mirrors the above definition, with, DISPLAYFORM2 where l k 1:t−1 indeed depends only on η 1:t−1 (see Eq. 10).
...
In particular, we consider a Gaussian likelihood, P (x = 0.5|ẑ) = N (x = 0.5;ẑ, σ), and thus, as σ decreases, there is increasing evidence thatẑ is close to 0.5.

Under the true model, with a hard-threshold, this can never be achieved:z must be either 0 or 1.

However, if we combine a relaxation (Eq. 4) with a Logistic approximate posterior, it is possible to reduce the variance of the logistic sufficiently such that the relaxed Bernoulli variable,ẑ, is indeed close to 0.5.To quantify this effect, we consider how the standard deviation ofẑ (which should be 0.5 under the true posterior), varies as we change the standard deviation of the likelihood, σ FIG0 ).
...
For the generative model, we take a weighted sum of inputs from past spikes, filtered by a temporal kernel κ u and κ v , which , inferred rates (middle) and inferred spiking (bottom), for the true posterior (dark gray), then models with factorised posteriors trained using VAE (red) and supervised (purple) procedures, and finally our new discrete flow trained using VAE (blue).
...
The second is the recognition temporal kernel κ v , which captures the anticorrelations induced by explaining away FIG2 ).All strategies, including factorised and autoregressive (flow) VAE's, and supervised training give roughly similar reconstructions FIG2 .
