...
We also assume that we are given a suitable loss function L(θ, x, y), for instance the cross-entropy loss for a neural network.

As usual, θ ∈ R p is the set of model parameters.

Our goal then is to find model parameters θ that minimize the risk DISPLAYFORM0 Empirical risk minimization (ERM) has been tremendously successful as a recipe for finding classifiers with small population risk.
...
We will refer to the quantity ρ(θ) as the adversarial loss of the network with parameters θ.
...
In particular, when the parameters θ yield a (nearly) vanishing risk, the corresponding model is perfectly robust to attacks specified by our threat model.
...
This raises the question how we can compute gradients ∇ θ ρ(θ) for the outer minimization problem.

Since the adversarial loss function ρ(θ) corresponds to a maximization problem, we cannot simply apply the usual backpropagation algorithm.
...
However, for the case of continuously differentiable functions, Danskin's theorem -a classic theorem in optimization -states that this is indeed true and gradients at maximizers of the inner problem correspond to descent directions for the saddle point problem (see Appendix C for details).Leveraging this connection, our goal now is to find a reliable algorithm for solving the inner maximization problem, i.e., to evaluate ρ(θ).

When instantiated for a batch of examples (instead of the expectation over the entire distribution D), finding a maximizer δ ∈ S of ρ(θ) corresponds exactly to finding an attack on the neural network.
...
We train and evaluate against perturbations of size ε = 0.3.
...
We trained the network against a PGD adversary with ∞ projected gradient descent again, this time using 7 steps of size 2, and a total ε = 8.
...
Resistance for different values of ε and 2 -bounded attacks.
...
On one hand, we investigate the resistance to ∞ -bounded attacks for different values of ε.
...
We emphasize that the models we are examining here correspond to training against ∞ -bounded attacks with the original value of ε = 0.3, for MNIST, and ε = 8 for CIFAR10.

In particular, our MNIST model retains significant resistance to 2 -norm-bounded perturbations too -it has good accuracy even for ε = 4.5.We provide a sample of corresponding adversarial examples in FIG0 of Appendix F. One can observe that some of the underlying perturbations are large enough that even a human could be confused.

Training Accuracy.

It is worth noting our MNIST and (wide) CIFAR10 networks reached 100% adversarial accuracy on the training set.

That is we can fit the training set even against a PGD adversary of ε = 0.3 and ε = 8 respectively.
...
The MNIST and CIFAR10 networks were trained against ε = 0.3 and ε = 8 PGD ∞ adversaries respectively (the training ε is denoted with a red dashed lines in the ∞ plots).

We notice that for ε less or equal to the value used during training, the performance is equal or better.
...
Convolutional layers are followed by 2 × 2 max-pooling layers and adversarial examples are constructed with ε = 0.3.
...
This effect is greater when considering adversarial examples with smaller ε.
...
For the case of smaller ε the loss is ofter linear enough in the ∞ ball around natural examples, that FGSM finds adversarial examples close to those found by PGD thus being a reasonable adversary to train against.
...
Recall that our goal is to minimize the value of the saddle point problem DISPLAYFORM0 In practice, we don't have access to the distribution D so both the gradients and the value of ρ(θ) will be computed using sampled input points.
...
In Figure 8 we plot our experimental findings on the MNIST dataset for ε = 0.3.
...
Combined with the bias and the ReLU that follows, this results in a thresholding filter, or equivalently ReLU(αx − β) for some constants α, β.

From the perspective of adversarial robustness, thresholding filters are immune to any perturbations on pixels with value less than β − ε.
