...
We propose a hierarchical process, where first features κ are sampled and later used to predict the potential of policies conditioned on these features, φ|κ →
...
The goal in contextual policy search is to find the best policy π * (θ|c) that maximizes the return

with reward r t (x t , u t ) at time step t for executing the motor commands u t in state x t .

For learning the policy vector and the context, we collect samples of the return J(θ [k] ) ∈ R 1 , the evaluated policy parameter vector θ
...
The main concept of Bayesian Optimization is to build a model for a given system based on the so far observed data D = {X, y}. The model describes a transformation from a given data point x ∈ X to a scalar value y ∈ y, e.g. from the parameter vector θ to the return J(θ).

Such model can either be parametrized or non-parametrized and is used for choosing the next query point by evaluating an acquisition function α(D).
...
This representation is fully defined by the mean m and the covariance K. We chose m(x) = 0 as mean {0, 1} flag indicating the success of rollout k function and as covariance function a Matérn kernel Matérn (1960) .

It is a generalization of the squared-exponential kernel that has an additional parameter ν which controls the smoothness of the resulting function.

The smoothing parameter can be beneficial for learning local models.

We used Matérn kernels

with ν = 5 / 2 and the gamma function Γ, A = (2 √ ν||x p − x q ||) /l and modified Bessel function H ν Abramowitz & Stegun (1965) .

The length-scale parameter of the kernel is denoted by σ, the variance of the latent function is denoted by σ y and δ pq is the Kronecker delta function (which is one if p = q and zero otherwise).

Note that for ν = 1/2 the Matérn kernel implements the squared-exponential kernel.

In our experiments, we optimized the hyperparameters θ = [σ, l, σ y ] by maximizing the marginal likelihood Williams & Rasmussen (2006) .

Predictions for a query points D * = {x * , y * } can then be determined as

The predictions are then used for choosing the next model evaluation point x n based on the acquisition function α(x; D).

We use expected improvement (EI) Mockus et al. (1978) which considers the amount of improvement

where τ is the so far best measured value max(y), Φ the standard normal cumulative distribution function, φ the standard normal probability density function and ξ ∼ σ ξ U (−0.5, 0.5) a random value to ensure a more robust exploration.
...
We learn a joint distribution p(J(θ
...
However, instead of directly conditioning on the most promising policy vectors using α BO = α(θ; D), we propose an iterative conditioning scheme.

Therefore, the two acquisition functions

are employed, where for Equation (7), the evaluated mean µ(θ; c) and variance σ(θ; c) for the parameter θ are conditioned on the features c. The hierarchical optimization process works then as follows:

In the first step we estimate the best feature values based on a GP model using the acquisition function from Equation (6)

These feature values are then used to condition the search for the best new parameter θ [k+1] using Equation (7) θ

We subsequently continue evaluating the policy vector θ [k+1] using the reward function presented in Equation (1)

To ensure robustness for Bayesian Optimization, mental replays can be generated.

Therefore, the new training data set J(θ

, generated by the policy parameter θ [k+1] , will be enlarged by augmenting perturbed copies of the policy parameter θ
...
Hence, perturbed copies of the parameters θ

[k+1] and features c [k+1] are generated keeping the objective J(θ [k+1] ) constant.
...
On average, participants required 6 trials (σ human = 3.1) to successfully complete the motion.
...
As parametrized policy, we use a via point [φ i ,φ i ], where φ i is the position of joint i at time t via andφ i the corresponding velocity.
...
The return of a roll-out J(θ) is composed according to J(θ) = −(c balance + c time + c control ) with the balancing costs c balance = 1/N end
