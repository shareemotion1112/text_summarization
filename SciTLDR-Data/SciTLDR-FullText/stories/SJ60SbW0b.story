...
As illustrated in FIG0 , given an input x, we draw a noisy vector η ∼ H and corrupt x according to A(x) as follows: DISPLAYFORM1 where 1 denotes a tensor of ones with the same shape as A(x), and all operations are performed element-wise.
...
In these structured cases, we suspect it is important to ensure that the noise vector η lies near the manifold of the input samples.

Based on this principle, we propose two methods of defining H via the generating process for η:• Constant noise η const : In domains where input features represent quantities with default value c (e.g. 0 word counts in a bag of words, 0 binary valued images), set η = c1, where 1 is a tensor of ones with the appropriate shape and c ∈ R.• Bootstrapped noise η boot : Draw uniform random samples from the training dataset.
...
This translates to the following optimization procedure: DISPLAYFORM0 where A(x) represents the attention mask learned specifically for sample x andx is a function of η, A and x defined in Eq. (2).
...
The absence of important words from other classes is explained by our choice of η 0 -noise, which produces more visually appealing attention-masks, but doesn't penalize the LAN for ignoring such words.
...
Furthermore, we explored two types of noise for input corruption: η const and η boot .
...
We use β = 5.0 and η = η boot for this experiment.
...
We used β = 5.0 for this experiment.
...
We used β = 7.0 and η = η boot for this experiment.
...
We used β = 50.0 and η = η const with a constant value of 0.
...
We used β = 50.0 and η = η boot for both experiments.

In this section, we illustrate the role of the β hyperparameter in the sample specific experiments.

As stated earlier, β controls the trade-off between the amount of corruption in the input and the similarity of the corrupted and uncorrupted inputs (measured as a function of the respective pretrained network outputs).

Based on the loss functions presented in equations 3 and 4, high values of β encourage a larger amount of input corruption and weight the corresponding term more heavily in the loss computation.
...
In these experiments we learned sample-specific attention masks for different settings of β on the images shown below.

For our input corruption, we used uniformly sampled RGB noise: η ∼ Uniform(0, 1).
...
We now observe the sample-specific attention masks trained from this classifier:The resulting attention mask (β = 1.0 with bootstrapped noise η = η boot ) assigns high importance to the pixels associated with the cloud while giving no importance to the region of the image containing the tank.
