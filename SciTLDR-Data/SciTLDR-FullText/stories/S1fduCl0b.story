...
The joint distribution can in turn be factorized using the conditional distribution P θ (x|z) or the posterior P θ (z|x).

DISPLAYFORM0 We model the conditional distribution P θ (x|z) by a decoder, typically a neural network.

Very often the marginal likelihood P θ (x) will be intractable because the integral in equation FORMULA0 does not have an analytical form nor an efficient estimator (Kingma (2017)).

As a result the respective posterior distribution, P θ (z|x), is also intractable.

Variational inference side-steps the intractability of the posterior by approximating it with a tractable distribution Q φ (z|x) ≈ P θ (z|x).

VAEs use an encoder (generally a neural network) to model the approximate posterior Q φ (z|x) and optimize the parameters φ to minimize the reverse KL divergence KL[Q φ (z|x)||P θ (z|x)] between the approximate posterior distribution Q φ (z|x) and the true posterior P θ (z|x).

Given that Q φ (z|x) is a powerful model (such that the KL divergence against the true posterior will be close to zero) we maximize the tractable Evidence Lower BOund (ELBO) to the intractable marginal likelihood.

DISPLAYFORM1 By sharing the variational parameters φ of the encoder across the data points (amortized inference Gershman & Goodman FORMULA0 ), variational autoencoders avoid per-data optimization loops typically needed by mean-field approaches.

The standard setting in maximum-likelihood generative modeling is to estimate the set of parameters θ that will maximize the marginal likelihood P θ (x) for data sample X generated IID from a single true data distribution P * (x).
...
Latent variable models P θ (x, z) = P θ (x|z)P (z) (such as VAEs) capture the complex structures in P * (x) by conditioning the observed variables x on the latent variables z and combining these in (possibly infinite) mixtures P θ (x) = P θ (x|z)P (z)δz.
...
Our goal is to sequentially build an approximation P θ (x) of the true mixture P * (x) by only observing data from a single component P * i (x) at a time.
...
At any given time the teacher contains a summary of all previous distributions within the learned parameters of the encoder Q Φ (z|x) and the decoder P Θ (x|z).

The teacher is used to generate synthetic samplesx from these past distributions by decoding samples from the priorẑ ∼ P (z) through the decoderx ∼ P Θ (x|ẑ).
...
The bottom part of figure 2 represents the student, which is responsible for updating the parameters of the encoder Q φ (z|x) and decoder P θ (x|z) models over the newly observed data.

The student is exposed to a mixture of learning instances x sampled from x ∼ P (ω)P (x|ω), ω ∼ Ber(π); it sees synthetic instances generated by the teacher P (x|ω = 0) = P Θ (x|z), and real ones sampled from the currently active training distribution P (x|ω = 1) = P * (x).

The mean π of the Bernouli distribution controls the sampling proportion of the previously learnt distributions to the current one.

If we have seen k distinct distributions prior to the currently active one then π = k k+1 .
...
Each new student instantiation uses the input data mix to learn a new approximate posterior Q φ (z|x).
...
To achieve this, we complement the classical VAE objective (equation FORMULA1 ) with a term minimizing the KL divergence KL[Q φ (z|x)||Q Φ (z|x)] between the student's and the teacher's posteriors over the synthetic datax.
...
A critical component of our model is the synthetic data generation by the teacher's decoder x ∼ P Θ (x|z).
...
The teacher generates these synthetic samples by first sampling the latent variable from the prior z ∼ P (z) followed by the decoding stepx ∼ P Θ (x|ẑ).

As we will describe shortly, the latent variableẑ has a categorical component which corresponds to all the past distributions.
...
We use the uniform multivariate categorical prior z d ∼ Cat( 1 J ) to represent it and the same parametric family for the approximate posterior Q Φ (z|x).

The continuous z c component is the global representation of the distributional variability and we use the multivariate standard normal as the prior z c ∼ N (0, I) and the isotropic multivariate normal N (µ, σ 2 I) for the approximate posterior.

When generating synthetic data, the teacher now independently samples from the discrete and continuous priorsẑ d ∼ P (z d ),ẑ c ∼ P (z c ) and uses the composition of these to condition the decoding stepx ∼ P Θ (x|ẑ d ,ẑ c ).

Since the discrete representationẑ d is associated with the true generative distribution components P * i (x), uniformly sampling the discrete prior ensures that that the distributions are well represented in the synthetic mix that the student observes.
...
DISPLAYFORM0 We sample the training instances x from x ∼ P (ω)P (x|ω), ω ∼ Ber(π) as described in section 4.

Thus they can either be generated from the teacher model (ω = 0) or come from the training set of the currently active distribution (ω = 1).
...
The λ hyper-parameter controls the importance of the mutual information regularizer.
...
We measure the ability of the models to recall the previously learned information by looking at the consistency between the posterior of the student and the teacher models over the test data set consistency: #{k : DISPLAYFORM0 We use the MNIST dataset in which we rotate each of the original digit samples by angles ν = [30 DISPLAYFORM1 We treat each rotation of a single digit family as an individual distribution {P * DISPLAYFORM2 .
...
Once done, we freeze the parameters of the encoder and the decoder and transfer the model into the teacher state (φ → Φ, θ → Θ).
...
We then decode these to obtain the samplesx ∼ P θ (x|z d , z c ) .
...
Variational inference BID10 side-steps the intractability of the posterior distribution by approximating it with a tractable distribution Q Φ (z|x); we then optimize the parameters Φ in order to bring this distribution close to P Φ (z|x).

The form of this approximate distribution is fixed and is generally conjugate to the prior P (z).

Variational inference converts the problem of posterior inference into an optimization problem over Φ. This allows us to utilize stochastic gradient descent to solve our problem.
...
The Gumbel-Softmax reparameterization over logits [linear output of the last layer in the encoder] p ∈ R M and an annealed temperature parameter τ ∈ R is defined as: DISPLAYFORM0 u ∈ R M , g ∈ R M .

As the temperature parameter τ → 0, z converges to a categorical.

Multilayer neural networks with sigmoidal activations have a VC dimension bounded between O(ρ 2 )Sontag (1998) and O(ρ 4 ) BID14 where ρ are the number of parameters.
