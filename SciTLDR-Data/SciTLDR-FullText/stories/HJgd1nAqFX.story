...
An MDP is defined as a tuple (S, A, T, R, γ) where S and A are the state space and the action space respectively, T (s |s, a) is the transition probability of reaching state s ∈ S by taking action a ∈ A from s ∈ S, R is the immediate reward by the transition, and γ is a discount factor.

The Q-value function for a tuple of actions is defined to be Q π (s, a) = E[ T t=0 γ t r t |s 0 = s, a 0 = a], where T is the number of timesteps till termination.
...
The optimal Q-value function Q * (s, a) = max π Q π (s, a), ∀s ∈ S, a ∈ A (Sutton & Barto, 1998) satisfies the Bellman optimality equation DISPLAYFORM0

For an undirected graph G = (V, E), the Message Passing Neural Network (MPNN) framework BID3 formulates two phases of the forward pass to update the node-level feature representations h v , where v ∈ V , and graph-level feature vectorŷ.
...
The readout phase uses the readout function R, and computes the graph-level feature vectorŷ = R(h T v |v ∈ G).
...
Let θ = (E, w GN N , w dom , w token , w mode ) be the model parameters including the embedding matrices, the weights of a graph neural network, and weights of the factorized Q-value function.

The model parameters are updated by minimizing the squared TD error BID16 : DISPLAYFORM4 where the transition pairs (s, a, r, s ) are sampled from the replay buffer and y DQN is the factorized target Q-value with the target network parameters θ − as in the standard DQN algorithm.
...
The weights, {α 1 .....α V }, are computed with the goal vector as the query and node-level features as keys.
