...
Our question selection method is closely related to the prior work of Rao & Daumé III, 2018; Kovashka & Grauman, 2013; Ferecatu & Geman, 2007; Guo et al., 2018) .
...
More recently, (Rao & Daumé III, 2018) proposes a learning-to-ask model by modeling the expected utility obtained by a question.

Our selection method can be considered as a special case when entropy is used as the utility.

In contrast to (Rao & Daumé III, 2018) , our work models the entire interaction history instead of a single turn of follow-up questioning.

Our model is trained using crowdsourced annotations, while (Rao & Daumé III, 2018) uses real user-user interaction data.
...
We estimate the enc(·) with parameters ψ by pre-training using the dataset,
...
The parameters ψ, w and b are fine-tuned using reinforcement learning during training of the policy controller (Section 4.5).
...
At the beginning of an interaction, the simulator selects a target labelŷ, and samples a query x from the associated query set to start the interaction.

Given a system clarification question q t at turn t, the simulator responds with an answer r t ∈ R(q t ) by sampling from a belief distribution p (r | q t ,ŷ).
...
The policy controller f (·, ·; θ) is a feed-forward network parameterized by θ that takes the top-k probability values and current turn t as input state.
...
We learn the policy controller f (·, ·; θ), and fine-tune p(r k | q j , y i ) by back-propagating through the policy gradient.
...
Second, we disable the parameterized estimatorp(r k | q j , y i ) by setting λ to 1.
...
w/ fixed turn 71 ± 1.0% 81 ± 0.9% 39 ± 0.2% 56 ± 0.4% w/ λ = 1 66 ± 0.8% 71 ± 1.0% 40 ± 0.1% 56 ± 0.2% Table 1 : Performance of our system against various baselines, which are evaluated using Accuracy@{1, 3}. For all interacting baselines, 5 clarification questions are used.
...
The model variant with λ = 1, which has fewer probability components leveraging natural language than our full model, achieves worse Accuracy@1.
...
The row mean # tags is the mean number of tags that are annotated to a target, N.A. is the percentage of the tasks are annotated as "none of the above", and mean κ is the mean pairwise Cohen's κ score.
