...
Our attempts to find quadratic and cubic Bézier curves BID8 with a decreasing objective between the two sparse solutions fails suggesting that the optimization process gets attracted into a "bad" local minima.• Finally, by removing the sparsity constraint from the path, we are consistently able to find decreasing objective Bézier curves between the two sparse solutions.
...
BID10 measured the training loss on MNIST BID17 along the line segment between the initial point θ s and the solution θ e , observing a monotonically decreasing curve.
...
In our linear interpolation experiments, we generate networks along the segment θ = tθ e + (1 − t)θ s for t ∈ [−0.2, 1.2] with increments of 0.01 and evaluate them on the training set of 500k images.
...
The objective is identical to the objective used during training, which includes a weight decay term scaled by 10 −4 .We now seek to find a non-linear path between the initial point and solution using parametric Bézier curves of order n = 2 and 3.

These are curves given by the expression DISPLAYFORM0 where θ 0 = θ e and θ n = θ s .

We optimize the following, DISPLAYFORM1 using the stochastic method as proposed by BID8 with a batch size of 2048 where L(θ) denotes the training loss as a function of trainable parameters.
...
The training loss along the linear segment and the parametric Bézier curve connecting the scratch and the pruned solutions are shown in FIG3 .
...
Using a third order Bézier curve also fails to decrease the maximum loss value over the second order curve (Appendix D).
...
We continue our experiments by removing the sparsity constraint from the quadratic Bézier curve and optimize over the full parameter space FIG3 .
...
FIG7 depicts the objective along the third order Bézier curves.
...
Training loss along the third order (cubic) Bézier curve found between the "pruning" and "scratch" solutions.
