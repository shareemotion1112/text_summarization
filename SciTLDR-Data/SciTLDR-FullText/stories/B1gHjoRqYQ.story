...
For non-targeted adversarial attacks, the constraint is defined as DISPLAYFORM1 where ε is the offset parameter.

As a common practice, ε is often set to a small negative number to ensure that the adversarial sample lies on the incorrect side of the decision boundary.

In this paper, we will only consider non-targeted attack, but all the discussions are applicable to targeted attacks (i.e. c(x) = max i =a l i (x) − l a (x) − ε for a target class a).
...
Formally, it solves: DISPLAYFORM0 where α (k) is the step size within [0, 1] .

Notice that the left hand side of the constraint in Eq. (3) is the first-order Taylor approximation of c(z DISPLAYFORM1 , so this constraint tries to move point closer to c(x) = 0 by α (k) .
...
Formally, DISPLAYFORM4 where β (k) is the step size within [0, 1]; a (k) and b (k) are two scalars, which will be specified later.
...
Final Tuning MARGINATTACK can only cause misclassification when c(x) ≤ ε.
...
1, where DISPLAYFORM3 and assuming DISPLAYFORM4 where DISPLAYFORM5 A and B are defined in Eq. (32).According to assumption 8, this implies DISPLAYFORM6 at the rate of at least 1/n ν .Proof.
...
First, for the restoration move DISPLAYFORM8 The first line is from the generalization of Mean-Value Theorem with jump discontinuities, and ξ = tz (k) + (1 − t)x (k) and t is a real number in [0, 1].
...
First, for restoration move DISPLAYFORM22 δm 2 (38) Line 4 is given by Eq. (3).
...
δm 2 (39) where DISPLAYFORM0 It can easily be shown that DISPLAYFORM1 Therefore DISPLAYFORM2 Combining Eqs. FORMULA2 and FORMULA1 , we have DISPLAYFORM3 Step Case: Assume Eq. (51) holds ∀k ≤ K, then Eqs. (31) and (45) holds ∀k ≤ K.• Proving |c(x (K+1) )| ≤ C: DISPLAYFORM4 where the last inequality is given by Eq. (50).
...
By definition DISPLAYFORM11 * is a local minimum to Eq. (1), ∃j ∈ I, ε < 1, ∀δ < ε, DISPLAYFORM12 65) Otherwise, if c(x δ ) ≤ 0, then x δ is a feasible solution to the problem in Eq. FORMULA0 and DISPLAYFORM13 which contradicts with the assumption that x * is a unique local optimum in B. DISPLAYFORM14 takes discrete values.

Therefore, to satisfy assumption 2, s j (x δ ) = s j (x * ), which implies DISPLAYFORM15 The first inequality is because DISPLAYFORM16 Eqs. FORMULA6 and FORMULA6 cause a contradiction.
...
P [x − x 0 ] 2 2 = 0, we have x − x = λs(x ).

From assumption 4, we know that c(x ) is monotonic along x − x = λs(x ).
...
Notice that the product of λ and ∇ T c(x )

y is constant, so if λ is to be minimized, then ∇ T c(x )y needs to be maximized.

Namely, y can be determined by solving which is the definition of dual norm.

Therefore y = s(x ) (74) Plug Eq. (74) into the constraint in Eq. (72), we can solve for λ.
...
1.5, we have DISPLAYFORM19 Plug Eqs. FORMULA7 to FORMULA7 into FORMULA6 , we have DISPLAYFORM20 On the other hand, let γ = m a /2 x * − x 0 2 , then ∀x satisfying Eq. (77) and DISPLAYFORM21 where the second line comes from Eq. (75) and the fact that x * is the optimal solution to the problem in Eq. (62).
...
T c(x)P T P (x − x * ), ∇ T d(x − x 0 )P T P (x − x 0 ) and γ(x − x 0 ) T P T P (x − x 0 ) are continuous functions, and therefore ∃B where assumption 5 also holds.
