...
A Markov decision process(MDP) is a tuple, (X , A, R, P, µ), where X is the set of states, A is the set of actions, R : X × A × X → R is the reward function, P :

is the transition probability function(where P (X ′ |X, a) is the probability of transitioning to state X ′ given that the previous state is X and the agent took action a in X), and µ : X →[0, 1] is the starting state distribution.

A policy π : X → P(A) is a map from states to probability distributions over actions, with π(a|X) denoting the probability of choosing action a in state X.

In reinforcement learning, we aim to select a policy π which maximizes

is the expected return by policy π after taking action a in state X.
...
Specifically, in the case that policy π(a|X; θ) is parameterized by θ, A2C uses the following equation to approximate the real policy gradient

where R t = ∑ ∞ i=0 γ i r t+i is the accumulated return from time step t, H(p) is the entropy of distribution p and b(X t ) is a baseline function, which is commonly replaced by V π (X t ).

A2C also includes the minimization of the mean square error between R t and value function V π (X t ).

Thus in practice, the total objective function in A2C can be written as:

where α 1 , α 2 are two coefficients.
...
Under review as a conference paper at ICLR 2020 for every X ∈ X , which appears as the standard cross-entropy loss 1 in supervised learning with a MI-regularizer, β is a coefficient that controls the magnitude of the regularizer.

Next we derive an information bottleneck framework in reinforcement learning.

Just like the label Y in the context of supervised learning as showed in (3), we assume the supervising signal Y in RL to be the accurate value R t of a specific state X t for a fixed policy π, which can be approximated by an n-step bootstrapping function

be the following distribution:

.This assumption is heuristic but reasonable: If we have an input X t and its relative label Y t = R t , we now have X t 's representation Z t , naturally we want to train our decision function V π (Z t ) to approximate the true label Y t .
...
In a network with representation parameter ϕ and policy-value parameter θ, policy lossĴ(Z; θ) in equation (1) and IB loss in (5) can be jointly written as:

where I(X, Z; ϕ) denotes the MI between X and Z ∼ P ϕ (·|X).

Notice that J(Z; θ) itself is a standard loss function in RL as showed in (2).
...
Policy π r = π θ r , parameter ϕ r , optimal policy π ⋆ = π θ ⋆ and its relevant representation parameter ϕ ⋆ are defined as following:
...
In particular, the difference lies in the term L 2 , which depends on the the distribution P ϕ (Z | X) we want to optimize (while in (Liu et al., 2017) , the corresponding term is a fixed prior).
...
, given a fixed policy-value parameter θ, representation distribution P ϕ (Z|X) and state distribution P (X).

Define a new representation distribution:

Though we have derived the optimal target distribution, it is still difficult to compute P ϕ (Z).

In order to resolve this problem, we construct a variational lower bound with a distribution U (Z) which is independent of ϕ. Notice that

.

Now, we can derive a lower bound of L(θ, ϕ) in (6) as follows:
...
via a direction function Φ ⋆ (·) in the unit ball of a reproducing kernel Hilbert space (RKHS) H:

where Φ * (·) is chosen as a direction to maximally decrease 2 the KL divergence between the particles' distribution P (Z) and the target distribution Q(Z) =Q

In fact, Φ * is chosen to maximize the directional derivative of F (P ) = −DKL(P ||Q), which appears to be the "gradient" of F distribution, C is normalized coefficient) in the sense that

where P [ϵΦ] is the distribution of Z + ϵΦ(Z) and P is the distribution of Z. (Liu & Wang, 2016) showed a closed form of this direction:
...
, which is equivalent to maximizeL(θ, ϕ), the greedy direction yields:

In practice we replace log U (Ẑ) with ζ log U (Ẑ) where ζ is a coefficient that controls the magnitude of ∇Ẑ log U (Ẑ).

Notice that Φ(Z i ) is the greedy direction that Z i moves towardŝ L(θ, ϕ)'s target distribution as showed in (13)(distribution that maximizesL(θ, ϕ)).

This means Φ(Z i ) is the gradient ofL(Z i , θ, ϕ):

Since our ultimate purpose is to update ϕ, by the chain rule,

Φ(Z i ) is given in equation(17).

In practice we update the policy-value parameter θ by common policy gradient algorithm since:

and update representation parameter ϕ by (18).
...
and compute their MI with MINE, every time we train MINE(update η) we just shuffle {Z i } n i=1 and roughly assume the shuffled representations {Z
...
In A2C with our framework, we sample Z by a network ϕ(X, ϵ) where ϵ ∼ N (·; 0, 0.1) and the number of samples from each state X is 32, readers are encouraged to take more samples if the computation resources are sufficient.

We set the IB coefficient as β = 0.001.

We choose two prior distributions U (Z) of our framework, the first one is uniform distribution, apparently when U (Z) is the uniform distribution, ∇Ẑ log U (Ẑ) |Ẑ =Z can be omitted.

The second one is a Gaussian distribution, which is defined as follows: for a given state X i , sample a batch of {Z

We also set ζ as 0.005∥∇Ẑ 1 β J(Ẑ; θ)/∇Ẑ log U (Ẑ)∥ |Ẑ =Z to control the magnitude of ∇Ẑ log U (Ẑ) |Ẑ =Z .
...
In summary, we implement the following four algorithms: A2C with uniform SVIB A2C with uniform SVIB A2C with uniform SVIB: Use ϕ(X, ϵ) as the embedding function, optimize by our framework(algorithm(A.4)) with U (Z) being uniform distribution.

A2C with Gaussian SVIB A2C with Gaussian SVIB A2C with Gaussian SVIB: Use ϕ(X, ϵ) as the embedding function, optimize by our framework(algorithm(A.4)) with U (Z) being Gaussian distribution.

A2C A2C A2C:Regular A2C in Openai-baselines with ϕ(X) as the embedding function.

A2C with noise A2C with noise A2C with noise(For fairness):A2C with the same embedding function ϕ(X, ϵ) as A2C with our framework.
...
and d π is the discounted future state distribution, readers can find detailed definition of d π in the appendix of (Chen et al., 2018) .

We can get:

We show the rigorous derivation of the target distribution in (11).

Denote P as the distribution of X, P Z ϕ (Z) = P ϕ (Z) as the distribution of Z. We use P ϕ as the short hand notation for the conditional distribution P ϕ (Z|X).

Moreover, we write

Take the functional derivative with respect to P ϕ of the first term L 1 :
...
A.3 Proof of Theorem 2 (X, Z; ϕ) , given a fixed policy-value parameter θ, representation distribution P ϕ (Z|X) and state distribution P (X), define a new representation distribution:
...
According to the positivity of the KL-divergence, we have L(θ,φ) ≥ L(θ, ϕ).

Algorithm 1 Information-bottleneck-based state abstraction in RL θ, ϕ ← initialize network parameters β, ζ ← initialize hyper-parameters in (17) ϵ ← learning rate M ← number of samples from

A.5 Integrate MINE to our framework MINE can also be applied to the problem of minimizing the MI between Z and X where Z is generated by a neural network P ϕ (·|X):
...
Every 2000 update steps(2560 frames each step), we re-initialize the parameter η, then sample a batch of inputs and their relevant representations

, n = 64, and compute the MI with MINE.

The learning rate of updating η is same as openai-baselines' A2C: 0.0007, training steps is 256 and the network architecture can be found in our code file "policy.py".
...
As policy π gets better, the agent might find new states, in this case, I(X, Z) might increase again because the agent needs to encode information from new states in order to learn a better policy.
