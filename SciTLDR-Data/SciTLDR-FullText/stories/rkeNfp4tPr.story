...
Despite the wide use of stochastic momentum (Algorithm 1) in practice, justification for the clear empirical improvements has remained elusive, as has any mathematical guidelines for actually setting the momentum parameter-it has been observed that large values (e.g. β = 0.9) work well in practice.
...
Step size parameter η and momentum parameter β.

2: Init: w0 ∈ R d and m−1 = 0 ∈ R d .

3: for t = 0 to T do 4:

Given current iterate wt, obtain stochastic gradient gt := ∇f (wt; ξt).
...
Specifically, we consider the stochastic nonconvex optimization problem, min w∈R d f (w) := E ξ∼D [f (w; ξ)], where we overload the notation so that f (w; ξ) represents a stochastic function induced by the randomness ξ while f (w) is the expectation of the stochastic functions.
...
The stochastic momentum m t satisfies Correlated Negative Curvature (CNC) at t with parameter γ > 0 if

As we will show, the recursive dynamics of SGD with heavy ball momentum helps in amplifying the escape signal γ, which allows it to escape saddle points faster.

We show that, under CNC assumption and some minor constraints that upper-bound parameter β, if SGD with momentum has properties called Almost Positively Aligned with Gradient (APAG), Almost Positively Correlated with Gradient (APCG), and Gradient Alignment or Curvature Exploitation (GrACE), defined in the later section, then it takes T = O((1 − β) log(1/(1 − β) ) −10 ) iterations to return an ( , ) second order stationary point.

Alternatively, one can obtain an ( , √ ) second order stationary point in T = O((1 − β) log(1/(1 − β) ) −5 ) iterations.

Our theoretical result demonstrates that a larger momentum parameter β can help in escaping saddle points faster.
...
In an iterative update scheme, at some time t 0 the parameters w t0 can enter a saddle point region, that is a place where Hessian ∇ 2 f (w t0 ) has a non-trivial negative eigenvalue, say λ min (∇ 2 f (w t0 )) ≤ − , and the gradient ∇f (w t0 ) is small in norm, say ∇f (w t0 ) ≤ .
...
On the other hand, if the iterates were to move in one particular direction, namely along v t0 the direction of the smallest eigenvector of ∇ 2 f (w t0 ), then a fast escape is guaranteed under certain constraints on the step size η; see e.g. ).
...
In the present paper we study stochastic momentum, and our CNC property requires that the update direction m t0 is strongly non-orthogonal to v t0 ; more precisely, E t0 [ m t0 , v t0 2 ] ≥ γ > 0.
...
Further, this effect is amplified in successive iterations through the momentum update when β is close to 1.

Assume that at some w t0 we have m t0 which possesses significant correlation with the negative curvature direction v t0 , then on successive rounds m t0+1 is quite close to βm t0 , m t0+2 is quite close to β 2 m t0 , and so forth; see Figure 1 for an example.
...
We answer the first question in the affirmative, and we answer the second question essentially by showing that momentum can help speed up saddle-point escape by a multiplicative factor of 1 − β.

On the negative side, we also show that β is constrained and may not be chosen arbitrarily close to 1.
...
All the algorithms use the same step size η = 5 × 10 −5 .
...
All the algorithms are initialized at the same point w0 ∼ N (0, I d /(10000d)) and use the same step size η = 5 × 10 −4 .
...
The second objective (4) appears in the phase retrieval problem, that has real applications in physical sciences (Candés et al. (2013); Shechtman et al. (2015) ).
...
Here we have sampled The empirical findings, displayed in Figure 2 , are quite stark: for both objectives, convergence is significantly accelerated by larger choices of β.
...
Further, we assume that the Hessian ∇ 2 f is ρ-Lipschitz.

These two properties ensure that ∇f

w−w 3 , ∀w, w .

Furthermore, we assume that the stochastic gradient has bounded noise ∇f (w) − ∇f (w; ξ) 2 ≤ σ 2 and that the norm of stochastic momentum is bounded so that m t ≤ c m .

We denote Π i M i as the matrix product of matrices {M i } and we use
...
We say that SGD with stochastic momentum satisfies Almost Positively Correlated with Gradient (APCG) with parameter τ if ∃c > 0 such that,

where the PSD matrix M t is defined as

for any integer 1 ≤ k ≤ τ − 1, and η is any step size chosen that guarantees each G s,t is PSD.
...
For (f), we let Mt = (Π 500 s=1 Gs,t)(Π 500 s=1 Gs,t) and we observe that the value is almost always nonnegative.
...
negative curvature, ∇f (w) ≤ and λ min (∇ 2 f (w)) ≤ − .
...
Our proof is structured into three cases: either (a) ∇f (w) ≥ , or (b) ∇f (w) ≤ and λ min (∇ 2 f (w)) ≤ − , or otherwise (c) ∇f (w) ≤ and λ min (∇ 2 f (w)) ≥ − , meaning we have arrived in a second-order stationary region.
...
Step size parameters r and η, momentum parameter β, and period parameter T thred .

2: Init:

Get stochastic gradient gt at wt, and set stochastic momentum mt := βmt−1 + gt.

Set learning rate:η := η unless (t mod T thred ) = 0 in which caseη := r 6: wt+1 = wt −ηmt.
...
If SGD with momentum (Algorithm 2) has APAG property when gradient is large ( ∇f (w) ≥ ), APCG T thred property when it enters a region of saddle points that exhibits a negative curvature ( ∇f (w) ≤ and λ min (∇ 2 f (w)) ≤ − ), and GrACE property throughout the iterations, then it reaches an ( , ) second order stationary point in

) iterations with high probability 1 − δ.

The theorem implies the advantage of using stochastic momentum for SGD.

Higher β leads to reaching a second order stationary point faster.

As we will show in the following, this is due to that higher β enables escaping the saddle points faster.
...
Remark 1: (constraints on β) We also need some minor constraints on β so that β cannot be too close to 1.

They are 1) On the other hand, for CNC-SGD, based on Table 3 in their paper, is T thred =Õ 1 η .

One can clearly see that T thred of our result has a dependency 1 − β, which makes it smaller than that of Daneshmand et al. (2018) for any same η and consequently demonstrates escaping saddle point faster with momentum.
...
Empirically, we find out that c ≈ 0 (Figure 3 ) and c h ≈ 0 (Figure 4 ) in the phase retrieval problem, so the condition is easily satisfied for a wide range of β.
...
Suppose that it enters the region exhibiting a small gradient but a large negative eigenvalue of the Hessian (i.e. ∇f (w t0 ) ≤ and λ min (∇ 2 f (w t0 )) ≤ − ).
...
6 ), the dependency on β suggests that larger β can leads to smaller T thred , which implies that larger momentum helps in escaping saddle points faster.
...
The proof of Lemma 2 is in Appendix D. Furthermore, we will use the quantities q v,t−1 , q m,t−1 , q q,t−1 , q w,t−1 , q ξ,t−1 as defined above throughout the analysis.
...
We know that λ min (H) ≤ − < 0.

Let v be the eigenvector of the Hessian H with unit norm that corresponds to λ min (H) so that Hv = λ min (H)v.
...
Observe that the lower bound in (8) is monotone increasing with t and the momentum parameter β.
...
Let F thred = O( 4 ) and η 2 T thred ≤ r 2 .
...
In this paper, we identify three properties that guarantee SGD with momentum in reaching a secondorder stationary point faster by a higher momentum, which justifies the practice of using a large value of momentum parameter β.
...
If SGD with momentum has the APAG property, then, considering the update step w t+1 = w t − ηm t , we have that
...
Assume that the step size η satisfies η ≤ 2 8Lc 2 m .

If SGD with momentum has the APAG property, then, considering the update step w t+1 = w t − ηm t , we have that

2 , where the last inequality is due to the constraint of η.

Lemma 8.

If SGD with momentum has the GrACE property, then, considering the update step w t+1 = w t − ηm t , we have that

Proof.

Consider the update rule w t+1 = w t − ηm t , where m t represents the stochastic momentum and η is the step size.

By ρ-Lipschitzness of Hessian, we have
...
Recall that the update is w t0+1 = w t0 − rm t0 , and w t0+t = w t0+t−1 − ηm t0+t−1 , for t > 1.
...
s=1 α s and let us rewrite g t = ∇f (w t ) + ξ t , where ξ t is the zero-mean noise.

We have that

To proceed, we need to upper bound E t0 [4η

We have that

where (a) is because E t0 [ξ t0+i ξ t0+j ] = 0 for i = j, (b) is by that ξ t 2 ≤ σ 2 and max t α t ≤ 1 1−β .
...
By using ρ-Lipschitzness of Hessian, we have that

By adding η ∇f (w t0+s−1 ), g t0+s−1 on both sides, we have η ∇f (w t0+s−1 ), g t0+s−1 ≤ f (w t0+s−1 ) − f (w t0+s ) + η ∇f (w t0+s−1 ), g t0+s−1 − m t0+s−1
...
• qv,t−1 := Π t−1 j=1 Gj − rmt 0 .

• qm,t−1 := − t−1 s=1 Π t−1 j=s+1 Gj β s mt 0 .
...
Then, w t0+t − w t0 = q v,t−1 + ηq m,t−1 + ηq q,t−1 + ηq w,t−1 + ηq ξ,t−1 .
...
where (a) we use the notation that Π t−1

and the notation that Π t−1 j=t G j = 1 and (b) is by the update rule.
...
Lemma 5 Let F thred = O( 4 ) and η 2 T thred ≤ r 2 .
...
) from (82) W.l.o.g, we assume that c m , L, σ 2 , c , c h , and ρ are not less than one and that ≤ 1.
...
The constraints upper-bound the value of β.

That is, β cannot be too close to 1.

We note that the β dependence on L, σ, and c are only artificial.
...
Similarly, the dependence on σ is not highly relevant, since we can always increase the variance of the stochastic gradient, for example by adding an O(1) gaussian perturbation.
...
where (a) is by the assumption of L-Lipschitz gradient and (b) is by applying the triangle inequality (s − k) times and that w t − w t−1 ≤ η m t−1 ≤ ηc m , for any t. We can also derive an upper bound of E t0 [ ∇f (w t0+s ) − ∇Q(w t0+s ) ],
...
To continue, let us analyze Π t−1 j=s+1 G j 2 first.

Above, we use the notation that θ j := j k=1 β j−k .

For (a), it is due to that λ := −λ min (H), λ max (H) ≤ L, and the choice of η so that 1 ≥ ηL 1−β , or equivalently,

For (b), it is due to that θ j ≥ 1 for any j and λ ≥ .
...
(1−z) 2 for any |z| ≤ 1 and substituting z = 1 1+η , which leads to

in which the last inequality is by chosen the step size η so that η ≤ 1.

By combining (42), (46), and (47), we have that

Proof.

where the last inequality is because η is chosen so that 1 ≥ ηL 1−β and the fact that λ max (H) ≤ L.

Lower bounding E t0 [2η q v,t−1 , q q,t−1 ]:
...
Lower bounding E t0 [2η q v,t−1 , q ξ,t−1 ]:
...
where (a) holds for some coefficients α k , (b) is by the tower rule, (c) is because q v,t−1 is measureable with t 0 , and (d) is by the zero mean assumption of ξ's.

Lower bounding E t0 [2η q v,t−1 , q m,t−1 ]:
...
To see that the matrix B is symmetric positive semidefinite, observe that each G j := (I − η j k=1 β j−k H) can be written in the form of G j = U D j U for some orthonormal matrix U and a diagonal matrix D j .
...
Lower bounding 2ηE t0 [ q v,t−1 , q w,t−1 ]: Lemma 14.
...
Yet, by Lemma 13 and Lemma 12, we have that ηE t0 [ q v,T thred −1 , q m,T thred −1 ]

≥ 0 and ηE t0 [ q v,T thred −1 , q ξ,T thred −1 ] = 0.

So, it suffices to prove that

and it suffices to show that Under review as a conference paper at ICLR 2020 as guaranteed by the constraint of η.

So,

where (a) is by using the inequality log(1 + x) ≥ x 2 with x = ηθ j ≤ 1 and (b) is by making
...
We can bound E[f (w (k+1)T thred ) − f (w kT thred )|Υ k ]
...
For the other condition, E[f (w (k+1)T thred ) − f (w kT thred )|Υ , it requires that the expected amortized increase of function value due to taking the large step size r is limited (i.e. bounded by δ ) when w kT thred is a second order stationary point.
...
So, let us consider the events in Lemma 15, Υ k := { ∇f (w kT thred ) ≥ or λ min (∇ 2 f (w kT thred )) ≤ − }.

We first show that E[f (w (k+1)T thred ) − f (w kT thred )|Υ k ] ≤ F thred .

When ∇f (w kT thred ) ≥ :

Now we are ready to use Lemma 15, since both the conditions are satisfied.

According to the lemma and the choices of parameters value on Table 3 , we can set T = 2T thred f (w 0 ) − min w f (w) /(δF thred ) = O((1 − β) log( Lcmσ 2 ρc c h

(1−β)δγ ) −10 ), which will return a w that is an ( , ) second order stationary point.

Thus, we have completed the proof.

(1−β)δγ ) −10 ) for Algorithm 2.

Before making a comparison, we note that their result does not have a dependency on the variance of stochastic gradient (i.e. σ 2 ), which is because they assume that the variance is also bounded by the constant (can be seen from (86) , Algorithm 2 is strictly better than that of Daneshmand et al. (2018) , which means that a higher momentum can help to find a second order stationary point faster.
