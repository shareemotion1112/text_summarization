...
We choose α(a, DISPLAYFORM1 , where w a ∈ R 3d is a trainable weight, • is element-wise multiplication, [;] is vector concatenation across row, and the implicit multiplication is matrix multiplication.
...
Interaction Layer: The interaction layer models the interaction between premise encoded representation P enc and hypothesis encoded representation H enc as follows: DISPLAYFORM7 whereP i is the i-th row vector ofP , andH j is the j-th row vector ofH. Though there are many implementations of interaction, we find β(a, b) = a • b very useful.
...
Once we have the interaction tensor I, we use a convolution with 1 × 1 kernel to scale down the tensor in a ratio, η, without following ReLU.

If the input channel is k then the output channel is f loor(k × η).
...
The transition scale down ratio in transition layer is θ.
...
An Adadelta optimizer(Zeiler, 2012) with ρ as 0.95 and as 1e−8 is used to optimize all the trainable weights.
...
The first scale down ratio η in feature extraction layer is set to 0.3 and transitional scale down ratio θ is set to 0.5.
