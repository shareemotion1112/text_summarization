...
The next-step prediction task can be succinctly formalized as follows: given a sequence {x t−τ +1 , · · · , x t } of frames of an audio waveform, predict the next value x t+1 in the sequence.
...
The head takes in τ frames of data from the trunk, where τ is the trunk's effective receptive field, and produces an output which represents the model's prediction for the next frame of audio in the sequence.

The next-step head treats this as a regression problem, using the mean squared error of the difference between predicted values and actual values as a loss function, i.e. given inputs {x t−τ +1 , · · · , x t }, the head produces an output y t from which we compute a loss L MSE (t) = (y t − x t+1 ) 2 and then aggregate over the frames to get the total loss.

We would like to note that the original WaveNet implementation treated next-step prediction as a classification problem, instead predicting the bin-index of the audio following a µ-law transform.
...
In defining the noise reduction task, we adopt the common approach of treating noise as an additive random process on top of the true signal: if {x t } denotes the clean raw audio waveform, we obtain the noisy version viax t := x t + ξ t where ξ t an arbitrary noise process.

For the denoising task, the model is trained to predict the clean sample, x t , given a window x t− 1 2 (τ −1) , · · · ,x t+ 1 2 (τ −1) of noisy samples.
...
It is trained to minimize a smoothed L1 loss between the clean and noisy versions of the waveform inputs, i.e. for each frame t, the head produces an outputŷ t , and we compute the loss DISPLAYFORM0 and then aggregate over frames to obtain the total loss.
...
L total = i α i L i , where the sum runs over all the tasks.
...
We used the "Adam" optimizer BID6 with parameters β 0 = 0.9, β 1 = 0.99 , ε = 10 −8 .
