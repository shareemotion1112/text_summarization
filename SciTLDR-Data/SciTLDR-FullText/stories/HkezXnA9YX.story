...
The FiLM layer is conditioned on the question at hand via prediction of the scaling and shifting parameters γ n and β n : DISPLAYFORM2 where BN stands for batch normalization BID15 , * stands for convolution and stands for element-wise multiplications.
...
Formally, our NMN is constructed by repeatedly applying a generic neural module f (θ, γ, s 0 , s 1 ), which takes as inputs the shared parameters θ, the question-specific parametrization γ and the lefthand side and right-hand side inputs s 0 and s 1 .
...
We refer to A = (α k,i ) and T = (τ k,j m ) as the parametrization attention matrix and the layout tensor respectively.
...
Equations 10 and 13 should be understood as taking vectors γ k and θ respectively and chunking them into weights and biases.

The main difference between Residual and Find is that in Residual all parameters depend on the questions words (hence θ is omitted from the signature of f Residual ), where as in Find convolutional weights are the same for all questions, and only the element-wise multipliers γ k vary based on the question.
...
Modules 1, 2 and 3 are parametrized based on the first object word, second object word and the relation word respectively, which is achieved by setting the attention maps α 1 , α 2 , α 3 to the corresponding one-hot vectors.
...
It is structured just like NMN-Tree but has α k computed as softmax(α k ), whereα k is a trainable vector.
...
Notably, there was very little agreement between predictions of these 5 runs (Fleiss κ = 0.05), suggesting that NMN-Chain performs rather randomly outside of the training set.
...
We experiment with N2NMNs using both Error rate (%) 1 rhs/lhs 2 rhs/lhs 18 rhs/lhs Figure 5 : Attention quality κ vs accuracy for Attention N2NMN models trained on different #rhs/lhs splits.

We can observe that generalization is strongly associated with high κ for #rhs/lhs=1, while for splits with 2 and 18 rhs/lhs blurry attention may be sufficient.
...
In these experiments, we fix the layout to be tree-like and sample the pre-softmax attention weightsα from a uniform distribution U [0; 1].
...
To better understand the relationship between successful layout induction and generalization, we define an attention quality metric κ = min w∈{X,Y } max k∈1,2 α k,w /(1 − α k,R ).

Intuitively, κ is large when for each word w ∈ X, Y there is a module i that focuses mostly on this word.

The renormalization by 1/(1 − α k,R ) is necessary to factor out the amount of attention that modules 1 and 2 assign to the relation word.

For the ground-truth parametrization that we use for NMN-Tree κ takes a value of 1, and if both modules 1 and 2 focus on X, completely ignoring Y, κ equals 0.

The scatterplot of the test error rate versus κ ( Figure 5 ) shows that for #rhs/lhs=1 high generalization is strongly associated with higher κ, meaning that it is indeed necessary to have different modules strongly focusing on different object words in order to generalize in this most challenging setting.

Interestingly, for #rhs/lhs=2 we see a lot of cases where N2NMN generalizes well despite attention being rather spurious (κ ≈ 0.6).In order to put Attention N2NMN results in context we compare them to those of MAC (see TAB1 ).
...
We can observe a clear correlation between κ and error rate for 1, 2 and 4 rhs/lhs.

Also note that perfect generalization is always associated with κ close to 1.Next, we experiment with a hard-coded variation of MAC.
