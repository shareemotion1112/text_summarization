...
Antolík and colleagues pioneered the idea of learning a shared nonlinear feature representation for a large population of neurons BID1 , which others have also used in both retina and V1 BID3 BID14 BID4 .
...
A function f : X → Y is called equivariant with respect to a group of transformations Π if such transformations of the input lead to predictable transformations of the output.

Formally, for every π ∈ Π there is a transformation ψ ∈ Ψ such that for every DISPLAYFORM0 CNNs are shift-equivariant by construction, meaning that every translation of the image leads to a matching translation in the feature maps (i. e. π and ψ are both translation by a number of pixels horizontally and vertically).
...
Formally, if π is a rotation of the image by, say, 45• , then ψ is a rotation of the feature maps by also 45• combined with a cyclic permutation of the feature maps by one rotation step.
