...
whereẼ is the transition probability matrix by normalizing columns of E i , c ∈ [0, 1] is a tradeoff parameter between random walk and restart, and e i is a vector of all zeros except the entry corresponding to the center node i.
...
Here α(·) and β(·) are transfer functions (such as Sigmoid) that adjust feature similarity and structure interaction scores before combining them.

For simplicity (and in our experiments), we use scalar α and β, which leads to a standard weighted average.
...
Adam SGD is used for optimization, with learning rate λ = 5 × 1e − 4.
