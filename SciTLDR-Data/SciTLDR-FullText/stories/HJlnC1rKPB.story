...
In this manner, the attention scores only depend on the shift δ := k − q. Above, the learnable vectors u and v are unique for each head, whereas for every shift δ the relative positional encoding r δ ∈ R Dp is shared by all layers and heads.
...
2 ) and α (h) determine the center and width of attention of each head, respectively.

On the other hand, δ = (δ 1 , δ 2 ) is fixed and expresses the relative shift between query and key pixels.
...
There exists a relative encoding scheme {r δ ∈ R Dp } δ∈Z 2 with D p ≥ 3 and parame-
...
Setting W key ∈ R D k ×Dp to the identity matrix (with appropriate row padding), yields A q,k = v r δ where δ := k − q. Above, we have assumed that D p ≤ D k such that no information from r δ is lost.

Now, suppose that we could write:

for some constant c. In the above expression, the maximum attention score over A q,: is −αc and it is reached for A q,k with δ = ∆. On the other hand, the α coefficient can be used to scale arbitrarily the difference between A q,∆ and the other attention scores.

In this way, for δ = ∆, we have

and for δ = ∆, the equation becomes lim α→∞ softmax(A q,: ) k = 0, exactly as needed to satisfy the lemma statement.

What remains is to prove that there exist v and {r δ } δ∈Z 2 for which eq. (14) holds.
...
The exact representation of one pixel requires α (or the matrices W qry and W key ) to be arbitrary large, despite the fact that the attention probabilities of all other pixels converge exponentially to 0 as α grows.

Nevertheless, practical implementations always rely on finite precision arithmetic for which a constant α suffices to satisfy our construction.

For instance, since the smallest positive float32 scalar is approximately 10 −45 , setting α = 46 would suffice to obtain hard attention.
...
Hence, the relative positional encoding of a key pixel at position k with a query pixel at position q is the concatenation of the row shift embedding δ 1 and the column shift embedding δ 2 (where δ = k − q).
...
Another interesting line of research has demonstrated that transformers are Turingcomplete (Dehghani et al., 2018; Pérez et al., 2019) , which is an important theoretical result but is not informative for practitioners.
...
Each head is parametrized by a center of attention ∆ and a covariance matrix Σ to obtain the following attention scores,

where, once more, δ = k − q. The last term can be discarded because the softmax is shift invariant and we rewrite the attention coefficient as a dot product between the head target vector v and the relative position encoding r δ (consisting of the first and second order combinations of the shift in pixels δ):
...
Each head was parametrized by ∆ ∈ R 2 and Σ −1/2 ∈ R 2×2 to ensure that the covariance matrix remained positive semi-definite.

We initialized the center of attention to ∆ (h) ∼ N (0, 2I 2 ) and Σ −1/2 = I 2 + N (0, 0.01I 2 ) so that initial attention probabilities were close to an isotropic Gaussian.
...
Some non-isotropic attention heads attend on "non-intuitive" patches of pixels: either attending a very thin stripe of pixels, when Σ −1 was almost singular, or attending all pixels uniformly, when Σ −1 was close to 0 (i.e. constant attention scores).
