...
We refer to the modified operationỹ as the matched version of y, with respect to the cost c and prior distribution p z .For completeness, we introduce the key concept of optimal transport theory in a simplified setting, i.e. assuming probability distributions are in euclidean space and skipping measure theoretical formalism.
...
Suppose z has components Z ∼ N (0, σ 2 ).

In this case, we can compute linear interpolation as before, y t = tz 1 + (1 − t)z 2 .

Since the sum of Gaussians is Gaussian, we get, Y t ∼ N (0, t 2 σ 2 + (1 − t) 2 σ 2 ).

Now, it is easy to see that with a proper scaling factor, we can adjust the variance of Y t back to σ 2 .
