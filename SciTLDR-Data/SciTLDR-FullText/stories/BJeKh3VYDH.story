...
Our goal is to let the network f s (X) infer a functionỸ which approximates a desired super-resolution output point cloud Y = {y 1 , y 2 , ..., y n } of size n ∈ [1, n max ] with y i ∈ R 3 , i.e. f s (X) =Ỹ ≈ Y .
...
Following Achlioptas et al. (2017) , we base our spatial loss L S on the Earth Mover's Distance (EMD), which solves an assignment problem to obtain a differentiable bijective mapping φ :ỹ →

y.

With φ we can minimize differences in position for arbitrary orderings of the points clouds via:

Figure 2: a) Schematic overview of fs(X).

Black arrows represent scalar data.

Point data is depicted as colored arrows with the color indicating data cardinality (brown=k, red = kmax, green = nmax, blue = n, and purple =ñ).
...
By using the EMD-based mapping φ established for the spatial loss in Equation (1), we can formulate the temporal constraint in a permutation invariant manner as
...
The entries of p allow us to compute the mask on the fly throughout the whole network, without having to pass through k. For an input of size k, our network has the task to generateñ = rk points.

As the size of the network output is constant with rk max , the outputs are likewise masked with M out to truncate it to lengthñ for all loss calculations, e.g., the EMD mappings.

Thus, as shown in Figure 2a ,ñ is used to truncate the point cloudȲ = {ȳ 1 ,ȳ 2 , ...,ȳ nmax } via a mask M out to form the final output
...
As such,ñ only provides an approximation of the true number of target points in the ground truth data.
...
In general,ñ provides a good approximation for our data sets.

However, as there is a chance to infer an improved estimate of the correct output size based on the input points, we have experimented with training a second network to predictñ in conjunction with a differentiable M out .

While this could be an interesting feature for future applications, we have not found it to significantly improve results.

As such, the evaluations and results below will use the analytic calculation, i.e.,ñ = rk.

For each input point the network generates r output points, which can be seen as individual groups g: ψ(g) = {Ỹ i |i ∈ [rg + 1, (r + 1)g]}. These groups of size r in the output are strongly related to the input points they originate from.
...
LN = ñ − n 2 2 is given as a measure of accuracy in terms of the size of the generated outputs (it is not part of the training).
...
In combination with the spatial and temporal terms from above, this leads to our final loss function L f inal = L S + γL EV + µL EA + νL M , with weighting terms γ, µ, ν.
...
For this we mapped the generated point cloudsỸ t = {y t 1 , y t 2 , ..., y t n } for 100 frames to evenly and dense sampled ground-truth points on the original mesh Y t = {y t 1 , y t 2 , ..., y t n } (the moving man shown in Fig. 1 ).
...
γ :ỹ → y. Using γ we divideỸ t into n subsetsŶ i = {ỹ j |γ(ỹ j ) = y i } which correlate with the corresponding ground-truth points.

For each subset we can now compute the mean position 1 |Ŷi| ŷ∈Ŷiŷ and the sample density |Ŷ i | measured by the number of points assigned to a ground-truth sample position.
...
For the loss, we used γ = 10, µ = 10, and ν = 0.001.
...
The loss parameters were γ = µ = 5, with ν = 0.001.
