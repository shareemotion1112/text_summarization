...
t = 0 corresponds to mode G for all plots.w a and w b by φ θ (t) : [0, 1] → R D , such that φ θ (0) = w a and φ θ (1) = w b .

To find a low loss path, we find the set of parameters θ ∈ R D that minimizes the following loss: DISPLAYFORM0 where U (0, 1) is the uniform distribution in the interval [0, 1].

To optimize (θ) for θ, we first need to chose a parametric form for φ θ (t).

One of the forms proposed by is a polygonal chain with a single bend at θ as follows DISPLAYFORM1 To minimize (θ), we sample t ∼ U [0, 1] at each iteration and use ∇ θ L(φ θ (t)) as an unbiased estimate for the true gradient ∇ θ (θ) to perform updates on θ, where θ is initialized with 1 2 (w a +w b ).
...
FIG0 shows the validation accuracy for models on each of the 6 connecting curves during the 20th, 40th, 60th and 80th epochs of the mode connectivity training procedure and also for models on the line segment joining the two endpoints (corresponding to the initialization for θ at epoch 0).

As described in Section 2.1, for a polychain curve GX (connecting modes G and X using the curve described by θ), model parameters φ θ (t) on the curve are given by p φ θ (t) = 2(tp θ + (0.5 − t)p G ) if 0 ≤ t ≤ 0.5 and p φ θ (t) = 2((t − 0.5)p X + (1 − t)p θ ) if 0.5 < t ≤ 1 where p G , p θ and p X are parameters of the models G, θ, and X respectively.

Thus φ θ (0) = G and φ θ (1) = X.In a few epochs of the curve training, for all 6 pairs, we can find a curve such that each point on it generalizes almost as well as models from the pair that is being connected.
...
The subsequent CCA step involves transforming L a and L b to a 1 L a and b 1 L b respectively where {a 1 , b 1 } is found by maximizing the correlation between the transformed subspaces, and the corresponding correlation is denoted by ρ 1 .

This process continues, using orthogonality constraints, till c = min{m a , m b } leading to the set of correlation values {ρ 1 , ρ 2 . . .

ρ c } corresponding to c pairs of canonical variables {{a 1 , b 1 }, {a 2 , b 2 }, . . .

{a c , b c }} respectively.

We refer the reader to Raghu et al. (2017) for details on solving these optimization problems.

The average of these c correlations 1 n i ρ i is then considered as a measure of the similarity between the two layers.
...
The learning rate at the t-th epoch in SGDR is given by the following expression in (1) where η min and η max are the lower and upper bounds respectively for the learning rate.
...
For our experiments, we choose T 0 = 10 epochs and T mult = 2 (warm restarts simulated every 10 epochs and the period T i doubled at every new warm restart), η max = 0.05 and η min = 10 −6 .

We also perform VGG training using SGD (with momentum of 0.9) and a step decay learning rate scheme (initial learning rate of η 0 = 0.05, scaled by 5 at epochs 60 and 150).
...
We use t-SNE BID27 to visualize these 7 modes and the θ points that define the connectivity for the 6 pairs presented in Section 2.1.1, in a 2-dimensional plot in FIG0 .
...
The W n in FIG3 is equivalent to meaning it is the point on the plane (linear combination of w 70 , w 150 and θ) with the least l-2 distance from the original point (iterate in this case).
