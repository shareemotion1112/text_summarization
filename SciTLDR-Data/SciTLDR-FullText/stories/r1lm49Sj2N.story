...
As stated, the ground truth inference problem for sparsely correlated regressors was solved in BID1 by using a compressed sensing approach to recover the R(R + 1)/2 moments, δ r1 δ r2 , for unbiased (δ r ≈ 0) regressors.
...
Consider two regressors giving estimates, DISPLAYFORM0 By the Ground Truth Postulate, these can be subtracted to obtain,ŷ DISPLAYFORM1 Note that the left-hand side involves observable values that do not require any knowledge of y i,true .

The right hand side contains the error quantities that we seek to estimate.

Squaring both sides and averaging over all the datapoints in the dataset we obtain our primary equation, DISPLAYFORM2 Since we are assuming that the regressors are independent in their errors (δ r1 δ r2 ≈ 0), we can simplify 7 to, DISPLAYFORM3 This is obviously unsolvable with a single pair of regressors.
...
A regressor may put out a signalŷ i,r , but its error signal δ i,r could be completely different.
...
Let us demonstrate that by using eq 6 to estimate the average bias, δ r , for the regressors.
...
Recovered square error moments (circles), δr 1 δr 2 , for the true error moments (squares) of 10 synthetic regressors on the pixels of a 1024x1024 image.
