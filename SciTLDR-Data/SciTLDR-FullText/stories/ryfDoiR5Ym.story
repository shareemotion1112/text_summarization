...
The final loss term is defined as follows: DISPLAYFORM1 where φ(·) s,t means the feature map taken after t-th convolution (after activation) before s-th maxpooling layer in the VGG19 network 2 , γ is the maximum margin in the hinge-loss based regularization.

We allow the modification up to γ.

Note that the hinge-loss based regularization does not incur any loss up to γ.
...
In our case, we add the hinge-loss to control the modification of the input sample x. If γ is large, more modifications are allowed and as a result, our watermark signals will be more robust.

However, the modified sample can be very different from the original sample in this case, which is not a desired result.

Therefore, γ should be adjusted very carefully.
...
By varying the number of convolution layers in the decoder and the margin γ, we repeat the experiment.
...
Our method (decoder size = 3 and γ = 0.01) marks the best decoding success rate, i.e., 100% in our method vs. 95.5% in the method of BID9 vs. 89.3% in the statistical watermarking method.

Other configurations in our method also outperform all the baseline methods.

Our method Decoder Size = 1 Decoder Size = 2 Decoder Size = 3 γ = 0.01 γ = 0.05 γ = 0.1 γ = 0.01 γ = 0.05 γ = 0.1 γ = 0.01 γ = 0.05 γ = 0.
...
Note that there are more modifications on the color tone of images as γ increases.
...
With the original images, the FR network's accuracy is 0.795576 and after watermarking them with γ = 0.01 and the decoder with 3 convolutions, it becomes 0.797864.
...
We report the decoding success rate for non-watermarked/watermarked cases with our method after varying the convolution numbers in the decoder (i.e. decoder size) and γ.

Our method Decoder size = 1 Decoder size = 3 γ = 0.01 γ = 0.05 γ = 0.1 γ = 0.01 γ = 0.05 γ = 0.1 81.2%/100.0% 89.2%/100.0% 92.0%/100.0% 99.0%/100.0% 98.0%/99.4% 99.5%/100.0% A ADDITIONAL EXPERIMENT RESULTS
...
With γ = 0.01, modifications are very limited.
...
In the decoder size of 3, the decoding success rates are very high for all γ configurations.

When there is only one convolution in the decoder, the decoding success rate is proportional to γ.
