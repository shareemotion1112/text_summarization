...
θ 1 is 1 We understand that human being's brain system is much more complex than what we demonstrated in this work and the flow in the brain is not one-directional BID27 BID2 .
...
loss1 and loss2 will be introduced in section 3.2.used to denote the parameters from FC 1 to FC 3 and θ 2 indicates the parameters in FC 4.

For the Shallow model, we trained all parameters from scratch.

We added EEG to the model because we first want to find a function f (χ) → s that maps the images space χ to the corresponding single trial P300 source signal s.
...
The training of the models without using EEG is straightforward, models were trained directly to minimize loss 2 (θ 1 , θ 2 ) by feeding images and the corresponding single trial P300 amplitude.

Training with EEG information is explained in Algorithm 1 and visualized in the "Flow 2" of Fig. 2 with two stages.

Stage 1 learns parameters θ 1 to predict P300 source signal while stage 2 learns parameters θ 2 to predict single trial P300 amplitude with θ 1 fixed.

Update θ 2 by descending its stochastic gradient: and without EEG.
