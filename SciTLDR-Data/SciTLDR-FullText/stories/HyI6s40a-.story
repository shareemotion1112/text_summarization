...
The normalization layer maps the latent feature variables, φ (x), into the Euclidean space such that the acquired data embeddings live in a d-dimensional hypersphere, i.e., φ (x) 2 = 1.
...
To do so, we optimize the defender module by incorporating the following loss function with the conventional cross entropy loss: DISPLAYFORM0 Here, γ is a trade-off parameter that specifies the contribution of the additive loss term, φ (x) is the corresponding feature vector of input sample x at the checkpoint location, y * is the ground-truth label, and C i denotes the center of all data abstractions (φ (x)) corresponding to class i. The center values C i and intermediate feature vectors φ (x) are trainable variables that are learned by fine-tuning the defender module.

In our experiments, we set the parameter γ to 0.01 and retrain the defender model with the same optimizer used for training the victim model.
...
The first term (loss 1 ) in Eq. (1) aims to condense latent data features φ (x) that belong to the same class.
...
Under the Gaussian distribution and our specific assumptions, the conditional covariance matrix contains two different eigenvalues a i > b i to be determined as shown in BID0 ).The learned pdf variables (i.e., mean and conditional covariance) are used to compute the probability of a feature point φ (x) coming from a specific class.

In particular, for each incoming test sample x, the probability p(φ (x)|y i ) is evaluated where y i is the predicted class (output of the victim neural network) and φ (x) is the corresponding data abstraction at the checkpoint location.
...
If we define η i j = η(α i |W j ) as the misclassification penalty 5 incurred for deciding W i when the true state is W j , the conditional risk in each of our checkpointing modules is equal to: DISPLAYFORM0 The fundamental rule to express the minimum-risk decision is to decide W 1 if R(α 1 |φ (x)) < R(α 2 |φ (x)).

In terms of the posterior probabilities, we decide W 1 if: DISPLAYFORM1 Generally speaking, the penalty incurred for making an error is greater than the cost incurred for being correct; thus both of the terms η 21 − η 11 and η 12 − η 22 are positive.

Following the Bayes' rule, we should select a sample as a legitimate one (W 1 ) if: DISPLAYFORM2 and select W 2 otherwise.

By reordering the aforementioned decision criteria we have: DISPLAYFORM3 Note that the right-hand term in Eq. FORMULA6 is application specific and is independent of the input data observation φ (x).
...
This term is tightly correlated with the user-defined cut-off threshold (security parameter) depicted in FIG2 .Under the GMM assumption, the conditional probability P(φ (x)|W 1 ) in Eq. (5) is computed as: DISPLAYFORM4 where y i is the output of the victim neural network (predicted class), µ i and Σ i are the output of the HDDA analysis, and N is the dimension of the latent feature space in the checkpoint module.
...
In this experiment, we have considered the FGS attack with different ε values on the MNIST benchmark.
...
In this experiment, the FGS attack is used to generate adversarial samples and the perturbation is adjusted by changing its specific attack parameter ε.ε > 0.25) while the latent defender module is sufficient to effectively distinguish malicious samples even with very small perturbations.
...
We visually evaluate the perturbed examples to determine the attack parameters (e.g., perturbation level ε and n iters ) such that the perturbations cannot be recognized by a human observer.
...
The FGS method ) is characterized with a single ε parameter.

The JSMA attack ) has two parameters: γ specifies the maximum percentage of perturbed features and θ denotes the value added to each selected feature.
