...
Removal of duplicate inter-object edges by computing a scaling factor for each vertex and scalingÃ accordingly.

Ã isÃ with self-loops already added back in.
...
Intra-object edges are removed by elementwise multiplying ( ) the distance matrix with the attention matrix ( Figure 2 ).Ã DISPLAYFORM1 A no longer has self-loops, so we need to add them back in at a later point to still satisfy |E| = |V | 2 .
...
Keep in mind thatÃ has no self-loops nor edges between proposals of the same object.

As a consequence, two nonzero rows inÃ are the same if and only if the proposals are the same.
...
We define a differentiable similarity between proposals i and j as DISPLAYFORM0 where X = f 4 (A) f 5 (D) is the same asÃ except with different activation functions.
...
, s n ]T is Θ(n 3 ) as there are n 2 pairs of rows and Θ(n) operations to compute the similarity of any pair of rows.
...
Recall that the diagonal that was removed when going from A toÃ contains the entries f 1 (a a).
...
Samples from this dataset are given in Appendix DThe classification task is to predict an integer countĉ of true objects, uniformly drawn from 0 to 10 inclusive, from a set of bounding boxes and the associated attention weights.
...
l is used to control the overlapping of bounding boxes: a larger l leads to the fixed number of objects to be more tightly packed, increasing the chance of overlaps.ĉ number of these boxes are randomly chosen to be true bounding boxes.
