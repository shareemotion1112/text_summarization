...
Next, a single layer perceptron is applied on each node separately and the weights W (t) are shared across all the nodes: DISPLAYFORM2 where W (t) ∈ R d h t+1 ×d h t is the weight matrix and σ(·) is an entry-wise activation function.
...
GCN is a special case of GNN which stacks two layers of specific propagation and perceptron: DISPLAYFORM3 with a choice of P =D −1/2ÃD−1/2 , whereÃ = A+I, I is the identity matrix,D = diag(Ã1) and 1 is the all-ones vector.
...
To better understand GCN, we remove the intermediate nonlinear activation units from GCN, which gives Graph Linear Network defined as DISPLAYFORM6 with the same choice of P =D −1/2ÃD−1/2 as in GCN.
...
AGNN is simple; it only has a single scalar parameter β (t) at each intermediate layer.
...
Note that the new propagation above is dynamic; propagation changes over the layers with differing β (t) and also the hidden states.
...
The output layer has a weight DISPLAYFORM3 The weights W (0) , W (1) , and β (t) 's are trained on a cross entropy loss in (2).
...
For Cora dataset the architecture consist of = 3 propagation layers, but the first prop- agation layer P (t=1) is non-trainable and the variable β (t=1) value is fixed at zero.
...
Similar to other settings the first propagation layer for Cora dataset is non-trainable with β (t=1) = 0.
