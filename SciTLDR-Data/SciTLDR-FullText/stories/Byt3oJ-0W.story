...
One sensible way to approximate a discrete category by continuous values is by using a temperature-dependent softmax function, component-wise defined as softmax τ (x) i = exp(x i /τ )/ j=1 exp(x j /τ ).

For positive values of τ , softmax τ (x) i is a point in the probability simplex.

Also, in the limit τ → 0, softmax τ (x) i converges to a vertex of the simplex, a one-hot vector corresponding to the largest x i 1 .
...
Our theoretical contribution is to show that M (X) can be obtained as the limit of S(X/τ ), meaning that one can approximate M (X) ≈ S(X/τ ) with a small τ .

Theorem 1 summarizes our finding.

We provide a rigorous proof in appendix A; briefly, it is based on showing that S(X/τ ) solves a certain entropy-regularized problem in B n , which in the limit converges to the matching problem in equation 2.

Theorem 1.

For a doubly-stochastic matrix P , define its entropy as h(P ) = − i,j P i,j log (P i,j ).

Then, one has, S(X/τ ) = arg max DISPLAYFORM3 Now, assume also the entries of X are drawn independently from a distribution that is absolutely continuous with respect to the Lebesgue measure in R. Then, almost surely, the following convergence holds: DISPLAYFORM4 Finally, we note that Theorem 1 cannot be realized in practice, as it involves a limit on the Sinkhorn iterations l.

Instead, we'll always consider the incomplete version of the Sinkhorn operator BID1 , where we truncate l in (1) to L. FIG0 in appendix A.3 illustrates the dependence of the approximation in τ and L.
...
We state this problem as a permutation-valued regression X i = P −1 θ,XiX i + ε i , where ε i is a noise term, and P θ,Xi is the permutation matrix mapping X i toX i , which depends onX i and parameters θ.

We are concerned with minimization of the reconstruction error 3 : DISPLAYFORM0 One way to express a complex parameterization of this kind is through a neural network: this network receivesX i as input, which is then passed through some intermediate, feed-forward computations of the type g h (W h x h + b h ), where g h are nonlinear activation functions, x h is the output of a previous layer, and θ = {(W h , b h )} h are the network parameters.

To make the final network output be a permutation, we appeal to constructions developed in Section 2: by assuming that the final network output P θ,X can be parameterized as the solution of the assignments problem; i.e., P θ,X = M (g(X, θ)), where g(·, θ) represents the outcome of all operations involving g h .

Unfortunately, the above construction involves a non-differentiable f (in θ).

We use Theorem 1 as a justification for replacing M (g(X, θ)) by the differentiable S(g(X, θ)/τ ) in the computational graph.

The value of τ must be chosen with caution: if τ is too small, gradients vanishes almost everywhere, as S(g(X, θ)/τ ) approaches the non-differentiable M (g(X, θ)).

Conversely, if τ is too large, S(X/τ ) may be far from the vertices of the Birkhoff polytope, and reconstructions P −1 θ,XX may be nonsensical (see Figure 2a ).

Importantly, we will always add noise to the output layer g(X, θ) as a regularization device: by doing so we ensure uniqueness of M (g(X, θ)), which is required for convergence in Theorem 1.
...
Then, these N outputs (each with N components) are used to create the rows of the matrix g(X, θ), to which we finally apply the (differentiable) Sinkhorn operator (i.e. g stacks the composition of the g h acting locally on each piece).
...
By applying some non-linear transformations, a Sinkhorn network richly parameterizes the mapping betweenX and the permutation P that once applied tõ X, will allow to reconstruct the original object as X rec = P X (the output).
...
The outputs lying on a latent space (rectangles surroundingX) are then connected through g 2 (arrows with empty circles) to conform the rows of the matrix g(X, θ); g(X, θ) i = g 1 • g 2 (X i ).
...
Applying S(·) leads to a 'soft-permutation' P θ,X that resolves inconsistencies in g(X, θ).

P θ,X is then used to recover the actual X at training, although at test time one may use the actual M (g(X, θ)).
...
noise γ(y); i.e., arg max y∈Y {X(y) + γ(y)} ∼ p(·|X).
...
One can verify, in a similar line as in BID29 DISPLAYFORM3 Unfortunately, as ii) with the categorical case, Gumbel-Matching distribution samples are not differentiable in X, but by appealing to Theorem 1, we define its relaxation for doubly stochastic matrices as follows: we say P follows the Gumbel-Sinkhorn distribution with parameter X and temperature τ , denoted P ∼ G.S.(X, τ ), if it has the distribution of S((X + ε)/τ ).

Samples of G.S.(X, τ ) converge almost surely to samples of the Gumbel-Matching distribution (see FIG0 DISPLAYFORM4 Unlike for the categorical case, neither the Gumbel-Matching nor Gumbel-Sinkhorn distributions have tractable densities.
...
To enable differentiability, we replace them by G.S.(X, τ ) distributions, leading to a surrogate ELBO that uses relaxed (continuous) variables.
...
DISPLAYFORM1 for our uniform prior over permutations we use the isotropic G.S.(X = 0, τ prior ) distribution, while for the variational posterior we consider the more generic G.S.(X, τ ).

FORMULA12 is intractable as there is not closed form expression for the density of G.S. random variables.

As a solution, we use that our prior and posterior are re-parameterizable in terms of matrices ε of Gumbel i.i.d variables: we have DISPLAYFORM2 DISPLAYFORM3 for the posterior and prior, respectively.

To obtain a tractable expression, we propose to use as 'code' or stochastic node Z, the variable (X + ε)/τ instead.

Then, the KL term substantially simplifies to KL((X + ε)/τ ε/τ prior ).
...
The network has a first fully connected layer that links a number with an intermediate representation (with 32 units), and a second (also fully connected) layer that turns that representation into a row of the matrix g(X, θ).
...
Given a trained model, 'soft' reconstructions are shown at different τ using S(X/τ ).
...
We show hard and soft reconstructions, with τ = 1.that produced our best results had around 1,050,000 parameters (see appendix B for a derivation), while in DeepPermNet, the layer connecting fc6 with fc7 has 512 × 4096 × 9 ≈ 19, 000, 000 parameters, let alone the AlexNet parameters (also to be learned).
...
Specifically, one states a linear dynamical system Y t = P W P T Y t−1 + ν t , where ν t is a noise term and W and P are latent variables with respective prior distributions.
...
In their work, they don't consider a temperature parameter τ , but their network still successfully learns, as τ = 1 happens to fall within the range of reasonable values.
...
Then, one has, S(X/τ ) = arg max DISPLAYFORM2 Now, assume also the entries of X are drawn independently from a distribution that is absolutely continuous with respect to the Lebesgue measure in R. Then, almost surely the following convergence holds: DISPLAYFORM3 We divide the proof of Theorem 1 in three steps.

First, in Lemma 1 we state a relation between S(X/τ ) and the entropy regularized problem in equation FORMULA0 .
...
S(X/τ ) = arg max DISPLAYFORM4 Proof.

We first notice that the solution P τ of the above problem exists, and it is unique.
...
By Sinkhorn's theorem, and our definition of the Sinkhorn operator, we must have that S(X/τ ) = P τ .Lemma 2.
...
Call P τ the solution to the problem in equation 10, i.e. P τ = P τ (X) = S(X/τ ).

Under the assumptions of Lemma 2, P τ → P 0 when if τ → 0 + .Proof.

Proof Notice that by Lemmas 1 and 2, P τ is well defined and unique for each τ ≥ 0.Moreover, at τ = 0, P 0 = M (X) is the unique solution of a linear program.

Now, let's define f τ (·) = ·, X F + τ h(·).

We observe that f 0 (P τ ) → f 0 (P 0 ).

Indeed, one has: DISPLAYFORM7 From which convergence follows trivially.

Moreover, in this case convergence of the values implies the converge of P τ : suppose P τ does not converge to P 0 .

Then, there would exist a certain δ and sequence τ n → 0 such that P τn − P 0 > δ.

On the other hand, since P 0 is the unique maximizer of an LP, there exists ε > 0 such that f 0 (P 0 ) − f 0 (P ) > ε whenever P − P 0 > δ, P ∈ B N .

This contradicts the convergence of f 0 (P τn ).

The first statement is Lemma 1.

Convergence (equation 11) is a direct consequence of Lemma 3, after noticing P τ = S(X/τ ) and P 0 = M (X).

We note that an alternative approach for the limiting argument is presented in BID8 .

Finally, we notice that all of the above results can be understood as a generalization of the wellknown approximation result arg max i x i = lim τ →0 + sof tmax(x/τ ).
...
This layer connects with another layer with N units, representing a row of g(X, θ).For images, the first layer is a convolution, composed by n f convolutional filters of receptive field size K s with n c channels (one or three) followed by a ReLU + max-pooling (with stride s) operations.
...
Here we show how to compute KL((X + ε)/τ ε/τ prior ), as defined in 4.1.

We first notice that the density of the variable h = (a + g)/b, where g has a Gumbel distribution and a, b are constants is given by: DISPLAYFORM0 Therefore, the log density ratio LR(z) between each component of h 1 = (x i,j + ε i,j )/τ and h 2 = ε i,j /τ prior is (suppressing indexing for simplicity) DISPLAYFORM1 We need to take expectations with respect to the distribution of h 1 .

To compute this expectation, we first express the above ratio in terms of ε DISPLAYFORM2 Now we appeal to the law of the unconscious statistician, and take the expectation with respect to ε.

• E(ε) = γ ≈ 0.5772 (the Euler-Mascheroni constant)• Moment generating function E(exp(tε)) = Γ(1 − t); implying E(exp(−ε)) = 1 and DISPLAYFORM0 we have: DISPLAYFORM1 From this, it easily follows (adding all the N 2 components) that DISPLAYFORM2 where S 1 = τ prior /τ i,j x i,j and S 2 = i,j exp (−x i,j τ prior /τ ).C SUPPLEMENTAL RESULTS
...
Fourth column: soft reconstructions using S(X/τ ) and τ = 1.
...
This relates to the estimation of the marginals E θ (P i,j ), known to be a #P hard problem.
...
Also, lets call H(µ) the entropy of (6) for the parameter θ(µ) such that µ = E θ(µ) (P ).

Then, E θ (P ) = arg max µ∈M θ, µ F + H(µ).Notice the only difference between the optimization problems in FORMULA0 and FORMULA0 is the entropy term, after identifying X with θ.

Therefore, one may understand the Sinkhorn operator as providing approximations for the partition function and the marginals, which will be accurate insofar as h(µ) is a good approximation for H(µ).

In this way, one can understand S(X) as an approximation for E θ (P ), that may complement more classical ones, as the Bethe and Kituchani's approximations for H(µ), and the corresponding approximate inference algorithms that they give rise to BID55 BID51 .
...
Concrete softmax((x + )/τ ) Gumbel-Sinkhorn GS(X, τ ) S((X + )/τ )
