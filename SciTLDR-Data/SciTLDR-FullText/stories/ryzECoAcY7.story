...
Π 2 uses as input the current state (joint positions θ and velocitiesθ) and goal state (yellow box) and outputs a subgoal state (green box) for Π 1 to achieve.

Π 1 takes in the current state and its goal state (green box) and outputs a subgoal state (purple box) for Π 0 to achieve.

Π 0 takes in the current state and goal state (purple box) and outputs a vector of joint torques.high-level policy may output a subgoal state for the low level to achieve, and the state to which this subgoal state leads will depend on the current low-level policy.
...
Figure 1 shows how an ant agent trained with HAC uses its 3-level policy hierarchy (π 2 , π 1 , π 0 ) to move through rooms to reach its goal.

At the beginning of the episode, the ant's highest level policy, π 2 , takes as input the current state, which in this case is a vector containing the ant's joint positions and velocities ([θ,θ] ), and its goal state, represented by the yellow box.

π 2 then outputs a subgoal state, represented by the green box, for π 1 to achieve.

π 1 takes as input the current state and its goal state represented by the green box and outputs the subgoal state represented by the purple box.

Finally, π 0 takes as input the current state and the goal state represented by purple box and outputs a primitive action, which in this case is a vector of joint torques.

π 0 has a fixed number of attempts to move to the purple box before π 1 outputs another subgoal state.

Similarly, π 1 has a fixed number of subgoal states that it can output to try to move the agent to the green box before π 2 outputs another subgoal.
...
A UMDP is a tuple U = (S, G, A, T, R, γ), in which S is the set of states; G is the set of goals; A is the set of actions; T is the transition probability function in which T (s, a, s ) is the probability of transitioning to state s when action a is taken in state s; R is the reward function; γ is the discount rate ∈ [0, 1).

At the beginning of each episode in a UMDP, a goal g ∈ G is selected for the entirety of the episode.

The solution to a UMDP is a control policy π : S, G → A that maximizes the value function v π (s, g) = E π [ ∞ n=0 γ n R t+n+1 |s t = s, g t = g] for an initial state s and goal g.
...
The UVFA will be used to estimate the action-value function of a goal-conditioned policy π, DISPLAYFORM0 In our experiments, the UVFAs used will be in the form of feedforward neural networks.
...
The purpose of our framework is to efficiently learn a k-level hierarchy Π k−1 consisting of k individual policies π 0 , . . .

, π k−1 , in which k is a hyperparameter chosen by the user.

In order to learn π 0 , . . .

, π k−1 in parallel our framework transforms the original UMDP, DISPLAYFORM0 In the remainder of the section, we will describe these tuples at a high-level.
...
HAC nests policies by embedding the policy at level i − 1, π i−1 , into the transition function at level i, T i .
...
π i−1 then has at most H attempts to achieve g i−1 , in which H, or the maximum horizon of a subgoal action, is another parameter provided by the user.

When either π i−1 runs out of H attempts or a goal g n , n ≥ i − 1, is achieved, the transition function terminates and the agent's current state is returned.

Level i's state transition function T i thus depends on the full policy hierarchy below level i, Π i−1 , due to the hierarchy's nested architecture.

Each action from π i−1 depends on T i−1 , which depends on π i−2 and so on.

Consequently, we use the notation T i|Πi−1 for level i's state transition function going forward as it depends on the full lower level policy hierarchy.
...
That is, whenever π i changes, the transition function at levels above i, T j|Πj−1 , j > i, can change.

The second cause is exploring lower level policies.

Because all levels have a deterministic policy in our algorithm, all levels will need to explore with some behavior policy π i b that is different than the policy it is learning π i .

For instance, in continuous domains, the agent may add Gaussian noise to its greedy policy: DISPLAYFORM0 2 .

Yet whenever a lower level policy hierarchy uses some behavior policy Π i−1 b to achieve a subgoal, the transition function at level i, T i|Πi−1 b , will also vary over time.

RL methods will likely not be effective at learning subgoal policies in parallel if each subgoal policy at level i is trained with respect to a transition function that uses the current lower level policy hierarchy Π i−1 or the behavior lower level policy hierarchy Π i−1 b .

RL methods need the distribution of states to which actions lead to be stable in order to effectively value actions and both Π i−1 and Π i−1 b are continually changing.
...
Hindsight action transitions use a simple technique to simulate the transition function that uses the optimal policy hierarchy below level i, T i|Π * i−1.
...
The robot begins in state s 0 when the high level policy π 1 outputs the subgoal state g 0 for the low level to achieve.

The low level policy π 0 then executes H = 5 primitive actions using some behavior policy π 0 b but is unable to achieve g 0 , instead landing in s 1 .

After executing H = 5 primitive actions, the first action by π 1 is complete and a hindsight action transition can be created.
...
The purpose of hindsight action transitions is to simulate a transition function that uses the optimal lower level policy hierarchy Π * i−1 .

Yet without a model of the environment, the exact path Π * i−1 would have taken is unknown.
...
Using the same process, the hindsight action transition created for the second action by π 1 would be [initial state = s 1 , action = s 2 , reward = -1, next state = s 2 , goal = yellow flag, discount rate = γ].Although none of the hindsight actions produced in the episode contained the sparse reward of 0, they are still helpful for the high level of the agent.
...
More importantly, these transitions are robust to a changing and exploring lower level policy π 0 because they assume a transition function that uses π * 0 and not the current low level policy π 0 or low level behavior policy π 0 b .
...
Beginning with the low level, after each of the at most H = 5 primitive actions executed by the low level policy π 0 per high level action, the low level will create two transitions.
...
For instance, assuming the same shortest path reward function described earlier, after the first primitive action in the episode, the low level will receive the transition [initial state = s 0 , action = joint torques, reward = -1, next state = first tick mark, goal = g 0 , discount rate = γ].

The second transition is a copy of the first transition, but the goal state and reward components are temporarily erased: [initial state = s 0 , action = joint torques, reward = TBD, next state = first tick mark, goal = TBD, discount rate = γ].
...
Assuming the last state reached s 5 is used as the hindsight goal, the first hindsight goal transition for the high level would be [initial state = s 0 , action = s 1 , reward = -1, next state = s 1 , goal = s 5 , discount rate = γ].
...
π i may adjust its policy to output these distant subgoals that have relatively high Q-values.

Yet the lower level policy hierarchy Π i−1 has not been trained to achieve distant subgoals, which may cause the agent to act erratically.
...
After level i proposes a subgoal a i , a certain fraction of the time λ, the lower level behavior policy hierarchy, Π i−1 b , used to achieve subgoal a i must be the current lower level policy hierarchy Π i−1 .
...
These primitive actions must follow π 0 exactly.
...
For this analysis, we use the notation |s − a| to refer to the number of actions required by an optimal version of the policy at the level below, π * i−1 , to move the agent from state s to subgoal state a.1.
...
|s − a| ≤ H and Achievable by Π i−1 : For those (state, goal, subgoal action) tuples in which the subgoal action can be achieved by the current lower level policy hierarchy Π i−1 , subgoal testing should have little to no effect.
...
|s − a| ≤ H and Not Achievable by Π i−1 : The effects of subgoal testing are a bit more subtle for those (state, goal, subgoal action) tuples in which the subgoal action can be achieved with at most H actions by an optimal version of the policy below, π * i−1 , but cannot yet be achieved with the current policy π i−1 .
...
Algorithm 2 Hierarchical Q-Learning (HierQ)Input:• Key agent parameters: number of levels in hierarchy k > 1, maximum subgoal horizon H, learning rate α Output:• k trained Q-tables Q 0 (s, g, a) , ..., Q k−1 (s, g, a)Use pessimistic Q-value initialization: DISPLAYFORM0 Sample initial state and task goal Initialize previous state arrays for levels i, 0 DISPLAYFORM1 Sample action using -greedy policy DISPLAYFORM2 DISPLAYFORM3 DISPLAYFORM4 Add state s 0 to all previous state arrays DISPLAYFORM5 end for end for end for end if s i ← s i end for return s iOutput current state end function
...
Note that the actionā passed to the first transition function must be generated by some hierarchical behavior policy Π i b .
...
γ i is set to 0 if a subgoal is tested and missed or if an action achieves the goal, but is otherwise γ from U original : γ i (a, s , g) = 0, ∀s : (s / ∈ a) ∨ (s ∈ g).
