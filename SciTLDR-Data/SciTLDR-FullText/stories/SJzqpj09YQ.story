...
Typically, the eigenfunctions of interest are approximated from a fixed number of points small enough to be stored in memory, and then the value of the eigenfunction at other points is approximated by use of the Nyström method (Bengio et al., 2004) .
...
Suppose that instead of a matrix A we have a symmetric (not necessarily positive definite) kernel k(x, x ) where x and x are in some measurable space Ω, which could be either continuous or discrete.

Let the inner product on Ω be defined with respect to a probability distribution with density p(x), DISPLAYFORM0 ].

In theory this could be an improper density, such as the uniform distribution over R n , but to evaluate it numerically there must be some proper distribution over Ω from which the data are sampled.
...
Replacing rows i and j with points x and x and sums with expectations, this becomes: DISPLAYFORM2 where the optimization is over all functions u : Ω → R N such that each element of u is an integrable function under the metric above.

Also note that as u i is a row vector while u(x) is a column vector, the transposes are switched.

This is equivalent to solving the constrained optimization problem DISPLAYFORM3 For clarity, we will use Σ = E x u(x)u(x)T to denote the covariance 1 of features and DISPLAYFORM4 T to denote the kernel-weighted covariance throughout the paper, so the objective in Eq. 6 becomes Tr(Σ −1 Π).

The empirical estimate of these quantities will be denoted asΣ andΠ.

The form of the kernel k often allows for simplification to Eq. 6.

If Ω is a graph, and k(x, x ) = −1 if x = x and are neighbors and 0 otherwise, and k(x, x) is equal to the total number of neighbors of x, this is the graph Laplacian, and can equivalently be written as: DISPLAYFORM0 for neighboring points (Sprekeler, 2011, Sec. 4 .1).
...
The equivalent kernel to the graph Laplacian for Ω = R n is DISPLAYFORM1 where e i is the unit vector along the axis i.
...
This means the maximum of DISPLAYFORM3 Λ ii with respect to u 1:n (x) spans the first n < N eigenfunctions.

If we additionally mask the gradients of Λ ii so they are also independent of any u j (x) where j is less than i: DISPLAYFORM4 and combine the gradients for each i into a single masked gradient∇ DISPLAYFORM5 ) which we use for gradient ascent, then this is equivalent to independently optimizing each u i (x) towards the objective Λ ii .
...
This gradient can then be passed as the error from u back to parameters θ, yielding: DISPLAYFORM7 To simplify notation we can express the above as DISPLAYFORM8 Where DISPLAYFORM9 ∂θ are linear operators that denote left-multiplication of the Jacobian of Π and Σ with respect to θ by A. A TensorFlow implementation of this gradient is given in the supplementary material in Sec. B.

The expression in Eq. 14 is a nonlinear function of multiple expectations, so naively replacing Π, Σ, L, Λ and their gradients with empirical estimates will be biased.
...
, x tN and x t1 , . . . , x tN 5: DISPLAYFORM0 T , covariance of minibatches DISPLAYFORM1 Compute gradient∇ θ Tr(Λ(Π t ,Σ t ,Ĵ Πt ,J Σt )) according to Eq. 14 11: DISPLAYFORM2 noisy unbiased estimates of the gradient of each: DISPLAYFORM3 .
...
Throughout the remainder of the paper, letX t denote the empirical estimate of a random variable X from the minibatch at time t, and letX t represent the estimate of X from a moving average, soΣ t andJ Σt are defined as: DISPLAYFORM6 DISPLAYFORM7 This moving average is equivalent to solving DISPLAYFORM8 by stochastic gradient descent and clearly has the true Σ and J Σ as a minimum for a fixed θ.

Note that Eq. 14 is a linear function of Π and J Π , so plugging inΠ t andĴ Πt gives an unbiased noisy estimate.

By also replacing terms that depend on Σ and J Σ withΣ t andJ Σt , then alternately updating the moving averages and θ t , we convert the problem into a two-timescale update.

Here θ t corresponds to x t ,Σ t andJ Σt correspond to y t ,∇ θ Tr(Λ(Π t ,Σ t ,Ĵ Πt ,J Σt )) corresponds to F(x t , y t ) and (Σ t−1 −Σ t ,J Σt−1 −Ĵ Σt ) corresponds to G(x t , y t ).
...
First, we have to compute an explicit estimateĴ Σt of the Jacobian of the covariance with respect to the parameters at each iteration.
...
We follow this practice in our experiments and pick constant values of α and β.
...
As a first experiment to demonstrate the correctness of the method on a problem with a known solution, we investigated the use of SpIN for solving the Schrödinger equation for a two-dimensional hydrogen atom.

The time-independent Schrödinger equation for a single particle with mass m in a potential field V (x) is a partial differential equation of the form: DISPLAYFORM0 whose solutions describe the wavefunctions ψ(x) with unique energy E. The probability of a particle being at position x then has the density |ψ(x)| 2 .
...
We trained a standard neural network to approximate the wavefunction ψ(x), where each unit of the output layer was a solution with a different energy E. Details of the training network and experimental setup are given in the supplementary material in Sec. C.1.

We found it critical to set the decay rate for RMSProp to be slower than the decay β used for the moving average of the covariance in SpIN, and expect the same would be true for other adaptive gradient methods.
...
This makes it possible to learn eigenfunctions over very high-dimensional spaces from very large datasets and generalize to new data without the Nyström approximation.
...
DISPLAYFORM0 T ], L be the Cholesky decomposition of Σ and Λ = L −1 ΠL −T .
...
Λ 1:n,1:n is independent of u n+1:n (x).The Cholesky decomposition of a positive-definite matrix is the unique lower triangular matrix with positive diagonal such that LL T = Σ. Expanding this out into blocks yields: DISPLAYFORM1 Σ 1:n,1:n Σ 1:n,n+1:N Σ n+1:N,1:n Σ n+1:N,n+1:N Inspecting the upper left block, we see that L 1:n,1:n L T 1:n,1:n = Σ 1:n,1:n .

As L 1:n,1:n is also lowertriangular, it must be the Cholesky decomposition of Σ 1:n,1:n .
...
The maximum of this is clearly proportional to the top eigenfunction, and DISPLAYFORM5 1:n,1:n u 1:n (x) are the first n eigenfunctions of K. Because u 1:n (x) span the first n eigenfunctions, and Λ ii is independent of u n+1 (x) for i < n+1, u 1:n+1 (x) is a maximum of n i=1 Λ ii no matter what the function u n+1 (x) is.

Training u n+1 (x) with the masked gradient ∇ u Tr(Λ) is equivalent to maximizing Λ (n+1)(n+1) , so for the optimal u n+1 (x), u 1:n+1 (x) will be a maximum of n+1 i=1 Λ ii .
...
We use the finite difference approximation of the differential Laplacian given in Sec. 3.2 with some small number (around 0.1), which takes the form: DISPLAYFORM1 when applied to ψ(x).

Because the Hamiltonian operator is a purely local function of ψ(x), we don't need to sample pairs of points x, x for each minibatch, which simplifies calculations.
...
We used the same block-sparse structure for the weights that was used in the Schrödinger equation experiments, with sparsity in weights between units extended to sparsity in weights between entire feature maps for the convolutional layers.

We trained with RMSProp with learning rate 1e-6 and decay 0.999 and covariance decay rate β = 0.01 for 1,000,000 iterations.

To make the connection to gradient descent clearer, we use the opposite convention to RMSProp: β = 1 corresponds to zero memory for the moving average, meaning the RMS term in RMSProp decays ten times more slowly than the covariance moving average in these experiments.
