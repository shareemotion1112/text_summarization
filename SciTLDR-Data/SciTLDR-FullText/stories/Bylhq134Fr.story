...
Variational inference was employed in prior work to infer z (and w implicitly), and to obtain a point estimate of θ, as a by-product of optimising the variational lower bound.
...
where we have assumed a zero-mean GP, k γ (·, ·) is a covariance function and γ is a small set of hyper-parameters.
...
are the lengthscales and σ 2 k is the kernel variance.
...
where the variational distribution over w is made to explicitly depend on remaining variables through the conditional prior, and q(z) is chosen to be a diagonal (mean-field) Gaussian densitie, q(z) = N (z; µ µ µ z , diag(σ σ σ 2 z )), and q(u) is chosen to be a correlated multivariate Gaussian, q(u) = N (u; µ µ µ u , Σ u ).
