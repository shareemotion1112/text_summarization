...
Train policy π beginning from P 1 with environmental reward; Collect trajectories & train Tendency Reward T (·); DISPLAYFORM0 Train policy π beginning from P i+1 with hybird Reward; Update Tendency Reward; i ← i + 1; return π;
...
At each time step t, the agent receives an observation O t and chooses an action a t from a set of action space A according to its policy π.

Then the environment will give the agent the next state s t+1 and a reward r t .

The goal of the agent is to maximize the expected return from each state s t through accumulating the reward by R t = ∞ k=0 γ k r t+k .
...
Naïve applications of conventional RL tend to fail in goal-oriented tasks with a large state space and sparse rewards.
...
The kernel is used with d most recent frames in a bid to detect repeated states, as is shown in Eq. FORMULA2 : DISPLAYFORM0 We assume that s now is a revisited state if ρ is larger than 0.9.

The parameter d is 20% of the max steps of each environment and σ = 1 & δ = 0 are constants in our experiments.
...
DISPLAYFORM1 where λ is a scale factor (10 −3 in our experiments) based on the following hypothesis: a positive reward received from the environment should be more important than the output of T (·).
...
The newly reached states would then be included into P i+1 if the agent's success rate on that state is between a lower-bound α and an upper-bound β, which guarantee the new states to a proper difficulty (α is 0.1 and β is 0.9 in our experiments).In each phase generation, a reasonable x i is required.
...
DISPLAYFORM0 {Q j }; DISPLAYFORM1 return π;C ENVIRONMENT DETAILS C.1 MAZE Our first environment is a maze task.
