...
These computations are implemented via three "update" functions (the φs) and three "aggregation" functions (the ρs), DISPLAYFORM1 where E i = {(e k , r k , s k )} r k =i .

The edges are updated by φ e , as a function of the sender vertex, receiver vertex, edge, and global attributes.

We term the updated edge attribute the "message", e k .

Next, each vertex, i, is updated by aggregating the e k messages for which i = r k , and computing the updated vertex attribute, v i , as a function (φ v ) of these aggregated messages, as well as the current vertex and global attributes.

Finally, the global attributes are updated, by φ u , as a function of the current global attribute and all aggregated e k and v i attributes.
...
These architecture can also be made recurrent in time by introducing a state graph, and using recurrent neural networks (RNNs) as the φ functions.
...
Architecture details are as follows: input graphs G t in go through a GN encoder block, a basic GN module whose φ v , φ e and φ u are three separate 64-unit MLPs, with 1 hidden layer, and ReLU activations and whose ρ functions are summations.

The output of the GN encoder block is used, in conjunction with a state graph G t−1 hid , in a "GraphGRU", where each φ function is a Gated Recurrent Unit (GRU) BID3 with a hidden state size of 32 for each of vertices, edges and globals.
...
VAIN networks are essentially single feed-forward GN blocks where the φ e and ρ e→v functions take particular and restricted form: DISPLAYFORM2 We also compared our full RFM against ablated variants, which allowed us to measure the importance of the relational reasoning component, and of time recurrence.
