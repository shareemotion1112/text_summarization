...
It shows that DFO algorithms can converge at Ω(1/ √ T ) rate in the best case (T refers to the total number of iterations), without assumptions on convexity and smoothness, even when the evaluation of black-box function is noisy.
...
We will discuss the rate of convergence, when introducing SHE 2 algorithm as a discrete-time approximation to SHE 2 .3.2 SHE 2 : ALGORITHM DESIGN AND ANALYSIS Given a black-box function f (x) and a sequence of non-negative step-size α t (t=0, 1, 2, . . .

, T ), which is small enough, as well as the scale of perturbation ε, we propose to implement SHE 2 as Algorithm 1.
...
SHE 2 should enjoy the same convergence rate Ω(1/ √ T ) without addressing any further assumptions.
...
, ζ T and |ζ(t)| 2 = ε for every time t. Through combining above two ODEs and Lemma 1, we can obtain the SDE of X(t) based on the perturbation ζ(t) as: DISPLAYFORM1 The sequence Y t (t=0, 1, 2, · · · T ) always exploits the minimum point that has been already found by X t at time t. Thus, we can consider Y t is the discrete-time of Y (t) that exploits the minimum traversed by X(t).
...
Specifically, for each pair of coupled processes, a new surrogation model Q δ (X i (t), Y i (t), Y (t)) has been proposed to measure the joint distance from X i (t) to Y i (t) and Y (t) respectively, where δ > 0 refers to a trade-off factor weighted-averaging the two distances.
...
Compared to Particle Swarm, which usually adopts constant stepsize settings (i.e., α t , β t and γ t are fixed as a constant value), P-SHE 2 proposes to use a small α t , while setting β t = −(α t + 3/t) and γ t = 3/t for each (the t th ) iteration.
...
We treat the loss functions of logistic regression and SVM as black-box functions and parameters (e.g., projection vector β for logistic regression) as optimization outcomes.
...
We say the point x * is a local minimum point of the function f (x) if and only if f (x * ) ≤ f (x) for any x ∈ U (x * ), where U (x * ) which is any open neighborhood around the point x * .Let us first remove the noise ζ(t) in our system (4).
...
SetẊ 0 (t) = P (t), we can write equation (a) in (10) in Hamiltonian form DISPLAYFORM2 Set H(X, Y, P ) = P 2 2 + Q(X, Y ).

Then we have DISPLAYFORM3 As we have DISPLAYFORM4 , we see from (12) that we have DISPLAYFORM5 Notice that by our construction part (b) of the coupled process (10), we have f (Y 0 (t)) ≤ f (X 0 (s)) for 0 ≤ s ≤ t. If X 0 (t) − Y 0 (t) = 0, then (X 0 (t) − Y 0 (t)) ·Ẏ 0 (t) = 0.

If X 0 (t) − Y 0 (t) = 0, then we see that Y 0 (t) = X 0 (s 0 ) for some 0 ≤ s 0 < t, and f (X 0 (s 0 )) < f (X 0 (t)).

By continuity of the trajectory of X 0 (t) as well as the function f (x), we see that in this caseẎ 0 (t) = 0, so that (X 0 (t) − Y 0 (t)) ·Ẏ 0 (t) = 0.
...
Let us now add the noise ζ(t) to part (a) of (10), so that we come back to our original system (4).

We would like to argue that with the noise ζ(t), we actually have lim t→∞ min 0≤s≤t f (X(s)) = f (x * ), and thus X(t) → x * when t → ∞ as desired.
...
We first notice that we have ζ(t) = ε ξ t ξ t 2 , where ξ t ∼ N (0, I) is a sequence of i.i.d.normal, so that ζ(t) 2 = ε, Eζ(t) = 0.

Viewing (10) as a small random perturbation (see BID10 , Chapter 2, Section 1)) of the system (4) we know that for any δ > 0 fixed, we have DISPLAYFORM6 as ε → 0.

From here we know that the process X(t) behaves close to X 0 (t) with high probability, so that by Lemma 1 we know that with high probability we have lim t→∞ f (X(t)) − min 0≤s≤t f (X(s)) = 0 .Our next step is to improve the above asymptotic to X(t) → x * as t → ∞. Comparing with (16), we see that it suffices to show DISPLAYFORM7 To demonstrate (17), we note that when t is large, we can ignore in (4) the damping term 3 tẊ (t) and obtain a friction-less dynamics f (X(τ ))
...
Step 2 in Lemma 2 shows that when t → ∞ the noise term ζ(t) helps the process X(t) to reach local
