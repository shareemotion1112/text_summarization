...
Optical flow can be described as the horizontal and vertical displacement of a pixel from one frame to the next (Farnebäck (2003) ).

Several algorithms for flow calculation exist, such as TV-L1 (Zach et al. (2007) ), Brox (Brox et al. (2004) ), and Farneback (Farnebäck (2003) ).
...
For correct centering we implemented a gamma correction with γ = exp ( −skewness /param), (where param was tweaked to maximize detection performance (here param = 4.3)) and applied a binary threshold at value 3 to separate the bladder from the rest of the fish.
...
They were stacked to 170 channels, alternating x and y. We used the optical flow algorithm by Farnebäck (2003) with parameters 2 detecting flow even when the tail moved fast.
...
Then we performed a grid-search to tune SVM-kernel, γ, and C 5 .
...
16.4 (van der Walt et al. (2011) ), 5 RBF-kernel, γ ∈ {1e-1, 1e-2, 1e-3, 1e-4}, C ∈ {0.01, 0.1, 1, 10}; linear-kernel, C ∈ {0.01, 0.1, 1, 10}.
