...
Further to this Pratt (1993), a hyperplane utility measure as defined by θ s from T t which then rescales the weight magnitudes was shown to perform well, showing faster convergence when transferred to T t .
...
The softmax function produces the attention weights α by passing all outputs of the source RNN, h S to the softmax conditioned on the target word of the opposite sentence h t .
...
The final attention vector α t is used as a weighted input of the context vector c t and the hidden state output h t parameterized by a xavier uniform initialized weight vector W c to a hyperbolic tangent unit.
...
We first start by describing a method that learns to guide weights on T t by measuring similarity between θŝ and θt during training by using moving averages on the slope of the error curve.
...
The prediction from few samples a l t is the single output from T l t and γ is the slope of the error curve that is updated at regular intervals during training.
...
In its simplest form, this can be represented as a moving average over the development set error curve so to choose δ t = E[∇ [t,t+k] ], where k is the size of the sliding window.
...
Equation 3 shows the regularized least squares function for a set of cubic smoothing splines ψ which are piecewise polynomials that are connected by knots, distributed uniformly across the given interval [0, T ].

Splines are solved using least squares with a regularization term λθ 2 j ∀ j and ψ j a single piecewise polynomial at the subinterval [t, t + k] ∈ [0, T ], as shown in Equation 3.

Each subinterval represents the space that γ is adapted for over time i.e change the influence of the T s Dropping Network as T t model learns from few examples over time.
...
All models are trained for 30,000 epochs, using a dropout rate p = 0.5 with Adaptive Momentum (ADAM) gradient based optimization Kingma & Ba (2014) in a 2-hidden layer network with an initial learning rate η = 0.001 and a batch size b T = 128.
...
Therefore, the baseline comparison simply transfers all weights from θ s → θ t
...
As aforementioned, this approach finds the slope of the online error curve between sub-intervals so to choose γ i.e the balance between the source ensemble and target model trained on few examples.
...
This is explained by the use of the weighting scheme proposed with spline smoothing of the error curve i.e γ decreases at a faster rate for T t due to the ineffectiveness of the ensembles created on the QM dataset.
...
The method for transfer only relies on one additional parameter γ.

We find that in practice using a higher decay rate γ (0.9-0.95) is more suitable for closely related tasks.

Decreasing γ in proportion to the slope of a smooth spline fitted to the online error curve performs better than arbitrary step changes or a fixed rate for γ (equivalent to static hard parameter ensemble transfer).
