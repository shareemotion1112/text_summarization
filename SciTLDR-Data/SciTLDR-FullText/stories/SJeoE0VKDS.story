...
Our agent interacts with its environment over discrete time steps, modeled as a Markov Decision Process (MDP), defined by the 6-tuple (S, S 0 , A, τ, R, G) (Puterman, 1994) .
...
At time step t in state s t ∈ S, the agent chooses an action a t ∈ A based on policy π : S × A → [0, 1], such that a t ∼ π(s t , ·).

After taking a t , the agent is in state s t+1 = τ (s t , a t ) and receives reward r t ∼ R(s t , a t ) and a discount factor γ t ∼ G(s t , a t ).

Over n environment steps, we define the history of visited states as H s = (s 1 , . . .

, s n ), where s i ∈ S ∀i ∈ N. In RL, the usual objective is to maximize the sum of expected future rewards V π (s) = E r t +

To learn a policy π that maximizes the expected return, an RL agent has to efficiently explore its environment.
...
In this work, we focus on learning a lower-dimensional representation of state when our state (or observations in the partially observable (Kaelbling et al., 1998) case) is high-dimensional (Dayan, 1993; Tamar et al., 2016; Silver et al., 2016; Oh et al., 2017; de Bruin et al., 2018; Ha and Schmidhuber, 2018; François-Lavet et al., 2018; Hafner et al., 2018; Gelada et al., 2019) .

We learn our lower-dimensional abstract representation by encoding the high-dimensional state with an encoder e :

S → X parameterized by θê, where X ∈ R n X .

The dynamics are learned via the following functions: a transition functionτ : X × A → X parameterized by θτ , a reward functionr : X × A → [0, R max ] parameterized by θr, and a per timestep discount factor function γ : X × A → [0, 1) parameterized by θγ.

In order to leverage all past experiences, we use an off-policy learning algorithm that sample transition tuples (s, a, r, γ, s ) from a replay buffer.

We first encode our current and next states:

x ←ê(s; θê), x ←ê(s ; θê), where x, x ∈ X and θê are our encoder parameters.

The Q-function is learned using the DDQN (van Hasselt et al., 2015) algorithm, which uses the target

where θ Q − and θê− are parameters of an earlier buffered Q-function (or our target Q-function) and encoder respectively.
...
For instance, states that are temporally close through the transition function are brought close in the lowerdimensional space-a phenomenon reported in François-Lavet et al. (2018) .
...
We also use a soft constraint between consecutive states that tends to enforce that two consecutive abstract representations are at a distance of at least ω:

where ω is a hyperparameter (a discussion of this loss is provided in Appendix A).

The hyperparameter ω can be used to estimate the accuracy of our transition loss, hence of our novelty estimates.
...
If ω is the minimum distance between successive states, then when

2 , the transitions are on average within a ball of radius ω δ of the target state.

Here δ > 1 is a hyperparameter that we call the slack ratio.
...
We minimize the sum of all the aforementioned losses through gradient descent with learning rate α:
...
For our approach, we calculate rollout estimates of next states based on our transition modelτ and sum up the corresponding rewards (which we denote as r : X × A → [0, R max ] and can be a combination of both intrinsic and extrinsic rewards).

We calculate expected returns based on the discounted rewards of our d-depth rollouts:

Note that we simulate only b-best options at each expansion step based on Q(x, a; θ Q ), where b ≤ |A|.
...
To get a novelty metric for our abstract representations, we can apply a weighted version of the novelty measure in abstract representational space to our current state s with our encoder applied x ←ê(s; θê), and a history of encoded states.
...
1 Initialization: transition buffer H, agent policy π; 2 Sample n init initial random transitions, let t = n init ; 3 while t ≤ n max do //

We update our dynamics model and Q-function every n f req steps

Sample batch of transitions (s, a, r extr , r intr , γ, s ) ∈ H;

Train dynamics model with (s, a, r extr , γ, s );

Train Q-function with (s, a, r extr + r intr , γ, s );
...
Indeed, our encoderê takes a high-dimensional input and compresses it to a lowdimensional representation.
...
As most of these secondary features are encoded only in the dynamics modelτ , our agent has to experience a transition in order to accurately represent both primary and secondary features.
...
While it may seem redundant to include the 1st nearest neighbor distance in this metric (which would be itself if we've visited the state), the 1st nearest neighbor is non-zero when we calculate the novelty of a predicted state using our learned transition functionτ .
...
On the other hand, minimization of our consecutive distance loss ensures that the distance between two consecutive states is ≤ ω.

We consider the minima of the combination of these two losses for a consecutive states s and s :

The minimum of the combination of the losses is obtained when the distance between s and s is ω.
...
We also use the same discount factor for all experiments (γ = 0.8) and the same freeze interval for target parameters 1000.

The reason behind our low discount factor is due to the high density of non-stationary intrinsic rewards in our state.

For all model-based abstract representation training, the following hyperparameters were all kept constant: minimum distance between consecutive states ω = 0.5, slack ratio δ = 6 and transition model dropout of 0.1.
...
E.1.1 ENCODER For all our non-control task inputs, we flatten our input and use the following feed-forward neural network architecture forê:
...
We use a learning rate of α = 0.00025 for all experiments.
...
We use α = 0.00025, n iters = 30000 for our model-free algorithms and α = 0.000025, n iters = 50000 for experiments that include a model-base component.
