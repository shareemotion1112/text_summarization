...
To formalize the problem, we denote the number of original training examples by n. To model the amount of noise, we dilute the dataset by adding α noisy examples to the training set for each original training example.

Thus, the total number of noisy labels in the training set is αn.

Note that by varying the noise level α, we do not change the available number of original examples.
...
For MNIST, we vary the ratio α of randomly labeled examples to cleanly labeled examples from 0 (no noise) to 100 (only 11 out of 101 labels are correct, as compared with 10.1 for pure chance).

For the more challenging dataset CIFAR-10, we vary α from 0 to 10.

For the most challenging dataset ImageNet, we let α range from 0 to 5.
...
The parameter δ parameterizes the degree of structure in the noise.

It defines how much more likely the second most likely class is over chance.creasing noise (though it still attains 40 percent accuracy, well above chance, at α = 100).
...
Note that = 1/(1 + α), where α is the ratio of noisy labels to certainly correct labels.

To induce structure in the noise, we bias noisy labels to certain classes.

We introduce the parameter δ to parameterize the degree of structure in the noise.

It defines how much more likely the second most likely class is over chance.

With δ = 0 the noise is uniform, whereas for δ = 1 the second most likely class is equally likely as the correct class.
...
FIG4 shows the results on MNIST for the three different types of structured noise, as δ varies from 0 to 1.
...
We interpolate between uniform noise, δ = 0, and noise so highly skewed that the most common false label is as likely as the correct label, δ = 1.

Except for δ ≈ 1, performance is similar to uniform noise. :
...
For example, at α = 10, 2,000 clean labels are needed to attain 90% performance, while at α = 50, 10,000 clean labels are needed.
...
For example, for the blue curve of α = 10 and 1,000 clean labels, the network is trained on 11,000 examples: 1,000 cleanly labeled examples and 10,000 with random labels.
...
It is because of the critical amount of required clean data that we have not attempted to train networks for α 100.
...
Consider the case of n training examples, and αn noisy labels that are sampled uniformly at random from the set {1, 2, . . .

, m}. Then, f (x) = f 0 (x) with probability 1 1+α , and otherwise it is 1, 2, . . .

, m, each with probability α m(1+α) .

As batch size increases, the expected value over the batch X is approximated more closely by these probabilities.

In the limit of infinite batch size, equation FORMULA1 takes the form of a noisy loss function H α : DISPLAYFORM2 We can therefore compare training using the cross-entropy loss with αn noisy labels to training using the noisy loss function H α without noisy labels.

The term on the right-hand side of (3) represents the noise contribution, and is clearly minimized whereŷ k are all equal.

As α increases, this contribution is weighted more heavily against − logŷ f0(x) X , which is minimized atŷ(x) = y(x).We show in Figure 9 the results of training our 2-layer ConvNet on MNIST with the noisy loss function H α , simulating αn noisy labels with infinite batch size.

We can observe that the network's accuracy does not decrease as α increases.

This can be explained by the observation that an increasing α is merely decreasing the magnitude of the true gradient, rather than altering its direction.
