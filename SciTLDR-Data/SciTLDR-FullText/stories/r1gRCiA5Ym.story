...
We study a feed-forward deep neural networks of the form: DISPLAYFORM0 where W j is the weight matrix for layer j, ψ j is the corresponding activation function (ReLU in this paper), x ∈ X is an input data point of d in dimensions andŷ(x) is the network's output prediction of d out dimensions, e.g., the logits before applying softmax.

We denote the hidden nodes on layer j to be h j , i.e., h j = W j ψ j−1 (W j−1 ψ j−2 (. . .

ψ 1 (W 1 x))), whose dimensionality is d j ; they represent the nodes after applying the activation function ash j = ψ(h j ).The above DNN formalization can generalize many DNN architectures used in practice.
...
FORMULA0 can be written as a piecewise linear function, i.e., the DNN in a region surrounding a given data point x is a linear model having the following form:ŷ DISPLAYFORM1 where W x j is the equivalent weight matrix after combining the resultant activation pattern with W j .

For instance, suppose we use ReLU activation ψ ReLU (z) = max(0, z); at every layer, we have an activation pattern for the input a j (x) ∈ {0, 1} dj , and a j (x)[p] = 0 indicates that ReLU sets the unit p to 0 or otherwise preserves the unit value.

Then, ψ ReLU (W 1 x) = W x 1 x, where W x 1 is modified from W 1 by setting the rows, whose corresponding activation patterns are 0, to be all zero vectors.
...
In particular, we firstly sample p ∼ N (0, σ) from a Gaussian distribution, and then take the absolute value |p| as the dropout rate.
...
We utilize the standard deviation σ as the hyper-parameter to control the amount of generalization enforcement.
...
Jumpout requires one main hyper-parameter σ to control the standard deviation of the half-normal distribution, and two auxiliary truncation hyperparameters (p min , p max ).
...
Hence, jumpout has three hyperparameters, although we only tuned σ and achieved good performance, as can be seen below.
