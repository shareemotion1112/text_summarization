...
This paper assumes a discrete-time finite-horizon Markov decision process (MDP), defined by (S, A, f, r, ρ 0 , T ), in which S ⊆ R n is the state space, A ⊆ R m the action space, f : S × A → S a deterministic transition function, r : S × A → R a bounded reward function, ρ 0 : S → R + an initial state distribution, and T the horizon.

We denote a stochastic policy π θ (a|s) as the probability of taking action a at state s. Let η(θ) denote its expected return: DISPLAYFORM0 where τ = (s 0 , a 0 , . . .

, a T −1 , s T ) denotes the whole trajectory, s 0 ∼ ρ 0 (.), a t ∼ π θ (.|s t ), and s t+1 = f (s t , a t ) for all t. We assume that the reward function is known but the transition function is unknown.

Our goal is to find an optimal policy that maximizes the expected return η(θ).
...
During training, model-based methods maintain an approximate MDP,M, where the transition function is given by a parameterized modelf φ learned from data.

The policy is then updated with respect to the approximate MDP.

Hence, the objective we maximize iŝ DISPLAYFORM1 DISPLAYFORM2 , and s t+1 =f φ (s t , a t ).We represent the stochastic policy 4 as a conditional multivariate normal distribution with a parametrized mean µ θ : S → A and a parametrized standard deviation σ θ : S → R m .

Using the re-parametrization trick , we can write down an action sampled from π θ at state s as µ θ (s) + σ θ (s)T ζ, where ζ ∼ N (0, I m ).

Given a trajectoryτ sampled using the policy π θ , we can recover the noise vectors {ζ 0 , ..., ζ T }.

Thus, the gradient of the objectiveη(θ; φ) can simply be estimated by Monte-Carlo methods: DISPLAYFORM3 This method of gradient computation is called backpropagation through time (BPTT), which can be easily performed using an automatic differentiation library.
...
We perform the updates until the policy no longer improves its estimated performanceη over a period of time (controlled by a hyperparameter), and then we repeat the process in the outer loop by using the policy to collect more data with respect to the real model 5 .
...
Since we are improving Collect samples from the real environment f using π θ and add them to D.

Train the modelf φ using D.6: DISPLAYFORM0 Collect fictitious samples fromf φ using π θ .

Update the policy using BPTT on the fictitious samples.

Estimate the performanceη(θ; φ).
...
First, we fit a set of dynamics models {f φ1 , . . .

, f φ K } (termed a model ensemble) using the same real world data.
...
Collect samples from the real system f using π θ and add them to D.

Train all models using D.

repeatOptimize π θ using all models.

Collect fictitious samples from {f φi } K i=1 using π θ .

Update the policy using TRPO on the fictitious samples.

Estimate the performancesη(θ; φ i ) for i = 1, ..., K.
...
The policy is trained with TRPO on the learned models using initial standard deviation 1.0, step size δ KL 0.01, and batch size 50000.
...
The step size δ KL that we used in all the experiments was of 0.05.Proximal Policy Optimization .
