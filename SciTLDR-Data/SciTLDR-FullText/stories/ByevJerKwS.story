...
After training, we aim to find the latent vectorẑ that achieves highest likelihood under the target network while being constrained to the data manifold learned by G. However, if not properly designed, the generator may not allow the target network to easily distinguish between different latent vectors.
...
To train the GAN in the first stage of our attack pipeline, we set λ d = 0.5 and use the Adam optimizer with the learning rate 0.004, batch size 64, β 1 = 0.5, and β 2 = 0.999 (Kingma and Ba, 2014) .

In the second stage, we set λ i = 100 and use the SGD optimizer to optimize the latent vector z with the learning rate 0.01, batch size 64 and momentum 0.9.
...
We investigate the implications of DP for MI attacks. ( , δ)-DP is ensured by adding Gaussian noise to clipped gradients in each training iteration Abadi et al. (2016) .
...
We set δ = 10 −5 and vary the noise scale to obtain target networks with different .
