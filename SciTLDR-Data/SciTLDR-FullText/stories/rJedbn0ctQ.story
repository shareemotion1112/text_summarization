...
BID10 proposesà la carte to learn a linear mapping to reconstruct the center word from its context.
...
From QR factorization theory, the novel orthogonal basis q i is also the normalized residual in the least square problem min Cx − v wi 2 , i.e. q It follows that α n is the exponential of the normalized distance between v wi and the subspace spanned by its context.
...
Their corresponding singular values {σ j } n j=1 , denoted by σ(A), represent the importance associated with {U :,j } n j=1 .
...
This can be quantified as σ(S i ) (q DISPLAYFORM1 , where denotes element-wise product.

Therefore, we define the significance of w i in its context to be: DISPLAYFORM2 It turns out α s can be rewritten as DISPLAYFORM3 We use the fact that V i is an orthogonal matrix and q i is orthogonal to all but the last column of S i , v wi .

Therefore, α s is essentially the distance between w i and the context hyper-plane, normalized by the context size.
...
Then the coarse-grained embedding for the ith sentence is defined as: DISPLAYFORM0 where f (σ j ) is a monotonically increasing function.
...
A sentence vector c s is computed as a weighted sum of its word embeddings, where the weights come from three scores: a novelty score (α n ), a significance score (α s ) and a corpus-wise uniqueness score (α u ).
...
It also compares favorably with most of parameterized models, includingà la carte BID10 , FastSent BID8 , InferSent, QT, Sent2Vec, SkipThought-LN (with layer normalization) BID11 , SDAE BID8 , STN BID23 and USE .
...
As shown in in Table 4 , every GEM weight (α n , α s , α u ) and proposed principal components removal methods contribute to the performance.
...
For example, three weights altogether improve the score in SUBJ task by 0.38% compared with only using α n .

Sensitivity Study.

We evaluate the effect of all four hyper-parameters in our model: the window size m in the contextual window matrix, the number of candidate principal components K, the number of principal components to remove h, and the power of the singular value in coarse sentence embedding, i.e. the power t in f (σ j ) = σ t j in Equation FORMULA9 .
...
The novelty score (α n ), significance score (α s ) and corpus-wise uniqueness score (α u ) are larger when a word w has relatively rare appearance in the corpus and can bring in new and important semantic meaning to the sentence.
...
Then when Z is sufficiently large, the MLE for c s is: The joint probability of sentence s is then To simplify the notation, let α = α n + α s + α u .

It follows that the log likelihood f (w i ) of word w i emitted from sentence s is given by f wi (c s ) = log( exp( c s , v wi ) Z + e −α ) − log(N )
