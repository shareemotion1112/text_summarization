...
For the utility, the notationÕ(·) hides the same constant and log factors for each bound.

The constants d and n denote the dimension of the model's parameters and the number of training points, respectively.

The numbers γ and β are positive constants that are strictly less than one, and D 0 , D σ , G are positive constants, which will be defined in Section 3.

σ , we will discuss this in detail in Section 4.
...
For vectors x and y and positive definite matrix A, we use x 2 and x A to denote the 2 -norm and the induced norm by A, respectively; x, y denotes the inner product of x and y; and λ i (A) denotes the i-th largest eigenvalue of A. We denote the set of numbers from 1 to n by [n] .
...
where η is the learning rate, ∇f i k denotes the stochastic gradient of F evaluated from the pair of input-output {x i k , y i k }, and B k is a random subset of size b from [n].

Let A σ = I − σL for σ ≥ 0 being a constant, where I ∈ R d×d and L ∈ R d×d are the identity and the discrete one-dimensional Laplacian matrix with periodic boundary condition, respectively.

Therefore,

When σ = 0, LSSGD reduces to SGD.

Note that A σ is positive definite with condition number 1 + 4σ that is independent of A σ 's dimension, and LSSGD guarantees the same convergence rate as SGD in both convex and nonconvex optimization.
...
In this scheme, we first inject the noise n to the stochastic gradient ∇f i k (w k ), and then apply the LS operator A −1 σ to denoise the noisy stochastic gradient, ∇f i k (w k ) + n, on-the-fly.
...
initial guess of w, ( , δ): the privacy budget, η: the step size, T : the total number of iterations.

and ν is defined in Theorem 1, and
...
Definition 1 (( , δ)-DP). (Dwork et al. (2006) ) A randomized mechanism M : S N → R satisfies ( , δ)-DP if for any two adjacent datasets S, S ∈ S N differing by one element, and any output subset O ⊆ R, it holds that

Theorem 1 (Privacy Guarantee).

Suppose that each component function f i is G-Lipschitz.

Given the total number of iterations T , for any δ > 0 and privacy budget 2 ≤ 5T log(1/δ)b 2 /n 2 , DP-LSSGD, with injected Gaussian noise N (0, ν 2 ) for each coordinate, satisfies ( , δ)-DP with ν 2 = 8T αG 2 /(n 2 ), where α = 2 log(1/δ)/ + 1.

Remark 1.

It is straightforward to show that the noise in Theorem 1 is in fact also tight to guarantee the ( , δ)-DP for DP-SGD.
...
Aσ and w * is the global minimizer of F , the DP-LSSGD outputw =

, where ω = 2σ+1− √ 4σ+1 2σ < 1.

That is, γ converge to 0 almost exponentially as the dimension, d, increases.

Remark 2.

In the above utility bound for convex optimization, for different σ (σ = 0 corresponds to DP-SGD), the only difference lies in the term γ(D σ + G 2 ).

The first part γD σ depends on the gap between initialization w 0 and the optimal solution w * .

The second part γG 2 decrease monotonically as σ increases.

σ should be selected to get an optimal trade-off between these two parts.

Based on our test on multi-class logistic regression for MNIST classification, σ = 0 always outperforms the case when σ = 0.
...
In the 2 -norm, DP-LSSGD has a bigger utility upper bound than DP-SGD (set σ = 0 in ζ).
...
We see that the training loss curve of DP-SGD (σ = 0) is much higher and more oscillatory (log-scale on the y-axis) than that of DP-LSSGD (σ = 1, 3).
...
Next, consider the testing accuracy of the multi-class logistic regression trained with ( , 10 −5 )-DP guarantee by DP-LSSGD includes σ = 0, i.e., DP-SGD.

We list the test accuracy of logistic regression trained in different settings in Table 2 .

These results reveal that DP-LSSGD with σ = 1, 2, 3 can improve the accuracy of the trained private model and also reduce the variance, especially when the privacy guarantee is very strong, e.g., (0.1, 10 −5 ).
...
Next, we consider the effects of the LS constant (σ) and the learning rate in training the DP-CNN for MNIST classification.

We fixed the NM to be 10, and run 60 epochs of DP-SGD and DP-LSSGD with different σ and different learning rate.

We show the comparison of DP-SGD with DP-LSSGD with different σ in the left panel of Figure 6 , and we see that as σ increases it becomes more stable in training CNNs with DP-guarantee even though initially it becomes slightly slower.
...
For α > 1 and ρ > 0, a randomized mechanism M : S n → R satisfies (α, ρ)-Rényi DP, i.e., (α, ρ)-RDP, if for all adjacent datasets S, S ∈ S n differing by one element, we have

where the expectation is taken over M(S ).

Lemma 1. )

Given a function q : S n → R, the Gaussian Mechanism M = q(S) + n, where n ∼ N (0, ν 2 I), satisfies (α, α∆ 2 (q)/(2ν 2 ))-RDP.
...
If a randomized mechanism M : S n → R satisfies (α, ρ)-RDP, then M satisfies (ρ + log(1/δ)/(α − 1), δ)-DP for all δ ∈ (0, 1).

With the definition (Def.

3) and guarantees of RDP (Lemmas 1 and 2), and the connection between RDP and ( , δ)-DP (Lemma 3), we can prove the following DP-guarantee for DP-LSSGD.
...
We will show that with the aforementioned Gaussian noise N (0, ν 2 ) for each coordinate of n, the output of DP-SGD,w, after T iterations is ( , δ)-DP.
...
the mechanism M k will satisfy α, (n 2 /b 2 ) log(1/δ)/ 2(α − 1)T -RDP.

By post-processing theorem, we immediately have that under the same noise,

According to Lemma 1,M k will satisfy α, log(1/δ)/(α−1)T -RDP provided that ν 2 ≥ 1/1.25, because τ = b/n.

Let α = 2 log(1/δ)/ + 1, we obtain thatM k satisfies 2 log(1/δ)/ + 1, /(2T ) -RDP as long as we have

Therefore, the following condition suffices

Therefore, according to Lemma 2, we have w k satisfies 2 log(1/δ)/ + 1, k /(2T ) -RDP.

Finally, by Lemma 3, we have w k satisfies k /(2T ) + /2, δ -DP.

Therefore, the output of DP-SGD,w, is ( , δ)-DP.

Remark 3.

In the above proof, we used the following estimate of the 2 sensitivity

σ g, then according to Osher et al. (2018) we have
...
, where i k are drawn uniformly from [n], and n ∼ N (0, ν 2 I).
...
where the second inequality uses Lemma 5 and the last inequality is due to 1 − η k L/2 > 1/2.

Now taking the full expectation and summing up over T iterations, we have

2 .

If we choose fix step size, i.e., η k = η, and rearranging the above inequality, and using
...
; therefore, it suffices to compute (11)) for nonnegative k. Set z = e iθ .

Observe that cos(θ) = 0.5(z + 1/z) and dz = izdθ.

Substituting in (11) and simplifying yields that

where the integral is taken around the unit circle, and

are the roots of quadratic −σz 2 + (2σ + 1)z − σ.

Note that α − lies within the unit circle; whereas, α + lies outside of the unit circle.

Therefore, because k is nonnegative, α − is the only singularity of the integrand in (12) within the unit circle.
...
First observe that we can re-write γ as

It remains to show that the above summation is equal to

.

This follows by lemmas 7 and standard sampling results in Fourier analysis (i.e. sampling θ at points {2πj/d}
