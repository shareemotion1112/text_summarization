...
We show that the improved performance is due to the increased diversity of reformulation strategies.

Reinforcement learning has proven effective in several language processing tasks, such as machine translation BID12 Ranzato et al., 2015; BID1 , question-answering BID9 Hu et al., 2017) , and text summarization (Paulus et al., 2017) .

In reinforcement learning efficient exploration is key to achieve good performance.
...
Therefore, we evaluate the proposed method on the same tasks they used: query reformulation for document retrieval and question-answering.
...
Recently, Shazeer et al. (2017) revisited the idea and showed strong performances in the supervised learning tasks of language modeling and machine translation.
...
Previous works used specialized agents to improve exploration in RL (Dayan & Hinton, 1993; Singh, 1992; Kaelbling et al., 1996) .

For instance, BID5 and Conti et al. (2017) use a population of agents to achieve a high diversity of strategies that leads to better generalization performance and faster convergence.

Rusu et al. (2015) use experts to learn subtasks and later merge them into a single agent using distillation (Hinton et al., 2015) .The experiments are often carried out in simulated environments, such as robot control (Brockman et al., 2016) and video-games (Bellemare et al., 2013) .
...
We, instead, evaluate our approach on tasks whose inputs (queries) and states (documents and answers) are diverse because they are in natural language, and the environment responses are slow (0.5-5 seconds per query).Somewhat similarly motivated is the work of Serban et al. (2017) .

They train many heterogeneous response models and further train an RL agent to pick one response per utterance.

We describe the proposed method using a generic end-to-end search task.
...
Following Nogueira & Cho (2017) and Buck et al. (2018b) we frame the task as a reinforcement learning problem, in which the query reformulation system is an RL-agent that interacts with an environment that provides answers and rewards.

The goal of the agent is to generate reformulations such that the expected returned reward (i.e., correct answers) is maximized.

The environment is treated as a black-box, i.e., the agent does not have direct access to any of its internal mechanisms.

Figure 1 -(b) illustrates this framework.3.2 SYSTEM Figure 1 -(c) illustrates the new agent.

An input query q 0 is given to the N sub-agents.

A subagent is any system that accepts as input a query and returns a corresponding reformulation.
...
Reformulators are independently trained on disjoint partitions of the dataset thus increasing the variability of reformulations.
...
The first step for training the new agent is to partition the training set.

We randomly split it into equal-sized subsets.

For an analysis of how other partitioning methods affect performance, see Appendix F. In our implementation, a sub-agent is a sequence-to-sequence model BID6 Cho et al., 2014) trained on a partition of the dataset.
...
We then use REINFORCE BID11 to train the sub-agent.
...
The relevance score s R j is the prediction that the result a j is relevant to query q 0 .

It is computed as: DISPLAYFORM1 where DISPLAYFORM2 W 1 ∈ R 4D×D and W 2 ∈ R D×1 are weight matrices, b 1 ∈ R D and b 2 ∈ R 1 are biases.
...
We now present experiments and results in a document retrieval task.

In this task, the goal is to rewrite a query so that the number of relevant documents retrieved by a search engine increases.

The environment receives a query as an action, and it returns a list of documents as an observation/state and a reward computed using a list of ground truth documents.

We use Lucene 2 in its default configuration as our search engine, with BM25 as the ranking function.

The input is a query and the output is a ranked list of documents.

To train and evaluate the models, we use three datasets: TREC-CAR: Introduced by Dietz et al. (2017) , in this dataset the input query is the concatenation of a Wikipedia article title with the title of one of its section.
...
MSA: Introduced by Nogueira & Cho (2017), this dataset consists of academic papers crawled from Microsoft Academic API.3 A query is the title of a paper and the ground-truth answer consists of the papers cited within.

Each document in the corpus consists of its title and abstract.

Since the main goal of query reformulation is to increase the proportion of relevant documents returned, we use recall as the reward: R@K = |D K ∩D * | |D * | , where D K are the top-K retrieved documents and D * are the relevant documents.

We also experimented using as a reward other metrics such as NDCG, MAP, MRR, and R-Precision but these resulted in similar or slightly worse performance than Recall@40.
...
For results in other metrics, see Appendix A. PRF: This is the pseudo relevance feedback method (Rocchio, 1971) .
...
RELEVANCE MODEL (RM3): This is our implementation of the relevance model for query expansion (Lavrenko & Croft, 2001 ).
...
RL-RNN: This is the sequence-to-sequence model trained with reinforcement learning from Nogueira & Cho (2017) .
...
At test time, we average the probability distributions of all the N agents at each time step and select the token with the highest probability, as done by BID6 .
...
It is similar to RL-N-Full but the multiple sub-agents are trained on random partitions of the dataset (see Figure 1 -(c)).BERT AGGREGATOR: We experimented replacing our simple aggregator with BERT (Devlin et al., 2018) , which recently achieved the state-of-the-art in a wide range of textual tasks.
...
We train it with the same objective used to train our aggregator (Equation 3).

To compare how well our proposed reformulation agents perform against the best non-neural reformulation method, we implemented two variants of the system described here.

One is when the initial list of candidate documents a j is given by RM3 (RM3 + BERT Aggregator), and the other is by RL-10-Sub (RL-10-Sub + BERT Aggregator).
...
Compared to the top-performing system in the TREC-CAR 2017 Track (MacAvaney et al., 2017) , an RL-10-Full with an ensemble of 10 aggregators yields a relative performance improvement of approximately 20%.By replacing our aggregator with BERT, we improve performance by 50-100% in all three datasets (RL-10-Sub + BERT Aggregator).

This is a remarkable improvement given that we used BERT without any modification from its original implementation.

Without using our reformulation agents, the performance drops by 3-10% (RM3 + BERT Aggregator).For an analysis of the aggregator's contribution to the overall performance, see Appendix C.NUMBER OF SUB-AGENTS: We compare the performance of the full system (reformulators + aggregator) for different numbers of agents in FIG3 .

The performance of the system is stable across all datasets after more than ten sub-agents are used, thus indicating the robustness of the proposed method.

For more experiments regarding training stability, see Appendix D.

To further assess the effectiveness of the proposed method, we conduct experiments in a questionanswering task, comparing our agent with the active question answering agent proposed by Buck et al. (2018b) .The environment receives a question as an action and returns an answer as an observation, and a reward computed against a ground truth answer.

We use either BiDAF (Seo et al., 2016) or BERT (Devlin et al., 2018) as a question-answering system.

Given a question, it outputs an answer span from a list of snippets.

We use as a reward the token level F1 score on the answer (see Section 5.3 for its definition).We follow Buck et al. (2018b) to train BiDAF and BERT.

We emphasize that their parameters are frozen when we train and evaluate the reformulation system.
...
We compare our agent against the following baselines and benchmarks: BIDAF/BERT: The original question is given to the question-answering system without any modification (see Figure 1-(a) ).AQA: This is the best model from Buck et al. (2018b) .

It consists of a reformulator and a selector.

The reformulator is a subword-based sequence-to-sequence model that produces twenty reformulations of an input question using beam search.

The answers for the original question and its reformulations are obtained from BiDAF.

These are given to the selector which then chooses one of the answers as final (see Figure 1-(b) ).

The reformulator is pretrained on translation and paraphrase data.

AQA-N-{FULL, SUB}: Similar to the RL-N-{Full, Sub} models, we use AQA reformulators as the sub-agents followed by an aggregator to create AQA-N-Full and AQA-N-Sub models, whose sub-agents are trained on the full and random partitions of the dataset, respectively.

For the training and hyperparameter details, see Appendix B.2.

Table 2 : Main result on the question-answering task (SearchQA dataset).

We did not include the training cost of the aggregator (0.2 days, 0.06 ×10 18 FLOPs).

F1: We use the macro-averaged F1 score as the main metric.
...
When using BiDAF as the underlying Q&A system, the proposed method (AQA-10-{Full, Sub}) have both better F1 and oracle performances than the single-agent AQA method, while training in one-tenth of the time.

Even when the ensemble method is given ten times more training time (AQA-10-Full, extra budget), our method achieves a higher performance.

We achieve the state-of-the-art on SearchQA by using BERT without any modification from its original implementation.
...
Here we evaluate how query diversity and performance are related.

For that, we use four metrics (defined in Appendix E): pCos, pBLEU, PINC, and Length Std.

TAB5 shows that the multiple agents trained on partitions of the dataset (AQA-10-Sub) produce more diverse queries than a single agent with beam search (AQA) and multiple agents trained on the full training set (AQA-10-Full).
...
One interesting orthogonal extension would be to introduce diversity on the beam search decoder BID8 Li et al., 2016) , thus shedding light on the question of whether the gains come from the increased capacity of the system due to the use of the multiple agents, the diversity of reformulations, or both.

Following Dietz et al. FORMULA1 , we report the results on four standard TREC evaluation measures: RPrecision (R-Prec), Mean-average Precision (MAP), Reciprocal Rank (MRR), and Normalize Discounted Cumulative Gain (NDCG).

We also include Recall@40 as this is the reward our agents are optimizing for.

The results for TREC-CAR, Jeopardy, and MSA are in TAB7 AGGREGATOR: The encoder f q0 is a word-level two-layer CNN with filter sizes of 9 and 3, respectively, and 128 and 256 kernels, respectively.
...
We present the results in TAB9 .
...
Reinforcement learning algorithms that use non-linear function approximators, such as neural networks, are known to be unstable BID7 Fairbank & Alonso, 2011; Pirotta et al., 2013; Mnih et al., 2015) .

Ensemble methods are known to reduce this variance (Freund, 1995; BID3 BID7 .
...
TAB12 shows four reformulation examples by various methods.
...
Parellelization of ensemble method in reinforement learning for query reformulation, speeding up training and improving the diversity of learnt freformulations

@highlight

The authors propose to train multiple distinct agents, each over a different subset of the training set.

@highlight

The authors propose an ensemble approach for query reformulation