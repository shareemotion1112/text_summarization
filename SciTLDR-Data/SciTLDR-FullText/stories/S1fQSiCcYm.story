...
A visualization of this idea is shown in FIG0 , where a smooth A critic network is fed interpolants and reconstructions and tries to predict the interpolation coefficient α corresponding to its input (with α = 0 for reconstructions).

The autoencoder is trained to fool the critic into outputting α = 0 for interpolants.
...
First, an input x ∈ R dx is passed through an "encoder" z = f θ (x) parametrized by θ to obtain a latent code z ∈ R dz .

The latent code is then passed through a "decoder"x = g φ (z) parametrized by φ to produce an approximate reconstructionx ∈ R dx of the input x. We consider the case where f θ and g φ are implemented as multi-layer neural networks.

The encoder and decoder are trained simultaneously (i.e. with respect to θ and φ) to minimize some notion of distance between the input x and the outputx, for example the squared L 2 distance x −x 2 .Interpolating using an autoencoder describes the process of using the decoder g φ to decode a mixture of two latent codes.

Typically, the latent codes are combined via a convex combination, so that interpolation amounts to computingx α = g φ (αz 1 +(1−α)z 2 ) for some α ∈ [0, 1] where z 1 = f θ (x 1 ) and z 2 = f θ (x 2 ) are the latent codes corresponding to data points x 1 and x 2 .
...
Ideally, adjusting α from 0 to 1 will produce a sequence of realistic datapoints where each subsequentx α is progressively less semantically similar to x 1 and more semantically similar to x 2 .
...
We find empirically that this constraint results in realistic and smooth interpolations in practice (section 3.1) in addition to providing improved performance on downstream tasks (section 4).To enforce this constraint we introduce a critic network BID12 which is fed interpolations of existing datapoints (i.e.x α as defined above).

Its goal is to predict α fromx α , i.e. to predict the mixing coefficient used to generate its input.

When training the model, for each pair of training data points we randomly sample a value of α to producex α .

In order to resolve the ambiguity between predicting α and 1 − α, we constrain α to the range [0, 0.5] when feedingx α to the critic.

In contrast, the autoencoder is trained to fool the critic to think that α is always zero.
...
Formally, let d ω (x) be the critic network, which for a given input produces a scalar value.

The critic is trained to minimize DISPLAYFORM0 where, as above, DISPLAYFORM1 for some x (not necessarily x 1 or x 2 ), and γ is a scalar hyperparameter.

The first term trains the critic to recover α from x α .
...
The autoencoder's loss function is modified by adding a regularization term: DISPLAYFORM2 where λ is a scalar hyperparameter which controls the weight of the regularization term.

Note that the regularization term is effectively trying to make the critic output 0 regardless of the value of α, thereby "fooling" the critic into thinking that an interpolated input is non-interpolated (i.e., having α = 0).

The parameters θ and φ are optimized with respect to L f,g (which gives the autoencoder access to the critic's gradients) and ω is optimized with respect to L d .
...
However, they are substantially different in that ours is used as a regularizer for autoencoders rather than for generative modeling and our critic attempts to regress the interpolation coefficient α instead of a fixed scalar hyperparameter.
...
It therefore will only be able to successfully recover α if the quality of the autoencoder's output degrades consistently across an interpolation as a function of α (as seen, for example, in fig. 3a where interpolated points become successively blurrier and darker).
...
Further, in contrast with the standard Generative Adversarial Network (GAN) setup BID12 , ACAI does not distinguish between "real" and "fake" data; rather, it simply attempts to regress the interpolation coefficient α.
...
For example, a definition of interpolation along the lines of "αz 1 + (1 − α)z 2 should map to αx 1 + (1 − α)x 2 " is overly simplistic because interpolating in "data space" often does not result in realistic datapointsin images, this corresponds to simply fading between the pixel values of the two images.
...
We consider 16-pixel-long lines beginning from the center of the image and extending outward at an angle Λ ∈ [0, 2π] (or put another way, lines are radii of the circle circumscribed within the image borders).

An example of 16 such images is shown in FIG2 (appendix A.1).

In this task, the data manifold can be defined entirely by a single variable: Λ. We can therefore define a valid interpolation from x 1 to x 2 as one which smoothly and linearly adjusts Λ from the angle of the line in x 1 to the angle in x 2 .
...
In this setup, f θ and g φ are both neural networks which respectively map the input to a deterministic latent code z and then back to a reconstructed input.

Typically, f θ and g φ are trained simultaneously with respect to x −x 2 .
...
While various corruption procedures q(x|x) have been used such as masking and salt-and-pepper noise, in this paper we consider the simple case of additive isotropic Gaussian noise wherex ∼ N (x, σ 2 I) and σ is a hyperparameter.

After tuning σ, we found simply setting σ = 1.0 to work best.
...
The encoder f θ can then be considered an approximation to the posterior p(z|x).

Then, the decoder g φ is taken to parametrize the likelihood p(x|z); in all of our experiments, we consider x to be Bernoulli distributed.
...
VAEs then use log-likelihood for the reconstruction loss (cross-entropy in the case of Bernoulli-distributed data), which results in the following combined loss function: −E[log g φ (z)] + KL(f θ (x)||p(z)) where the expectation is taken with respect to z ∼ f θ (x) and KL(·||·) is the KL divergence.

Minimizing this loss function can be considered maximizing a lower bound (the "ELBO") on the likelihood of the training set, producing a likelihood-based generative model which allows novel data points to be sampled by first sampling z ∼ p(z) and then computing g φ (z).

A common choice is to let q(z|x) be a diagonalcovariance Gaussian, in which case backpropagation through sampling from q(z|x) is feasible via the "reparametrization trick" which replaces z ∼ N (µ, σI) with ∼ N (0, I), z = µ + σ where µ, σ ∈ R dz are the predicted mean and standard deviation produced by f θ .
...
Instead of minimizing a KL divergence between the distribution of latent codes and a prior distribution, a critic network is trained in tandem with the autoencoder to predict whether a latent code comes from f θ or from the prior p(z).
...
In the VQ-VAE, the encoder f θ (x) produces a continuous hidden representation z ∈ R d z which is then mapped to z q , its nearest neighbor in a "codebook" {e j ∈ R dz , j ∈ 1, . . .

, K}. z q is then passed to the decoder for reconstruction.

The encoder is trained to minimize the reconstruction loss using the straight-through gradient estimator BID2 , together with a commitment loss term β z − sg(z q ) (where β is a scalar hyperparameter) which encourages encoder outputs to move closer to their nearest codebook entry.
...
For the hyperparameters λ and γ we found values of 0.5 and 0.2 to achieve good results, though the performance was not very sensitive to their values.
...
We define our Mean Distance and Smoothness metrics as follows: Let x 1 and x 2 be two input images we are interpolating between and DISPLAYFORM0 be the decoded point corresponding to mixing x 1 and x 2 's latent codes using coefficient α = n−1 /N−1.
...
Finding the closest image among all possible line images is infeasible; instead we first generate a size-D collection of line images D with corresponding angles Λ q , q ∈ {1, . . .

, D} spaced evenly between 0 and 2π.
...
Note that if, for example, the interpolated lines go from Λ q 1 = π /10 to Λ q N = 19π /10 then the angles corresponding to the shortest path will have a discontinuity from 0 to 2π.

To avoid this, we first "unwrap" the angles {Λ q 1 , . . .

, Λ q N } by removing discontinuities larger than π by adding multiples of ±2π when the absolute difference between Λ q n−1 and Λ q n is greater than π to produce the angle sequence {Λ q 1 , . . .
