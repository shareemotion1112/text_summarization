...
Algorithm 1 demonstrates a slightly more unusual formulation in which a pretrained MLE model π θ is combined with an arbitrary reward function R to maximize reward by alternately sampling and retraining via Eq. 9 on the k-highest reward sequences BID32 , which we call Hillclimb-MLE (HC-MLE) .
...
From an RL perspective, each symbol in the SMILES sequence corresponds to an action a t taken at sequence step t. The goal of an RL agent here is to develop a policy π θ , parameterized by θ, to calculate an action a t from the current state s t = (a 1 , . . . , a t−1 ) that maximizes the expected reward.
...
This can be seen as an actor-critic model that separates the policy training from the value estimation in which the actor is the policy π θ and the baseline value estimate V st is the critic that gives the approximate value of that state BID6 BID33 BID22 .
