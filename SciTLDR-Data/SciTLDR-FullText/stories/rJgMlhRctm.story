...
Specifically, we compute the probability that an object o i is a cube by σ ShapeOf(o i ), v Cube − γ τ , where ShapeOf(·) denotes the neural operator, v Cube the concept embedding of Cube and σ the Sigmoid function.

γ and τ are scalar constants for scaling and shifting the values of similarities.
...
Our goal is to find the optimal parameters Θ v of the visual perception module Perception (including the ResNet-34 for extracting object features, attribute operators.

and concept embeddings) and Θ s of the semantic parsing module SemanticParse, to maximize the likelihood of answering the question Q correctly: DISPLAYFORM0 where P denotes the program, A the answer, S the scene, and Executor the quasi-symbolic executor.

The expectation is taken over P ∼ SemanticParse(Q; Θ s ).Recall the program executor is fully differentiable w.r.t.

the visual representation.

We compute the gradient w.r.t.

DISPLAYFORM1 We use RE-INFORCE (Williams, 1992) to optimize the semantic parser Θ s : DISPLAYFORM2 , where the reward r = 1 if the answer is correct and 0 otherwise.
...
Object (i.e., the Unique operation), we compute: Object = softmax(σ −1 (ObjectSet)), where σ −1 (x) = log(x/(1 − x)) is the logit function.
...
To classify the objects as being Red or not, we compute: DISPLAYFORM0 where σ denotes the Sigmoid function, ·, · the cosine distance between two vectors.
...
Θ s , the parameters of the semantic parser: DISPLAYFORM0 i ) DISPLAYFORM1 i | + γc)/(γc · τc) Table 9 : All operations in the domain-specific language for CLEVR VQA.

γ c = 0.5 and τ c = 0.25 are constants for scaling and shift the probability.

During inference, one can quantify all operations as Yi et al. (2018) .where P ∼ SemanticParse(s; Θ s ).
...
In the third stage of the curriculum learning, we search for the set Q offline based on the quantified results of concept classification and compute the exact gradient ∇Θ s .
