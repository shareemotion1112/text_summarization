...
In particular, the architecture is given by DISPLAYFORM2 where φ is last layer, or possibly last few layers, and θ constitutes all of the remaining layers.

We will refer to φ colloquially as the "classifier", and to θ as the "features" or "representation".

(This is consistent with the usage in related papers, see for example .)

Now, the source and target will share features, but not classifiers; that is, the source network will be given by F (x; θ, φ s ), whereas the target network will be F (x; θ, φ t ).

The features θ are shared between the two, and this is what allows for transfer learning.

The weighted source loss is given by DISPLAYFORM3 where α j ∈ [0, 1] is a weight assigned to each source training example; and (·, ·) is a per example classification loss, in this case cross-entropy.

The use of the weights α j will allow us to decide which source images are most relevant for the target classification task.
...
In the latter case, the source is the noisy dataset, the target is the clean dataset, and they share a classifier (i.e. φ t = φ s ) as well as a label space; high weights will indicate which source examples do not have label noise, and are therefore reliable.
...
Specifically, in the interior level we find the optimal features and source classifier as a function of the weights α, by minimizing the source loss: DISPLAYFORM5 In the exterior level, we minimize the target loss, but only through access to the source weights; that is, we solve: min DISPLAYFORM6 Why might we expect this bilevel formulation to succeed?
...
Note that there will be an identical descent equation for the classifier φ s , which we omit for clarity.
...
We can then take a gradient descent step of this equation, yielding: DISPLAYFORM11 where in the final line, we have made use of the fact that λ p is small.

Of course, there will also be a descent equation for the classifier φ t .
...
That is, we would like to explicitly require that each α j ∈ [0, 1].

We may achieve this by requiring α j = σ(β j ) where the new variable β j ∈ R, and σ : R → [0, 1] is a sigmoid-type function.

As shown in Appendix A, for a particular piecewise linear sigmoid function, replacing the Update Equation (4) with a corresponding update equation for β is equivalent to modifying Equation (4) to read DISPLAYFORM12 where CLIP [0, 1] clips the values below 0 to be 0; and above 1 to be 1.

FORMULA7 and FORMULA12 , along with the descent equations for the source and target classifiers φ s and φ t .
...
Note that the target derivatives ∂L t /∂θ and ∂L t /∂φ t are evaluated over a target mini-batch; we suppress this for clarity.
...
Regarding space-complexity, in addition to the ordinary network parameters we need to store the source weights α.
...
In all experiments, we use the SGD optimizer without learning rate decay, and we use λ α = 1.

We initialize the α-values to be 1, and in practice clip them to be in the slightly expanded range [0, 1.1]; this allows more relevant source points some room to grow.
...
In FIG0 and 1(c), we choose a threshold of 0.1 on the weights α, and colour the points accordingly.
...
For SOSELETO, we use the following settings: λ p = 10 −4 , the target batch-size is 32, and the source batch-size is 256.

We use a larger source batch-size to enable more α-values to be affected quickly.
...
Furthermore, it does so in each case with only half of the clean samples used in BID33 BID39 .We perform further analysis by examining the α-values that SOSELETO chooses on convergence, see Figure 4 .2.

To visualize the results, we imagine thresholding the training samples in the source set on the basis of their α-values; we only keep those samples with α greater than a given threshold.
...
Looking at these plots, we see for example that for the 30% noise level, if we take the half of the training samples with the highest α-values, we are left with only about 4% which have incorrect labels.
...
For SOSELETO, we use the following settings: λ p = 10 −2 , the source batch-size is 32, and the target batch-size is 10 (it is chosen to be small since the target itself is very small).
...
Recall that our goal is to explicitly require that α j ∈ [0, 1].

We may achieve this by requiring DISPLAYFORM0 where the new variable β j ∈ R, and σ(·) is a kind of piecewise linear sigmoid function.

Now we will wish to replace the Update Equation (4), the update for α, with a corresponding update equation for β.

This is straightforward.

Define the Jacobian ∂α/∂β by ∂α ∂β ij = ∂α i ∂β jThen we modify Equation (4) to read DISPLAYFORM1 The Jacobian is easy to compute analytically: where CLIP [0, 1] clips the values below 0 to be 0; and above 1 to be 1.
...
DISPLAYFORM0 Now, we can use the evolution of the weights α.
