...
, r (N ) , ρ 0 , γ , where S is the set of states, A (i) represents the action space of agent i, where i ∈ {1, 2, . . .

, N }, P : S × A

(1) × A (2) × · · · × A (N ) × S → R is the state transition probability distribution, ρ 0 : S → R is the distribution of the initial state s 0 , and γ ∈ [0, 1] is the discounted factor.

Each agent i holds its policy

] to make decisions and receive rewards defined as r

We use −i to represent the set of agents except i, and variables without superscript i to denote the concatenation of all variables for all agents (e.g., π represents the joint policy and a denotes actions of all agents).

For an arbitrary function f : s, a → R, there is a fact that
...
) is the value function of agent i under state s, and Π (i) is the set of policies available to agent i.
...
In single-agent settings, such demonstrations come from behavior trajectories sampled with the expert policy, denoted as τ E = {(s t , a

.

However, in multi-agent settings, demonstrations are often interrelated trajectories, that is, which are sampled from the interactions of policies among all agents, denoted as Ω E = {(s t , a
...
) depends on others' actions, which makes its cumulative return subjected to the joint policy π.

One common joint policy modeling method is to decouple the π with assuming conditional independence of actions from different agents (Albrecht & Stone, 2018 ):
...
) is the conditional policy, with which agent i regards all potential actions from its opponent policies π (−i) (a (−i) |s), and makes decisions through the marginal policy

In multi-agent settings, for agent i with policy π (i) , it seeks to maximize its cumulative reward against demonstrator opponents who equip with demonstrated policies π (−i) E via reinforcement learning:

where H(π (i) ) is the γ-discounted entropy (Bloem & Bambos, 2014; Haarnoja et al., 2017) of policy π (i) and λ is the hyperparameter.
...
To solve this problem, we first introduce the occupancy measure, namely, the unnormalized distribution of s, a pairs correspond to the agent interactions navigated by joint policy π:

With the definition in Eq. (7), we can further formulate ρ π from agent i's perspective as
...
The IRL regarding demonstrator opponents is a dual form of following occupancy measure matching problem with regularizer ψ, and the induced optimal policy is the primal optimum:

With setting the regularizer ψ = ψ GA similar to Ho & Ermon (2016) , we can obtain a GAIL-like imitation algorithm to learn π
...
However, such an algorithm is not practical, since we are unable to access the policies of demonstrator opponents π
...
Let µ be an arbitrary function such that µ holds a similar form as π (−i) , then

Proof.

Substituting π (−i) with µ in Eq. (9) by importance sampling.
...
Thus, we fix α = 1 in our implementation, and as the experimental results have shown, it has no significant influences on performance.
...
where discriminator D (i) is parametrized by ω, and the policy π (i) is parametrized by θ.

It is worth noting that the agent i considers opponents' action a (−i) while updating its policy and discriminator, with integrating all its possible decisions to find the optimal response.

However, it is unrealistic to have the access to opponent joint policy π(a (−i) |s) for agent i. Thus, it is possible to estimate opponents' actions via approximating π (−i) (a (−i) |s) using opponent modeling.
...
Therefore, each agent i must infer the opponents model σ (i) to approximate the unobservable policies π (−i) , which can be achieved via supervised learning.
...
Since we fix the policies of agents −i as π

E , the RL procedure mentioned in Eq. (5) can be regarded as a single-agent RL problem.

Similarly, with a fixed π (−i) E , the IRL process of Eq. (6) is cast to a single-agent IRL problem, which recovers an optimal reward function r (i) * which achieves the best performance following the joint action π E .
...
Generally, random policies that keep vast entropy are not always considered as sub-optimal solutions or demonstrated policies π

E in most reinforcement learning environments.

As we do not require those random policies, we can remove them from the candidate policy set Π (i) , which indicates that H(π (i) ) is bounded in small values, so as .

Empirically, we adopt a small λ, and attain the demonstrator policy π E with an efficient learning algorithm to become a close-to-optimal solution.

Thus, we conclude that the objective of our CoDAIL assumes that demonstrated policies institute an -NE solution concept (but not necessarily unique) that can be controlled the hyperparameter λ under some specific reward function, from which the agent learns a policy.
...
Fig. 2 illustrates that the choice of λ has little effect on the total performance.
