...
The question is encoded to produce a query q 1 , which produces sentencelevel attention α and word-level attention β for each sentence.
...
DISPLAYFORM2 The product of α i β j can be viewed as a hierarchical way of achieving a word-level attention which normalizes over the passage.

When using multiple steps of reasoning, shared GRU encoders are used but different attention weight matrices W α and W β are used for each step.

At timestep t > 1, the query q t is a combination of the memory m (t−1) of last time step and the original encoded query q 1 , computed as: DISPLAYFORM3 Then, based on Equation 1, we can compute the sentence-level attention α, word-level attention β, and memory for each step.
...
With the attention weights α and β, our model exposes an explicit representation of what part of the passage it consults at each timestep.
...
We assume access to a series of subjectively "correct" sentence-level attention targets α * = (α * 1 , . . .

, α * n ) for the n steps of inference.

We can encourage the model to attend to these targets at each step by supervising the attention weight; this supervision can be accomplished by incorporating a loss term L = − t log(α * t ) We train with this extra supervision for the first several epochs during training, remove it after the model performance converges on the development set, then keep training only with the downstream supervision.
