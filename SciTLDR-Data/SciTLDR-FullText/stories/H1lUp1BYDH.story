...
, N , the goal of i th agent is to find a policy π θi : O i → ∆(A i ) that chooses optimal actions so as to maximize the long term reward, R i = t γ t r t i where r t i is the reward received by agent i at time-step t and γ ∈ (0, 1] is the discount factor.
...
Here, β assigns relative importance to agent's own rewards and the rewards of its neighbors (β ∈ (0, 1]) and U(i) is the set of neighbors of agent i. The agents are trained to maximize long term rewardsR i = t γ trt i .
...
Intuitively, an agent enquires about the unaccounted information (which it cannot model) in O i and the weights (α) are a way to understand the contributions of its neighbours in the state information.
...
Here W ∈ R d×d h , q ∈ R d and α ∈ R d are the attention parameters.
...
The arrival rate of agent A is modelled using Poisson, with rate λ, which is a function of the environment.

As agent B takes an action, the environment is no longer stationary and λ varies in a way that agent A is incapable of modelling (let us assume λ now becomes (λ − δλ)).

Hence, the agent learns to ignore these variables altogether.

In order to make sense of the unaccounted changes in its observation space, agent B has to communicate the difference in λ to agent A which can be assumed to be Poisson distributed with rate δλ (superposition of independent Poisson processes results in a new Poisson process whose rate is the sum of the rates of the independent Poisson processes); (ii) Coordination: Even while groups of agents implicitly come up with a common communication protocol, there should be a higher level harmony amongst all agents, i.e. the groups of agents in a network must systematically synchronize or coordinate to realize the same goal.
