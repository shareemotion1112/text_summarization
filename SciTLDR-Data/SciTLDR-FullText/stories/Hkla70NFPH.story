...
The set Ω can potentially impose a constraint over M , such as the very popular constraint of restricting it to be orthogonal (Xing et al., 2015) .
...
We use data from the TED (Qi et al., 2018) , OpenSubtitles (Lison and Tiedemann, 2016) , WikiMatrix (Schwenk et al., 2019) , bible (Malaviya et al., 2017) , and JW300 (Agić and Vulić, 2019) datasets.
...
For example, the MUSE Greek-English dictionary lists the word peaceful as the translation for all ειρηνικός, ειρηνική, ειρηνικό, ειρηνικά, which are the male, female, and neutral (singular and plural) inflections of the same adjective.
...
Coming back to our Greek-Italian example, this means that for the form ειρηνικός we would only keep pacifico as a candidate translation (we show more examples in Table 1 ).
...
µ best : average using the best hub language.

µ En : average using the En as the hub.
...
In fact, the average performance (over all evaluation pairs) when using En as the hub (denoted as µ En ) is 1.3 percentage points worse than the optimal (µ best ).
