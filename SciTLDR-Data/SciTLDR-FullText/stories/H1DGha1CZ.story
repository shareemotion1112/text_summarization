...
After the addition of the batch normalization layer, the overall joint transformation performed by the block (composed layer) is given by: DISPLAYFORM0 For a moment, consider the intermediate activation y produced inside the block: DISPLAYFORM1 Without loss of generality, assume γ = 1 and β = 0 BID11 .
...
Therefore, it can be written: DISPLAYFORM4 Considering that a distribution withÊ[ŷ (k) ] = 0 has to present negative values, replacing all negative activations with zeros makes the variance of z (k) necessarily lower than the variance of the original distributionŷ (k) .
...
It generalizes both ReLU and SReLU by allowing its inflection to move diagonally from the origin to any point of the form (−δ, −δ).

If δ = 0, DReLU becomes ReLU.

If δ = 1, DReLU becomes SReLU.
...
In fact, instead of always prolong the identity to the point (−1, −1), in DReLU we established a hyperparameter δ that defines the most appropriate point (−δ, −δ) where the inflection should happen.

In this sense, SReLU is a particular case of DReLU where δ = 1.

Considering that the experiments we performed to determine the DReLU hyperparameter contemplates δ = 1 as a possible value for δ, it was not necessary to include SReLU in the present comparative study.

Indeed, our experiments showed that the addition of the parameter δ allowed DReLU to significantly outperform SReLU in all of our hyperparameter definition experiments (Appendix C.6).
...
In this case, we can conclude thatÊ[z (k) ] DReLU is much probably near to zero thanÊ [z (k) ] ReLU .
...
The following equation defines LReLU: DISPLAYFORM1 LReLU has no zero slope if β = 0.
...
PReLU is also defined by the Eq. 11, but in this case β is a learnable rather than a fixed parameter BID6 FIG10 .
...
In fact, consider that the stochastic gradient decent (SGD) is being used to optimize the parameters θ of a deep model.
...
Therefore, we kept β = 0.25 for both LReLU and PReLU, and for ELU we maintained α = 1.0.
...
Clearly, δ must not go to infinity because we would lose the nonlinearity in such a case.
...
Based on these experimental results, we decided to set δ = 0.05.

The blue line presents the performance of SReLU, which can be observed is considerably lower than the DReLU using the chosen hyperparameter δ = 0.05.
