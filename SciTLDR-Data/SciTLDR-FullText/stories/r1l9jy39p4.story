...
However, in reference (5) a more pragmatic approach was taken to argue that a bound on the probability P (x) that x obtains upon random sampling of inputs can be approximated as DISPLAYFORM0 whereK(x) is a suitable approximation to the Kolmogorov complexity of x. Here a and b are constants that are independent of x and which can often be determined from some basic information about the map.

These constants pick up multiplicative and additive factors in the approximation to K(x) and to the O(1) terms.

In addition to the simplicity of the the input-output map f (condition (1)), the map also needs to obey conditions BID1 Redundancy: that the number of inputs N I is much larger than the number of outputs N O , as otherwise P (x) can't vary much; 3) Large systems where N O 0, so that finite size effects don't play a dominant role; 4) Nonlinear: If the map f is linear it won't show bias and 5) Well-behaved: The map should not have a significant fraction of pseudorandom outputs because it is hard to find good approximationsK(x).

For example many randomnumber generators produce outputs that appear complex, but in fact have low K(x) because they are generated by a relatively simple algorithms with short descriptions.

Some of the steps above may seem rather rough to AIT purists.

For example: Can a reasonable approximation to K(x) be found?

What about O(1) terms?

And, how do you know condition 5) is fulfilled?
...
Similar robust simplicity bies behaviour to that seen in this figure was observed for the other maps.

Similar scaling (5) was also observed for this map with a series of other approximations to K(x), suggesting that the precise choice of complexity measure was not critical, as long as it captures some basic essential features.

In summary then, the simplicity bias bound (1) works robustly well for a wide range of different maps.
...
Understanding the conditions resulting in biased maps is very much an open area of investigation.

The question we will address here is: Can deep learning be re-cast into the language of input-output maps, and if so, do these maps also exhibit the very general phenomenon of simplicity bias?2.
...
In order to explore the properties of the parameterfunction map, we consider random neural networks.
...
We consider Gaussian and uniform distributions over parameters.

In the following, when we say "probability of a function", we imply we 112 113 114 115 116 117 118 119 120 121 122 123 124 125 126 127 128 129 130 131 132 133 134 135 136 137 138 139 140 141 142 143 144 145 146 147 148 149 150 151 152 153 154 155 156 157 158 159 160 161 162 163 164 Simplicity bias in the parameter-function map of deep neural networks are using a Gaussian or uniform i.i.d.

distribution over parameters unless otherwise specified.

For a convolutional network with 4 layers and no pooling BID0 , we have used the Gaussian process approximation(7; 8) to estimate the probability of different labellings 2 on a random sample of 1000 images from CIFAR10.

We used the critical sample ratio (CSR) (4) as a measure of the complexity of the functions BID2 .

FIG1 , depicts the strong correlation between the log probability and CSR consistent with Eq. (1).In FIG7 , we also show the values of the log probability for a CNN and a FC network, on a larger sample of 10k images from CIFAR10, MNIST, and fashion-MNIST.

This illustrates the large range of orders of magnitude for the probability of different labellings, as well as the negative correlation with increasing label corruption.

While label corruption is not a direct measure of descriptional complexity, it is almost certainly the case that increasing label corruption generally corresponds to an increase in measures of Kolmogorov complexity.

Direct measurements of simplicity bias are extremely computationally expensive, and so far we have not been able to obtain directly sampled results for datasets larger than CIFAR10.To further isolate the phenomenon of simplicity bias in the parameter-function map, and study the effect of us-1 See Appendix A for more details on the architecture 2 See Appendix A.1 for details on the Gaussian process approximation BID2 See Appendix C for more details on complexity measures (a) (b) FIG7 .

log probability of a labelling on 10k random images on three different datasets, as the fraction of label corruption increases.

The probability is computed using the Gaussian process approximation for (a) a CNN with 4 layers and no pooling.

(b) a 1 hidden layer FC network.

ing different complexity measures, we look at a series of MLPs with 7 Boolean input neurons, 1 Boolean output neuron, and a number of layers of 40 hidden neurons each.

We use ReLU activation in all hidden layers.

For each such architecture, we sample the parameters i.i.d.

according to a Gaussian or uniform distribution.

We then count how often individual Boolean functions are obtained, and use the normalized empirical frequencies as estimate of their probability.

In FIG2 , we plot the probability (in log scale) versus the Lempel-Ziv complexity, which is defined in Appendix C.1.

In Appendix C.3, we show similar results for other complexity measures, demonstrating that the simplicity bias is robust to the choice of complexity measure, as long as it not too simple (like entropy).

The simplicity bias observed in FIG2 very closely resembles that seen for the other maps in (5).In FIG2 in Appendix C.3, we show the same data as in FIG2 , but as a histogram, highlighting that most of the probability mass, when sampling parameters, is close to the upper bound.

In addition, in FIG9 in Appendix D, we show the probability versus complexity plots for fully-165 166 167 168 169 170 171 172 173 174 175 176 177 178 179 180 181 182 183 184 185 186 187 188 189 190 191 192 193 194 195 196 197 198 199 200 201 202 203 204 205 206 207 208 209 210 211 212 213 214 215 216 217 218 219 Simplicity bias in the parameter-function map of deep neural networks connected networks with increasing number of layers, showing that the correlation is similar in all cases, although for deeper networks, the bias is a bit stronger.

In our experiments in the previous two sections, we observe that the range of probabilities of functions spans many orders of magnitude.

Here we show that for the experiment in Section 4, when we plot the probabilities versus the rank of the function (when ranked by probability), we find that the probabilities asymptotically follow Zipf's law: DISPLAYFORM0 where N O is the total number of functions.

This was also observed in (9) for networks of Boolean functions, and in (10) for more general circuits.

We formalize our findings for neural networks in the following conjecture:Conjecture 1 (Zipf's law beahviour) The probabilityrank relation of functions for sufficiently overparametrized neural networks follows Zipf's law (Equation 2)By "sufficiently over-parametrized" we mean that the networks are in the regime where the Gaussian process approximation to the distribution over function is a good approximation (which appears to be the case for many cases in practice (11; 8; 12) ).

We now explain the experiments we conducted to test this conjecture.

In FIG3 , we show the probabilities of the functions versus their rank (when ranked by probability), for different choices of parameter distribution.

We observe that for all the parameter distributions the probability-rank plot appears to approach Zipf's law.

To determine the parameter N O in Zipf's law, we checked that this architecture can produce almost all Boolean functions of 7 inputs, by finding that it could fit all the functions in a sample of 1000 random Boolean functions.

We thus used N O = 2 DISPLAYFORM1 in the Zipf's law curve in FIG3 , finding excellent agreement with no free parameters.

These results imply in particular that the distribution is extremely biased, with probabilities spanning a huge range of orders of magnitudes.

Note that the mean P (x) over all functions is 1/N O ≈ 3 × 10 −39 , so that in our plots we are only showing a tiny fraction of functions for which P (x) is orders of magnitude larger than the mean.

We can't reliably obtain a probability-rank plot for the experiments we did on CIFAR10, because our sampling of the space of functions is much sparser in this case.

How- are weight and bias variances, respectively, where n is the number of hidden neurons ever, the probabilities still span a huge range of orders of magnitude, as seen in FIG1 .

In fact, the range of orders of magnitude appears to scale with m, the size of the input space.

This is consistent with a probability-rank following Zipf's law.

The fact that neural networks are biased towards simple solutions has been conjectured to be the main reason they generalize (4; 3; 12).

Here we have explored a form of simplicity bias encoded in the parameter-function map.

It turns out that this bias is enough to guarantee good generalization, as was shown in (12) via a PAC-Bayesian formalism.

In FIG0 in Appendix B, we show the PACBayesian bounds versus the true generalization error they obtained, finding that the bounds are not only nonvacuous but follow the trends of the true error closely.
...
The filter sizes alternated between (2, 2) and BID4 BID4 , and the padding between SAME and VALID, the strides were 1 (same default settings as in the code for FORMULA0 ).

We used default Keras settings for initialization (Glorot uniform).

In recent work ((2; 3; 1; 4)), it was shown that infinitely-wide neural networks (including convolutional and residual networks) are equivalent to Gaussian processes.
...
distribution over parameters isP with zero mean, then for a set of n inputs (x 1 , ..., x n ), DISPLAYFORM0 whereỹ = (ỹ 1 , ...,ỹ n ).

The entries of the covariance matrix K are given by the kernel function k as DISPLAYFORM1 The kernel function depends on the choice of architecture, and properties ofP , in particular the weight variance σ 2 w /n (where n is the size of the input to the layer) and the bias variance σ 2 b .

The kernel for fully connected ReLU networks has a well known analytical form known as the arccosine kernel ( (5) ), while for convolutional and residual networks it can be efficiently computed BID0 .The main quantity in the PAC-Bayes theorem, P (U ), is precisely the probability of a given set of output labels for the set of instances in the training set, also known as marginal likelihood, a connection explored in recent work ((6; 7)).
...
Then, for a training set U = {(x 1 , y 1 ), ..., (x m , y m )}, DISPLAYFORM2 This distribution no longer has a Gaussian form because of the output nonlinearity σ.

We will discuss approximations to deal with this in the following.

There is also the more fundamental issue of neural networks not being infinitely-wide in practice.

However, the Gaussian process limit has been found to be a good approximation for nets with reasonable widths (3; 4; 8) .In order to calculate P (U ) using the GPs, we use the expectation-propagation (EP) approximation, implemented in (9), which is more accurate than the Laplacian approximation (see BID9 for a description and comparison of the algorithms).In (8) , the authors compare the two approximations and find that EP appears to give better approximations.

One of the key steps to practical application of the simplicity bias framework of Dingle et al. in FORMULA0 is the identification of a suitable complexity measureK(x) which mimics aspects of the (uncomputable) Kolmogorov complexity K(x) for the problem being studied.

It was shown for the maps in (11) that several different complexity measures all generated the same qualitative simplicity bias behaviour: DISPLAYFORM0 1 We use the code from (1) to compute the kernel for convolutional networks but with different values of a and b depending on the complexity measure and of course depending on the map, but independent of output x. Showing that the same qualitative results obtain for different complexity measures is sign of robustness for simplicity bias.

Below we list a number of different descriptional complexity measures which we used, to extend the experiments in Section 4 in the main text.
...
We use the variation of Lempel-Ziv complexity from FORMULA0 which is based on the 1976 Lempel Ziv algorithm ( FORMULA0 ): DISPLAYFORM0 where n is the length of the binary string, and N w (x 1 ...x n ) is the number of words in the Lempel-Ziv "dictionary" when it compresses output x.

The symmetrization makes the measure more fine-grained, and the log 2 (n) factor as well as the value for the simplest strings ensures that they scale as expected for Kolmogorov complexity.
...
For a given binary string this is defined DISPLAYFORM1 N , where n 0 is the number of zeros in the string, and n 1 is the number of ones, and 110 111 112 113 114 115 116 117 118 119 120 121 122 123 124 125 126 127 128 129 130 131 132 133 134 135 136 137 138 139 140 141 142 143 144 145 146 147 148 149 150 151 152 153 154 155 156 157 158 159 160 161 162 163 164 DISPLAYFORM2 This measure is close to 1 when the number of ones and zeros is similar, and is close to 0 when the string is mostly ones, or mostly zeros.

Entropy and K LZ (x) are compared in FIG1 , and in more detail in supplementary note 7 (and supplementary information figure 1) of reference BID10 .
...
L. Franco et al. have introduced a complexity measure for Boolean functions, designed to capture how difficult the function is to learn and generalize ( FORMULA0 ), which was used to empirically find that simple functions generalize better in a neural network ( FORMULA0 ).
...
The first term is also known (up to a normalization constant) as average sensitivity ( FORMULA0 ).

The terms in the series have also been called "generalized robustness" in the evolutionary theory literature ( FORMULA0 ).

Here we use the first two terms, so the measure is: DISPLAYFORM3 where Nei i (x) is all neighbours of x at Hamming distance i.

Critical sample ratio.

A measure of the complexity of a function was introduced in BID16 to explore the dependence of generalization with complexity.
...
In FIG1 , we compare the different complexity measures against one another.

We also plot the frequency of each complexity; generally more functions are found with higher complexity.

In FIG7 we show how the probability versus complexity plots look for other complexity measures.

The behaviour is similar to that seen for the LZ complexity measure in FIG0 of the main text.

In FIG3 we show probability versus LZ complexity plots for other choices of parameter distributions.

In FIG2 , we show a histogram of the functions in the log probability -complexity plane.

The histogram counts are weighted by the probability of the function, so that the total weight is proportional to the probability of obtaining a function in a particular bin, when sampling the parameters.

In FIG9 we show the effect of the number of layers on the bias (for feedforward neural networks with 40 neurons per layer).
...
167 168 169 170 171 172 173 174 175 176 177 178 179 180 181 182 183 184 185 186 187 188 189 190 191 192 193 194 195 196 197 198 199 200 201 202 203 204 205 206 207 208 209 210 211 212 213 214 215 216 217 218 219 220 221 222 223 224 225 226 227 228 229 230 231 232 233 234 235 236 237 238 239 240 241 242 243 244 245 246 247 248 249 250 251 252 253 254 255 256 257 258 259 260 261 262 263 264 265 266 267 268 269 270 271 272 273 parameters, for a network of shape BID6 40, 40, BID0 .

Points with a frequency of 10 −8 are removed for clarity because these suffer from finite-size effects (see Appendix E).

The measures of complexity are described in Appendix C. 277 278 279 280 281 282 283 284 285 286 287 288 289 290 291 292 293 294 295 296 297 298 299 300 301 302 303 304 305 306 307 308 309 310 311 312 313 314 315 316 317 318 319 320 321 322 323 324 325 326 327 328 329 FIG2 .

Histogram of functions in the probability versus Lempel-Ziv complexity plane, weighted according to their probability.

Probabilities are estimated from a sample of 10 8 parameters, for a network of shape BID6 40, 40, BID0 The histograms therefore show the distribution over complexities when randomly sampling parameters BID1 We can see that between the 0 layer perceptron and the 2 layer network there is an increased number of higher complexity functions.
...
E. Finite-size effects for sampling probabilitySince for a sample of size N the minimum estimated probability is 1/N , many of the low-probability samples that arise just once may in fact have a much lower probability than suggested.

See Figure 7) , for an illustration of how this finite-size sampling effect manifests with changing sample size N .
