...
Minimising the disagreement among multiple views helps the model to learn rich feature representations of the data and, also after training, the ensemble of the feature vectors from multiple views can provide an even stronger generalisation ability.
...
Models built upon distributional similarity don't explicitly require human-annotated training data; the supervision comes from the semantic continuity of the language data.
...
We draw inspiration from the lateralisation and asymmetry in information processing of the two hemispheres of the human brain where, for most adults, sequential processing dominates the left hemisphere, and the right hemisphere has a focus on parallel processing BID9 , but both hemispheres have been shown to have roles in literal and non-literal language comprehension BID14 BID15 .Our proposed multi-view frameworks aim to leverage the functionality of both RNN-based models, which have been widely applied in sentiment analysis tasks BID57 , and the linear/log-linear models, which have excelled at capturing attributional similarities of words and sentences BID5 BID6 BID24 BID51 for learning sentence representations.
...
The encoder learns to produce a vector representation for the current input, and the decoder learns to generate sentences in the adjacent context given the produced vector BID28 BID24 BID20 BID50 .
...
It is shown BID24 that the consistency between supervised and unsupervised evaluation tasks is much lower than that within either supervised or unsupervised tasks alone and that a model that performs well on supervised tasks may fail on unsupervised tasks.

BID13 subsequently showed that, with a labelled training corpus, such as SNLI BID8 and MultiNLI BID56 , the resulting representations of the sentences from the trained model excel in both supervised and unsupervised tasks.

Multi-task learning BID48 also gives impressive performance on downstream tasks while labelled data is costly.
...
One encoder is a bi-directional Gated Recurrent Unit BID10 f (s; φ), where s is the input sentence and φ is the parameter vector in the GRU.
...
The framework has an RNN encoder f , and a linear decoder h. Given an input sentence s i , the encoder produces a vector z f i = f (s i ; φ), and the decoder h projects the vector to DISPLAYFORM0 , which has the same dimension as the word vectors v w .
...
DISPLAYFORM1 where v w k are pretrained word vectors for w k , the empirical distribution P e (w) is the unigram distribution raised to power 0.75 BID38 , and K is the number of negative samples.
...
Often, as the dimensionality of vector z f i doesn't necessarily need to match that of word vectors v w , U is not a square matrix BID0 .
...
Specifically, the update rule BID11 for the regularisation is: DISPLAYFORM2 where β is set to 0.01.
...
An RNN encoder f (s; φ) and a linear avg-on-word-vectors g(s; W ) produce a vector representation z DISPLAYFORM0 where DISPLAYFORM1 where τ is the trainable temperature term, which is essential for exaggerating the difference between adjacent sentences and those that are not.
...
Three unlabelled corpora from different genres are used in our experiments, including BookCorpus BID59 , UMBC News BID21 and Amazon Book Review 2 BID35 ; six models are trained separately on each of three corpora with each of two objectives.
...
Pretrained word vectors, fastText BID7 , are used in our frameworks and fixed during learning.
...
Representation: For a given sentence input s with M words, suggested by BID45 BID30 , the representation is calculated as z = ẑ f +ẑ g /2, whereẑ refers to the post-processed and normalised vector, and is mentioned in Table 2 .
...
Supervised learning: ParaNMT BID55 is included as a supervised learning method as the data collection requires a neural machine translation system trained in supervised way.

InferSent 3 BID13 trained on SNLI and MultiNLI is included as well.

The results are presented in TAB1 .

Since the performance of FastSent BID24 and QT BID33 were only evaluated on STS14, we compare to their results in TAB2 .All six models trained with our learning frameworks outperform other unsupervised and semi supervised learning methods, and the model trained on the UMBC News Corpus with discriminative objective gives the best performance likely because the STS tasks contain multiple news-and headlines-related datasets which is well matched by the domain of the UMBC News Corpus.
...
The results are presented in TAB3 .Comparison: Our results as well as related results of supervised task-dependent training models, supervised learning models, and unsupervised learning models are presented in TAB3 .

Note that, for fair comparison, we collect the results of the best single model (MC-QT, BID33 ) trained on BookCorpus.
...
The orthonormal regularisation applied on the linear decoder to enforce invertibility in our multi-view framework encourages the vector representations produced by f and those by h −1 , which is g in testing, to agree/align with each other.

A direct comparison is to train our multi-view framework Table 6 : Ablation study on our multi-view frameworks.

Variants of our frameworks are tested to illustrate the advantage of our multi-view learning frameworks.

In general, under the proposed frameworks, learning to align representations from both views helps each view to perform better and an ensemble of both views provides stronger results than each of them.

The arrow and value pair indicate how a result differs from our multi-view learning framework.

Better view in colour.

DISPLAYFORM0 Multi-view with g 1 and g 2 : a ij = cos(z without the invertible constraint, and still directly use U as an additional encoder in testing.

The results of our framework with and without the invertible constraint are presented in Table 6 .

DISPLAYFORM1 The ensemble method of two views, f and g, on unsupervised evaluation tasks (STS12-16 and SICK14) is averaging, which benefits from aligning representations from f and g by applying invertible constraint, and the RNN encoder f gets improved on unsupervised tasks by learning to align with g. On supervised evaluation tasks, as the ensemble method is concatenation and a linear model is applied on top of the concatenated representations, as long as the encoders in two views process sentences distinctively, the linear classifier is capable of picking relevant feature dimensions from both views to make good predictions, thus there is no significant difference between our multi-view framework with and without invertible constraint.

In order to determine if the multi-view framework with two different views/encoding functions is helping the learning, we compare our framework with discriminative objective to other reasonable variants, including the multi-view model with two functions of the same type but parametrised independently, either two f -s or two g-s, and the single-view model with only one f or g. Table 6 presents the results of the models trained on UMBC News Corpus.

As specifically emphasised in previous work BID24 , linear/log-linear models, which include g in our model, produce better representations for unsupervised evaluation tasks than RNN-based models do.

This can also be observed in Table 6 as well, where g consistently provides better results on unsupervised tasks than f .
...
Provided the results of models with generative and discriminative objective in Table 6 , we confidently show that, in our multi-view frameworks with f and g, the two encoding functions improve each other's view.
...
We proposed multi-view sentence representation learning frameworks with generative and discriminative objectives; each framework combines an RNN-based encoder and an average-on-word-vectors linear encoder and can be efficiently trained within a few hours on a large unlabelled corpus.
...
Suppose there is a batch of representations Z = [z1, z2 ..., zN ] ∈ R 2d×N from either f or g, the Power Iteration method is applied here to estimate the top eigenvector of the covariance matrix 2 : C = ZZ , and it is described in Algorithm 1:Algorithm 1 Estimating the First Principal Component BID39 Input: Covariance matrix C ∈ R 2d×2d , number of iterations T Output: First principal component u ∈ R u ← Cu, 4: u ← u ||u||In our experiments, T is set to be 5.
...
The word vectors are fixed to be those in the FastText BID7 , and we don't finetune them.
...
The temperature term is used to convert the agreement aij to a probability distribution pij in Eq. 1 in the main paper.

In our experiments, τ is a trainable parameter initialised to 1 that decreased consistently through training.

Another model trained with fixed τ set to the final value performed similarly.

Table 2 : Effect of the Post-processing Step. 'WR' refers to the post-processing step BID6 which removes the principal component of a set of learnt vectors.
...
5 Combining both generative and discriminative objective in our multi-view framework Models with both generative and discriminative objectives are trained to see if further improvement can be provided by combining an RNN encoder, an inverse of a linear decoder in the generative objective and a linear encoder in the discriminative objective.

The results of models trained on BookCorpus and UMBC News are presented in TAB1 .

As presented in the table, no further improvement against models with only one objective is shown.

In our understanding, the inverse of the linear decoder in generative objective behaves similarly to the linear encoder in the discriminative objective, which is presented in Table 6 in the main paper.

Therefore, combining two objectives doesn't perform better than only one of them.

The number of parameters of each of the selected models is:1.

Ours: ≈ 8.8M2.

Quick-thought BID33 : ≈ 19.8M3.

Skip-thought BID28 : ≈ 57.7M
