...
In B2, the neighborhood in the graph contains all adjacent nodes in graph; the neighborhood in the latent space contains the nodes within the dashed circle whose radius is ρ.

The relational operator τ is illustrated by a colorful 3 × 3 grid where each unit is corresponding to a geometric relationship to the red target node.
...
Based on the graph and the latent space, we then build a structural neighborhood, N (v) = ({N g (v) , N s (v)}, τ ), for the next aggregation.

The structural neighborhood consists of a set of neighborhood {N g (v), N s (v)}, and a relational operator on neighborhoods τ .

The neighborhood in the graph, N g (v) = {u|u ∈ V, (u, v) ∈ E}, is the set of adjacent nodes of v. The neighborhood in the latent space, N s (v) = {u|u ∈ V, d(z u , z v ) < ρ}, is the set of nodes from which the distance to v is less than a pre-given parameter ρ.
...
The relational operator τ is a function defined in the latent space.
...
A requirement on τ is that it should guarantee that each ordered position pair has only one geometric relationship.

For example, τ is illustrated in Fig. 1B by a colorful 3 × 3 grid in a 2-dimensional Euclidean space, in which each unit is corresponding to a geometric relationship to node v.
...
The output of high-level aggregation is a vector m , are given by a non-linear transform, wherein W l is a learnable weight matrix on the l-th layer shared by all nodes, and σ(·) is a non-linear activation function, e.g., a ReLU.
...
Definition 1. Let a bijective function ψ : V → V be a permutation for nodes, which renames v ∈ V as ψ(v) ∈ V .

Let V and E be the node and edge set after a permutation ψ, respectively.

A mapping of graph, φ(G), is permutation-invariant if, given any permutation ψ, we have

Proof.

Let G be an isomorphic graph of G after a permutation ψ, as defined in Definition 1.

If φ 2 (G) is permutation-invariant, we have φ 2 (G) = φ 2 (G ).
...
The structural neighborhood N (v) = ({N g (v) , N s (v)}, τ ) of node v includes its neighborhoods in both the graph and latent space.

The neighborhood-in-graph N g (v) consists of the set of v's adjacent nodes in the graph, and the neighborhood-in-latent-space N s (v) those nodes whose distances to v are less than a parameter ρ in the latent space.

We determine ρ by increasing ρ from zero until the average cardinality of N s (v) equals to that of N g (v), ∀v ∈ V -i.e., when the average neighborhood sizes in the graph and latent spaces are the same.
...
Here we simply implement the geometric operator τ as four relationships of the relative positions between two nodes in a 2-D Euclidean or hyperbolic space.

Particularly, the relationship set R = {upper left, upper right, lower left, lower right}, and a τ (z v , z u ) is given by Table 1.
...
One can design a more sophisticated operator τ , such as borrowing the structure of descriptors in manifold geometry (Kokkinos et al., 2012; Monti et al., 2017) , thereby preserving more and richer structural information in neighborhood.
...
where deg(v) is the degree of node v in graph, and δ(·, ·) is a Kronecker delta function that only allows the nodes with relationship r to v to be included.
...
where we use ReLU as the non-linear activation function σ(·) and W l is the weight matrix to estimate by backpropagation.
...
We specify the dimension of embedding space as two, and use the relationship operator τ defined in Table 1 , and apply mean and concatenation as the low-and high-level aggregation function, respectively.
...
A large β value implies that the homophily, in term of node label, is strong in a graph, i.e., similar nodes tend to connect together.

From Table 4 , one can see that assortative graphs (e.g., citation networks) have a much larger β than disassortative graphs (e.g., WebKB networks).
