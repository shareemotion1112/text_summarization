...
The core is its update step: w t = w t−1 − α t g t , where t (from 1 to T ) is the iteration, w are the parameters to be learned, g is the gradient estimator for the objective function 2 F , and α t is the learning rate, also known as step size.

Given base learning rate α 0 , we can define the ratio β t = α t /α 0 .

Then the set of {β t } T t=1 is called the learning rate schedule, which specifies how the learning rate should vary over the course of training.
...
Such hypothesis is questioned by recent analysis on mode connectivity, which has revealed that there does exist a descent path between solutions that were previously thought to be isolated local minima (Garipov et al., 2018; Dräxler et al., 2018; Gotmare et al., 2019) .
...
• step decay: drop the learning rate by a multiplicative factor γ after every d epochs.

The default for γ is 0.1, but d varies significantly across tasks.

• exponential: β t = γ t .

There is no default parameter for γ and it requires manual tuning.
...
where f (p) is the ratio of learning rate at step t to the base learning rate α 0 .
...
Given a particular rate schedule β t = f (t), one simple method for making it budget-aware is to rescale it, i.e., β p = f (pT 0 ), where T 0 is the budget used for the original schedule.
...
No parameter other than γ = 0.9 is used in published work.

• cosine (Loshchilov & Hutter, 2017) :

(1 − η)(1 + cos(πp)).

η specify a lower bound for the learning rate, which defaults to zero.

• htd (Hsueh et al., 2019) :

Here η has the same representation as in cosine.
...
"step-dx" denotes decay x times at even intervals with γ = 0.1.
...
Noticeably, when exponential schedule is well-tuned for this task (γ = 0.97), it fails to generalize across budgets.
...
We follow the step decay schedule used in (Huang et al., 2017b; PyTorch, 2019) , which drops twice at uniform interval (γ = 0.1 at p ∈ { 1 3 , 2 3 }).
...
For training, we adopt the 1x schedule (90k iterations), and the off-the-shelf (He et al., 2017) step decay that drops 2 times with γ = 0.1 at p ∈ { 2 3 , 8 9 }.
...
We follow the 4-GPU 300k iteration schedule (Wang et al., 2018) , which features a step decay that drops 2 times with γ = 0.1 at p ∈ { 1 2 , 5 6 }.
...
t || vanishes over time (color curves) as learning rate α t (black curves) decays.
...
We empirically find that the dynamics of ||g * t || over time highly correlates with the learning rate α t (Fig 3) .
...
Given that α t and ||g * t || decrease, the overall update ||−α t g t || also decreases 4 .
...
To examine why, we derive the equivalent rate β t for AMSGrad (Appendix B) and show that it is dramatically larger than our defaults, suggesting the optimizer is too aggressive.
...
(b) for SGDR, the correlation between full gradient norm ||g * t || and learning rate αt is also observed.
...
The metric we adopt is Kendall's rank correlation coefficient (τ ), a standard statistics metric for measuring rank similarity.

It is based on counting the inversion pairs in the two ranks and (τ + 1)/2 is approximately the probability of estimating the rank correctly for a pair.
...
(2) step decay (γ = 0.1, decay at p ∈ { The results suggest that with more budget, we can better estimate the full-budget rank between architectures.
...
Here we present the derivation for the equivalent learning rate β t .

Let η 1 , η 2 and be hyper-parameters, then the momentum SGD update rule is:
...
In our experiments, we use the default hyper-parameters (which also turn out to have the best validation accuracy): α
...
One common such instance in convex optimization is α t = α 0 /t.
