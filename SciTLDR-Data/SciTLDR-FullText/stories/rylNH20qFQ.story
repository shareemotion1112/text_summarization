...
The problem of inferring programs from voxels is closely related to inverse procedural graphics, where a procedural graphics program is inferred from an image or declarative specification BID11 Št'ava et al., 2010) .
...
Neural Program Interpreters (NPI) have been extensively studied for programs that abstract and execute tasks such as sorting, shape manipulation, and grade-school arithmetic BID10 Cai et al., 2017; Bošnjak et al., 2017) .
...
During training, we minimize the sum of the weighted binary cross-entropy losses over all voxels via DISPLAYFORM0 where v is a single voxel of the whole voxel space V , y v andŷ v are the ground truth and prediction, respectively, while w 0 and w 1 balance the losses between vacant and occupied voxels.
...
Ellis et al. (2015) used the performant Z3 SMT solver (De Moura & Bjørner, 2008) to infer 2D graphics programs, taking 5-20 minutes for problems arguably simpler than our 3D shape programs.
