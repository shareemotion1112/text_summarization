...
The interaction process is defined as a Partially Observable Markov Decision Process (POMDP) < S, A, T, R, Ω, O, γ > (Monahan, 1982; Kaelbling et al., 1998) , where S is a finite state space, A is a finite action space, R(s, a) is the reward function, T (·|s, a) is the transition function, Ω is a set of possible observations, O is a function which maps states to probability distributions over observations and γ ∈ [0, 1) is a discount factor used to compute the cumulative discounted reward.

Within this framework, at each step t, the environment is in some unobserved state s ∈ S. When the agent takes an action a, the environment transits to the new state s with probability T (s |s, a), providing the agent with a new observation o ∈ Ω with probability O(o |s ) and reward r ∼ R(s, a).
...
We want to learn a representation mapping Φ :

C , where Y = {1, ..., C} is a set of C events and Φ(o t ) is the probability distribution of a discrete random variable z which (soft-)assigns o t to the elements in Y and can be defined as: Φ c (o t ) = P (z = c|o t ) (Ji et al., 2019) .

Φ(·) is our unsupervised encoder and it is based on a CNN followed by a fully-connected layer and a softmax layer (Fig. 1) , parametrized by φ.

The clustering process in IIC is based on pairs of images randomly transformed which contain

Figure 1: Event discovery.

This figure shows two replicas of our unsupervised encoder architecture (Φ) which separately map two different observations in corresponding event probability distributions.

The latter are used to maximize the MI over the latent assignment variable z, in this way obtaining a set of events Y and the supervision signal used to update the parameters (φ) of Φ.
...
Once φ is updated (see Sec. 4), we use Φ(o t ; φ) to represent an observation at time t and update the agent's policy.

In more detail, we represent the agent with an Actor-Critic architecture approximated with a NN π parametrised by θ.
...
Specifically, for each t ∈ {t, ..., t − S + 1} let Φ(o t ; φ) be the probability distribution of o t for the current set of events Y .

Our proposed history representation is simply given Use B to update φ in n E epochs by the concatenation of these cluster soft-assignment vectors, computed over the past S steps:

We empirically observed that the history matrix H(t) is very sparse (Sec. 5.3), being most of the elements in Φ(o t ) close to 0 (Φ(o t ) is usually very peaked).
...
I(t) is input to a second CNN (Ψ) which extracts a vector v t representing the instantaneous visual information (v t is obtained by flattening the last CNN feature map).

We call the parameters of Ψ θ I , to differentiate them from the agent's proper parameters θ A which control the policy and the value function (θ = [θ I ; θ A ]).
...
We highlight that the CNN of the unsupervised encoder (Φ) is different from Ψ, and the two CNNs do not share parameters (φ = θ I ).

Moreover, while the encoder is trained in an unsupervised fashion, Ψ, used to extract visual information (v t ) from I(t), is trained using the RL reward.
...
Next, using the collected experience, we update θ for n P epochs (policy optimisation stage) and φ for n E epochs (event discovery stage).
...
These three stages are iterated, so to adapt Φ(·) (and Y ) to the observation distribution change.

The full algorithm is described in Alg.

1.

The weights φ and θ are initialized with a (semi-) orthogonal matrix, as described in (Saxe et al., 2014) , apart from the head of Φ (i.e., the fully-connected layer on top of the CNN, see Fig. 1 ), which is initialized randomly.

All the biases are initialized with zeros.

Following (Ji et al., 2019) , to improve robustness, the head of Φ is duplicated 5 times with different initial random parameters.

Moreover, Φ includes an auxiliary overclustering head with C × 5 clusters.
...
1) the agent receives only I(t) as input and θ and φ (and each head in Φ) are updated as usual (lines 8 and 9 in Alg.
...
Φ is composed of a CNN and a fullyconnected layer (Sec. 3).

Specifically, its CNN configuration is similar to Ψ, except the first layer, which takes only one channel as input.
...
Ψ is the same as in EDHR and in PPO (Schulman et al., 2017) .

However, past observations are represented as follows.

The encoder Φ is replaced with a similar network (Υ) but without the softmax layer and having 256 neurons as output of the fully-connected layer instead of C. For each t ∈ {t, ..., t − S + 1}, ω t = Υ(o t ) is input to a GRU (Cho et al., 2014 ) with a hidden layer of size 512 (re-initiated with zeros each S steps).
...
In EDHR, visual information is represented using two different networks: Φ and Ψ. The latter is trained using a reward signal, so it presumably extracts task-specific information from the observations.

On the other hand, the encoder Φ is trained using self-supervision, and thus it focuses on patterns which are repeated in the data stream, potentially leveraging a larger quantity of supervision signal.
...
Specifically, "Without self-supervision" is equal to EDHR, but Φ is trained using the gradient of the reward signal, jointly with Ψ. This experiment shows a dramatic degradation in the MsPacman and the SpaceInvaders environments, which highlights the gain which can be obtained using the proposed additional self-supervision.
...
4 concerns a variant of the PPO-RNN algorithm, where the input I(t) and the corresponding CNN Ψ are removed.
...
Φ is a network mapping an image into a probability distribution Φ : X → [0, 1] C , where C is the number of clusters (in our case, events).

Φ(x) is the probability distribution of a discrete random variable z which (soft-)assigns x to the elements in Y = {1, ..., C} and can be defined as: Φ c (x) = P (z = c|x).
...
In more detail, a pair (x, x ) corresponds to the two variables (z, z ), with conditional joint distribution P (z = c, z = c |x, x ) = Φ c (x) · Φ c (x ), where the equality holds when z and z are independent (Ji et al., 2019) .
