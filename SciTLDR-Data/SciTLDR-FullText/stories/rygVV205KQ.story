...
Following the notation of BID32 , a Markov Decision Process (MDP) is a tuple (S, A, R, P, γ) with states S, actions A, reward function R(s, a), transition distribution P (s |s, a), and discount γ.

An agent in state s ∈ S takes action a ∈ A according to its policy π and moves to state s ∈ S according to the transition distribution.

The goal of training an agent is to find a policy that maximizes the expected sum of discounted rewards, represented by the action value function DISPLAYFORM0 , where E π is an expectation over trajectories starting from s 0 = s and taking action a 0 = a and thereafter running the policy π.

DDPG BID23 is an actor-critic method in which the actor, or policy π(s|φ) and the critic, or action-value function Q(s, a|θ) are represented by neural networks with parameters φ and θ, respectively.

New transitions (s, a, r, s ) are added to a replay buffer B by sampling from the policy according to a = π(s|φ) + , with ∼ N added for better exploration.

The action-value function is trained to match the 1-step returns by minimizing where the transition is sampled from the replay buffer B, and π , Q are target actor and critic networks parameterized by φ and θ respectively.
...
The GAIL objective is formulated as follows: DISPLAYFORM0 where π is the agent policy, π E the expert policy, and H(π) an entropy regularizer.
...
Note that by D(.) we mean the sigmoid of the logits, σ(x) = 1/(1 + exp(−x)), which bounds the imitation reward between 0 and 1.
...
An important detail in the actor is the use of early termination of the episode when the discriminator score is below a threshold β, which is a hyperparameter.

This prevents the agent from drifting too far from the expert trajectories, and avoids wasting computation time.

In practice we set β = 0.1.
...
We used expert states in the final 1/3rd of each sequence as positive examples for D goal , and set α = 0.5 in the imitation reward function.
...
Instead of using a scalar state-action value function, we adopt Distributional Q fuctions where Q(s, a|θ) = EZ(s, a|θ) for some random variable Z. In this paper, we adopt a categorical representation of Z such that Z(s, a|θ) = z i w.p.

p i ∝ exp(ω i (s, a|θ)) for i ∈ {0, · · · , l − 1}.
...
p i ∝ exp(ω i (s t+K , π(s t+K |φ )|θ )) where z i = N −1 n=0 γ j r t+n + γ N z i .

Notice Z is not likely have the same support as Z. We therefore adopt the categorical projection Φ proposed in BID8 .

The loss function for training our distributional value functions is L N (θ) = E (st,at,{rt,··· ,r t+N −1 },s t+N )∼B [H(Φ(Z ), Z(s t , a t |θ))] ,where H represents cross entropy.
