...
Correlation based Similarity(COR) as used by BID8 , computes similarities using the estimated pearsons correlation ρ between the latent representation z i and the centroids w j .

In this study we compute the COR as follows: DISPLAYFORM4 where ρ is the pearson's correlation, given by ρ x,y = cov(x,y) σxσy and cov is the covariance.
