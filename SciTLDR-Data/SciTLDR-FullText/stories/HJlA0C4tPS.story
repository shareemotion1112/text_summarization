...
Specifically,X = {x (m+1) ,x (m+2) , · · · ,x (n) } represents the set of latent parallel sentences in D 1 , whilē

} represents the set of latent parallel sentences in D 2 .

Then the goal of unsupervised text transduction is to infer these latent variables conditioned the observed non-parallel corpora; that is, to learn p(ȳ|x) and p(x|y).
...
Directly modeling p(ȳ|x) and p(x|y) in the unsupervised setting is difficult because we never directly observe parallel data.

Instead, we propose a generative model of the complete data that defines a joint likelihood, p(X,X, Y,Ȳ ).
...
We let θ x|ȳ and θ y|x represent the parameters of the two transduction distributions respectively.
...
The surrogate objective introduces q(ȳ|x (i) ; φȳ |x ) and q(x|y (j) ; φx |y ), which represent two separate inference network distributions that approximate the model's true posteriors, p(ȳ|x (i) ; θ x|ȳ ) and p(x|y (j) ; θ y|x ), respectively.
...
For example, the inference network q(ȳ|x (i) ; φȳ |x ) and the generative distribution p(y|x (i) ; θ y|x ) both aim to transform D 1 to D 2 .

Therefore, we use the same architecture for each inference network as used in the transduction models, and tie their parameters: φx |y = θ x|ȳ , φȳ |x = θ y|x .

This means we learn only two encoder-decoders overall -which are parameterized by θ x|ȳ and θ y|x respectively -to represent two directions of transfer.
...
α decays from 1.0 to 0.0 linearly in the first k epochs.
...
We vary the weight λ for the NLL term (BT+NLL) or the KL term (our method) as {0.001, 0.01, 0.03, 0.05, 0.1}.
