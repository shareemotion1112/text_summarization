...
For any choice of stepsize δ and momentum α ∈ [0, 1], ∃T large enough such that ∀t ≥ T , we have, DISPLAYFORM1 where C(κ, δ, α) depends on κ, δ and α (but not on t).Thus, to obtain w s.t.

w − w * ≤ , HB requires Ω(κ log 1 ) samples and iterations.

On the other hand, ASGD can obtain -approximation to w * in O( √ κ log κ log 1 ) iterations.

We note that the gains offered by ASGD are meaningful when κ > O(c) (Jain et al., 2017) ; otherwise, all the algorithms including SGD achieve nearly the same rates (upto constant factors).
...
The algorithm takes three inputs: short step δ, long step parameter κ and statistical advantage parameter ξ.

The short step δ is precisely the same as the step size in SGD, HB or NAG.

For convex problems, this scales inversely with the smoothness of the function.

The long step parameter κ is intended to give an estimate of the ratio of the largest and smallest curvatures of the function; for convex functions, this is just the condition number.

The statistical advantage parameter ξ captures trade off between statistical and computational condition numbers -in the deterministic case, ξ = √ κ and ASGD is equivalent to NAG, while in the high stochasticity regime, ξ is much smaller.
...
Given κ which stands for condition number, we consider the following two distributions:Discrete: a = e 1 w.p.

0.5 and a = 2 κ · e 2 with 0.5; e i is the i th standard basis vector.
...
We vary κ from {2 4 , 2 5 , ..., 2 12 } and for each κ in this set, we run 100 independent runs of all four methods, each for a total of t = 5κ iterations.

We define that the algorithm converges if there is no error in the second half (i.e. after 2.5κ updates) that exceeds the starting error -this is reasonable since we expect geometric convergence of the initial error.
...
We measure convergence performance of a method using: DISPLAYFORM1 Figure 1: Plot of 1/rate (refer equation FORMULA3 We compute the rate (1) for all the algorithms with varying condition number κ.

Given a rate vs κ plot for a method, we compute it's slope (denoted as γ) using linear regression.

Table 1 presents the estimated slopes (i.e. γ) for various methods for both the discrete and the Gaussian case.

The slope values clearly show that the rate of SGD, HB and NAG have a nearly linear dependence on κ while that of ASGD seems to scale linearly with √ κ.
...
So, for each of HB and NAG, we choose the learning rate and decay factors by grid search, use these values for ASGD and do grid search only over long step parameter κ and momentum α for ASGD.
...
Loizou & Richtárik (2017) state their method as Stochastic HB but require stochastic gradients that nearly behave as exact gradients; indeed, their rates match that of the standard HB method BID5 .
...
Furthermore, these methods require computing a batch gradient or require memory requirements (typically Ω(| training data points|)).
...
Let θ DISPLAYFORM0 t+1 denote the concatenated and centered estimates in the j th direction for j = 1, 2.

DISPLAYFORM1 , j = 1, 2.Since the distribution over x is such that the coordinates are decoupled, we see that θ (j) t+1 can be written in terms of θ (j) t as: DISPLAYFORM2 t+1 denote the covariance matrix of θ DISPLAYFORM3 with, B (j) defined as DISPLAYFORM4 We prove Proposition 3 by showing that for any choice of stepsize and momentum, either of the two holds:• B (1) has an eigenvalue larger than 1, or,• the largest eigenvalue of B (2) is greater than 1 − 500 κ .

This is formalized in the following two lemmas.

Lemma 4.

If the stepsize δ is such that δσ DISPLAYFORM5 (1) has an eigenvalue ≥ 1.Lemma 5.

If the stepsize δ is such that δσ DISPLAYFORM6 (2) has an eigenvalue of magnitude DISPLAYFORM7 Given this notation, we can now consider the j th dimension without the superscripts; when needed, they will be made clear in the exposition.

Denoting x def = δσ 2 and t def = 1 + α − x, we have: DISPLAYFORM8 The analysis goes via computation of the characteristic polynomial of B and evaluating it at different values to obtain bounds on its roots.
...
Substituting z = 1 − τ in the characteristic equation of Lemma 6, we have: DISPLAYFORM12 Proof of Lemma 4.
...
This follows in a straightforward manner from corollary (7) by substituting τ = 0 in equation (2), and this yields, DISPLAYFORM13 As α < 1, x = δσ 2 > 0, we have the following by setting D(1) ≤ 0 and solving for x: DISPLAYFORM14 Since D(1) ≤ 0 and D(z) ≥ 0 as z → ∞, there exists a root of D(·) which is ≥ 1.Remark 8.

The above characterization is striking in the sense that for any c > 1, increasing the momentum parameter α naturally requires the reduction in the step size δ to permit the convergence of the algorithm, which is not observed when fast gradient methods are employed in deterministic optimization.

For instance, in the case of deterministic optimization, setting c = 1 yields δσ 2 1 < 2(1 + α).

On the other hand, when employing the stochastic heavy ball method with x (j) = 2σ 2 j , we have the condition that c = 2, and this implies, δσ DISPLAYFORM15 We now prove Lemma 5.

We first consider the large momentum setting.

Lemma 9.

When the momentum parameter α is set such that 1 − 450/κ ≤ α ≤ 1, B has an eigenvalue of magnitude ≥ 1 − 450 κ .Proof.

This follows easily from the fact that det(B) DISPLAYFORM16 Remark 10.

Note that the above lemma holds for any value of the learning rate δ, and holds for every eigen direction of H. Thus, for "large" values of momentum, the behavior of stochastic heavy ball does degenerate to the behavior of stochastic gradient descent.
...
The natural implication of this bound is that the terms that are lower order, such as O(1/κ 4 ) and O(1/κ 5 ) will be negative owing to the large constant above.

Let us verify that this is indeed the case by considering the terms having powers of O(1/κ 4 ) and O(1/κ 5 ) from equation (3) : DISPLAYFORM21 κ 4 The expression above evaluates to ≤ 0 given an upperbound on the value of c. The expression above follows from the fact that l ≤ 9, κ ≥ 1.

3 ) and O(1/κ 2 ), in particular, DISPLAYFORM0 In both these cases, we used the fact that DISPLAYFORM1 κ .
...
But before we start looking at each dimension separately, let us consider the j th dimension, and detail the approach we use to prove the claim: the idea is to examine the subspace spanned by covariance E θ DISPLAYFORM2 2 , ..., for every starting iterate θ DISPLAYFORM3 in the largest eigen direction of B (j) , and this decays at a rate that is at best λ max (B (j) ).Since B (j) ∈ R 4×4 , we begin by examining the expected covariance spanned by the iterates DISPLAYFORM4 This implies that k just appears as a scale factor.

This in turn implies that in order to analyze the subspace spanned by the covariance of iterates θ DISPLAYFORM5 1 , ..., we can assume k (j) = 1 without any loss in generality.

This implies, θ DISPLAYFORM6 Note that with this in place, we see that we can now drop the superscript j that represents the dimension, since the analysis decouples across the dimensions j ∈ {1, 2}. Furthermore, let the entries of the vector θ k be represented as DISPLAYFORM7 Furthermore, DISPLAYFORM8 Let us consider the vectorized form of Φ j = E [θ j ⊗ θ j ], and we denote this as vec(Φ j ).

Note that vec(Φ j ) makes Φ j become a column vector of size 4 × 1.

Now, consider vec(Φ j ) for j = 0, 1, 2, 3 and concatenate these to form a matrix that we denote as D, i.e. DISPLAYFORM9 Now, since we note that Φ j is a symmetric 2 × 2 matrix, D should contain two identical rows implying that it has an eigenvalue that is zero and a corresponding eigenvector that is 0 −1/ √ 2 1/ (2) 0 .

It turns out that this is also an eigenvector of B with an eigenvalue α.

Note that det(B) = α 4 .

This implies there are two cases that we need to consider: (i) when all eigenvalues of B have the same magnitude (= α).

In this case, we are already done, because there exists at least one non zero eigenvalue of D and this should have some component along one of the eigenvectors of B and we know that all eigenvectors have eigenvalues with a magnitude equal to λ max (B).
...
In this case, note that det(B) = α 4 < (λ max (B)) 4 implying λ max (B) > α.
...
This implies, we first define the following: let q γ = (t − γ) 2 + (c − 1)x 2 .

Then, R can be expressed as: DISPLAYFORM11 Then, DISPLAYFORM12 Note that this determinant can be zero when DISPLAYFORM13 We show this is not possible by splitting our argument into two parts, one about the convergent regime of the algorithm (where, δσ DISPLAYFORM14 .We will now prove that δ DISPLAYFORM15 is much larger than one allowed by the convergence of the HB updates, i.e., δσ DISPLAYFORM16 .

In particular, if we prove that DISPLAYFORM17 for any admissible value of α, we are done.
...
Now, let us consider the divergent regime of the algorithm, i.e., when, δσ DISPLAYFORM19 c+(c−2)α .

Furthermore, for the larger eigendirection, the determinant is zero when δσ for all admissible values of c, we are done.
...
Combining Lemmas 9 and 12, we see that no matter what stepsize and momentum we choose, B (j) has an eigenvalue of magnitude at least 1 − 500 κ for some j ∈ {1, 2}. This proves the lemma.

We begin by writing out the updates of ASGD as written out in Jain et al. (2017) , which starts with two iterates a 0 and d 0 , and from time t = 0, 1, ...T − 1 implements the following updates: DISPLAYFORM0 DISPLAYFORM1 Next, we specify the step sizes β 1 = c 2 3 / √ κ κ, α 1 = c 3 /(c 3 + β), γ 1 = β/(c 3 λ min ) and δ 1 = 1/R 2 , where κ = R 2 /λ min .

Note that the step sizes in the paper of Jain et al. (2017) with c 1 in their paper set to 1 yields the step sizes above.

Now, substituting equation 8 in equation 9 and substituting the value of γ 1 , we have: DISPLAYFORM2 We see that d t+1 is precisely the update of the running averagew t+1 in the ASGD method employed in this paper.

We now update b t to become b t+1 and this can be done by writing out equation 6 at t + 1, i.e: DISPLAYFORM3 By substituting the value of α 1 we note that this is indeed the update of the iterate as a convex combination of the current running average and a short gradient step as written in this paper.

In this paper, we set c 3 to be equal to 0.7, and any constant less than 1 works.

In terms of variables, we note that α in this paper's algorithm description maps to 1 − β 1 .
...
For SGD, for instance, denoting Φ DISPLAYFORM0 , where B is a linear operator acting on d × d matrices such that DISPLAYFORM1 Similarly, HB, NAG and ASGD also have corresponding operators (see Appendix A for more details on the operator corresponding to HB).
...
We now detail the range of parameters explored for these results: the condition number κ was varied from {2 4 , 2 5 , .., 2 28 } for all the optimization methods and for both the discrete and gaussian problem.

For each of these experiments, we draw 1000 samples and compute the empirical estimate of the fourth moment tensor.

For NAG and HB, we did a very fine grid search by sampling 50 values in the interval (0, 1] for both the learning rate and the momentum parameter and chose the parameter setting that yielded the smallest λ max (B) that is less than 1 (so that it falls in the range of convergence of the algorithm).
...
The statistical advantage parameter of ASGD was chosen to be 3κ/2 for the Gaussian case and 2κ/3 for the Discrete case, and the a long step parameters of 3κ and 2κ were chosen for the Gaussian and Discrete case respectively.

The reason it appears as if we choose a parameter above the theoretically maximal allowed value of the advantage parameter is because the definition of κ is different in this case.

The κ we speak about for this experiment is λ max /λ min unlike the condition number for the stochastic optimization problem.
...
Figure 7 presents these results, where for each method, we did grid search over all parameters and chose parameters that give smallest λ max .

We see the same pattern as in Figure 1 from actual runs -SGD,HB and NAG all have linear dependence on condition number κ, while ASGD has a dependence of √ κ.

Table 3 : Slopes (i.e. γ) obtained by fitting a line to the curves in Figure 7 .

A value of γ indicates that the error decays at a rate of exp −t κ γ .

A smaller value of γ indicates a faster rate of error decay.
