...
Neural networks are parametrized functions (by a weight vector θ) between the example and label spaces f (x; θ) : X → Y.
...
This paper shows that there is a subtle trade-off between δ c and δ adv .

In particular, we can find δ c such that δ c >> δ adv and f (x) = f (C(x adv )).
...
We consider two schemes, Gaussian ∼ N (0, σ) and additive Uniform noise ∼ U (−B, B) which add independent noise to each pixel.
...
2 is exactly the distortion measure δ c described at the beginning of Section 3.1.
...
For a single plot, we increase the L 2 distance of the output from the channel to the adversarial example by increasing the Gaussian noise (controlled by parameter σ).
...
is a mask that limits the F [ω] to a certain band of frequencies.
...
For the noise based channels, we increase the strength of the noise by controlling the epsilon parameter (in case of the Gaussian noise, it corresponds to the sigma parameter σ).
