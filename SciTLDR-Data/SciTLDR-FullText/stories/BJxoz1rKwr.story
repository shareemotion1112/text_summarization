...
Specifically, Π-Model (Laine & Aila, 2017 ) requires model's predictions to be invariant when various perturbations are added to the input data.
...
Unsupervised data augmentation (Li et al., 2018) upgrades Π-Model with advanced data augmentation methods.
...
The goal of a SemSL algorithm is to learn a better posterior probability estimator over y, i.e., p(y|x, θ) with θ denoting model parameters, from both labeled and unlabeled training samples.
...
Once z is defined, SlfSL is equivalent to training a predictor to model p(z|x; θ).
...
One branch for modeling p(y|x; θ) and another branch for modeling p(z|x; θ).

However, in both methods the pretext target z predictor p(z|x; θ) is implicitly related to the task of predicting y.

Our framework is different in that we explicitly incorporate y into the predictor for z. Specifically, we treat y as the latent factor in p(z|x; θ) and factorize p(z|x; θ) through marginalization:

Eq. 1 suggests that the pretext target predictor p(z|x; θ) can be implemented as two parts: a model to estimate p(y|x; θ) and a model to estimate z conditioned on both x and y, i.e., p(z|x, y; θ).

For the labeled samples, the ground-truth y is observed and can be used for training p(y|x; θ).

For unlabeled samples, the estimation from p(y|x; θ) and p(z|x, y; θ) will be combined together to make the final prediction about z. Consequently, optimizing the loss for p(z|x; θ) will also provide gradient to back-propagate through p(y|x; θ).

This is in contrast to the case of S 4 L, where the gradient generated from the unlabeled data will not flow through p(y|x; θ).

Theoretically, p(z|x; θ) and p(y|x; θ) can be two networks, but in practise we model them as two branches connecting to a shared feature extractor.

p(z|x; θ) suggested by Eq. 1 is essentially a pretext target predictor with a special structure and partial observations on its latent variable, i.e. y. The benefits of using such a predictor can be understood from three perspectives: (1) p(y|x; θ) in Eq. 1 acts as a soft selector to select p(z|x, y; θ) for predicting z. If the estimation of p(y|x; θ) is accurate, it will select p(z|x, y =ŷ(x); θ) for prediction and update, whereŷ(x) is the true class of x. This selective updating will make p(z|x, y; θ) give more accurate prediction over z if y matchesŷ(x).

After such an update, p(z|x, y; θ) will in turn encourage p(y|x; θ) to attain higher value for y =ŷ(x) since the prediction from p(z|x, y =ŷ(x); θ) is more likely to be accurate.

Thus, the terms p(y|x; θ) and p(z|x, y; θ) will reinforce each other during training.

(2) even if p(y|x; θ) is not accurate (this may happen at the beginning of the training process), p(z|x, y; θ) can still perform the pretext target prediction and act as an unsupervised feature learner.

Thus, the features will be gradually improved in the course of training.

With a better feature representation, the estimation of p(y|x; θ) will also be improved.

(3) Finally, to predict z in Eq. 1, p(z|x, y; θ) needs to be evaluated for each candidate y.

This in effect is similar to creating an ensemble of diversified pretext target predictors and with the combination weight given by p(y|x; θ) according to the marginalization rule.

Thus, training features with Eq. 1 may enjoy the benefit from ensemble learning.

Again, this will lead to better features and thus benefit the modelling of p(y|x; θ) and p(z|x, y; θ).

The above framework provides a guideline for turning a SlfSL method into a SemSL algorithm: (1) modifying a SlfSL predictor p(z|x; θ) by p(z|x, y; θ) and introducing a branch for p(y|x; θ) (2) optimizing the prediction of z on the SemSL dataset and update the branches p(z|x, y; θ), p(y|x; θ) and their shared feature extractor.

(3) using p(y|x; θ) as downstream task predictor or adding an additional branch for training p(y|x; θ) only with the labeled data as in S 4 L. More details about the additional branch will be explained in Section 4.
...
In this case, z in Eq. 1 is the rotation angle and y is the class label of input image x. We realize p(z|x, y; θ) by allocating a rotation angle prediction branch for each class.

The prediction from each branch is then aggregated with the aid of p(y|x; θ) for the final prediction of z as shown in Eq. 1.
...
It contains C rotation predictors which corresponds to p(z|x, y; θ) and a semantic classifier which generates p(y|x; θ).
...
We postulate the reason is that the p(y|x; θ) branch in auxiliary branches is mainly trained by the supervision generated from the optimization of p(z|x; θ).
...
In the following, we will extend CRAP from two perspectives: improving p(y|x; θ) and improving p(z|x, y; θ).

As discussed in Section 3, our method essentially introduces a network module with a special structure and partial observations on the latent variable y. Besides using labeled data to provide supervision for y, we can also use existing SemSL techniques to provide extra loss for modeling p(y|x; θ).
...
Then we use the cross entropy betweenp i and p(y|x; θ) (in auxiliary branches) as an additional loss.
...
Specifically, we require the rotation prediction branch to predict rotation angle for a mixed version of the rotated image, that is, we randomly mix the input image x i with another randomly sampled rotated image x j via x mix = αx i + (1 − α)x j , with α sampled from [0.5, 1].

Meanwhile, the class prediction p(y|x i ; θ) is calculated from the unmixed version of the input x i .
...
The purpose of introducing this modified task is to make the SlfSL task more challenging and more dependent on the correct prediction from p(y|x; θ).
...
This ambiguity can only be resolved from the output of p(y|x; θ) since its input is unmixed target image.

Therefore, this improved rotation prediction task relies more on the correct prediction from the semantic classifier and training through CRAP is expected to give stronger supervision signal to p(y|x; θ).
...
Similar to (Berthelot et al., 2019) , several SemSL baselines are considered: Pseudo-Label, Π-Model, Mean Teacher, Virtual Adversarial Training (VAT), MixUp and MixMatch 2 .
...
Extension 1 in Section 4.1, i.e., a stronger p(y|x; θ) modeling, perhaps leads to greater improvement.

(2) Using an additional semantic classifier leads to a slight performance improvement over the strategy of directly utilizing p(y|x; θ) in the auxiliary branches for testing (method in third line from the bottom).
