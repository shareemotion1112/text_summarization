...
Right two: Representative example of the evolution of the top 30 (decreasing, red to blue) eigenvalues of the Hessian for a SimpleCNN model during training (with η = 0.005, note that η is close to 1 λmax = 1 160 ).consistent pattern for the different networks and datasets that we explore.
...
Additional results on Batch-Normalization are presented in the Appendix Figure 2: Top: Evolution of the top 10 eigenvalues of the Hessian for SimpleCNN and Resnet-32 trained on the CIFAR-10 dataset with η = 0.1 and S = 128.
...
By η and S we denote the SGD learning rate and batch size, respectively.

H is the Hessian of the empirical loss at the current D-dimensional parameter value evaluated on the training set, and its eigenvalues are denoted as λ i , i = 1 . . .

D (ordered by decreasing absolute magnitudes).

λ max = λ 1 is the maximum eigenvalue, which is equivalent to the spectral norm of H. The top K eigenvectors of H are denoted by e i , for i ∈ {1, . . .

, K}, and referred to in short as the sharpest directions.

We will refer to the mini-batch gradient calculated based on a batch of size S as g (S) (t) and to ηg (S) (t) as the SGD step.
...
In the first experiment we train SimpleCNN and Resnet-32 using SGD with η = 0.1 and S = 128 and estimate the 10 largest eigenvalues of the Hessian, throughout training.
...
FIG3 shows the evolution of the two largest eigenvalues of the Hessian during training of the SimpleCNN and Resnet-32 on CIFAR-10, and an LSTM on PTB, for different values of η and S. We observe in this figure that a larger learning rate or a smaller batch-size correlates with a smaller and earlier peak of the spectral norm and the subsequent largest eigenvalue.
...
We study the same SimpleCNN and Resnet-32 models as in the previous experiment in the first 6 epochs of training with SGD with η=0.01 and S = 128.The sharpest direction and the SGD step.
...
For all training iterations, we compute DISPLAYFORM0 for α ∈ {0.25, 0.5, 1, 2, 4}; the expectation is approximated by an average over 10 different mini-batch gradients.

We find that E[L(θ(t) − αηg 1 (t))] increases relative to L(θ(t)) for α > 1, and decreases for α < 1.

More specifically, for SimpleCNN we find that α = 2 and α = 4 lead to a 2.1% and 11.1% increase in loss, while α = 0.25 and α = 0.5 both lead to a decrease of approximately 2%.

For Resnet-32 we observe a 3% and 13.1% increase for α = 2 and α = 4, and approximately a 0.5% decrease for α = 0.25 and α = 0.5, respectively.
...
We find the gradient to be highly aligned with the sharpest direction, that is, depending on η and model the maximum average cosine is roughly between 0.2 and 0.4.
...
Experiments on SimpleCNN and Resnet-32, trained with η = 0.01, S = 128, learning curves are provided in the Appendix.

Left two: DISPLAYFORM1 , for α = 0.5, 1, 2 corresponding to red, green, and blue, respectively.
...
At iteration t we plot the loss L(θ(t) + ke 1 ∆θ 1 (t)), around the current parameters θ(t), where ∆θ 1 (t) is the expected norm of the SGD step along the top eigenvector e 1 .

The x-axis represents the interpolation parameter k, the y-axis the epoch, and the z-axis the loss value, the color indicated spectral norm in the given epoch (increasing from blue to red).direction and the SGD step we scaled the visualization using the expected norm of the SGD step ∆θ 1 (t) = ηE(|g 1 (t)|) where the expectation is over 10 mini-batch gradients.

Specifically, we evaluate L(θ(t) + ke 1 ∆θ 1 (t)), where θ(t) is the current parameter vector, and k is an interpolation parameter (we use k ∈ [−5, 5]).

For both SimpleCNN and Resnet-32 models, we observe that the loss on the scale of ∆θ 1 (t) starts to show a bowl-like structure in the largest eigenvalue direction after six epochs.
...
Training and validation accuracy are reported in Appendix B. Furthermore, in the Appendix B we demonstrate that a similar phenomena happens along the lower eigenvectors, for different η, and in the later phase of training.
...
Validation accuracy and the λ max and Frobenius norm (y axis, solid and dashed) using increasing K (blue to red) compared against SGD baseline using the same η (black), during training (x axis).

Rightmost: Test accuracy (red) and Frobenius norm (blue) achieved using NSGD with an increasing K (x axis) compared to an SGD baseline using the same η (blue and red horizontal lines).Experiment.
...
NSGD is implemented as follows: instead of using the standard SGD update, ∆θ(t) = −ηg (S) (t), NSGD uses a different learning rate, η = γη, along just the top K eigenvectors, while following the normal SGD gradient along all the others directions 4 .

In particular we will study NSGD with a low base learning rate η, which will allow us to capture any implicit regularization effects NSGD might have.
...
We investigated NSGD with a different number of sharpest eigenvectors K, in the range between 1 and 20; and with the rescaling factor γ ∈ {0.01, 0.1, 1, 5}. The top eigenvectors are recomputed at the beginning of each epoch 5 .
...
We highlight that NSGD with K = 20 reaches a maximum λ max of approximately 8 · 10 3 compared to baseline SGD reaching approximately 150.
...
1 that using γ < 1 can result in finding a significantly sharper endpoint exhibiting a slightly improved generalization performance compared to baseline SGD using the same η = 0.01.

On the contrary, using γ > 1 led to a wider endpoint and a worse generalization, perhaps due to the added instability.
...
In comparison, NSGD using γ = 0.01 found endpoint corresponding to 87.0% test accuracy and ||H|| F = 1018 (87.4% and ||H|| F = 287 on SimpleCNN).

Finally, note that in the case of Resnet-32 K = 20 leads to 88% test accuracy and ||H|| F = 1100 which closes the generalization gap to SGD using η = 0.1.

We note that runs corresponding to η = 0.01 generally converge at final cross-entropy loss around 0.01 and over 99% training accuracy.
...
We report here the evolution of the spectral and Frobenius norm of the Hessian when using a simple learning rate schedule in which we vary the length of the first stage L; we use η = 0.1 for L epochs and drop it afterwards to η = 0.01.
...
In this section we report that large momentum similarly to large η leads to a reduction of spectral norm of the Hessian, see FIG0 for results on the VGG11 network on CIFAR10 and CIFAR100.
...
To ensure that the results do not depend on the learning rate, we rerun the Resnet-32 and SimpleCNN experiments with η = 0.05, see FIG0 .
...
In the case of Fashion-MNIST we observe that the final generalization for the case of γ < 1 and γ = 1.0 is similar.
...
In the next two experiments we explored how NSGD performs when using either a large base learning η, or when using momentum.

Results are reported in Tab.

3 (learning rate η = 0.1) and Tab.
...
This is visible in FIG14 , where we report results of a similar study as in Sec. 3.2, but for NSGD (K = 5, γ = 0.01).
...
First, recall that update in Newton method is typically computed as: DISPLAYFORM0 where λ is a scalar.

Now, if we assume that H is diagonal and put diag(H) = [100, 100, 100, 100, 100, 1, 1], and finally let λ = 0, it can be seen that the update of NSGD with γ = 0.01 and K = 5 is equivalent to that of Newton method.
...
We test η ∈ {0.025, 0.05, 0.1} and S ∈ {2, 8, 32}. As in Sec. 3.1 we observe that the learning rate and the batch-size limit the maximum curvature along the training trajectory.

In this experiment the phase in which the curvature grows took many epochs, in contrast to the CIFAR-10 experiments.

The results are summarized in FIG0 .Next, we tested Nudged SGD with η = 0.01, S = 8 and K = 1.

We test γ ∈ {0.1, 1.0, 5.0}. We increased the number of parameters of the base model by increasing by a factor of 2 number of filters in the first convolutional layer and the number of neurons in the dense layer to encourage overfitting.

Experiments were repeated over 3 seeds.

We observe that in this setting NSGD for γ < 1 optimizes significantly faster and finds a sharper region initially.

At the same time using γ < 1 does not result in finding a better generalizing region.
