...
Changes on the cause will be most informative, since they will have direct effects on B. This is sufficient to fully identify the causal graph (Hauser & Bühlmann, 2012) .

In order to demonstrate the advantage of choosing the causal model A → B over the anti-causal B → A, we can compare how fast the two models can adapt to samples from the transfer distributioñ p.
...
Let G be a causal graph, and p a (training) distribution that factorizes according to G, with parameters θ.
...
the parameters θ i such that V i / ∈ C of the log-likelihood under the transfer distribution will be zero
...
If we define a prior over graphs p(G) and a prior p(θ G | G) over the parameters of each graph G, the Bayesian score is defined as score B (G ;

(4) In the online likelihood, the adapted parameters θ (t) G act as summary of past data x 1:t−1 .

Eq. (3) can be seen as an approximation of the marginal likelihood in Equation (4), where the posteriors over the parameters p(θ G | x 1:t−1 , G) is approximated by the point estimate θ (t) G .
...
Recall that here, we only have two hypotheses to choose from: either A → B or B → A. We represent our belief of having an edge connecting A to B with a structural parameter γ such that p(A → B) = σ(γ), where σ(γ) = 1/(1 + exp(−γ)) is the sigmoid function.

We propose, as a meta-transfer objective, the negative log-likelihood R (a form of regret) over the mixture of these two models, where the mixture parameter is given by σ(γ):
...
This gradient is pushing σ(γ) towards the posterior probability that the correct model is A → B, given the evidence from the transfer data: Proposition 2.

The gradient of the negative log-likelihood of the transfer data D int in Equation (5) wrt.

the structural parameter γ is given by
...
This proposition (proved in Appendix C.3) shows that optimizing γ is equivalent to picking the hypothesis that has the smallest regret (or fastest convergence), measured as the accumulated loglikelihood of the transfer dataset D int during adaptation.
...
To illustrate the convergence result from Proposition 3, we experiment with learning the structural parameter γ in a bivariate model.
...
Since the correct causal model is A → B, the structural parameter should converge correctly, with σ(γ) → 1.
...
Accumulate the online log-likelihood for both models L A→B and L B→A as they adapt 9: Do one step of gradient ascent for both models: θ

Compute the regret R(D int )

Compute the gradient of the regret wrt.

γ (see Proposition 2) 12: Do one step of gradient descent on the regret w.r.t.
...
where the parameter θ is an N × N matrix.
...
In Figure 2 , we show the evolution of σ(γ), which is the model's belief of A → B being the correct causal model, as the number of episodes increases, for different values of N .

As expected, the structural parameter converges correctly to σ(γ) → 1, within a few hundreds episodes.
...
Similar to Figure 2 , we found that the structural parameter σ(γ) consistently converges to the correct causal model as well.
...
We consider the parameters of the encoder, as well as γ (see Section 3.2), as part of the set of structural meta-parameters to be optimized.
...
An extreme view of disentangling is that the explanatory variables should be marginally independent, and many deep generative models (Goodfellow et al., 2016) , and Independent Component Analysis models (Hyvärinen et al., 2001; Hyvärinen et al., 2018) , are built on this assumption.
...
While most of these approaches only rely on observational data, it is sometimes possible to extend the definition of these scores to interventional data (Hauser & Bühlmann, 2012) .
...
the structural parameter γ is

which concludes the first part of the proof.

Moreover, given Equation (35), it is sufficient to show that p(A → B | D int ) = σ(γ + ∆) to prove the equivalent formulation in Equation (25).

Using the logit function σ −1 (z) = log z 1−z , and the expression in Equation (28), we have
...
Framing the stationary point in terms of p rather than γ gives us a constrained optimization problem, with inequality constraints −p ≤ 0 and p − 1 ≤ 0, and no equality constraint.
...
with π A a probability vector of size N , and π B|a a probability vector of size N , which depends on the value of the variable A. In our experiment, each random variable can take one of N = 10 or N = 100 values.
...
This distribution corresponds to a fixed choice of π A emphasizes the fact that this defines the distribution prior to an intervention, with the mechanism p(B | A) being unchanged by the intervention.
...
The parameters θ A , θ B|A , θ B & θ A|B of the maximum likelihood estimate will be used as the initial parameters for the adaptation on the new transfer distribution.

Similar to the way we defined the training distribution, we can define a transfer distribution (p in the main text) as an intervention on the random variable A. In this experiment, this accounts for changing the distribution of A, that is with a new probability vector π (2) A , also sampled from a uniform Dirichlet distribution π
...
Consider a family of joint distributions p µ (A, B) over the causal variables A and B, defined by the following structural causal model (SCM):
...
The transfer distribution is the result of an intervention on A, where we shift the distribution p µ (A) with µ sampled uniformly in [−1, 1].

In Figure D .1, we plot samples from the training distribution (µ = 0), as well as two transfer distributions (µ = ±4).
...
Figure D. 2 shows the evolution of σ(γ) as training progresses.
...
where µ ∈ R d , β 0 ∈ R d and β 1 ∈ R d×d .

Σ and Σ are two d × d covariance matrices.
...
To build the training distribution, we draw µ (1) , β 0 and β 1 from a Gaussian distribution, and Σ

( 1) and Σ from an inverse Wishart distribution.

The transfer distribution is the result of an intervention on A, meaning that the marginalp(A) changes.

To do so, we sample new parameters µ (2) from a Gaussian distribution, and Σ (2) from an inverse Wishart distribution as well.
...
In Figure D .5, we show that, after 200 episodes, σ(γ) converges to 1, indicating the success of the method on this particular task.
...
B|a are the parameters of the conditional distribution before intervention, and π
...
The different modules and their corresponding parameters are defined in Table D In Figure D .8, we show the evolution of the structural parameter σ(γ), the model's belief that A → B is the correct causal model, as the number of episodes increases.

Unlike our previous experiments in Section 3.3, the structural parameter now converges to σ(γ) → 0, corresponding to a strong belief that the model is B → A. We are therefore unable to recover the correct causal graph here under the assumption that p(B | A) changes.
...
These variables are then mapped to observations (X, Y ) ∼ p µ (X, Y ) via a hidden (and unknown to the learner) decoder D = R θ D , where R θ is a rotation of angle θ.

The observations are then mapped to the hidden state (U, V ) ∼ p µ (U, V ) via the encoder E = R θ E ; in this experiment, the angle θ E is the only additional meta-parameter, besides the structural parameter γ.

The computational graph is depicted in Figure 3 .

In our experiment, θ D = −π/4 is fixed for all our observation and intervention datasets.
...
Note that here the online likelihood depends on the parameters of the encoder E (here, θ E ).
...
(80) On the one hand, the gradient of R(D int ; γ, θ E ) with respect to the structural parameter γ can be computed using Proposition 2, similar to the fully observable case.

On the other hand, the gradient of the meta-transfer objective with respect to the meta-parameter θ E is computed using backpropagation through the T updates of the parameters θ G of the modules in Equation (79); this process is similar to backpropagation through time.
...
The structural parameters, here, are the O(n 2 ) scalars γ ij .
...
the structural parameter γ ij is given by
...
(1) ij depend on the structural parameter γ ij .

Therefore we can now easily compute the gradient of R wrt.

γ ij only ∂R ∂γ ij = − ∂ ∂γ ij log σ(γ ij )E

(1) ij + (1 − σ(γ ij ))E (0) ij

If we substract σ(γ ij ) from this expression gives us

(1) ij + σ(γ ij )(1 − σ(γ ij ))E (0) ij

Denoting the previous expression as x, we can also easily compute 1 − x:

Using the logit function σ −1 (x) = log x 1−x , we can conclude that σ −1 σ(γ ij ) − ∂R ∂γ ij = log σ(γ ij )E
...
The gradient of the regret with respect to the meta-parameters γ i of node i is

Note that with the sigmoidal parametrization of p(B i ), log p(B i ) = B ij log σ(γ ij ) + (1 − B ij ) log(1 − σ(γ ij ))

as in the cross-entropy loss.

Its gradient can similarly be simplified to

A biased, but asymptotically unbiased, estimator of ∂R/∂γ ij is thus obtained by sampling K graphs (over which the means below are run):
...
We can therefore adapt Algorithm 1 using the gradient estimate in Proposition 6 to update the structural parameters γ ij , without having to explicitly compute the full regret R(D int ).
...
Let {B (k) } K k=1 be K binary matrices (corresponding to sample graphs), sampled from independent Bernoulli distributions depending on the structural parameters γ ij
...
the structural parameter γ ij by
