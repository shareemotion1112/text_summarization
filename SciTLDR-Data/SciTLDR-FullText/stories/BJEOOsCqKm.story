...
The resulting objective is given by DISPLAYFORM1 where max(Ŷ shuffled ) is the maximum predicted value of all shuffled, and min(Ŷ ref ) is minimum predicted value of all reference inputs, and W are the model weights.
...
Since our MinMax-loss depends onŶ ref andŶ shuffled the classifier is trained by alternately propagating reference and shuffled texture patches through the same network (cf.
...
The model by Schütt & Wichmann FORMULA1 is a psychophysical, image-computable model for early spatial visual processing.
...
We use the following configuration: Gabor standard deviations σ F = 0.5945 for the spatial frequency and σ θ = 0.2965 for the orientation, nonlinearity constant C = 0.0027, nonlinearity exponent p = 2.1698, difference exponents q = 1.8667, and pool orientation ω θ = 0.1112.The SW-features are 96 times overcomplete.
...
In particular, the normalized features are represented by a vector containing the feature maps of a certain layer which are normalized by the squared ℓ 2 -norm.
...
Again, for all models we choose ν = 0.001 as upper bound on the fraction of training errors.
...
The model trained with the VGG-19 texture representation achieves AUC scores greater than 0.9 on both textures, when evaluating examples distorted by additive Gaussian noise with a standard deviation of σ = 0.1.
