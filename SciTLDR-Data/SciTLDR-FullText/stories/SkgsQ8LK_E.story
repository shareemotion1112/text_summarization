...
It contains two branches: 1) the structural content branch forces the representation into a Gaussian spatial probability distribution with an hourglass network eω.

2) the style branch E φ learns a complementary style representation to the content.

Equation 1 learns a deterministic mapping e(·; θ) from x to y, which we assume y is following a Gaussian distribution over N (e(x; ω), Σ).

Term − log p(y|x) is non-negative.

In the second line of the equation, we start to consider the factor z. Similar to VAE, we address the issue of intractable integral by introducing an approximate posterior q(y, z|x; φ) to estimate the integral using evidence lower bound (ELBO).
...
Following the notations from BID22 , we denote the direct outputs of the hourglass network e ω as landmark heatmaps h, and each channel of which represents the spatial location of a structural point.
...
σ can be regarded as a normalization factor here.
...
BID4 proposed a Joint-β-VAE by adding different prior distribution over several latent factors to disentangle continuous and discrete factors from data.
...
We assume that the decoder D θ is able to reconstruct original input x from latent representation y and z, which isx = G(y, z).
...
We consequently define the reconstruction loss as DISPLAYFORM1 , where ψ l is the feature obtained from l-th layer of a VGG-19 model BID32 pre-trained on ImageNet.
...
We model q(z|x, y) as a parametric Gaussian distribution which can be estimated by the encoder network E φ .

Therefore, the style code z can be sampled from q(z|x, y).

Meanwhile, the prior p(z|y) can be estimated by the encoder network E θ .
...
The framework only makes sure z is from x and the Decoder D θ will recover as much information of x as possible.

There is no guarantee that z will learn a complementary of y. Towards this end, as shown in Fig. 2 , we design the network as fusing the encoded content representation by E θ with the inferred style code z. Then, the fused representation is decoded together by D θ .

Meanwhile, skip-connections between E θ and D θ are used to pass multi-level content information to the decoder.
...
A one-stack hourglass network BID22 ) is used as a geometry extractor e ω to project input image to the heatmap y ∈ R 256×256×30 , in which each channel represents one point-centered 2D-Gaussian map (with σ = 4).

y is drawn in a single-channel map for visualization in Fig. 2 .

Same network (with stride-2 convolution for downsampling) is used for both E θ and E φ to obtain style representation z and the embedded content representation as two 128-dimension vectors.

A symmetrical deconvolution network with skip connection is used as the decoder D θ to get the reconstructed result x. All of the networks are jointly trained from scratch end-to-end.
...
We compare perceptual quality with the three most related unsupervised representation learning methods in FIG4 e., VAE (Kingma & Welling, 2014), β-VAE BID8 and InfoGAN .
