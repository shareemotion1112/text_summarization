...
Hence, we propose to model the vector-valued generator function Gpzq : R d Ñ R o by a high-order multivariate polynomial of the latent vector z, whose unknown parameters are naturally represented by high-order tensors.
...
The mode-m unfolding of a tensor X P R I1ˆI2ˆ¨¨¨ˆI M maps X to a matrix X pmq P R ImˆĪm with

I k such that the tensor element x i1,i2,...,i M is mapped to the matrix element x im,j where j " 1`ř

n"1 n‰m I n .

The mode-m vector product of X with a vector u P R Im , denoted by Xˆn u P R I1ˆI2ˆ¨¨¨ˆIn´1ˆIn`1ˆ¨¨¨ˆI N , results in a tensor of order M´1:

(1)

The Khatri-Rao product (i.e., column-wise Kronecker product) of matrices A P R IˆN and B P R JˆN is denoted by A d B and yields a matrix of dimensions pIJqˆN .

The Hadamard product of A P R IˆN and B P R IˆN is defined as A˚B and is equal to A pi,jq B pi,jq for the pi, jq element.
...
An M th-order tensor X P R I1ˆI2ˆ¨¨¨ˆI M has rank-1, when it is decomposed as the outer product of M vectors tu
...
where the scalar β i , and the set of tensors W

are the parameters of the polynomial expansion associated to each output of the generator, e.g., pixel.

Clearly, when n " 1, the weights are d-dimensional vectors; when n " 2, the weights, i.e., W r2s i , form a dˆd matrix.

For higher orders of approximation, i.e., when n ě 3, the weights are n th order tensors.

By stacking the parameters for all pixels, we define the parameters β .
...
, U rns,pn`1q ssu N n"1 , with U rns,1 P R oˆk , and U rns,m P R dˆk , for m " 2, . . .
...
• Let assume W r2s being a superposition of of two weights tensors, namely W r2s " W r2s 1:2Ẁ r2s 1:3 , with W r2s i:j denoting parameters associated with the second order interactions across the i-th and j-th order of approximation.
...
The parameters are C P R oˆk , U rms P R dˆk for m " 1, 2, 3.
...
Let us first introduce learnable hyper-parameters b rns P R ω ( N n"1 , which act as scaling factors for each parameter tensor.
...
˙S r3s * T with C P R oˆk , A rns P R dˆk , S rns P R kˆk , B rns P R ωˆk for n " 1, . . .
...
Orig': the Hadamard products are ditched, while use b r1s Ð z, i.e., there is a composition of linear layers that transform the noise z.

Sinusoidal: We assess the polynomial-based generator on a sinusoidal function in the bounded domain r0, 2πs.
...
The input to the generator is z P R and the output is rx, sin xs with x P r0, 2πs.
...
We capitalize on the expressivity of the recent resnet-based generator (Miyato et al., 2018; Brock et al., 2019) , to devise a new polynomial generator Gpzq : R 128 Ñ R 32x32 .
...
κ "´S rns κ`pB rns q T b rns¯˚´p A rns q T v7 end 8 x " β`Cκ.
...
For a set of matrices tX m P R ImˆN u N m"1 the Khatri-Rao product is denoted by:

In this section, we prove the following identity connecting the sets of matrices tA ν R IνˆK u N ν"1 and
...
Initially, both sides of the equation have dimensions of KˆL, i.e., they match.
...
From the definition of Khatri-Rao, it is straightforward to obtain the pρ, iq element with ρ " pk 1´1 qI 2`k2 , (i.e. ρ P r1, I 1 I 2 s) of A 1 d A 2 as A 1,pk1,iq A 2,pk2,iq .

Similarly, the pρ, jq element of B 1 d B 2 is B 1,pk1,jq B 2,pk2,jq .

The respective pi, jq element of the left hand side (lhs) of (13) is:

In the last equation, we replace the sum in ρ (ρ P r1, I 1 I 2 s) with the equivalent sums in k 1 , k 2 .
...
The rhs includes the Hadamard products of the matrices A T ν¨Bν .

Each matrix multiplication (A T ν¨Bν ) results in a matrix of KˆL dimensions.

Thus, the rhs is a matrix of KˆL dimensions.

The lhs is a matrix multiplication of two Khatri-Rao products.

The first Khatri-Rao product has dimensions Kˆp ś ν I ν q, while the second p ś ν I ν qˆL. Altogether, the lhs has KˆL dimensions.
...
where s 1 " i and s ν is a recursive function of the s ν´1 .

However, the recursive definition of s ν is summed in the multiplication and we obtain: Below, we prove that (7) (main paper) is equivalent to the three-layer neural network as shown in Figure 2 .
...
with ω as in Claim 2.

Then, it holds for Gpzq of (10) that Gpzq " λ.
...
´A r1s dB r1s¯Sr2s ı * and M 2 "´A r2s dB r2s¯.

The last term of (27) becomes:

Replacing (28) into (27), we obtain (10).

Note that the λ in Claim 3 is the equation behind Figure 3 .
...
As done in Section 2.2, we capture the n th order interactions by decomposing the parameter tensor W rns (with 2 ď n ď N ) as:
...
The term λ is equal to the κ " pn´1q th order of (31), while there is only a single term for n " N .
...
Astroid: We implement a superellipse with parametric expression rα cos 3 t, α sin 3 ts for t P r´α, αs.
...
The function we want to learn is Gpzq : R 2 Ñ R 3 with the data manifold described by the vector rx, y, sinp10˚ax 2`y2 qs for x, y P r´0.5, 0.5s.
...
Gabriel's Horn: The three dimensional vector rx, α¨c os t x , α¨s in t x s for t P r0, 160πs and x P r1, 4s forms the data manifold.
...
We rescale all of the images into 64ˆ64 for our analysis.
...
CIFAR10 includes 50, 000 training images of 32ˆ32ˆ3 resolution.
...
• CIFAR10 (Krizhevsky et al., 2014) includes 60, 000 images of 32ˆ32 resolution.
...
We reshape the images to 128ˆ128 resolution.
...
where x is a generated sample and ppy|xq is the conditional distribution for labels y. The distribution ppyq over the labels is approximated by 1 M ř M n"1 ppy|x n q for x n generated samples.
...
Denoting the representations of real images as N pµ r , C r q and the generated (fake) as N pµ f , C f q, FID is:

In the experiments, we use M " 10, 000 to compute the mean and covariance of the real images and M " 10, 000 synthesized samples for µ f , C r .
...
Sinusoidal on 2D: The data distribution is described by rx, sinpxqs with x P r0, 2πs (see Section 4.1 for further details).
