...
We use p θ (y|x i ) to denote the output distribution over classes produced by a model with parameters θ on input x i .
...
First, the model produces soft targets for the current example:ŷ i = p θ (y|x i ).

The model is then trained to minimize the consistency cost DISPLAYFORM1 where D is a distance function (we use KL divergence) and η is a perturbation to the input that can be chosen randomly or adversarially.
...
It outputs a distribution over labels, usually with a softmax layer (an affine transformation followed by a softmax activation function) applied to this representation: DISPLAYFORM2 At test time, only the main prediction layer p θ is used.
...
We add the distances between the output distributions of the teacher and auxiliary students to the consistency loss, resulting in a cross-view consistency (CVC) loss: DISPLAYFORM3 We combine the supervised and CVC losses into the total loss, L = L sup + λ 2 L CVC , and minimize it with stochastic gradient descent.
...
λ 1 and λ 2 are hyperparameters controlling the strength of the auxiliary prediction layers and the strength of the unsupervised loss.

For all experiments we set λ 1 = k and λ 2 = 1 unless indicated otherwise.

See FIG0 for an illustration of the training procedure.

Although adding noise or an adversarial perturbation to the input generally improves results, L CVC can be trained without this enhancement (i.e., setting η = 0).
...
In contrast, L consistency requires a nonzero η to make the student and teacher output different distributions.
...
The primary prediction layer for our CNNs take as input the mean of H over the first two dimensions, which results in a d-dimensional vector that is fed into a softmax layer: p θ (y|x i ) = SML(global average pool(H)).We add n 2 auxiliary softmax layers to the top of the CNN.

The jth layer takes a single feature vector as input, as shown in the left of FIG1 : p j θ (y|x i ) = SML(H j/n ,j mod n ).
...
Mathematically, the probability of an edge is given as p θ ((u, t, r) DISPLAYFORM1 Where s is the scoring function s(z 1 , z 2 , r) = MLP head (z 1 )(W r + W )MLP dep (z 2 ).
...
We optimize L with λ 1 = 1 and each minibatch consisting of 32 labeled and 128 unlabeled examples.

Miyato et al. use Virtual Adversarial Training (VAT) , minimizing L consistency with the input perturbation η chosen adversarially.
...
We note we only tried these experiments with λ 1 = 1, but this value of λ 1 did work well for the semi-supervised setting.
