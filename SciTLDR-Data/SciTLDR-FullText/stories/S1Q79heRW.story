...
The consistency requirement is defined as y satisfying a prior θ(y), which embodies all the the constraints and correlations between features in the vector.
...
If the features of x e and x e are consistent, then it will be easier for y to satisfy the prior θ(y).
...
Distributions over these discrete vectors are represented as continuous vectors of log-odds X, so P (x i =1) = σ(X i ), where σ is the logistic sigmoid.
...
DISPLAYFORM2 where E Y,X e ,Xe is the expectation over the distribution defined by the log-odds vectors Y, X e , X e , and log and σ are applied componentwise.

The term θ(Y ) is used to indicate the net effect of the prior on the vector Y .

Note that, in the formula (3) for inferring Y , the contribution − log σ(−X) of each word vector is also a component of the definition of Y > X from equation (1).
...
This function − log σ(−X) is a nonnegative transform of X, as shown in figure 2.
...
DISPLAYFORM3 Figure 2: The function − log σ(−X) used in inference and the > operator, versus X.Both vectors X e and X p are parameters of the model, which need to be learned.

Thus, there is no need to explicitly model the prior, thereby avoiding the need to choose a particular form for the prior θ, which in general may be very complex.
...
In particular, we note that although we want X p to approximate the effects θ(Y ) of the prior as in equation 4, in fact X p is only dependent on one of the two words, and thus can only incorporate the portion of θ(Y ) which arises from that one word.
