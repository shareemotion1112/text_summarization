...
Here, we choose a flexible class of local deformations of image coordinates, i.e., maps τ : R 2 → R 2 such that ∇τ ∞ < C for some C, similar to BID19 .
...
For a representation r mapping from an input image (e.g., 224x224x3) to some layer of a CNN (e.g., a tensor of size 7x7x512), we measure sensitivity of the representation r to a deformation τ using the Normalized Cosine Distance: DISPLAYFORM0 where d cos is the cosine distance.
...
We smooth filters by convolving with a Gaussian filter with standard-deviation σ and then measure the sensitivity to deformation.

As we increase the smoothness of the filters by increasing σ, the representations became less sensitive to deformation.
