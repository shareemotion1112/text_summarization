...
However, the L 1 norm is proportional to the scaling of parameters, i.e., ||αW || 1 = α||W || 1 , with a non-negative constant α.
...
λ s and λ o are the decay parameters for the sparsity-inducing regularization loss and the orthogonality loss respectively, which are hyperparameters of the proposed training process.

λ o can be chosen as a large positive number to enforce the orthogonality of singular vectors, and λ s can be modified to explore the tradeoff between accuracy and FLOPs of the achieved low-rank model.
...
The resulted low-rank model will then be finetuned with λ s set to zero for further performance recovery.
...
We separately train two decomposed model with the same optimizer and hyperparameters, one with the orthogonality loss of λ o = 1.0 and the other with λ o = 0.
...
In this section, we comprehensively explore the accuracy-compression rate tradeoff of ResNet models in various depths and under different configurations by changing the strength of the sparsity-inducing regularizer (λ s in Equation (3)).
...
During the SVD training, the decay parameter of the orthogonality regularizer λ o is set to 1.0 for both channel-wise and spatial-wise decomposition on CIFAR-10.

On ImageNet, λ o is set to 10.0 for channel-wise decomposition and 5.0 for spatial-wise decomposition.

The decay parameter λ s for the sparsity-inducing regularizer and the energy threshold used for singular value pruning are altered through different set of experiments to fully explore the accuracy-#FLOPs tradeoff.
...
The λ s and the energy thresholds used in each set of the experiments are reported alongside the experiment results in Appendix B.
