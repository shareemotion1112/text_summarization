...
Each layer except f 0 is associated with a parameter sub-vector θ l that collectively make up the parameter vector θ = (θ 1 , .., θ L ).
...
In a nutshell, all we have to do to construct such a network is to take an arbitrary network of desired depth that can be trained successfully and scale each layer function f l and each parameter sub-vector θ l by R −l for some constant R > 1.
...
When f k is a fully-connected linear layer without trainable bias parameters and θ k contains the entries of the weight matrix, GSC(k, l) DISPLAYFORM1 measures the quadratic expectation of the relative size of the change in the value of f l in response to a small random change in θ k .
...
Let the terms in df0 dx containing λ or more residual Jacobians be called 'λ-residual' and let res λ be the sum of all λ-residual terms.

Then: DISPLAYFORM2 Again, if p < 1, the right hand side decreases exponentially in λ for sufficiently large λ, for example when λ > L 2 , so the combined size of λ-residual terms is exponentially small.
...
Only sets of layers of size at most λ where res λ is not negligably small co-adapt.

The largest such λ is called the 'effective depth' of the network.
...
We can simply choose arbitrary initial functions i l and define r l (θ l ) := f l (θ l ) − i l .

Specifically, if we train a network f from some fixed initial parameter θ (0) , we can set DISPLAYFORM0 Then training begins with all residual functions being zero functions.
...
The proof is based on the insight that the relative size of a gradient-based update ∆θ l on θ l is bounded by the inverse of the GSC if that update is to be useful.
...
Let's assume we apply a random update to θ l with relative magnitude (0,l) .
...
This is not reflective of the true behavior of the function f 0 in response to changes in θ l of this magnitude.
...
But to reach a certain effective depth, a certain magnitude of λ-residual terms is required and thus a certain magnitude of residual functions relative to corresponding initial functions is required, and thus exponentially many updates.

Theorem 1.

Under certain conditions, if an MLP has exploding gradients with explosion rate r and intercept c on some dataset, then there exists a constant c such that training this neural network with a gradient-based algorithm to have effective depth λ takes at least c cr Importantly, the lower bound on the number of updates required to reach a certain effective depth stated by theorem 1 is independent of the nominal depth of the network.
...
We devised a conservative, computationally tractable estimate of the cumulative size of updates that stem from λ-residual terms.

See section D.2 for details.

The effective depth depicted in FIG5 is the largest value of λ such that this estimate has a length exceeding 10 −6 .
...
Under certain conditions, for any neural network f with random parameter θ composed of layer functions f l that are surjective endomorphisms on the hypersphere, where the absolute singular values of the Jacobian of each layer are IID and differ by at least with probability δ, gradients explode in expectation with rate r(δ, ).
...
The larger the value of k, the more ρ b is "diluted" by a linear function, bringing f b itself closer and closer to a linear function.
...
We let the initial function i encompass not just the skip function s, but also the initial block function ρ.
...
The skip function s and block function ρ can be scaled with constants to achieve arbitrary, desired levels of dilution (Szegedy et al., 2016) .
...
We say a residual network f (θ) has an 'orthogonal initial state' if each initial function i l is multiplication with an orthogonal matrix or a slice / multiple thereof and r l (θ l ) is the zero function.
...
For a tanh MLP with no biases, using gradient vector length, vanishing is achieved for σ w < 1, stability for σ w = 1 and explosion for σ w > 1.

(σ w denotes the standard deviation of weight matrix entries times the square root of the right dimsion of the weight matrix, as defined in Poole et al. (2016) .)

For a tanh MLP with no biases, using the GSC, vanishing is impossible, stability is achieved for σ w ≤ 1 and explosion for σ w > 1.

For a ReLU MLP with no biases, using gradient vector length, vanishing is achieved for σ w < √ 2, stability for σ w = √ 2 and explosion for σ w > √ 2.
...
For a ReLU MLP with no biases and σ w > √ 2, infinitesimal noise grows (chaos), yet the correlation converges sub-exponentially to zero, which is a behavior we would expect from the edge of chaos.
...
In a tanh MLP with no bias and σ w slightly larger than 1, correlation converges to 0 and eventually, gradients explode.

Yet the domain can still collapse dramatically in the short term as shown in FIG3 to cause pseudolinearity.

In a tanh MLP with no bias and σ w very large, again, correlation converges to 0 and gradients explode.
...
We write f l (θ, x) as a short form of x) )..)).

Sometimes, we omit x and / or θ.

In that case, x and / or θ remains implicit.

f l (θ, X) is an analogous short form.

DISPLAYFORM0 • We write DISPLAYFORM1 .

Sometimes, we omit f k and / or θ.

In that case, f k and / or θ remain implicit.

f l (θ, F k ) is an analogous short form.•
...
Let a 'gradient-based algorithm' for training a mutable parameter vector θ from an initial value θfor a network f be defined as a black box that is able to query the gradient DISPLAYFORM0 at arbitrary query points (X, Y ) but only at the current value of the mutable parameter vector θ.

It is able to generate updates ∆θ which are added to the mutable parameter vector θ.

Let the sequence of updates be denoted as ∆θ(1) , ∆θ (2) , ...

We define the successive states of θ recursively as θ (t) = θ (t−1) +∆θ (t) .
...
We call a term 'λ-residual' if it contains λ or more Jacobians of residual functions, as opposed to Jacobians of initial functions.

Let res λ l (f, θ, X, Y ) be the sum of all λ-residual terms in DISPLAYFORM2 Now consider two scenarios.

In scenario (1), when the algorithm queries the gradient, it receives { DISPLAYFORM3 , .., DISPLAYFORM4 e. the "regular" gradient.

In scenario (2), it receives DISPLAYFORM5 e. a version of the gradient where all λ-residual terms are removed.

Let the parameter vector attain states θ (1) , θ (2) , .. in scenario (1) and θ (1,λ) , θ (2,λ) , .. in scenario (2).

Then we say the 'λ-contribution' at time t is θ (t) − θ (t,λ) .

Finally, we say the 'effective depth at time t with threshold h' is the largest λ such that there exists an l with ||θ DISPLAYFORM6 There is no objectively correct value for the threshold h. In practice, we find that the λ-contribution decreases quickly when λ is increased beyond a certain point.

Hence, the exact value of h is not important when comparing different networks by effective depth.

The impact that the shift θ DISPLAYFORM7 has on the output of the network is influenced by the scale of θ (t) l as well as GSC(l, 0).
...
Then SGD computes DISPLAYFORM0 DISPLAYFORM1 For any update t and query point b, we estimate its λ-contribution at layer l as follows.

For unparametrized layers, ||r k || op is set to zero.

For linear layers, it is the operator norm of the residual weight matrix.

The final estimate of the length of the λ-contribution at layer l for the entire training period is then simply the sum of the lengths of the estimated λ-contributions over all time points and query points.
...
In other words, we assume that in λ-residual terms, the large singular values of layer-wise Jacobians do not compound disproportionately compared to other terms.
...
We use the sum of the lengths of all λ-residual terms in a batch as a conservative bound on the length of the λ-contribution of the batch.

In essence, we assume that all λ-residual terms have the same orientation.

Finally, we use the sum of the lengths of the λ-contributions within each update as an estimate of the length of the total λ-contribution of the entire training period.

On the one hand, this is conservative as we implicitly assume that the λ-contributions of each batch have the same orientation.

On the other hand, we ignore indirect effects that λ-contributions in early batches have on the trajectory of the parameter value and hence on λ-contributions of later batches.

Since we are ultimately interested in effective depth, we can ignore these second-order effects as they are negligible when the total λ-contribution is close to a small threshold h.
...
We write f n (f m ) = s n (f m ) + ρ n (f m ), where f n is the layer at which the skip connection terminates, f m is the layer at which the skip connection begins, s n is the function computed by the skip connection and ρ n (f m ) = ρ n (f n+1 (..f m−1 (f m )..)) is the function computed by the residual block.

We write f k = i k + r k for n + 1 ≤ k ≤ m − 1 and ρ n = i n + r n , i.e. we break down each layer in the residual block into an initial function and a residual function.
...
The algorithm can be used to compute T updates by querying gradients of f at the current parameter value and at query points in D which cause θ to attain a valueθ where f attains error E final on D and makes the same predictions as f (θ) on D.

Proof.

Since Θ and D are closed and bounded, so is Θ × D. Therefore for all 0 ≤ l ≤ k ≤ L, both ||J l k || and ||T l k || attain their infimum on that domain if it exists.

||.|| is non-negative, so the infimum exists.

||.|| is non-zero on the domain, so the infimum, and therefore the minimum, is positive.

Since f has finite depth, there is an r such that for all tuplets (θ, X, Y, k, l), we have ||J l k || ≥ r k−l and DISPLAYFORM0 r .
...
Whenever the algorithm queries some gradient value df dθ , we instead submit to it the value R −1 ( df dθ ).

Whenever the algorithm wants to apply an update ∆θ to the parameter, we instead apply R −1 (∆θ).

Let S (t) , 0 ≤ t ≤ T be the state of the system after applying t updates to θ under this training procedure.

Let S (t) , 0 ≤ t ≤ T be the state of the system after applying t updates to θ when the algorithm is run on f .

Then the following invariances hold.

DISPLAYFORM3 , where θ (t) is the value of θ under S (t) and θ (t) is the value of θ under S (t) .B f makes the same predictions and attains the same error on D under S (t) as f under S (t) .C Any state the algorithm maintains is equal under both S (t) and S (t) .We will show these by induction.

At time t = 0, we have θ (0) = R −1 (θ (0) ) as chosen, so (A) holds.
...
Assuming that θ (t) = R −1 (θ (t) ), it is easy to check that DISPLAYFORM4 ).
...
Because the algorithm returns the same update in both cases, after the prescribed post-processing of the update under f , we have ∆θ DISPLAYFORM6 ).

This completes the induction step for (A) and again, (B) follows easily from (A).(B) implies directly that claim (2) holds.

Finally, for any tuplet (θ, X, Y, k, l), we have ||T DISPLAYFORM7 Therefore, claim (3) also holds, which completes the proof.
...
We use LΣR T to denote the singular value decomposition and s i to denote singular values.

DISPLAYFORM9 Proposition 3.

Let U be the uniform distribution over the hypersphere.

Assume f k is a fullyconnected linear layer without trainable bias parameters and θ k contains the entries of the weight matrix.
...
Throughout this derivation, we will use θ k to refer to both the parameter sub-vector and the weight matrix.

Similarly, we will use u to refer to both a perturbation of the parameter sub-vector and of the weight matrix.

We use LΣR T to denote the singular value decomposition and s i to denote singular values.

DISPLAYFORM13 Further, assume that θ k is random and fulfills the conditions stated.

Under those conditions, θ k is the product of a random scalar length variable and an independent random vector orientation variable θ k of unit length.

Then for all DISPLAYFORM14 .

Since all entries of θ k have the same quadratic expectation, all entries in θ k have the same quadratic expectation.

Further, DISPLAYFORM15 , so the expectation of the product of two different entries of θ k is 0.Then, we have: DISPLAYFORM16 The conditions stated in the proposition for the random parameter sub-vector θ k are fulfilled, for example, if the corresponding weight matrix is either Gaussian initialized or orthogonally initialized.

Therefore, the most popular initialization strategies for weight matrices are covered by this proposition.

Proposition 4.

Given: DISPLAYFORM0 • constants c 2 , .., c L and γ 1 , .., γ L• a network f of nominal depth L defined via its layer functions as follows.
...
Any endomorphism on the hypersphere composed of (i) a strictly monotonic, continuous nonlinearity σ that has σ(0) = 0, (ii) multiplication with a full-rank matrix and (iii) length-only layer normalization is bijective.
...
Let this point on the ray be x. Assume its inverse image contains two points y and z. Then for 1 ≤ i ≤ d, σ(y(i)) = x(i) and σ(z(i)) = x(i) and so σ(y(i)) = σ(z(i)).

But y = z, so there exists an i such that y(i) = z(i).

So there exist two different values y(i) and z(i) at which σ returns the same result.

But σ is strictly monotonic.

Contradiction.

Claim 2: If two points x 1 and x 2 on the ray have inverse images y 1 and y 2 and x 1 is closer to the origin than x 2 , then for 1 ≤ i ≤ d, we have |y 1 (i)| ≤ |y 2 (i)|.For 1 ≤ i ≤ d, σ attains x 2 (i) at y 2 (i) and 0 at 0.

Since σ is strictly monotonic, continuous and 0 ≤ |x 1 (i)| ≤ |x 2 (i)| and x 1 (i) and x 2 (i) have the same sign, σ attains x 1 (i) at a point between 0 and y 2 (i).
...
Then we must have DISPLAYFORM2 Since σ is strictly monotonic and σ(0) = 0, σ either preserves the sign of all inputs or reverses the sign of all inputs.
...
Since σ is strictly monotonic and σ(0), σ either preserves the sign of all values or it reverses the sign of all values.
...
Because σ is continuous, its value at y lim (i) is the limit of σ(y n (i)).
...
As σ is strictly monotonic and σ(0) = 0, σ either preserves the sign or reverses it for all inputs.
...
Since σ is continuous, the forward image of y lim is the limit of forward images of the y n .
...
Since σ has σ(0) = 0 and it is strictly monotonic, y(i) = 0.

For i ∈ I, let σ max (i) = σ(2y(i)).

Since σ has σ(0) = 0 and it is strictly monotonic, we have |σ(2y(i))| > |σ(y(i))| > 0 and so |σ max (i)| > |x(i)| > 0 and also σ max (i) and x(i) have the same sign.

Let C = min i∈I σmax(i)x(i) .

Take some vector y that can vary.

Since σ is continuous, as y (i) varies between y(i) and 2y(i), it attains all values between x(i) and σ max (i).

So for all 1 ≤ c ≤ C, we can set y (i) to some value y c (i) such that σ(y c (i)) = cx(i).
...
The proposition also applies if each neuron in the nonlinearity layer uses a different nonlinearity σ i as long as it fulfills the stated conditions.

We say a random function f b is 'k-diluted in expectation' with respect to random vector v if there exists a random matrix S b and a random function DISPLAYFORM0 We say a random function ρ(v) is 'scale-symmetric decomposable' (SSD) if it can be written as u ρ ρ ( v ||v||2 )||v|| 2 , where ρ is a random scalar function and u ρ is uniformly distributed on the hypersphere and independent of both ρ and v.
...
Given random functions f b , 1 ≤ b ≤ B, which are k b -diluted in expectation with respect to u, a matrix S b that is either SSD or a multiple of the identity and an SSD random function ρ b := f b − S b where all the S b and ρ b are independent, DISPLAYFORM1 2 -diluted in expectation with respect to u.
...
For any u, ρ 1 (u) is radially symmetric because ρ 1 is SSD.
...
Because the orientation of ρ 1 (u) is governed only by u ρ1 which is independent of both u and S 1 , the orientations of S 1 u and ρ 1 (u) are independent.
...
We also have DISPLAYFORM10 is radially symmetric with an orientation independent of that of S 1 f • u because it is governed only by u ρ1 .
...
DISPLAYFORM11 is radially symmetric by the induction hypothesis so as before, S 1 ρ • (u) is radially symmetric.
...
Unfortunately, many ResNets do not quite fulfill the SSD condition on ρ b , but they come close.

If the last operation of each ρ b is multiplication with an SSD matrix, a popular choice, the orientation of ρ b is indeed governed by an independent, uniform unit length vector u ρ as required.

However, many choices of ρ b do not preserve the length of the incoming vector ||v|| 2 as in an SSD function.

However, because the inputs to each ρ b are random and high-dimensional, we do not expect their lengths to vary much, especially if the original inputs to the network have themselves been normalized.
...
A block function ρ b that is SSD, for example, is any such function that is composed of only linear and ReLU layers, where the final layer is Gaussian or orthogonally initialized.
...
Let each linear layer be the sum of an unparametrized initial function i ln and a parametrized residual function r ln (θ ln ).
...
r ln (θ ln ) represents multiplication with the residual weight matrix and is used entries of the residual weight matrix.

Let an N -trace φ N be a subset of {1, .., N }.

Let Φ N be the set of all possible N -traces and let Φ λ N be the set of all N -traces of size λ or more.

We define the 'gradient term' G(φ N , f, θ, X, Y ) : DISPLAYFORM0 if layer k is not a linear layer, J k = r ln (θ ln ) if layer k corresponds to linear layer l n and n ∈ φ N , and J k = i ln if layer k corresponds to linear layer l n and n ∈ φ N .Let res DISPLAYFORM1 Theorem 1.

Consider an MLP f as defined above with all parameter sub-vectors initialized to θ (0) ln = 0.
...
Note that for convenience, we divide the size of the update ||∆θ DISPLAYFORM5 ln || 2 by the weight matrix in the initialized state ||i ln || 2 instead of ||θ (t−1) ln || 2 .
...
Firstly, ignoring the alg() function, it bounds the length of the sum of the Λ-residual terms.
...
In other words, we assume that in λ-residual terms, the large singular values of layer-wise Jacobians do not compound disproportionately compared to the full gradient.

The bound is however also very conservative in the sense that it implicitly assumes that all λ-residual terms have the same orientation.
...
Importantly, the bound on T is exponential in Λ and independent of both L and N .
...
Consider a neural network f with random parameter θ composed of layer functions f l that are surjective endomorphisms on the d-dimensional hypersphere.

Let J l := J l l+1 .

Let S be the hypersphere and let S be the uniform distribution on it.

Let F l , 1 ≤ l ≤ L, be random vectors independent of θ where F l ∼ S. Assume:1.

The θ l are independent of each other.2.

Each Jacobian J l (θ l , F l+1 ) has d − 1 nonzero singular values which, as F l+1 and θ l vary, are independent and drawn from some distribution P l .3.

There exist some > 0 and δ > 0 such that DISPLAYFORM0 4.

f l (θ l , F l+1 ) is a uniformly distributed vector on the hypersphere.5.

For any unit length vector u, J l (θ l , F l+1 )u can be written as l (θ l , F l+1 , u)u l .

Here, l is random scalar independent of both u l and f l (θ l , F l+1 ).

u l , conditioned on f l (θ l , F l+1 ), is uniformly distributed in the space of unit length vectors orthogonal to f l (θ l , F l+1 ).Let X ∼ S be a random vector independent of θ.

Then setting r(δ, ) := √ 1 + δ 2 we have DISPLAYFORM1 Proof.
...
We use ||J l || qm to denote the quadratic mean of the singular values excluding the zero singular value, i.e. DISPLAYFORM2 As f l is surjective, for fixed θ l , we have by integration by substitution DISPLAYFORM3 But we also have DISPLAYFORM4 i=1 |s i | ≥ 1.
...
Now, let's look at the second term Q θ l ,f l+1 ,u l+1 ||J l l+1 (θ l , f l+1 )u l+1 || 2 .
...
Since u l+1 is also independent of θ l , we have DISPLAYFORM10 Putting those results together, we obtain DISPLAYFORM11 This is the desired claim.
...
If we exchange those two operations then the last operation applied in both f l (θ l , F l+1 ) and J l (θ l , F l+1 )u is the orthogonal transformation, which decouples the orientations of both terms from the length of J l (θ l , F l+1 )u as well as decoupling the orientations of the terms from each other up to preserving their angle.

Finally, note that J l (θ l , F l+1 )u always lies in the left-null space of J l (θ l , F l+1 ).

But that space is orthogonal to f l (θ l , F l+1 ), and hence the two terms are orthogonal as required.
...
Consider a function f that is k-diluted with respect to u, a matrix S and a function ρ.

Let R(v) be the Jacobian of ρ at input v. Let r := DISPLAYFORM0 Q||ρ(u)||2Q||g||2 .Assume that E(Su).(ρ(u)) = 0 and that E(gR(u)).(gS) = 0.
...
r represents a type of expectation over the ratio DISPLAYFORM3 ||ρ(u)||2||g||2 , ignoring the skip connection.

Therefore r can be viewed as the growth of the GSC.

Similarly, DISPLAYFORM4 Q||ρ(u)+Su||2Q||g||2 represents the growth of the GSC after the skip connection has been added.

The key assumptions are E(Su).(ρ(u)) = 0 and E(gR(u)).(gS) = 0.
...
Then the definition of the Taylor expansion remains as above, except a term s l (T m (θ, X)) is added at each layer l where a skip connection, represented by skip function s l , terminates.
