...
The parameters to the graph-based CRF constituency parser are the unary potentials γ i , reflecting the score of the token x i forming a unary constituency node and δ ikj reflecting the score of span ij forming a binary constituency node with k as the splitting point.
...
For computational and statistical efficiency given this parameterization, we drop the dependence on the splitting point in this score, resulting in a score for each span δ ij representing how "constituent-like" the span is, independent of the merging of its children in the tree.

This allows for a slightly-modified computation of the inside score in the inside-outside algorithm.

Where the inside score α ij is typically computed as α ij = i≤k<j δ ikj α ik α (k+1)j , we instead compute it as DISPLAYFORM1 Up to this point, the tags of constituency nodes are not considered 1 , leading to an unlabeled tree structure.
...
The inside pass is described in Algorithm 1, where γ for i:=1 to n − width + 1 do 7:j := i + width − 1 8: DISPLAYFORM2 10: The α values are the inside scores for all the spans in the sentence, which are basically the unnormalized scores indicating the whether the spans are proper constituents.
