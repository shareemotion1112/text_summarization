...
Let S ,D,θ denote the set of (100-)% small-loss examples of the network f parameterized by θ out of examples in D. Since it is generally hard to learn only all clean examples especially on the highly corrupted training set, it is problematic to regard all examples in S ,D,θ as being clean.

To mitigate this, we suggest a simple idea: to find noisy examples among examples in S ,D,θ .
...
This means that even if training losses of noisy examples are small, they can be easily increased under a certain perturbation δ, i.e., for (x, y) ∈ D noisy ,
...
Thus, their training losses are consistently small in the presence of the perturbation δ, i.e., for (x, y) ∈ D clean ,

This suggests that noisy examples can be identified from the inconsistency of losses under certain perturbation δ.

Based on this, we regard examples in the intersection of (100-)% small-loss examples of an ensemble of M networks generated by adding perturbations δ 1 , δ 2 , ..., δ M to θ, i.e.,
...
Require: noisy dataset D with noise ratio %, duration of warmingup Tw, # of networks used for filtering M , perturbation δ 1: Initialize θ randomly 2: for epoch t = 1 :
...
Now the goal is to find a perturbation δ to be injected to distinguish between generalized and memorized features.

We present three LECs with different perturbations in the following.

The pseudocodes can be found in Section A.1.3.

• Network-Ensemble Consensus (LNEC): Inspired by the observation that an ensemble of networks with the same architecture is correlated during generalization and is decorrelated during memorization (Morcos et al., 2018) , the perturbation δ comes from the difference between M networks.
...
Since the uncertainty of predictions also can be captured by multiple stochastic predictions (Gal & Ghahramani, 2016) , the perturbation δ comes from the difference between M stochastic predictions of a single network.
...
• Temporal-Ensemble Consensus (LTEC): Inspired by the observation that during training, atypical features are more easily forgetful compared to typical features (Toneva et al., 2018) , the perturbation δ comes from the difference between networks at current and preceding epochs.
...
f (x; θn)) where f (; θ) denotes softmax output of network parameterized by θ.
...
The initial learning rate α is set to 0.1.
...
The momentum parameters β 1 and β 2 are set to 0.9 and 0.999, respectively.

β 1 is linearly annealed to 0.1 during the last 120 epochs for MNIST and CIFAR-10, and during the last 100 epochs for CIFAR-100.
...
S ,B b ,θ := (100 − )% small-loss examples of f θ within B b
...
Pt := (100 − )% small-loss examples of f θ within D Small-loss examples are computed from the 2nd epoch 8:
