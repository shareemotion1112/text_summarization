...
A predicate is denoted by an n-ary predicate symbol p applied to a sequence of zero or more terms (τ 1 , τ 2 , ..., τ n ) -terms are either constants or variables.
...
A planning domain definition Ξ is represented by a pair Σ, A , which specifies the knowledge of the domain, and consists of a finite set of facts Σ (e.g., environment properties) and a finite set of actions A.Definition 4 (Planning Instance).

A planning instance Π is represented by a triple Ξ, I, G , where Ξ = Σ, A is the domain definition; I ⊆ Σ is the initial state specification, which is defined by specifying values for all facts in the initial state; and G ⊆ Σ is the goal state specification, which represents a desired state to be achieved.

Definition 5 (Plan).

An s-plan π for a planning instance Π = Ξ, I, G is a sequence of actions a 1 , a 2 , ..., a n that modifies a state s into a state S |= G in which the goal state G holds by the successive execution of actions in π starting from s. An I-plan is just called a plan.

A plan π * with length |π * | is optimal if there exists no other plan π for Π such that |π | < |π * |.A goal recognition problem aims to select the correct goal of an agent among a set of possible goals using as evidence a sequence of observations.
...
An observation sequence O = o 1 , o 2 , ..., o n is said to be satisfied by a plan π = a 1 , a 2 , ..., a m , if there is a monotonic function f that maps the observation indices j = 1, ..., n into action indices i = 1, ..., n, such that a f (j) = o j .Definition 7 (Goal Recognition Problem).

A goal recognition problem is a tuple T GR = Ξ, I, G, O , where: Ξ = Σ, A is a planning domain definition; I is the initial state; G is the set of possible goals, which include a correct hidden goal G * (i.e., G * ∈ G); and O = o 1 , o 2 , ..., o n is an observation sequence of executed actions, with each observation o i ∈ A, and the corresponding action being part of a valid plan π (from Definition 5) that transitions I into G * through the sequential execution of actions in π.

Definition 8 (Solution to a Goal Recognition Problem).

A solution to a goal recognition problem T GR = Ξ, I, G, O is a nonempty subset of the set of possible goals G ⊆ G such that ∀G ∈ G there exists a plan π G generated from a planning instance Ξ, I, G and π G is consistent with O.
...
These heuristics rely on constraints from different sources of information that every plan π (Definition 5) must satisfy.

All operator-counting constraints contain variables of the form Y a for each operator a such that setting Y a to the number of occurrences of a in π satisfies the constraints.
...
Input: Ξ planning domain definition, I initial state, G set of candidate goals, and O observations.
...
10-11, h C be the overconstrained heuristic that accounts for all observations o ∈ O, and s a state of Π. Then h C (s) ≥ h(s).Proof.
...
We call the resulting heuristic h δ and formalize this approach in Algorithm 3.

Here we compute the LP twice, once with only the basic operator-counting constraints (Line 4), and once with the constraints enforcing the observations in the operator counts (Line 8), using these two values to compute h δ (Line 10).

The algorithm then returns goal hypotheses that minimize h δ (Line 11).
...
We argue that this is attributable to our inefficient implementation rather than the technique, for the h δ approach, the overhead of the FAST-DOWNWARD pre-processing step is paid multiple times.
...
We note that the results for noisy observations show the greatest impact of h δ with an overall higher accuracy and lower spread across all domains but KITCHEN.Finally, results for the KITCHEN domain stand out in our experiments in that our some of our approaches consistently show underwhelming performance both in noisy and nonnoisy domains.
