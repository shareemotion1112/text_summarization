...
Here, K * is a minimizer of C(·).Gradient descent on C(K), with a fixed stepsize η, follows the update rule: DISPLAYFORM2 It is helpful to explicitly write out the functional form of the gradient.

Define P K as the solution to: DISPLAYFORM3 and, under this definition, it follows that C(K) can be written as: DISPLAYFORM4 Also, define Σ K as the (un-normalized) state correlation matrix, i.e. DISPLAYFORM5 Lemma 1. (Policy Gradient Expression) The policy gradient is: DISPLAYFORM6 Observe: DISPLAYFORM7 This implies: DISPLAYFORM8 x t x t using recursion and that x 1 = (A − BK)x 0 .
...
Let π θ (u|x) be a parametric stochastic policy, where u ∼ π θ (·|x).

The policy gradient of the cost, C(θ), is: DISPLAYFORM0 where the expectation is with respect to the trajectory {x t , u t } induced under the policy π θ and where Q π θ (x, u) is referred to as the state-action value.

The REINFORCE algorithm uses Monte Carlo estimates of the gradient obtained by simulating π θ .The natural policy gradient.

The natural policy gradient BID18 ) follows the update: DISPLAYFORM1 where G θ is the Fisher information matrix.

There are numerous succesful related approaches (Peters & Schaal, 2007; Schulman et al., 2015; BID12 ).

An important special case is using a linear policy with additive Gaussian noise (Rajeswaran et al., 2017b) DISPLAYFORM2 where K ∈ R k×d and σ 2 is the noise variance.

Here, the natural policy gradient of K (when σ is considered fixed) takes the form: DISPLAYFORM3 To see this, one can verify that the Fisher matrix of size kd × kd, which is indexed as DISPLAYFORM4 where i, i ∈ {1, . . .

k} and j, j ∈ {1, . . .

d}, has a block diagonal form where the only non-zeros blocks are [G K ] (i,·),(i,·) = Σ K (this is the block corresponding to the i-th coordinate of the action, as i ranges from 1 to k).
...
The generic approach can be described as follows: define the perturbed function as DISPLAYFORM5 For small σ, the smooth function is a good approximation to the original function.
...
This expression implies a straightforward method to obtain an unbiased estimate of the ∇f σ 2 (x), through obtaining only the function values f (x + ε) for random ε.
...
Corollary 3. (Stationary point characterization) If ∇C(K) = 0, then either K is an optimal policy or Σ K is rank deficient.

This lemma is the motivation for using a distribution over x 0 (as opposed to a deterministic starting point): E x0∼D x 0 x 0 being full rank guarantees that Σ K is full rank, which implies all stationary points are a global optima.
...
A function f : R d → R is said to be gradient dominated if there exists some constant λ, such that for all x, DISPLAYFORM0 If a function is gradient dominated, this implies that if the magnitude of the gradient is small at some x, then the function value at x will be close to that of the optimal function value.

The following corollary of Lemma 8 shows that C(K) is gradient dominated.

DISPLAYFORM1 where λ is a problem dependent constant (and ·, · denotes the trace inner product).With gradient domination and no (spurious) local optima, one may hope that recent results on escaping saddle points (Nesterov & Polyak, 2006; BID16 BID17 immediately imply that gradient descent converges quickly.
...
Z denotes the spectral norm of a matrix Z; Tr(Z) denotes the trace of a square matrix; σ min (Z) denotes the minimal singular value of a square matrix Z. Also, it is helpful to define DISPLAYFORM0
...
These can then be used in the policy gradient and natural policy gradient updates as follows: DISPLAYFORM0 Natural policy gradient descent: DISPLAYFORM1 where Algorithm 1 is called at every iteration to provide the estimates of ∇C(K n ) and Σ Kn .The choice of using zeroth order optimization vs using REINFORCE (with Gaussian additive noise, as in Equation 3) is primarily for technical reasons 2 .

It is plausible that the REINFORCE estimation procedure has lower variance.

One additional minor difference, again for technical reasons, is that Algorithm 1 uses a perturbation from the surface of a sphere (as opposed to a Gaussian perturbation).Theorem 6. (Global Convergence in the Model Free Setting) Suppose C(K 0 ) is finite, µ > 0, and that x 0 ∼ D has norm bounded by L almost surely.
...
For the linear quadratic setup, direct iterative computation of the feedback gain has been examined in (Mårtensson & Rantzer, 2009) , and explored further in (Mårtensson, 2012) with a view towards distributed implementations.
...
Suppose K has finite cost and µ > 0.
...
Recall that a function f is said to be smooth (or C 1 -smooth) if it satisfies for some finite β, it satisfies: DISPLAYFORM14 for all x, y (equivalently, it is smooth if the gradients of f are continuous).

Lemma 9. ("Almost" smoothness) C(K) satisfies: DISPLAYFORM15 To see why this is related to smoothness (e.g. compare to Equation 12), suppose K is sufficiently close to K so that: DISPLAYFORM16 and the leading order term 2Tr(Σ K (K − K) E K ) would then behave as Tr((K − K) ∇C(K)).
...
The next lemma spectral norm bounds on P K and Σ K are helpful: Lemma 10.
...
Using Lemma 9 and the condition on η, DISPLAYFORM1 where the last step uses Lemma 8.With this lemma, the proof of the convergence rate of the Gauss Newton algorithm is immediate.

Proof. (of Theorem 5, Gauss-Newton case) The theorem is due to that η = 1 leads to a contraction of 1 − ηµ Σ K * at every step.
...
Since K = K − ηE K , Lemma 9 implies: DISPLAYFORM2 The last term can be bounded as: DISPLAYFORM3 Continuing and using the condition on η, DISPLAYFORM4 using Lemma 8.With this lemma, the proof of the natural policy gradient convergence rate can be completed.
...
As informally argued by Equation 13, the proof seeks to quantify how Σ K changes with η.

Then the proof bounds the one step progress of gradient descent.

DISPLAYFORM0 This subsections aims to prove the following: Lemma 13. (Σ K perturbation) Suppose K is such that: DISPLAYFORM1 It holds that: DISPLAYFORM2 The proof proceeds by starting with a few technical lemmas.
...
For a unit norm vector v ∈ R d and unit spectral norm matrix X, DISPLAYFORM6 The proof is completed using the upper bound on Σ K in Lemma 10.Also, with respect to K, define another linear operator on symmetric matrices: DISPLAYFORM7 Let I to denote the identity operator on the same space.
...
By Lemma 9, DISPLAYFORM2 where the last step uses Lemma 8.By Lemma 13, DISPLAYFORM3 using the assumed condition on η.

Using this last claim and Lemma 10, DISPLAYFORM4 using the condition on η.
...
Using Lemma 10, DISPLAYFORM7 By Lemma 8, DISPLAYFORM8 µ which proves the first claim.
...
Based on Lemma 10 and Lemma 19, by choosing η to be an appropriate polynomial in DISPLAYFORM10 , σ min (Q) and µ, the stepsize condition in Equation 16 is satisfied.
...
Thus, Lemma 18 can again be applied for the update at time t + 1 to obtain: DISPLAYFORM12 ≤ ε, and the result follows.
...
The dependencies on various parameters are not optimized, and the notation h is used to represent different polynomial factors in the relevant factors ( DISPLAYFORM0 µσmin(Q) , A , B , R , 1/σ min (R)).
...
This section shows that as long as there is an upper bound on C(K), it is possible to approximate both C(K) and Σ(K) with any desired accuracy.

Lemma 20.

For any K with finite C(K), let Σ DISPLAYFORM0 Proof.

First, the bound on Σ K is proved.
...
The last step follows because F j (Σ K ) is also a PSD matrix so the spectral norm is bounded by trace.
...
Although Gaussian smoothing is more standard, the objective C(K) is not finite for every K, therefore technically E u∼N (0,σ 2 I) [C(K + u)] is not well defined.
...
Let ∇ be equal to The third term is just what was bounded earlier, using h r,trunc (1/ ) = h r (2/ ) and making sure h sample,trunc (d, 1/ ) ≥ h sample (d, 2/ ), this guarantees that it is smaller than /2.For the second term, choose ≥ Therefore,∇ − ∇ is again a sum of independent vectors with bounded norm, so by Vector Bernstein's inequality, when h sample,trunc (d, 1/ , L 2 /µ) is a large enough polynomial, ∇ − ∇ ≤ /4 with high probability.

Adding all the terms finishes the proof.

Theorem 25.

There are fixed polynomials h GD,r (1/ ), h GD,sample (d, 1/ , L 2 /µ), h GD, (d, 1/ ) such that if every step the gradient is computed as Lemma 24 (truncated at step ), pick step size η and T the same as the gradient descent case of Theorem 5, it holds that C(K T ) − C(K ) ≤ with high probability (at least 1 − exp(−d)).

Before the Theorem for natural gradient is proven, the following lemma shows the variance can be estimated accurately.

satisfies Σ − Σ K ≤ .

Further, when ≤ µ/2, it holds that σ min (Σ K ) ≥ µ/2.Proof.

This is broken into three terms: let Σ Therefore, standard concentration bounds show that when h varsample,trunc is a large enough polynomial, Σ −Σ ( ) ≤ /2 holds with high probability.

Adding these three terms gives the result.

Finally, the bound on σ min (Σ K ) follows simply from Weyl's Theorem.

Theorem 27.

Suppose C(K 0 ) is finite and and µ > 0.

The natural gradient follows the update rule: DISPLAYFORM0 Suppose the stepsize is set to be: DISPLAYFORM1 If the gradient and variance are estimated as in Lemma 24, Lemma 26 with r = 1/h N GD,r (1/ ), with m ≥ h N GD,sample (d, 1/ , L 2 /µ) samples, both are truncated to h N GD, (d, 1/ ) iterations, then with high probability (at least 1 − exp(−d)) in T iterations where DISPLAYFORM2 µσ min (R) log 2(C(K 0 ) − C(K * ))

ε then the natural gradient satisfies the following performance bound: DISPLAYFORM3
