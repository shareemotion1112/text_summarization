...
Furthermore, we consider task conditional policies π(a|s, i).

The overall objective is defined as J(π) = E i∼I E π,p(s0) ∞ t=0 γ t r i (s t , a t ) |s t+1 ∼ p(·|s t , a t ) = E i∼I E π,p(s) Q π (s, a, i) ,

(1) where all actions are drawn according to the policy π, that is, a t ∼ π(·|s t , i) and we used the common definition of the state-action value function -here conditioned on the task -Q π (s, a, i) = E π [ ∞ t=0 γ t r i (s t , a t ) |a 0 = a, s 0 = s, a t ∼ π(·|s t , i), s t+1 ∼ p(·|s t , a t )].
...
with π H and π L respectively representing a "high-level" switching controller (a categorical distribution) and a "low-level" sub-policy (components of the resulting mixture distribution), where o is the index of the sub-policy.

Here, θ denotes the parameters of both π H and π L , which we will seek to optimize.
...
Note that, in the above formulation only the high-level controller π H is conditioned on the task information i; i.e. we employ a form of information asymmetry (Galashov et al., 2018; Tirumala et al., 2019; Heess et al., 2016 ) to enable the low-level policies to acquire general, task-independent behaviours.
...
Concretely, we first introduce an intermediate non-parametric policy q(a|s, i) and consider optimizing J(q) while staying close, in expectation, to a reference policy π ref (a|s, i)

where KL(· ·) denotes the Kullback Leibler divergence, defines a bound on the KL, D denotes the data contained in a replay buffer, and assuming that we have an approximation of the ground-truth state-action value functionQ(s, a, i) ≈ Q π (s, a, i) available (see Equation (4) for details on learninĝ Q from off-policy data).

Starting from an initial policy π θ0 we can then iterate the following steps to improve the policy π θ k :

Policy Evaluation: UpdateQ such thatQ(s, a, i) ≈Q π θ k (s, a, i), see Equation (4).

Policy Improvement:

-Step 1: Obtain q k = arg max q J(q), under KL constraints with π ref = π θ k (Equation (3)).

-Step 2: Obtain θ k+1 = arg min θ E s∼D,i∼I KL q k (·|s, i) π θ (·|s, i) , under additional regularization (Equation (6)).
...
Formally, we assume access to a replay buffer containing data gathered from all tasks, which is filled asynchronously to the optimization (similar to e.g. Espeholt et al. (2018) ) where for each trajectory snippet τ = {(s 0 , a 0 , R 0 ), . . .

, (s L , a L , R L )} we record the rewards for all tasks R t = [r i1 (s t , a t ), . . .

, r i |I| (s t , a t )] as a vector in the buffer.

Using this data we define the retrace objective for learningQ, parameterized via φ, following (Munos et al., 2016; Riedmiller et al., 2018) as min
...
where η is a temperature parameter (corresponding to a given bound ) which is optimized alongside the policy optimization (see Appendix A.1.1 for a detailed derivation of the multitask case).

As mentioned above, this policy representation is independent of the form of the parametric policy π θ k ; i.e. q only depends on π θ k through its density.

This, crucially, makes it easy to employ complicated structured policies (such as the one introduced in Section 3.1).

The only requirement here, and in the following steps, is that we must be able to sample from π θ k and calculate the gradient (w.r.t.

θ k ) of its log density (but the sampling process itself need not be differentiable).

In the second step we fit a policy to the non-parametric distribution obtained from the previous calculation by minimizing the divergence E s∼D,i∼I [KL(q k (·|s, i) π θ (·|s, i))].
...
For this purpose let us define the joint distribution over states s ∼ µ(s) together with randomly sampled tasks i ∼ I as µ(s, i) = p(s|D)U(i ∈ I), where U denotes the uniform distribution over possible tasks.
...
We solve for θ by iterating the inner and outer optimization programs independently: We fix the parameters θ to their current value and optimize for the Lagrangian multipliers (inner minimization) and then we fix the Lagrangian multipliers to their current value and optimize for θ (outer maximization).
...
In particular, in all experiments we made use of a mixture of Gaussians parametrization, where the high level policy π H θ is a categorical distribution over low level π L θ Gaussian policies, i.e,

where j denote the index of components and α is the high level policy π H assigning probabilities to each mixture component for a state s given the task and the low level policies are all Gaussian.

Here α j s are the probabilities for a categorical distribution over the components.
...
In order to bound the change of categorical distributions, means and covariances of the components independently -which makes it easy to control the convergence of the policy and which can prevent premature convergence as argued in Abdolmaleki et al. (2018a) log π θ (a j |s t , i) following Eq. 6
...
, r i |I| (s t , a t )] as a vector in the buffer, we define the retrace objective for learningQ, parameterized via φ, following Riedmiller et al. (2018) as

where the importance weights are defined as c k = min (1, π θ k (a k |s k ,i) /b(a k |s k )), with b(a k |s k ) denoting an arbitrary behavior policy; in particular this will be the policy for the executed tasks during an

Input: N trajectories number of total trajectories requested, T steps per episode, ξ scheduling period initialize N = 0 while N < N trajectories do fetch parameters θ // collect new trajectory from environment τ = {} for t in [0...T ] do if t (mod ξ) ≡ 0 then // sample active task from uniform distribution i act ∼ I end if a t ∼ π θ (·|s t , i act ) // execute action and determine rewards for all tasks r = [r i1 (s t , a t ), . . .

, r i |I| (s t , a t )]

τ ← τ ∪ {(s t , a t ,r, π θ (a 0 |s t , i act ))} end for send batch trajectories τ to replay buffer N = N + 1 end while episode as in (Riedmiller et al., 2018) .

Note that, in practice, we truncate the infinite sum after L steps, bootstrapping withQ. We further perform optimization of Equation (4) via gradient descent and make use of a target network (Mnih et al., 2015) , denoted with parameters φ , which we copy from φ after a couple of gradient steps.
...
The main difference is in the policy parameterization: SAC-U used a monolithic policy for each task π(a|s, i) (although a neural network with shared components, potentially leading to some implicit task transfer, was used).
...
The flat policy is given by a Gaussian distribution with a diagonal covariance matrix, i.e, π(a|s, θ) = N (µ, Σ).

The neural network outputs the mean µ = µ(s) and diagonal Cholesky factors A = A(s), such that Σ = AA T .
...
, where I denotes the identity matrix and σ θ (s, i) is computed from the network output via a softplus activation function.
