...
MF tries to give an estimationŷ ui of y ui by the inner product of p u and q i :
...
Generalized Matrix Factorization (GMF) ) is a simple nonlinear generalization of MF, which makes a predictionŷ ui of y ui as follows:

where denotes the element-wise product of vectors, h is a weight vector, and σ(·) is an activation function.
...
In order to avoid the conflict that user u may overly concentrate on item i if the target item is i, we will exclude i from N(u) (denoted by N(u)\{i}) when predictingŷ ui .

Similarly, we will also exclude u from N(i) (denoted by N(i)\{u}) when predictingŷ ui .
...
Then we reuse the GMF model as the prediction model, and the predicted interaction scoreŷ ui is given by:ŷ

where h is the weight vector of the prediction model.

We empirically use the sigmoid function σ(x) = 1/(1 + exp(−x)) as the activation function throughout this paper.

Since this paper focuses on learning from implicit feedback data, the outputŷ ui of our AGMF model is constrained in the range of [0, 1], which could provide a probability explanation.
...
As aforementioned, the GMF model makes prediction byŷ ui = σ(h (p u q i )), while our AGMF model makes prediction byŷ ui = σ(h ((p u + p u ) (q i + q i ))).
