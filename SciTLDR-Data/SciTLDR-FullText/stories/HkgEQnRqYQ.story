...
Indeed, many existing approaches have been trying to either implicitly or explicitly model one or a few of the above relation patterns BID3 BID29 BID17 Table 1: The score functions f r (h, t) of several knowledge graph embedding models, where · denotes the generalized dot product, • denotes the Hadamard product, ⊗ denotes circular correlation, σ denotes activation function and * denotes 2D convolution.
...
Our motivation is from Euler's identity e iθ = cos θ + i sin θ, which indicates that a unitary complex number can be regarded as a rotation in the complex plane.
...
By doing this, r i is of the form e iθr,i , which corresponds to a counterclockwise rotation by θ r,i radians about the origin of the complex plane, and only affects the phases of the entity embeddings in the complex vector space.
...
Here we use a loss function similar to the negative sampling loss BID19 for effectively optimizing distance-based models: DISPLAYFORM0 where γ is a fixed margin, σ is the sigmoid function, and (h i , r, t i ) is the i-th negative triplet.
...
Specifically, we sample negative triples from the following distribution: DISPLAYFORM1 where α is the temperature of sampling.
...
The ranges of the hyperparameters for the grid search are set as follows: embedding dimension 5 k ∈ {125, 250, 500, 1000}, batch size b ∈ {512, 1024, 2048}, self-adversarial sampling temperature α ∈ {0.5, 1.0}, and fixed margin γ ∈ {3, 6, 9, 12, 18, 24, 30}. Both the real and imaginary parts of the entity embeddings are uniformly initialized, and the phases of the relation embeddings are uniformly initialized between 0 and 2π.

No regularization is used since we find that the fixed margin γ could prevent our model from over-fitting.
...
We ignore the specific positions in the relation embedding θ r and plot the histogram of the phase of each element in the relation embedding, i.e., {θ r,i }
...
We can find that the embedding phases are either π (r i = −1) or 0, 2π (r i = 1 BID5 1.00 ± 0.00 1.00 ± 0.00 0.95 ± 0.00 model does infer and model the symmetry pattern.
...
All the additive embedding phases are 0 or 2π, which represents that r 1 = r −12 .
...
Figure 2d -2g illustrate such a r 1 = r 2 • r 3 case, where θ 2,i + θ 3,i = θ 1,i or θ 2,i + θ 3,i = θ 1,i + 2π.
