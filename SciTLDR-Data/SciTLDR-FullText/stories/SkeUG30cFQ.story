...
LetĀ i be the matrix obtained by a SVD approximation of rank k of matrix A i .

Let σ i,j is the j th singular value of DISPLAYFORM6 where R is an upper bound on norm of the output of any layer in N , and σ max,j = max i σ i,j .Basically, this proposition shows that we can approximate matrices in a neural network by low rank matrices, and control the approximation error.

In general, the term σ l max,1 could seem large, but in practice, it is likely that most singular values in deep neural network are small in order to avoid divergent behaviors.

We can now prove the result on Deep diagonal-circulant ReLU networks: DISPLAYFORM7 of depth l and width n. Let σ max,j = max i σ i,j where σ i,j is the j th singular value of A i .
...
DISPLAYFORM9 , where eachĀ i is the matrix obtained by a SVD approximation of rank k of matrix A i .With proposition 4, we have an error bound on N (x) −N (x) .

Now each matrixĀ i can be replaced by a product of k diagonal-circulant matrices.
...
Clearly, for all x ∈ X we have h 1 (x) ≥ 0, so ReLU • h 1 (x) = h 1 (x).More generally, for all j < n − 1 define β j+1 = 1 n Ω − A j+1 β j .
...
By lemma 1, we know that there exists DISPLAYFORM3 Now if tends to zero, f DinCin,βin • . . .

• f Di1Ci1,βi1 − ReLU (A i x + b i ) will also tend to zero for any x ∈ X , because the ReLU function is continuous and X is compact.
...
, B 4k+1 , where B i is a circulant matrix if i is odd, and a diagonal matrix otherwise, such that DISPLAYFORM4 T be the SVD decomposition of M where U, V and Σ are n × n matrices.
...
Note that OΣO T is diagonal, because all three are diagonal.
...
We can apply this formula to bound our error as follows x l −x l ≤ (σ l max,1 −1)σ max,k maxi xi σmax,1−1 .Lemma 2.

Let A ∈ C n×n with singular values σ 1 . . .

σ n , and let x,x ∈ C n .

LetĀ be the matrix obtained by a SVD approximation of rank k of matrix A. Then we have: Finally, it is easy to see that for any pair of vectors a, b ∈ C n , we have ReLU (a) − ReLU (b) ≤ a − b .
