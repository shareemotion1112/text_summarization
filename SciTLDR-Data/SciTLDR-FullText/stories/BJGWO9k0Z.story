...
Mazes are constructed as a random two dimensional, L × L, black and white array of cells (I) where the probability (ρ) of having a black cell is given by ρ c = 0.59274(6), while the other cells are white.
...
At the particular value chosen for ρ, the problem is at the percolation-threshold which marks the phase transition between the two different connectivity properties of the maze: below ρ c the chance of having a solvable maze decays exponentially with r (the geometrical distance between the ending and starting points).

Above ρ c it tends to a constant at large r. Exactly at ρ c the chance of having a solvable maze decays as a power law (1/r η , η = 5/24).
...
Notably, as the system size grows larger, the chance of producing solvable mazes decay as 1/L η , and so, for very large L, the labels will be biased towards unsolvable.

There are several ways to de-bias the dataset.

One is to select an unbiased subset of it.

Alternatively, one can gradually increase the size of the starting-point to a starting-square whose length scales as L η .
...
It performs the operation DISPLAYFORM0 , where * denotes a 2D convolution, the K convolutional kernel is 1×1, the K hot kernel is 3×3, b is a bias, and σ(x) = (1+e −x ) −1 is the sigmoid function.
...
A choice of parameters which achieves this is λ ≥ λ c = 9.727±0.001, DISPLAYFORM0 T , where q center is the index of H n which corresponds to the center of the maze.
...
The quantity v l is defined as the smallest of the three real solutions of the equation v l = σ(5v l − 0.5λ).

Notably we previously chose λ > λ c as this is the critical value above which three real solutions to v l (rather than one) exist.
...
This amounts to bounding the expression σ(K hot * H i−1 + K * I + b) for all possibly 3 × 3 sub-arrays of H i−1 and 1 × 1 sub-arrays of I. There are thus 2 10 possibilities to be examined.
...
Case A. Having a white cell at x implies I[x] = 0 and therefore the argument of the above sigmoid is smaller than −0.5λ c this regardless of H i−1 at and around x. Thus H i [x] < v l and so it is Off.
...
Here the argument is smaller or equal to 5v l − 0.5λ c , and so the activation remains Off as desired.
...
Here the argument is larger than v h λ c − 0.5λ c = 0.4λ c .
...
The main result of this subsection, derived below, is that the density of bugs (or chance of bug per cell) scales as DISPLAYFORM0 for (λ − λ c ) <≈ 0 and zero for λ − λ c >= 0 where C ≈ 1.7.

Following the analysis below, we expect the same dependence to hold for generic small perturbations only with different C and λ c .

We have tested this claim numerically on several other types of perturbations (including ones that break the π/2 rotational symmetry of the weights) and found that it holds.

To derive Eq.(1), we first recall the analysis in Sec. 4, initially as it is decreased λ has no effect but to shift v l (the Off activation threshold) to a higher value.

However, at the critical value (λ = λ c , v l = v l,c ) the solution corresponding to v l vanishes (becomes complex) and the correspondence with the BFS algorithm no longer holds in general.
...
In fact using the same arguments one can show that infinitesimally below λ c only uniform black mazes will cause the network to malfunction.
...
For almost all I's the spectrum of L consists of localized eigenfunctions (φ m ).

Any such function is centered around a random site (x m ) and decays exponentially away from that site with a decay length of χ which in our case would be a several cells long.

Thus given φ m with an eigenvalue |E m | > 1, t repeated actions of the convolutional-sigmoid layer will make ψ n [x] in a χ vicinity of x m grow in size as e Emt .
...
Our original question of determining the chance of bugs now translates into a linear algebra task: finding, Nλ, the number of eigenvalues in L I which are larger than 1 in magnitude, averaged over I, for a given λ.

Sinceλ simply scales all eigenvalues one finds that Nλ is the number of eigenvalues larger thanλ −1 in L I withλ = 1.

Analyzing this latter operator, it is easy to show that the maximal eigenvalues occurs when φ n (r) has a uniform pattern on a large uniform region where the I is black.

Indeed if I contains a black uniform true box of dimension l u × l u , the maximal eigenvalue is easily shown to be E lu = 5 − 2π 2 /(l u ) 2 .
...
Following this we find DISPLAYFORM3 DISPLAYFORM4 The range of integration is chosen to includes all eigenvalues which, following a multiplication byλ, would be larger than 1.To conclude we found the number of isolated unwanted On activations which develop on l × l Off regions.

Dividing this number by l 2 we obtain the density of bugs (ρ bug ) near λ ≈ λ c .

The last technical step is thus to express ρ bug in terms of λ.

Focusing on the small ρ bug region or ∆E → 0 + , we find that ∆E = 0 occurs when dσ dx (σ −1 (η ∞ (λ))) = 1/(5λ),λ = 1/5, and λ = λ c = 9.72(7).Expanding around λ = λ c we find DISPLAYFORM5 2 ).

Approximating the integral over P (x) and taking the leading scale dependence, we arrive at Eq. FORMULA3 with C = C 10λc 49−λc .

In this subsection we wish to understand the large scale effect of ρ bug namely, its effect on the test error and the cost function.
...
The probability distribution of s for percolation is known and given by p(s) = Bs 1−τ , τ = 187/91 BID6 ), with B being an order of one constant which depends on the underlying lattice.

Since clusters have a fractional dimension, the maximal cluster size is L d f .

Consequently, p err (I) averaged over all I instances is given by DISPLAYFORM3 ds, which can be easily expressed in terms of Gamma functions (Γ(x), Γ(a, x)) (see BID0 ).

In the limit of ρ bug <≈ L −d f , where its derivatives with respect to ρ bug are maximal, it simplifies to DISPLAYFORM4 whereas for ρ bug > L −d f , its behavior changes to p err = (−BΓ(2 − τ ))ρ DISPLAYFORM5 bug .

Notably once ρ bug becomes of order one, several of the approximation we took break down.
...
Combining Eqs. 1 and 5 we obtain our key results, Eqs. (3, 4)As a side, one should appreciate a potential training obstacle that has been avoided related to the fact that err ∝ ρ 5/91 big .

Considering L → ∞, if ρ bug was simply proportional to (λ c − λ), err will have a sharp singularity near zero.
...
Interestingly however, the essential singularity in ρ bug (λ), derived in the previous section, completely flattens the gradients near λ c .Thus the essentially singularity which comes directly from rare events in the dataset strongly regulates the test error and in a related way the cost function.

However it also has a negative side-effect concerning the robustness of generalization.

Given a finite dataset the rarity of events is bounded and so having λ < λ c may still provide perfect performance.
...
We expand them as a sum of two functions DISPLAYFORM0 where the function A and B are normalized such that their variance on the data (α and β, respectively) is 1.

Notably near the reported total neglect minima we found that α β ≈ e −10 .

Also note that for the biased dataset the maze itself is uncorrelated with the labels and thus β can be thought of as noise.

Clearly any solution to the Maze testing problem requires the starting-point dependent part (α) to become larger than the independent part (β).

We argue however that in the process of increasing α the activations will have to go through an intermediate "noisy" region.

In this noisy region α grows in magnitude however much less than β and in particular obeys α < β 2 .

As shown in the Appendix the negative log-likelihood, a commonly used cost function, is proportional to β 2 − α for α, β 1.

Thus it penalizes random false predictions and, within a region obeying α < β 2 it has a minimum (global with respect to that region) when α = β = 0.

The later being the definition of a total neglect minima.

Establishing the above α β 2 conjecture analytically requires several pathological cases to be examined and is left for future work.
...
For many parameters ranges it exhibits a typical correlation length (χ).

That is a scale beyond which two activations are statistically independent.

Clearly to solve the current problem χ has to grow to an order of L such that information from the input reaches the output.

However as χ gradually grows, relevant and irrelevant information is being mixed and propagated onto the final layer.

While β depends on information which is locally accessible at each layer (i.e. the maze shape), α requires information to travel from the first layer to the last.

Consequently α and β are expected to scale differently, as e −L/χ and e −1/χ resp. (for χ << L).

Given this one finds that α β 2 as claimed.

Further numerical support of this conjecture is shown in the Appendix where an upper bound on the ratio α/β 2 is studied on 100 different paths leading from the total neglect miminum found during training to the checkerboard-BFS minimum.

In all cases there is a large region around the total neglect minimum in which α β 2 .
...
We have implemented the architecture described in the main text using Theano BID38 ) and tested how cost changes as a function of δ = λ c − λ (λ c = 9.727..) for mazes of sizes L = 24, 36 and depth (number of layers) 128.

These depths are enough to keep the error rate negligible at δ = 0.
...
We numerically obtained the normalized performance (cost L (δ)) as a function of L and δ.

As it follows from Eq. (4) in the main text the curve, log(L −2+5/24 cost L (δ)), for the L = 24 and L = 36 results should collapse on each other for ρ bug < L −d f .
...
As our results are only asymptotic, both in the sense of large L and λ → λ c , minor discrepancies are expected.

To prepare the action of sigmoid-convolutional for linearization we find it useful to introduce the following variables on locations (r b , r w ) with black (b) and white (w) cells DISPLAYFORM0 a(r w ) = e −5.5λ .homogeneous.

Consequently destabilization occurs exactly atλ = 1/5 and is not blurred by the inhomogeneous terms.

Recall that λ c is defined as the value at which the two lower solutions of .

It is now easy to verify that even within the linear approximation destabilization occurs exactly at λ c .
...
Consequently near λ ≈ λ c the d term projected on the largest eigenvalues can, to a good accuracy, be ignored and stability analysis can be carried on the homogeneous equation ψ = S ψ where s n < 1 means stability and s n > 1 implies a bug.
...
Notably at α = 1/2, β = 0 it yields P f while at α , β = 0 it is simply the maximum entropy distribution.

Let us measure the log-likelihood of P α ,β under P f for α , β 1 L(α , β ) = We thus find that β reduces the log-likelihood in what can be viewed as a penalty to false confidence or noise.

Assuming, as argued in the main text, that α is constrained to be smaller than β 2 near β ≈ 0, it is preferable to take both α and β to zero and reach the maximal entropy distribution.

We note by passing that the same arguments could be easily generalized to f (x), g(x) taking real values leading again to an O(α) − O(β 2 ) dependence in the cost function.

Let us relate the above notations to the ones in the main text.

Clearly x = ({I}, H 0 ) and {0, 1} = {U nsolvable, Solvable}. Next we recall that in the main text α and β multiplied the vectors function representing the H 0 -depended and H 0 -independent parts of H n .
...
While (α , β ) = (N A α, N B β).

Recall also that by construction of the dataset, the g we thus obtain is uncorrelated with f .

2 CONJECTUREHere we provide numerical evidence showing that α β 2 in a large region around the total neglect minima found during the training of our architecture on the biased dataset (i.e. the one where marginalizing over the starting-point yields a 50/50 chance of being solvable regardless of the maze shape).For a given set of K h ot, K and b parameters we fix the maze shape and study the variance of the top layer activations given O(100) different starting points.

We pick the maximal of these and then average this maximal variance over O(100) different mazes.

This yields our estimate of α.

In fact it is an upper bound on α as this averaged-max-variance may reflect wrong prediction provided that they depend on H 0 .We then obtain an estimate of β by again calculating the average-max-variance of the top layer however now with H 0 = 0 for all maze shapes.

Next we chose a 100 random paths parametrized by γ leading from the total neglect minima (γ = 0) for the total neglect through a random point at γ = 15, and then to the checkerboard-BFS minima at γ = 30.
...
The graph below shows the statistics of α/β 2 on these 100 different paths.

Notably no path even had α > e −30 β 2 within the hyper-cube.

We have tried three different other lengths for the hyper cube (12 and 1) and arrived at the same conclusions.

The natural logarithm of an upper bound to α/β 2 as a function of a paramterization (γ) of a path leading from the numerically obtained total neglect minima to the checkerboard BFS minima through a random point.

The three different curves show the max,mean, and median based on a 100 different paths.

Notably no path violated the α β 2 constrain in the vicinity of the total neglect minima.
