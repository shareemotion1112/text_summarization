...
A dependent endogenous Delta-distribution variable j that has a binary functional output g(f (. . .), ε j ) and a related "noise" exogenous Bernoulli variable ε j .

Func-

The exogenous noise variable ε j with predefined probability q flips (i.e. maps from 1 to 0 and vice versa) the value of f if ε j = 1.

Probability q is sampled such that q ∼ UniformContinuous[0.3, 0.7] during network generation.

Vector θ j is a vector with elements θ j,k ∼ Beta(5, 5) sampled randomly during network generation and then unit normalised.
...
Nodes N 3 and N 4 are "dependent" endogenous Delta-distribution variables, where parents of those variable include its own "exogenous noise variable" ε j .
...
"Pyro with guide", in which we define a guide (proposal model) for "noise" Bernoulli variables {ε i }.
...
The model is a simple Gaussian model with two latent variables X and Z and one emission variable Y with additional Gaussian noise ε.
...
The alternative is to force the explicit noise variable ε to a value that allows emission variable Y = X + Z + ε to match its observation.
...
In this example, ε is additive.

In reality, it can enter in the Structural Causal Model in any form so long as there is a function observed noise invert function fε such that fε(Y, X, Z) = ε (it can also return none (i.e. leading to rejection) or multiple (i.e. additional sampling must be made) appropriate ε-s).

In the case of this example, ε = fε(Y, X, Z) = Y − X − Z.
...
For example, a Normal procedure Normal(µ, σ) can have a sibling procedure ObservableNormal(µ, σ).
...
Figure 4b is a representation of the same model but it explicitly represents the exogenous noise variable as an independent variable (highlighted with gradient) such that the emission variable Y is a deterministic function of X, Z and ε (that way, it is aligned with the general requirements of structural causal models).

That way, variable Y is just a sum of three variables.

For the purpose of "observational" inference both representations have the same joint posterior P (X, Z | Y =ŷ).

However, counterfactual query P (Y | Y =ŷ, do(Z =z)) will be different for Figures 4a and 4b , because in the former case the randomness over the noise has not been recorded in the joint and cannot be used for the counterfactual prediction.
...
Note that if anyone tries to, "technically", compute the "counterfactual" query P (Y | Y =ŷ, do(Z =z)) given the model representation in Figure 4a , it will be the same as counterfactual query P (Y 2 | Y 1 =ŷ, do(Z =z)) in Figure 4c rather than in Figure 4b as it might had been expected.
...
Figure 6 illustrates the Observable Normal ERP with its inverse function (for the proposal) ε := ObservedV alue−f (X1, . . .

, XN ), if there is an observation.

That way, the observation is satisfied.

Figure 6 Figure 7 shows the Observable Bernoulli ERP where its noise variable ε flips the output of the binary function f (X1, . . .

, XN ) if ε = 1; however, if ε = 0, then the emission variable Y just returns the value of function f (X1, . . .
...
Each parent j, if active, can switch the noisy-OR variable Y but with only probability 1 − λ j .

In other words, there is a noisy variable associated with each parent: only if both the parent j is T rue and the associated noise variable ε j is T rue, then the noisy-OR variable Y becomes T rue.

There is also one more, independent, "leak" cause for the noisy-OR variable Y to be switched on: that is if the noise (leak) variable ε 0 = Bernoulli(1.0 − λ 0 ) is T rue.

By that definition, the noisy-OR variable Y is F alse only if all ε j , such that j includes j = 0 and j includes all parents that have state T rue, are F alse; that is exactly what the equation above calculates.

For the Observable Noisy-OR procedure, the proposal for noise variables ε j is more sophisticated.

For example, a proposal might be as follows:

1.

If the observed value is F alse, then: (a) for any Xi that is T rue, the associated noise variables ε j should be set (i.e. proposed with probability 1.0) to F alse; (b) the noise variable ε 0 should be set to F alse; (c) all other noise variables ε j can be sampled from any non-degenerated proposal (i.e. such that both T rue and F alse states have non-zero probability).

2.

If the observed value is T rue, then: (a) all ε j s.t.

j >= 1 can be sampled from any non-generative proposal; then (b-1) if that is enough, with the states of the parents, to enable Y to be T rue, then variable ε 0 can be sampled from any non-generative distribution; alternatively, (b-2) if that is not enough, variable ε 0 must be set to T rue.

Note that the proposal describe just above is one of many possible proposals.

Another proposal might be that if the observed value is T rue, then absolutely all ε j including j = 0 are sampled from some non-generative proposal, and if that does not result in variable Y being T rue, then that sample is just rejected (in other words, its weight will be 0).
