...
It is a tuple {S, A, P, r, γ} with S the state space, A the action space we consider as finite in the present work, P the transition kernel defining the dynamics of the environment, r a bounded reward function and γ ∈ (0, 1) a discount factor.

The policy π maps states to distributions over actions: π(·|s).

The (random) discounted return is defined as G = t≥0 γ t r t .

The policy π is trained to maximize the agent expected discounted return.

The function V π (s) = E π [G|s 0 = s] denotes the value function of policy π (where E π [·] denotes the expectation over all possible trajectories generated by policy π).

We also call µ 0 the initial state distribution and ρ(π) = E s∼µ0 [V π (s)] the expected cumulative reward starting from µ 0 .

Value-based algorithms (Mnih et al., 2015; Hessel et al., 2018) use the value function, or more frequently the action-value function Q π (s, a) = E π [G|s 0 = s, a 0 = a], to compute π.

To handle large state spaces, deep RL uses deep neural networks for function approximation.

For instance, value-based deep RL parametrize the action-value function Q ω with a neural network of parameters ω and deep actor-critics (Mnih et al., 2016) directly parametrize their policy π θ with a neural network of parameters θ.
...
Given a classifier C, an input x, a bound on a norm . , an adversarial example is an input x = x + η such that C(x) = C(x ) while x − x ≤ .
...
From a linear approximation of C, it computes the attack η as:

with l(θ, x, y) the loss of the classifier and y the true label.

As an adversary, one wishes to maximize the loss l(θ, x + η, y) w.r.t.

η.

Presented this way, it is an untargeted attack.

It pushes C towards misclassifying x in any other label than y. It can easily be turned into a targeted attack by, instead of l(θ, x + η, y), optimizing for −l(θ, x + η, y target ) with y target the label the adversary wants C to predict for x .
...
As a policy can be seen as a mapping S → A, untargeted FGSM (1) can be applied to a deep RL agent to stop it from taking its preferred action: a * = arg max a∈A π(a|s).
...
Formally, CopyCAT is composed of a set of additive masks ∆ = {δ i } 1≤i≤|A| than can be used to drive a policy π to follow any policy π target .

Each additive mask δ i is pre-computed to lure π into taking a specific action a i when added to the current observation regardless of the content of the observation.
...
Notations We denote π the attacked policy and π target the target policy.

At time step t, the policy π outputs an action a t taking the state s t as input.
...
If CopyCAT is successful, π is going to behave as π target and thus may experience observations out of the distribution represented in D. Yet, as will be shown, CopyCAT transfers to unseen observations.

We hypothesize that, as we build a universal attack, the learned attack is able to move the whole support of observations in a region of R N where π chooses a precise action.

Training A natural strategy for building an adversarial example targeted towards labelŷ is the following.

Given a classifier P(y|x) parametrized with a neural network and an input example x, one computes the adversarial examplex = x + δ by maximizing log P(ŷ|x) subject to the constraint:
...
For each a i ∈ A we build δ i , the additional masks luring π into taking action a i , by maximizing over δ i :
...
CopyCAT has two main parameters: ∈ R + , a hard constraint on the L ∞ norm of the attack and α ∈ R + , a regularization parameter on the L 2 norm of the attack.

Inference Once ∆ is computed, the attack can be used on π to make it follow any policy π target .

At each time step t and given past observations, π target infers an action a target t

given the sequence of observations and the corresponding mask δ a target t ∈ ∆ is applied to the last observation o t before being passed to the agent.
...
We wish to build an attack targeted towards the policy π target .

At a time step t, the attack is said to be successful if π under attack indeed chooses the targeted action selected by π target .

When π is not attacked, the attack success rate corresponds to the agreement rate between π and π target , measuring how often the policies agree along an unattacked trajectory of π.

Note that we only deal with trained policies and no learning of neural policies is involved.

In other words, π and π target are trained and frozen policies.

What we really want to test is the ability of CopyCAT to lure π into having a specific behavior.
...
The agent has a policy π trained with DQN (Mnih et al., 2015) .

The policy π target is trained with Rainbow (Hessel et al., 2018) .
...
This way, in addition to measuring the attack success rate, we can compare the cumulative reward obtained by π under attack ρ(π) to ρ(π target ) as a proxy of how well π's behavior is matching the behavior induced by π target .

In this setup, if the attacked policy indeed gets cumulative rewards as high as the ones obtained by π target , it will mean that we do not simply turned some actions into other actions we targeted, but that the whole behavior induced by π under attack matches the one induced by π target .
...
We argue that measuring cumulative reward is thus a reasonable proxy for monitoring the behavior of π under attack.
...
The gradient from Eq. 1: ∇ st l(θ, s t , a target ) becomes ∇ ot l(θ, f (o t , o 1:t−1 ), a target ).

To keep the comparison fair, a target is always computed with the exact same policy π target as in CopyCAT.

FGSM-L ∞ has the same parameter as CopyCAT, bounding the L ∞ norm of the attack.

CopyCAT has an additional regularization parameter α allowing the attack to have, for a same , a lower energy and thus be less detectable.

We will compare CopyCAT to the attack from Huang et al. (2017) showing how behaviors of π under attacks match π target when these attacks are of equal energy.
...
For learning the masks of ∆, we gather trajectories generated by π in order to fill D with 10k observations.

We use a batch size of 8 and the Adam optimizer (Kingma & Ba, 2014) with a learning rate of 0.05.

Each point of each plot is the average result over 5 policy π seeds and 80 runs for each seed.

Only one seed is used for π target to keep comparison in terms of cumulative reward fair.

CopyCAT has an extra parameter α, we test its influence on the L 2 norm of the produced attack.

For a given , FGSM-L ∞ computes an attack η of maximal energy.

As given by Eq. 1, its L 2 norm is η 2 = √ N 2 with N the input dimension.

For a given , CopyCAT produces |A| masks.

We show in Fig. 1 the largest L 2 norm of the |A| masks for a varying α (plain curves) and compare it to the norm of the FGSM-L ∞ attack (dashed lines).
...
As can be seen on Fig. 1, for a given and for the range of tested α, the attack produced by CopyCAT has lower energy than FGSM-L ∞ .
...
First, we study results in terms of attack success rate -rate of action chosen by π matching a target when shown attacked observations-as done in supervised learning.

Second, we study the behavior matching through the cumulative rewards under attack ρ(π).
...
If, in addition, the average cumulative reward obtained by the agent under attack reaches ρ(π target ) it means that the attack is really successful in terms of behavior.

We recall that we attack a policy with a target policy reaching higher average cumulative reward.

We show on Fig. 2 and 3 (two different games) the attack success rate (left) and the cumulative reward (right) for CopyCAT (plain curves) for different values of the parameters α and , as well as for unattacked π (green dashed line) and π target (black dashed lines).

We observe a gap between having a high success rate and forcing the behavior of π to match the one of π target .

There seems to exist a threshold corresponding to the minimal success rate required for the behaviors to match.

For example, as seen on the left, CopyCAT with = 5 and α < 10 −5 (green curve) is enough to get a 85% success rate on the attack.

However, as seen on the right, it is not enough to get the behavior of π under attack to match the one of the target policy as the reward obtained under attack never reaches ρ(π target ).

Overall, we observe on Fig. 2-right and Fig. 3 -right that with high enough ≥ 0.04 and α < 10 −6 , CopyCAT is able to consistently lure the agent into following the behaviour induced by π target .
...
These values are plotted (i) against the L 2 norm of the attack for FGSM and (ii) against the largest L 2 norm of the masks: max i δ i 2 for CopyCAT.

We only plot the standard deviation on the attack success rate because it corresponds to the intrinsic noise of CopyCAT.

We do not plot it for cumulative reward for the reason that one seed of π target has a great variance (with the sticky actions) and matching π target , even perfectly, implies matching the variance of its cumulative rewards.

The same phenomenon can be observed on Fig. 2 and 3: CopyCAT is not itself unstable (left figures, when α decreases or increases, the rate of successful attacks consistently increases).

Yet the cumulative reward is noisier, as the behavior of π is now matching with a high-variance policy.
...
However, it is never able to make π's behavior match with π target 's behavior as seen on Fig. 4-right and Fig. 5 -right.

The average cumulative reward obtained by π under FGSM attack never reaches the one of π target .

On the contrary, CopyCAT is able to successfully lure π into following the desired macroscopic behavior.

First, it turns more than 99% of the taken actions into the targeted actions.

Second, it makes ρ(π) under attack reach ρ(π target ).
...
The patch δ i aiming at action "no-op" (i.e. do nothing) is applied to an agent playing Space Invaders.
...
Below the images, the action taken by the same policy π when shown the different situations in an online setting.
...
Transferring targeted examples may not be easy and would probably require to train imitation models to obtain mappings similar to π in order to compute transferable adversarial examples.
...
To further assess CopyCAT's ability to lure a policy into following another policy, we therefore attack an untrained DQN, with random weights, to follow the policy π target (still obtained from a trained Rainbow agent).
...
We see that in this case, FGSM is able to lure π into following π target at least as well as CopyCAT.
...
CopyCAT is also able to lure the agent into following π target .
...
We consider a setting where the adversary (i) is given a set of proxy models {π 1 , ..., π n } trained with the same algorithm as π, (ii) can also query the attacked model π, but (iii) has no access to its weights.
...
For each action a i , we compute the mask δ i by maximizing over 100 epochs on the dataset D:
...
This accuracy is computed by querying π on states built as follows.

We take four consecutive observations in D, apply 3 masks randomly selected among the previously computed masks on the first 3 observations; the mask δ i that is actually being tested is applied on the last observation.

The attack is considered successful if π outputs the action a i corresponding to δ i .
...
Four proxy models of DQN are used to attack π.

Again, it is attacked to make it follow the policy π target given by Rainbow.
...
The figure on the left gives the value of α (on the y-axis) corresponding to each color.
