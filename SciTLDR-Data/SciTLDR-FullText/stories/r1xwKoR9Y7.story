...
ιn n at the mid-level, where ι i marks implicit versus non-implicit arguments.
...
Coq represents a proof state PS as a tuple Γ, M, n of a local context Γ, a goal term M , and a unique proof state identifier n. A context Γ provides a list of identifiers and their types that expresses all the current assumptions.

For instance, the proof state with identifier 3 has Γ = n : nat, IHn : n = n + 0.
...
We can apply this idea to obtain a new embedding function E : Term × Env → R D that additionally takes an environment ρ of type Env which is a lookup table mapping variables to embedding vectors.

Whenever we encounter a binding form such as a dependent product Πx : M 1 .

M 2 (similar to an anonymous function λx : M 1 .

M 2 ), we bind a new meaning for x within its scope (i.e., the term M 2 ).

To do so, we sample a random vector v ∼ N D according to a standard (multivariate) normal distribution 2 N D and extend the local environment ρ with the mapping x → v, written ρ[x → v], so that mentions of the variable x in M 2 can be looked up.

Then the embedding of a binding form, such as a dependent product term (Prod), is obtained recursively as DISPLAYFORM1 The embedding for the variable case (which occur at the leaves of the AST) is E(x, ρ) = ρ(x), which corresponds to a variable lookup from the environment.

We resample the corresponding v from N D every forward pass in training when we embed the term Πx : M 1 .
