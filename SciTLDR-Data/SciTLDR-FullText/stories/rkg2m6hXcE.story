...
An LTL specification ϕ is constructed from a set of propositions V , the standard Boolean operators, and a set of temporal operators.

Its truth value is determined with respect to a trace, π, which is an infinite or finite sequence of truth assignments for all propositions in V .

The notation π, t |= ϕ indicates that ϕ holds at time t.

The trace π satisfies ϕ (denoted by π |= ϕ) iff π, 0 |=

ϕ. The minimal syntax for LTL can be described as follows: DISPLAYFORM0 where p is a proposition, and ϕ 1 and ϕ 2 are valid LTL specifications.
...
X reads as "next" where Xϕ evaluates as true at t if ϕ holds in the next time step t + 1.

U reads as "until" where ϕ 1 Uϕ 2 evaluates as true at time step t if ϕ 1 is true at that time and going forward, until a time step is reached where ϕ 2 becomes true.

In addition to the minimal syntax, we also use higher-order temporal operators, F (eventually), G (global), and R (release).

Fϕ holds true at t if ϕ holds for some time step ≥ t. Gϕ holds true at t if ϕ holds for all time steps ≥ t. ϕ 1 Rϕ 2 holds true at time step t if either there exists a time step t 1 ≥ t such that ϕ 2 holds true until t 1 where both ϕ 1 and ϕ 2 hold true simultaneously, or no such t 1 exists and ϕ 2 holds true for all time steps ≥ t.
...
A contrastive explanation is a constraint ϕ that it is satisfied by one set of plan traces (positive set, π A ), but not by the other (negative set, π B ).The constraint ϕ can be seen as a classifier trying to separate the provided positive and negative traces.

Its performance measure corresponds to standard classification accuracy, computed by counting the number of traces in π A that satisfy ϕ and, conversely, the number of traces in π B where ϕ is unsatisfied.

Formally, accuracy of ϕ is: DISPLAYFORM0 Accuracy is 1 for a perfect contrastive explanation, and approaches zero if both sets contains no valid trace with respect to ϕ (i.e., all traces in π A dissatisfy ϕ and all traces in π B satisfy ϕ).

The input to the problem is a pair of sets of traces (π A , π B ).

Each π i ∈ π is a trace on the set of propositions V (we refer to V as the vocabulary).

The output is a set of specifications, {ϕ}, where each ϕ achieves perfect or near-perfect contrastive explanation.

This is an unsupervised classification problem.

We use LTL specifications for the choice of ϕ. Planning is sequential, and so temporal patterns can offer greater expressivity and explanatory power for identifying plan differences rather than static facts.
...
The candidate formula ϕ is then composed as a conjunction of multiple instantiations of a template T based on a set of selections {p} ⊆ V n T .
...
Instead of extracting specifications narrowed down to a single template query, our hypothesis space Φ is set to include a number of predefined templates, T 1 , T 2 , ...T k .

With k representing the number of possible templates, the full hypothesis space of Φ grows with O(k · 2 |V | n T ).

Employing brute force enumeration to find {ϕ} that achieves the contrastive explanation criterion rapidly becomes intractable with increasing vocabulary size.

We model specification learning as a Bayesian inference problem, building on the fundamental Bayes theorem: DISPLAYFORM0 Our goal is to infer ϕ * = argmax Φ P (ϕ|X).

P (ϕ) represents the prior distribution over the hypothesis space, and Figure 1 : A graphical model of the generative distribution.

ϕ represents the latent LTL specification that we seek to infer given the evidence X (in our case, the traces).P (X | ϕ) is the likelihood of observing the evidence X = (π A , π B )

given ϕ. We adopt a probabilistic generative modeling approach that has been used extensively in topic modeling BID2 .

Below, we describe each component of our generative model, depicted in Figure 1 .Prior Function ϕ is generated by choosing a LTL template, T , the number of conjunctions, N , and then the proposition instantiations, p for each conjunct.
...
w T is a hyperparameter that the designer can set to assert preferences for mining certain types of templates over others (e.g., preferring templates with "global" operators than "until" operators).The number of conjunctions, N = |{p}|, is generated using a geometric distribution with a decay rate of λ.

Thus, the the probability of ϕ is reduced by λ for each addition of a conjunct, incentivizing low-complexity specifications defined in terms of having a fewer number of conjunctions (which also implies fewer total propositions).
...
The full prior function, P (ϕ), is evaluated as follows: DISPLAYFORM2 The derivation follows from the definition that T , N , {p} completely describe ϕ (i.e. P (ϕ | T, N, {p}) = 1), and the assumption that the three probability distributions are independent of each other.
...
Formally: Likelihood Function The likelihood function P (X | ϕ) is the probability of observing the input sets of traces in the satisfying set π A and the non-satisfying set π B given the contrastive specification.

The traces in π A and π B are generated by different solutions to the planning problem that satisfy the problem specification.
...
With the conditional independence assumption, the likelihood can then be factored as follows: DISPLAYFORM3 DISPLAYFORM4 LTL satisfaction checks are conducted over all traces belonging to sets π A and π B ; P (π i |ϕ) is set equal to 1 − α if π i |= ϕ, and α otherwise.

Conversely, P (π j |ϕ) is set equal to 1−β if π j ϕ, and β otherwise.

α and β permit non-zero probability to traces not adhering to the constrastive explanation criterion, thereby providing robustness to noisy traces and outliers.

α and β may be set to different values to reflect the relative importance of the positive and negative sets (e.g., may be used to counteract imbalanced sets).In order to perform LTL satisfaction checks on a trace, we follow the method developed by BID17 , in which ϕ is represented as a tree and each temporal operator is recursively evaluated according to its semantics.

Since sub-trees of two different ϕ may be identical, we memoize and re-use evaluation results to significantly speed up LTL satisfaction checks.

Proposal Function Exact inference methods to find maximum a posterior (MAP) estimates, {ϕ * }, are intractable.
...
MH sampling requires a user-defined proposal function F (ϕ |ϕ) that samples a new candidate ϕ given the current ϕ. Our F behaves similar to an -greedy search, utilizing a drift kernel (i.e. a random walk) with a probability of 1-or sampling from the prior distribution (i.e. a restart) with a probability of .
...
Note that the drift kernel perturbs ϕ, but stays within the current template.

ϕ transitions to a new template (probabilistically) when choosing to sample from the prior.
...
A new sample ϕ is accepted at every MH iteration with the following probability: DISPLAYFORM6 The set of accepted samples approximates the true posterior, and the MAP estimates (the output {ϕ * }) are determined from the relative frequencies of accepted samples.
...
The plan traces in π A were generated by first injecting the ground truth ϕ ground into the original PDDL domain and problem files, enforcing valid plans on the modified domain/problem files to satisfy ϕ ground .
...
Similarly, the above steps were repeated to generate execution traces for π B , wherein the negation of the ground truth specification, ¬ϕ ground , was injected to the planning files, and then a set of traces was collected.

Such a setup guarantees the existence of contrastive explanation solutions on (π A , π B ), which includes (but is not limited to) ϕ ground .
...
For each of these domains, we tested three different problem instances of increasing vocabulary size, and on twenty randomly generated ϕ ground specifications for each problem instance.

For each test case, ϕ ground was randomly generated using one of the seven LTL templates listed in Table 1 ; thus the hypothesis space Φ was set to include all possible specifications over the predefined templates.

The categorical distribution weights, w T and w p , were set to be uniform.

Other hyperparameters were set as follows: α = β = 0.01, to put equal importance of positive and negative sets, λ = 0.7 to penalize ϕ for every additional conjunct, and = 0.2 to apply -greedy search in the the proposal function.
...
Because enumerating through full space of Φ would result in a time out, we tested delimited enumeration with only a random subset of brute force samples.

This baseline selects a random subset of size num brute from Φ. Then, a function proportional to the posterior distribution (numerator in Equation 4) is evaluated for each of the samples to determine {ϕ * }.
...
For evaluation, we measured M = |{ϕ * }|, the number of unique contrastive explanations extracted by the different approaches, along with the explanations' accuracy.

Each domain-problem combination row shows the average statistics over twenty ϕ ground test cases.
...
The solution set {ϕ * } almost always included ϕ ground .
...
For example, on Figure 3 : The accuracy of ϕ * with respect to increasing noise rate.

ϕ * is inferred from the perturbed, noisy data and then is evaluated (generalized) on the original input X. Each domain subplot shows the averages across all three problem instances and all twenty ϕ ground test cases.

95% confidence intervals are displayed.problems where holding a block is a prerequisite to placing it onto a table, ϕ 1 = F(holding A) ∧ F(holding B) will be satisfied in concert with the satisfaction of ϕ 2 = F(ontable A)∧F(ontable B).
...
For a contrastive ϕ, it is possible to create a new contrastive ϕ that includes stationary propositions or tautologies specific to the planning problem.

For example, if ϕ 1 = F(holding A) ∧ F(holding B) is a contrastive explanation, so is ϕ 3 = F(holding A) ∧ F(holding B) ∧ F(earth is round).

Our posterior distribution assigns a lower probability to ϕ 3 than ϕ 1 based on the decay rate on the number of conjunctions.
...
This opens up interesting research avenues for determining a minimal set of {ϕ}. Assessing logical dependence or metric space between two arbitrary LTL specifications, however, is non-trivial.
...
The detail of the input was as follows: |π A |=16, |π B |=8, |V |=15, and the average length of traces involved 11 time steps.

Within a second (2,000 samples), our model generated ten unique contrastive explanations, all with accuracy of 0.96.

ϕ * 1 = G(attrition < 0.25) ∧ G(striker not shot) represents how friendly attrition rate should be always less than 25% and that the striker aircraft should never be shot upon.

ϕ * 2 = (attrition < 0.25) U (weapon release) asserts how friendly attrition rate has to be less than 25% before releasing the weapon.
...
After discussion with the expert, we discovered that the model could not generate the perfect contrastive ϕ ground , because it required having multiple conjuncts that incorporate different LTL templates (which is not part of our defined solution space).
