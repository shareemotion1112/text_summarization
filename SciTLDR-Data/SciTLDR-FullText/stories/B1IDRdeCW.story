...
Second, the non-differentiable binarize function (θ(x) = 1 if x > 0 and −1 otherwise) is replaced with a continuous one during backpropagation.
...
In the backward propagation step, one normally computes the derivative of the cost with respect to the input of a transformer via the Jacobian of the forward function and the derivative of the cost with respect to the output of that transformer (δu ≡ dC/du where C is the cost function used to train the network).
...
In particular, we show that the angle between a random vector (from a standard normal distribution) and its binarized version converges to arccos 2/π ≈ 37• as the dimension of the vector goes to infinity.
...
The result follows from the central limit theorem because the cosine angle between two such random vectors is normally distributed with µ = 0 and σ ∼ 1/ DISPLAYFORM0 Building upon this work, we study the way in which binary vectors are distributed relative to continuous vectors.
...
For each layer, the distribution of the angles between the binary and continuous weights is sharply peaked near the d → ∞ expectation of arccos 2/π.
...
While we know that w b = θ(w c ), there are many continuous weights that map onto a particular binary weight vector.
...
In order to be more precise, we define the Generalized Binarization Transformation (GBT) DISPLAYFORM0 where x is a column vector, R is a fixed rotation matrix, and θ is the pointwise binarization function from before.

The rows of R are called the axes of binarization.

If R is the identity matrix, then θ R = θ and the axes of binarization are the canonical basis vectors (..., 0, 1, 0, ...).
...
The angle between a vector and its binarized version is dependent on the dot product: x · θ R (x), which is equal to x T θ R (x) = (Rx) T θ(Rx) = y · θ(y) where y = Rx.
...
Note that the correlation is weaker in the first layer.of the GBT, consider the case where x ∼ N (0, Σ) and Σ i,j = δ i,j exp(2ki) for k = 0.1 (therefore y ∼ N (0, RΣR T )).

As the dimension goes to infinity, the angle between a vector drawn from this distribution and its binarized version approaches π/2.
...
Since the Gaussian is no longer isotropic, the red curve no longer peaks at θ = arccos 2/π.
...
Consider a vector, v, chosen from a standard normal distribution of dimension n. DISPLAYFORM0 where Γ is the Gamma function.

Proof: Begin by considering the integral DISPLAYFORM1 where I is an indicator function.

The desired distribution comes from taking the derivative of this cumulative distribution g( DISPLAYFORM2 Thus we can write out the integral DISPLAYFORM3 The integral factorizes and all of the terms are independent of ρ 0 except the integral over DISPLAYFORM4 Using the substitution ρ = cos φ (which is also consistent with the definition of ρ above), dρ = − sin(φ)dφ, DISPLAYFORM5 Taking the derivative with respect to ρ 0 and using the fundamental theorem of calculus gives g(ρ) ∼ (1 − ρ 2 ) (n−3)/2 .

The normalization constant is equal to a beta function that evaluates to the desired result (substitute t = ρ 2 ).

Γ(z+α) DISPLAYFORM0 • Distribution of angles between two random vectors.
...
Then the cosine angle between those two vectors is ρ as defined above.

While the exact distribution of ρ is given by Lemma 1, we note that -E(ρ) = 0 due to the symmetry of the distribution.

DISPLAYFORM1 • Angles between a vector and the binarized version of that vector, η = DISPLAYFORM2 Γ((n−1)/2) (substitute u = ρ 2 and use Γ(x + 1) = xΓ(x) ).
...
As E(η) has been calculated, it suffices to calculate E(η 2 ).

Expanding out η 2 , E(η DISPLAYFORM4 ).

Below we show that E( DISPLAYFORM5 πn .
...
Going back to the calculation of that expectation, change variables to v 1 = r cos θ, DISPLAYFORM8 The integration over the volume element dv 3 . . .
...
Then DISPLAYFORM10 Then substitute r = p cos φ, z = p sin φ where DISPLAYFORM11 The first integral is 2 n(n−2) using u = sin 2 φ.

The second integral is 2 (n−2)/2 Γ(n/2) using u = p 2 /2 and the definition of the gamma function.

Simplifying, the result is 2 π * n .Thus the angle between a vector and a binarized version of that vector converges to arccos DISPLAYFORM12 which is a very small angle in high dimensions.
...
In this case, all of the components of w become independent: δw = * (α − θ(w)) where is the learning rate and α is the entry of C yx corresponding to a particular element, w. Compared to regular linear regression, it is clear that the stable point of these equations is when w = α.

Since the weight is binarized, that equation cannot be satisfied.

However, it can be shown ( ) that in this special case of binary weight linear regression, E(θ(w c )) = α.
...
In particular, if w > 0, then the step is − · |1 − α| and if w < 0, the step is · (α + 1).

It is evident that after a sufficient burn-in period, |w| ≤ * max(|1 − α|, 1 + α) ≤ 2 .

Suppose w > 0 occurs with fraction p and w < 0 occurs with fraction 1 − p.

In order for w to be in equilibrium, oscillating about zero, these steps balance out on average: p(1 − α) = (1 − p)(1 + α) → p = (1 + α)/2.

Then the expected value of θ(w) is 1 * p + (−1) * (1 − p) = α.

When |α| > 1, the dynamics diverge because α − θ(w) will always have the same sign.
