...
Denote an input to the network as x ∈ R d with correct labelŷ ∈ Y ⊂ N, and let m θ : R d → R |Y| be the mapping performed by the neural network which is parameterized by θ ∈ R p .

Let L : Y ×R |Y| → R denote the loss we are trying to minimize (e.g., the cross-entropy).

When training a neural network we seek to solve minimize DISPLAYFORM0 over variable θ, where D is the data distribution.

Given any fixed θ we can generate (untargeted) adversarial inputs by perturbing the input x so as to maximize the loss.
...
In other words we replace the problem in eq. FORMULA0 with DISPLAYFORM2 where ρ ≥ 0 is the weighting parameter andx is an adversarial example generated from x at model parameters θ using, for example, the update in eq. (2).
...
If we take a Bayesian approach to the classification problem, then we have a prior over possible model parameters, p(θ), a likelihood of the data, p(D|θ), and a probability of a label y given an input and a model, p(y|x, θ).
...
We can consider approximating it by the following approach, sample initial parameters from the prior p(θ) and run an iterative procedure to (approximately) maximize the likelihood p(D|θ).

Very loosely speaking, we can consider this procedure as approximately sampling from the posterior over models p(θ|D) ∝ p(D|θ)p(θ).

Consequently, we output the classification DISPLAYFORM2 Algorithm 1 Adversarial ensemble training using PGD under ∞ norm constraint input: k neural networks m θi , i = 1, . . .

, k; attack steps N ; step sizes η,η; initial variance σ; adversarial loss weighting ρ; perturbation width δ initialize: neural network parameters θ DISPLAYFORM3 end for update parameters for each i = 1, . . .
...
When using ensembles the loss function for adversarial training in (3) is replaced by the mean of the loss over the k models, i.e., now we want to solve DISPLAYFORM5 over variables θ i , i = 1, . . .
...
Here, n indicates the number of parameters in the base model,ŷ indicates the ground trouth label, x is a clean input from the dataset,x is that input after a number of steps of the chosen adversarial training attack (7 steps of IFGSM in our experiments), y is the output distribution according to the network based on clean input x, andỹ is the output based on adversarial inputx.
...
The cross-entropy losses with regard to the ground truth labels for both the adversarial and clean images are averaged to obtain gradients for the model (i.e. ρ = 1).
