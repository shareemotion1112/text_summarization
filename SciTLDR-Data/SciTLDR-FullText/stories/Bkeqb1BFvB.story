...
If P = −W + Π T W Π with Π ∈ P(n) then the perturbation is a permutation of the node indexing.
...
If M is a matrix associated to G, we denote M the equivalent matrix for G. We denote λ(X) the eigenvalue of a square matrix X in ascending order, λ i (X) the i th smallest eigenvalue.

Hypothesis 1.

Without loss of generality, we assume that G 2 is a perturbed version of G 1 , i.e. ∃P * the sparsest |V 2 |-square perturbation matrix and Π * ∈ P(|V 2 |) a |V 2 |-square permutation matrix such that W 2 = Π * T W 1 + P * Π * .
...
Two graphs G 1 and G 2 are isomorphic if and only if ∃Π ∈ P( Merris, 1994) , hence when they are structurally equivalent irrespective to the vertex ordering.
...
The DGI between graphs G 1 and G 2 is generally the minimal Frobenius norm of the difference between L 1 and Π −1 L 2 Π. Considering this definition, the following Lemma links the graph-isomorphism problem and the Laplacian of the hypothetical perturbation P * and show that this divergence is the norm of L P * : Lemma 1.
...
As a first illustration of deformation-based results presented in Section 4, we propose to use ErdosRnyi random graphs (Erdős & Rényi, 1959) with parameter p = 0.05.
...
More sophisticated algorithms consider features based on attributes of random walks on the graph (Gómez & Delvenne) while others are graphlet based (Kondor et al., 2009) . (Kondor & Borgwardt, 2008) explicitly built permutation-invariant features by mapping the adjacency matrix to a function on the symmetric group. (Verma & Zhang, 2017) proposed a family of graph spectral distances to build graph features.
...
Now let (λ, x) be any eigen couple of a matrix M ∈ M n×n .
...
Hyper parameters C and γ are tuned among respectively {0.5, 1, 5} and {0.0001, 0.001, 0.01, 0.1, 0.5, 1, 5} for the molecular datasets, and {0.5, 1, 5, 25, 50} and {0.0001, 0.001, 0.01, 0.1} for the social network datasets.
