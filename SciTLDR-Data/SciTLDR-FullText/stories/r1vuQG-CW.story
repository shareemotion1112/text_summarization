...
More formally, let L t denote the operator that translates a feature map f : Z 2 → R K by t ∈ Z 2 , and let ψ denote a filter.
...
If instead we apply a rotation r, we obtain: DISPLAYFORM1 That is, the convolution of a rotated image L r f by a filter ψ equals the rotation of a convolved image f by a inversely rotated filter L r −1 ψ.

There is no way to express [L r f ] ψ in terms of f ψ, so convolution is not rotation equivariant.
...
In practice, this is implemented as an indexing operation Ψ[I] using a precomputed static index array I.The second step of the group convolution is a planar convolution of the input f with the transformed filterbank Ψ [I] .
...
In summary, a filter transformation is applied to a learnable filter bank Ψ resulting in a filter bank Ψ than can be used to compute the group convolution using (multiple) planar convolutions (see the top of Figure 1 for a visual portrayal of this transformation).
...
Recall that in our mathematical model, filters and feature maps are considered functions ψ : X → R K , where X = Z 2 or X = G. In the filter transformation step, we need to compute the transformed filter L r ψ for each rotation r ∈ H and each filter ψ, thus increasing the number of output channels by a factor |H|.

The rotation operator L r is defined by the equation [L r ψ](h) = ψ(r −1 h).

Our goal is to implement this as a single indexing operation Ψ [I] , where Ψ is an array storing our filter, and I is a precomputed array of indices.
...
A filter ψ will be stored in an n-dimensional array Ψ, where n = 2 if X = Z 2 and n = 3 if X = G. An n-dimensional array has a set of valid indices I ⊂ Z n .

Thus, we can think of our array as a function that eats an index and returns a scalar, i.e. Ψ : I → R. If in addition we have an invertible indexing function ι : X → I, we can consider the array Ψ as a representation of our function ψ : X → R, by setting ψ(x) = Ψ[ι(x)].

Conversely, we can think of ψ as a representation of Ψ, because Ψ[i] = ψ(ι −1 (i)).
...
Specifically, we can define: DISPLAYFORM1 That is, we convert our index i to group element h = ι −1 (i).

Then, we compose with r −1 to get r −1 h, which is where we want to evaluate ψ.

To do so, we convert r −1 h back to an index using ι, and use it to index Ψ.To perform this operation in one indexing step as Ψ[I], we precompute indexing array I: DISPLAYFORM2 Finally, we need choose a representation of our group elements h that allows us to compose them easily, and choose an indexing map ι.
