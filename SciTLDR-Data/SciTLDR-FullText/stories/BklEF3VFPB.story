...
Let the feature extractor G e (x; θ e ) be a function parameterized by θ e which maps an input sample x to a deep embedding z. Let the label predictor G y (z; θ y ) be a task-specific function parameterized by θ y which maps an embedding z to a task-specific predictionŷ.

The reconstruction network G r (z; θ r ) is a decoding function parameterized by θ r that maps an embedding z to its corresponding reconstructionx.
...
This is motivated by the covariate shift assumption (Shimodaira, 2000) that indicates if the feature distributions S(z) = {G e (x; θ e )|x ∼ D S } and T (z) = {G e (x; θ e )|x ∼ D T } are similar, the source label predictor G y can achieve a similar high accuracy in the target domain.
...
where L y denotes the negative log-likelihood of the ground truth class for labeled sample (x s i , y s i ) and α controls the interaction of the loss terms.
...
For the penalty term α, we choose 0.02 by searching over the grid {10 −2 , 1}. We also obtain the optimal margin m = 5 by a search over {10 −1 , 10}. Then we use the same hyperparameter settings for all tasks to show the robustness.

For the optimization, we simply use Adam Optimizer (lr = 2 × 10 −4 , β 1 = 0.5, β 2 = 0.999) and train all experiments for 50 epochs with batch size 128.
...
We investigate the effect of α and m on SVHN→MNIST.

The results in Table 3 show that ARN achieves good performance as α ∈ [0.01, 0.1] and even with larger α ARN is able to achieve convergence.

In comparison, denoting α as the weight of adversarial loss, the DANN cannot converge when α > 0.2.
...
In Figure 2 (b), ARN has smoother and more effective gradients (L r ) for all α, while the loss of DAT domain classifier (L d ) gets extremely small at the beginning.
...
We use α = 0.02 and m = 5.0, and we reckon that better results can be obtained by tuning the hyperparameters for specific tasks.
...
It is observed that the accuracy increases when α drops or the margin m increases.

The reason is very simple: (1) when α is too large, it affects the effect of supervised training on source domain; (2) when the margin m is small, the divergence between source and target domain (i.e. H H-distance) cannot be measured well.
