...
In this case, µ + = 1 |S+| s∈S+ a u (s) is the average value of unit activation when forwarding a set of sentences S + , where S + denotes that sentences including candidate concept c n .However, it induces a bias which is similar to section A.1.
...
If such a phrase occurs only one time over the entire corpus, µ + is equal to the activation value of the sentence, which is relatively very high than µ + for other candidate concepts.

This error could be alleviated on a very large corpus where every candidate concept occurs enough in the corpus so that estimation of µ + get relatively accurate, which is practically not possible.
