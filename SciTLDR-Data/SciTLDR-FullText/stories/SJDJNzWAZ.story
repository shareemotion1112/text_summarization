...
With this, h t = f (h t−1 , e t , d t ; θ) where d t is the duration of event e t .
...
DISPLAYFORM0 φ is a nonlinear transformation of d t and is implemented as a feedforward neural network parameterized by θ.

d t is log transformed before it is fed to φ to effectively cover the wide numerical range of duration values, e.g., it can range from seconds to hours for app usage events.
...
We compute a time mask by linearly transforming c d with weights W d ∈ R C×E and bias b d ∈ R E , which is followed by a sigmoid nonlinear activation, σ, to generate a mask m d ∈ R E and R E → [0, 1].
