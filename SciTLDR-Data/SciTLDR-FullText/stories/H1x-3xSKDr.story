...
We use the notation of Yang et al. (2019) , where α is an index for units in a layer l, and i for a mini-batch of B samples from the dataset; N l denotes the number of units in layer l, W l is the matrix of weights and b l is the vector of biases that parametrize layer l.

The batch mean is defined as µ α = 1 B i h αi , and the variance is σ
...
This is described in equation 1, where a per-unit nonlinearity φ, e.g., ReLU, is applied after the normalization.
...
3 BOUNDARY TILTING Tanay & Griffin (2016) relate the adversarial vulnerability of linear classifiers to the tilting angle θ of the decision boundary w.r.t.
...
and one task-irrelevant dimension (α = 2).
...
Compared with the decision boundary of a linear model (θ ≈ 0 • ), the batchnormalized model has θ = 66.7

• .

On the right is the dataset seen by the BNGD classifier.

We use Σ 11 = 1, Σ 22 = 0.01, Σ 12 = Σ 21 = 0.05, ν 0 = [−5, 0] , and ν 1 = [5, 0] .

Consider the binary classification task of identifying two different types of input x subject to Gaussian noise with a linear classifier w x + b. This can be modeled by the class-conditional distribution p(x|y = j) = N (ν j , Σ) with label y ∼ Ber(0.5).

The Bayes-optimal solution to this problem is given by the weight vector w = Σ −1 ν 0 − ν 1 , and b =
...
We analyze the effect of batch-normalizing the input to the classifier for this problem (i.e., h αi = x αi ), first in the simplest setting where γ α = 1, β α = 0 ∀α.

We select the class distribution means ν j to be symmetric around zero, so that the batch mean computed by BN is µ α = 0 ∀α.

The batch-normalized linear classifier is thus defined as: f (x) = w x+b √ σ 2 +c
...
We report the tilting angle (θ) and accuracies of linear models trained on MNIST 3 vs. 7 for vanilla GD, GD with L2 weight decay "WD"(λ = 0.1), and BNGD.

Affine = "F" indicates γ = 1 and β = 0, whereas "T" means they are randomly initialized and learnable.

AWGN = N (0, 1), FGSM used with = 1 /10.

Entries are the mean and its standard error over five random seeds.

The tilting angle θ of the batch-normalized decision boundary w.r.t.
...
To compute θ, we divide the weights w by √ σ 2 + c, and then normalize w/ w 2 , such that θ = cos −1 (w w * ).

From this analysis it follows that the order of magnitude of c is important relative to the data variance: if c > σ 2 α then the effective weight value w α is reduced, and if c < σ 2 α and σ 2 α is small, then w α increases greatly, causing boundary tilting along direction α.
...
Results consistent with the boundary tilting theory are shown in Table 1 , which not only shows that BN causes tilting, but that this is unaffected by the parameters γ and β.

Post-normalization, there is no signal to γ and β about the variances of the original dataset.

This is consistent with other works that observe γ and β do not influence the studied effect (van Laarhoven, 2017; Zhang et al., 2019a; Yang et al., 2019) Increasing the numerical stability constant c increases robustness in terms of absolute test accuracy for additive white Gaussian noise (AWGN) on MNIST and CIFAR-10 datasets by 33% and 41% respectively (at the cost of standard accuracy).
...
We show that this can be achieved by simpler means and with little to no trade-off through 2 weight decay, where the regularization constant λ corrects the loss scaling as the norm of the input increases with d.
...
Table 7 shows that by adding sufficient 2 regularization (λ = 0.01) to recover the original (
...
Finally, following the guidance in the original work on batch norm (Ioffe & Szegedy, 2015) to the extreme (λ = 0): when we reduce weight decay when using batch norm, accuracy for the ∞ = 0.1 perturbation is degraded by 79.3 ± 0.3% for
...
Although it can be shown that robust optimization is equivalent to parameter norm regularization for linear models if we allow the -ball (aka disturbance δ) to vary with each example (Xu et al., 2009), we maintain that the latter is a more efficient approach.
...
For these experiments we fixed γ α = 1 and β α = 0.
...
Increasing either c or λ has a similar effect to trade clean test accuracy for increased robustness, until the effect is too large and both accuracies degrade.
...
Furthermore, good robustness can be experienced simultaneously with good clean test accuracy in unnormalized networks, whereas the regimes of good clean accuracy and robustness are still mostly non-overlapping in Figure 6 (b). (2019) by training fully-connected models of depth L and constant width (N l =384) with ReLU units by SGD, and learning rate η = 10 −5 B for batch size B on MNIST.

We train for 10 and 40 epochs in (a) and (b) respectively.

The BN parameters γ and β were left as default, momentum disabled, and c = 1e-3.
...
We find that the same can be achieved through a much simpler approach: 2 weight decay, with parameter λ chosen dependent on d to correct for the loss scaling.
...
In Figure 9 we show that, using the same architecture as in (Gilmer et al., 2018) , the batchnormalized network is highly sensitive to the learning rate η.
...
Figure 11 : By increasing the numerical stability constant c to between 1e-2 and 1e-1, we can achieve the same tilting angle θ as obtained by not using tracking (10(b)), but which works with arbitrary batch sizes at test time.
...
Figure 10 shows that by eliminating tracking, i.e. going from plot 10(a) to 10(b), the boundary tilting angle θ w.r.t.

the nearest-centroid classifier is reduced by 13.7

• .

Eliminating BN altogether reduces θ by an additional 25.2

• , and using per-image normalization (rather than per-pixel as in BN) achieves a further reduction of θ by 31.3
...
Uses an L 2 weight decay regularization constant λ = 0.01.

The two convolution layers of the LetNet-5 are batch-normalized, with default settings (γ and β enabled, numerical stability constant set to 1e-5, momentum of 0.1).
...
Formally, we sample a starting point x 0 ∼ N (0, 0.1) ∈ R 28×28 , then update x according to x n = x n−1 + δ, using n = 1 to 100 iterations of PGD-2 w.r.t.
...
We use the arbitrary misclassification objective, confidence parameter k = 4 to encourage reasonably high confidence predictions, a "learning rate" of 1e-2, nine (9) binary search steps, and an initial value for the constant c of 1e-3, which balances the minimization of the 2 -norm of the perturbation δ, and prediction confidence.

11 We allow up to 10,000 attack iterations, and show the resulting examples in Figure 13 .

For the same confidence threshold, the mean 2 norm of the perturbations required to change the predictions of the batch-normalized model was δ 2 = 0.95, while δ 2 = 2.89 was required for the unnormalized model.
...
As in §J.1, for the no-BN baseline we use an L 2 weight decay regularization constant λ = 0.01, and per-image preprocessing.
...
Plot (b) shows the perturbed images (x adv = x + δ) for the baseline L 2 regularized model ( δ 2 = 2.89), while plot (d) shows those of the batch-normalized model.
...
For the batch-normalized model, whose adversarial examples are shown in (d), the examples are classified with the same or higher confidence, yet the perturbations are not perceptible and their 2 -norms are significantly lower ( δ 2 = 0.95).
...
To evaluate robustness, we evaluate accuracy on several variants of the test set: "Clean"-the original test set, "AWGN"-with additive white Gaussian noise (σ 2 = 1 /16), "FSGM"-with perturbation crafted by one-step fast gradient sign method ∞ , and "PGD"-a 40-step PGD-∞ perturbation, with a step size of /30.
