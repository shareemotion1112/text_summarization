...
This finally leads us to the WAE objective: DISPLAYFORM3 where Q is any nonparametric set of probabilistic encoders, D Z is an arbitrary divergence between Q Z and P Z , and λ > 0 is a hyperparameter.
...
Initialize the parameters of the encoder Q φ , decoder G θ , and latent discriminator Dγ.

while (φ, θ) not converged do Sample {x1, . . .

, xn} from the training set Sample {z1, . . .

, zn} from the prior PZ Samplezi from Q φ (Z|xi) for i = 1, . . .

, n Update Dγ by ascending: DISPLAYFORM0 Update Q φ and G θ by descending: DISPLAYFORM1 end while ALGORITHM 2 Wasserstein Auto-Encoder with MMD-based penalty (WAE-MMD).Require: Regularization coefficient λ > 0, characteristic positive-definite kernel k.
...
, n Update Q φ and G θ by descending: DISPLAYFORM2 end whileWe point out once again that the encoders Q φ (Z|x) in Algorithms 1 and 2 can be non-random, i.e. deterministically mapping input points to the latent codes.

In this case Q φ (Z|x) = δ µ φ (x) for a function µ φ : X → Z and in order to samplez i from Q φ (Z|x i ) we just need to return µ φ (x i ).
...
In order to compute the values (1) or (2) of OT we need to handle non-trivial constraints, either on the coupling distribution Γ or on the function f being considered.
...
Later BID11 proposed to relax the same constraint by penalizing the objective of (2) with a term λ · E ( ∇f (X) − 1) 2 which should not be greater than 1 if f ∈ F L .

In a more general OT setting of W c BID5 proposed to penalize the objective of (1) with the KLdivergence λ · D KL (Γ, P ⊗ Q) between the coupling distribution and the product of marginals.
...
Experimental setup In all reported experiments we used Euclidian latent spaces Z = R dz for various d z depending on the complexity of the dataset, isotropic Gaussian prior distributions P Z (Z) = N (Z; 0, σ 2 z · I d ) over Z, and a squared cost function c(x, y) = x − y 2 2 for data points x, y ∈ X = R dx .

We used deterministic encoder-decoder pairs, Adam BID15 ) with β 1 = 0.5, β 2 = 0.999, and convolutional deep neural network architectures for encoder mapping µ φ : X → Z and decoder mapping G θ : Z → X similar to the DCGAN ones reported by BID27 with batch normalization BID14 .

We tried various values of λ and noticed that λ = 10 seems to work good across all datasets we considered.
...
We used Gaussian encoders Q(Z|X) = N Z; µ φ (X), Σ(X) with mean µ φ and diagonal covariance Σ. For both MNIST and CelebA we used Bernoulli decoders parametrized by G θ .

Functions µ φ , Σ, and G θ were parametrized by deep nets of the same architectures as used in WAE.
...
If the codesz = µ φ (x) for some of the training points x ∈ X end up far away from the support of P Z (which may happen in the early stages of training) the corresponding terms in the U-statistic k(z,z) = e − z−z 2 2 /σ 2 k will quickly approach zero and provide no gradient for those outliers.

This could be avoided by choosing the kernel bandwidth σ 2 k in a data-dependent manner, however in this case per-minibatch U-statistic would not provide an unbiased estimate for the gradient.
...
In all experiments we used C = 2d z σ 2 z , which is the expected squared distance between two multivariate Gaussian vectors drawn from P Z .

This significantly improved the performance compared to the RBF kernel (even the one with σ Random samples are generated by sampling P Z and decoding the resulting noise vectors z into G θ (z).
...
To see this, notice that during training the decoder function G θ is presented only with encoded versions µ φ (X) of the data points X ∼ P X .
...
In order to quantitatively assess the quality of the generated images, we use the Fréchet Inception Distance introduced by BID12 and report the results on CelebA in Table 1 .
...
We take random points x from the held out test set and report their auto-encoded versions G θ (µ φ (x)).

Next, pairs (x, y) of different data points are sampled randomly from the held out test set and encoded: z x = µ φ (x), z y = µ φ (y).
...
The conditional distribution P G (X|Z) is often parametrized by a deep net G and can have any form as long as its density p G (x|z) can be computed and differentiated with respect to the parameters of G. A typical choice is to use Gaussians P G (X|Z) = N (X; G(Z), σ 2 · I).
...
However, in order to make the D KL term of (6) tractable in closed form, the original implementation of VAE uses a standard normal P Z and restricts Q to a class of Gaussian distributions Q(Z|X) = N Z; µ(X), Σ(X) with mean µ and diagonal covariance Σ parametrized by deep nets.
...
They also report an equally good performance of different types of conditional distributions Q(Z|X), including Gaussians as used in VAEs, implicit models Q e as used in AVB, and deterministic encoder mappings, i.e. Q(Z|X) = δ µ(X) with µ : X → Z.
...
Note that P G defined in (3) and used throughout this work is the marginal distribution of Y when (Y, Z) ∼ P G,Z .In the optimal transport problem (1), we consider joint distributions Γ(X, Y ) which are called couplings between values of X and Y .

Because of the marginal constraint, we can write Γ(X, Y ) = Γ(Y |X)P X (X) and we can consider Γ(Y |X) as a non-deterministic mapping from X to Y .
...
P(P X , P G ) and we immediately get the following upper bound: DISPLAYFORM0 If P G (Y |Z) are Dirac measures (i.e., Y = G(Z)), it turns out that P X,Y = P(P X , P G ):adversary in WAE-GAN to α = 5 × 10 −4 .
...
For WAE-MMD we used λ = 100 and for WAE-GAN λ = 1.

Both used σ 2 z = 2.

For WAE-MMD the learning rate of Adam was initially set to α = 10 −3 .

For WAE-GAN the learning rate of Adam for the encoder-decoder pair was initially set to α = 3 × 10 −4 and for the adversary to 10 −3 .
...
Our VAE model used a cross-entropy reconstruction loss (Bernoulli decoder) and α = 10 −4 as the initial Adam learning rate and the same decay schedule as explained above.
