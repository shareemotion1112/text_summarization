...
Consider an L-hidden-layer fully-connected neural network with hidden layers of width N l (for layer l) and pointwise nonlinearities φ.
...
We will refer to these as the post-and pre-activations. (We let x 0 i ≡ x i for the input, dropping the Arabic numeral superscript, and instead use a Greek superscript x α to denote a particular input α).

Weight and bias parameters for the lth layer have components W l ij , b l i , which are independent and randomly drawn, and we take them all to have zero mean and variances σ 2 w /N l and σ 2 b , respectively.
...
We introduce the shorthand DISPLAYFORM4 to emphasize the recursive relationship between K l and K l−1 via a deterministic function F whose form depends only on the nonlinearity φ.
...
We will assume a noise model consisting of a Gaussian with variance σ 2 centered at z.
...
Populate a matrix F containing a lookup table for the function F φ in Equation 5.
...
Bilinear interpolation has constant cost, so this has cost O L(n 2 train + n train n test ) .This computational recipe allows us to compute the covariance matrix for the NNGP corresponding to any well-behaved nonlinearity φ.
...
In this case the covariance K l (x, x ) → q * for every pair of inputs x, x , where q * is a constant that depends only on σ 2 w and σ 2 b .

All inputs have unit correlation asymptotically with depth.

By contrast in the chaotic phase the weight variance σ 2 w dominates and similar inputs become dissimilar with depth as they are randomly projected by the weight matrices.

In this case, the covariance K l (x, x ) → q * for x = x but q * c * for x = x .

Here c * < 1 is the fixed point correlation.

In each of these regimes, there is also a finite depth-scale ξ which describes the characteristic number of layers over which the covariance function decays exponentially towards its fixed point form.

Exactly at the boundary between these two regimes is a line in (σ FORMULA0 that this approach to the fixed-point covariance fundamentally bounded whether or not neural networks could successfully be trained.
...
In this case for all σ 2 w and σ 2 b , K ∞ (x, x ) = q * for all x, x and every point becomes asymptotically correlated.
...
A DRAWS FROM AN NNGP PRIOR FIG5 illustrates the nature of the GP prior for the ReLU nonlinearity by depicting samples of 1D functions z(x) drawn from a ReLU GP, GP(0, K L ), with fixed depth L = 10 and (σ Figure 6: The angular structure of the kernel and its evolution with depth.
...
Default value for the target noise σ 2 was set to 10 −10 and was increased by factor of 10 when Cholesky decomposition failed while solving Equation 8 and 9.
