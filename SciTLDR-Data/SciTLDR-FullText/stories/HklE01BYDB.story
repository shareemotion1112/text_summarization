...
A fully observable Markov decision process (MDP) is described by tuple S, A, P, R, γ , where S is the state space, A is the action space, P (s t+1 |s t , a t ) is the probability distribution over transitions, R(s t , a t , s t+1 ) is the reward function, and γ is the discount factor (Bellman, 1957 ).
...
The goal of standard RL is to learn a policy π(a t |s t ) that can maximize the agent's expected cumulative reward

, where ρ π is a state-action marginal distribution induced by the policy π(a t |s t ) and transition distribution P (s t+1 |s t , a t ).

An important modification (Ziebart et al., 2008) auguments this objective with an entropy term H(π(·|s t )) to encourage exploration and robustness to noise.

The resulting maximum entropy objective is then defined as:

where α is a temperature parameter that balances between optimizing for the reward and for the stochasticity of the policy.
...
The soft policy improvement step then attempts to learn a parametric policy π(a t |s t ) (actor) by directly minimizing the KL divergence between the policy and a Boltzmann distribution induced by the current soft Q-function, producing the following objective:

The policy π(a t |s t ) is parametrized as a diagonal Gaussian to handle continuous action spaces.
...
Or in the case of β-VAE (Kingma & Welling, 2013; Higgins et al., 2017a) , where the variational distribution is parametrized as diagonal Gaussian, the objective is defined as:

where z t = f enc (o t ) and σ 2 t = f enc std (o t ).
...
We follow Lange & Riedmiller (2010) ; Finn et al. (2015) and in Section 4.3 try an iterative unsupervised pretraining of an autoencoder that reconstructs pixels and is parameterized by β-VAE as per Nair et al. (2018); Higgins et al. (2017a) .

Exploring the training procedure used in previous work shows it to be sub-optimal and points towards the need for end-to-end training of the β-VAE with the policy network.

Our investigation in Section 4.4 renders this approach useless due to severe instability in training, especially with larger β values.
...
Higgins et al., 2017a) in the iterative re-training setup, we choose to employ a β-VAE likewise.

We then proceed to first learn a representation space by pretraining the f enc , f enc std , and f dec networks of the β-VAE according to the loss J(VAE) Equation (4) on data collected from a random policy.

We then learn a control policy on top of the frozen latent representations z t = f enc (o t ).

We tune β for best performance, and find large β to be worse, and that very small β ∈ [10 −8 , 10 −6 ] performed best.
...
We emphasize that the gradients are never shared between the β-VAE for learning the representation space, and the actor-critic learning the policy.

These results suggest that if we can combine the representation pretraining via a β-VAE together with the policy learning in a stable end-to-end procedure, we would expect better performance.
...
Our findings and the results from Jaderberg et al. (2017) motivate us to allow gradient propagation to the encoder of the β-VAE from the actor-critic, which in our case is SAC.

We enable end-to-end learning by allowing the encoder to not only update with gradients from the J(VAE) loss (Equation (4), as done in Section 4.3, but also with gradients coming from the J(Q) and J(π) (Equations (1) and (2)) losses specified in Section 3.

Results in Figure 3 show that the end-to-end policy learning together with the β-VAE in unstable in the off-policy setting and prone to divergent behaviours that hurt performance.
...
We notice that electing to learn deterministic latent representations, rather than stochastic as in the β-VAE case, has a stabilizing effect on the end-to-end learning in the off-policy regime.

We thus use a deterministic autoencoder in a form of the regularized autoencoder (RAE) (Ghosh et al., 2019) , that has many structural similarities with β-VAE.
...
In particular, we extend the standard reconstruction loss for a deterministic autoencoder with a L 2 penalty on the learned representation z and add weight decay on the decoder parameters θ dec :

.

We set λ z = 10 −6 and λ θ = 10 −7 .
...
We also provide a comprehensive overview of all the remaining hyper parameters in Temperature Adam's β 1 0.5 Init temperature 0.1 Table 4 : A complete overview of used hyper parameters.
...
Figure 9 provides additional results for the experiment described in Section 4.3. : An unsuccessful attempt to propagate gradients from the actor-critic down to the encoder of the β-VAE to enable end-to-end off-policy training.
