...
Our abstraction network takes in an observed demonstration τ obs and predict a sequence of latent variables {z}. These {z} are each decoded into their corresponding motor programs via the motor program network.
...
Given a set of N unlabelled demonstrations {τ i } i=1,2,...,N that consist of sequences of robot states {s 1 , s 2 , ..., s T } ∈ S, our aim is to learn the shared space of motor programs via the motor program network M. However, as each demonstration τ i is unannotated, we do not apriori know what subsequences of the demonstration correspond to a distinct motor program.
...
Thus in addition to learning the network M, we also learn a mapping A from each demonstrated trajectory τ i to the underlying sequence of motor programs {M 1 , M 2 , ..., M K } (and associated latent variables {z 1 , z 2 , ..., z K }) executed during the span of the trajectory.
...
Learning Overview and Objective: Our approach is outlined in Fig 2, where given an input demonstration trajectory τ obs , we use the abstraction network to predict a sequence of (a variable number of) latent codes {z k }.

These are each decoded into corresponding sub-trajectories via the learned motor program network M.

Given the decoding of the predicted motor programs, we can recompose these sub-trajectories to obtain a recomposed demonstration trajectory τ rec , and penalize the discrepancy between the observed and the recomposed demonstrations.

Denoting by ⊕ the concatenation operator, our loss for a given demonstration τ obs is therefore characterized as:

As the trajectories τ obs , τ rec are possibly of different lengths, we use a pairwise matching cost between the two trajectories, where the optimal alignment is computed via dynamic time warping (Berndt & Clifford, 1994) .
...
Given two trajectories τ a ≡ (

, and a distance metric δ over the state space, the discrepancy measure between trajectories can be defined as the matching cost for the optimal matching path P among all possible valid matching paths P (i.e. paths satisfying monotonicity, continuity, and boundary conditions (Berndt & Clifford, 1994) ):
...
To prevent this, we additionally incorporate a smoothness loss L sm (τ rec ) that penalizes the state change across consecutive time-steps if they are larger than a certain margin.
...
While the loss function above can directly yield gradients to the predicted motor program encoding z k via M, we use gradients using REINFORCE Williams (1992) (with ∆(τ obs , τ rec ) + L sm (τ rec ) as negative reward) to learn prediction of p k .
...
We first randomly sample a set of 500 trajectories unseen during training, then pass these trajectories through our model, and retrieve the predicted latent variables {z} for each of these trajectories and their corresponding movement sequences {τ }.
