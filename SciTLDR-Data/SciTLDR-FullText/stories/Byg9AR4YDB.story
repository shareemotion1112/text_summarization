...
The resulting matrix is flattened and convolved (with a kernel size of 1 on the pixel level) from A i ∈ R (|s|×K,64,64) → R (1,64,64) followed by a sigmoid σ(·) activation.
...
Given segmentation map S, ground truth Y and noise δ, the generated image is X = G(S, δ).
...
where H(·) is entropy, µ X and σ X are the mean and standard deviation of X, c 1 , c 2 are constants.
...
, W }, the COMp = (x,ȳ) is defined asx = (x,y)∈V xXx,y (x,y)∈V Xx,y andȳ = (x,y)∈V yXx,y (x,y)∈V Xx,y .
...
We define θ t as the angle between v t , u t .

If cos θ t > 0, that is to say if the cosine similarity is positive, the COM of a CD8 T cellp CD8 t , moves correctly towards the COM of the added tumor cellp Tumor t .
...
We consider each cell X in polar coordinates (r, θ) with respect to its centroid, integrate its expression along the radius coordinates, and evaluate the resulting histogram hist(X) along the angle coordinate.
