...
By maximizing the similarity function δ(b i , d j ) (detailed in Section 2.1), the optimal binary code b p is obtained.
...
Fix binary codes b i and d j of the user i and the item j, the bag-of-words vector u i of the user i (v j of the item j) is generated via p(θ u ).
...
The prior is modeled as the multivariate Bernoulli distribution on hash codes: p(b i ) ∼ B(ρ u ), and p(d j ) ∼ B(ρ v ), thus the prior probability are as follows:
...
is the regularizer term with parameters Θ and Φ. By training the objective in eq. (10), we obtain binary codes, but some bits probably be correlated.
