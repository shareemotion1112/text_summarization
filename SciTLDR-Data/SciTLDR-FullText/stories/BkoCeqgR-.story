...
Let us denote our data by X, our target space by Y and our network by φ : X → Y .
...
We say that a net is invariant under such a transformation T provided that ψ(x) = ψ(T (x)).Putting invariance in a network is an classical question of neural networks.
...
Contrary to invariance, equivariant networks are aware of the transformation, thus there exists a transformation T on the target space such that ψ(T (x)) = T ψ(x).
...
Formally we can write this as p(x, y, z) = p(σ(x, y, z)).
...
So the baseline net, let us denote it by ψ, takes an image x and outputs its class y = ψ(x, w).
