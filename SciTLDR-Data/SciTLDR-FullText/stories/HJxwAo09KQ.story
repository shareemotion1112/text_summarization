...
The update rule is a parameterized function that defines how to map the weights at iteration t to iteration t + 1: w (t+1) = u(w (t) ; θ).

Here, θ represents the parameters of the learned optimizer.

In the outer loop, these optimizer parameters (θ) are updated so as to minimize some measure of optimizer performance, the outer-objective L(θ).
...
These are updated by the learned optimizer, and depend implicitly on θ and D train .

DISPLAYFORM1 Loss on inner-problem, for minibatch x. θ Parameters of the optimizer.

u (·; θ) Function defining the learned optimizer.

The inner-loop update is DISPLAYFORM2 L train (θ) Outer-level objective targeting training loss, for L will be the average value of the target loss measured over either training or validation data.
...
In order to train an optimizer, we wish to compute derivatives of the outer-objective L with respect to the optimizer parameters, θ.
...
That is, rather than compute the full gradient from iteration t = 0 to t = T , we compute gradients in windows from t = a to t = a + τ .
...
Following the notation in FIG2 , we define the optimizer as DISPLAYFORM0 where θ is a scalar learning rate that we wish to learn for minimizing some target problem (w (t) ).
...
(d) Similar to c, where the inner problem is a MLP, the learned optimizer is the one used this paper, and for a 1D slice through the outer-parameters θ along the gradient direction.

(e) 2D rather than 1D slices through θ, for different numbers of innerloop steps.

Intensity indicates value of L train (θ).

Similar pathologies are observed to those which manifest in the toy problem.

The quantity we are interested in is the outer-gradient, the derivative of the loss after T steps of gradient descent with respect to θ.
...
Consider the task of optimizing a loss surface with two local minima defined as (w) = (w − 4)(w − 3)w 2 with initial condition w (0) = −1.2 using a momentum based optimizer with a parameterized momentum value θ ( FIG3 .
...
With increasing unrolling steps, the loss surface as a function of the momentum θ becomes less and less smooth, and develops near-discontinuities at some values of the momentum.
...
As more inner-steps are taken, the bias increases, eventually flipping the outer-gradient direction resulting in increasing outer-loss.3d and 3e for slices through the loss landscape L (θ) of the outer-problem for a neural network optimizer.
...
To do this, instead of optimizing L(θ) directly, we instead optimize a smoothed outer-loss L (θ), DISPLAYFORM0 where σ 2 is a fixed variance (set to 0.01) which determines the degree of smoothing.

This is the same approach taken in variational optimization BID32 .

We can construct two different unbiased gradient estimators for L (θ): one via the reparameterization trick BID16 , and one via the log-derivative trick similar to what is done in evolutionary strategies (ES) BID37 .

We denote the two estimates as g rp and g es respectively, Following the insight from BID25 in the context of reinforcement learning 1 , we combine these estimates using inverse variance weighting BID8 , DISPLAYFORM1 DISPLAYFORM2 where σ 2 rp and σ 2 es are the empirical variances of g rp and g es respectively.
...
In theory, the samples used to evaluate σ 2 rp , σ 2 es must be independent of g es and g rp , but in practice we found good performance using the same samples for both.
...
We can update the parameters of the optimizer (θ) by computing the derivative of this final loss with respect to θ: DISPLAYFORM0 dθ .
...
This expression highlights where the exploding outer-gradient comes from: the recursive definition of DISPLAYFORM2 dθ means that computing it will involve a product of the Hessian at every iteration.
...
for Each truncation in the inner loop do Sample ES perturbation: e ∼ N (0, σ 2 I).

Sample a number of steps per truncation, k, based on current outer-training index.

Compute a positive, and negative sequence starting from w (t) by iteratively applying (for k steps), u(·; θ + e), to w DISPLAYFORM0 Compute a pair of outer-objectives L + , and L − using the 2 sequences of w from t to k using either the train or validation inner-problem data.

Compute a single sample of DISPLAYFORM1 θ, σ 2 I)) Store the sample of (g rp , g es ) in a buffer until a batch of samples is ready.
...
update meta-parameters with SGD: θ ← θ − αg combined where α is a learning rate.
...
To emulate learning rate schedules, the current training iteration is fed in transformed via applying a tanh squashing functions at different timescales: tanh(t/η − 1) where η is the timescale.
...
As such, the network produces two outputs, and we combine them in an exponential manner: exp(λ exp o 1 )λ lin o 2 making use of two temperature parameters λ exp and λ lin which are both set to 1e−3.

Without these scaling terms, the default initialization yields steps on the order of size 1 -far above the step size of any known optimizer and result in highly chaotic regions of θ.
...
We optimize the outer-parameters, θ, using Adam (Kingma & Ba, 2014) with a batch size of 128 and with a learning rate of 0.003 for the training outer-objective and 0.0003 for the validation outer-objective, and β 1 = 0.5(following existing literature on non-stationary optimization BID1 ).While both values of learning rate work for both outer-objectives, we find the validation outerobjective to be considerably harder, and training is more stable with the lower learning rate.
