...
The corresponding MDP is characterized as (G, {S i , A i } i∈V , p, r) where S i and A i are the local state space and action space of agent i. Let S := × i∈V S i and A := × i∈V A i be the global state space and action space, MDP transitions follow a stationary probability distribution p : S × A × S → [0, 1], and global step rewards be denoted by r : S × A → R. In a multi-agent MDP, each agent i follows a decentralized policy π i :

S i × A i → [0, 1] to chose its own action a i,t ∼ π i (·|s i,t ) at time t. The MDP objective is to maximize E[R π 0 ], where R π t = T τ =t γ τ −t r τ is the long-term global return with discount factor γ.

Here the expectation is taken over the global policy π : S ×A → [0, 1], the initial distribution s t ∼ ρ, and the transition s τ +1 ∼ p(·|s τ , a τ ), regarding the step reward r τ = r(s τ , a τ ), ∀τ < T , and the terminal reward r T = r T (s T ) 2 .

The same system can be formulated as a centralized MDP.

Defining V π (s) = E[R π t |s t = s] as the state-value function and
...
To see this, note s i,t ∈ S i ⊆ S does not provide sufficient information for π i .

Even assuming

is non-stationary if the behavior policies of other agents π −i := {π j } j∈V\{i} are evolving over time.
...
where 0 ≤ α ≤ 1 is the spatial discount factor, and d ij is distance between agents i and j.
...
each agent performs local greedy control; when α → 1, each agent performs global coordination and

, since the immediate local reward of each agent is only affected by controls within its closed neighborhood.

Now we assume each agent is A2C, with parametric models π θi (s i ) and V ωi (s i , a Ni ) for fitting the optimal policy π * i and value function V πi .

Note ifs i is able to provide global information through cascaded neighborhood communications, both π θi and V ωi are able to fit return R π i,t .

Also, global and future information, such as R π i,τ and a Ni,τ , are always available from each rollout minibatch in offline training.

In contrast, only local informations i,t is allowed in online execution of policy π θi .

Proposition 3.1 (Spatiotemporal RL with A2C).

Let {π θi } i∈V and {V ωi } i∈V be the decentralized actor-critics, and {(s i,τ , m Nii,τ , a i,τ , r i,τ )} i∈V,τ ∈B be the on-policy minibatch from spatiotemporal MDPs under stationary policies {π θi } i∈V .

Then each actor and critic are updated by losses

is the estimated state-value, and β is the coefficient of the entropy loss.
...
where h i,t is the hidden state (or the belief ) of each agent and e λi and g νi are differentiable message encoding and extracting functions 3 .

To avoid dilution of state and policy information (the former is for improving observability while the later is for reducing non-stationarity), state and policy are explicitly included in the message besides agent belief, i.e., m i,t = s i,t ∪ π i,t−1 ∪ h i,t−1 , or s i,t := s Vi,t ∪π Ni,t−1 ∪h Ni,t−1 as in Eq. (5).

Note the communication phase is prior-decision, so only h i,t−1 and π i,t−1 are available.
...
The communication attentions can be integrated either at the sender as µ i,t (m i,t ), or at the receiver as µ i,t (m Ni,t ).

Replacing the input (s i,t ) of Eq. (3)(4) with the belief (h i,t ), the actor and critic become π θi (·|h i,t ) and V ωi (h i,t , a Ni,t ), and the frozen estimations are π i,t and v i,t , respectively.
...
To see this mathematically, π θi,t (·|h i,t ) = πθ i,t (·|s Vi,t , π Ni,t−1 ), with

(·|s Vi,t+1 , π Ni,t , {s Nj ,t , π Nj ,t−1 } j∈Ni ), with 3 Additional cell state needs to be maintained if LSTM is used.

In other words, {λ i , ν i } will be updated for improving actors π θj , ∀j ∈ V, as soon as they are included inθ j ; meanwhile, r i will be included in R π j .

In contrast, the policy is fully decentralized in execution, as g νi depends ons i only.
...
So we adopt a simple heuristic optimal velocity model (OVM) (Bando et al., 1995) to perform longitudinal vehicle control under above constraints, whose behavior is affected by hyper-parameters: headway gain α

• , relative velocity gain β

• , stop headway h st = 5m and full-speed headway h go = 35m.

Usually (α • , β • ) represent the human driver behavior, here we train NMARL to recommend appropriate (α • , β • ) for each OVM controller, selected from four levels {(0, 0), (0.5, 0), (0, 0.5), (0.5, 0.5)}. Assuming the target headway and velocity profile are h * = 20m and v * t , respectively, the cost of each agent is
...
In particular, the local message sizes are O(|s i | + |π i | + |h i |) for DIAL and NeurComm, O(|s i | + |h i |) for CommNet, O(|s i | + |π i |) for FPrint, and O(|s i |) for IA2C and ConseNet.

The implementation details are in C.1.

All algorithms use the same DNN hidden layers: one fully-connected layer for message encoding e λ , and one LSTM layer for message extracting g ν .

All hidden layers have 64 units.

The encoding layer implicitly learns normalization across different input signal types.

We train each model over 1M steps, with γ = 0.99, actor learning rate 5 × 10 −4 , and critic learning rate 2.5 × 10 −4 .

Also, each training episode has a different seed for generalization purposes.

In ATSC, β = 0.01, |B| = 120, while in CACC, β = 0.05, |B| = 60, to encourage the exploration of collision-free policies.
...
First, we investigate the impact of spatial discount factor, by comparing the learning curves among α ∈ {0.8, 0.9, 1} for IA2C and CommNet.

Fig. 3 reveals a few interesting facts.

First, α * CommNet is always higher than α * IA2C .

Indeed, α * CommNet = 1 in almost all scenarios (except for ATSC Monaco).
...
Second, learning performance becomes much more sensitive to α when the task is more difficult.

Specifically, all α values lead to similar learning curves in CACC Catch-up, whereas appropriate α values help IA2C converge to much better policies more steadily in other scenarios.

Third, α * is high enough: α * IA2C = 0.9 except for CACC Slow-down where α * IA2C = 0.8.
...
Fig. 4 compares the learning curves of all MARL algorithms, after tuned α * ∈ {0.6, 0.8, 0.9, 0.95, 1}. As expected, α * for non-communicative policies are lower than those for communicative policies.

Tab.

1 summarizes α * of controllers across different NMARL scenarios.

For challenging scenarios like ATSC Monaco, lower α is preferred by almost all policies (except NeurComm).

This demonstrates that α is an effective way to enhance MARL performance in general, especially for challenging tasks like ATSC Monaco.

From another view point, α serves as an informative indicator on problem difficulty and algorithm coordination level.
...
In CACC scenarios, α enhanced FPrint policy achieves the best execution performance.
...
Kaiqing Zhang, Zhuoran Yang, Han Liu, Tong Zhang, and Tamer Başar.
...
, and v τ = V ω − (s τ ), based on on-policy minibatch from a MDP {(s τ , a τ , r τ )} τ ∈B .

Now we consider spatiotemporal MDP, which has transition in Eq. (1), optimizes return in Eq. (2), and collects experience (s i,t , m Nii,t , a i,t ,r i,t ), wherer i,t = j∈V α dij r j,t .

In Theorem 3.1 of Zhang * i (·|s) and ai∈Ai π i (a i |s)Q πi (s, a) under global observations, respectively.

Now assuming the observations and communications are restricted to each neighborhood as in Definition 3.1, then the actor and critic become π θi (s i ) ≈π θi (s) and V ωi (s i , a Ni ) ≈Ṽ ωi (s, a −i ), with the best observability.

Hence, replacing π θ (a|s), V ω (s), r by π θi (a i |s i ), V ωi (s i , a Ni ), andr i , respectively, we establish Eq. (3)(4) from Eq. (6)(7), which concludes the proof.

Note partial observability and non-stationarity are present in π θi (a i |s i ) and V ωi (s i , a Ni ).
...
= s i,t−1:t ∪ {s j,t−1:t , π j,t−2:t−1 } j∈Ni ∪ {s j,t−1 , π j,t−2 } j∈{V|dij =2}

∪ {h j,t−2 } j∈{V|dij ≤2}

⊃ . . .

⊃ s i,0:t ∪ {s j,0:t , π j,t−2:t−1 } j∈Ni ∪ {s j,0:t−1 , π j,0:t−2 } j∈{V|dij =2}

∪ . . .

∪ {s j,0:t+1−dmax , π j,0:t−dmax } j∈{V|dij =dmax} , which concludes the proof.
...
Hence, m i,τ is included in the meta-DNN of agent j at time τ + d ij − 1.

In other words, {ν i , λ i } receive gradients from L(θ j ), L(ω j ), ∀j ∈ {V|j = i}, except for the first d ij − 1 experience samples.

Assuming d max |B|, {ν i , λ i } receive almost all gradients from loss signals of all other agents, which concludes the proof.
...
FPrint: h i,t = LSTM(h i,t−1 , concat(relu(s Vi,t ), relu(π Ni,t−1 ))).

NeurComm: h i,t = LSTM(h i,t−1 , concat(relu(s Vi,t ), relu(π Ni,t−1 ), relu(h Ni,t−1 ))).
...
Also, the actor and critic are π i,t = softmax(h i,t ), and v i,t = linear(concat(h i,t , onehot(a Ni,t ))) C.2 EXPERIMENTS IN ATSC ENVIRONMENT C.2.1 ACTION SPACE Fig. 7 illustrates the action space of five phases for each intersection in the ATSC Grid scenario.
