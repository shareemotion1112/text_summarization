...
In the context of planning on-line with a model, there are a number of approaches that use progressive widening to consider increasing large action spaces over the course of search (Chaslot et al., 2008) , including in planning for continuous action spaces (Couëtoux et al., 2011) .
...
We formalise our problem as a MDP, specified by a tuple < S, A, P, r, γ >.

The set of possible states and actions are given by S and A, P is the transition function that specifies the environment dynamics, and γ is a discount factor used to specify the discounted return R = T t=0 γ t r t for an episode of length T .

We wish our agent to maximise this return in expectation by learning a policy π that maps states to actions.
...
⊂ A N −1 ⊆ A. A policy restricted to actions A is denoted π (a|s).

The optimal policy in this restricted policy class is π * (a|s), and its corresponding action-value and value functions are Q * (s, a) and V * (s) = max a Q * (s, a).
...
We use a simple linear schedule on a mixing parameter α ∈ [0, N ].

Initially α = 0 and we always choose = 0.

Later, we pick = α with probability α − α and = α with probability α − α (e.g. if α = 1.1, we choose = 1 with 90% chance and = 2 with 10% chance).
...
For GAS experiments, we keep the mixing coefficient α = 0 for 25000 environment steps, and then increase it linearly by 1 every 25000 steps until reaching the maximum value.

We use γ = 0.998 for our Acrobot experiments, but reduce it to γ = 0.99 for Mountain Car to prevent diverging Q-values.
