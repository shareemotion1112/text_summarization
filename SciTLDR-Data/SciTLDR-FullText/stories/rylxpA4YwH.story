...
Among these, the most accepted automated visual quality metrics are Inception Score (IS) (Salimans et al., 2016) and Fréchet Inception Distance (FID) (Heusel et al., 2017) .
...
However, it scales poorly with the number of unique conditions, as the computationally intensive FID calculation must be repeated for each case, and because FID behaves poorly when the sample size is small (Bińkowski et al., 2018) .
...
Given two multivariate Gaussian distributions N (µ, Σ) and N (μ,Σ), Fréchet Distance (FD) is defined as:

When evaluating a generative model, N (µ, Σ) represents the data (reference) distribution, obtained by fitting a Gaussian to samples from a reference dataset, and N (μ,Σ) represents the learned (generated) distribution, a result of fitting to samples from a generative model.
...
, and an Inception embedding function f , we estimate the Gaussian parameters µ, Σ,μ andΣ as:
...
To ensure reproducibility, when reporting FJD scores it is important to include details such as which conditioning embedding function was used, which dataset is used for the reference distribution, and the α value.
...
In order to control the relative contribution of the image component and the conditioning component to the final FJD value, we scale the conditioning embedding by a constant α.

In essence, α indicates how much we care about the conditioning component compared to the image component.

When α = 0, the conditioning component is ignored and FJD is equivalent to FID.

As the value of α increases, the perceived importance of the conditioning component is also increased and reflected accordingly in the resulting measure.

To equally weight the image component and the conditioning component, we recommend setting α to be equal to the ratio between the average L 2 norm of the image embedding and the conditioning embedding.
...
We note that α should be calculated on data from the reference distribution (real data distribution), and then applied to all conditioning embeddings thereafter.

See Appendix F for an example of the effect of the α hyperparameter.
...
The generated dataset is simulated by duplicating the reference dataset and adding Gaussian noise drawn from N (0, σ) to the images, where σ ∈ [0, 0.25] and pixel values are normalized (and clipped after noise addition) to the range [0, 1].
...
Results are shown in Figure 2 , where we plot both FID and FJD as a function of the added Gaussian noise (σ is indicated on the x-axis as Noise Magnitude).
...
Values for α reported here are calculated according to the balancing mechanism recommended in Section 4.2.
...
We follow the same experimental procedure as outlined in Section 5.2: Gaussian noise is drawn from N (0, σ) and add to the images, where σ ∈ [0, 0.25] and pixel values are normalized (and clipped after noise addition) to the range [0, 1].
...
The α parameter in the FJD equation acts as a weighting factor indicating the importance of the image component versus the conditional component.

When α = 0, then FJD is equal to FID, since we only care about the image component.

As the value of α increases, the magnitude of the conditional component's contribution to the value of FJD increases as well.

In our experiments, we attempt to find a neutral value for α that will balance the contribution from the conditional component and the image component.

This balancing is done by finding the value of α that would result in equal magnitude between the image and conditioning embeddings (as measured by the average L2 norm of the embedding vectors).

Instead of reporting FJD at a single α, an alternative approach is to calculate and plot FJD for a range of α values, as shown in Figure 9 .

Plotting α versus FJD allows us to observe any change in rank of models as the importance weighting on the conditional component is increased.

Here we use the truncation trick to evaluate BigGAN (Brock et al., 2019) at several different truncation values σ.

The truncation trick is a technique wherein the noise vector used to condition a GAN is scaled by σ in order to trade sample diversity for image quality and conditional consistency, without needing to retrain the model (as shown in Table 7 ).

Table 7 : Comparison of BigGAN model evaluated with different truncation values σ.

FJD is calculated at α = 17.8.

As σ increases, classification accuracy decreases and diversity increases.

Note that FID and FJD are consistent in their choice of the preferred model at σ = 1.0, however, the relative ranking of σ = 0.25 and σ = 2.0 is different between the two metrics.

We find that in several cases, the ranking of models changes when comparing them at α = 0 (equivalent to FID), versus comparing them using FJD at higher α values.

Models with low truncation values σ initially achieve good performance when α is also low.

However, as α increases, these models rapidly drop in rank due to lack of sample diversity, and instead models with higher σ values are favoured.

This is most obvious when comparing σ = 0.25 and σ = 1.75 (blue and yellow lines in Figure 9 ) respectively.
...
In this experiment we add a weighting parameter λ, which weights the importance of the conditional component of the generator loss.

The original formulation of ACGAN is equivalent to always setting λ = 1, but it is unknown whether this is the most suitable setting as it is never formally tested.

To this end, we train models on the MNIST dataset and perform a sweep over the λ parameter in the range [0, 5] , training a single model for each λ value tested.
...
We find that FID is optimized when λ = 0.25 (Figure 10a ).

This produces a model with good image quality, but almost no conditional consistency.

Accuracy is optimized when λ = 5.0 (Figure 10c ), yielding a model with good conditional consistency, but limited image quality.

Finally, FJD is optimized when λ = 1.0 (Figure 10b ), producing a model that demonstrates a balance between image quality and conditional consistency.
