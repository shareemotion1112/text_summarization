...
We denote by f (x, θ) the neural network function associated evaluated on input x, where θ are the parameters.

We will assume a loss function of the form L(y, z) = − log r(y|z), where r is the density function associated with a predictive distribution R. The loss associated with a single training case is then given by L(y, f (x, θ)) ≡ − log r(y|f (x, θ)).
...
An alternative explanation for the empirical success of the natural gradient method is that it is a 2nd-order method, whose update minimizes the following local quadratic approximation to the objective h(θ + δ): DISPLAYFORM0 This is similar to the 2nd-order Taylor series approximation of h(θ + δ), but with the Fisher substituted in for the Hessian.
...
We note that E T [T ] = i ω i T i , where T i are the different values of T , and ω i 0 are normalized weights (with i ω i = 1) that measure their proportions in the training set.
...
For this to yield a well-defined model we require that Ψ has spectral radius < 1.The intuition behind this model structure is clear.
...
Define the following "transformed" versions of F T and Ψ: DISPLAYFORM0 As shown in Section A.1 of the appendix we havê DISPLAYFORM1 where DISPLAYFORM2 (Note that rational functions can be evaluated with matrix arguments in this way, as discussed in Section A.1.)Our goal is to computeF −1 , from which we can recover F −1 via the simple relation DISPLAYFORM3 Unfortunately it doesn't appear to be possible to simplify this formula sufficiently enough to allow for the efficient computation ofF DISPLAYFORM4 −1 whenΨ is a Kronecker product (which it will be when V 0 and V 1 are).

The difficulty is due to both the appearance ofΨ and its transpose (which are not codiagonalizable/commutative in general), and various higher powers ofΨ.To proceed from this point and obtain a formula which can be efficiently evaluated whenΨ is a Kronecker product, we will make one of two simplifying assumptions/approximations, which we call "Option 1" and "Option 2" respectively.

These are explained in the next two subsections.3.5.2 OPTION 1: DISPLAYFORM5 If V 1 (the cross-moment over time) is symmetric, this implies thatΨ DISPLAYFORM6 is also symmetric.

Thus by eqn.

7 we havê DISPLAYFORM7 Let U diag(ψ)U =Ψ be the eigen-decomposition ofΨ. By the above expression forF T we have DISPLAYFORM8 DISPLAYFORM9 We thus havê DISPLAYFORM10 Inverting both sides of this yieldsF DISPLAYFORM11 where we have defined DISPLAYFORM12 This expression can be efficiently evaluated whenΨ is a Kronecker product since the eigendecomposition of a Kronecker product can be easily obtained from the eigendecomposition of the factors.

DISPLAYFORM13 ) and is thus easy to perform.

See Section 3.5.5 for further details.

V 1 is symmetric if and only ifΨ is symmetric.

And as shown in the proof of Proposition 1 (see Appendix A.1)Ψ has the interpretation of being the transition matrix of an LGGM which describes the evolution of "whitened" versions of the w t 's (given byŵ t = V −1/2 0 w t ).
...
If V 1 is not well approximated as symmetric, another option is to approximatê DISPLAYFORM0 , where we definê DISPLAYFORM1 This is essentially equivalent to the assumption that the training sequences are all infinitely long, which may be a reasonable one to make in practice.
...
This approximation can break down if some of the linear components ofŵ t have temporal autocorrelations close to 1 (i.e.[ψ] i ≈ 1 for some i) and T is relatively small.

In such a case we will have that [ψ]T i is large for some i (despite being raised to the T -th power) so thatF (∞) T may essentially "overcount" the amount of temporal correlation that contributes to the sum.

This can be made more concrete by noting that the approximation is essentially equivalent to taking DISPLAYFORM3 We can express the error of this as DISPLAYFORM4 It is easy to see how this expression, when evaluated at x = [ψ] i , might be large when [ψ] i is close to 1, and T is relatively small.

The formulae forF −1 from the previous sections depend on the quantityΨ DISPLAYFORM0 , and so it remains to compute Ψ.

We observe that DISPLAYFORM1 Right-multiplying both sides by V 0 yields Ψ = V 1 V −1 0 .

Thus, given estimates of V 0 and V 1 , we may compute an estimate ofΨ asΨ DISPLAYFORM2 In practice we estimate V 0 and V 1 by forming estimates of their Kronecker factors and taking the product.
...
Note that as long as V 0 is computed as the 2nd-order moment of some empirical data, and V 1 computed as the 2nd-order moment between that same data and a temporally shifted version, the spectral radius ofΨ = V −1/2 0 DISPLAYFORM5 (and similarly Ψ = V 1 V −1 0 ) will indeed be less than or equal to 1, as we prove in Section B.2 of the appendix.
...
All quantities related to the original model have their analogues in the transformed model, which we indicate with the hat symbol·.In the transformed model the 2nd-order moments of theŵ t 's are given bŷ DISPLAYFORM4 We observe thatV 0 = I.Analogously to the original model, the transformed version obeyŝ DISPLAYFORM5 =V 1 (usingV 0 = I).

This can be seen by noting that DISPLAYFORM6 It also remains true that the spectral radius ofΨ is less than 1, which can be seen in at least one of two ways: by noticing that the transformed model is well-defined in the infinite limit if and only if the original one is, or thatΨ DISPLAYFORM7 is a similar matrix to Ψ (in the technical sense) and hence has the same eigenvalues.
...
we can expressF T aŝ DISPLAYFORM8 It is a well-known fact that one can evaluate rational functions, and functions that are the limiting values of sequences of rational functions, with matrix arguments.
...
In what follows all quantities are computed using their defining formulae, starting from the estimated values of A 0 , A 1 , G 0 , and G 1 .First we observe that sinceΨ DISPLAYFORM0 is similar to Ψ (in the technical sense of the word), they share the same eigenvalues.

Thus it suffices bound to the spectral radius of DISPLAYFORM1 Because the eigendecomposition of a Kronecker product is the Kronecker product of the decompositions of the factors we have that ρ( DISPLAYFORM2 , where ρ(X) denotes the spectral radius of a matrix X.Thus it suffices to show that ρ(A 1 A −1 0 ) ≤ 1 and ρ(G 1 G −1 0 ) ≤ 1 for A i and G i as computed by the estimation scheme outlined in Section 3.5.4.
...
The inequality ρ(A 1 A −1 0 ) ≤ 1 now follows from the following lemma.

Lemma 1 Consider a real, symmetric, positive semi-definite block matrix DISPLAYFORM8 If B is invertible then we have ρ(CB −1 ) ≤ 1, and further if the block matrix is positive definite we have ρ(CB −1 ) < 1.Proof Because similarity transformations preserve eigenvalues, the statement is equivalent to DISPLAYFORM9 .
...
However one immediate useful insight we can make before specializing to Option 1 or Option 2 is thatΨ can be written as a Kronecker product as follows: DISPLAYFORM14 DISPLAYFORM15 , is symmetric.

We note that V 1 = A 1 ⊗ G 1 will be symmetric if and only if both A 1 and G 1 are.

Our task is to compute the matrix-vector product DISPLAYFORM16 where U diag(ψ)U is the eigendecomposition ofΨ, and γ(x) is defined as in Section 3.5.3, eqn.

8.

To do this we first multiply the vector by U , then by diag(γ(ψ)), and then finally by U .The eigendecomposition ofΨ can be computed efficiently using its Kronecker product structure.

In particular, we compute the eigendecompositions of each factor as DISPLAYFORM17 from which we can write the eigendecomposition U diag(ψ)U −1 ofΨ aŝ DISPLAYFORM18 In other words, we have U = U A ⊗ U G andψ = vec(ψ Gψ A ).To multiply z by U = U A ⊗ U G we use the identity (C ⊗ B) vec(X) = vec(B XC ) which gives DISPLAYFORM19 Similarly, the multiplication of z by U = U A ⊗ U G can be computed as vec(U G ZU A ).

where denotes entry-wise multiplication of matrices.

In summary we have thatF −1 z can be computed in matrix form as DISPLAYFORM20 We note that computing γ([ψ G ] i [ψ A ] j ) is trivial since it is just a scalar evaluation of the rational function γ(x).

For Option 2 we must compute the matrix-vector product DISPLAYFORM0 To do this we will first multiply by I −Ψ , then by (I −Ψ Ψ ) −1 , and then by I −Ψ, before finally dividing the result by i ω i T i .To compute the matrix-vector product (I −Ψ )z we use the identity (C ⊗ B) vec(X) = vec(B XC ) while noting thatΨ = (Ψ A ⊗Ψ G ) =Ψ A ⊗Ψ G .

This gives The matrix form of this is simply Z −Ψ G ZΨ A .We may similarly compute (I −Ψ)z in matrix form as Z −Ψ G ZΨ A .The harder task is to compute (I −Ψ Ψ ) −1 z, which is what we tackle next.
...
Using the eigendecomposition the inverse can then be easily computed as DISPLAYFORM3 Thus (I −Ψ Ψ ) −1 z may be computed by first multiplying by (E A ⊗ E G ), then by diag(vec(ll − m G m A )) −1 (which in matrix form corresponds to element-wise division by ll − m G m A ), and then by E A ⊗ E G .
