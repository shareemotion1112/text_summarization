...
, n. We randomly split the dataset into n train triplets for training and n val for choosing the sparsity regularization parameter λ.
...
However, the combination of non-negativity and L1 penalty leads to very similar solutions across random initializations; we discuss this further in Section 3.1.We select the regularization parameter λ out of a grid of candidate values by choosing the one that achieves the lowest cross-entropy CE v on the validation set, defined by DISPLAYFORM4 The dimensionality of the embedding, p, can be determined heuristically from the data.
...
This is also a special case of the quadruplet task, with pairs (c i , c j ) and DISPLAYFORM0 where d i,j = ||x i − x j || 2 and where µ is some constant to be determined empirically.
...
Finally, the model in BID17 could be extended to the three-choice triplet task with sparsity and positivity constraints, but it would require an extra tuning parameter µ, in addition to the L1 sparsity penalty λ which is needed to obtain sparse embeddings.
...
We used Tensorflow BID0 to fit the model (3) to the 1,450,119 triplets collected, using a 90-10 train-validation split to pick the regularization parameter λ.

Searching over parameters {0.0070, 0.0072, . . .

, 0.0100} we identified the parameter λ = 0.008 with the lowest loss on the validation set.
