...
A well-studied algorithm to solve such problems is Stochastic Gradient Descent (SGD), which uses updates of the form: x t+1 := x t − α∇f it (x t ), where α is a step size, andf it is a function chosen randomly from {f 1 , f 2 , . . .

, f k } at time t.

Often in neural networks, "momentum" is added to the SGD update to yield a two-step update process given as: v t+1 = µv t − α∇f it (x t ) followed by x t+1 = x t + v t+1 .

This algorithm is typically called the Heavy-Ball (HB) method (or sometimes classical momentum), with µ > 0 called the momentum parameter (Polyak, 1987) .
...
In the full-batch setting, we demonstrate that ADAM with very high values of the momentum parameter (β 1 = 0.99) matches or outperforms carefully tuned NAG and RMSProp, in terms of getting lower training and test losses.
...
The sign function We define the function sign : DISPLAYFORM0 DISPLAYFORM1 c) σ f < ∞ is an upperbound on the norm of the gradients of f i and (d) f has a minimizer, i.e., there exists x * such that f (x * ) = min x∈R d f (x).
...
Then corresponding to any , ξ > 0 and a starting point x 1 for Algorithm 2, we can define, DISPLAYFORM2 (1−β2)ξ s.t.

we are guaranteed that the iterates of Algorithm 2 using a constant step-length of, α = DISPLAYFORM3 will find an −critical point in at most T steps in the sense that, min t=1,2..
...
Convergence of deterministic RMSProp -the version with standard speeds (Proof in subsection A.2) Let f : R d → R be L−smooth and let σ < ∞ be an upperbound on the norm of the gradient of f .

Assume also that f has a minimizer, i.e., there exists x * such that f (x * ) = min x∈R d f (x).

Then the following holds for Algorithm 2 with a deterministic gradient oracle:For any , ξ > 0, using a constant step length of DISPLAYFORM5 , where x 1 is the first iterate of the algorithm.

One might wonder if the ξ parameter introduced in the algorithms above is necessary to get convergence guarantees for RMSProp.

Towards that in the following theorem we show convergence of another variant of deterministic RMSProp which does not use the ξ parameter and instead uses other assumptions on the objective function and step size modulation.

But these tweaks to eliminate the need of ξ come at the cost of the convergence rates getting weaker.

Theorem 3.3.

Convergence of deterministic RMSProp -the version with no ξ shift (Proof in subsection A.3) Let f : R d → R be L−smooth and let σ < ∞ be an upperbound on the norm of the gradient of f .

Assume also that f has a minimizer, i.e., there exists x * such that f (x * ) = min x∈R d f (x), and the function f be bounded from above and below by constants B and DISPLAYFORM6 the Algorithm 2 with a deterministic gradient oracle and ξ = 0 is guaranteed to reach a t-th iterate s.t.

1 ≤ t ≤ T and ∇f (x t ) ≤ .In Section 5.3 we show results of our experiments with full-batch ADAM.

Towards that, we analyze deterministic ADAM albeit in the small β 1 regime.

We note that a small β 1 does not cut-off contributions to the update direction from gradients in the arbitrarily far past (which are typically significantly large), and neither does it affect the non-triviality of the pre-conditioner which does not depend on β 1 at all.

Theorem 3.4.

Deterministic ADAM converges to criticality (Proof in subsection A.4) Let f : R d → R be L−smooth and let σ < ∞ be an upperbound on the norm of the gradient of f .

Assume also that f has a minimizer, i.e., there exists x * such that f (x * ) = min x∈R d f (x).

Then the following holds for Algorithm 3: DISPLAYFORM7 , there exist step sizes α t > 0, t = 1, 2, . . .

and a natural number T (depending on β 1 , ξ) such that ∇f (x t ) ≤ for some t ≤ T .In particular if one sets β 1 = +2σ , ξ = 2σ, and DISPLAYFORM8 3( +2σ) 2 where g t is the gradient of the objective at the t th iterate, then T can be taken to be DISPLAYFORM9 , where x 2 is the second iterate of the algorithm.
...
It is sometimes believed that ADAM gains over RMSProp because of its "bias correction term" which refers to the step length of ADAM having an iteration dependence of the following form, 1 − β t 2 /(1 − β t 1 ).

In the above theorem, we note that the 1/(1 − β t 1 ) term of this "bias correction term" naturally comes out from theory!
...
Then the outputẑ ∈ R d of the autoencoder is defined asẑ DISPLAYFORM10 This defines an autoencoder with 2 − 1 hidden layers using weight matrices and 2 bias vectors.
...
The loss function, for an input z is then given by: f (z; x) = z −ẑ 2 .Such autoencoders are a fairly standard setup that have been used in previous work (Arpit et al., 2015; Baldi, 2012; Kuchaiev & Ginsburg, 2017; Vincent et al., 2010) .
...
The ξ parameter is a feature of the default implementations of RMSProp and ADAM such as in TensorFlow.
...
We see in FIG1 that on increasing the value of this fixed shift parameter ξ, ADAM in particular, is strongly helped towards getting lower gradient norms and lower test losses though it can hurt its ability to get lower training losses.

The plots are shown for optimally tuned values for the other hyper-parameters.

To check whether NAG, ADAM or RMSProp is capable of consistently moving from a "bad" saddle point to a "good" saddle point region, we track the most negative eigenvalue of the Hessian λ min (Hessian).
...
In Figure 3 , we show how the training loss, test loss and gradient norms vary through the iterations for RMSProp, ADAM (at β 1 = 0.9 and 0.99) and NAG (at µ = 0.9 and 0.99) on a 3 hidden layer autoencoder with 1000 nodes in each hidden layer trained on mini-MNIST.
...
Conclusions from the full-batch experiments of training autoencoders on mini-MNIST:• Pushing β 1 closer to 1 significantly helps ADAM in getting lower training and test losses and at these values of β 1 , it has better performance on these metrics than all the other algorithms.

One sees cases like the one displayed in Figure 3 where ADAM at β 1 = 0.9 was getting comparable or slightly worse test and training errors than NAG.

But once β 1 gets closer to 1, ADAM's performance sharply improves and gets better than other algorithms.• Increasing momentum helps NAG get lower gradient norms though on larger nets it might hurt its training or test performance.
...
However, for larger nets, the full-batch behavior continues, i.e., when ADAM's momentum parameter β 1 is pushed closer to 1, it gets better generalization (significantly lower test losses) than NAG at any momentum tested.• In general, for all metrics (test loss, training loss and gradient norm reduction) both ADAM as well as NAG seem to improve in performance when their momentum parameter (µ for NAG and β 1 for ADAM) is pushed closer to 1.
...
Curiously enough, this regime of β 1 being close to 1 is currently not within the reach of our proof techniques of showing convergence for ADAM.
...
Further our instantiation of the oracle is equivalent to doing the uniformly at random sampling, (g t ) i ∼ {a pi } p=1,...,k .Given that we have, DISPLAYFORM15 i +di where we have defined DISPLAYFORM16 This leads to an explicit form of the needed expectation over the t th −oracle call as, DISPLAYFORM17 Substituting the above (and the definition of the constants a pi ) back into equation 3 we have, DISPLAYFORM18 Substituting this, the above expression can be written as, DISPLAYFORM19 Note that with this substitution, the RHS of the claimed lemma becomes, DISPLAYFORM20 Therefore our claim is proved if we show that for all i, DISPLAYFORM21 To further simplify, we define DISPLAYFORM22 −µ min .

We therefore need to show, DISPLAYFORM23 We first bound d i by recalling the definition of σ f (from which it follows that a 2 pi ≤ σ 2 f ), DISPLAYFORM24 The inequality follows since β 2 ∈ (0, 1]Putting this all together, we get, DISPLAYFORM25 Now our assumption that for all x, sign(∇f p (x)) = sign(∇f q (x)) for all p, q ∈ {1, . . .
...
Substituting this in a lowerbound on the LHS of equation 13 we get, DISPLAYFORM19 Summing the above we get, DISPLAYFORM20 Now we substitute α t = α √ t and invoke the definition of B and B u to write the first term on the RHS of equation 15 as, DISPLAYFORM21 Now we bound the second term in the RHS of equation 15 as follows.

Lets first define a function P (T ) as follows, DISPLAYFORM22 2 and that gives us, DISPLAYFORM23 So substituting the above two bounds back into the RHS of the above inequality 15and removing the factor of 1 − β T 2 < 1 from the numerator, we can define a point x result as follows, DISPLAYFORM24 Thus it follows that for T = O( 1 4 ) the algorithm 2 is guaranteed to have found at least one point DISPLAYFORM25 Proof.
...
Now we define, t := min k=1,..,t,i=1,..,d (g 2 k ) i and this gives us, DISPLAYFORM34 We solve the recursion for m t to get, DISPLAYFORM35 Then by triangle inequality and defining σ t := max i=1,..,t ∇f (x i ) we have, m t ≤ (1 − β t 1 )σ t .

Thus combining this estimate of m t with equation 18 we have, DISPLAYFORM36 m t To analyze this we define the following sequence of functions for each i = 0, 1, 2.., t DISPLAYFORM37 This gives us the following on substituting the update rule for m t , DISPLAYFORM38 Lets define, σ t−1 := max i=1,..
...
Also by definition 0 < β 1 < 1 and hence we have 0 < β 1 − β t 1 < β 1 .
...
This allows us to define a constant, DISPLAYFORM47 Similarly our definition of ξ allows us to define a constant θ 2 > 0 to get, DISPLAYFORM48 Putting the above back into the lowerbound for Q t in equation 22 we have, DISPLAYFORM49 Now we substitute the above and equation 19 into equation 17 to get, DISPLAYFORM50 In the theorem statement we choose to call as the final α t the lowerbound proven above.

We check below that this smaller value of α t still guarantees a decrease in the function value that is sufficient for the statement of the theorem to hold.

A consistency check!

Let us substitute the above final value of the step length DISPLAYFORM51 DISPLAYFORM52 The RHS above can be simplified to be shown to be equal to the RHS in equation 24 at the same values of θ 1 and θ 2 as used above.
...
NAG has two hyper-parameters, the step size α and the momentum µ.

The main hyper-parameters for RMSProp are the step size α, the decay parameter β 2 and the perturbation ξ.

ADAM, in addition to the ones in RMSProp, also has a momentum parameter β 1 .

We vary the step-sizes of ADAM in the conventional way of α t = α 1 − β t 2 /(1 − β t 1 ).
...
The initial set of step sizes used were: {3e−4, 1e−4, 3e−5, 1e−5, 3e−6}. We tune over β 2 ∈ {0.9, 0.99}. We set the perturbation value ξ = 10 −10 , following the default values in TensorFlow, except for the experiments in Section 5.1.

In Section 5.1, we show the effect on convergence and generalization properties of ADAM and RMSProp when changing this parameter ξ.
...
ADAM The initial set of step sizes used were: {3e−4, 1e−4, 3e−5, 1e−5, 3e−6}. For ADAM, we tune over β 1 values of {0.9, 0.99}. For ADAM, We set β 2 = 0.999 for all our experiments as is set as the default in TensorFlow.

Unless otherwise specified we use for the perturbation value ξ = 10 −8 for ADAM, following the default values in TensorFlow.

Contrary to what is the often used values of β 1 for ADAM (usually set to 0.9), we found that we often got better results on the autoencoder problem when setting β 1 = 0.99.

In Figure 5 , we show the same effect of changing ξ as in Section 5.1 on a 1 hidden layer network of 1000 nodes, while keeping all other hyper-parameters fixed (such as learning rate, β 1 , β 2 ).

These other hyper-parameter values were fixed at the best values of these parameters for the default values of ξ, i.e., ξ = 10 −10 for RMSProp and ξ = 10 −8 for ADAM.
...
Our main results extend to this input dimension, where we see ADAM with β 1 = 0.99 both converging the fastest as well as generalizing the best, while NAG does better than ADAM with β 1 = 0.9.
