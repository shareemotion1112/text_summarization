...
The cross entropy loss objective function typically used to train function approximator DISPLAYFORM0 with label vectors y(x i ) has the form: DISPLAYFORM1 For hard decision labels, y(x i ) is an indicator vector (i.e. one-hot encoding), and (2) collapses to argmax Θ n −1 i∈{1,...,n} log g t(xi),Θ (x i ) where t(x i ) is the element in indicator vector y(x i ) that is equal to one for the i th sample x i .

Following the analysis in BID19 , this means the stochastic gradient descent training algorithm minimizing (2) will inherently constrain the weights θ c in the final classification layer(s) of (1) to try to output zeros for elements not corresponding to the correct class.
...
In this study we target the classification layers in g Θ of (1) as the key layers for improvement assuming the supervised feature learning has adequately reached the information bottleneck limit BID21 .
...
, ..., h θ c 1 which generally implement a softmax multinomial logistic regression with a Bayesian maximum a posteriori (MAP) classifier using plug-in class-conditional density estimates.
...
This vulnerability was revealed to be the case in BID1 .We deviate from Papernot's defensive distillation approach here by using class conditional density estimatep Ψ (x|k) on the class-partitioned input data with K total classes to create our labels.
...
We learn the BNP-MFA model with parameters Ψ from the original class-partitioned signals/images as input and then estimate the class-conditional pdf over the input space aŝ DISPLAYFORM0 Then, we assign our label vector as the posterior DISPLAYFORM1 .., n and ∀k = 1, 2, ..., Kwhere p(k) is the class prior.

The term β ki is a combined correction factor and normalization factor to scale the correct class label higher than incorrect classes for the cases where DISPLAYFORM2 and where k * is the correct class index.

The β ki term also enforces that k y ki = 1, ∀i.
...
Once we learn the labels {y i } n i=1 from FORMULA12 , we apply those labels to learn the neural network g Θ from g Θ (x i ) = y i , ∀i = 1, ..., n using traditional backprogation with SGD training on a cross entropy loss function.

After the model converges, we extract features z i = h Θ f (x i ), ∀i = 1, ..., n and learn a new BNP-MFA with model parameters Φ over the feature space.

To learn the feature space class-conditional pdfs we simply swap out x with z and the Ψ with Φ in (6) to obtain the approximate class-conditional likelihood functions over the feature vectors.

The approximate posterior pdf is then simplyp Φ (k|z) ∝p Φ (z|k)p(k).
...
We also experimented with hyperparameter settings for the Dirichlet process concentration parameter η and the ratio of the Beta process parameters a b .

We observed that up to one order of magnitude in change in η and a b there was demonstrable change in the ultimate classification performance.
