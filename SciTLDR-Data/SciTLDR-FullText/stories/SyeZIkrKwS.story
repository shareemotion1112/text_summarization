...
We denote the fixed kernels as w i t , the dynamic kernels as w t , the coefficients as η i t , where t = 0, ..., C out , i = 0, ..., g t .
...
We denote the noises in x (i) as d−1 j=0 α j y j , where α j ∈ R and {y 0 , y 1 , ..., y d−1 } are the base vectors of noise space Ψ. Then the kernels in one convolutional layer can be represented as {w 0 , w 1 , ..., w c }.

The space expanded by {w 0 , w 1 , ..., w c } is Ω. We can prove if the kernels are trained until Ψ ⊂ Ω, then for each w k / ∈ Ψ, we can get the noise-irrelevant f i (x white ) = x

, w k by the cooperation of other kernels w 0 , w 1 , ....

Firstly x (i) can be decomposed as:

where β ∈ R andx ∈ R n is vertical to w k and y j .

For concision we assume the norm of w k and y j is 1.

Then,

When there is no noise, i.e. α j = 0 for j = 0, 1, ..., d − 1, the white output f i (x white ) becomes:

It is proved in the Appendix A.2 that:

where β 0 , ..., β c is determined by the input image.

Eq. 9 is fulfilled by linearly combine convolution output w k , x (i) and w t , x (i) for those β t = 0 in the following layers.
...
In Eq. 9, we can observe that the computation cost can be reduced to one convolution operation by linearly fusing those kernels to a dynamic one: w = (a 00 + β k )w k + t =k,βt =0 β t w t f i (x white ) = w, x (i) .

In Eq. 10, the coefficients β 0 , β 1 , ... is determined by α 0 , α 1 , ..., thus they should be generated based on the input of network.
...
Then, g ij (x) = x (i) , y j = x (i) + βw k +

. . .

We simplify this equation as:

Because w k / ∈ Ψ, we can denote w k as:

where w ⊥ is vertical to y 0 , ..., y d−1 and γ ⊥ = 0.
...
If we denote the elements of the first row of A −1 as a 00 , a 01 , ..., a 0d , then f i (x white ) = β = a 00 f i (x) +

Because Ψ ⊂ Ω, there exists {β t ∈ R|t = 0, 1, ..., c} that

Then, f i (x white ) = a 00 w k + t β t w t , x (i) = (a 00 + β k ) w k , x (i) + t =k
