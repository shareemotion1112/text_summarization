...
In other words, the estimated speech spectrogramŶ t,f is computed by multiplying the estimated maskM t,f on the input spectrogram X t,f as follows:Published as a conference paper at ICLR 2019 DISPLAYFORM0 In this state, the real and imaginary values of the estimated cRM is unbounded.
...
After then MSE between clean source Y and estimated sourceŶ was computed in STFT-domain to train the model.
...
A popular loss function for audio source separation is mean squared error (MSE) between clean source Y and estimated sourceŶ on the STFT-domain.
...
The original loss function loss V en suggested by Venkataramani et al. is formulated upon the observation from Equation 4, where y is the clean source signal andŷ is the estimated source signal.
...
Finally, the loss function is not scale sensitive, meaning that the loss value is the same forŷ and cŷ, where c ∈ R.To resolve these issues, we redesigned the loss function by giving several modifications to Equation 5.
...
DISPLAYFORM1 Expecting to be complementary to source prediction and to propagate errors for noise-only samples, we also added a noise prediction term loss SDR (z,ẑ).
...
The final form of the suggested weighted-SDR loss is as follows: DISPLAYFORM2 where,ẑ = x −ŷ is estimated noise and α = ||y|| 2 /(||y|| 2 + ||z|| 2 ) is the energy ratio between clean speech y and noise z. Note that although weighted SDR loss is a time-domain loss function, it can be backpropagated through our framework.
...
Let x denotes noisy speech with T time step, y denotes target source andŷ denotes estimated source.

Then, loss wSDR (x, y,ŷ) is defined as follows: DISPLAYFORM0 where, α is the energy ratio between target source and noise, i.e., y 2 /( y 2 + x − y 2 ).Proposition 1.

loss wSDR (x, y,ŷ) is bounded on [-1,1] .

Moreover, for fixed y = 0 and x − y = 0, the minimum value -1 can only be attained whenŷ = y, if x = cy for ∀c ∈ R.Proof.
...
When we parameterizeŷ = g θ (x), the loss wSDR (x, y, g θ (x)) has a non-zero gradient with respect to θ even if the target source y is empty.

Proof.

We can calculate partial derivatives as follows: DISPLAYFORM2 Thus, the non-zero gradients with respect to θ can be back-propagated.
