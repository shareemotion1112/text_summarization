...
we need the aid of quality metric of generative models, such as Fréchet inception distance (FID) or sliced Wasserstein distance (SWD) (Borji, 2018) .
...
Let N (0, Σ) signify the center and covariance matrix of the cluster C of interest, where C = {x i |x i ∈ R d , i = 1, . . .

, n}. To make it formal, we can write the equation by The annulus formed by of removing the inner ellipse from the outer one.

where χ To analyze the hardness of training generative models, a fundamental aspect is to examine the number n(E) of given data points falling in a geometric entity E 1 and the number N (E) of lattice points in it.
...
Inspired by the information theory of encoding data of normal distributions (Roman, 1996; Ma et al., 2007) , we count the number of small spheres S ε of radius ε packed in the ellipsoid E instead.

Thus we can use this number to replace the role of N (E) as long as the radius of the sphere S ε is set properly.

With a little abuse of normal d=2 d=20 d=50 d=100 d=500 d=1000 lattice d=2 d=20 d=50 d=100 d=500 d=1000 Figure 4 : Comparison between the number n(A) of data points sampled from the isotropic normal distributions and N (A) of spheres (lattice) packed in the annulus A with respect to the Chi quantile χ α2 .

d is the dimension of data points.

For each dimension, we sample 70,000 data points from N (0, I).

The scales of y-axis and x-axis are normalized by 10,000 and χ α1 , respectively.
...
For a set C = {x i |x i ∈ R d } of n data points drawn from normal distribution N (0, Σ), the ellipsoid E α of confidence 1 − α is defined as x Σ −1 x ≤ χ 2 α , where Σ has no zero eigenvalues and α ∈ [0, 1].

Let N (E α ) be the number of spheres of radius ε packed in the ellipsoid E α .

Then we can establish

We can see that N (E α ) admits a tidy form with Mahalanobis distance χ α , dimension d, and sphere radius ε as variables.

The proof is provided in Appendix.

The geometric region of interest for cluster curriculum is the annulus A formed by removing the ellipsoid 2 E α2 from the ellipsoid E α1 , as Figure 3 (b) displays.

We investigate the varying law between n(A) and N (A) in the annulus A when the inner ellipse E α2 grows with cluster curriculum.

For this purpose, we need the following two corollaries that immediately follows from Theorem 1.

Corollary 1.

Let N (A) be the number of spheres of radius ε packed in the annulus A that is formed by removing the ellipsoid E α1 from the ellipsoid E α1 , where α 1 ≤ α 2 .

Then we have

It is obvious that N (A) goes infinite when d → ∞ under the conditions that χ α1 > χ α2 and ε is bounded.

Besides, when E α2 (cluster) grows, N (A) reduces with exponent d if E α1 is fixed.

In light of Corollary 1, we can now demonstrate the functional law between n(A) and N (A).

First, we determine χ α1 as follows

which means that E α1 is the ellipsoid of minimal Mahalanobis distance to the center that contains all the data points in the cluster.

In addition, we need to estimate a suitable sphere radius ε, such that n(E α1 ) and N (E α1 ) have comparable scales in order to make n(A) and N (A) comparable in scale.

To achieve this, we define an oracle ellipse E where n(E) = N (E).

For simplicity, we let E α1 be the oracle ellipse.

Thus we can determine ε with Corollary 3.

Corollary 3.

If we let E α1 be the oracle ellipse such that n(E α1 ) = N (E α1 ), then the free parameter ε can be computed with ε = χ α1 det(Σ)/n(E α1 )
...
Figure 4 shows that n(A) gradually exhibits the critical phenomena of percolation processes 3 when the dimension d goes large, implying that the data points in the annulus A are significantly reduced when E α2 grows a little bigger near the critical point.

In contrast, the number N (A) of lattice points is still large and varies negligibly until E α2 approaches the boundary.
...
According to convention, we opt the Fréchet inception distance (FID) (Borji, 2018) for ProGAN as the quality metric.
...
We determine the parameter σ of edge weights by enforcing the geometric mean of weights to be 0.8.
...
2 ) if x j is one of the nearest neighbors of x i and 0 otherwise, where d ij is the distance between x i and x j and σ is a free parameter.
...
Then we derive the length of semi-axis with respect to χ α , i.e.

For a d-dimensional ellipsoid E, the volume of E is

where r i the leng of semi-axis of E and Γ(·) is the Gamma function.

Substituting (10) into the above equation, we obtain the final formula of volume

Using the volume formula in (11), it is straightforward to get the volume of the packing sphere S ε

By the definition of N (E α ), we can write
