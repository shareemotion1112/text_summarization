...
For perturbations of the input d (f (x; θ), f (x; θ)) shall be minimized, whereas d is a distance measurement like euclidean distance or Kullback-Leibler divergence andx is the perturbed input.
...
The perturbation r adv of the input x can be computed as DISPLAYFORM0 ,where ξ and are hyperparameters, which have to be tuned for each task.

After the perturbation was added to x consistency regularization is applied.

The distance between the clean (not perturbed) prediction and perturbed prediction d(f (x, θ), f (x + r adv , θ)) shall be minimized.
