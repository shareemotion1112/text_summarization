...
This is well illustrated by considering the case of a simple layered backpropagation network BID18 in which the "net input" of unit i, η i , is a weighted sum of the activations of units in the previous layer, and the activation of unit i is g(η i ), where g(·) is the unit activation function.

In this case, we can define a sensitivity value for each unit, s i , as the derivative of the network output with regard to η i .

Using the chain rule of calculus, it is easy to show that the sensitivity of an output unit is g (ηi), and, for units in earlier layers the gradients are computed as follows: DISPLAYFORM1 where k iterates over all units in the immediately downstream layer from unit i and w ki is the connection weight from unit i to unit k. This calculation may be performed, layer by layer, from outputs to inputs, until s i values for each pixel input unit are available.
...
Formally, we define the parameters of the linear mapping to the four bounding box coordinates as a 4 × M matrix,Ŵ , (where M is the number of pixels in an image) and a 4-dimensional vector of"bias weights",ŵ.

Given a sensitivity map, s, the output is (Ŵ s +ŵ).

Given a training set of N images, the mapping is found by minimizing the following objective function with regard toŴ and w: DISPLAYFORM0 where s i is the sensitivity map for the i th image, and B i,j is the j th coordinate of the bounding box for the i th image.
...
where β p is the predicted bounding box and β gt is the ground truth bounding box BID27 .
...
However, this process is highly depends on the smoothing σ parameter.

The obtained results from different σ values are reported in TAB3 .
