...
(In fact, they proved that, when Φ is symmetric and has negative eigenvalues, GD for linear ResNets with zero-initialization does not converge.)
...
We show that if the neural network width satisfies m " r Ω`krκ 2 log 2 p1{ q¨n 2 {B 2˘, with constant probability, SGD can converge to the global minimum up to an -error within r O`κ 2 ´1 logp1{ q¨n{B˘iterations, where n is the training sample size and B is the minibatch size of stochastic gradient.
...
We use N pµ, σ 2 q to denote the Gaussian distribution with mean µ and variance σ 2 .
...
In addition, we denote by σ min pXq, σ max pXq and σ r pXq the smallest, largest and r-th largest singular values of X respectively.

For a square matrix A, we denote by λ min pAq and λ max pAq the smallest and largest eigenvalues of A respectively.

For two sequences ta k u kě0 and tb k u kě0 , we say a k " Opb k q if a k ď C 1 b k for some absolute constant C 1 , and use a k " Ωpb k q if a k ě C 2 b k for some absolute constant C 2 .

Except the target error , we use r Op¨q and r Ωp¨q to hide the logarithmic factors in Op¨q and Ωp¨q respectively.

Model.

In this work, we consider deep linear ResNets defined as follows:

where x P R d is the input, f W pxq P R k is the corresponding output, τ ą 0 is a scaling parameter, A P R mˆd , B P R kˆm denote the weight matrices of input and output layers respectively, and W 1 , . . .

, W L P R mˆm denote the weight matrices of all hidden layers.
...
, x n q P R dˆn be the input data matrix and Y " py 1 , . . .

, y n q P R kˆn be the corresponding output label matrix.
...
Then it can be easily observed that the condition in Theorem 3.1 becomes LpW p0q q´LpW˚q ď C´2.
...
In Algorithm 1, if each entry in A is independently generated from N p0, 1{mq and each entry in B is independently generated from N p0, 1{kq, then if m ě C¨pd`k`logp1{δqq for some absolute constant C, with probability at least 1´δ, it holds that
...
Let τ " 1{L, suppose }Y} F " Op}X} F q. Then using Gaussian random input and output transformations in Proposition 3.3, if the neural network width satisfies m " r Ωpkr¨κ 2 q, with high probability, the output of GD in Algorithm 1 achieves training loss at most LpW˚q` within T " r O`κ¨logp1{ q˘iterations, where κ " }X} 2 2 {σ 2 r pXq denotes the condition number of the covariance matrix of training data.

Remark 3.5.

For standard deep linear networks, Du & Hu (2019) proved that GD with Gaussian random initialization can converge to a -suboptimal global minima within T " Opκ¨logp1{ qq iterations if the neural network width satisfies m " OpLkr¨κ 3 q. In stark contrast, training deep linear ResNets achieves the same convergence rate as training deep linear networks and linear regression, while the condition on the neural network width is strictly milder than that for training standard deep linear networks by a factor of OpLκq.
...
Let τ " 1{L. There are absolute constants C, C 1 and C 2 , such for any 0 ă δ ď 1{6 and ą 0, if the input and output weight matrices satisfy
...
Let τ " 1{L, suppose }Y} F " Op}X} F q. Then using Gaussian random input and output transformations in Proposition 3.3, for sufficiently small ą 0, if the neural network width satisfies m " r Ω`krκ 2 log 2 p1{ q¨n 2 {B 2˘, with constant probability, SGD in Algorithm 1 can find a point that achieves training loss at most LpW˚q` within T " r O`κ 2 ´1 logp1{ q¨n{B˘iterations.

From Corollaries 3.7 and 3.4, we can see that compared with the convergence guarantee of GD, the condition on the neural network width for SGD is worse by a factor of r Opn 2 log 2 p1{ q{B 2 q and the iteration complexity is higher by a factor of r Opκ ´1¨n {Bq.
...
We further consider the special case that LpW˚q " 0, which implies that there exists a ground truth matrix Φ such that for each training data point px i , y i q we have y i " Φx i .
...
Let τ " 1{L. There are absolute constants C, and C 1 such that for any 0 ă δ ă 1, if the input and output weight matrices satisfy

LpW p0q q, 2 One can boost this probability to 1´δ by independently running logp1{δq copies of SGD in Algorithm 1.
...
Let τ " 1{L, suppose }Y} F " Op}X} F q.
...
We first consider the transformations that A " rI dˆd , 0 dˆpm´dq s J and B " a m{k¨rI kˆk , 0 kˆpm´kq s. which is equivalent to the setting in Bartlett et al. (2019) when m " k " d. Then it is clear that σ min pBq " σ max pBq " a m{k and σ min pAq " σ max pAq " 1.
...
where the second inequality is due to the fact that LpW˚q ď }Y} 2 F {2.

Then it is clear if }X} F ě ?
...
Assuming m ě d`k, we can construct two sets S 1 , S 2 Ă rms satisfying

Then we construct matrices A and B as follows:

where α is a parameter which will be specified later.

In this way, it can be verified that BA " 0, σ min pAq " σ max pAq " 1, and σ min pBq " σ max pBq " α.
...
The R.H.S. of the above inequality does not depend on α, which implies that we can choose sufficiently large α to make this inequality hold.
...
Specifically, we randomly generate X P R 10ˆ1000 from a standard normal distribution and set Y "´X`0.1¨E, where each entry in E is independently generated from standard normal distribution.
...
This is consistent with the theory as our condition on the neural network width is m " Opkrκ 2 q (please refer to Corollary 3.4), which has no dependency in L, while the condition for training standard linear network is m " OpLkrκ 3 q (Du & Hu, 2019), which is linear in L.
...
Let τ " 1{L, then for any weight matrices satisfying max lPrLs }W l } 2 ď 0.5, it holds that,

In addition, , the stochastic gradient G l in Algorithm 1 satisfies

where B is the minibatch size.

The gradient lower bound can be also interpreted as the Polyak-Łojasiewicz condition, which is essential to the linear convergence rate.

The gradient upper bound is crucial to bound the trajectory length, since this lemma requires that max lPrLs }W l } ď 0.5.
...
Let τ " 1{L. Then for any two collections of weight matrices, denoted by

Based on these two lemmas, we are able to complete the proof of all theorems, which are provided as follows.

Proof of Theorem 3.1.

In order to simplify the proof, we use the short-hand notations λ A , µ A , λ B and µ B to denote }A} 2 , σ min pAq, }B} 2 and σ min pBq respectively.
...
Now, assume that the inductive hypothesis holds for s ă t.

We first prove that max lPrLs }W ptq l } F ď 0.5.
...
Under the condition that µ

{2 {σ 2 r pXq, it can be readily verified that }W ptq l } F ď 0.5.

Since this holds for all l P rLs, we have proved Part (i) of the inductive step, i.e., max lPrLs }W ptq l } F ď 0.5.

Induction for Part (ii): Now we prove Part (ii) of the inductive step, bounding the improvement in the objective function.

Note that we have already shown that W ptq satisfies max lPrLs }W ptq l } F ď 0.5, thus by Lemma A.2 we have

where we use the fact that W ptq l´W pt´1q l "´η∇ W l LpW pl´1q q. Note that LpW pt´1q q ď LpW p0q q and the step size is set to be
...
Bounds on the singular values: Specifically, we set the neural network width as m ě 100¨`amaxtd, ku`a2 logp12{δq˘2

By Corollary 5.35 in Vershynin (2010), we know that for a matrix U P R d1ˆd2 (d 1 ě d 2 ) with entries independently generated by standard normal distribution, with probability at least 1´2 expp´t 2 {2q, its singular values satisfy

Based on our constructions of A and B, we know that each entry of ? kB and ?

mA follows standard Gaussian distribution.

Therefore, set t " 2 a logp12{δq and apply union bound, with probability at least 1´δ{3, the following holds,

where we use the facts that σ min pαUq " ασ min pUq and σ max pαUq " ασ max pUq for any scalar α and matrix U. Then applying our choice of m, we have with probability at least 1´δ{3, 0.9 ď σ min pAq ď σ max pAq ď 1.1 and 0.9 a m{k ď σ min pBq ď σ max pBq ď 1.1 a m{k.
...
Note that }X} F ď ? r}X} 2 , thus the condition (A.3) can be satisfied if m " Opkrκ 2 q, where κ " }X} 2 2 {σ 2 r pXq.

In addition, based on the results in Proposition 3.3, it can be computed that η " O`kL{pm}X} 2 2 q˘. Then in order to achieve -suboptimal training loss, the iteration complexity is
...
The guarantee is already achieved by W p0q if ě LpW p0q q´LpW˚q, so we may assume without loss of generality that ă LpW p0q q´LpW˚q.

Similar to the proof of Theorem 3.1, we use the short-hand notations λ A , µ A , λ B and µ B to denote }A} 2 , σ min pAq, }B} 2 and σ min pBq respectively.

Then we rewrite the condition on A and B, and our choices of η and T as follows
...
Define E t to be the event that there is no s ď t such that LpW ptq q´LpW˚q ď 1 .

If 1pE t q " 0, then there is an iterate W s with s ď t that achieves training loss within 1 of optimal.
...
Recalling that ă LpW p0q q´LpW˚q, it is easy to verify that the inductive hypothesis holds when s " 0.

For the inductive step, we will prove that if the inductive hypothesis holds for s ă t, then it holds for s " t.

When 1pE t´1 q " 0, then 1pE t q is also 0 and we are done.

Therefore, the remaining part is to prove the inductive hypothesis for s " t under the assumption that 1pE t´1 q " 1, which implies that (i), (ii) and (iii) hold for all s ď t´1.
...
Besides, combining Part (i) for s " t´1 and our choice of η and T implies that max lPrLs }W pt´1q l } F ď 0.5.
...
By Part (iii) for s " t´1, we know that LpW pt´1q q ď 2LpW p0q q. Then by Part (i) for s " t´1, it is evident that
...
Thus, as mentioned earlier, the inductive hypothesis implies that max lPrLs }W pt´1q l } F ď 0.5.

By Part (i) for s " t, which has been verified in (A.5), it can be proved that max lPrLs }W ptq l } F ď 0.5, then we have the following by Lemma A.2,
...
Recalling that η ď L{p6eλ Therefore, setting the step size as

we further have (A.14) where the second inequality is by (A.13) and the last inequality is by the fact that we assume 1pE t´1 q " 1, which implies that LpW pt´1q q´LpW˚q ě 1 ě 4γ 1 η{γ 0 .
...
Plugging the definition of γ 0 , we are able to complete the proof of the inductive step of Part (ii).

Recalling that for this part, we are going to prove that either LpW ptq q ď 2LpW p0q q or 1pE t q " 0, which is equivalent to LpW ptq q¨1pE t q ď 2LpW p0q q since LpW p0q q and LpW ptq q are both positive.

We will prove this by martingale inequality.

Let F t " σtW p0q ,¨¨¨, W ptq u be a σ-algebra, and F " tF t u tě1 be a filtration.

We first prove that ErLpW ptq q 1pE t q|F t´1 s ď LpW pt´1q q 1pE t´1 q. Apparently, this inequality holds when 1pE t´1 q " 0 since both sides will be zero.

Then if 1pE t´1 q " 1, by (A.14) we have ErLpW ptq q|W pt´1q s ď LpW pt´1q q since LpW˚q is the global minimum.

Therefore, ErLpW ptq q 1pE t q|F t´1 , 1pE t´1 q " 1s ď ErLpW ptq q|F t´1 , 1pE t´1 q " 1s ď LpW pt´1q q.

Combining these two cases, by Jensen's inequality, we further have E " log`LpW ptq q 1pE t q˘|F t´1 ‰ ď log`ErLpW ptq q 1pE t q|F t´1 sď log`LpW pt´1q q 1pE t´1 q˘, which implies that tlog`LpW ptq q¨1pE t q˘u tě0 is a super-martingale.
...
By Part (i) for s " t´1, Lemma A.1, we further have (A.15) where the second inequality follows from the choice of η that

Using the fact that 1pE t q ď 1 and 1pE t´1 q " 1, we further have

which also holds for the case 1pE t´1 q " 0.

Recall that tlog`LpW ptq q¨1pE t q˘u tě0 is a supermartingale, thus by one-side Azuma's inequality, we have with probability at least 1´δ 1 , log`LpW ptq q¨1pE t q˘ď log`LpW p0q q˘`3 eηnλ

BL¨a 2t logp1{δ 1 q.

Setting δ 1 " δ{T , using the fact that t ď T and leveraging our choice of T and η, we have with probability at least 1´δ{T ,

, which implies that (A.16) This completes the proof of the inductive step of Part (iii).

Note that this result holds with probability at least 1´δ{T .

Thus applying union bound over all iterates tW ptq u t"0,...,T yields that all induction arguments hold for all t ď T with probability at least 1´δ.

Moreover, plugging our choice of T and η into Part (ii) gives

By Markov's inequality, we further have with probability at least 2{3, it holds that rLpW pT q qĹ pW˚qs¨1pE t q ď 3 1 " .

Therefore, by union bound (together with the high probability arguments of (A.16)) and assuming δ ă 1{6, we have with probability at least 2{3´δ ě 1{2, one of the iterates of SGD can achieve training loss within 1 of optimal.
...
where λ A , µ A , λ B and µ B denote }A} 2 , σ min pAq, }B} 2 and σ min pBq respectively.
...
Induction for Part (i) We first prove that max lPrLs }W ptq l } F ď 0.5.

By triangle inequality and the update rule of SGD, we have

where the second inequality is by Lemma A.1, the third inequality follows from Part (ii) for all s ă t and the fact that p1´xq 1{2 ď 1´x{2 for all x P r0, 1s.

Then applying our choice of m implies that }W ptq l } F ď 0.5.
...
It can be simply verfied that where the second inequality is by logp1`xq ď x. Then similar to the proof of Theorem 3.6, we are going to apply martingale inequality to prove this part.

Let F t " σtW p0q ,¨¨¨, W ptq u be a σ-algebra, and F " tF t u tě1 be a filtration, the above inequality implies that where the second inequality is by (A.18), the third inequality follows from the fact that´at`b ?

t ď b 2 {a, and the last inequality is by our choice of η that

Then it is clear that with probability at least 1´δ 1 , (A.19) which completes the induction for Part (ii).

Similar to the proof of Theorem 3.6, (A.19) holds with probability at least 1´δ 1 for a given t. Then we can set δ 1 " δ{T and apply union bound such that with probability at least 1´δ, (A.19) holds for all t ď T .

This completes the proof. (1994) ).

Let U, V P R dˆd be two positive definite matrices, then it holds that λ min pUqTrpVq ď TrpUVq ď λ max pUqTrpVq.

The following Lemma is proved in Section B.3.

Lemma B.3.

Let U P R dˆr be a rank-r matrix.

Then for any V P R rˆk , it holds that
...
, by Lemma B.1 and the definition of LpW˚q, we know that there exist a matrix Φ P R kˆd such that

Therefore, based on the assumption that max lPrLs }W l } F ď 0.5, we have

pW´LpW˚q˘. Note that we set τ " 1{L. Then using the inequality p1`0.5{Lq 2L´2 ď p1`0.5{Lq 2L ď e, we are able to complete the proof of gradient upper bound.

Proof of the upper bound of }∇ W l pW; x i , y i q} where the second inequality is by our choice τ " 1{L, and the last inequality is by the fact that p1`0.5{Lq 2L´2 ď e.

Proof of the upper bound of stochastic gradient: Define by B the set of training data points used to compute the stochastic gradient, then define byX andȲ the stacking of tx i u iPB and ty i u iPB respectively.

Let U " BpI`τ W L q¨¨¨pI`τ W 1 qA, the minibatch stochastic gradient takes form where the second inequality is by the assumptions that τ " 1{L and max lPrLs }W l } F ď 0.5, and the last inequality follows from the the fact that p1`0.5{Lq 2L´2 ď p1`0.5{Lq 2L ď e. Note thatX andȲ are constructed by stacking B columns from X and Y respectively, thus we have }X} We begin by working on the first term.

Let V " pI`τ W L q¨¨¨pI`τ W 1 q and r V " pIτ Ă W L q¨¨¨pI`τ Ă W 1 q, we have r U´U " Bp r V´VqA. Breaking down the effect of transforming V " ś 1 j"L pI`τ W j q into r V " ś 1 j"L pI`τ Ă W j q into the effects of replacing one layer at a time, we get r V´V "

pI`τ Ă W j q¸ff and, for each l, pulling out a common factor of´ś
...
pI`τ W l´1 q¨¨¨pI`τ W s`1 qp Ă W s´Ws qpI`τ Ă W s´1 q¨¨¨pI`τ Ă W 1 q.

Recalling that }W l } F , } Ă W l } F ď 0.5 for all l P rLs, by triangle inequality we have }V 2 } F ď τ 2 p1`0.5τ q

where the third inequality follows from the fact that p1`0.5τ q L " p1`0.5{Lq L ď ?

e and the last inequality is by Jensen's inequality.

Next, we are going to upper bound the second term of (B. 9) where the last inequality is by Jensen's inequality and the fact that τ " 1{L. Plugging (B.7), (B.8) and (B.9) into (B.5), we have
