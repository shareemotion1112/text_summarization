...
Consider a neural network f θ (x) = Softmax(z(x)) that classifies images, x, into C classes, where the output of f gives the probability of each class.

Let δ be a small perturbation in an l p -ball of radius , that is, δ p < .

Then x = x + δ is an adversarial example for x if arg max i z(x) i = arg max i z(x ) i , i.e. the perturbation changes the prediction.
...
The framework for our robustness metric is very general, requiring only a) a neural network f θ , b) a property function s(x; f, φ), and c) an input model p(x).
...
The property function, s(x; f θ , φ), is a deterministic function of the input x, the trained network f θ , and problem specific parameters φ.

For instance, in the MNIST example, φ = arg max i f θ (x ) i is the true output of the unperturbed input.
...
We will omit the dependency on f θ and φ from here on for notional conciseness, noting that these are assumed to be fixed and known for verification problems.
...
Specifically, we construct a sequence of intermediate targets, DISPLAYFORM0 , to bridge the gap between the input model p(x) and the target π(x).
...
Our approach starts by first drawing N samples, {x DISPLAYFORM2 , from π 0 (·) = p(·), noting that this can be done exactly because the perturbation model is known.
...
Adapt g(x |x) based on MH acceptance rates 14: end while 15: return ({x DISPLAYFORM4 To estimate the other P k , we need to be able to draw samples from π k−1 (·).

For this we note that if {x DISPLAYFORM5 are distributed according to π k−2 (·), then the subset of these samples for which DISPLAYFORM6 Furthermore, setting L k−1 up to ensure this event is not rare means a significant proportion of the samples will satisfy this property.
...
To do this, we first carry out a uniform resampling with replacement from the set of samples satisfying s(x (k−1) n ) ≥ L k to generate a new set of N samples which are distributed according to π k (·), but with a large number of duplicated samples.

Starting with these samples, we then successively apply M Metropolis-Hastings (MH) transitions targeting π k (·) separately to each sample to produce a fresh new set of samples {x DISPLAYFORM7 (see Appendix A for full details).
...
Choosing the ρth quantile of the values of the property results in discarding a fraction (1 − ρ) of the chains at each step of the algorithm.

This allows explicit control of the rarity of the events to keep them at a manageable level.

We note that if all the sample property values are distinct, then this approach gives P k = ρ, ∀k < K. To give intuition to this, we can think about splitting up log(I) into chunks of size log(ρ).

For any value of log(I), there is always a unique pair of values {K, P K } such that log(I) = K log(ρ) + log(P K ), K ≥ 0 and P K < ρ.
...
These formal verification methods do not calculate the value of I, but rather confirm the existence of a counterexample for which s(x) > 0.We ran our approach on all 500 properties, setting ρ = 0.1, N = 10 4 , M = 1000 (the choice of these hyperparameters will be justified in the next subsection), and using a uniform distribution over the input constraints as the perturbation model, along with a uniform random walk proposal.
...
As demonstrated by BID0 , AMLS is unbiased under the assumption that perfect sampling from the targets, {π k } K−1 k=1 , is possible, and that the cumulative distribution function of s(X) is continuous.
...
(b) Mean AMLS estimate relative to naive MC estimate for different ρ holding M = 1000 fixed, for those properties with log 10 I > −6.5 such that they could be estimated accurately.

The bias decreases both as ρ and the rareness of the event decrease.

(c) As per (b) but with varying M and holding ρ = 0.1 fixed.initialization points for each target means that sampling is less than perfect, but improves with larger values of M and N .

The variance, on the other hand, theoretically strictly decreases with larger values of N and ρ BID0 .

In practice, we found that while larger values of M and N were always beneficial, setting ρ too high introduced biases into the estimate, with ρ = 0.1 empirically providing a good trade-off between bias and variance.

Furthermore, this provides faster run times than large values of ρ, noting that the smaller values of ρ lead to larger gaps in the levels.

To investigate the effect of the parameters more formally, we further ran AMLS on the SAT properties of COLLISIONDETECTION, varying ρ ∈ {0.1, 0.25, 0.5}, N ∈ {10 3 , 10 4 , 10 5 } and M ∈ {100, 250, 1000}, again comparing to the naive MC estimate for 10 10 samples.

We found that the value of N did not make a discernible difference in this range regardless of the values for ρ and M , and thus all presented results correspond to setting N = 10 4 .

As shown in FIG0 , we found that the setting of ρ made a noticeable difference to the estimates for the relatively rarer events.

All the same, these differences were small relative to the differences between properties.

As shown in FIG0 , the value of M made little difference when ρ = 0.1,.

Interesting though, we found that the value of M was important for different values of ρ, as shown in Appendix C.1, with larger values of M giving better results as expected.
...
After training the classifiers, multilevel splitting was run on ten samples from the test set at multiple values of , with N = 10000 and ρ = 0.1, and M ∈ {100, 250, 1000} for MNIST and M ∈ {100, 250, 500, 1000, 2000} for CIFAR-10.
...
Estimates for I on adversarial properties of a single datapoint with ρ = 0.1, and N ∈ {10000, 10000, 300} for MNIST/CIFAR-10/CIFAR-100 respectively.
...
Figure 4: Mean AMLS estimate relative to naive (unbiased) MC estimate for different M = holding ρ fixed to 0.25 (left) and 0.5 (right), for those properties whose naive MC estimate was greater than log 10 I = −6.5 such that they could be estimated accurately.
