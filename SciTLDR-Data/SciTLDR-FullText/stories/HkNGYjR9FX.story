...
During the binarization process, each element of the full-precision weight matrix W ∈ R d I ×d J with entries w i,j is binarized by w i,j = α i,j w B i,j , where α i,j ≥ 0, i ∈ {1, . . .

, d I }, j ∈ {1, . . .

, d J } and w B i,j ∈ {−1, +1}. In BinaryConnect ), the binarized weight element w B i,j is obtained by the sign function while using a fixed scalling factor α for all the elements: w B i,j = α × sign(w i,j ).
...
Ternary weight networks (TWNs) were then proposed to learn the factor α by minimizing the L2 distance between the full-precision and ternary weights for each layer.
...
introduced the trained ternary quantization (TTQ) method that uses two assymetric scaling parameters (α 1 for positive values and α 2 for negative values) to ternarize the weights.
...
Recently, BID13 introduced a new method that builds the full-precision weight matrix W as k multiple binary weight matrices: BID45 also uses a binary search tree to efficiently derive the binary codes β k i,j , improving the prediction accuracy.
...
The logistic sigmoid function and Hadamard product are denoted as σ and , respectively.
...
The mean and standard deviation of the normalized vector are determined by model parameters φ and γ.
...
The main goal of our method is to represent each element of the full-precision weight W either as DISPLAYFORM1 , where α is a fixed scaling factor for all the weights and initialized from BID11 .

To this end, we first divide each weight by the factor α to normalize the weights.
...
Moreover, batch normalization regulates the scale of binarized/ternarized weights using its parameter φ in addition to α.
