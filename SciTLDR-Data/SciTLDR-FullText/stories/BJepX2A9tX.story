...
Its only difference from standard convolution is that the filter being convolved is rotated by rπ/2, r ∈ {0, 1, 2, 3}, depending upon the corresponding quadrant of the domain.

We show that for quadrant convolution, rotations of π/2 in the input are straightforwardly associated with rotations in the output feature map, which is a special form of equivariance called same-equivariance (as coined by BID8 ).For convenience, we represent feature maps, of dimension K, f : Z 2 → R K , and filters, φ : Z 2 → R K , of a network as functions over 2D space, as in BID6 .

Relevant to our formulation is the set, or group, G of two-dimensional rotation matrices of π/2, which can be easily parameterized by g(r), and which acts on points in Z 2 by matrix multiplication, i.e, for a given point DISPLAYFORM0 Let T g denote the transformation of a function by a rotation in G, where DISPLAYFORM1 ) applies the inverse of g to an element of the domain of f .

For an operation Φ : F → F, F being the set of K-dimensional functions f , to exhibit same-equivariance, applying rotation either before or after the operation yields the same result, i.e. DISPLAYFORM2 We now define quadrant convolution.
...
Quadrant convolution can be interpreted as weighting the convolution for each rotation with a function ω : Z 2 → [0, 1] that simply "selects" the appropriate quadrant of the domain, which we define as DISPLAYFORM4 Since the origin does not strictly belong to a particular quadrant, it is handled simply by averaging the response of the filter at all four rotations.
...
For example, if ω is simply the constant 1/4, we have the simple example of equivariance mentioned above, equivalent to averaging the filter responses.

The above formulation can be generalized to conic convolution in which the rotation angle is decreased by an arbitrary factor of π/2R, for some positive integer R, instead of being fixed to π/2.
...
The weighting function is changed to have value one only over this conic region: DISPLAYFORM1 of which ω 1 = ω q is a special case.
...
In particular, the filters will have to be interpolated for rotations that are not a multiple of π/2.
...
This defect notwithstanding, it can be shown that conic convolution maintains equivariance to rotations of π/2, and as our experiments show in the following section, the approximation of finer angles of rotation can still improve performance.
...
We denote the feature map generated by the penultimate convolutional layer by f ∈ R M ×M ×K , where M ∈ Z >1 .In a fully-convolutional network, the final convolutional layer is in reality just a fully-connected layer, in which the input f is passed through N fully-connected filters, φ (n) ∈ R M ×M ×K , n ∈ {0, 1, . . .

, N − 1}. The operation of this layer can be interpreted as the inner product of the function and filter, φ (n) , f .

If we again consider rotations of the filter from the group G R , DISPLAYFORM0 this is equivalent to the first layer of a G-CNN, mapping from the spatial domain to G R (though this group does not include the translation group since the convolution is only applied at the origin), and rotations of the final convolutional layer f will correspond to permutations of G R , which are just circular shifts in of the second dimension of the matrix Ψ.The magnitude response of the 2D-DFT can be applied to Φ to transform these circular shifts to an invariant space, DISPLAYFORM1 This process of encoding rotation invariance corresponds to the 'Convolutional-to-Full Transition' in FIG1 .
...
This representation Ψ is then passed through the 2D-DFT, as in Eqn.
...
From these properties, it is easy to show that such an operation Φ is same-equivariant for rotation.
...
, K} is described by the parameters Λ of the GMM: DISPLAYFORM0 , where µ g ∈ [−1, 1] 2 and the number of Gaussians per class is a parameter G of the data set.
...
To generate a sample image from the generating distribution, first, a constant background intensity is set for the image according to b ∼ Exp(0, λ B ), so I(p) = b, ∀p ∈ R 2 .

Then a random angle θ ∼ Uniform [0, 2π] is drawn to determine the rotation of the image.

The mean η g ∼ N ( µ g , Σ) for each Gaussian of the class is drawn from an underlying Gaussian with mean µ g , which introduces some small jitter of the relative locations of the Gaussians.

A number n g ∼ N (µ n,g , σ n ) of points {p} in [−1, 1] 2 , which vary for each Gaussian, are drawn from this Gaussian according to p ∼ N R η g , RΣ g R −1 , where the realized mean and covariance have been rotated by θ by the rotation matrix: R = cos(θ) − sin(θ) sin(θ) cos(θ) .For each point p, its corresponding intensity value is drawn according to I(p) ∼ Uniform [

µ I − m I , µ I + m I ], replacing the background value.

Having drawn all of the points, the image is smoothed with a Gaussian kernel with variance σ s to emulate the point-spread function of the imager and pixel noise is added: I(p) = I(p) + Exp(0, λ I ).
