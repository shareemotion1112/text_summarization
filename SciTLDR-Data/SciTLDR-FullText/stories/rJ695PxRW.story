...
We use reparametrized trick to draw Q(z|s; ) from Gaussian N µ Q, (s), 2 Q, (s)I where µ Q, (s) and Q, (s) are learnable functions.

Next, we consider two types of distribution family for P(s 0 |s, z; ✓): Bernoulli and Gaussian.

If s 2 {0, 1} p (i.e., a binary image), we define log P(s 0 |s, z; ) as: DISPLAYFORM1 where is element-wise multiplication and g (s, z) : DISPLAYFORM2 If s 2 R p (i.e., a real-valued feature vector), we choose P(s 0 |s, z; ) to be fixed variance factored DISPLAYFORM3 ⌘ , where µ P, (s, z) : R p+z !
...
Namely, the probability for generating the query sample s q in the chain of class c is DISPLAYFORM1 In a generative viewpoint, the predicted classĉ for s q is determined bŷ c = arg max c P(s q |c).For fair comparisons, we use the same architecture specified in BID24 to extract 1600-dimensional features.
