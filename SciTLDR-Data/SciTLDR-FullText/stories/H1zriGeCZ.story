...
We observe that the classical sample complexity bound of n O(log(s/ε)) due to BID23 can be improved to quadratic in the size of the treeÕ(s 2 /ε · log n) while matching the best known quasipolynomial bound in running time.
...
In addition, we obtain the optimal dependence on the error parameter ε.
...
The best algorithm in terms of running time for learning k-sparse Boolean functions is due to BID8 , and requires time 2 Ω(n/ log n) .
...
In contrast, any computation that does not involve function evaluation is considered less expensive, such as computations that require time Ω(n d ) for "somewhat large" d or are subexponential (we still consider runtimes that are exponential in n to be costly).
...
We write g ≡ ε f and say that f, g are ε- DISPLAYFORM0 Definition 1.

BID29 We say a family of functions ψ 1 , . . .

, ψ N (ψ i maps X to R) is a Random Orthonormal Family with respect to D if DISPLAYFORM1 The expectation is taken with respect to probability distribution D. We say that the family is Kbounded if sup x∈X |ψ i (x)| ≤ K for every i. Henceforth we assume K = 1.An important example of a random orthonormal family is the class of parity functions with respect to the uniform distribution on {−1, 1} n :Definition 2.

A parity function on some subset of variables DISPLAYFORM2 It is easy to see that the set of all 2 n parity functions {χ S }, one for each S ⊆
...
Let {χ S } be the parity basis, and let C be a class of functions mapping {−1, 1} n to R. Thus for f ∈ C, f = Sf (S)χ S .
...
BID26 Let f be such that DISPLAYFORM6 The function g is constructed by taking all coefficients of magnitude ε/s or larger in f 's expansion as a polynomial.
...
The usual method for recovering the signal proceeds by solving a convex optimization problem consisting of 1 minimization as follows (for some parameter λ > 0): DISPLAYFORM0 (1)The above formulation comes in many equivalent forms (e.g., Lasso), where one of the objective parts may appear as a hard constraint.
...
For Kbounded random orthonormal family F = {ψ 1 , . . .

, ψ N }, and m independent draws z 1 , . . . , z DISPLAYFORM2 Rauhut gives the following result for recovering sparse vectors x: Theorem 5 (Sparse Recovery for Random Orthonormal Families, BID29 Theorem 4.4).

Given as input matrix A ∈ R m×N and vector y with y i = Ax + e i for some vector e with e 2 ≤ η √ m, mathematical program (1) finds a vector x * such that for constants c 1 and c 2 , DISPLAYFORM3 + c 2 η with probability 1 − δ as long as for sufficiently large constant C, DISPLAYFORM4 The term σ s (x) 1 is equal to min{ x − z 1 , z is s sparse}. Recent work BID5 BID13 has improved the dependence on the polylog factors in the lower bound for m.
...
Algorithm 1 Harmonica-1 1: Input: oracle for f , number of samples T , sparsity s, degree d, parameter λ.

2: Invoke PSR(f, T, s, d, λ) (Procedure 2) to obtain (g, J), where g is a function defined on variables specified by index set J ⊆ [n].

3: Set the variables in [n] \ J to arbitrary values, compute a minimizer x ∈ arg min g(x).

4: return x Theorem 6 (Noiseless recovery).

Let {ψ S } be a 1-bounded orthonormal polynomial basis for distribution D. Let f : R n → R be a (0, d, s)-bounded function as per definition 3 with respect to the basis ψ S .

Then Algorithm 1, in time n O(d) and sample complexity T =Õ(s · d log n), returns x such that x ∈ arg min f (x).This theorem, and indeed most of the results in this paper, follows from the main recovery properties of Procedure 2.
...
Let {ψ S } be a 1-bounded orthonormal polynomial basis for distribution DISPLAYFORM0 DISPLAYFORM1 4: Let S 1 , ..., S s be the indices of the largest coefficients of α.
...
Then C is learnable with respect to the uniform distribution in time n O(log(s/ε)) and sample complexity m =Õ(s 2 /ε · log n).

Further, if the labels are corrupted by arbitrary noise vector v such that v 2 ≤ √ γm, then the output classifier will have squared-error at most ε + O(γ).See the proof of Corollary 8 in Section A.3.Comparison with the "Low-Degree" Algorithm.
...
For learning noiseless, Boolean decision trees, the low-degree algorithm uses quasipolynomial time and sample complexityÕ(s 2 /ε 2 ·log n) to learn to accuracy ε.
...
Here q is the number of stages for which we apply the PSR subroutine, and the restriction size t serves as a tie-breaking rule for the best minimizers (which can be set to 1).Algorithm 3 Harmonica-q 1: Input: oracle for f , number of samples T , sparsity s, degree d, regularization parameter λ, number of stages q, restriction size t, base hyperparameter optimizer ALG.

2: for stage i = 1 to q do 3:Invoke PSR(f, T, s, d, λ) (Procedure 2) to obtain (g i , J i ), where g i is a function defined on variables specified by index set J i ⊆
...
See TAB3 in Appendix for stable range for regularization term λ and the number of samples.
...
Let X be an m dimensional random vector, with expected value µ = E[X], and covariance matrix DISPLAYFORM0 If V is a positive definite matrix, for any real number δ > 0: DISPLAYFORM1 For ease of notation we assume K = 1.

Let f be an (ε/4, s, d)-bounded function written in the orthonormal basis as Sf (S)ψ S .

We can equivalently write f as f = h + g, where h is a degree d polynomial that only includes coefficients of magnitude at least ε/4s and the constant term of the polynomial expansion of f .Since L 1 (f ) = S |f S | ≤ s, by Fact 4 we have that h is 4s 2 /ε + 1 sparse.

The function g is thus the sum of the remainingf (S)ψ S terms not included in h.

Draw m (to be chosen later) random labeled examples {(z 1 , y 1 ), . . .

, (z m , y m )} and enumerate all N = n d basis functions ψ S for |S| ≤ d as {ψ 1 , . . .

, ψ N }.

Form matrix A such that A ij = ψ j (z i ) and consider the problem of recovering 4s 2 /ε + 1 sparse x given Ax + e = y where x is the vector of coefficients of h, the ith entry of y equals y i , and e i = g(z i ).We will prove that with constant probability over the choice m random examples, e 2 ≤ √ εm.

Applying Theorem 5 by setting η = √ ε and observing that σ 4s 2 /ε+1 (x) 1 = 0, we will recover x such that x − x 2 2 ≤ c 2 2 ε for some constant c 2 .

As such, for the functionf DISPLAYFORM2 ε by Parseval's identity.

Note, however, that we may rescale ε by constant factor 1/(2c 2 2 ) to obtain error ε/2 and only incur an additional constant (multiplicative) factor in the sample complexity bound.

By the definition of g, we have DISPLAYFORM3 where eachf (R) is of magnitude at most ε/4s.
...
Here we can apply linearity of variance (the covariance of ψ i and ψ j is zero for all i = j) and calculate the variance DISPLAYFORM5 With the same calculation as (3), we know Var(g(z i )) is at most ε/2.

Now consider the covariance matrix V of the vector e which equals E[ee ] (recall every entry of e has mean 0).

Then V is a diagonal matrix (covariance between two independent samples is zero), and every diagonal entry is at most ε/2.

Applying Fact 10 we have DISPLAYFORM6 Hence with probability at least 1/2, we have that e 2 ≤ √ εm.

From Theorem 5, we may choose m =Õ(s 2 /ε · log n d ).
...
There are at most N = n d polynomials ψ S with |S| ≤ d. Let the enumeration of these polynomials be ψ 1 , . . .

, ψ N .

Draw m labeled examples {(z 1 , y 1 ), . . . , (z m , y m )} independently from D and construct an m × N matrix A with A ij = ψ j (z i ).

Since f can be written as an s sparse linear combination of ψ 1 , . . .

, ψ N , there exists an s-sparse vector x such that Ax = y where the ith entry of y is y i .

Hence we can apply Theorem 5 to recover x exactly.

These are the s non-zero coefficients of f 's expansion in terms of {ψ S }.

Since f is recovered exactly, its minimizer is found in the optimization step.

A.3 PROOF OF COROLLARY 8As mentioned earlier, the orthonormal polynomial basis for the class of Boolean functions with respect to the uniform distribution on {−1, 1} n is the class of parity functions {χ S } for S ⊆ {−1, 1} n .

Further, it is easy to show that for Boolean function DISPLAYFORM0 The corollary now follows by applying Lemma 7 and two known structural facts about decision trees: 1) a tree of size s is (ε, log(s/ε))-concentrated and has L 1 norm bounded by s (see e.g., BID26 ) and 2) by Fact 4, for any function f with L 1 norm bounded by s (i.e., a decision tree of size s), there exists an s 2 /ε sparse function g such that E[(f − g) 2 ] ≤ ε.
...
If at every stage of Harmonica, the target function can be approximated by an s sparse function, we only needÕ(qs log s) samples where q is the number of stages.

For real world applications such as deep neural network hyperparameter tuning, it seems (empirically) reasonable to assume that the hidden function is indeed sparse at every stage (see Section 5).For Hyperband BID22 , SH or Random Search, even if the function is s-sparse, in order to cover the optimal configuration by random sampling, we need Ω(2 s ) samples.
...
Let h(x) = s 1,1 (x) + s 2,c1,1(x) (x)+s 3,c1,1(x) * 32+c 2,c 1,1 (x) (x) (x)+ξ, where ξ is the noise uniformly sampled from [−A, A] (A is the noise level).
