...
Given the agent's current state s, the policy π(a|s) : S → A selects an action a for the agent to take.
...
γ t R(s t , a t ) , while satisfying constraints, i.e., making a cumulative discounted cost constraint below a desired threshold h, denoted by

where γ is the discount factor, τ is the trajectory (τ = (s 0 , a 0 , s 1 , · · · )), and τ ∼ π is shorthand for showing that the distribution over the trajectory depends on π : s 0 ∼ µ, a t ∼ π(a t |s t ), s t+1 ∼ T (s t+1 |s t , a t ), where µ is the initial state distribution.
...
where d π is the discounted future state distribution, denoted by d π (s) .

Here Q π R (s, a) is the discounted cumulative reward obtained by the policy π given the initial state s and action a, and V π R (s) is the discounted cumulative reward obtained by the policy π given the initial state s.
...
First, we optimize a reward function by maximizing the reward advantage function A π R (s, a) subject to a Kullback-Leibler (KL) divergence constraint that constraints the intermediate policy π

This update rule with the trust region, denoted by {π : (Schulman et al., 2015a) .

It effectively constraints the policy changes and guarantees reward improvement.

Step.

Second, we project the intermediate policy π k+ 1 2 onto the constraint set by minimizing distance measure D subject to the constraint set:

The projection step says that the constraint-satisfying policy π k+1 is within the neighbourhood of π k+ 1 2 .
...
To give performance guarantees for PCPO with KL divergence projection, we analyze worst-case performance degradation for each policy update when the current policy π k satisfies the constraint.
...
If the current policy π k satis-ment, and upper bound on constraint violation for each policy update are

(1 − γ) 2 , where δ is the step size in the reward improvement step.
...
To give performance guarantees for PCPO with KL divergence projection, we analyze worst-case performance degradation for each policy update when the current policy π k violates the constraint.
...
If the current policy π k violates the constraint, and KL divergence projection is used, then the lower bound on reward improvement, and the upper bound on constraint violation for each policy update are
...
Note that Theorem 3.2 boils down to Theorem 3.1 if the current policy π k satisfies the constraint (b + = 0).
...
However, with a small step size δ, we can approximate the reward function and constraints with a first order expansion, and approximate the KL divergence constraint in reward improvement step, and the KL divergence measure in projection step with a second order expansion.
...
= J C (π k ) − h, and θ is the parameter of the policy.

Step.

First, we linearize the objective function at π k subject to second order approximation of KL divergence constraint in order to obtain the following updates:

Algorithm 1 Projection Based Constrained Policy Optimization (PCPO)

and store trajectories in D Compute g, a, H, and b using D Obtain θ k+1 using update in Eq. (4) Empty D
...
Again, we linearize the cost constraint at π k .
...
However, if the step size δ is small, then the linearization of the constraint set is accurate enough to locally approximate it.
...
Define η as the coefficient for the reward updates in Eq. (4), i.e., η .

=

, where δ is the step size for the reward improvement step, g is the gradient of f, H is the Fisher information matrix, σ max (A) is the largest singular value of matrix A, and a is the gradient of cost constraint function.

Then PCPO with KL divergence projection converges to stationary points with g ∈ −a, and the objective value changes by

, and PCPO with L 2 norm projection converges to stationary points with H −1 g ∈ −a, and if σ max (H) ≤ 1, then the objective value changes by
...
And the objective of L 2 norm projection decreases when η < 2 L , implying that condition number of H is upper bounded:
...
Similarly, Chow et al. (2019) proposed θ-projection.

This projected the policy parameters θ onto the constraint set.
...
where λ k is updated using

Here β is a fixed learning rate.

(3) Fixed-point Policy Optimization (FPO).

A variant of PDO that solves Eq. (6) using a constant λ.
...
The step size δ is set to 10 −4 for all tasks and all tested algorithms.
...
Moreover we observe that (1) CPO has more constraint violation than PCPO, (2) PDO is too conservative in optimizing the reward, and (3) FPO requires a significant effort to select a good vlaue of λ.
...
This phenomena suggests that PDO is not able to adjust the weight λ k quickly enough to meet the constraint threshold, which hinders the efficiency of learning constraint-satisfying policies.

If learning rate β is too big, the agent will be too conservative in improving the reward.
...
However, in practice FPO requires a lot of engineering effort to select a good value of λ.
...
To prove the policy performance bound when the current policy is feasible, we prove KL divergence between π k and π k+1 for KL divergence projection.

We then prove our main theorem for worst-case performance degradation.

Lemma A.1.

If the current policy π k satisfies the constraint, the constraint set is closed and convex, the KL divergence constraint for the first step is E s∼d π k D KL (π

, where δ is the step size in the reward improvement step, and KL divergence projection is used, then we have

Proof.

By the Bregman divergence projection inequality, π k being in the constraint set, and π k+1 being the projection of the π k+ 1 2 onto the constraint set, we have
...
If the current policy π k satisfies the constraint, and KL divergence projection is used, then the lower bound on reward improvement, and the upper bound on constraint violation for each policy update are
...
To prove the policy performance bound when the current policy is infeasible (i.e., constraintviolating), we prove KL divergence between π k and π k+1 for KL divergence projection.

We then prove our main theorem for worst-case performance degradation.

Lemma B.1.

If the current policy π k violates the constraint, the constraint set is closed and convex, the KL divergence constraint for the first step is
...
We define the sublevel set of cost constraint function for the current infeasible policy π k :

This implies that the current policy π k lies in L By Three-point Lemma, for these three polices π k , π k+1 , and π k+1 l , with ϕ(x) .
...
If the constraint violation of the current policy π k is small, i.e., b + is small,

can be approximated by second order expansion.

By the update rule in Eq. (4), we have is the projection of π k+ 1 2 onto the sublevel set of the constraint set.

We want to find the KL divergence between π k and π k+1 .

where

And since δ is small, we have ∇ϕ(π k ) − ∇ϕ(π k+1 l ) ≈ 0 given s. Thus, the third term in Eq. (9) can be eliminated.
...
If the current policy π k violates the constraint, and the KL divergence projection is used, then the lower bound on the reward improvement, and the upper bound on the constraint violation for each policy update are
...
where g, a, θ ∈ R n , b, δ ∈ R, δ > 0, and H, L ∈ R n×n , L = H if using KL divergence projection, and L = I if using L 2 norm projection.
...
Let θ * and λ * denote the solutions to the primal and dual problems, respectively.

In addition, the primal objective function is continuously differentiable.

Hence the Karush-Kuhn-Tucker (KKT) conditions are necessary and sufficient for the optimality of θ * and λ * .
...
for a given θ ∈ C, θ ∈ C be such that θ = θ * , and α ∈ (0, 1).

Then we have

Since the right hand side of Eq. (23) can be made arbitrarily small for a given α, and hence we have:

We show that θ * must be the optimal solution.

Let θ ∈ C and θ = θ * .

Then we have

* is the optimal solution to the optimization problem, and θ * = Proj
...
= 2δ g T H −1 g , where δ is the step size for the reward improvement step, g is the gradient of f, H is the Fisher information matrix, σ max (A) is the largest singular value of matrix A, and a is the gradient of cost constraint function.
...
By Eq. (28), L-smooth continuous function f, and if σ max (H) ≤ 1, we have

To see why we need the assumption of σ max (H) ≤ 1, we define H = U ΣU T as the singular value decomposition of H with u i being the column vector of U .

Then we have

If we want to have

then every singular value σ i (H) of H needs to be smaller than 1, and hence σ max (H) ≤ 1, which justifies the assumption we use to prove the bound.
...
We use GAE-λ approach (Schulman et al., 2015b) Note that we do not use a learned model to predict the probability of entering an undesirable state within a fixed time horizon as CPO did for cost shaping.
...
The step size δ is set to 0.01.
...
However, we conjecture that if using smaller δ, the effect of line search is not significant.
