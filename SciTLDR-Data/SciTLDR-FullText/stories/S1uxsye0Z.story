...
Let us assume a labeled dataset DISPLAYFORM0 DISPLAYFORM1 For an input sample feature vector x ∈ R d , the function before the activation of the j th neuron in the DISPLAYFORM2 where φ : R → R + is the rectified linear activation function (Nair & Hinton, 2010, ReLU) .
...
, L} , given θ, the empirical Rademacher complexity of the loss for the dropout neural network defined above is bounded by DISPLAYFORM3 where k is the number of classes to predict, θ l is the k l -dimensional vector of Bernoulli parameters for the dropout random variables in the l th layer, σ i s are i.i.d.
...
We optimize with the empirical loss Loss(S, f L (·; W, θ)), i.e., without any regularizer for 200 epochs with minibatch fo 100.
...
We have shown that the Rademacher complexity of a neural network is bounded by a function of the dropout rates, i.e., Bernoulli parameters θ.

This makes it possible to unify the dropout rates and the network coefficients W in one objective.

By imposing our upper bound of Rademacher complexity to the loss function as a regularizer, we have DISPLAYFORM0 where the variable λ ∈ R + is a weighting coefficient to trade off the training loss and the generalization capability.

The empirical loss Loss(S, f L (·; W, θ)) and regularizer function Reg(S, W, θ) are defined as DISPLAYFORM1 where W l j is the j th column of W l and θ l is the retain rate vector for the l th layer.

The variable k is the number of classes to predict and X ∈ R n×d is the sample matrix.

In addition to the Rademacher regularizer Reg(S, W, θ), the empirical loss term Loss(S, f L (·; W, θ)) also depends on the dropout Bernoulli parameters θ.

Intuitively, when θ becomes smaller, the loss term Loss(S, f L (·; W, θ)) becomes larger, since the model is less capable to fit the training samples (i.e., less representation power), the empirical Rademacher complexity bound becomes smaller (i.e., more generalizable), and vice versa.
...
DISPLAYFORM2 We now incorporate the Bernoulli parameters θ into the optimization objective as in Eqn.

FORMULA12 , i.e., the objective is a function of both weight coefficient matrices W and retain rate vectors θ.

In particular, the model parameters and the dropout rates are optimized using a block coordinate descent algorithm.

We start with an initial setting of W and θ, and optimize W and θ in an alternating fashion.

For the retain rate probability θ, due to the stochastic nature of dropout framework, it is very expensive to compute the exact objective value.
...
Therefore, in our case, during the optimization of θ, we use the expected value of the Bernoulli dropout variables to rescale the output from each layer, to approximate the true objective f L (x; W, θ).
...
Note that using the expected value of Bernoulli dropout random variables to rescale a layer output is an approximation to the true objective f L (x; W, θ).
...
We update the parameters using mini-batch stochastic gradient descent with Nesterov momentum of 0.95 BID20 .For Rademacher complexity term, we perform a grid search on the regularization weight λ ∈ {0.05, 0.01, 0.005, 0.001, 1e −4 , 1e −5 }, and update the dropout rates after every I ∈ {1, 5, 10, 50, 100} minibatches.
...
For instance, recall the neural network defined in Section 3, the Rademacher complexity regularizer with p = ∞, q = 1 after scaling is DISPLAYFORM0 where W l j is the j th column of the weight coefficient matrix W l and θ l is the retain rate vector for the l th layer.
...
Note with the truncation, the cross entropy loss is still 1-Lipschitz so the empirical Rademacher complexity bound still holds for the truncated lossl(f L (x; W, θ), y).Theorem 6.1.

For the dropout neural network defined in section (3), if truncated cross entropy lossl (21) is used, then ∀δ ≥ 0, with probability at least 1 − 2δ, ∀l • f L : DISPLAYFORM1 Note here the empirical Rademacher complexity for the bounded loss function R S (l • f L ) admits the same bound as the empirical Rademacher complexity for the unbounded cross entropy loss R S (l • f L ).
