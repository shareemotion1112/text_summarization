...
Formally, for an input x = (x 1 , ..., x d ) ∈ R 2d where x i ∈ R 2 , the output of the network is given by: DISPLAYFORM0 where W ∈ R 2k×2 is the weight matrix whose rows are the w (i) vectors followed by the u DISPLAYFORM1 vectors, and σ(x) = max{0, x} is the ReLU activation applied element-wise.
...
We say that a positive point (x, 1) is diverse if for all z ∈ {±1} 2 there exists 1 ≤ i ≤ d such that x i = z. We say that a negative point DISPLAYFORM3 For φ ∈ {−, +} define p φ to be the probability that x is diverse with respect to D φ .
...
DISPLAYFORM7 for γ ≥ 1.

4 We assume that gradient descent runs with a constant learning rate η and the weights are randomly initiliazed with IID Gaussian weights with mean 0 and standard deviation σ g .
...
DISPLAYFORM0 2 the ith convolutional filter at iteration t. Similarly, for DISPLAYFORM1 t ∈ R 2 to be the k + i convolutional filter at iteration t. We assume that each DISPLAYFORM2 0 is initialized as a Gaussian random variable where the entries are IID and distributed as N (0, σ 2 g ).
...
For any , δ and training algorithm let m( , δ) be the sample complexity of a training algorithm, namely, the number of minimal samples the algorithm needs to get at most generalization error with probability at least 1 − δ.
...
One surprising fact of this theorem is that m 1 (0, δ) ≤ 2.
...
In all of the examples we assume that for the distribution D, the probability to sample a positive point is 1 2 and 4 In practice it is common to set γ to 1.

In our analyis we will need γ ≥ 8 to guarantee generalization.

In Section 8.3 we show empirically, that for this task, setting γ to be larger than 1 results in better test performance than setting γ = 1.5 Note that BID6 show that fixing the last layer weights to ±1 does not degrade performance in various tasks.
...
For k ≥ 120, given 2 samples, with probability at least 1 − δ, gradient descent converges to a global minimum with at most test error.

On the other hand, for k = 2 and given number of samples less than 2 log DISPLAYFORM1 , with probability greater than δ, gradient descent converges to a global minimum with error greater than .

See Section 5 for further details.

DISPLAYFORM2 (it is easy to constuct such distributions).

In the first example, we assume that p + = p − = 0.98 and δ = 1 − 0.98 DISPLAYFORM3 05.

In this case we get that for any 0 ≤ < 0.005, m 1 ( , δ) ≤ 2 whereas m 2 ( , δ) ≥ 129.

For the second example consider the case where p + = p − = 0.92.

It follows that for δ = 0.16 and any 0 ≤ < 0.02 it holds that m 1 ( , δ) ≤ 2 and m 2 ( , δ) ≥ 17.

For = 0 and any δ > 0, by setting p + and p − to be sufficiently close to 1, we can get an arbitrarily large gap between m 1 ( , δ) and m 2 ( , δ).
...
Assume that gradient descent runs with parameters η = cη k where c η ≤ DISPLAYFORM0 , k = 2 and γ ≥ 1.

Then the following holds: DISPLAYFORM1 48 , gradient descent converges to a global minimum that has non-zero test error.

Furthermore, for c d ≥ 2c η , there exists at least one pattern which is not detected by the global minimum with confidence c d .2.
...
Formally, for an input x = (x 1 , ..., x d ) ∈ X the output of the network is given by DISPLAYFORM2 where W ∈ R 2k×d1 is the weight matrix which contains in the first k rows the vectors w (i) ∈ R d1 , in the next k rows the vectors u (i) ∈ R d1 and σ(x) = max{0, x} is the ReLU activation applied element-wise.
...
Define the following sums for φ ∈ {+, −}: DISPLAYFORM3 By the conditions of the theorem, with probability at least (p + p − ) m all the points in the training set are diverse.
...
Let Z be a random variable distributed as N (0, σ 2 ).
...
For i ∈ {2, 4} and j ∈ U + 0 (i), it holds that u DISPLAYFORM14 As before, by Lemma 8.2 we have N W2 (x + ) < γ and −N W2 (x − ) < 1.
...
For all t ≥ 0 we have DISPLAYFORM6 0 + α t ηx 2 for α t ∈ Z. This follows since the inequalities u DISPLAYFORM7 .

Assume by contradiction that there exist an iteration t for which u DISPLAYFORM8 0 + α t−1 ηx 2 where α t−1 ∈ Z. 9 Since the coefficient of x i changed in iteration t, we have j ∈ U + t−1 (1) ∪ U + t−1 (3).
...
Let DISPLAYFORM13 and Y DISPLAYFORM14 Then for all t, DISPLAYFORM15 9 Note that in each iteration βt changes by at most η.
...
In Lemma 8.4 we showed that for all t ≥ 0 and j ∈ W DISPLAYFORM19 t · x 2 ≤ η .
...
By Lemma 8.5 it holds that U DISPLAYFORM20 Therefore, by Lemma 8.6 we have u (j) t x 1 < η, from which the claim follows.
...
Therefore, DISPLAYFORM5 In the second case, where we have DISPLAYFORM6 t · x i < η for i ∈ {1, 3}. Note that by Lemma 8.6, any DISPLAYFORM7 .
...
If DISPLAYFORM18 η , where the last inequality follows from the previous observation.

Hence, DISPLAYFORM19 The proof of the second claim follows similarly.

DISPLAYFORM20 The third claim holds by the following identities and bounds DISPLAYFORM21 η by the previous claims.
...
However, by Lemma 8.10, it follows that after 5 consecutive iterations t < t < t + 6 in which DISPLAYFORM4 To see this, first note that for all t, N Wt (x + ) ≤ γ+3c η by Lemma 8.12.
...
Assume by contradiction that GD has not converged to a global minimum after T = 7(γ+1+8cη) DISPLAYFORM6 iterations.
...
Assume that for all 0 ≤ t ≤ T 1 it holds that N Wt (x + ) < γ and −N Wt (x − ) < 1.

By continuing the calculation of Lemma 8.3 we have the following: DISPLAYFORM2 2.

For i ∈ {2, 4} and j ∈ W + 0 (i), it holds that w DISPLAYFORM3 Therefore, there exists an iteration DISPLAYFORM4 It suffices to show that for all T 1 ≤ t < T 2 the following holds: DISPLAYFORM5 The first claim follows since at any iteration N Wt (x + ) can decrease by at most 2ηk = 2c η .

For the second claim, let t < t be the latest iteration such that N W t (x + ) ≥ γ.

Then at iteration t it holds that −N W t (x − ) < 1 and N W t (x + ) ≥ γ.

Therefore, for all i ∈ {1, 3}, l ∈ {2, 4} and DISPLAYFORM6 t + ηx l .
...
We know by Lemma 8.15 that there exists T 2 < T such that S + T2 ≥ γ+1−3c η and that N Wt (x + ) < γ and −N Wt (x − ) ≥ 1 only for t > T 2 .

Since S + t ≤ γ + 1 + 8c η for all t by Lemma 8.12, there can only be at most DISPLAYFORM11 where the second inequality follows by Lemma 8.1 and the third inequality by the assumption on k.
...
Combining this fact with Eq. 19 and Eq. 20 we get DISPLAYFORM3 which implies together with Eq. 18 that X DISPLAYFORM4 where the first inequality is true because DISPLAYFORM5 The second inequality in Eq. 21 follows since P + T ≤ c η and by appyling Lemma 8.16.
...
Let DISPLAYFORM2 Then for all t, there exists X, Y ≥ 0 such that |X| ≤ η U + 0 (2) , |Y | ≤ η U + 0 (4) and DISPLAYFORM3 Proof.
...
In either case, by Eq. 11, we have the following update at iteration t + 1: DISPLAYFORM6 where a ∈ {−1, 0, 1}. Hence, u DISPLAYFORM7 − (a t + a)ηx 2 .
...
We will first prove that for all t there exists an integer a t ≥ 0 such that for DISPLAYFORM14 · x 4 + ηa t .
...
By the induction hypothesis, there exists an integer a t ≥ 0 such that u DISPLAYFORM16 , it follows that if a t ≥ 1 we have the following update at iteration T + 1: DISPLAYFORM17 where a ∈ {−1, 0, 1}. Hence, u DISPLAYFORM18 0 ·x 2 +η(a t +a) and u DISPLAYFORM19 .
...
With probability at least 1 − √ 2k √ πe 8k − 16e −8 Proposition 8.13 and Lemma 8.18 hold.
...
By Lemma 8.20 and Lemma 8.18 DISPLAYFORM27 and by Lemma 8.19 and Lemma 8.18 there exists Y ≤ c η such that: DISPLAYFORM28 Plugging these inequalities in Eq. 24 we get: DISPLAYFORM29 By Lemma 8.16 we have X − T ≤ 34c η .

Hence, by using the inequality S − T ≤ c η we conclude that DISPLAYFORM30 where the last inequality holds for k > 64 We will now prove pattern detection results.

In the case of over-paramterized networks, in Proposition 8.17 we proved that DISPLAYFORM31 1+α(k) .
...
Assume, for example, that arg max 1≤l≤4 u( FORMULA0 t · x l = 3 and arg max l∈{2,4} u(1) t · x l = 2 and assume without loss of generality that N W T · x 1 ≤ 2c η and therefore, x 1 cannot be detected with confidence greater than 2c η .
...
DISPLAYFORM0 We consider running gradient descent with a constant learning rate η ≤ cη k , c η ≤ 1 and IID gaussian initialization with mean 0 and standard deviation σ g = cη 16k 3/2 .
