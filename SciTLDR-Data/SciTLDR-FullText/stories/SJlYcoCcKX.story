...
Back to the date 2006, Buciluǎ et al. first proposed to train a neural network to mimic the output of a complex and large ensemble.
...
Based on the idea of (Buciluǎ,Geoffrey et al.) originally introduced a student-teacher paradigm in transferring the knowledge from a deeper and wider network (teacher) to a shallow network (student).
...
To perform knowledge distillation we train the student network to minimize the following loss function (averaged across all data items): DISPLAYFORM0 where δ(·) is the softmax function, T is a temperature parameter and α is a parameter controlling the ratio of the two terms.
...
At each chosen layer i of the teacher network, we collect the spatial map of the activations for channel j into the vector a DISPLAYFORM0 where β is a hyper-parameter.
...
Let assume the feature points {f i } I i=1 ∈ F are bounded, that is, there exist a distance h such that h = min DISPLAYFORM0 And we also assume the feature points are compact with density ρ.

That is DISPLAYFORM1 whereB(m, r) is a closed ball of radius r and centered at m such that ||f i − f j || ≤ hσ for 1 ≤ i ≤ j ≤ I and σ > 0.
...
The reason we doing this is because the second term on right side of equation FORMULA7 θ(·) is a weight function such that lim x→∞ θ(x) → 0.
...
We formulated m(x) as follow: DISPLAYFORM4 θ(s) is a non-negative weight function (rapidly decreasing as s → 0), and || · || is the Euclidean norm.
...
For KD, we set the temperature for softened softmax to 4 and α = 16, following BID7 .

For AT, the β = 64 and the spatial attention mapping function is defined as sum of absolute values.
...
As for our NMT, we set manifold approximation function's θ(s) as θ(s) = 1 s 2 and λ = 22 to achieve best performance.
