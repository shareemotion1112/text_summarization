...
In the following, we consider a loss : Θ × (X, Y ) → R over a hypothesis space Θ ⊂ R M and example/label spaces X and Y .

We assume that the hypothesis space is closed and bounded w.r.t.

infinity norm with sup θ∈Θ ||θ|| ∞ ≤ B. We further assume that is L-Lipschitz in its first argument w.r.t.

the same norm, i.e., ∀θ,

), U denotes the uniform distribution, and λ > 0 is a scalar.

The algorithm averages 2 the ERM objective of random neighbors of the solutionθ.
...
The proof (Appendix C) uses the compression lemma, θ) , but applied to the variational parameters θ in contrast with Germain et al. (2016) and Sheth and Khardon (2017) that applied it on w.

This new approach is the source of jitter in the randomized ERM objective.

Specifically we apply the compression lemma with q(θ) = q jit (θ|θ ERM ) and

.

This bounds the potential overfitting, expressed by 1 λ f (θ), by a KL term that we can compute explicitly and log E p(θ) e f (θ) which results in Ψ.
...
Corollary 2 If the loss is bounded, i.e., | | ≤ c, then using λ = √ n we have

In the (zero-mean) sparse GP model of Titsias (2009), w represents the latent function at the M inducing inputs

Here, the pseudo-posterior is given by q(w|θ) = N (w|m, C C) and the parameter space Θ includes both the mean and the Cholesky factor of the covariance of the pseudo-posterior, i.e., θ m vec(C)

.

Given q(w|θ), the induced distribution q(f |θ) w p(f |w)q(w)dw can be calculated exactly from Gaussian identities.
...
Assume for now that p ≤ ξ < ∞. We use a smoothing parameter α ∈ (0, 1) and define nlog

is Lipschitz w.r.t.

θ and infinity norm yielding that nlog

and λ min (K U U ) denotes the minimum eigenvalue of K U U .

Therefore, to apply Corollary 2 to any non-conjugate sparse GP model with smoothed log loss, all we need is to (i) verify that ∃ξ s.t.

E q(f * |θ) [p(y * |f * )] < ξ and (ii) calculate bounds on d df * p(y * |f * ) and

is easily achieved when Y is discrete, e.g., for binary classification and count regression.

For standard regression, we can guarantee this by lower bounding the noise variance σ 2 Y and upper bounding the range of X, Y .
...
In this case, given a posterior q(w) and example x with label y true , the Bayesian predictor first identifies the optimal predictionŷ =ŷ q(w) (x) = arg min y∈Y E q(w)p(y |x,w) [ (y, y )] and then suffers the loss (q(w), (x, y true )) = (ŷ q(w) (x), y true ).

Therefore, the natural loss term for optimization is L B = i (ŷ q(w) (x i ), y i ).
...
Our theory does not directly apply to the square loss (ŷ − y) 2 because of the need for smoothing.

However, it is interesting to consider the use of DLM for square loss and the resulting algorithms.

In this case, sGP uses the standard regression model with Gaussian noise for prediction, that is, for calculatingŷ.
...
Let Θ ⊂ R M and || · || denote the infinity norm.

Recall that we assume a bounded loss for the application of the discretization approach, i.e., | | ≤ c. Since Θ is assumed bounded, there exists a finite ρ-cover of Θ,Θ, i.e., ∀θ ∈ Θ, ∃θ ∈Θ s.t.

||θ −θ|| ≤ ρ.

Let

For an arbitrary θ ∈ Θ, letθ denote the closest point inΘ to θ.

Since the loss is assumed to be L-Lipschitz in the hypothesis parameter, we have that

In addition, by combining the union bound and Hoeffding's bound for bounded loss | | ≤ c we have that, with probability ≥ 1 − δ over the choice of sample S, for all θ ∈Θ:

Let θ be any competitor for the posterior parameters.

With probability ≥ 1 − δ we have

≤ E (x,y)∼D (θ, (x, y)) + 2c 2 log(2|Θ|/δ) n + 2Lρ

where (e) follows because ERM minimizes training set loss.

With |Θ| ≤ 2B ρ M , the terms on the RHS of (8) depending on ρ are given by 2c 2 log(2/δ) + 2M log(

The last expression is optimized when ρ = 4cM 3 √ nL

.

Hence, we have that, with probability ≥ 1 − δ over the choice of S, ∀θ ∈ Θ,
...
Meir and Zhang (2003) assume a set of parameterized predictors h(x; w) : X → Y and, in addition, assume that predictions can be averaged so that E q(w|θ) [h(x; w)] is a meaningful prediction.

One can then apply the loss (y, E q(w|θ) [h(x; w)]).

For Bayesian predictors, we average the probabilities inp = E q(w|θ) [p(y|x, w)] but not the predictions themselves.
...
Assuming a family of distributions Q over w and an upper bound p(y|x, w) ≤ p y|w (where p y|w is a constant), uniform convergence for the averaged predictor E q(w) [p(y|x, w) ] under the smoothed log loss nlog (α) () will be shown.

From Theorem 26.5.1 of Shalev-Shwartz and Ben-David (2014) 4 , for all q ∈ Q, the following holds with probability 1 − δ over the choice of S:
...
for Rademacher variables σ, and c = max{| log(α)|,

Next, we slightly adapt the argument outlined in Sections 5 and 6.1 of Meir and Zhang (2003) .

Fix some constant λ ∈ (0, ∞) and sample-independent distribution p(w) over w. By applying the compression lemma (Banerjee, 2006)

where (11) follows from the inequality E σ i exp(σ i a i ) ≤ exp(a 2 i /2) (Lemma A.6 of ShalevShwartz and Ben-David (2014)), and we have defined A sup q∈Q KL(q(w), p(w)).
...
Utilizing (13) in (10), we have that with probability 1 − δ over the choice of S, for all q ∈ Q,

Defining Q A {q ∈ Q s.t.

KL(q, p) ≤ A}, and the ERM hypothesis as

we can use the above with the standard argument for ERM to get that, with probability 1 − δ over the choice of S, for all q ∈ Q A ,
...
Here, we only need L (α) whereas other proofs require a Lipschitz condition w.r.t.
...
LetΘ denote some known subset of Θ, i.e.,Θ ⊂ Θ, and let {q jit (θ|θ)} denote a family of distributions over Θ parameterized by members of the subsetΘ. The members of the family are as yet unspecified, but represent "jitter" distributions which will be defined shortly.
...
holds for allθ ∈Θ: Sinceθ ERM is the ERM hypothesis, it follows that ∀θ ∈Θ,
...
Here, we use it at the level of parameters θ defining the posterior distribution, which requires us to introduce the jitter, whereas previous work applied it at the level of base parameter w.

This gives a qualitatively different result.

Lemma 2.

Let p(θ) be any sample-independent distribution over Θ and define

Then, ∀θ ∈Θ,

Proof First, apply Fubini's theorem to change the order of expectations of

with q(θ) = q jit (θ|θ ERM ) and f (θ) = λ E (x,y)∼D (θ, (x, y)) − 1 n n i=1 (θ, (x i , y i )) to the resulting expression within the expectation w.r.t.
...
Let some norm || · || over Θ be given.

For an L-Lipschitz function (θ) w.r.t.

|| · ||, we have that ∀θ ∈Θ, ∀θ ∈ Θ,

Since this holds for all values of θ (given some θ ), it also holds in expectation over any distribution in θ, specifically q jit (θ|θ).

Lemma 4.

Let p(θ) be any sample-independent distribution over Θ and (θ, (x, y)) be L-Lipschitz in its first argument.

Then, ∀θ ∈ Θ,

where D max θ ∈Θ\Θ minθ ∈Θ ||θ − θ || and Ψ(λ, n) is defined in Lemma 2.

Proof Following from the left inequality of (18) withθ = θ =θ ERM , we have

Utilizing (20) in (17) yields that, ∀θ ∈Θ,

Following from the right inequality of (18), we have that, ∀θ ∈Θ, ∀θ ∈ Θ,

Utilizing (22) in (21) yields that, ∀θ ∈Θ, θ ∈ Θ,

Next, we develop a uniform bound over θ for the term E q jit (θ|θ) ||θ − θ || .

Since (23) holds for allθ ∈Θ, we consider howθ can be selected per θ ∈ Θ. First, note that

Now, when θ ∈Θ, a uniform bound overθ for E q jit (θ|θ) ||θ −θ|| translates to a uniform bound over θ for E q jit (θ|θ) ||θ − θ || since it is possible to selectθ = θ in (24).

When θ ∈ Θ\Θ, the distance to the "closest" point inΘ to θ is minθ ∈Θ ||θ − θ ||.

Then, the second term of (24) is uniformly upper-bounded over θ by D max θ ∈Θ\Θ minθ ∈Θ ||θ − θ ||.

Combining the two cases yields the lemma.

Jitter distributions.

First, we define Θ parametrically as a function of some ρ > 0 and relative toΘ. AssumeΘ is a subset of some space T (equipped with norm || · ||), and define Θ as the set {θ ∈ T s.t.

minθ ∈Θ ||θ −θ|| ≤ ρ}. Then,Θ ⊂ Θ, and D ≤ ρ.

The ρ-ball centered atθ ∈Θ is denoted B ρ (θ) {θ ∈ Θ s.t.

||θ −θ|| ≤ ρ}. Let the jitter distribution q jit (θ|θ) be defined as the following uniform density with support in Θ:

q jit (θ|θ) = .

For this choice of jitter distribution and prior, (19) becomes

If vol(B ρ (θ)) = κρ M for some constants κ, M , then the RHS of (26) be the 1-norm.
