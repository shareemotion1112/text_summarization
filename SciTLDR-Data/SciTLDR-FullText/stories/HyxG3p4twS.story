...
In Tensorflow, this can be implemented as z = tf.stop gradient(ẑ -z) +z.

The weights for individual code-words in the mixture are computed by applying a kernel κ to the distances between the values and the code-words, which can be organized into a distance matrix D:
...
The best combination of kernel parameters (v = 50, γ = 25) is highlighted in red and used in all subsequent experiments.
...
Since standard quality control (e.g., quality level in JPEG, or number of channels in DCN) leads to a wide variety of bpps, we fit individual images to a parametric curve f (x) = (1 + e −αx β +γ ) −1 − δ and show the averaged fits.
...
If needed, channel EC can be easily parallelized, and the ANS codec could be re-implemented to run on GPU as well (Weißenberger & Schmidt, 2019) .
...
where λ c is used to control the balance between the objectives (we consider values from 10 −3 to 1).
...
The figure compares standard JPEG compression (diamond markers), pre-trained basic DCN models (connected circles with black border), and fine-tuned DCN models for various regularization strenghts λ c (loose circles with gray border).
...
Secondly, we observe that fine-tuning the DCN model leads to a sudden increase in payload requirements, minor improvement in quality, and gradual increase in manipulation detection accuracy (as λ c → 0).
...
We can observe that DCN fine-tuning moves the model to a different point (greater payload, better quality), but doesn't seem to visibly deteriorate the rate-distortion trade-off (with the exception of the smallest regularization λ c = 0.001 which consistently shows better accuracy and worse fidelity).
...
The results are shown in Fig. 9 for the pre-trained DCN model (1st column) and fine-tuned models with decreasing λ c (increasing emphasis on accuracy).
...
The pattern seem to have a stable trajectory, despite independent runs of the experiment with different regularization strengths λ c .
...
Interestingly, except for the weakest regularization (λ c = 0.001), all fine-tuned models seem to be equivalent (w.r.t.
...
In some replications it actually managed to improve, e.g., for the shown cases with λ c = 0.005 and 0.001.
