...
α m (x i ) is called mixing coefficient and indicates the probability of component m being responsible for generation of the sample x i .

φ m denotes the probability density function of component m for t i |x i .
...
In equation 2, c is the dimension of the target vector, µ m is the component mean and σ m is the common variance for the elements of the target.
...
In this case, µ m from equation 2 would be used to represent offsets predicted by different components.

Then the density of the ground truth offset vectors G conditioned on image I is given by equation 3, where the density φ m for each component is given by equation 4.
...
σ m (I) is the standard deviation of the component m in two dimensions, X and Y. It is shared by all keypoints of an instance.

However, in order to account for scale differences of keypoints, in equation 4 for each keypoint we divided σ m (I) by its scale factor provided in COCO dataset.
