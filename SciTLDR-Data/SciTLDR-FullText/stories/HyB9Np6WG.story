...
Here ω ijk is the weight assigned to each tensor element X ijk , and we use the weighting proposed byGloVe: ω ijk = min X ijk xmax α , 1 .

We set hyperparameters x max = 10, and α = 0.75 in this work.
...
As for the weighted decomposition, we train for 20 iterations, and its hyperparameters are set as x max = 10, and α = 0.75.We also include two baselines, word2vec's CBOW model and GloVe, for comparison.
...
Hyperparameters in GloVe are set as: window size=6, x max =10, α=0.75 and minimum word count=5.
