...
Let W i k be the layer of learned weights (e.g., affine or convolutional) for task i at shared depth k, with φ k an optional nonlinearity.
...
That is, DISPLAYFORM0 DISPLAYFORM1 where ρ i is a task-specific permutation of size D, and ρ i is fixed before training.
...
In the first experiment, m = 128 and φ = I. Although adding depth does not add expressivity in the single-task linear case, it is useful for examining the effects of permuted ordering, and deep linear networks are known to share properties with nonlinear networks BID41 .

In the second experiment, m = 16 and φ = ReLU.The results are shown in FIG2 .
...
The constraint that all s (i,j,k) sum to 1 for any (i, k) is implemented via softmax, and emphasizes the idea that a soft ordering is what is being learned; in particular, this formulation subsumes any fixed layer ordering ρ i by s (i,ρi(k),k) = 1 ∀ (i, k).
