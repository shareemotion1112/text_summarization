...
Thus, the distribution of the training set can be described with the following joint probability: DISPLAYFORM0 where p(f w (I) | y; θ y ) represents the likelihood of observing the feature vector x as part of class y, and π y is the prior probabilities for class y.

The likelihood is a function with parameters θ y (e.g., parameters of a multivariate Gaussian) that describes the distribution of the feature vectors in the embedding.

Thus, the joint probability of the data p(x, y; w, θ) is a function with parameters composed by the the encoder w parameters, and the Bayesian parameters θ which include the likelihood parameters θ y , and priors π y .

In practice, the likelihood parameters proves most crucial as it is not sensitive to class priors, which can be misleading in the long-tailed setting (as discussed in Section 3.1).Given the above joint probability model, the posterior probability for class y given a feature vector x can be computed using Bayes rule as follows: DISPLAYFORM1 where θ is a concatenation of the likelihood parameters and priors of all the classes.

Thus, the class posterior probability is a function that depends on the encoder parameters w and the Bayesian parameters θ.

The overall objective of this work is to jointly learn the weights w of the feature encoder and the Bayesian parameters θ to guarantee a good classification performance.

Given a training dataset of images and label pairs D = {(x i , y i )}, we propose to learn parameters by maximizing the Bayesian class-posterior probability of the true class labels: DISPLAYFORM2 where MLE is a function that computes the closed form maximum likelihood estimates of the parameters of our Bayesian model, a procedure commonly known as Empirical Bayes BID1 .To use existing solvers for learning deep networks, we reformulate the problem shown in Eq. FORMULA2 as an unconstrained optimization by using a Lagrangian penalty BID2 ) that penalizes solutions which violate the constraint: DISPLAYFORM3 where λ ≥ 0.

In this formulation, the optimization explicitly searches over the feature encoder parameters w and the Bayesian parameters θ so as to maximize class posterior probabilities.

The last term penalizes deviations of the θ parameters from their MLE estimates, effectively acting as a regularizer.
...
This is because its parameters (the centroid µ, covariance matrix Σ, and prior π) have an intuitive meaning and closed-form-maximum-likelihood estimators.
...
The remaining parameters to be estimated are then the class means µ = (µ 1 , . . .
...
This relationship can be obtained by expanding the squared distance terms in the classposterior probability, yielding the following: DISPLAYFORM0 where v j = µ j and DISPLAYFORM1 2 is a common term between the numerator and denominator.
...
To do so, this experiment compared the proposed method with a hyperparemeter λ = 0, leaving only the Bayesian classifier, and the configuration tested in the previous Section.
...
The table shows the average accuracies per class for the proposed method using a regularizer with λ = 0.001 (top row), and without a regularizer (bottom row).
...
The plot shows the accuracy gains obtained by comparing the accuracies of the proposed method using a regularizer with λ = 0.0001 across all classes.
