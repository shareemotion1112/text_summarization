...
We assume the input data points x that belong to the -th cluster obey x − c 2 ≤ ε 0 , with ε 0 > 0 denoting the input noise level.

We assume the labels y i belong to one ofK ≤ K classes.

Specifically, we assume y i ∈ {α 1 , α 2 , . . .

, αK} with {α }K =1 ∈ [−1, 1] denoting the labels associated with each class.
...
Finally, we assume the labels are separated in the sense that α r − α s ≥ δ for r ≠ s,(1.1) with δ > 0 denoting the class separation.
...
Note that the maximum number of classes are dictated by the separation δ.

In particular, we can have at mostK ≤ 2 δ +1 classes.
...
DISPLAYFORM1 be an (ε 0 , δ) clusterable dataset with α 1 , α 2 , . . . , αK denoting theK possible class labels.

DISPLAYFORM2 as follows.

For each cluster 1 ≤ ≤ K, at most ρn of the labels associated with that cluster (which contains n points) is assigned to another label value chosen from {α }K =1 .

We shall refer to the initial labels {ỹ i } n i=1 as the ground truth labels.
...
The labels associated to classes 1, 2, and 3 are α1 = −1, α2 = 0.1, and α3 = 1, respectively so that δ = 0.9.
...
We consider a network with one hidden layer that maps R d to R. Denoting the number of hidden nodes by k, this network is characterized by an activation function φ, input weight matrix W ∈ R k×d and output weight vector v ∈ R k .
...
We will also assume φ has bounded first and second order derivatives, i.e. φ ′ (z) , φ ′′ (z) ≤ Γ for all z. The network's prediction at an input sample x is given by DISPLAYFORM3 where the activation function φ applies entrywise.
...
We will quantify this notion of diversity via a notion of condition number related to a covariance matrix involving the activation φ and the cluster centers {c } K =1 .Definition 2.1 Define the matrix of cluster centers DISPLAYFORM0 Define the neural net covariance matrix Σ(C) as DISPLAYFORM1 Here ⊙ denotes the elementwise product.

Also denote the minimum eigenvalue of Σ(C) by λ(C) and define the condition number associated with the cluster centers C as DISPLAYFORM2 One can view Σ(C) as an empirical kernel matrix associated with the network where the kernel is given by DISPLAYFORM3 Note that Σ(C) is trivially rank deficient if there are two cluster centers that are identical.

In this sense, the minimum eigenvalue of Σ(C) will quantify the ability of the neural network to distinguish between distinct cluster centers.

Therefore, one can think of κ(C) as a condition number associated with the neural network which characterizes the distinctness/diversity of the cluster centers.

The more distinct the cluster centers, the larger λ(C) and smaller the condition number κ(C) is.

Indeed, based on results in (5) when the cluster centers are maximally diverse e.g. uniformly at random from the unit sphere κ(C) scales like a constant.

Throughout we shall assume that λ(C) is strictly positive (and hence κ(C) < ∞).

This property is empirically verified to hold in earlier works (6) when φ is a standard activation (e.g. ReLU, softplus).

As a concrete example, for ReLU activation, using results from (5) one can show if the cluster centers are separated by a distance ν > 0, then λ(C) ≥ ν 100K 2 .

We note that variations of the λ(C) > 0 assumption based on the data points (i.e. λ(X) > 0 not cluster centers) (5; 7; 8) are utilized to provide convergence guarantees for DNNs.
...
Throughout we use c Γ , C Γ , etc.

to denote constants only depending on Γ. We note that this Theorem is slightly simplified by ignoring logarithmic terms and precise dependencies on Γ. See Theorem E.13 for precise statements.

DISPLAYFORM4 with κ(C) the neural net cluster condition number pre Definition 2.1.

Then as long as 0 ≤c Γ K 2 and ρ ≤ δ 8 with probability at least 1 − 3 K 100 , after DISPLAYFORM5 ) iterations, the neural network 3 If k is odd we set one entry to zero ⌊ DISPLAYFORM6 f (⋅, W τ0 ) found by gradient descent assigns all the input samples x i to the correct ground truth labelsỹ i .

That is, arg min DISPLAYFORM7 holds for all 1 ≤ i ≤ n. Furthermore, for all 0 ≤ τ ≤ τ 0 , the distance to the initial point obeys DISPLAYFORM8 Theorem 2.2 shows that gradient descent with early stopping has a few intriguing properties: Robustness.

The solution found by gradient descent with early stopping degrades gracefully as the label corruption level ρ grows.

In particular, as long as ρ ≤ δ 8, the final model is able to correctly classify all samples including the corrupted ones.

In our setup, intuitively label gap obeys δ ∼ 1 K , hence, we prove robustness to Total Number of corrupted labels ≲ n K .This result is independent of number of clusters and only depends on number of classes.
...
Our main insight is that while a neural network maybe expressive enough to fit a corrupted dataset, the model has to travel a longer distance from the point of initialization as a function of the distance from the cluster centers ε 0 and the amount of corruption.
...
Assume φ ′ , φ ′′ ≤ Γ. Let y 1 and y 2 be two scalars satisfying DISPLAYFORM9 ∼ N (0, 1).

Then, with probability at least 1−2e DISPLAYFORM10 holds, we have DISPLAYFORM11 In words, this result shows that in order to fit to a data set with a single corrupted label, a randomly initialized network has to traverse a distance of at least δ ε 0 .
...
For this result we consider a randomized model with ε 2 0 input noise variance.

DISPLAYFORM12 with labels y i = y and {x i } s i=1 with labelsỹ i =ỹ and assume these two labels are δ separated i.e. y −ỹ ≥ δ.

Also suppose s ≤ d and φ DISPLAYFORM13 with probability at least 1 − e −d 2 .Unlike Theorem E.15 this result lower bounds the network norm in lieu of the distance to the initialization W 0 .
...
In contrast, we note that the conclusions of the upper bound in Theorem 2.2 show that to be able to fit to the uncorrupted true labels the distance to initialization grows at most by τ ε 0 after τ iterates.
...
Towards this goal, we consider a general formulation where we aim to fit a general nonlinear model of the form x ↦ f (θ, x) with θ ∈ R p denoting the parameters of the model.

For instance in the case of neural networks θ represents its weights.

Given a data set of n input/label pairs {( DISPLAYFORM0 fit to this data by minimizing a nonlinear least-squares loss of the form DISPLAYFORM1 which can also be written in the more compact form DISPLAYFORM2 To solve this problem we run gradient descent iterations with a constant learning rate η starting from an initial point θ 0 .

These iterations take the form DISPLAYFORM3 Here, J (θ) is the n × p Jacobian matrix associated with the nonlinear mapping f defined via DISPLAYFORM4 Our approach is based on the hypothesis that the nonlinear model has a Jacobian matrix with bimodal spectrum where few singular values are large and remaining singular values are small.
...
Assumption 1 (Bimodal Jacobian) Let β ≥ α ≥ > 0 be scalars.

Let f ∶ R p → R n be a nonlinear mapping and consider a set D ⊂ R p containing the initial point θ 0 (i.e. θ 0 ∈ D).
...
When << α the Jacobian is approximately low-rank.
...
387 388 389 390 391 392 393 394 395 396 397 398 399 400 401 402 403 404 405 406 407 408 409 410 411 412 413 414 415 416 417 418 419 420 421 422 423 424 425 426 427 428 429 430 431 432 433 434 435 436 437 438 439 Assumption 2 (Low-rank Jacobian) Let β ≥ α > 0 be scalars.

Consider a set D ⊂ R p containing the initial point θ 0 (i.e. θ 0 ∈ D).

Let S + ⊂ R n be a subspace and S − be its complement.

For all θ ∈ D, v ∈ S + and w ∈ S − with unit Euclidian norm, we have that DISPLAYFORM7 Our dataset model in Definition 1.2 naturally has a lowrank Jacobian when 0 = 0 and each input example is equal to one of the K cluster centers {c } K =1 .

In this case, the Jacobian will be at most rank K since each row will be in the span of DISPLAYFORM8 .

The subspace S + is dictated by the membership of each cluster as follows: Let Λ ⊂ {1, . . .
...
Define the n-dimensional residual vector r where DISPLAYFORM0 A key idea in our approach is that we argue that (1) in the absence of any corruption r(θ) approximately lies on the subspace S + and (2) if the labels are corrupted by a vector e, then e approximately lies on the complement space.

Before we state our general result we need to discuss another assumption and definition.

The Jacobian mapping J (θ) associated to a nonlinear mapping DISPLAYFORM0 Additionally, to connect our results to the number of corrupted labels, we introduce the notion of subspace diffusedness defined below.
...
around an initial point θ 0 and y = [y 1 . . .

y n ] ∈ R n denoting the corrupted labels.

Also letỹ = [ỹ 1 . .

.ỹ n ] ∈ R n denote the uncorrupted labels and e = y −ỹ the corruption.

Furthermore, suppose the initial residual f (θ 0 ) −ỹ with respect to the uncorrupted labels obey f (θ 0 ) −ỹ ∈ S + .

Then, running gradient descent updates of the from (C.1) with a learning rate DISPLAYFORM3 Furthermore, assume ν > 0 is a precision level obeying ν ≥ Π S+ (e) ∞ .

Then, after τ ≥ 5 ηα 2 log r0 2 ν iterations, θ τ achieves the following error bound with respect to the true labels DISPLAYFORM4 Furthermore, if e has at most s nonzeros and S + is γ diffused per Definition C.1, then using DISPLAYFORM5 This result shows that when the Jacobian of the nonlinear mapping is low-rank, gradient descent enjoys two intriguing properties.
...
We make this observation rigorous below by showing that it is more difficult to fit to the portion of the residual that lies on the noise space compared to the portion on the signal space (assuming α ≫ ).Theorem C.3 Denote the residual at initialization θ 0 by r 0 = f (θ 0 ) −

y. Define the residual projection over the signal and noise space as DISPLAYFORM6 Suppose Assumption 1 holds over an Euclidian ball D of radius R < max DISPLAYFORM7 around the initial point θ 0 with α ≥ .

Then, over D there exists no θ that achieves zero training loss.

In particular, if D = R p , any parameter θ achieving zero training loss (f (θ) = y) satisfies the distance bound DISPLAYFORM8 This theorem shows that the higher the corruption (and hence E − ) the further the iterates need to stray from the initial model to fit the corrupted data.
...
We also generate the input samples at random around these two clusters uniformly at random on a sphere of radius ε 0 = 0.5 around the corresponding cluster center.
...
We picked K = 2 classes and set n = 400 and ε0 = 0.5.
...
DISPLAYFORM0 The residualsr = f (θ) − y, r = f (θ) − y obey the following equationr = (I − ηC(θ))r.

Proof Following Definition E.1, denoting f (θ) − y =r and f (θ) − y = r, we find that DISPLAYFORM1 Here (a) uses the fact that Jacobian is the derivative of f and (b) uses the fact that ∇L(θ) = J (θ) T r.

Using Assumption C.1, one can show that sparse vectors have small projection on S + .Lemma E.3 Suppose Assumption C.1 holds.

If r ∈ R n is a vector with s nonzero entries, we have that DISPLAYFORM2 Proof First, we bound the 2 projection of r on S + as follows DISPLAYFORM3 where we used the fact that v i ≤ √ γ v 2 √ n. Next, we conclude with DISPLAYFORM4 Proof The proof will be done inductively over the properties of gradient descent iterates and is inspired from the recent work (13).
...
Set r τ = f (θ τ ) − y and let r 0 = f (θ 0 )−y be the initial residual.

We keep track of the growth of the residual by partitioning the residual as r τ =r τ +ē τ whereē τ = Π S− (r τ ) ,r τ = Π S+ (r τ ).

We claim that for all iterations τ ≥ 0, the following conditions hold.ē DISPLAYFORM5 Assuming these conditions hold till some τ > 0, inductively, we focus on iteration τ + 1.

First, note that these conditions imply that for all τ ≥ i ≥ 0, θ i ∈ D where D is the Euclidian ball around θ 0 of radius DISPLAYFORM6 .

This directly follows from (E.6) induction hypothesis.

Next, we claim that θ τ +1 is still within the set D. This can be seen as follows: DISPLAYFORM7 Proof Since range space of Jacobian is in S + and η ≤ 1 β 2 , we begin by noting that DISPLAYFORM8 In the above, (a) follows from the fact that row range space of Jacobian is subset of S + via Assumption 2.

(b) follows from the definition ofr τ .

(c) follows from the upper bound on the spectral norm of the Jacobian over D per Assumption 2, (d) from the fact that η ≤ 1 β 2 , (e) from α ≤ β.

The latter combined with the triangular inequality and induction hypothesis (E.6) yields (after scaling (E.6) by 4 α) DISPLAYFORM9 concluding the proof of θ τ +1 ∈ D.To proceed, we shall verify that (E.6) holds for τ + 1 as well.

Note that, following Lemma E.2, gradient descent iterate can be written as DISPLAYFORM10 Since both column and row space of C(θ τ ) is subset of S + , we have thatē DISPLAYFORM11 This shows the first statement of the induction.

Next, over S + , we havē DISPLAYFORM12 where the second line uses the fact thatē τ ∈ S − and last line uses the fact thatr τ ∈ S + .

To proceed, we need to prove that C(θ τ ) has desirable properties over S + , in particular, it contracts this space.
...
Then, for any θ 1 , θ 2 ∈ D we have DISPLAYFORM14 where for (a) we utilized the induction hypothesis (E.6) and (b) follows from the upper bound on η.

Now that (E.22) is established, using following lemma, we find DISPLAYFORM15 The β 2 upper bound directly follows from Assumption 2 by again noticing range space of Jacobian is subset of S + .Lemma E.4 (Asymmetric PSD perturbation) Consider the matrices A, C ∈ R n×p obeying A − C ≤ α 2.

Also suppose CC T ⪰ α 2 P S+ .

Furthermore, assume range spaces of A, C lies in S + .

Then, DISPLAYFORM16 7 We say A ⪰ B if A − B is a positive semi-definite matrix in the sense that for any real vector v, v DISPLAYFORM17 Proof For r ∈ S + with unit Euclidian norm, we have DISPLAYFORM18 Also, for any r, by range space assumption r T AC T r = Π S+ (r)T AC T Π S+ (r) (same for CC T ).

Combined with above, this concludes the claim.

What remains is proving the final two statements of the induction (E.6).

Note that, using the claim above and recalling (E.19) and using the fact that J (θ τ +1 , θ τ ) ≤ β, the residual satisfies DISPLAYFORM19 where we used the fact that η ≤ 1 2β 2 .

Now, using the fact DISPLAYFORM20 which establishes the second statement of the induction (E.6).

What remains is obtaining the last statement of (E.6).To address this, completing squares, observe that DISPLAYFORM21 On the other hand, the distance to initial point satisfies DISPLAYFORM22 1 4 α) and using induction hypothesis (E.6), we find that DISPLAYFORM23 This establishes the final line of the induction and concludes the proof of the upper bound on θ τ − θ 0 2 .

To proceed, we shall bound the infinity norm of the residual.

Using DISPLAYFORM24 (E.27) What remains is controlling r τ ∞ .

For this term, we shall use the naive upper bound r τ 2 .

Using the rate of convergence of the algorithm (E.6), we have that DISPLAYFORM25 We wish the right hand side to be at most ν > 0 where ν ≥ Π S+ (e) ∞ .

This implies that we need DISPLAYFORM26 To conclude, note that since DISPLAYFORM27 ), we find that r τ ∞ ≤ r τ 2 ≤ ν, which guarantees DISPLAYFORM28 which is the advertised result.

If e is s sparse and S + is diffused, applying Lemma C.1 we have DISPLAYFORM29 Since Jacobian is derivative of f , we have that DISPLAYFORM30 Now, define the matrices J + = Π S+ (J ) and J − = Π S− (J ).

Using Assumption 1, we bound the spectral norms via DISPLAYFORM31 To proceed, projecting the residual on S + , we find for any θ with f (θ) = y DISPLAYFORM32 The identical argument for S − yields θ − θ 0 2 ≥ E− .

Together this implies DISPLAYFORM33 If R is strictly smaller than right hand side, we reach a contradiction as θ ∈ D. If D = R p , we still find (E.30).This shows that if is small and E − is nonzero, gradient descent has to traverse a long distance to find a good model.
...
Approximately speaking, this intuition corresponds to the 2 regularized problem min DISPLAYFORM34 If we set R = E+ β , we can hope that solution will learn only the signal and does not overfit to the noise.

The next section builds on this intuition and formalizes our algorithmic guarantees.

Throughout, σ min (⋅) denotes the smallest singular value of a given matrix.
...
Theorem E.7 (Jacobian Properties at Cluster Center) DISPLAYFORM3 The Jacobian mapping with respect to the input-to-hidden weights obey the following properties.• Smoothness is bounded by DISPLAYFORM4 • Top singular value is bounded by J (W , X) ≤ Γ X .• Let C > 0 be an absolute constant.
...
To proceed, given a matrix X ∈ R n×d and a subspace S ⊂ R n , we define the minimum singular value of the matrix over this subspace by σ min (X, S) which is defined as DISPLAYFORM7 Here, P S ∈ R n×n is the projection operator to the subspace.
...
Theorem E.8 (Jacobian Properties at Clusterable Dataset) Let input samples (x i ) n i=1 be generated according to (ε 0 , δ) clusterable dataset model of Definition 1.1 and define DISPLAYFORM8 T .
...
Recall the definition of the support sets Λ from Definition E.5.

We have the identitỹ DISPLAYFORM15 This impliesp ∈ S + hence range(J (W ,X)) ⊂ S + .
...
, x n ∈ R d with unit euclidean norm ( x i 2 = 1) aggregated as rows of a matrix X ∈ R n×d and the corresponding labels given by y ∈ R n generated accoring to (ρ, ε 0 = 0, δ) noisy dataset (Definition 1.2).
...
First, by construction y 2 ≤ √ n. What remains is bounding v T φ W 0 X T 2 .

Since ε 0 = 0 there are K unique rows.
...
Let w be a row of W 0 and x be a row of X. Since φ is Γ Lipschitz and DISPLAYFORM19 for some constant c > 0, concluding the proof.

E.2.1.

PROOF OF THEOREM E.14 We first prove a lemma regarding the projection of label noise on the cluster induced subspace.

DISPLAYFORM20 be an (ρ, ε 0 = 0, δ) clusterable noisy dataset as described in Definition 1.2.

Let {ỹ i } n i=1be the corresponding noiseless labels.

Let J (W , C) be the Jacobian at the cluster center matrix which is rank K and S + be its column space.

Then, the difference between noiseless and noisy labels satisfy the bound DISPLAYFORM21 Proof Let e = y−ỹ.

Observe that by assumption, th cluster has at most s = ρn errors.
...
Since cluster membership is non-overlapping, we have that DISPLAYFORM23 Similarly since supports of 1( ) are non-overlapping, we have that DISPLAYFORM24 Now, using e ∞ ≤ 2 (max distance between two labels), observe that DISPLAYFORM25 Since number of errors within cluster is at most n ρ, we find that DISPLAYFORM26 C where L is the Lipschitz constant of Jacobian spectrum.

Denote r τ = f (W τ ) − y. Using Lemma E.9 with probability 1 − K −100 , we have that r 0 2 = y − f (W 0 ) 2 ≤ Γ c 0 n log K 128 for some c 0 > 0.

Corollary E.8 guarantees a uniform bound for β, hence in Assumption 2, we pick DISPLAYFORM27 We shall also pick the minimum singular value over S + to be DISPLAYFORM28 We wish to verify Assumption 2 over the radius of DISPLAYFORM29 neighborhood of W 0 .

What remains is ensuring that Jacobian over S + is lower bounded by α.

Our choice of k guarantees that at the initialization, with probability 1 − K −100 , we have DISPLAYFORM30 Suppose LR ≤ α = α 0 2.

Using triangle inequality on Jacobian spectrum, for any W ∈ D, using W − W 0 F ≤ R, we would have DISPLAYFORM31 Finally, since LR = 4L r 0 2 α ≤ α, the learning rate is DISPLAYFORM32 Overall, the assumptions of Theorem C.2 holds with stated α, β, L with probability 1 − 2K −100 (union bounding initial residual and minimum singular value events).

This implies for all τ > 0 the distance of current iterate to initial obeys DISPLAYFORM33 The final step is the properties of the label corruption.

Using Lemma E.10, we find that DISPLAYFORM34 Substituting the values corresponding to α, β, L yields that, for all gradient iterations with DISPLAYFORM35 denoting the clean labels byỹ and applying Theorem C.2, we have that, the infinity norm of the residual obeys (using DISPLAYFORM36 This implies that if ρ ≤ δ 8, the network will miss the correct label by at most δ 2, hence all labels (including noisy ones) will be correctly classified.
...
Set learning rate η ≤ K 2cupnΓ 2 C 2 and maximum iterations τ 0 satisfying DISPLAYFORM8 where C 1 ≥ 1 is a constant of our choice.

Suppose input noise level ε 0 and number of hidden nodes obey DISPLAYFORM9 ∼ N (0, 1).
...
Suppose it holds until t ≤ τ 0 − 1.

At t + 1, via (E.37) we have that DISPLAYFORM11 DISPLAYFORM12 Right hand side holds since L ≤ 1 2ητ0Θ.

This establishes the induction for d t+1 .Next, we show the induction on p t .

Observe that 3d t +d t+1 ≤ 10τ 0 ηΓ √ nε 0 Θ(1 + 8ητ 0 β 2 ).

Following (E.39) and using DISPLAYFORM13 Concluding the induction since L satisfies the final line.

Consequently, for all 0 ≤ t ≤ τ 0 , we have that DISPLAYFORM14 which is implied by k ≥ O(Γ 10 K 2 C 4 λ(C) 4 log( DISPLAYFORM15 ).Finally, following (E.40), distance satisfies DISPLAYFORM16 ).E.3.1.

COMPLETING THE PROOF OF THEOREM 2.2 Theorem 2.2 is obtained by the theorem below when we ignore the log terms, and treating Γ, λ(C) as constants.

We also plug in η = K 2cupnΓ 2 C 2 .Theorem E.13 (Training neural nets with corrupted labels) Let {(x i , y i )} n i=1 be an (s, ε 0 , δ) clusterable noisy dataset as described in Definition 1.2.

Let {ỹ i } n i=1 be the corresponding noiseless labels.

Suppose φ(0) , φ ′ , φ ′′ ≤ Γ for some Γ ≥ 1, input noise and the number of hidden nodes satisfy ε 0 ≤O( λ(C) DISPLAYFORM17 )

.where C ∈ R K×d is the matrix of cluster centers.

Set learning rate η ≤ ∼ N (0, 1).

With probability 1 − 3 K 100 , after DISPLAYFORM18 ) log( Γ n log K ρ ) iterations, for all 1 ≤ i ≤ n,we have that• The per sample normalized 2 norm bound satisfies DISPLAYFORM19 • Suppose ρ ≤ δ 8.

Denote the total number of prediction errors with respect to true labels (i.e. not satisfying (E.46)) by err(W ).

With same probability, err(W τ ) obeys DISPLAYFORM20 log( Γ √ n log K ρ ).•

Suppose ρ ≤ δ 8 and ε 0 ≤ c DISPLAYFORM21 , then, W τ assigns all input samples x i to correct ground truth labelsỹ i i.e. (E.46) holds for all 1 ≤ i ≤ n.• Finally, for any iteration count 0 ≤ t ≤ τ the total distance to initialization is bounded as DISPLAYFORM22 .

Proof Note that proposed number of iterations τ is set so that it is large enough for Theorem E.14 to achieve small error in the clean input model (ε 0 = 0) and it is small enough so that Theorem E.12 is applicable.

In light of Theorems E.12 and E.14 consider two gradient descent iterations starting from W 0 where one uses clean dataset (as if input vectors are perfectly cluster centers)X and other uses the 1157 1158 1159 1160 1161 1162 1163 1164 1165 1166 1167 1168 1169 1170 1171 1172 1173 1174 1175 1176 1177 1178 1179 1180 1181 1182 1183 1184 1185 1186 1187 1188 1189 1190 1191 1192 1193 1194 1195 1196 1197 1198 1199 1200 1201 1202 1203 1204 1205 1206 1207 1208 1209 original dataset X. Denote the prediction residual vectors of the noiseless and original problems at time τ with respect true ground truth labelsỹ byr τ = f (W τ ,X) −ỹ and r τ = f (W τ , X) −ỹ respectively.

Applying Theorems E.12 and E.14, under the stated conditions, we have that r τ ∞ ≤ 4ρ and (E.42) DISPLAYFORM0 First statement: The latter two results imply the 2 error bounds on r τ = f (W τ , X) −ỹ.

Second statement: To assess the classification rate we count the number of entries of r τ = f (W τ , X) −ỹ that is larger than the class margin δ 2 in absolute value.

Suppose ρ ≤ δ 8.

Let I be the set of entries obeying this.

For i ∈ I using r τ ∞ ≤ 4ρ ≤ δ 4, we have r τ,i ≥ δ 2 ⇒ r τ,i + r τ,i −r τ,i ≥ δ 2 ⇒ r τ,i −r τ,i ≥ δ 4.Consequently, we find that r τ −r τ 1 ≥ I δ 4.Converting 2 upper bound on the left hand side to 1 , we obtain c √ n ε 0 Γ 3 K √ n log K λ(C) log( Γ √ n log K ρ )

≥ I δ 4.Hence, the total number of errors is at most DISPLAYFORM1 Third statement -Showing zero error:

Pick an input sample x from dataset and its clean versionx.

We will argue that f (W τ , x) − f (W τ ,x) is smaller than δ 4 when ε 0 is small enough.

We again write DISPLAYFORM2 The first term can be bounded via DISPLAYFORM3 Next, we need to bound DISPLAYFORM4 where DISPLAYFORM5 ), x −x 2 ≤ ε 0 and W 0 i.i.d.∼ N (0, I).

Consequently, using by assumption we have DISPLAYFORM6 and applying an argument similar to Theorem E.15 (detailed in Appendix G), with probability at 1 − 1 n 100 , we find that DISPLAYFORM7 Combining the two bounds above we get DISPLAYFORM8 Hence, if ε 0 ≤ c DISPLAYFORM9 , we obtain that, for all DISPLAYFORM10 If ρ ≤ δ 8, we obtain f (W τ , x i ) −ỹ i < δ 2 hence, W τ outputs the correct decision for all samples.

Fourth statement -Distance: This follows from the triangle inequality DISPLAYFORM11 We have that right hand side terms are at most O(Γ K log K λ(C)) and O(tηε 0 Γ 4 Kn λ(C) log( DISPLAYFORM12 Theorems E.12 and E.14 respectively.
