...
Let Ω ⊆ R d denote a closed and convex set.

Our objective is to minimize f : Ω → R, which is an unknown, differentiable, convex, and β -smooth function.

The following summary is based on .

where · denotes the Euclidean norm.

Let (S , Σ, µ) be a probability space (and let E denote expectation).
...
The update rule for conventional full-precision projected SGD is w t+1 = P Ω (w t − αg(w t )), where w t is the current parameter input, α is the learning rate, and P Ω is the Euclidean projection onto Ω.

We say the stochastic gradient has a second-moment upper bound B when E[ g(w) 2 ] ≤ B for all w ∈ Ω. Similarly, the stochastic gradient has a variance upper bound
...
Let f : Ω → R denote a convex and β -smooth function and let R 2 sup w∈Ω w − w 0 2 .

Suppose that the projected SGD update is executed for T iterations with α = 1/(β + 1/γ) where γ = r 2/T /σ .

Given repeated and independent access to stochastic gradients with a variance upper bound σ 2 , projected SGD satisfies
...
where g l (w t ) is the stochastic gradient computed and broadcasted by processor l. Provided that g l (w t ) is a stochastic gradient with a variance upper bound σ 2 for all l, then Data-parallel SGD is described in Algorithm 1.

Full-precision data-parallel SGD is a special case of Algorithm 1 with identity encoding and decoding mappings.

Otherwise, the decoded stochastic gradientĝ i (w t ) is likely to be different from the original local stochastic gradient g i (w t ).
...
Let f , R, and γ be as defined in Theorem 1 and let ε > 0.

Suppose that the projected SGD update is executed for T iterations with α = 1/(β + √ K/γ) on K processors, each with access to independent stochastic gradients of f with a second-moment bound B. The smallest T for the full-precision data-parallel SGD that guarantees
...
The result in Theorem 2 implies that if g(w) is a stochastic gradient with a second-moment bound η, then Q s (g(w)) is a stochastic gradient with a variance upper bound ε Q η.
...
Let f and R be defined as in Theorem 1, let ε Q be defined as in Theorem 2, let ε > 0,B = (1 + ε Q )B, and let γ > 0 be given by γ 2 = 2R 2 /(BT ).

With ENCODE and DECODE defined as in Appendix A, suppose that Algorithm 1 is executed for T iterations with a learning rate α = 1/(β + √ K/γ) on K processors, each with access to independent stochastic gradients of f with a second-moment bound B.
...
How do QSGD and NUQSGD compare in terms of bounds on the expected number of communication bits required to achieve a given suboptimality gap ε?
...
For any (nonrandom) number of iterations T , an upper bound, N A , holding uniformly over iterations k ≤ T on the expected number of bits used by an algorithm A to communicate the gradient on iteration k, yields an upper bound T N A , on the expected number of bits communicated over T iterations by algorithm A. Taking T = T A,ε to be the (minimum) number of iterations needed to guarantee an expected suboptimality gap of ε based on the properties of A, we obtain an upper bound, ζ A,ε = T A,ε N A , on the expected number of bits of communicated on a run expected to achieve a suboptimality gap of at most ε.

Theorem 5 (Expected number of communication bits).

Provided that s = o(log(d)) and

Focusing on the dominant terms in the expressions of overall number of communication bits required to guarantee a suboptimality gap of ε, we observe that NUQSGD provides slightly stronger guarantees.
...
where r i = |v i |/ v and p(r),s(r), τ(r) are defined in Section 3.1.
...
where τ j l j+1 − l j for j ∈ {0, · · · , s}.

Substituting τ 0 = 2 −s and τ j = 2 j−1−s for j ∈ {1, · · · , s} into (9), we have
...
Recall from Appendix A that the quantized gradient Q s (v) is determined by the tuple ( v , ρ ρ ρ, h).
...
Let g(w) andĝ(w) denote the full-precision and decoded stochastic gradients, respectively.
...
Ignoring all but terms depending on d and s, we have T ε = O(B/ε 2 ).

Following Theorems 2 and 3 for NUQSGD, ζ NUQSGD,ε = O(N Q ε Q B/ε 2 ).
...
In the following theorem, we show that for any given set of levels, there exists a distribution of points with dimension d such that the variance is in Ω( √ d), and so our bound is tight in d.
...
An upper bound on the nonuniform quantization of v is given by ε LP v 2 where ε LP is the optimal value of the following LP:
...
Finally, note that τ 0 = τ 1 = 1/2 minimizes the optimal value of P 0 (38).

In this section, we focus on the special case of exponentially spaced collection of levels of the form L p = (0, p s , · · · , p 2 , p, 1) for p ∈ [0, 1] and an integer number of levels, s. In this case, we have τ 0 = p s and τ j = (1 − p)p s− j for j = 1, · · · , s.
...
Let f : Ω → R denote a possibly nonconvex and β -smooth function.

Let w 0 ∈ Ω denote an initial point, ε Q be defined as in Theorem 2, T ∈ Z >0 , and f * = inf w∈Ω f (w).

Suppose that Algorithm 1 is executed for T iterations with a learning rate α = O(1/β ) on K processors, each with access to independent stochastic gradients of f with a second-moment bound B. Then there exists a random stopping time R ∈ {0, · · · , T } such that NUQSGD guarantees E[ ∇ f (w R ) 2 ] ≤ ε where ε = O β ( f (w 0 ) − f * )/T + (1 + ε Q )B .
