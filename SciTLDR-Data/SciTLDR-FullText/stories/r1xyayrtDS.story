...
The goal is to learn a policy π : S → A that selects the agents actions in order to maximize the sum of future discounted rewards

For a given state-action pair (s, a) the value function is defined as Q π (s, a) := E si∼pπ,ai∼π [R t |s, a], which is the expected return when executing action a in state s and following π afterwards.

We write π φ for the policy with parameters φ, that we learn in order to maximize the expected return J(φ) = E si∼pπ,ai∼π [R 0 ].

The parameters can be optimized with the gradient of J w.r.t.

the policy parameters φ.

The deterministic policy gradient Silver et al. (2014) is given by

In practice the value function Q π is not given and has to be approximated.
...
Deep Q-learning Mnih et al. (2015) is a variant of this algorithm and can be used to learn the parameters θ of an artificial neural network Q θ : S × A → R that approximates the value function.

The network is updated by regressing its value at (s t , a t ) to its 1-step TD targets

where Qθ, πφ are the target networks of Q φ , π θ and the corresponding parametersφ andθ are updated according to a exponential moving average:θ ← τ θ + (1 − τ )θ, similarly for φ.

If a learned critic Q θ is used in the deterministic policy gradient (eq. 1), the actor π φ is updated through Q θ , which in turn is learned with the rewards obtained from the environment.
...
In this approach two Q-networks (Q θ1 , Q θ2 ) are learned in parallel.
...
Figure 2: The plots show the average over 10 runs of the weighting parameter β for three OpenAI gym environments.

targets.

The TD target y k for the k-th Q-network Q θ k , k ∈ {1, 2} is defined as a weighted average of the network itself and the minimum of both networks
...
However, setting β manually is difficult.

The perfect β that maximally reduces bias may change from environment to environment and also over the time of the training process.

Consequently, we adjust β over the course of the training.

As the goal is to minimize bias and since β controls in which direction more bias is introduced, we use samples of the Q-values to learn β.

After every episode we compute for every seen state-action pair (s t , a t ) the actual discounted future return from that pair onwards R t = T i=t γ i−t r i , which is a sample for the quantity the Q-networks Q θ k (s t , a t ) try to estimate.

If the Q-estimates are higher than R t , they overestimated and β should be decreased to give the "min" term in eq. 4 more weight.

If on the other hand the Q-estimates are lower, we observe the case of underestimation and β should be increased.

This behaviour can be achieved by

Initialize critic networks Q θ1 , Q θ2 , and actor network π φ with random parameters θ 1 , θ 2 , φ Initialize target networksθ 1 ← θ 1 ,θ 2 ← θ 2 ,φ ← φ, set k = 0 and β ∈ [0, 1] Initialize replay buffer B for t = 1 to total timesteps do Select action with exploration noise a ∼ π φ (s) + , ∼ N (0, σ) and observe reward r, new state s and binary value d indicating if the episode ended Store transition tuple (s, a, r, s , d) in B and set k ← k + 1 if k ≥ beta update rate and
...
where we restrict β to be in the interval [0, 1], E is the number of episodes we optimize over, s (j) t is the t-th state in the j-th considered episode (similarly for the actions a (j) t ), T j is the number of time steps in episode j and R (j) t are the future discounted returns.

The parameter β is updated every time the sum over all time steps in the episodes since the last update, E j=1 T j , exceeds a fixed threshold.

We set this threshold to be the maximum number of episode steps that are possible.

To optimize β we use stochastic gradient descent.

We note that learning β increases the computational complexity only minimal, as it is just one parameter that has to be optimized.
...
This indicates that the weighting parameter β can indeed be adjusted over the course of the training such that the two opposing biases cancel each other out.

The behaviour of β is visualized in Figure 2 as an average over 10 runs per environment.

For the Hopper task it can be observed that after some time the parameter β gets very close to zero, which corresponds to using only CDQ to update the critic.
...
Adjusting β over time allows to put more weight on the term that currently gives a more accurate estimate.

This prevents the accumulation of errors introduced by bootstrapping in the TD target.

From the plots it can also be seen that treating β as an hyperparameter might be difficult, as it would have to be tuned for every environment.

Furthermore, leaving β fixed could not account for the changing learning dynamics over the course of training.

In Figure 2 it can be seen that different drifts exist in each environment.

For example in Walker2d the learned β decreases on average after an initial stabilization period.
...
We set the learning rate for the weighting parameter β to 0.05 and initialize β = 0.5 at the beginning of the training for all environments.
...
To further understand the influence of the dynamic weighting scheme we trained BTD3 with a fixed value for β.

We evaluated for the values β ∈ {0.00, 0.25, 0.50, 0.75, 1.00}, where β = 0.00 corresponds to TD3 and β = 1.00 corresponds to DDPG.

The averaged results over 10 runs are shown in Figure 4 .

From the plots we can make two observations.

First, it is essential that β is adjusted during the training.

For any of the considered values of β leaving it fixed leads to a worse performance compared to BTD3 and in most cases also worse than TD3.
...
The dynamic adjustment to prevent accumulating errors is not possible when β is fixed.

Second, it is surprising that fixed values for β that would seem promising to try from Figure 2 can perform worse than other fixed values.

For example inspecting the plots in Figure 2 the value β = 0.75 seems a good fit for the HalfCheetah environment.

But the evaluation shows that β = 0.25 and β = 0.50 perform better.
...
The show algorithms are BTD3 and versions of it with a fixed value of β.
