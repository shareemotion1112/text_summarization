...
The inferred latents using their method (termed as β-VAE ) are empirically shown to have better disentangling properties, however the method deviates from the basic principles of variational inference, creating increased tension between observed data likelihood and disentanglement.
...
Unlike β-VAE , our approach does not introduce any extra conflict between disentanglement of the latents and the observed data likelihood, which is reflected in the overall quality of the generated samples that matches the VAE and is much better than β-VAE.

This does not come at the cost of higher entanglement and our approach also outperforms β-VAE in disentangling the latents as measured by various quantitative metrics.
...
We start with a generative model of the observed data that first samples a latent variable z ∼ p(z), and an observation is generated by sampling from p θ (x|z).

The joint density of latents and observations is denoted as p θ (x, z) = p(z)p θ (x|z).

The problem of inference is to compute the posterior of the latents conditioned on the observations, i.e., p θ (z|x) = DISPLAYFORM0 .
...
One successful way of achieving this for variational inference is to have a so-called recognition model, parameterized by φ, that encodes an inverse map from the observations to the approximate posteriors (also referred as variational autoencoder or VAE) BID34 BID49 .

The recognition model parameters are learned by optimizing the problem min φ E x KL(q φ (z|x) p θ (z|x)), where the outer expectation is over the true data distribution p(x) which we have samples from.
...
Often, the density forms of p(z) and q φ (z|x) are chosen such that their KL-divergence can be written analytically in a closed-form expression (e.g., p(z) is N (0, I) and q φ (z|x) is N (µ φ (x), Σ φ (x))) BID34 .
...
Further, in cases when q φ (·) can be written as a continuous transformation of a fixed base distribution (e.g., the standard normal distribution), a low variance estimate of the gradient over φ can be obtained by coordinate transformation (also referred as reparametrization) BID22 BID34 BID49 .
...
The complexity of the observed data is absorbed in the conditional distribution p θ (x|z) which encodes the interactions among the latents.
...
For inferring disentangled factors, this should be factorizable along the dimensions, i.e., DISPLAYFORM1 This can be achieved by minimizing a suitable distance between the inferred prior q φ (z) and the disentangled generative prior p(z).

We can also define expected posterior as DISPLAYFORM2 If we take KL-divergence as our choice of distance, by relying on its pairwise convexity (i.e., KL( BID56 , we can show that the distance between q φ (z) and p θ (z) is bounded by the objective of the variational inference: DISPLAYFORM3 DISPLAYFORM4 In general, the prior p(z) and expected posterior p θ (z) will be different, although they may be close (they will be same when p θ (x) = p θ (x|z)p(z)dz is equal to p(x)).
...
This can be due to: (i) p(x) and p θ (x) being far apart which in turn causes p(z) and p θ (z) being far apart, and (ii) the non-convexity of the ELBO objective which prevents us from achieving the global minimum of E x KL(q φ (z|x) p θ (z|x)) (which is 0 and implies KL(q φ (z) p θ (z)) = 0).

In other words, maximizing the ELBO (Eq. (1)) might also result in reducing the value of KL(q φ (z) p(z)), however, due to the aforementioned reasons, the gap between KL(q φ (z) p(z)) and E x KL(q φ (z|x) p θ (z|x)) could be large at the stationary point of convergence.

Hence, minimizing KL(q φ (z) p(z)) or any other suitable distance D(q φ (z), p(z)) explicitly will give us better control on the disentanglement.

This motivates us to add D(q φ (z) p(z)) as part of the objective to encourage disentanglement during inference, i.e., DISPLAYFORM5 where λ controls its contribution to the overall objective.

We refer to this as DIP-VAE (for Disentangled Inferred Prior) subsequently.

Optimizing FORMULA7 directly is not tractable if D(·, ·) is taken to be the KL-divergence KL(q φ (z) p(z)), which does not have a closed-form expression.

One possibility is use the variational formulation of the KL-divergence BID45 BID46 ) that needs only samples from q φ (z) and p(z) to estimate a lower bound to KL(q φ (z) p(z)).

However, this would involve optimizing for a third set of parameters ψ for the KL-divergence estimator, and would also change the optimization to a saddle-point (min-max) problem which has its own optimization challenges (e.g., gradient vanishing as encountered in training generative adversarial networks with KL or Jensen-Shannon (JS) divergences BID24 BID2 ).

Taking D to be another suitable distance between q φ (z) and p(z) (e.g., integral probability metrics like Wasserstein distance BID54 ) might alleviate some of these issues but will still involve complicating the optimization to a saddle point problem in three set of parameters 3 .
...
Most existing work on the VAE models uses q φ (z|x) having the form DISPLAYFORM8 , where µ φ (x) and Σ φ (x) are the outputs of a deep neural net parameterized by φ.
...
Further, Σ φ (x) is commonly taken to be a diagonal matrix which means that cross-correlations (off-diagonals) between the latents are due to only DISPLAYFORM10 .

This suggests two possible options for the disentangling regularizer: DISPLAYFORM11 which we refer as DIP-VAE-II.

Penalizing just the off-diagonals in both cases will lead to lowering the diagonal entries of Cov p(x) [µ φ (x)] as the ij'th off-diagonal is really a derived attribute obtained by multiplying the square-roots of i'th and j'th diagonals (for each example x ∼ p(x), followed by averaging over all examples).

This can be compensated in DIP-VAE-I by a regularizer on the diagonal entries of Cov p(x) [µ φ (x)] which pulls these towards 1.

We opt for two separate hyperparameters controlling the relative importance of the loss on the diagonal and off-diagonal entries as follows: DISPLAYFORM12 The regularization terms involving Cov p(x) [µ φ (x)] in the above objective (6) can be efficiently optimized using SGD, where Cov p(x) [µ φ (x)] can be estimated using the current minibatch 4 .For DIP-VAE-II, we have the following optimization problem: DISPLAYFORM13 As discussed earlier, the term DISPLAYFORM14 Penalizing the off-diagonals of Cov p(x) [µ φ (x)] in the Objective (7) will contribute to reduction in the magnitude of its diagonals as discussed earlier.
...
It is also possible to match higher order central moments of q φ (z) and the prior p(z).

In particular, third order central moments (and moments) of the zero mean Gaussian prior are zero, hence 2 norm of third order central moments of q φ (z) can be penalized.

Recently proposed β-VAE proposes to modify the ELBO by upweighting the KL(q φ (z|x) p(z)) term in order to encourage the inference of disentangled factors: DISPLAYFORM0 where β is taken to be great than 1.

Higher β is argued to encourage disentanglement at the cost of reconstruction error (the likelihood term in the ELBO).

Authors report empirical results with β ranging from 4 to 250 depending on the dataset.

As already mentioned, most VAE models proposed in the literature, including β-VAE, work with N (0, I) as the prior p(z) and N (µ φ (x), Σ φ (x)) with diagonal Σ φ (x) as the approximate posterior q φ (z|x).

This reduces the objective (8) to DISPLAYFORM1 For high values of β, β-VAE would try to pull µ φ (x) towards zero and Σ φ (x) towards the identity matrix (as the minimum of x − ln x for x > 0 is at x = 1), thus making the approximate posterior q φ (z|x) insensitive to the observations.

This is also reflected in the quality of the reconstructed samples which is worse than VAE (β = 1), particularly for high values of β.

Our proposed method does not have such increased tension between the likelihood term and the disentanglement objective, and the sample quality with our method is on par with the VAE.Finally, we note that both β-VAE and our proposed method encourage disentanglement of inferred factors by pulling Cov q φ (z) (z) in Eq. (5) towards the identity matrix: β-VAE attempts to do it by making Cov q φ (z|x) (z) close to I and E q φ (z|x) (z) close to 0 individually for all observations x, while the proposed method directly works on Cov q φ (z) (z) (marginalizing over the observations x) which retains the sensitivity of q φ (z|x) to the conditioned-upon observation.3 QUANTIFYING DISENTANGLEMENT: SAP SCORE propose a metric to evaluate the disentanglement performance of the inference mechanism, assuming that the ground truth generative factors are available.
...
Given the inferred z x := µ φ (x) for each example x, they compute the absolute difference of these vectors for each pair, followed by averaging these difference vectors.
...
It is computed as follows: (i) We first construct a d × k score matrix S (for d latents and k generative factors) whose ij'th entry is the linear regression or classification score (depending on the generative factor type) of predicting j'th factor using only i'th latent [µ φ (x)] i .
...
For inactive latent Table 1 : Z-diff score , the proposed SAP score and reconstruction error (per pixel) on the test sets for 2D Shapes and CelebA (β 1 = 4, β 2 = 60, λ = 10, λ 1 = 5, λ 2 = 500 for 2D Shapes; β 1 = 4, β 2 = 32, λ = 2, λ 1 = 1, λ 2 = 80 for CelebA).
...
We consider two baselines for the task of unsupervised inference of disentangled factors: (i) VAE BID34 BID49 , and (ii) the recently proposed β-VAE .
...
Table 1 shows the Z-diff scores and the proposed SAP Figure 2 : The proposed SAP score and the Z-diff score as a function of average reconstruction error (per pixel) on the test set of CelebA data for β-VAE and the proposed DIP-VAE.

The plots are generated by varying β for β-VAE, and λ od for DIP-VAE-I and DIP-VAE-II (the number next to each point is the value of these hyperparameters, respectively).scores along with reconstruction error (which directly corresponds to the data likelihood) for the test sets of CelebA and 2D Shapes data.

Further we also show the plots of how the Z-diff score and the proposed SAP score change with the reconstruction error as we vary the hyperparameter for both methods (β and λ od , respectively) in Fig. 1 (for 2D Shapes data) and Fig. 2 (for CelebA data).

The proposed DIP-VAE-I gives much higher Z-diff score at little to no cost on the reconstruction error when compared with VAE (β = 1) and β-VAE, for both 2D Shapes and CelebA datasets.
...
The reason is that DIP-VAE-I enforces [Cov p(x) [µ φ (x)]] ii to be close to 1 and this may affect the disentanglement adversely by splitting a generative factor across multiple latents for 2D Shapes where the generative factors are much less than the latent dimension.

For real datasets having lots of factors with complex generative processes, such as CelebA, DIP-VAE-I is expected to work well which can be seen in Fig. 2 where DIP-AVE-I yields a much lower reconstruction error with a higher SAP score (as well as higher Z-diff scores).Binary attribute classification for CelebA. We also experiment with predicting the binary attribute values for each test example in CelebA from the inferred µ φ (x).

For each attribute k, we compute the attribute vector set, and project the µ φ (x) along these vectors.
...
The proposed DIP-VAE outperforms both VAE and β-VAE for most attributes.

The performance of β-VAE gets worse as β is increased further.

DISPLAYFORM0

Adversarial autoencoder BID40 also matches q φ (z) (which is referred as aggregated posterior in their work) to the prior p(z).

However, adversarial autoencoder does not have the goal of minimizing KL(q φ (z|x)||p θ (z|x)) which is the primary goal of variational inference.

It DISPLAYFORM0 , where D is the distance induced by a discriminator that tries to classify z ∼ q φ (z) from z ∼ p(z) by optimizing a cross-entropy loss (which induces JS-divergence as D).
...
Unlike β-VAE, our variational objective does not have any conflict between the data log-likelihood and the disentanglement of the inferred latents, which is reflected in the empirical results.
