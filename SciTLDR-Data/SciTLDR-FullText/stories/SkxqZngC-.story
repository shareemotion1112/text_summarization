...
More specifically, the stickbreaking process generates an infinite sequence of mixture weights π = {π k } ∞ k=1 as follows: DISPLAYFORM0 This is often denoted as π ∼ GEM(α), where GEM stands for Griffiths, Engen and McCloskey BID7 and α is the concentration parameter.

One can see that π k satisfies 0 ≤ π k ≤ 1 and DISPLAYFORM1

Variational Auto-Encoder (VAE) is among the most successful generative models recently.

Specifically, it assumes that a sample x is generated mathematically as follows: DISPLAYFORM0 where p(z) is the prior of the latent representation z and p φ (x|z) is a conditional distribution with parameters φ to generate x. The approximation to the posterior of the generative process, q ψ (z|x), is parametrized by an inference neural network with parameters ψ.
...
The parameters φ and ψ can be optimized jointly by applying the Stochastic Gradient Variational Bayes (SGVB) BID16 estimator and Reparameterization Trick (RT).
...
The key difference is that the DISPLAYFORM0 is drawn from the GEM distribution to make sure of ∞ k=1 π k = 1 and 0 ≤ π k ≤ 1.

Specifically, we suppose each topic θ k = σ(φ k ) is a probability distribution over vocabulary, where φ k ∈ R V is the parameter of the topic-specific word distribution, σ(·) is the softmax function and V is the vocabulary size.

In iTM-VAE, there are unlimited number of topics and we denote Θ = {θ k } ∞ k=1 and Φ = {φ k } ∞ k=1 as the collections of these countably infinite topics and the corresponding parameters.

The generation of a document by iTM-VAE can then be mathematically described as: DISPLAYFORM1 where α is a hyper-parameter, Cat(θ i ) is a categorical distribution parameterized byθ i , and δ θ k (θ) is a discrete dirac function, which equals to 1 when θ = θ k and 0 otherwise.
...
In practice, following BID24 , we factorize the parameter φ k of topic θ k as φ k = t k W where t k ∈ R H is the k-th topic factor vector, W ∈ R H×V is the word factor matrix and H ∈ R + is the factor dimension.

For simplicity, we still use Φ to denote all the parameters regarding to the generative procedure of iTM-VAE.

Note that, although we parameterize the generative process with Φ, iTM-VAE is still a nonparametric model, since it has potentially infinite model capacity, and can grow the number of parameters with the amount of training data.
...
In this section, we describe the inference process of iTM-VAE, i.e. how to draw π given a document w 1:N .

To elaborate, suppose ν = [ν 1 , ν 2 , . . . , ν K−1 ] is a K − 1 dimensional vector where ν k is a random variable sampled from a Kumaraswamy distribution κ(ν; a k , b k ) parameterized by a k and b k , iTM-VAE models the joint distribution q ψ (ν|w 1:N ) as: DISPLAYFORM0 where g(w 1:N ; ψ) is a neural network with parameters ψ.

Then, π = {π k } K k=1 can be drawn by: DISPLAYFORM1 In the above procedure, we truncate the infinite sequence of mixture weights π = {π k } ∞ k=1 by K elements, and ν K is always set to 1 to ensure K k=1 π k = 1.
...
Specifically, we compute a mixed topic distributionθ = σ( ∞ k=1 π k φ k ) for each document, where π k is sampled from GEM(α), and then each word of the document is sampled from Cat(θ).

The benefit of the products-of-experts is demonstrated in Section 5.

In the generative process, the concentration parameter α of GEM(α) can have significant impact on the growth of number of topics.

The larger the α is, the more "breaks" it will create, and consequently, more topics will be used.

Hence, it is generally reasonable to consider placing a prior on α so that the model can adjust the concentration parameter to data automatically.

Concretely, since the Gamma distribution is conjugate to Beta(1, α), we place a Gamma(s 1 , s 2 ) prior on α.

Then the ELBO of iTM-VAE can be written as: DISPLAYFORM0 where p(α|s 1 , s 2 ) = Gamma(s 1 , s 2 ), p(v k |α) = Beta(1, α), q(α|γ 1 , γ 2 ) is the variational posterior for α with γ 1 , γ 2 as parameters across the whole dataset.
...
We use a two-layer fully-connected neural network for g(w 1:N ; ψ) of Equation 8, and the number of hidden units is set to 256 and 512 for 20News and RCV1-V2, respectively.
...
The hyper-parameter α for GEM distribution is cross-validated on validation set from [10, 20, 30, 50, 100] .
...
As mentioned in Section 4.4, the concentration parameter α of GEM(α) has significant impact on the growth of the number of topics, which affects the performance of iTM-VAE significantly.

As a result, α has to be chosen by cross-validation for better performance.

FIG4 (a) also confirms that the number of effective topics increases with α.

Consequently, iTM-VAE-G, which places a Gamma(s 1 , s 2 ) prior on α, is proposed to enable the model to adapt α to data automatically.
...
In our model, this means the choice of α will control the number of the learned topics very tightly when the decoder is strong (e.g. iTM-VAE-Prod with large factor dimension H), which might cause the model to be lack of adaptive power.
...
For iTM-VAE-Prod, we set α = 4, and try different tricks to alleviate the collapse problem: KL annealing where the relative weight of the KL divergence term in the ELBO at epoch n is min(0.005n, 1); Decoder regularization where the weight of the L2 regularization on the decoder is set to 0.1.

For iTM-VAE-G, we add a relatively non-informative prior Gamma(1, 0.25) on α, and initialize the global variational parameters γ 1 and γ 2 of Equation 13 the same as the non-informative prior.

A SGD optimizer with a learning rate of 0.01 is used to optimize γ 1 and γ 2 .
...
However, by placing a prior on the concentration parameter α, iTM-VAE-G can increase the adaptive power of the model.

The corpus-level variational posterior of α and the number of effective topics learned by iTM-VAE-G is shown in Table 3 .

As for iTM-VAE-G, before training, E q(α|γ1,γ2) [α], the expectation of α given the variational posterior q(α|γ 1 , γ 2 ) is 4.

Once the training is done, E q(α|γ1,γ2) [α] will be adjusted to the training set.

Table 3 illustrates γ 1 , γ 2 , E q(α|γ1,γ2) [α] and the number of effective topics that are learned from data.

We can see that, if the training set contains only 1 class of documents, E q(α|γ1,γ2) [α] will drop to 2.35, and only 6 effective topics are used to model the dataset.

Whereas, when the training set consists of 10 classes of documents, E q(α|γ1,γ2) [α] increases to 6.62, and 11 effective topics are discovered by the model to explain the dataset.

This indicates that iTM-VAE-G can learn to adjust α to data.
...
Table 3 : Learned posterior distribution of α and number of effective topics learned by iTM-VAE-G on subsets of 20News dataset.
...
One possible reason is that the stick-breaking prior for the document-specific π encourages the model to learn sparse representation, and the model can adjust the number of topics according to the data.
...
We sample one topic assignment π for each document, sort and then average across the test set.
