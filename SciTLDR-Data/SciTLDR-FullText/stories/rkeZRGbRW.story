...
The generative adversarial framework entails training a generating function G θ : Z → X , where z ∈ Z is noise sampled from a relatively simple prior (such as spherical Gaussian), p(z), and G θ is modeled by a deep neural network with parameters, θ.

The objective is to find the optimal parameters such that the induced or generated distribution, Q θ , matches a desired target distribution, P. In order to accomplish this, adversarial models define a discriminator function, D φ,d : X → R, modeled by a deep neural network with parameters, φ.

The objective of the discriminator is to estimate a measure of distance or divergence, d(P, Q θ ), between the two distributions, P and Q θ .
...
Generally, the estimator given by the family of functions defined by D φ,d represent a family of lower-bounds of the respective divergence: DISPLAYFORM0 In BID7 DISPLAYFORM1 where V JSD (D φ,d , G θ , P) is a lower bound to JSD = 2 * JSD −2 log 2.

Here, D = σ •φ 1 is the composition of the sigmoid function, σ(y) = 1 1+exp(−y) , and a neural network, φ.
...
The above family of f -divergence estimators such as the JSD one above are powerful and can produce generators with high quality samples on complex datasets BID20 , but there is some evidence that they provide poor learning signals for the generator parameters, θ.

As shown in , the JS divergence will be flat everywhere important if P and Q θ both lie on low-dimensional manifolds (as is likely the case with real data) and do not prefectly align.
...
In addition, the proxy loss, E x∼Q θ [log D(x)], which is commonly used as an alternative to the GAN objective in training the discriminator can be highly unstable, and is generally plagued by missing modes and model collapse.
...
A well-known scenario is one in which the discriminator can perfectly discern samples from P from samples from Q θ .

Moreover, show that the discriminator is likely to be constant in the regime of Q θ , thus emitting zero gradients and no learning signal for the generator.

Consider such a discriminator; if x, y ∼ Q θ are samples, then as the generator attempts to minimize any discrepancy between P and Q θ , the slope of the tangent hyperplane connecting (x, φ(x)) and (y, φ(y)) likely obeys DISPLAYFORM0 where d X (·, ·) is a metric defined on X .

This roughly corresponds to a flat discriminator over Q θ which doesn't emit any gradients and impedes the generator from learning.

As a result, we find DISPLAYFORM1 Alternatively, a flat discriminator in the regime of Q θ can be interpreted as having minimal variance (see Figure 1) .

That is, when the discriminator is mostly constant over P and Q θ , the discriminator's unnormalized output distribution is peaked and tends to mimic a bi-modal Dirac distribution.

This is because, in a sense, the discriminator acts as a bi-modal mapping: real examples from P are mapped to, for example, 1 while generated examples from Q θ (x) are mapped to 0.

In contrast, when the discriminator is non-constant over the data/generated manifolds and exhibits variance in its output over within P and/or Q θ , its unnormalized output distribution is spread out and not peaked.
...
Let p(y|l = 0) and p(y|l = 1) be two univariate Gaussian distributions with unit variance and means µ f and µ r , respectively, and where the indicator variable as values, l = 0 that corresponds to "fake" or generated and l = 1 that corresponds to real.
...
As the discriminator is really a classifier that in principle is trying to minimize the Bayes risk, under the above constraint, the bound is maximized when the means, µ f and µ r , are as far apart as possible.
...
y i } i drawn from a Gaussian with mean µ f and unit variance from a batch taken from the discriminator's output over fake samples.
...
Meanwhile, the original GAN formulation is only slightly perturbed, as the generator tries to match Q θ to P while the discriminator attempts to differentiate real and fake samples.
...
In practice, however, we find that driving the discriminator's output towards µ r using a least squares loss also works well: DISPLAYFORM5 Algorithm 1 Variance Regularizing Adversarial Learning (VRAL) using meta-discriminators.

Default parameter values are α = 10 −4 , β 1 = 0.5, β 2 = 0.999, = 3 × 10 −6 , m = 64.

n batches is determined by the chosen dataset.

Input: n F ,R,φ , the number of updates to perform on F, R and φ per update to G θ .1: while θ G not converged do

for i = 1 . . .

n batches do 3:for k = 1 . . .

n F ,R,φ do 4: DISPLAYFORM0 DISPLAYFORM1 10: DISPLAYFORM2 12: DISPLAYFORM3 13: DISPLAYFORM4 The training procedure for our proposed GAN with regularizing networks is similar to standard GAN training, however requiring each of the meta-discriminators to be updated prior to the discriminator update.
...
at values µ r and µ f , respectively.
...
This useful insight results in the modified objective in which the discriminator's output over fake examples is no longer fixed to a distribution centered at µ f , but can move around and preserves unit variance.

Consequently, each occurrence of {φ(x (i) )} i where x ∼ Q θ in equation 10 is replaced with {φ( DISPLAYFORM0 .
...
Consider a similar setup to Variance Regularizing GANs in which only one meta discriminator M is used, and its objective is DISPLAYFORM0 where µ * = K and σ * can vary, but is typically close to 0 (e.g. 0.1).
...
In essence, the single meta-discriminator M is distinguishing samples drawn from a Gaussian distribution fixed at µ * versus the slope of the hyperplane between x and y, where x and y are real and generated examples, respectively.

Linear Discriminant Analysis (LDA) is a commonly used technique for dimensionality reduction and/or binary classification.

Consider two sets of classes, P and Q θ , and a set of observations {φ(x)}, where each x ∈ X (i.e., φ(x) may be interpreted as the result of dimensionality reduction on x).

The objective is to preserve the discrepancy between classes while maintaining intra-class variance.

Let p φ (φ(x); x ∼ P) and p φ (φ(x); x ∼ Q θ ) be the probability densities of each class, assumed to follow a Gaussian distribution with means µ r and µ f and variances σ 2 r and σ 2 f , respectively.

For the sake of simplicity, assume σ 2 r = σ 2 f = 1.

The LDA classification rule is then to predict φ(x) belonging to the class of samples from Q θ if DISPLAYFORM0 otherwise P, and T is a chosen threshold.

Notice that when φ(x) is near µ r , the LHS of equation 14 becomes small (assuming φ(x) is far from µ f ) and vice-versa when φ(x) is close to µ f .McGan employs mean and covariance feature matching between P and Q θ .

To achieve this, a function Φ : X → R m , parametrized as a deep neural network, which maps x ∈ X to an embedding space is learned.

The objective is to minimize the mean and covariance between Φ(P) and Φ(Q θ ).
...
Interestingly, the discriminators in standard and Least Squares GANs exhibit almost no output variance within P or Q θ , suggesting the discriminator is constant over these regimes in image space and would emit zero gradients w.r.t.
...
Figure 4 indicates that as the discriminator achieves perfection by being updated more frequently than the generator, ||∇ x∼Q θ L φ || approaches zero.
...
Adhering to our earlier discussion of using a single meta-discriminator to force the discriminator to approximate a K-Lipschitz function, we trained a meta-discriminator M to perform batch-wise discrimination between samples drawn from N (y; µ * = 1, σ * = 0.1) and |φ(x)−φ(y)| d X (x,y) where x ∼ P, y ∼ Q θ .
