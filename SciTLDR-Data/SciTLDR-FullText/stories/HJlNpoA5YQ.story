...
Let S be a set, Σ be a σ-algebra, and ρ be a measure such that (S, Σ, ρ) constitutes a measure space.

Consider the set of square-integrable real-valued functions L 2 (S, Σ, ρ) = {f : S → R s.t.

S |f (u)| 2 dρ(u) < ∞}. When associated with the inner-product, DISPLAYFORM0 this set of functions forms a complete inner product Hilbert space BID8 Riesz, 1910) .
...
We denote H = L 2 (S, Σ, ρ) and additionally restrict ρ to be a probability measure, i.e. S 1 dρ(u) = 1.
...
Any eigenfunction with associated eigenvalue λ of the Laplacian is an eigenfunction with eigenvalue 1 − λ for D, and vice-versa.
...
1 The mapping φ : DISPLAYFORM2 ] then defines an embedding or representation of the space S.
...
In this section, we specify the meaning of the Laplacian in the RL setting (i.e., how to set ρ, D appropriately).
...
The agent's policy produces a probability distribution over possible actions π(a|s t ) from which it samples a specific action a t ∈ A to act on the environment.
...
We consider defining the Laplacian with respect to a fixed behavior policy π.

Then, the transition distributions P π (s t+1 |s t ) form a Markov chain.

We assume this Markov chain has a unique stationary distribution.

We now introduce a choice of ρ and D for the Laplacian in the RL setting.

We define ρ to be the stationary distribution of the Markov chain P π such that for any measurable U ⊂ S we have DISPLAYFORM0 As D(u, v) represents the pairwise affinity between two vertices u and v on the graph, it is natural to define D(u, v) in terms of the transition distribution.

DISPLAYFORM1 is the density function from a probability measure to ρ for all u. We define DISPLAYFORM2 which satisfies these conditions 3 .

In other words, the affinity between states u and v is the average of the two-way transition probabilities: If S is finite then the first term in FORMULA11 is P π (s t+1 = v|s t = u)/ρ(v) and the second term is P π (s t+1 = u|s t = v)/ρ(u).

Given this definition of the Laplacian, we now aim to learn the eigen-decomposition embedding φ.

In the model-free RL context, we have access to states and pairs of states (or sequences of states) only via sampling; i.e. we may sample states u from ρ(u) and pairs of u, v from ρ(u)P π (v|u).
...
The resulting penalized graph drawing objective is DISPLAYFORM3 where β is the penalty weight (KKT multiplier).
...
The attractive term minimizes the squared distance of embeddings of randomly sampled transitions experienced by the policy π, while the repulsive term repels the embeddings of states independently sampled from ρ.
...
We use the following evaluation protocol: (i) Given an embedding φ : S → R d , we first find its principal d-dimensional orthonormal basis h 1 , ..., h d , onto which we project all embeddings in order to satisfy the orthonormality constraint of the graph drawing objective; (ii) the evaluation metric is then computed as the value of the graph drawing objective using the projected embeddings.
...
If the goal space is the same as the state space, i.e. S = Z, the reward function can be defined as r t = − φ(s t+1 ) − φ(z g ) .

If S = Z we propose two options: (i) The first is to learn an embedding φ : Z → R d of the goal space and define r t = − φ(h(s t+1 )) − φ(z g ) . (ii) The second options is to learn an an embedding φ : S → R d of the state space and define DISPLAYFORM0 DISPLAYFORM1 , where h −1 (z) is defined as picking arbitrary state s (may not be unique) that achieves h(s) = z. We experiment with both options when S = Z.
...
Unlike the gridworld environments the (x, y) goal space is distinct from the state space, so we apply our two introduced methods to align the spaces: (i) learning φ to only embed the (x, y) coordinates of the state (mix) or (ii) learning φ to embed the full state (fullmix).
...
Since L = I − D if λ is an eigenvalue of D then 1 − λ is an eigenvalue of L. So we turn to discuss the existence of the largest d eigenvalues of D. According to our definition D is a compact self-adjoint linear operator on H. So it has the following properties according to the spectral theorem:• D has either (i) a finite set of eigenvalues or (ii) countably many eigenvalues {λ 1 , λ 2 , ...} and λ n → 0 if there are infinitely many.

All eigenvalues are real.• Any eigenvalue λ satisfies − D ≤ λ ≤ D where · is the operator norm.
...
To introduce a more general definition of D, we first introduce a generalized discounted transition distribution P π λ defined by DISPLAYFORM0 where λ ∈ [0, 1) is a discount factor, with λ = 0 corresponding to the one-step transition distribution P π 0 = P π .

Notice that P π λ (v|u) can be also written as P π λ (v|u) = E τ ∼q λ [P π (s t+τ = v|s t = u)] where q λ (τ ) = λ τ −1 − λ τ .

So sampling from P π λ (v|u) can be done by first sampling τ ∼ q λ then rolling out the Markov chain for τ steps starting from u.

Note that for readability we state the definition of P π λ in terms of discrete probability distributions but in general P π λ (·|u) are defined as a probability measure by stating the discounted sum (8) for any measurable set of states U ∈ Σ, U ⊂ S instead of a single state v.

Also notice that when λ > 0 sampling v from P π λ (v|u) required rolling out more than one steps from u (and can be arbitrarily long).
...
In practice, after sampling u = s t in a trajectory and τ from q λ (τ ) we discard this sample if t + τ > T .With the discounted transition, distributions now the generalized D is defined as DISPLAYFORM1 We assume that P π λ (·|u) is absolutely continuous to ρ for any u so that the Radon Nikodym derivatives are well defined.

This assumption is mild since it is saying that for any state v that is reachable from some state u under P π we have a positive probability to sample it from ρ, i.e. the behavior policy π is able to explore the whole state space (not necessarily efficiently).Proof of D(u, ·) being a density of some probability measure with respect to ρ.

We need to show that S D(u, v) dρ(v) = 1.

Let f (·|u) be the density function of P First notice that if ρ is the stationary distribution of P π it is also the stationary distribution of DISPLAYFORM2 (Property of the stationary distribution.)which means that g is the density function of ρ with respect to ρ.
...
Because proving D(u, ·) to be a density requires the fact that ρ is the stationary distribution of P π , the astute reader may suspect that sampling from the replay buffer will differ from the stationary distribution when the initial state distribution is highly concentrated, the mixing rate is slow, and the time horizon is short.

In this case, one can adjust the definition of the transition probabilities to better reflect what is happening in practice: Define a new transition distribution by adding a small probability to "reset":P π (·|u) = (1 − δ)P π (·|u) + δP 0 .
...
Then, ρ and D can be defined in the same way with respect toP π .

Now the replay buffer can be viewed as rolling out a single long trajectory withP π so that sampling from the replay buffer approximates sampling from the stationary distribution.
...
When δ (e.g. 1/T ) is small, the chance of sampling these "reset" pairs is very small, so our adjusted definition still approximately reflects what is being done in practice.

Hyperparameters For representation learning we use d = 20.

In the definition of D we use the discounted multi-step transitions (9) with λ = 0.9.

For the approximate graph drawing objective (6) we use β = 5.0 and δ jk = 0.05 (instead of 1) if j = k otherwise 0 to control the scale of L2 distances.
...
In the definition of D we use the discounted multi-step transitions (9) with λ = 0.99 for PointMass and λ = 0.999 for Ant.

For the approximate graph drawing objective (6) we use β = 2.0 and δ jk = 0.1 (instead of 1) if j = k otherwise 0 to control the scale of L2 distances.
...
For online training the agent moves faster in the maze during policy learning so we anneal the λ in D from its inital value to 0.95 towards the end of training with linear decay.
