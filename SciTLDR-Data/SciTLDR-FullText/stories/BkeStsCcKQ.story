...
Given the current weights w 0 of the network, we find the diagonal matrix Σ that minimizes: DISPLAYFORM1 where β is a parameter that controls the smoothness of the approximation.

Notice that L can be minimized efficiently using the method in BID14 .

To see how this relates to the Fisher Information Matrix, assume that L(w) can be approximated locally in DISPLAYFORM2 Taking the derivative with respect to Σ, and setting it to zero, we obtain Σ ii = β/H ii .

We can then use Σ to estimate the trace of the Hessian, and hence of the Fisher information.
