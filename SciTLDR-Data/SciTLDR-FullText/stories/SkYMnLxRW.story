...
For the Weighted Transformer, we propose a branched attention that modifies the entire attention layer in the Transformer network (including both the multi-head attention and the feed-forward network).The proposed attention layer can be mathematically described as: DISPLAYFORM0 DISPLAYFORM1 DISPLAYFORM2 where M denotes the total number of branches, κ i , α i ∈ R + are learned parameters and W Oi ∈ R dv×dmodel .
...
Further, we require that κ i = 1 and α i = 1 so that Equation FORMULA8 is a weighted sum of the individual branch attention values.
...
In the equations above, κ can be interpreted as a learned concatenation weight and α as the learned addition weight.

Indeed, κ scales the contribution of the various branches before α is used to sum them in a weighted fashion.
...
While it is possible that α and κ could be merged into one variable and trained, we found better training outcomes by separating them.

It also improves the interpretability of the models gives that (α, κ) can be thought of as probability masses on the various branches.
...
Instead of using (α, κ) learned weights, it is possible to also use a mixture-ofexperts normalization via a softmax layer .
...
Attention dropout randomly drops out elements BID31 from the softmax in (1).As in BID33 , we used the Adam optimizer BID18 with (β 1 , β 2 ) = (0.9, 0.98) and = 10 −9 .

We also use the learning rate warm-up strategy for Adam wherein the learning rate lr takes on the form: DISPLAYFORM0 for the all parameters except (α, κ) and DISPLAYFORM1 for (α, κ).This corresponds to the warm-up strategy used for the original Transformer network except that we use a larger peak learning rate for (α, κ) to compensate for their bounds.

Further, we found that freezing the weights (κ, α) in the last 10K iterations aids convergence.
...
Weighted Transformer 24.8 Train κ, α fixed to 1 24.5 Train α, κ fixed to 1 23.9 α, κ both fixed to 1 23.6 Without the simplex constraints 24.5 Table 3 : Model ablations of Weighted Transformer on the newstest2013 English-to-German task for configuration (C).

This shows that the learning both (α, κ) and retaining the simplex constraints are critical for its performance.
...
In Figure 3 , we present the behavior of the weights (α, κ) for the second encoder layer of the configuration (C) for the English-to-German newstest2013 task.
...
The results show that having both α and κ as learned, in conjunction with the simplex constraint, was necessary for improved performance.

Figure 3: Convergence of the (α, κ) weights for the second encoder layer of Configuration (C) for the English-to-German newstest2013 task.
...
Weights (α, κ) BLEU Learned 24.8 Random 21.1 Uniform 23.4 Table 4 : Performance of the architecture with random and uniform normalization weights on the newstest2013 English-to-German task for configuration (C).

This shows that the learned (α, κ) weights of the Weighted Transformer are crucial to its performance.
...
In order to better understand whether the network benefits from the learned weights or if, at test time, random or uniform weights suffice, we propose the following experiment: the weights for the Weighted Transformer, including (α, κ) are trained as before, but, during test time, we replace them with (a) randomly sampled weights, and (b) 1/M where M is the number of incoming branches.
