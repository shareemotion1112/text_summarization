...
See Sec. 3.4.The optimization is continuous over T AB and binary programming over α i,j .

Since this is computationally hard, we replace the binary constraint on α by the following relaxed version: DISPLAYFORM1 In order to enforce sparsity, we add an entropy constraint encouraging sparse solutions.

DISPLAYFORM2 The final optimization objective becomes: DISPLAYFORM3 The positivity α ≥ 0 and i α i,j = 1 constraints are enforced by using an auxiliary variable β and passing it through a Sof tmax function.
...
Since α is multiplied with all mapped examples T AB (x), it might appear that mapping must be performed on all x samples at every batch update.

We have however found that iteratively updating T AB for N epochs, and then updating β for N epochs (N = 10) achieves excellent results.

Denote the β (and α) updates-α iterations and the updates of T AB -T iterations.

The above training scheme requires the full mapping to be performed only once at the beginning of the α iteration (so once in 2N epochs).
...
Initially β are all set to 0 giving all matches equal likelihood.

We use an initial burn-in period of 200 epochs, during which δ = 0 to ensure that T AB and T BA align the distribution before aligning individual images.

We then optimize the examplar-loss for one α-iteration of 22 epochs, one T -iteration of 10 epochs and another α-iteration of 10 epochs (joint training of all losses did not yield improvements).
...
In all experiments the β parameters are shared between the two mapping directions, to let the two directions inform each other as to likelihood of matches.
...
Let us define the feature maps for images I 1 and I 2 as φ m 1 and φ m 2 (m is an index running over the feature maps).
...
The VGG features were subsampled as before due to the heavy computational cost.α iterations only: Train AN − GAN as described in Sec. 3.3 but with α iterations only, without iterating over T XY .AN − GAN : Train AN − GAN as described in Sec. 3.3 with both α and T XY iterations.

We evaluate our method on 4 public exact match datasets:Facades: 400 images of building facades aligned with segmentation maps of the buildings (Radim Tyleček, 2013).Maps: The Maps dataset was scraped from Google Maps by .
...
We also run the α iterations step on mapped source domain images and target images.

This method matched linear combinations of mapped images rather than a single image (the largest α component was selected as the match).

This method is less sensitive to outliers and uses the same β parameters for both sides of the match (A → B and B → A) to improve identification.
...
When optimized with the auxiliary losses, the exemplar loss was able to converge through α − T iterations.
...
In this case both the DiscoGAN map and DiscoGAN + α iterations present poor matches.
