...
This allows us to show convergence results for the gradient descent algorithm, enabling us to establish an upper bound on the number of iterations for finding an ε-approximate first-order optimal solution ( ∇f () ≤ ε).

Therefore our algorithm will generate an ε-approximate first-order optimal solution which satisfies aforementioned properties of the hidden layer.
...
We say that a collection of vectors, DISPLAYFORM1 Similarly, we say that collection of matrices, DISPLAYFORM2 A fully connected two-layer neural network has three parameters: hidden layer W , output layer θ and activation function h. For a given activation function, h, we define neural network function as DISPLAYFORM3 In the above equation, W ∈ R n×d is hidden layer matrix, θ ∈ R n is the output layer.
...
In Algorithm 1, we use techniques inspired from alternating minimization to minimize with respect to θ and W .

For minimization with respect to θ, we add gaussian noise to the gradient information.

This will be useful to prove convergence of this algorithm.

We use randomness in θ to ensure some "nice" properties of W which help us in proving that matrix D generated along the trajectory of Algorithm 1 is full column rank.
...
For each outer loop iteration, there is an inner loop which optimizes objective function with respect to θ using a stochastic gradient descent algorithm.

In the stochastic gradient descent, we generate a noisy estimated of ∇ θ f (W, θ) as explained below.

Let ξ ∈ R d be a vector whose elements are i.i.d.

Gaussian random variable with zero mean.

Then for a given value of W we define stochastic gradient w.r.t.

θ as follows: DISPLAYFORM9 (3.4)Then we know that DISPLAYFORM10 .

We can choose a constant σ > 0 such that following holds DISPLAYFORM11 Moreover, in the algorithm we consider a case where θ ∈ R. Note that R can be kept equal to R d but that will make parameter selection complicated.
...
Initialize N o to predefined iteration count for outer ietaration Initialize N i to predefined iteration count for inner iteration Begin outer iteration: DISPLAYFORM0 Notice that the problem of minimization with respect to θ is a convex minimization problem.
...
If it is costly to evaluate function values then we can implement the whole SGD for convex problem with respect to θ as specified in inner iteration of the algorithm above.
...
Now note that in the first-order condition, given in (3.3), row of matrix D are scaled by constant factors θ[j]'s, j ∈ [d].

Notice that we may assume θ[j] = 0 because otherwise there is no contribution of corresponding j-th row of W to the Problem (2.1) and we might as well drop it entirely from the optimization problem.
...
However, the analysis of Lemma 4.1 motivates the idea that stochastic noise of θ might help in obtaining the required properties of W and D along the trajectory of Algorithm 1.

In this section we first prove that by using random noise in stochastic gradient on θ gives a non-singular W k in every iteration.
...
More specifically, we have to show that σ min (D) decreases at the rate slower than the first-order convergence rate of the algorithm so that the overall algorithm converges to the global optimal solution.

Even if it is very difficult to prove such a result in theory, we think that such an assumption about σ min (D) is reasonable in practice.
...
Essentially ξ [k] contains the record of all random samples used until the k-th outer iteration in Algorithm 1 and ξ j [Ni] contains record of all random samples used in the inner iterations of j-th outer iteration.
...
We use techniques inspired from Lemma 4.1 but this time we use Lebesgue measure over Θ rather than data.

Over randomness of Θ, we can show that our algorithm will not produce any W such that corresponding matrix D is rank deficient.

Since Θ is essentially designed to be independent of data so we will not produce rank deficient D throughout the process of randomized algorithm.
...
Remark 5.8 Note that one can easily estimate value of L given data and θ.

Moreover, if we put constraints on θ 2 then L is constant in every iteration of the algorithm 1.

As mentioned in section 3, this will provide an easier way to analyze the algorithm.

Lemma 5.9 Assuming that scalar function h is such that |h(·)| ≤ u then there exists L θ s.t.

DISPLAYFORM9 Notice that Lemma 5.9 gives us value of L θ irrespective of value of W or data.
...
Theorem 5.10 Assume that stepsizes β i satisfy 0 < β i ≤ 1/2L θ , ∀ i ≥ 1.

Let {θ av i+1 } i≥1 be the sequence computed according to Algorithm 1.

Then we have, DISPLAYFORM11 5) DISPLAYFORM12 i where θ 1 is the starting point for inner iteration and σ is defined in (3.5).

Now we look at a possible strategy of selecting stepsize β i .
...
DISPLAYFORM2 8) where R/2 is the radius of origin centred ball, R in algorithm, defined as R : DISPLAYFORM3 In view of Theorem 5.11, we can derive a possible way of choosing γ k , σ and N i to obtain a convergence result.

More specifically, if DISPLAYFORM4 L and β k τ is chosen according to (5.6) then we have DISPLAYFORM5 Note that in the algorithm 1, we have proved that having a stochastic noise helps keeping matrix D full rank for all finite iterations.
...
So we need methods by which we can reduce the value of the estimate of L. One possible idea would be to use l 1 -ball for feasible region of θ.

More specifically, if R = B 1 (R/2) then we can possibly enforce sparsity on θ which will allow us to put better bound on L.
...
So by inductive hypothesis, rank of collection of matrices h(y i )y DISPLAYFORM8 2 then λ 2 = · · · = λ N = 0 with measure 1, then by (A.3) we have w 1 = 0 with measure 1, which is contradiction to the fact that w 1 = 0 with measure 1.

This gives us DISPLAYFORM9 Notice that (A.4) in its matrix form can be written as linear system DISPLAYFORM10 By (A.6), we have that vector of λ's lies in the null space of the matrix.
...
This implies last (2d − 2) equations then function must be dependent on each other: DISPLAYFORM18 for some fixed combination α j , β j .
...
Recall that gradient of f (W, θ) with respect to W can be written as DISPLAYFORM24 Notice that in effect, we are multiplying i-th row of the rank one matrix h (W u i )u i T by i-th element of vector θ.

So this can be rewritten as a matrix product DISPLAYFORM25 where Θ := diag{θ[i], i = 1, . . .

, d}. So iterative update of the algorithm can be given as DISPLAYFORM26 Notice that given ξ [k] , vector θ k+1 and corresponding diagonal matrix Θ k+1 are found by SGD in the inner loop so θ k+1 is a random vector.

More specifically, since {ξ DISPLAYFORM27 induces a Lebesgue measure on random variable DISPLAYFORM28 then W k is deterministic quantity.
...
DISPLAYFORM29 Now the last equation is of the form DISPLAYFORM30 Suppose we can find such θ with positive probability.

Then we can find hypercuboid Z := {x ∈ R d |a < x < b} such that any θ k+1 in given hypercuboid can solve equation (A.10).
...
Since measure on θ k+1 was induced by {ξ DISPLAYFORM32 A.4 PROOF OF LEMMA 5.2We use induction on d. For d = 1 this is trivially true.
...
Let us take v ∈ V and ε small enough such that v := v + εe 1 ∈ V. Correspondingly we have z i and z i .
...
Notice that since we are moving only one coordinate of random DISPLAYFORM41 ) (by ε incremental changes) keeping all other elements of v constant so we will have y i as constants which implies g i , G, G are constant.

So for the sake of simplicity of notation we define l : DISPLAYFORM42 Now, we look at the remaining part of two equation (A.12),(A.13): DISPLAYFORM43 which can be rewritten as DISPLAYFORM44 After (A.15) − (A.14), we have DISPLAYFORM45 Now note that (A.16), characterizes incremental changes in C, C, µ due to ε.

So taking the limit as ε → 0, we have DISPLAYFORM46 Here, last equation is due to product rule in calculus.
...
Let θ max := max DISPLAYFORM50 Suppose the Lipschitz constants for the first and second term are L i,L and L i,R respectively.
...
r 1 − r 2 2 , ∀ r 1 , r 2 because: h (r 1 x) − h (r 2 x) 2 ≤ L h r 1 x − r 2 x ≤ L h x 2 r 1 − r 2 2 From the relation above we have the following: Noting that DISPLAYFORM53 DISPLAYFORM54 we have DISPLAYFORM55 where u 1 and u 2 are upper bounds on scalar functions |h(·)| and |h (·)| respectively and d is rowdimension of W .A.7 PROOF OF THEOREM 5.11We know by Lemma 5.6 that f (·, θ) is a Lipschitz smooth function.

So using (5.7) we have DISPLAYFORM56 DISPLAYFORM57 (γ k − L/2γ 2 k ).

Now taking expectation with respect to ξ [No] (which is defined in (5.1)), we have DISPLAYFORM58 DISPLAYFORM59 .
