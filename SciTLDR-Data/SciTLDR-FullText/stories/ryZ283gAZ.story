...
Gastaldi (2017) introduced a stochastic affine combination of multiple branches in a residual block, which can be expressed as DISPLAYFORM0 where η ∼ U(0, 1).
...
We can write the forward propagation as DISPLAYFORM5 where P(η n = 1) = p n , P(η n = 0) = 1 − p n .
...
Runge-Kutta method is a set of higher order one step methods, which can be formulate aŝ DISPLAYFORM0 a ij f (û j , t n + c j ∆t), DISPLAYFORM1 b j f (û j , t n + c j ∆t).Here,û j is an intermediate approximation to the solution at time t n +c j ∆t, and the coefficients {c j } can be adjusted to achieve higher order accuracy.
...
The general form of a k−step linear multi-step method is given by where, α j , β j are scalar parameters and α 0 = 0, |α j | + |β j | = 0.

The linear multi-step method is explicit if β 0 = 0, which is what we used to design the linear multi-step structure.
