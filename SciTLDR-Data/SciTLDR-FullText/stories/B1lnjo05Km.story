...
Consider a given layer in an artificial neural network, and let V = {ν 1 , . . . , ν n } be the neurons in this layer.

These neurons can essentially be regarded as functions of inputs to the neural network, and map each input to an n dimensional vector ν( DISPLAYFORM0 T ∈ R n .

Typically, no particular structure is being directly imposed on the range of ν beyond general notions of bounded L p norm (e.g., Euclidean L 2 norm or L 1 norm as a proxy for sparsity).
...
Then, for any given input x to the network, we consider the activations [ν 1 (x), . . .

, ν n (x)] as a signal ν over this neuron graph.
...
Then, the eigenvectors ψ 0 , . . .

, ψ n−1 of L (indexed by convention in ascending order of the corresponding eigenvalues) are considered graph Fourier harmonics.

These can be shown to converge to discrete Fourier harmonics when considering a ring graph, in which case their associated eigenvalues, 0 = λ 0 ≤ · · · ≤ λ n−1 , correspond to squared frequencies.

This understanding gives rise to the graph Fourier transform defined as a linear transformation characterized by the matrix Ψ whose columns are the graph harmonics {ψ j } n−1 j=0 .

When applied to a neuron-activation signal ν in our setting we get its graph Fourier coefficients as ν = Ψ T ν, where ν is associated with the graph (squared) frequency λ j .

Similarly, the inverse Fourier transform is also defined via Ψ, as it can be verified that Ψ ν = ΨΨ T ν = ν, since the graph Laplacian (for all graphs considered in this work) yields a full orthonormal set of eigenvectors, and thus Ψ is an orthogonal matrix.

The graph Fourier transform allows us to consider the activations ν(x) for a given input x of the network in two domains.

First, in their original form, these activations form a signal over the neuron graph, which can essentially be considered as a function of individual neurons, with ν (x) [i] = ν i (x), i = 1, . . .

, n, which we refer to as the neuron-domain representation.

Alternatively, we can consider this signal in the spectral domain, via its Fourier coefficients, as a function ν (x) [j], j = 0, . . .
...
We note that one of the most popular regularization traditionally used in deep learning is the L 2 regularization, which essentially adds the squared Euclidean norm of the activations in the neuron domain (i.e., ν(x) 2 2 ) as another term in the main optimization target for gradient descent.
...
Since it can be verified that the graph Fourier transform is energy preserving (i.e., due to the orthonormality of graph harmonics), this regularization can equivalently be considered in the spectral domain by using ν(x) 2 2 instead of ν(x) 2 2 .

However, unlike the neuron domain, in the spectral domain we can associate with each element in ν(x) a frequencial interpretation given by {λ j } n j=0 , and therefore we can also generalize the spectral L 2 regularization to be weighted by functions of these squared-frequencies.

Namely, given weights µ j = µ(λ j ), j = 0, . . .

, n − 1, for some function µ, we define the spectral L 2 (µ) regularization as adding the term DISPLAYFORM1 where µ = diag(µ 0 , . . .

, µ n−1 ), to the optimization loss of the neural network.
...
Instead, to apply the spectral regularization in the neuron domain, we consider the matrix form on the RHS, and by combining it together with the definition of the graph Fourier transform we can write the L 2 (µ) spectral regularization term in the neuron domain as the quadratic form [ν(x)]

DISPLAYFORM2 T is a matrix that is independent of the network input x, and can thus be directly computed in advance from the neuron graph structure, and the predetermined spectral weights in µ. Finally, while in this work we focus on the L 2 (µ) form of spectral regularizations, in general other weighted norms (e.g., L 1 (µ) or more generally L p (µ), p ≥ 1) can also be used to define spectral regularizaions based on based on the harmonic structure induced by the neuron graph.

Laplacian smoothing regularization: we now focus on given class of L 2 (µ) spectral regularizations with nonnegative weights, i.e., µ ≥ 0.
...
A natural choice for such weights is to simply set them by the identity µ(λ j ) = λ j , which results in F(µ) =

L. We refer to this regularization as Laplacian smoothing, as the resulting quadratic loss term for it is [ν(x)]T Lν(x), and demonstrate its utility for producing interpretable latent representations in hidden layers of neural networks in Section 3.
...
Under this terminology, the graph Fourier transform is based on a dictionary {ψ j }j = 0 n consisting of the graph harmonics as atoms that are extremely local in frequency.
...
Let {φ ξ } ξ=1 ⊂ R n be such a dictionary, and let Φ be a n × matrix whose columns are the atoms in the dictionary.

We note that while in general, the atoms in the dictionary need not be orthonormal, we assume they are chosen such that the matrix Φ has a suitable pseudoinverse Φ † such that Φ † Φ = Id × .

Then, the best approximation of a neuron signal ν(x), for network input x, by a linear combination of dictionary atoms is given by ΦΦ † ν(x), which can be written directly as a linear combination ξ=1 ϕ ν(x) ξ φ ξ with the dictionary coefficients given by DISPLAYFORM0 The dictionary coefficients computed by ϕ provide a dictionary-based extension of the Fourier coefficients computed by the graph Fourier transform.
...
In particular, here we consider a bottleneck that forces the network to project its neuron activation signal ν(x) on a single dictionary atom, which would be chosen adaptively depending on the input x, and then only pass this atom to subsequent layer.

To achieve this bottleneck, we split the graph-structured layer into two parts ν and ν , before and after the bottleneck (correspondingly), such that DISPLAYFORM1 where softmax is defined as a function that maps vectors in R n to unit L 1 norm approximately one-hot vectors BID1 .

Therefore, previous layers in the network feed into the neuron graph signal ν, which is then passed through the bottleneck to produce a filtered signal ν based on approximately one atom out of the provided ones in the dictionary, and then the activations form this new signal are passed on to subsequent layers.
...
We can see with too much smoothing (α = 0.1) we get a less meaningful heatmap.

But more reasonable values of α result in activations of lower period both vertically and horizontally.
...
Let ν be the activation vector on the embedding layer, then we add a penalty term αν T Lν where α is a weighting hyperparameter to the standard mean squared error loss.

To build a spectral bottleneck layer let Φ be an n × matrix whose columns are the atoms in the dictionary.

Then we replace the embedding layer with a layer that computes Φsoftmax(Φ † ν).

Effectively, we transform the activation vector ν into the spectral domain, compute the softmax function on it, and restore the output to activations in the neuron domain.
