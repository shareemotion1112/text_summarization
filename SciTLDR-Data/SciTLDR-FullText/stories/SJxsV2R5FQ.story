...
Finally, A is a set of action templates DISPLAYFORM2 where Ψ is the space of executable control programs.

Each action template is a function parameterized by continuous parameters α i ∈ R di and a tuple of n i objects that the action operates on.

In this work, we assume that P, F and A are given.

A problem instance is characterized by I = (D, U), where D is a domain defined above and U ⊂ Υ is a finite universe of objects with |U| = N U .

For simplicity, we assume that, for a particular instance, the universe of objects remains constant over time.

In the problem instance I, we characterize a state s in terms of the concrete values of all properties in P on all objects in U; that is, DISPLAYFORM0 A problem instance induces the definition of its action space A, constructed by applying every action template A i ∈ A to all tuples of n i elements in U and all assignments α i to the continuous parameters; namely, DISPLAYFORM1
...
Here I N U is an identity matrix of size N U , and diag ([σ DISPLAYFORM2 represents a square diagonal matrix with σ i on the main diagonal, denoting the default variance for property P i if no rule applies.
...
Transition rule T = (A, Γ, ∆, φ θ , v default ) is characterized by an action template A, two ordered lists of deictic references Γ and ∆ of size N Γ and N ∆ , a predictor φ θ and the default variances DISPLAYFORM3 for each property P i under this rule.
...
Similarly for DISPLAYFORM8 , the first deictic reference selected o 3 and then γ2 is applied on o 3 to get o 1 .

The predictors φ θ are neural networks that map the fixed-length input to a fixed-length output, which is applied to a set of objects computed from a relational graph on all the objects, derived from the reference list DISPLAYFORM9 , to compute the whole next state s .

Because δ DISPLAYFORM10 θ is only predicting a single property, we use a "de-aggregator" function h to assign its prediction to both objects o 4 , o 6 .

based on previously designated objects.

In particular, Γ generates a list of objects whose properties affect the prediction made by the transition rule, while ∆ generates a list of objects whose properties are affected after taking an action specified by the action template A.We begin with the simple case in which every function returns a single object, then extend our definition to the case of sets.
...
So, reference γ 1 can only refer to the objects (O i ) n i=1 that are named in the action, and determines an object O n+1 .

Then, reference γ 2 can refer to objects named in the action or those that were determined by reference γ 1 , and so on.

When the function DISPLAYFORM13 ) ∈ Γ returns a set of objects rather than a single object, this process of adding more objects remains almost the same, except that the O t may denote sets of objects, and the functions that are applied to them must be able to operate on sets.
...
Then, by applying F =above starting from the initial set O 0 = {A}, we get an ordered list of sets of objects DISPLAYFORM14 Returning to the definition of a transition rule, we now can see informally that if the parameters of action template A are instantiated to actual objects in a problem instance, then Γ and ∆ can be used to determine lists of input and output objects (or sets of objects).

We can use these lists, finally, to construct input and output vectors.

The input vector x consists of the continuous action parameters α of action A and property P i (O t ) for all properties P i ∈ P and objects O t ∈ O NΓ that are selected by Γ in arbitrary but fixed order.
...
Similar for the desired output construction, we use the references in the list ∆, initializeÔ (0) = O (0) , and gradually add more objects to construct the output set of objectsÔ =Ô (N∆) .

The output vector is y = [P (ô)]ô ∈Ô,P ∈P where ifô is a set of objects, we apply a mean aggregator on the properties of all the objects inô.

The predictor φ θ is some functional form φ (such as a feed-forward neural network) with parameters (weights) θ that will take values x as input and predict a distribution for the output vector y.
...
A transition rule T = (A, Γ, ∆, φ θ , v default ) applies to a particular state-action (s, a) pair if a is an instance of A and if none of the elements of the input or output object lists is empty.

To construct the input (and output) list, we begin by assigning the actual objects o 1 , . . .

, o n to the object variables O 1 , . . .

, O n in action instance a, and then successively computing references γ i ∈ Γ based on the previously selected objects, applying the definition of the deictic reference F in each γ i to the actual values of the properties as specified in the state s.

If, at any point, a γ i ∈ Γ or δ i ∈ ∆ returns an empty set, then the transition rule does not apply.

If the rule does apply, and successfully selects input and output object lists, then the values of the input vector x can be extracted from s, and predictions are made on the mean and variance values Pr( DISPLAYFORM15 θ2 (x) be the vector entry corresponding to the predicted Gaussian parameters of property P i of j-th output object setô j and denote s[o, P i ] as the property P i of object o in state s, for all o ∈ U. The predicted distribution of the resulting state p(s | s, a; T ) is computed as follows: DISPLAYFORM16 where v i ∈ v default is the default variance of property P i in rule T .
...
We illustrate how we can use transition rules to build a SPARE in Fig. 2 .For each transition rule T k ∈ T and state s ∈ S, we assign the score function value to be 0 if T k does not apply to state s. Otherwise, we assign the total number of deictic references plus one, N Γ + N ∆ + 1, as the score.
...
First, we introduce our strategy for learning φ θ , which predicts a Gaussian distribution on y, given x. Then, we describe our algorithm for learning reference lists Γ and ∆ for a single transition rule, which enable the extraction of x and y from E. Finally, we present an EM method for learning multiple rules.

For a particular transition rule T with associated action template A, once Γ and ∆ have been specified, we can extract input and output features x and y from a given set of experience samples E. We would like to learn the transition rule's predictor φ θ to minimize Eq. (2).

Our predictor takes the form φ θ (x) = N (µ θ (x), Σ θ (x)) and a neural network is used to predict both the mean µ θ (x) and the diagonal variance Σ θ (x).
...
We use θ, v default ← LEARNDIST(D, E, Γ, ∆) to refer to this learning and optimization procedure for the predictor parameters and default variance.

Algorithm 1 Greedy procedure for constructing Γ. DISPLAYFORM0 train model using Γ 0 = ∅, save loss L 0

i ← 1 4: DISPLAYFORM0 for all γ ∈ R i do 7: DISPLAYFORM1 else breakIn the simple setting where only one transition rule T exists in our domain D, we show how to construct the input and output reference lists Γ and ∆ that will determine the vectors x and y. Suppose for now that ∆ and v default are fixed, and we wish to learn Γ. Our approach is to incrementally build up Γ by adding DISPLAYFORM2 ) tuples one at a time via a greedy selection procedure.

Specifically, let R i be the universe of possible γ i , split the experience samples E into a training set E train and a validation set E val , and initialize the list Γ to be Γ 0 = ∅. For each i, compute γ i = arg min γ∈Ri L(T γ ; D, E val ), where L in Eq. (2) evaluates a SPARE T γ with a single transition rule T = (A, Γ i−1 ∪ {γ}, ∆, φ θ , v default ), where θ and v default are computed using the LEARNDIST described in Section 4.1 2 .

If the value of the loss function L(T γi ; D, E val ) is less than the value of L(T γi−1 ; D, E val ), then we let Γ i = Γ i−1 ∪{γ i } and continue.

Else, we terminate the greedy selection process with Γ = Γ i−1 , since further growing the list of deictic references hurts the loss.

We also terminate the process when i exceeds some predetermined maximum allowed number of input deictic references, N Γ .

Pseudocode for this algorithm is provided in Algorithm 1.In our experiments we set ∆ = Γ and construct the lists of deictic references using a single pass of the greedy algorithm described above.
...
Alternatively, we could learn ∆ via an analogous greedy procedure nested around or, as a more efficient approach, interleaved with, the one for learning Γ.
...
Instead we first learn a single transition rule T = (A, Γ, ∆, φ θ , v default ) from E using the algorithm in Section 4.2, use the resulting Γ and ∆ to transform E into x and y, and then run k-means clustering on the concatenation of x, y, and values of the loss function when T is used to predict each of the samples.
...
Whereas a probabilistic transition rule has been defined as T = (A, Γ, ∆, φ θ , v default ), a mixture rule is T = (A, π Γ , π ∆ , Φ), where π Γ represents a distribution over all possible lists of input references Γ (and similarly for π ∆ and ∆), of which there are a finite number, since the set of available reference functions F is finite, and there is an upper bound N Γ on the maximum number of references Γ may contain.

For simplicity of terminology, we refer to each possible list of references Γ as a shell, so π Γ is a distribution over possible shells.

DISPLAYFORM2 is a collection of κ transition rules (i.e., predictors φ θ (k) , each with an associated Γ (k) , ∆ (k) , and v (k) default ).

To make predictions for a sample (s, a) using a mixture rule, predictions from each of the mixture rule's κ transition rules are combined according to the probabilities that π Γ and π ∆ assign to each transition rule's Γ (k) and ∆ (k) .

Rather than having our EM approach learn K transition rules, we instead learn K mixture rules, as the distributions π Γ and π ∆ allow for smoother sorting of experience samples into clusters corresponding to the different rules, in contrast to the discrete Γ and ∆ of regular transition rules.

As before, we focus on the case where for each mixture rule, DISPLAYFORM3 , and π Γ = π ∆ as well.

Our EM-like algorithm is then as follows:1.

For each j ∈ [K], initialize distributions π Γ = π ∆ for mixture rule T j as follows.
...
In the process of greedily assembling reference lists Γ = ∆, data likelihood loss function values are computed for multiple explored shells, in addition to the shell Γ = ∆ that was ultimately selected.

Initialize π Γ = π ∆ to distribute weight proportionally, ac- DISPLAYFORM5 , with the summation taken over all explored shells Γ, is a normalization factor so that the total weight assigned by π Γ to explored shells is 1− .

The remaining probability weight is distributed uniformly across unexplored shells.

, where we have dropped subscripting according to j for notational simplicity: DISPLAYFORM0 default ) using the procedure in Section 4.2 on the weighted experience samples E Zj , where we choose DISPLAYFORM1 to be the list of references with k-th highest weight according to π Γ = π ∆ .

(b) Update π Γ = π ∆ by redistributing weight among the top κ shells according to a voting procedure where each training sample "votes" for the shell whose predictor minimizes the validation loss for that sample.

In other words, the i-th experience sample DISPLAYFORM2 ).

Then, shell weights are assigned to be proportional to the sum of the sample weights (i.e., membership probability of belonging to this rule) of samples that voted for each particular shell: the number of votes received by the k-th shell is V (k) = |E| i=1 1 v(i)=k · z i,j , for indicator function 1 and k ∈ [κ].

Then, π Γ (k), the current k-th highest value of π Γ , is updated to become V (k)/ξ, where ξ is a normalization factor to ensure that π Γ remains a valid probability distribution. (Specifically, ξ = ( DISPLAYFORM3 Step 2a, in case the κ shells with highest π Γ values have changed, in preparation for using the mixture rule to make predictions in the next step.3.
...
In our domain D = (Υ, P, F, A), the object universe Υ is composed of blocks of different sizes and weight, the property set P includes shapes of the blocks (width, length, height) and the position of the block ((x, y, z) location relative to the table).

We have one action template, push(α, o), which pushes toward a target object o with parameters α = (x g , y g , z g , d), where (x g , y g , z g ) is the 3D position of the gripper before the push starts and d is the distance of the push.
...
Neural network (NN) We compare to a neural network function approximator that takes in as input the current state s ∈ R N P ×N U and action parameter α ∈ R N A , and outputs the next state s ∈ R N P ×N U .
...
Each node of the graph corresponds to an object in the scene, and the action α is concatenated to the state of each object.
