...
Then self-attention has been widely applied in seq-to-seq tasks such as machine translation, image generation (Yang et al. (2016); Zhang et al. (2018a) ) and graph-structured data (Veličković et al. (2017) ).
...
where the weights α T , α L and α M are trainable parameters.
...
where σ = σ(T,L,M) denotes the arbitrary arrangement of sequence (T,L,M), e.g., σ =(L,T,M) and σ(T) = L. Effectively, the arrangement σ is the order of attention maps to update V .
...
As shown in Fig. 5 , different measurements may exhibit different correlation, i.e., PM2.5 and PM 10 are significantly positively correlated (ρ PM2.5, PM 10 = 0.8278) while NO 2 and O 3 are negatively correlated (ρ NO2,O3 = −0.5117).
...
Burst loss simulation: As shown in Fig. 11 , we term the loss area marked as blue as the Burst loss area where for a certain camera, the data is continuously missed for a δ time slots.

After statistics and analysis, we found the the length of time slots 2 ≤ δ ≤ 134.

Then, for those time slots of burst loss, we calculate the mean µ = 6.350773 and standard deviation δ = 9.809643.
...
Optimizer We use the Adam optimizer Kingma & Ba (2014) while the initial learning rate in each epoch is set as lr(e) = r 0 × α ceil(max(0,e−d)/i) .
...
During training, we set (r 0 , α, d, batch size) as (0.008, 0.5, 40, 16).
