...
In order to find the most-likely failure, the reward function must be structured as follows: DISPLAYFORM0 where the parameters are:• α: A large number, to heavily penalize trajectories that do not end in the target set.

• βf (s): An optional heuristic.
...
• ηh(s): An optional training heuristic given at each timestep.
...
The environment action is δ F , the disturbance force applied to the cart at each time-step.

The reward function uses α = 1 × 10 4 , β = 1 × 10 3 , and f (s) as the normalized distance of the final state to failure states.
...
The reward function for this scenario uses α = −1 × 10 5 and β = −1 × 10 4 , with f (s) = DIST p v , p p as the distance between the pedestrian and the SUT at the end of a trajectory.
...
The reward function also uses g(a) = M (a, µ a | s), which is the Mahalanobis distance function BID8 .
...
The reward function for this scenario uses α = ∞ and no heuristics.
...
The state s = [x,ẋ, θ,θ] represents the cart's horizontal position and speed as well as the bar's angle and angular velocity.

The control policy, a neural network trained by TRPO, controls the horizontal force F applied to the cart.

The failure of the system is defined as |x| > x max or |θ| > θ max .
