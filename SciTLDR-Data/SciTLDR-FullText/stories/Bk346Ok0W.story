...
In all synthetic noise experiments, a Gaussian noise source with µ = 0 and σ between [0, 3] was added to zero-mean and unit-variance normalized samples during training.

We further refer to σ as the noise level.
...
With the noisy test set, each sensor's data is corrupted by unique random walk noise with a noise level between [0, 3σ] .
...
For regularization, Gaussian noise on the inputs (µ = 0, σ = 0.6), dropout (p=0.3) and weight decay (1e-4) were applied.
...
When removing sensors, we observed that the standard deviation of the merged representation increased with the number of removed sensors from around σ = 0.85 (all channels active) to σ = 1 (one channel active), which could push the network out of its preferred operating range and consequently cause the performance loss.
...
The noise model aims to have an approximately uniform coverage of noise level over a range [0, σ max ) and no settle-in time that could introduce a sequence length dependence on the noise.

The standard deviation of the noise σ for an input sequence of t timesteps can be calculated thusly: DISPLAYFORM0 with σ 0 drawn uniformly over the range [0, σ max /2) and n i drawn from a gamma distribution with shape k and scale θ.

The signum function extracts positive and negative signs from s i with equal probability.

A parameter search during our experiments yielded σ max = 3, k = 0.8 and θ = 0.2 as an appropriate set of parameters.

We define the reflection function φ(a, σ max ) as DISPLAYFORM1 where mod(a, 2σ max ) = a − 2σ max a/2σ max maintains the values within the desired range (0, 2σ max ) and the subsequent shift and magnitude operations map the values to the range [0, σ max ) while avoiding discontinuities.

Finally the input data x at feature index k and time index t is mixed with the noise sampled from a normal distribution as follows: DISPLAYFORM2 The reflection function φ(a, σ max ) performs similarly to the mod operator, but at the edges, produces a continuous reflection about the edge instead of a discontinuous wrap.

Therefore, this forms a constrained random walk, limited by σ max , which will become the standard deviation of normally distributed random noise added to the input x at feature index k and time point t. This noise model generates sequences that provide a useful training ground to tune the attentional mechanism of STAN models, as the noise level varies over time and allows periods of low noise (high attention desired) and high noise (low attention desired).
...
In (b), the random walk becomes bounded after applying the reflection operator φ in Eq. 4.
