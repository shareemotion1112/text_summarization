...
The main contributions of this work include: L Σ regularization, a novel approach penalizing the covariance between dimensions in a representation (see Section 2).
...
The experimental evaluation shows how different models can benefit from using L Σ regularization.

From autoencoders on synthetic data to deep convolutional autoencoders trained on CIFAR-10, we show that L Σ helps us learn uncorrelated and disentangled representations (see Section 3).
...
The covariance regularization term (L Σ ) for a layer, henceforth referred to as the coding layer, is computed as DISPLAYFORM0 where p is the dimensionality of the coding layer, DISPLAYFORM1 is the element wise L1 matrix norm of C, and C ∈ R p×p is the sample covariance of the activations in the coding layer over N examples DISPLAYFORM2 Further, H = [h 1 ; ...; h N ] is a matrix of all activations in the batch, 1 N is an N -dimensional column vector of ones, andh is the mean activation.

As L Σ has the structure of a regularizer, it can be applied to most gradient based models without changing the underlying architecture.

In particular, L Σ is simply computed based on select layers and added to the error function, e.g. Loss = Error + λL Σ

This section describes the experimental evaluation performed using L Σ regularization on different models in various settings, from simple multi-layer perceptron-based models using synthetic data (see Section 3.2 and 3.3) to convolutional autoencoders on real data (see Section 3.4).
...
The model is trained using the proposed covariance regularization L Σ on the coding layer.

The data is generated by sampling a d = 4 dimensional vector of independent features z ∼ N (0, Σ), where Σ ∈ R d×d is constrained to be non-degenerate and diagonal.

However, before the data is fed to the autoencoder it is pushed through a random linear transformation x = Ωz.
...
9 experiments were performed with different values for the regularization constant λ.

The first point on each curve (in Figure 2 and 3) is λ = 0, i.e. no regularization, followed by 8 points logarithmically spaced between 0.001 and Figure 2 : In this figure we compare the amount of residual linear correlation after training the model with L Σ and L 1 regularization respectively, measured in MAPC (left) and CVR (right).

The first point on each curve corresponds to λ = 0, i.e. no regularization, followed by 8 points logarithmically spaced between 0.001 and 1.

All scores are averaged over 10 experiments using a different random projection (Ω).Figure 3: The resulting dimensionality the coding layer after training the model with L Σ and L 1 regularization respectively, measured in TdV (left) and UD 90% (right).

The first point on each curve corresponds to λ = 0, i.e. no regularization, followed by 8 points logarithmically spaced between 0.001 and 1.All scores are averaged over 10 experiments using a different random projection (Ω).1.

Each experiment is repeated 10 times using a different random projection Ω and the average is reported.
...
The first thing to notice is that L Σ consistently lead to lower correlation while incurring less MSE penalty compared to L 1 .

Further, looking at the MAPC it is interesting to notice that it is optimal for a very small values of L Σ .

This is because higher amounts of L Σ leads to lowering of the dimensionality of the data, see Figure 3 , which in turn yields unpredictable Pearson correlation scores between these inactivated neurons.

However, this effect is compensated for in CVR for which L Σ quickly converges towards the optimal value of one, which in turn indicates no presence of linear correlation.

Turning the attention to dimensionality reduction, Figure 3 shows that L Σ consistently outperform L 1 .

Further, looking closer at the TdV score, L Σ is able to compress the data almost perfectly, i.e. TdV=1, at a very small MSE cost while L 1 struggle even when accepting a much higher MSE cost.

Further, the UD 90% scores again show that L Σ achieves a higher compression at lower MSE cost.

In this instance the underlying data was of 4 dimensions which L Σ quickly achieves.

At higher amounts of L Σ the dimensionality even locationally fall to 3, however, this is because the threshold is set to 90%.
...
The model was trained on XOR examples, e.g. [1,0]=1, in a random order until convergence with L Σ applied to both hidden layers and added to the cost function after scaling it with λ = 0.2.
...
For comparison, see Figure 5 for the same model trained without L Σ .

Figure 4: Covariance matrix (left) and spectrum (right) of the hidden layers of a feed forward neural network trained with L Σ regularization to solve the XOR problem.
...
We compare the results from using L Σ regularization with L1 regularization and with no regularization at all.
...
The regularization parameter λ was chosen to be 0.08, for a reasonable trade-off between performance and covariance/variance ratio.
...
The results (see Table 1 ) show that the high-level features become more disentangled and has a lower CVR (6.56) using L Σ regularization.

Without regularization, the score is 20.00, and with L1 regularization the score is 4.03.

The model with L Σ regularization obtains a reconstruction error (MSE) of 0.0398, roughly the same as without regularization (0.0365), both of which are much better than using L1 regularization, with an MSE of 0.0569.

Figure 6 shows the CVR score plotted against the MSE, illustrating that the L Σ technique leads to more disentangled representations while retaining a better MSE score.

As you increase the regularization factor both L Σ regularization pushes down the CVR quickly, while retaining an MSE error that is almost constant.
...
The UD 90% results show that L Σ encourages representations that concentrate the variation, and the model constantly learns representations with lower UD 90% score than using L1.

With λ > 0.08, the MSE, the CV R, and the UD 90% all becomes much worse when using L1 regularization, while the L Σ seems to continue smoothly to improve CV R and UD 90% , as the MSE starts to grow.
...
The authors trained one policy π i (a|s) for each dimension i of the representation, such that the policy can interact with the environment and learn how to modify the input in a way that modifies the representation only at dimension i, without changing any other dimensions.
...
In this paper, we have presented L Σ regularization, a novel regularization scheme based on penalizing the covariance between dimensions of the internal representation learned in a hierarchical model.
