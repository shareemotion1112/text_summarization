...
The embeddings are defined asẑ = f θ (x) for a deep neural network f parameterized by weights θ.

Instead of learning to predict the target y, we assume that there exists an estimate of similarity between targets, d(y, y ).

The task of the network is then to learn this similarity by attempting to match ẑ −ẑ with d(y, y ) under some predefined norm in the embedding space.
...
We next describe the loss function, L(θ), that we minimize in order to train our CNN model.
...
τ z and τ y are normalizing scale factors estimated Note that while L cls performs best on supervised classification, L sim allows for better retrieval performance, however this is degraded unless L KL is also included to regularize towards binary embeddings.
...
We use a slowly decaying form for the weight, DISPLAYFORM1 with parameter values γ = 0.1 and ρ = 2.

Our empirical loss for minimizing the KL divergence KL(p||q) .

= dzp(z) log(p(z)/q(z)) between the sample embedding distribution p(z) and a target distribution q(z) is based on the Kozachenko-Leonenko estimator of entropy BID7 , and can be defined as DISPLAYFORM0 where ν(ẑ b ; z) denotes the distance ofẑ b to a nearest vector z b , where z is a sample (of e.g. size B) of vectors from a target distribution.

We employ the beta distribution with parameters α = β = 0.1 as this target distribution, which is thus moderately concentrated to binary values in the embedding space.
