...
The authors proposed to use a vanilla RNN with a multilayer perceptron to define the feature vector h i : DISPLAYFORM2 with DISPLAYFORM3 and σ is the sigmoid function, A, B, U, V are the weight parameters to learn.
...
The authors proposed the following transfer function: DISPLAYFORM3 where η ij act as edge gates, and are computed by: DISPLAYFORM4 These gated edges are very similar in spirit to the Tree-LSTM proposed in BID31 .
...
We leverage the vanilla graph ConvNet architecture of BID30 , Eq. FORMULA13 , and the edge gating mechanism of Marcheggiani & Titov (2017), Eq.(8) , by considering the following model: DISPLAYFORM0 where h =0 i = x i , ∀i, and the edge gates η ij are defined in Eq. (9).
