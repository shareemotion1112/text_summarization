...
Given the vehicle model from BID14 , a human driver model represented by ACT-R BID17 a set of initial conditions S, and a temporal logic formula ϕ [3], we are interested in (1) verification: computing the probability that the Human-Vehicle model satisfies ϕ in S, i.e, P S (ϕ); and (2) synthesis: designing an ADAS that optimizes the probability of satisfying ϕ by the Human-Vehicle-ADAS system in S, i.e., P S (ϕ) with ∈ {max, min}.

To verify the human driver model under ϕ, we first abstract it to a Markov Chain M h = (S, P, s 0 , AP, L), where S is a finite set of states, P : S × S → [0, 1] is a transition probability function, s 0 ∈ S is the initial state, AP is a set of atomic propositions, and L : S → 2 AP is a labeling function.
...
We define M h which unifies both modules through the use of µ ∈ {1, 2}, where µ = 1 corresponds to the control step and µ = 2 to the decision making and monitoring stage.

A state s ∈ S is a tuple s = (µ, x, λ, a, v, t), where x is bounded to a finite length of the road according to the situation, v is the speed of the vehicle, a is the acceleration and λ represents the index of the lane of the ego, abstracting away the y variable which reduces the size of the model.
...
The optimal ADAS design is reduced to finding an optimal policy over M ADAS for a certain specification given as a temporal logic formula ϕ defined over AP .
...
Intuitively, ϕ 1 refers to how unsafe the system is, while ϕ 2 corresponds to the time efficiency of it.

FIG1 shows an example of a trajectory under ϕ 1 for a given highly unsafe initial situation S, in which the ADAS effectively leads the system to a safer situation.

Fig. 3 showcases the difference in probabilities of satisfying (a) ϕ 1 and (b) ϕ 2 , assuming each specification to be optimized individually.

In both cases, all randomly generated scenarios tested lead to a decrease in the case of ϕ 1 and an increase in the case of ϕ 2 , i.e. improved satisfaction of the desired properties.
...
FIG2 presents the Pareto frontier of optimizing ϕ 1 and ϕ 2 for a given S in a multi-objective setting, showing that, as expected, there is a trade-off between the two properties.
