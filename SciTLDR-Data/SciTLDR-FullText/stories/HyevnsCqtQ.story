...
The winner activation after the pruning mask, A m , obeys the rule: DISPLAYFORM1 where θ is the threshold derived at run-time from the activation winner rate for each layer, and A orig is the result from original activation function.
...
The solution of determining threshold θ in Equation (2) for activation masks is actually a canonical argpartion problem to find top-k arguments in an array.
...
An alternate threshold θ is predicted by selecting top-αk elements from the down-sampled activation set comprising αN elements with α as the down-sampling rate.

θ is applied for the original activation set afterwards.
...
By selecting a proper static threshold θ in Equation (2), more activations can be pruned with little impact on model accuracy.
...
For the static pruning setup, the θ for leaky ReLU is assigned in the range of [0.07, 0.14], which brings different activation sparsity patterns.

40% 45% 50% 55% 60% 65% Non-zero activation percentage As the result of leaky ReLU with static threshold shown in FIG5 , the accuracy starts to drop rapidly when non-zero activation percentage is less than 58.6% (θ = 0.08).
...
From the overall results, it's practical to down-sample 10% (α = 0.1) of activations by keeping the accuracy drop less than 0.5%.
