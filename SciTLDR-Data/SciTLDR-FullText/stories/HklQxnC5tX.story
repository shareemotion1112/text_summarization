...
In contrast, recent works of BID13 Hamilton et al. (2017) ; BID1 have adopted the approach of defining the embeddings as a function of node attributes F := f θ (X, A) and solving the optimization problem DISPLAYFORM2 where f θ is defined by a neural network.
...
DISPLAYFORM0 whereÂ =D DISPLAYFORM1 2 is the normalized adjacency matrix,Ã = A + I, andD the corresponding degree matrix.
...
Bernoulli-Poisson link, defined as ξ(X uv ) = 1 − exp(−X uv ), is a common probabilistic model for overlapping community detection BID37 BID28 .
...
This is an unrealistic assumption, which can be easily fixed by adding a small offset ε > 0, that is ξ(X uv ) = 1 − exp(−X uv − ε).
...
Since a non-negative F implies that the interactions between every pair of nodes X uv are at least 0, the edge probability under the sigmoid model is always above σ(0) = 0.5.

This can be fixed by introducing an offset: g(X uv ) = σ(X uv − b).
...
To summarize, we use stochastic gradient descent to optimize the objective DISPLAYFORM1 where the parameters θ are the weights of the neural network, Datasets.
...
We assign node u to community c if F uc is above a threshold ρ.

We set ρ = 0.4 for DOC-BP and ρ = 0.2 for DOC-Sigmoid, as these are the values that achieve the best performance on the Facebook-1912 dataset.
...
Once the parameters θ are learned, we perform a forward pass of each model using the full adjacency and attribute matrix.
...
We set the hyperparameters to α = 1, β = 2, κ = 5, as recommended in the paper, and run optimization for 20 iterations.• We use the Python implementation of LDense provided by the authors (https:// research.cs.aalto.fi/dmg/software.shtml), and run the algorithm with the recommended parameter settings.
