...
However, the post-softmax predictions have lost some relative uncertainties that are more informative, thus a temperature parameter τ is used to smooth predictions P T and P S to be P τ T and P τ S , which are denoted as soft predictions: DISPLAYFORM0 Then, consider P τ T as the target, knowledge distillation optimizes the following loss function DISPLAYFORM1 wherein W cls S is the parameter of the student network, and y cls is the identity label.

For simplicity, we omit min and the number of samples N , and denote the upper right symbol cls as the classification task.

In addition, H(, ) is the cross-entropy, thus the first term is the softmax loss, while the second one is the cross-entropy between the soft predictions of the teacher and student network, with α balancing between the two terms.

This multi-task training is advantageous because the target P τ T cannot be guaranteed to be always correct, and if the target is not confident, the identity label y cls will take over the training of the student work.
...
One problem is that non-classification tasks have their own task-specific targets, but given the additional soft predictions P τ T , which one should we use?
...
Φ (W S , y) is the task-specific loss function, and Ψ (K S , K T ) is the task-specific distillation term with the targets selected as K T and K S in teacher and student networks.

Besides, α and β are the balancing terms between classification and non-classification tasks.

In Eqn.(5), the above problem has become how to set α and β for a given non-classification task.
...
In distillation, except for the available soft predictions P τ T (classification target), another one is the task-specific target that can be the hidden layer K T BID15 , and it satisfies R T = f c (K T ) with f c being a fully-connected mapping.
...
Therefore, in face alignment, the hidden layer is preferred for distillation, which gives Eqn.(7) by setting α < β, as shown in FIG0 .
...
Besides, λ controls the margin between positive and negative pairs.

Similar to face alignment, we consider the hidden layer K T and soft prediction P τ T as two possible targets in distillation.
...
Therefore, in face verification, the soft prediction is preferred for distillation, which gives the following loss function by setting α > β, as shown in FIG0 .
...
As analyzed above, α and β should be set differently in the distillation of different tasks.

The key is to measure the relevance of objectives between classification and non-classification tasks.

For a given task, if it is highly related to the classification task, then α > β is necessary, or α < β should be set.
...
Besides, in distillation, student networks are trained with the targets of the teacher network generated online, and the temperature τ and margin λ are set to be 3 and 0.4 by cross-validation.

Finally, the balancing terms α and β have many possible combinations, and we show later how to set them by an experimental trick.

(1)Scratch: student networks are not initialized; (2)P retrain: student networks are trained with the task-specific initialization; (3)Distill: student networks are initialized with W cls S ; (4)Sof t: the soft prediction P τ T ; (5)Hidden: the hidden layer K T .
...
One important thing is how to set α and β in Eqn.(7).

As there are many possible combinations, we use a simple trick by measuring their individual influence and discard the target with the negative impact by setting α = 0 or β = 0; while if they both have positive impacts, α > 0, β > 0 should be set to keep both targets in distillation.

As shown in Table.

3, when the initializations of P retrain and Distill are used, α = 1, β = 0(soft prediction) always decreases performance, while α = 0, β = 1(hidden layer) gets consistent improvements, which implies that the hidden layer is preferred in the distillation of face alignment.
...
Besides, the highest distillation performance 3.21% is obtained with Distill and α = 0, β = 1, and it can be competitive to the one of the teacher network(3.02%).

In this part, we give the evaluation of distillation in face verification.

Similar to alignment, we select α and β in the same way.
...
It can be observed that no matter which student network or initialization is used, α = 0, β = 1(hidden layer) always decreases the baseline performance, while α = 1, β = 0(soft prediction) remains almost the same.

As a result, we discard the hidden layer and only use the soft prediction.

One interesting observation in Table.

4 is that α = 0, β = 0 always obtains the best performance, and the targets do not work at all.
...
As a result, α = 1, β = 0 can get the best performance, which is even much higher than the teacher network, e.g., 79.96% of ResNet-50/2 with Distill and α = 1, β = 0.
