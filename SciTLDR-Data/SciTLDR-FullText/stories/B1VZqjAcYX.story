...
Apart from weight pruning, there are approaches focused on structured simplification such as pruning filters BID24 ; BID28 ), structured sparsity with regularizers BID39 ), low-rank approximation BID16 ), matrix and tensor factorization BID31 ), and sparsification using expander graphs BID33 ) or Erdős-Rényi random graph BID26 ).
...
Hence, the effect of connection j on the loss can be written as: DISPLAYFORM3 (5) In fact, ∂L/∂c j is an infinitesimal version of ∆L j , that measures the rate of change of L with respect to an infinitesimal change in c j from 1 → 1 − δ.

This can be computed efficiently in one forward-backward pass using automatic differentiation, for all j at once.

Notice, this formulation can be viewed as perturbing the weight w j by a multiplicative factor δ and measuring the change in loss.
...
Based on this hypothesis, we define connection sensitivity as the normalized magnitude of the derivatives: DISPLAYFORM4 Once the sensitivity is computed, only the top-κ connections are retained, where κ denotes the desired number of non-zero weights.

Precisely, the indicator variables c are set as follows: DISPLAYFORM5 wheres κ is the κ-th largest element in the vector s and 1[·] is the indicator function.

Here, for exactly κ connections to be retained, ties can be broken arbitrarily.

We would like to clarify that the above criterion FORMULA8 ) is different from the criteria used in early works by BID29 DISPLAYFORM6 Pruning: choose top-κ connections 6: w * ← arg min w∈R m L(c w; D) Regular training 7: w * ← c w * connection sensitivity.
...
This means that their saliency criteria (i.e. −∂L/∂w or −∂L/∂α; α refers to the connectivity of neurons), in fact, depend on the loss value before pruning, which in turn, require the network to be pre-trained and iterative optimization cycles to ensure minimal loss in performance.
...
Figure 1: Test errors of LeNets pruned at varying sparsity levelsκ, whereκ = 0 refers to the reference network trained without pruning.
...
Experiment setup For brevity, we define the sparsity level to beκ = (m − κ)/m · 100 (%), where m is the total number of parameters and κ is the desired number of non-zero weights.

For a given sparsity levelκ, the sensitivity scores are computed using a batch of 100 and 128 examples for MNIST and CIFAR experiments, respectively.
...
We prune the LeNets for different sparsity levelsκ and report the performance in error on the MNIST image classification task.

We run the experiment 20 times for eachκ by changing random seeds for dataset and network initialization.

The results are reported in Figure 1 .The pruned sparse LeNet-300-100 achieves performances similar to the reference (κ = 0), only with negligible loss atκ = 90.
...
These approaches typically perform many pruning operations even if the network is well pretrained, and require additional hyperparameters (e.g., pruning frequency in BID9 , annealing schedule in Carreira-Perpiñán & Idelbayev FORMULA1 ).

Some methods augment the training objective to handle pruning together with training, increasing the complexity of the algorithm (e.g., augmented Lagrangian in Carreira-Perpiñán & Idelbayev (2018), variational inference in BID27 ).
...
Each column represents the visualizations for a particular class obtained using a batch of 100 examples with varying levels of sparsityκ, from 10 (top) to 90 (bottom).
...
As the sparsity increases, the parameters connected to the discriminative part of the image for classification survive and the irrelevant parts get pruned.(i.e., digits for MNIST and fashion items for Fashion-MNIST) with varying sparsity levelsκ.
...
We test with to visualize the remaining parameters, and set the sparsity levelκ = 90.
...
We set the sparsity levelκ = 90, and train with Adam optimizer BID18 ) with learning rate of 0.001 without weight decay.
...
Specifically, we train LeNet-5-Caffe for both the reference model and pruned model (withκ = 99) on MNIST with either true or randomly shuffled labels.
...
We provide a further analysis on the effect of varyingκ in Appendix B.
...
Furthermore, using ∂L/∂w results in different pruning patterns between the original and inverted data in some cases (e.g., the 2 nd columns between (c) and ( Figure 7: The effect of varying sparsity levels (κ).

The lowerκ becomes, the lower training loss is recorded, meaning that a network with more parameters is more vulnerable to fitting random labels.
