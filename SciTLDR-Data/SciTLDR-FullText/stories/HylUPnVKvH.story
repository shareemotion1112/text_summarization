...
Modifying the spatial size of an input to a convolutional layer by a factor γ, will yield an output with size scaled by the same factor γ.
...
We introduce an exponentially moving weighted average of the gradients' normḡ t (scalar) which is updated according toḡ t = αḡ t−1 + (1 − α)g t where
