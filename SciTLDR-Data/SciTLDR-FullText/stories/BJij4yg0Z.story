...
For simplicity we first consider a classification model M with a single parameter ω, training inputs x and training labels y.

We can infer a posterior probability distribution over the parameter by applying Bayes theorem, DISPLAYFORM0 The likelihood, P (y|ω, DISPLAYFORM1 , where H(ω; M ) = − i ln (P (y i |ω, x i ; M )) denotes the cross-entropy of unique categorical labels.

We typically use a Gaussian prior, P (ω; M ) = λ/2πe−λω 2 /2 , and therefore the posterior probability density of the parameter given the training data, P (ω|y, x; M ) ∝ λ/2πe −C(ω;M ) , where C(ω; M ) = H(ω; M ) + λω 2 /2 denotes the L2 regularized cross entropy, or "cost function", and λ is the regularization coefficient.

The value ω 0 which minimizes the cost function lies at the maximum of this posterior.

To predict an unknown label y t of a new input x t , we should compute the integral, P (y t |x t , y, x; M ) = dω P (y t |ω, x t ; M )P (ω|y, x; M ) (2) DISPLAYFORM2 However these integrals are dominated by the region near ω 0 , and since P (y t |ω, x t ; M ) is smooth we usually approximate P (y t |x t , x, y; M ) ≈ P (y t |ω 0 , x t ; M ).

Having minimized C(ω; M ) to find ω 0 , we now wish to compare two different models and select the best one.
...
Since this integral is dominated by the region near the minimum ω 0 , we can estimate the evidence by Taylor expanding DISPLAYFORM5 Within this "Laplace" approximation, the evidence is controlled by the value of the cost function at the minimum, and by the logarithm of the ratio of the curvature about this minimum compared to the regularization constant.

Thus far we have considered models of a single parameter; in realistic models with many parameters DISPLAYFORM6 ω0 , where |∇∇C(ω)| ω0 is the determinant of the Hessian, and p denotes the number of model parameters BID17 .

The determinant of the Hessian is simply the product of its eigenvalues, ( p i=1 λ i ), and thus, DISPLAYFORM7 The contribution (λ DISPLAYFORM8 ω0 ) is often called the "Occam factor", because it enforces Occam's razor; when two models describe the data equally well, the simpler model is usually better BID12 .
...
Since this fraction is always less than one, we propose to approximate equation 9 away from local minima by only performing the summation over eigenvalues λ i ≥ λ.
...
Clearly, we should only assign any confidence to the predictions of our model if E(ω 0 ) < 0.The evidence supports the intuition that broad minima generalize better than sharp minima, but unlike the curvature it does not depend on the model parameterization.
...
Since Occam's factor arises from the log ratio, ln (λ i /λ), these two effects cancel out 2 .
...
Unsurprisingly, the overall model performance also improves.b) The best observed batch size is proportional to the size of the training set once N 20000.where is the learning rate, N the training set size, DISPLAYFORM1 dCi dω the true gradient, and DISPLAYFORM2 dCi dω the estimated gradient evaluated on a mini-batch.
...
We adopt the central limit theorem and model the gradient error α = ( dĈ dω − dC dω ) with Gaussian random noise (We discuss this approximation briefly in appendix C).

It is easy to show that α = 0, while α 2 = N ( DISPLAYFORM4 To continue, we interpret equation 11 as the discrete update of a stochastic differential equation BID21 BID9 , DISPLAYFORM5 Where t is a continuous variable, η(t) represents noise, η(t) = 0 and η(t)η(t ) = gF (ω)δ(t−t ).
...
A similar analysis of the SGD was recently performed by BID23 , although their treatment only holds near local minima where the covariances F (ω) are stationary.
...
Instead of minimizing the cost function, Bayesian usually prefer to sample parameter values from the posterior (MacKay, 1992), DISPLAYFORM0 where C(ω; M ) is the regularized summed cost function, as shown in section 2 of the main text.

It is well known that one can sample this posterior by simulating the overdamped Langevin equation BID9 , which is described by the stochastic differential equation, DISPLAYFORM1 where t is a continuous variable, and η(t) describes Gaussian noise with mean η(t) = 0 and variance η(t)η(t ) = 2T Iδ(t−t ).
...
Indeed, if the gradient covariances F (ω) were stationary and proportional to the identity, then the SGD would integrate an overdamped Langevin equation with temperature proportional to the SGD noise scale g. As t → ∞, the probability of sampling any particular parameter vector ω from the Langevin equation, P (ω, t → ∞) ∝ e −C/T .We obtain posterior samples if T = 1.

In order to draw posterior samples in practice, we repeatedly integrate the Langevin equation (at temperature T = 1), over a finite step t → t + /N , DISPLAYFORM2 where α denotes a Gaussian random variable with mean α = 0 and variance α 2 = 2 I/N , which introduces isotropic noise to the gradient update as described in section 4 of the main text.

Note that, since C(ω; M ) denotes the summed cost function, we chose to scale our step size by the training set size N .
...
Since the probability of sampling any given parameter vector ω is proportional to the posterior, the probability of sampling a parameter vector belonging to any given local minimum is proportional to the integral of the posterior over the bowl of attraction D which surrounds that minimum.
...
This demonstrates that bayesian posterior samples are biased in favor of local minima whose evidence is large, which explains why a single posterior sample ω p often achieves lower test error than the cost function minimum ω 0 .

In the experiments of section 4 of the main text, the L2 regularization coefficient λ = 0.

In figure 8 , we plot the evolution of the training curves when λ = 0.1, for both small batch and full batch training.
...
In section 5 of the main text, we approximated the difference between the full batch gradient and the mini-batch gradient estimate, α = ( DISPLAYFORM0 , by a Gaussian random variable.
...
Momentum simulates a generalized Langevin equation (with structured fluctuations), DISPLAYFORM0 λ is the "damping coefficient" and η(t) describes Gaussian noise, whose statistics η(t) = 0 and η(t)η(t ) = gλF (w)δ(t − t ).

As before, the coefficient g describes the scale of random fluctuations in the dynamics, and F (ω) describes the gradient covariances between parameters.

We include a factor of λ in the noise variance to satisfy the fluctuation-dissipation theorem, which states that we can vary the damping coefficient without changing the probability of sampling any particular configuration of parameters in the limit t → ∞, if we proportionally increase the noise variance.
