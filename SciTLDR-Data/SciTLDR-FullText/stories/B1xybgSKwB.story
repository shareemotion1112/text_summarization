...
The vector α t contains the normalized attention weights for the prediction at timestep t and sums to 1.
...
We use a weighted sequential cross-entropy as the loss function over the class-wise model predictions f θ,c , writing τ (o, a) the subtrajectory of τ ending with the observation-action couple (o, a) to translate the effect of the binary mask:
...
The reward shaping theorem states that if there exists a function φ such that F : (s, a, s ) → γφ(s ) − φ(s), then M and M admit the same set of optimal policies.

φ is called a potential function.
...
Computing the potential function We define the redistributed return R ← τ of a trajectory τ as:

where α i←j is the attention weight on (o i , a i ) when predicting the reward r j and s i are environment states.
...
In the reward shaping formalism, the potential function φ depends only on the state.

To stay within its bounds, we define φ as the forwarded redistributed return.
...
For those states, we set to potential to 0, which results in a −φ(s) additional reward when transitioning to those from the state s.
...
To do so, we introduce a simple heuristic to binarize the attention scalars: we consider all values above a threshold α to correspond to events to be credited.
...
All agents mentioned in that section are PPO learners with a learning rate of 0.00019, an entropy coefficient of 0.0011, 12 actors, a discount factor γ = 0.99.
