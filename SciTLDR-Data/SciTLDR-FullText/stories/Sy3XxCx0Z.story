...
The hidden states generated by these two LSTMs at each time step are concatenated to represent that time step and its context: DISPLAYFORM2 The hidden states of the unidirectional LSTM (h → t or h ← t ) is calculated as follows: DISPLAYFORM3 DISPLAYFORM4 DISPLAYFORM5 DISPLAYFORM6 DISPLAYFORM7 DISPLAYFORM8 where σ is the sigmoid function, is the element-wise multiplication of two vectors.
...
Here we use F (r ij ) = λ1(r ij ), where λ is a hyper-parameter tuned on the development set and 1 is the indication function.1(r ij ) = 1 if r ij is not zero vector ; 0 if r ij is zero vector .Intuitively, the word pairs with semantic relationship in various features are probably aligned together.
...
DISPLAYFORM1 , a DISPLAYFORM2 DISPLAYFORM3 where α ∈ R M ×N and β ∈ R M ×N are the normalized attention weight matrices with respect to the 2-axis and 1-axis.
...
The trade-off λ for calculating co-attention in Equation FORMULA10 is selected in [0.1, 0.2, 0.5, 1, 2, 5, 10, 20, 50] based on the development set.
...
Meanwhile, for less training data, λ is usually set to a larger value.

For example, the optimal λ tuned on the development set is 20 for 0.8% training set, 2 for the 4% training set, 1 for the 20% training set and 0.2 for the 100% training set.
