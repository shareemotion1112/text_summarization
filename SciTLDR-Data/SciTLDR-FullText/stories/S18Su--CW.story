...
Assume that we have a logistic regressor with weight matrix w. Consider an image x ∈ R n which is perturbed into x = x + η by some noise η such that η ∞ ≤ ε for some ε.

The probability that the model assigns to the true class is equal to: DISPLAYFORM0 If the perturbation η is adversarial, such as in the case where DISPLAYFORM1 , then the input to the sigmoid is increased by ε · n. If n is large, as is typically the case in images and other highdimensional spaces of interest, this linearity implies that even imperceptibly small values of ε can have a large effect on the model's prediction, making the model vulnerable to adversarial attacks.
...
Comparison of regular inputs, quantized inputs, and discretized inputs (16 levels, projected to one dimension) on MNIST, adversarially trained with ε = 0.3.
...
Quantized inputs are also affected approximately linearly by perturbations where ε is greater than the bucket width.
...
When starting at the most common pixel-values for MNIST, 0 and 1, any perturbation of the pixels (where ε ≤ 0.3) has barely any effect on the input to the network.
...
For a real number θ ∈ [0, 1] define b(θ) to be the largest index α ∈ {1, . . .

, k} such that θ ≤ b α .

For an index j ∈ {1, . . . , k} let χ(j) ∈ R k be the indicator or one-hot vector of j, i.e., DISPLAYFORM0 The discretization function is defined pixel-wise for a pixel i ∈ {1, . . .
...
, k}, let τ (j) ∈ R k be the thermometer vector defined as DISPLAYFORM0 Then the discretization function f is defined pixel-wise for a pixel i ∈ {1, . . .
...
For ease of presentation, we will describe the attacks assuming that f : R → R k discretizes inputs into thermometer encodings; in order to attack one-hot encodings, simply replace all instances of f therm with f onehot , τ with χ, and C with the identity function I. We represent the adversarial image after t steps of the attack as z t , where the value of the ith pixel is z t i .
...
Following PGD BID13 , we initialize DGA by placing each pixel into a random bucket that is within ε of the pixel's true value.

At each step of the attack, we look at all buckets that are within ε of the true value, and select the bucket that is likely to do the most 'harm', as estimated by the gradient of setting that bucket's indicator variable to 1, with respect to the model's loss at the previous step.
...
At each step of a multi-step attack, we anneal this value via exponential decay with rate δ.

DISPLAYFORM0 We initialize each of the logits randomly with values sampled from a standard normal distribution.

At each step, we ensure that the model does not assign any probability to buckets which are not within ε of the true value by fixing the logits to be −∞. The model's loss is a continuous function of the logits, so we can simply utilize attacks designed for continuous-valued inputs, in this case PGA with step-size ξ.
...
We found that in all cases, LS-PGA was strictly more powerful than DGA, so all attacks on discretized models use LS-PGA with ξ = 0.01, δ = 1.2, and 1 random restart.

To be consistent with BID13 , we describe attacks in terms of the maximum ∞ -norm of the attack, ε.

All MNIST experiments used ε = 0.3 and 40 steps for iterative attacks; experiments on CIFAR used ε = 0.031 and 7 steps for iterative attacks; experiments on SVHN used ε = 0.047 and 10 steps for iterative attacks.
...
For CIFAR-10 and CIFAR-100 we use the Momentum optimizer with momentum 0.9, 2 weight decay of λ = 0.0005 and an initial learning rate of 0.1 which is annealed by a factor of 0.2 after epochs 60, 120 and 160 respectively as in Zagoruyko & Komodakis (2016) .
...
DISPLAYFORM0 Algorithm 1: Sub-routine for getting an ε-discretized mask of an image.

In this section we list the additional experiments we performed using discretized models on MNIST.The main hyperparameters of Algorithm 3 are the step size ξ used to perform the projected gradient ascent, and the annealing rate of δ.

We found that the choice of these hyperparameters was not critical to the robustness of the model.

In particular, we performed experiments with ξ = 1.0 and ξ = 0.001, and both achieved similar accuracies as in TAB2 .

Additionally, we found that without annealing, i.e., δ = 1.0, the performance was only slightly worse than with δ = 1.2.We also experimented with discretizing by using percentile information per color channel instead of using uniformly distributed buckets.

This did not result in any significant changes in robustness or accuracy for the MNIST dataset.

Input: Image x, label y, discretization function f , loss L(θ θ θ, f (x), y), l attack steps, parameter ε Output: Adversarial input to the network TAB7 : Comparison of adversarial robustness to white-box attacks on MNIST using 16 levels and with various choices of the hyperparameters ξ and δ for Algorithm 3.
...
We choose ξ = 0.01 and δ = 1.2 for the LS-PGA attack hyperparameters.
...
In FIG6 , we plot the error for different models on various values of ε.
...
The ε bound for both axes is [−1.0, 1.0].
