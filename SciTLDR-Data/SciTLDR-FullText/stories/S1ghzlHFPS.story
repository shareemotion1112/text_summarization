...
s i is used, for each event type k ∈ K, to define some time-varying intensity function λ k : (t i−1 , ∞) → R ≥0 .
...
As our baseline method, we take the neural Hawkes process (Mei & Eisner, 2017) to be our method for computing s i and defining the intensity function λ k from it.
...
2 Under an inhomogenous Poisson process, disjoint intervals generate events independently, and the number of events on the interval (a, b] is Poisson-distributed with mean b a λ k (t) dt.

Thus, on a sufficiently narrow interval (t, t + dt], the probability of a single event is approximately λ k (t) dt and the probability of more than one event is approximately 0, with an error of O(dt 2 ) in both cases.
...
where Ψ is inspired by the structure of an LSTM, the affect parameters are given by matrix U and the event embeddings w k , and the depend parameters are given by matrix V.
...
where σ(·) is the sigmoid function, r ranges over all the rules and r depend(k, b) means "the rule r proves the fact depend(k, b)".
...
Having equations (13) and (16), we pass ψ 0,b,k + ψ 1,b,k through the activation functions and obtain the updated h b,new .
...
for the node blocks that each k depends on in order to compute log λ ki (t i ) and the Monte Carlo approximation to
...
The time t i has density p i (t) = λ(t) exp − t ti−1 λ(s)ds where λ(t) = k∈K λ k (t), and we choose ∞ ti−1 tp i (t)dt as the time prediction because it has the lowest expected L 2 loss.

Given the next event time t i , the most likely type would simply be arg max k λ k (t i ), but the most likely next event type without knowledge of t i is arg max k
...
Given t i , we draw k i from the distribution where the probability of each type k is proportional to λ k (t i ).
...
Then equations (24)- (25) define c 1 (from c 0 def = 0), c 1 , δ 1 , and o 1 .

This is the initial configuration of the system as it waits for the first event to happen: this initial configuration determines the hidden state h(t) and the intensity functions λ k (t) over t ∈ (0, t 1 ].
...
Equation (10) falls back to f k (v k σ(Aσ(BCh global (t)))) which is not exactly the same with, yet at least as expressive as equation (1).
...
It gives an unbiased estimate of the total intensity k∈K λ k (t), yet remains much less computationally expensive especially when J |K|.
...
As future work, we will explore sampling from proposal distributions where the probability of choosing any k is (perhaps trained to be) proportional to its actual intensity λ k (t), in order to further reduce the variance.
...
How do we construct the upper bound λ * (line 8 of Algorithm 2)?

We express the upper bound as λ * = k∈K λ * k and find λ * k ≥ λ k (t) for each k. We copy the formulation of λ k (t) here for easy reference:

where each summand g dd h bd (t) = g dd ·o id ·(2σ(2c d (t))− 1) is upper-bounded by max c∈{c id ,c id } g dd · o id · (2σ(2c) − 1).
