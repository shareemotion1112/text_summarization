...
We consider the meta-learning scenario where we have a distribution over tasks p(T ), and a model f parametrized by θ, that must learn to adapt to tasks T i sampled from p(T ).
...
By gradient-based meta-learning, we imply that f is trained using information about the gradient of a certain loss function L(f (D i ; θ)) on the tasks.

Throughout this work the loss function is the cross-entropy between the predicted and true class.

MAML learns an initial set of parameters θ such that on average, given a new task T order approximation of MAML, where these second-order derivatives are ommited, and we refer to that other algorithm as First-Order MAML.
...
(1): Curvature of the loss for an adapted meta-test solutionθi (for a task Ti), is measured as the spectral norm of the hessian matrix of the loss.
...
A direction vector is obtained by dividing a trajectory displacement vector (from meta-train solution θ s to meta-test solutioñ
...
This accuracy is computed after f , starting from a given meta-training parametrization θ s , has optimized its parameters to the task T , we consider the optimization landscapes L(f (D i ; θ)), and 1) the properties of these loss landscapes evaluated at the solutionsθ test i

; 2) the adaptation trajectories when f , starting from θ s , adapts to those solutions; as well as 3) the properties of those landscapes evaluated at the meta-train solutions θ s .

See Figure 1 for a visualization of our different metrics.

We follow the evolution of the metrics as meta-training progresses: after each epoch, which results in a different parametrization θ s , we adapt f to several meta-test tasks, compute the metrics averaged over those tasks, and compare with
...
We start our analysis of the objective loss landscapes by measuring properties of the landscapes at the adapted meta-test solutionsθ i .
...
After s meta-training iterations, we have a model f parametrized by θ s .

During the meta-test, f must adapt to several meta-test tasks T i independently.

For a given T i , f adapts by performing a few steps of full-batch gradient descent on the objective landscape L(f (D i ; θ) ), using the set of support samples D i , and reaches an adapted solutionθ i .

Here we are interested in the curvature of L(f (D i ;θ i )), that is, the objective landscape when evaluated at such solution, and whether on average, flatter solutions favour better generalization.

Considering the hessian matrix of this loss w.r.t the model parameters, defined as

, we measure the curvature of the loss surface aroundθ i using the spectral norm · σ of this hessian matrix:

as illustrated in Figure 1 (

We define the average loss curvature for meta-test solutionsθ i , obtained from a meta-train solution θ s , as:

Note that we do not measure curvature of the loss at θ s , since θ s is not a point of convergence of f for the meta-test tasks.

In fact, at θ s , since the model has not been adapted to the unseen meta-test classes, the target accuracy for the meta-test tasks is random chance on average.

Thus, measuring the curvature of the meta-test support loss at θ s does not relate to the notion of flatness of minima.

Instead, in this work we characterize the meta-train solution θ s by measuring the average inner product between the meta-test gradients, as explained later in Section 4.3.
...
Let's consider a model f adapting to a task T i by starting from θ s , moving in parameter space by performing T steps of full-batch gradient descent with ∇ θ L(f (D i ; θ)) until reachingθ i .

We define the adaptation trajectory to a task T i starting from θ s as the sequence of iterates (θ s , θ

i , ...,θ i ).

To simplify the analyses and alleviate some of the challenges in dealing with trajectories of multiple steps in a parameter space of very high dimension, we define the trajectory displacement vector (θ i − θ s ).

We define a trajectory direction vector θ i as the unit vector:

We define a metric for the coherence of adaptation trajectories to meta-test tasks T i , starting from a meta-train solution θ s , as the average inner product between their direction vectors:

The inner product between two meta-test trajectory direction vectors is illustrated in Figure 1 (2).

In addition to characterizing the adaptation trajectories at meta-test time, we characterize the objective landscapes at the meta-train solutions θ s .
...
Note that the learning rate α is constant and is the same for all experiments on a same dataset.
...
The resulting metric is thus the average inner product between meta-test gradients evaluated at θ s .

We define the average inner product between meta-test gradient vectors g i , evaluated at a meta-train solution θ s , as:

The inner product between two meta-test gradients, evaluated at θ s , is illustrated in Figure 1 (3).
...
After each training epoch, we compute E[ H θ (D i ;θ i ) σ ] using a fixed set of 60 randomly sampled meta-test tasks T i .

Across all settings, we observe that MAML first finds sharper solutionsθ i until reaching a peak, then as the number of epoch grows, those solutions become flatter, as seen in Figure  2 .
...
We perform the same analysis for our finetuning baseline (Figures 4a, 4c) , with results suggesting that flatness of solutions might be more linked with E[L(f (D i ;θ i ))], the average level of support loss attained by the solutionsθ i (see Figures 4b and 3b) , which is not an indicator for generalization.

We also noted that across all settings involving MAML and First-Order MAML, this average meta-test support loss E[L(f (D i ;θ i ))] decreases monotonically as meta-training progresses.
...
We also perform the analysis on the Finetuning baselines, which reach much lower target accuracies, and where we see that E[ θ T i θ j ] remains much closer to zero, meaning that trajectory directions are roughly orthogonal to each other, akin to random vectors in high dimension (see Figure 6a) .

As an added observation, here we include our experimental results on the average meta-test trajectory norm E[ θ i − θ

Despite the clear correlation between E[ θ T i θ j ] and generalization for the settings that we show in Figure 5 and 11, we observed that for some other settings, this relationship appears less linear.
...
Here we present our results on the characterization of the objective landscapes at the meta-train solutions θ s , by measuring the average inner product between meta-test gradient vectors g i .
...
Based on our observations on the coherence of adaptation trajectories, we take first steps in this direction by adding a regularization term based on E[ θ T i θ j ] .

Within a meta-training iteration, we first let f adapt to the n training tasks T i following Eq 1.

We then compute the average direction vector θ µ = 1 n n i=1 θ i .

For each task, we want to reduce the angle defined by θ T i θ µ , and thus introduce the penalty on T i θ µ , obtaining the regularized solutionsθ i .

The outer loop gradients are then computed, just like in MAML following Eq 2, but using these regularized solutionsθ i instead ofθ i .
...
Compute the corrected inner loop updates: 7: for all T i do 8:θ i =θ i −γ∇ θ Ω(θ) where Ω(θ) = − θ T i θ µ 9: end for 10: Perform the meta-update as in Eq. 2, but using the corrected solutions:
...
During meta-training, the inner loop updates are performed via five steps of full batch gradient descent (except for Section 5.3 where T = 1), with a fixed learning rate α of 0.1 for Omniglot and 0.01 for MiniImagenet, while ADAM is used as the optimizer for the meta-update, without any learning rate scheduling, using a meta-learning rate β of 0.001.
