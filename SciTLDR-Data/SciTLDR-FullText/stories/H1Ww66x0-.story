...
We assume that the learner made a mistake if y t =ŷ t whereŷ t is the predicted label.
...
Let K : X × X → R (kernel on input space) and Ω : N × N → R (output kernel) be symmetric, positive semi-definite (p.s.d) multitask kernel functions and denote H as their corresponding RKHS of functions with the norm · H on multitask examples BID26 ; BID12 BID14 ).
...
To account for the examples from the different tasks, we consider both the kernel on the input space K and the output kernel Ω in our classification function.

DISPLAYFORM0 We set α s = y s .

The predicted label for a new example is computed from the linear combination of the labels of the examples from the support set S weighted by their input similarity K and the task similarity Ω to the new example.

Using the kernel trick, one can write: DISPLAYFORM1 Note that, in the above representation, we need to learn both the support set S and the output kernel Ω from the data.
...
Our objective function for the lifelong learning problem is given as follows: DISPLAYFORM0 where (·) is some loss function such as hinge loss or logistic loss, R(·) is the regularization on the task relationship matrix Ω and λ is the regularization parameter.
...
Consider the dual function of the above equation, at time-step t (see BID4 ; BID14 ): DISPLAYFORM1 When we consider the entry-wise l p norm between Ω and Ω (t−1) from the previous iteration as our regularization i.e., R(Ω, DISPLAYFORM2 we get the update function in Equation 2.

Similarly, if we consider the generalized KL-divergence between Ω and Ω (t−1) i.e., R(Ω, FIG1 ) itk , we get the update function in Equation 3.

Unlike in the previous work, we update only the row (and the corresponding column) of the task relationship matrix Ω specific to the task i t , which significantly reduces the time taken per example.

DISPLAYFORM3 We can see that the update equations are simple and quite intuitive.

For a given new example (x t , i t ) at round t, the algorithm updates Ω itk (for some k ∈ [K]) by computing the similarity between the new example and the examples in the support set S that belongs to the task k. If the two examples have similar (different) labels and high similarity K(x t , x s ), then the Ω itk is increased to reflect the positive (negative) correlation and vice versa.

A value close to 0 implies no significant relationship between the tasks.

The update to the Ω itk is normalized by the regularization parameter λ for scaling.

It is worth noting that our update equations do not violate the p.s.d constraints on Ω in Equation 5.If Ω from the previous iteration is a p.s.d matrix and the update is a p.s.d matrix (as it is computed using the Gram matrix of the example from the previous iteration), the sum and Hadamard product of two p.s.d matrices satisfy the p.s.d constraint (using the Schur Product Theorem).Algorithm 1 outlines the key steps in our proposed method.

We write f ((x t , i t )) as f it (x t ) for notational convenience.

At each time-step t, the learner receives an example x t ∈ X and predicts the output label y t usingŷ t = sign(f it (x t )).

We update both the support set S and the output kernel Ω it· when the learner makes a mistake.
...
In Algorithm 1, we can see that both the classification function f and the update equations for Ω use the support set S. When the target function changes over time, the support set S grows unboundedly.
...
Next, we implemented two online multitask learning related to our approach: FOML -initializes Ω with fixed weights BID7 , Online Multitask Relationship Learning (OMTRL) BID24 -learns a task covariance matrix along with task parameters.
