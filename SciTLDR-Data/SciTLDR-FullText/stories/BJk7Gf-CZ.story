...
Given a matrix A, let σ max (A) and σ min (A) denote the largest and smallest singular values of A, respectively.
...
Notice also that the product DISPLAYFORM2 LetÛ ∈ R dy×k be a matrix consisting of the first k columns of U .
...
Then the optimal solution to (2) is DISPLAYFORM3 which is the orthogonal projection of Y X T (XX T ) −1 onto the column space ofÛ .
...
Recall that when we take the SVD Y X T (XX T ) −1 X = U ΣV T ,Û ∈ R dy×k is defined to be a matrix consisting of the first k columns of U .
...
Using Fréchet derivatives of the population risk with respect to each function h i , they showed that when all h i 's are close to identity, any critical point of the population risk is a global minimum.
...
Given a function f , let J[f ](x) be the Jacobian matrix of function f evaluated at DISPLAYFORM0 is a linear functional that maps a function (direction) η ∈ F i to a real number (directional derivative).
...
To emphasize that the Fréchet derivative D hi [L(h)] is a linear functional that outputs a real number, we will write DISPLAYFORM0 This notation also helps avoiding confusion coming from multiple parentheses and square brackets.
...
By definition of Fréchet derivatives, we have DISPLAYFORM0 Therefore, DISPLAYFORM1 This equation (6) will be used in the proof of Theorems 4.1 and 4.2.A.3 PROOF OF THEOREM 4.1 DISPLAYFORM2 .

Since A(X) has full row rank by assumption, A(X)A(X) T is invertible.

Then define a particular directioñ DISPLAYFORM3 .

It remains to check ifη ∈ F 1 .

It is easily checked thatη(0) = 0 because h H+1:1 (0) − h * (0) = 0.
...
Also, h H+1:1 − h * ∈ F, so we can conclude thatη is differentiable.

Moreover, if we decompose A(X) with SVD, A(X) = U ΣV T , Σ is of the form Σ = [Σ 1 0] and DISPLAYFORM4 from which we can see that DISPLAYFORM5 by our assumption.

Note that, for any X ∈ R dx , DISPLAYFORM6 Since this holds for any X, we have DISPLAYFORM7 which ensures thatη ∈ F 1 .
...
Then definẽ DISPLAYFORM1 We need to check ifη ∈ F j .

It is easily checked thatη(0) = 0.
...
Hence, we can conclude thatη is differentiable.
...
Since d i ≥ d y for all i, we can check that h H+1 • · · · • h 1 = h * , and h i ∈ F i for all i. Moreover, for all z ∈ R d1 , J[h H+1:2 ](z) is all 0 except 1's in diagonal entries, so σ min (J[h H+1:2 ](z)) ≥ 1 and h H+1:2 (z) is twice-differentiable.(Theorem 4.2) It is given that we have j ∈ {1, . . .
...
Now, (2) becomes a problem of minimizing RX − Y X T (XX T ) −1 X 2 F subject to the rank constraint rank(R) ≤ k. The optimal solution for this is obtained when RX is the k-rank approximation of Y X T (XX T ) −1 X. Then, k-rank approximation of Y X T (XX T ) −1 X can be expressed asÛÛ T Y X T (XX T ) −1 X, whereÛ is unique due to our assumption that all singular values are distinct.
