...
Let m Φ (x) = Φ(x)dx be an antiderivative of the standard normal CDF.

Then the solution to equation 2 is given by, DISPLAYFORM6 where σ = σ 2 1 + σ 2 2 , soft(x) = log(1 + exp(x)) is the softplus function, the antiderivative of the logistic sigmoid, and ρ = σ 1.702 .Proof.

The first line is proved in Appendix A, the second approximation follows from the approximation of Φ by a logistic sigmoid given in BID3 .Note that, in the zero-temperature limit, as ρ goes to zero, we recover the formula DISPLAYFORM7 with equality in the last line because (a, b) and (c, d) are intervals.
...
This is true for both the exact formula using Φ(x)dx, and the softplus approximation.

Unfortunately, for any ρ > 0, multiplication of Gaussian-smoothed indicators does not give a valid meet operation on a function lattice, for the simple reason that f 2 = f , except in the case of indicator functions, violating the idempotency requirement of Section 3.1.More importantly, for practical considerations, if we are to treat the outputs of p φ as probabilities, the consequence is DISPLAYFORM8 which complicates our applications that train on conditional probabilities.
...
To evaluate equation 8, recall the identity BID8 BID21 DISPLAYFORM2 For convenience, let τ : DISPLAYFORM3 .

Applying Fubini's theorem and using equation 9, we have DISPLAYFORM4 and therefore, with σ = τ −1 , DISPLAYFORM5
