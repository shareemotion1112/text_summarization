...
Let G be the generator implemented as a neural network with parameters θ, and let E be a pretrained neural network with L hidden layers.
...
In practice, we train G by sampling mini-batches of true data and generated (fake) data and optimizing the parameters θ using stochastic gradient descent (SGD) with backpropagation.
...
Let E and D be the encoder and the decoder networks with parameters φ and ψ, respectively.
...
1 show GFMN performance in terms of Inception Score (IS) and Fréchet Inception Distance (FID) BID15 for CIFAR10 in the case where the (DCGAN-like) encoder is used as feature extractor.
...
We evaluate our models using two quantitative metrics: Inception Score (IS) and Fréchet Inception Distance (FID) BID15 .
