...
for L in layers 2 to N : P L ←P returnŷ Capsule Layers: The primary capsules P 1 are computed by applying a convolution layer and Layer Normalization to the backbone feature F. The non-primary capsules layers P 2:N are initialized to be zeros 1 .
...
The goal is to obtain the predicted class logitsŷ from the last capsule layer (the class capsules) P N .
...
We update the parameters θ, W 1:N −1 by stochastic gradient descent.
