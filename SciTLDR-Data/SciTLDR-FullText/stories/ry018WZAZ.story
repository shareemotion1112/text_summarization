...
2 gives a(1− )(1−δ) 2 -approximation guarantee for (6), where δ = max e∈S k({e})/K.Proof sketch: The criterion (8) we use guarantees that each update we make is reasonably good.
...
In a practical label acquisition process, the budget we set for each round is usually much larger than the length of the longest sentence in the unlabeled set, making δ negligible.

In our experiments, δ was around 0.01.
