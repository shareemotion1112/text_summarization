...
This backtracking model p(s t , a t |s t+1 ), is trained to predict, given a state s t+1 , which state s t the agent visited before s t+1 and what action a t ∼ π was performed in s t to reach s t+1 .
...
A backtracking model aims to augment the experience of the trajectory τ leading to a high-value state by generating other possible tracesτ that could have also caused it.
...
We consider a Markov decision process (MDP) defined by the tuple (S, A, P, r, γ), where the state space S and the action space A may be discrete or continuous.
...
The environment emits a bounded reward r : S × A → [r min , r max ] on each transition and γ ∈ (0, 1) is the discount factor.

Let π denote a stochastic policy over actions given states, and let R(π) = E π T t=0 γ t r(s t ) denote the expected total return when policy π is followed.

The standard objective in reinforcement learning is to maximize the discounted total return R(π).

Throughout the text we will refer to experienced trajectories as τ = (s 1 , a 1 , · · · , s T , a T ) and we will refer to simulated experiences as tracesτ .

We introduce the backtracking model B φ = q φ (s t , a t |s t+1 ), which is a density estimator of the joint probability distribution over the previous (s t , a t )-tuple parameterized by φ.

This distribution is produced by both a learned backward policy π b = q(a t |s t+1 ) and a state generator q(s t |a t , s t+1 ).
...
With these models, we may decompose q φ (s t , a t |s t+1 ) as q(s t |a t , s t+1 )q(a t |s t+1 ).However, for training stability with continuous-valued states, we model the density of state variation ∆s t = s t − s t+1 rather than the raw s t .

Therefore, our density models are given by q φ (∆ t , a t |s t+1 ) = q(∆s t |a t , s t+1 )q(a t |s t+1 ).Note that for readability, we will drop the φ-subscript unless it is necessary for clarity.
...
These generated transitions are then stored as a potential traceτ which terminates at some final state.

The backtracking model B φ is learned by maximum likelihood, using the policy's trajectories as observations, as described in Section 3.1.Producing Intended High Value States.
...
Training q(τ ) by minimizing the KL(q(τ )||p(τ |R > L)) term is hard due to the direction of the KL-divergence.

Taking inspiration from the wake-sleep algorithm BID21 , we instead minimize the KL in the opposite direction, KL(p(τ |R > L)||q(τ )).

This can be done by sampling trajectories from p(τ |R > L) (e.g. by rejection sampling, keeping only the forward-generated trajectories which lead to R > L) and maximizing their log-probability under q(τ ).
...
Therefore, in our experiments, we sample fairly short tracesτ from the backtracking model, where the length is adjusted manually based on the time-scale of each task.
