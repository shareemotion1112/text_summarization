...
The algorithm is based on a (soft) clustering network that produces a distribution over the k clusters: DISPLAYFORM0 such that θ c is the parameter set of the clustering network, h(x) is a nonlinear representation of a point x computed by the clustering network and w 1 , . . .

, w k , b 1 , . . . , b k ∈ θ c are the parameters of the softmax output layer.
...
Denote the autoencoder associated with cluster i by f i (x; θ i ) where θ i is the parameter-set of the network autoencoder.

We can view the reconstructed object f i (x; θ i ) ∈ R d as a data-driven centroid of the cluster i that is tuned for the input x.
...
The points that were assigned by the k-means algorithm to cluster i are next used to pre-train the i-th autoencoder f i (x; θ i ).
...
Assume we replace each autoencoder in our network by a constant function f i (x t , θ i ) ≡ µ i ∈ R d and we replace the clustering network by a hard decision based on the reconstruction error, then we obtain exactly the classical k-means algorithm.
...
Network components:• A nonlinear representation: x → h(x; θc) DISPLAYFORM6 • A set of autoencoders (one for each cluster): fi(xt; θi), i = 1, . . .
...
Clustering is obtained by minimizing the reconstruction error: DISPLAYFORM0 The final (hard) clustering is: DISPLAYFORM1 p(ct = i|xt; θc), t = 1, . . .
...
This is also reflected by decision of the clustering network p(c = 4|x; θ c ) = 0.99.
...
The observed signal is x t = (σ(W · v t )) 2 t = 1, · · · , n, where σ is the sigmoid function, W ∈ R 100×2 and v t is the t-th point in the latent domain.
