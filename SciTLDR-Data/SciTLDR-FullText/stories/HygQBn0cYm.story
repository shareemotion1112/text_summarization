...
The stochastic model f θ (s 1:t , a t , z t ) takes as input a sequence of observed or previously predicted states s 1:t , an action a t , and a latent variable z t which represents the information about the next state s t+1 which is not a deterministic function of the input.

During training, latent variables are sampled from a distribution whose parameters are output by a posterior network q φ (s 1:t , s t+1 ) conditioned on the past inputs and true targets.
...
Once the forward model is trained, we use it to train a parameterized policy network π ψ , which we assume to be stochastic.
...
More precisely, let f θ1 , ..., f θ K denote our prediction model with K different dropout masks applied to its hidden units (this can also be viewed as changing its weights).
...
The distribution over model predictions for s t+1 is given by: DISPLAYFORM0 The distribution p(θ, z|D) reflects the posterior over model weights and latent variables given the data, and is intractable to evaluate.

We instead approximate it with the variational distribution q parameterized by η = {φ, θ * }: DISPLAYFORM1 Here q φ represents a distribution over latent variables represented using a posterior network with parameters φ, which could be a diagonal Gaussian or the mixture distribution described in Section 2.1.

The distribution q θ * is the dropout approximating distribution over forward model parameters described in BID13 , a mixture of two Gaussians with one mean fixed at zero.
...
Once the forward model is trained, for a given input we can obtain an approximate distribution over outputs p(ŝ t+1 |s 1:t , a) induced by the approximate posterior by sampling different latent variables and dropout masks.

We now show that the covariance of the outputsŝ t+1 can be decomposed into a sum of two covariance matrices which represent the aleatoric and epistemic uncertainty, using a similar approach as BID10 .

Using the conditional covariance formula we can write: DISPLAYFORM2 The first term is the covariance of the random vector E z [ŝ t+1 |s 1:t , a, θ] when θ ∼ q θ * (θ).

This term ignores any contribution to the variance from z and only considers the effect of θ.

As such it represents the epistemic uncertainty.

The second term represents the covariance of the predictions obtained by sampling different latent variables z ∼ p(z) averaged over different dropout masks, and ignores any contribution to the variance from θ.
...
The work of BID10 ) also proposed adding an uncertainty penalty when training paramaterized policies, but did so in the context of BNNs trained using α-divergences applied in low-dimensional settings, whereas we use variational autoencoders combined with dropout for high-dimensional video prediction.

α-BNNs can yield better uncertainty estimates than variational inference-based methods, which can underestimate model uncertainty by fitting to a local mode of the exact posterior BID9 BID23 .
...
We instead approximate it with the variational distribution q parameterized by η: DISPLAYFORM1 Here q φ represents a distribution over latent variables parameterized by φ, which could be a diagonal Gaussian or the mixture distribution described in Section 2.1.

The distribution q θ * is the dropout approximating distribution over model parameters described in BID13 (Section 3.2 of Supplement).
...
The parameters of the variational distribution are thus η = {θ * , φ}, and can be optimized by maximizing the evidence lower bound, which is equivalent to minimizing the Kullback-Leibler divergence between the approximate posterior and true posterior: DISPLAYFORM2 Here p 0 (z, θ) = p 0 (z) · p 0 (θ) represents a prior over latent variables and model parameters.

By applying the chain rule for KL divergences together with the fact that z and θ are independent, we obtain: DISPLAYFORM3 Setting both Gaussians in p 0 (θ) to have zero mean, the second KL term becomes equivalent to scaled 2 regularization on the model parameters, and can be set arbitrarily small (Section 4.2 in Supplement of BID13 ).

Ignoring this term and approximating the integral in Equation 3 using a single sample, we obtain: DISPLAYFORM4 whereθ ∼ q θ * (θ) and z ∼ q φ (s 1:t , s t+1 )).

Assuming a diagonal Gaussian likelihood on the outputs with constant variance 1/β, we can rewrite this as: DISPLAYFORM5 Multiplying by β does not change the maximum.
...
The architecture of our forward model consists of four neural networks: a state encoder f enc , an action encoder f act , a decoder f dec , and the posterior network f φ .
...
All MPUR policies use a weighting of λ = 0.5 for the uncertainty cost.
