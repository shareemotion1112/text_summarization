...
We denote the mean and variance of z i by µ i and σ i , respectively.
...
[n] = {1, ..., n}. Per S we define a basis function, φ S (z) := i∈S φ i (z i ), where for the empty set φ ∅ (z) = 1 and φ i is the z-score normalized dimension, i.e., φ i := zi−µi σi .

For example, under the uniform Bernoulli distribution for the i-th dimension,

The 2 n functions φ S form an orthonormal basis for the space of Boolean functions f ,
...
Further, we have n degree-1 coefficientsf (p) (i) = f, φ i and so on.
...
If we expand the function f in terms of its φ S basis as in equation 2 and focus on the i-th dimension, by Lemma 1 we can show that the REINFORCE gradient is given by
...
Here we note that although the final expression in equation 7 can also be derived by a finite difference method, it does not make explicit, as in equation 31, the dependence on z i and µ i of the term inside the expectation.
...
For a Boolean function f the discrete derivative on the i-th latent dimension with a basis function φi is defined as

The Fourier expansion of the discrete derivative equals

The Fourier expansion of the discrete derivative is derived by equation 2: (i) all bases that do not contain the i-th dimension are constant to φi and thus set to zero, while (ii) for the rest of the terms ∂φ S dφ i = φ S\i from the definition of basis functions φS(z).
...
The following relation follows from the fact that when working with the Fourier representation, f (z) is multilinear, E p(z) [zi] = µi and the linearity of expectation.

As the partial derivative of f (u) w.r.t.

µi is equivalent to discrete derivative of

, and keeping in mind that φi = (zi − µi)/σi, we have that

We then note that the discrete derivative of f w.r.t.

zi, Dz i f (u) (µ), from the left hand side of equation 19, is equivalent to the partial derivative of f w.r.t.
...
By expanding the function f in terms of its φS basis as in equation 2 and focusing on the i-th dimension, we have thatf
...
This follows from the fact that when moving from p(z) to p i→1/2 (z), (i) we have that φi = zi, and (ii) no other term under the p(z) expansion contributes to the zi term under the p i→1/2 (z) expansion.
...
Here we note that the although final expression in equation 32 can also be derived by a finite difference method, it does not make explicit, as in equation 31, the dependence on zi and µi of the term inside the expectation.
...
Letting h(zi) = 2zi ∈ {−1, 1}, the functions φi are now given
