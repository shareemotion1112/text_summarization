...
We define an operator from the subsequences of real valued quantities to d-dimensional control signals π(z i , z i−1 , . . .

, z i−p+1 |Θ π ) : DISPLAYFORM0 , p ∈ N is the number of frames used to produce the control signal, and Θ π are the parameters of the operator π(·).

We denote The problem is stated as follows: given the set of N sequences ẑ j ∈ Z lj N j=1 DISPLAYFORM1 and the set of corresponding signal sequences ĉ DISPLAYFORM2 , produced by some external control method called a 'reference actor', find the parameters Θ π of the actor π(·|Θ π ), which minimise a loss function (the used loss function is defined in formula FORMULA9 ).
...
Every time the agents performs an action it receives a reward r(x i , c i ) ∈ R ).The emitted actions are defined by the policy π, mapping states into actions.
...
For such a deterministic policy, one can express the discounted future reward using the following form of the Bellman equation for the expectation of discounted future reward Q(·, ·) ): DISPLAYFORM0 where π(·) is an operator from states to actions, γ ∈ [0, 1] is a discount factor.
...
In many of the state-of-the-art works on reinforcement learning, the actor's parameters are trained by maximisation of the critic's approximation of Q(·, ·) , using gradient descent with gradients, obtained using the chain rule , BID18 ): DISPLAYFORM1 where Θ Q and Θ π are the trainable parameters of the actor and the critic, respectively.
...
The optimisation problem is based on the regularised Q-function f (x, π(x|Θ π )) : DISPLAYFORM2 DISPLAYFORM3 where α ≥ 0 is some coefficient,π(·) is the reference actor, and ρ(·, ·) is a differentiable distancelike function.

One can easily see that in the case α = 0 it completely coincides with the reinforcement problem, and with α → ∞ the problem becomes a standard supervised training one.

The weight Q(x,π(x)) is given to encourage the actor to follow the reference policy if the expected reward is high, and not to encourage much otherwise.

After differentiating this expression with respect to Θ π one can see that DISPLAYFORM4 As in , the update of the critic is carried out by solving the following optimisation problem: DISPLAYFORM5 whereX andĈ are taken from the replay buffer,Q π is the discounted reward function approximation.
...
For supervised finetuning and pretraining stages, we use the following loss function: DISPLAYFORM0 where is a reasonably small constant (with respect to c, we use = 10 −2 ), π(x) is an operator, transforming input vectors x to the control signals, c is the reference actor control signal.
...
Similarly, the distance-like function ρ in formula (4) is defined as: DISPLAYFORM1 where is a reasonably small constant (we use = 10 −4 ).To implement the formalisation, described in section 2.1, we use a syamese network architecture (see BID1 ), given in Figure 2 .
...
TAB1 and Figure 3 contain the comparison of the results given different values of the parameter α.

When α = 0, we rely solely on reinforcement learning; the higher is the parameter α, the more restriction is put to stick to the supervised labels rather than improving the policy by reinforcement learning.
...
The left figure shows the results for all tested parameters, while the right one shows the comparison between the best performing one and the one with the largest value of α (i.e. the closest to supervised active learning).
...
One can see from the figure that the smaller values of the coefficient α tend to yield worse performance; however, it may be attributed to longer convergence time as it implies more reliance on reinforcement learning exploration.

At the same time, the unlimited increase of the coefficient α does not help gaining further performance improvements.

Even more, one can see from the right figure that after certain point the curve for α = 100 slowly declines; we suggest that it could be caused by overfitting to the bot values.

We also see that for α = 0, when the supervised data is used only during the pre-training stage, the performance is much lower than for the rest of the graphs.

TAB1 shows the total rewards for different values of parameter α.
...
L 2 distance has been used as ρ for the equation (4).

Also, in order to maintain the correctness of regularisation assumptions, as the condition Q ≥ 0 is not met for some low reward values in MuJoCo, Q(x,π(x)) was substituted by max(0, Q(x,π(x)).
...
the original DDPG algorithm (referenced as α = 0) 2.

the DDPG algorithm with fixed regularisation coefficient, α = 0.1 3.

the DDPG algorithm with exponential decay, initial value of α is 1, the value is decayed with the coefficient 0.01 every 20, 000 timesteps.
...
It could be concluded from the graphs for the InvertedDoublePendulum task that convergence depends in this case on the initial value of parameter α.
