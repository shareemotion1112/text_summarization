...
A link is denoted by an unordered node pair {i, j} ∈ E. LetÂ denote the network's adjacency matrix, with elementâ ij = 1 for {i, j} ∈ E andâ ij = 0 otherwise.
...
Each of these can be expressed as sets of constraints on the expectations of the sum of various subsets S ⊆ V 2 of elements from the adjacency matrix: E {i,j}∈S a ij = {i,j}∈Sâ ij , where the expectation is taken w.r.t.
...
Without losing generality (as it merely fixes the scale), we set σ 1 = 1 through out this paper.
...
The magnitudes of the gradient terms are also affected by parameter σ 2 and prior P (G): a large σ 2 gives stronger push and pulling effect.

In our quantitative experiments we always set σ 2 = 2.Computing this gradient w.r.t.
...
Note that each of the terms is bound in norm by the diameter of the embedding, as the other factors are bound by 1 for σ 1 = 1, σ 1 < σ 2 .
...
As a larger σ 2 in general appears to give better visual separation between node clusters, we set σ 2 = 15.For comparison, we first apply CNE with uniform prior (overall network density).
