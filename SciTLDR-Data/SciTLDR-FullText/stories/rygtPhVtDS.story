...
In parametric estimation, the PDFf is assumed to belong to a parametric family F = {f θ (·)|θ ∈ Θ} where the density function is described by a finite dimensional parameter θ ∈ Θ. The standard method for estimating θ is maximum likelihood estimation, wherein θ is chosen so that the likelihood of the data D is maximized.

This is equivalent to minimizing the Kullback-Leibler divergence between the empirical data distribution p D (x) = 1 n n i=1 δ(||x − x i ||) (i.e., mixture of point masses in the observations x i ) and the parametric distributionf θ :
...
While this line of work corresponds to noise in the parameter space, other research suggests to augment the training data through random and/or adversarial transformations of the data (Sietsma & Dow, 1991; Burges & Schölkopf, 1996; Goodfellow et al., 2015; Yuan et al., 2017) .
...
When considering expressive families of conditional densities, standard maximum likelihood estimation of the model parameters θ is ill suited.
...
This can be understood as replacing the original data points (x i , y i ) by random variables tions K x (ξ x ) and K y (ξ y ) respectively.
...
For notational brevity, we set Z := X × Y, z := (x , y ) and denotef θ (z) :=f θ (y|x).
...
Let l ( D) be a loss function over a set of data points D = {z 1 , ..., z n }, which can be partitioned into a sum of losses l(D) = n i=1 l(z i ), corresponding to each data point z i : The expected loss l(z i +ξ), resulting from adding random perturbations, can be approximated by a second order Taylor expansion around z i .

Using the assumption about ξ in (4), the expected loss an be written as

where l(z i ) is the loss without noise and

∂z 2 (z) zi the Hessian of l w.r.t z, evaluated at z i .

Assuming that the noise ξ is small in its magnitude, O(ξ 3 ) is negligible.

This effect has been observed earlier by Webb (1994) and Bishop (1994) .

See Appendix A for derivations.

When concerned with maximum likelihood estimation of a conditional densityf θ (y|x), the loss function coincides with the negative conditional log-likelihood l(y i , x i ) = − logf θ (y i |x i ).

Let the standard deviation of the additive data noise ξ x , ξ y be h x and h y respectively.
...
The second term in (6) penalizes large negative second derivatives of the conditional log density estimate logf θ (y|x) w.r.t.
...
a continuous function of z and θ.

Moreover, we assume that the parameter space Θ is compact.
...
As we typically just have a finite number of samples from p(z), the respective empirical estimatê

is used as training objective.

Note that we now define the loss as function of θ, and, for fixed θ, treat l n (θ) as a random variable.

Under some regularity conditions, one can invoke the uniform law of large numbers to show consistency of the empirical ML objective in the sense that sup θ∈Θ |l n (θ) − l(θ)| a.s.
...
n,r (θ) is based on samples from the kernel density estimate, which are obtained by adding random noise vectors ξ ∼ K(·) to our original training data.

Since we can sample an unlimited amount of such random noise vectors, r can be chosen arbitrarily high.

This allows us to make sup θ∈Θ |l

n (θ)| arbitrary small by the uniform law of large numbers.

In order to make sup θ∈Θ |l (h) n (θ) − l(θ)| small in the limit n → ∞, the sequence of bandwidth parameters h n needs to be chosen appropriately.
...
Initially, we have to make asymptotic integrability assumptions that ensure that the expectations in l (h) n (θ) and l(θ) are well-behaved in the limit (see Appendix C for details).
...
n (θ) is intractable and, thus, replaced by its sample estimatel

n,r .

Since we can draw an arbitrary amount of samples fromq (h) n , we can approximate l (h) n (θ) with arbitrary precision.

Given a fixed data set D of size n > n 0 , this means that lim r→∞ sup θ∈Θ

n (θ) = 0 almost surely, by (29) and the uniform law of large numbers.
...
Theorem 1 Suppose the regularity conditions (28) and (29) are satisfied, h n fulfills (11) and Θ is compact.
...
As result of this optimization, one obtains a parameter vectorθ (h) n,r , which we hope is close to the minimizing parameterθ of the ideal objective function l(θ).
...
Since a minimizing parameterθ of l(θ) may not be unique, we define Θ * = {θ * | l(θ * ) ≤ l(θ) ∀θ ∈ Θ} as the set of global minimizers of l(θ), and d(θ, Θ * ) = min θ * ∈Θ * {||θ − θ * || 2 } as the distance of an arbitrary parameter θ to Θ * .

Based on these definitions, it can be shown that Algorithm 1 is asymptotically consistent in a sense that the minimizer ofθ

n,r converges almost surely to the set of optimal parameters Θ * .

Theorem 2 Suppose the regularity conditions (28) and (29) are satisfied, h n fulfills (11) and Θ is compact.

For r > 0 and n > n 0 , letθ

n,r ∈ Θ be a global minimizer of the empirical objectivel

almost surely.

Note that Theorem 2 considers global optimizers, but equivalently holds for compact neighborhoods of a local minimum θ * (see discussion in Appendix C).
...
| is minimized, which is practically not feasible since l(θ) is intractable.
...
Under the assumption that p(z) and the kernel function K are Gaussian, the optimal bandwidth can be derived as h = 1.06σn (Silverman, 1986) .

In that,σ denotes the estimated standard deviation of the data, n the number of data points and d the dimensionality of Z. This formula is widely known as the rule of thumb and often used as a heuristic for choosing h.
...
In all experiments we use Gaussian pertubations of the data, i.e., K(ξ) = N (0, I).
...
Also, let each z i be perturbed by a random noise vector ξ ∼ K(ξ) with zero mean and i.i.d.

elements, i.e. E ξ∼K(ξ) [ξ] = 0 and E ξ∼K(ξ) ξ n ξ j = h 2 I (16) The resulting loss l(z i + ξ) can be approximated by a second order Taylor expansion around z i

Assuming that the noise ξ is small in its magnitude, O(ξ 3 ) may be neglected.

The expected loss under K(ξ) follows directly from (17):
...
Corollary 1 Let Θ be a compact set and andf θ :
...
Lemma 1 states regularity conditions ensuring that the expectations in l (h) n (θ) and l(θ) are wellbehaved in the limit.
...
Inequality (30) shows that we can make |l (h) n (θ) − l(θ)| small by reducing the l 1 -distance between the true density p and the kernel density estimateq (h) n .
...
For any δ > 0 let B n denote the event
...
Respectively, we only consider realizations of our training data Z (n) and noise samples I (r) , Ξ

for which the convergence in (13) holds (see proof of Theorem 1 for details on this notation).

For such realization, let (θ

From the triangle inequality, it follows that for any > 0 there exists k 0 so that ∀k > k 0

given the convergence established in Theorem 1 and the continuity of l in θ.
...
the minimizer of µ i k ,j k , and µ i k ,j k (θ) − l(θ) < by Theorem 1.

Because can be made arbitrarily small, l(θ 0 ) ≤ l(θ) as k → ∞. Because θ ∈ Θ is arbitrary, θ 0 must be in Θ * .

In turn, since (n i ) i , (r i,j ) j and (i k ) k , (j k ) k were chosen arbitrarily, every limit point of a sequence (v i k ,j k ) k must be in Θ * .
...
Theorem 2 considers global optimizers over a set of parameters Θ, which may not be attainable in practical settings.

However, the application of the theorem to the context of local optimization is straightforward when Θ is chosen as a compact neighborhood of a local minimum θ * of l (Holmstrom & Koistinen, 1992b (Bishop, 1994) .
...
The conditional density estimatep(y|x) follows as weighted sum of K Gaussianŝ

wherein w k (x; θ) denote the weight, µ k (x; θ) the mean and σ 2 k (x; θ) the variance of the k-th Gaussian component.

All the GMM parameters are governed by the neural network with parameters θ and input x.

The mixing weights w k (x; θ) must resemble a categorical distribution, i.e. it must hold that K k=1 w k (x; θ) = 1 and w k (x; θ) ≥ 0 ∀k.

To satisfy the conditions, the softmax linearity is used for the output neurons corresponding to w k (x; θ).

Similarly, the standard deviations σ k (x) must be positive, which is ensured by a sofplus non-linearity.

Since the component means µ k (x; θ) are not subject to such restrictions, we use a linear output layer without non-linearity for the respective output neurons.
...
For each of the kernel centers, M different scale/bandwidth parameters σ m are chosen.
...
Let K be the number of kernel centers µ k and M the number of different kernel scales σ m .

The KMN conditional density estimate reads as follows:

As previously, the weights w k,m correspond to a softmax function.

The M scale parameters σ m are learned jointly with the neural network parameters θ.

The centers µ k are initially chosen by k-means clustering on the {y i } n i=1 in the training data set.
...
The data generating process (x, y) ∼ p(x, y) resembles a bivariate joint-distribution, wherein x ∈ R follows a normal distribution and y ∈ R a conditional skew-normal distribution (Anděl et al., 1984) .

The parameters (ξ, ω, α) of the skew normal distribution are functionally dependent on x. Specifically, the functional dependencies are the following:

ξ(x) = a * x + b a, b ∈ R (47)

α(x) = α low + 1 1 + e −x * (α high − α low )

y ∼ SkewN ormal ξ(x), ω(x), α(x)

Accordingly, the conditional probability density p(y|x) corresponds to the skew normal density function:

In that, N (·) denotes the density, and Φ(·) the cumulative distribution function of the standard normal distribution.

The shape parameter α(x) controls the skewness and kurtosis of the distribution.

We set α low = −4 and α high = 0, giving p(y|x) a negative skewness that decreases as x increases.
...
The component means are sampled from a spherical Gaussian with zero mean and standard deviation of σ = 1.5.

The covariance matrices Σ y,k ) and Σ y,k ) are sampled from a Gaussian with mean 1 and standard deviation 0.5, and then projected onto the cone of positive definite matrices.
