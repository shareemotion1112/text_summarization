...
DISPLAYFORM2 where σ(x) = 1/(1 + e −x ) is the standard logistic function.
...
Then for any bounded 1-Lipschitz loss function l : R → [0, B] and C U , C V > 0, with probability 1 − δ (over the sampling of E ± ), the following inequality holds DISPLAYFORM4 where ||A σ || 2 is the spectral norm of the randomized adjacency matrix A σ defined as follows: DISPLAYFORM5 The proof can be found in the appendix.
...
A rough estimate of E σ ||A σ || 2 can be found in the appendix for interested readers.
...
In other words, without norm regularization, the number of training samples required for learning D-dimensional embedding vectors is at least Ω(nD).Considering the fact that many large-scale graphs are sparse (with average degree < 20) and the default embedding dimension commonly ranges from 100 to 400, it is highly unlikely that the the dimensionality constraint by itself could lead to good generalization performance.
...
The SGD learning rate is standard: γ t = (t + c) −1/2 .
...
FIG2 shows the average l 2 norm of the embedding vectors during the first 50 SGD epochs (with varying value of λ r ).
...
As we can see, the generalization performance of embedding vectors starts to drop after 5 ∼ 20 epochs when λ r is small, indicating that they are overfitting the training dataset afterwards.

The generalization performance is worst near the end of SGD execution when λ r = 0, which coincides with the fact that embedding vectors in that case also have the largest norm among all settings.

Thus, FIG4 and FIG2 collectively suggest that the generalization of linear graph embedding is determined by vector norm.

FIG6 shows the generalization AP score on Tweet dataset with varying value of λ r and embedding dimension D after 50 epochs.

As we can see in FIG6 , without any norm regularization (λ r = 0), the embedding vectors will overfit the training dataset for any D greater than 10, which is consistent with our analysis in Claim 1.

On the other hand, with larger λ r , the impact of embedding dimension choice is significantly less noticeable, indicating that the primary factor for generalization is the vector norm in such scenarios.
...
As we can see, the quality of embeddings vectors is very bad when λ r ≈ 0, indicating that proper norm regularization is necessary for generalization.

The value of λ r also affects the gap between training and testing performance, which is consistent with our analysis that λ r controls the model capacity of linear graph embedding.
...
In all experiments, λ + = 1, λ − = 0.03, which achieves the optimal generalization performance according to cross-validation.
...
samples from P, by the uniform convergence theorem BID0 BID12 , with probability 1 − δ: DISPLAYFORM0 is the hypothesis set, and R(H C U ,C V ) is the empirical Rademacher Complexity of H C U ,C V , which has the following explicit form: DISPLAYFORM1 Here σ a,b are i.i.d.

Rademacher random variables: Pr(σ a,b = 1) = Pr(σ a,b = −1) = 0.5.
