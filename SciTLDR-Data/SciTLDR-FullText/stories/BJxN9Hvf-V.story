...
That is, we assume that there exists a strictly positive constant ϵ 0 such that DISPLAYFORM1 This, for example, covers complex scenarios with time-varying arrival rates to the different dispatchers that are not necessarily independent.
...
That is, dispatcher j forwards all its incoming jobs at time slot t to server i * where i * ∈ arдmin i {Q DISPLAYFORM2 (ties are broken randomly).Finally, we term a load balancing policy as an Loosely-ShortestQueue (Local Shortest Queue) policy if it respects Assumptions 1 and 2.
...
Assume the system is sub-critical, i.e., assume that there exists ϵ > 0 such that DISPLAYFORM1 Then, any Loosely-Shortest-Queue policy is strongly stable.
...
Assume that there existsε > 0 such that DISPLAYFORM2 Then, Assumption 1 holds.
...
Using the induction hypothesis in (34) yields DISPLAYFORM5 where we used the fact that by the definition of C * it holds that λ (1) + max i µ(1) i ≤εC * .
...
Fix dispatcher j and server i. Consider time slot t. By (4), with probability of at least ϵ 0 , dispatcher j samples d out of n Algorithm 1: Loosely-Shortest-Queue-Sample(d) (push-based Loosely-Shortest-Queue example)Code for dispatcher j ∈ M ; Route jobs:

foreach time slot t do Forward jobs to server i *

∈ ar дmin i Q j i (t ) ; DISPLAYFORM0 DISPLAYFORM1 end end Update local state: foreach time slot t do if new jobs arrive at time slot t then Uniformly at random pickservers uniformly at random disregarding the system state at time slot t.
...
Now consider the DISPLAYFORM0 Since the tie breaking rule is random, by (1), FORMULA1 , (2) , (5) and (6) , there existsε i > 0 such that P(A 1 ) >ε i .
...
In this subsection the job arrival process at a dispatcher is a Poisson process with parameter λ and the server service processes are geometrically distributed with a parameter 2p for a weak server and a parameter p for a strong server.

In a simulation with n s strong servers, n w weak servers and m dispatchers we set p = n s +0.5n w 100mand sweep 0 ≤ λ < 100.
...
Again, in this subsection the job arrival process at a dispatcher is a Poisson process with parameter λ but the server service processes are geometrically distributed with parameter 10p for a weak server and a parameter p for a strong server.

In a simulation with n s strong Throughput optimality Comm.

overhead

Per job arrival servers, n w weak servers and m dispatchers we set p = n s +0.1n w 100m DISPLAYFORM0 and sweep 0 ≤ λ < 100.
