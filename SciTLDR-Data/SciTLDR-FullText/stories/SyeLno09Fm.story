...
Typically, the goal of "forward" RL is to maximize the expected discounted return R(τ ) = T t=1 γ t−1 r(s t , a t ).In IRL, we instead assume that the reward function is unknown but that we instead have access to a set of expert demonstrations D = {τ 1 , . . .

, τ K }, where DISPLAYFORM0 The goal of IRL is to recover the unknown reward function r from the set of demonstrations.

We build on the maximum entropy (MaxEnt) IRL framework by BID55 , which models the probability of the trajectories as being distributed proportional to their exponentiated return DISPLAYFORM1 where Z is the partition function, Z = τ exp(R(τ ))dτ .
...
Let f θ denote the learner, and let a task be defined by learning from K training examples DISPLAYFORM0 One approach to meta-learning is to directly parameterize the meta-learner with an expressive model such as a recurrent or recursive neural network BID8 BID27 conditioned on the task training data and the inputs for the test task: DISPLAYFORM1 .
...
Each task T i has a set of demonstrations D T = {τ 1 , . . .

, τ K } from an expert policy which we partition into disjoint D tr T and D test T sets.
...
During this meta-test phase, the algorithm must infer the parameters of the reward function r φ (s t , a t ) for the new task from a few demonstrations.
...
In order to meta-learn a reward function that can act as a prior for new tasks and new environments, we first formalize the notion of a good reward by defining a loss L T (θ) on the reward function r θ for a particular task T .

We use the MaxEnt IRL loss L IRL discussed in Section 3, which, for a given D T , leads to the following gradient BID55 : DISPLAYFORM0 DISPLAYFORM1 via chain rule 16:Compute updated parameters DISPLAYFORM2 Sample demos DISPLAYFORM3 via chain rule Compute meta-gradient 20: DISPLAYFORM4 Update initial parameters where µ τ are the state visitations under the optimal maximum entropy policy under r θ , and µ D are the mean state visitations under the demonstrated trajectories.
...
In particular, we would like to find a θ such that the parameters DISPLAYFORM5 lead to a reward function r φ T for task T , such that the IRL loss (corresponding to negative loglikelihood) for a disjoint set of test demonstrations, given by L DISPLAYFORM6 IRL , is minimized.

The corresponding optimization problem for θ can therefore be written as follows: DISPLAYFORM7 Our method acquires this prior θ over rewards in the task distribution p(T ) by optimizing this loss.
...
First, we perform the update in Eq. 6 by computing the expected state visitations µ, which is the expected number of times an agent will visit each state.

We denote this overall procedure as STATE-VISITATIONS-POLICY, and follow BID55 by first computing the maximum entropy optimal policy in Eq. 2 under the current r θ , and then approximating µ using dynamic programming.
...
This allows us to recover the IRL gradient according to Eq. 5, which we can then apply to compute φ T according to Eq. 6.Second, we need to differentiate through this update to compute the gradient of the meta-loss in Eq. 7.
...
IRL with respect to the post-update parameters φ T , and then apply the chain rule to compute the meta-gradient: DISPLAYFORM0 where on the second line we differentiate through the MaxEnt-IRL update.

The derivation of this expression is somewhat more involved and provided in the supplementary Appendix D.Meta-testing.

Once we have acquired the meta-trained parameters θ that encode a prior over p(T ), we can leverage this prior to enable fast, few-shot IRL of novel tasks in {T j ; j = 1..M }.
...
We apply this gradient to adapt the parameters θ to the new task.
...
DISPLAYFORM0 Figure 2: Our approach can be understood as approximately learning a distribution over the demonstrations τ , in the factor graph p(τ ) = 1 Z T t=1 Φr(φ T , st, at)Φdyn(st+1, st, at) (above) where we learn a prior over φT , which during meta-test is used for MAP inference over new expert demonstrations.
...
For any non-quadratic likelihood, this is an approximation in a local neighborhood around θ (i.e. up to convex quadratic approximation).
...
We turned over the learning rate α, the inner learning rate β of our approach and 2 weight decay on the initial parameters.
...
We define the quality of reward function r θ parameterized by θ ∈ R k on task T with the MaxEnt IRL loss, L T IRL (θ), described in Section 4.

The corresponding gradient is DISPLAYFORM0 where ∂ r θ /∂ θ is the k ×|S||A|-dimensional Jacobian matrix of the reward function r θ with respect to the parameters θ.

Here, µ τ ∈ R |S||A| is the vector of state visitations under the trajectory τ (i.e. the vector whose elements are 1 if the corresponding state-action pair has been visited by the trajectory τ , and 0 otherwise), and µ D T = 1 |D T | τ ∈D T µ τ is the mean state visitations over all demonstrated trajectories in D T .

Let φ T ∈ R k be the updated parameters after a single gradient step.

Then DISPLAYFORM1 Let L test T be the MaxEnt IRL loss, where the expectation over trajectories is computed with respect to a test set that is disjoint from the set of demonstrations used to compute L test T (θ) in Eq. 10.

We seek to minimize DISPLAYFORM2 over the parameters θ.
...
In order to compute this gradient, however, we must take the gradient of the expectation E τ [µ τ ] with respect to the reward function r θ .

This can be done by expanding the expectation as follows DISPLAYFORM7 τ exp(µ τ r θ )(µ τ µ τ ) − exp(µ τ r θ ) ( τ exp(µ τ r θ )) 2 τ (µ τ µ τ ) exp(µ τ r θ ) DISPLAYFORM8
