...
σ is sigmoid function, so the generation probability p gen has a range of [0, 1].
...
Finally, we reweight the classification loss with hyperparameter λ and the objective function is:
...
Our model is optimized using Adam (Kingma & Ba, 2014) with a learning rate of 0.001, β 1 = 0.9, β 2 = 0.999.

We reduce the learning rate to half to avoid overfitting when the validation loss increases.

We set the hyperparameter λ to 0.5 in the objective function and the batch size to eight.
