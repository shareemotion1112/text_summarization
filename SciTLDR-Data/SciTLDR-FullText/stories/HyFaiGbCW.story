...
We set the spectral radius γ (maximal absolute eigenvalue of W res to be < 1 to ensure that the reservoir is operating in the chaotic regime BID22 ).
...
The stationary condition is DISPLAYFORM3 DISPLAYFORM4 DISPLAYFORM5 where η is a regularization constant and I is the identity matrix.
...
Further, we look at the reservoir performance as a function of γ in FIG1 .

γ is varied from 0 to 1 while looking for the optimal performance region where the reservoir has memory or is in the 'echo state' (Vengamoorthy G.K., 2009), however we find no indicative pattern.
...
We use a reservoir of 1000 nodes with γ = 0.5 and sparsity 0.9.
