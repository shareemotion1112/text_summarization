...
DISPLAYFORM0 We assume that the parameter space Ω is a convex set.

Furthermore, for the loss function to be L-Lipschitz and and strongly convex, we further assume that Ω is compact.

Since Ω is compact, the SGMM update requires projection.

The update rule for projected SGMM is given by: DISPLAYFORM0 where P denotes the Euclidean projection onto Ω, α > 0 is the learning rate 1 , µ > 0 is the momentum parameter, i t is a randomly selected index, and f (w t ; z it ) is the loss evaluated on sample z it .
...
In the following, we assume that the loss function f (·; z) is β-smooth, L-Lipschitz, and γ-strongly convex for all z.

Theorem 2 (Stability bound) Suppose that the SGMM update (5) is executed for T steps with constant learning rate α and momentum µ. Provided that DISPLAYFORM0 The result in Theorem 2 implies that the stability bound decreases inversely with the size of the training set.

It increases as the momentum parameter µ increases.

These properties are also verified in our experimental evaluation.

Theorem 3 (Convergence bound) Suppose that the SGMM update (5) is executed for T steps with constant learning rate α and momentum µ.

Then we have DISPLAYFORM1 whereŵ T denotes the average of T steps of the algorithm, i.e.,ŵ T = DISPLAYFORM2 Theorem 3 bounds the optimization error, i.e., the expected difference between the empirical risk achieved by SGMM and the global minimum.

Upon setting µ = 0 and γ = 0 in FORMULA8 , we can recover the classical bound on optimization error for SGM BID20 , (Hardt et al., 2016, Theorem 5.2) .
...
The last term depends on the learning rate, α, the momentum parameter µ, and the Lipschitz constant L. This term can be controlled by selecting α sufficiently small.

Proposition 1 (Upper-bound on true risk) Suppose that the SGMM update (5) is executed for T steps with constant learning rate α and momentum µ, satisfying the conditions in Theorem 2 and DISPLAYFORM3 T , we have: DISPLAYFORM4 where DISPLAYFORM5 andŵ T as well as the constants W 0 , · · · , W 3 are defined in Theorem 3.Proposition 1 provides a bound on the expected true risk of SGMM in terms of the global minimum of the empirical risk.

The bound in FORMULA11 is obtained by combining Theorem 2 and Theorem 3 and minimizing the expression over α.

The choice of α simplifies considerably when µ is sufficiently small, as stated in Proposition 1.
...
We consider the updates w t+1 = G t (w t ) + µ(w t − w t−1 ) and w t+1 = G t (w t ) + µ(w t − w t−1 ) with G t (w t ) = w t − α∇ w f (w t ; z it ) and G t (w t ) = w t − α∇ w f (w t ; z it ), respectively, for t = 1, · · · , T .

We denote δ t ∆ = w t − w t .

Suppose w 0 = w 0 , i.e., δ 0 = 0.

We first establish an upper-bound on E A [δ t+1 ] in terms of E A [δ t ] and E A [δ t−1 ] in the following lemma, whose proof is provided in the supplementary document.

DISPLAYFORM0 Using the result of Lemma 1, in the following, we develop an upper bound on E A [δ T ].

Let us consider the recursion DISPLAYFORM1 withδ 0 = δ 0 = 0.

Upon inspecting (10) it is clear that DISPLAYFORM2 as we simply drop the remainder of positive terms.

Substituting (11) into (10), we have DISPLAYFORM3 where the second inequality holds due to µ ≥ αβγ β+γ − 1 2 .Noting that DISPLAYFORM4 where the second expression holds since 0 ≤ µ < αβγ 3(β+γ) is assumed.
...
Let use denote y t+1 ∆ = w t + µ(w t − w t−1 ) − α∇ w f (w t ; z it ).

Then, for any feasible w ∈ Ω, (17) holds for y t+1 , i.e., DISPLAYFORM10 Note that the LHS of (25) can be written as DISPLAYFORM11 We note that µw t + (1 − µ)w ∈ Ω for any w ∈ Ω and w t ∈ Ω since Ω is convex.

Now in projected SGMM, we have DISPLAYFORM12 since projection a point onto Ω moves it closer to any point in Ω. This shows inequality (19) holds, and the convergence results do not change.
...
We set the learning rate α = 0.01.
...
First, we observe that the generalization error (with respect to both cross entropy and classification accuracy) decreases as n increases for all values of µ, which is suggested by our stability upper-bound in Theorem 2.

In addition, for sufficiently large n, we observe that the generalization error increases with µ, consistent with Theorem 2.
...
We observe that the training error decreases as the number of epochs increases for all values of µ, which is consistent with the convergence analysis in Theorem 3.
