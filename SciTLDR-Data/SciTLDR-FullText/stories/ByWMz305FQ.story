...
For machine translation, our objective is to learn a model, p(y|x; θ).
...
These sequence-to-sequence models consist of an encoder, Enc(x) = z = (z 1 , z 2 , ...z m ) parameterized with θ enc , and a decoder that learns to map from the latent representation z to y by modeling p(y|z; θ dec ), again parameterized with θ dec .
...
We now explore two classes of regularizers, Ω, that explicitly force the model to make the representations in all other languages similar to their English counterparts.
...
The loss function we then minimize is: DISPLAYFORM0 where L CE is the cross-entropy loss and λ is a hyper-parameter that controls the contribution of the alignment loss Ω.
...
To enable zero-shot translation, our objective then is to minimize the discrepancy between these distributions by explicitly optimizing the following domain adversarial loss BID17 : DISPLAYFORM0 where Disc is the discriminator and is parametrized by θ disc .
...
For adversarial alignment, the discriminator is a feedforward network with 3 hidden layers of dimension 2048 using the leaky ReLU(α = 0.1) nonlinearity.

λ was tuned to 1.0 for both the adversarial and the cosine alignment losses.
...
In comparison, the cosine distance loss was relatively stable, with λ being the only hyper-parameter controlling the weight of the alignment loss with respect to the translation loss.
...
We use the cosine loss with λ set to 0.001 because of how easy it is to tune.
