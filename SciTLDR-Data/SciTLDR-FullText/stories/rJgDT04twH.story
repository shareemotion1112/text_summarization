...
We show that ErrPs of an observer can be learned for a specific game, and the definition used as-is for another game without requiring re-learning of the ErrP. This is notably different from previous approaches (Chavarriaga & Millán, 2010; Salazar-Gomez et al., 2017) , where the labeled ErrPs are obtained in the same environment (where the RL task is performed).
...
Consider a Markov Decision Process (MDP) problem M , as a tuple < X , A, P, P 0 , R, γ >, with state-space X , action-space A, transition kernel P , initial state distribution P 0 , accompanied with reward function R, and discounting factor 0 ≤ γ ≤ 1.
...
In model-free RL method, the central idea of most prominent approaches is to learn the Q-function by minimizing the Bellman residual, i.e., L(Q) = E π Q(x, a) − r − γQ(x ,â) 2 , and temporal difference (TD) (Tesauro, 1995) update where the transition tuple (x, a, r, x ) consists of a consecutive experience under behavior policy π.
...
Error-potentials in the EEG signals is studied under two major paradigms in human-machine interaction tasks, (i) feedback and response ErrPs: error made by human (Carter et al., 1998; Falkenstein et al., 2000; Blankertz et al., 2003; Parra et al., 2003; Holroyd & Coles, 2002) , (ii) interaction ErrPs: error made by machine in interpreting human intent (Ferrez & Millán, 2005) .

Another interesting paradigm is when human is watching (and silently assessing) the machine performing a specific task (Chavarriaga & Millán, 2010) .

The manifestation of these potentials across these paradigms were found quite similar in terms of their general shape, timings of negative and positive peaks, frequency characteristics etc., (Ferrez & Millán, 2005; Chavarriaga & Millán, 2010) .
...
Imitation learning from a small number of demonstrations followed by RL fine-tuning is a promising paradigm to improve the sample efficiency in such cases (Večerík et al., 2017; Hester et al., 2018; Gao et al., 2018) .
...
Bayesian DQN The first model we use is a DQN architecture where the Q-function is approximated as a linear function, with weights ω a , of the feature representation of states φ θ (x) ∈ R d , parameterized by neural network with weights θ (Osband et al., 2013) .

Here by utilizing the DQN architecture and imposing Gaussian distributions on ω a , based on Bayesian linear regression (BLR) (Rasmussen, 2003) , the posterior of ω a can be calculated by

where we construct disjoint replay buffer D a corresponding to experience with action a, and a matrix Φ θ a ∈ R d×|Da| , vector y a , i.e., the concatenation of state features and target values in set D a .
...
It is implemented by K ∈ N bootstrapped estimates of the Q value in parallel, i.e., Q k (s, a; θ), s = 1, . . .

, K.

In this work, three acquisition functions are explored in selecting trajectory for ErrP labeling.

The trajectory is defined as a sequence of state-action pairs τ := {(s 0 , a 0 ), . . .

, (s T , a T )}.

We denote the trajectory set as D, the learned Q network as Q, and acquisition function as a(τ, Q).
...
where α is a free parameter, tuned empirically.

The likelihood of positive and negative state-action pair are denoted as π Q (a|s) and 1 − π Q (a|s).
...
After reward learning, consisting of learning Q function and baseline function, for any transition tuple (s, a, s ), the learned reward function can be represented as Q B (s, a) − γ max a ∈A Q B (s , a ).
