...
(2) Again, y t ∈ R d denotes the observation at time t. The self map ϕ : M → M models the predicted frame transition and describes the deterministic part of the dynamics.

In this work, it is assumed to be differentiable.

Furthermore, the function ψ vt : M → M describes a displacement by v t in the in the tangent space T yt M of M at y t , followed by a retraction onto M. The displacement is sampled from i.i.d.

zero-mean Gaussian noise.

Eq. FORMULA0 is a special case of Eq. (2) , where ϕ(y) = CAC + (y −ȳ) +ȳ and ψ v (y) = y + Cv.
...
For a model Eq. (3) to substitute Eq. (2), it needs to transform the transition ϕ to a multiplication by a matrix and the perturbation ψ vt to a superposition by zero-mean Gaussian noise.

The latter is generally easy to achieve, if the perturbation is sufficiently small and C is a diffeomorphism.

In the remainder of the section, we thus focus on linearization of ϕ.
...
We thus propose the following assumption on the transition function ϕ of Eq. (2).Assumption 3.

The differentiable self map ϕ : M → M has at least one fixed point y * .However, such an approach might fail for real-world processes due to the curse of dimensionality.

A remedy is to employ an representation function Γ that maps the system observations to a lowerdimensional latent space prior to performing the linearization.

We call such a function a local linearizer.

DISPLAYFORM0 The map Γ is said to be a local linearizer at y * ∈ M of ϕ , if there exists a matrix Φ ∈ R n×n such that the following equality holds true with y ∈ R d .
...
DISPLAYFORM2 locally linearizes ϕ, if the rows and columns of the Jacobi matrix J ϕ of ϕ at y * lie in the row space of the Jacobi matrix J Γ of Γ at y * .Proof.
...
Moreover, it does not provide any instructions on how to find an appropriate representation Γ and matrix Φ. Traditionally, this task is approached by learning Γ and Φ separately while considering the sampled observations of the process globally, as will be described in the following.

To characterize the problem of global linearization, we need to introduce a measure.

It is sensible to consider the expected squared distance between the result of a transformation ϕ and its linear approximation.
...
Let Γ : M → R n denote a data representation mapping and ϕ : M → M the transition function to be linearized.

Consider the following expression DISPLAYFORM0 The denominator 2n is a normalization factor.

If Γ(y) has standard Gaussian distribution, then q(Γ, ϕ, Φ) < 1 denotes that the linear prediction with Φ is smaller than the expected distance of two independently drawn samples.

A common method of linearizing ϕ, is to first choose a representation Γ that is assumed to have good linearization properties, and then inferring Φ by minimizing Eq. (7) BID8 BID3 ; BID12 ).

In that case, the solution is given byΦ DISPLAYFORM1 if Γ(y) has standard Gaussian distribution.
...
To summarize, we are looking for the function C and a matrix A, such that the random variableỹ DISPLAYFORM2 has the same probability distribution as y 2 .

Estimation of probability distributions for high-dimensional data is still an ongoing research topic in deep learning.

The problem is typically framed as an approximation of a function f θ , parameterized by a vector θ in a finite-dimensional euclidean space.

The purpose of f θ is to map realizations of low-dimensional, standard Gaussian noise to samples that abide the data distribution.

Typically, the function f θ is realized by a neural network with trainable weights θ.

Most of the widely employed deep generative models, including the GAN, and the VAE, adopt this approach.

What these models differ in, is merely the way the network parameters θ are trained.
...
For the following sections, it is sufficient to assume that these techniques are capable of approximating a function f θ that maps from a low-dimensional standard Gaussian to a high-dimensional data distribution.
...
Remember, that a function f θ learned by a deep generative model is capable of transforming samples x of standard Gaussian noise to samples from a high dimensional data distribution.

Let θ = (A, B, η) contain the matrices A, B ∈ R n×n such that the constraint AA + BB = I n (15) is fulfilled, and the parameter η that defines the model for C η = C. Consider the following definition for f θ .

DISPLAYFORM4 Assume that we can train f θ , as defined in Eq. FORMULA0 , to map x ∼ N (x; 0, I 2n ) tos = f θ (x) with the probability distributions ∼ p(s).(17) Then, A and C = C η indeed fulfill the condition in Eq. FORMULA0 for h 1 , h 2 with joint probability distribution Eq. (10).

In order to implement f θ by a neural network, we propose the architecture depicted in FIG0 .

The first layer is linear and will be referred to as the dynamic layer in the following.

The dynamic layer implements the multiplication with the matrix DISPLAYFORM5 The output of the dynamic layer is split into an upper half h 1 and a lower half h 2 and both halves are fed to the subnetwork that implements the observer function C η .

The weights of the dynamic layer contain the matrices A, B. Thus, they can be trained along with η, by back-propagation of the stochastic gradient.
...
We thus substitute L(θ) bỹ DISPLAYFORM6 where λ > 0 is a regularizer constant.

Here, L refers to the variational bound for the VAE or to the Generator loss for the (Wasserstein) GAN.

FORMULA0 ) as the foundation for implementing the observer function C η .
...
We train C η and A simultaneously by means of the dynamic layer introduced in Section 4.3.
...
Synthesis is performed by sampling from the MAR model described by DISPLAYFORM7 and mapping the latent states to the observation space by means of C η .
...
An isotropic standard Gaussian model N (s; f θ (x), σ 2 ) is assumed for the conditional data distribution.

The Adam optimizer with a step size of 2.5 * 10 −4 is used to train the architecture.

The regularizer constant is set to λ = 100.
...
We choose σ = 4 and use sequences of length 10000 for training the network for 25 epochs.
...
An observation we make is that higher values of σ improve the probability of synthesizing the correct sequence, but decrease the variability of digit shapes.
...
We train our architecture for 100 epochs with σ = √ 5 to synthesize sequences depicting azimuthal rotations of 20• per frame.
...
We employ the Wasserstein GAN for dynamic texture synthesis experiments and set the regularizer to λ = 1.
...
Let Φ ∈ R n×n be a matrix.

Since Γ is a diffeomorphism, we define DISPLAYFORM0 holds.

Let us denote the Jacobian of φ at y * by J φ .

Because Γ maps y * to the origin, we can reformulate the requirement as DISPLAYFORM1 This requirement is fulfilled if the Jacobi matrices of ϕ and φ coincide, i.e. DISPLAYFORM2 The Jacobian of φ at y * is given, according to he chain rule, by DISPLAYFORM3 where J Γ −1 ∈ R d×n is the Jacobian matrix of Γ −1 at Γ(y * ).

A matrix Φ ∈ R n×n can be always found, such that Eq. FORMULA2 is fulfilled, if the columns and rows of J ϕ lie in the column space of J Γ −1 and the row space of J Γ , respectively.

The column space of J Γ −1 coincides with the row space of J Γ , due to the identity J Γ J Γ −1 = I n .
...
This can be ensured by using the regularizer DISPLAYFORM4 with λ 1 , λ 2 , λ 3 > 0.
