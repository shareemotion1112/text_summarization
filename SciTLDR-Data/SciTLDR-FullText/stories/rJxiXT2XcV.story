...
We propose a general logic-based framework for explanation generation, where given a knowledge base KB 1 (of an agent) that entails a formula φ and a knowledge base KB 2 (of a human user) that does not entail φ, the goal is to identify an explanation ⊆ KB 1 such that KB 2 ∪ entails φ.
...
DISPLAYFORM1 For our later use, we will assume that a negation operator ¬ over formulas exists; and ϕ and ¬ϕ are contradictory with each other in the sense that for any KB and B ∈ ACC L (KB), if ϕ ∈ B then ¬ϕ ∈ B; and if ¬ϕ ∈ B then ϕ ∈ B. ⊆ KB is called a sub-theory of KB.

A theory KB subsumes a theory KB , denoted DISPLAYFORM2 Conclusions of a knowledge base can also be derived using rules.

A rule system Σ L of a logic L is a set of rules of the form DISPLAYFORM3 where ϕ i are formulas.
...
Given a knowledge base KB and a rule system Σ L , we say KB Σ L ϕ if either ϕ ∈ KB or there exists a sequence of rules r 1 , . . .

, r n in Σ L such that body(r 1 ) ⊆ KB, head(r n ) = ϕ, head(r i ) ∈ KB for i = 1, . . .

, n − 1, and body(r i ) ⊆ KB or body(r i ) ⊆ body(r 1 ) ∪ {head(r j ) | j = 1, . . . , i − 1} for every i = 2, . . .

, n. We call the sequence = r 1 ; . . . ; r n as a proof from KB for ϕ w.r.t.

Σ L and say that the proof has the length n. DISPLAYFORM4
...
We encode (P, n) as a formula Φ such that a 0 , a 1 , . . . , a n−1 is a solution for (P, n) if and only if Φ can be satisfied in a way that makes the fluents a 0 , a 1 , . . . , a n−1 true.

The formula Φ is a conjunction of the following formulae:• Initial State: Let F be the set of possible facts in the planning problem: DISPLAYFORM0 • Goal: Let G be the set of goal facts:{f n |f ∈ G}• Action Scheme:

For every action a i at time step i: DISPLAYFORM1 • Explanatory Frame Axioms: Formulae describing what does not change between steps i and i + 1: DISPLAYFORM2 • Complete Exclusion Axioms: Only one action can occur at each time step: DISPLAYFORM3 Finally, we can extract a plan by finding an assignment of truth values that satisfies Φ (i.e., for i = 0, . . .
...
In this section, we introduce the notion of an explanation in the following setting: Explanation Generation Problem: Given two knowledge bases KB 1 and KB 2 and a formula ϕ in a logic L. Assume that KB 1 |= L ϕ and KB 2 |= L ϕ.

The goal is to identify an explanation (i.e., a set of formulas) ⊆ KB 1 such that KB 2 ∪ |= ϕ.We first define the notion of a support of a formula w.r.t.

a knowledge base.

DISPLAYFORM0 Assume that is a support of ϕ w.r.t.

KB.

We say that ⊆ KB is a ⊆-minimal support of ϕ if no proper subtheory of is a support of ϕ. Furthermore, is a ¡-general support of ϕ if there is no support of ϕ w.r.t.

KB such that subsumes .We now define below two types of explanationsmodel-theoretic and proof-theoretic explanations.

Definition 2 (m-Explanation) Given two knowledge bases KB 1 and KB 2 in logic L and a formula ϕ. Assume that KB 1 |= L ϕ and KB 2 |= L ϕ.A model-theoretic explanation (or m-explanation) for ϕ from KB 1 for KB 2 is a support w.r.t.
...
Now, consider KB 4 = {¬a}. In this case, we have no m-explanation for c from KB 1 for KB 4 .Proposition 1 For two knowledge bases KB 1 and KB 2 in a monotonic logic L, if KB 1 |= L ϕ and KB 2 |= L ¬ϕ, then there exists no m-explanation for ϕ from KB 1 for KB 2 .The KB 4 in Example 2 and Proposition 1 show that m-explanations alone might be insufficient.

Sometimes, we also need to persuade the other agent that its knowledge base is not correct.

We leave this for the future.

In this paper, we assume that KB 2 |= L ¬ϕ and KB 2 |= L ϕ and, thus, an m-explanation always exists.

Definition 3 (p-explanation) Given a logic L with a sound and complete rule system Σ L and two knowledge bases KB 1 and KB 2 in logic L and a formula ϕ. Assume that KB 1 L ϕ and KB 2 L ϕ.A proof-theoretic explanation (or p-explanation) for ϕ from KB 1 for KB 2 is a proof r 1 ; . . . ; r n from KB 1 for ϕ such that DISPLAYFORM0 Example 3 Consider the theories KB 1 = {a, b, a → c, a ∧ b → c} and KB 2 = {a} from Example 2.

Let us assume that Σ L is the set of rules of the form l L l and l, ¬l ∨ p p for any literals l, p in the language of KB 1 and KB 2 .

Then, a, ¬a ∨ c L c is a proof from KB 1 for c, which is also a p-explanation for ϕ from KB 1 for DISPLAYFORM1 Proposition 2 Assume that Σ L is a sound and complete rule system of a logic L, KB 1 is a knowledge base, and ϕ is a formula in L. For each proof r 1 ; . . . ; r n from DISPLAYFORM2 Proposition 2 implies that each proof from KB 1 for ϕ could be identified as a p-explanation for ϕ from KB 1 if Σ L is sound and complete.

This provides the following relationship between m-explanations and pexplanations.

Proposition 3 Assume that Σ L is a sound and complete rule system of a logic L, KB 1 and KB 2 are two knowledge bases in L, and ϕ is a formula in L. Then,• for each m-explanation for ϕ from KB 1 for KB 2 , there exists a p-explanation r 1 ; . . . ; r n for ϕ from DISPLAYFORM3

Given KB 1 and KB 2 and a formula ϕ, there might be several (m-or p-) explanations for ϕ from KB 1 for KB 2 .

For brevity, we will now use the term xexplanation for x ∈ {m, p} to refer to an x-explanation for ϕ from KB 1 for KB 2 .
...
We assume a cost function C x L that maps pairs of knowledge bases and sets of explanations to nonnegative real values, i.e., C DISPLAYFORM0 where Ω is the set of x-explanations and R ≥0 denotes the set of non-negative real numbers.
...
At a high level, Algorithms 1 and 2 can be used for computing most-preferred explanations given a formula ϕ and two knowledge bases KB 1 and KB 2 of a logic L with the cost function C x L .
...
Proposition 5 For two propositional theories KB 1 and KB 2 and a formula ϕ, Algorithm 3 returns a most preferred m-explanation w.r.t.

C m L for ϕ from KB 1 to KB 2 if one exists.

Given a cost function C p L on p-explanations, Algorithm 4 computes a most-preferred p-explanation w.r.t.

C p L from KB 1 to KB 2 for ϕ or returns nil if none exists.
...
We also write ϕ1,ϕ2 ϕ to indicate that ϕ is the result of The algorithm uses the same two data structurespriority queue q and set checked -as in Algorithm 3.
...
DISPLAYFORM0 Proposition 6 For two propositional theories KB 1 and KB 2 and a formula ϕ, Algorithm 4 returns a most preferred p-explanation w.r.t.

C p L for ϕ from KB 1 to KB 2 if one exists.
...
Particularly, we form the knowledge base of (a) Experimental Results on Random Knowledge Bases the agent, namely KB, by adding the encoded formula Φ (represented in CNF clauses) as well as the optimal plan of the specific planning problem.

Then, we define the explanation in terms of KB and plan optimality as follows: DISPLAYFORM0 DISPLAYFORM1 Definition 6 (Optimal Plan Explanation) Given a knowledge base KB and a plan π n = a 0 , a 1 , . . .

, a n−1 , we say that π n is optimal in KB if and only if KB |= φ, where ∀t = 1, . . .

n − 1 : φ = ¬goal t .In other words, the formula φ that we seek to explain is that no plan of lengths 1 to n−1 exists, and that a plan of length n exists.

Therefore, combined, that plan must be an optimal plan.

Now, given a second knowledge base KB 2 (i.e that of a human user), where KB 2 |= φ, we can compute a model-or proof-theoretic explanation as defined in Definitions 2 and 3.
...
The formula ϕ that we seek to explain is one of the randomly chosen conclusions in the clauses generated, which we ensure is entailed by KB 1 .
...
The formula ϕ that we seek to explain is then that no plan of lengths 1 to h − 1 exists, and that a plan of length h (i.e., the plan found by FASTDOWNWARD) exists.
...
It is worth to pointing out that the problem of computing a most preferred explanation for ϕ from KB 1 to KB 2 might look similar to the problem of computing a weakest sufficient condition of ϕ on KB 1 under KB 2 as described by BID3 .
...
This is similar to our definition of the explanation generation problem where we want to identify an explanation ⊆ KB 1 (i.e., a set of formulae) such that KB 2 ∪ |= φ.
