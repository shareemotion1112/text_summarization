...
BID2 investigated RNNs with only elementwise linear recurrence relations h t = α t h t−1 + (1 − α t ) x t and developed linear variants of LSTM and GRU that perform similarly to standard non-linear RNNs on text generation tasks.
...
To avoid this problem, we will only consider diagonal matrices Λ t , in which case both matrix-matrix and matrix-vector multiplication have cost proportional to n and C pscan ∈ O(6n(T /p + lg p)) and C sscan ∈ O(2nT ).

This gives a parallel speedup factor of pT /3(T + lg p).

Assuming p T , then C pscan ≤ C sscan when p ≥ 3.As we are only considering diagonal matrices, we write the linear recurrence as h t = λ t h t−1 + x t where indicates elementwise multiplication.

Limiting Λ t to be diagonal may seem like a severe constraint but there are several reasons to do so beyond the favorable parallelization performance.
...
Applying the same matrix Λ to each sequence element is also unappealing considering that a matrix multiplication can be thought of as a rotation and a scaling.

The same rotation at every element seems unlikely to be useful, and the scaling is exactly what's captured in diagonal vectors λ t .

Recurrent coefficient vectors λ t provide enough flexibility to implement schemes such as exponential moving averages or a gating mechanism.
...
Gate g t uses the sigmoid activation function to give values in [0,1] for reasonable gating semantics, while impulse i t can use any activation function τ .
...
Consider evaluating an RNN with recurrence h t = σ(U h t−1 + V x t + b) from m inputs to n hidden units on a sequence of length T with minibatch size b using a serial evaluation strategy.
