...
At each timestep t the agent makes observations x t P X , takes actions a t P A, and receives rewards rpx t , a t q P R. Although we will in general make no assumptions about the inputs X , we will assume that the environments considered in this work have real-valued actions A " R d .In this standard setup, the agent's behavior is controlled by a policy π : X Ñ A which maps each observation to an action.

The state-action value function, which describes the expected return conditioned on first taking action a P A from state x P X and subsequently acting according to π, is defined as DISPLAYFORM0 and is commonly used to evaluate the quality of a policy.

While it is possible to derive an updated policy directly from Q π , such an approach typically requires maximizing this function with respect to a and is made complicated by the continuous action space.

Instead we will consider a parameterized policy π θ and maximize the expected value of this policy by optimizing Jpθq " ErQ π θ px, π θ pxqqs.

By making use of the deterministic policy gradient theorem BID19 one can write the gradient of this objective as DISPLAYFORM1 where ρ is the state-visitation distribution associated with some behavior policy.

Note that by letting the behavior policy differ from π we are able to empirically evaluate this gradient using data gathered off-policy.
...
By introducing the Bellman operator pT π Qqpx, aq " rpx, aq`γE " DISPLAYFORM2 whose expectation is taken with respect to the next state x 1 , we can minimize the temporal difference (TD) error, i.e. the difference between the value function before and after applying the Bellman update.

Typically the TD error will be evaluated under separate target policy and value networks, i.e. networks with separate parameters pθ 1 , w 1 q, in order to stabilize learning.
...
In order to introduce the distributional update we first revisit (1) in terms of the return as a random variable Z π , such that Q π px, aq " E Z π px, aq.
...
This is done by taking the expectation with respect to the action-value distribution, i.e. DISPLAYFORM2 Algorithm 1 D4PG Input: batch size M , trajectory length N , number of actors K, replay size R, exploration constant , initial learning rates α 0 and β 0 1: Initialize network weights pθ, wq at random 2: Initialize target weights pθ 1 , w 1 q Ð pθ, wq 3: Launch K actors and replicate network weights pθ, wq to each actor 4: for t " 1, . . .

, T do

Sample M transitions px i:i`N , a i:i`N´1 , r i:i`N´1 q of length N from replay with priority p i 6: DISPLAYFORM0 Compute the actor and critic updates DISPLAYFORM1 Update network parameters θ Ð θ`α t δ θ , w Ð w`β t δ w

If t " 0 mod t target , update the target networks pθ 1 , w 1 q Ð pθ, wq

If t " 0 mod t actors , replicate network weights to the actors 11: end for 12: return policy parameters θ DISPLAYFORM0 Sample action a " π θ pxq` N p0, 1q DISPLAYFORM1 Execute action a, observe reward r and state x 1 4:Store px, a, r, x 1 q in replay 5: until learner finishes As before, this update can be empirically evaluated by replacing the outer expectation with a samplebased approximation.
...
Note from Equations (2,4) that the actor and critic updates rely entirely on sampling from some state-visitation distribution ρ.
...
Here the actor and critic parameters are updated using stochastic gradient descent with learning rates, α t and β t respectively, which are adjusted online using ADAM BID7 .
...
In all experiments we use a replay table of size R " 1ˆ10 6 and only consider behavior policies which add fixed Gaussian noise N p0, 1q to the current online policy; in all experiments we use a value of " 0.3.
...
In the next section we will present a suite of simple control problems for which this value corresponds to α 0 " β 0 " 1ˆ10´4; for the following, harder problems we set this to a smaller value of α 0 " β 0 " 5ˆ10´5.
...
Categorical Following Bellemare et al. FORMULA0 , we first consider the categorical parameterization, a layer whose parameters are the logits ω i of a discrete-valued distribution defined over a fixed set of atoms z i .
...
The categorical projection of Bellemare et al. FORMULA0 is given by pΦpq i " ř ´1 j"0 h zi pz j qp j , @i, where h is a piecewise linear 'hat' function, DISPLAYFORM4
...
The reward is given by r " ψppalm height´o bj height ; c, mqwhere ψp ; c, mq is a soft indicator function similar to one described by BID2 ψp ; c, mq " " 1´tanhp w m q 2 if ą c, 1 otherwise.
...
The target angle is initialized uniformly, and then incremented on each time step using temporally correlated noise drawn from an Ornstein-Uhlenbeck process (σ=0.025˝, θ=0.01; Uhlenbeck & Ornstein 1930).

The reward consists of two multiplicative components r " ψpcos´1pobj yaxis||xy , obj target yaxis||xy qq; c rot , m rot qψpcos´1pobj zaxis , obj target zaxis q; c ori , m ori q FORMULA0 where c rot =5˝, m rot =40˝, c ori =45˝, m ori =45˝, and ||xy denotes projection onto the global xy plane.
