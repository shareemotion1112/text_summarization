...
The corresponding loss functions are DISPLAYFORM1 Finally, we consider the least-squares loss (LS) which corresponds to minimizing the Pearson χ 2 divergence between P and Q BID18 .
...
It normalizes the pre-activations of nodes in a layer to mean β and standard deviation γ, where both β and γ are parameters learned for each node in the layer.

The normalization is done on the batch level and for each node separately.

In contrast, with Layer normalization, all the hidden units in a layer share the same normalization terms β and γ, but different samples are normalized differently BID2 .
...
Finally, the Fréchet distance between these two Gaussians is computed, i.e. DISPLAYFORM1 where (µ x , Σ x ), and (µ y , Σ y ) are the mean and covariance of the embedded samples from P and Q, respectively.
...
Here, by model we denote everything that needs to be specified for the training -including all hyper-parameters, like learning rate, λ, Adam's β, etc.
...
Figure 11 presents heatmaps for hyperparameters, namely the learning rate, β1, β2, n disc , and λ for each combination of neural architecture and data set.
