...
A sentence S is represented by s = F θ (S) and its corresponding video V S by v s = G θ (V S ), where F (resp.

G) is a sentence (resp.

video) encoder parameterized by θ (resp.

θ ).

We propose to use a joint approach where the sentence encoder F θ is learned by jointly optimizing a textual objective L T (θ) on C T and a visual objective L V (θ, θ ) on C V .

So far, this method has only been applied to words, with good results BID29 BID53 .

Note that C T and C V are not parallel corpora but that θ is shared between both objectives; in other terms, sentence representations are influenced by their distinct textual and visual contexts.

Any sentence encoder F θ and textual objective L T can be used such as SkipTought BID28 , FastSent BID19 or QuickThought BID34 .
...
We translate this hypothesis into the loss L P = −ρ vis , where ρ vis = ρ(cos(s, s ), cos(v s , v s )) and ρ is the Pearson correlation.

The final multimodal loss is a linear combination of the aforementioned objectives, weighted by hyperparameters α T , α P and α C : DISPLAYFORM4 To evaluate the impact of visual semantics on sentence grounding, we examine several types of visual context.
...
We set: v = n k=1 β k i k , where β k = softmax(< w u w , N.i k >).
...
We note these variants M I V (α T , α P , α C ), which depend on:• the initialization I ∈ {p, ∅}: the sentence encoder F θ is either pretrained using the textual objective L T (I = p), or initialized randomly (I = ∅).• the visual representation V ∈ {F, A, T, R}: where F , A or T are the video modelings described in Section 2.3.
...
We note P I V (α T ) the variants of this baseline using the global loss DISPLAYFORM1 • Sequential (SEQ): Inspired by Collell Talleda et al. FORMULA2 , we learn a linear regression model (W, b) to predict the visual representation from the SkipThought representations.
...
To analyze the quality of the textual space, we report some measures (computed in %) defined on the MSVD test set:• ρ vis measures if the similarities between sentences correlate with the similarities between videos.• E intra = E vs=v s [cos(s, s )] measures the homogeneity of each cluster, by measuring the average similarity of sentences within a cluster.• E inter = E vs =v s [cos(s, s )] measures how well clusters are separated from each other (i.e. average similarity between sentences of two different clusters).

Videos are sampled at a 3 frames per second rate; afterwards, frames are processed using a pretrained VGG network BID45 .

The multimodal loss L is optimized with Adam optimizer (Kingma & Ba, 2014) and a learning rate λ = 8.10 −4 .

Hyperparameters are tuned using the Pearson correlation measure on SICK trial: m = m = 0.5, µ = 2.5.10 −4 , and mini-batch size of 32 for L V .
...
Finally, regarding the different ways to encode a video, we observe that leveraging more than one frame can be slightly beneficial to learn grounded sentence representations, e.g. A obtains +3.3% average relative improvement over F on ρ SICK P earson .
...
Using only perceptual information in M p logically leads to highly correlated textual and visual spaces (highest ρ A vis ), but the local neighborhood structure is not well preserved (lowest E intra and highest E inter ).
...
However, the main difference lies in the fact that M b is better at preserving the geometry of the visual space (higher ρ Table 4 .3 reports the effectiveness of the sentence embeddings obtained from our scenarios and baselines on semantic relatedness and classification tasks.
...
Second, we notice that a high α T leads to high classification scores, whereas a low α T leads to high semantic relatedness scores.

There is a trade-off between semantic relatedness and classification scores, that we can set properly by tuning α T .
