...
LearnerUpdate(k, w): // Run on learner k B ←(split P k into batches of size B) for each local epoch ε from 1 to E do for each batch b ∈ B do w ← w − η∇ (w; b) end for end for return w to server Contributions.
...
is the model parameters after the τ -th local iterations in the round t + 1 (R is the number of local iterations of each learner per round).
...
Under the FedProx, in order to make local updates do not deviate excessively from the current global model parameters, at each round t each learner uses the following surrogate loss function that adds a proximal term to the original objective function: (w) + µ 2 w − w t−1 2 .
...
− E φ 2 at each round, by allowing each learner k collect E φ 2 k as well (2018)).
...
Also, it is additionally remarked that for Batch Renormalization we simply used α = 0.01, r max = 2, and d max = 2 in the experiments (see Ioffe (2017) for the description of the three hyperparameters).
