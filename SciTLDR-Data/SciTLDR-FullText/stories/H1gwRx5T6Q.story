...
For example, to measure the compatibility between a node z i and z j , let y ij be the binary variable that is 1 if and only if z j is the parent of z i , and let b j be a bag for z i and z j respectively, then the potential ψ (0) for "bag 0" scores a coreference decision as: DISPLAYFORM0 where w is a real-valued weight and t is a real-valued translation parameter for potential ψ (0) .
...
To understand simhash, it is first helpful to consider the following randomized process: Imagine that we have two vectors a and b on the unit hypersphere in the Euclidean space R d with angle θ between them, and we want to produce an estimate of cos(θ).
...
Charikar BID7 notes that for some applications, 1 − θ π may be a good enough approximation of cos(θ), so that one can use Xn directly as an estimator of cos(θ).otherwise.
...
Thus, by counting the number of positions where the hashes are distinct and dividing by n, we get X n , thereby producing an estimate of cos(θ).Rather than constructing the hash function H by sampling the u 1 , . . .
...
Statistical Properties Recall that since E[π(1 − X n )] = θ, we can derive a plausible estimate of cos(θ) from X n .

In particular, let g(x) = cos(π(1−x)) and consider the estimator C n = g(X n ).
...
DISPLAYFORM0 Since g is continuous, by the continuous mapping theorem BID40 ], DISPLAYFORM1 The first degree Taylor series for g(x) about µ is: DISPLAYFORM2 where R is the remainder term.
...
Moreover, previous work shows that the norm-preserving property of a linear random projection A implies an angle-preserving property BID24 ; that is, for vectors u, v, let θ = (v, u) andθ = (Av, Au).

The result is the following: DISPLAYFORM0 where DISPLAYFORM1 and n ≥ 60 −2 log m.

Therefore, we are justified in using the statistics underlying simhash to directly estimate the cosine similarity; viz., cos θ ≈ cosθ.
...
The Jaccard similarity between two sets S 1 , S 2 ∈ Ω is J = |S 1 ∩S 2 | |S 1 ∪S 2 | .

Minhash applies a random permutation (which in practice is accomplished using a random hash function, such as seeded 64-bit murmur hash) π : Ω → Ω, on a given set S ⊂ Ω and then extracts the minimum value h π (S) = min(π(S)).
...
In practice multiple (n) hash functions are employed to reduce the variance, resulting in a vector v S = h π 0 (S), · · · , h π n−1 (S) of n minimum values, one per hash function.
