...
The original Inception score BID23 relies on the 'crispness' of class predictions, whereas the Fréchet Inception distance (FID) BID12 and the kernel Inception distance (KID) BID2 statistically compare high-level representations instead of the final network outputs.
...
This approach is more robust against rotations than a naïve estimate by simply counting the pixels.
...
After estimating the second-order image moments, we define the slant based on the horizontal shear: DISPLAYFORM0 where x ij is the intensity of pixel (i, j), and (ī,j) are the centroid coordinates.
...
FIG0 ).We sweep the image top-to-bottom with a horizontal boundary to compute a vertical marginal cumulative distribution function (CDF), and likewise left-to-right with a slanted boundary for a horizontal marginal CDF, with angle α as computed above.
...
Given a radius R, a centre location r 0 and a strength parameter γ > 1, the coordinates r of pixels within distance R of r 0 are nonlinearly warped according to a radial power transform: DISPLAYFORM0 leaving the remaining portions of the image untouched and resampling with bicubic interpolation.

In the experiments and released dataset, we set γ = 7 and R = 3 √ θ/2, where θ is thickness.

Unlike simple linear scaling with θ, this choice for R produces noticeable but not exaggerated effects across the thickness range observed in the dataset (cf.
...
We exemplify this scenario with a vanilla GAN BID9 ) and a β-VAE , both with generator (resp.

decoder) and discriminator architecture as used in the MNIST experiments in BID6 , and encoder mirroring the decoder.

We train a β-VAE with β = 4 and a GAN, both with 64-dimensional latent space.
...
The test results in TAB0 seem to confirm the mismatch of the low-dimensional GAN's samples, whereas the β-VAE and larger GAN do not show a significant departure from the data distribution.
