...
Consider an agent, interacting with the environment by responding to the states s t , t ≥ 0, from the state space S, which are assumed to be also the observations, with actions a t from the action space A chosen by the policy distribution π θ (·|s t ), where θ are the parameters of the policy.

The initial state distribution is ρ 0 : S → R. Every time the agent produces an action, the environment gives back a reward r(s t , a t ) ∈ R, which serves as a feedback on how good the action choice was and switches to the next state s t+1 according to the transitional probability P (s t+1 |s t , a t ).

Altogether, it can be formalised as an infinite horizon γ-discounted Markov Decision Process (S, A, P, r, ρ 0 , γ), γ ∈ [0, 1) BID28 BID19 .

The expected discounted return BID3 ) is defined as per BID19 : DISPLAYFORM0 The advantage function A π BID2 , the value function V π and the Q-function Q π are defined as per ; BID19 : DISPLAYFORM1 DISPLAYFORM2 DISPLAYFORM3 In all above definitions s 0 ∼ ρ 0 (s 0 ), a t ∼ π(a t |s t ), s t+1 ∼ P (s t+1 |s t , a t ), and the policy π = π θ is defined by its parameters θ.

A straightforward approach for learning a policy is to perform unconstrained maximisation ρ(π θ ) with respect to the policy parameters θ.
...
Therefore, they proposed to replace the unconstrained optimisation with a similar constrained optimisation problem, the Trust Region Policy Optimisation (TRPO) problem: DISPLAYFORM0 where D KL is the KL divergence between the old and the new policy π θ old and π θ respectively, and δ is the trust region radius.
...
We define a generalised Q-function for multiple policies {π 1 , . . .

, π n , . . .

, π N } as DISPLAYFORM0 DISPLAYFORM1 |s n t , a n t ), a n t ∼ π n (a n t |s n t ).(9) We also define the generalised value function and the generalised advantage function as DISPLAYFORM2 DISPLAYFORM3 DISPLAYFORM4 P (s → x, k, π), as in BID24 , is the probability of transition from the state s to the state x in k steps using policy π.

Theorem 1.

For the set of policies {π 1 , . . .

, π N } the following equality will be true for the gradient: DISPLAYFORM5 where θ are the joint parameters of all policies {π n } and b πn (s) is a bias function for the policy.

The proof of Theorem 1 is given in Appendix B. Applying a particular case of the bias function b πn (s) = −V π (s) and using the likelihood ratio transformation, one can get DISPLAYFORM6
...
Ψ = Update the value function estimator parameters {Stage 4} For every path in R p , estimate the advantage function using Equation (23).{Stage 5} Update parameters of the policy θ for N ITER PL UPDATE iterations using the gradient from Equation FORMULA1 and a barrier function defined in Equation (26). end while During Stage 1, the data are collected for every path until the termination state is received, but at least TIMESTEPS PER BATCH steps in total for all paths.
...
For the constrained optimisation we add the linear barrier function to the function ρ(θ): DISPLAYFORM9 where α > 0 is a barrier function parameter and θ old are the parameters of the policy on the previous iteration.
...
As the underlying numerical optimisation algorithms are out of the scope of the paper, the parameters of K-FAC optimiser from have been used for the experiments; for the Adam algorithm BID11 , the default parameters from Tensorflow BID0 implementation (β 1 = 0.9, β 2 = 0.999, = 1 · 10 −8 ) have been used.
...
The difference between the two k-th estimators is given as DISPLAYFORM1 By substituting this into the GAE estimator difference one can obtain ∆Ã πn (s t , a t ) = (1 − λ)(γ∆V 1 + λγ 2 ∆V 2 + λ 2 γ 3 ∆V 3 + . . .
