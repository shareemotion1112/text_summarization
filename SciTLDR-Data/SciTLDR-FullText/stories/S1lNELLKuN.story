...
That is, G B→A (G A→B (a)) ≈ a for all a ∈ A. Formally, the cycle-consistency loss for translation from A to B and back is defined as: DISPLAYFORM1 Symmetrically, an additional cycle consistency term The full objective optimized by a CycleGAN is given as: DISPLAYFORM2 DISPLAYFORM3 where λ A→B and λ B→A are hyperparameters controlling the relative strength of the cycle consistent terms.
...
DISPLAYFORM4 where λ A ≥ 0 and λ B ≥ 0 are hyperparameters that reflect the strength of the MLE terms for domains A and B respectively.
...
For different choices of λ A and λ B , we cover the following three cases:1.

Adversarial training only: For λ A = λ B = 0, we recover the CycleGAN objective in Eq. 4, with the additional benefits of exact cycle consistency and a single invertible generator.

In this case, the prior over Z plays no role in learning.

On the other extreme for large values of λ A , λ B such that λ A = λ B → ∞, we can perform pure maximum likelihood training to learn the invertible generator.

Here, the critics C A , C B play no role since the adversarial training terms are ignored in Eq. 11.3.

Hybrid:

For any finite, non-zero value of λ A , λ B , we obtain a hybrid objective where both the adversarial and MLE terms are accounted for during learning.

AlignFlow can be used for both conditional and unconditional sampling at test time.

For conditional sampling, we are given a datapoint b ∈ B and we can draw the corresponding cross-domain translation in domain A via the mapping G B→A .For unconditional sampling, we require λ A = 0 since doing so will activate the use of the prior p Z via the MLE terms in the learning objective.
...
For finite non-zero values of λ A and λ B , the AlignFlow objective consists of three parametric models: one generator G B→A ∈ G, and two critics C A ∈ C A , C B ∈ C B .
...
For hybrid models, we set λ A = λ B .
...
DISPLAYFORM0 Z→B globally optimizes all the individual loss terms in the AlignFlow objective in Eq. 11, it globally optimizes the overall objective for any value of λ A ≥ 0, λ B ≥ 0.
...
In the following result, we identify one such class of non-identifiable model families for the MLE-only objective of AlignFlow (λ A = ∞, λ B = ∞).
...
For a symmetric prior p Z (e.g., isotropic Gaussian), there exists an optimal solution G † B→A ∈ G to the AlignFlow objective (Eq. 11) for λ A = λ B = ∞ such that G † B→A = G * B→A .Proof.

We will prove the proposition via contradiction.

That is, let's assume that G * B→A is a unique solution for the AlignFlow objective for λ A = λ B = ∞ (Eq. 11).
...
When λ A = λ B = ∞, for any choice of C A , C B we have: DISPLAYFORM3 The above equation implies that G † B→A is also an optimal solution to the AlignFlow objective in Eq. 11 for λ A = λ B = ∞. Thus, we arrive at a contradiction since G * B→A is not the unique maximizer.
