...
Our model combines the advantage of a sequence-to-sequence model that captures inter-dependencies within sequence of predictions and a SELECT`Height (ft)Ẁ HERE Name="Willis Tower" AND Location="Chicago" DISPLAYFORM0 What is the height of Willis Tower in Chicago?Figure 1: Our running example.
...
Formally, we let P θ (y|x) = P θ (a|x), where θ is model parameters.
...
We first assume that we have some context-sensitive representations r The main component of our decoder is to model a probability distribution P θ (a|x, a <i ) over potential parser actions a conditioned on input x and past actions a <i .
...
In that scenario, Equation 1 is reduced to a naïve crossentropy loss.
...
The choice of meaning representation is usually task-dependent, including lambda calculus BID31 , lambda dependency-based compositional semantics (Liang, 2013, λ-DCS) , and SQL BID36 .
