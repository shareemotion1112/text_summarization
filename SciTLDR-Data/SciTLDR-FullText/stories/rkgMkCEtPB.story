...
More formally, we first define our base model to be neural network with meta-initialization parameters θ; let this be represented by f θ .
...
i signify θ after i gradient updates for task T b , and let θ (b) 0 = θ.

In the inner loop, during each update, we compute θ
...
) is the loss on the target set of T b after m inner loop updates, making clear the dependence of f θ (b) m on θ.

The outer optimization loop then updates θ as
...
For the ANIL algorithm, mathematically, let θ = (θ 1 , ..., θ l ) be the (meta-initialization) parameters for the l layers of the network.

Following the notation of Section 3.1, let θ
...
We plot the per-layer average Euclidean distance between the initialization θ and the finetuned weights θ
...
Consider a simple, two layer linear network with a single hidden unit in each layer:ŷ(x; θ) = θ 2 (θ 1 x).

In this example, θ 2 is the head.
...
where L(·, ·) is the loss function, (e.g. mean squared error) and θ
...
Considering the update for θ 1 in more detail for our simple, two layer, linear network (the case for θ 2 is analogous), we have the following update for MAML:
...
Note the lack of inner loop update for θ 1 , and how we do not remove second order terms in ANIL (unlike in first-order MAML); second order terms still persist through the derivative of the inner loop update for the head parameters.
