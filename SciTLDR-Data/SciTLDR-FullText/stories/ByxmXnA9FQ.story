...
Under a Bayesian framework with data pair D = (x, y), we propose to use variational inference to approximate such "true" latent distribution P(z) = p(z|y) by a parameterized Dirichlet posterior p θ (z|x), which is approximated by a deep neural network.
...
Therefore, we propose to parameterize a posterior model p θ (z|x) and optimize its parameters to approach such "true" posterior p(z|y) given a pairwise input (x, y) by minimizing their KL-divergence D KL (p θ (z|x)||p(z|y)).

With the parameterized posterior p θ (z|x), we are able to infer the higher-order distribution over z given an unseen image x * and quantitatively study its statistical properties to estimate the higher-order uncertainty.

In order to minimize the KL-divergence D KL (p θ (z|x)||p(z|y)), we leverage the variational inference framework to decompose it into two components as follows (details in appendix): DISPLAYFORM1 where L(θ) is better known as the variational evidence lower bound, and log p(y) is the marginal likelihood over the label y. DISPLAYFORM2 Since the marginal distribution p(y) is constant w.r.t θ, minimizing the KL-divergence D KL (p θ (z|x)||p(z|y)) is equivalent to maximizing the evidence lower bound L(θ).

Here we propose to use Dirichlet family to realize the higher-order distribution p θ (z|x) = Dir(z|α) due to its tractable analytical properties.

The probability density function of Dirichlet distribution over all possible values of the K-dimensional stochastic variable z can be written as: DISPLAYFORM3 where α is the concentration parameter of the Dirichlet distribution and DISPLAYFORM4 is the normalization factor.

Since the LHS (expectation of log probability) has a closed-formed solution, we rewrite the empirical lower bound on given dataset D as follows: DISPLAYFORM5 where α 0 is the sum of concentration parameter α over K dimensions.
...
Here, we assume the prior distribution is as Dirichlet distribution Dir(α) with concentration parametersα and specifically talk about three intuitive prior functions in FIG2 .

The first uniform prior aggressively pushes all dimensions towards 1, while the *-preserving priors are less strict by allowing one dimension of freedom in the posterior concentration parameter α.

This is realized by copying the value from k th dimension of posterior concentration parameter α to the uniform concentration to unbind α k from KL-divergence computation.

Given the prior concentration parameterα, we can obtain a closed-form solution for the evidence lower bound as follows: DISPLAYFORM6 Γ denotes the gamma function, ψ denotes the digamma function.

We write the derivative of L(θ) w.r.t to parameters θ based on the chain-rule: DISPLAYFORM7 ∂θ , where is the Hardamard product and DISPLAYFORM8 is the Jacobian matrix.

In practice, we parameterize Dir(z|α) via a neural network with α = f θ (x) and re-weigh the two terms in L(θ) with a balancing factor η.

Finally, we propose to use mini-batch gradient descent to optimize the network parameters θ as follows: DISPLAYFORM9 where B(x, y) denotes the mini-batch in dataset D. During inference time, we use the marginal probability of assigning given input x to certain class label i as the classification evidence: DISPLAYFORM10 Therefore, we can use the maximum α's index as the model prediction class during inferenceŷ = arg max i p(y = i|x) = arg max i α i .

After optimization, we obtain a parametric Dirichlet function p θ (z|α) and compute its entropy E as the higher-order uncertainty measure.

Formally, we write the such metric as follows: DISPLAYFORM0 where α is computed via the deep neural network f θ (x).

Here we use negative of entropy as the confidence score C(α).

By investigating the magnitude distribution of concentration parameter α for in-distribution test cases, we can see that α is either adopting the prior α = 1.0 or adopting a very large value α 1.0.
...
In order to resolve such issue, we propose to generally decrease model's confidence by smoothing the concentration parameters α, the smoothing function can lead to opposite behaviors in the uncertainty estimation of in-and out-of-distribution data to enlarge their margin.

Concentration smoothing In order to construct such a smoothing function, we experimented with several candidates and found that the log-smoothing functionα = log(α + 1) can achieve generally promising results.
...
Detection For each input x, we first use input perturbation to obtainx, then we feed it into neural network f θ (x) to compute the concentration α, finally we use log-scaling to calibrate α and compute C(α).

Specifically, we compare the confidence C(α) to the threshold δ and say that the data x follows in-distribution if the confidence score C(α) is above the threshold and that the data x follows out-of-distribution, otherwise.
...
For each sample fed into the neural network, we will calculate the Dirichlet entropy based on the output concentration α, which will be used to predict which distribution the samples come from.
...
By gradually increasing the weight of KL loss (increasing the balancing factor η from 0 to 10), we plot their training loss curve in FIG8 .

With a too strong KL regularization, the model's classification accuracy will decrease significantly.

As long as η is within a rational range, the classification accuracy will become stable.
...
For the very small value η → 0, the variational Dirichlet framework degrades into a marginal log-likelihood, where the concentration parameters are becoming very erratic and untrustworthy uncertainty measure without any regularization.

For larger η > 1, the too strong regularization will force both in-and outof-distribution samples too close to prior distribution, thus erasing the difference between in-and out-of-distribution becomes and leading to worse detection performance.

We find that adopting a hyper-parameter of η = 0.01 can balance the stability and detection accuracy.
