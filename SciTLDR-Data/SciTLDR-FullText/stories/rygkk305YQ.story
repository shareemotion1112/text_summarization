...
The joint probability can be written as: Specifically, it is assumed that p(y l ) = K −1 to be a non-informative prior to encourage every component to be used, and p(z l | y l ) = N (µ y l , diag(σ y l )) to be diagonal-covariance Gaussian with learnable means and variances.
...
Each observed class (e.g. each speaker) forms a mixture component in this continuous space, whose conditional distribution is a diagonal-covariance Gaussian p(z o | y o ) = N (µ yo , diag(σ yo )).
...
For this purpose, we adopted a per-dimension LDA, which computed a between and within-mixture scattering ratio: DISPLAYFORM0 , where µ y l ,d and σ y l ,d are the d-th dimension mean and variance of mixture component y l , andμ l,d is the d-th dimension mean of the marginal dis- DISPLAYFORM1 .
...
To determine the effective range of the target dimension, we approximate the multimodal distribution as a Gaussian and evaluate values spanning four standard deviationsσ l,d around the mean.
...
Their F 0 tracks, obtained using the YIN (De Cheveigné & Kawahara, 2002) F 0 tracker, are shown on the left.
...
To objectively evaluate the ability of the proposed GMVAE-Tacotron model to control individual attributes, we compute two metrics: (1) the average F 0 (fundamental frequency) in voiced frames, computed using YIN (De Cheveigné & Kawahara, 2002) , and (2) the average speech duration.
