...
We model the parameters of each Gumbel Softmax distribution M with a single-layer feedforward neural network followed by applying a log softmax, which induces the log-probabilities of the th distribution: ω = log(softmax(W h + b)).

W and b are shared across all tokens, to have a constant number of parameters with respect to the sequence length.

We control the sharpness of the distributions with the temperature parameter τ .
...
Finally, the Classifier block contains for each aspect a i a two-layer feedforward neural networks followed by a softmax layer to predict the sentimentŷ ai .

Multi-Aspect Masker Trained on sent and no constraint Trained on sent with λ p , sel , and cont i stayed at daulsol in september 2013 and could n't have asked for anymore for the price ! !
...
The first objective to optimize is the sentiment loss, represented with the cross-entropy between the true aspect sentiment label y ai and the predictionŷ ai :
...
For the first term sel , we calculate the probability p sel of tagging a word as aspect and then compute the cross-entropy with a parameter λ p .

The hyper-parameter λ p can be interpreted as the prior on the number of selected words among all aspects, which corresponds to the expectation of Binomial(p sel ), as the optimizer will try to minimize the difference between p sel and λ p .
...
Finally, we train our Multi-Aspect Masker in an end-to-end manner, and optimize the final loss M AM = sent +λ sel · sel +λ cont ·

cont , where λ sel and λ cont control the impact of each constraint.
...
We used Adam (Kingma & Ba, 2015) for training with a learning rate of 0.001, β 1 = 0.9, and β 2 = 0.999.

The temperature τ for Gumbel-Softmax distributions was fixed at 0.8.

The two regularizer terms and the prior of our model are λ sel = 0.03, λ cont = 0.04, and λ p = 0.11 for the Filtered Beer dataset; λ sel = 0.03, λ cont = 0.03, and λ p = 0.15 for the Full Beer dataseet; and λ sel = 0.02, λ cont = 0.02 and λ p = 0.10 for the Hotel dataset.
