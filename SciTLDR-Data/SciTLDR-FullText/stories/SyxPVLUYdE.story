...
Latent variables can also be used to define complex variational distributions q. As before, we define q(z|x) = q(z|λ, x)q(λ|x)dλ in terms of tractable conditional distributions q(z|λ, x) and q(λ|x).

BID0 show that DISPLAYFORM0 where r(λ|z, x) is a variational distribution meant to model q(λ|z, x), and the identity follows from the fact that q(z|x) = q(z,λ|x) q(λ|z,x) .

Similar to Eq. (1), Eq. (2) shows the gap introduced by using r(λ|z, x) to deal with the intractability of q(z|x).
...
In other words, we can define auxiliary variables λ and distributions q(λ|x), q(z|λ, x), r(λ|z, x) such thatp DISPLAYFORM2 with z, λ ∼ q(z, λ|x).
...
In this case, the auxiliary latent variables λ are the locations of the proposal samples z 1:K and the index of the selected sample, i.
...
This model draws samples from a proposal π(x), weights them according to a potential function U (x), and then draws a sample from the empirical distribution formed by the weighted samples.
...
As K → ∞, p SN IS (x) becomes proportional to π(x) exp(U (x)).

For finite K, p SN IS (x) interpolates between the tractable π(x) and the energy model π(x) exp(U (x)).

Interestingly, log π(x) only shows up once in the lower bound, and simply lower-bounding it still gives a lower bound on log p SN IS (x).

This expands the class of allowable distributions for the proposal π to include Variational Autoencoders (VAEs) BID20 BID38 .To train the SNIS generative model, we can perform stochastic gradient ascent on Eq. (6) with respect to the parameters of the proposal distribution π and the potential function U .

When the data

Require: Proposal distribution π(x) and potential function U (x).

DISPLAYFORM0 Sample x k ∼ π(x).
...
In fact, if we consider π(x) as our noise distribution p N (x) and set U (x) =Ũ (x) − log p N (x), then up to a constant, we recover the ranking NCE loss.
...
Published as a workshop paper at ICLR 2019Then, plugging Eqs. (8) and (9) into Eq. (7) with λ = (z 1:K , i) gives log p(x) ≥ E q(z,λ|x) log p(x, z)r(λ|z, x) q(z, λ|x) = E q(λ|x) log p(x, z i )
