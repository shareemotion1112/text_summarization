...
We use ∈ R drawn from Gaussian distribution such that ∼ N (0, σ 2 ) to reflect the variance of human domain knowledge from true Y .

We use σ = 0.05 throughout this paper.

We need a training algorithm A({X 1 , y 1 }, X 2 , Q}) such that it trains a multi-layer perceptron DISPLAYFORM0 , where W i is model's weights, Φ is an nonlinear function like ReLU, and σ is the softmax function.
...
Inspired by BID11 and BID5 , the activation of final softmax layer of classifier σ(·) can reflect neural network's confidence towards a certain label.
...
Then our updating loss function is DISPLAYFORM2 , where λ 1 , λ 2 and λ 3 are hyperparameter constants.
...
In Bootstrap phase, we require users to provide an entry supervision mark {Θ}. It marks entries that has unusually high probabilities of occurrence from target probability distribution.
...
Then given a batch {X, y} ∈ R c×m × R n , the mean-pooling Ω = 1 c c i=0 f (X i ) is a probability distribution.

Proof of Lemma 1.1 Without loss of generality, we write function f (x) as σ(·) • g(x) for some function g(x).

Then for all x ∈ X, f (x) is in range of σ(·), that is, ∃y ∈ R n , f (x) = σ(y).

Then for a vector z ∈ R n , for any label i, DISPLAYFORM6 Then we perform mean-pooling on σ(y) i , we have DISPLAYFORM7 The pooled output activation Ω i is trivially larger than 0, and since σ(z) i < 1 for all z, DISPLAYFORM8 Then we conclude it fullfills all the axiom of a probability distribution.
...
For simplicity, we choose λ 1 , λ 2 and λ 3 to be 1.

A rule of thumb is to make λ 1 original ≈ λ 2 c .
