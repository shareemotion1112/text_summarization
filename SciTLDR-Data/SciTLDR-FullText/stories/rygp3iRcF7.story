...
As the first step to explore area attention, we define the key of an area, µ i , simply as the mean vector of the key of each item in the area.
...
DISPLAYFORM4 where φ is a nonlinear transformation such as ReLU, and DISPLAYFORM5 DISPLAYFORM6 A is the maximum size of an area, which is S in the one dimensional case and W H in the 2-dimensional case.
...
By dividing v x1,y1,x2,y2 with the size of the area, we can easily compute µ x1,y1,x2,y2 .

Based on the summed area table, σ DISPLAYFORM9 The core component for computing these quantities is to be able to quickly compute the sum of vectors in each area after we obtain the integral image DISPLAYFORM10 (H−h)×(W −w) ; Fill tensor with value h for the height of each area; Vector mean µ, standard deviation σ and sum U as well as height S h and width S w of each area.

1 Acquire U , S h and S w using Algorithm 1 with input G; 2 Acquire U using Algorithm 1 with input G G where is for element-wise multiplication; 3 µ ← U S where is for element-wise division; 4 µ ← U S ; 5 σ ← µ − µ µ ; 6 return µ, σ, U , as well as S h and S w .
