...
Let DISPLAYFORM0 Let L be a function dependent on the normalized activations z i defined for each j by z j = (x j − µ) σ This, along with partial derivatives, are shown in the left figure.
...
BID10 defined BN as DISPLAYFORM0 where σ, µ are batch moments, but b and c are learned per-channel parameters persistent across batches.
...
We also ignore a numerical stability hyperparameter .We examine back-propagation of partial derivatives through this normalization, where µ and σ are viewed as functions of x. Notably, µ and σ are functions of each x i , and thus the division by σ is not affine.

We write the normalized output as DISPLAYFORM1 We review ordinary least squares of a single variable with intercept BID2 .Let g j = α + βz j + j where α and β are parameters, z and g are observations.
...
We wish to fit α and β α,β = arg min DISPLAYFORM2 The least-squares problem in equation 3 is satisfied byβ = Cov(z, g) DISPLAYFORM3 When z are normalized activations and g are partial derivatives, then Ez = 0 and Var(z) = 1.

In this special case, the solution simplifies intô DISPLAYFORM4 Theorem 1 (Normalization gradients are least-squares residuals).

Let i ∈ {1 . . .

N } be indices over some set of activations {x i }.

Then the moment statistics are defined by µ = N i=1x i N and DISPLAYFORM5 Let L be a function dependent on the normalized activations z i defined for each j by z j = (x j − µ) σ .
...
DISPLAYFORM9 Through substitution of equations 10 into 9, we get DISPLAYFORM10 Noting that {z i } normalized over i has mean 0 and variance 1, we recoverβ andα, in the sense of equations 4 and 5, from equation 13.
...
As illustrated in Figure 1 , during back-propagation, the residual then divides away σ to become ∂L ∂x (·) , the gradient for the unnormalized activations.
...
It uses an analogous set of parameters λ k and activations w k for variances.

We sketch the back-propagation of a simplified version of SN in the perspective of gradient-least-squares.

We ignore both the division and downstream affine z → c · z + b. The normalization calculation inside SwN can be written as: DISPLAYFORM1 where Ω = {BN, LN, IN }.
...
We define helper quantities u i = x i − µ. Note that each u j depends on all of x i via µ. Next, we write out useful identities DISPLAYFORM2 We prepare to differentiate with rule of total derivative: DISPLAYFORM3 Making use of equations 21, 22, 23 and 25, We simplify ∂σ ∂x i for any i as follows.
