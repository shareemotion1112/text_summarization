...
from a possibly unknown distribution ν, a model that is parametrized by w ∈ R M ; so that the number of examples is N and the number of parameters of the system is M .
...
We wish to find a parameter w * such that w * = arg min L(w) where, DISPLAYFORM0 (f (w, x i ), y i ).In particular, one is curious about the relationship between L(w) andL(w) := d(ν).
...
However, if (f (ŵ)) and ∇ 2 f (ŵ) are not correlated, then we can ignore the second term.
...
Also, the width of the nontrivial distribution essentially depends on the ratio α.
...
If Σ = diag( , 1, . . . , 1), > 1, and M, N → ∞ with M N = α ≥ 1.

Let c = 1 + √ α, and let's call the top eigenvalue of the sample covariance matrix as λ max then:• If 1 ≤ < c then λ max is at the right edge of the spectrum with Tracy-Widom fluctuations.• If c < then λ max is an outlier that is away from bulk centered at (1 + α −1 ) with Gaussian fluctuations.

Typically, due to the correlations in the problem we don't have Σ to be the identity matrix or a diagonal matrix with spikes.
...
If d = M , Σ − Id has bounded rank, log N is comparable to log M , and entries of X are independent with mean zero and variance one, then the spectrum of Σ can be precisely mapped to the one of G as M, N → ∞ for fixed α = M N .
...
It is straightforward to calculate that the gradient per sample is of the form g = c(w, x)Id M x for a positive constant c = c(w, x) that doesn't depend on y. This case falls into the classical Marčhenko-Pastur law (left pane of FIG2 .
