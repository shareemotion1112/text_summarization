...
The decoding distribution p(o t |s t ) is a unit variance Gaussian with the meanō t predicted by a convolutional decoder.
...
The GCP model outputs a sequences of observationsô t ...ô T leading from the start to the goal.
...
Instead of targeting the predicted observationsô t with an inverse model, it is also possible to train the inverse model to infer actions based on the encodings of the observationsô t = dec(ê t ).
...
We provide evaluations of all models on the perceptual metrics Fréchet Video Distance (FVD, Unterthiner et al. (2018) ) and Learned Perceptual Image Patch Similarity (LPIPS, ) in the appendix, Tab.
...
For each method and dataset, we performed a manual sweep of the hyperparameter β in the range from 1e−0 to 1e−4.

The convolutional encoder and decoder both have five layers.

We use the Rectified Adam optimizer (Liu et al., 2019; Kingma & Ba, 2015) with β 1 = 0.9 and β 2 = 0.999, batch size of 16 for GCP-sequential and 4 for GCP-tree, and a learning rate of
