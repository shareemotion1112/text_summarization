...
We model the world as an infinite horizon discounted Markov Decision Process (MDP), which is characterized by the tuple: M = {S, A, R, T , γ}. S ∈ R n and A ∈ R m represent the continuous (real-valued) state and action spaces respectively.

R : S × A → R represents the reward function.

T : S × A × S → R + represents the dynamics model, which in general could be stochastic, and γ ∈ [0, 1) is the discount factor.

A policy π : S × A → R + describes a mapping from states to actions.

The value of a policy at a state is the average discounted reward accumulated by following the policy from the state: DISPLAYFORM0 The overall performance of the policy over some start state distribution β is given by: DISPLAYFORM1 For notational simplicity, we use s to denote the next state visited after (from) s.
...
The optimal policy can be recovered from the value function as: DISPLAYFORM1 For more complex MDPs, computing the optimal value function exactly is not tractable except in a few well known cases like the LQR (Åström & Murray, 2004) and LMDPs BID46 BID12 .

Thus, various approximate techniques have been considered in prior works.

One popular approach is fitted value iteration BID9 Munos & Szepesvári, 2008) , where a function approximator (e.g. neural network) is used to approximate the optimal value function.

The core structure of fitted value iteration considers a collection of states (or a sampling distribution ν), and a parametric value function approximatorV θ .

Inspired by value iteration, fitted value iteration updates parameters as: DISPLAYFORM2 where BV θi (s) are targets for the regression problem computed at the specific state s according to Eq. (1).

After sufficient iterations of the procedure in Eq. (2) to get a good approximation, the policy is recovered asπ(s) = arg max a E[r(s, a) + γV θ (s )].

The success and convergence of this overall procedure depends critically on at least two components: the capacity and structure of the function approximator (θ); and the sampling distribution (ν).Lemma 1.

BID9 LetV be an approximate value function with ∞ error := max s |V (s) − V * (s)|.

Letπ(s) = arg max a E[r(s, a) + γV (s )] be the induced greedy policy.

For all MDPs and β, the bound in Eq. (3) holds.

Furthermore, for any size of the state space, there exist MDPs andV for which the bound is tight (holds with equality).

DISPLAYFORM3 Intuitively, this suggests that performance ofπ degrades with a dependence on effective problem horizon determined by γ.
...
In practice, we are often interested in temporally extended tasks where γ ≈ 1, and hence this possibility is concerning.

Furthermore, the arg max operation inπ could inadvertently exploit approximation errors to produce a poor policy.

The performance of fitted value iteration based methods also rely critically on the sampling distribution to propagate global information (Munos & Szepesvári, 2008) , especially in sparse reward settings.
...
DISPLAYFORM0 Here, we use x, u,π as dummy variables for states, actions, and policy to distinguish the "imagined" evolution of the MDP used for the trajectory optimization with the actual states (s) observed in the true evolution of the MDP.
...
Let {π * k } be the local time-indexed policies obtained as the solution to the optimization problem in (4).

After solving the optimization problem, the first local time-indexed policy is used asπ M P C (·|s t ) :=π * t (·|x t ).
...
Suppose the terminal reward in Eq. FORMULA6 is chosen as r f (s H ) =V (s H ), and let the MPC policy bê π M P C (·|s t ) :=π * t (·|x t ) (from Eq. 4).

Then, for all MDPs and β, the performance of the MPC policy can be bounded as: DISPLAYFORM0 Proof.
...
Also, without a terminal value function, we have = O(r max /(1 − γ)) in the worst case, which adds an undesirable scaling with the problem horizon.
...
Note that one way to realize B H is to simply apply B H times, with each step providing a contraction by γ.
...
The agent can consider a hypothesis of potential reward regions in the state space, and then execute the optimal trajectory conditioned on this belief, resulting in a Algorithm 1 Plan Online and Learn Offline (POLO) 1: Inputs: planning horizon H, value function parameters θ 1 , θ 2 , . . .

θ K , mini-batch size n, number of gradient steps G, update frequency Z 2: for t = 1 to ∞ do 3:Select action a t according to MPC (Eq. 4) with terminal reward r f (s) ≡V (s) from Eq. FORMULA13 4:Add the state experience s t to replay buffer D Sample n states from the replay buffer, and compute targets using Eq. FORMULA14 8:Update the value functions using Eq. (6) (see Section 2.5 for details) 9: end for 10:end if 11: end for temporally coordinated sequence of actions.
...
Consider a motivating setting of regression, where we have a parametric function approximator f θ with prior P(θ).

The dataset consists of input-output pairs: DISPLAYFORM0 , and we wish to approximate P(θ|D).

In the Bayesian linear regression setting with Gaussian prior and noise models, the solution to the following problem generates samples from the posterior BID25 BID4 BID26 : DISPLAYFORM1 DISPLAYFORM2 is a noisy version of the target andθ ∼ P(θ) is a sample from the prior.
...
We consider K value function approximatorsV θ with parameters θ 1 , θ 2 , . . .

θ K independently trained based on Eq. (6).

We consider the softmax of the different samples as the value at a state: DISPLAYFORM3 Since the above scheme approximates mean + variance for small κ > 0, this procedure encourages the agent to additionally explore parts of the state space where the disagreement between the function approximators is large.
...
After every Z steps of acting in the world and collecting experience, the value functions are updated by: (a) constructing the targets according to Eq. (8); (b) performing regression using the randomized prior scheme using Eq. (6) where f θ corresponds to the value function approximator.

For state s in the buffer and value network k with parameters θ k , the targets are constructed as: DISPLAYFORM0 which corresponds to solving a N −step trajectory optimization problem starting from state s.
...
The importance of having a wide and relevant state distribution has been pointed out in numerous prior works (Munos & Szepesvári, 2008; BID6 BID32 .
