...
A formal regular grammar is represented as the tuple (N, Σ, P, N 0 ) where N is a finite non-empty set of non-terminals, Σ is a finite set of terminals (or output symbols), P is a set of production rules, and N 0 is the starting non-terminal symbol, N 0 ∈ N .

Productions rules in a regular grammar are of the form A → aB, A → b, and A → , where A, B ∈ N , a, b ∈ Σ, and is the empty Figure 1 : Overview of the adversarial grammar model.
...
That is, at the heart of the proposed method is learning nonlinear function G : N → {(N, Σ)} that maps a non-terminal to a set of (non-terminal, terminal) pairs.
...
The model contains a number of nonterminals and terminals which are learned: |N | non-terminals of dimensionality D, and |Σ| terminals of dimensionality C (the latter naturally correspond to the number and dimensionality of the desired outputs).
...
Activity Prediction For activity prediction, the number of non-terminals (|N |) was set to 64, the number of terminals (|Σ|) was set to the number of classes in the dataset (e.g., 65 in MultiTHUMOS and 157 in Charades).
...
3D Pose estimation For 3D pose, the number of non-terminals (|N |) was set to 1024, the number of terminals (|Σ|) was set to 1024, where each terminal has size of 128 (32 joints in 4D quaternion representation).
