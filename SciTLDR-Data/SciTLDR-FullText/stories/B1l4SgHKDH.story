...
where P LM (x) is a locally normalized language model which is fixed during training, and E θ is the energy function parameterized by θ.

The resulting model P θ (x) is globally normalized due to the energy term.
...
Third, training can be made efficient by using the conditional noise contrastive estimation objective (Gutmann & Hyvärinen, 2010) .
...
In particular, we adopt the conditional noise contrastive estimation (NCE) objective (Ma & Collins, 2018; Gutmann & Hyvärinen, 2010 ) to our residual model energy function and then sample from the joint model using importance sampling.
...
where Z θ (x 1 , · · · , x p ) is a normalizing factor known as partition function.
...
We call P θ the joint model, and E θ the residual energy function since P LM is fixed throughout training.
...
Instead, we train our residual energy function using Noise Contrastive Estimation (NCE) (Gutmann & Hyvärinen, 2010) , and more specifically its conditional version (Ma & Collins, 2018) .

NCE requires two distributions: The model distribution and a noise distribution.

In our case, the model distribution is the joint model of Eq. 2, P θ , while the noise distribution is the pretrained language model, P LM .

NCE then trains a binary classifier on the difference of log-probability scores of these two models.

Since our joint model is the product of the energy function (whose parameters we want to learn) with P LM , the difference reduces to: log P θ − log P LM = −E θ .
...
If P LM has the same support as P data , then the objective function in Eq. 3 reaches its maximum at log P LM (x) − E θ (x) = log P data , if there exists such θ.

Proof.

This theorem directly follows from the proof in Gutmann & Hyvärinen (2010) .
...
, we derive two estimators for the log-partition function log Z θ based on the work of Nowozin (2018) .
...
log P RALM (x t |x <t ) = log P LM (x t |x <t ) + log P φ (x t |x <t ) + const (6) where P φ takes the form of another auto-regressive language model.

The parameters of P φ are trained by exact maximum likelihood training of P RALM .
...
If the joint model p θ matches the data distribution p d , then statistics computed on a large population of samples from the two distributions should also match.
