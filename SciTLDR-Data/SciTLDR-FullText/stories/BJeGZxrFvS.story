...
These tests turn the gradient into a random vector, and if ξ 1 , ξ 2 are random vectors, then x ξ 1 and x ξ 2 are visually quite similar when x is an image.
...
Let y and µ x,y be random variables denoting respectively the logit value for label y on input x and the corresponding saliency map.

For simplicity we assume the saliency method correlates with a certain ground truth saliency as follows.

There exists a subset S x,y of pixels (e.g., the pixels of x that a human may find salient for label y) such that for each pixel in S x,y the corresponding coordinate of µ x,y is distributed as N (c, σ 2 1 ) and each pixel in S x,y is distributed as N (0, σ 2 2 ) where c = c(x, y) and σ 1 , σ 2 are constants.
...
Now µ x,y · 1 is the sum of pixel saliencies (where 1 is the all-1's vector), which by completeness, is y .
...
After applying competition, the saliency map changes to µ x,y I x,y where I x,y is the vector with 1 in coordinates where label y wins the competition for pixels, and 0 elsewhere.

Now we note various plausible conditions under which µ x,y ⊗ I x,y can be seen as a reasonable approximation to µ x,y .

For simplicity we consider how well competition preserves completeness, though obviously other properties can be analysed.

Theorem 1 (Informal meta-theorem) For many plausible conditions on the S x,y 's there is a scale factor γ > 0 (depending on the conditions) such that the vector γµ x,y I x,y satisfies completeness up to a small additive error.

Proof (sketch) One uses measure concentration.

As an illustrative example suppose S x,y 's are disjoint for different y's.

For each coordinate in S x,y there is a certain probability p = p(c, σ 1 , σ 2 ) that the coordinate is nonzero in I x,y .

Thus the expected sum of coordinates in the map µ x,y I x,y is p|S x,y |, and so rescaling coordinates by c/p makes them satisfy completeness, up to an additive error given by measure concentration.
...
Theorem 3 (Modified meta theorem) If a saliency method satisfies (α, β)-approximate completeness, then under various plausible conditions on the S x,y 's similar to Theorem 1 there are constants (α , β ) such that after applying competition the maps satisfy (α , β )-approximate completeness.
