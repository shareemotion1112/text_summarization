...
Therefore 0 is an eigenvalue of L and we write the n eigenvalues of L as 0 = λ 0 ≤ λ 1 ≤ · · · ≤ λ n−1 with corresponding n × 1 orthonormal eigenvectors 1/ √ n = ϕ 0 , ϕ 1 , . . .

, ϕ n−1 .

If the graph G is connected, then λ 1 > 0.
...
Since ϕ 0 is constant and every other eigenvector is orthogonal to ϕ 0 , it is natural to view the eigenvectors ϕ k as the Fourier modes of the graph G, with a frequency magnitude √ λ k .

Let x : V → R be a signal defined on the vertices of the graph G, which we will consider as an n × 1 vector with entries x(v ).

It follows that the Fourier transform of x can be defined as x(k) = x · ϕ k , where x · y is the standard dot product.
...
Thus each Ψ j x partially recovers x(k) for k ≥ 1.

The value Ψ j x(v ) aggregates the signal information x(v m ) from the vertices v m .
...
Generally, the smaller j the higher the frequencies Ψ j x recovers in x. These high frequency wavelet coefficients up to the scale 2 J are denoted by: DISPLAYFORM3 Since 2 J controls the maximum scale of the wavelet, in the experiments of Sec. 4 we select J such that 2 J ∼ diam(G).
...
We can separate and recapture the high frequencies of x by computing its wavelet coefficients Ψ (J) x, which were defined in (2).

However, Ψ (J) x is not invariant to permutations of the vertex indices; in fact, it is covariant (or equivariant).

Before summing the individual wavelet coefficient vectors Ψ j x, though, we must first apply a pointwise nonlinearity.

Indeed, define the n × 1 vector d(v ) = deg(v ), and note that Ψ j x · d = 0 since one can show that d is a left eigenvector of P with eigenvalue 1.

If G is a regular graph then d = c1 from which it follows that Ψ j x · 1 = 0.
...
As in (3), we extract invariant coefficients from |Ψ j x| by computing its moments, which define the first order geometric scattering moments: DISPLAYFORM4 These first order scattering moments aggregate complimentary multiscale geometric descriptions of G into a collection of invariant multiscale statistics.
...
Furthermore, such layer by layer statistics have proven useful in characterizing signals of other types (e.g., texture synthesis in BID19 .A graph wavelet transform Ψ (J) x decomposes the geometry of G through the lens of x, along different scales.
