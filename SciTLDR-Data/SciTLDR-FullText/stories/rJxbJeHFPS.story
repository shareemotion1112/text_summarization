...
Many neural networks can represent algorithms (Pérez et al., 2019) .
...
Thus, algorithm steps that perform computation over many objects may require many samples for an MLP module to learn, since the number K of polynomials or β
...
In our analysis, the Lipschitz constants and the universe size are constants going into O( ) and O(δ).
...
One subset sum algorithm is exhaustive search, where we enumerate all 2 |S| possible subsets τ and check whether τ has zero-sum.
...
where φ and g are parameterized by MLPs.

If φ is a function that ignores X t so that φ (X s , X t ) = ρ(X s ) for some ρ, e.g., by letting part of the weight matricies in φ be 0, then we essentially get a Deep Sets in the following form.

For any such ρ, we can get the corresponding φ via the construction above.

Hence for any Deep Sets, we can express it with an one-layer GNN.

The same result applies to GNNs with multiple layers (message passing iterations), because we can express a function ρ(X s ) by the composition of multiple ρ (k) 's, which we can express with a GNN layer via our construction above.
...
Let and δ be the given error rate and failure probability.

Moreover, suppose we choose some error rate 0 and failure probability δ 0 for the output/function of each entry.
...
Now let us bound the overall error rate and failure probability given 0 and δ 0 for each entry.

The probability that we fail to learn each of the g i is at most δ 0 .

Hence, by a union bound, the probability that we fail to learn any of the g i is at most m · δ 0 .

Thus, with probability at least 1 − mδ 0 , we successfully learn all g i for i = 1, ..., m, so the error for every entry is bounded by 0 .

The error for the vector output is then at most m i=1 0 = m 0 .

Setting mδ 0 = δ and m 0 = gives us δ 0 = δ m and 0 = m .

Thus, if we can successfully learn the function for each output entry independently with error /m and failure rate δ/m, we can successfully learn the entire vector-valued function with rate and δ.
...
We have input S ∼ D, and our goal is to bound g(S) −ĝ(S) with high probability.
...
Hence, by the learnability assumption on A 1 , f 1 (x) −f 1 (x) < with probability at least 1 − δ.

The error for the input of N 2 is then O( ) with failure probability O(δ), because there are a constant number of terms of aggregation of f 1 's output, and we can apply union bound to upper bound the failure probability.

Next, we proceed by induction.

Let us fix a k. Let z denote the input for f k , which are generated by the previous f j 's, and letẑ denote the input forf k , which are generated by the previousf j 's. Assume z −ẑ ≤ O( ) with failure probability at most O(δ).
...
Since there are a constant number of terms for aggregation, our goal is then to bound f (ẑ) − f (z) .
...
Moreover,f is generated by the learning algorithm A on the perturbed samples, i.e.,f = A ({ẑ i , y i }).

By the algorithm stability assumption, f andf should be close if the input samples are only slightly perturbed.

It then follows that

where z i andẑ i are the training samples at the same layer k. Here, we apply the same induction condition as what we had for z andẑ: z i −ẑ i ≤ O( ) with failure probability at most O(δ).

We can then apply union bound to bound the probability of any bad event happening.

Here, we have 3 bad events each happening with probability at most O(δ).

Thus, with probability at least 1 − O(δ), we have
...
where β ij has 1 at the i-th entry, −1 at the j-th entry and 0 elsewhere.

Hence β ij p · p = 4.

It then follows from Theorem 3.5 and union bound that it takes O(( 2 + log(1/δ))/ˆ 2 ) to learn g, wherê = 2 andδ = 2 δ.

Here, as we have discussed above, the same aggregation errorˆ occurs in the aggregation process of f , so we can simply considerˆ for both.

Thus, comparing O(log(1/δ)/ˆ 2 ) and O((
...
The model Neural Exhaustive Search (NES) enumerates all possible non-empty subsets τ of S, and passes the numbers of τ to an MLP, in a random order, to obtain the hidden feature.
