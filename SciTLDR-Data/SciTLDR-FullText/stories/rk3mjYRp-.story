...
Policy gradient methods BID28 ; BID18 look to directly maximize the functional of expected reward under a certain policy π.

π(a|s) is the probability of taking action a in state s under policy π.

A policy can hence be identified to a probability measure π ∈ P, the space of all policies.
...
Let r(a) be the reward obtained by taking action a. The expected reward with respect to a policy π is: DISPLAYFORM0 Shannon entropy is often added as a regularization term to improve exploration and avoid early convergence to suboptimal policies.
...
We are interested in the process of policy iteration, that is, finding a sequence of policies (π n ) converging towards the optimal policy π * .

In this section we follow closely the exposition by BID23 .

Policy iteration is often implemented using gradient ascent according to DISPLAYFORM0 Rearranging gives the explict Euler method DISPLAYFORM1 In this article we are more interested in the implicit Euler method which simply replaces ∇J(π DISPLAYFORM2 This is a policy iteration method.

If integrated and interpreted as an L 2 regularized iterative problem, it is strictly equivalent to finding a solution to the proximal problem: DISPLAYFORM3 Rather than just the L 2 distance between policies for constraining and regularization, one can envision the more general case of any policy distance d: DISPLAYFORM4 DISPLAYFORM5 A gradient flow is obtained in the small step limit τ → 0.

This proximal mapping is an example of Moreau envelope, and remains in a convex optimization setting when J is convex.

The Wasserstein distance W 2 is defined for pairs of measures (µ, ν) seen as marginals of a coupling γ by: DISPLAYFORM6 The optimal coupling γ * in the infimum above is also called the optimal transport plan.

Also note that later, we will reformulate this Monge- Kantorovich problem Kantorovich. (1942) as an equivalent linear problem of inner product minimization in L 2 (P 2 ): DISPLAYFORM7 DISPLAYFORM8 π(0) = π 0 (12) which describes a gradient flow.
...
Hence all absolutely continuous curves, or flows of measures, (π t ) in W 2 (P) are solutions of the Euler continuity equation.
...
In the case we are looking at: DISPLAYFORM10 where δJ δπ (π) is the first variation density defined via the Gateaux derivative as δJ δπ DISPLAYFORM11 for every perturbation ξ = π − π.
...
It remains to compute the first variation density δJ δπ (π) for entropy regularised policy gradients.
...
The entropy-regularized rewards J is convex in the policy π, which means there is a single optimal policy.
...
Given that we converge to the optimal policy, we know that at optimality ∂ t π = 0.

Using equation 16 then gives us a necessary condition for the optimal policy π * : DISPLAYFORM0 By setting δJ δπ (π) = 0 and substituting in 20 we get DISPLAYFORM1 which gives us the optimal policy DISPLAYFORM2 This is the Gibbs measure of the rewards density per action -also seen as an energy-based policy, in line with the static result of BID22 .

The unregularized case β = 0 appears degenerate.

The gradient flow associated with the pure entropy functional βH is the heat equation DISPLAYFORM0 Here the Laplacian ∆ is in action space.

This is one of the key messages of the derivations we have done in this part: the Wasserstein gradient flow turns the entropy into the Laplacian operator 2 .For the full, entropy-regularized reward J(π), there is an extra transport term generated by the rewards, and the PDE is therefore the Fokker-Planck equation.

This means that taking policy gradient ascent steps in W 2 (according to equation 8), is equivalent to solving the Fokker-Planck equation for the policy π with potential field equal to the gradient of rewards r. Alternately, we can say, the policy undergoes diffusion and advection -it concentrates around actions with high reward.

A partial differential equation for diffuse measures also admits a particle interpretation.

The result above can also be written, through Ito's lemma BID21 , as the stochastic diffusion version of the Fokker-Planck equation BID14 : DISPLAYFORM0 with Π t a finite dimensional discretization of policy π t , and B t a Brownian motion of same dimensionality.
...
We note the Kullback-Leibler and Wasserstein-2 problems are related by the Talagrand pinequalities BID13 , which for p = 2 and under some conditions, ensure that for some constant C and given a suitable reference measure ν, ∀µ ∈ P(X), W 2 2 (µ, ν) ≤ C · D KL (µ, ν).
...
We admit BID23 ) that the solution of the minimization problem has to satisfy: DISPLAYFORM1 with φ W2 a Kantorovich potential function for transport cost c(x, y) = 1 2 |x − y| 2 .
...
BID7 gives proofs of Γ-convergence of W 2, to W 2 .
...
If we let be a small positive real number, then we define W 2 2, as a regularization of the W 2 minimization in equation 9: DISPLAYFORM0 with the entropy H(γ) now extended to two-dimensional coupling space Γ(µ, ν) as, in the discretized case, with cardA = q, DISPLAYFORM1 and by an analogue of continuity named Γ-convergence Carlier et al. FORMULA0 , W DISPLAYFORM2 This change from a linear to a convex problem makes the solution set better conditioned numerically; the solution does not have to lie on a vertex of a convex polytope (by analogy with the simplex algorithm, see BID19 ) anymore, and therefore, is more robust to initial conditions.
...
This yields the equivalent problem, as detailed in Peyré (2015) DISPLAYFORM3 At this stage, the link with earlier sections becomes intuitively very clear, since the reference measure .
...
Assume we give ourselves a convex function Ψ and that we consider the associated Bregman divergence D Ψ Amari.

FORMULA0 defined by DISPLAYFORM0 We look to minimize this Bregman divergence D Ψ on the intersection of convex sets C = ∩ C i .In our case of interest there are two such sets C 1 and C 2 .
...
We now proceed to explicit the P DΨ C [n] operators, which in our case are P KL C [n] KL proximal steps, and integrate them into an efficient practical algorithm.

We start with the need to minimize the convex form 1 ij p ij log p ij + p ij c ij , subject to marginal constraints that the discretized measure µ is transported by p onto ν.

From a matrix perspective this translates into the two following constraints: that the sum in column of P being equal to vector µ, and the sum across lines of P equal to ν: DISPLAYFORM0 The matrix set U (µ, ν) is convex.

We can form the Lagrangian of this optimization problem in p ij using vector Lagrangian multipliers α, β, DISPLAYFORM1 A necessary condition for optimality is then DISPLAYFORM2 Hence we have shown that the optimal coupling is a diagonal scaling of the ground cost's Gibbs kernel.

With the two positive vectors u, v defined as diag(u) = e DISPLAYFORM3 as min DISPLAYFORM4 then this reads as a two-dimensional KL projection algorithm of pointγ on set of marginal constraints C 1,µ enforcing u (Kv) = µ, and DISPLAYFORM5 and ultimately yields the Sinkhorn balancing algorithm: DISPLAYFORM6 these two updates being merged in Algorithm 1.
...
The 2-dimensional, relaxed coupling γ can be discretized to a 2-dimensional matrix P with entries (p i,j ).

We show (see Appendix) that necessarily DISPLAYFORM0 where the matrix exponential of the ground cost is taken term-by-term.

Recalling the equality constraints on the row and column sums given by µ and ν in 39, we find that we have to solve a matrix balancing problem, using the terminology of linear algebra.

Once we have formed K, and have policies µ and ν as inputs, we can run through iterations of the Sinkhorn algorithm till convergence to a fixed point.

This is done below and runs a one-line while loop on vector (x) i , the component-wise inverse of (u) i .

Dotted operations are taken component-wise:Algorithm 1 Computation of policy transport W c (µ, ν) by Sinkhorn iteration.

Input C, , µ, ν.

I = (µ > 0); µ=µ(I); c = c(I, :) ; K=exp(-C/ ) x=ones(length(µ),size(ν,2))/length(µ); while x has not converged do x=diag(1./µ)*K*(ν.*(1./(K'*(1./x)))) end while u=1./x; v=ν.*(1./(K'*u)) W c (µ,ν)=sum(u.*((K.*C)*v)) ∂W c (µ,ν) ∂µ = log u (up to a constant parallel shift)The Sinkhorn algorithm converges linearly.
...
Under this form, the algorithm is compatible with function approximation, where policy µ is a function of a parameter and reads µ θ .
