...
That is, given a distribution family P (P rog|X, θ) for a parameter set θ, we choose θ * = arg max θ i log P (Prog i | X i , θ).

Then, g(X) = arg max Prog P (Prog|X, θ * ).The key innovation of our approach is that here, learning happens at a higher level of abstraction than (X i , Prog i ) pairs.
...
Let us define a random variable Y = α(Prog).

We assume that the variables X, Y and Prog are related as in the Bayes net in Figure 2 .

Specifically, given Y , Prog is conditionally independent of X. Further, let us assume a distribution family P (Y |X, θ) parameterized on θ.
...
Here, τ 0 , τ 1 , . . . range over a finite set of API data types that AML programs can use.
...
We abstract this call into an abstract method call "call τ.a(τ 1 , . . .

, τ k )", where τ is the type of e and τ i is the type of e i .
...
Equation 1 leaves us with the problem of computing arg max θ i log P (Y i |X i , θ), when each X i is a label and Y i is a sketch.
...
We describe this decoder in Appendix C. As for P (Z|X, θ), this distribution can, in principle, be picked in any way we like.

In practice, because both P (Y |Z, θ) and P (Z|X, θ) have neural components with numerous parameters, we wish this distribution to regularize the learner.
...
Note that this particular P (Z|X, θ) only follows directly from the Normal ( 0, I) prior on Z and Normal likelihood P (X|Z, θ) if the encoding function f is 1-1 and onto.
...
Now that we have chosen P (X|Z, θ) and P (Y |Z, θ), we must choose θ to perform CLE.

Note that: DISPLAYFORM4 where the ≥ holds due to Jensen's inequality.

Hence, L(θ) serves as a lower bound on the loglikelihood, and so we can compute θ * = arg max θ L(θ) as a proxy for the CLE.

We maximize this lower bound using stochastic gradient ascent; as P (Z|X i , θ) is Normal, we can use the reparameterization trick common in variational auto-encoders BID14 while doing so.

The parameter set θ contains all of the parameters of the encoding function f as well as σ Types , σ Calls , and σ Keys , and the parameters used in the decoding distribution funciton P (Y |Z, θ).
...
For example, the term "x 1 .a(x 2 ); τ 1 .b(τ 2 )", which sequential composes an abstract method call to b and a "concrete" method call to a, is a PCS.
...
There are two main differences: (i) the GSNN's decoder is also conditioned directly on the input label X in addition to Z, which we accomplish by concatenating its initial state with the encoding of X, (ii) the GSNN loss function has an additional KL-divergence term weighted by a hyper-parameter β.

We subjected the GSNN to the same training and crossvalidation process as our model.

In the end, we selected a model that happened to have very similar hyper-parameters as ours, with β = 0.001.
...
Each method name a is associated with a type signature (τ 1 , . . .

, τ k ) → τ 0 , where τ 1 , . . . , τ k are the method's input types and τ 0 is its return type.
