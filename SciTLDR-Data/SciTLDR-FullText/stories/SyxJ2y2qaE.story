...
The classical theory of strongly convex optimization tells us that optimization is slow when the spectrum simultaneously contains very small and very large eigenvalues (i.e., optimization rate is dependent on κ = λ max /λ min ).
...
Let H ∈ R n×n be the Hessian of the training loss function with respect to the parameters of the model: DISPLAYFORM0 where θ ∈ R n is the parameter vector, and L(θ) is the training loss.
...
Let λ 1 ≥ λ 2 · · · ≥ λ n be the eigenvalues of the Hessian.

The theory of convex optimization suggests that first-order methods such as SGD slow down dramatically when the relative differences among the eigenvalues of the loss Hessian are large; in particular, results from convex analysis suggest that as | λi λ1 | becomes smaller, the optimization in the direction of the eigenvectors associated with λ i slows down BID1 BID2 1 .

Following this intuition, when the distribution of the eigenvalues of H has heavy tails (equivalently large outliers), we expect the network to train slowly as there will be many eigenvalues where λ i /λ 1 is small.
...
In our case, we compute these statistics on the Hessian eigenvalues by observing that for v ∼ N (0, I n ): DISPLAYFORM3 Due to the rapid concentration of the quadratic form in high dimensions (for concrete bounds, see BID7 ) we expect extremely accurate approximation of E[λ k ] using a few i.i.d.
