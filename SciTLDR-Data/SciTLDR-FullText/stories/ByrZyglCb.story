...
Under this approximation, the vector r(x) hence captures the local geometry of the decision boundary in the vicinity of datapoint x. We assume a local decision boundary model in the vicinity of datapoints x ∼ µ, where the local classification region of x occurs in the halfspace r(x)T v ≤ r(x) 2 2 .

Equivalently, we assume that outside of this half-space, the classifier outputs a different label thank(x).

However, since we are analyzing the robustness to universal directions (and not vectors), we consider the following condition, given by DISPLAYFORM1 (3) where B(ρ) is a ball of radius ρ centered at 0.
...
In all the theoretical results of this paper, we assume that r(x) 2 = 1, for all x ∼ µ, for simplicity of the exposition.
...
Let S be an m dimensional subspace such that P S r(x) 2 ≥ 1 − ξ for almost all x ∼ µ,, where P S is the projection operator on the subspace.

Assume moreover that L s (x, ρ) holds for almost all x ∼ µ, with ρ = √ em δ(1−ξ) .

Then, there exists a universal noise vector v, such that v 2 ≤ ρ and DISPLAYFORM2 The proof can be found in supplementary material, and relies on the construction of a universal perturbation through randomly sampling from S. The vulnerability of classifiers to universal perturbations can be attributed to the shared geometric properties of the classifier's decision boundary in the vicinity of different data points.
...
Hence, by applying the result with ξ = 0, and m = L − 1, we obtain that linear classifiers are vulnerable to universal noise, with magnitude proportional to √ L − 1.
...
Under this decision boundary model, we study the existence of universal perturbations satisfyingk(x + v) =k(x) for most x ∼ µ. the decision boundary is positively curved for many data points.
...
Recalling the definitions of Sec. 2, a quadratic approximation of the decision boundary at z = x + r(x) gives x + {v : DISPLAYFORM0 , where H z denotes the Hessian of F at z, and α x = ∇F (z) 2 r(x) 2 , with F = f i − f j .
...
We assume a local decision boundary model in the vicinity of datapoints x ∼ µ, where the local classification region of x is bounded by a quadratic form.

Formally, we assume that there exists ρ > 0 where the following condition holds for almost all x ∼ µ: DISPLAYFORM1 An illustration of this quadratic decision boundary model is shown in Fig. 2b .
...
In Fig. 7 (a) , the average of κ S (x) across points sampled from the validation set is shown (as well as the standard deviation) in function of the subspace dimension m, for a LeNet architecture BID17 trained on the CIFAR-10 dataset.
...
Then, DISPLAYFORM0 with β 1 (δ, m) = max((1/e)δ 2/m , 1 − 2(1 − δ 2/m ), and β 2 (δ, m) = 1 + 2 DISPLAYFORM1 We use the above lemma to prove our result, which we recall as follows: DISPLAYFORM2 Let S be an m dimensional subspace such that P S r(x) 2 ≥ 1 − ξ for almost all x ∼ µ,, where P S is the projection operator on the subspace.

Assume moreover that L s (x, ρ) holds for almost all x ∼ µ, with ρ = √ em δ(1−ξ) .

Then, there exists a universal noise vector v, such that v 2 ≤ ρ and DISPLAYFORM3 Proof.

Define S to be the unit sphere centered at 0 in the subspace S. Let ρ = √ em δ(1−ξ) , and denote by ρS the sphere scaled by ρ.
...
Hence, the above expression simplifies to DISPLAYFORM5 where we have used the assumption of the projection of r(x) on the subspace S. Hence, it follows from Lemma 1 that DISPLAYFORM6 Hence, there exists a universal vector v of 2 norm ρ such that DISPLAYFORM7

Theorem 2.

Let κ > 0, δ > 0 and m ∈ N. Assume that the quadratic decision boundary model DISPLAYFORM0 where H r(x),v z = Π T H z Π with Π an orthonormal basis of span(r(x), v), and S denotes the unit sphere in S. Then, there is a universal perturbation vector v such that v 2 ≤ ρ and DISPLAYFORM1 Proof.

Let x ∼ µ. We have To bound the above probability by δ, we set = C ρ √ m , where C = 2 log(2/δ).

We therefore choose ρ such that DISPLAYFORM2 The solution of this second order equation gives DISPLAYFORM3 Hence, for this choice of ρ, we have by construction DISPLAYFORM4 We therefore conclude that E v∼ρS P x∼µ k (x + v) =k(x) ≥ 1 − δ − β.

This shows the existence of a universal noise vector v ∼ ρS such thatk(x + v) =k(x) with probability larger than 1 − δ − β.
