...
the σ-algebra σ X , and similarly B(X , L) be the same bounded by L < ∞.

A Markov Decision Process (MDP) is defined as a 5-tuple M =< S, A, P, R, γ >, where S is the state space, A is the action space, P : S × A → S is the transition distribution where P(s |s, a)

is the probability of reaching state s when performing action a in state s, R : S × A × S → R is the reward function, and γ ∈ (0, 1] is the discount factor.

A deterministic policy π maps, for each state, the action to perform: π : S → A. Given a policy π, the value of an action a in a state s represents the expected discounted cumulative reward obtained by performing a in s and following π thereafter:

, where r i+1 is the reward obtained after the i-th transition.

The expected discounted cumulative reward is maximized by following the optimal policy π * which is the one that determines the optimal action values, i.e., the ones that satisfy the Bellman optimality equation (Bellman, 1954) :

>

where t ∈ {1, . . . , T } and T is the number of MDPs.

For each MDP M (t) , a deterministic policy π t :
...
where γ tki are independent standard normal variables.

We also need to define the following quantity, taken from Maurer (2016): let γ be a vector of m random standard normal variables, and f ∈ F :
...
, f T ), w = (w 1 , . . . , w T ), and define the minimizers of Equation (3) asŵ, h, andf .
...
The minimum task-averaged risk, given the set of tasks µ and the hypothesis classes W, H and F is ε * avg , and the corresponding minimizers are w * , h * and f * .
...
with E(ε 0 , . . . , ε K−1 ; r) = K−1 k=0 α 2r k ε k , the two coefficients c VI1,ρ,ν , c VI2,ρ,ν , the distributions ρ and ν, and the series α k are defined as in Farahmand (2011).

In the multi-task scenario, let the average approximation error across tasks be:

where Q t,k+1 =f t,k •ĥ k •ŵ t,k , and T * t is the optimal Bellman operator of task t. In the following, we extend the AVI bound of Theorem 1 to the multi-task scenario, by computing the average loss across tasks and pushing inside the average using Jensen's inequality.
...
We bound the task-averaged approximation error ε avg at each AVI iteration k involved in (8) following a derivation similar to the one proposed by Maurer et al. (2016) , obtaining: Theorem 3.

Let µ, W, H and F be defined as above and assume 0 ∈ H and f (0) = 0, ∀f ∈ F. Then for δ > 0 with probability at least 1 − δ in the draw ofZ ∼ T t=1 µ n t we have that
...
The last term to be bounded in (9) is the minimum average approximation error ε * avg at each AVI iteration k. Recalling that the task-averaged approximation error is defined as in (7), applying Theorem 5.3 by Farahmand (2011) we obtain:
...
The w t block maps each input x t from task µ t to a shared set of layers h which extracts a common representation of the tasks.
...
We want to empirically evaluate the benefit of our theoretical study in the problem of jointly learning T different tasks µ t , introducing a neural network architecture for which our bounds hold.

Following our theoretical framework, the network we propose extracts representations w t from inputs x t for each task µ t , mapping them to common features in a set of shared layers h, specializing the learning of each task in respective separated layers f t , and finally computing the output (Figure 1(a) ).
...
In both cases, the targets are learned for each task µ t in its respective output block f t .
...
Figure 1(b) shows the L 1 -norm of the difference between Q * and Q π K averaged over all the tasks.
...
We compute the average expected loss across tasks: (11) with γ = max

VI,ρ,ν (K; t, r), and R max,avg = 1 /T T t=1 R max,t .
...
avg (ε avg,0 , . . . , ε avg,K−1 ; r)

with ε avg,k = 1 /T T t=1 ε t,k and E avg (ε avg,0 , . . . , ε avg,K−1 ; r) =

Proof of Lemma 4.

Let us start from the definition of optimal task-averaged risk:

where Q * t,k , with t ∈ [1, T ], are the minimizers of ε avg,k .

Consider the taskť such thatť
...
with E(ε 0 , . . . , ε K−1 ; r) = K−1 k=0 α 2r k ε k , the three coefficients c PI1,ρ,ν , c PI2,ρ,ν , c PI3,ρ,ν , the distributions ρ and ν, and the series α k are defined as in Farahmand (2011).
...
with ε avg,k = 1 /T T t=1 ε t,k and E avg (ε avg,0 , . . . , ε avg,K−1 ; r) = K−1 k=0 α 2r k ε avg,k .

Proof of Theorem 3.

Let w * 1 , . . .

, w * T , h * and f * 1 , . . . , f * T be the minimizers of ε * avg , then:

We proceed to bound the three components individually:

• C can be bounded using Hoeffding's inequality, with probability 1 − δ /2 by ln( 2 /δ) /(2nT ), as it contains only nT random variables bounded in the interval [0, 1];
...
E sup g∈F γ, g(y) − g(y ) = E sup f ∈F T ti γ ti (f t (y ti ) − f t (y ti ))
