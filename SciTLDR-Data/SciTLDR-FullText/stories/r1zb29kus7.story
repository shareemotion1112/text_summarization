...
In BID9 BID10 , the authors used the same λ g for all groups and adopted Group LASSO for R g .

Recently Wang et al. BID8 use the squared L 1 norm for R g and vary the regularization parameters λ g for different groups.
...
Pruning of different weight groups for 3D CNN is shown in FIG0 .In BID8 , Wang et al. theoretically proved that by increasing the regularization parameter λ g , the magnitude of weights tends to be minimized.

The more λ g increases, the more magnitude of weights are compressed to zero.

Therefore, we can assign different λ g for the weight groups based on their importance to the network.
...
The update of λ g is determined by the following formula: λ DISPLAYFORM2 Here ∆λ g is the function of average rank r, we follow the formula proposed by Wang BID8 as follows: DISPLAYFORM3 Here A is a hyperparameter which controls the speed of convergence.

According to Eqn.(2), we can see that ∆λ g is zero when r = RN g because we need to increase the regularization parameters of the weight groups whose ranks are below RN g to further decrease their L 1 norms; and for those with greater L 1 norms and rank above RN g , we need to decrease their regularization parameters to further increase their L 1 norms.

Thus, we can ensure that exactly RN g weight groups are pruned at the final stage of the algorithm.

When we obtain λ (new) g , the weights can be updated through back-propagation deduced from Eqn.
...
We set the weight decay factor λ to be the same as the baseline and set hyper-parameter A to half of λ.
