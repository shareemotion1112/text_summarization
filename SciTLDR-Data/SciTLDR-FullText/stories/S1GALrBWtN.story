...
We want to learn HGNs for fully observable nondeterministic (FOND) planning BID19 Speck, Ortlieb, and Mattmüller 2015; BID69 .
...
A solution is represented as a policy π : S → A, a mapping from the possible states in the world S to actions A, indicating for any given state s ∈ S, what action π(s) to take.

Given a policy π, an execution trace is any sequence s 0 π(s 0 ) s 1 π(s 1 ) . . .

π(s n ) s n+1 , where s i is an state that can be reached from state s i−1 after applying action π(s i−1 ).A solution policy π is weak if there exists an execution trace from s 0 to a state satisfying g. Weak solutions guarantee that a goal state can be successfully reached sometimes.
...
Under the fairness assumption, stating that "every action executed infinitely often will exhibit all its effects infinitely often" (D'Ippolito, Rodrıguez, and Sardina 2018), a solution π is either strong cyclic or strong if (1) every terminal state entails the goals and (2) for every state s that the agent might finds itself in after executing π from s 0 , there exists an execution trace from the state s to a state satisfying g. The difference is that in strong cyclic solutions the same state might be visited more than once whereas in strong solutions this never happens.
...
Specifically, the learning problem can be defined as follows: given a set of actions A and a collection of traces Π generated using actions in A, to obtain a collection of HGN methods.

A collection of methods M is correct if given any (initial state, goal) pair (s 0 , g), and any solution plan π generated by a sound HGN planner using M and A, π is a correct plan solving the planning problem (s 0 , g,A) .
...
Planning follows a recursive procedure, starting with π = , choosing the first element, g 1 , in G, and either (1) applying an HGN method m decomposing g 1 into m's subgoals g 1 , . . .

, g k , concatenating m's subgoals into G (i.e, G = g 1 , . . .

, g k , g 1 , . . .

, g n are the new open goals), or (2) applying an action a ∈ A achieving g, appending a to π (i.e., π ← π · a) and removing g from G. In either case it will check if the preconditions of m (respectively, a) are satisfied in the current state.

When a is applied, the current state is transformed in the usual way BID18 .

When G = ∅, π is returned.
...
Specifically, we propose viewing the collection of input traces Π as text: each plan trace π = s 0 a 0 s 1 a 1 . . .
