...
We define the difference of densities ρ(a) = p y (a) − p x (a) which should be pushed toward zero for all a ∈ R m during learning.
...
In conventional GANs, D(a) is usually optimized to approximate the probability of seeing a target sample, or ρ(a) or some similar function.
...
The objective of the generator is a scalar function D(G(z)), therefore the gradient of the objective function is just a scaled version of the gradient ∇ a D(a) which would then propagate further to the parameters of G. This gradient ∇ a D(a) tells the generator in which direction ρ(a) becomes larger, i.e., in which direction the ratio of target examples increases.

The generator changes slightly so that z is now mapped to a new a = G (z), moving the sample generated by z a little bit towards the direction where ρ(a) was larger, i.e., where target examples were more likely.

However, ρ(a) and its derivative only take into account the local neighborhood of a, since regions of the sample space that are distant from a do not have much influence on ρ(a).
...
The electric potential (Coulomb potential) created by the point charge Q is Φ C = 1 4πε0 Q r , where r is the distance to the location of Q and ε 0 is the dielectric constant.

Motivated by this electric potential, we introduce a similar concept for GAN learning: Instead of the difference of densities ρ(a), we rather consider a potential function Φ(a) defined as DISPLAYFORM0 with some kernel k (a, b) which defines the influence of a point at b onto a point at a. The crucial advantage of potentials Φ(a) is that each point can influence each other point in space if k is chosen properly.

If we minimize this potential Φ(a)

we are at the same time minimizing the difference of densities ρ(a): For all kernels k it holds that if ρ(b) = 0 for all b then Φ(a) = 0 for all a. We must still show that (i) Φ(a) = 0 for all a then ρ(b) = 0 for all b, and even more importantly, (ii) whether a gradient optimization of Φ(a) leads to Φ(a) = 0 for all a.

This is not the case for every kernel.

Indeed only for particular kernels k gradient optimization of Φ(a) leads to BID22 ) (see also Theorem 1 below).
...
If the potential field is constant, then the difference of densities ρ is constant, too.
...
Only the generated samples are allowed to move freely, in order to minimize ρ.
...
Analogously to electrostatics, the potential Φ(a) from Eq. (1) gives rise to a field E(a) = −∇ a Φ(a).
...
Ultimately, the Coulomb GAN aims to make the potential Φ zero everywhere via the field E(a), which is the negative gradient of Φ. For proper kernels k, it can be shown that (i) Φ can be pushed to zero via its negative gradient given by the field and (ii) that Φ(a) = 0 for all a implies ρ(a) = 0 for all a, therefore, p x (a) = p y (a) for all a BID22 ) (see also Theorem 1 below).

During learning we do not change Φ or ρ directly.
...
This conservation is expressed by the continuity equation BID39 ) that describes how the difference between distributions ρ(a) changes as the particles are moving along the field, i.e., how moving samples during the learning process changes our densities:ρ DISPLAYFORM0 for sample density difference ρ and unit charges that move with "velocity" v(a) = sign(ρ(a))E(a).

The continuity equation is crucial as it establishes the connection between moving samples and changing the generator density and thereby ρ.
...
If generator samples are moved away from a location a then ρ(a) is increasing while ρ(a) is decreasing when generator samples are moved toward a. The continuity equation is also obtained as a first order ODE to move particles in a potential field BID11 , therefore describes the dynamics how the densities are changing.

We obtaiṅ DISPLAYFORM1 Published as a conference paper at ICLR 2018The density difference ρ(a) indicates how many samples are locally available for being moved.

At each local minimum and local maximum a of ρ we obtain ∇ a ρ(a) = 0.

Using the product rule for the divergence operator, at points a that are minima or maxima, Eq. (3) reduces tȯ DISPLAYFORM2 In order to ensure that ρ converges to zero, it is necessary and sufficient that sign(∇ · E(a)) = sign(ρ(a)), where ÷aρ(a) = 0, as this condition ensures the uniform decrease of the maximal absolute density differences |ρ(a max )|.

As discussed before, the choice of kernel is crucial for Coulomb GANs.

The m-dimensional Coulomb kernel and the m-dimensional Plummer kernel lead to (i) Φ that is pushed to zero via the field it creates and (ii) that Φ(a) = 0 for all a implies ρ(a) = 0 for all a, therefore, p x (a) = p y (a) for all a BID22 .
...
Therefore the Coulomb potential Φ(a) for the Coulomb GAN was constructed by a low-dimensional Plummer kernel k with parameters d m − 2 and : DISPLAYFORM0 The original Plummer kernel is obtained with d = m − 2.

The resulting field and potential energy is DISPLAYFORM1 DISPLAYFORM2 The next theorem states that for freely moving generated samples, ρ converges to zero, that is, p x (.) = p y (.), when using this potential function Φ(a).
...
This gives rise to a mini-batch specific potential, where in Eq. FORMULA8 we use ρ(a) = p y (a) − p x (a) and replace the expectations by empirical means using the drawn samples: DISPLAYFORM0 It is tempting to have a generator network that directly minimizes this potentialΦ between generated and training set points.

In fact, we show thatΦ is an unbiased estimate for Φ in Appendix Section A.4.
...
, N y , they create a batch-specific potentialΦ. The goal of the discriminator is to learn E X ,Y (Φ(a)) = Φ(a), i.e., the potential averaged over many mini-batches.
...
In other words, any equilibrium point of the Coulomb GAN that is found to be local optimal has to be the one global Nash equilibrium as the minimization of the energy F (ρ) in Eq. (33) leads to a single, global optimum at p y = p x .
...
The main problem with learning Coulomb GANs is to approximate the potential function Φ, which is a complex function in a high-dimensional space, since the potential can be very non-linear and non-smooth.
...
In a first step, we prove that for local maxima or local minima a of ρ, the expression sign(∇ · E(a)) = sign(ρ(a)) holds for small enough.

For proving this equation, we apply the Laplace operator for spherical coordinates to the low-dimensional Plummer kernel.

Using the result, we see that the integral ∇ · E(a) = − ρ(b)∇ 2 a k (a, b) db is dominated by large negative values of ∇ 2 a k around a. These negative values can even be decreased by decreasing .

Therefore we can ensure by a small enough that at each local minimum and local maximum a of ρ sign(ρ(a)) = −sign(ρ(a)).

Thus, the maximal and minimal points of ρ move toward zero.

In a second step, we show that new maxima or minima cannot appear and that the movement of Φ toward zero stops at zero and not earlier.

Since ρ is continuously differentiable, all points in environments of maxima and minima move toward zero.

Therefore the largest |ρ(a)| moves toward zero.

We have to ensure that moving toward zero does not converge to a point apart from zero.

We derive that the movement toward zero is lower bounded byρ(a) = −sign(ρ(a))λρ 2 (a).

Thus, the movement slows down at ρ(a) = 0.

Solving the differential equation and applying it to the maximum of the absolute value of ρ gives |ρ| max (t) = 1/(λt + (|ρ| max (0)) −1 ).

Thus, ρ converges to zero over time.

DISPLAYFORM0 , where the theorem has already been proved for small enough BID22 .At each local minimum and local maximum a of ρ we have ∇ a ρ(a) = 0.

Using the product rule for the divergence operator, Eq. (3) reduces tȯ DISPLAYFORM1 The term ∇ · E(a) can be expressed as DISPLAYFORM2 We next consider ∇ 2 a k (a, b) for the low-dimensional Plummer kernel.
...
For d = m − 3 we have to restrict in the following the sphere S τ (a) to τ < √ m and ensure increase of ∇ 2 k(a, b) with .If ρ(b) = 0, then we define a sphere S τ (a) with radius τ around a for which holds sign(ρ(b)) = sign(ρ(a)) for each b ∈ S τ (a).

Note that ∇ 2 k(a, b) is continuous differentiable.

We have DISPLAYFORM8 Using τ , we now bound T \Sτ (a) ρ(b) ∇ 2 a k (a, b) db independently from , since ρ is a difference of distributions.

For small enough we can ensure DISPLAYFORM9 Therefore we have sign(∇ · E(a)) = sign(ρ(a)) .Therefore we have at each local minimum and local maximum a of ρ sign(ρ(a)) = − sign(ρ(a)) .(23) Therefore the maximal and minimal points of ρ move toward zero.

Since ρ is continuously differentiable as is the field, also the points in an environment of the maximal and minimal points move toward zero.
...
Since the contribution of a environment S τ (a) dominates the integral Eq. (19), for small enough there exists a positive 0 < λ globally for all minima and maxima as well as for all time steps for which holds: DISPLAYFORM10 The factor λ depends on k and on the initial ρ.

λ is proportional to d. Larger d lead to larger |∇ · E(a)| since the maximum or minimum ρ(a) is upweighted.

There might exist initial conditions ρ for which λ → 0, e.g. for infinite many maxima and minima, but they are impossible in our applications.

Therefore maximal or minimal points approach zero faster or equal than given bẏ Consequently, ρ converges to the zero function over time, that is, p x (.) becomes equal to p y (.).
...
For Coulomb GANs that means, D * has learned the potential Φ induced by G * perfectly, because L D is convex in D, thus if D * is optimal within an neighborhood U (D * ), it must be the global optimum.

This means that G * is directly minimizing (G(z)) ).

The Coulomb potential energy is according to Eq. (7) DISPLAYFORM1 DISPLAYFORM2 Only the samples from p x stem from the generator, where p x (a) = δ(a − G(z))p z (z)dz.

Here δ is the δ-distribution centered at zero.
...
F has one minimum, F = 0, which implies Φ(a) = 0 and ρ(a) = 0 for all a, therefore also p y = p x according to Theorem 1.

Each Φ(a) = 0 would mean there exist potential differences which in turn would cause forces on generator samples that allow to further minimize the energy.

Since we assumed that the generator can reach the minimum p y = p x for any p y , it will be reached by local (stepwise) optimization of − 1 2 E pz (Φ(G(z))) with respect to G. Since the pair (D * , G * ) is optimal within their neighborhood, the generator has reached this minimum as there is not other local minimum than the global one.
...
N x }.For such finite samples, i.e. point charges, we have to use delta distributions to obtain unbiased estimates of the the model distribution p x (.) and the target distribution p y (.): DISPLAYFORM0 where δ is the Dirac δ-distribution centered at zero.
...
Note that when calculating the derivative ofΦ(a i ; X , Y), it is important to only derive with respect to a, and not wrt.
...
Algorithm 1 Minibatch stochastic gradient descent training of Coulomb GANs for updating the the discriminator weights w and the generator weights θ.while Stopping criterion not met do • Sample minibatch of N x training samples {x 1 , . . .
