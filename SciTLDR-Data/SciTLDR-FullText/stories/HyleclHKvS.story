...
is the minibatch gradient, t is the training iteration, and α (t) is the learning rate.
...
In the outer loop, for every T steps, we evaluate a large batch gradientḡ = 1 N N i ∇ θ (mT ) L i , where N b, and m is the outer loop index, and we store the parameters θ (mT ) .

In the inner loop, the update rule of the parameters is given by,

whereĝ

Note that in our analysis, the reference point is chosen to be the last iterate of previous outer loop θ (mT ) , recommended as a practical implementation of the algorithm by the original SVRG paper Johnson & Zhang (2013) .
...
We also assume the data covariance matrix Σ is diagonal.
...
We would like to train a student model θ that minimizes the squared loss over the data distribution:
...
When Σ is diagonal, the loss can further be reduced to
...
When Σ is diagonal, we further analyze and decompose C(M(θ)) as a function of m(θ) so as to derive the following dynamics and decay rate for SGD.

Theorem 2 (SGD Dynamics and Decay Rate).

Given the noisy linear regression objective function (Eq. 3), under the assumption that x ∼ N (0, Σ) with Σ diagonal and θ * = 0, we can express C(θ) as a function of m(θ):
...
To derive the dynamics of the second moment of the parameters following the SVRG update, we look at the dynamics of one round of inner loop updates, i.e., from θ (mT ) to θ ((m+1)T ) : Lemma 3.
...
Observe that the variance reduction term 5 decays exponentially throughout the inner loop, with decay rate I − αΣ, i.e. P .
...
Specifically, if we want to reduce the term 1 as fast as possible, we would prefer a small decay rate and a large learning rate, i.e. α → 1 λmax(Σ) .
...
Given the noisy linear regression objective function (Eq. 3), under the assumption that x ∼ N (0, Σ) with Σ diagonal and θ * = 0, the dynamics for SVRG in m(θ) is given by:

In the absence of the label noise (i.e., σ y = 0), we observe that both SGD and SVRG enjoy linear convergence as a corollary of Theorem 2 and Theorem 4: Corollary 5.
...
Specifically they assume that ∇L i (θ * ) = 0 for all i, and our setting without label noise clearly also belongs to this regime.
...
We picked a data distribution, with data dimension d = 100, and the spectrum of Σ is given by an exponential decay schedule from 1 to 0.01.
...
Given the noisy linear regression objective function (Eq. 3), under the assumption that x ∼ N (0, Σ) with Σ diagonal and θ * = 0, we have

Proof.

In the following proof, we define the entry-wise p power on vector x as x •p .

Under our assumption µ = 0, θ
...
Notice, this conclusion can be generalized to any square matrix A not only for E[θ (t) θ (t) ], i.e. for any square matrix A ∈ R d×d , with x ∼ N (0, Σ) and Σ diagonal, since
...
Given the noisy linear regression objective function (Eq. 3), under the assumption that x ∼ N (0, Σ) with Σ diagonal and θ * = 0, we can express C(M(θ)) as a function of m(θ):

Then we derive following dynamics of expected second moment of θ:

Proof.

Since, E[ b ] = 0, and b is independent with θ (t) , X b , we have:

and,

Since X b is independent with θ (t) , we have:
...
Likewise, since E[ N ] = 0 and N is independent with X b , X N and θ (t) , we have the expectation of equation 46, equation 47 equal to 0.
...
and N is independent with X N and θ (mT ) , the expectation of Eq. 53 is equal to 0.

Therefore,

which suggests the covariance betweenĝ (mT +t) andg (mT +t) is exponentially decayed.
...
We first take expectation over θ (mT +t) θ (mT ) with Eq. 54 due to the independence among X b , X N and θ.
...
= R Rm(θ (mT +T −2) ) − QP T −2 m(θ (mT ) ) + F m(θ (mT ) ) + N −1 n (80)

In other word, Eq. 79 describe the dynamic of expected second moment of iterate between two nearby snapshots, m(θ ((m+1)T ) ) = λ(α, b, T, N, Σ)m(θ (mT ) ) + (I − R T )(I − R)

In our theoretical analysis (Section 3), we evaluate a large batch gradientḡ to control variance.

That is because any number of data points can be directly sampled form the true distribution.

But in order to compare the computational cost between SVRG and SGD, we set the number of data points used to calculateḡ as N , which is slightly different with the original SVRG's setup of full-batch gradient.
