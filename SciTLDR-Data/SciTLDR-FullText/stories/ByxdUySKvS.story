...
The data of each batch is augmented by multiple pre-processing components with sampled policies {τ 1 , τ 2 , · · · , τ M }, respectively.
...
We use the augmentation policy network A(·, θ) as an adversary, which attempts to increase the training loss of the target network F(·, w) through adversarial learning.
...
The problem is usually solved by vanilla SGD with a learning rate η and batch size N , and the training procedure for each batch can be expressed as
...
where τ (·) represents the augmentation policy generated by the network A(·, θ).

Accordingly, the training rule can be rewritten as

where we introduce M different instances of each input example augmented by adversarial policies {τ 1 , τ 2 , · · · , τ M }.

For convenience, we denote the training loss of a mini-batch corresponding to the augmentation policy τ m as
...
where p m represents the probability of the policy τ m .

To reduce the variance of gradient ∇ θ J(θ), we replace the training loss of a mini-batch L m with L m a moving average over a certain minibatches 2 , and then normalize it among M instances as L m .
...
Initialization: target network F(·, w), augmentation policy network A(·, θ) Input: input examples x, corresponding labels y 1: for 1 ≤ e ≤ epochs do 2:
...
Update θ e+1 via Equation 9; 11: Output w * , θ *
