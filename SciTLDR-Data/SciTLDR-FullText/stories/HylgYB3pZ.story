...
P γ is an m dimensional probability distribution whose expected value is γ1 m , where γ ∈ R and 1 m is an m dimensional vector whose elements are all one.

Proposition 1.

Let a be a random vector in R m that follows P γ .

Given w ∈ R m such that w > 0, the expected value of w · a is |γ| √ m w cos θ w , where θ w is the angle between w and 1 m .

where E(x) denotes the expected value of random variable x.

Definition 2.

From Proposition 1, the expected value of w · a depends on θ w as long as γ = 0.

The distribution of w · a is then biased depending on θ w ; this is called angle bias.

In FIG1 , if we denote the i-th row vector of W and the j-th column vector of A by w i and a j , respectively, a j follows P γ with γ = 0.5.
...
Note that if the random matrix A has both positive and negative elements, W A also shows a stripe pattern as long as each column vector of A follows P γ with γ = 0.We can generalize Proposition 1 for any m dimensional distributionP, instead of P γ , as follows: DISPLAYFORM0 Letâ be a random vector that follows an m dimensional probability distributionP whose expected value isμ ∈ R m .

Given w ∈ R m such that w > 0, it follows that DISPLAYFORM1 whereθ w is the angle between w andμ.

Proof.

The proof is the same as that of Proposition 1.Proposition 2 states that the distribution of w ·â is biased according toθ w unless μ = 0.

(1) and BID21 .

Repeating the operations through multiple layers, the variance of θ l i and a l i will shrink to small values.
...
In contrast, no stripe pattern appears in Layer 1, because each element of the input vector is scaled into the range [−1, 1] and its mean value is near zero; this corresponds to the case in which μ ≈ 0 in Proposition 2.

(3) , for each sample 1 .

FIG6 shows boxplot summaries of θ l i on the first ten neurons in layers 1, 3, 5, 7, and 9, in which the 1%, 25%, 50%, 75%, and 99% quantiles are displayed as whiskers or boxes.

We see the mean of θ l i are biased according to the neurons in the layers other than Layer 1.

We also see that the variance of θ l i shrink through layers.
...
Figure 5 shows boxplot summaries of θ l i on the first ten neurons in layers 1, 10, 20, 30, and 40 .

We see that the mean of θ l i are biased according the neurons in the layers other than Layer 1.

We also see that the variance of θ l i shrink through layers, but the shrinking rate is much moderate compared to that in FIG6 .
...
The first one is to somehow make the expected value of the activation of each neuron near zero, because angle bias does not occur if μ = 0 from Proposition 2.
...
Let a be an m dimensional random variable that follows P γ .

Given w ∈ W LC such that w > 0, the expected value of w · a is zero. . .

, m) will likely be more similar to each other.

The activation vector in layer l, each of whose elements is given by Equation (1) , is then expected to follow P γ .

Therefore, if the input vector a 0 follows P γ , we can inductively reduce the angle bias in each layer of an MLP by using weight vectors that are included in W LC .
...
FIG9 shows boxplot summaries of θ l i on the first ten neurons in layers 1, 3, 5, 7, and 9 of the MLP with LCW.
...
FIG11 shows boxplot summaries of θ l i of the MLP after 10 epochs training.

We see that the mean of θ l i is slightly biased according to the neurons.

However, the variance of θ l i do not shrink even in deeper layers.
...
FIG1 shows boxplot summaries of θ l i on the first ten neurons in layers BID19 10, 20, 30 , and 40 of the MLP with LCW.
...
MLP LCW was initialized using the minibatch-based method described in Section 3.3 with σ z = 0.5.
