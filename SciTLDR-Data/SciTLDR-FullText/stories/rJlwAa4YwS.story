...
Under review as a conference paper at ICLR 2020 where λ > 0 is a parameter that controls the importance of the representation cost for the overall optimization process.
...
When clear from the context, we will use Λ to refer to a lattice.

We define P 0 (Λ) to be the set of all points in R m whose closest lattice point is the origin.

Given x ∈ R m , we define K Λ (x) to be the operator (the "quantizer") that takes x and maps it to the closest element of the lattice Λ:
...
For a given m dimensional lattice Λ, let U be uniformly distributed over P 0 (Λ).
...
We will use the notation P A (a) as a summary form of P ([A = a]); we sometimes will use a random vector as an argument, thus, for example, −E A log PÂ(A) can be interpreted as the average cost, in nats, of using a code designed forÂ on the random vector A, which is drawn using a (generally) different probability law.
...
Notice also that the expression in the expectation does not use directly any global statistics about X; instead it relies on fX −U , which is a density that the designer can specify when the statistics ofX are specified and when the lattice Λ is designed.
...
Notice the conditioning in PẐ |U on the dither value U .
...
Notice also that − log PẐ |U may not be the very best code one could be using which is − log P Z|U , in fact
...
In the left hand side, we have a network that employs pre/post dithered quantization of a representation quantized using a lattice Λ; in the right, there is subtractive dithering but no quantization.
...
Now in reference to the optimization objective in Equation (3), note that one of the things we need to do is to define a code that will be used to encode Z. For this purpose, we will use a code constructed using a distribution PẐ |U .
...
Next, as in Equation (6) we definê

The distribution PẐ |U is the one associated with this definition.

Encoding Z, the lattice representation of D, using the code implied by PẐ |U incurs on the cost
...
Thus to complete the cost needed to represent the data D, we need the cost of representing Z, which is in our work is given by − log PẐ |U (Z|U ).
...
Those versed in the literature of variational autoencoders, might recognize the famous Evidence Lower BOund (ELBO) (for λ = 1) in the expression above.
...
Now denote Z * to be an m dimensional Gaussian vector with independent entries each with a variance equal to σ 2 Λ , and let U m denote a random vector uniformly distributed over P 0 (Λ).
...
In light of (13), let us analyze the performance of a good high dimensional lattice Λ by assuming U to be also i.i.d.

Gaussian with variance σ 2 Λ .
...
For a given dimension m ≥ 1, let η : R m → R be a twice differentiable function.

Assume that U is a column vector uniformly distributed over the cell P 0 (Λ) of a lattice Λ, and assume that

where H η denotes the Hessian (the matrix of second order derivatives) of η.

Proof.

It is easy to see that if u ∈ P 0 (Λ), then −u ∈ P 0 (Λ).

Due to this property, the first moment of a lattice is zero:

Using a Taylor series expansion, assuming σ Λ is small,

Lattices that achieve G m have white dithers.

This is a result by Poltyrev and can be found in (Zamir & Feder, 1996) : Lemma 4.

For any m ≥ 1, if a lattice Λ that achieves G m and U is uniformly distributed over

If one keeps constant the volume V 2/m Λ of the lattice cell then by definition,
...
In the above, η was a generic function but in our case, we would like to identify it with .
...
The parameter λ which weights the representation cost in the total objective function is slowly annealed from a value close to 0 to its (asymptotic) value of 1.0; the value of 0.5 is achieved at 100k steps.
