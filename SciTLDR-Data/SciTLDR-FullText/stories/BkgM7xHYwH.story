...
where W xh , W mh , W hm , W mm are the model's parameters and σ is a non-linear activation function (tanh in our experiments), and m t is the final output of the layer.
...
Fortunately, the gradient propagation can be controlled in the linear recurrence by tuning the spectral radius ρ of the recurrent matrix W mm , for example, by using an orthogonal parameterization.

In our experiments we use soft orthogonality constraints by adding a regularization term λ W W − I 2 as in Vorontsov et al. (2017) .

Alternatively, it is possible to build a fading memory system by imposing ρ < 1 (Tino et al., 2004) .
...
We overcome this problem by adding to the loss function a regularization term β
...
which shows how to reconstruct any input vector x τ , τ = 1, . . .
...
A soft orthogonality constraint λ W W − I 2 is added to the cost function as in Vorontsov et al. (2017) , with λ chosen {0, 10 −5 , 10 −4 , 10 −3 , 10 −2 }.
...
We selected λ by cross-validation in {0, 1, 10, 100}. It is important to note that, while on this benchmark the best result is achieved by the LMN, the best model does not use the LAES initialization.

The best configuration for the LMN use an orthogonal initialization but does not use soft orthogonality constraints since the best configuration is obtained when the hyperparameter for the soft orthogonality constraints is λ = 0.
...
Other approaches constrain the parameters with soft or hard orthogonality constraints (Vorontsov et al., 2017; Zhang et al., 2018; Wisdom et al., 2016; Lezcano-Casado & Martínez-Rubio, 2019) .
...
Tiňo & Rodan (2013) and White et al. (2004) study the short-term memory properties of linear and orthogonal memories.
