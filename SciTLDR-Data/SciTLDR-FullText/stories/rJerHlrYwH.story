...
In the CPC architecture, each input image is first divided into a set of overlapping patches x i,j , each of which is encoded with a neural network f θ into a single vector z i,j = f θ (x i,j ).

To make predictions, a masked convolutional network g φ is then applied to the grid of feature vectors.
...
The predictions are made linearly: given a context vector c i,j , a prediction length k > 0, and a prediction matrix W k , the predicted feature vector isẑ i+k,j = W k c i,j .
...
This loss is called InfoNCE (van den Oord et al., 2018) as it is inspired by Noise-Contrastive Estimation (Gutmann & Hyvärinen, 2010; Mnih & Kavukcuoglu, 2013) and has been shown to maximize the mutual information between c i,j and z i+k,j (van den Oord et al., 2018) .

Having trained an encoder network f θ , a context network g φ , and a set of linear predictors {W k } using the CPC objective, we use the latents z = f θ (x) as a representation of new observations x for downstream tasks, and discard the rest.

We then train a model h ψ to classify these representations given a dataset of labeled images.
...
We consider three labeled datasets D l for evaluation, each with an associated classifier h ψ and supervised losse L Sup (see Fig. 2 , right).

This protocol is sufficiently generic to allow us to later compare the CPC representation to other methods which have their own means of learning a feature extractor f θ .

Linear classification is the standard benchmark for evaluating the quality of unsupervised image representations.

In this regime, the classification network h ψ is restricted to mean pooling followed by a single linear layer, and the parameters of f θ are kept fixed.
...
For this task, the classifier h ψ is an arbitrary deep neural network (we use an 11-block ResNet architecture with 4096-dimensional feature maps and 1024-dimensional bottleneck layers).
...
As such D l is the entire PASCAL-2007 dataset (comprised of 5011 labeled images); h ψ and L Sup are the Faster-RCNN architecture and loss (Ren et al., 2015) .

In addition to color-dropping, we use scale-augmentation (Doersch et al., 2015) for training.

For linear classification, we keep the feature extractor f θ fixed to assess the representation in absolute terms.
...
In this regime, we initialize the feature extractor and classifier with the solutions θ * , ψ * found in the previous learning phase, and train them both for the supervised objective.
...
The former formulates an objective to learn a feature extractor f θ in an unsupervised manner, whereas the latter directly constrains the classifier h ψ using the unlabeled data.
...
CPC is a contrastive representation learning method that maximizes the mutual information between spatially removed latent representations with InfoNCE (van den Oord et al., 2018), a loss function based on Noise-Contrastive Estimation (Gutmann & Hyvärinen, 2010; Mnih & Kavukcuoglu, 2013) .
...
We follow the same paradigm as for the supervised baseline (training and evaluating a separate classifier for each size subset), stacking a neural network classifier on top of the CPC latents z = f θ (x) rather than the raw image pixels x (see section 2.2, efficient classification, and Appendix).
...
In this setting, we replaced the neural network classifier h ψ used previously with a Faster-RCNN (Ren et al., 2015) image detection architecture, and use the pre-trained feature extractor on ImageNet.
...
The proposed method, which learns only from ImageNet data using a single unsupervised objective, reaches 70.6% when equipped with a ResNet-101 feature extractor f θ (as for most competing methods (Doersch & Zisserman, 2017) but not all (Caron et al., 2018; .

Equipped with the more powerful ResNet-161 feature extractor f θ , our method reaches 72.7%.
...
We suppose we have just calculated a set of latents z i,j = f θ (x i,j ) for i, j ∈ {1, . . .
