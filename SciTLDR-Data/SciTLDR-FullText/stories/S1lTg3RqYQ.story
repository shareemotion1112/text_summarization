...
2) Meanwhile, x A is also fed into a sub-network to compute feature masks m A .3) The target domain exemplar image x B is fed to a sub-network to compute affine parameters γ B and β B for AdaIN .

4) The content representation c A is transferred to the target domain using m A , γ B , β B , and is further decoded to an image x AB by target domain generator G B .unsupervised setting, BID34 proposed to use a cycle-consistency loss, which assumes that a mapping from domain A to B, followed by its reverse operation approximately yields an identity function, that is, F (G(x A )) ≈ x A .
...
As in BID9 , means and variances are calculated for each channel of f B and used as AdaIN's affine parameters, DISPLAYFORM0 DISPLAYFORM1 DISPLAYFORM2 where µ(·) and δ(·) respectively denote a function to compute the mean and variance across spatial dimensions.
...
The feature masks m A , which can be regarded as attention modules, are computed by applying a nonlinear activation function and a threshold to feature maps f A , i.e. DISPLAYFORM3 where η is a threshold and σ is the sigmoid function.
...
The threshold parameter η is used to adjust how much the feature mask affects the information flow.

Setting η = 0, i.e. using the feature maps (paper Fig. 2 top branch) as feature mask directly, leads to useful information being dropped out and artifacts in the results.

Setting η = 1, i.e. not using feature mask at all, leads to results without semantic consistency (see paper FIG4 ).

After experimenting with different values, we fixed it to 0.5.

We use the Adam BID14 optimizer with β1 = 0.5 and β2 = 0.999.
...
The loss weights in LUNIT are following BID22 , and λc, λs are chosen according to the dataset variations and tasks.

For data augmentation, we do left-right flip and random crop.

In addition, we set a low λc for face gender translation, since we need to change the shape and add/remove hair in this translation task.
