...
x is an original sample, y is corresponding label for x, is the perturbation norm, sign indicates the element-wise sign function and L(θ, x, y) is the loss of the classifier parameterized by θ.

Fast Gradient Sign Method (FGSM) [3] generates the adversarial result x F GSM with the following formula.

x F GSM = x + sign [∇ x L(θ, x, y)].
...
, y) where x + S is the set of allowed perturbation region for sample x that is limited by , x (0) is the random starting points in x + S, x+S [] means the projection result on x + S and α is the step size [10] .
...
It tries to reduce the weighted summation of standard loss (empirical risk) E [L(θ, x, y)] and adversarial loss E [L(θ, x , y)], i.e. αE [L(θ, x, y)] + (1 − α)E [L(θ, x , y)] where α is a hyperparameter for adversarial training, and x is an adversarially perturbed sample from x with x − x ≤ .

(Usually, α = 0.5 is used for adversarial training.)
...
Assume f is parameterized by θ and L(θ, x, y) is the loss of the classifier provided the input x and the label y ⊆ Y.
...
L(θ, x , c x ) and their opponents x * * = argmin
...
To better understand GEM [4] , let us assume that there are losses l 1 (θ), · · · , l T (θ) with lexicographical preference, in other words, we want to reduce l t (θ) without increasing l 1 (θ), · · · , l t−1 (θ) for t ∈ {1, · · · , T }.

We also have (pre-projection) parameter updates g 1 , · · · , g T where g t = − ∇l t (θ) and is a learning rate.
...
When we have suggested parameter updatesg 1 , · · · ,g T , let's consider their weighted meang Onestep = T t=1 α tgt where α 1 , · · · , α T ≥ 0 and
...
MixTrain [12] uses adversarial training by dynamically adjusting the hyperparameter α for adversarial training.
...
We used α = 0.5 for standard adversarial training [3] .

In order to apply comparable effects on the training of OLSRC, we used α 1 , α 2 = 0.5 for weights of Onestep method and 10 −10 was used for numerical stability in GEM algorithm [4] .

We used 1 λ = 1.0 for TRADES [31] training.
...
In order to apply comparable effects on the training of OLSRC and OLGRC, we used α 1 , α 2 = 0.5 for weights of Onestep method.
