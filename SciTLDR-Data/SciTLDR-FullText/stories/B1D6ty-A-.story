...
, L} has n l nodes and is characterized by a linear operator W l ∈ R n l−1 ×n l and a non-linear activation function φ l : R n l → R n l .
...
Each layer uses activations being fed into it to compute its own activations as a l = φ l W l , a l−1 ∈ R n l , where φ ., . denotes φ( ., . ) for simplicity of notation.
...
DISPLAYFORM4 An important observation here is that if we fix W 1 , then Eqn (5) turns into a set of Generalized Linear Model problems with φ 2 as the activation function, i.e. DISPLAYFORM5 where z = φ 1 W 1 , x .
...
Consider the autoencoder problem below for a single hidden layer network: DISPLAYFORM0 Upon fixing the parameters of the lower layer i.e. W 1 , it is easy to see that we are left with a set of GLM problems: min DISPLAYFORM1 where z = φ 1 W 1 , x .
...
Similarly, fixing the parameters of the upper layer, i.e. W 2 , we are left with another set of problems: DISPLAYFORM2 where φ W2 · = φ 2 W 2 , φ 1 · .
...
DISPLAYFORM4 at least one of the following applies: DISPLAYFORM5 where B (z, /κ) refers to a ball centered at z with radius /κ.
...
Also let v be a point /κ-close to minima w * with κ = 2b 3 W a .

Let g be the subgradient of the generalized ReLU activation and G be the subgradient ofê rr m (w). (Note that as before, g ., . denotes g( ., . )).

Then: DISPLAYFORM15 In the above proof, we first use the fact (in Step 1) that in the GLM, there is some w * such that φ w * , x i = y i .
...
Let , δ, G, M, κ > 0, let f : R d → R and w * = arg min w f (w).

Assume that for b ≥ b 0 ( , δ, T ), with probability ≥ 1 − δ, f t defined in Algorithm 1 is ( , κ, w * )-SLQC ∀w, and |f t | ≤ M ∀t ∈ {1, · · · , T } .

If we run SNGD with T ≥ DISPLAYFORM17 and η = κ , and b ≥ max DISPLAYFORM18 The results so far show that SNGD provides provable convergence for idealized and noisy GLM problems with both sigmoid and ReLU family of activation functions.
...
Importantly, note that the convergence rate of SNGD depends on the κ parameter.

Whereas the GLM error function with sigmoid activation has κ = e W Hazan et al. FORMULA0 , we obtain κ = 2b 3 W a (i.e. linear in W ) for the generalized ReLU setting, which is an exponential improvement.

This is significant as in Theorem 3.6, the number of iterations T depends on κ 2 .
...
Algorithm 3: DANTE for a multi-layer autoencoder Input :Encoder e with weights U, Decoder d with weights V, Number of hidden layers 2n − 1, Learning rate η, Stopping threshold , Number of iterations of alternating minimization Output :U, V on single-layer autoencoders.
...
Here, ∀i, y i ∈ [0, 1], the following holds: DISPLAYFORM1 where DISPLAYFORM2 are zero mean, independent and bounded random variables, i.e. ∀i ∈ [m], ||ξ i || ≤ 1.

Then,ê rr m (w) may be written as follows (expanding y i as in Eqn 6): DISPLAYFORM3 Therefore, we also have (by definition of noisy GLM in Defn 3.2): DISPLAYFORM4 Consider ||w|| ≤ W such thatê rr m (w) −ê rr m (w * ) ≥ .

Also, let v be a point /κ-close to minima w * with κ = 2b 3 W a .

Let g be the subgradient of the generalized ReLU activation and G be the subgradient ofê rr m (w), as before.

Then: DISPLAYFORM5 Here, λ i (w) = 2g w, DISPLAYFORM6 The above proof uses arguments similar to the proof for the idealized GLM (please see the lines after the proof of Theorem 3.4, viz.
...
Given an (unknown) distribution D, let the layer be characterized by a linear operator W ∈ R d×d and a non-linear activation function defined by φ : R → R. Let the layer output be defined by φ W, x , where x ∈ R d is the input, and φ is used element-wise in this function.
...
Let x, z ∈ R d×d , κ, > 0 and let f : R d×d → R be a differentiable function.

Then f is said to be ( , κ, z)-Strictly-Locally-Quasi-Convex (SLQC) in x, if at least one of the following applies:1.

f (x) − f (z) ≤ 2.

∇f (x) > 0, and ∀y ∈ B (z, /κ), T r(∇f (x)T (y − x)) ≤ 0 where B (z, /κ) refers to a ball centered at z with radius /κ.

We now prove that theê rr(W) of a multi-output single-layer neural network is indeed SLQC in W. This corresponds to proving that the one-hidden layer autoencoder problem is SLQC in W 2 .
...
Let an idealized single-layer multi-output neural network be characterized by a linear operator W ∈ R d×d = [w 1 w 2 · · · w d ] and a generalized ReLU activation function φ : R → R. Let the output of the layer be φ W, x where x ∈ R d is the input, and φ is applied element-wise.

Assuming ||W * || ≤ C,ê rr(W) is , DISPLAYFORM0 The remainder of the proof proceeds precisely as in Theorem 3.4.Theorem A.4.

Let an idealized two-layer neural network be characterized by a linear operator w 1 ∈ R d×d , w 2 ∈ R d and generalized ReLU activation functions φ 1 : R d → R d , φ 2 : R → R with a setting similar to Equation 5.
