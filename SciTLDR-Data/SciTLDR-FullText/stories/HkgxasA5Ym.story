...
This expresses the noise contrastive prior where inputs are distributed according to the convolved distribution, DISPLAYFORM0 The variances σ 2 x and σ 2 y are hyperparameters that tune how far from the boundary we sample, and how large we want the output uncertainty to be.

We choose µ x = 0 to apply the prior equally in all directions from the data manifold.

The output mean µ y determines the default prediction of the model outside of the training distribution, for example µ y = 0.

We set µ y = y which corresponds to data augmentation BID39 BID0 , where a model is trained to recover the true labels from perturbed inputs.
...
The hyperparameter γ sets the relative trade-off between them.

Interpretation as function prior The noise contrastive prior can be interpreted as inducing a function prior.

This is formalized through the prior predictive distribution, DISPLAYFORM2 The distribution marginalizes over network parameters θ as well as data fantasized from the data prior.

The distribution p(θ |x,ỹ) represents the distribution of model parameters after fitting the prior data.
...
This is similar to classical work on conjugate priors: a Beta(α, β) prior on the probability of a Bernoulli likelihood implies a Beta posterior, and if the posterior mode is chosen as an optimal parameter setting, then the prior translates to α − 1 successes and β − 1 failures.
...
Consider a regression task with data {x, y} that we model as p(y | x, θ) = Normal(µ(x), σ 2 (x)) with mean and variance predicted by a neural network from the inputs.
...
Therefore, the distribution of the mean induced by the weight prior, q(µ(x)) = µ(x, θ)q φ (θ) dθ, represents the model's epistemic uncertainty.

Note that this is different from the predictive distribution, which combines both uncertainty and noise.

We place an NCP on the distribution of the mean, resulting in the loss function DISPLAYFORM1 Here,x are the perturbed inputs and q φ (θ) forms an approximate posterior over weights.
...
In practice, we find that NCP is sufficient as a prior for the BNN and set β = 0.
...
Moreover, we place a softmax distribution on the information gain for all available data points and acquire labels by sampling with a temperature of τ = 0.5 to get diversity when selecting batches of labels, DISPLAYFORM1 where σ 2 (x) is the estimated aleatoric noise and q(µ(x)) is the epistemic uncertainty projected into output space.

Since our Bayesian neural networks only use a weight belief for the output layer, Var[q(µ(x))] is Gaussian and can be computed in closed form.
...
The experimental setup is the same as for the active learning experiment described in Section 5.2, but with uniform or normal input noise with different variance (σ 2 x ∈ {0.1, 0.2, · · · , 1.0}).

For uniform input noise, this means noise is drawn from the interval [−2σ x , 2σ x ].
...
The neural network weights θ are estimated using a point estimate, so we do not maintain a belief distribution over them.

The classifier prediction p(o | x, θ) captures uncertainty in this model.

We apply the NCP p(o |x, θ) = δ(o = 1|x, θ) to this variable, which assumes noised-up inputs to be OOD.

During training on the data set, {x, y} and o = 0 are observed, as training data are in-distribution by definition.

Following Equation 2, the loss function is DISPLAYFORM1 Analogously to the Bayesian neural network model in Section 3, we can either set µ y , σ 2 y manually or use the neural network prediction for potentially improved generalization.
...
Above, we reparameteterized the KL in weight space as a KL in output space; by the change of variables, this is equivalent if the mapping µ(·, θ) is continuous and 1-1 with respect to θ.
