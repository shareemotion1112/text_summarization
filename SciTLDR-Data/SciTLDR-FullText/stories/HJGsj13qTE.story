...
Hence, we normalize the computed singular values by the largest singular value of the activation matrix, yielding statistic Σ (Φ) = 1 σmax Σ(Φ).

Effective Dimension Based on the aforementioned criteria, we propose the following metric for dimensionality of a collection of feature vectors: DISPLAYFORM1 where the last equality holds since the elements of Σ 2 are non-negative.
...
In the MLP12 model with lower performance, the decay in d eff (Φ (l) ) with respect to l is more gradual, and in the VGG-16 model tested on Tiny ImageNet, this dropoff is very noisy and slow.
...
Appendix is scaled by factor α, the spectral norm of α * Φ (l) is α * σ max (Φ (l) ).

This is also true in a ReLU network with α > 0.

Such a scaling is achieved while preserving φ (l+1) : DISPLAYFORM0 While Srebro et al. BID10 directly apply the trace-norm to bound the complexity of a completed matrix, we apply spectral normalization in Equation 1 to correct for this scale sensitivity.
...
The decay of the effective dimension, then, is dominated by the strict decay in later layers of the σ i for large i.
