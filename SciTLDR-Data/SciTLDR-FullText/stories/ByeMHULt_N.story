...
To define it, first let τ α (x α , z α ) ∈ [0, 1] be the marginal probability of random subvectors x α and z α taking on the values x α and z α , respectively, which can be obtained by marginalizing out all variables that do not participate in the factor α.

We will refer to the vector consisting of the concatenation of the marginal probabilities for each instantiation of each factor as τ ∈ [0, 1] n .

As a concrete example, consider the 10 factors in FIG1 Following BID46 , the Bethe free energy is then defined as DISPLAYFORM0 where we use the notation q τ (x, z) to refer to a distribution with marginals τ , and where ne(x v ) refers to the number of factor-neighbors node v has in the graphical model.
...
Note that there are pseudo-marginals τ ∈ C that do not correspond to the marginals of any distribution; see BID39 for an example.
...
In particular, from the previous section we have that DISPLAYFORM0 where F x (τ x ) is the Bethe free energy of equation 2 with x clamped to particular values.

That is, F x does not consider marginals corresponding to settings of x that do not agree with x, and thus τ x will in general be smaller than τ .

In particular, to continue the example of FIG1 (c) from the previous section, where all variables are assumed to be binary, τ x will be in [0, 1] 32 instead of [0, 1] 40 , since for each variable in x 1 , . . .

, x 4 we ignore the two marginals corresponding to the unobserved value.

We may then define the following loss, as an approximation (but neither an upper nor lower bound) of equation 1: DISPLAYFORM1 and thus we arrive at the following saddle-point learning problem: min DISPLAYFORM2 3.1 CONSTRAINED OPTIMIZATION While LBP can be used to find pseudo-marginals representing fixed points of F and F x , it is somewhat unappealing in the context of deep generative modeling, primarily because it is an iterative message-passing algorithm often requiring multiple rounds for convergence, and where the order in which messages are passed appears to significantly impact results BID48 BID19 .Instead, we propose to predict approximate minimizers of the Bethe free energy using trainable inference networks f (G; φ) and f x (G, x; φ x ), which will compute approximate minimizers of F and F x , respectively.
...
Equality Constraints It is clear that the constraints that the pseudo-marginals sum to one and that they exhibit local consistency are linear constraints on τ , and so they can be expressed as Aτ = b, where A ∈ {0, 1} m×n consists of m linearly independent constraint rows and and n is the length of τ .

It is standard in linear-equality constrained optimization to optimize over the subspace defined by these linear constraints instead of the original optimization variable τ BID2 .

In particular, given a feasible pointτ and a basis V ∈ R n×n−m for the null space of A, any solution can be written as V u +τ , and so we may optimize over u instead of τ .When using inference networks to compute minimizers, however, it is more natural to have these networks output vectors of length n (i.e., the size of τ , which is linear in the factors of the MRF), rather than of size n − m, which depends on the number of constraints.

Accordingly, we may equivalently write minimizers as V V + ρ +τ , where ρ ∈ R n , and V + is the Moore-Penrose pseudoinverse of V .

In particular, V V + is an orthogonal projection on the range of V , a basis for the null space of A. Thus, given an inference network f that computes vectors in R n , the vector (V V + f (G; φ) +τ ) ∈ R n will satisfy the equality constraints imposed by C, assumingτ does.
...
Bethe Inference Networks In the case of sequential models like HMMs, it is fairly simple to parameterize f (G; φ) and f x (G, x; φ x ).

For f (G; φ) we form an embedding for each factor Ψ α (z s , z t ) by concatenating embedding vectors corresponding to the s'th and t'th timesteps with the K 2 log potential values log Ψ α (z s = k 1 , z t = k 2 ) (as given above), and then run a bidirectional LSTM over these factor-embeddings, ordered in ascending order of z s .

We then predict pseudo-marginals for each factor with a linear layer applied to the LSTM output.

The parameterization of f x (G, x; φ x ) is similar, except we also concatenate the log observation potentials log Ψ α (x s , z s ) and log Ψ α (x t , z t ) onto the embedding of factor Ψ α (z s , z t ) before feeding it to the LSTM.
...
The saddle-point objective F was optimized by alternating a maximization step wrt φ, a minimization step wrt φ x , and a minimization step wrt θ.

Projection matrices P for each graph structure were pre-calculated and stored, and feasible pseudo-marginalsτ can be obtained by assigning all marginals to be uniform.
