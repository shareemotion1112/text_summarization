...
where L(x, y; θ) is a loss function used to measure how well a model with parameters θ assigns label y to image x, and θ * (∆) are the network parameters found by training on the perturbed training data X + ∆. Note that equation 1 is a bi-level optimization problem (Bard (2013) ) -the minimization for ∆ involves the parameters θ * (∆), which are themselves the minimizer of the training problem
...
We run natural training, following the parameters θ toward low training error.
...
Update all models by taking a first-order step minimizing L θ M (X tr , Y tr ).
...
We answer this question by (i) selecting source images from the training set, (ii) computing a parameter update to θ with respect to the training loss, (iii) passing the target image x t through the resulting model, and (iv) evaluating the target adversarial loss L(x t , y source ) on the target image x t to measure the discrepancy between the network output and adversarially chosen label.
