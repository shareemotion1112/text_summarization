...
It is based on the notion of mechanism independence similar to Janzing et al. (2012) ; Zhang et al. (2015) , does not assume causal sufficiency similar to Silva et al. (2006) ; Shimizu et al. (2009) ; Janzing et al. ( , 2012 ; Zhang et al. (2015) ; Schölkopf et al. (2016) , can theoretically work on arbitrary graph structures that possibly include latent variables similar to Spirtes et al. (1993) , and can discriminate between Markov equivalent structures similar to Shimizu et al. (2006) ; Zhang and Hyvärinen (2008); Hoyer et al. (2009); Janzing et al. (2012); Zhang et al. (2015) .
...
The set of parent vertices of a vertex v ∈ V G is denoted by π(v), while we denote its continuous parents by x π(v) , and discrete parents by r π (v) .
...
where 1 ≤ t ≤ T , φ is an arbitrary basis function with the convention φ({}) = 1, and

's are the parameters of the conditional distributions.

Namely, θ k is the conditional distribution table of r k , w n is the weights of the basis functions, and ρ n is the precision parameter of the conditional distribution of x n .
...
where γ k|r π(r k ) , m n|r π(xn) , Λ n|r π(xn) , a n|r π(xn) , b n|r π(xn) are the prior parameters, i.e. hyperparameters, of our generative model.
...
where P = p(r 1:T 1:K , θ 1:K , ρ 1:N , w 1:N | x 1:T 1:N ) is the full posterior distribution, and ELBO is denoted by B P [Q] .

In a typical scenario of VB, Q is assumed to be a member of a restricted family of distributions.

In its most common form, also known as mean-field approximation, Q is assumed to factorize over some partition of the latent variables, in a way that is reminiscent to a rank-one approximation in the space of distributions Q(r 1:T 1:K , θ 1:K , ρ 1:N , w 1:N ) = q(r 1:T 1:K ) q(θ 1:K , ρ 1:N , w 1:N ) ELBO is then maximized with respect to Q which is restricted to the class of factorized distributions.
...
Gamma(ρ; a, b) = exp((a − 1) log ρ − bρ − log Γ(a) + a log b)
...
Here, ψ(x) is the digamma function which is defined as ψ(x) = d log Γ(x) dx .
...
where µ is the mean parameter and ρ is the precision parameter, i.e. ρ −1 is the variance.
...
where µ is the mean vector and Λ is the precision matrix, i.e. Λ −1 is the covariance matrix.
...
E N G(m,Λ,â,b) {− log N G(w, ρ; m, Λ, a, b)} = −a log b + log Γ(a) − 1 2 log det(Λ) + 1 2 tr(Λ −1 Λ) + M 2 log 2π − a + M 2 − 1 (ψ(â) − logb) +â b b +â 2b (m − m) T Λ(m − m)
...
We now pursue the same strategy for the expression in (7) q(θ 1:K , ρ 1:N , w 1:
...
Update logθ t Update expected sufficient statistics
...
Variational distribution Q treats r t 1:K and θ 1:K as independent variables.
...
r 1 ∈ R 1 , m n|r 1 's were set to 0, and Λ n|r 1 's were set to 1 10 I each; while for all values of r 1 ∈ R 1 , γ 1 (r 1 )'s were set to 10.
