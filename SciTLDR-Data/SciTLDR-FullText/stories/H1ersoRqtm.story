...
Concretely, for each node v in the graph, we compute a weight σ(w(h (T ) v )) ∈ [0, 1] using a learnable function w and the logistic sigmoid σ and compute a graph-level representation aŝ e = 1≤i≤N σ(w(e i )) · ℵ(e i ), where ℵ is another learnable projection function.

We found that best results were achieved by computing the final e as W · (eê) for some learnable matrix W .This method can easily be extended to support additional nodes not present in the original sequence S after running SE (e.g., to accommodate meta-nodes representing sentences, or non-terminal nodes from a syntax tree).
