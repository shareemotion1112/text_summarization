...
We use the Adam optimizer with learning rate equal to 0.25 × 10 −3 , β 1 , β 2 equal to 0.5, 0.999 and train the model for 45 × 10 4 generator steps for CIFAR-10 and 25 × 10 4 generator steps for Anime Faces and LSUN-bedroom datasets.
...
Inspired by the well-known Fréchet Inception Score concept (Heusel et al., 2017) , we consider the Euclidean distance between the outputs of the last layer of the pretrained InceptionV3 network (Szegedy et al., 2016) for semantic distance evaluation.
...
As a standard evaluation measure, we use the Fréchet Inception Distance (FID) introduced in Heusel et al. (2017) .
