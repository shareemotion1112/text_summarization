...
During question coding, we drop the p θz (a|z, i) term from Eq. 1, yielding the objective DISPLAYFORM0 (2) Recent theory from Alemi et al. FORMULA2 prescribes changes to the ELBO, setting β < 1 as a recipe to learn representations which avoid learning degenerate latent representations in the presence of powerful decoders.
...
Thus, one way to achieve a desired behavior (of say high mutual information I(x, z)), is to set β to a lower value than the standard elbo (c.f.

Eqn.

6 in BID1 .)

This aligns with our goal in question coding, and so we use β < 1.

As this no longer a valid variational lower bound, denote our objective as J(x; φ, θ) from now on.
...
Thus, we apply REINFORCE on the entire sequence and compute rewards using the full monte carlo gradient,i.e., we do not optimize the KL term separately, but instead maximize: DISPLAYFORM2 wherex,ẑ are the questions and programs we observe in the dataset.
...
Our results in Section 5 show that the proposed question coding formulation significantly improves performance relative to BID26 We do this by optimizing the joint elbo(x, a|i) in (1) with respect to the parameters θ z of the neural modules.

We write this objective for each instance as DISPLAYFORM3 Notably, while we write this objective as a function of all neural modular network parameters θ z , for a given instance, only those modules present in the program z ∼ q(z|x) are updated.

In practice, we take approx.

argmax decoded programs from q φ (z|x) to simplify training, but perform sampling during joint training in the next stage.
...
A rich literature of work exists in this area, around two major threads: firstly, there is a line of work in building hybrid models DISPLAYFORM0 ∇ φ ← (R − B) log q φ (zi|x) +

α∇ φ log r φ (ẑ|x); where R = log DISPLAYFORM1 which are hybrids of symbolic reasoning and neural networks BID4 BID20 , the second thread looks at studying if neural networks can indeed do symbolic reasoning and building models and architectures to achieve this for specific cases BID11 BID40 .
...
Finally, the best of these module networks (based on val) is trained jointly with the program predictor under settings of γ ∈ {1, 10, 100} and the max (on val) is chosen.
...
Firstly, we disable the KL-divergence term by setting β = 0 in Equation (2) NMN BID26 Table 1 : Results in %, on the SHAPES dataset with varying amounts of question-program supervision (%x ↔ z), β = 0.1.
...
With more supervision the choice of beam search or sampling does not matter, which makes intuitive sense, as the this means that the programs that get sampled tend to be the correct ones more often (that is, the confidence of the model increases with the improvement in correctness).Effect of optimizing the true ELBO, β = 1.

Next, we empirically validate the claim that approximate inference learned using the ELBO leads to autodecoding behavior as predicted by BID1 , and that better program representations for our problem can be found for β < 1 (as shown above).

At 15% program supervision, VQA accuracy drops from 96.90% (±0.96) at β = 0.1 to 78.49% (± 1.37) at β = 1 (for the empirical prior).

Question reconstruction drops from 97.43 % (±3.42) at β = 0.1, to 43.40 % (±5.34) at β = 1, showing auto-decoding behavior (as predicted by theory).Finally, the N2NMN approach BID21 evaluates their question-attention based module networks in the fully unsupervised setting, getting to 96.19% on TEST.
...
Typical values of β are set to 0.1, α is set to 100 and γ is chosen on a validation set among (1.0, 10.0, 100.0).Simulating programs from known syntax: We follow a simple two-stage heuristic procedure for generating a set of samples to train the program prior in our model.
