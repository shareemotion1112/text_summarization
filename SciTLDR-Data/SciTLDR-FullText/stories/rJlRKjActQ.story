...
For the linear interpolation between factors, we define a variable λ and we sample from p(λ).

Following (Zhang et al., 2018) , we always use a beta distribution p(λ) = Beta(α, α).

With α = 1.0, this is equivalent to sampling from U (0, 1).We consider interpolation in the set of layers S k and minimize the expected Manifold Mixup loss.
...
With α = 2.0, about 5% of the time λ is within 5% of 0 or 1, which essentially means that an ordinary example is presented.
...
DISPLAYFORM6 where h i here means the ith column of matrix H, and ζ i ∈ {1, . . .
...
More precisely, the following relations hold: DISPLAYFORM7 1 Due to the consistency theorem that proves neural networks with nonlinear activation functions are dense in the function spaceThe final equality is a direct result of A T h ζi + b = y ζi for i = 1, . . .
...
This way, one can assign g * (X i ) = h ζi +e i , where h j and ζ i (for j = 1, . . .
...
Further analysis of how Manifold Mixup changes the representations is given in Appendix BThere are a couple of important questions to ask: how sensitive is the performance of Manifold Mixup with respect to the hyperparameter α and in which layers the mixing should be performed.

We found that Manifold Mixup works well for a wide range of α values.
...
2.37 Input Mixup (α = 1.5) 2.41 Manifold Mixup (α = 1.5) 1.92 Manifold Mixup (α = 2.0) 1.90 Figure 9 : CIFAR-10 train set Binary Cross Entropy Loss (BCE) on Y-axis using PreActResNet18, with respect to training epochs (X-axis).
...
For supervised loss, we used α (of λ ∼ Beta(α, α)) from the set { 0.1, 0.2, 0.3... 1.0} and found 0.1 to be the best.

For unsupervised loss, we used α from the set {0.1, 0.5, 1.0, 1.5, 2.0.
...
The Fast Gradient Sign Method (FGSM) BID6 is a simple one-step attack that produces x = x + ε sgn(∇xL(θ, x, y)).
...
Mixup proposes the following objective 4 : max DISPLAYFORM0 where x1, x2 can be either real or fake samples, and λ is sampled from a U nif orm(0, α).
...
From these results, the best-performing mixup experiments (both input and Manifold Mixup) is with α = 0.5, with mixing in all layers (both resblocks and input) achieving an average Inception / FID of 8.04 ± 0.08 / 21.2 ± 0.47, input mixup achieving 8.03 ± 0.08 / 21.4 ± 0.56, for the baseline experiment 7.97 ± 0.07 / 21.9 ± 0.62.
...
If we consider any two points, the interpolated point between them is based on a sampled λ and the soft-target for that interpolated point is the targets interpolated with the same λ.
...
If we consider two points A,B with different labels, then the point which is halfway between them will be given the soft-label of 50% the label of A and 50% the label of B (and so on for other λ values).It is clear that for many arrangements of data points, it is possible for a point in the space to be reached through distinct interpolations between different pairs of examples, and reached with different λ values.
...
We compare the performance of Manifold Mixup using different values of hyper-parameter α by training a PreActResNet18 network on Cifar10 dataset, as shown in TAB6 .
