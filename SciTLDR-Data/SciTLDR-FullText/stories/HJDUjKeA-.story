...
For each (a, b) ∈ M , define the mean position x µ (a, b) ∈ R 2 as the center of mass within this window: DISPLAYFORM4 We could use x µ (a, b) as an object position.
...
Thus define x σ (a, b) 2 ∈ R 2 to be the diagonal of the covariance matrix of the mask within the window B r (a, b).
...
Thus to discourage useless objects from appearing, it is sometimes necessary to add a loss penalty to discourage the "expected slot occupancy" going beyond some amount α, DISPLAYFORM0 (In our application to Atari, we take α = 0.5.)We have experimented with other schemes, such as sampling objects in proportion to their presence value, but have found that these more complex schemes do not perform as well.
...
Then the output mask is m = σ(Conv FIG0 ).
...
For each object, we split the output into two tensors X, Y ∈ R 15×15×d , set X = pσ(X − 4) and Y = pY (where p is the corresponding object's presence), recombine X and Y , and add together into a "buffer" with dimensions h×w×2d (the buffer is zero where there are no objects drawn).
...
Let α, β, γ be the input stream sizes, and let a, b, c be the output stream sizes (for all except the first and last layer, α = a, β = b, and γ = c).

The layer will be a mapping DISPLAYFORM0 reduce(C) i,u = (1/k) j p j σ(C i,j,1 )C i,j,u ∈ R k×γ ,i.e., the last object axis is averaged over (as in interaction networks), with additional gating on presence (as required), and self-gating (which improves performance).Additionally define a restricted stream tensor for A and B for computational expediency: letÃ ∈ R min(α,b) andB ∈ R k×min(β,c/2) be the restrictions of A and B to the first b and c/2 indices in the non-object axis respectively.

As in interaction networks, define marshall(B) ∈ R k×k×min(2β,c) by marshall(B) s,t,u = B s,u if u ≤ min(β, c/2), B t,u−k otherwise.

Further define broadcast(Ã) ∈ R k,min(α,b) to be A repeated along an additional first axis.

Now (finally) let A = concat(A, reduce(C)), B = concat(broadcast(Ã), B, reduce(C)), and C = concat(marshall(B), C), where concat is defined as the concatenation of the tensors over the nonobject axis.
