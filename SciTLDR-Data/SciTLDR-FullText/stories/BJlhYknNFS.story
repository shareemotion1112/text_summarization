...
The general framework used here for inference amortization is a neural ordinary differential equation (ODE) system , a differentiable deterministic transformation from a reference distribution q 0 to the desired target density q Φ (· | x).

This transformation is parametrized as a on latent particles z,

where conditioning is achieved by providing x as a constant control input to the neural network f Φ .

The numerical computation at inference time is then performed by a standard ODE solver, integrating independent particle trajectories along the dynamics in Equation (1), from initial conditions z 0 ∼ q 0 to approximate posterior samples z 1 ∼ q Φ (· | x).
...
This divergence term is equivalent to the trace of the Jacobian of f Φ .
...
In order to constrain the architecture of the flow network f Φ to respect the necessary statistical independence structure, the weight matrix of each layer h Φ l is masked with the adjacency H of the minimally faithful inverted graphical model, i.e., the output reads

Here the column (h Φ l (ẑ, ·)) i l of activations across layers l corresponds to a node i in the graphical model, σ is the activation function tanh, b is a bias, and η l,· are time dependent linear gating functions modelling time dependencies of the flow as in .
...
While the forward KL term measures the quality of density estimation on the support of the true posterior, the reverse KL term incentivizes samples from q Φ to behave similarly to the latter.

Efficient estimation of this objective is possible in this setting, because the joint model is available and the variational posterior q Φ is reparametrized.

Importantly, in contrast to expected forward or reverse KL alone, L [q Φ ] does not contain the unknown constant factor E x ∼ X ln p(x).
