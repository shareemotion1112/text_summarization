...
Concretely, we study the cosine similarity NNS problem mentioned above and analyze the running time of the popular hyperplane LSH BID3 .Suppose that a query q makes an angle of α with the closest vector.

Under reasonable assumptions on the balance of the data, we show the following statement in Appendix C. If we want to ensure that the probability of finding the closest vector is at least, 1 − exp(−5) ≈ 99.33%, the expected number of candidates considered in a nearest neighbor search, for a dataset of size n, grows with α as DISPLAYFORM1 We defer our formal theorem with its proof to the supplementary material.
...
For n = 10 6 data points, improving the angle α from π/3 (60 degrees) to π/4 (45 degrees) reduces the number of nearest neighbor candidates (i.e., distance computations) from about 16K to 1.5K, whichis roughly a 10× speed-up..

In FIG0 (a), we plot the expression (1) as a function of the angle α for n = 10 6 .
...
For our purposes, a neural network consists of three sets of parameters: (i) the network weights θ ∈ Θ defining a mapping φ θ : X → R d , (ii) a set of class vectors {v y } y∈ [C] that also lie in R d , and (iii) a bias vector b ∈ R C .

We will often write the class vectors as a matrix V .

We refer to the vectors φ θ (x) as the representation vectors.
...
To understand the softmax objective better, we write it as DISPLAYFORM2 The training process aims to achieve a small loss, i.e., we want the correct dot product φ θ (x) v y to be large and the other dot products φ θ (x) v j to be small.
...
The norm of the representation vector φ θ (x) is irrelevant for the 0-1 prediction loss objective.

Yet it can have significant influence on the softmax loss.3.

A small angle between the representation vector φ θ (x) and the class vector v j is beneficial for both the prediction loss objective as well as the softmax loss objective.
...
Our main focus is then to improve the mean correct angle between the representation vector φ θ (x) and the correct class vector v y .
...
As discussed in Section 3.1, scaling the representation vectors φ θ (x) can change the loss function without changing the prediction accuracy.

The dynamics of (stochastic) gradient descent update parameters so as to increase the inner product φ θ (x) v j , or equivalently, the product DISPLAYFORM0 With a constraint on the term φ θ (x) 2 , the training process tends to increase the remaining terms instead, which leads to smaller angles ∠(φ θ (x), v j ).
...
Batch normalization (Ioffe & Szegedy, 2015) can be used to ensure that each activation has mean 0 and variance 1 at equilibrium, which implies that the mean squared norm of the representation is d. Finally, one can simply "regularize" by adding a λ DISPLAYFORM1 2 2 term to the loss.
...
Use All Directions In a standard ReLU-based neural network, the representation vector φ(x) is the output of a ReLU.

In particular, this means that each coordinate of φ(x) is non-negative, i.e., the representation vector always lies in the non-negative orthant.
...
To ameliorate this issue, we propose to either 0 1000 2000 3000 4000 5000Step Step FORMULA12 Step 8 remove the last layer ReLU or to place it before the batch norm so that the final representation vector φ θ (x) can have both positive and negative coordinates.
...
The appealing property of this synthetic example is that we can plot the representation vectors φ θ (x) and class vectors v i directly in two dimensions.
...
For a query q, the probability that that a dataset point within an angle of α lies in the same LSH bucket as q is p α eq DISPLAYFORM0 k .

To ensure that the failure probability is small, say, exp(−5) ≈ 0.67%, it suffices to take m to be 5/p α .
...
Using the earlier formula for the collision probability p α , we get the following formula for the number of candidate points as a function of angle α: DISPLAYFORM7 This expression allows us to estimate the number of candidate distance computations as we decrease the angle between the pre-softmax vector and the correct class vectors.

We illustrate our estimate by substituting concrete numbers: if n = 10 6 , improving α from π/3 (60 degrees) to π/4 (45 degrees) reduces the number of hash bucket lookups from about 16K to 1.5K, i.e., an order of magnitude improvement.

In FIG0 , we plot the expression (C) as a function of the angle α for n = 10 6 .
...
LN: Layer Normalization BID1 .Scaling: Using a normalized representationφ θ (x) = φ θ (x)/ φ θ (x) .To evaluate these options, we used multiMNIST data set with 1K classes.
