...
In our approach, we follow the convention to optimise over the perturbation δ instead of the adversarial example x directly (Chen et al., 2017; Alzantot et al., 2018; Tu et al., 2018) .
...
There are 2 key components in BO: a statistical surrogate, such as a Gaussian process (GP) or a Bayesian neural network (BNN) which models the unknown objective, and an acquisition function α(·) which is maximised to recommend the next query location by trading off exploitation and exploration.
...
which is specified by a mean function µ and a kernel/covariance function k (we use the Matern-5/2 kernel in our work).

In our work, we normalise the objective function value and thus use a zeromean prior µ(·) = 0.

The predictive posterior distribution for y t at a test point δ t conditioned on the observation data

where

where

The optimal GP hyper-parameters θ * such as the length scales and variance of the kernel function k can be learnt by maximising the marginal likelihood p(D t−1 |θ) which has analytic form as presented in Appendix F. For a detailed introduction on GPs, please refer to (Rasmussen, 2003) .

Based on the predictive posterior distribution, we construct the acquisition function α(δ|D t−1 ) to help select the next query point δ t .

The acquisition function can be considered as a utility measure which balances the exploration and exploitation by giving higher utility to input regions where the functional value is high (high µ y ) and where the model is very uncertain (high k y ).
...
When the unknown function is highdimensional, estimating it with non-parametric regression becomes very difficult because it is impossible to densely fill the input space with finite number of sample points, even if the sample size is very large (Györfi et al., 2006) .
...
The optimal perturbations in all the subspaces {δ (Aj ) * } M j=1 are then combined to give the next query point δ t .

Generating the successful adversarial example x ∈ R d by searching perturbation in a reduced dimension δ ∈ R d r has become a popular practice that leads to significant improvement in query efficiency (Chen et al., 2017; Tu et al., 2018; Alzantot et al., 2018) .
...
Given that our BayesOpt attack uses a statistical surrogate (i.e. GP in our case) to model the unknown relation between the attack objective score y and the adversarial perturbation δ, this naturally translates to the criterion of maximising the posterior for d r : (Rasmussen, 2003) .
...
We set δ max = 0.3 for attacking MNIST and δ max = 0.05 for CIFAR10, which are used in (Alzantot et al., 2018) .
...
Therefore, we only consider the final adversarial examples whose L ∞ distances from the original images lie within [−δ max , δ max ] as the successful attacks 3 .
...
However, we need to bear in mind that although we limit the mean L inf norm of the adversarial perturbation found by AutoZOOM to δ max , AutoZOOM still have the advantage of exploring beyond the δ max for many dimensions.
...
The key to finding the right hyperparameters θ * in light of D t−1 in a principled way is given by the marginal likelihood in Equation (10).

where we have introduced a slightly augmented notation of K 1:t−1 ≡ K 1:t−1 (θ) to highlight the dependence on the kernel hyperparamaters θ.

In a truly Bayesian approach, one could shy away from fixing one set of hyperparameters and use Equation (10) to derive a posterior distribution of θ based on one's prior beliefs about θ expressed through some distribution p 0 (θ).

However, as such an approach generally requires sampling using methods like Markov chain Monte Carlo (MCMC) due to intractability of integrals, in practice it is often easier and computationally cheaper to replace the fully Bayesian approach by a maximum likelihood approach and simply maximise the likelihood p(D t−1 |θ) w.r.t.
...
After evaluating a grid of 5 000 points, we start gradient ascent on each of the 5 most promising points using the following equation for the gradient (Rasmussen & Williams, 2006) to find the hyperparameters θ * which maximise the marginal likelihood:

Model selection.

Once the optimal hyperparameters θ * are found as described above, they can be plugged back into Equation (10) to choose amongst different models.
