...
FIG1 illustrates the behavior of the generalization error as a function of increasing (from left to right, or decreasing, from right to left) the load parameter α.

There is a critical value α c where the the generalization properties change dramatically, and for other values of α the generalization properties change smoothly.• Two-dimensional phase diagram.

FIG1 illustrates the phase diagram in the two-dimensional space defined by the α and τ parameters.
...
Starting from the (α, τ ) point A, which exhibits good generalization behavior, adding noise casues α to decrease, leading to point B, which exhibits poor generalization.

This can be offset by adjusting (for A → B → C, this means decreasing) the number of iterations to modify the τ parameter, again leading to good generalization.

FIG1 (c) also illustrates that, starting from the (α, τ ) point A , adding noise casues α to decrease, leading to point B , which also has poor generalization, and this can be offset by adjusting (except for A → B → C , this means increasing) the number of iterations to modify the τ parameter to obtain point C .The VSDL model and these consequences are described in more detail in Sections 3.1 and 3.2.We should note that the SM approach to generalization can lead to quantitative results, but to achieve this can be technically quite complex (9; 30; 11; 34; 31) .
...
BID77 For example, in the Hopfield model of associative memory, depending on the values of the load parameter α = m/N (where m is the number of random memories stored in a Hopfield network of N neurons) and the temperature parameter τ , the system can be in a high-temperature ergodic phase or a spin glass phase (in which the states have a negligible overlap with the memories) or a low-τ low-α memory phase (which has valleys in configuration space that are close to the memory states); and, as the control parameters α and τ are changed, the system can change its retrieval properties dramatically and qualitatively.
...
In the simplest case, the dependence of f on various parameters and hyperparameters of the learning process will not be modeled, with the exception of a load-like parameter α and a temperature-like parameter τ .

Thus, the function f implemented by the DNN system depends parametrically (or hyperparametrically) on α and τ , i.e., f = f (x; α, τ ).

We refer to this model as a Very Simple Deep Learning (VSDL) model.

Importantly, both the α parameter and the τ parameter can be easily controlled during the DNN training.
...
Another example is provided by the Erdős-Rényi random graph model: for values of the connection probability p < 1/n, there does not exist a giant component, while for values of the connection probability p > 1/n, there does exist a giant component.
...
Since we are interested in rethinking generalization and understanding deep learning, we are primarily interested in macroscopic properties of DNN learning systems, rather than their microscopic improvements.• Adding noise decreases an effective load α.
...
By analogy with the capacity load parameter in associative memory models, we can define an effective load-like parameter, denoted α, to be α = m ef f /N .
...
Thus, adding noise to the training data has the effect of decreasing the load α on the network.
...
Early stopping increases an effective temperature τ .
...
Thus, from the fluctuation-dissipation theorem, there is a temperature τ that corresponds to the learning rate of the stochastic dynamics.

BID8 Operationally, this τ has to do with the annealing rate schedule of the SGD algorithm, which decreases the variability of, e.g., the NN weights, and thus with t −1 * , where t * is the number of steps taken by the stochastic iterative algorithm when terminated.

This temperature-like parameter will be denoted by τ ; and it depends on t * as τ = τ (t −1 * ) in some manner that we won't make explicit.
...
The point is that both α and τ are parameters that the practitioner can use to control the learning process, e.g., by adding noise to the input data or by early-stopping.
...
As a function of the load-like parameter α, e.g., for τ = 0, one should expect the VSDL model to have error plots that look qualitatively like the ones shown in FIG1 (a).

In this figure, the generalization and training errors are plotted as a function of α.• Two-dimensional phase diagram.

In addition, when the temperature-like parameter τ is also varied, one should expect the VSDL model to have a phase diagram that looks qualitatively like the one shown in FIG1 (b).

In this figure, the τ -α plane is shown, and rather than plotting a third axis to show the generalization error and training error as a function of τ and α, we instead show lines between different phases of learning.

To understand these diagrams, consider first FIG1 (a).

Observe that, for τ = 0 as in that figure, as one increases α from a small value (i.e., as one obtains more data), the generalization error decreases gradually (as is intuitive), and then as one passes through a critical value α c it decreases rather dramatically.

Alternatively, as one decreases α from a large value (e.g., as one adds noise to the data), the generalization error increases gradually (again, as is intuitive) and then as one passes through the critical value α c it increases rather dramatically.

The transition from α > α c to α < α c means that there is a dramatic increase in generalization error, where one can fit the training data well, but where one does a very poor job fitting the test data.

This is illustrated pictorially along the τ = 0 axis of FIG1 (b).

These observations hold for any given value of τ , e.g., τ = 0 or τ > 0, although the value α c may depend on τ .

This is shown more generally in FIG1 .

Moreover, for certain values of τ greater than a critical value, i.e., for τ > τ c , the sharp transition in learning as a function of α may disappear, in which case the system exhibits only one phase of learning.
...
See FIG1 (c) for a pictorial representation, in the (α, τ ) plane, of the the process of adding noise to data and adjusting algorithm knobs to compensate.

Assume that a DNN is trained to a point A, with parameter values (α A , τ A ), and assume that at this point the system exhibits good generalization behavior, e.g., as for α > α c in FIG1 .

If then some fraction of the data labels are randomly changed, the system moves to point B, with parameter values (α B , τ B ), where τ B = τ A .

At point B, the DNN can still be trained to fit the new noisy data, but if enough of the data have their labels changed, then the effective load parameter α < α c , for this value of τ .

In this case, the generalization properties on new noisy data are much worse, as for α < α c in FIG1 .

Of course, this could then be compensated for by adjusting the temperature parameter τ , e.g., by performing early stopping BID9 , after which the system moves to C, with parameter values (α C , τ C ), where α C = α B .

For this new point, if the iterative algorithm is stopped properly, then α > α c for this new value of τ , in which case the generalization properties are then much better, as for α > α c in FIG1 (a).
...
For realistic NNs and DNNs as well as the VSDL model, there typically is not a global control parameter like the Tikhonov value off λ or the number k of vectors to keep in the TSVD that permits control on generalization for any phase.

This is in contrast to linear or linearizable learning, and it is discussed in more detail in Section A.5.

That is, for certain values of τ and α, which are the parameters used to control the learning process, the system is in a phase where it can't not overfit.
...
That is, for a given value of α, i.e., for a given value of the noise added to the data, the only control parameter that can prevent overfitting is τ .

That is, in this idealized model of realistic DNNs, where τ and α are the two control parameters, for a given amount of effective data, the only way to prevent overfitting is to decrease the number of iterations. (This is simply Observation 2.)That is, given our three main claims, our two motivating observations follow immediately.
...
FIG3 (a) shows the generalization error ε as a function of the control parameter αβ, BID11 where α is a load parameter and β is a temperature parameter, and it illustrates the discontinuous behavior of ε as a function of αβ.• Tree-based parity machine.
...
FIG3 shows the generalization error ε as a function of the control parameter α, for several different values of K, and it illustrates the discontinuous behavior of ε as a function of α.• One-layer reversed-wedge Ising perceptron.

This model is a single layer network, but it is interesting since it has a non-trivial activation function.

Given any input vector S ∈ R N , and the set of weights given by a vector J ∈ R N , if we define λ = DISPLAYFORM3

where γ is a parameter.

The non-monotonicity of the activation function is a simple model of representation ability, and the classification is +1 or −1 depending on the value of λ with respect to the value of γ.

See (49; 50) for more details on this model; and see FIG3 (c) for the corresponding learning curve.

FIG3 shows the generalization error ε as a function of the control parameter α, for several different values of γ, and it illustrates the discontinuous behavior of ε as a function of α.
...
The quality of the performance of the student on the training set can be quantified by the training error ε t , which is the fraction of disagreements between the student and teacher output on inputs in the training set (where in the realizable situation one may achieve ε t = 0).

Then, one is interested in characterizing the difference between training error and the generalization error |ε t − ε| .(1) The behavior of Eqn.
...
This is the probably approximately correct (PAC) framework (71), where two accuracy parameters, δ and γ are used.
...
Thus, one typically considers the case that m, N → ∞ such that α = m/N is a fixed constant.

BID14 This α is analogous to the load on the network in associative memory models, and it is a control parameter.
...
Given any input vector S ∈ R N , the basic single-layer perceptron has a set of weights given by a vector J ∈ R N , and the output classification rule is given by DISPLAYFORM0 That is, the classification is +1 or −1 depending on whether the angle between S and J is smaller or larger than π/2.
...
In this model, J ∈ R N , subject to the constraint that J 2 = N (and the output σ ∈ {−1, +1}).
...
In this model, J ∈ R N , subject on the constraint that each J i ∈ ±1 (and the output σ ∈ {−1, +1}).
...
To quantify the probability that a vector J remains compatible with the teacher when a new example is presented, we can group the vectors J into classes depending on their overlap R with T , i.e., depending on the average generalization error ε.

For all J with overlap R (or generalization error ε), the chance of producing the same output as T on a randomly chosen input is (by definition) 1 − ε.

If we let Ω 0 (ε) denote the volume of vectors J with overlap R (or generalization error ε) before any data are presented, then, since the examples are independent, and since the Gibbs learning procedure returns a random element of the version space, each example will reduce this by a factor of 1−ε on average.

Thus, the average volume of compatible students with generalization error ε after being presented m training examples is DISPLAYFORM7 Results from the traditional SM approach.

In this approach to SM, for this problem, generalization is characterized by the volume Ω m (ε).

In more detail, it is controlled by the balance between an energy and an entropy, where entropy density s(ε) refers to the logarithm of the volume DISPLAYFORM8 , it is not the thermodynamic entropy, which arises in other contexts) and energy e(ε) refers to the penalty due to incorrect predictions e(ε) = α ln(1− ).
...
For the continuous perceptron, we have that DISPLAYFORM9 From this, it follows that the entropy behaves as DISPLAYFORM10 ∼ ln(ε) (for small ε or large α).

Observe that the entropy slowly diverges to −∞, as ε → 0 or as R → 1.

Since the examples are independent, the energy behaves as e(ε) ∼ −α ln(1 − ε) ∼ αε (for small ε or large α).
...
To determine the maximum, we consider optimizing the difference s(ε) − e(ε).

See Sec. V.B of (9) for a more complete discussion; but for the small ε or large α, we have that DISPLAYFORM11 From this, we obtain the scaling DISPLAYFORM12 showing the smooth decrease in the generalization error with increasing number of examples.
...
In this case, we have that DISPLAYFORM13 where recall that R = cos(πε).

From this it follows that the entropy behaves as DISPLAYFORM14 Observe that the entropy approaches zero as ε → 0 or as R → 1, meaning that there is exactly one state with R = 1.

Since the examples are independent, the energy behaves as e(ε) ∼ −α ln(1 − ε) (11) ∼ αε (for small ε or large α).Following the same asymptotic analysis as with the continuous perceptron, we again consider minimizing s(ε) − e(ε) by exploiting the first order condition.

See Sec. V.D of FORMULA22 for a more complete discussion; but for the small ε or large α, we have that DISPLAYFORM15 For small-to-moderate values of α, i.e., when not too much data has been presented, this expression has a solution; but for large values of α, this equation has no solution, indicating that the optimal value of of the expression is not inside the interval ε ∈ [0, 1] (or R ∈ [−1, 1]), but instead at the boundary ε = 0 (or R = 1).

From this, we should not expect a continuous and smooth decrease of the generalization error with increasing training set size, and in general there is a discontinuous change (drop if α is increasing and jump if α is decreasing) in ε as a function of α at a critical value α c .
...
For the discrete Ising perceptron, when the only control parameter is α, which determines the amount of data, the generalization behavior is more complex.

In this case, there is a one-dimensional phase diagram, and depending on the value of α, there are two phases in which the learning system can reside-one in which the generalization is large and smoothly decreasing with increasing α, and one in which it is small or zero-and there is a discontinuous change in the generalization error between them.
...
In general, the learning algorithm may not find a random point from the version space, the problem may not be realizable, etc., and in these cases there are additional control parameters, e.g., a temperature parameter τ , e.g., to avoid reproducing the training data exactly.

In this case, the qualitative properties we have been discussion remain, but since there are two control parameters the phase diagram becomes two-dimensional, and one can have non-trivial behavior as a function of both α and τ .

The full two-dimensional phase diagram of the discrete Ising perceptron is shown in FIG5 , where it shows-depending on the values of α and τ -a phase of perfect generalization, a phase of poor generalization, a (mean field) spin glass phase, metastable regimes, etc.

For completeness, the trivial two-dimensional phase diagram of the continuous perceptron (it is trivial since there is only one phase, where generalization varies continuously with α and τ in intuitive ways) is shown in FIG5 (b).
...
Recall that the version space V (S) ⊆ F is the set of all functions that are consistent with the target function T on the sample S, i.e., it is a sample-dependent subclass of F. Also of interest is the -ball around the target function, defined to be the set of functions with generalization error ε not larger than , i.e., B( ) = {h ∈ F : ε(h) ≤ } , which is a sample-independent subclass of f .

Both V (S) and B( ) contain the target function f ; and V (S) ⊆ B( ) implies that any consistent h has generalization error ε(h) ≤ .

Thus, lower bounds on δ = Pr [V (S) ⊆ B( )], where the probability is taken over the m independent draws from D used to obtain S, or equivalently upper bounds on 1 − δ = Pr [V (S) ⊆ B( )], provide bounds on the generalization error ε = ε(h) of any consistent learning algorithm outputting h ∈ V (S).From Eqn.

(5) , the probability that a function h with generalization error ε(h) remains in DISPLAYFORM16 m .

If we let B( ) = F − B( ) denote the functions in F with generalization error greater than , then DISPLAYFORM17 If the failure probability δ rather than the error value is fixed, then it follows that if h ∈ F is any function consistent with the m random examples of a target function in F, then with probability at least 1 − δ, we have that DISPLAYFORM18 That is, the generalization error ε(h) is given by a sum of quantities over B( ), and one wants to minimize in this expression to obtain improved bounds.

As a straw-man bound, assume, for simplicity, that |F| < ∞. Then, DISPLAYFORM19 from which it follows that any consistent h satisfies ε(h) ≤ 1 m ln (|F|/δ), with probability at least 1 − δ.

This PAC/VC-like bound does not depend on the distribution D or the target function T , and it depends on F only via |F|.

It can, however, be very weak. (The PAC bound holds for all h, but it does not guarentee that the algorithm finds the best h.) In particular, we can have values of ε(h) that are much larger than ; and, if we let Q j = |{f ∈ F : ε(f ) = j }| be the number of functions in F with generalization error exactly j , then in general Q j |F|.More refined upper bounds on the left hand side of Eqn.
...
BID17 for which > * + τ , where τ > 0 is arbitrary, then in this thermodynamic limit the sum equals 0, and thus in this limit we can bound generalization error by * + τ .

More precisely, DISPLAYFORM24 To understand this in terms of the trade-off between entropy and energy, observe that s( ) and −α log(1 − ) are non-negative functions, and that 0 = −α log(1 − ) ≤ s( ) for = 0.
...
This is shown in FIG5 (e), along with the plots of −α log(1 − ) for three different values of α.

In this case, if the rightmost intersection points are plotted as a function of α, then one obtains FIG5 (f), which plots the learning curve corresponding to the energy-entropy competition of FIG5 (e).

This figure shows a gradual smooth decrease of ε with increasing α, consistent with the results from Eqn.

BID7 , and in accordance with PAC/VC theory.• Ising perceptron.

For the Ising perceptron, it can be shown that an entropy upper bound is s( ) = H(sin 2 (π /2)), where H(x) = −x log x − (1 − x) log(1 − x).

The function s( ) is shown in FIG5 (g), along with the plots of −α log(1 − ) for three different values of α.
...
In this case, if the rightmost intersection points are plotted as a function of α, then one obtains FIG5 , which plots the learning curve corresponding to the energy-entropy competition of FIG5 .

Importantly, for smaller values of α, i.e., when working with only a modest amount of data, the rightmost crossover point is obtained at a non-zero value, and it decreases gradually as α is increased.

Then, as more data are obtained, one hits a critical value of α, and the plot of s( ) and −α log(1 − ) do not intersect, except at the boundary 0.

At this critical value of α, the plot in FIG5 (h) decreases suddenly to 0; and for larger values of α, the minimum is given at the boundary.

This non-smooth decrease of ε with α is not described even qualitatively by PAC/VC theory, but it is consistent with the results from Eqn.

BID12 , which show that the expression for the first order condition has a solution only for small-to-moderate values of α.
...
It is known that the REM exhibits a transition in its entropy density at a non-zero value of the temperature parameter τ , at which point the entropy vanishes; see, e.g., Chapter 5 of (75).

That is, above a critical value τ c , there is a relatively large number of configurations, and below that critical value τ c , there is a single configuration (or constant number of configurations).

As described for the Ising perceptron, and as opposed to the continuous perceptron, this phenomenon of having a small entropy s(ε) for configurations with loss ε slightly above the minimum value is the mechanism responsible for the complex learning behavior we have been discussing.
...
Given a matrix A ∈ R n×p and a vector b ∈ R n , one wants to find a vector x ∈ R p such that Ax = b. A naïve solution involves computingx = A −1 b, but there are a number of subtleties that arise.
...
The Tikhonov-Phillips solution was to replace Problem BID18 with the related problemx DISPLAYFORM1 for some λ ∈ R + , the solution to which iŝ DISPLAYFORM2 The TSVD method replaces Problem BID18 with the related problem DISPLAYFORM3 where A k ∈ R n×p is the matrix obtained from A by replacing the bottom p − k singular values with 0, i.e., which is the best rank-k approximation to A. The solution to Problem (22) is given bŷ DISPLAYFORM4 where A + k is the Moore-Penrose generalized inverse of A. This should be familiar, but several things should be emphasized about this general approach.• First, the value of the control parameter λ controls the radius of convergence of the inverse of A T A + λ 2 I (i.e., of the linear operator used to compute the estimatorx in the Tikhonov-Phillips approach).

Similarly, the value of the control parameter k restricts the domain and range of A k (i.e., of the linear operator used to compute the estimatorx in the TSVD approach).• Second, one can always choose a value of λ (or k) to prevent overfitting, potentially at the expense of underfitting.

That is, one can always increase the control parameter λ (or decrease the control parameter k) enough to prevent a large difference between training and test error, even if this means fitting the training data very poorly.

This is due to the linear structure of A T A + λ 2 I (and of A k ).
...
FORMULA5 and FORMULA5 are not applicable, but it is still the case that one can always increase a control parameter (λ, the number k of support vectors, etc.) enough to prevent overfitting, even if this means underfitting.
...
First, when one can reduce the generalization problem to an optimization problem of the formx = argmin x f (x) + λg(x), then one can clearly accomplish this simply by increasing λ.
