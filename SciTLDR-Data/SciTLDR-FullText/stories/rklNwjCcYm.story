...
, h T ∈ R d2 by DISPLAYFORM0 are trainable weight matrices and biases, tanh denotes hyperbolic tangent, σ denotes sigmoid function, and denotes element-wise multiplication.
...
For each head i ∈ {1..m}, the attention weight matrix and context matrix are computed by DISPLAYFORM0 where W qi , W ki , W vi ∈ R d h ×dc are trainable projection matrices and σ performs softmax along the second dimension.

Each row of the resulting α 1 , α 2 , . . .

, α m ∈ R n×n contains the attention weights of a token to its context, and each row of C 1 , C 2 , . . .

, C m ∈ R n×dc is its context vector.

DISPLAYFORM1 H , the computation of α i and C i models the cross interaction between past and future.
...
FIG2 shows one of the samples, where the attention weights α 2 , α 3 , α 4 of a sentence containing the B White I house E are visualized.
...
We observe that α 2 and especially α 3 have a tendency to focus on the previous tokens: the diagonal shifted left.

In contrast, α 4 tends to look at the immediate following tokens: the diagonal shifted right.
...
This behavior can be observed throughout the heat maps of α 2 , α 3 , α 4 .

In particular for the White house, C all predicts the B White I house O as Saturday is wrongly focused by α 4 for house.
...
With attention, in Figure 3 , we see that α 1 attends to Dutch and English at the same time for the two tokens and attends to Chinese and English at the same time for the other two.

On the other hand, α 5 focuses on all possible LAN tokens, including a small mis-attention to Taiwanese in the second sentence, which is actually a NORP in this case.
...
In Section 4.1, we briefly mentioned that the computation of attention weights α i and context features C i models the cross interaction between past and future.
