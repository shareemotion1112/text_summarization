...
If W 1 x i +b 1 = 0, then the sign of (W 1 +∆ 1 )x i +(b 1 +δ 1 ) for any small perturbations ∆ 1 and δ 1 stays invariant.
...
DISPLAYFORM0 and similarly, the slope is s − for ∆ 1 x i + δ 1 ≤ 0.

This means that the "gradient" (as well as higher order derivatives) of R depends on direction of (∆ 1 , δ 1 ).Thus, every boundary data point x i bisects the space of perturbations (∆ j , δ j ) 2 j=1 into two halfspaces by introducing a hyperplane through the origin.
...
We can expand R((W j + ∆ j , b j + δ j ) 2 j=1 ) and obtain the following Taylor-like expansion for small enough perturbations (see Lemma 2 for details) DISPLAYFORM1 j=1 .

Notice now that in (1), at nondifferentiable points the usual Taylor expansion does not exist, but the corresponding "gradient" g(·) and "Hessian" H(·) now depend on the direction of perturbation η.

Also, the space of η is divided into at most 2 M regions, and g(z, η) and H(z, η) are piecewise-constant functions of η whose "pieces" correspond to the regions.
...
Given this expansion (1) and the observation that derivatives stay invariant with respect to scaling of η, one can note that (a) g(z, η) T η ≥ 0 for all η, and (b) η T H(z, η)η ≥ 0 for all η such that g(z, η)T η = 0 are necessary conditions for local optimality of z, thus z is a "SOSP" (see Definition 2.2).

The conditions become sufficient if (b) is replaced with η T H(z, η)η > 0 for all η = 0 such that g(z, η)T η = 0.
...
Whenever a test fails, we get a strict descent direction η, and the algorithm returns η and terminates.
...
ForStep (b), we formulate linear programs (LPs) per each 2 M region, so that checking whether all LPs have minimum cost of zero is equivalent to checking g(z, η)T η ≥ 0 for all η.
...
It turns out that there are only 2M extreme rays, each shared by 2 M −1 cones, so testing g(z, η) T η ≥ 0 can be done with only O(M ) inequality/equality tests instead of solving exponentially many LPs.

In Step (b), we also record the flat extreme rays, which are defined to be the extreme rays with g(z, η)T η = 0, for later use in Step (c).

Step (c), we test if the second-order perturbation η T H(·)η can be negative, for directions where g(z, η)T η = 0.

Due to the constraint g(z, η) T η = 0, the second-order test requires solving constrained nonconvex QPs.
...
Suppose that a function f (z) : Ω → R is locally Lipschitz around the point z * ∈ Ω, and differentiable in Ω \ W where W has Lebesgue measure zero.
...
Consider perturbing parameters DISPLAYFORM2 , then the perturbed outputỸ (x) of the network and the amount of perturbation dY (x) can be expressed as DISPLAYFORM3 where J(x) can be thought informally as the "Jacobian" matrix of the hidden layer.
...
Next, we define the total number of parameters p, and vectorized perturbations η ∈ R p : DISPLAYFORM10 , packed in the same order as η.
...
We first present a lemma that shows the explicit form of the perturbed empirical risk R(z +η) and identify first and second-order perturbations.

The proof is deferred to Appendix B.2.

end if 10: end for 11: For all η's s.t.

g(z, η) DISPLAYFORM0 return SOSP.

14: else 15:return Local Minimum.

16: end if Lemma 2.

For small enough perturbation η, DISPLAYFORM1 where g(z, η) and H(z, η) satisfy DISPLAYFORM2 .

Also, g(z, η) and H(z, η) are piecewise constant functions of η, which are constant inside each polyhedral cone in space of η.

Rough pseudocode of SOSP-CHECK is presented in Algorithm 1.

As described in Section 2.1, the algorithm consists of three steps: (a) testing first-order stationarity (b) testing g(z, η)T η ≥ 0 for all η, and (c) testing DISPLAYFORM3 .
...
If a strict descent direction η is found, the algorithm terminates immediately and returns η.
...
More details are in Appendix A.1.1.Test for W 1 and b 1 is more difficult because g(z, η) depends on ∆ 1 and δ 1 when there are boundary data points.
...
For each i ∈ B k , the extreme rayv DISPLAYFORM8 Note that there are 2M k extreme rays, and one extreme rayv i,k is shared by 2 DISPLAYFORM9 regardless of {σ j } j∈B k \{i} .
...
FO-INCREASING-TEST computes all possible extreme raysṽ k and tests if they satisfy DISPLAYFORM11 If the inequality is not satisfied by an extreme rayṽ k , then this is a descent direction, so we returnṽ k .
...
How many flat extreme rays (g k (z,ṽ k ) Tṽ k = 0) are there?
...
Lemma A.1 in Appendix A.2 shows the conditions for having flat extreme rays; in short, there is a flat extreme ray DISPLAYFORM12 The second-order test checks η T H(z, η)η ≥ 0 for "flat" η's satisfying g(z, η) T η = 0.

This is done with help of the function SO-TEST (Algorithm 5).

Given its input {σ i,k } k∈[d h ],i∈B k , it defines fixed "Jacobian" matrices J i for all data points and equality/inequality constraints for boundary data points, and solves the QP of the following form: DISPLAYFORM13 Constraints and number of QPs.

There are d h equality constraints of the form DISPLAYFORM14 These equality constraints are due to the nonnegative homogeneous property of activation h; i.e., scaling [W 1 ] k,· and [b 1 ] k by α > 0 and scaling [W 2 ] ·,k by 1/α yields exactly the same network.

So, these equality constraints force η to be orthogonal to the loss-invariant directions.
...
The other constraints make sure that the union of feasible sets of QPs is exactly {η | g(z, η)T η = 0} (please see Lemma A.3 in Appendix A.3 for details).
...
Consider the QP, where Q ∈ R p×p is symmetric and A ∈ R q×p has full row rank: DISPLAYFORM18 Then, projected gradient descent (PGD) updates DISPLAYFORM19 with learning rate α < 1/λ max (Q) converges to a solution or diverges to infinity exponentially fast.
...
Note that it takes O(p 2 q) time to compute (I − A T (AA T ) −1 A)(I − αQ) in the beginning, and each update takes O(p 2 ) time.
...
In short, we transform η to define an equivalent problem, and use classical results in copositive matrices (Martin & Jacobson, 1981; Seeger, 1999; Hiriart-Urruty & Seeger, 2010) ; the problem can be solved by computing the eigensystem of a (p−q −r)×(p−q −r) matrix, and testing copositivity of an r × r matrix.
...
Also, we can expand R(z + η) so that DISPLAYFORM0 return (True,ṽ k , {∅} i∈B k )8: .
...
To see why, suppose DISPLAYFORM5 If we apply perturbation (γ∆ j , γδ j ) 2 j=1 where γ > 0, we can immediately check that dY 1 ( DISPLAYFORM6 and also that DISPLAYFORM7 .

Then, by scaling γ sufficiently small we can achieve R(z + η) < R(z), which disproves that (W j , b j ) 2 j=1 is a local minimum.

A.1.2 TEST OF FIRST-ORDER STATIONARITY FOR W 1 AND b 1 (LINES 6-10 AND 15-17)Test for W 1 and b 1 is more difficult because g(z, η) depends on ∆ 1 and δ 1 when there are boundary data points.
...
For the case M k > 0, we saw that for boundary data points DISPLAYFORM1 Since the subdifferential is used many times, we give it a specific name DISPLAYFORM2 It solves a convex QP (2), and returns {s * DISPLAYFORM3 and apply perturbation (γ∆ j , γδ j ) 2 j=1 where γ > 0.

With this perturbation, we can check that DISPLAYFORM4 T η is strictly negative with magnitude O(γ).

It is easy to see that η T H(z, η)η = O(γ 2 ), so by scaling γ sufficiently small we can disprove local minimality of (W j , b j ) DISPLAYFORM5 Linear program formulation.

Lines 11-14 are essentially about testing if DISPLAYFORM6 T is constant in each polyhedral cone, we can let σ i ∈ {−1, +1} for all i ∈ B k , and define an LP for each {σ i } i∈B k ∈ {−1, +1} M k : DISPLAYFORM7 Solving these LPs and checking if the minimum value is 0 suffices to prove DISPLAYFORM8 Note that any component of v k that is orthogonal to V k is also orthogonal to g k (z, v k ), so it does not affect the objective function of any LP (3).
...
Moreover,x T jv i,k = 0 for j ∈ B k \ {i}, which indicates that For both direction of extreme raysṽ k =v i,k andṽ k = −v i,k (Line 4), we check if g k (z,ṽ k ) Tṽ k ≥ 0.

Whenever it does not hold (Lines 6-7),ṽ k is a descent direction, so FO-INCREASING-TEST returns it with True.

Line 13 of Algorithm 2 uses thatṽ k to return perturbations, so that scaling by small enough γ > 0 will give us a point with R(z + γη) < R(z).

If equality holds (Lines 8-9), this means v k is a direction of perturbation satisfying g(z, η) DISPLAYFORM10 T η = 0, so this direction needs to be checked if η T H(z, η)η ≥ 0 too.
...
After checking if g k (z,ṽ k ) Proof First note that we already assumed that all extreme raysṽ k satisfy g k (z,ṽ k ) Tṽ k ≥ 0, so SOSP-CHECK will reach Line 14 at the end.

Also note thatx i 's in i ∈ B k are linearly independent (by Lemma 1), sox DISPLAYFORM11 DISPLAYFORM12 Thus,ṽ k is a flat extreme ray.
...
Note that the equality As mentioned in Section 2.1, given that g(z, η) and H(z, η) are constant functions of η in each polyhedral cone, one can define inequality constrained optimization problems and try to solve for KKT conditions for z directly.
...
DISPLAYFORM15 DISPLAYFORM16 The second-order test checks η T H(z, η)η ≥ 0 for "flat" η's satisfying g(z, η) T η = 0.

This is done with help of the function SO-TEST in Algorithm 5.

Given its input {σ i,k } k∈[d h ],i∈B k , it defines fixed "Jacobian" matrices J i for all data points and equality/inequality constraints for boundary data points, and solves the QP (4).
...
Hence, these d h equality constraints on u k 's and v k 's force η to be orthogonal to the loss-invariant directions.
...
Note that in Line 19, we call SO-TEST with {σ i,k } k∈[d h ],i∈B k = 0, which results in a QP (4) with d h + M equality constraints.
...
The second boolean is to indicate that there are nonzero η that satisfies η T H(z, η)η = 0.

After solving all QPs, if any of SO-TEST calls found out η = 0 such that g(z, η)T η = 0 and η T H(z, η)η = 0, then we conclude SOSP-CHECK with "SOSP."
...
Specifically, since the parameter space is partitioned into polyhedral cones, we have g(z, η) = g(z, γη) and H(z, η) = H(z, γη) for any γ > 0.
...
, w p , andŴ be an submatrix of W whose columns are w q+1 , . . .

, w p .

With this definition, note that I − A T (AA T ) −1 A =ŴŴ T .Suppose that we are given η (t) satisfying Aη (t) = 0.
...
Then λ ∈ Π(Q) if and only if there exists an index set J ∈ 2[r]

\ ∅ and a vector ξ ∈ R |J| such that DISPLAYFORM0 In such a case, the vector η ∈ R r by DISPLAYFORM1 is a Pareto-eigenvector of Q associated to the Pareto eigenvalue λ.
...
We will first transform η to eliminate the equality constraints and obtain an inequality constrained problem of the form minimize w:Bw≥0 w T Rw.
...
Transforming η and testing positive definiteness take O(p 3 ) time and testing copositivity takes O(r 3 2 r ) time, so the test in total is done in O(p 3 + r 3 2 r ) time.

We now describe how to transform η and get an equivalent optimization problem of the form we want.

We assume without loss of generality that A = [A 1 A 2 ] where A 1 ∈ R q×q is invertible.

If not, we can permute components of η.
