...
The LMDP can be solved by finding the desirability function z(s) = e V (s)/λ which is the exponentiated cost-to-go function for a specific state s.

Here λ is a temperature-like parameter related to the stochasticity of the solution.
...
All results presented in this section correspond to solutions to Eqn.(2) for β = 1 so that the cost function is taken to be the KL-divergence (although the method does not appear to be overly sensitive to β ∈ [1, 2]).
...
Let us denote the dependence of d β (·) on the decomposition factor simply as f (k).
