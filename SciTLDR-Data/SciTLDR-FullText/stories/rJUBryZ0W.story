...
Let P ∈ M be some prior distribution over H. Then for any δ ∈ (0, 1], DISPLAYFORM0 where DISPLAYFORM1 The bound (3) can be interpreted as stating that with high probability the expected error er (Q, D) is upper bounded by the empirical error plus a complexity term.
...
from an unknown tasks distribution τ .The goal of the lifelong-learner is to extract some knowledge from the observed tasks that will be used as prior knowledge for learning new (yet unobserved) tasks from τ .
...
Then for any δ ∈ (0, 1] the following inequality holds uniformly for all hyper-posteriors distributions Q with probability of at least 1 − δ, DISPLAYFORM0 Notice that the transfer error FORMULA8 is bounded by the empirical multi-task error (7) plus two complexity terms.
...
The second step, similarly to BID25 , bounds the generalization error at the task-environment level (i.e, the error caused by observing only a finite number of tasks), er (Q, τ ), by the average generalization error in the observed tasks plus the environment-complexity term.
...
Given a parametric family of priors P θ : θ ∈ R N P , the space of hyperposteriors consists of all distributions over R N P .

We will limit our search to a certain family of hyper-posteriors by choosing a Gaussian distribution in the space of prior parameters, DISPLAYFORM0 where κ Q > 0 is a predefined constant.

Notice that Q appears in the bound (8) in two forms (i) divergence from the hyper-prior D KL (Q||P) and (ii) expectations over P ∼ Q.First, by setting the hyper-prior as Gaussian, P = N 0, κ 2 P I N P ×N P , where κ P > 0 is another constant, we get a simple form for the KLD term, DISPLAYFORM1 Note that the hyper-prior serves as a regularization term for learning the prior.

Second, the expectations can be easily approximated using by averaging several Monte-Carlo samples of P .

Notice that sampling from Q θ P means adding Gaussian noise to the prior parameters θ P during training, θ P = θ P + ε P , ε P ∼ N 0, κ 2 Q I N P ×N P .
...
We will use a procedure which minimizes J i (θ P ) due to the following advantages: (i) It minimizes a bound on the generalization error of the observed task (see section 8.1). (ii) It uses the prior knowledge gained from the prior P to get a tighter bound and a better learning objective. (iii) As will be shown next, formulating the single task learning as an optimization problem enables joint learning of the shared prior and the task posteriors.

To formulate the single-task learning as an optimization problem, we choose a parametric form for the posterior of each task Q φi , φ i ∈ R N Q (see section 4.3 for an explicit example).

The single-task learning algorithm can be formulated as φ * i = argmin φi J i (θ P , φ i ), where we abuse notation by denoting the term J i (θ P ) evaluated with posterior parameters φ i as J i (θ P , φ i ).The lifelong-learning problem of minimizing J(θ P ) over θ P can now be written more explicitly, DISPLAYFORM3
...
The techniques presented next will be mostly based on BID4 .Next we define the posteriors Q φi and the prior P θ P as factorized Gaussian distributions 7 , DISPLAYFORM0 where for each task, the posterior parameters vector φ i = (µ i , ρ i ) ∈ R 2d is composed of the means and log-variances of each weight , µ i,k and ρ i,k = log σ 2 P,k , k = 1, ..., d.8 The shared prior vector θ P = (µ P , ρ P ) ∈ R 2d has a similar structure.
...
Since Q φi and P θ P are factorized Gaussian distributions the KLD takes a simple analytic form, DISPLAYFORM1
...
The complexity term is a simple function of D KL (Q φi ||P θ P ) (16), which can easily be differentiated analytically.

However, evaluating the gradient of the empirical error term is more challenging.

Recall the definition of the empirical error, er (Q φi , S i ) = E w∼Q φ i (1/m i ) mi j=1 (h w , z i,j ).
...
We report the best results obtained with all combinations of the following representative hyperparameters: 1-3 gradient steps in meta-training, 1-20 gradient steps in meta-testing and α ∈ {0.01, 0.1, 0.4}. As can be seen in TAB0 , the best results are obtained with the "oracle" method.
...
Define a 'loss function' g(f, X) : DISPLAYFORM0 .., X K } be a sequence of K independent random variables distributed according to X. Let π be some prior distribution over F (which must not depend on the samples X 1 , ..., X K ).
...
We define the 'prior over hypothesis', π (P, P ), as some distribution over M × H in which we first sample P from P and then sample h from P .

According to Theorem 3, the 'posterior over hypothesis' can be any distribution (even sample dependent), in particular, the bound will hold for the following family of distributions over M × H, ρ (Q, Q(S i , P )), in which we first sample P from Q and then sample h from Q = Q(S i , P ).

The KLD term is, DISPLAYFORM0 Plugging in to (17) we obtain that for any δ i > 0 15 Recall that Q(Si, P ) is the posterior distribution which is the output of the learning algorithm Q() which uses the data Si and the prior P .where we define, ε(K, ρ, π, δ) = 1 K D KL (ρ||π) + log 2 √ K δ , and, er ρ, X DISPLAYFORM1 Using the above theorem we get an alternative intra-task bound to (19), DISPLAYFORM2 2ε i + 2ε i er (Q(S i , P ), S i ), ∀Q ≥ 1 − δ i , where, DISPLAYFORM3 While the classical bound of Theorem 1 converges at a rate of about 1/ √ m (as in basic VC-like bounds), the bound of Theorem 4 converges even faster (at a rate if
...
In both networks we use ELU BID6 (with α = 1) as an activation function.
