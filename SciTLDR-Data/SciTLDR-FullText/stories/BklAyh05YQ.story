...
In our setting, we assume each class c (the arms of the bandit) has associated with it a vector z c and that the probability of a class label is modeled by: DISPLAYFORM0 , where σ is the logistic function and f θ is a feature function parameterized by θ.

In our case, f θ defines a neural network, while z c can be seen as the last layer of the network 2 .Our goal is to get low regret, R = T i r * i − T i r i , where r * i is the optimal reward at step i. The key to getting low regret is employing a policy for balancing exploration and exploitation.
...
If we hold f θ fixed, then we can easily model this distribution by doing an online Bayesian regression on the outputs of f θ , which gives us closed form formulas for updating the posterior over the last layer (specifically, its mean and covariance) given a single datapoint.3 When f θ is a neural network, then this is an instance of a BLL method.
...
Previous work has found them to be a pragmatic method for obtaining approximate uncertainty estimates for exploration BID13 BID17 BID16 BID2 and as proxies for Gaussian processes in both Bayesian Optimization problems (Snoek et al., 2015) and general regression tasks BID11 .If f θ is fixed, then z c can be updated efficiently in an online manner.

An unanswered question still remains however: how does one actually update and learn f θ ?

If you don't care about achieving low regret (ie you only care about offline performance), then the answer is easy; just gather your data, train f θ offline, possibly with off-policy learning methods (Strehl et al., 2010; BID6 , and learn the Bayesian regression post-hoc.

Of course, if you are concerned about online performance (regret) then this is not a viable option.

A training method for f θ must take care of two things: when do we update the feature functions and what do we update them with?
...
Additionally, assume a zero mean Gaussian prior over the last layer, p(z|θ) that is constant with respect to θ.

Recall that during online training we fix the parameters θ = θ t−1 , and model the distribution Q = p(z c |D t , D t−1 , θ t−1 ).

We want a value of θ such that both our data and the last layer values drawn from Q are likely.

Thus our objective is to maximize: DISPLAYFORM0 We can write the marginal likelihood as a convolution between a logistic function and a Gaussian (based on our assumption of zero mean Gaussian prior p(z|θ)), which can be approximated in closed form as per MacKay FORMULA1 : DISPLAYFORM1 Where µ is the mean of p(z|θ).

Since we have a zero mean prior, the above term evaluates to σ(0) whose value is a constant 1 2 .
...
For the first expectation term, we can use a second order Taylor expansion around E z∼Q [log p(D t |z, θ)] to get a closed form approximation 4 : DISPLAYFORM3 This approximation was used in Teh et al. (2007) and shown to work well empirically.

The expectations in equation FORMULA4 The KL term in equation FORMULA3 can also be approximated locally with a second Taylor expansion around the current value of θ = θ t−1 .

Let K(θ) = KL(Q||p(z|D t−1 , θ)).

Then, the second order Taylor expansion around θ = θ t−1 is: DISPLAYFORM4 Utilizing properties of KL divergence, as well as equation FORMULA2 , it can be derived 5 that K (θ t−1 ) will evaluate to 0, and K (θ t−1 ) will evaluate to βF P , where β = DISPLAYFORM5 and F P is the Fisher Information Matrix of P = p(z|D t−1 , θ t−1 ).

Getting rid of constants, we can write the local KL approximation (when θ is close to θ t−1 ) as: DISPLAYFORM6 The term β defines the ratio between the expected data likelihood given the last layer z distributed as z ∼ p(z|D t−1 , θ t−1 ) and the expected data likelihood given the last layer is distributed under the prior p(z|θ).

This indicates that the better our previous values of θ t−1 and z explain the data, the more we should regularize when incorporating new data (i.e raising the value of β).

In practice, these values may be computed or approximated, however for efficiency, we treat β as a hyperparameter, and linearly increase it throughout the learning process.
...
The algorithm alternates between two stages:Online Phase: As input, take in a set of data D t−1 , the posteriors (one for each arm) of the last layer conditioned on previous data p(z|D t−1 ) as well as a fixed value of f θ .
...
After T rounds, the learner outputs the updated posteriors over z, and the data collected this phase, D t .Offline/Retraining Phase: As input, take in D t , D t−1 , and the posteriors over z. Retrain f θ using method described in Section 3.

Set D t−1 = D t ∪D t−1 .

Recompute the posteriors over z conditioned on D t−1 using the new value of f θ .

Output D t−1 , f θ , and the updated posteriors p(z|D t−1 ).The marginalization method described in Section 3 is one type of retraining method.
...
The second expectation is equal to the negative cross entropy between Q = p(z|D t , D t−1 , θ t−1 ) and P θ = p(z|D t−1 , θ).

Using the fact that KL(Q||P θ ) = CE(Q||P θ ) − H(Q), we can replace the negative cross entropy with −KL(Q||P θ ) − H(Q), where H(Q) is the entropy of Q which is constant and can be ignored, yielding equation (3).We next derive the KL term.

As mentioned, we approximate the KL term locally around θ t−1 with a second order Taylor expansion.

Again, let K(θ) = KL(p(z|D t , D t−1 , θ t−1 )||p(z|D t−1 , θ)).

The second order Taylor expansion around θ = θ t−1 is: DISPLAYFORM1 We can rewrite K (θ) with respect to θ as: DISPLAYFORM2 This can also be done for K .

Let ∇ i indicate either the gradient (i = 1) or the Hessian (i = 2).

Then we can rewrite the above expectation with respect to the distribution P = p(z|D t−1 , θ t−1 ) instead: DISPLAYFORM3 Using the fact that p(z|D t , D t−1 , θ t−1 ) = p(D t |θ t−1 ) −1 p(D t |z, θ t−1 )p(z|D t−1 , θ t−1 ), we can rewrite the above as: DISPLAYFORM4 the UCB is straightforward; if Σ is the covariance of our posterior over z, and x is the context, then with probability at least 1 − δ, the term (1 + ln(2/δ)/2) f θ (x) T Σf θ (x) is an upper confidence bound.

The term α = (1 + ln(2/δ)/2) is treated as a hyperparameter.
