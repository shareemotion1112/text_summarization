...
We consider the discounted RL setting, where we seek to optimize a policy π for a Markov Decision Process described by states s, actions a, initial state distribution ρ env 0 (s 0 ), transition probabilities P env (s t+1 |s t , a t ), reward function r(s t , a t ), and discount factor γ ∈ (0, 1).

In deep RL, the policy π θ (a t |s t ), which specifies the probability that the agent takes action a t in state s t at time t, is described by a neural network with parameters θ.

We consider problems where both the states s and actions a may be discrete or continuous.

Two functions play a central role in RL: the state-value function V π (s t ) = E at,st+1,at+1,... ∞ k=0 γ k r(s t+k , a t+k ) and the state-action value function Q π (s t , a t ) = E st+1,at+1,... ∞ k=0 γ k r(s t+k , a t+k ) = r(s t , a t ) + γE st+1 V π (s t+1 ) , where s 0 ∼ ρ env 0 (s 0 ), a t ∼ π(a t |s t ), and s t+1 ∼ P env (s t+1 |s t , a t ).

In the usual formulation of the RL problem, the goal is to find a policy π that maximizes the expected return given by J(π) = E s0,a0,s1,a1,... ∞ t=0 γ t r(s t , a t ) .
...
(b) Schematic of the agents, with the policy (θ) and value (φ) networks sharing most of their parameters through a shared input encoder and LSTM [or Transformer-XL (TrXL) for single Atari levels].
...
In contrast, V-MPO is primarily (though not exclusively) designed to be an on-policy algorithm in which the E-step constructs a joint distribution ψ(s, a), and in the absence of a learned Q-function only one action per state is used.
...
In general, policy iteration uses the fact that the true state-value function V π corresponding to policy π can be used to obtain an improved policy π .

Thus we can 1.

Generate trajectories τ from an old policy π θold (a|s) whose parameters θ old are fixed.
...
Evaluate the policy π θold (a|s) by learning the value function V π θ old (s) from empirical returns and estimating the corresponding advantages A π θ old (s, a) for the actions that were taken (Section 4.1).

3.

Based on A π θ old (s, a), estimate an improved policy π θ (a|s) which we call the "online" policy to distinguish it from the fixed target network (Section 4.2).
...
In the present setting, policy evaluation means learning an approximate state-value function V π (s) given a policy π(a|s), which we keep fixed for T target learning steps (i.e., batches of trajectories).
...
Thus in this section π refers to π θold , while the value function update is performed on the current φ, which may share parameters with the current θ.

We fit a parametric value function V π φ (s) with parameters φ by minimizing the squared loss
...
In this section we show how, given the advantage function A π θ old (s, a) for the state-action distribution p θold (s, a) = π θold (a|s)p(s) induced by the old policy π θold (a|s), we can estimate an improved policy π θ (a|s).
...
Then we would like to find the mode of the posterior distribution over parameters θ conditioned on this event, i.e., we seek the maximum a posteriori (MAP) estimate

where we have written p(I = 1|θ) as p θ (I = 1) to emphasize the parametric nature of the dependence on θ.

We use the well-known identity log p(

is the Kullback-Leibler divergence between ψ(Z) and p(Z|X) with respect to Z, and the first term is a lower bound because the KL divergence is always non-negative.

Then considering s, a as latent variables,

Policy improvement in V-MPO consists of the following two steps which have direct correspondences to the expectation maximization (EM) algorithm (Neal & Hinton, 1998) : In the expectation (E) step, we choose the variational distribution ψ(s, a) such that the lower bound on log p θ (I = 1) is as tight as possible, by minimizing the KL term.

In the maximization (M) step we then find parameters θ that maximize the corresponding lower bound, together with the prior term in Eq. 2.

In the E-step, our goal is to choose the variational distribution ψ(s, a) such that the lower bound on log p θ (I = 1) is as tight as possible, which is the case when the KL term in Eq. 3 is zero.

Given the old parameters θ old , this simply leads to ψ(s, a) = p θold (s, a|I = 1), or

Intuitively, this solution weights the probability of each state-action pair with its relative improvement probability p θold (I = 1|s, a).

We now choose a distribution p θold (I = 1|s, a) that leads to our desired outcome.

As we prefer actions that lead to a higher advantage in each state, we suppose that this probability is given by

for some temperature η > 0, from which we obtain the equation on the right in Eq. 12.

This probability depends on the old parameters θ old and not on the new parameters θ.

Meanwhile, the value of η allows us to control the diversity of actions that contribute to the weighting, but at the moment is arbitrary.

It turns out, however, that we can tune η as part of the optimization, which is desirable since the optimal value of η changes across iterations.

The convex loss that achieves this, Eq. 13, is derived in Appendix A by minimizing the KL term in Eq. 3 subject to a hard constraint on ψ(s, a).
...
In the E-step we found the nonparametric variational state-action distribution ψ(s, a), Eq. 4, that gives the tightest lower bound to p θ (I = 1) in Eq. 3.

In the M-step we maximize this lower bound together with the prior term log p(θ) with respect to the parameters θ, which effectively leads to a constrained weighted maximum likelihood problem.
...
Note, however, that so far we have worked with the joint state-action distribution ψ(s, a) while we are in fact optimizing for the policy, which is the conditional distribution π θ (a|s).

Writing p θ (s, a) = π θ (a|s)p(s) since only the policy is parametrized by θ and dropping terms that are not parametrized by θ, the first term of Eq. 6 is seen to be the weighted maximum likelihood policy loss
...
As in the original MPO algorithm, a useful prior is to keep the new policy π θ (a|s) close to the old policy π θold (a|s): log p(θ) ≈ −αE s∼p(s) D KL π θold (a|s) π θ (a|s) .

While intuitive, we motivate this more formally in Appendix B.

It is again more convenient to specify a bound on the KL divergence instead of tuning α directly, so we solve the constrained optimization problem
...
which we can optimize by following a coordinate-descent strategy, alternating between the optimization over θ and α.

Since η and α are Lagrange multipliers that must be positive, after each gradient update we project the resulting η and α to a small positive value which we choose to be η min = α min = 10 −8 throughout the results presented below.
...
Each trajectory consists of an unroll of length n of the form τ = (s t , a t , r t+1 ), . . .
...
where φ are the parameters of the value network, θ the parameters of the policy network, and η and α are Lagrange multipliers.
...
We note, however, that the value network parameters φ are considered fixed for the policy improvement loss, and gradients are not propagated.

The policy evaluation loss for the value function, L V (φ), is the standard regression to n-step returns and is given by Eq. 1 above.

The policy improvement loss L V-MPO (θ, η, α) is given by

Here the policy loss is the weighted maximum likelihood loss

where the advantages A target (s, a) for the target network policy π θtarget (a|s) are estimated according to the standard method described above.
...
The η, or "temperature", loss is

We perform the alternating optimization over θ and α while keeping a single loss function by alternately applying a "stop-gradient" to the Lagrange multiplier and KL term.
...
given Batch size B, unroll length n, T target , KL bounds η , α . initialize Network parameters θ online , φ online , Lagrange multipliers η, α.

repeat θ target ← θ online for i = 1, . . .

, T target do Use policy π θtarget to act in the environment and collect B trajectories τ of length n. Update θ online , φ online , η, α using Adam to minimize the total loss in Eq. 10.

η ← max(η, η min ) α ← max(α, α min ) end for until Fixed number of steps.
...
where λ = η log p θold (I = 1) after multiplying through by η, which up to this point in the derivation is given.

We wish to automatically tune η so as to enforce a bound η on the KL term D KL ψ(s, a) p θold (s, a) multiplying it in Eq. 16, in which case the temperature optimization can also be viewed as a nonparametric trust region for the variational distribution with respect to the old distribution.

We therefore consider the constrained optimization problem

s.t.

s,a ψ(s, a) log ψ(s, a) p θold (s, a) < η and s,a ψ(s, a) = 1.
...
However, the value of η can now be found by optimizing the corresponding dual function.

Plugging Eq. 21 into the unconstrained objective in Eq. 19 gives rise to the η-dependent term

Replacing the expectation with samples from p θold (s, a) in the batch of trajectories D leads to the loss in Eq. 13.

Here we give a somewhat more formal motivation for the prior log p(θ).

Consider a normal prior N (θ; µ, Σ) with mean µ and covariance Σ. We choose Σ −1 = αF (θ old ) where α is a scaling parameter and F (θ old ) is the Fisher information for π θ (a|s) evaluated at θ = θ old .

Then

T F (θ old )(θ − θ old ) + {term independent of θ}, where the first term is precisely the second-order approximation to the KL divergence D KL (θ old θ).

We now follow TRPO (Schulman et al., 2015) in heuristically approximating this as the state-averaged expression, E s∼p(s) D KL π θold (a|s) π θ (a|s) .
...
This uses the fact that the KL divergence between two d-dimensional multivariate normal distributions with means µ 1 , µ 2 and covariances Σ 1 , Σ 2 can be written as

where | · | is the matrix determinant.

Since the first distribution and hence Σ 1 in the KL divergence of Eq. 9 depends on the old target network parameters, we see that we can separate the overall KL divergence into a mean component and a covariance component:

With the replacement D KL π θold π θ → D C KL π θold π θ for C = µ, Σ and corresponding α → α µ , α Σ , we obtain the total loss

where L π (θ) and L η (η) are the same as before.

Note, however, that unlike in Abdolmaleki et al. (2018a) we do not decouple the policy loss.

We generally set Σ to be much smaller than µ (see Table 7 ).
...
(29) Multiplying by λ since it can be treated as a constant up to this point, we then see that this corresponds exactly to the (Lagrangian form) of the problem
...
We can compensate for this by multiplying the exponentiated advantages by importance weights ρ(s, a):

where θ D are the parameters of the behavior policy that generated D and which may be different from θ target .

The clipped importance weights ρ(s, a) are given by
...
All settings of the PBT experiment were the same as without except the learning rates were also sampled log-uniformly from [8 × 10 −5 , 3 × 10 −4 ) and η from [0.05, 0.5).

Along with α sampled loguniformly from [0.001, 0.01) as in the original experiment, hyperparameters were evolved via copy and mutation operators roughly once every 4 × 10 8 environment frames.
