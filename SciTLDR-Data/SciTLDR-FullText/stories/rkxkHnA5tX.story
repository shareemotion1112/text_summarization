...
A finite-horizon Markov Decision Process (MDP) can be defined as M = (S, A, P, R, γ, p 0 , T ), where S and A denotes state and action spaces, P : S × A × S → R + denotes the state transition probability, R : S × A → R is a reward function, p 0 an initial state distribution, γ is the reward discount factor, and T is the horizon of the MDP.

Let τ = (s 0 , a 0 , ..., s T , a T ) be a trajectory of state and action pairs, the associated reward can be written as R(τ ) = T t=0 γ t R(s t , a t ).

Reinforcement learning seeks a policy π θ (a|s) parameterized by θ that maximizes the expected total return: DISPLAYFORM0 In imitation learning regime, demonstrated expert behaviors are given while the reward function is missing, and the agent acquire desired skills learning from these demonstrations.
...
Denote f θ as the policy model mapping observations to actions parameterized by θ, the desired adaptive meta-parameters.

The inner objective in the MAMLbased meta-imitation-learning is an imitation objective, where θ is updated to θ i after learning from demonstration v i by gradient descent with learning rate α, which can be written as: DISPLAYFORM0 Here L vi denotes the imitation loss imitating the sampled demonstration v i .

And hence the meta objective and the corresponding meta-imitation-update can be derived as (with meta learning rate β): DISPLAYFORM1
...
Specifically, each inner update from θ to θ i is computed with reward − log(D i (s, a)) via TRPO (Schulman et al., 2015) update rule.
...
However, R task would bias the training of the adaptive parameters θ, that θ should be optimized for quick adaptation to any given demonstration rather than succeeding the task.

Therefore, we drop the R task term and denote our reward definition as R(s t , a t ) = R IL , 0 ≤ t ≤ T .In order to adapt to unseen demonstrations, we train a meta-discriminator D meta along with training the adaptive policy parameters θ.

Similar to Eq.2, D meta is meta-updated as: DISPLAYFORM0 where τ i denotes the trajectories sampled from f θ i (after learning from demonstration v i ), v i is the trajectory of demonstration as one episode of state-action pairs (expert trajectory in GAIL setting).
...
In behavioral cloning, the inner update in Eq.1 is simply a supervised training, where L vi = 1 T t f θ (s i,t ) −â i,t 2 using a mean squared error withâ i,t denoting the expert action from demonstration v i at time-step t.

In order to overcome the potential challenge of learning from an unbalanced demonstration distribution, we augment the inner update in meta-training phase with a regularization loss: maximization of the mutual information I(v i , ∆π vi ) between the induced behavioral differences and the demonstration being imitated.

∆π vi here denotes the differences of behaviors induced after an inner update learns from a particular demonstration v i , which we estimate using a sample-based method.

Maximizing I(v i , ∆π vi ) can be approximated by optimizing the following variational lower bound: DISPLAYFORM0 where Q denotes the posterior network jointly trained with the adaptive policy parameters.

Since v i is drawn from a fixed given demonstration distribution (a set), H(v i ) can be treated as a constant, we thus only need to consider optimizing Q. Q can be trained by estimating ∆π vi through sampling trajectories before and after the inner imitation update, denoted as τ old and τ new .
...
We sample two sets of trajectories for both τ old and τ new as one used for training Q, and the other (test set) for estimating E[logQ(v i |∆π vi )] to be used as the regularization reward.

We then augment the original reward function in GAIL with R M I = −E[logQ(v i |∆π vi )] as follows: DISPLAYFORM1 However, this regularization term should be applied to the parameters before the imitation update, since it is interpreted as the probability of such behavioral differences induced by a particular demonstration v i .

Denote the parameters for inner imitation update from demonstration v i as θ i , our inner objective thus becomes: DISPLAYFORM2 and hence the update where we revert θ i back to θ i with the additional regularization term can be written as: DISPLAYFORM3 Which implies we can compute the gradients with respect to the old trajectories but update on the new parameters θ i .

In practice, we implement an additional value network dedicated for this additional regularization reward.

As for behavioral cloning, we update the policy parameters θ utilizing policy gradients computed from this R M I , while other parts remained as standard supervised training.
...
K is essentially a scoring function which takes as input trajectories generated from a policy π and outputs a real-valued number, ie.

score π = K(τ π ) ∈ R. Denote π θ as the meta-trained policy, and π θi is the policy adapted to demonstration v i , the demonstration suitability assessment score is then computed as: DISPLAYFORM0 The most suitable demonstration v iopt|e j is then selected by: DISPLAYFORM1
...
During meta-update, the meta-prior θ is updated by a weighted sum of gradients computed when adapting to each demonstration DISPLAYFORM0 After each meta-testing, c i is adjusted based on the difference between current assessment score S vi | ej and the baseline score obtained from the meta-parameters S θ | ej .
...
1 T i s i,t −ŝ i,t + a i,t −â i,t , where (ŝ i ,â i ) denotes the trajectories from demonstration v i , and (s i , a i ) denotes the generated trajectories from parameters adapted to v i .
...
In order to verify the efficiency as compared to demonstration judging via training from scratch, We set a limited quota be the total number of iterations required for meta-training the adaptive parameters θ plus the number of gradient updates during adaptation to each demonstration, which sums up to around 6,000 and 7,500 for our martial arts and handstand environments respectively.
...
The learning rate in the inner objective Eq.1 is set to be α = 1.0 with KL-divergence constraints introduced by TRPO (Schulman et al., 2015) set to 0.15.

α is halved after the first iteration during adaptation for free throw and penalty save environments.

For handstand and martial arts environments, we enlarge α by twice for the first iteration to 2.0, and then use 0.5 like the two aforementioned environments.
...
Algorithm 1 Info Meta Imitation Learning (Blued lines are GAIL ver.)Require: V = {v 1 , v 2 , ..., v n }: A set of demonstrations Require: E = {e 1 , e 2 , ..., e k }: An environment with k goals (k = 1 for 3 of our environments) Require: α, β: step size hyperparameters 1: randomly initialize θ, Q, D meta 2: Sample a batch of Demonstrations v i ∈ V 3: while not done do

for all v i do

Sample a batch of environments e j ∈ E 6:for all e j do 7:Sample K trajectories τ = {(s 1 , a 1 , ..., s T )} using f θ for updating D meta to D meta,i 8:Evaluate ∇L ILi (f θ ) using τ and v i , and update DISPLAYFORM0 Update D meta to D meta,i with the gradient: DISPLAYFORM1 Sample K trajectories τ ij = {(s 1 , a 1 , ..., s T )} using f θ i after imitating v i under e j

Update posterior Q with τ , τ ij , and encoded v i

Sample new trajectories using f θ and f θ i as τ new and τ ij,new respectively 13:Compute R M Ii with Q, using newly sampled τ new and τ ij,new as FORMULA5 14: DISPLAYFORM0 ) with TRPO as FORMULA7 15:Sample K trajectories τ ij = {(s 1 , a 1 , ..., s T )} using f θ i Meta Update the θ using all the τ ij with TRPO update rules as FORMULA2 19:Meta Update the D meta using all the gradients collected from D meta,i as (3) 20: end while C MORE RESULTS
...
Require: V = {v 1 , v 2 , ..., v n }: A set of demonstrations Require: E = {e 1 , e 2 , ..., e k }: An environment with k goals (k = 1 for 3 of our environments) Require: α, β: step size hyperparameters Require: max_iter: maximum iterations for the meta-testing Require: θ: the meta-trained parameters Require: Pre-defined Task Heuristics K Require: Meta-Trained Discriminator D meta 1: for all e j ∈ E do 2:for all v i ∈ V do 3:Initialize θ ij as meta parameters θ 4: DISPLAYFORM0 for iter = 0 to max_iter do 6:Sample K trajectories τ = {(s 1 , a 1 , ..., s T )} using f θij

Update D i with the gradient: DISPLAYFORM0 Evaluate ∇L IL (f θij ) using τ and v i , and update θ ij ← θ ij − α∇L IL (f θij ) with TRPO 10:end for

Evaluate θ ij for M random seeds 12:Compute DSA score DISPLAYFORM0 end for 14:Record the index i opt for the most suitable demonstration under e j with i opt = arg max i S vi | ej , ∀v i ∈ V 15: end for

Require: V = {v 1 , v 2 , ..., v n }: a set of demonstrations Require: N : predefined iterations Require: β: constant for multiplying and dividing weights of demonstrations Require: : threshold for selecting final subset of demonstrations Require: W = {w 1 , w 2 , ..., w n }: weights for meta-learning each demonstration.

1: Initialize all w i to 1 2: while not done do DISPLAYFORM0 Update θ with Imitation Learning using current set V for N iterations, applying weights W for the demonstrations during meta-update.

Get heuristic score for meta-trained parameters θ, store in S 5: DISPLAYFORM0 if ∆ K,ij > 0 then 8: DISPLAYFORM1 else 10: In FIG6 we show that our framework can also work on selecting suitable demonstrations given an unseen demonstration set.
