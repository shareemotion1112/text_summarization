...
MS-COCO, PASCAL VOC, ImageNet (Deng et al., 2009) Figure 2 : Faster R-CNN head architecture for our approach: We extended the existing Faster R-CNN head to predict the parameters of multivariate mixture of Gaussian µ, φ and Σ Cityscapes (Cordts et al., 2016) fall into the first category.
...
Formally, we predict the distribution p θ (x|I) given the feature maps I of a region of interest (RoI).

The distribution is parameterized by θ, which is a neural network (e.g., Faster R-CNN head, Figure 2 ).

The distribution has

T , which is the most probable bounding box coordinates relative to the RoI, estimated by the component:

Σ is the covariance matrix, which is a symmetric semi-positive definite matrix in general.

To be able to compute the inverse Σ −1 , we constrain the covariance matrix to be a symmetric positive definite matrix.

In this case, the precision matrix Σ −1 is also a symmetric positive definite matrix.

During training, the model estimates the precision matrix Σ −1 instead of the covariance matrix Σ, so that we do not need to compute the inverse every time during training which we also find more stable in our experiments.

To ensure the properties of the precision matrix Σ −1 , we parameterize it using the Cholesky decomposition:
...
We parameterize the mixture weights φ i using Softmax, so that they range from 0 to 1 and sum to 1:

z i , u ii and µ i are outputs produced by a fully-connected layer on top of the final fully-connected layer fc7 on the Faster R-CNN head.
...
During training, we estimate the parameters θ with maximum likelihood estimation on a given dataset {I , µ * | = 1, 2, ..., N }, where µ * represents the ground truth coordinates for RoI feature maps I and N is the number of observations:
...
Note that we use different parameters θ for different classes in practice.
...
Notice that the covariance matrix Σ i is not involved in inference.
...
where 2 ln 2π is a constant which can be ignored during training.
...
When the bounding box regression is inaccurate (large (µ * j − µ j ) 2 /2), the variance 1/(U ) 2 jj tends to be larger.
...
We initialize the weights of µ i , z i and u ii layers (Figure 2 ) using random Gaussian initialization with standard deviations 0.0001 and biases 0, −1 and 0 respectively.

So that at the start of training, bounding box coordinate µ i is at an unbiased position, U i is an identity matrix and φ i treats each mixture component equally.
...
Shown in Table 5 , Gaussian (Eq. 9) and multivariate Gaussian (Eq. 7) neither slow down the inference nor increase the number of parameters, since we can discard the covariance Σ at inference time (Section 3.1).
...
Note that we measure the inference latency on MS-COCO where there are 80 classes, such that the number of parameters for µ is 1024 × 80 × K (1024 is the number of output channels of fc7, Figure 2 ).

On CrowdHuman where there is only one class (human), the number of parameters for µ is only 1024 × K, which will consume even fewer computation and memory resources.
