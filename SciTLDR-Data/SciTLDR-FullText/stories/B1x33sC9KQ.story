...
Without loss of generality, we assume a prepossessing step has been made so that the average value in the tensor zero i.e., X = µ = 0 (we do not lose generality since we can always subtract and add this mean).

Assuming bit-width M , we would like to quantize the values in the tensors uniformally to 2 M discrete values.

Commonly (e.g., in GEMMLOWP), integer tensors are uniformly quantized in the range [−α, α], where α is determined by the tensor maximal absolute value.

In the following we show that the this choice of α is suboptimal, and suggest a model where the tensor values are clipped to reduce quantization noise.

For any x ∈ R, we define the clipping function clip(x, α) as follows DISPLAYFORM0 Denoting by α the clipping value, the range [α, −α] is partitioned to 2 M equal quantization regions.

Hence, the quantization step ∆ between two adjacent quantized values is established as follows: DISPLAYFORM1 Our model assumes values are rounded to the midpoint of the region (bin) i.e., for every index DISPLAYFORM2 are rounded to the midpoint q i = −α + (2i + 1) ∆ 2 , as illustrated in Figure 1 .
...
The first and last terms quantify the contribution of clip(x, α) to the expected mean-square-error.

Note that for symmetrical distributions around zero (e.g., Gaussian N (0, σ 2 ) or Laplace(0, b)) these two terms are equal and their sum can therefore be evaluated by multiplying any of the terms by 2.The second term corresponds to the expected mean-square-error when the range [−α, α] is quantized uniformly to 2 M discrete levels.
...
For sufficient resolution and a smooth density function, we found that the uniform distribution in the range [−α, α] enables much simpler analysis with little effect on the accuracy.
...
By substituting the uniform density function f (x) = 1 2αinto Equation 4, the following simpler rounding error can be computed: DISPLAYFORM5 By substituting Equation 5 into Equation 3, and using the symmetrical argument mentioned above, Equation 3 can be simplified for symmetrical distributions as follows: DISPLAYFORM6 In the following we provide a closed form solution for the case where the density probability distribution function f (x) is either Gaussian N (0, σ 2 ) or Laplace(0, b).

In the following we develop an expression based on Equation 6 for the Laplace case.

Since we assume that µ = 0, we have the following Laplace density function f (x) = .

In order to derive a closed form solution for Equation 6, we need to evaluate DISPLAYFORM0 Let Ψ(x) represent the expression below: DISPLAYFORM1 By taking the derivative of Ψ(x) with respect to x, it is easy to see that Ψ(x) is the correct antiderivative of the integrand in equation 7.

Hence, DISPLAYFORM2 We can finally state Equation 6 for the laplace case as follows.

DISPLAYFORM3 Finally, in order to find the α that give the minimum MSE, the corresponding derivative with respect to α is set equal to zero as follows: DISPLAYFORM4

We now turn to evaluate Equation 6 for the Gaussian case.

Given a Gaussian random variable X ∼ N (0, σ 2 ), we define Ψ(x) to represent the expression below: DISPLAYFORM0 As in subsection 4.1, one can observe that by taking the derivative of Ψ(x) with respect to x, it is easy to show that Ψ(x) is the correct antiderivative of Equation 7 for the case where f represents the Gaussian density function i.e., f (x) = DISPLAYFORM1 2σ 2 .

Next, we use Ψ(x) on the range [α, ∞] to evaluate Equation 7 for the Gaussian case as follows: DISPLAYFORM2 Equation FORMULA6 can thus be written for the case of Gaussian distribution as follows: DISPLAYFORM3 In order to find the optimal clipping values for which mean-square-error is minimized, we need to differentiate E[(X − Q(X)) 2 ] with respect to α and set the derivative equal to zero as follows.
...
Rather, as N (0, σ 2 ) = σ · N (0, 1) and Laplace(0, b) = b · Laplace(0, 1), it is sufficient to store the optimal clipping values for N (0, 1) and Laplace(0, 1) and scale these values by σ and b, which are estimated from the tensor values.
...
A PIECE-WISE LINEAR APPROXIMATIONHere we provide a more accurate analysis related to the qunatization noise (i.e., the second term in Equation 3), measured as the expected mean-square-error when the range [−α, α] is quantized uniformly to 2 M discrete levels.
...
We turn to calculate the linear equation for each line segment of the piece-wise linear function g, falling in the range [−α + i · ∆, −α + (i + 1) · ∆].
