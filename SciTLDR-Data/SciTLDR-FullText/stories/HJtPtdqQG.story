...
This version of Bayesian Optimization in Algorithm 1 is a one-step sim- DISPLAYFORM0 3: Query objective function to obtain y 0 4: return η 0 plification of the one described in BID11 .

We next describe the acquisition function α(·; ·) and other statistical details of Algorithm 1.

A Gaussian Process is a nonparamteric statistical model that is characterized by µ 0 , σ 0 , K(·, ·) -initial mean, initial variance and kernel function respectively.

Consider the sequence of points, η 1:n (inputs) and y 1:n (noisy observations).

We introduce auxillary variables f 1:n , such that f 1:n |η 1:n ∼ N (m, K) where DISPLAYFORM0 Then we have that y 1:n |f 1:n , σ 2 0 ∼ N (f 1:n , σ 2 0 I).

Given this GP, we "update" µ n (·), σ n (·) given some observation {(η i , y i )} n i=1 , by Algorithm 2 DISPLAYFORM1
...
Consider the improvement function I(η, v, θ) = (v − τ )I(v > τ ); this captures the amount of improvement over a given threshold τ .

Then expected improvement is just E[I(η, v, θ)] assuming that v is normally distributed with some mean and variance parametrized by η.

Then we have DISPLAYFORM0 Typically τ = min n y n , i.e., the minimum of noisy observations.
...
Let (η 1 , t 1 ), (η 2 , t 2 ) be the new hyperparameters where t i is the epoch number, then our kernel is of the form DISPLAYFORM0 Now, the K 1 (·, ·) kernel can be one of RBF kernel or Matern-x kernels over the true hyperparameters, as in standard literature, we will describe K 2 (·, ·), now referred to as the time kernel, in the following section.
...
The kernel K 2 (·, ·) is of the form DISPLAYFORM0 where the last equality is obtained by choosing ψ(λ) = β α Γ(α) λ α−1 exp (−λβ).

This construction is motivated from the observation that SGD decays as O(1/N 1/2 ) in N iterations, which in our case reduced to α = 1, β = 1/2, and from work in BID14 .
...
We only use data parallelism for gradient computation in SGD across different GPUs, i.e., if there are k GPUs and a batch size of α then each GPU calculates the gradient on a batch size of α/k and this is summed across GPUs before weight update.
...
it must be true that η k → 0 as k → ∞, where η k is the learning rate at iteration k. In FIG0 we observe that Hyperdyn tuning automatically tends to 0 as the learning process progresses.
...
We use Hyperdyn to optimize the hyperparameters of Adam, i.e. learning rates β 1 , β 2 in BID5 .
...
As our HPO algorithm proceeds, we observe convergence of learning rates DISPLAYFORM0 Figure 3: Hyperdyn tuned Adam as β 1 → 0 and β 2 is around 1, i.e. the momentum coefficient β 1 becomes very small (Fig. 3(b) ).
