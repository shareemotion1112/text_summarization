...
The second step is then to consider the logistic loss of the normalized networkf with f = ρf , with ρ = ρ 1 ...ρ K where ρ i is the Frobenius norm of the weight matrix of layer i.
...
We define a deep network with K layers with the usual elementwise scalar activation functions σ(z) : R → R as the set of functions
...
where W K = ρ kWK and ||W K || = 1 and ρ 1 · · · ρ K = ρ .
...
Furthermore, during gradient descent on separable data -most data sets are separable by overparametrized deep networks -it can be shown (see Poggio et al. (2018) ) that the ρ factor continue to increase with time towards infinity, driving an exponential type loss to zero without affecting the classification performance, which only depends on the sign of y n f (x n )∀n.
...
A typical generalization bound that holds with probability at least (1 − δ), ∀f ∈ F has the form Bousquet et al. (2003) :
...
This is typically when W > N and corresponds to the limit for λ = 0 of regularization, that is the pseudoinverse.
...
The cross-entropy loss is then defined simply aŝ

It's very easy to see that this reduces to the logistic loss in the binary case.

Classification now depends only on the margin ηn = fy n (W ; x) − max j =yn {fj(W ; x)} -if ηn > 0 then the example is correctly classified.
...
One final property of note is that for separable data, the loss monotonically decreases with increasing ρ.

To see this, let us write αnj = fy n (W ; x) − fj(W ; x), which is a positive quantity in the separable case.

Additionally define gn = − log j =yn e −α nj = − log j =yn e −ρα nj , which is clearly a monotonic function of ρ if all αnj > 0.

We can now rewrite the Cross-entropy loss aŝ
...
Then the following bound holds for binary classification in terms of the so-called ψ-transform of the logistic loss :

where the ψ function for the logistic is similar to the ψ for the exponential loss which is ψ(x) = 1 − √ 1 − x 2 .

The key point here is that ψ −1 is monotonic: minimizing the logistic or cross-entropy loss implies minimizing the classification error.

Our arguments imply that among two unnormalized minimizers of the exponential loss that achieve the same given small empirical lossL = , the minimizer with higher product of the norms ρ1, · · · , ρK has the higher capacity and thus the highest expected loss L. Experiments support this claim, see Figure 13 .
...
where η is the margin, L binary (f ) is the expected classification error, Lsurr(f ) is the empirical loss of a surrogate loss such as the logistic or the exponential.

For a point x, the margin is η ∼ yρf (x).

Since RN (F) ∼ ρ, the margin bound says that the classification error is bounded by 1 η that is by the value off on the "support vectors" -that is on the xi, yi s.t arg minn ynf (xn).
