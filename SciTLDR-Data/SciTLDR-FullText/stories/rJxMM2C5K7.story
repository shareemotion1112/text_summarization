...
Given a real number x ∈ R, x is the nearest integer to x. For a random variable u, u ∼ U [a, b] if its probability distribution is uniform over interval [a, b] and u ∼ N (µ, σ 2 ) if it follows a Gaussian distribution with mean µ and variance σ 2 .
...
Adding this controlled perturbation can cause the statistical behavior of the quantization error to be more desirable BID14 ; BID6 ; BID5 .Let Q(·) be an M-level uniform quantizer with quantization step size of ∆, i.e., Q(v) = ∆ v/∆ where α is the nearest integer to α.
...
If 1) the quantizer does not overload, i.e., |x + u| ≤ M ∆ 2 for all input signals x and dither u, and 2) The characteristic function of the dither signal, defined as M u (jν) = E u e jνu , satisfies M u (j 2πl ∆ ) = 0 for all l = 0, then the quantization error e = x −x is uniform over [−∆/2, ∆/2] and it is independent of the signal x.
...
At each iteration of the training, each worker computes a stochastic gradient of the parameters g k , or the update in the parameters δW k , based on its own available data.
...
The average of all gradients or the updates (ḡ orδ W ) is then broadcasted back to all workers.
...
The dithered quantized SG is given byg DISPLAYFORM0 where the scale factor κ = g ∞ = max i |g i | maps the gradient into the range [−1, 1].

By Thm.

1, the scaled quantization noise e = (g − g)/κ will be independent from g and uniformly distributed over [−∆/2, ∆/2].
...
For the simplicity of analysis assume that the partitions are of equal length, n/K. Simple calculations reveal that DISPLAYFORM3 2 Usually, the SG is computed as g = 1 L l ∇wf (x l ; w) and for large enough L, due to the central limit DISPLAYFORM4 for an appropriate fixed covariance matrix Σ.The first term decreases logarithmically w.r.t.

the number of partitions.

On the other hand, each partition requires transmitting an additional scale factor (κ in (2), see Alg.
...
The p-th worker, first computes the stochastic gradient g p and then using the scale parameter κ p = g p ∞ , computes the quantization index q p (see Remark 1).

Hence, the DQSG is given by g p = κ p (∆.q p − u p ).
...
Then for sufficiently small > 0, after T steps of training with constant step size η t , where DISPLAYFORM9 , and η t = /( + 1.1σ 2 /P ),we have DISPLAYFORM10 Let T c be the training time without any quantization in the above setup.
...
, P : -Get a batch of training data and compute the stochastic gradients gp.-Generate a pseudo-random sequence up, uniformly distributed over [−∆/2, ∆/2] using seed sp.-Compute the quantization index: qp = t/∆ where t = gp/κp + up and κp = g ∞.-Update the seed number sp.-Send κp and qp (or the corresponding quantization bin).

Server :-Reproduce the pseudo-random sequence up using the seed number sp.-Reconstruct the gradient of the p-th worker asgp = κp (∆.qp − up).-Update the seed number sp.-Compute the average SG,¯ g = 1 P pg p, and broadcast it to the workers.
...
Let (Q 1 , Q 2 ) be a pair of nested quantizers with quantization step sizes ∆ 1 and ∆ 2 , respectively and 0 < α ≤ 1 be a shrinkage factor whose value to be determined later.

To quantize and transmit x, the worker first generates a random dither u ∼ U[−∆ 1 /2, ∆ 1 /2] and computes t = αx + u. Then it quantizes and encodes it as DISPLAYFORM0 i.e., it transmits the position of the fine quantization bin relative to the coarse one (shown by indexes −1, 0, 1 in FIG2 .
...
Assume α = 1, hence s = Q 1 (−3.9) − Q 2 (−3.9) = −4 − (−3) = −1 is the signal to be transmitted.
...
The stochastic gradient, computed by the p-th worker in a distributed training system, can be considered as a noisy estimate of the true gradient, i.e., g p = ∇ w L + ν p where ν p is a zero-mean noise.
...
The workers in P 2 use nested quantizer with step-sizes (∆ DISPLAYFORM3 2 ) and scale α p .
...
If the SG at a worker is modeled by g =¯ g + z, E z 2 i = σ 2 z , and the worker uses nested quantizer with parameters ∆ 1 , ∆ 2 and α, then with probability at least 1 − p, g i will be estimated correctly (i.e., g i and g i are in the same coarse quantization bin), where DISPLAYFORM4 DISPLAYFORM5 2α , then p = 0.

In this case, DISPLAYFORM6 Note that setting α = 1 or α = 1 − ∆ 2 1 /12σ 2 z results in the same quantization variance as dithered quantization with step-size ∆ 1 .
...
1, e = Q(g/κ + u) − (g/κ + u) is independent from g/κ and uniformly distributed over [−∆/2, ∆/2].

On the other hand,g = g + κe.

Hence, DISPLAYFORM1 (a) DISPLAYFORM2 = ∇L, where (a) is due to the fact that κ = g ∞ is independent of e and (b) because of unbiasedness of stochastic gradient and e having mean zero.

For the variance, DISPLAYFORM3 This is a direct result of (Bubeck, 2015, §6) .

Note that E g − ∇ w L 2 2 ≤ V + n∆ 2 12 E g 2 2 ≤ V (1 + n ∆ 2 /12) + nB ∆ 2 /12 = σ 2 and since there are P workers, the variance bound on g would be σ 2 /P .

Then after T iterations of (DQSGD) with step size η t = 1/( + 1/γ) for γ = DISPLAYFORM4 For < 0.2σ 2 /P L, set T = 2.5 R 2 σ 2 P 2 .

Then, it can be easily verified that for the given step-size, the results hold.

E PROOF OF THM.

6Let e = αg + u − Q 1 (αg + u) and r = s − u − α¯ g. Then, DISPLAYFORM5 Since¯ g i = g i + z i , it can be shown that r i − Q 2 (r i ) = αz i − e i − Q 2 (αz i − e i ).

i =¯ g i + α(αz i − e i ) − αQ 2 (αz i − e i ).

The correct decoding occurs when Q 2 (αz i − e i ) = 0.

Hence, the probability of correct recovery would be 1 − p where DISPLAYFORM0 In that case,ĝ i = g i − (αe i + (1 − α 2 )z i ).Since e i ∼ U[−∆ 1 /2, ∆ 1 /2] and z i are independent from each other and from g i , simple calculations show that DISPLAYFORM1
