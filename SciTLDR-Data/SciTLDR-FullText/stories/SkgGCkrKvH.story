...
• On the theory side, we are the first to show that CHOCO-SGD converges at rate O 1 / √ nT + n /(ρ 4 δ 2 T ) on non-convex smooth functions, where n denotes the number of nodes, T the number of iterations, ρ the spectral gap of the mixing matrix and δ the compression ratio.

The main term, O 1 / √ nT , matches with the centralized baselines with exact communication and shows a linear speedup in the number of workers n. Both ρ and δ only affect the asymptotically smaller second term.
...
We in particular focus on approaches based on gossip averaging (Kempe et al., 2003; Xiao & Boyd, 2004; Boyd et al., 2006) whose convergence rate typically depends on the spectral gap ρ ≥ 0 of the mixing matrix (Xiao & Boyd, 2004) .

Lian et al. (2017) combine SGD with gossip averaging and show convergence at the rate O 1 / √ nT + n /(ρ 2 T ) .
...
Recently, also proximal updates and variance reduction have been studied in combination with quantized updates Horváth et al., 2019) .

Decentralized Optimization with Quantization.

It has been observed that gossip averaging can diverge (or not converge to the correct solution) in the presence of quantization noise (Xiao et al., 2005; Carli et al., 2007; Nedić et al., 2008; Dimakis et al., 2010; Carli et al., 2010b; Yuan et al., 2012) .
...
, where δ > 0 measures the compression quality.
...
This will not only guarantee ρ > 0 but these weights can easily be computed in a local fashion on each node (Xiao & Boyd, 2004) .
...
In contrast to the quantization operators used in e.g. (Alistarh et al., 2017; Horváth et al., 2019) , compression operators defined as in (2) are not required to be unbiased and therefore supports a larger class of compression operators.
...
where c := ρ 2 δ 82 denotes the convergence rate of the underlying consensus averaging scheme of (Koloskova et al., 2019) .
...
Algorithm 2 CHOCO-SGD with Momentum input: Same as for Algorithm 1, additionally: weight decay factor λ, momentum factor β, local momentum memory v
...
For CHOCO-SGD and DeepSqueeze the consensus learning rate γ is also tuned.
...
For this, we will first derive a slightly more general statement: in Theorem A.2 we analyze CHOCO-SGD for arbitrary stepsizes η, and then derive Theorem 4.1 as a special case.
...
It converges at the rate c = ρ, where ρ is an eigengap of mixing matrix W , defined in Assumption 1.
...
Under Assumptions 1-2 the iterates of the Algorithm 3 with constant stepsize η satisfy
...
Verifying that r t ≤ η 2 4A c 2 satisfy recursion completes the proof as E X

Theorem A.2.

Under Assumptions 1-3 with constant stepsize η = n T +1 for T ≥ 64nL 2 , the averaged iterates
...
Substituting the convergence rate for exact averaging with W gives the rate O( 1 / √ nT + n /(T ρ 2 )), which recovers the rate of D-PSGD (Lian et al., 2017) .

CHOCO-SGD with the underlying CHOCO-GOSSIP averaging scheme converges at the rate

2 )).

The dependence on ρ (eigengap of the mixing matrix W ) is worse than in the exact case.
...
Using Lemma A.1 to bound the third term and using that η ≤

Rearranging terms and averaging over t

Substituting η = n T +1 and using that T ≥ 64nL 2 we get the statement of the theorem.
...
Under Assumptions 2, 1 with constant stepsize η and the consensus stepsize γ := ρ 2 δ 16ρ+ρ 2 +4β 2 +2ρβ 2 −8ρδ where β = I − W 2 ∈ [0, 2]
...
In contrast to Theorem A.2, this rate holds for any T , however the first term is worse than in Theorem A.2 because σ 2 is usually much smaller than G 2 .
...
Rearranging terms, taking α = 1 and averaging over t we are getting statement of the theorem

Corollary A.5 (Convergence of local weights x (t) i ).

Under Assumtions 2, 1, algorithm 1 with η = n T +1 converges at the speed
...
Using Theorem A.4 and Lemma A.1 and setting α = 1

where we set η = n T +1 .
...
The learning rate is tuned by finding the optimal learning rate per sampleη, where the learning rate (used locally) is determined by a linear scaling rule (i.e., degree of node ×η × per node mini-batch size).

The optimalη is searched in a pre-defined grid and we ensure that the best performance was contained in the middle of the grids.
