...
In practice, one typically uses either beam search to approximate this decoding, or a sequence of greedy predictionŝ y t := arg max a∈A p(a|ŷ 1 , . . .

,ŷ t−1 , x).If we use the "teacher forcing" regimen, where the inputs to the RNN cell are the ground truth tokens (as opposed to its own greedy predictions), we obtain the probability of each ground truth sequence according to the RNN model.
...
For each token a, we obtain a predicted sequenceŷa.
...
Then, at each step t of this trajectory, we compute the costs c t (a) associated with each possible token a. To do so we pick a at this step and then follow a roll-out policy to finish the output sequenceŷ a .

We then compareŷ a with the ground truth using the test error itself, rather than a surrogate.
...
Update the parameters of the network ω by doing a single gradient step 18: end for Choosing a multi-class classifier.
...
DISPLAYFORM0 α is a scaling parameter that controls how peaky the target distributions are.
...
This is akin to a mixed roll-in -an idea which also appears in BID8 ).Wiseman & Rush (2016, BSO) adapt one of the early variants of the L2S framework: the "Learning A Search Optimization" approach of Daumé & Marcu (2005, LASO) to train RNNs.
...
L t (s t ; c t ) = − log e st(a )+αct(a ) A i=1 e st(i)+αct(i) where a = arg min a∈A c t (a) .

(6) α is a scaling parameter that ensures that the scores of the model and the costs are not too dissimilar, and can be chosen using a validation set.
