...
The classical way of using machine learning for predicting molecular properties is combining an expressive, hand-crafted representation of the atomic neighborhood (Bartók et al., 2013) with Gaussian processes (Bartók et al., 2010; or neural networks (Behler & Parrinello, 2007) .
...
Our proposed message embeddings can also be interpreted as directed edge embeddings. (Undirected) edge embeddings have already been used in previous GNNs (Jørgensen et al., 2018; Chen et al., 2019) .
...
Some models additionally use auxiliary information Θ such as bond types or electronegativity of the atoms.

We do not include auxiliary features in this work since they are hand-engineered and non-essential.

In summary, we define an ML model for molecular prediction with parameters θ via f θ : {X, z} → R.
...
where the targett =Ê is the ground-truth energy (usually available as well),F are the ground-truth forces, and the hyperparameter ρ sets the forces' loss weight.

For stable simulations F i must be continuously differentiable and the model f θ itself therefore twice continuously differentiable.

We hence cannot use discontinuous transformations such as ReLU non-linearities.

Furthermore, since the atom positions X can change arbitrarily we cannot use pre-computed auxiliary information Θ such as bond types.
...
The edge embeddings e (l) (ij) usually only depend on the interatomic distances, but can also incorporate additional bond information or be recursively updated in each layer using the neighboring atom embeddings (Jørgensen et al., 2018) .
...
E.g. at a cutoff of roughly 2 Å a regular GNN would not be able to distinguish between a hexagonal (e.g. Cyclohexane) and two triangular molecules (e.g. Cyclopropane) with the same bond lengths since the neighborhoods of each atom are exactly the same for both (see Appendix, Fig. 6 ).
...
with the group action in the input and output space ϕ X g and ϕ Y g .
...
We use the directional information associated with each embedding by leveraging the angle α (kj,ji) = ∠x k x j x i when aggregating the neighboring embeddings m kj of m ji .
...
For the interaction function f int in Eq. 4 we use a joint representation a (kj,ji) SBF of the angles α (kj,ji) between message embeddings and the interatomic distances d kj = x k − x j 2 , as well as a representation e (ji) RBF of the distances d ji .
...
Recall that our first goal is to construct a joint 2D basis for d kj and α (kj,ji) , i.e. a function that depends on d and a single angle α.
...
Normalizing Ψ SBF inside the cutoff distance c yields the 2D spherical Fourier-Bessel basisã (kj,ji) SBF ∈ R NSHBF·NSRBF , which is illustrated in Fig. 2 and defined byã
...
They furthermore enable us to bound the highest-frequency components by ω α ≤ NSHBF 2π , ω d kj ≤ NSRBF c , and ω dji ≤ NRBF c .
...
We furthermore fine-tune the Bessel wave numbers k n = nπ c used inẽ RBF ∈ R NRBF via backpropagation after initializing them to these values, which we found to give a small boost in prediction accuracy.
...
First, DimeNet uses the self-gated Swish activation function σ(x) = x · sigmoid(x) (Ramachandran et al., 2018) instead of a regular ReLU activation function.

Second, we multiply the radial basis functionsẽ RBF (d) with an envelope function u(d) that has a root of multiplicity 3 at the cutoff c. Finally, DimeNet does not use any auxiliary data but relies on atom types and positions alone.
...
E.g. for Benzene, depending on the force weight ρ, DimeNet achieves 0.035 kcal mol −1 MAE for the energy or 0.07 kcal mol −1 and 0.17 kcal mol
...
An example of this would be Cyclohexane and two Cyclopropane molecules with slightly stretched bonds, when the GNN either uses the molecular graph or a cutoff distance of c ≤ 2.5 Å. Directional message passing solves this problem by considering the direction of each bond.
...
To illustrate the filters learned by DimeNet we separate the spatial dependency in the interaction function f int via f int (m, d ji , d kj , α (kj,ji) ) = n [σ(W m + b)]

n f filter1,n (d ji )f filter2,n (d kj , α (kj,ji) ).

The filters f filter1,n : R + → R and f filter2,n : R + × [0, 2π] → R F are given by

where W RBF , W SBF , and W are learned weight matrices/tensors, e RBF (d) is the radial basis representation, and a SBF (d, α) is the 2D spherical Fourier-Bessel representation.

Fig. 5 shows how the first 15 elements of f filter2,n (d, α) vary with d and α when choosing the tensor slice n = 1 (with α = 0 at the top of the figure) .
