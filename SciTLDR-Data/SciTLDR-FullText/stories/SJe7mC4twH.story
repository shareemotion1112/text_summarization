...
where c 1 , c 2 , α, β are tunable hyper-parameters.
...
As such, in Section 4, we mainly present the results obtained by setting α = β.
...
As for losses, we set c 1 = 1, c 2 = 1, α = 0.001, β = 0.001.

The scale of α and β are chosen to make α · L Q→E and β · L E→Q have smaller but close magnitude of L Q and L E .
...
In our experiments, We didn't find significant change of results when we tuned the ratio of α and β.
...
Specifically, when training Q-inner on MNIST, we set α = 0.0001, β = 0.0001 and use the second inactive concept updating strategy (otherwise it could not converge), keeping other hyperparameters unchanged.
