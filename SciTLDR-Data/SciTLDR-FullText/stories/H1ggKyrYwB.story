...
Works on differentiable theorem proving (Rocktäschel & Riedel, 2017) and neural reasoning (Peng et al., 2015; Marra et al., 2019) learn vector representations of symbols under constraints such as mutual proximity of similar symbols.
...
In the model depicted in Fig. 2 , these layers make up the functions f θ (·), g φ (·), and h(·).

We use the supervised task loss together with soft regularizers.

The task loss for VQA is the binary cross-entropy L VQA (a,â) whereâ is the onehot encoding of the ground truth answer.
...
The factors λ equ , λ ent , and λ ops are non-negative scalar hyperparameters (see Section D).
...
The projected embeddings are then used by the remaining of the network, i.e. a = h ψ (x , y).
...
The second step of our training procedure aims at improving the layers (f θ ) that produce the embeddings such that the constraints are more closely respected.
...
Our solution is to retrain the layers x = f θ (Q) by distillation, using the projected embeddings x as targets (Fig. 2) .

Practically, the other parts of the network are frozen (g φ (·) and h ψ (·)) and we optimize the following objective: min θ Σ i L dst ||x − x|| 2 , with x = f θ (Q).
...
The input question Q and image I are passed through the embedding functions f θ (·) and g φ (·), respectively.
...
These embedding layers produce the two vector representations x = f θ (Q) and y = g φ (I), with x ∈ R M , y ∈ R N .

These vectors are passed to a fusion and output stage that produces the vector a = h ψ (x, y) containing scores over a large set of candidate answers.
...
Optimal loss weights were determined empirically as λ 1 =0.5, λ 2 =0.1, and λ 3 =0.1 for best performance on the validation set.
...
In the projection by gradient descent in Section 3.2, the number of steps T =10 and the step size α=0.01.
