...
Then we replace the x r with uninformative reference valuê x r to test which region x r is important for classifier's output p M (c|x r , x \r ).
...
Then given a binary mask 2 z ∈ {0, 1} U and the original image x, we define an infilling function 3 φ as a convex mixture of the input and reference with binary weights, DISPLAYFORM0
...
The label for this image is "goose".over masks q θ (z) and optimize its parameters.

The SDR problem is a minimization w.r.t θ of DISPLAYFORM1 On the other hand, SSR aims to find a masked region that maximizes classification score while penalizing the size of the mask.

For sign consistency with the previous problem, we express this as a minimization w.r.t θ of DISPLAYFORM2 Naively searching over all possible z is exponentially costly in the number of pixels U .

Therefore we specify q θ (z) as a factorized Bernoulli: DISPLAYFORM3 Bern(z u |θ u ).This corresponds to applying Bernoulli dropout BID14 to the input pixels and optimizing the per-pixel dropout rate.

θ is our saliency map since it has the same dimensionality as the input and provides a probability of each pixel being marginalized (SDR) or retained (SSR) prior to classification.

We call our method FIDO because it uses a strong generative model (see section 4.1) to Fill-In the DropOut region.

To optimize the θ through the discrete random mask z, we follow in computing biased gradients via the Concrete distribution BID9 BID7 ; we use temperature 0.1.

We initialize all our dropout rates θ to 0.5 since we find it increases the convergence speed and avoids trivial solutions.
...
We instead optimize the parameters of a Bernoulli dropout distribution q θ (z), which enables us to sample reference valuesx from a learned generative model.

Our method uses mini-batches of samples z ∼ q θ (z) to efficiently explore the huge space of binary masks and obtain uncertainty estimates, whereas BBMP is limited to a local search around the current point estimate of the mask z. See Figure 5 for a pseudo code comparison.

In Appendix A.1 we investigate how the choice of algorithm affects the resulting saliency maps.

To avoid unnatural artifacts in φ(x, z), BID3 and BID2 additionally included two forms of regularization: upsampling and total variation penalization.

DISPLAYFORM0 With φ, compute L by Equation 4 or 5 Update z with ∇ z L end while Return z as per-feature saliency map DISPLAYFORM1 With φ, compute L by Equation FORMULA3 Upsampling is used to optimize a coarser θ (e.g. 56 × 56 pixels), which is upsampled to the full dimensionality (e.g. 224 × 224) using bilinear interpolation.

Total variation penalty smoothes θ by a 2 regularization penalty between spatially adjacent θ u .
...
Blur generatesx by blurring x with Gaussian kernel (σ = 10) BID3 .

Random samplesx from independent per-pixel per-channel uniform color with Gaussians (σ = 0.2).
...
In Figure 7 we generate saliency maps of different infilling techniques by interpreting ResNet using L SSR with sparsity penalty λ = 10 −3 .
...
To quantify the artifacts in the saliency maps by a proxy: the proportion of the MAP configuration (θ > 0.5) that lies outside of the ground truth bounding box.
...
We put both algorithms on equal footing by using λ = 1e−3 for FIDO and BBMP (see Section A.1 for further comparisons).
...
After thresholding the saliency map θ above 0.5, we compute the smallest bounding box containing all salient pixels.
...
We follow the procedure of mean thresholding in BID3 : we normalize the heatmap between 0 and 1 and binarize by threshold θ = αµ i where µ i is the average heatmap for image i.

Then we take the smallest bounding box that encompasses all the binarized heatmap.

We search α between 0 to 5 with 0.2 step size on a holdout set to get minimun WSL error.

The best α are 1.2, 2 and 1 respectively.
...
Because the continuous mask of BBMP does not naturally partition the features into observed/unobserved, BBMP-CA first thresholds the masked region r = I(z > τ ) before generating the reference φ(x r , x \r ) with a sample from CA-GAN.

We sweep the value of τ as 1, 0.7, 0.5, 0.3, 0.1 and 0.
...
One potential concern in qualitatively comparing these methods is that each method might have a different sensitivity to the sparsity parameter λ.

Subjectively, we observe that BBMP requires roughly 5 times higher sparsity penalty λ to get visually comparable saliency maps.

In our comparisons we sweep λ over a reasonable range for each method and show the resulting sequence of increasingly sparse saliency maps FIG0 ).

We use λ = 5e−4, 1e−3, 2e−3, 5e−3.We observe that all methods are prone to artifacts in the low λ regime, so the appropriate selection of this value is clearly important.
...
Here we examine the effect of learning a reduced dimensionality θ that upsampled to the full image size during optimization.

We consider a variety of upsampling rates, and in a slight abuse of terminology we refer to the upsampling "size" as the square root of the dimensionality of θ before upsampling, so smaller size implies more upsampling.
...
We show the performance of BBMP-CA with various thresholds τ on both WSL and SM on subset of 1, 000 images in TAB3 .

We also show more qaulitative examples in FIG14 .

We find BBMP-CA is relatively brittle across different thresholds of τ .

Though with τ = 0.3, the BBMP-CA perform slightly better than BBMP and FIDO with heuristics infilling, it still performs substantially inferior to FIDO-CA.
