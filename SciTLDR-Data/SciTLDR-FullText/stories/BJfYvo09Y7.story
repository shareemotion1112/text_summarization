...
Single-clip tracking policies For each clip, we train a policy π θ (a|s, t) with parameters θ such that it maximizes a discounted sum of rewards, r t , where the reward at each step comes from a custom scoring function (see eqns.
...
From the energy, we write the reward function: DISPLAYFORM2 where w total is the sum of the per energy-term weights and β is a sharpness parameter (β = 10 throughout).
...
Gradient updates to the policy were performed using short time windows, {s τ , a τ } τ =1...T , sampled from replay: DISPLAYFORM3 where η was fixed in our experiments.
...
Secondly, we found that attempting to learn the variance of the policy actions tended to result in premature convergence, so best results were obtained using a stochastic policy with fixed noise (we used noise with σ = .1).
...
For example, clip 1 would generate a list of fragments: DISPLAYFORM0 10 (a|s t , τ ).

When fragment 1 was chosen, τ the time-indexing variable was set to τ = 0 initially and ticked until, say, τ = 0.1.

Choosing fragment 2, π 1 2 , would likewise send a signal to the clip 1 policy starting from τ = 0.1, etc.
...
To assess the visual features that drove movements, we computed saliency maps BID37 showing the intensity of the gradient of the selected action's log-probability with respect to each pixel: DISPLAYFORM0 c |∇ Ix,y,c log π(a HL t |h t )|) with normalization Z and clipping g (Fig. 7) .
