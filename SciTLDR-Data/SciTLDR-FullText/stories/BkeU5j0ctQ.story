...
Finally, among ESs, Estimation of Distribution Algorithms (EDAs) are a specific family where the population is represented as a distribution using a covariance matrix Σ BID16 ).

This covariance matrix defines a multivariate Gaussian function and samples at the next iteration are drawn according to Σ. Along iterations, the ellipsoid defined by Σ is progressively adjusted to the top part of the hill corresponding to the local optimum θ * .
...
In more details, each individual x i is sampled by adding Gaussian noise around the mean of the distribution µ, according to the current covariance matrix Σ, i.e. x i ∼ N (µ, Σ).

The problemdependent fitness of these new individuals (f i ) i=1,...,λ is computed, and the top-performing K e individuals, (z i ) i=1,...

,Ke are used to update the parameters of the distribution as follows: DISPLAYFORM0 (1) DISPLAYFORM1 where (λ i ) i=1,...
...
In its standard formulation, CEM uses the new estimate of the mean µ to compute the new Σ, whereas CMA-ES uses the current µ (the one that was used to sample the current generation) as is the case in (2).

We used the latter as BID10 shows it to be more efficient.

The algorithm we are using can thus be described either as CEM using the current µ for the estimation of the new Σ, or as CMA-ES without evolutionary paths.
...
The mean actor of the CEM population, referred to as π µ , is first initialized with a random actor network.

A unique critic network Q π managed by TD3 or DDPG is also initialized.

At each iteration, a population of actors is sampled by adding Gaussian noise around the current mean π µ , according to the current covariance matrix Σ. Half of the resulting actors are directly evaluated.

The corresponding fitness is computed as the cumulative reward obtained during an episode in the environment.

Then, for each actor of the other half of the population, the critic is updated using this actor and, reciprocally, the actor follows the direction of the gradient given by the critic Q π for a fixed number of steps.

The resulting actors are evaluated after this process.

The CEM algorithm then takes the top-performing half of the resulting global population to compute its new π µ and Σ. The steps performed in the environment used to evaluate all actors in the population are fed into the replay buffer.
...
By contrast, in cases where the gradient steps decrease performance, the resulting actors are ignored by CEM, which instead focuses on standard samples around π µ .
...
Actually, the key contribution of CMA-ES with respect to CEM consists of the evolutionary path mechanism (see Section 3.2), but this mechanism results in some inertia in Σ updates, which resists to the beneficial effect of applying RL gradient steps.
...
Require: max steps, the maximum number of steps in the environment τ CEM , σ init , σ end and pop size, hyper-parameters of the CEM algorithm γ, τ, lr actor and lr critic , hyper-parameters of DDPG 1: Initialize a random actor π µ , to be the mean of the CEM algorithm 2: Let Σ = σ init I be the covariance matrix of the CEM algorithm 3: Initialize the critic Q π and the target critic Q π t 4: Initialize an empty cyclic replay buffer R 5: total steps, actor steps = 0, 0 6: while total steps < max steps:7:Draw the current population pop from N (π µ , Σ) with importance mixing (see Algorithm 2 in Appendix B)8:for i ← 1 to pop size/2:

DISPLAYFORM0 Set the current policy π to pop [i] 10:Initialize a target actor π t with the weights of π 11:Train Q π for 2 * actor steps / pop size mini-batches 12:Train π for actor steps mini-batches 13:Reintroduce the weights of π in pop 14:actor steps = 0

for i ← 1 to pop size:16:Set the current policy π to pop [i] 17:(fitness f , steps s) ← evaluate(π)18:Fill R with the collected experiences 19:actor steps = actor steps + s total steps = total steps + actor steps 20:Update π µ and Σ with the top half of the population (see (1) and (2) in Section 3.2) 21: end while• How does CEM-RL perform compared to ERL?
...
The discount rate γ was set to 0.99, and the target weight τ to 5e −3 .

All populations contained 10 actors, and the standard deviations σ init , σ end and the constant τ cem of the CEM algorithm were respectively set to 1e −3 , 1e −5 and 0.95.
...
The fact that the covariance matrix Σ moves faster with CEM-RL may explain this result, as it prevents the reuse of samples.
...
For CEM-RL, the effect varies greatly from environment to environment, but the gain in sample reuse is almost null if min(1, DISPLAYFORM0 Append z i to the current generation g new 7:Draw z i according to the current pdf p(., θ new )8: DISPLAYFORM1 Append z i to the current generation g new 10:size = |g new |
