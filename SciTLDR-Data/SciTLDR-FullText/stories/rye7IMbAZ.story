...
The regularized objective functionJ that is to be optimized is the sum of the standard objective function J and the regularizer Ω(w).
...
L 2 penalty Our baseline penalty for transfer learning is the usual L 2 penalty, also known as weight decay, since it drives the weights of the network to zero: DISPLAYFORM0 where α is the regularization parameter setting the strength of the penalty and · p is the p-norm of a vector.
...
Then, the GL-SP penalty is: DISPLAYFORM7 where w 0 G0 = w 0 S = 0, and, for g > 0, α g is a predefined constant that may be used to balance the different cardinalities of groups.

In our experiments, we used α g = p 1/2 g .
...
Cross validation is used for searching the best regularization hyperparameters α and β: α differs across experiments, and β = 0.01 is consistently picked by cross-validation for regularizing the last layer.
...
This scaling is equal to λi λi+α for the i-th eigenvector of eigenvalue λ i .

A similar analysis can be used for the L 2 -SP regularization.

We recall that J(w) is the unregularized objective function, andJ(w) = J(w) + α w − w 0 2 2 is the regularized objective function.
