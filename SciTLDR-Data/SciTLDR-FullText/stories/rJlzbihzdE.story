...
Compute a set of initial parameters θ, so that the loss L τi of a task drawn from the distribution of tasks P (τ ) is minimized after m iterations.

This is shown in Equation FORMULA0 .

DISPLAYFORM0 Here, f DISPLAYFORM1 In Equation 2,θ = f m τi (θ), and J τi (θ) is the Jacobian matrix of the function f m τi .

Here, we can interpret f m τi as the summation of gradient updates after m iterations of training.

The key idea of firstorder meta-learning is treating these gradients as constants, so that the Jacobian matrix J τi (θ) can be replaced with an identity operation.

Hence, the outer-loop optimization on the initial parameters simply becomes gradient = L τi (θ) BID1 .The Reptile BID7 algorithm further simplifies the above first-order update by replacing the gradient vector L τi (θ) to be the difference between the vectors of updated parameters and initial parametersθ − θ.
...
This is implemented by defining two new hyper-parameters γ and ψ, that takes effect during the meta-step update of the training process.

γ defines a threshold value for (θ − θ) difference in the meta-step and ψ decides the number of iterations that gradient pruning will be effective, towards the end of the entire training process.
...
Algorithm 1 Reptile-based first-order meta-learning algorithm with meta-step gradient pruning Initialize parameters θ Define hyper-parameters γ, ψ, for iterations = 1,2,...m do Sample a training task τ i from P (τ ) and compute f m τi (θ) and loss L τi Updating model m times using Adam to obtainθ if current portion of executed iteration < ψ then DISPLAYFORM2 The key intuition behind this approach is that, smaller meta-step updates towards the end of the training process tend to overfit on the train set tasks rather than generalizing.
...
Secondly, we investigated the effects of introducing deeper networks in the inner-loop of f m τi (θ) and evaluated the performance of the algorithm on the benchmark tasks.
...
The hyper-parameters γ and ψ from Algorithm 1 were tuned during training.
