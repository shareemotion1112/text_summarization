...
The fundamental problem associated with the aforementioned challenges is the need to efficiently calculate an unbiased low-variance gradient wrt parameters γ for an expectation objective of the form E qγ (y) [f (y)]1 .

We are interested in general distributions q γ (y), for which the components of y may be either continuous or discrete.

Typically the components of y have a hierarchical structure, and a subset of the components of y play a role in evaluating f (y).Unfortunately, classical methods for estimating gradients of E qγ (y) [f (y)] wrt γ have limitations.
...
To motivate this paper, we begin by briefly elucidating common machine learning problems for which there is a need to efficiently estimate gradients of γ for functions of the form E qγ (y) [f (y)].

Assume access to data samples {x i } i=1,N , drawn i.i.d.

from the true (and unknown) underlying distribution q(x).

We seek to learn a model p θ (x) to approximate q(x).

A classic approach to such learning is to maximize the expected log likelihoodθ = argmax θ E q(x) [log p θ (x)], perhaps with an added regularization term on θ.

Expectation E q(x) (·) is approximated via the available data samples, aŝ θ = argmax θ 1 N N i=1 log p θ (x i ).

It is often convenient to employ a model with latent variables z, i.e., p θ (x) = p θ (x, z)dz = p θ (x|z)p(z)dz, with prior p(z) on z. The integral wrt z is typically intractable, motivating introduction of the approximate posterior q φ (z|x), with parameters φ.

The well-known evidence lower bound (ELBO) BID16 BID2 ) is defined as ELBO(θ, φ; x) = E q φ (z|x) [log p θ (x, z) − log q φ (z|x)](1) DISPLAYFORM0 where p θ (z|x) is the true posterior, and KL(· ·) represents the Kullback-Leibler divergence.

Variational learning seeks (θ,φ) = argmax θ,φ N i=1 ELBO(θ, φ; x i ).

While computation of the ELBO has been considered for many years, a problem introduced recently concerns adversarial learning of p θ (x), or, more precisely, learning a model that allows one to efficiently and accurately draw samples x ∼ p θ (x) that are similar to x ∼ q(x).

With generative adversarial networks (GANs) BID8 , one seeks to solve DISPLAYFORM1 where D β (x) is a discriminator with parameters β, quantifying the probability x was drawn from q(x), with 1 − D β (x) representing the probability that it was drawn from p θ (x).

There have been many recent extensions of GAN BID28 Zhao et al., 2016; BID1 BID20 , but the basic setup in (3) holds for most.

To optimize (1) and (3), the most challenging gradients that must be computed are of the form ∇ γ E qγ (y) [f (y)]; for (1) y = z and γ = φ, while for (3) y = x and γ = θ.

The need to evaluate expressions like ∇ γ E qγ (y) [f (y)] arises in many other machine learning problems, and consequently it has generated much prior attention.
...
Specifically, it was assumed q γ (y) = v q γ (y v ), omitting explicit dependence on data x, for notational convenience; y v is component v of y. In BID17 DISPLAYFORM0 , with mean µ v (γ) and variance σ 2 v (γ).

In the following we generalize q γ (y v ) such that it need not be Gaussian.

Applying integration by parts BID32 ) DISPLAYFORM1 where y −v denotes y with y v excluded, and Q γ (y v ) is the cumulative distribution function (CDF) of q γ (y v ).

The "0" term is readily proven to be zero for any Q γ (y v ), with the assumption that f (y) doesn't tend to infinity faster than ∇ γ Q γ (y v ) tending to zero when y v → ±∞.The "Key" term exactly recovers the one-dimensional Rep when reparameterization BID32 .

Further, applying ∇ γ q γ (y v ) = q γ (y v )∇ γ log q γ (y v ) in (6) yields REINFORCE.
...
The GO gradient in Theorem 1 can only handle single-layer mean-field q γ (y), characterized by an independence assumption on the components of y. One may enlarge the descriptive capability of q γ (y) by modeling it as a marginal distribution of a deep model BID32 BID2 .

Hereafter, we focus on this situation, and begin with a 2-layer model for simple demonstration.

Specifically, consider DISPLAYFORM0 where γ = {γ y , γ λ }, y is the leaf variable, and the internal variable λ is assumed to be continuous.

Components of y are assumed to be conditionally independent given λ, but upon marginalizing out λ this independence is removed.

, and via Theorem 1 DISPLAYFORM0 Lemma 1.

Equation (10) exactly recovers the Rep gradient in (5), if γ = γ y and q γ (y) has reparameterization y = τ γ (λ), λ ∼ q(λ) for differentiable τ γ (λ) and easily sampled q(λ).Lemma 1 shows that Rep is a special case of our deep GO gradient in the following Theorem 2.

Note neither Implicit Rep gradients nor pathwise derivatives can recover Rep in general, because a neural-network-parameterized y = τ γ (λ) may lead to non-trivial CDF Q γ (y).For the gradient wrt γ λ , we first apply Theorem 1, yielding DISPLAYFORM1 For continuous internal variable λ one can apply Theorem 1 again, from which DISPLAYFORM2 Now extending the same procedure to deeper models with L layers, we generalize the GO gradient in Theorem 2.
...
, y (L−1) } are assumed continuous (these generalize λ from above).
...
The deep GO gradient in Theorem 2 exactly recovers back-propagation BID36 when each element distribution q γ (l) (y DISPLAYFORM8 v |y (l−1) ) is specified as the Dirac delta function located at the activated value after activation function.
...
Recall the motivating discussion in Section 2, in which we considered generative model p θ (x, z) and inference model q φ (z|x), the former used to model synthesis of the observed data x and the latter used for inference of z given observed x. In recent deep architectures, a hierarchical representation for cumulative latent variables z = (z (1) , . . .
...
In the latter, only the leaf variable DISPLAYFORM3 Published as a conference paper at ICLR 2019 is because in Section 4 the underlying model is a marginal distribution of z (L) , i.e., q γ (z (L) ), which is relevant to the generators of GANs; see (3), with DISPLAYFORM4 were added there to enhance the modeling flexibility of q γ (z (L) ).

In this section, the deep set of random variables z = (z (1) , . . .

, z (L) ) are inherent components of the underlying generative model for x, i.e., p θ ( DISPLAYFORM5 Hence, all components of z manifested via inference model q φ (z|x) = q φ (z (1) , . . .

, z (L) |x) play a role in f (z).

Besides, no specific structure is imposed on p θ (x, z) and q φ (z|x) in this section, moving beyond the aforementioned first-order Markov structure.

For a practical application, one may employ domain knowledge to design suitable graphical models for p θ (x, z) and q φ (z|x), and then use the following Theorem 3 for training.

Theorem 3 (Statistical Back-Propagation).

For expectation objectives DISPLAYFORM6 where DISPLAYFORM7 represents L − I continuous or discrete leaf variables with no children except f (·), and q φ (·) is constructed as a hierarchical probabilistic graphical model DISPLAYFORM8 with each element distribution q φ (z v |pa(z v )) having accessible variable-nablas as defined in Theorem 1, pa(z v ) denotes the parent variables of z v , the General and One-sample (GO) gradient for φ k ∈ φ is defined as DISPLAYFORM9 where ch(φ k ) denotes the children variables of φ k , and with DISPLAYFORM10 and DISPLAYFORM11 Statistical back-propagation in Theorem 3 is relevant to hierarchical variational inference (HVI) BID32 BID13 BID23 ) (see Appendix G), greatly generalizing GO gradients to the inference of directed acyclic probabilistic graphical models.
...
The corresponding generator p θ (x) is expressed as DISPLAYFORM0 For brevity, we integrate the generator's parameters θ into the NN notation, and do not explicitly express them.
...
DISPLAYFORM1 , and we have DISPLAYFORM2 where DISPLAYFORM3 , we then apply integration by parts (or partial integration) to get DISPLAYFORM4 With Q γ (∞) = 1 and Q γ (−∞) = 0, it's straightforward to verify that the first term is always zero for any Q γ (y v ), thus named the "0" term.
...
Accordingly, we have DISPLAYFORM1 (a n+1 − a n )B n .Substituting n = y v , a n = f (y), b n = ∇ γ q γ (y v ), and B n = ∇ γ Q γ (y v ) into the above equation, we have DISPLAYFORM2 .
...
Γ(x, y) is the upper incomplete gamma function.
...
Assume q γ (y) being the marginal distribution of the following 2-layer model DISPLAYFORM0 where γ = {γ y , γ λ }, q γ (y, λ) = q γ y (y|λ)q γ λ (λ) = v q γ y (y v |λ) · k q γ λ (λ k ), and both the leaf variable y and the internal variable λ could be either continuous or discrete.

Accordingly, the objective becomes DISPLAYFORM1 For gradient wrt γ y , using Theorem 1, it is straight to show DISPLAYFORM2 For gradient wrt γ λ , we first have DISPLAYFORM3 Withf (λ) = E qγ y (y|λ) [f (y)], we then apply Theorem 1 and get DISPLAYFORM4 where DISPLAYFORM5 DISPLAYFORM6 Next, we separately discuss the situations where λ k is continuous or discrete.
...
Accordingly, we have the gradient for γ λ in FORMULA37 as DISPLAYFORM5 One can straightforwardly verify that, with the generalized variable-nabla defined in (25), the chain rule applies to DISPLAYFORM6 , where one can freely specify both leaf and internal variables to be either continuous or discrete.
...
Practically, if one has to deal with a q γ (y (1) , · · · , y (L) ) with discrete internal variables y (l) , l < L, we suggest the strategy in Figure 7 , with which one should expect a close performance but enjoy much easier implementation with statistical back-propagation in Theorem 3 of the main manuscript.
...
C PROOF OF LEMMA 1First, a marginal distribution q γ (y) with reparameterization y = τ γ ( ), ∼ q( ) can be expressed as a joint distribution, namely DISPLAYFORM0 where q γ (y| ) = δ(y − τ γ ( )), δ(·) is the Dirac delta function, and τ γ ( ) could be flexibly specified as a injective, surjective, or bijective function.
...
Firstly, with the internal variable λ being continuous, FORMULA11 and FORMULA13 in the main manuscript are proved by FORMULA37 and FORMULA37 in Section B, respectively.
...
When all q γ (i) y (i) |y (i−1) s are specified as Dirac delta functions, namely DISPLAYFORM0 where σ(γ (i) , y (i−1) ) denotes the activated values after activation functions, the objective becomes DISPLAYFORM1 where DISPLAYFORM2 Back-Propagation.

For the objective in (28), the Back-Propagation is expressed as DISPLAYFORM3 where DISPLAYFORM4 Deep GO Gradient.

We consider the continuous special case, where DISPLAYFORM5 With q γ (i+1) y (i+1) |y (i) s being Dirac delta functions, namely, DISPLAYFORM6 By substituting the above equation into (12) in Theorem 2, and then comparing it with(29), one can easily verify Corollary 1.
...
In Hierarchical Variational Inference, the objective is to maximize the evidence lower Bound (ELBO) DISPLAYFORM0 For the common case with z = {z (1) , · · · , z (L) }, it is obvious that Theorem 3 of the main manuscript can be applied when optimizing φ.

Practically, there are situations where one might further put a latent variable λ in reference q φ (z|x), namely q φ (z|x) = q φ z (z|λ)q φ λ (λ)dλ with φ = {φ z , φ λ }.
...
DISPLAYFORM1 where the second "0" term is straightly verified as DISPLAYFORM2 Eliminating the "0" term from (31), one still has another problem, that is, log q φ (z|x) is usually non-trivial when q φ (z|x) is marginal.

For this problem, we follow BID32 to use another lower bound ELBO2 of the ELBO in (30).

DISPLAYFORM3 where r ω (λ|z, x), evaluable, is an additional variational distribution to approximate the variational posterior q φ (λ|z, x).

Accordingly, we get the ELBO2 for Hierarchical Variational Inference as DISPLAYFORM4 Note similar to (31), the unnecessary "0" term related to log q φ (z, λ|x) should also be removed.
...
The only difference between GRep and GRep-Stick (also RSVI and RSVI-Stick) is the latter does NOT analytically express the entropy E q φ (z) For the discrete case corresponding to the NB distribution, GO is compared to REINFORCE (Williams, 1992) .
...
100 Monte Carlo samples are used to calculate the ELBO in the NB toys.• 1-layer linear model: DISPLAYFORM1 where σ(·) is the sigmoid function.• Nonlinear model: DISPLAYFORM2 where tanh(·) is the hyperbolic-tangent function.
...
In fact, we should take a step back and start from (8) of the main manuscript, which is equivalent to analytically express an expectation in (9), namely DISPLAYFORM3 where DISPLAYFORM4 with P v (γ) being the Bernoulli probability of Bernoulli random variable y v .
...
Practically, an easy-to-use trick for changing gradients of any function h(x) is to definê DISPLAYFORM7 Rely on the mature auto-differential software for back-propagating gradients For efficient implementation of D y f (y), one should exploit the prior knowledge of function f (y).
...
Again we take (34) as an example, and assume f (y) has the special structure DISPLAYFORM8 where σ(·) is an element-wise nonlinear activation function, and r(·) is a function that takes in a vector and outputs a scalar.
...
DISPLAYFORM10 •Step 1: Define Ξ y h as the matrix whose element [Ξ y h] vj represents the "new" h * j when input {
...
For a quantitative assessment of the computational complexity, our PyTorch code takes about 30 minutes to get the most challenging 4-bit task in Figs. 4 and 11, with a Titan Xp GPU.Firstly, recall that the generator p θ (x) of the developed multinomial GAN (denoted as MNGAN-GO) is expressed as ∼ N (0, I), x ∼ Mult(1, NN P ( )), where NN P ( ) denotes use of a neural network to project to distribution parameters P. For brevity, we integration the generator's parameters θ into the NN notation and do not explicitly express them.
...
The main reason is: the latent code z (l) s and their gamma shape parameters Φ (l+1) z (l+1) s are usually extremely sparse, meaning most elements are almost zero; a gamma distribution z ∼ Gam(α, β) with almost-zero α has an increasingly steep slope when z approaches zero, namely the gradient wrt z shall have an enormous magnitude that unstablize the learning procedure.
