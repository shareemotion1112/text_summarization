...
where x is a sample from the read data, G θ (z) is a sample generated by the generator with the initialization z that is drawn from the noise distribution p z (e.g., standard normal distribution).
...
If the generator generates a sample G θ (z) that is better (>) than its previously generated sample x r , it receives a positive reward; if G θ (z) is worse (<) than x r , it receives a negative reward; while if the quality of G θ (z) is classified as similar (≈) to x r , it receives zero credit.
...
In text generation, the generator G θ obtains the reward only when one sample has been completely generated, which means no intermediate reward is gained.

To relieve this problem, following the practice in SeqGAN (Yu et al., 2017) , we utilize the Monte Carlo rollout method to approximate intermediate rewards by sampling unknown tokens with generated prefix Y 1:t following generator policy G θ till sample completion.
...
With likelihood ratio (Sutton et al., 2000) , we can formulate the gradient of the objective function for generator G θ as:
...
Update G θ via Eq (6) 12: end for 13: Update M with G θ 14: until Convergence based on Texygen (Zhu et al., 2018) , which is a benchmark platform for evaluating adversarial text generation models.
