...
Action Unit The agent also has a stochastic policy network π(α|s; W π ) where W π is the parameter of the network.

Specifically, we use a two-layer feedforward network that takes the agent's state s as its input: DISPLAYFORM3 where W(1) DISPLAYFORM4 π ∈ R 3×H and b DISPLAYFORM5 Following the learned policy, the agent decomposes a question of length N by generating a sequence of actions α t ∈ {1st, 2nd, 3rd}, t = 1, 2, . . .
...
There is another reward function R = Σ log P (Y * | X) that is widely used in the literature of using reinforcement learning for natural language processing task BID1 BID37 .
...
We use the RMSProp optimizer BID27 to train all the networks with the parameters recommended in the original paper except the learning rate α.
