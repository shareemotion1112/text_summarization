...
Large-scale distributed machine learning has motivated the development of numerous distributed optimization meth-ods in the past decade (see, e.g., BID5 Zhang et al., 2013; Li et al., 2014a; Shamir et al., 2014; Reddi et al., 2016; Zhang et al., 2015; Richtárik & Takáč, 2016; BID3 .
...
Definition 1 (γ-inexact solution).

For a function h(w; w 0 ) = F (w) + µ 2 w − w 0 2 , and γ ∈ [0, 1], we say w * is a γ-inexact solution of min w h(w; w 0 ), if ∇h(w * ; w 0 ) ≤ γ ∇h(w 0 ; w 0 ) , where ∇h(w; w 0 ) = ∇F (w) + µ(w − w 0 ).

Note that a smaller γ corresponds to higher accuracy.

We use γ-inexactness in our analysis (Section 4) to measure the amount of local computation from each local solver.

In experiments (Section 5), we simply run an iterative local solver for some number of local epochs, which can be seen as a proxy for γ-inexactness.
...
Note that FedAvg is a special case of FedProx with µ = 0.
...
In order for ρ in Theorem 3 to be positive, we need γB < 1.

Moreover, we also need DISPLAYFORM4 These conditions help to quantify the trade-off between dissimilarity bound (B) and the algorithm parameters (γ, K).Finally, we can use the above sufficient decrease to characterize the rate of convergence under Assumption 1.

Note that these results hold for general non-convex F k (·).Theorem 5 (Convergence rate: FedProx).

Given some > 0, assume that for B ≥ B , µ, γ and K the assumptions of Theorem 3 hold at each iteration of FedProx.
...
From left to right, as data become more heterogeneous, convergence becomes worse for FedProx with µ=0 (FedAvg).

Setting µ > 0 is particularly useful in heterogeneous settings although that may slow convergence for IID data.

Properties of FedProx Framework.

The key parameters of FedProx that affect performance are the number of local epochs, E, and the proximal term scaled by µ. We study FedProx under different values of E and µ using the federated datasets described in TAB3 in Appendix B.1.
...
In Figure 3 , e.g., we show that FedProx with a large E (E=50) and an appropriate µ (µ=0.2) leads to faster and more stable convergence compared with E=1, µ=0 (slow convergence) and E=50, µ=0 (unstable convergence).
...
The method with µ = 0 corresponds to FedAvg.

Increasing heterogeneity leads to worse convergence, but setting µ > 0 can help to combat this.

(2) Dependence on µ. We consider the effect of µ on convergence in Figure 2 (middle).

We observe that the appropriate µ can force divergent methods to converge or increase the stability for unstable methods ( Figure 5 , Appendix B.3), thus making the performance of FedProx less dependent on E. In practice, µ can be adaptively chosen based on the current performance of the models.

For example, one simple heuristic is to increase µ when seeing the loss increasing and decreasing µ when seeing the loss decreasing.
...
Therefore, smaller dissimilarity indicates better convergence, which can be enforced by setting µ appropriately.

Proof.

Using our notion of γ-inexactness for each local solver (Definition 1), we can define e t+1 k such that: DISPLAYFORM1 Now let us definew DISPLAYFORM2 .

Based on this definition, we know DISPLAYFORM3 Let us defineμ = µ − L − > 0 andŵ t+1 k = arg min w h k (w; w t ).

Then, due to theμ-strong convexity of h k , we have DISPLAYFORM4 Note that once again, due to theμ-strong convexity of h k , we know that DISPLAYFORM5 .
...
Based on the L-Lipschitz smoothness of f and Taylor expansion, we have DISPLAYFORM9 From the above inequality it follows that if we set the penalty parameter µ large enough, we can get a decrease in the objective value of f (w t+1 ) − f (w t ) which is proportional to ∇f (w t ) 2 .
...
If we replace these bounds in (13) we get DISPLAYFORM16 Combining (9), (12), (10) and (16) and using the notation α = 1 µ we get DISPLAYFORM17
...
In addition, let the iterate w t be such that ∇f (w t ) 2 ≥ , and let E k ∇F k (w) − ∇f (w) 2 ≤ σ 2 hold instead of the dissimilarity condition.

If µ, K and γ in Algorithm 1 are chosen such that DISPLAYFORM2 then at iteration t of Algorithm 1, we have the following expected decrease in the global objective: DISPLAYFORM3 where S t is the set of K devices chosen at iteration t.
...
In addition, let F k (·) be convex and γ = 0, i.e., all the local problems are solved exactly.

If 1 B ≤ 0.5 √ K, then we can choose µ ≈ 6LB 2 from which it follows that ρ ≈ 1 24LB 2 .Proof.

In the convex case, where L − = 0 andμ = µ, if γ = 0, i.e., all subproblems are solved accurately, we can get a decrease proportional to ∇f (w t ) 2 if B < √ K. In such a case if we assume 1 << B ≤ 0.5 √ K, then we can write DISPLAYFORM0 In this case, if we choose µ ≈ 6LB 2 we get DISPLAYFORM1 Note that the expectation in FORMULA0 is a conditional expectation conditioned on the previous iterate.
...
In other words, under the bounded variance assumption, for small , if we replace B with its upper-bound in Corollary 6 and choose µ large enough, then the iteration complexity of FedProx when the subproblems are solved exactly and DISPLAYFORM0 ), which is the same as SGD BID6 .
...
In particular, for each device k, we generate synthetic samples DISPLAYFORM0 , where the covariance matrix Σ is diagonal with Σ j,j = j −1.2 .

Each element in the mean vector v k is drawn from N (B k , 1), B k ∼ N (0, β).

Therefore, α controls how much local models differ from each other and β controls how much the local data at each device differs from that of other devices.

We vary α, β to generate three heterogeneous distributed datasets, Synthetic (α, β), as shown in Figure 1 .
...
Note that FedAvg (µ = 0) and FedProx (µ ≥ 0) perform the same amount of work at each round when the number of local epochs, E, is the same; we therefore report results in terms of rounds rather than FLOPs or wall-clock time.
...
We also create synthetic data to directly test the effect of heterogeneity on convergence, as in Section 5.• Synthetic: We set (α, β)=(0,0), (0.5,0.5) and (1,1) respectively to generate three non-identical distributed datasets (Figure 1 ).

In the IID data, we set the same W, b ∼ N (0, 1) on all devices and X k to follow the same distribution N (v, Σ) where each element in the mean vector v is drawn from N (0, 1) and Σ is diagonal with Σ j,j = j −1.2 .
...
For each dataset, we set E to be 1, 20, and 50 while keeping µ = 0 (FedProx reduces to FedAvg in this case) and show the convergence in terms of the training loss.
...
We consider the effect of µ on convergence in Figure 5 .

For each experiment, in the case of E = 50, we compare the results between µ = 0 and the best µ. For three out of the four datasets (all but Sent140) we observe that the appropriate µ can increase the stability for unstable methods and can force divergent methods to converge.
...
We show a simple adaptive heuristic of setting µ on four synthetic datasets in Figure 11 .
...
Under either sampling scheme, the settings with µ = 1 demonstrate more stable performance than settings with µ = 0.for simple quadratic problems.
