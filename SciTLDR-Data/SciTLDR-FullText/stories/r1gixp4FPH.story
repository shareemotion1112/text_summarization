...
In the interpolated setting (no additive noise) their analysis yields a convergence rate of O(P oly(κ,κ) exp(−
...
For loss function with µ-strong convexity and L-smoothness, the condition number κ is defined as κ = L/µ.
...
Let L and µ be the largest and the smallest non-zero eigenvalues of the Hessian respectively.

Then the condition number is then κ = L/µ (note that zero eigenvalues can be ignored in our setting, see Section 4).

Given a mini-batch size m, we define the m-stochastic condition number as κ m := L m /µ.

Following (8), we introduce the quantityκ (called statistical condition number in (8)), which is the smallest positive real number such that E x 2

Hence, the quadratic loss function is also L m -smooth, for all m ≥ 1.

By the definition of κ m , we also have

Remark 2.

It is important to note thatκ ≤ κ 1 , since E x 2
...
Moreover, the optimal step size η * (m) of SGD that induces fastest convergence guarantee is proven to be 1/L m (12).
...
Specifically, we demonstrate a setting where SGD+Nesterov can be proved to have convergence rate of (1 − O(1/κ)) t , which is same (up to a constant factor) as SGD.
...
For any step size η > 0 and momentum parameter γ ∈ (0, 1) of SGD+Nesterov with random initialization, with probability one, there exists a T ∈ N such that ∀t > T ,

where C > 0 is a constant.

Compared with the convergence rate (1−1/κ) t of SGD (12) , this theorem shows that SGD+Nesterov does not accelerate over SGD.
...
Intuitively, the key reason for the non-acceleration of SGD+Nesterov is a condition on the step size η required for non-divergence of the algorithm.

Specifically, when momentum parameter γ is close to 1, η is required to be less than
...
In particular, the condition on the step-size of SGD+Nesterov excludes η * that achieves fastest convergence for SGD.

We show in the following corollary that, with the step size η * , SGD+Nesterov diverges.
...
Let step-size η = and acceleration parameter γ ∈ [0.6, 1], then SGD+Nesterov, with random initialization, diverges with probability 1.
...
We show that MaSS can converge exponentially for all the step sizes that result in convergence of SGD, i.e., η ∈ (0, 2/L m ).

Importantly, we derive a convergence rate exp(−t/ √ κ 1κ ), whereκ ≤ κ 1 , for MaSS which is faster than the convergence rate for SGD exp(−t/κ 1 ).
...
There is a bijection between the hyper-parameters (η 1 , η 2 , γ) and (η, α, δ), which is given by:

Remark 3 (SGD+Nesterov).

In the literature, the Nesterov's method is sometimes written in a similar form as the R.H.S. of Eq.8.

Since SGD+Nesterov has no compensation term, δ has to be fixed as η/α, which is consistent with the parameter setting in (15).
...
Let µ be the smallest non-zero eigenvalue of the Hessian matrix H. Let L m be as defined in Eq.2.

Denoteκ m :=κ/m + (m − 1)/m.

In MaSS with mini batch of size m, if the positive hyper-parameters η, α, δ satisfy the following two conditions:
...
By condition Eq.10, the admissible step size η is (0, 2/L m ), exactly the same as SGD for interpolated setting (12) .
...
Using that w t −w * 2 ≤ αF t /δ, we get the final conclusion.

See detailed proof in Appendix D.

Hyper-parameter Selection.

From Theorem 2, we observe that the convergence rate is determined by (1−α) t .

Therefore, larger α is preferred for faster convergence.

Combining the conditions in Eq.10, we have

By setting η * = 1/L m , which maximizes the right hand side of the inequality, we obtain the optimal selection α * = 1/ √ κ mκm .

Note that this setting of η * and α * determines a unique δ * = α * /µ by the conditions in Eq.10.

In summary,

By Eq.9, the optimal selection of (η 1 , η 2 , γ) would be:

κ m is usually larger than 1, which implies that the coefficient η * 2 of the compensation term is non-negative.

The non-negative coefficient η 2 indicates that the weight u t is "over-descended" in SGD+Nesterov and needs to be compensated along the gradient direction.
...
which is faster than the rate O(e −t/κm ) of SGD (see (12) ), since κ m ≥κ m .

Remark 7 (MaSS Reduces to the Nesterov's method for full batch).

In the limit of full batch m → ∞, we have κ m → κ,κ m → 1, the optimal parameter selection in Eq.14 reduces to

It is interesting to observe that, in the full batch (deterministic) scenario, the compensation term vanishes and η * 1 and γ * are the same as those in Nesterov's method.

Hence MaSS with the optimal hyper-parameter selection reduces to Nesterov's method in the limit of full batch.

Moreover, the convergence rate in Theorem 3 reduces to O(e −t/ √ κ ), which is exactly the well-known convergence rate of Nesterov's method (15; 4).
...
and the convergence rate in Eq. In the linear scaling regime, the hyper-parameter selections follow a Linear Scaling Rule (LSR): When the mini-batch size is multiplied by k, multiply all hyperparameters (η, α, δ) by k. This parallels the linear scaling rule for SGD which is an accepted practice for training neural networks (6) .
...
In all the tasks and for all the algorithms, we select the best hyper-parameter setting over dense grid search, except that we fix the momentum parameter γ = 0.9 for both SGD+Nesterov and MaSS, which is typically used in practice.
...
Moreover, in our experiment, SGD+Nesterov with large step size η = 0.3 diverges in 5 out of 8 runs on CNN and 2 out of 5 runs on ResNet-32 (for random initialization), while MaSS and SGD converge on every run.

Step-size η 1 , secondary step-size η 2 , acceleration parameter γ ∈ (0, 1).
...
A differentiable function f : R d → R is µ-strongly convex (µ > 0), if f (x) ≥ f (z) + ∇f (z), x − z + µ 2 x − z 2 , ∀x, z ∈ R d . (21) Definition 2 (Smoothness).
...
For the ease of analysis, we define u := 1 − γ ∈ (0, 1] and t j := ησ 2 j , j = 1, 2.

Without loss of generality, we assume σ 2 1 = 1 in this section.

In this case, t 1 = η and t 2 = η/κ, where κ is the condition number.
...
By definition of Φ in Eq.27, we can see that the convergence rate is lower bounded by the convergence rates of the sequences { Φ

[j] t } t .

By the relation Eq.28, we have that the convergence rate of the sequence { Φ t } t is controlled by the magnitude of the top eigenvalue λ max of B, if Φ t has nonzero component along the eigenvector of B with eigenvalue λ max (B).

Specifically, if |λ max | > 1, Φ

[j] t grows at a rate of |λ max | t , indicating the divergence of SGD+Nesterov; if |λ max | < 1, then

converges at a rate of |λ max | t .
...
We will analyze the dependence of the eigenvalues on κ, when κ is large to obtain Lemma 2.

For any u ∈ (0, 1], if step size

then, B [2] has an eigenvalue of magnitude 1 − O (1/κ).

Finally, we show that Φ t has non-zero component along the eigenvector of B with eigenvalue λ max , hence the convergence of SGD+Nesterov is controlled by the eigenvalue of B [j] with the largest magnitude.
...
By combining Lemma 1, 2 and 3, we have that SGD+Nesterov either diverges or converges at a rate of (1 − O(1/κ)) t , and hence, we conclude the non-acceleration of SGD+Nesterov.
...
We will show that at least one of the eigenvalues of B [2] is 1 − O(1/κ), under the condition in Eq.32.

First, we note that t 2 = t 1 /κ = η/κ, which is O(1/κ).

We consider the following cases separately:

2 ) and o(1); and 5) u is Θ(1), the last of which includes the case where momentum parameter is a constant.

Note that, for cases 1-4, u is o(1).

In such cases, the step size condition Eq.32 can be written as

It is interesting to note that η must be o(1) to not diverge, when u is o(1).
...
Recall that t 2 = η/κ, then we have
...
This implies that t 0.5 2 = o(1/κ) and u = o(1/κ).

Therefore, all the eigenvalues λ i are of order 1 − O(1/κ).

Case 3: u is O(t 2 ).

This case if forbidden by the assumption of this lemma.

This is because, κ is o(1) ).

This is contradictory to u is O(t 2 ).

Case 4: u is Ω(t 0.5 2 ) and o (1) .
...
Note that the u-only terms cancel in λ 2 , so the first order after the constant term must be of t 2 (could be t 2 /u 2 , t 2 /u etc.).
...
where f (u) and g(u) are u-terms only, which, by the above analysis in Eq.37, are shown to contribute nothing to λ 2 .

Hence, we use the first terms of T 1 and T 2 above to analyze the first order term of λ 2 .

Plugging in these term to the expression of λ 2 , and keeping the lowest order of t 2 , we find a zero coefficient of the lowest order t 2 -term:

Hence, λ 2 can be written as:

where c is the coefficient.

On the other hand, by Eq.35 and t 2 = η/κ, we have

Therefore, we can write Eq.38 as λ 2 = 1 − O(1/κ).

.

This is the case where the momentum parameter is κ-independent.
...
The idea is to examine the subspace spanned by Φ t , t = 0, 1, 2, · · · , j = 1, 2, and to prove that the eigenvector of B

[j] corresponding to top eigenvalue (i.e., eigenvalue with largest magnitude) is not orthogonal to this spanned subspace.

This in turn implies that there exists a non-zero component of Φ [j] t in the eigen direction of B [j] corresponding to top eigenvalue, and this decays/grows at a rate of λ t max (B [j] ).

[j]

t has non-zero component in the eigen direction of B [j] with top eigenvalue, then Φ

[j]

t should also have non-zero component in the same direction.

Thus, it suffices to show that at least one of Φ
...
Denote the vectorized form of Φ

t ), which is a 4 × 1 column vector.

We stack the vectorized forms of Φ
...
[j] with eigenvalue γ(1 − ησ

Hence, v is not the eigenvector along top eigenvalue, and therefore, is orthogonal to the eigen space with top eigenvalue.

In order to prove at least one of Φ t , t = 0, 1, 2, 3, has a non-zero component along the eigen direction of top eigenvalue, it suffices to verify that M
...
Hence, at least one of Φ t , t ∈ {0, 1, 2, 3} has non-zero component in the eigen direction with top eigenvalue.
...
Lemma 1 and 2 show that, for any hyper-parameter setting (η, γ) with η > 0 and γ ∈ (0, 1), either top eigenvalue of B [1] is larger than 1 or top eigenvalue of B [2] is 1 − O(1/κ).

Hence, |λ max | is either greater than 1 or is 1 − O(1/κ).

Lemma 3 shows that Φ t has non-zero component along the eigenvector of B with eigenvalue λ max (B).

By Eq.28 and Lemma 1 and 2, the sequence { Φ t } t either diverges or converges at a rate of

t .

By definition of Φ in Eq.27, we have that { t } t either diverges or converges at a rate of (1 − O(1/κ)) t .

Note that, for the two-dimensional component decoupled data, we have

Therefore, the convergence rate of SGD+Nesterov is lower bounded by (1−O(1/κ)) t .

Note that the convergence rate of SGD on this data is (1 − O(1/κ)) t , hence SGD+Nesterov does not accelerate SGD on the two-dimensional component decoupled dataset.

Proof of Corollary 1.

When η = 1/L 1 and γ ∈ [0.6, 1], the condition in Eq.31 is satisfied.

By Lemma 1, the top eigenvalue λ [1] max of B [1] is larger than 1.

By Lemma 3, Φ [1] has non-zero component along the eigenvector with this top eigenvalue λ [1] max .
...
where e 1 , e 2 ∈ R 2 are canonical basis vectors, σ 1 > σ 2 > 0.
...
x i are independently drawn from N (0, diag(σ Step size: η * = 1/L 1 = 1/6, and momentum parameter: γ is 0.9 or 0.99. performed on either 3-d Gaussian or component decoupled data with fixed σ 1 and σ 2 .

For each setting of (σ 1 , σ 2 ), we randomly select w * , and generate 2000 samples for the dataset.
...
As discussed in Corollary 1, SGD+Nesterov diverges with step size η * = 1/L 1 (when γ ∈ [0.6, 1]), which is the optimal choice of step size for both SGD and MaSS.

We run SGD+Nesterov, with step size η * = 1/L 1 , to optimize the square loss function on component decoupled data mentioned above.

Figure 8 shows the divergence of SGD+Nesterov with two common choices of momentum parameter γ: 0.9 and 0.99.

with the parameter ρ on the loss function they prove convergence rate 1 − 1/ρ 2 κ t of their method (called SGD with Nesterov acceleration in their paper), where t is the iteration number.

In the following, we show that, on a simple (zero-mean) Gaussian distributed data, this rate is slower than that of SGD, which has a rate of (1 − 1/κ) t .

On the other hand, MaSS achieves the accelerated rate 1 − 1/ (2 + d)κ t .
...
For each training process, we compute the convergence speed s(m), which is defined to be the inverse of the number of iterations needed to achieve a training error of ε.
