...
where P w F , P w R are one-dimensional projections of the high dimensional real and fake samples to w and Ω denotes a sufficiently high number of projections on the unit sphere.
...
Max-sliced Wasserstein GANs can be introduced by selecting the worst projected distribution from Ω noted by w max and since the projection can be implemented by a fully connected layer, one can re-introduce the mini-max game used in regular GAN training: min
...
compute surrogate loss s(ωD i , ωF
...
In this case a parameter (ν) can be set, determining a limit and if the difference between the two distances is larger than this value the sorted distance will be used.

This way Wasserstein distance with the hybrid assignment (W H P ) can be defined as:

In case of ν = 0 the greedy assignment will only be used in those cases, where it also results the minimal transportation.

If ν is set to one, the greedy assignment will be used if the distance calculated this way is less than twice the minimal distance.

We were using ν = 1 in our experiments.
