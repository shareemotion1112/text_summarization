...
In this paper, we reconstruct a video sequence from the compressive measurements in (1) by jointly optimizing over the latent codes z t and the network parameters γ.
...
For a single image reconstruction, deep image prior solve the following optimization to obtain optimalγ, arg min

In this optimization, z is initialized randomly and kept unaltered.

To jointly optimize the latent codes and generator parameters for a video sequence, we use the similar formulation as in (2) but optimize it over the z t and γ.

The resulting optimization problem can be written as

The reconstructed video sequence can be generated using the estimated latent codes (ẑ 1 , . . .

,ẑ T ) and generator weights (γ) asx t = Gγ(ẑ t ).

We initialize latent codes with samples drawn from a Gaussian distribution and normalize them to have unit norm.

We initialize γ with random weights using the initialization scheme in [24] .
...
At the end of our reconstruction process, we have a single set of trained weightsγ, reconstructed framesx t and their corresponding optimal latent codesẑ t .
...
where α ij is the weight of the corresponding basis vector.
