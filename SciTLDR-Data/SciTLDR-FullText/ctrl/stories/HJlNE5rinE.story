...
We let Ω denote the set of all observed entries, and for any matrix Z, definē Z = P Ω (Z) asZ ij = Z ij if (i, j) ∈ Ω and 0 otherwise.
...
Also, TNR can be considered as a special case of StaTNA by taking λ S = ∞, which explains why StaTNA will in general be more robust compared to TNR.
...
Next, as aforementioned, TNR is a special case for StaTNA, so to solve TNR, we simply take λ S = ∞ in Algorithm 1.
...
Furthermore, both experiments involve two hyperparameters, namely λ L and λ S , which are tuned using 10-fold cross validation.
...
As aforementioned, TNR is a special case for StaTNA, so to solve TNR, we simply take λ S = ∞ in Algorithm 1, which forces all S k andŜ k to 0.
