...
Function f is an element-wise nonlinear activation function; for instance, it can be the sigmoid function σ(u) = e u /(1 + e u ).
...
In the attention mechanism below, we generate the weights α t with an RNN structure, and summarize c t in a decoder series z t ,

Here f AT T and φ θ are chosen as tanh layers in our experiments.

Note that the attention weight α t at state t depends on the previous attention weight α t−1 , the embeddings, and the previous decoder state z t−1 , and the decoder series z t sums up information of c t up to state t.
...
Specifically, we assign the attention weights α 1 , . . .

, α T for the query, and β 1 , . . .

, β T for the search results.
...
While they both consider the previous weights α t−1 and β t−1 , the pre-softmax weight e tm considers the m-th component of the query q m and all information from the search results, and f tn considers the opposite.
...
, T , α t = sof tmax(e t ), β t = sof tmax(f t ).

(2) The attention weights α t and β t can then be assigned to create the context vectors c t for the query andd t for the search results, which are defined as follows,
...
"RSVM-100k" means that 10 5 pairs are sampled for the SVM, "λMART-2%" means that 2% of all feature vectors are sampled for each tree in LambdaRank, and so forth.
