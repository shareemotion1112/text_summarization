...
Our overall loss combines the triplet loss and the entropy regularizer, as DISPLAYFORM1 where the parameter λ ≥ 0 controls the trade-off between ranking quality and uniformity.
...
Both datasets contain 1M vectors that serve as a reference set, 10k query vectors and a very large training set of which we use 500k elements for training, and 1M vectors that we use a base to cross-validate the hyperparameters d out and λ.
...
For all methods, we train our neural network on the training set, cross-validate d out and λ on the validation set, and use a different set of vectors for evaluation.
...
Similarly, the regularizer λ needs to be set to a large value for small dimensions and low bitrates, but higher dimensions and higher bitrates require lower values of λ (cf.
...
The optimal value of the regularizer λ decreases with the dimension, as shown by TAB2 : Optimal values of the regularization parameter λ for Deep1M, using a fixed radius of r = 10.
...
normalize y with N , store the permutation σ that sorts coordinates of |y| 2.

exhaustively search the atom z that maximizes N (y) z 3.

apply the inverse permutation σ −1 that sorts y to z to obtain z 4.
...
In this experiment, we consider different thresholds ε for the range search and perform a set of queries for each ε.
...
Without our mapping, there is a large variance on the number of results for a given ε.

In contrast, after the mapping it is possible to use a unique threshold to find most neighbors.

For example: to obtain 80% recall, the search in the original space requires to set ε = 0.54, which returns 700 results per query on average, while in the transformed space ε = 0.38 returns just 200 results.
