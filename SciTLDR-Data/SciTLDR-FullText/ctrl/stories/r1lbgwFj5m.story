...
Specifically, we train a range of architectures from scratch where N m in each block is proportional to that found in the Fisher-pruned architecture -block j can be represented by B(N i , αN mj , N o ): N mj is a block-specific value corresponding to the height of each bar in FIG1 (right), and α scales the whole architecture.

We vary α to produce networks with different parameter counts as before, which are then trained from scratch.
