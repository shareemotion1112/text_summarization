...
Using a realworld dataset of movie ratings, we demonstrate that our framework yields the best performances in terms of two well-known metrics (see Footnote 10): Kendall tau distance (Kendall, 1938) and normalized discounted cumulative gain (Järvelin & Kekäläinen, 2002) .
...
We defineŷ AB as the estimate of Pr [y AB = 1].
...
We denote byŷ an estimate of y. We use subscripts as in y AB when y concerns a comparison between A and B. We use superscripts as in w (t) when w is updated iteratively.
...
One can consider α as a step size in the update.
...
1 As in (Negahban et al., 2016) , α = 1/dmax where dmax := maxi di and di is the number of distinct items to which item i is compared.
...
(3) Obtain {ŷ AB } (A,B)∈D train for each group comparison sample in D train by using w (T ) obtained in (2) above and module G.
...
Algorithm 1 estimates (ŷ AB ,ŷ BC ,ŷ CA ) = (0.55, 0.6, 0.7), and Algorithm 2 yields (ŷ AB ,ŷ BC ,ŷ CA ) = (0.45, 0.7, 0.8).
...
We measure the performance in terms of two well-known metrics 10 : the Kendall tau distance (Kendall, 1938) and normalized discounted cumulative gain (NDCG@K) (Järvelin & Kekäläinen, 2002) .
...
10 Kendall tau distance is defined as |{(i, j) : i < j, (τ1(i) < τ1(j) ∧ τ2(i) > τ2(j)) ∨ (τ1(i) > τ1(j) ∧ τ2(i) < τ2(j))}| where τ1(i) and τ2(i) are the rankings of item i in τ1 and τ2.
