...
Since W j and x i are all vectors, the inner product of them can be formulated as W j x i cos(θ j ), where θ j is the angle of W j and x i .

After formulating the softmax loss to an inner product form with a factor cos(θ), it is clear that the softmax classifier is prone to enforce examples in a class have similar θ and different θ for different classes.
...
For a certain class i, we can regard features of this class as a normal distribution N (µ i , Σ i ) where µ i is the mean of this class and Σ i is the covariance matrix.

Then there exits an orthogonal matrix U and an eigenvalue-diagonal matrix D satisfying Σ i = U DU .
...
Inspired by FIG2 (c) and the discussion of isotropic multivariate normal distribution, the spatial distribution of each class will be more compact if we penalize features such that N (µ i , Σ i ) of class i will be like an isotropic one, say that the variances of different dimensions are approximately uniform.
...
Input: A batch of feature B={x 1 , ..., x m } produced by deep neural networks Output: isotropic loss L I for a mini-batch The total joint loss named the isotropic softmax (isomax) loss is formulated as DISPLAYFORM0 DISPLAYFORM1 where L S is the softmax loss and α controls the trade-off between the isotropic loss and the softmax loss.

A small α may be not enough to form an isotropic distribution while a large α will restrain the supervision of the softmax loss, making features indistinguishable near the hypersphere.

In the following section, we will discuss the influence of different α.
...
To find out the hyper-parameter α, we first examine the 2-D distribution of MNIST.

After 20,000 step training, the feature distribution of 1,000 test samples are shown in FIG3 .

We can see that the result is much like what we expect when α=0.005 or α=0.05.

But for a large α, the force of the isotropic loss is so powerful that the softmax loss loses its ability to shape classes.

During training, we find that even very small α is able to cut down the isotropic loss quickly, the only influence of α is the speed of convergence.

But if α > 0.1, not only the training procedure converges slowly, but the result is also unacceptable.

This threshold is suitable for almost all networks and datasets we try.

In following sections, unless otherwise specified, the value of α is set to 0.05.
...
The triplet loss directly minimizes the Euclidean distance of samples from the same class and maximizes the distance of samples from the different class until reaching a selection margin α.
...
They introduce γ (k) and β (k) for each activation x (k) in a d-dimensional feature x, and transform original feature x=( DISPLAYFORM0 By adding Batch-Norm layer (BN layer) to a certain layer in networks, each dimension of the feature of this layer is transformed according to Equation 7.
...
To make it clearer, we interpret a simplified example by letting γ (k) =1 and β (k) =0.
