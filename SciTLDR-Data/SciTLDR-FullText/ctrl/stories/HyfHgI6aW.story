...
Given an initial state s 0 ∈ S with m[s 0 ] = 0 (obstacle-free) and a goal region S goal , find a function µ : S → A such that applying the actions a t := µ(s t ) results in a sequence of states s 0 , s 1 , . . . , s T satisfying s T ∈ S goal and m[s t ] = 0 for all t = 0, . . .

, T .Instead of trying to estimate the hidden labeling m using a mapping approach, our goal is to learn a policy µ that maps the sequence of sensor observations z 0 , z 1 , . . .
...
A partially observable problem can be represented via a Markov Decision Process (MDP) over the history space H. More precisely, we consider a finite-horizon discounted MDP defined by M(H, A, T , r, γ), where γ ∈ (0, 1] is a discount factor, T : H × A → H is a deterministic transition function, and r : H → R is the reward function, defined as follows: DISPLAYFORM0 The reward function definition stipulates that the reward of a state s can be measured only after its occupancy state has been observed.

Given observations z 0:t , we can obtain an estimatem = max{ τ z τ , −1} of the map of the environment and use it to formulate a locally valid, fully-observable problem as the MDP M t (S, A, f, r, γ) with transition function given by the agent dynamics f and reward r(s t ) :=m[s t ] given by the map estimatem.
...
By iterating multiple times over all states and all actions possible in each state, we can get a policy π = arg max a Q(s, a).
...
As stated before in Section 2.1, this observation can be formulated as a fully observable problem M t (S, A, f, r, γ).

It is possible to plan in M t and calculate the optimal policy for this local space, π * l independent of previous observations FIG2 .

It is then possible to use any planning algorithm to calculate the optimal value function V * l from the optimal policy π * This policy learned by the convolutional network is purely reactive as it is computed for the z new observation independent of the previous observations.
...
Further, when entering and exiting such a structure, the agent's observations are the same, i.e z 1 = z 2 , but the optimal actions under the policies π 1 l and π 2 l (computed by the convolutional network) at these time steps are not the same, i.e a π 1 = a π 2 .
...
For our experiment, we generate a tuple of (x, y, θ) associated with every observation.
...
The output from the access module is concatenated with the output from the LSTM controller and sent through a linear layer followed by a soft max to get probability distributions for (x, y, θ).
