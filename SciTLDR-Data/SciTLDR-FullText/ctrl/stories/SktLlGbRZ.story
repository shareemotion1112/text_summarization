...
For K-way classification with a cross-entropy loss, this corresponds to DISPLAYFORM0 where σ denotes the softmax function.
...
We show image to image translation results under three different settings of the model hyperparameter, λ, which controls the tradeoff between the reconstruction loss and the visual style loss.

When λ = 10 (Figure 9 right), the resulting image converges to a near replica of the original image, thus preserving content but lacking the correct target style.

When λ = 1 or λ = 2.5 (Figure 9 left), the results lack any consistent semantics making it difficult to perceive the style of the transformed image.
