...
We construct a perception model for our agents using a conditional variational autoencoder (CVAE) which generates an estimation of an embedded state, φ t+1 , given the embedded state itself, φ t+1 .

This generative model is conditioned on the last embedded state, φ t , and action, a t .
...
The state embeddings are encoded by a neural network submodule in our architecture which computes feature vectors φ t and φ t+1 from states s t and s t+1 respectively.
...
We first define p(φ t+1 |z, φ t , a t ) as the generative distribution from which we draw the output φ t+1 .

The prior distribution of the latent space is given by p(z|φ t , a t ) which is relaxed in the CVAE formulation to make the latent variable, z, statistically independent of the input variables.
...
The distribution of this representation, q(z|φ t+1 , φ t , a t ), approximates p(z|φ t , a t ).
...
The final component of our perception model is a neural network predicting a t from φ t and φ t+1 built off the inverse model presented by Pathak et al. (2017) .
...
We tune the hyper-parameters λ 1 , λ 2 , and λ 3 to weight the contribution of each loss in our model.
...
In our approach, this probabilistic reward takes the form r t = − log p(φ t+1 |φ t , a t ).
...
= log E (z∼q(z|φt+1,φt,at)) p(φ t+1 |z, φ t , a t )p(z) q(z|φ t+1 , φ t , a t )

≥ E (z∼q(z|φt+1,φt,at)) log p(φ t+1 |z, φ t , a t )p(z) q(z|φ t+1 , φ t , a t )

We use the reconstruction loss of our model to compute the conditional probability log p(φ t+1 |z, φ t , a t ).
...
We trained our decoder on the game images (s t ) and the image embeddings (φ t ).

Then, we used our decoder to reconstruct images from our predicted image embeddings ( φ t ).
