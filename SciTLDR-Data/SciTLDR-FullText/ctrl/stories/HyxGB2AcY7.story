...
To perform action classification, we first compute a convolutional feature map φ s t = φ(s t ) ∈ R H×W ×K based on the observation s t using a convolutional neural network φ.
...
The local convolution features and feature differences for consecutive frames are fed into a shared multi-layer perceptron (MLP) to derive the logits as: DISPLAYFORM1 We then compute an attention mask α t ∈ R H×W corresponding to frame t, which indicates the controllable parts of the observation s t .

Such attention masks are computed via a separate MLP from the features of each region (i, j), and then converted into a probability distribution using softmax or sparsemax operators (Martins & Astudillo, 2016) : DISPLAYFORM2 so that i,j α t (i, j) = 1.

The sparsemax operator is similar to softmax but yields a sparse attention, leading to more stable performance.

Finally, the logits e t (i, j) from all regions are linearly combined using the attention probabilities α t : DISPLAYFORM3 Training.
...
Based on this formulation, the attention probability α t (i, j) should be high only on regions (i, j) that are predictive of the agent's actions.
...
We encourage the attention distribution to attain a high entropy by including an attention entropy regularization loss, i.e., L ent = −H(α t ).
...
The entire training objective becomes: DISPLAYFORM4 where λ ent is a mixing hyperparameter.
...
In 2D environments, a natural discretization (x, y) = argmax (j,i) α t (i, j) provides a good approximation of the agent's location within the current observation 3 .

This provides a key piece of information about the current state of the agent.

Inspired by previous work (Bellemare et al., 2016; BID13 , we add an exploration bonus of r + to the environment reward, where r + (s) = 1/ #(ψ(s)) and #(ψ(s)) denotes the visitation count of the (discrete) mapped state ψ(s), which consists of the contingent region (x, y).

We want to find a policy π that maximizes the expected discounted sum of environment rewards r ext plus count-based exploration rewards r DISPLAYFORM0 , where β 1 , β 2 ≥ 0 are hyperparameters that balance the weight of environment reward and exploration bonus.

For every state s t encountered at time step t, we increase the counter value #(ψ(s t )) by 1 during training.
...
More implementation details can be found in Appendix A and B.For the ADM, we take observation frames of size 160×160 as input (resized from the raw observation of size 210 × 160).5 We employ a 4-layer convolutional neural network that produces a feature map φ(s t ) with a spatial grid size of H × W = 9 × 9.
...
In such setting, we may denote ψ(s) = (x, y, c, R).
...
In order to find a balance between the environment reward and the exploration bonus reward, we perform a hyper-parameter search for the proper weight of the environment reward β 1 and the exploration reward β 2 for A2C+CoEX+RAM, as well as for A2C+CoEX.
...
DISPLAYFORM0 Perform actor-critic using on-policy samples in E θ A2C ← θ A2C − η∇ θA2C L
...
The objective L A2C of Advantage Actor-Critic (A2C) is as in (Mnih et al., 2016; BID9 : DISPLAYFORM1 DISPLAYFORM2 DISPLAYFORM3 where DISPLAYFORM4 is the n-step bootstrapped return and α is a weight for the standard entropy regularization loss term H t (π θ ).

We omit the subscript as θ = θ A2C when it is clear.
...
For the embedding of these observations, we cluster them based on a threshold value τ .

The value of τ for each game with change of rooms is listed in TAB6 .
...
Initialize parameter θ c for context embedding projector if applicable (which is not trainable) Initialize threshold τ for clustering Initialize clusters set C ← ∅ for each observation s do Get embedding of the observation from the random projection v ← f θc (s)Find a cluster to which the current embedding fits, if any Find a cluster c ∈ C with smallest mean(c) − v ≤ τ , or NIL if there is no such if c = NIL then c ← c ∪ v else if there's no existing cluster that v should be assigned to, create a new one C ← C ∪ {v} end if end forIn Figure 7 , we also show the samples of observation in each cluster.
...
On FREEWAY, the effect of loss terms is more clear; in the beginning the agent tends to behave suboptimally by taking mostly single actions only (UP out of three action choices -UP, DOWN, and NO-OP), hence very low entropy H(π(·|s)), which can confuse the ADM of telling which part is actually controllable as the action classifier would give correct answer regardless of attention.
...
We present a result of additional ablation study on the state representation ψ(s) used in count-based exploration.

The following baselines are considered:• A2C+CoEX(c): Uses only the context embedding for exploration, i.e., ψ(s) = (c).• A2C+CoEX(c, R): Uses only the context embedding and the cumulative reward for exploration without contingent region information, i.e., ψ(s) = (c, R).• A2C+CoEX(x, y, c): Uses the contingent region information (x, y) as well as the context embedding c, however without the cumulative reward component, i.e., ψ(s) = (x, y, c).One can also consider another baseline similar to A2C+CoEX(c, R) with ψ(s) = (x, y, c, R), where the location information (x, y) is replaced with random coordinates uniformly sampled from the grid.
