...
Notice that P (Y = y) becomes the integral of D over the region R y .Let φ S and φ T be density functions of D S and D T respectively.

Then, the above optimization problem becomes: DISPLAYFORM2 Where ∆ ⊃ U T is the set of all distributions over X and region R y = {x : DISPLAYFORM3 The Lagrangian equivalent for the above problem can be stated as, DISPLAYFORM4 where λ y ∈ R. Using the Euler-Lagrange equation to solve above problem, DISPLAYFORM5 Intuitively we find that D T is a scaled version of D S with scaling factor as the ratio of P T (Y ) and P S (Y ).
...
However φ T (x) allows us to assign soft domain labels to the source domain instances, which indicates the probability of that instance belonging to the target domain.
...
DANN models starts with a mapping J f : X → R d with a parameter θ f , which projects instances to a latent representation space, we call this the feature extractor.

These features are then mapped to primary label space with a mapping function J y (label predictor) with parameter θ y .

The same features from latent representation space are mapped to domain label by J d (domain classifier), with parameter θ d .
...
From this observation, we propose to replace domain classification loss (second loss term of Eq. 7) with auxiliary label classifier loss L a (θ f , θ a ), where θ a is the parameter for the auxiliary label classifier J a .
...
At each iteration, we remove λ × 0.33 numbers of features, where we decrease λ at each iteration as: 0.95 × λ.
...
Additionally, the gradient reversal weight λ = K 1+exp −10 * li , is related to the i th step of training, where l i = i/num steps.
...
Further, 4 (b) and 5 (b), show the effect of the gradient reversal on the learning process as the λ progressively improves.
