...
The underlying idea is to learn a probabilistic decoder p θ (x | z) , z ∼ N (0, I) that can mimic the true data sample x 1 , . . .

, x N drawn from an existing but unknown data distribution p d (x).

VAE is developed based on the following lower bound: DISPLAYFORM0 where q φ (z | x) is the approximate posterior distribution.
...
More specifically, assuming that DISPLAYFORM2 where the source of randomness ∼ N (0, I) and µ φ (z) , σ φ (z) are two neural networks representing the mean and covariance matrix of the approximate Gaussian posterior.
...
The L2 Wasserstein divergence between two distributions is defined as: DISPLAYFORM1 where Π (q, p) specifies the set of all joint distributions over p, q which admits p, q as marginals.

If p, q are two Gaussians, i.e., p (z) = N (z | µ 1 , Σ 1 ) and q (z) = N (z | µ 2 , Σ 2 ) then both KL divergence and L2 WS distance can be computed in close forms as: DISPLAYFORM2 where · F is the Frobenius norm and Σ 1 Σ 2 = Σ 2 Σ 1 .
...
Inspired by the Variational Auto-Encoder BID11 , we propose to use a probabilistic decoder p θ (x | z) such that for z ∼ p 0 (z), x drawn from p θ (x | z) can mimic those drawn from p 0 (x) and for z ∼ p 1 (z), x drawn from p θ (x | z) can mimic those drawn from p 1 (x).

In other words, we aim to learn the probabilistic decoder p θ (x | z) satisfying: DISPLAYFORM1 For any approximate posterior q φ (z | x), we have the following lower bounds: DISPLAYFORM2 Figure 2: Maximal divergence sequential auto-encoder.
...
Note that we use the same network for q φ (z | x, y = 0) and q φ (z | x, y = 1) and they are discriminated by the source of data used to fit.

Using the architecture shown in Figure 2 , we consider the probabilistic decoder p θ (x | z) of the following parametric form DISPLAYFORM3 and hence we can further derive the lower bounds as: DISPLAYFORM4 We arrive at the following optimization problem: max DISPLAYFORM5 It is worth noting that since we are minimizing: DISPLAYFORM6 the encoding z ∼ q φ (z | h L ) with y = 0 are absorbed (compressed) into the prior p 0 (z).

Similarly, the encoding z ∼ q φ (z | h L ) with y = 1 are compressed into the prior p 1 (z).

Therefore, to maximize the difference between the encodings of data in the two classes, we propose to maximize the divergence between p 0 (z) and p 1 (z) and arrive the following optimization problem: DISPLAYFORM7 where α > 0 is a non-negative trade-off parameter and D p 0 (z) p 1 (z) is the divergence between the two priors.

To facilitate the evaluation, we endow these two priors with Gaussian distributions as follows: DISPLAYFORM8 We also propose using the Gaussian approximate posterior as: DISPLAYFORM9 1/2 , where the source of randomness ∼ N (0, I) and µ φ (z) , σ φ (z) are two neural networks representing the mean and covariance matrix of the approximate Gaussian posterior.
...
If we train the classifier simultaneously, the final optimization problem is as follows: DISPLAYFORM12 where C ψ (x) stands for the probability to classify x as a vulnerable binary code (y = 1), and α, β > 0 are two non-negative trade-off parameters.

It is worth noting that to model the conditional distributions p θ (x i | h i−1 , z), we only take into account the opcode of the machine instruction x i .

Since this opcode lies in a fixed vocabulary of the opcodes, we can use the softmax distribution to define the corresponding distribution p θ (x i | h i−1 , z).
...
For our model, the size of the latent space was set to 4,096, the trade-off parameters α, β were set to 2×10 −2 and 10 −4 respectively.
