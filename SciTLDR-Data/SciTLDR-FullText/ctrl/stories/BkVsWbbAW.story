...
Formally, our problem setting can be called Sequential Multitask Learning and is characterized by a set of tasks T, which are to be learnt by a model parameterized by weights θ (e.g. a neural network).
...
Given new incoming samples (X, Y ), DGR first computes the fraction of total samples that should come from incoming samples (η tasks ) and the fraction to come from previous task samples (η gen ) proportionate to the number of tasks (counting repetitions).

It allots a minimum fraction κ of the memory capacity N max per new task.
...
At least κ = 0.05 fraction of memory was ensured per new task and consolidation in DGDMN happened after n ST M = 5 tasks.
...
All models were trained with RMSProp (Hinton, 2012) using learning rate = 0.001, ρ = 0.9, = 10 −8 and no decay.
...
DGDMN has two new hyperparameters: (i) κ: minimum fraction of N max reserved for incoming tasks, and (ii) n ST M : number of STTMs (also sleep/consolidation frequency).

Both these have straightforward interpretations and can be set directly without complex hyperparameter searches.κ ensures continual incorporation of new tasks by guaranteeing them a minimum fraction of LTM samples during consolidation.
...
So choosing κ = 0.5 K works well in practice (or as a good starting point for tuning).

We made this choice in section 4.3 with K = 10 and κ = 0.05, and hence plotted the average accuracy over the last 10 tasks as a metric.n ST M controls the consolidation cycle frequency.
...
Hence, n ST M can be set as approximately 0.25 κ in practice (as we did in section 4.3), or as a starting point for tuning.
...
κ was set to be small, so that it does not come into play for Digits, Permnist, Shapes and Hindi since we already provided memories with full capacity for all samples.

For TDigits, we used κ = 0.05 which would let us incorporate roughly 10 out of the 40 tasks well.
