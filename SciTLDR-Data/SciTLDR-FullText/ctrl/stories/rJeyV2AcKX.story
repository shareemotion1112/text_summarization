...
In other words, we decouple the feature extraction network f θ and the classification weight W of a standard classification network.

We train f θ as a standard multiclass classification task and learn another network g φ to generate the classification weight W. Since f θ is trained as a standard multi-class classification task to distinguish all classes within the training set, it is supposed to be able to generate more discriminative feature representations for images of unseen classes than that generated by a model trained in episode-based fashion where the model is train to distinguish several classes within mini-batches.

Meanwhile, we train g φ in episode-based fashion by constantly sampling new classes and minimizing the classification loss (cross entropy loss on top of Softmax outputs) using the weights generated by g φ .

After training, whenever some new classes come, along with supporting information in the form of either attribute vectors (ZLS) or a few-labeled samples (FSL), g φ is supposed to be able to generate generic classification weights that can effectively classify query images that belong to these new classes.

Thanks to this network reparameterization strategy, we are able to get a powerful and flexible UCC model.

We adopt the cosine similarity based cross entropy loss to train the weight generator g φ .
...
Their objective function is as follows: DISPLAYFORM0 where f θ is a feature extraction model which outputs a representation vector f θ (x i ) using image x i as input.

h ψ is a mapping function which projects attribute vector a yi of class y i to the embedding space where f θ (x i ) lies.
...
We directly learn a network g φ that outputs the classification weights for classifying visual features and use the cross-entropy loss on top of Softmax outputs to guide learning g φ .
...
Update g φ through back-propagation. end while Visual features of the same classes should be assigned with the same label (compactness), while visual features of different classes are assigned with different labels (separability).We follow the network reparameterization scheme by decoupling the feature extraction module f θ and the classification weight module which is generated by g φ .

The feature extraction module f θ is trained as a standard multi-class classification task to enable us to obtain a discriminative feature representation for any given image.

To learn g φ , we adopt the episode based training scheme by continuously exposing g φ with new (randomly sampled) ZSL tasks so as to secure good performance when new real tasks arrive in the testing stage.
...
With a batch of M z N z images B v and M z attribute vectors B a , we train g φ by minimizing the loss function defined in Eq. 5.

In the testing stage, given attributes of unseen classes A u , or S = A t ∪ A u for all (seen and unseen) classes as in generalized ZSL setting, we generate the corresponding classification weights using g φ .

The generated classification weights, integrated with the feature extraction network f θ serve to classify images of unseen classes.
...
Our novel categorization framework can be easily extended from ZSL to FSL, simply by replacing the semantic attribute vectors with feature embedding vectors as the input to the classification weight generation network g φ .

To train g φ , we keep randomly sampling FSL tasks from D t = {X t , Y t }, each of which consists of a support set and a query set.

Images in the both sets are from the same classes.

The support set consists of M f classes and N f images for each class.

With the feature embeddings B e of the M f N f images as input, g φ generates the classification weights for the M f classes, which are then used to classify the feature embeddings of images from the query set.

Note that if N f > 1, i.e., each class has multiple support samples, we average the embeddings of all images belonging to the same class and feed the averaged embedding to g φ .
...
Following BID29 ), we adopt ResNet101 as our feature extraction model f θ which results in a 2048-dimension vector for each input image.

For the weight generation model g φ , we utilize two FC+ReLU layers to map semantic vectors to visual classification weights.

The dimension of the intermediate hidden layer are 1600 for all the five datasets.

We train g φ with Adam optimizer and a learning rate 10 −5 for all datasets by 1,000,000 randomly sample ZSL tasks.

Each task consists of 32 randomly sampled classes, 4 samples for each class, i.e., M z = 32 and N z = 4.

The hyper-parameters λ is chosen as 10 −4 , 10 −3 , 10 −3 , 10 −5 and 10 −4 for AwA1, AwA2, CUB, SUN and aPY, respectively.
...
We use ResNet18 as our feature extraction model f θ which results in a 512-dimension vector for each input image after average pooling.

We train f θ on the two experimental datasets by following the standard classification learning pipeline: We use Adam optimizer with an initial learning rate 10 −3 which decays to the half every 10 epochs.

The model is trained with 100 epochs.

As for g φ , we use two FC+ReLU layers, same as in ZSL.

The dimension of the intermediate hidden layer is 512 for both datasets.

We train g φ using Adam optimizer with a learning rate 10 −5 and set the hyper-parameters λ = 10 −5 for both datasets.
