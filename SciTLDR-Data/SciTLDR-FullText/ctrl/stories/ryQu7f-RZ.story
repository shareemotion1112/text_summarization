...
The simplest algorithm for this setting is the standard online gradient descent algorithm (Zinkevich, 2003) , which moves the point x t in the opposite direction of the gradient g t = ∇f t (x t ) while maintaining the feasibility by projecting onto the set F via the update rule x t+1 = Π F (x t − α t g t ), where Π F (y) denotes the projection of y ∈ R d onto the set F i.e., Π F (y) = min x∈F x − y , and α t is typically set to α/ √ t for some constant α.
...
Input: x1 ∈ F, step size {αt > 0} T t=1 , sequence of functions {φt, ψt} T t=1for t = 1 to T do gt = ∇ft(xt) mt = φt(g1, . . .

, gt) and Vt = ψt(g1, . . .

, gt) DISPLAYFORM0 "averaging" functions φ t and ψ t have not been specified.

Here φ t : DISPLAYFORM1 For ease of exposition, we refer to α t as step size and α t V −1/2 t as learning rate of the algorithm and furthermore, restrict ourselves to diagonal variants of adaptive methods encapsulated by Algorithm 1 where V t = diag(v t ) .
...
For instance, the first adaptive method ADAGRAD BID2 , which propelled the research on adaptive methods, uses the following averaging functions: DISPLAYFORM4 and step size α t = α/ √ t for all t ∈ [T ].

In contrast to a learning rate of α/ √ t in SGD, such a setting effectively implies a modest learning rate decay of DISPLAYFORM5 When the gradients are sparse, this can potentially lead to huge gains in terms of convergence (see BID2 ).
...
The key difference is to use an exponential moving average as function ψ t instead of the simple average function used in ADAGRAD.

ADAM 1 , a particularly popular variant, uses the following averaging functions: DISPLAYFORM6 for some β 1 , β 2 ∈ [0, 1).
...
A value of β 1 = 0.9 and β 2 = 0.999 is typically recommended in practice.
...
For theoretical analysis, one requires α t = 1/ √ t for t ∈ [T ], although, a more aggressive choice of constant step size seems to work well in practice.

RMSPROP, which appeared in an earlier unpublished work BID7 is essentially a variant of ADAM with β 1 = 0.

In practice, especially in deep learning applications, the momentum term arising due to non-zero β 1 appears to significantly boost the performance.
...
One key observation is that for SGD and ADAGRAD, Γ t 0 for all t ∈ [T ].
...
However, this is not necessarily the case for exponential moving average variants like ADAM and RMSPROP i.e., Γ t can potentially be indefinite for t ∈ [T ] .
...
The large gradient C is unable to counteract this effect since it is scaled down by a factor of almost C for the given value of β 2 , and hence the algorithm converges to 1 rather than −1.
...
Note that above example also holds for constant step size α t = α.
...
This example also provides intuition for why large β 2 is advisable while using ADAM algorithm, and indeed in practice using large β 2 helps.

However the following result shows that for any constant β 1 and β 2 with β 1 < √ β 2 , we can design an example where ADAM has non-zero average rate asymptotically.

Theorem 2.

For any constant β 1 , β 2 ∈ [0, 1) such that β 1 < √ β 2 , there is an online convex optimization problem where ADAM has non-zero average regret i.e., R T /T 0 as T → ∞.The above results show that with constant β 1 and β 2 , momentum or regularization via will not help in convergence of the algorithm to the optimal solution.

Note that the condition β 1 < √ β 2 is benign and is typically satisfied in the parameter settings used in practice.
...
For any constant β 1 , β 2 ∈ [0, 1) such that β 1 < √ β 2 , there is a stochastic convex optimization problem for which ADAM does not converge to the optimal solution.

These results have important consequences insofar that one has to use "problem-dependent" , β 1 and β 2 in order to avoid bad convergence behavior.

In high-dimensional problems, this typically amounts to using, unlike the update in Equation (3), a different , β 1 and β 2 for each dimension.
...
While the results stated above use constant β 1 and β 2 , the analysis of ADAM in BID3 actually relies on decreasing β 1 over time.

It is quite easy to extend our examples to the case where β 1 is decreased over time, since the critical parameter is β 2 rather than β 1 , and as long as β 2 is bounded away from 1, our analysis goes through.

Thus for the sake of clarity, in this paper we only prove non-convergence of ADAM in the setting where β 1 is held constant.
...
To understand the design of our algorithms, let us revisit the quantity Γ t in (2).

For ADAM and RMSPROP, this quantity can potentially be negative.

The proof in the original paper of ADAM erroneously assumes that Γ t is positive semi-definite and is hence, incorrect (refer to Appendix D for more details).

For the first part, we modify these algorithms to satisfy this additional constraint.

Later on, we also explore an alternative approach where Γ t can be made positive semi-definite by using values of β 1 and β 2 that change with t.

AMSGRAD uses a smaller learning rate in comparison to ADAM and yet incorporates the intuition of slowly decaying the effect of past gradients on the learning rate as long as Γ t is positive semidefinite.
...
By doing this, AMSGRAD results in a non-increasing step size and avoids the pitfalls of ADAM and RMSPROP i.e., Γ t 0 for all t ∈ [T ] even with constant β 2 .

Also, in Algorithm 2, one typically uses a constant β 1t in practice (although, the proof requires a decreasing schedule for proving convergence of the algorithm).To gain more intuition for the updates of AMSGRAD, it is instructive to compare its update with ADAM and ADAGRAD.
...
Suppose β 1t = β 1 λ t−1 in Theorem 4, then we have DISPLAYFORM2 The above bound can be considerably better than O( BID2 .

Furthermore, in Theorem 4, one can use a much more modest momentum decay of β 1t = β 1 /t and still ensure a regret of O( √ T ).
...
To enable fair comparison, we set β 1 = 0.9 and β 2 = 0.99 for ADAM and AMSGRAD algorithm, which are typically the parameters settings used for ADAM in practice.
...
The step size parameter α t is set to α/ √ t for both ADAM and AMSGRAD in for our experiments, consistent with the theory.

We use a minibatch version of these algorithms with minibatch size set to 128.

We set β 1 = 0.9 and β 2 is chosen from the set {0.99, 0.999}, but they are fixed throughout the experiment.

The parameters α and β 2 are chosen by grid search.
...
Similar to the previous experiment, we use β 1 = 0.9 and β 2 is chosen from {0.99, 0.999}. We use a fully connected 100 rectified linear units (ReLU) as the hidden layer for this experiment.

Furthermore, we use constant α t = α throughout all our experiments on neural networks.
...
An alternative approach is to use an increasing schedule of β 2 in ADAM.

This approach, unlike Algorithm 2 does not require changing the structure of ADAM but rather uses a non-constant β 1 and β 2 .

The pseudocode for the algorithm, ADAMNC, is provided in the appendix (Algorithm 3).

We show that by appropriate selection of β 1t and β 2t , we can achieve good convergence rates.

Theorem 5.

Let {x t } and {v t } be the sequences obtained from Algorithm 3, α t = α/ √ t, β 1 = β 11 and β 1t ≤ β 1 for all t ∈ [T ].

Assume that F has bounded diameter D ∞ and ∇f t (x) ∞ ≤ G ∞ for all t ∈ [T ] and x ∈ F. Furthermore, let {β 2t } be such that the following conditions are satisfied:

DISPLAYFORM0 Then for x t generated using the ADAMNC (Algorithm 3), we have the following bound on the regret DISPLAYFORM1 The above result assumes selection of {(α t , β 2t )} such that Γ t 0 for all t ∈ {2, · · · , T }.
...
Suppose β 1t = β 1 λ t−1 and β 2t = 1 − 1/t in Theorem 5, then we have DISPLAYFORM2 The above corollary follows from a trivial fact that v t,i = t j=1 g 2 j,i /t for all i ∈ [d] when β 2t = 1 − 1/t.
...
It is easy to generalize this result for setting similar settings of β 2t .

Similar to Corollary 1, one can use a more modest decay of β 1t = β 1 /t and still ensure a data-dependent regret of O( √ T ).
...
Consider the execution of ADAM algorithm for this sequence of functions with DISPLAYFORM1 Note that since gradients of these functions are bounded, F has bounded L ∞ diameter and β 2 1 / √ β 2 < 1.
...
We observe the following: αC DISPLAYFORM4 The second inequality follows from the step size choice that α < √ 1 − β 2 .
...
In order to prove this claim, we show thatx 3t+4 ≥ 1, which readily translates to x 3t+4 = 1 because x 3t+4 = Π F (x 3t+4 ) and F = [−1, 1] here Π F is the simple Euclidean projection (note that in one-dimension, Π F , √ Vt = Π F ).
...
The last equality is due to following fact: DISPLAYFORM11 for the choice of β 2 = 1/(1 + C 2 ).
...
Throughout the proof, we assume β 1 < √ β 2 , which is also a condition (Kingma & Ba, 2015) assume in their paper.

In this proof, we consider the setting where f t are linear functions and F = [−1, 1].

In particular, we define the following function sequence: DISPLAYFORM0 where C ∈ N, C mod 2 = 0 satisfies the following: DISPLAYFORM1 where γ = β 1 / √ β 2 < 1.

It is not hard to see that these conditions hold for large constant C that depends on β 1 and β 2 .
...
Let T be such that t + C ≤ τ 2 t for all t ≥ T where τ ≤ 3/2.
...
The second inequality follows from the definition of τ that t + C ≤ τ 2 t for all t ≥ T .

The third inequality is due to the fact that v t+i−1 ≥ (1 − β 2 )β i−2 2 C 2 .
...
The last inequality follows from the fact that β i −1 2 C 2 ≤ 1 and β C 2 ≤ β 2 .

Furthermore, from the above inequality, we have DISPLAYFORM11 Note that from our choice of C, it is easy to see that λ ≥ 0.

Also, observe that λ is independent of t. Thus, x t+C ≥ min{1, x t + λ/ √ t}. From this fact, we also see the following:1.

If x t = 1, then x t+C = 1 for all t ≥ T such that t mod C = 0.2.

There exists constant T 1 ≥ T such that x T 1 = 1 where T 1 mod C = 0.The first point simply follows from the relation x t+C ≥ min{1, x t + λ/ √ t}. The second point is due to divergent nature of the sum DISPLAYFORM12 where kC ≥ T 1 .
...
Let δ be an arbitrary small positive constant, and C be a large enough constant chosen as a function of β 1 , β 2 , δ that will be determined in the proof.
...
as follows: DISPLAYFORM0 Cx with probability p := 1+δ C+1 −x with probability 1 − pThe expected function is F (x) = δx; thus the optimum point over [−1, 1] is x = −1.
...
For a large enough constant C (as a function of β 1 , β 2 , δ), DISPLAYFORM2 denote expectation conditioned on all randomness up to and including time t − 1.
...
First, for T 1 , we have DISPLAYFORM4 Next, we bound DISPLAYFORM5 log(1/β1) .

This choice of k ensures that β DISPLAYFORM6 Let E denote the event that for every DISPLAYFORM7 Assuming E happens, we can bound m t−1 as follows: DISPLAYFORM8 and so T 2 ≥ 0.With probability at most kp, the event E doesn't happen.
...
Since F is a monotonically increasing function, we have E[F (x t )] ≥ F (0) = 0, whereas F (−1) = −δ.
...
The main issue in their proof is the incorrect assumption that Γ t defined in their equation FORMULA11 is positive semidefinite, and we also identified problems in lemmas 10.3 and 10.4 in their paper.
...
The second inequality is due to the fact that β 1k ≤ β 1 for all k ∈ [T ].
...
Using the above lemma in Equation FORMULA8 , we have: DISPLAYFORM10 The first inequality and second inequality use the fact that β 1t ≤ β 1 .
...
We further bound the above inequality in the following manner: DISPLAYFORM4 The first inequality and second inequality use the fact that β 1t ≤ β 1 .

Furthermore, from the theorem statement, we know that that {(α t .β 2t )} are selected such that the following holds: DISPLAYFORM5 Using the L ∞ bound on the feasible region and making use of the above property in Equation FORMULA9 , we have: DISPLAYFORM6 The equality follows from simple telescopic sum, which yields the desired result.
...
Similar to the proof of Theorem 1, we assume that the initial point is x 1 = 1 and the parameters are:β 1 = 0, β 2 = 2 (1 + C 2 )C 2 and α t = α √ t where α < √ 1 − β 2 .
...
For (3t + 1) th update, the only change from the update of in Equation FORMULA8 is the additional in the denominator i.e., we havê x 3t+2 = x 3t+1 − αC (3t + 1)(β 2 v 3t + (1 − β 2 )C 2 + ) DISPLAYFORM2 The last inequality follows by simply dropping v 3t term and using the relation that α < √ 1 − β 2 .

Therefore, we have 0 <x 3t+2 < 1 and hence x 3t+2 =x 3t+2 > 0.

Furthermore, after the (3t + 2) th and (3t + 3) th updates of ADAM in Equation FORMULA8 , we have the following:x 3t+3 = x 3t+2 + α (3t + 2)(β 2 v 3t+1 + (1 − β 2 ) + ) , x 3t+4 = x 3t+3 + α (3t + 3)(β 2 v 3t+2 + (1 − β 2 ) + ) .Since x 3t+2 > 0, it is easy to see that x 3t+3 > 0.

To complete the proof, we need to show that x 3t+4 = 1.

The only change here from the proof of Theorem 1 is that we need to show the The first inequality is due to the fact that v t ≤ C 2 for all t ∈ N. The last equality is due to following fact: DISPLAYFORM3 for the choice of β 2 = 2/[(1 + C 2 )C 2 ] and = 1.
...
For any non-negative real numbers y 1 , · · · , y t , the following holds: for all the t ∈ [T ], y 1 ∈ F and furthermore, there exists i ∈ [T ] such that δ j ≤ 0 for all j ≤ i and δ j > 0 for all j >
...
y 1 + i j=1 δ j since δ j ≤ 0 for all j ≤ i. Furthermore, also observe that y T +1 ≥ min{b, y i+1 + T j=i+1 δ j } since δ j ≥ 0 for all j > i. Combining the above two inequalities gives us the desired result.
