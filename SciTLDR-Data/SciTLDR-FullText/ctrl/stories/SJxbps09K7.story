...
The generator G is trained to fool the discriminator D. The generator can be any differentiable function; in MUSE, it is a linear transform Ω. Let e ∈ E be an English word vector, and f ∈ F a French word vector, both of dimensionality d. The goal of the generator is then to choose Ω ∈ R d×d such that ΩE has a distribution close to F .
...
The generator in MUSE, which learns a linear transform Ω, has very limited capacity, for example, and we are updating Ω rather than p g .
...
From ΩE and F , we can extract a bilingual dictionary using nearest neighbor queries, i.e., by asking what is the nearest neighbor of ΩE in F , or vice versa.
...
The mean similarity of a source language embedding Ωe to its k nearest neighbors in the target language (k = 10 suggested) is defined as µ DISPLAYFORM3 where cos is the cosine similarity.
...
ICP typically minimizes the λ 2 distance (mean squared error) between nearest neighbor pairs.
...
To see this, note that a GAN minimizes the Jensen-Shannon divergence between F and ΩE. The optimal solution to this is F = ΩE. As sample size goes to infinity, this means the L 2 loss in ICP goes to 0.
...
A function Ω from E to F is a linear transformation if Ω(f + g) = Ω(f ) + Ω(g) and Ω(kf ) = kΩ(f ) for all elements f, g of E, and for all scalars k. An invertible linear transformation is called an isomorphism.
...
Isospectrality varies with Procrustes fit; to see this, we show that DISPLAYFORM1 F , in this case Ω = I. Two isomorphic graphs also have the same set of sorted eigenvalues, i.e., DISPLAYFORM2 In general, it holds that if we add an edge to a graph G, to form G , its spectrum changes monotonically BID29 .
...
Note that if the probability of getting stuck in a local optimum that leads to a poor alignment is β, using n random restarts and oracle model selection we increase the probability of finding a good alignment to 1 − (1 − β) n .
