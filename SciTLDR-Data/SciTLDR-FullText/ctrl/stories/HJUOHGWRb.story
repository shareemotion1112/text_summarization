...
We call such predictors explanations, since they explicitly relate interpretable variables, X, to the targets, Y. For example, when the targets are scalar and binary, explanations may take the form of linear logistic models; when the targets are more complex, dependencies between the components of Y can be represented by a graphical model, e.g., a conditional random field (Lafferty et al., 2001 ).CENs assume that each explanation is context-specific: p w (θ | C) defines a conditional probability of an explanation θ being valid in the context C. To make a prediction, we marginalize out θ's; to interpret a prediction, Y = y, for a given data instance, (x, c), we infer the posterior, p w (θ | Y = y, x, c).

The main advantage of this approach is to allow modeling conditional probabilities, p w (θ | C), in a black-box fashion while keeping the class of explanations, p(Y | X, θ), simple and interpretable.

For instance, when the context is given as raw text, we may choose p w (θ | C) to be represented with a recurrent neural network, while p(Y | X, θ) be in the class of linear models.

Implications of the assumptions made by (1) are discussed in Appendix A. Here, we move on to describing a number of practical choices for p w (θ | C) and learning and inference for those.

In practice, we represent p w (θ | C) with a neural network that encodes the context into the parameter space of explanations.

There are multiple ways to construct an encoder, which we consider below.

Deterministic Encoding.

Consider p w (θ | C) := δ (φ w (C), θ), where δ(·, ·) is a delta-function and φ w is the network that maps C to θ.
...
To address these issues, we constrain the space of explanations by introducing a global dictionary, DISPLAYFORM1 , where each atom of the dictionary, θ k , is sparse.

The encoder generates context-specific explanations using soft attention over the dictionary, i.e., each explanation becomes a convex combination of the sparse atoms ( FIG11 ): DISPLAYFORM2 where α w (c) is the attention over the dictionary.
...
So far, we represented p w (θ | C) by a delta-function centered around the output of the encoder.

It is natural to extend p w (θ | C) to a mixture of delta-distributions, in which case CENs recover the mixtures of experts (MoE, Jacobs et al., 1991) .
...
Modeling p(Y | X, C) in the form of (1) avoids representing the joint distribution, p(θ, C), which is a good decision when the data is abundant.
...
on the parameters of explanations, θ.

We accomplish this by representing p(θ, C) with a variational autoencoder (VAE) BID25 BID3 whose latent variables are explanation parameters FIG0 ).

The generative process and the evidence lower bound (ELBO) are as follows: DISPLAYFORM7 where DISPLAYFORM8 , and q w (θ | C) and p u (C | θ) the encoder and decoder, respectively.

We consider encoders that also make use of the global learnable dictionary, D, and represent q w (θ | C) in the form of logistic normal distribution over the simplex spanned by the atoms of D. For the prior, p γ (θ), we use a Dirichlet distribution with parameters α k < 1 to induce sharp attention.
...
LIME BID22 constructs explanations as local linear approximations of the decision boundary of a model f in the neighborhood of a given point (x, c) via optimization: DISPLAYFORM0 where L(f, θ, π x,c ) measures the quality of the linear model g θ : X → Y as an approximation to f in the neighborhood of (x, c), and Ω(θ) is a regularizer.

The typical choice for L and Ω is L 2 and L 1 losses, respectively.

The neighborhood of (x, c) is defined by a distribution π x,c concentrated around the point of interest.
...
The question we ask:How does the local approximation,θ, relate to the actual explanation, θ , generated and used by CEN to make a prediction in the first place?For the case of binary 3 classification, it turns out that when the context encoder is deterministic and the space of explanations is linear, local approximations,θ, obtained by solving (6) recover the original CEN-generated explanations, θ .

Formally, our result is stated in the following theorem.

Theorem 1.

Let the explanations and the local approximations be in the class of linear models, p(Y = 1 | x, θ) ∝ exp x θ .

Further, let the encoder be L-Lipschitz and pick a sampling distribution, π x,c , that concentrates around the point (x, c), such that p πx,c ( z − z > t) < ε(t), where z := (x, c) and ε(t) → 0 as t → ∞. Then, if the loss function is defined as DISPLAYFORM1 the solution of (6) concentrates around θ as DISPLAYFORM2 Intuitively, by sampling from a distribution sharply concentrated around (x, c), we ensure thatθ will recover θ with high probability.
...
At the same time, CENs naturally let us represent p(Y | X, θ) using arbitrary graphical models.
...
We define CRF-based CEN as: DISPLAYFORM0 Note that here we have explanations for each time point, θ 1:m , and use an RNN-based encoder φ t .

The potentials between attributes, x, and targets, y 1:m , are linear functions parameterized by θ 1:m ; the pairwise potentials between targets, ω(y i , y i+1 ), ensure that configurations (y i = 1, y i+1 = 0) are improbable (i.e., ω(1, 0) = −∞ and ω(0, 0) = ω 00 , ω(0, 1) = ω 01 , ω(1, 1) = ω 10 are learnable parameters).

Given these constraints, the likelihood of an uncensored event at time t ∈ [t j , t j+1 ) is DISPLAYFORM1 and the likelihood of an event censored at time t ∈ [t j , t j+1 ) is DISPLAYFORM2 The joint log-likelihood of the data consists of two parts: (a) the sum over the non-censored instances, for which we compute log p(T = t | x, Θ), and (b) sum over the censored instances, for which we use log p(T ≥ t | x, Θ).
...
As described in the main text, CENs represent the predictive distribution in the following form: DISPLAYFORM0 and the assumed generative process behind the data is either: DISPLAYFORM1 when we model the joint distribution of the explanations, θ, and contexts, C, e.g., using encoder-decoder framework.
...
Such mixture models could be quite powerful as long as the mixing distribution, p(θ | C), is rich enough.
...
The two caveats are:(i) In practice, p(θ | C) is limited, e.g., either deterministic encoding, a finite mixture, or a simple distribution parametrized by a deep network. (ii) The classical setting of predictive mixtures does not separate inputs into two subsets, (C, X).We do this intentionally to produce hypotheses/explanations in terms of specific features that could be useful for interpretability or model diagnostics down the line.
...
To address (i), we consider p(θ | C) that fully factorizes over the dimensions of θ: p(θ | C) = j p(θ j | C), and assume that hypotheses, p(Y | X, θ), factorize according to some underlying graph, DISPLAYFORM2 The following proposition shows that in such case p(Y | X, C) inherits the factorization properties of the hypothesis class.

DISPLAYFORM3 explanations also factorizes according to G. DISPLAYFORM4 , where α denotes subsets of the Y variables and MB(α) stands for the corresponding Markov blankets.

Using the definition of CEN, we have: DISPLAYFORM5 Remark 1.

All the encoding distributions, p(θ | C), considered in the main text of the paper, including delta functions, their mixtures, and encoders parametrized by neural nets fully factorize over the dimensions of θ.
...
Now, consider the case when f is defined by a CEN, instead of x we have (c, x), and the class of approximations, G, coincides with the class of explanations, and hence can be represented by θ.

In this setting, we can pose the same problem as: DISPLAYFORM7 Suppose that CEN produces θ explanation for the context c using a deterministic encoder, φ.

The question is whether and under which conditionsθ can recover θ .
...
Here, we prove Theorem 1 for a little more general class of log-linear explanations: DISPLAYFORM8 where a is a C-Lipschitz vector-valued function whose values have a zero-mean distribution when (x, c) are sampled from π x,c 9 .

For simplicity of the analysis, we consider binary classification and omit the regularization term, Ω(g).

We define the loss function, L(f, θ, π x,c ), as: DISPLAYFORM9 where (x k , c k ) ∼ π x,c and π x,c := π x π c is a distribution concentrated around (x, c).

Without loss of generality, we also drop the bias terms in the linear models and assume that a(x k − x) are centered.

Proof of Theorem 1.

The optimization problem (16) reduces to the least squares linear regression: DISPLAYFORM10 We consider deterministic encoding, p(θ | c) := δ(θ, φ(c)), and hence logit p(Y = 1 | x k − x, c k ) takes the following form: DISPLAYFORM11 To simplify the notation, we denote a k := a(x k − x), φ k := φ(c k ), and φ := φ(c).

The solution of (18) now can be written in a closed form: DISPLAYFORM12 Note thatθ is a random variable since (x k , c k ) are randomly generated from π x,c .

To further simplify the notation, denote M : DISPLAYFORM13 To get a concentration bound on θ − θ , we will use the continuity of φ(·) and a(·), concentration properties of π x,c around (x, c), and some elementary results from random matrix theory.

To be more concrete, since we assumed that π x,c factorizes, we further let π x and π c concentrate such that p πx ( x −x > t) < ε x (t) and p πc ( c −c > t) < ε c (t), respectively, where ε x (t) and ε c (t) both go to 0 as t → ∞, potentially at different rates.
...
Finally, we need a concentration bound on λ min M/(Cτ ) 2 to complete the proof.

Note that DISPLAYFORM19 , where the norm of a kCτ is bounded by 1.

If we denote µ min (Cτ ) the minimal eigenvalue of Cov a k Cτ , we can write the matrix Chernoff inequality BID23 as follows: DISPLAYFORM20 where d is the dimension of a k , α := L C 2 t , and D(a b) denotes the binary information divergence: DISPLAYFORM21 The final concentration bound has the following form: DISPLAYFORM22 We see that as τ → ∞ and t → ∞ all terms on the right hand side vanish, and henceθ concentrates around θ .

Note that as long as µ min (Cτ ) is far from 0, the first term can be made negligibly small by sampling more points around (x, c).

Finally, we set τ ≡ t and denote the right hand side by δ K,L,C (t) that goes to 0 as t → ∞ to recover the statement of the original theorem.

Remark 3.

We have shown thatθ concentrates around θ under mild conditions.

With more assumptions on the sampling distribution, π x,c , (e.g., sub-gaussian) one could derive precise convergence rates.

Note that we are in total control of any assumptions we put on π x,c since precisely that distribution is used for sampling.
...
Note that concentration analysis of a more general case when the loss L is a general convex function and Ω(g) is a decomposable regularizer could be done by using results from the M-estimation theory BID24 ), but would be much more involved and unnecessary for our purposes.

As noted in the main text, to make a prediction, MoE uses each of the K experts where the predictive distribution is computed as follows: DISPLAYFORM0 Since each expert contributes to the predictive probability, we can explain a prediction,ŷ, for the instance (x, c) in terms of the posterior weights assigned to each expert model: DISPLAYFORM1 If the p(k |ŷ, x, c) assigns very high weight to a single expert, we can treat that expert model as an explanation.

Note that however, in general, this may not be the case and posterior weights could be quite spread out (especially, if the number of experts is small and the class of expert models, p(Y | X, θ), is too simple and limited).
...
We can express the evidence for contextual variational autoencoders as follows: DISPLAYFORM0 DISPLAYFORM1 (1) the expected conditional likelihood of the explanation, E qw [log p(Y | X, θ)], (2) the expected context reconstruction error, E qw [log p u (C | θ)], and (3) the KL-based regularization term, −KL (q(θ | C) p(θ)).We can optimize the ELBO using first-order methods by estimating the gradients via Monte Carlo sampling with reparametrization.
...
In particular, sampling from the encoder is as follows: DISPLAYFORM2 The samples, θ, will be logistic normal distributed and are easy to be re-parametrized.

For prior, we use the Dirichlet distribution over γ with the parameter vector α.
...
The derivative of the survival function is called the hazard function, λ(t), which is the instantaneous rate of failure: DISPLAYFORM0 This allows us to model survival on a population level.

Now, proportional hazard models assume that λ is also a function of the available features of a given instance, i.e., λ(t; x).

Cox's proportional hazard model assumes λ θ (t; x) := λ 0 (t) exp(x θ).

Aalen's model is a time-varying extension and assumes that λ θ (t; x) := x θ(t), where θ(t) is a function of time.
...
Each hidden state of the output LSTM was used to generate the corresponding θ t that were further used to construct the log-likelihood for CRF.of binary random variables, Y := (y 1 , . . .
...
Expressions for the likelihoods of censored and non-censored inputs are the same as given in Section 3.3.Finally, CENs additionally take the context variables, C, as inputs and generate θ t for each time step using a recurrent encoder.
...
Each θ t was generated using a constrained deterministic encoder with a global dictionary, D of size 16.
