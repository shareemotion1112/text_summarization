...
Our composition f θ • g Σ combines a free-form f θ with a structured Gaussian g Σ .
...
The structure of a Gaussian is controlled by its covariance Σ, which for a spatial 2D Gaussian is Covariances come in families with progressively richer structure: spherical has one parameter for scale, diagonal has two parameters for scale and aspect, and full has three parameters for scale, aspect, and orientation/slant.

We compose free-form filters f θ and structured Gaussian filters g Σ by convolution * to define a more general family of semi-structured filters than can be learned by either alone.
...
We can keep the diagonal positive by storing its log, hence log-Cholesky, and exponentiating it when forming Σ.Composing with Convolution and Covariance The computation of our composition reduces to convolution, and so it inherits the efficiency of aggressively tuned convolution implementations.
...
Each compositional filter can always be explicitly formed by g Σ * f θ for visualization (see FIG1 ) or other analysis.
