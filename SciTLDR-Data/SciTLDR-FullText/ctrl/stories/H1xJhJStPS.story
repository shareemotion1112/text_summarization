...
We consider the supervised setting, where we want to predict a target y given an input x. The model is a recurrent neural network (RNN) parametrized by θ and evolving according to the dynamics:
...
The number of timesteps T is a hyperparameter that we choose large enough so that s T = s * for the current value of θ.

The goal is to optimize the parameter θ in order to minimize a loss:
...
EP (Scellier and Bengio, 2017 ) is a learning algorithm that computes the gradient of L * in the particular case where the transition function F derives from a scalar function Φ, i.e. with F of the form F (x, s, θ) = ∂Φ ∂s (x, s, θ).

The algorithm consists in two phases (see Alg.

1 of Fig. 1 Scellier and Bengio (2017) have shown that the gradient of the loss L * can be estimated based on the two steady states s * and s β * .
...
In Eq. (5) we have used that s β 0 = s * and s β t → s β * as t → ∞. Here lies the very intuition of continual updates motivating this work; instead of keeping the weights fixed throughout the second phase and updating them at the end of the second phase based on the steady states s * and s β * , as in EP (Alg.

1 of Fig. 1 ), the idea of the C-EP algorithm is to update the weights at each time t of the second phase between two consecutive states s β t−1 and s β t (Alg.
...
In the second phase of C-EP the parameter variable is regarded as another dynamic variable θ t that evolves with time t along with s t .

The dynamics of s t and θ t in the second phase of C-EP depend on the values of the two hyperparameters β (the hyperparameter of influence) and η (the learning rate), therefore we write s β,η t and θ β,η t to show explicitly this dependence.

With now both the neurons and the synapses evolving in the second phase, the dynamic variables s

The difference in C-EP compared to EP is that the value of the parameter used to update s β,η t+1 in Eq. (6) is the current θ β,η t , not θ.

Provided the learning rate η is small enough, i.e. the synapses are slow compared to the neurons, this effect is weak.

Intuitively, in the limit η → 0, the parameter changes are negligible so that θ β,η t can be approximated by its initial value θ β,η 0 = θ.

Under this approximation, the dynamics of s β,η t in C-EP and the dynamics of s β t in EP are the same.

See Fig. 3 for a simple example, and Appendix A.3 for a proof in the general case.

Now we prove that, provided the hyperparameter β and the learning rate η are small enough, the dynamics of the neurons and the weights given by Eq. (6) follow the gradients of BPTT (Theorem 1 and Fig. 2 ).
...
, showing that in the second pĥase of C-EP, neurons and synapses descend the gradients of the loss L obtained with BPTT, with the hyperparameters β and η playing the role of learning rates for s β,η t and θ β,η t , respectively.
...
where σ is an activation function, W is a symmetric weight matrix connecting the layers s and W x is a matrix connecting the input x to the layers s. Although the dynamics are not directly defined in terms of a primitive function, note that s t+1 ≈ ∂Φ ∂s (s t , W ) with Φ(s, W ) = 1 2 s · W · s if we ignore the activation function σ.
...
For C-VF results, the initial angle between forward (θ f ) and backward (θ b ) weights is Ψ(θ f , θ b ) = 0

• .

Right: Test error rate on MNIST achieved by C-VF as a function of the initial Ψ(θ f , θ b ).
...
As seen on Fig. 5 (b) , the finite learning rate η of continual updates leads to ∆ C−EP (β, η, t) curves splitting apart from the −∇ BPTT (t) curves.
...
As an evidence, we show in Appendix F.2 that when we use extremely small learning rates throughout the second phase (θ ← θ + η tiny ∆ C−EP θ ) and rescale up the resulting total weight update (θ ← θ − ∆θ tot + η ηtiny ∆θ tot ), we recover standard EP results.
...
For each C-VF simulations, we tune the initial angle between forward weights (θ f ) and backward weights (θ b ) between 0 and 180

• .

We denote this angle Ψ(θ f , θ b ) -see Appendix F.1 for the angle definition and the angle tuning technique employed.
...
As expected, high angles between gradients of C-VF and BPTT lead to high error rates that can reach 90% for Ψ ∆ C−VF (tot), −∇ BPTT (tot) over 100
...
From standard EP with one hidden layer to C-VF with two hidden layers, the test error increases monotonically with Ψ ∆(tot), −∇ BPTT (tot) but does not exceed 5.05% on average.
...
In the limit of small learning rate, i.e. η → 0, the (normalized) updates of C-EP are equal to those of EP:

Lemma 3 (Equivalence of EP and RBP).

Assume that the transition function derives from a primitive function, i.e. that F is of the form F (x, s, θ) = ∂Φ ∂s (x, s, θ).

Then, in the limit of small hyperparameter β, the normalized updates of EP are equal to the gradients of RBP:
...
First, recall the dynamics of C-EP in the second phase: starting from s β,η 0 = s * and θ β,η 0 = θ we have ∀t ≥ 0 :
...
We want to compute the limits of ∆ C−EP s (β, η, t) and ∆ C−EP θ (β, η, t) as η → 0 with η > 0.

First of all, note that under mild assumptions -which we made here -of regularity on the functions Φ and (e.g. continuous differentiability), for fixed t and β, the quantities s

It follows from Eq. (20) and Eq. (21) that

Now let us compute lim η→0 (η>0) ∆ C−EP s (β, η, t).
...
A consequence of Lemma 2 is that the total update of C-EP matches the total update of EP in the limit of small η, so that we retrieve the standard EP learning rule of Eq. (4).

More explicitly, after K steps in the second phase and starting from θ
...
Eq. (1) rewrites in the form s t+1 = F (x, s t , θ t+1 = θ), where θ t denotes the parameter of the model at time step t, the value θ being shared across all time steps.

This way of rewriting Eq. (1) enables us to define the partial derivative ∂L ∂θt as the sensitivity of the loss L with respect to θ t when θ 1 , . . .

θ t−1 , θ t+1 , . . .

θ T remain fixed (set to the value θ).

With these notations, the gradient ∂L ∂θ reads as the sum:

BPTT computes the 'full' gradient ∂L ∂θ by first computing the partial derivatives ∂L ∂st and ∂L ∂θt iteratively, backward in time, using the chain rule of differentiation.

In this work, we denote the gradients that BPTT computes:

Proposition 5 (Gradients of BPTT).

The gradients ∇ BPTT s (t) and ∇ BPTT θ (t) satisfy the recurrence relationship
...
, s T in order to compute the gradients ∇ BPTT s (t) and ∇ BPTT θ (t) as in Eq. 30-31.

However, in our specific setting with static input x, if the network has reached the steady state s * after T − K steps, i.e. if s T −K = s T −K+1 = · · · = s T −1 = s T = s * , then we see that, in order to compute the first K gradients of BPTT, all one needs to know is ∂F ∂s (x, s * , θ) and ∂F ∂θ (x, s * , θ).
...
Similarly to BPTT, it computes quantities ∇ RBP s (t) and ∇ RBP θ (t), which we call 'gradients of RBP', iteratively for t = 0, 1, 2, . . .

RBP s (t) and ∇ RBP θ (t) are defined and computed iteratively as follows:
...
In BPTT, the state s T −t is required to compute ∂F ∂s (x, s T −t , θ) and ∂F ∂θ (x, s T −t , θ) ; thus it is necessary to store in memory the sequence of states s 1 , s 2 , . . .

, s T .

In contrast, in RBP, only the steady state s * is required to compute ∂F ∂s (x, s * , θ) and ∂F ∂θ (x, s * , θ) ; it is not necessary to store the past states of the network.

In this subsection we motivate the name of 'gradients' for the quantities ∇ RBP s (t) and ∇ RBP θ (t) by proving that they are the gradients of L * in the sense of Proposition 7 below.
...
∇ RBP s (t) and ∇ RBP θ (t) can also be expressed as gradients of L t = (s t , y), the cost after t time steps.
...
The 'RBP gradients' ∇ RBP s (t) and ∇ RBP θ (t) can be expressed as gradients of the projected cost:
...
In order to compute ∂s * ∂θ , we differentiate the steady state condition (Eq. 40) with respect to θ, which yields
...
To illustrate the equivalence of the four algorithms (BPTT, RBP, EP and CEP), we study a simple model with scalar variable s and scalar parameter θ:

where s * is the steady state of the dynamics (it is easy to see that the solution is s * = θ).

The dynamics rewrites s t+1 = F (s t , θ) with the transition function F (s, θ) = 1 2 (s + θ), and the loss rewrites L * = (s * ) with the cost function (s) = 1 2 s 2 .

Furthermore, a primitive function of the system 1 is Φ(s, θ) = 1 4 (s + θ) 2 .
...
Also notice that the steady state of the dynamics is s * = θ.
...
Notice that s β t → θ as β → 0 ; for small values of the hyperparameter β, the trajectory in the second phase is close to the steady state s * = θ.
...
1 The primitive function Φ is determined up to a constant.

First, rearranging the terms in the second equation, we get

It follows that ∆

Therefore, all we need to do is to compute ∆ C−EP s (β, η, t).
...
The Gradient Descending Dynamics property (GDD, Theorem 1) states that, when the system dynamics derive from a primitive function, i.e. when the transition function F is of the form F = ∂Φ ∂s , then the normalized updates of C-EP match the gradients provided by BPTT.

Remarkably, even in the case of the C-VF dynamics that do not derive from a primitive function Φ, Fig. 5 shows that the biologically realistic update rule of C-VF follows well the gradients of BPTT.
...
where σ is an activation function and W is a square weight matrix.

In the second phase, starting from s β,η 0 = s * and W β,η 0 = W , the dynamics read:

Now let us define the transition function F (s, W ) = σ(W · s), so that the dynamics of the first phase rewrites
...
Then, in the limit η → 0 and β → 0, the first K normalized updates of C-VF follow the the first K gradients of BPTT, i.e. ∀t = 0, 1, . . .

, K :

A few remarks need to be made:

Ignoring the factor σ (W · s), we see that if W is symmetric then the Jacobian of F is also symmetric, in which case the conditions of Theorem 9 are met.
...
BPTT s (t) and ∇ BPTT θ (t) satisfy the recurrence relationship

Lemma 10 (Updates of C-VF).

Define the (normalized) neural and weight updates of C-VF in the limit η → 0 and β → 0:
...
The activation function used is σ(x) = tanh(x).
...
Then, we perform on the one hand BPTT over K steps (to compute the gradients ∇ BPTT ), on the other hand C-EP (or C-VF) over K steps for given values of β and η (to compute the normalized updates ∆ C−EP or ∆ C−VF ) and compare the gradients and normalized updates provided by the two algorithms.

Precise values of the hyperparameters , T , K, β and η are given in Tab.
...
According to the definition of ∆ C−EP θ in Eq. (19), for every layer and every t ∈ [0, K]:
...
According to the definition of ∆ C−EP θ in Eq. (19), for every layer W nn+1 and every t ∈ [0, K]:
...
(β, η, t) ∀θ ∈ {W nn+1 } (94) As usual, y denotes the target.
...
∀θ ∈ {W nn+1 } (97) where is a time-discretization parameter and y denotes the target.

According the definition of the C-EP dynamics (Eq. (6)), the definition of ∆ C−EP θ (Eq. (7)) and the explicit form of Φ (Eq. 96), for all time step t ∈ [0, K], we have: Under review as a conference paper at ICLR 2020
...
We add an s or θ index to specify whether we analyse neuron or synapse updates and gradients.

Each C-VF simulation has been realized with an angle between forward and backward weights of 0 degrees (i.e. Ψ(θ f , θ b ) = 0

• ).

For each figure, left panels demonstrate the GDD property with C-EP with η = 0 and the right panels show that, upon using η > 0, dashed and continuous lines start to split appart.
...
Use of a randomized β.

The option 'Random β' appearing in the detailed table of results (Table 3) refers to the following procedure.

During training, instead of using the same β accross mini-batches, we only keep the same absolute value of β and sample its sign from a Bernoulli distribution of probability 1 2 at each mini-batch iteration.
...
In Table 4 .1, we investigate C-VF initialized with different angles between the forward and backward weights -denoted as Ψ in Table 4 .1.

Denoting them respectively θ f and θ b , the angle κ between them is defined here as:

where Tr denotes the trace, i.e. Tr(A) = i A ii for any squared matrix A. To tune arbitrarily well enough κ(θ f , θ b ), the procedure is the following: starting from θ b = θ f , i.e. κ(θ f , θ b ) = 0, we can gradually increase the angle between θ f and θ b by flipping the sign of an arbitrary proportion of components of θ b .

The more components have their sign flipped, the larger is the angle.

More formally, we write θ b in the form θ b = M (p) θ f and we define:

where M (p) is a mask of binary random values {+1, -1} of the same dimension of θ f : M (p) = −1 with probability p and M (p) = +1 with probability 1 −

p. Taking the cosine and the expectation of Eq. (103), we obtain:

Thus, the angle Ψ between θ f and θ f M (p) can be tuned by the choice of p through:

Hyperparameter search for EP.

We distinguish between two kinds of hyperparameters: the recurrent hyperparameters -i.e.

T , K and β -and the learning rates.

A first guess of the recurrent hyperparameters T and β is found by plotting the ∆ C−EP and ∇ BPTT processes associated to synapses and neurons to see qualitatively whether the theorem is approximately satisfied, and by conjointly computing the proportions of synapses whose ∆ C−EP W processes have the same sign as its ∇ BPTT W processes.
...
The typical range of learning rates is [10 −3 , 10 −1 ], [10, 1000] for T , [2, 100] for K and [0.01, 1] for β.
