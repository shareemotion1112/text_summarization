...
dataset D, the target is to model p θ (x) with parameters θ.

A flow, f θ is defined to be an invertible transformation that maps observed data x to the latent variable z. A flow is invertible, so the inverse function T maps z to x, i.e.

The log-likelihood can be expressed as,

where ∂f θ (x) ∂x T is the Jacobian of f θ at x. The invertibile nature of flow allows it to be capable of being composed of other flows of compatible dimensions.

In practice, flows are constructed by composing a series of component flows.

Let the flow f θ be composed of K component flows, i.e.
...
We apply a flow (f θ ) with K component flows to
...
s×s×c , where α, β ∈ {0, · · · , s} and γ ∈ {0, · · · , c} (10) s.t.
...
where σ i (W) are the singular values of the weight matrix W. For affine blocks in Glow, same method as RealNVP is adopted.
