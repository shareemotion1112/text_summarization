...
In particular, the architecture is given by F (x; θ, φ), where φ denotes the last classification layer(s), and θ constitutes all of the "representation" layers.

The source and target networks are thus F (x; θ, φ s ), and F (x; θ, φ t ), respectively.

By assigning a weight α j ∈ [0, 1] to each source example, we define the weighted source loss as: DISPLAYFORM1 is a per example classification loss (e.g. cross-entropy).

The weights α j will allow us to decide which source images are most relevant for the target classification task.
...
In the latter case, the source is the noisy dataset, the target is the clean dataset, and they share a classifier (i.e. φ t = φ s ) as well as a label space; high weights will indicate which source examples are likely to be reliable.
...
Specifically, in the interior level we find the optimal features and source classifier as a function of the weights α, by minimizing the source loss: DISPLAYFORM3 In the exterior level, we minimize the target loss, but only through access to the source weights; that is, we solve: min DISPLAYFORM4 Why might we expect this bilevel formulation to succeed?
...
Specifically, at each iteration, we take a gradient step in the interior level problem (1): DISPLAYFORM5 and Q(θ, φ s ) is a matrix whose j th column is given by DISPLAYFORM6 ).

An identical descent equation exists for the classifier φ s , which we omit for clarity.

Given this iterative version of the interior level of the bilevel optimization, we may now turn to the exterior level.

Plugging Equation (3) into Equation FORMULA4 gives: min α,φ t L t (θ m − λ p Qα, φ t ).

We can then take a gradient descent step of this equation, yielding: DISPLAYFORM7 where we have made use of the fact that λ p is small.
...
FIG0 and 1(c), we choose a threshold of 0.1 on the weights α, and colour the points accordingly.
...
We set: λ p = 10 −4 , target and source batch-sizes: 32, and 256.
...
We set: λ p = 10 −2 , source and the target batch-sizes: 32, 10.
...
DISPLAYFORM0 end while SOSELETO consists of alternating the interior and exterior descent operations, along with the descent equations for the source and target classifiers φ s and φ t .
...
Define the Jacobian ∂α/∂β by DISPLAYFORM1 Then we modify Equation (4) to read DISPLAYFORM2 The Jacobian is easy to compute analytically: DISPLAYFORM3 Due to this very simple form, it is easy to see that β m will never lie outside of [0, 1]; and thus that α m = β m for all time.
...
However, it is clear that if we set the learning rate λ α sufficiently large, the second term will eventually dominate the first term, and the target loss will be decreased.

Indeed, we can do a slightly finer analysis.

Ignoring the final term (which is always negative), and setting v = Q T ∂Lt ∂θ , we have that DISPLAYFORM0 where in the second line we have used the fact that all elements of α are in [0, 1]; and in the third line, we have used a standard bound on the L 1 norm of a vector.
...
We perform further analysis by examining the α-values that SOSELETO chooses on convergence, see FIG3 .
