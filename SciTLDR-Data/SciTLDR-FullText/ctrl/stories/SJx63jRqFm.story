...
Third, while the discriminator in Gregor et al. (2016) while not converged do Sample skill z ∼ p(z) and initial state s0 ∼ p0(s) for t ← 1 to steps_per_episode do Sample action at ∼ π θ (at | st, z) from skill.

Step environment: st+1 ∼ p(st+1 | st, at).

Compute q φ (z | st+1) with discriminator.

Set skill reward rt = log q φ (z | st+1) − log p(z) Update policy (θ) to maximize rt with SAC.

Update discriminator (φ) with SGD.Figure 1: DIAYN Algorithm: We update the discriminator to better predict the skill, and update the skill to visit diverse states that make it more discriminable.the final state, our discriminator looks at every state, which provides additional reward signal.
...
In summary, we maximize the following objective with respect to our policy parameters, θ: DISPLAYFORM0 We rearranged our objective in Equation 2 to give intuition on how we optimize it.
...
As we cannot integrate over all states and skills to compute p(z | s) exactly, we approximate this posterior with a learned discriminator q φ (z | s).

Jensen's Inequality tells us that replacing p(z | s) with q φ (z | s) gives us a variational lower bound G(θ, φ) on our objective F(θ) (see BID1 for a detailed derivation): DISPLAYFORM1

We implement DIAYN with soft actor critic (SAC) (Haarnoja et al., 2018) , learning a policy π θ (a | s, z) that is conditioned on the latent variable z. Soft actor critic maximizes the policy's entropy over actions, which takes care of the entropy term in our objective G. Following Haarnoja et al. FORMULA0 , we scale the entropy regularizer H[a | s, z] by α.

We found empirically that an α = 0.1 provided a good trade-off between exploration and discriminability.
...
In this case, the discriminator maximizes E[log q φ (z | f (s))].
...
We consider the setting where we are given an expert trajectory consisting of states, without actions, defined as τ * = {(s i )} 1≤i≤N .
...
The log p(z) term in Equation 3 is a baseline that does not depend on the policy parameters θ, so one might be tempted to remove it from the objective.
...
Second, assuming our discriminator q φ is better than chance, we see that q φ (z | s) ≥ p(z).
...
To clarify, we will use π z to refer to the policy for skill z.

We use ρ πz to indicate skill z's stationary distribution over states, andρ πz as the empirical distribution over states within a single episode.

Our assumption is equivalent to saying ρ πz (s) =ρ πz (s) ∀s ∈ S One way to ensure this is to assume infinite-length episodes.
...
A pair of skills with state distributions given below (and shown in FIG8 ) are an optimum for the DIAYN objective with no entropy regularization (α = 0).
...
We found that learning was most stable when we scaled the maximum entropy objective (H[A | S, Z] in Eq. 1) by α = 0.1.
...
Note that log 2 (1/20) ≈ −3, setting a lower bound for log q φ (z | s).
...
We vary the entropy regularization α, with larger values of α corresponding to policies with more stochastic actions.

With small α, we learn skills that move large distances in different directions but fail to explore large parts of the state space.

Increasing α makes the skills visit a more diverse set of states, which may help with exploration in complex state spaces.

It is difficult to discriminate skills when α is further increased.
...
Given the expert trajectory, we use our learned discriminator to estimate which skill was most likely to have generated the trajectory:ẑ = arg max z Π st∈τ * q φ (z | s t )As motivation for this optimization problem, note that each skill induces a distribution over states, p z p(s | z).

We use p * to denote the distribution over states for the expert policy.

With a fixed prior distribution p(z) and a perfect discriminator q φ (z | s) = p(z | s), we have p(s | z) ∝ q φ (z | s) as a function of z. Thus, Equation G is an M-projection of the expert distribution over states onto the family of distributions over states, P = {p z }:arg min DISPLAYFORM0 For clarity, we omit a constant that depends only on p * .
...
For each expert trajectory, we retrieve the closest DIAYN skillẑ using Equation 4.2.3.

Evaluating q φ (ẑ | τ * ) gives us an estimate of the probability that the imitation will match the expert (e.g., for a safety critical setting).
