...
DISPLAYFORM0 Given these assumptions and a Gaussian initial belief, the belief can be represented by the mean µ and covariance matrix Σ over the estimate.

At each timestep, the filter predictsμ andΣ given the process model.
...
It uses the non-linear models for predictingμ and the corresponding observationsẑ (Equations 1, 5).

For computing the prediction and update of Σ and K, these models are linearized around the current mean of the state and the Jacobians F |µt and H |µt replace A and H in Equations 2 -4 and 7.
...
In practice, we found λ difficult to tune since placing the sigma points too far from the mean increases prediction uncertainty and can even destabilize the filter.

Simon J. Julier (1997) suggested to chose λ such that λ + n = 3.

This however results in negative values of λ if n > 3, for which the estimated covariance matrix is not guaranteed to be positive semidefinite anymore BID21 .
...
However, as described above, finding the correct scaling parameter λ can be difficult, especially if the state is high dimensional.

Instead of relying on the unscented transform to calculate the mean and covariance of the state at the next timestep, we can also resort to Monte Carlo methods, as proposed by Wüthrich et al. (2016) .
...
The main idea of variational inference is to approximate the intractable posterior distribution p θ (x|z) by an approximate distribution q φ (x|z) that is easier to compute, e.g. because it factorizes over variables.

The parameters θ of the true generative distribution p (i.e. the model parameters) can be optimized jointly with the parameters φ of the approximate distribution q by maximizing the evidence lower bound (ELBO).
...
In all variants of the Kalman Filter, the process noise enters the prediction step in the update of the covariance matrix Σ (Equations 2, 12) and influences the update step through the Kalman Gain (Equation 4).
...
DISPLAYFORM1 Here l 0...T is the ground truth state sequence, µ 0...T and Σ 0...T denote the sequence of prediction mean and covariance respectively.

w contains the weights of the trainable noise models (which influence the prediction of µ and Σ) like w o or w g .

The λ i are scaling factors that can be chosen dependent on the magnitude of the loss components.
...
The state is 5-dimensional and includes the position p and orientation θ of the car as well as the current linear and angular velocity v andθ.

As the control inputs are unknown, the estimated velocities are predicted by sampling random accelerations a,θ, according to the process noise for v andθ.
...
It predicts the linear and angular velocity of the object (v, ω) given the pusher velocity u and the current state.
...
Besides from the friction-related parameters, the orientation of the object, θ t , is the only state component that cannot be estimated directly from the input images.
...
Instead, we train the network to observe the change in orientation ω between the two images (up to symmetries).In contrast to the visual odometry task in the previous experiment, we do not assume that the initial state is correct.
...
For constructing the sigma points, it is necessary to compute the matrix square root of the estimated covariance Σ. This is commonly done using the Cholesky Decomposition, which is also available in tensorflow.
...
Internally, this also relies on the Cholesky decomposition and thus requires Σ to be positive semidefinite at all times.7.1.3 PF Our particle Filter implementation is very similar to the variant proposed by BID9 that is available online 1 .
...
The process model for the visual odometry task is defined as p x p y t = p x p y t−1 + ∆tv t−1 sin(θ t−1 ) cos(θ t−1 ) v t = v t−1 + ∆ta t θ t = θ t−1 + ∆tθ t−1θt =θ t−1 + ∆tθ tFor the architecture of the preprocessing network, we refer to BID6 .
...
We add the left part that computes ω, the difference in object rotation between the current and the previous image.

For this, we extract patches around the predicted object position in both images and feed both into a convolutional and fully-connected network to infer ω.

Process Model Given the output of the analytical model (v t , ω t ), we formulate the process model f (x t , u t ) as p t+1 = p t + v t r t+1 = r t + u t θ t+1 = θ t + ω t n t+1 = R(ω t )n t l t+1 = l t s t+1 = s t m t+1 = m t Here, we make the simplifying assumption that the pusher will not make or break contact and that s is thus constant.
...
Given this assumption, the only thing we need to do is to adapt the orientation of the normal to the rotation of the object, where R(ω t ) denotes a rotation matrix that rotates n by ω t .
