...
However, a machine-learned model will typically only approach those properties to a certain level, so the usual relaxations are made:Property 1 (Partial soundness) A model M is partially sound with precision α iff., for a proportion α of the pairs of words w, w such that w ⊆

M w holds, w ⊆ w holds as well.

Property 2 (Partial completeness) A model M is partially complete with recall α iff., for a proportion α of the pairs of words w, w such that w ⊆ w holds, then w ⊆ M w holds as well.
...
DISPLAYFORM2 However, now, we can define a relaxed inclusion relation, corresponding to a proportion of ρ of p included in q:Property 4 (relaxed space-inclusion compatibility) There exists P : DISPLAYFORM3 In the following, we call ρ the relaxation factor.
...
To begin, hyperplanes defined by θ and b will (stochastically) always intersect therefore one must take into account the actual density of the fastText embeddings.
...
That is, we define the following quantity DISPLAYFORM0 which is the proportion of elements of w that are found in w. This value corresponds to the relaxation parameter ρ in Property 4.If w ⊆ w holds, then we want Q(w , w ) to be close to 1, and close to 0 otherwise.
...
Thus, the exact choice of ρ has little influence on accuracy for the model.

For ρ = 0.5, the recall is 88.8%.
...
BID10 makes use of the Poincaré ball model, which has been found to be useful for hosting vectorial embeddings of complex networks BID5 , and especially tree-like structures.
...
So, each word w is associated a vector θ(w), and P (w, x) = δ ( i x i ≤ θ(w) i ).
