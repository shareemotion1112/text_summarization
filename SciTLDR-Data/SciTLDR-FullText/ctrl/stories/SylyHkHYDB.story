...
The RL agent maintains a policy π(a|s), which defines the probability of taking an action in state s given there are t s steps remaining till the total time budget is exhausted.

A policy π generates rollouts τ π .

A rollout τ π is defined as a sequence of states, actions and rewards obtained from the MDP τ π = (s 1 , a 1 , r 1 , s 2 , a 2 , r 2 , . . .

s H , r H ).

Since the policy π can be stochastic, it is modelled as a random variable.

The goal of RL agent is to find an optimal policy π = arg max π J π , which attains the best possible return.

The return is defined as

R(s t , s t+1 ) .

Given a policy and the number of timesteps t remaining till the end of episode, we define a value function V (s) = E τ ts−1 i=0 R(s i , s i+1 ) s 0 = s , where t s denotes the remaining time budget at state s.

The optimal value function V is defined as the value function of an optimal policy π .
...
This formulation ensures that return J π equals the total cost reduction attained along the length of the rollout τ .
...
H are all one-layer dense networks, σ denotes the sigmoid function, and
...
Thus the value function estimatê V (s) can be obtained from per-step value estimateV (s) asV (s) = t s ·V (s).
...
In the program obtained by ksc (middle listing), three temporary variables mat x, mat x 6, and mat y corresponding to the result of A·B, α·mat x, and β ·C, respectively, are created.
...
In addition to A search, we compare the performance of Monte Carlo Tree Search (Browne et al., 2012) using the UCT formula (Auer et al., 2002; Kocsis & Szepesvári, 2006) .
...
The tree policy π t and rollout policy π r are defined as follows.

π t (a|s) = arg max a∈A X(s ) n(s ) + β ln n(s) n(s )+1

(6) π r (a|s) = softmax a∈A (R(s, a) + V (s ), α)
...
For MCTS, we used α = 5.0 and β = 0.5 for both training and test.
