...
, T we observe observation x t and seek to predict its labelŷ t using x t and possibly any information we had obtained prior to step t.

Partially: after we make a predictionŷ t , the environment may not provide feedback (we will use -1 to encode its absence) and we must proceed to step t + 1 without knowledge of the true y t .Rewarded: suppose there are K possible labels y t ∈ {1, . . .

, K}. The environment at step t will not provide true y t , but instead a response h t ∈ {−1, 0, 1}, where h t = 0 indicatesŷ t = y t and h t = 1 indicatesŷ t = y t (-1 indicates missing response).

UpdateÂ with new edges using x t

Update GCN weights W DISPLAYFORM0 Retrieve GCN embeddings g(X) (k) 8: DISPLAYFORM1 (k) t 10: DISPLAYFORM2 (k) t

Predictŷ t = argmax k (µ k + σ k ) and observe h t 14: DISPLAYFORM0 ∀k, y DISPLAYFORM1 Append t to each C k and 1 to r ·,k ifŷ t = k and 0 otherwise DISPLAYFORM2 Append t to Cŷ t , output ofŷ t -th GCN to r ·,ŷtRewarded Online GCN (ROGCN) is a natural extension of GCN, adapted to the online, partially rewarded setting along with a potential absence of true graph information.
...
To take advantage of this for K > 2, we propose to use a suite of class specific GCNs, where the hidden layer representation from the k-th class GCN, i.e. g(X)(k) =Â ReLU(ÂXW (k) 1 ) and g(X) (k) t denotes the embedding of observation x t , is used as context by the contextutal bandit for the predictions of the k-th arm.
...
Therefore instead of dynamically updating bandit parameters, we maintain a set of indices for each of the arms C k = {t :ŷ t = k or h t = 1} and use observations and responses from only these indices to update the corresponding bandit parameters.
...
As it was done by Defferrard et al. (2016) we set σ = A ij is the diagonal matrix of node degrees.
...
, T each algorithm is given a feature vector x t and is ought to make a predictionŷ t .

The environment response h t ∈ {−1, 0, 1} is then observed and algorithms moves onto step t + 1.

To compare performance of different algorithms at each step t we compareŷ t to true label y t available from the dataset (but concealed from the algorithms themselves) to evaluate running accuracy.

DISPLAYFORM1 For GCNUCB we use baseline LINUCB for first 300 steps, and for both we use explorationexploitation trade-off parameter α = 0.25.
...
To see how this procedure improves upon the baseline LINUCB utilizing input features as context, we project the context and the corresponding bandit weight vectors, θ 1 , . . .

, θ K , for both LINUCB and GCNUCB to a 2-dimensional space using t-SNE BID11 .

In this experiment we analyzed CNAE-9 dataset with 25% missing labels.

Recall that the bandit makes prediction based on the upper confidence bound of the regret: argmax k (θ k x k,t + σ k ) and that x k,t = x t ∀k = 1, . . .

, K for LINUCB and x k,t = g(X) (k) t for GCNUCB.

To better visualize the quality of the learned weight vectors, for this experiment we set α = 0 and hence σ k = 0 resulting in a greedy bandit, always selecting an arm maximizing expected reward θ k x t,k .
