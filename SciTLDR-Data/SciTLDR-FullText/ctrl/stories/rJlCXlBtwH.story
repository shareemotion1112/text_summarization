...
where 0 ≤ γ ≤ 1 is the discount factor and r t is the reward at time-step t.

In terms of a policy π : S → A, the value function can be given by Bellman equation as:

Using Bellman's optimality equation, we can define, for any MDP, a policy π is greater than or equal to any other policy π if value function v π (s t ) ≤ v π (s t ) for all s t ∈ S. This policy is known as an optimal policy (π * ) and its value function is known as optimal value function (v * ).
...
Considering the expert χ is executing an MDP M = (S, A, p, γ), the reward for χ is assumed to be sampled from a prior (known) distribution P R defined as:
...
The likelihood thus defines our confidence in χ's ability to select the optimal action.

This is modeled as a exponential distribution for the likelihood of trajectory τ with Q * as:

where α χ is a parameter representing the degree of confidence in χ's ability.
...
For a family of approximate densities ζ over the latent variables, we try to find a member of the family that minimizes the Kullback-Leibler (KL) divergence to the exact posterior
...
Given the current state and action, the decoder maps the latent space into the state space and reconstructs the next stateŝ t+1 from a sampled r t (from the normal distribution).
