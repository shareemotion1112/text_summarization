...
We consider a discrete-time Markov decision process (MDP), defined by (S, A, P, ρ 0 , r, γ), where S ⊆ R n is a set of n-dimensional states, A ⊆ R m is a set of m-dimensional actions, P : S × A × S → [0, 1] is the state transition probability distribution, ρ 0 : S → [0, 1] is the distribution over initial states, r : S × A → R is the reward function, and γ ∈ (0, 1) is the discount factor.

We denote a stochastic policy as π : S × A → [0, 1], which aims to optimize the expected return η(π) = E τ [ ∞ t=0 γ t r(s t , a t )], where τ = (s 0 , a 0 , ...) is the trajectory following s 0 ∼ ρ 0 , a t ∼ π(a t |s t ), s t+1 ∼ P(s t+1 |s t , a t ).

We use V π (s t ) = E at,st+1,at+1,... ∞ l=0 γ l r(s t+l , a r+l )|s t to define the value function, and Q π (s t , a t ) = E st+1,at+1,... ∞ l=0 γ l r(s t+l , a r+l )|s t , a t to define the state-action value function.
...
Practical algorithms often use the undiscounted state visitation frequency (i.e., γ = 1 in ρ π ), which can make the estimation slightly biased BID12 .Estimating the policy gradient using Monte Carlo estimation for the Q function suffers from high variance (Mnih et al., 2016) .
...
The policy gradient estimation with a baseline in Equation FORMULA0 DISPLAYFORM2 While an optimal baseline exists (Greensmith et al., 2004; BID20 , it is hard to estimate and often replaced by the value function b(s t ) = V π (s t ) BID9 Mnih et al., 2016) .
...
The reward corresponding to this goal is r t = −τ × j, where τ is the time elapsed since the last action and j is total number of enqueued jobs.
...
An input-driven MDP is defined by (S, A, Z, P s , P z , ρ s 0 , ρ z 0 , r, γ), where Z ⊆ R k is a set of k-dimensional input values, P s (s t+1 |s t , a t , z t ) is the transition kernel of the states, P z (z t+1 |z 0:t ) is the transition kernel of the input process, ρ z 0 (z 0 ) is the distribution of the initial input, r(s t , a t , z t ) is the reward function, and S, A, ρ s 0 , γ follow the standard definition in §2.
...
We focus on two cases, corresponding to the graphical models shown in FIG3 : Case 1: z t is a Markov process, and ω t = (s t , z t ) is observed at time t. The action a t can hence depend on both s t and z t .

Case 2: z t is a general process (not necessarily Markov), and ω t = s t is observed at time t.

The action a t hence depends only on s t .In Appendix B, we prove that case 1 corresponds to a fully-observable MDP.

This is evident from the graphical model in FIG3 by considering ω t = (s t , z t ) to be the 'state' of the MDP at time t. Case 2, on the other hand, corresponds to a partially-observed MDP (POMDP) if we define the state to contain both s t and z 0:t , but leave z 0:t unobserved at time t (see Appendix B for details).
...
We propose to use an input-dependent baseline of the form b(ω t , z t:∞ ) -a function of both the observation at time t and the input sequence from t onwards.
...
The first lemma shows that under the input-driven MDP definition, the input sequence z t:∞ is conditionally independent of the action a t given the observation ω t , while the second lemma states the policy gradient theorem for input-driven MDPs.

Lemma 1.

Pr(z t:∞ , a t |ω t ) = Pr(z t:∞ |ω t )π θ (a t |ω t ), i.e., z t:∞ − ω t − a t forms a Markov chain.

Proof.

See Appendix C. Lemma 2.

For an input-driven MDP, the policy gradient theorem can be rewritten as DISPLAYFORM0 where DISPLAYFORM1 denotes the discounted visitation frequency of the observation ω and input sequence z, and Q(ω, a, z) = E ∞ l=0 γ l r t+l ω t = ω, a t = a, z t:∞ = z .Proof.

See Appendix D.Equation FORMULA3 generalizes the standard Policy Gradient Theorem in Equation (1).

ρ π (ω, z) can be thought of as a joint distribution over observations and input sequences.

Q(ω, a, z) is a "state-actioninput" value function, i.e., the expected return when taking action a after observing ω, with input sequence z from that step onwards.

The key ingredient in the proof of Lemma 2 is the conditional independence of the input process z t:∞ and the action a t given the observation ω t (Lemma 1).Theorem 1.

An input-dependent baseline does not bias the policy gradient.

Proof.

Using Lemma 2, we need to show: DISPLAYFORM2 We have: DISPLAYFORM3 Since a π θ (a|ω)∇ θ log π θ (a|ω) = a ∇ θ π θ (a|ω) = ∇ θ a π θ (a|ω) = 0, the theorem follows.
...
See Appendix E.Operationally, for observation ω t at each step t, the input-dependent baseline takes the form b(ω t , z t:∞ ).

In practice, we use a simpler alternative to Equation FORMULA7 : DISPLAYFORM5 This can be thought of as a value function V (ω t , z t:∞ ) that provides the expected return given observation ω t and input sequence z t:∞ from that step onwards.
...
A straightforward way to learn b(ω t , z t:∞ ) for different input instantiations z is to train one value network to each particular instantiation of the input process.
...
To learn a separate baseline function for each input sequence, we use N value networks with independent parameters θ V1 , θ V2 , · · · , θ V N , and single policy network with parameter θ.

During training, we randomly sample an input sequence z i , execute a rollout based on z i with the current policy π θ , and use the (state, action, reward) data to train the value network parameter θ Vi and the policy network parameter θ (details in Appendix I).Meta-learning approach.
...
Require: α, β: meta value network step size hyperparameters 1: Initialize policy network parameters θ and meta-value-network parameters θ V 2: while not done do DISPLAYFORM0 Generate a new input sequence z 4:Sample k rollouts T 1 , T 2 , ..., T k using policy π θ and input sequence z

Adapt θ V with the first k/2 rollouts: DISPLAYFORM0 Estimate baseline value DISPLAYFORM1 Adapt θ V with the second k/2 rollouts: DISPLAYFORM2 Update policy with Equation (2) using the values from line (6) and (8) as baseline 10:Update meta value network: DISPLAYFORM3 The pseudocode in Algorithm 1 depicts the training algorithm.
...
BID6 replace the Monte Carlo return with a λ-weighted return estimation (similar to TD(λ) with value function bootstrap BID11 ), improving performance in high-dimensional control tasks.
...
For simplicity, let the policy be parametrized as π θ (a t = +1|s t ) = e θ /(1+e θ ), with θ initialized to 0 at the start of training.
...
This is because E[ DISPLAYFORM0 γ t (a t + z t )] = 0 since both actions a t and inputs z t are i.i.d.

with mean 0.

Also note that ∇ θ log π θ (a t = +1) = 1/2 and ∇ θ log π θ (a t = −1) = −1/2; hence ∇ θ log π θ (a t ) = a t /2.
...
An input-driven decision process satisfying the conditions of case 1 in FIG3 is a fully observable MDP, with states t := (s t , z t ), and actionã t := a t .Proof.

Pr(s t+1 |s 0:t ,ã 0:t ) = Pr(s t+1 , z t+1 |s 0:t , z 0:t , a 0:t ) = Pr(s t+1 , z t+1 |s t , z t , a t ) (by definition of case 1 in FIG3 ) = Pr(s t+1 |s t ,ã t ).Proposition 2.

An input-driven decision process satisfying the conditions of case 2 in FIG3 , with states t := (s t , z 0:t ) and actionã t := a t is a fully observable MDP.

If only ω t = s t is observed at time t, it is a partially observable MDP (POMDP).Proof.

Pr(s t+1 |s 0:t ,ã 0:t ) = Pr(s t+1 , z 0:t+1 |s 0:t , z 0:t , a 0:t ) = Pr(s t+1 |s 0:t , z 0:t+1 , a 0:t ) Pr(z 0:t+1 |s 0:t , z 0:t , a 0:t ) = Pr(s t+1 |s t , z 0:t+1 , a t ) Pr(z 0:t+1 |s t , z 0:t , a t ) (by definition of case 2 in FIG3 ) = Pr(s t+1 , z 0:t+1 |s t , z 0:t , a t ) = Pr(s t+1 |s t ,ã t ).Therefore, (s t ,ã t ) is a fully observable MDP.

If only ω t = s t is observed, the decision process is a POMDP, since the z 0:t component of the state is not observed.

Proof.

From the definition of an input-driven MDP (Definition 1), we have Pr(z 0:∞ , ω t , a t ) = Pr(z 0:t , ω t , a t ) Pr(z t+1:∞ |z 0:t , ω t , a t ) = Pr(z 0:t , ω t ) Pr(a t |z 0:t , ω t ) Pr(z t+1:∞ |z 0:t ) = Pr(z 0:t , ω t )π θ (a t |ω t ) Pr(z t+1:∞ |z 0:t ) = Pr(z 0:∞ , ω t )π θ (a t |ω t ).Notice that Pr(a t |z 0:t , ω t ) = π θ (a t |ω t ) in both the MDP and POMDP cases in FIG3 .
...
Using the definition of Q(ω, a, z), we obtain: DISPLAYFORM2 E PROOF OF THEOREM 2 DISPLAYFORM3 For any input-dependent baseline b(ω, z), the variance of the policy gradient estimate is given by DISPLAYFORM4 Notice that the baseline is only involved in the last term in a quadratic form, where the second order term is positive.

To minimize the variance, we set baseline to the minimizer of the quadratic equation, i.e., 2E a∼π θ G(ω, a) ω, z b(ω, z) − 2E a∼π θ G(ω, a)Q(ω, a, z) ω, z = 0 and hence the result follows.
...
TRPO maximizes a surrogate objective, subject to a KL divergence constraint: DISPLAYFORM1 subject to DISPLAYFORM2 in which δ serves as a step size for policy update.
...
An input-dependent baseline does not change the optimal solution of the optimization problem in TRPO, that is argmax θ E (ω,z)∼ρold,a∼πold DISPLAYFORM6 Proof.

DISPLAYFORM7 which is independent of θ.

Therefore, b(ω, z) does not change the optimal solution to the optimization problem.
...
Require: pregenerated input seuqnces {z 1 , z 2 , · · · , z N }, step sizes α, β 1: Initialize value network parameters θ V1 , θ V1 , · · · , θ V N and policy parameters θ 2: while not done do Sample k rollouts T 1 , T 2 , ..., T k using policy π θ and input sequence z i

Update policy with Equation (2) using baseline estimated with θ Vi
...
Similar to §3, the job sizes follow a Pareto distribution (scale x m = 100, shape α = 1.5), and jobs arrive in a Poisson process (λ = 55).
...
We use γ = 0.995 for both environments.
...
We turned off the GAE enhancement by using λ = 1 for fair comparison.

We found that it makes only a small performance difference (within ±5% using λ = {0.95, 0.96, 0.97, 0.98, 0.99, 1}) in our environments.

We use γ = 0.99 for all three environments.
...
The KL divergence constraint δ is 0.01.
