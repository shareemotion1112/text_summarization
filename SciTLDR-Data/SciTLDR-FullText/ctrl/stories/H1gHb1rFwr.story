...
Given an input image I and Gaussian kernel G(x, y, σ) as below

where σ denotes the variance.

Also, difference of Gaussian (DoG) is defined as

where ⊗ is the convolution operation, and I 1 = G(x, y, σ) ⊗ I 0 .
...
Projected gradient descent (PGD) iteratively applies FGSM with a small step size α i (Kurakin et al., 2017a; Madry et al., 2018) with formulation as below:

where i is the iteration number, α = /T with T being the number of iterations. '
...
Recall DoG in Equation 2, it repeatedly convolves input images with the same Gaussian kernel in which kernel size σ is designable, and then computes the differences for adjacent Gaussian blurred images.
...
Note that the learnable convolution kernel is not required to be symmetric since some recent evidence shows that non-symmetric DoG may perform even better (Einevoll & Plesser, 2012; Winnemöller, 2011) .
...
where θ is a learnable truncated parameter.

Note that this function is discontinued at x = θ and x = −θ.
...
Different from the original ReLU, tReLU introduces a threshold parameter θ and keeps elements with higher magnitude.

θ can be either a block-level parameter (each block has one global threshold) or a channel-level parameter (each channel holds a separate threshold).

By default, we take θ as a block-level parameter.
...
(3) To eliminate contrast or scale impact, we normalize v to obtainṽ = v/ v p , while · p means the p norm.

the normalized vectorṽ is fed into classification layer for prediction purpose.
...
The EVPConv block introduces very few additional parameters: w for shared depth-wise convolution, θ for tReLU and parameters for SE module.

Note that we allow each EVPConv block having its own w and θ.
...
For tReLU, the channel-level parameter θ is initialized by uniformly sampling from [0, 1].
...
Specifically, we feed both benign examples and adversarial examples into the evaluated networks, and compute the normalized L 2 distance for each res-block outputs as γ = x − x 2 / x 2 , where x is the response vector of benign example, and x is the response vector for the adversarial example.
