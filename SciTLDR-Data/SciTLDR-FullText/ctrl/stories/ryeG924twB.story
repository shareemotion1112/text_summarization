...
Our research focuses on single-leader multi-follower Stackelberg Markov Games (SMG) (Mguni et al., 2019; Sabbadin & Viet, 2013) , which can be formulated as a tuple G = N , S, A, Ω, P, R, γ .
...
ω ∈ Ω = × k∈N Ω k is an action for the leader and

k is a goal and a bonus that the leader assigns to the k-th follower.

P : S × A → ∆(S) is the transition function 1 and R = × k∈N r k × r l is the reward function set where r k : S × A × Ω → R is the reward function for the k-th follower and r l : S × A × Ω → R is the reward function for the leader.

γ is the discount factor and a is a joint action of followers.

The leader's policy is defined as µ = µ k k∈N where µ k : Ω × S → ∆(Ω k ) is the leader's action to the k-th follower given the leader's action in the previous timestep ω t−1 and the current state s t .

∆(·) is a probability distribution.

The followers' joint policy is defined as π = π k where

is the k-th follower policy given the leader's action ω k t and the current state s t .

Given the policy profile of the leader and followers µ, π , the follower's utility is defined as at, ωt) and the leader's utility is J(µ, π) = E T t=0 γ t r l t (st, at, ωt) .

We assume that the leader and followers aim to maximize their own utilities.

We define the trajectory τ as a sequence of state, leader's action, and followers' actions ω −1 , (s t , a t , ω t ) T t=0 where ω −1 is the first step leader's action and is set to zero.
...
Dotted line means that β affects the final result of ω indirectly.

ω−1 is set to be zero.
...
where µ is the leader's policy as we defined above and β k (s t , ω t−1 ) : S × Ω → [0, 1] is the termination function for the k-th follower, to indicate the probability whether the leader's action to the k-th follower changes (ω k t−1 = ω k t ).

Based on these definitions, we formulate the one-step option-state transition function with decay as: (Bacon et al., 2017) .

Differently, we do not have the low-level policy here (the low-level policy is the follower) and since we only focus on the finite time horizon, γ is set to be 1.
...
We use the notation e k i = t i , ω k ti to represent the leader's action to the k-th follower at step t i , i is the index of the event.

Since we focus on the change of the actions from the leader, we further define a set that represents a collection of new actions (ω k t = ω k t−1 ) taken by the leader within that trajectory:
...
Similarly, we can further define the probability of the whole trajectory τ as:

Comparing with P (A T ), P (τ ) includes the probability of the followers as well as the state transition.

Do note that our goal is to maximize max

, indicating that the leader is required to select an action that can maximize the accumulated reward, where R τ (T ) = T t=0 γ t r l t is the accumulated reward and τ is to stress that its accumulated reward is from the trajectory τ .
...
The policy gradients for the termination function β k (s ti , ω ti ) and leader's policy function µ k ω k ti |s ti , ω ti−1 can be written as:

where θ and ϑ are the parameters for the termination function β k θ and leader's policy µ k ϑ .
...
π −k indicates the joint policy without the k-th agent's.
...
If both the assumptions of game regularization and policy bound are satisfied, for ∀ > 0, k ∈ N , there exists δ > 0, such that |µ − µ | ≤ implies π k − π k ≤ δ, where µ and π k are the new policies for the leader and the k-th follower respectively.
...
π k meta (z|ŝ) is the meta policy for the k-th follower and z is the high-level (meta) action.

We hypothesize that the lower-level policy (the policy to choose the primitive actions) is already known (rule-based) and deterministic, i.e., π k lower (a k |ŝ, z) = 1.

For instance, given the example of the navigation task, the π k meta can be the selection to which landmark to explore while π k lower is a specific route planning algorithm (such as Dijkstra Algorithm).

Based on this assumption, we can design a novel policy gradient to train the meta policy:

where λ k is the parameter for meta-policy π k meta (Details can be found in Lemma 3).
...
Following the same logic of (Shu & Tian, 2019) , two baseline functions φ g (c t ) and φ b (c t ) are also introduced to further reduce the variance.
...
For the loss function, we set the λ 1 = 0.01 and λ 2 = 0.001.
...
To encourage exploration, we use the ι-greedy 2 .
...
where θ and ϑ are the parameters for the termination function β k θ and the leader's policy µ k ϑ .
...
For brevity, we use P (τ ) to represent P (s 0 ) k∈N
...
For any agent k, the corresponding reward function r k w.r.t ω is CLipschitz continuous.

Where C = (1 − γ) −1 C.The last equation is drawn form the Assumption 1 and the inequality of a geometric series: Proof.

By combining Lemma 2 and Assumption 2, we can draw that:

If there exist |ω − ω | < and we have:

And we set δ ≥ (1 − γ)C, the consistency is established.

Lemma 3.

(Action Abstraction Policy Gradient.)

Under the assumption that the low-level follower policy π When π k lower (a|ŝ, z) is fixed and deterministic, the equation can be:

lower (a t |ŝ t , z t ) = 1 is the partition function.
...
We adopt the idea of successor representation (Rabinowitz et al., 2018; Shu & Tian, 2019) as two expected baseline functions:φ g (c t ) andφ b (c t ).

For the gain baseline function:

For the bonus-based baseline function:

Two baseline neural network functions with parametersθ g andθ b are trained through minimizing the mean square error:
...
We can find that the performance where the followers' learning rate α (1 × 10 −3 ) is much larger than the leader's β (3 × 10 −4 ) is better than the performance where the leader's learning rate is similar to the followers (1 × 10 −3 ).
