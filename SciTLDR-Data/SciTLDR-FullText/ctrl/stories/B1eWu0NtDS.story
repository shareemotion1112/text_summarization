...
Formally, a characteristic function ν: 2 N → R maps each coalition (subset) C ⊆ N to a real number ν(C).

Therefore, a coalitional game is defined by a tuple (N, ν), where N is a set of players and ν is a function that assigns payoffs to every coalition of N .
...
As a result, we choose the accuracy on a validation set as the characteristic function, that is, ν(C) = acc(C) and ν(N ) = acc(N ).
...
Formally, let π denote a permutation, π(i) a place of the neuron n i in the permutation π, and C π (i) the coalition formed by the predecessors of n i such that C π (i) = {n j ∈ π : π(j) before π(i)}. For example, in the permutation n 5 n 3 n 7 ...n N ...n 2 , π(3) = n 7 and C π (3) = {n 5 , n 3 }.
...
Input to the next layer after going through a nonlinearity σ:
...
When there is no prior knowledge, i.e., a priori we don't know which feature would be more important for prediction, so we treat them all equally important features by setting the same value to each parameter, i.e., α 0 = α 0 * 1 D l where 1 D l is a vector of ones of length D l .
...
In this case, if we set α 0 < 1 , this puts the probability mass toward a few components, resulting in only a few components that are non-zero, i.e., inducing sparse probability vector.

If we set α 0 > 1, all components become similar to each other.
...
We denote the variational parameters by φ l , where each element of the vector can choose any values above 0.

Our posterior distribution over the switch is, hence, defined by

With this parametric form of prior and posterior, we optimize the variational parameters φ l over each layer's importance switch by maximizing the variational lower bound with freezing all the weights to the pre-trained values,
...
Computing the gradient of equation 8 with respect to φ l requires computing the gradients of the integral (the first term on RHS) and also the KL divergence term (the second term on RHS), as both depends on the value of φ l .
...
, where the shape parameter of Gamma is φ l,j and the scale parameter is 1.
...
Hence, when training for the importance switch in each layer, we use the analytic mean of the Dirichlet random variable to make a point estimate of the integral q φ l (s l ) log p(D|s l )ds l ≈ log p(D|s l ), wherẽ s l,j = φ l,j / D l j =1 φ l,j , which allows us to directly compute the gradient of the quantity without sampling from the posterior.
