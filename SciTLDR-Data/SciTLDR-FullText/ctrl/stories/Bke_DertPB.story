...
where Π(P 1 , P 2 ) is the set of distributions on the product space X × X whose marginals are P 1 and P 2 , respectively.

The optimal π achieving the infimum in (2) is called the optimal coupling of P 1 and P 2 , and is denoted by π * .
...
Motivated by the fact that the optimal critic should have unit gradient norm on lines connecting the coupled points (x 1 , x 2 ) ∼ π * according to (2), they proposed a regularizer that enforces unit gradient norm along these lines, which not only enforces the Lipschitz constraint, but other properties of the optimal solution as well.

However, π * is not known in practice, which is why Gulrajani et al. (2017) proposed to apply GP on samples of the induced distribution P i , by interpolating samples from the marginals P 1 and P 2 .

The critic in the WGAN-GP formulation is regularized with the loss

where P i denotes the distribution of samples obtained by interpolating pairs of samples drawn from P r and P g , and λ is a hyperparameter acting as a Lagrange multiplier.

Theoretical arguments against GP were pointed out by Petzka et al. (2018) and Gemici et al. (2018) , arguing that unit gradient norm on samples of the distribution P i is not valid, as the pairs of samples being interpolated are generally not from the optimal coupling π * , and thus do not necessarily need to match gradient norm 1.
...
As P τ they evaluated the interpolation method described above, and also sampling random local perturbations of real and generated samples, but found no significant improvement compared to P i .
...
r 0 is a randomly sampled unit vector and ξ is another hyperparameter.
...
In practice, one adds the Monte Carlo approximation of the expectation (averaged over a minibatch of samples) of either (15) or the square of (15) (or both) to the training objective, multiplied by a Lagrange multiplier λ.
...
The hyperparameters of the approximation scheme are k, ξ and those of P .
...
If it is too high, it means that either K is too small and should be increased, or the regularization effect is too weak, so one should increase λ.
...
In the former case, one should decrease λ.
...
We have not observed any significant effect when changing the value of ξ in any of the tasks considered.
...
where P r,g is a combination of the real and generated distributions (meaning that a sample x can come from both), λ is the Lagrange multiplier, and the adversarial perturbation is defined as
...
Closely following Gulrajani et al. (2017) , we used the Adam optimizer (Kingma and Ba, 2015) with parameters β 1 = 0, β 2 = 0.9 and an initial learning rate of 2 × 10 −4 decaying linearly to 0 over 100000 iterations, training the critic for 5 steps and the generator for 1 per iteration with minibatches of size 64 (doubled for the generator).

We used (17) as a loss function to optimize the critic.

K = 1 was an obvious choice, and we found λ = 100 to be optimal (the training diverged for λ = 0.1, and was stable but performed worse for λ = 10 and 1000).

The hyperparameters of the approximation of r adv were set to ξ = 10, P being the uniform distribution over [0.1, 10], and k = 1 power iteration.
...
To see that ALR indeed restricts the Lipschitz constant of the critic, we monitored the gradient norms during training, which converged to ≈ 5 with λ = 100.

This was also the case using LP with λ = 0.1, but the number of Lipschitz constraint violations found by the algorithm were much higher in this case than with ALR.
...
The latter is based on the fact that in WGAN, the optimal critic has unit gradient norm on lines connecting points from the optimal coupling π * .

Petzka et al. (2018) showed that since π * is not known in practice, one should use the one-sided penalty, while Gemici et al. (2018) proposed a method to approximate π * with an auto-encoding scheme.
...
Since the critic f corresponds to the potential function in the dual formulation of the optimal transport problem, at optimality its gradient at x points towards its coupling y, where (x, y) ∼ π * .

From this perspective, sampling pairs (x, x + r adv ) using (18) can be seen as an approximation of the optimal coupling π * .
...
which performed similarly to the one-sided case with λ = 10, but was less stable for other values of λ.
...
The optimal hyperparameters were λ = 0.1, P being the uniform distribution over [0.1, 100], ξ = 10 and k = 1 step of power iteration.
...
The baseline VAT method was ALR specialized with K = 0, d X being the Euclidean metric, d Y being the KL divergence, fixed = 8 and λ = 1.

This setting achieved maximal validation performance of 84.2% and test performance 82.46%.

After some experimentation, the best performing choice was K = 0, d X being the l 2 metric, d Y the mean squared difference over the logit space (which parametrize the categorical output distribution over which the KL divergence is computed in the case of VAT), P being the uniform distribution over [1, 10] and λ = 1.
...
The hyperparameter ξ = 0 is introduced here.
...
It is easy to see that the optimal approximation with respect to the mean squared error iŝ
...
(h) ∇xf ALP,λ=0.1,k=0 2 (i) ∇xf ALP,λ=1,k=0 2 (j) ∇xf ALP,λ=10,k=0 2 (k) ∇xf ALP,λ=0.1,k=1 2 (l) ∇xf ALP,λ=1,k=1 2 (m) ∇xf ALP,λ=10,k=1 2 (n) ∇xf ALP,λ=0.1,k=5 2 (o) ∇xf ALP,λ=1,k=5 2 (p) ∇xf ALP,λ=10,k=5 2
...
When the network is regularized using LP (9), the regularization strength can be controlled by tuning the value of λ.

We trained with λ = 0.1, 1 and 10.

Without BN, the highest of these values seems to work the best.

With BN, the resulting mapping is visibly highly irregular.

With ALR, in addition to λ, we have additional control over the regularization by the hyperparameters of the approximation scheme of r adv .

After some experimentation, we have found the best P for this case was the uniform distribution over [10 −6 , 10 −5 ].

We trained with λ = 0.1, 1 and 10, and k = 0, 1 and 5 power iterations.

Arguably, both with and without BN the λ = 1 and k = 5 case seems like the best choice.
