...
An MDP is a tuple X , A, P, R, γ where X is the state space, A is the action space, P : X × A → X is the Markovian transition stochastic function, R : X × A → R is the immediate reward stochastic function, and γ is the discount factor.

A trajectory x(t), a(t), x(t + 1), r(t) t∈ 0,T −1 is the projection into the MDP of the task episode.

The goal is to generate trajectories with high discounted cumulative reward, also called more succinctly return: DISPLAYFORM0 To do so, one needs to find a policy π : DISPLAYFORM1 Advisor Advisor ...
...
For the further analysis, we restrict ourselves to the linear decomposition of the rewards: R(x, a) = j w j R j (x j , a), which implies the same decomposition of return if they share the same γ.

The advisor's state representation may be locally defined by φ j : X → X j , and its local state is denoted by x j = φ j (x) ∈ X j .

We define the aggregator function f Σ (x) as being greedy over the Q j -functions aggregation Q Σ (x, a): DISPLAYFORM0 We recall hereunder the main theoretical result of BID30 : a theorem ensuring, under conditions, that the advisors' training eventually converges.
...
The Q-values for each action are easy to compute: DISPLAYFORM3 As a consequence, if γ > r 1 /(r 1 + r 2 ) and γ > r 2 /(r 1 + r 2 ), the local egocentric planning commands to execute action a 0 sine die.
...
More generally in a deterministic 1 task where each action a in a state x can be cancelled by a new action a -1 x , it can be shown that the condition on γ is a function of the size of the action set A. Theorem 4 is proved in Section A of the appendix.
...
Nevertheless, acting greedily with respect to Q agn Σ (x, a) only guarantees to be better than a random policy and in general may be far from being optimal.
...
More formally, the local Bellman equilibrium equation is the following one: Q ap j (x j , a) = E r j + γQ ap j (x j , f Σ (x )) .
...
DISPLAYFORM0 Σ is the unique solution to the global Bellman optimality equation, and therefore equals the optimal value function, quod erat demonstrandum.

However, most MAd-RL settings involve taking advantage of state space reduction to speed up learning, and in this case, there is no guarantee of convergence because function f Σ (x ) can only be approximated from the local state space scope.

As a result the local estimatef j (x ) is used instead of f Σ (x ) and the reconstruction of max a ∈A Q ap Σ (x , a ) is not longer possible in the global Bellman equation: DISPLAYFORM1
...
A deep neural network (DNN) is fitted to the ground-truth value function V π * γ for various objective functions: TSP is the optimal number of turns to gather all the fruits, RL is the optimal return, and egocentric is the optimal MAd-RL return.

This learning problem is fully supervised on 1000 samples, allowing us to show how fast a DNN can capture V π * γ while ignoring the burden of finding the optimal policy and estimating its value functions through TD-backups or value iteration.
...
Finally, we found that the MAd-RL performance tends to dramatically decrease when γ gets close to 1, because of attractors' presence.
...
Six different settings are compared: the two baselines linear Q-learning and DQN-clipped, and four MAd-RL settings: egocentric with γ = 0.4, egocentric with γ = 0.9, agnostic with γ = 0.9, and empathic with γ = 0.9.
...
egocentric-γ = 0.4 adopts a near optimal policy coming close to the ghosts without taking any risk.

The fruit collection problem is similar to the travelling salesman problem, which is known to be NP-complete BID18 .

However, the suboptimal small-γ policy consisting of moving towards the closest fruits is in fact a near optimal one.

Regarding the ghost avoidance, egocentric with small γ gets an advantage over other settings: the local optimisation guarantees a perfect control of the system near the ghosts.

The most interesting outcome is the presence of the attractor phenomenon in egocentric-γ = 0.9: Pac-Boy goes straight to the centre area of the grid and does not move until a ghost comes too close, which it still knows to avoid perfectly.

This is the empirical confirmation that the attractors, studied in Section 4.1, present a real practical issue.

empathic is almost as good as egocentric-γ = 0.4.
...
egocentric-γ = 0.9 barely performs better than linear Q-learning, DQN-clipped is still far from the optimal performance, and gets hit by ghosts from time to time.

agnostic is closer but only rarely eats all the fruits.

Finally, egocentric-γ = 0.4 and empathic are near-optimal.

Only egocentric-γ = 0.4 trains a bit faster, and tends to finish the game 20 turns faster too (not shown).Results with noisy rewards -Using a very small γ may distort the objective function and perhaps even more importantly the reward signal diminishes exponentially as a function of the distance to the number of turns network trained on vector egocentric objective network trained on summed egocentric objective network trained on RL objective network trained on TSP objective optimal policy given egocentric objective optimal policy given RL objective optimal policy given TSP objective (a) Value function training.

goal, which might have critical consequences in noisy environments, hence the following experiment: several levels of Gaussian centred white noise η σ with standard deviation σ ∈ {0.01, 0.1} have been applied to the reward signal: at each turn, each advisor now receivesr j = r j + η σ instead.
...
Indeed, because of the noise, the fruit advisors are only able to consistently perceive the fruits that are in a radius dependent on γ and σ.

The egocentric planning, incompatible with high γ values, is therefore myopic and cannot perceive distant fruits.

The same kind of limitations are expected to be encountered for small γ values when the local advisors rely on state approximations, and/or when the transitions are stochastic.
...
We train all learners in parallel with off-policy learning, with Bellman residuals computed as presented in Section 4 and a constant α = 0.1 parameter.

The aggregator function sums the Q-values for each action a ∈ A: Q Σ (x, a) := j Q j (x j , a), and uses -greedy action selection with respect to these summed values.
...
As for the γ condition, it is not only sufficient but also necessary, since being surrounded by goals of equal value is an attractor if γ > 1/3.

In practice, an attractor becomes stable only when there is an action enabling it to remain in the attraction set.

Thus, the condition for not being stuck in an attractor set can be relaxed to γ ≤ 1/(|A|−2).

Hence, the result of γ > 1/2 in the example illustrated by FIG2 .Baselines -The first baseline is the standard DQN algorithm BID16 with reward clipping (referred to as DQN-clipped).
