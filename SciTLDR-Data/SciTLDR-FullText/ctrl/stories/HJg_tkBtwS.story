...
Of the class of integral probability metrics (Müller, 1997) and f -divergences (Csiszár, 1964) , the KLdivergence simplifies estimation and allows for reuse of the model structures and code from the standard task of predicting the response from the features.
...
Computing any divergence K, like an integral probability metric (Müller, 1997) or an f -divergence (Csiszár, 1964) , between the left hand side and right hand side of Equation (3) would be a proper test statistic.
...
The third distributioñ q j (y | x j , x −j ) requires a regression model with corrupted inputs.
...
The other two pieces of δ j require building a model from x to y and ( x j , x −j ) to y. To learn these models, the data D N is split into training and test sets.

Let θ, β be parameters, q θ ( x j | x −j ) and q β (y | x) are fit to the training set.

The distribution q γ (y | x j , x −j ) is fit to the training part of D j,N .

To compute the expectations in δ j , these models are evaluated on their corresponding test sets.
...
The models q θ ( x j | x −j ) and q β (y | x) can be shared across the replications, however q γ (y | x j , x −j ) must be recomputed for each replication.
...
To speed up the computation of AMI-CRT, we use the average of the original model q β (y | x) and a single model trained for q γ (y | x j , x −j ).

This averaged model is used to estimate the two terms in the AMI-difference δ j .
...
To estimate each of q θ and q β , standard regression models like logistic regression, neural networks, and random forests can be used at no more computational cost than training.
...
While predictive models q β for q(y | x) suffice for understanding population distributions, they might not be sufficient to perform IWFS.
...
Recall that given a model q β for q(y | x) that satisfies the instance-wise sufficient conditions in Proposition 1,
...
We first sample x ∼ N (0, Σ D ) N times, where Σ D is a Ddimensional covariance matrix.
...
We first sample x ∼ N (0, Σ D ) N times, where Σ D is a D-dimensional covariance matrix, and D ≥ 11.
...
The fast-ami-crt achieves a middle ground between the hrt and ami-crt by yielding well-calibrated p-values while requiring only one null model q γ (y | x j , x −j ) per feature.
...
We use the orange dataset and set all off-diagonal values of Σ D to 0.5.
...
To model the distribution q(x S |x −S ), we make use of a generative inpainting model π g (Yu et al., 2018) .
...
where Lβ is an log-likelihood estimate using q (k,m) β end end

Let x be a dataset such that x −j = x −j , and x j is randomly sampled from q θ (
