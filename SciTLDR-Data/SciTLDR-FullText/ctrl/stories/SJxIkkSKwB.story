...
, K}. The classification model M is taken to be g θ : X → Y parameterized with θ.

The softmax output of the model is given by p = softmax(g θ (x)) ∈ ∆ K−1 .
...
ALG is designed with the aim of maximizing the prediction accuracy E p X ×Y [(h θ (x) = y)].
...
where ε is the probability of a label flip, i.e., p(y = y) = ε.

We resort to the usage of K-SC because of its simplicity, and in addition, it abstracts the oracle noise strength with a single parameter ε.
...
Batch selection: The construction of batch active learning algorithm by solving the aforementioned first two problems begins with assignment of an importance score (ρ) to each sample in the pool.
...
Finally, with all similar samples clustered together, we perform importance sampling of the cluster centroids using their importance score, and a random centroid c is selected as p(c = k) ∝ ρ k .
...
We denote the model uncertainty as σ ∈ [0, 1], such that σ is normalized between 0 and 1 with 0 being complete certainty and 1 for fully uncertain model.

For rest of the work, we compute the uncertainty measure σ as variation ratio of the output of model's multiple stochastic forward passes as mentioned in (Gal & Ghahramani, 2016 ).

In the event of an uncertain model (σ → 1), we randomly select samples from the pool initially.

However, as the model moves towards being more accurate (low σ) by acquiring more labeled samples through active learning, the selection of samples should be biased towards importance sampling and clustering.
...
Compute uncertainty estimate σ (t−1) of the model M (t−1) , and

Sample ζ from the Gibbs distribution p(ζ = s|B (t) , c, β
...
Set P ← P \ B (t) 13: end for the cluster centroid c and any sample x in the pool, and temperature analogue as uncertainty estimate σ of the model.
...
, s n already drawn, the probability of drawing a sample ζ given the cluster centroid c, distance matrix D = [d i,j ] and inverse temperature (or inverse uncertainty) β is written as

where P = P\s 1:n .

In theory, the inverse uncertainty β can be any f such that f : [0, 1] → R + ∪{0} and f (σ) → ∞ as σ → 0 and f (σ) = 0 for σ = 1.

For example, few possible choices for β (= f (σ)) are − log(σ), e 1/σ − 1.
...
Next, since we have drawn the cluster centroid c according to p(c = k) ∝ ρ k , the probability of drawing a sample s from the pool P is written as

We can readily see that upon setting β → 0 in (4), p(ζ = s|s 1:n , β, D) reduces to 1/|P | which is nothing but the uniform random distribution in the leftover pool.

On setting β → ∞, we have ζ = c with probability ρ c / c ρ c and ζ = c with probability 0, i.e., selecting cluster centroids from the pool with importance sampling.

For all other 0 < β < ∞ we have a soft bridge between these two asymptotic cases.
...
The uncertainty measure σ is computed as the variation ratio of the output prediction across 100 stochastic forward passes, as coined in (Gal & Ghahramani, 2016) , through the model using a validation set which is fixed apriori.

The inverse uncertainty function β = f (σ) in Algorithm 1 is chosen from l (e 1/σ − 1), −l log(σ), where l is a scaling constant fixed using cross-validation.

The cross-validation is performed only for the noise-free setting, and all other results with different noise magnitude ε follow this choice.
...
The degradation in the performance worsens with increasing oracle noise strength ε.
...
The prediction accuracy gap between algorithm with/without denoising layer elevates with increase in the noise strength ε.
...
The uncertainty measure σ plays a key role for the proposed algorithm.
...
The active learning algorithm performance for oracle noise strength of ε = 0.2 and ε = 0.4 are presented in Figure 6 .

Similarly to what discussed in Section 5, we observe that the performance of proposed algorithm dominates all other existing works for ε = 0.2.

We witnessed that the proposed algorithm performance (without denoising layer) is not able to match other algorithms (BALD and Entropy) when ε = 0.4, even with more training data.

The reason for this behavior can be explained using the uncertainty measure σ output in the Figure 7 .
...
Since the uncertainty estimates improve, as we see in Figure 7 , for ε = 0.4, the proposed algorithm along with the denoising layer performs very well and has significant improvement in performance as compared to other approaches.

The results for CIFAR10 dataset with oracle noise strength of ε = 0.2 and 0.4 are provided in the Figure 8 .

We see that the proposed algorithm without/with using the denoising layer outperforms other benchmarks.

We provide the active learning accuracy results for SVHN dataset with oracle noise strength of ε = 0.2 and 0.4 in the Figure 8 .

Similar to other results, we see that the proposed algorithm without/with using the denoising layer outperforms other benchmarks for ε = 0.2.

For oracle noise strength of ε = 0.4, we see a similar trend as MNIST regarding performance compromise to the proposed uncertainty based batch selection.

The reason is again found in the uncertainty estimates plot in Figure 10 for ε = 0.4.
...
The incorporation of denoising layer helps in countering the affects of noisy oracle as we demonstrate by varying the noise strength ε = 0.1, 0.3.
