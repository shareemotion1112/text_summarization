...
In our approach, an agent can perform motions through the use of policies π θ , parametrized by θ ∈ Π, and those policies have an effect on the environment, which we call the outcome ω ∈ Ω. The agent is then to learn the mapping between Π and Ω: it learns to predict the outcome ω of each policy π θ (the forward model M ), but more importantly, it learns which policy to choose for reaching any particular outcome (an inverse model L).

The outcomes ω can be of different dimensionality and thus be split in subspaces Ω i ⊂ Ω. The policies do not only consist of one primitive but of a succession of primitives (each encoded by the parameters in Π) that are executed sequentially by the agent.

Hence, policies also are of different dimensionality and are split in policy spaces Π i ⊂ Π (where i corresponds to the number of primitives used in each policy of Π i ).

Complex policies are represented by concatenating the parameters of each of its primitive policies in execution order.

We take the trial and error approach, and suppose that Ω is a metric space, meaning the learner has a means of evaluating a distance between two outcomes d(ω 1 , ω 2 ).
...
More formally, a procedure is built by choosing previously known outcomes (t 1 , t 2 , ..., t n ∈ Ω) and is noted t 1 t 1 ... t n .Executing a procedure t 1 t 1 ... t n means building the complex policy π θ corresponding to the succession of policies π θi , i ∈ 1, n (potentially complex as well) and execute it (where the π θi reach best the t i ∀i ∈ 1, n respectively).

As the subtasks t i are generally unknown from the learner, the procedure is updated before execution (see Algo.

1) to subtasks t i which are the closest known by the learner according to its current skill set (by executing respectively π θ 1 to π θ n ).
...
2, FIG0 ) learns by episodes, where an outcome ω g ∈ Ω to target and an exploration strategy σ have been selected.
...
In an episode under the procedural space exploration strategy, the learner builds a size 2 procedure t i t j such as to reproduce the goal outcome ω g the best (Goal-Directed Procedure Optimization(ω g )).
...
1.In an episode under the mimicry of one policy teacher strategy, the learner requests a demonstration ξ d from the chosen teacher.

ξ d is selected by the teacher as the closest from the goal outcome ω g in its demonstration repertoire.

The learner then repeats the demonstrated policy (Mimic Policy(ξ d )).

It is a strategy directly also available in the SGIM-ACTS algorithm.

In an episode under the mimicry of one procedural teacher strategy, the learner requests a procedural demonstration of size 2 t di t dj which is built by the chosen teacher according to a preset function which depends on the target outcome ω g .

Then the learner tries to reproduce the demonstrated procedure by refining and executing it, following Algo.

1 (Mimic Procedure(t di t dj )).In both autonomous exploration strategies, the learner uses a method, Goal-Directed Optimization, to optimize its input parameters (procedure for the procedure exploration and policy for the policy exploration) to reach ω g best.

This generic method works similarly in both cases, by creating random inputs, if the goal outcome ω g is far from any previously reached one, or local optimization which uses linear regression.
...
It computes its competence in reaching the goal outcome ω g by computing the distance d(ω r , ω g )) with the outcome ω r it actually reached.

Then it updates its interest model by computing the interest interest(ω, σ) of the goal outcome and each outcome reached (including the outcome spaces reached but not targeted): DISPLAYFORM0 where K(σ) is the cost of the strategy used and the progress p(ω) is the derivate of the competence.

The learning agent then uses these interest measures to partition the outcome space Ω into regions of high and low interest.

For each strategey σ, the outcomes reached and the goal are added to their partition region.
...
Then generate ω g ∈ R n at random; • mode 3: choose σ and R n like in mode 2, but generate a goal ω g ∈ R n close to the outcome with the highest measure of progress.
...
The motions of each of the three joints of the robot are encoded using a one-dimensional Dynamic Movement Primitive BID18 , defined by the system: DISPLAYFORM0 where x and v are the position and velocity of the system; s is the phase of the motion; x 0 and g are the starting and end position of the motion; τ is a factor used to temporally scale the system (set to fix the length of a primitive execution); K and D are the spring constant and damping term fixed for the whole experiment; α is also a constant fixed for the experiment; and f is a non-linear term used to shape the trajectory called the forcing term.

This forcing term is defined as: DISPLAYFORM1 where ψ i (s) = exp(−h i (s − c i ) 2 ) with centers c i and widths h i fixed for all primitives.

There are 3 weights ω i per DMP.The weights of the forcing term and the end positions are the only parameters of the DMP used by the robot.
...
Therefore a primitive policy π θ is parametrized by: DISPLAYFORM2 where DISPLAYFORM3 ) corresponds to the DMP parameters of the joint i, ordered from base to tip, and z is the fixed vertical position.

When combining two or more primitive policies (π θ0 , π θ1 , ...), in a complex policies π θ , the parameters (θ 0 , θ 1 , ...) are simply concatenated together from the first primitive to the last.

The outcome subspaces the robot learns to reach are hierarchically organized and defined as:• Ω 0 : the position (x 0 , y 0 , z 0 ) of the end effector of the robot in Cartesian coordinates at the end of a policy execution; • Ω 1 : the position (x 1 , y 1 , z 1 ) of the pen at the end of a policy execution if the pen is grabbed by the robot; • Ω 2 : the first (x a , y a ) and last (x b , y b ) points of the last drawn continuous line on the floor if the pen is functional (x a , y a , x b , y b ); • Ω 3 : the position (x 3 , y 3 , z 3 ) of the first joystick at the end of a policy execution if it is grabbed by the robot; • Ω 4 : the position (x 4 , y 4 , z 4 ) of the second joystick at the end of a policy execution if it is grabbed by the robot; • Ω 5 : the position (x 5 , y 5 ) of the video-game character at the end of a policy execution if moved.

To help the SGIM-PB learner, procedural teachers were available so as to provide procedures for every outcome subspaces but Ω 0 .
...
Each teacher was capable to provide demonstrations (as policies executable by the robot) linearly distributed in its outcome space: To evaluate our algorithm, we created a benchmark dataset for each outcome space Ω i , linearly distributed across the outcome space dimensions, for a total of 27,600 points.
...
DISPLAYFORM0 Then to asses our algorithm efficiency, we compare its results with 3 other algorithms:• SAGG-RIAC: performs autonomous exploration of the policy space Π guided by intrinsic motivation; • SGIM-ACTS: interactive learner driven by intrinsic motivation.

Choosing between autonomous exploration of the policy space Π and mimicry of one of the available policy teachers; • IM-PB: performs both autonomous exploration of the procedural space and the policy space, guided by intrinsic motivation; • SGIM-PB: interactive learner driven by intrinsic motivation.
...
The value of γ for this experiment is 1.2.
...
If we look at the evaluation on each individual outcome space (Fig. 4) , we can see that the learners with demonstrations (SGIM-PB and SGIM-ACTS) outperforms the other algorithms, except for the outcome space Ω 5 where IM-PB is better, due to the fact that IM-PB practiced much more on this outcome space (500 iterations where the goal was in Ω 5 against 160 for SGIM-PB) on this outcome space.

SGIM-PB and SGIM-ACTS are much better than the other algorithms on the two joysticks outcome spaces (Ω 3 and Ω 4 ).
...
It was capable to request demonstrations from the relevant teachers depending on the task at hand, except for the outcome space Ω 0 which had no human teachers and therefore could not find a better teacher to help it.

Indeed, for the outcome space Ω 2 , the procedural teacher (ProceduralTeacher2) specially built for this outcome space was greatly chosen.
...
We draw 1,000,000 goal outcomes for each of the Ω 0 , Ω 1 and Ω 2 subspaces (chosen because they are increasingly complex) and we let the learner choose the known policy that would reach the closest outcome.
...
It chooses longer policies for the Ω 1 subspace (policies of size 2 and 3 while using mostly policies of size 1 and 2 for Ω 0 ) and even longer for the Ω 2 subspace (using far more policies of size 3 than for the others).
