...
4: DISPLAYFORM0 Sample data x from χ 6: DISPLAYFORM1 end for 8: DISPLAYFORM2 Select threshold: DISPLAYFORM3 end for 14:All-reduce DISPLAYFORM4 wt+1 ← SGD (wt, Gt) 16: end for BID18 .
...
It is automatically satisfied in Equation 2 where the T in the learning rate ηT and batch size N bT are canceled out.
...
Sample data x from χ 5: DISPLAYFORM0 end for

All-reduce G w t+1 ← SGD (w t , G t ) 9: end for

The conventional update rule for Nesterov momentum SGD BID25 follows, DISPLAYFORM0 where m is the momentum, N is the number of training nodes, and k,t = 1 N b x∈B k,t f (x, w t ).Before momentum correction, the sparse update follows, DISPLAYFORM1 sparse (v k,t+1 ) , w t+1 = w t − ηu t+1After momentum correction sharing the same methodology with Equation 7, it becomes, u k,t+1 = mu k,t + k,t , v k,t+1 = v k,t +(m ·

u k,t+1 + k,t ) , w t+1 = w t −η C LOCAL GRADIENT CLIPPING When training the recurrent neural network with gradient clipping, we perform the gradient clipping locally before adding the current gradient G k t to previous accumulation G k t−1 in Algorithm 1.
