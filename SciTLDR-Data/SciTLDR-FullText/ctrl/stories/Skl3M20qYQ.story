...
The original Variational autoencoder (VAE) framework BID17 ; BID21 ) has been used extensively for the task of disentanglement by modifying the original ELBO formulation; for instance β-VAE is presented in BID12 which increases the latent capacity by penalising the KL divergence term with a β hyperparameter.
...
Notably, in BID11 , they describe comprehensively the β-VAE model using a neuroscience and information theory approaches; they suggest that by encouraging redundancy reduction the model achieves statistical independence within the latents.
...
Besides, from the VAE standard framework, we know that the distribution p(A i |x) is intractable which is why we need to use an approximate distribution q φ (A i |x) parametrised by φ parameters.
...
We show the first 4000 steps for the Synergy term (i.e. α * Imax), since most of the interaction happens in the first steps.
