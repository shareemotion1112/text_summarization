...
Given a model with parameters θ, the cross-entropy loss (θ) reflects how well the model fits the training samples:
...
We divide the training data into a labeled set and an unlabeled set by setting a threshold τ on w i .

However, training a model using the data divided by itself could lead to confirmation bias (i.e. the 1 Input: θ

(1) and θ (2) , training dataset (X , Y), clean probability threshold τ , number of augmentations M , sharpening temperature T , unsupervised loss weight λu, Beta distribution parameter α for MixMatch.

// standard training (with confidence penalty) 3 while e < MaxEpoch do

) // model per-sample loss with θ (1) to obtain clean proabability for θ

) // model per-sample loss with θ (2) to obtain clean proabability for θ
...
Having acquiredX (andÛ) which consists of multiple augmentations of labeled (unlabeled) samples and their refined (guessed) labels, we follow MixMatch to "mix" the data, where each sample is interpolated with another sample randomly chosen from the combined mini-batch ofX andÛ. Specifically, for a pair of samples (x 1 , x 2 ) and their corresponding labels (p 1 , p 2 ), the mixed (x , p ) is computed by:

MixMatch transformsX andÛ into X and U .

Equation 6 ensures that X are "closer" toX thanÛ. The loss on X is the cross-entropy loss and the loss on U is the mean squared error:

Under high levels of noise, the network would be encouraged to predict the same class to minimize the loss.

To prevent assigning all samples to a single class, we apply the regularization term used by Tanaka et al. (2018) and Arazo et al. (2019) , which uses a uniform prior distribution π (i.e. π c = 1/C) to regularize the model's average output across all samples in the mini-batch:

Finally, the total loss is:

In our experiments, we set λ r as 1 and use λ u to control the strength of the unsupervised loss.
...
For all CIFAR experiments, we use the same hyperparameters M = 2, T = 0.5, and α = 4.

τ is set as 0.5 except for 90% noise ratio when it is set as 0.6.

We choose λ u from {0, 25, 50, 150} using a small validation set.
...
• To study the effect of model ensemble during test, we use the prediction from a single model θ
...
The performance further decreases compared to θ (1) .
...
For CIFAR experiments, the only hyperparameter that we tune on a per-experiment basis is the unsupervised loss weight λ u .

Table 7 shows the value that we use.

A larger λ u is required for stronger regularization under high noise ratios or with more classes.

For both Clothing1M and WebVision, we use the same set of hyperparameters M = 2, T = 0.5, τ = 0.5, λ u = 0, α = 0.5, and train the network using SGD with a momentum of 0.9, a weight decay of 0.001, and a batch size of 32.
...
Table 7 : Unsupervised loss weight λ u for CIFAR experiments.
