...
Define DISPLAYFORM3 , where µ 1 is the treatment response function.

It denotes the outcomes of the units who received treatment.

µ 0 is defined analogously for control units.

To estimate τ (x) (the CATE), we compute estimatesμ 0 ,μ 1 for µ 1 and µ 0 and then subtract to get DISPLAYFORM4 Below, we will discuss four common strategies for estimatingτ .

In the T-Learner, we estimateμ 0 andμ 1 directly with any arbitrary function approximator.

Let f i be such a function approximator.

Thenμ DISPLAYFORM0 We then estimate the CATE by taking differences: DISPLAYFORM1 Under strong assumptions, it is possible to provide convergence rate guarantees for the T-learner BID10 BID14 .
...
More concretely,û DISPLAYFORM1 The R-Learner learner was proposed in BID14 .
...
Defineμ 0 andμ 1 as in the T-learner.

For each of these estimates, we can produce a corresponding imputed treatment effect DISPLAYFORM0 Note that this learner does in fact use the control estimator µ 0 on the treatment data X 1 and similarly for µ 1 and X 0 .

This is the correct way to impute the treatment effect estimate fromμ 0 andμ 1 .

From here, we can get to the CATE by estimating the imputed treatment effects and then summing the estimatesτ DISPLAYFORM1 For a theoretically grounded justification of this procedure, including convergence rate analysis, see BID10 .
...
In the first stage, the outcome functions,μ 0 andμ 1 , are estimated and the individual treatment effects are imputed: DISPLAYFORM0 In the second stage, estimators for the CATE are derived by regressing the features X on the imputed treatment effects.

DISPLAYFORM1 In the X-learner paper, random forests were used to obtain the estimatesμ 0 andμ 1 .

However, suppose we used neural networks instead.

In fact, suppose f θ0 and f θ1 estimate µ 0 and µ 1 .

Then we can write DISPLAYFORM2 .

Suppose we also want to use neural networks for the second stage.

Then we can writê DISPLAYFORM3 When written in this way, it is clear that we should at least try to jointly optimize f τ and f θi .

That is, when we are optimizing f τ1 , we should also backprop through the netwowk f θ0 and similarly for f τ0 and f θ1 .
...
DISPLAYFORM4 Update the network f θ0 to predict Y obs i 3:Update the network f θ1 to predict Y DISPLAYFORM5 Update the network f τ to predict f θ1 ( DISPLAYFORM6 Update the network f θ0 to predict DISPLAYFORM7 Update the network f θ1 to predict Y obs i

Update the network f τ to predict DISPLAYFORM0
...
This was not surprising, because we figured the joint optimization strategy of backpropgating through f θ1 and f θ0 in lines 4 and 9 of Algorithm 1 would allow those estimators to more directly benefit the final CATE estimation network f τ .

However, if we stopped gradients from going through f θ0 and f θ1 when backpropogating through f τ , we saw there was no major loss in performance.
...
One key difference between the two is that the Y-learner updates f θ0 , f θ1 , and f τ continuously and in-step with one another, whereas in the X-learner the imputation networks D 0 and D 1 are fixed before training the CATE estimation networks f τ1 and f τ0 .
...
Second, the Y-learner with full backpropogation through f θ0 , f θ1 when training f τ .

This is labeled 'Y.' Third, the Y-learner with no backpropogation through f θ0 , f θ1 when training f τ .

This is labeled 'Y no backprop.'

For the final learner, we train a Y-learner to completion.

We then hold the trained f τ fixed and use the same dataset to train a new f θ1 and f θ2 from scratch.

Finally, we hold the f θ1 and f θ2 that we just trained fixed and use them to train a new f τ from scratch.

The goal of the last learner is to test the importance of co-learning f θ1 , f θ2 , and f τ for the Y-learner.
