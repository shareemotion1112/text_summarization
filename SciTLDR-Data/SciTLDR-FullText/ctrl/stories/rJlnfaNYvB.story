...
For an appropriate choice of α, loss scaling can achieve state of the art results that are competitive with regular FP32 training.

Unfortunately, there is no single value of α that will work in arbitrary models, and so it often needs to be tuned per model.

Its value must be chosen large enough to prevent underflow issues from affecting training accuracy.

However, if α is chosen too large, it could amplify rounding errors caused by swamping (Higham, 1993) or even result in overflow.
...
Including the time spent tuning α, the total training time of MPT can even exceed regular FP32 training.
...
δ N +1 ← initial error gradient; for i ← layer indices in reversed topological order do j ← Output(i);
...
Note that they differ only in that in Algorithm 2, the initial error gradients δ N +1 from the output layer are scaled by α before the start of the backward pass, and that the weight gradient update GradW is then unscaled by the same α just before the weight update.

Recall that α should be chosen large enough to prevent underflow issues from affecting training, while also being small enough to prevent overflow.
...
After the forward pass has completed, starting from the rightmost node, the gradients δ are first propagated from loss L and are then scaled by a scalar α 4 before being consumed by the last linear layer.

The weight gradients for this layer are then scaled by 1/α 4 just before the weight update for W 3 in order to prevent the particular choice of α 4 from affecting the computed gradient magnitudes.

It is at this point that our approach begins to differ from the standard loss scaling method.

In the standard method, the same α 4 would be used
...
Section 3.1 explains the symbols in this figure, specifically, β 1 , β 2 , and β 3 denote the loss scale values calculated locally for each layer, and α 1 , α 2 , and α 3 stand for accumulated scales.

α 4 is an optional initial scale.

for all layers in the network.

However, in our method, each layer i calculates its own local loss scale value β i based on the statistics of its output gradients and weights in the current iteration, in order to minimize underflow in its computed input gradients.

This β i is then used to scale weights W i before computing the scaled input activation gradients for layer i. Since the scaling effects from these local loss scales accumulate, when unscaling gradients for updating, we use the product of all previous scale values, indicated by α i = α 4 3 j=i+1 β j .

Thus, our approach attempts to minimize underflow in every layer simultaneously through the use of layer-local loss scales β i which are also computed automatically based on the current layer statistics.
...
We use a 2-tuple notation to denote the propagated entity from layer i: α i , δ i , in which α i is the loss scale value for layer i and δ i is the gradient that has been scaled by α i .

To be more specific about this notation, we can take a N -layer MLP as an example (see Figure 3 for the notation).

In this case, layer i takes in α i+1 , δ i+1 , updates its weight by (y T i−1 δ i+1 )/α i+1 , and produces α i , δ i for the previous layer i − 1.

We will elaborate more on how α i is calculated in the following sections.
...
α N +1 , δ N +1 ← initial loss scale, and error gradient scaled by α 0 for i ← layer ID in a reversed topological order do j ← GetLayerOutput(i)
...
We start with the error gradients δ N +1 computed from the output loss value for the last layer N + 1.

We may optionally scale this gradient by α N +1 .

Normally we keep it as 1.

2.

As visiting each previous layer i in a reversed topological order of the computational graph, we retrieve the tuple α j , δ j propagated to it from the next downstream layer that represents the scaled loss for layer i's output.

We calculate a local loss scale value β i , which will be used to scale δ j before we calculate the activation gradients for the previous layer.

3.

We use δ j and other cached inputs (omitted) to compute the gradients for W i .

However, since these gradients have been scaled, we must unscale them using α j before performing the weight update.

4.

Since β i contributes to the magnitude of the gradient δ i , we calculate the loss scale value δ i to be passed to the next previous layer as α j β i .
...
Now consider the backward pass for this same layer, in which the "input" to the layer consists of the tuple α, δ received from the next downstream layer, where δ = α ∂L ∂Y .

Note that α represents the total loss scale (i.e., product of all downstream layer-wise scales).

As shown earlier, the weight gradients for W are computed by (X T δ)/α.

We assume that both W and δ can be characterized by two i.i.d.

normal random variables w and g, respectively, so that w ∼ N (µ w , σ 2 w ) and g ∼ N (µ g , σ 2 g ).

We also assume that their product p = wg is another random variable with a zero-mean normal distribution, i.e., p ∼ N (0, σ 2 p ).
...
Since w and g are uncorrelated, σ Intuitively, if the probability that p experiences underflow is reduced, the final result will have less underflow rate as well.
...
Suppose either w or g is scaled by α before the multiplication, then P (α|p| ≤ u) ≤ T uf becomes the expected outcome of loss scaling.

Since p is assumed to be N (0, σ 2 p ), it implies that |p| is a random variable with half-normal distribution, and P (α|p| ≤ u) = erf(u/(ασ p √ 2)).
...
Note that the computation of σ p requires the statistics of w and g. These in turn require computing the sample mean and variance of W and δ, which is the main source of computational overhead, with around the same computation budget as batch normalization.
...
, and α j is the maximum loss scale among them, then the rescaling works by multiplying any gradient other than δ j by a factor to produce the gradient that has the same loss scale as j, which is calculated by α j /α i .

And if α j is too large and may cause overflow in other gradients,which is decided by checking the scaled maximum absolute gradient, we will search in a descending order of all scales among inputs until we find one.
...
Otherwise, due to the nature of floating-point, a scaled gradient cannot always be unscaled by the same scale, i.e., (αx)/α = x if α is not a power of two (Muller et al., 2010) .

The upper bound of layer-wise loss scale is determined by avoiding overflow.

For the GEMM case, it can be simply calculated by choosing the maximal numbers from both operands, multiplying them together, and then taking u max over that multiplication result as the largest possible loss scale, i.e., u max /(max(W ) × max(δ)).
...
We further compare the maximum standard deviation σ max of all gradients at different iterations between adaptive and fixed, and we observe that the ratio of σ max of fixed over adaptive is around 20 times, both for ResNet-18/50, which can be the major cause for accuracy drop since that high variance can increase accumulation error (Sakr et al., 2019) .
