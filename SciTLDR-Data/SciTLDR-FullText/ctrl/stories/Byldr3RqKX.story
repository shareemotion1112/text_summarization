...
A CGM M is extrinsically disentangled with respect to endomorphism T : I M → I M and subset of latent variables L, if there exists an endomorphism T of the latent variables such that for any image generated by a realization z = Z(ω) of the latent variables (I = g M (z)): DISPLAYFORM0 where T (z) only affects values of components of z in L. We call sparsity of the disentanglement the minimum size n of the subset L. We say T is n-disentangled in M .Extrinsic disentanglement can be seen as a form of intervention on the CGM as illustrated in FIG0 .
...
We used the official tensorlayer DCGAN implementation 2 and a plain β-VAE 3 BID5 ).
...
Performance of our procedure on such model (β divided by 10) is shown in FIG4 , where we can see better overall image quality, but a slightly more artificial hybridization with for example a slight overlay of the hair of both original images.
