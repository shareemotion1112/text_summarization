...
Similar to a Transformer block (Vaswani et al., 2017) , we first use two self-attention networks, SA q and SA r , to encode the question and the regional features asQ = SA q (Q, Q; φ) andR = SA r (R, R; φ), respectively, whereQ ∈ R M ×D ,R ∈ R N ×D , and D is the network's hidden dimension.

Based on this, a cross-attention network CA is applied to use the question as guidance to refine the visual features into V = CA(R,Q; φ) ∈ R N ×D , whereQ is used as the query vector, and φ denotes all the parameters in the Visual Encoder.
...
In the first attention layer, r f is fed into an attention network g d as the query vector to incorporate the output (ô 1:K ) of neighbor modules on which the current module is dependent.
...
The final output from the is denoted as g(r f ,ô 1:

Here is how the instantiation process of Meta Module works.

First, we feed a function f to instantiate the meta module g into an instance module g f (ô 1:K , V; ψ), where ψ denotes the parameters of the meta module.
...
During training, we optimize the parameters ψ (in Meta Module) and the parameters φ (in Visual Encoder) to maximize the likelihood p φ,ψ (a|P, Q, R) on the training data, where a is the answer, and P, Q, R are programs, questions and visual features, respectively.
...
The 'Teacher' exhibits these results as guideline γ for the 'Student' instance module g f to adhere to during training.
...
If j a i,j > 0, which means that there exists detected bounding boxes overlapping with the ground-truth object, we normalize a i,j over R to obtain a guideline distribution γ i,j = ai,j j ai,j and append an extra 0 in the end to obtain γ i ∈ R N +1 .
...
Student Training: To explicitly teach the student module g f to follow the learning guideline from the Symbolic Teacher, we add an additional head to each module output o(f i ) to predict the execution result distribution, denoted asγ i = sof tmax(M LP (o(f i ))).

During training, we propel the instance module to align its predictionγ i with the guideline distribution γ i by minimizing their KL divergence KL(γ i ||γ i ).

Formally, given the quadruple of (P, Q, R, a) and the pre-computed guideline distribution γ, we propose to add KL divergence to the standard loss function with a balancing factor η:
...
We investigate the influence of module supervision by changing the hyper-parameter η from 0 to 2.0.
...
By increasing η from 0.1 to 0.5, accuracy steadily improves, which reflects the importance of module supervision.

Further increasing the value of η did not improve the performance empirically.
...
The visual encoder and multi-head attention network is illustrated in Figure 6 and Figure 7 , respectively.̂#̂$̂%̂&
