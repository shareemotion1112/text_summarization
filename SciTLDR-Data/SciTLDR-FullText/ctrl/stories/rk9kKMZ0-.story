...
The training instances with loss values larger than a threshold, λ, are neglected during training and λ dynamically increases in the training process to include more complex samples, until all training instances are considered.
...
Assuming we have a training set with N input-label pairs D = {x n , y n } N n=1 , the Magnet loss learns the distribution of distances for each example, from K clusters assigned for each class, c, denoted as {I DISPLAYFORM0 The mapping of inputs to representation space are parametrized by f(·; Θ), where their representations are defined as r n = {f(x n ; Θ)} N n=1 .

The approach then repositions the different cluster assignments using an intermediate K-Means++ clustering BID0 .

Therefore, for each class c, we have, {I The class of representation r and its assigned cluster center are defined as C(r) and µ(r), respectively.
...
This results in the following stochastic approximation of the Magnet Loss objective: DISPLAYFORM2 where {·} + is the hinge function, α ∈ R is a scalar, the cluster means approximation DISPLAYFORM3 , and the variance of all samples from their respective centers is given byσ = DISPLAYFORM4
...
This leads to an objective similar to the one presented in SPLD: DISPLAYFORM3 where λ, γ are the two pacing parameters for sampling based on easiness and the true diverseness as sample importance priors, respectively.
...
The student CNN receives a diverse cluster of samples, the up-to-date model parameters θ, λ, γ and outputs the optimal of W of min W E(θ, W; λ, γ) for extracting the global optimum of this optimization problem.

The detailed algorithm to train the student CNN with LEAP is presented in Algorithm 2.

Step 18 of Algorithm 2 selects the "easy" samples for training when L(y DISPLAYFORM4 DISPLAYFORM5 , θ)) < λ + γ, which represents the "hard" samples with higher losses.

We select other samples by ranking a sample w.r.t to its loss value within its cluster, denoted by i.

Then, we compare the losses to a threshold λ + γ DISPLAYFORM6 Step 18 penalizes samples repeatedly selected from the same cluster, seeing as this threshold decreases as the sample's rank i grows.
...
The λ and γ pace parameters are kept consistent between the SPLD strategy in LEAP and the original SPLD algorithm to ensure a fair evaluation.
