...
We experiment on a variety of standard datasets and report improvements over state-of-art formulations 2 RELATED WORK BID8 formulated GAN as a minimax game between two neural networks: generator G θ and discriminator D φ .
...
Let the distance between set A and set B (defined as inf a∈A,b∈B a − b ) be β (> 0 due to high distinction between the sets).

Let M A , M B be subsets of Z mapping to A and B respectively.

For any arbitrary z 1 ∈ M A and z 2 ∈ M B , from the Lipschitz condition, we have: DISPLAYFORM1 Hence, the distance between any two points of M A and M B must be at least β K , as a result of which, the distance between the sets M A and M B is at least β K .

Clearly, in our hypothetical case, this results in a gap of at least β K wherever M A ends and M B begins.

Since we have assumed uniform distribution, a probability measure of β 2K is lost to undesired outputs.
...
For vanilla GANs, DISPLAYFORM1 Here L c (y, C(x)) is the cross-entropy classification loss for input image x, which was generated by partition y, and C(x) is the classifier's softmax output vector.α is a hyperparameter introduced to control the relative importance of generating good samples w.r.t encouraging diversity between the outputs of different partitions.
...
In this method, the noise vectors are drawn from a mixture-of-Gaussians with trainable means (µ 1 , . . .

, µ N ) and covariance matrices FIG0 , . . .
...
We then usê z = z σ i + µ i as the latent vector for generating the sample.

We let the gradients backpropagate to µ i and σ i , allowing for a learnable latent distribution.
...
We use N = 10 for MNIST, and N = 8 for celebA, STL-10, and CelebRoom.α is fixed to 1.0 for all experiments with multiple partitions.
