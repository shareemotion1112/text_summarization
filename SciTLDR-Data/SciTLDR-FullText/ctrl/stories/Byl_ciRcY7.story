...
Two particular sections of large margin dynamics are highlighted in (b), one at 8.3 on x-axis that measures the percentage of normalized training margins no more than 8.3 (training margin error) and the other at 0.8 on y-axis that measures the normalized margins at quantile q = 0.8 (i.e. 1/γ q,t ).
...
We equip F with the Lipschitz semi-norm, for each f , DISPLAYFORM1 where · σ is the spectral norm and DISPLAYFORM2 For all the examples in this paper, we use ReLU activation σ i that leads to L σi = 1.

Moreover we consider the following family of hypothesis mapping, DISPLAYFORM3 where [·] j denotes the j th coordinate and we further define the following class induced by Lipschitz semi-norm bound on F, DISPLAYFORM4 Lastly, rather than merely looking at whether a prediction f (x) on y is correct or not, we also consider the margin defined as ζ(f (x), y) = [f (x)]

y −max {j:j =y} [f (x)] j .

Therefore, we can define the ramp loss and margin error depending on the confidence of predictions.

Given two thresholds γ 2 > γ 1 ≥ 0, define a ramp loss to be DISPLAYFORM5 where ∆ := γ 2 − γ 1 .

In particular γ 1 = 0 and γ 2 = γ, we also write γ = γ for simplicity.

Define the margin error to measure if f has margin no more than a threshold γ, DISPLAYFORM6 In particular, e 0 (f (x), y) is the common mis-classification error and DISPLAYFORM7 Note that e 0 ≤ γ ≤ e γ , and γ is Lipschitz bounded by 1/γ.
...
Given a γ 0 > 0, then, for any δ ∈ (0, 1), with probability at least 1 − δ, the following holds for any f ∈ F with f F ≤ L, DISPLAYFORM9 is the Rademacher complexity of function class H L with respect to n samples, and the expectation is taken over x i , ε i , i = 1, ..., n.

Unfortunately, direct application of such bound for a constant γ 0 will suffer from the so-called scaling problem.
...
Given γ 1 and γ 2 such that γ 2 > γ 1 ≥ 0 and ∆ := γ 2 − γ 1 ≥ 0, for any δ > 0, with probability at least 1 − δ, along the training epoch t = 1, . . .

, T , the following holds for each f t , DISPLAYFORM12 where DISPLAYFORM13 Remark.

In particular, when we take γ 1 = 0 and γ 2 = γ > 0, the bound above becomes, DISPLAYFORM14 Theorem 1 says, we can bound the normalized test margin distribution DISPLAYFORM15 Recently BID12 investigates for normalized networks, the strong linear relationship between cross-entropy training loss and test loss when the training epochs are large enough.

As a contrast, we consider the whole training process and normalized margins.

In particular, we hope to predict the trend of generalization error by choosing γ 1 = 0 and a proper γ.

For this purpose, the following facts are important.

First, we do not expect the bound, for example (10), is tight for every choice of γ > 0, instead we hope there exists some γ such that the training margin error nearly monotonically changes with generalization error.

FIG1 shows the existence of such γ such that the training margin error successfully recover the tendency of generalization error on CIFAR10 dataset.

Moreover, in Appendix Figure 8 shows the rank correlation between training margin error at various γ and training/test error.
...
Since for any constant c > 0, normalization byL = cL works in practice where the constant could be absorbed to γ, we could ignore the Lipschitz constant introduced by general activation functions in the middle layers.

However, it is a natural question whether a reasonable γ with prediction power exists.
...
More detailed discussions will be given in Section 3.2.The most serious limitation of Theorem 1 lies in we must fix a γ along the complete training process.

In fact, the first term and second term in the bound (10) vary in the opposite directions with respect to γ, and thus different f t may prefer different γ for a trade-off.

As in FIG0 (b) of the example, while choosing γ is to fix an x-coordinate section of margin distributions, its dual is to look for a y-section which leads to different margins for different f t .

This motivates the quantile margin in the following theorem.

Letγ q,f be the q th quantile margin of the network f with respect to sample S, DISPLAYFORM16 Theorem 2.

Assume the input space is bounded by M > 0, that is x 2 ≤ M, ∀x ∈ X .

Given a quantile q ∈ [0, 1], for any δ ∈ (0, 1) and τ > 0, the following holds with probability at least 1 − δ for all f t satisfyingγ q,ft > τ , DISPLAYFORM17 DISPLAYFORM18 Remark.

We simply denote γ q,t for γ q,ft when there is no confusion.

Compared with the bound (10), (12) make the choice of γ varying with f t and the cost is an additional constant term C 2 q and the constraintγ q,t > τ that typically holds for large enough q in practice.

In applications, stochastic gradient descent (SGD) often effectively improves the training margin distributions along the drops of training errors, a small enough τ and large enough q usually meetγ q,t > τ .

Moreover, even with the choice τ = exp(−B), constant term [log log 2 (4(M + l)/τ )]/n = O( log B/n) is still negligible and thus very little cost is paid in the upper bound.

In practice, tuning q ∈ [0, 1] is far easier than tuning γ > 0 directly and setting a large enough q ≥ 0.9 usually provides us lots of information about the generalization performance.
...
The relations between generalization error and training margin error e γ (f (x), y) with γ = 9.8, inverse quantile margin 1/γ q,t with q = 0.6 are shown in FIG1 .
...
In each figure, we show training error (red solid), training margin error γ = 10 (red dash) and inverse quantile margin (red dotted) with q = 0.6 and generalization error (blue solid).
...
There exists a trade-off on the choice of γ from the linear bounds (9) (and parallel arguments hold for q).

The training margin error with a small γ is close to the training error, while a large γ is close to generalization error and it's illustrated in Appendix Figure 8 where we show the Spearman's ρ rank correlation 1 between training margin error and training error, generalization error against threshold γ.

2.

The training margin error (or inverse quantile margin) is closely related to the dynamics of training margin distributions.

For certain choice of γ, if the curve of training margin error (with respect to epoch) is V-shape, the corresponding dynamics of training margin distributions will have a cross-over, where the low margins have a monotonic increase and the large margins undergo a phase transition from increase to decrease, as illustrated by the red arrow in FIG0 .

3.

Dynamics of quantile margins can adaptively select γ t for each f t without access to the complexity term.

Unlike merely looking at the training margin error with a fixed γ, quantile margin bound (12) shows a stronger prediction power than (10) and even be able to capture more local information as illustrated in FIG2 .
...
However, if we consider the dynamics of training margin errors, it's rarely possible to recover the two valleys at the same time since their critical thresholds γ t1 and γ t2 are different.
...
For convolution operator with kernel w, i.e. W x := w * x, there holds w * x 2 ≤ w 1 x 2 .In other words, W σ ≤ w 1 .Proof.

DISPLAYFORM0 where the second last step is due to Cauchy-Schwartz inequality.

A. 1 -norm.

The convolutional operator (spectral) norm can be upper bounded by the 1 -norm of its kernels, i.e. W σ ≤ w 1 .
...
B are mean and variance of batch samples, while keeping an online averaging asμ andσ 2 .

Then BN rescales x + by estimated parametersα,β and outputx =αx + +β.

Therefore the whole rescaling of BN on the kernel tensor w of the convolution layer isŵ = wα/ √σ 2 + and its corresponding rescaled operator is DISPLAYFORM1 (b) Activation and pooling: their Lipschitz constants L σ can be known a priori, e.g. L σ = 1 for ReLU and hence can be ignored.

In general, L σ can not be ignored if they are in the shortcut as discussed below.(d) Shortcut: In residue net with basic block in FIG5 , one has to treat the mainstream (Block 2 , Block 3 ) and the shortcut Block 1 separately.

Since f + g F ≤ f F + g F , in this paper we take the Lipschitz upper bound by DISPLAYFORM2 where Ŵ i σ denotes a spectral norm estimate of BN-rescaled convolutional operator W i .

In particular L σout can be ignored since all paths are normalized by the same constant while L σin can not be ignored due to its asymmetry.
...
Then (16) follows from the McDiarmid's inequality (Lemma D.2) using B i = B/n and δ = exp(−2nε 2 /B 2 ).As to (17), DISPLAYFORM10 that ends the proof.
...
Without loss of generality, we assume L σi = 1, i = 1, . . .
...
, K} such that where f F ≤ Π l i=1 W i σ = 2L/2 = L, and thus h ∈ H L by definition.
...
Consider l (γ1,γ2) (ζ(f (x), y)), wheref := f /L f is the normalized network, ζ(f (x), y) ∈ G 1 .

Then for any γ 2 > γ 1 ≥ 0, DISPLAYFORM0 ≤ P n (γ1,γ2) (f (x), y) + 2R n (l (γ1,γ2) • G 1 ) + log(1/δ) 2n , ≤ P n (γ1,γ2) (f (x), y) + 2 ∆ R n (G 1 ) + log(1/δ) 2n , ≤ P n γ1,γ2 (f (x), y) + 2K 2 ∆ R n (H 1 ) + log(1/δ) 2n , ≤ P n γ2 (f (x), y) + 2K 2 ∆ R n (H 1 ) + log(1/δ) 2n , where the first and last inequality is implied from 1[ζ < γ 1 ] ≤ (γ1,γ2) (ζ) ≤ 1[ζ < γ 2 ], the second inequality is a direct consequence of Lemma D.1, the third inequality results from Rademacher Contraction Inequality (Lemma D.3) and finally the fourth equation is implied from Lemma D.4.

Proof of Theorem 2.

Firstly, we show after normalization, the normalize margin has an upper bound, DISPLAYFORM0 , where x i = σ i (W i x i−1 + b i ) with x 0 = x,W i = (W i , b i ) and L σi is the Lipschitz constant of activation function σ i with σ i (0) = 0, i = 1, . . .

, L. Then, for normalized networkf = f /L f with DISPLAYFORM1 Therefore ζ(f (x), y) ≤ 2 f (x) 2 = 2(M + L) =: M 1 , and the quantile margin is also bounded γ q,t ≤ M 1 for all q ∈ (0, 1), t = 1, . . .

, T .The remaining proof is standard.

For any > 0, we take a sequence of k and γ k , k = 1, 2, . . .

by k = + log k n and γ k = M 1 2 −k .

Then by Theorem 1, DISPLAYFORM2 where A k is the event P[ζ(f t (x), y) < 0] > P n [ζ(f (x), y) < γ k ] + 2K 2 γ k R(H 1 ) + k , and the probability is taken over samples {x 1 , ...x n }.

We further consider the probability for none of A k occurs, DISPLAYFORM3 2 ), ≤ 2 exp(−2n 2 ).Hence, fix a q ∈ [0, 1], for any t = 1, . . .

, T , as long asγ q,t > 0, there exists ak ≥ 1 such that, γk +1 ≤γ q,t < γk.

Therefore, DISPLAYFORM4 ⊇ P[ζ(f t (x), y) < 0] > P n [ζ(f t (x), y) <γ q,t ] + 4K 2 γ q,t R(H 1 ) + k +1 , = P[ζ(f t (x), y) < 0] > P n [ζ(f t (x), y) >γ q,t ] + 4K 2 γ q,t R(H 1 ) + + log(k + 1) n , ⊇ P[ζ(f t (x), y) < 0] > P n [ζ(f t (x), y) >γ q,t ] + 4K 2 γ q,t R(H 1 ) + + log log 2 (2M 1 /γ q,t ) n .The first inequality is implied from P n [ζ(f t (x), y) <γ q,t ] > P n [ζ(f t (x), y) < γk +1 ], since γk +1 ≤ γ q,t .

The second inequality is implied fromγ q,t < 2γk +1 and thus, 1/γk +1 < 2/γ q,t .

The third equality is the direct definition of k .

The last inequality is implied fromk + 1 = log 2 (M 1 /γk +1 ) and again, 1/γk +1 < 2/γ q,t .

The conclusion is proved immediately if we do a transform from to δ.
