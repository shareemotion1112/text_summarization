...
To obtain a single synthetic minority point x * of class k, our method solves an optimization problem starting from another training sample x 0 of a (relatively) major class k 0 < k: where L CE denotes the standard cross entropy loss and λ > 0 is a hyperparameter.
...
Here, the regularization term λ · f k0 (x) on logit reduces the risk when x * is labeled to k, whereas it may contain significant features of x 0 in the viewpoint of f .
...
where (·) + := max(·, 0), and β ∈ [0, 1) is a hyperparameter which controls the reliability of g: the smaller β, the more reliable g. For example, if β = 0.999, the synthetic samples are accepted with probability more than 99% if N k0 − N k > 4602.

When β = 0.9999, on the other hand, it requires N k0 − N k > 46049 to achieve the same goal.
...
We only accept the result sample x * only if L(g; x * , k) is less than γ > 0 for stability.
...
We choose hyperparameters in our method from a fixed set of candidates, namely β ∈ {0.99, 0.999}, λ ∈ {0.1, 0.5} and γ ∈ {0.9, 0.99}, based on its validation accuracy.
...
To simulate the long-tailed distribution frequently appeared in imbalanced datasets, we control the imbalance ratio ρ > 1 and artificially reduce the training sample sizes of each class except the first class, so that: (a) N 1 /N K equals to ρ, and (b) N k in between N 1 and N K follows an exponential decay across k. We keep the test dataset unchanged during this process, thereby the evaluation can be done in the balanced setting.

We compare the (balanced) test accuracy of various training methods (including ours) on CIFAR-LT-10 and 100, considering two imbalance ratios ρ ∈ {100, 10} for each (See Figure 2 (a) and 2(b) for an illustration of the sample distribution).
...
In the optimization objective (2) for the synthesis in AMO, we impose a regularization term λ · f k0 (x) to improve the quality of synthetic samples as they might confuse f if it still contains important features of the original class in a viewpoint of f .

To verify the effect of this term, we consider an ablation that λ is set to 0, and compare the performance to the original method.
...
Each embedding is computed from a randomly-chosen subset of training samples in the CIFAR-LT-10 (ρ = 100), so that it consists of 50 samples per each class.
