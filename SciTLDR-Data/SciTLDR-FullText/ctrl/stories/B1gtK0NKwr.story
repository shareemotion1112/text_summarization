...
The state of the world is therefore characterized by the offset δ and the viewing angle θ.

A neural network takes the camera image as input and predicts the offset and the viewing angle.

The state space includes the δ and θ of interest.
...
We tile the state space using a grid on (δ, θ).

Each state tile gives a bound on the ground truth of δ and θ.
...
We denote the ground truth of y using a function λ : S → Y. This specifies the ground truth for each input, which we denote as a mappingf :X → P(Y).f (x) is the set of possible ground truth values of y for a given x:

f (x) = {y|∃s ∈ S, y = λ(s), x ∈ g(s)}.
...
X andf are instead determined implicitly by S, g, and λ.
...
Problem formulation of correctness verification: Given a trained neural network f and a specification (X ,f ) determined implicitly by S, g, and λ, compute upper bounds on error e(x) for any feasible input x ∈X .
...
The state of the world s is characterized by the offset δ and angle θ of the camera position.

We therefore label the states as s δ,θ .

We consider the camera position between the range δ ∈ [−40, 40] (length unit of the scene, the road width from the centerline to the side lines is 50 units) and θ ∈ [−60
...
The feasible input spaceX is the set of all images that can be taken with δ ∈ [−40, 40] and θ ∈ [−60

The quantity of interest y is the camera position (δ, θ).

The ground truth function λ is simply λ(s δ,θ ) = (δ, θ).
...
The output layer is a linear layer with 2 output nodes, corresponding to the predictions of δ and θ.

The network is trained on 130k images and validated on 1000 images generated from our imaging process.

The camera positions of the training and validation images are sampled uniformly from the range δ ∈ [−50, 50] and θ ∈ [−70

The network is trained with an l 1 -loss function, using Adam (Kingma and Ba, 2014) (see Appendix E for more training details).

For error analysis, we treat the predictions of δ and θ separately.

The goal is to find upper bounds on the prediction errors e δ (x) and e θ (x) for any feasible input x ∈X .

Tiler Figure 1c presents a schematic of how we apply Tiler to this problem.

Tiles are constructed by dividing S on (δ, θ) into a grid of equal-sized rectangles with length a and width b. Each cell in the grid is then We next encapsulate each tile X i with an l ∞ -norm ball B i by computing, for each pixel, the range of possible values it can take within the tile.
...
For each l ∞ -norm ball, we solve 4 optimization problems: maximizing and minimizing the output entry for δ, and another two for θ.

Denote the objectives solved as δ Experimental results We run Tiler with a cell size of 0.1 (the side length of each cell in the (δ, θ) grid is a = b = 0.1).
...
We compute global error bounds by taking the maximum of e i δ and e i θ over all tiles.

The global error bound for δ is 12.66, which is 15.8% of the measurement range (80 length units for δ); for θ is 7.13
...
We present the visualizations of the error bound landscape by plotting the error bounds of each tile as heatmaps over the (δ, θ) space.

Figures 2a and 2d present the resulting heatmaps for e i δ and e i θ , respectively.

To further inspect the distribution of the error bounds, we compute the percentage of the state space S (measured on the (δ, θ) grid) that has error bounds below some threshold value.
...
To study the tightness of the error bounds, we compute empirical estimates of the maximum errors for each S i , denoted asē i δ andē i θ .

We sample multiple (δ, θ) within each cell S i , generate input images for each (δ, θ), then take the maximum over the errors of these points as the empirical estimate of the maximum error for S i .
...
We take the maximum ofē i δ 's andē i θ 's to get a lower bound estimate of the global maximum error.

The lower bound estimate of the global maximum error for δ is 9.12 (11.4% of the measurement range); for θ is 4.08
...
We run Tiler with a sequence of cell sizes (0.05, 0.1, 0.2, 0.4, 0.8) for the (δ, θ) grid.
...
The sensor can vary its distance d and angle θ with respect to the sign, but its height is fixed, and it is always facing towards the sign.

Figure 4a shows the schematic of the set-up.

Assume the working zone for the LiDAR sensor is with position d ∈ [30, 60] and θ ∈ [−45

• , 45 • ].

Then the state space S has 3 dimensions: two continuous (d and θ), and one discrete (sign shape c).
...
We divide the θ dimension uniformly into 90 intervals and the d dimension uniformly in the inverse scale into 60 intervals to obtain a grid with 5400 cells per shape.
...
We extend this lower and upper bound by 5σ, where σ is the standard deviation of the Gaussian measurement noise.

This way we have P (x ∈ B i |x ∼ g(s), s ∈ S i ) >= (P (|a| <= 5σ|a ∼ N (0, σ 2 ))) N > 0.999, where N = 32 × 32 is the input dimension.
...
Step 2: For each S i , compute the ground truth bound as a set C i ⊆ Y, such that ∀s ∈ S i , λ(s) ∈ C i .
...
But {i|x ∈ X i } ⊆ {i|x ∈ B i } (Condition 4.3), which giveŝ
...
Input: S, g, λ, f Output:

e global , {e i }, {B i } 1: procedure TILER(S, g, λ, f ) 2:
...
For each pixel in the lower half of the image, if we trace the intersection point between the projection ray from this pixel and the road plane, it will sweep over a closed region as the camera position varies in the δ-θ cell.
...
The extrema on x can only be achieved at: 1) the four corners of the δ-θ cell; 2) the points on the two edges δ = δ 1 and δ = δ 2 where θ gives the ray of that pixel perpendicular to the y axis (if that θ is contained in [θ 1 , θ 2 ]).
...
The distance measurement contains a Guassian noise n ∼ N (0, σ 2 ), where σ = 0.001 × MAX_RANGE.
...
There is one exception: at d = d 1 , as θ varies in [θ 1 , θ 2 ], if the intersection point shifts from sign to background (or vice versa), then the minimum distance of p can occur at (d 1 , θ ) where θ is not equal to θ 1 or θ 2 .

To handle this case, if at the previous step we find that p do occur on the sign plane in the tile, then we add the distance of the intersection point between the beam and the sign plane at position (d 1 , θ 1 ) (or (d 1 , θ 2 ), whichever gives a smaller distance) to the list of critical distances.
