...
The algorithm updates parameter vector x k according to DISPLAYFORM0 whereḡ k is the mini-batch stochastic gradient and k is the learning rate.
...
Upon receiving query x, the stochastic gradient oracle gives us an independent estimateĝ satisfying DISPLAYFORM2 Remark: this assumption is standard for stochastic optimization, except that the variance upper bound is now stated for every dimension separately.
...
We wanted to investigate the naïve intuition that gradient rescaling should help flee saddle points-or in the words of Zeyuan Allen-Zhu-swing by them.
