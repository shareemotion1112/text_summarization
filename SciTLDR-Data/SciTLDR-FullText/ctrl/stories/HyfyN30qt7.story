...
Instead of using the quantized valueŵ = Q ∆ (w) of a weight w in the forward pass,ŵ = (1 − M )Q ∆ (w) + M (w − e) is used with e ∼ Uni(−∆/2, ∆/2), where ∆ denotes size of the quantization bin.
...
In the case of a pre-trained network destined for quantization, we have found that the optimal block size is a single layer with the corresponding activation, while using more than one epoch of training with the noise injection per block does not improve performance. (The parameter c w is defined per layer and is initialized with c w = mean(w) + β × std(w), where w are the weighs of the layer and β is a hyper-parameter.

Given c w , we uniformly quantize the clamped weight into B w bits according toŵ DISPLAYFORM0 where [·] denotes the rounding operation.
...
We used the initialization c a = mean(a) + α × std(a) with the statistics computed on the training dataset and α set as a hyper-parameter.
...
In this experiment α was set to 5.
