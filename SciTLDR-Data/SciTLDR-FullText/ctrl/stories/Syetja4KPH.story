...
An MDP is a tuple < S, A, P, R, γ >, where S is a state space, A a finite action space, P : S × A − → ∆(S), is a transition kernel, and R : S × A − → R a reward function.
...
As mentioned above, the agent seeks an optimal policy π * : S − → ∆(A), mapping an environment state to probabilities over the agent's executable actions.

γ ∈ (0, 1) is the discount factor -a scalar representing the trade-off between immediate and delayed reward.
...
The Q-function is assumed to be in the form Q(s, a) = φ(s, a)

T w, where φ(s, a) is a hand-designed state-action representation.
...
Given a state s t , the activations of the last hidden layer of the target network applied to this state are denoted as φ(s t ) = LastLayerActivations(Q θtarget (s t )).
...
2 ), however, like Riquelme et al. (2018) the noise parameter σ 2 is formulated as a random variable, which is distributed according to the Inverse-Gamma distribution.
...
Formulating σ 2 as a random variable allows adaptive exploration where the adaptation is derived directly from the observed data.
...
Since RLSVI requires a fixed representation and the target network's weights are fixed, we use the last layer activations of the target network, denoted φ(s), as state-representation.
...
We evaluate both old and new representation {φ(

.

Our goal is to find a solution Σ ψ 0 that will match the covariance of the likelihood in the new representation to the old one:

Using the cyclic property of the trace, this is equivalent to finding Trace(ψ(

Adding the constraint that Σ ψ 0 should be Positive-Semi-Definite as it is a covariance matrix, we end up with the following Semi-Definite Program (SDP) (Vandenberghe & Boyd, 1996) :
...
Then, every step we sample an index i a ∈ 1, .., j for every action, which is computationally cheap, and act greedy accordingly: a t = arg max αw T α,iα φ(s t ).
...
In this experiment, we compared our algorithm with variants that do not model σ 2 as a random variable.

We experimented with various constant σ 2 values.

We can see that modeling σ 2 as a random variable (red) leads to lower regret compared to constant σ 2 variants (Figure 2 (b) ).

Note that choosing a small value for σ 2 (blue) results in near-deterministic posterior function.
...
On the other hand, choosing a high value for σ 2 (purple) results in a very noisy sampling of the posterior approximation, therefore we get a policy which is relatively random concluding in a bad performance.

Choosing σ 2 with the appropriate size for the given MDP (green, black) results in better performance, as indicated by the lower regret.

However, as σ 2 is constant it doesn't adapt.
...
The algorithm maintains two DNNs, the Q-network with weights θ and the target network with weights θ target .
...
The algorithm uses Stochastic Gradient Descent (SGD) to update the network's weights, by minimizing the mean squared error of the Bellman equation defined as E[||Q θ (s t , a t ) − y t || 2 , where the target y t = r t if s t+1 is terminal, otherwise y t = r t + γ max a Q θtarget (s t+1 , a ).

The weights of the target network are set to θ every fixed number of time steps, T target .
...
Input: Q θ (s, a), Q θtarget (s, a), , ER buffer s 0 = EnvironmentReset() for t = 0, 1, ...
