...
Most algorithms for RNA Design employ a structural loss function L ω (F(φ)) to quantify the difference between the target structure ω and the structure resulting from folding an RNA sequence φ.

A minimizer of this loss corresponds to a solution to the RNA Design problem for a specified target structure ω: DISPLAYFORM0 A common loss function, which we also employ in this work, is the Hamming distance (Hamming, 1950) between two structures.
...
We propose to model the RNA Design problem with respect to a given target structure ω as the undiscounted decision process D ω := (S, A, R ω , P ω ); its components (the state space S, the action space A, the reward function R ω and the transition function P ω ) are specified in the paragraphs below.

The RNA Design problem is defined with respect to a folding algorithm, which we denote as F(·); further, we denote the set of dot-bracket encoded RNA secondary structures with Ω .Action space In each episode, the agent has the task to design an RNA sequence that folds into the given ω ∈ Ω. To design a candidate solution φ ∈ N |ω| , the agent places nucleotides by choosing an action a t at each time step t. For unpaired sites, a t corresponds to one of the four RNA nucleotides (G, C, A or U); for paired sites, two nucleotides are placed simultaneously.

In our formulation, these two nucleotides correspond to one of the Watson-Crick base pairs (GC, CG, AU, or UA).

At time step t, the action space can then be defined as DISPLAYFORM0 where C ω (t) is the t-th character of the target structure ω.
...
For this we set s t to the (2κ + 1)-gram centered around the t-th site of the target structure ω, where κ is a hyperparameter we dub the state radius.

To be able to construct this centered n-gram at all sites, we introduced κ padding characters at the start and the end of the target structure.
...
Reward Function At the terminal time step T the agent has assigned nucleotides to all sites of the candidate solution φ and the environment generates the (only non-zero) reward R T ω (φ).

This reward is based on the Hamming distance d H (F(φ), ω) between the folded candidate solution F(φ) and the target structure ω.

We normalize this distance with respect to the sequence length |ω| to formulate the loss function L ω (F(φ)) := d H (F(φ), ω) / |ω|.

To solve the optimization problem in Equation 1, we set DISPLAYFORM3 where α > 1 is a hyperparameter to shape the reward.

Additionally, we include a local improvement step to increase sample efficiency and boost performance of the stochastic RL agent as follows: If d H (F(φ), ω) < ξ, where ξ is a hyperparameter, we search through neighboring primary sequences by exhaustively trying all combinations for the mismatched sites, returning the minimum Hamming distance observed.

In our experiments, we set ξ = 5, which corresponds to at most 4 4 = 256 neighboring sequences.

Pseudocode for computing R T ω (φ) can be found in Appendix A.

We use deep reinforcement learning to learn the parameters θ of policy networks π θ .
...
We propose several strategies to learn the parameters θ of a given policy network as detailed below.

LEARNA The LEARNA strategy learns to design a sequence for the target structure ω in an online fashion, from scratch.

The parameters θ are randomly initialized before the agent episodically interacts with the decision process D ω .
...
Each of the target structures ω i ∈ Ω train defines a different decision process D ωi ; using asynchronous parallel PPO updates, we train a single policy network on all of these.

Once the training is finished, the parameters θ are fixed and π θ can be applied to the decision process D ω by sampling from the learned generative model.

Meta-LEARNA-Adapt Meta-LEARNA-Adapt combines the previous two strategies: First, we obtain an initialization for the parameters θ by running Meta-LEARNA on Ω train .

Then, when applied to the decision process D ω , the parameters θ are further adapted using the LEARNA strategy.
