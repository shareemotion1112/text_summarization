...
If the model has p real valued parameters, taking values within a set Θ ⊆ R p , the parameter-function map, M, is defined as: DISPLAYFORM0 where f θ is the function implemented by the model with choice of parameter vector θ.
...
We put a probability distribution over the space of parameters Θ, and are interested in the distribution over functions induced by this distribution via the parameter-function map of a given neural network.
...
Given some distribution over parametersP (θ), the distribution over functions P (c) is determined by the parameter-function map as P (c) =P (M −1 (c)).
...
The entries of the covariance matrix K are given by the kernel function DISPLAYFORM0 The kernel function depends on the choice of architecture, and properties ofP , in particular the weight variance σ 2 w /n (where n is the size of the input to the layer) and the bias variance σ 2 b .
...
For binary classification, these labels are binary, and are related to the real-valued outputs of the network via a nonlinear function such as a step functionwhich we denote σ.

Then, for a training set DISPLAYFORM1 This distribution no longer has a Gaussian form because of the output nonlinearity σ.
...
The Gaussian process parameters are σ w = 10.0, and σ b = 10.0.

For advSGD, we have removed functions which only appared once in the whole sample, to avoid finite-size effects.

In the captions, ρ refers to the 2-tailed Pearson correlation coefficient, and p to its corresponding p value.
...
Note that for the fully connected network, the variance of the weights σ w seems to have a much bigger role.

This is consistent with what is found in Lee et al. (2017) .

Furthermore, in Lee et al. (2017) they find, for smaller depths, that the neural network Gaussian process behaves best above σ w ≈ 1.0, which marks the transition between two phases characterized by the asymptotic behavior of the correlation of activations with depth.
...
For example, the Gaussian kernel ensures that points within a ball of radius λ are highly correlated.

On the other hand, points separated by a distance greater than λ can be very different.

Intuitively, we can divide the space into regions of length scale λ.

If the input domain we re considering has O(1) volume, and has dimensionality d (is a subset of R d ), then the volume of each of these regions is of order λ d , and the number of these regions is of order 1/λ d .
...
The number of such functions is 2 a d , where we let a := 1/λ.
...
For instance, the concepts of minimum description length (MDL) BID9 , Blumer algorithms Blumer et al. (1987); BID22 , and universal induction Ming & Vitányi (2014) all rely on a bias towards simple hypotheses.
...
The simplicity bias from Dingle et al. Dingle et al. (2018) arises from a simpler version of the coding theorem of Solomonoff and Levin Ming & Vitányi (2014) .
