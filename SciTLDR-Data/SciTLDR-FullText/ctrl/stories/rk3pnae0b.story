...
As illustration 2 shows, our model uses two internal representations (can be assumed as sampled from ϕ(A l ) and ϕ(Z s ) respectively) to generate a question (can be assumed as sampled from ϕ(Q sl )) which learns a distribution mapping method DISPLAYFORM1 .

When given an answer a i and a tqt z i which never appeared as a pair in the training data, benefited from the distribution mapping, our model can generate a question similar to q j because a i and a j are from the same distribution ϕ(A l ) and, z i and z j are from the same distribution ϕ(Z s ).
...
Here we only give the calculation formula for tc t : DISPLAYFORM5 where b i is the hidden state at time step i for the bidirectional LSTM in topic encoder, and its weight α i is calculated by: DISPLAYFORM6 where e i = h t W a b i scores how well h t and b i match.
...
The weighted sum tc t in equation 7 needs to add the weight (or filter) γ i calculated by the pre-decode result, denoted as tc t : DISPLAYFORM8 where DISPLAYFORM9 where e i = f t W a b i ; α i has the same calculation method as that for equation 8 except e i = h t W a b i .
...
We set the topic selection threshold K to 0.4, and the word similarity threshold λ to 0.3 (section 4.1).
