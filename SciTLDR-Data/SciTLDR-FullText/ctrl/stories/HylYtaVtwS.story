...
Let φ(x) denote the ddimensional embedding of an image x into this layer.

The cache is then a key-value dictionary consisting of the keys µ k ≡ φ(x k ) for each training image x k in the dataset and the values are the corresponding class labels represented as one-hot vectors v k (Orhan, 2018) .
...
The hyper-parameter θ in Equation 1 acts as an inverse temperature parameter for this distribution, with larger θ values producing sharper distributions.

We optimized θ only in one of the experimental conditions below (the graybox adversarial setting) by searching over 9 uniformly spaced values between 10 and 90 and fixed its value for all other conditions.
