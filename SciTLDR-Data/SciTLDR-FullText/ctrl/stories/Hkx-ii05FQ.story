...
Let X be a random variable that is defined over X , and which is distributed according to a parametric distribution P θ that the algorithm maintains.
...
During the optimization, the algorithm iteratively samples solutions x according to P θ , and updates the parameters θ ∈ R d which govern P θ in a manner that reflects how good is the objective value y = f (x) with which x is associated.

Initially P θ (X = x) is multi-variate uniform (fully specified in section 5), but as the optimization continues, the algorithm decreases the entropy in the distribution until eventually only few solutions become likely, and sampling some x from P θ , with high probability, returns some locally optimal solution.

Since we discuss an iterative algorithm that at each iteration t updates the parameters θ t , we refer to the random variables that are associated with P θt by X t and Y t .

Lastly, as a short hand notation, we refer to P θ (X = x) simply by P θ (x).Since we learn a distribution function, we say that our learning objective J(θ) is to maximize the expectation over x ∼ P θ of the original objective which we denote as E θ [Y ] .

To find the parameters θ which maximize E θ [Y ], we derive a gradient ascent algorithm which relies on estimates of DISPLAYFORM1 To calculate the gradient, we use the log-derivative trick, DISPLAYFORM2 .

Next, we can use Monte Carlo estimation BID27 DISPLAYFORM3 of some fixed size K is sampled using P θt .

Denoting this estimate by ∆ t , then the update at iteration t takes the following form, DISPLAYFORM4 (1) DISPLAYFORM5 where η t is a learning rate parameter that is predetermined.
...
Turns out that for positive learning rates this stochastic optimization scheme converges to a local maximum of J, and when using the optimal parameters θ * , sampling from P θ * returns locally optimal solutions x * ∈ X * f with high probability BID30 .
...
We start by examining equations 1 and 2 and observing that if we update θ t in the direction of ∇ θ log P θ (x t ), η t y t can be considered as the step we're taking in that direction.

Thus, the sign and magnitude of η t y t essentially determine whether we increase or decrease the likelihood of x t , and to what extent we do so.

The implication this has over the optimization is that the distributions of {η t Y t } T t=1 determine the course of the optimization.

If for example |η t Y t | is unbounded from above, we might take steps that are too large, which might cause us to diverge.
...
Lastly, since in general we don't know ahead of time the distribution of each Y t , it seems that if we follow the construction presented in section 2, we won't be able to determine the series {η t } T t=1 in a manner that would fit all scenarios.
...
Due to equation 2, if we use a sampling distribution for which ∇ θ log P θ (x t ) is bounded, then since w (y t ) is bounded as well, ∆ t will be bounded for every x t and y t .

The main implication of this property is that we can control how different the parameters will be between any two iterations, i.e., that for any two iterations t and t − k where k ∈ [t − 1] we can make θ t − θ t−k as small as we want simply by changing η t .

Thus, instead of drawing a large sample in each iteration, we can say the last objective values y t−1 , . . .

, y t−k are approximately i.i.d from P θt−1 .

Therefore, if we use small enough learning rates, we can usê DISPLAYFORM0 is the indicator function.

In our experiments, using some fixed learning rate η ∈ (0, 1) along with k = 1 η seem to work quite well.
...
The purpose of the first option we present is to illustrate the connection between our algorithm, and the CE method, and for that reason we denote this weight function byŵ CE (y) = g CE F (y) , and its associated transformation by g CE .

Given some small ρ ∈ [0, 1] which is decided by the user a-priori (typically, 0.1 or 0.01), g CE is a thresholding function g CE (z) = I [z ≥ 1 − ρ].

Clearly, for any fixed ρ,ŵ CE is monotonic increasing and bounded, andŵ CE (Y ) is a Bernoulli random variable with probability ρ.

Notably, using g CE in equation 3 leads to an update which can be considered as an online version of the CE method.

There are two main disadvantages toŵ CE .

First of all, it relies on another parameter ρ that requires manual tuning.

More importantly,ŵ CE uses only the highest ρ percentile of the examples to update P θ while in fact the worst xs supply valuable information -they have low objective values, and thus, their likelihood should be decreased rather than simply ignored.
...
To do so, we makeŵ (y) = 2F (y) − 1.

By construction, it follows thatŵ (Y ) is uniform discrete on [−1, 1].

In this fashion, when applied with some fixed learning rate,ŵ determines whether the likelihood of DISPLAYFORM0 * which had the highest y * some example will be increased or decreased, and to what extent.

Notably, this is achieved along with full specification of the distribution ofŵ (Y ).
...
We summarize Cakewalk withŵ, and any gradient addition rule Add (this includes hyper-parameters such as learning rate) in algorithm 1.
...
Turns out that when CE is applied with discrete sampling distributions the likelihood-ratio term cancels out, and the construction is equivalent to maximizing the likelihood of the examples whose objective values belong to the highest ρ percentile BID9 .

Thus, in this case CE's update step is equivalent to maximizing the surrogate objectiveŵ CE described in section 3.1.

As discussed in section 3.1,ŵ CE has two major shortcomings, and these lead us to suggest a different surrogate objective which makes the basis for Cakewalk.
...
For example, actor critic methods Sutton & Barto (2017) use estimatesμ of E (Y ) that are produced with some model of the objective, and can be used to make y −μ zero mean.
...
In this manner, the number of parameters required to represent P θ (x) grows only linearly with M N , instead of the exponential number of parameters that is required to represent the full joint distribution.

Formally, each x j is drawn independently according to a softmax distribution P θ (x j = i) = Next, we describe ∇ θ log P θ (x) in terms of partial derivatives, DISPLAYFORM0 Note that since ∇ θ log P θ (x) is bounded, we can estimateF in an online manner.
...
Next, we add a parameter κ ∈ [0, 1] that rewards larger subgraphs, and which could indicate to an algorithm it should prefer larger subgraph over smaller ones.

To achieve this, we change aforementioned denominator to |U x | (|U x | − 1 + κ).

Lastly, to avoid division by zero for cases |U x | < 2, we can wrap the denominator with max (·, 1).

Altogether, DISPLAYFORM1 max (|U x | (|U x | − 1 + κ) , 1) To see why higher κ can reward larger cliques we focus on the case that |U x | ≥ 2, and observe that for U x which is clique, when κ = 0, f SCS (x, G, 0) = 1.

However, when κ = 1, f SCS (x, G, 1) = |Ux|−1 |Ux| , and thus, the larger U x is, the closer this ratio is to 1.

In this manner, increasing κ gives larger subgraphs a 'boost' compared with smaller one, though it could be that some subgraph which isn't a clique will receive a higher value than some smaller subgraph which is a clique (only for κ = 0 we get that f SCS (x, G, 0) = 1 necessarily means that U x is clique).

Empirically, we see that the algorithms we've tested aren't very sensitive to the value of κ.
...
We tested each method on all 80 graphs, letting it maximize the soft-clique-size function using various values of κ.

To determine if a clique is inclusion maximal, since a-priori we don't know which κ will lead to such clique, we've executed each method using each of the values 0.0, 0.1, . . .
...
As we focus on online algorithms, for CE, we used the online version that we derived in this work, using two threshold values suggested by Rubinstein, ρ = 0.1 and ρ = 0.01, and refer to these as OCE 0.1 and OCE 0.01 , with O standing for online.

Next, we've experimented with three versions of REINFORCE.

First is the vanilla version, second is a version where the meanμ is subtracted from y as a baseline, and a third uses the objective's estimated z-score y−μ σ .

We refer to these by REINF, REINF B , and REINF Z .

For Cakewalk, we used both the unscaled empirical CDFF , and its scaled counterpart w, denoting these as CWF and CWŵ.

Note however that the former is only used for comparisons, and that we identify Cakewalk with the latter.

For estimatingμ,σ andF , we've used the last 100 objective values, and thus, both REINF B , and REINF Z make for important comparison as these only transform the objective values, but do not fix its distribution as CE and Cakewalk do.
...
Altogether, we've tested 8 optimization methods, 3 update steps, on 80 graphs, and 11 values of κ, leading to a total of 21120 separate executions.
...
Then, to test inclusion maximality of the returned solutions, since the soft-clique-size doesn't guarantee convergence to cliques, for every graph, we tested whether a method returned at least one inclusion maximal clique when applied with some κ.
...
Lastly, we performed multiple hypothesis tests to compare every optimizer to CWŵ in all the experimental conditions using one sided sign test BID11 , and to control the false discovery rate Wasserman (2013), we determined the significance threshold at a level of 10 −2 using the Benjamini-Hochberg method BID27 .
...
Both CWŵ and OCE 0.1 rely on such surrogates, and both outperform Exp3 and all versions of REINFORCE which do not employ such surrogates.
...
CWŵ clearly outperforms OCE 0.1 in table 1, and the latter only comes close in the more permissive comparison which selects the best result out of 11 different executions (different values of κ) as reported in table 2.

In terms of sample efficiency, the results in table 3 show that even though OCE 0.1 can recover locally optimal solutions, it is not as efficient as CWŵ which finds the best solution considerably faster.

When considering the various gradient updates, it appears that CWŵ with AdaGrad produces the best combination as it outperforms all others methods in almost all measures (CWŵ with Adam converges slightly faster, though at the cost of worse optimality rates).
...
We applied AdaGrad with δ = 10 −6 , and Adam with β 1 = 0.9, β 2 = 0.999, = 10 −6 .
...
We used Cakewalk with AdaGrad BID10 using the same hyper-parameters as specified in section A, except for the learning rate η, which was set to 0.02 instead of 0.01 as we've seen that it led to faster convergence than the latter.
