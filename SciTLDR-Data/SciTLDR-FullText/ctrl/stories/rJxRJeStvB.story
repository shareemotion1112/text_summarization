...
time τ i to complete -we call this the processsing time.
...
However, we can't compute {Q k (h k )} since we are not given φ (H k |H i ) nor φ (H k |X k ).
...
structure2vec embeds the mean-field inference procedure, i.e. fixed point iteration on {Q k (h k )}, into fixed point iterations of neural networks on vectors {μ k }.

Derivation of such fixed point iterations of neural networks can be found in Dai et al. (2016) and can be written asμ k = σ W 1 x k + W 2 j =kμ j where σ denotes Relu function and W denotes parameters of neural networks.
...
Given that we start from state s t and action a t , consider a random experiment "sequential decision making using policy φ".
...
where Z k is a normalizing constant and φ m is the clique potential for clique m.
...
The fixed point iteration in Theorem 1 for posterior marginal P ({H k }|{x k }) can be embedded in a nonlinear function mapping with embedding vectorμ k as
...
The output vectors {μ 1 k } of structure2vec embeds a local graph information around that vector node [Dai et al. (2016) ].
...
According to [Dai et al. (2016) ], the output vectors {μ 1 k } of structure2vec will include sufficient information about the relative graphical distance from all robots to each task.
...
Remind that vectors {μ 1 k }, output vectors of the first step, carries information about the relative graphical distance from all robots to each task.

We concatenate 'age' of each tasks {age k } to each corresponding vector in {μ 1 k } and use the resulting graph as an input ({x k } in lemma 1) of second structure2vec, as illustrated in Figure 1 .

Again, vectors {μ 2 k } of the output graph of second structure2vec operation embeds a local graph structure around each node.

Our intuition is that {μ 2 k } includes sufficient information about 'How much value is likely in the local graph around each task'.

Step 3.

Computing Q(s t , a t ).

To infer Q(s t , a t ), we aggregate the embedding vectors for all nodes, i.e.,μ 2 = kμ 2 k to get one vectorμ 2 which embeds the 'value likeliness' of the global graph.

We then use a layer of neural network to mapμ 2 into Q(s t , a t ).
...
We call it auction-based policy(ADP) and denote it as φ Q θ , where Q θ indicates that we compute φ Q θ using current Q θ estimator.

At time-step t, a state s t is a graph G t = (R t ∪ T t , E t ) as defined in section 2.1.

Our ADP, φ Q θ , finds an action a t (which is a matching in bipartite graph ((R t ∪ T t ), E RT t ) of graph G t ) through iterations between two phases: the bidding phase and the consensus phase.
...
Then the central decision maker chooses Y as φ Q θ (s k ).

One can easily verify that the computational complexity of computing φ Q θ is O (|L R | |L T |), which is only polynomial.
...
In fitted Q-iteration, we fit θ of Q θ (s t , a t ) with stored data using Bellman optimality equation.

That is, chooses θ that makes

small.

Note that every update of θ needs at least one max-operation.

To solve this issue, we suggest a learning framework we call auction-fitted Q-iteration.

What we do is simple: when we update θ, we use auction-based policy(ADP) defined in section 3.3 instead of max-operation.

That is, we seek the parameter θ that minimizes
...
Recall that we use Q θ (s k , a k ) to get policy φ Q θ (s k ).

Note that θ denotes all neural network parameters used in the structure2vec iterations introduced in Section 5.

Since Q θ (s k , a k ) is parametrized by θ, exploration with φ Q θ (s k ) can be performed by exploration with parameter θ.
...
When conducting exploration, apply a random perturbation on the neural network parameters θ in structure2vec.

The resulting a perturbation in the Q-function used for decision making via the auction-based policy φ Q θ (s k ) throughout that problem.
...
We consider two reward rules: Linearly decaying rewards obey f (age) = 200 − age until reaching 0, where age is the task age when served; For nonlinearly decaying rewards, f (t) = λ t for λ = 0.99.
...
Given starting state s t and action a t , a person can repeat a random experiment of "sequential decision making using policy φ".
...
For example, suppose that at time-step t we are given robots {A, B}, tasks {1, 2, 3, 4, 5}, and policy φ.
