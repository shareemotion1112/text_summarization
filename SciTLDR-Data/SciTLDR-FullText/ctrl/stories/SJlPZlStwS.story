...
As a simplified backbone of EdgeGANRob, EdgeNetRob consists of two stages: First, we exploit an edge detection method to extract edge maps from an image, and then a standard image classifier f θ (·) is trained on the extracted edge maps.
...
(3)Noise masking: We reduce the noise in the presence of adversarial perturbations by thresholding the gradient magnitudes by a level α.
...
(5)Double threshold: Using a lower threshold and a higher threshold (θ l , θ h ) for the gradient magnitude after NMS, pixels are mapped to 3 levels: strong, weak, and non-edge pixels, (6)Edge tracking by hysteresis: Edge pixels are detected by finding strong pixels, or weak pixels that are connected to other strong pixels.
...
Thus, we set all gradient magnitudes lower than a threshold α to zero to mitigate the perturbation noise.
...
In addition to the masking operation, we find that the parameters of Canny (e.g. standard deviation of gaussian filter σ, thresholds θ l , θ h ) also affect the robustness level.

Specifically, we notice that larger σ and higher thresholds θ l , θ h result in better robustness due to the stronger smoothing and pruning effects.

This however comes at the cost of clean accuracy as larger σ leads to blurrier images and higher θ l , θ h may eliminate useful information in the output edges.

To obtain a robust edge detector, we should carefully choose its parameters (e.g., σ, θ l , θ h ).
...
where L adv , L F M denote the adversarial loss (Goodfellow et al., 2014a) and feature matching loss (Johnson et al., 2016) with λ adv and λ F M control their relative importance.
...
For Fashion MNIST, we set σ = 1, θ l = 0.1, θ h = 0.2, α = 0.3.

For CelebA, we set σ = 2.5, θ l = 0.2, θ h = 0.3, α = 0.2.
