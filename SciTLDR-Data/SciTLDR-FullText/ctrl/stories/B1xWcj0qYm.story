...
Denote byỹ andp(·) the corrupted label and distributions.
...
To the best of our knowledge, the proposed method is the first MCD method based on ERM.3 LEARNING FROM ONE SET OF U DATA From now on, we prove that knowing π p and θ is insufficient for rewriting R(g).
...
In Eq. (6), the expectation is with respect to p tr and θ is a free variable in it.

The impossibility will be stronger, if θ is unspecified and allowed to be adjusted according to π p .Theorem 2.

Let be 01 , or any bounded surrogate loss satisfying that DISPLAYFORM2 Assume p p and p n are almost surely separable.

Then, R(g) is not rewritable though θ is free.
...
From now on, we prove that knowing π p , θ and θ is sufficient for rewriting R(g).
...
There are two free variables θ and θ in p tr and p tr .

The possibility will be stronger, if θ and θ are already specified and disallowed to be adjusted according to π p .Theorem 4.

Fix θ and θ .

Assume θ > θ ; otherwise, swap p tr and p tr to make sure θ > θ .
...
DISPLAYFORM6 Special cases Consider some special cases of (10) by specifying θ and θ .

It is obvious that (10) reduces to (5) for supervised learning, if θ = 1 and θ = 0.

Next, (10) reduces to DISPLAYFORM7 if θ = 1 and θ = π p , and we recover the unbiased risk estimator in positive-unlabeled learning BID10 BID18 .

Additionally, (10) reduces to a fairly complicated unbiased risk estimator in similar-unlabeled learning BID1 , if θ = π p , θ = π 2 p /(2π 2 p − 2π p + 1) or vice versa.
...
X tr and X tr of the same sample size are drawn according to Eq. (1) , where θ and θ are chosen as 0.9, 0.1 or 0.8, 0.2.
...
When θ = 0.9 and θ = 0.1 (cf.

the left column), UU is comparable to PN oracle in most cases.

When θ = 0.8 and θ = 0.2 (cf.

the right column), UU performs slightly worse but it is still better than small PN baselines.

This is because the task becomes harder when θ and θ become closer, which will be intensively investigated next.

On the closeness of θ and θ It is intuitive that if θ and θ move closer, X tr and X tr will be more similar and thus less informative.

To investigate this, we test UU and CCN BID31 on MNIST by fixing θ to 0.9 or 0.8 and gradually moving θ from 0.1 to 0.5, and the experimental results are reported in FIG3 .

We can see that when θ moves closer to θ, UU and CCN become worse, while UU is affected slightly and CCN is affected severely.

The phenomenon of UU can be explained by Theorem 5, where the upper bound in (13) is linear in α and α which, as θ → θ, are inversely proportional to θ − θ .

On the other hand, the phenomenon of CCN is caused by stronger covariate shift when θ moves closer to θ rather than the difficulty of the task.

This illustrates CCN methods do not fit our problem setting, so that we called for some new learning method (i.e., UU).

Note that there would be strong covariate shift not only by changing θ and θ but also by changing n and n .

The investigation of this issue is deferred to Appendix C.2 due to limited space.

Robustness against inaccurate training class priors Hitherto, we have assumed that the values of θ and θ are accessible, which is rarely satisfied in practice.

Fortunately, UU is a robust learning method against inaccurate training class priors.

To show this, let and be real numbers around 1, ϑ = θ and ϑ = θ be perturbed θ and θ , and we test UU on MNIST and CIFAR-10 by drawing data using θ and θ but training models using ϑ and ϑ instead.

The experimental results in TAB1 imply that UU is fairly robust to inaccurate ϑ and ϑ and can be safely applied in the wild.
...
For a fair comparison, we also implement BER by fixing π p to 0.5 in UU, so that UU and BER only differ in the performance measure.
...
The rows are arranged according to πp.

In this table, # Sub means the amount of subsampled L training data, # Train means the amount of generated U training data, and ∆θ means θ − θ .

The cell N/A (in MNIST row and pSVM column) is since pSVM is based on maximum margin clustering and is too slow on MNIST.

The task would be harder, if πp is closer to 0.5, or # Train or ∆θ is smaller.
...
Furthermore, in order to try different π p , we first subsample the original datasets to match the desired π p and then calculate the sample sizes n and n according to how many P and N data there are in the subsampled datasets, where θ and θ are set as close to 0.9 and 0.1 as possible.
...
Moreover, the closer π p is to 0.5, the better BER and BER-FC are; however, the closer π p is to 0 or 1, the worse they are, and sometimes they are much worse than pSVM.
...
We prove the theorem by contradiction, namely, for any such p(x, y) (with almost surely separable p p and p n ), for all a, b and all θ, we are able to find some g for which (6) fails.
...
Plugging them into (3) and FORMULA3 , we obtain that DISPLAYFORM0 Secondly, let g(x) = −∞ identically; this time (g(x)) = 1 and (−g(x)) = 0, and we obtain that a = π p .Thirdly, let g(x) = +∞ over p p and g(x) = −∞ over p n .
...
Finally, given any satisfying FORMULA5 , it is not difficult to verify that the three g above lead to the same contradiction with exactly the same a, b and θ by solving a bit more complicated equations.
...
For any δ > 0, let C δ = (ln 2/δ)/2, then we have with probability at least 1 − δ, DISPLAYFORM0 where the probability is over repeated sampling of data for evaluating R uu (g).Proof.

Consider the one-side uniform deviation sup g∈G R uu (g) − R(g).

Since 0 ≤ (z) ≤ C , the change of it will be no more than C α/n if some x i is replaced, or no more than C α /n if some x j is replaced.

Subsequently, McDiarmid's inequality BID26 tells us that DISPLAYFORM1 or equivalently, with probability at least 1 − δ/2, DISPLAYFORM2 By symmetrization BID54 , it is a routine work to show that DISPLAYFORM3 and according to Talagrand's contraction lemma BID44 , DISPLAYFORM4 The one-side uniform deviation sup g∈G R(g) − R uu (g) can be bounded similarly.
...
The two training distributions are created following (1) with class priors θ = 0.9 and θ = 0.4.

Subsequently, the two sets of U training data were sampled from those distributions with sample sizes n = 2000 and n = 1000.

Moreover, p p (x) and p n (x) are combined to form the test distribution p(x, y) with weights 0.3 and 0.7, so π p = 0.3.Note that p(x) changes between training and test distributions (which can be seen from FIG1 by comparing (c) and (d) in the left panel and the right panel).

This is the key difference between UU and CCN BID31 .For training, a linear (-in-input) model g(x) = ω x + b where ω ∈ R 2 and b ∈ R, and a sigmoid loss sig (z) = 1/(1 + exp(z)) were used.
...
The optimizer was Adam with the default momentum parameters (β 1 = 0.9 and β 2 = 0.999), an initial learning rate 1e-3, and a batch size 500.
...
In the experiments on the closeness of θ and θ and on the robustness against inaccurate training class priors, we sampled 40,000 training data from all the training data of MNIST in order to make it feasible to perform class-prior changes.
...
Likewise, we test UU and CCN on MNIST by fixing n to 20,000 and gradually moving n from 20,000 to 4,000, where θ is fixed to 0.4 and θ is chosen from 0.9 or 0.8.
