...
To compare RL algorithms on the basis of their performances, we focus on the comparisons of the central tendencies (µ 1 , µ 2 ): the means or the medians of the associated random variables X 1 , X 2 .

BID2 Unfortunately, we cannot know µ 1 , µ 2 exactly.

Given a sample x i of X i , we can estimate µ i by the empirical mean: DISPLAYFORM0 (resp.
...
In this example, we have µ 1 < µ 2 but x 1 > x 2 .
...
It is noted µ i while its empirical estimate is noted x i .
...
It defines two hypothesis: 1) the null hypothesis H 0 : ∆µ = µ 1 −µ 2 = 0 and 2) the alternative hypothesis H a : |∆µ| > 0.

When performing a test, one initially assumes the null hypothesis to be true.

After having observed (x 1 , x 2 ), statistical tests usually estimate the probability to observe two samples whose empirical central difference is at least as extreme as the observed one (|∆x| = |x 1 −x 2 |) under H 0 (e.g. given ∆µ = 0).
...
The significance level α (usually ≤ 0.05) draws the line between rejection and conservation of H 0 : if p-value < α, H 0 is rejected.

Further experiments demonstrate it is not always the case, which is why we prefer to note the false positive rate α * .
...
The false negative rate is noted β * .Trade-off between false positive and statistical power.

Ideally, we would like to set α = 0 to ensure the lowest possible false positive rate α * .
...
This probability of true positive 1−β * is called the statistical power of a test.
...
Generally, the sample size is chosen so as to obtain a theoretical statistical power of 1−β * = 0.8.
...
In the t-test for instance, the statistic t α is computed as t α = |∆x|/σ pool , where σ pool is the pooled standard deviation (σ pool = (σ 2 1 + σ 2 2 )/2).

Under the t-test assumptions, this statistic follows the analytic Student's distribution with density function f S (t).

The probability to obtain a difference more important than the sample difference ∆x (p-value) can be rewritten p-value = P (|t| > t α ) and can be computed as the area under f S (t) such that |t| > t α .Relative effect size.

The relative effect size is the absolute effect size |∆µ|, scaled by the pooled standard deviation σ pool , such that = |∆µ|/σ pool .
...
The distance between percentiles α×100 2 and 100(1− α 2 ) is considered to be the 100(1−α)% confidence interval around the true mean difference ∆µ. If it does not include 0, the test rejects the null hypothesis with confidence level α.
...
H 0 is rejected if the proportion of |∆x| that falls below the original difference |∆x| is higher than 1−α.
...
All these distributions are tuned so that µ = 0, σ = 1.
...
We also compare SAC and TD3 which have different standard deviations (σ T D3 = 1.15 σ SAC ).Measuring false positive rates.

To test for false positive rates α * , we simply enforce H 0 by aligning the central performances of the two distributions: µ 1 = µ 2 = 0 (the median for the MannWhitney test and the ranked t-test, the mean for others).

Given one test, two distributions and a sample size, we sample x 1 and x 2 from distributions X 1 , X 2 and compare them using the test with α = 0.05.

We repeat this procedure N r = 10 3 times and estimate α * as the proportion of H 0 rejection.

BID4 Using the spinning up implementation of OpenAI: https://github.com/openai/spinningupThe standard error of this estimate is: se(α * ) = (α * (1−α * )/N r .
...
median) is shifted by an effect size ∆µ. Given one test, two distributions (the second being shifted) and the sample size, we repeat the procedure above and obtain an estimate of the true positive rate.
...
Same distributions, equal standard deviations. , we can directly compare the mean estimates (the lines) to the significance level α = 0.05, the standard errors being smaller than the widths of these lines.

BID5 α * is very large when using bootstrap tests, unless large sample sizes are used (>40).

Using small sample sizes (<5), the permutation and the ranked t-test also show large α * .
...
Figure 4 (a) (bimodal distributions) shows that Mann-Whitney and ranked t-test (median tests) constantly overestimate α * , no matter the sample size (α * > 0.1).

For log-normal distributions on the other hand ( Figure 4(b) ), the false positive rate using these tests respects the confidence level (α * ≤ α) with sample sizes higher than N = 10.

However, other tests tend to show large α * , even for large sample sizes (α * ≈ 0.07 up to N > 50).Different distributions, equal standard deviations.
...
Welch's α * is always a bit lower than the t-test's α * .Statistical power.
...
However, the false positive rate often remains superior to the confidence level α * > α when the distributions are not normal.

When in doubt, we recommend using lower confidence levels α < 0.05 (e.g. α = 0.01) to ensure that α * < 0.05.
...
The Bonferroni correction for instance adapts the confidence level α Bonf.

= α/n c BID13 .

This ensures that the FWER stays below the initial α.
...
We want to constrain the probability FWER that our criterion is met by pure chance to a confidence level α=0.05.

This probability is: FWER = α×N c /N crit .

To make it satisfy FWER = 0.05, we need to correct α such as α corrected = α×N crit /N c (α corrected = α/2 in our case).

Table 6 : Statistical power when comparing samples from two bimodal distribution with different standard deviation.

The first is centered in 0 (µ 1 = 0, σ 1 = 1 mean or median depending on the test), the other shifted by the relative effect size (µ 2 = σ pool , σ 2 = 2).

Both have same standard deviation σ 1 = σ 2 = 1.
