...
Our forensic metric is defined as DISPLAYFORM0 where α k are parameters to be learned.

Now, with the convention that the anchor images feed through branch 0, "same" through branch 1, and "different" through branch 2, we define DISPLAYFORM1 where σ() is the sigmoid function.

Our triplet loss is then DISPLAYFORM2 where B is a batch of triplets (A, S, D), i.e., anchor, same, different.

This loss forces σ s ≥ 1 2 (thus f 01 ≤ 1), and σ d < 1 2 (thus f 12 > 1), therefore imposing a virtual threshold of 1 as criteria for similarity as measured by f ij .
...
Figure 3 contains additional analysis of the network's predictions on these sets in terms of σ (equation 2,3).Unfortunately at this time we are unable to publish real-world example images due to copyright issues.
...
The following DISPLAYFORM0 Figure 3: (a, recpectively b) 100 · σ (equation 2,3) for one run of tests on the real-world (resp.
...
below) the horizontal line of likelihood equal to 50 (those that are, are colored green, those that are not, are colored red). (c, respectively d) similarity between classes of duplicates, as measured (via σ) for a pair of images taken one from each different class, for the real-world (resp.
