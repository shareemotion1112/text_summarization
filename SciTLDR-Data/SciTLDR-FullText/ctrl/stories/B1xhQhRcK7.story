...
To design a more efficient adversary, the experimenter is allowed to collect historical data of the form (X 1 , θ 1 , C 1 ), . . .

, (X n , θ n , C n ) while they are training their agent.

Here, the initial condition X t and θ t ∈ Θ are chosen by the training procedure, where θ t encodes information about the agent used in round t to generate the observation C t = c(X t , Z t ).
...
Given some ρ > 1, δ ∈ (0, 1), an algorithm is said to be (ρ, δ) correct if the estimateP produced belongs to the [p/ρ, pρ] interval with probability at least 1 − δ.

WhenP belongs to the said interval, we say that it is a ρ-approximation of p.
...
Rejection sampling as described produces a sample from Q f .To increase the robustness of the sampling procedure against errors introduced by f = f * , we introduce a "hyperparameter" α > 0 so that q f is redefined to be proportional to f α .

Note that α = 1/2 is what our previous result suggests.

However, if f is overestimated, a larger value of α may work better, while if f is underestimated, a smaller value of α may work better.
...
Input: AVF f , budget T ∈ N and tuning parameter α > 0 Returns: Failure probability estimateP Initialize S ← 0 for t = 1 to T do repeat Sample proposal instance X t ∼ P X Accept X t with probability f α (X t ) until accepting initial condition X t Evaluate the agent on X t and observe DISPLAYFORM0 The particular problem we consider, agent evaluation, has additional structure that we can leverage.
...
Concretely, we collect data during training of the form (X 1 , θ 1 , C 1 ), . . .

, (X n , θ n , C n ) where X t is the initial condition in iteration t of training, and θ t characterizes essential features of the agent's policy used to collect the failure indicator C t .

We then train a network to predict P(C t = 1|X t , θ t ) at input (X t , θ t ).

In the simplest implementation, θ t merely encodes the training iteration t. In many common off-policy RL methods, additional stochasticity is used in the policy at training time to encourage exploration.

In these cases, we also use θ t to encode the degree of stochasticity in the policy.
...
Suppose the true AVF f * factorizes as f * (x, θ) = g * (x)h * (θ).

Suppose we have two agents described by θ 1 , θ 2 , such that the latter agent is significantly more robust, i.e., h * (θ 1 )/h * (θ 2 ) = r 1.
...
On the other hand, accurately learning h(θ 2 ) is difficult, since positive examples on θ 2 are rare.

However, even when h is learned inaccurately, this AVF is still useful for both failure search and risk estimation.

Concretely, if two AVFs DISPLAYFORM1 h 2 (θ) differ only in h, then both the adversaries and estimators induced by f 1 and f 2 are equivalent.
...
First, for both failure search and risk estimation, it is the shape of the learned AVF f (θ, s) which matters more than its magnitude.
...
Further, using flexible parameterizations for f , such as neural networks, may even allow f to represent interactions between θ and s, provided there is sufficient data to learn these regularities.
...
This provides a fairly accurate estimate of the probability ofP failing to be a ρ = 3-approximation of p.

Results are summarized in Fig. 1 .

In Appendix E.3 we include plots for other choices of ρ, to show that the results are not very sensitive to ρ.

Discussion.

At the confidence level 1 − δ = 0.95, on the Driving domain, the AVF estimator needs only 750 experiments to achieve a 3-approximation, while the VMC estimator needs 11, 000 experiments for the same.

This means that here the AVF estimator requires 14 times less environment interaction.

Similarly, on the Humanoid domain, at the confidence level 1 − δ = 0.95, the AVF estimator requires 15, 000 experiments to achieve a 3-approximation, while the VMC estimator requires 5.1e5 experiments, a 34-fold improvement.
...
While in this work we primarily focus on agent evaluation, rather than training, we note recent work on safe reinforcement learning (see García & Fernández, 2015 , for a review).
...
In statistical learning theory, the objective is to find a model θ which minimizes the expected risk, DISPLAYFORM0 , where θ (x) denotes the loss of the model θ on a data point x. For example, in classification, is typically the 0 − 1 loss, indicating whether θ makes the correct prediction on x. In reinforcement learning, returns the negative discounted return on a rollout from state x.

To assess a model θ, since we typically can only sample from D, the expected risk can be estimated on a test set of n i.i.d.

points x i ∼ D using the empirical risk :Ĵ = 1 n n−1 i=0 θ (x i ).
...
If 0 ≤ (·) ≤ a, then Hoeffding's inequality yields DISPLAYFORM1 Equivalently, the empirical test risk is within of the expected risk with probability 1 − δ, provided the number of data points in the test set is at least DISPLAYFORM2 In many commonly studied settings, such as classification with the 0 − 1 loss, or reinforcement learning with [0, 1]-bounded rewards and fixed episode lengths BID29 Beattie et al., 2016) , these bounds guarantee fairly good efficiency.
...
We use different exploration rates on actors, as in BID1 , with noise drawn from a normal distribution with standard deviation σ evenly spaced from 0.0 to 0.4.
...
To compute weights w i , each point is embedded by a 1-layer MLP f into 16 dimensions, and the weight of each neighbor is computed w i = κ(f (x), f (x i )), where κ is a Gaussian kernel, κ(x, y) = exp( x − y 2 2 /2).
...
In general, we may be interested in a ρ-approximation, that is we might want the estimates to fall in the range (p/ρ, pρ) where p is the probability of failure that we wish to measure.

One might ask whether our results are sensitive to the choice of ρ.

In other words, did we select ρ = 3 so that our results look good?

To address this concern, we include results for lower and higher choices of ρ.

We see that the AVF estimator performs significantly better than the VMC estimator across choices of ρ.
...
Thus, when we report robustness in FIG1 for averages over multiple agents, we first average the failure probabilities of these agents, before applying the recip- Figure 3: This figure shows the reliability of the AVF and VMC estimators for various choices of r = ρ.
...
This is not because of the uncertainty in the plots but is a property of ρ-estimators when the sample size is small, that is on the order of 1/p.
...
Related work in safe reinforcement learning (see García & Fernández, 2015 , for a review) tries to avoid catastrophic failures either during learning or deployment.
