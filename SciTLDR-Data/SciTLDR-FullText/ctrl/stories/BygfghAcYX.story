...
Following Bartlett et al. (2017) , we define the margin operator µ : R c × [c] → R as a function that given the scores f (x) ∈ R c for each label and the correct label y ∈ [c], it returns the difference between the score of the correct label and the maximum score among other labels, i.e. DISPLAYFORM1 .

We now define the ramp loss as follows: DISPLAYFORM2 For any distribution D and margin γ > 0, we define the expected margin loss of a predictor f (.) as DISPLAYFORM3 The loss L γ (.) defined this way is bounded between 0 and 1.

We useL γ (f ) to denote the empirical estimate of the above expected margin loss.

As setting γ = 0 reduces the above to classification loss, we will use L 0 (f ) andL 0 (f ) to refer to the expected risk and the training error respectively.
...
We define β i = u i − u 0 i 2 as the unit capacity of the hidden unit i.
...
We define α i = v i 2 as the unit impact, which is the magnitude of the outgoing weights from the unit i.
...
Given a training set S = {x i } m i=1 and γ > 0, Rademacher complexity of the composition of loss function γ over the class F W defined in equations FORMULA6 and (5) is bounded as follows: DISPLAYFORM0 The proof is given in the supplementary Section C. The main idea behind the proof is a new technique to decompose the complexity of the network into complexity of the hidden units.
...
The generalization bound in Theorem 1 is for any function in the function class defined by a specific choice of α and β fixed before the training procedure.

To get a generalization bound that holds for all networks, it suffices to cover the space of possible values for α and β and take a union bound over it.

The following theorem states the generalization bound for any two layer ReLU network 2 .Theorem 2.

For any h ≥ 2, γ > 0, δ ∈ (0, 1) and U 0 ∈ R h×d , with probability 1 − δ over the choice of the training set DISPLAYFORM1 and U ∈ R h×d , the generalization error is bounded as follows: DISPLAYFORM2 The above generalization bound empirically improves over the existing bounds, and decreases with increasing width for networks learned in practice (see Section 2.3).
...
The additive factorÕ( h/m) in the above bound is the result of taking the union bound over the cover of α and β.
...
The complete proof is given in the supplementary Section C.3.Published as a conference paper at ICLR 2019The above complexity lower bound matches the first term, DISPLAYFORM2 , in the upper bound of Theorem 1, upto 1 γ , which comes from the 1 γ -Lipschitz constant of the ramp loss l γ .

To match the second term in the upper bound for Theorem 1, consider the setting with c = 1 and β = 0, resulting in, DISPLAYFORM3 where DISPLAYFORM4 In other words, when β = 0, the function class DISPLAYFORM5 , and therefore we have the above lower bound, showing that the upper bound provided in Theorem 1 is tight.
...
This lower bound is non-trivial, in the sense that the smaller function class excludes the neural networks with all rank-1 matrices as weights, and thus shows a Θ( √ h)-capacity gap between the neural networks with ReLU activations and linear networks.
...
Given W spec with bounds s 1 and s 2 , choosing α and β such that α 2 = s 1 and max i∈[h]

β i = s 2 results in W ⊂ W spec .
...
For any h, p ≥ 2, γ > 0, δ ∈ (0, 1) and U 0 ∈ R h×d , with probability 1 − δ over the choice of the training set DISPLAYFORM0 and U ∈ R h×d , the generalization error is bounded as follows: DISPLAYFORM1 where .
...
Under the settings of Theorem 5, with probability 1 − δ over the choice of the training set S = {x i } m i=1 , for any function f (x) = V[Ux] + , the generalization error is bounded as follows: DISPLAYFORM3 We start by stating a simple lemma which is a vector-contraction inequality for Rademacher complexities and relates the norm of a vector to the expected magnitude of its inner product with a vector of Rademacher random variables.

We use the following technical result from Maurer (2016) in our proof.

Lemma 7 (Propostion 6 of Maurer FORMULA2 ).

Let ξ i be the Rademacher random variables.
...
Given a training set S = {x i } m i=1 and γ > 0, Rademacher complexity of the class F W defined in equations (5) and (4) is bounded as follows: DISPLAYFORM7 Proof.

We prove the inequality in the lemma statement using induction on t in the following inequality: DISPLAYFORM8 where for simplicity of the notation, we let φ DISPLAYFORM9 The above statement holds trivially for the base case of t = 1 by the definition of the Rademacher complexity (3).

We now assume that it is true for any t ≤ t and prove it is true for t = t + 1.

DISPLAYFORM10 The last inequality follows from the √ 2 γ Lipschitzness of the ramp loss.

The ramp loss is 1/γ Lipschitz with respect to each dimension but since the loss at each point only depends on score of the correct labels and the maximum score among other labels, it is √ 2 γ -Lipschitz.
...
Let φ i : R → R satisfy φ i (0) = 0 and be L-Lipschitz.

Let ξ i be independent Rademacher random variables.
...
Given any , D, β > 0, p ≥ 2, consider the set S DISPLAYFORM0 Proof.

We prove the lemma by construction.

DISPLAYFORM1 By Lemma 11, picking = ((1 + µ) 1/p − 1), we can find a set of vectors, DISPLAYFORM2 Lemma 13.

For any h, p ≥ 2, c, d, γ, µ > 0, δ ∈ (0, 1) and U 0 ∈ R h×d , with probability 1 − δ over the choice of the training set DISPLAYFORM3 c×h and U ∈ R h×d , the generalization error is bounded as follows: DISPLAYFORM4 where DISPLAYFORM5 and .
...
Finally, we have γ √ m 4 ≥ 1 otherwise the second term in Eq. 12 is larger than 1.

Therefore, DISPLAYFORM7 We next use the general results in Lemma 13 to give specific results for the case p = 2.Lemma 14.

For any h ≥ 2, c, d, γ > 0, δ ∈ (0, 1) and U 0 ∈ R h×d , with probability 1 − δ over the choice of the training set S = {x i } m i=1 ⊂ R d , for any function f (x) = V[Ux] + such that V ∈ R c×h and U ∈ R h×d , the generalization error is bounded as follows: DISPLAYFORM8 Proof.

To prove the lemma, we directly upper bound the generalization bound given in Lemma 13 for p = 2 and µ = 3 √ 2 4 − 1.

For this choice of µ and p, we have 4(µ + 1) 2/p ≤ 3 √ 2 and ln N p,h is bounded as follows:ln N p,h = ln h/µ + h − 2 h − 1 ≤ ln e h/µ + h − 2 h − 1 h−1 = (h − 1) ln e + e h/µ − 1 h − 1 ≤ (h − 1) ln e + e h/µ h − 1 ≤ h ln(e + 2e/µ) ≤ 5hProof of Theorem 2.

The proof directly follows from Lemma 14 and usingÕ notation to hide the constants and logarithmic factors.
...
For any h, p ≥ 2, c, d, γ > 0, δ ∈ (0, 1) and U 0 ∈ R h×d , with probability 1 − δ over the choice of the training set S = {x i } m i=1 ⊂ R d , for any function f (x) = V[Ux] + such that V ∈ R c×h and U ∈ R h×d , the generalization error is bounded as follows: DISPLAYFORM9 .

p,2 is the p norm of the column 2 norms.

Proof.

To prove the lemma, we directly upper bound the generalization bound given in Lemma 13 for µ = e p − 1.

For this choice of µ and p, we have (µ + 1) 2/p = e 2 .

Furthermore, if µ ≥ h, N p,h = 0, otherwise ln N p,h is bounded as follows: DISPLAYFORM10 = ( h/(e p − 1) − 1) ln e + e h − 1 h/(e p − 1) − 1 ≤ ( e 1−p h − 1) ln (eh)Since the right hand side of the above inequality is greater than zero for µ ≥ h, it is true for every µ > 0.Proof of Theorem 5.

The proof directly follows from Lemma 15 and usingÕ notation to hide the constants and logarithmic factors.

Proof of Theorem 3.

We will start with the case h = d = 2 k , m = n2 k for some k, n ∈ N.We will pick V = α = [α 1 . . .

α 2 k ] for every ξ, and DISPLAYFORM0 , where x i := e i n .
...
We further define j (ξ) = For any ξ ∈ {−1, 1} n , let Diag(β) be the square diagonal matrix with its diagonal equal to β and F(ξ) be the following: F(ξ) := [ f 1 , f 2 , . . .

, f 2 k ] such that if i (ξ) ≥ 0, f i = f i , and if i (ξ) < 0, f i = 0, and we will choose U(ξ) as Diag(β) × F(ξ).Since F is orthonormal, by the definition of F(ξ), we have F(ξ) 2 ≤ 1 and the 2-norm of each row of F is upper bounded by 1.

Therefore, we have U(ξ) 2 ≤ Diag(β) 2 F(ξ) 2 ≤ max i β i , and
