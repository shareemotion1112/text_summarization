...
Then we have the parameter saving ratio: DISPLAYFORM0 and when the spatial filter size h = w = 3 and the number of convolutional filters desired for each layer m 3 2 , we have the parameter saving ratio τ = 10m m+9 ≈ 10.

Similarly for spatial filter size h = w = 5 and m 5 2 , the parameter saving ratio τ = 26m m+25 ≈ 26.

For spatial filter size h = w = 7 and m 7 2 , the parameter saving ratio τ = 50m m+49 ≈ 50.

If we do not include the 1 × 1 convolutions for both standard CNN and PolyCNN, and thus make m = q = p, readers can verify that the parameter saving ratio τ becomes m. Numerically, PolyCNN saves around 10×, 26×, and 50× parameters during learning for 3 × 3, 5 × 5, and 7 × 7 convolutional filters respectively.
...
At layer l, let x π ∈ R (p·h·w)×1 be a vectorized single patch from the p-channel input maps at location π, where h and w are the spatial sizes of the convolutional filter.
...
In a standard CNN, this patch x π is taken dot-product with (projected onto) the filter w, followed by the non-linear activation resulting in a single output feature value d π , at the corresponding location π on the feature map.
...
For an early fan-out PolyCNN layer, a single seed filter w S is expanded into a set of m convolutional filters W ∈ R m×p×h×w where w i = w•βi S , and the power terms β 1 , β 2 , . . .

, β m are pre-defined and are not updated during training.

The • is again the Hadamard power.

The corresponding output feature map value d early π for early fan-out PolyCNN layer is a linear combination of multiple elements from the intermediate response maps (implemented as 1 × 1 convolution).

Each slice of this response map is obtained by convolving the input map x with W , followed by a non-linear activation.

The corresponding output feature map value d early π is thus obtained by linearly combining the m response maps via 1 × 1 convolution with parameters α 1 , α 2 , . . .
...
Then, we obtain a set of m response maps by expanding the response map with Hadamard power coefficients β 1 , β 2 , . . .

, β m which are pre-defined and not updated during training, just like in the early fan-out case.

The corresponding output feature map value d late π is also a linear combination of the corresponding elements from the m response maps via 1 × 1 convolution with parameters α 1 , α 2 , . . .

, α m .

This process follows: DISPLAYFORM5 where φ βi (·) is the point-wise polynomial expansion with Hadamard power β i .

With similar reasoning and the mild assumption that c relu is not an all-zero vector, the approximation d late π≈ d π will hold.
...
Here is a description of a general convolutional layer: DISPLAYFORM0 where φ k (·) and φ l (·) are kernel functions, σ(·) is a non-linearity and α kl are linear weights.

If both φ k (·) and φ l (·) are linear functions then the expression reduces to a module consisting of convolutional layer, non-linear activation and 1 × 1 convolutions.

Under this general setting, the learnable parameters are α and w. Setting the parameters w to fixed sparse binary values would allow us to arrive a general version of local binary CNN (Juefei-Xu et al., 2017).

Now we consider some special cases.

If φ k (·) is a linear function, then the expression reduces to: DISPLAYFORM1 Similarly if φ l (·) is a linear function, then the expression reduces to: DISPLAYFORM2 At location π of the input x, the convolutions can be reduced to: DISPLAYFORM3 where K kl is a base kernel function defined between w and image patch x π at the π location.
...
The weights α kl can be learned via 1 × 1 convolutions.

In general α kl > 0 must hold for a valid overall kernel function, but perhaps this can be relaxed or imposed during the learning process.
