...
Our goal is to find the optimal parameters of a model (e.g. a neural network) φ : R p → R o , with p parameters w ∈ R p and o outputs (the notation does not show the dependency on the training data, which is subsumed in φ for compactness).
...
GD updates the parameters using the iteration w ← w − βJ(w), where β > 0 is the learning rate and J(w) ∈ R p is the gradient (or Jacobian) of the objective function f with respect to the parameters w. A useful variant is to augment GD with a momentum variable z (Polyak, 1964), which can be interpreted as a decaying average of past gradients: DISPLAYFORM1 with a momentum parameter ρ.
...
In neural networks, GD is usually replaced by its stochastic version (SGD), where at each iteration one computes the gradient not of the model f = L(φ(w)), but of the model f t = L t (φ t (w)) assessed on a small batch of samples, drawn at random from the training set.
...
To address this issue, the Hessian is usually regularized with a parameter λ > 0, obtaining what is known as the Levenberg (Moré, 1978) method: DISPLAYFORM0 where H ∈ R p×p , J ∈ R p and I ∈ R p×p is the identity matrix.
...
The Jacobian of L(φ(w)) (eq. 1) is generally computed as J = J φ J L where J φ ∈ R p×o and J L ∈ R o×1 are the Jacobians of the model and loss, respectively.
...
One of the most widely used is the Gauss-Newton approximation BID21 BID2 Wright & Nocedal, 1999, p. 254 ): DISPLAYFORM0 When H L is PSD, which is the case for all convex losses (e.g. logistic loss, L p distance), the resultinĝ H is PSD by construction.

For the method that we propose, and indeed for any method that implicitly inverts the Hessian (or its approximation), only computing Hessian-vector productsĤv is required.
...
In general, a step z is found by minimizing eq. 14, either via explicit inversionĤ −1 J BID21 BID2 or the conjugate-gradient (CG) method BID20 .
...
By contrasting it to momentum GD (eq. 2-3), we can see that it is equivalent, except for an extra curvature termĤ(w)z.

In order to establish this equivalence, we introduced a factor ρ that decays z each step (Algorithm 1, line 4), whereas a pure gradient descent step on z would not include this factor.

We can obtain it formally by simply regularizing the quadratic model in eq. 13 with the term (1 − ρ) z 2 , which is small when ρ 1 (the recommended setting for the momentum parameter BID11 ).
...
At any given step, the optimal ρ and β can be obtained by solving a 2 × 2 linear system BID21 , sec. 7): DISPLAYFORM6 Note that, in calculating the proposed update (eq. 16), the quantities ∆ z , J T φ z and J L have already been computed and can now be reused.

Together with the fact thatĤ = J φ H L J T φ , this means that the elements of the above 2 × 2 matrix can be computed with only one additional forward pass.

Automatic λ hyper-parameter rescaling.

The regularization term λI (eq. 4) can be interpreted as a trust-region (Wright & Nocedal, 1999, p. 68) .

When the second-order approximation holds well, λ can be small, corresponding to an unregularized Hessian and a large trust-region.

Conversely, a poor fit requires a correspondingly large λ.

We can measure the difference (or ratio) between the objective change predicted by the quadratic fit (f ) and the real objective change (f ), by computing γ = (f (w + z) − f (w)) /f (z).
...
This makes it a very attractive estimate of the trust region, with γ = 1 corresponding to a perfect approximation.

Following (Wright & Nocedal, 1999, p. 69) , we evaluate γ every 5 iterations, decreasing λ by a factor of 0.999 when γ > 3/2, and increasing by the inverse factor when γ < 1/2.

We noted that our algorithm is not very sensitive to the initial λ.
...
Specifically, we use the stochastic variant of this function BID39 , R : R 2 → R: DISPLAYFORM0 where at each evaluation of the function, a noise sample i is drawn from a uniform distribution U[λ 1 , λ 2 ] with λ 1 , λ 2 ∈ R (we can recover the deterministic Rosenbrock function with λ 1 = λ 2 = 1).To assess robustness to noise, we compare each optimiser on the deterministic formulation and two stochastic variants (with differing noise regimes).
...
We report the number of iterates taken to reach the solution, with a tolerance of τ = 10 −4 .
...
With some simple algebraic manipulations, we can use the Taylor recursion to write this update in a form that is similar to ours: z r+1 = z r −Ĥz r −J. This looks similar to our gradient-descent-based update with a learning rate of β = 1 (Alg.
...
Second, they invert the Hessian independently for each mini-batch, while our method aggregates the implicit Hessian across all past mini-batches (with a forgetting factor of ρ).
...
Third, while their method fixes β to 1, we found that setting it correctly can be used to attain convergence on ill-conditioned problems.
...
In future work, we intend to bring more improvements to the wall-clock time of our method by engineering the FMAD operation to the same standard as back-propagation, and study optimal trust-region strategies to obtain λ in closed-form.
...
Left-multiplying eq. 35 by the inverse, DISPLAYFORM5 This is the transition matrix R i that characterizes the iteration, and taking its power models multiple iterations in closed-form: DISPLAYFORM6 The two eigenvalues of R i are given in closed-form by: DISPLAYFORM7 The series in eq. 37 converges when |eig (R i )| < 1 simultaneously for both eigenvalues, which is equivalent to: DISPLAYFORM8 with ρ > 0 and βh i > 0.

Note that when using the Gauss-Newton approximation of the Hessian, h i > 0 and thus the last condition simplifies to β > 0.Since eq. 39 has to be satisfied for every eigenvalue, we have 3 2 βh max − 1 < ρ < 1 + βh min ,with h min and h max the smallest and largest eigenvalues of the Hessian H, respectively, proving the result.
...
Graphically, the regions of convergence for different eigenvalues will differ only by a scale factor along the βh i axis (horizontal stretching of FIG3 ).

Moreover, the largest possible range of βh i values is obtained when ρ = 1, and that range is 0 < βh i < 4 3 .

We can infer that the intersection of the regions of convergence for several eigenvalues will be maximized with ρ = 1, for any fixed β.

DISPLAYFORM9

Theorem A.2.

Let the HessianĤ t+1 be positive definite (which holds when the objective is convex or when Gauss-Newton approximation and trust region are used).

Then the update z t+1 in Algorithm 1 is a descent direction when β and ρ are chosen according to eq. 18, and z t+1 = 0.Proof.
...
Since the surrogate HessianĤ t+1 is positive definite (PD) by construction, the update z t+1 = ρz t − β∆ zt+1 satisfies z Figure 5 : Hyper-parameter evolution during training.

Average momentum ρ (left), learning rate β (middle), and trust region λ (right), for each epoch for the basic CNN on CIFAR10, with and without batch normalisation (BN).

To make their scales comparable, we plot λ divided by its initial value (which is λ 0 = 1 with batch normalisation and λ 0 = 10 without).
