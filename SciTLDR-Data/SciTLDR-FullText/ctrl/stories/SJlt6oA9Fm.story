...
In essence, we implement this function by re-indexing and blocking the input channel-wise: CD(X) i := g i · X πi with an index pointer π i ∈ {1, 2, · · · , I}, a gate variable g i ∈ {0, 1} for i = 1, 2, · · · , I. Here, we notice that CD(X) may contain a channel copied multiple times, i.e., multiple π i 's can have the same value.

Since SCU has different parameters for each channel, setting multiple π i 's has an effect of allocating more parameters to better process the channel pointed by π i .
...
To summerize, CD(X) i takes (a) the channel which π i is pointing, in the spatially shifted form with bias b i , or (b) 0 if the gate g i is closed.
...
Formally, NC is a channel-wise re-scaling layer: NC(X) := X θ, 1 where θ = (θ i ) I i=1 are parameters to be learned.

For the channel-wise sparsity, we impose sparsity-inducing regularization specifically on θ.

Although any sparsity-inducing regularization can be used for θ BID28 BID48 , in this paper we adopt the Bayesian pruning approach proposed by BID35 2 for two reasons: (a) it is easy to incorporate into training process, and (b) we found that noise incurred from Bayesian parameters helps to recover damage from channel pruning.

In general, a Bayesian scheme regards each parameter θ as a random variable with prior p(θ).

Updating the posterior p(θ|D) from data D often leads the model to have much sparsity, if p(θ) is set to induce sparsity, e.g., by 1 denotes the element-wise product.

2 For completeness, we present for the readers an overview of BID35 in Appendix B.log-uniform prior BID23 .

Meanwhile, p(θ|D) is usually approximated with a simpler model q φ (θ), where φ are parameters to be learned.
...
We follow BID35 for the design choices on q φ (θ) and p(θ), by fully-factorized log-normal DISPLAYFORM4
...
Under assuming that BN(x) ∼ N (β, γ 2 ) where γ and β are the scaling and shifting parameter in BN, respectively, it is elementary to check: DISPLAYFORM3 where φ N and Φ N denote the p.d.f.
...
Consider a CNN model p(Y|X, Θ) employing SCU, where Θ denotes the collection of model parameters.

For easier explanation, we rewrite Θ by (V, W): V consists of (π, g) in CDs, and W is the remaining ones.
...
Recall that, each noise θ from a NC is assumed to follow q φ (θ) = LogN(θ|µ, σ 2 ).

Then, θ can be "re-parametrized" with a noise ε from the standard normal distribution as follows: DISPLAYFORM1 2 ).

Stochastic variational inference BID22 ) allows a minibatch-based stochastic gradient method for θ, in such case that θ can be re-parametrized with an non-parametric noise.
...
A single re-allocation of a channel S i to a closed channel S j consists of several steps: DISPLAYFORM6 ← 0, (iv) re-initialize the shifting bias b j , and (v) set π j ← π i .
...
For NC, on the other hand, one can still use it for inference, but efficient inference can be performed by replacing each noise θ i by constant E[θ i ], following the well-known approximation used in many dropout-like techniques BID15 .
...
Consider a probabilistic model p(Y|X, θ) between two random variables X and Y, and suppose one wants to infer θ from a dataset D = {(x n , y n )} N n=1 consisting N i.i.d.

samples from the distribution of (X, Y).

In Bayesian inference, θ is regarded as a random variable, under assuming some prior knowledge in terms of a prior distribution p(θ).

The dataset D is then used to update the posterior belief on θ, namely p(θ|D) = p(D|θ)p(θ)/p(D) from the Bayes rule.

In many cases, however, computing p(θ|D) through Bayes rule is intractable since it requires to compute intractable integrals.

To address the issue, variational inference approximates p(θ|D) by another parametric distribution q φ (θ), and tries to minimize the KL-divergence D KL (q φ (θ) p(θ|D)) between q φ (θ) and p(θ|D).

Instead of directly minimizing it, one typically maximizes the variational lower bound L(φ), due to the following: DISPLAYFORM0 where DISPLAYFORM1 In case of complex models, however, expectations in (9) are still intractable.

BID22 proposed an unbiased minibatch-based Monte Carlo estimator for them, which can be used when q φ (θ) is representable by θ = f (φ, ε) with a non-parametric noise ε ∼ p(ε).

For a minibatch DISPLAYFORM2 Now we can solve optimize L(φ) by stochastic gradient ascent methods, if f is differentiable.

For a model having non-Bayesian parameters, say W, we can still apply the above approach by maximizing DISPLAYFORM3 where φ and W can be jointly optimized under DISPLAYFORM4
...
Meanwhile, the prior p(θ) is often chosen by a fully-factorized log-uniform distribution, e.g., Sparse Variational Dropout , and SBP use the truncated version: DISPLAYFORM4 The reason why they use truncations for q φ (θ) and p(θ) is to prevent D KL (q φ (θ) p(θ)) to be improper.
...
Note that, each θ i ∼ q φ (θ i ) = LogN(θ i |µ i , σ 2 i ) in the noise vector θ can be re-parametrized with a non-parametric uniform noise ε i ∼ U(ε|0, 1) by: DISPLAYFORM5 where DISPLAYFORM6 σi , and Φ denotes the cumulative distribution function of the standard normal distribution.

Now one can optimize φ = (µ, σ) jointly with the weights W of a given neural network via stochastic variational inference described in Section A. Unlike , SBP regards W as a non-Bayesian parameter, and the final loss L SBP to optimize becomes DISPLAYFORM7 Here, the KL-divergence term is scaled by α to compensate the trade-off between sparsity and accuracy.

In practice, SBP starts from a pre-trained model, and re-trains it using the above loss.

Due to the sparsity-inducing behavior of log-uniform prior, θ is forced to become more noisy troughout the re-training.

Neurons with θ of signal-to-noise ratio (SNR) below 1 are selected, and removed after the re-training: DISPLAYFORM8 C BAYESIAN PRUNING AND IDENTITY CONNECTIONS SCU requires "training-time removal" of input channels for the channel de-allocation and reallocation to work.
...
Recall the variational lower bound objective in (12), for Bayesian parameters φ and non-Bayesian W. If the gradient of the first term DISPLAYFORM9 , that is, to follow the prior p(θ).
...
In that case then, q φ (θ) will become p(θ) too fast because of the "uncertain" W, thereby many channels will be pruned forever, in SBP for example.

This problem is usually dealt with in one of two ways: (a) using a pre-trained network as a starting point of W BID35 , and (b) a "warm-up" strategy, where the KL-divergence term is rescaled by β that increases linearly from 0 to 1 during training BID44 BID30 .
...
Then, we have: DISPLAYFORM10 Now, check that ECDS(S i ) becomes: DISPLAYFORM11 By the assumption that DISPLAYFORM12 ) for all h, w, we get: DISPLAYFORM13 DISPLAYFORM14 where φ N and Φ N denote the probability distribution function and the cumulative distribution function of the standard normal distribution.

Therefore, the desired formula for ECDS(S i ) can be obtained by using the linearity of expectation: DISPLAYFORM15

To validate whether the assumption BN(CD(X; W CD ); W BN ) i,h,w ∼ N (β i , γ 2 i ) holds in modern CNNs, we first observe that, once we ignore the effects from spatial shifting, 5 a necessary condition of the assumption is that (X i,:,: ) are identically distributed normal for a given channel X i .
...
From Θ = (V, W): V consists of (π, g) in CDs, we further rewrite W by (W NC , W C ): W NC the parameters in NCs, and W C is the remaining ones.

One can safely ignore the effect of V during training of (W NC , W C ), since they remain fixed.

Recall that each noise θ from a NC is assumed to follow LogN(θ|µ, σ 2 ).

They can be re-written with a noise ε from the standard normal distribution, i.e., θ = f ((µ, σ), ε) = exp (µ + σ · ε), where ε ∼ N (0, 1 2 ).

In such case that each noise θ from NC can be "re-parametrized" with an non-parametric noise and the corresponding parameters φ = (µ, σ), we can then use stochastic variational inference BID22 for the optimization of (W NC , W C ) with a minibatch-based stochastic gradient method (see Appendix A for more details).
...
In fact, in our case, i.e. q φ (θ) = LogN(θ|µ, σ 2 ) and DISPLAYFORM2 As we explain in Appendix B, SBP bypasses this issue by using truncated distributions on a compact interval [a, b] for q φ (θ) and p(θ).

We found that, however, this treatment also imposes extra computational overheads on several parts of training process, such as on sampling noises and computing D KL (q φ (θ) p(θ)).

These overheads are non-negligible on large models like ResNet or DenseNet, which we are mainly focusing on.

Therefore, unlike SBP, here we do not take truncations on q φ (θ) and p(θ) due to practical consideration, assuming an approximated form between the truncated distributions of q φ (θ) and p(θ) on a large interval.

Then we can replace each D KL (q φ (θ) p(θ)) in (24) by − log σ for optimization.

In other words, each noise θ in NC is regularized to a larger variance, i.e., the more "noisy".

We observed that this approximation does not harm much on the performance of SCU.

Nevertheless, one should be careful that q φ (θ) and p(θ) should not be assumed as the un-truncated forms itself, but instead as approximated forms of truncated distributions on a large interval, not to make the problem ill-posed.
...
is employed in a model, we initialize W NC = (µ, σ) by (0, e −3 ), and DISPLAYFORM0 .
