...
Consider the standard infinite-horizon reinforcement learning formulation which can be defined by tuple (S, A, π, ρ 0 , r, γ).

S represents the set of states and A denotes the set of actions.

π : S → P(A) is a policy that represents an agent's behavior by mapping states to a probability distribution over actions.

ρ 0 denotes the distribution of the initial state s 0 .

Reward function r : S → R defines the reward obtained from the environment and γ ∈ (0, 1) is a discount factor.

In this paper, the policy is a differentiable function regarding parameter θ.
...
We also adopt the definition of γ-discounted state visitation distribution as ρ θ (s) = (1 − γ) et al., 2016) , in which the coefficient 1 − γ is added to keep the integration of ρ θ (s) as 1.

Correspondingly, γ-discounted state-action visitation distribution (Ho et al., 2016) , also known as occupancy measure (Ho & Ermon, 2016) , is defined as ρ θ (s, a) = ρ θ (s) × π θ (a|s), in which π θ (a|s) stands for the policy under parameter θ.
...
in which ρθ(s) = ∞ t=0 γ t P (s t = s).

θ denotes the parameter of the new policy whileθ is that of the old one.

Trajectory is represented by τ = s 1 , a 1 , s 2 , a 2 , ....

The objective function L T RP O (θ) can be given out in the form of expeted return:
...
Similar to equation 4, Lθ(θ) can be correspondingly given out in the following form:

For the record, though it seems that equation 6 makes it possible for off-policy learning, it can be used as the objective only when policy π θ is close to the old policy πθ, i.e. within the trust region.
...
For the original goal g and an alternative goal g , the object function of HTRPO Lθ(θ) is given by:
...
For the original goal g and an alternative goal g , the gradient ∇ θ Lθ(θ) of HTRPO object function with respect to θ is given by the following expression:
...
In TRPO, the KL divergence expectation under ρθ(s) is estimated by averaging all the values of KL divergence.
...
To solve this problem, we firstly transform the KL divergence to an expectation under occupancy measure ρθ(s, a) = ρθ(s) × πθ(a|s).
...
Given two policies πθ(a|s) and π θ (a|s), the expectation of their KL-divergence over states s ∼ ρθ(s) is written as:
...
For policy πθ(a|s) and π θ (a|s), and for η = π θ (a|s) − πθ(a|s),

Theorem 4.2 demonstrates that when θ andθ is of limited difference, the expectation of log πθ(a|s)− log π θ (a|s) can be sufficiently estimated by the expectation of its square.

The proof is provided in Appendix B.2.

In fact, Es,a∼ρθ(s,a) 1 2 (log πθ(a|s) − log π θ (a|s)) 2 is the expectation of an fdivergence, where f (x) = 1 2 x(log x) 2 .
...
Theorem 4.3 illustrates that there is a decrease from the variance of log πθ(a|s) − log π θ (a|s) to the variance of its square, and furthermore indicates that the variance is effectively reduced.

The proof is given in detail in Appendix B.3.

In fact, the closer it is betweenθ and θ, the more the variance decreases.
...
For the original goal g and an alternative goal g , the constraint between policy πθ(a|s) and policy π θ (a|s) is given by:
...
For that γ t π θ (at|st,g) πθ(at|st,g) Aθ(s t , a t , g) is independent from τ 2 conditioned on τ 1 ,

t,a1:t∼pθ(s1:t,a1:t|g)
...
Starting from equation 24, since that π θ (a t |s t , g ) is the only term relavant to θ, the corresponding gradient of the objective function is computed by: log πθ(a|s) − log π θ (a|s)
...
Lemma B.1 Given two distibutions p(x) and q(x), q(x) = p(x) + η(x), in which η(x) is the variation of q(x) at p(x).
...
in which µ 1 is a constant.

Then the equation can be converted by the following steps:

Thus, when Y = Y 0 , the two factors in equation 34, (X 1 − µ 1 ) and (X 2 − E(X2)) equal to 0 simultaneously.

Also, it is easy to notice that when Y ∈ [0, 0.5], X 1 and X 2 are strictly increasing with the increase of Y .

Thus, (X 1 − µ 1 ) and (X 2 − E(X2)) are either both positive or both negative, if not zero.
...
we have log πθ(a|s) − log π θ (a|s) .

With the transitivity of inequality, combining equation 40 and equation 41, we know that log πθ(a|s) − log π θ (a|s) .
...
Denote the constraint function as fθ(θ),

To write the constraint function in goal-conditioned form,

In a similar way with the proof for Theorem 3.1, denote every time step of fθ(θ) as fθ(θ, t), in other words,

for trajectory τ 1 = s 1 , a 1 , s 2 , a 2 , ..., s t , a t and τ 2 = s t+1 , a t+1 , ...,

. (49) For that 1 2 (log πθ(a t |s t , g) − log π θ (a t |s t , g)) 2 is independent from τ 2 conditioned on τ 1 ,

Accordingly,

Furthermore, by importance sampling, for a new goal g , the constraint can be converted to the following form

. (52) in which τ = s 1 , a 1 , s 2 , a 2 , ..., s t , a t .
...
in which λ = N ·N g and g is supposed to follow uniform distribution.
...
For any policy parameter θ in the neighborhood of the parameterθ, approximate the optimization problem with linear objective function and quadratic constraint:

Noticeably, g(θ) = 0 and ∇ θ g(θ) = 0, which further simplifies the optimization problem to the following form: max
...
Thus, we compute [∇ 2 θ g(θ)] −1 ∇ θ f (θ) with conjugate gradient algorithm by solving the following equation:

in which [∇ 2 θ g(θ)]x can be practically calculated through the following expansion: Sample a trajectory τ = {(s t , a t , r t , s t+1 , g, π θ (a t |s t , g))} T t=1 using current policy θ;
...
for τ in B origin do 10:

for t = 0 to T do

Compute π θ (a t |s t , g (i) );
