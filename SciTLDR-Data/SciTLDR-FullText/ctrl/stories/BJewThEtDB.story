...
where σ(·) is an activation function and z t , h t , b ∈ R L .
...
is the probability density function representation of MDRA parametrized by tensor W, p(α) and p(β) are its prior distributions for a probability vector α = (α 1 , · · · , α K ) and a precision parameter β > 0.

We applied the Gaussian mixture model as our probabilistic model.

Hence p(α) and p(β) were given by Dirichlet and gamma distributions respectively which are the conjugate prior distributions of multinomial and Gaussian distributions.

These specific distributions are given as follows:

Here, θ 0 > 0, ν 0 > 0 and λ 0 > 0 are hyperparameters and g(h n |W k dec ) = X n dec,k denotes decoder mapping of RNN from the encoded n-th data h n , H = (h 1 , · · · , h N ) and T n D is the total signal dimension of input signal X n including dimension of input data.
...
Above equation can be divided into the two terms including α and β respectively, E q(yn) [y nk ] = 1 · q(y nk = 1) + 0 · q(y nk = 0) = q(y nk = 1) = r nk to the above equation, we obtain

On the other hand,

Similarly, by applying E q(yn) [y nk ] = r nk , we obtain log q(β)

We finally calculate log ρ nk in Eq. (3).We first calculate log q(α) and log q(β), Gamma(β|ν,λ) .

By using the expectations of β and log β by gamma distribution

Similarly, q(α) turns out to be the Dirichlet distribution with parameters (θ 1 , · · · ,θ K ), and
...
By putting β = e x , we obtain x = log β, dβ = e x dx,

In this section, we show the parameter setting of the experiments in Section 5.

Here L is the dimension of hidden variable h, capacity, fft and cpx are parameters of EUNN (Jing et al., 2017) , K is the number of the decoders, θ 0 , ν 0 , λ 0 are hyperparameters of prior distributions.
