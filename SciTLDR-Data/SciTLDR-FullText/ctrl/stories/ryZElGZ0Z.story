...
The agent does not see the underlying states, but rather only observes observation vector o t ∈ O ⊂ R m for corresponding state s t ∈ S. The agent follows a fixed behaviour policy, µ : S × A → [0, ∞), taking actions according to µ(s t , ·).
...
Additionally, there has been quite a bit of work in off-policy learning, with gradient temporal difference (GTD) algorithms BID22 BID13 , which can be used to estimate the value functions for a GVF policy π, from experience generated by a different behaviour policy µ. We leverage these works, to develop a new updating approach for GVF networks in the next section.
...
Consider a generic update DISPLAYFORM0 where ∆ t ∈ R d is the update for this step, for weights w t ∈ R d and constant stepsize α.
...
The weights w t evolve as a function of the previous weights, with stepsize α acting as a control input for how this system evolves.

A criteria for α to maintain stability in the system is to keep the norm of the update small DISPLAYFORM1 for a small > 0 that provides a minimum stepsize.

The update ∆ t (α) on this time step is dependent on the stepsize α, because that stepsize influences w t and past updates.

The expected value is over all possible update vectors ∆ t (α), for the given stepsize and assuming the system started in some w 0 .

If α is small enough to ensure updates are bounded, and policy π and MDP satisfy the standard requirements for ergodicity, a stationary distribution exists, with ∆ t (α) not dependent on the initial w 0 and instead only driven by the underlying state dynamics and target for the weights.

In the next sections, we derive an algorithm to estimate α for this dynamical system, first for a general off-policy learning update and then when adding regularization.
...
For TD(0), the update is DISPLAYFORM0 where α 0 = 1.0, ψ 1 = 0,ᾱ is a meta-stepsize and β is a forgetting parameter to forget old gradient information (e.g., β = 0.01).
...
For GTD(λ), u t = −γ t+1 (1 − λ t+1 )x t+1 .
...
Though the above updates are exact, when implementing such a recursive form in practice, we can only obtain an estimate of ψ t , if we want to avoid storing all past data.

In particular, when using ψ t−1 computed on the last time step t − 1, this gradient estimate is in fact w.r.t.

to the previous stepsize α t−2 , rather than α t−1 .

Because these stepsizes are slowly changing, this gradient still provides a reasonable estimate of the actual ψ t−1 for the current stepsize.

However, for many steps into the past, these accumulates gradients in ψ t andψ t are inaccurate.

For example, even if the stepsize is nearing the optimal value, ψ t will include larger gradients from the first step when the stepsizes where inaccurate.

To forget these outdated gradients, we maintain an exponential moving average, which focuses the accumulation of gradients in ψ t to a more recent window.

The adjusted update with forgetting parameter 0 < β < 1 gives the recursive form for ψ t+1 andψ t+1 in (3).
...
The GVF is learned off-policy, with GTD(λ) and AdaGain-R with the GTD(λ) updates.

Both GTD(λ) and AdaGain-R receive the same experience from the behaviour policy.
...
For discounts, we consider myopic discounts (γ = 0), horizon discounts (γ ∈ (0, 1)) and termination discounts (the discount is set to 1 everywhere, except for at an event, which consists of a transition (o, a, o )).
...
A PSR can be represented as a GVF network by using myopic γ = 0 and compositional predictions.
...
The only difference is in the gradient of the weights, ψ t = ∂ ∂α w t , with no change in the gradients of δ t and of h t (i.e.,ψ t ).
...
To derive the update, letw = w t + α∆ t be the weights before applying the proximal operator.

The subderivative of the proximal operator w.r.t.

α, which we call dprox αηR , is DISPLAYFORM2 The proximal operator usesw t,i andψ t,i for the element-wise update.

The resulting updates, including the exponential average with forgetting parameter β, is DISPLAYFORM3 : otherwise with ψ 1 =ψ 1 =ψ 1 = 0.

The initial step size for GTD(λ) and AdaGain-R was chosen to be 0.1, with the step size of GTD(λ) normalized over the number of active features (same as AdaGain-R's normalization factor).

AdaGain-R has a regularization parameters τ = 0.001, η = 1 normalized over the number of active features, and a meta-stepsizeᾱ = 1.0.
...
For both (b) and (c), the meta-parameters are η = 0.1, = 0.01 normalized with the number of features.to predict the observation bit in three time steps.
