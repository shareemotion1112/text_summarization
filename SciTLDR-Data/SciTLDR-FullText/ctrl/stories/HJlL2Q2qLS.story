...
Now consider a measurementŷ for which we would like to infer the posterior distribution of x. For this we use (1) and set the prior distribution to be equal to the true distribution, that is p
...
As mentioned in section 1, we wish to infer and characterize the uncertainty in the vector of parameters x from a noisy measurementŷ , where f is a known map that connects x and y. We also have several prior measurements of x, contained in the set S. To solve this problem we train a GAN with a generator g(z) on S, and then sample x from p post X (x|y) given in (6).
