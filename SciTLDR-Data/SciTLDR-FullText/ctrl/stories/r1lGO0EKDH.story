...
However, such a naïve approach has a worst-case time complexity of O(N 2 ), which certainly does not scale to large graphs.
...
For each edge in the attribute graph, we assign its weight w i,j according to the cosine similarity of two nodes' attribute vectors: w i,j = (X i,: · X j,: )/( X i,: X j,: ), where X i,: and X j,: are the attribute vectors of node i and j. Finally, we can construct the fused graph by combining the topological graph and the attribute graph: A f usion = A topo + βA f eat , where β allows us to balance the graph topological and node attribute information in the fusion process.
...
The refined embedding matrixẼ i is obtained by solving Eq. (5), whose first term enforces the refined embeddings to agree with mapped embeddings while the second term employs Laplacian smoothing to smoothẼ i over the graph.
...
Let the term (I + L) −1 denoted by h(L), then its corresponding graph filter in spectral domain is h(λ) = (1 + λ) −1 .

To avoid the inversion term, we approximate h(λ) by its first-order Taylor expansion, namely,h(λ) = 1 − λ.
...
Thus, we modify the adjacency matrix asÃ = A + σI, where σ is a small value to ensure every node has its own self-loop.
...
Fortunately, it has been shown that by adding self-loops to each node in the graph as followsÃ = A+σI (shown in Figure 5b , 5c, 5d, where σ = 0.5, 1.0, 2.0), the distribution of Laplacian eigenvalues can be squeezed to the left (towards zero) (Maehara, 2019) .

By properly choosing σ such that large eigenvalues will mostly lie in the stop band (e.g., σ = 1.0, 2.0 shown in Figure 5c and 5d), the graph filter will be able to effectively filtered out high-frequency components (corresponding to high eigenvalues) while retaining low-frequency components, which is similar to a low-pass graph filter as shown in Figure 5f .

It is worth noting that if σ is too large, then most eigenvalues will be very close to zero, which makes the graph filter less effective for removing noises.

In this work, we choose σ = 2.0 for all our experiments.
