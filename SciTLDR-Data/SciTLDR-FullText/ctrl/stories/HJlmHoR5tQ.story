...
We consider a Markov decision process (MDP) represented as a tuple (S, A, P, R, ρ 0 , γ) where S denotes the state-space, A denotes the action-space, P represents the transition probability distribution, i.e., P : S × A × S → [0, 1], R(s, a) corresponds to the reward function, ρ 0 is the initial state distribution ρ 0 : S → R, and γ ∈ (0, 1) is the discount factor.
...
Let π be a stochastic policy that takes a state and outputs a distribution over actions such that π : S × A → [0, 1].

Let τ and τ E denote a set of trajectories, a sequence of state-action pairs (s 0 , a 0 , · · · s T , a T ), generated by a policy π and an expert policy π E , respectively, where T denotes the terminal time.

Finally, let Φ(s) be a potential function that quantifies a utility of a given state s ∈ S, i.e., Φ : S → R. In our proposed work, we use an empowerment-based potential function Φ(·) to regularize policy update under MaxEnt-IRL framework.
...
MaxEnt-IRL BID24 ) models expert demonstrations as Boltzmann distribution using parametrized reward r ξ (τ ) as an energy function, i.e., DISPLAYFORM0 where r ξ (τ ) = T t=0 r ξ (s t , a t ) is a commutative reward over given trajectory τ , parameterized by ξ, and Z is the partition function.

In this framework, the demonstration trajectories are assumed to be sampled from an optimal policy π * , therefore, they get the highest likelihood whereas the suboptimal trajectories are less rewarding and hence, are generated with exponentially decaying probability.
...
Intuitively, the empowerment Φ(s) of a state s quantifies an extent to which an agent can influence its future.
...
The lower bound was derived (for complete derivation see Appendix A.1) by representing MI in term of the difference in conditional entropies H(·) and utilizing the non-negativity property of KL-divergence, i.e., DISPLAYFORM1 where DISPLAYFORM2 is a variational distribution with parameters φ and w θ (·) is a distribution over actions with parameters θ.

Finally, the lower bound in Eqn.

5 is maximized under the constraint H(a|s) < η (prevents divergence, see BID12 ) to compute empowerment as follow: DISPLAYFORM3 where β is η dependent temperature term.
...
6 with respect to w θ (a|s) and q φ (a|s , s).

Given a set of training trajectories τ , the maximization of Eqn.

6 w.r.t q φ (·) is shown to be a supervised maximum log-likelihood problem whereas the maximization w.r.t w θ (·) is determined through the functional derivative ∂I/∂w = 0 under the constraint a w(a|s) = 1.
...
6 showed that the empowerment Φ(s) = 1 β log Z(s) (for full derivation, see Appendix A.2).Note that w * (a|s) is implicitly unnormalized as there is no direct mechanism for sampling actions or computing Z(s).

BID12 introduced an approximation w * (a|s) ≈ log π(a|s) + Φ(s) where π(a|s) is a normalized distribution which leaves the scalar function Φ(s) to account for the normalization term log Z(s).

Finally, the parameters of policy π and scalar function Φ are optimized by minimizing the discrepancy, l I (s, a, s ), between the two approximations (log π(a|s) + Φ(s)) and β log q φ (a|s , s)) through either absolute (p = 1) or squared error (p = 2), i.e., DISPLAYFORM5 3 EMPOWERED ADVERSARIAL INVERSE REINFORCEMENT LEARNINGWe present an inverse reinforcement learning algorithm that learns a robust, transferable reward function and policy from expert demonstrations.

Our proposed method comprises (i) an inverse model q φ (a|s , s) that takes the current state s and the next state s to output a distribution over actions A that resulted in s to s transition, (ii) a reward r ξ (s, a), with parameters ξ, that is a function of both state and action, (iii) an empowerment-based potential function Φ ϕ (·) with parameters ϕ that determines the reward-shaping function F = γΦ ϕ (s ) − Φ ϕ (s) and also regularizes the policy update, and (iv) a policy model π θ (a|s) that outputs a distribution over actions given the current state s. All these models are trained simultaneously based on the objective functions described in the following sections to recover optimal policies and generalizable reward functions concurrently.3.1 INVERSE MODEL q φ (a|s, s ) OPTIMIZATION As mentioned in Section 2.3, learning the inverse model q φ (a|s, s ) is a maximum log-likelihood supervised learning problem.

Therefore, given a set of trajectories τ ∼ π, where a single trajectory is a sequence states and actions, i.e., τ i = {s 0 , a 0 , · · · , s T , a T } i , the inverse model q φ (a|s , s) is trained to minimize the mean-square error between its predicted action q(a|s , s) and the action a taken according to the generated trajectory τ , i.e., DISPLAYFORM6 Empowerment will be expressed in terms of normalization function Z(s) of optimal w * (a|s), i.e., DISPLAYFORM7 .

Therefore, the estimation of empowerment Φ ϕ (s) is approximated by minimizing the loss function l I (s, a, s ), presented in Eqn.

7, w.r.t parameters ϕ, and the inputs (s, a, s ) are sampled from the policy-generated trajectories τ .

To train the reward function, we first compute the discriminator as follow: DISPLAYFORM0 where r ξ (s, a) is the reward function to be learned with parameters ξ.

We also maintain the target ϕ and learning ϕ parameters of the empowerment-based potential function.

The target parameters ϕ are a replica of ϕ except that the target parameters ϕ are updated to learning parameters ϕ after every n training epochs.

Note that keeping a stationary target Φ ϕ stabilizes the learning as also mentioned in BID11 .

Finally, the discriminator/reward function parameters ξ are trained via binary logistic regression to discriminate between expert τ E and generated τ trajectories, i.e., DISPLAYFORM1 3.4 POLICY OPTIMIZATION POLICY π θ (a|s)We train our policy π θ (a|s) to maximize the discriminative rewardr(s, a, s ) = log(D(s, a, s ) − log(1 − D(s, a, s ))) and to minimize the loss function l I (s, a, s ) = β log q φ (a|s, s ) − (log π θ (a|s) + Φ ϕ (s)) p which accounts for empowerment regularization.

Hence, the overall policy training objective is: DISPLAYFORM2 where policy parameters θ are updated using any policy optimization method such as TRPO BID21 or an approximated step such as PPO BID22 .Algorithm 1 outlines the overall training procedure to train all function approximators simultaneously.

Note that the expert samples τ E are seen by the discriminator only, whereas all other models are trained using the policy generated samples τ .

Furthermore, the discriminating rewardr(s, a, s ) boils down to the following expression (Appendix B.1): DISPLAYFORM3 where f (s, a, s ) = r ξ (s, a) + γΦ ϕ (s ) − Φ ϕ (s).
...
The analysis of this alternative expression is given in Appendix B to highlight that our policy update rule is equivalent to MaxEnt-IRL policy objective BID4 except that it also maximizes the empowerment, i.e., DISPLAYFORM6 where, λ and γ are hyperparameters, andĤ(·) is the entropy-regularization term depending on π(·) and q(·).
...
w,q E p(s |a,s)w(a|s) [− 1 β log w(a|s) + log q(a|s , s)]As mentioned in section 2.3, given a training trajectories, the maximization of Eqn.
...
For simplicity, we consider discrete state and action spaces, and the derivation is as follow: By using the constraint a w(a|s) = 1, it can be shown that the optimal solution w * (a|s) = 1 Z(s) exp(u(s, a)), where u(s, a) = βE p(s |a,s) [log q(a|s , s)] and Z(s) = a u(s, a).
...
Let τ be a trajectory sampled from expert demonstrations D and p ξ (τ ) ∝ p(s 0 )Π T −1 t=0 p(s t+1 |s t , a t ) exp r ξ (st,at) be a distribution over τ .
...
Thus the above gradient becomes: DISPLAYFORM2 We train our importance-sampler/policy π to maximize the empowerment Φ(·) for generalization and to reduce divergence from true distribution by minimizing DISPLAYFORM3 , the matching terms of π(τ ) and p ξ (τ ) cancel out, resulting into entropy-regularized policy update.

Furthermore, as we also include the empowerment Φ(·) in the policy update to be maximized, hence the overall objective becomes: DISPLAYFORM4 Our discriminator is trained to minimize cross entropy loss as mention in Eqn.
...
The discriminative rewardr(s, a, s ) simplifies to: DISPLAYFORM0 where f (s, a, s ) = r(s, a) + γΦ(s ) − Φ(s).

The entropy-regularization is usually scaled by the hyperparameter, let say λ h ∈ R, thusr(s, a, s ) = f (s, a, s ) − λ h log π(a|s).

Hence, assuming single-sample (s, a, s ), absolute-error for l I (s, a, s ) = | log q φ (a|s, s ) − (log π(a|s) + Φ(s))|, and l i > 0, the policy is trained to maximize following: DISPLAYFORM1 = r(s, a) + γΦ(s ) − Φ(s) − λ h log π(a|s) − log q(a|s, s ) + log π(a|s) + Φ(s) = r(s, a) + γΦ(s ) − λ h log π(a|s) − log q(a|s, s ) + log π(a|s)Note that, the potential function Φ(s) cancels out and we scale the leftover terms of l I with a hyperparameter λ I .

Hence, the above equation becomes: r π (s, a, s ) = r(s, a, s ) + γΦ(s ) + (λ I − λ h ) log π(a|s) −

λ I log q(a|s, s )We combine the log terms together as: r π (s, a, s ) = r(s, a) + λ I Φ(s ) + λĤ(·)where λ is a hyperparameter, andĤ(·) is an entropy regularization term depending on q(a|s, s ) and π(a|s).
...
We use two-layer ReLU network with 32 units in each layer for the potential function h ϕ (·) and Φ ϕ (·), reward function r ξ (·), discriminators of GAIL and GAN-GCL.

Furthermore, policy π θ (·) of Figure 5 : The top and bottom rows show the path followed by a 2D point-mass agent (yellow) to reach the target (green) in training and testing environment, respectively.all presented models and the inverse model q φ (·) of EAIRL are presented by two-layer RELU network with 32 units in each layer, where the network's output parametrizes the Gaussian distribution, i.e., we assume a Gaussian policy.

For all experiments, we use the temperature term β = 1.
...
The hyperparameter λ I was set to 1.0 for reward learning and 0.001 for policy learning.

The target parameters of the empowerment-based potential function Φ ϕ (·) were updated every 5 and 2 epochs during reward and policy learning respectively.
