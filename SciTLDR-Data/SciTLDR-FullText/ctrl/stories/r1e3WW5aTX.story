...
In this paper, we model relation representation as learning a parametrised operator f (a, b; θ) such that we can accurately represent the relation between two given words a and b from their word representations a and b, without modifying the input word embeddings.
...
Our goal in this paper is to learn a parametrised two-argument function f (·, ·; θ) that can accurately represent the relation between two given words a and b using their pre-trained d-dimensional word embeddings a, b ∈ R d .

Here, θ denotes the set of parameters that governs the behaviour of f , which can be seen as a supervised operator that outputs a relation representation from two input word representations.
...
Therefore, we limit supervised relational operators such that they return vectors as given by FORMULA0 DISPLAYFORM1 , and for vector concatenation we have f (a, b; θ) = a ⊕ b (m = 2d), where ⊕ denotes concatenation of two vectors.

In unsupervised operators, θ is a constant that does not influence the output relation embedding.
...
Specifically, GloVe model was trained with window size 15, 50 iterations, weighting function parameters x max = 100, α = 0.75.
...
The optimal 2 regularisation coefficient λ was 0.001.
...
The regularised training objective L(D; θ) is given by (5).

DISPLAYFORM3 Here, δ is a margin hyperparameter set to 1 according to the best accuracy on the SAT validation dataset.

The regularisation coefficient λ is set separately for each parameter in the different supervised relation composition operators using the SAT validation dataset.
