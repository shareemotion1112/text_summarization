...
Let W be an m × n matrix and denote by σ its vector of singular values.

Then DISPLAYFORM1 where the minimum is taken over all U : m×min(m, n) and V : min(m, n)×n such that W = U V .

Furthermore, if W =Ũ ΣṼ * is a singular value decomposition of W , then equality holds in (2) for the choice U =Ũ √ Σ and V = √ ΣṼ * .The procedure to take advantage of this characterization is as follows.

First, for each large GEMM in the model, replace the m × n weight matrix W by the product W = U V where U : m × min(m, n) and V : min(m, n) × n. Second, replace the original loss function (W ) by DISPLAYFORM2 where λ is a hyperparameter controlling the strength of the approximate trace norm regularization.
...
-Replace the original loss function (W ) by DISPLAYFORM4 where λ is a hyperparameter controlling the strength of the trace norm regularization.-Train the model to convergence.
...
As described in Appendix B.2, in our factorization scheme, each Figure 1: CER dependence on λ rec and λ nonrec for trace norm regularization (left) and DISPLAYFORM0 GRU layer involves two matrix multiplications: a recurrent and a non-recurrent one.
...
For both trace norm and 2 regularization, we found it beneficial to introduce separate λ rec and λ nonrec parameters for determining the strength of regularization for the recurrent and non-recurrent weight matrices, respectively.

In addition to λ rec and λ nonrec in initial experiments, we also roughly tuned the learning rate.
...
The dependence of final CER on λ rec and λ nonrec is shown in Figure 1 .

Separate λ rec and λ nonrec values are seen to help for both trace norm and 2 regularization.

However, for trace norm regularization, it appears better to fix λ rec as a multiple of λ nonrec rather than tuning the two parameters independently.
...
Denote by σ the d-dimensional vector of singular values of W .

Then we define the nondimensional trace norm coefficient of W as follows: We show in Appendix A that ν is scale-invariant and ranges from 0 for rank 1 matrices to 1 for maximal-rank matrices with all singular values equal.

Intuitively, the smaller ν(W ), the better W can be approximated by a low rank matrix.
...
At very high regularization strengths, 2 regularization also leads to small ν values.
...
The stage 2 models were trained without regularization (i.e., λ rec = λ nonrec = 0) and with the initial learning rate set to three times the final learning rate of the stage 1 model.
...
Figure 7: Contours of ||σ|| 1 and ||σ|| 2 .

||σ|| 2 is kept constant at σ.

For this case, ||σ|| 1 can vary from σ to √ 2σ.

In this section, we describe some of the properties of the non-dimensional trace norm coefficient defined in Section 3.1.

DISPLAYFORM0 (iii) ν(W ) = 0 if and only if W has rank 1.(iv) ν(W ) = 1 if and only if W has maximal rank and all singular values are equal.

Proof.

Since we are assuming W is nonzero, at least one singular value is nonzero and hence ||σ|| 2 = 0.

Property (i) is immediate from the scaling property ||cσ|| = |c| · ||σ|| satisfied by all norms.

To establish the other properties, observe that we have DISPLAYFORM1 The first inequality holds since singular values are nonnegative, and the inequality is strict unless σ i or σ j vanishes.

The second inequality comes from an application of Jensen's inequality and is strict unless σ i = σ j .

Thus, replacing (σ i , σ j ) by (σ i + σ j , 0) preserves ||σ|| 1 while increasing ||σ|| 2 unless one of σ i or σ j is zero.

Similarly, replacing (σ i , σ j ) by ( DISPLAYFORM2 2 σ j ) preserves ||σ|| 1 while decreasing ||σ|| 2 unless σ i = σ j .

By a simple argument by contradiction, it follows that the minima occur for σ = (σ 1 , 0, . . .

, 0), in which case ν(W ) = 0 and the maxima occur for σ = (σ 1 , . . . , σ 1 ), in which case ν(W ) = 1.We can also obtain a better intuition about the minimum and maximum of ν(W ) by looking at the 2D case visualized in Figure 7 .

For a fixed ||σ|| 2 = σ, ||σ|| 1 can vary from σ to √ 2σ.

The minimum ||σ|| 1 happens when either σ 1 or σ 2 are zero.

For these values ||σ|| 2 = ||σ|| 1 and as a result ν(W ) = 0.

Similarly, the maximum ||σ|| 1 happens for σ 1 = σ 2 , resulting in ν(W ) = 1.
...
For the recurrent layers, we employ the Gated Recurrent Unit (GRU) architecture proposed in BID5 , where the hidden state h t is computed as follows: DISPLAYFORM0 where σ is the sigmoid function, z and r are update and reset gates respectively, U z , U r , U h are the three recurrent weight matrices, and W z , W r , W h are the three non-recurrent weight matrices.
