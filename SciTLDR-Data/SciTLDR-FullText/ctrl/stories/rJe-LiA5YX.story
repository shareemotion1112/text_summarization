...
In this section, we deal with the well-posed case that there exists a connected closed subset Ω ⊂ R N , where J is regular and the equation has a unique solution ξ.

Therefore, the positive matrix G = J T J induces a Riemannian metric g on Ω and (Ω, g) becomes a Riemannian manifold under some appropriate conditions.

Let us then consider time evolution of variable w on this manifold.

Our main purpose in this section is to study the characteristics of dynamical systems in which w(t) moves on geodesics between any point in Ω and ξ with respect to the metric g.

Let L be a Lagrangian given by DISPLAYFORM0 with v = dw/dt (also written asẇ).

The Euler-Lagrange equation for L is then expressed as dp dt DISPLAYFORM1 with momentum vector p = Gv.

If the boundary condition at two points in Ω, w(t 0 ) = w 0 , w(t 1 ) = w 1 , is imposed on (2), a geodesic between w 0 and w 1 is obtained as the solution.
...
Let w 0 be an arbitrary point in Ω and (w(t), p(t)) be a solution of equation FORMULA1 with the following initial condition; DISPLAYFORM2 Then w satisfies F w(t) = (1 − t)F w 0 , (4) for t ∈ [0, 1].
...
Now we take a different expression that the coefficient (1 − t) in (6) is replaced by a different monotonically decreasing function, that is, DISPLAYFORM6 where ρ denotes a monotonically decreasing smooth function from (t 0 , t 1 ) onto (0, 1).
...
Let w 0 ∈ Ω be an arbitrary point.
...
With respect to the choice of ρ, the end point t 1 can be set as ∞ under some appropriate conditions and Theorem 2.2 still holds in the sense that DISPLAYFORM9 In particular, if we set ρ(t) = e −t , then χ(t) = −1 and F can be represented as F (t) = e −t F 0 , so that the convergence rate of F is exponential.
...
If we apply the Euler method to the differential equation (9) with step size t, the corresponding iteration step can be written as DISPLAYFORM11 which recovers the Newton method with step size η i = t · χ(τ i ).

Consider smooth functions ϕ : DISPLAYFORM0 In this section, we develop a method based on EDF for a mathematical optimization problem of the form DISPLAYFORM1 In this section, the area Ω ⊂ R N is supposed to be a compact subset where there exists no stationary point except for a minima.
...
Note that the gradient can be expressed as ∇ L(w) = J T ϕ F (w) with Jacobian J ϕ = ∂ϕ/∂w.
...
However, if M = N and J ϕ is invertible, then w moves on another geodesic with respect to a different metric DISPLAYFORM8 In addition, if ρ(t) = e −t , F converges exponentially, which implies that ∇ L = J ϕ F also converges exponentially in (18) as well as in FORMULA0 .

In general cases, if a condition that DISPLAYFORM9 is satisfied, then F converges to 0.

This shows that in the neighborhood of solution ξ of equation ∇ L = 0, the momentum Gẇ sufficiently approximates Hẇ by (16).
...
So, we set DISPLAYFORM3 where λ is a continuous positive function of t. Then, we take the same approximation as FORMULA1 with A = G+λI. The damping factor λ plays several important roles in solving practical problems.
...
If we choose λ such that it approaches to 0 as rapidly as the gradient in (18), the asymptotic behavior of FORMULA1 is almost the same as that of FORMULA0 .

Particularly, in the case that χ = −1, we set λ = a J T ϕ F b with a, b > 0, so that the convergence rate of (24) stays exponential.
...
Since the orbit of FORMULA1 is affected by the gradient due to the damping factor λ, the method based on this equation could take advantage of stochastic approaches as other gradient methods do. (For implementation of the algorithm, see Appendix A.) Finally, to accelerate the EDF-based methods, it is sometimes effective to change equation FORMULA0 into a second-order differential equation, particularly in the case in which the approximation method with k is applied.

Specifically, we take the equation DISPLAYFORM4 where κ is a real-valued function of t. The convergence properties of the methods are controlled by the following differential equation; DISPLAYFORM5 There are two ways of determining κ.

The first one is to set κ = α with constant α (W1), which leads to a similar scheme to the momentum gradient decent method.

The other one is to set κ(t) = αt −1 (W2).
...
We write ϕ nn for neural networks with parameters w ∈ R N , and define ϕ by the direct sum of vectors {ϕ j } given by ϕ j (w) = ϕ nn (x j , w), that is, ϕ = ⊕ϕ j .
...
As already mentioned, in this case, F = ϕ and H L = I. In the second case, the loss is given by cross entropy with softmax function, that is, DISPLAYFORM0 where θ denotes the softmax function and θ j = θ(ϕ j ).
...
The momentum coefficient was fixed to α = 0.9 for the W1 acceleration (see 25).For each of the tasks, we ran tests on 10 different learning rates; for EDF between 0.1 and 2.0, for Momentum SGD between 0.1 and 10.0, for Adam between 0.0001 and 0.1.
...
Let us divide the dataset {x, y} = {x j , y j } n−1 j=0 into distinct subsets of size p such that DISPLAYFORM0 Then, gradients and Jacobian-vector-multiplications are calculated as DISPLAYFORM1 where ϕ (i) and F (i) are the subcomponents of ϕ and F , respectively, corresponding to the decomposition above.
