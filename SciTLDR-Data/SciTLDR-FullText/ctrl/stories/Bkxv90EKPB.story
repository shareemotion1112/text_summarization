...
With our notation, each of the network and the related dataset is called a task, which is denoted as τ .
...
Specifically, let z k denote the k-th element of a sample z to be generated;z denotes a sample from last round; Γ denotes the task-specific information.

In our case, we set Γ (z, ∇z log p(z)) with p(·) denoting the target distribution (with possible hidden parameters).

NIAF generates the sample z = (z 1 , · · · , z k , · · · ) via an autoregressive manner, as:

where { k } are noise samples; G(·, ·; φ φ φ) is an invertible function (generator) parameterized by φ φ φ and implemented as a DNN; and T is an autoregressive conditioner model to generate the parameters of the generator G at each step k, which is itself parameterized by ψ ψ ψ and implemented as a deep sigmoidal flow or a deep dense sigmoidal flow as in (Huang et al., 2018) .
...
Here and in the following, we consider the task network parameterized as a one-layer MLP for notation simplicity, although our method applies to all other network structures; and we have used NIAF({ k }, Γ) to denote the output of meta samples from the NIAF.
...
A general Bayesian sampling framework based on optimal transport Let a task-specific target distribution be p τ (z), indexed by τ .

A standard way is to adapt the samples based on a Markov chain whose stationary distribution equals p τ , e.g., via SG-MCMC.
...
Since we aim to adapt meta samples to new tasks, it is reasonable to define the adaptation via task-wise WGFs, i.e., for each task, there is a WGF with a specific functional energy and the corresponding first variation, denoted respectively as E τ and F τ δEτ δρ with the task index τ .

Here ρ denotes the underlying density of the samples.

Consequently, ρ will evolve with a variant of the PDE by replacing E with E τ in equation 8 for each task.

To solve the corresponding PDE, we prove Theorem 1 based on a discrete approximation of ρ with the evolved meta samples, which is termed optimal-transport Bayesian sampling (OT-Bayesian sampling).

Theorem 1 (Optimal-Transport Bayesian Sampling) Let ρ t at time t be approximated by parti-

.

This is a useful result to derive a learning algorithm for the meta sampler described in Section 2.4.

Energy functional design Choosing an appropriate energy function E τ is important for efficient sample adaptation.

To achieve this, the following conditions should be satisfied: i) E τ (ρ) should be convex w.r.t.

ρ; ii) The first variation F τ could be calculated conveniently.
...
Since the density ρ(z) required in evaluating r is not readily available due to its implicit distribution, we follow Chen et al. (2018b) and use the meta samples {z (i) k } at the k-th step for approximation, resulting in
...
Training in the sample-parameterization setting In this case, one only needs to optimize the conditional model T (·; ψ ψ ψ) as all parameters of other networks are directly generated.

Specifically, because the energy functionals for each task τ are designed so that the minima correspond to the target distributions, the objective thus can be defined over the whole task distribution p(τ ) as:

For notation simplicity, we will not distinguish among the adapted samples (i.e., z (i) k ) for different tasks.

Since the only parameter is ψ ψ ψ in the autoregressive conditioner model T (see Figure 2 , and note the parameters φ φ φ and W for the meta generator and task network do not need to be learned as they are the outputs of T and G, respectively), its gradient can be directly calculated using chain rule:

where "

= " follows by the results from Section 2.3 and ∇ z Training in the multiplicative-parameterization setting In this case, two sets of parameters are to be learned, ψ ψ ψ and W.

Since ψ ψ ψ and W are decoupled, one can still optimize ψ ψ ψ by adopting the same update equation as in the sample-parameterization setting.

For W, we follow Louizos & Welling (2017) and adopt variational inference.

Specifically, we first augment the task network with an auxiliary network with a conditional likelihood Ranganath et al. (2016) ; Louizos & Welling (2017) , with the inference network defined in (2), and writing the implicit distribution of z asq ψ ψ ψ (z) and the prior distribution of W as p(W), we arrive at the following ELBO: Different from Louizos & Welling (2017) , we only update (θ, φ φ φ) by optimizing the above ELBO; while leave the update of ψ ψ ψ with the gradient calculated in equation 7, which reflects gradients of samples from the NIAF.
...
New-task sample generation After training, samples for a new task can be directly generated by feeding the task information Γ and some noise to the meta sampler depicted in Figure 1 .
...
We follow Liu & Wang (2016) and apply Gaussian priors for the parameters p 0 (w|α) = N (w; 0, α −1 I) with p 0 (α) = Gamma(α, 1, 0.01).
...
In optimal transport, a density function, ρ t , evolves along time t to a target distribution optimally, i.e., along the shortest path on the space of probability measures P(Ω), with Ω being a subset of R d .

The optimality is measured in the sense that ρ t moves along the geodesic of a Riemannian manifold induced by a functional energy, E : P(Ω) → R, under the 2-Wasserstein distance metric.

Formally, the trajectory of ρ t is described by the following partial differential equation (PDE):

where

∂ zi f is the divergence operator; and F δE δρt (ρ t ) is called the first variation of E at ρ t (functional derivative on a manifold in P(Ω)).

To ensure ρ t to converge to a target distribution p such as the posterior distribution of the model parameters, one must design an appropriate E such that p = arg ρ min E(ρ).

A common choice is the popular KL-divergence, KL(ρ, p).
...
For example, recent works (Chen et al., 2018b; Liu et al., 2019b; a) consider approximating ρ t with samples and evolve the samples according to equation 8.
...
The autoregressive flow (AF) parameterizes a Gaussian conditional distribution for each z i , e.g., p(z i | z 1:i−1 ) = N (z i |µ i , exp(α i )), where µ i = g µi (z 1:i−1 ) and α i = g αi (z 1:i−1 ) are outputs of two neural networks g µi and g αi .

The sample generation process is: z i = i exp(α i ) + µ i , with µ i = g µi (z 1:i−1 ), α i = g αi (z 1:i−1 ) and i ∼ N (0, 1).
...
The neural autoregressive flow (NAF) (Huang et al., 2018) replaces the affine transformation used in the above flows with a deep neural network (DNN), i.e., t = f (z t , φ φ φ = T (z 1:t−1 )), where f is a DNN transforming a complex sample distribution, p(z), to a simple latent representation q 0 ( ).
...
Note the µ i and α i are computed differently for AF and IAF, i.e., previous variables z 1:i−1 are used for AF and previous random noise 1:i−1 are used for IAF.
...
Require: p(T ): distribution over tasks; Require: α, β step size hyperparameter; randomly initialize θ, φ φ φ, ψ ψ ψ.
...
In the f-divergence case, this corresponds tof (ρ) = p τ f ( ρ pτ ).
...
The sinewave function is defined as y(t) = sin(2πf t), with a uniform prior U ([0, 2]) and a Gaussian likelihood N (y i ; y f (t i ), 0.125).
