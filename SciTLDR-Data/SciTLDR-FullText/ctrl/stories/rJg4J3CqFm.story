...
The p-Wasserstein distance between probability distributions µ and ν over a metric space X is DISPLAYFORM0 where the infimum is taken over transport plans π that distribute the mass in µ to match that in ν, with the p-th power of the ground metric d(x 1 , x 2 ) on X giving the cost of moving a unit of mass from support point x 1 ∈ X underlying distribution µ to point x 2 ∈ X underlying ν.
...
In this case, the transport plan π matching µ and ν in Equation 1 becomes discrete as well, supported on the product of the two support sets.
...
The resulting Sinkhorn divergence solves a modified version of Equation 3: DISPLAYFORM0 where log(·) is applied elementwise and λ ≥ 0 is the regularization parameter.

For λ > 0, the optimal solution takes the form DISPLAYFORM1 , where ∆(r) and ∆(c) are diagonal matrices with diagonal elements r and c, resp.

Rather than optimizing over matrices T , one can optimize for r and c, reducing the size of the problem to M + N .

This can be solved via matrix balancing, starting from an initial matrix K := exp( −D p λ ) and alternately projecting onto the marginal constraints until convergence: DISPLAYFORM2 Here, ./ denotes elementwise division for vectors.
...
Given two metric spaces A and B, an embedding of A into B is a map φ : A → B that approximately preserves distances, in the sense that the distortion is small: DISPLAYFORM0 for some uniform constants L > 0 and C ≥ 1.

The distortion of the embedding φ is the smallest C such that Equation 6 holds.
...
While learned vector space embeddings have a long history, there is a recent trend in the representation learning community towards more complex target spaces, such as spaces of probability distributions BID54 BID4 , Euclidean norm balls BID36 BID35 , Poincaré balls BID39 , and Riemannian manifolds BID40 .
...
Our objective is to find a map φ : C → W p (X ) such that the relationship r(u, v) can be recovered from the Wasserstein distance between φ(u) and φ(v), for any u, v ∈ C. Examples include:1.

METRIC EMBEDDING: r is a distance metric, and we want W p (φ(u), φ(v)) ≈ r(u, v) for all u, v ∈ C. 2.

GRAPH EMBEDDING: C contains the vertices of a graph and r : C×C → {0, 1} is the adjacency relation; we would like the neighborhood of each φ(u) in W p to coincide with graph adjacency.
...
Given a set of training samples DISPLAYFORM0 ⊂ C × C × R, we want to learn a map φ : C → W p (X ).

We must address two issues.

First we must define the range of our map φ.
...
As in BID20 , we replace W p with the Sinkhorn divergence W λ p , which is solvable by a the fixed-point iteration in Equation 5.
...
The collection of vertices for each network serves as the input space C for our embedding, and our goal is to learn a map φ : C → W p (R k ) such that the 1-Wasserstein distance W 1 (φ(u), φ(v)) matches as closely as possible the shortest path distance between vertices u and v, for all pairs of vertices.

We learn a minimum-distortion embedding: Given a fully-observed distance metric d C : C ×C → R in the input space, we minimize the mean distortion: DISPLAYFORM0 φ is parameterized as in §3.2, directly specifying the support points of the output distribution.
...
For random networks, we use three standard generative models: Barabási-Albert BID0 , BID55 , and the stochastic block model BID23 .

Random scale-free networks are generated from the Barabási-Albert model, and possess the property that distances are on average much shorter than in a Euclidean spatial graph, scaling like the logarithm of the number of vertices.
...
For word embedding, we use a contrastive loss function BID22 φ * = arg min φ s xi,xj ∈s DISPLAYFORM0 which tries to embed words x i , x j near each other in terms of 1-Wasserstein distance (here W λ 1 ) if they co-occur in the context; otherwise, it prefers moving them at least distance m away from one another.
...
We choose a vocabulary of 8000 words and a context window size of l = 2 (i.e., 2 words on each side), λ = 0.05, number of epochs of 3, negative sampling rate of 1 per positive and Adam BID26 for optimization.
...
Since X embeds within W(X ) using δ-functions, this might be viewed as a general "lifting" procedure increasing the capacity of a representation.
