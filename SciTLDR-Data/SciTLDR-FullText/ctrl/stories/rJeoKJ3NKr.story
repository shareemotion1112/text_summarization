...
The starting point is an unnormalized PGM parameterized by θ.

Its probability density can be expressed as p(x; θ) = p(v, h; θ) ∝ exp(φ(v, h; θ)) , where v are the visible variables available in our data and h are the hidden variables.
...
The query-trained neural network (QT-NN) follows from specifying a graphical model φ(v, h; θ), a temperature T and a number of inference timesteps N over which to run parallel BP.
...
We minimize this loss wrt θ, T via stochastic gradient descent, sampling from the training data and some query distribution.
...
For a given set of graphical model parameters θ and temperature T we can write a feedforward function that approximately resolves arbitrary inference queries by unrolling the parallel BP equations for N iterations.
...
The portion of the parameters θ relevant to the factor between variables i and j is represented by θ ij = θ ji , and the portion that only affects variable i is contained in θ i .

Observe that all layers share the same parameters.

The functions f θ ij (·) are directly derived from φ(x; θ) using the BP equations, and therefore inherit its parameters.
...
• The output of the network isv andĥ, the inferred probability of 1 for both the visible and hidden units.

The outputĥ is inferred but actually not used during training.
