...
More formally, the NAT task is to learn an encoder function f θ : X → Z from input space to a latent representation space.
...
In some cases, the optimally assigned target y opt is not the nearest target to a latent representation z. In this case, we remove the assigned target and reassign a copy of the nearest target for that z with some probability α as in Algorithm 1 (see also FIG1 ).
...
Input : A batch of latent representations z, the optimal NAT assignment y opt , and a probability of copying α Output: A new NAT assignment y new = y Model definition During initialization each example x i in the dataset is paired with a random target y i , uniformly sampled from a d-dimensional sphere.
...
Delete-and-copy (DnC): With a probability α, delete the optimal target y

The L 2 loss between targets and latent representations is taken and combined with the auxiliary loss.

Gradients w.r.t.

θ are then taken and back-propagated along.
...
Warm-Up stage: α = 0 and λ is very small.2.

Transition stage: α increases gradually from 0 to 1, λ also increases gradually to a larger value (approximately 100× larger than its initial value).3.

Clustering stage: α = 1, λ is large.
...
The value of α varies throughout the training of a NATAC model.

In our experiments, we initially set α to zero for a period of pre-training, after which we incrementally increase the value to 1 over several epochs.
...
Models trained on the train set of 20 Newsgroups and evaluated on the test set.λfinal \λinital 10 but after 100 epochs of pre-training, the model does not significantly benefit from any more pretraining.
...
Similarly to α, we set λ to a small value for warm-up stage of training, and then progressively increase λ to a larger value afterwards.

In the other experiments involving 20 Newsgroups, warm-up training uses λ inital = 10 −4 and a final value of λ final = 10 −2 .

The transition happens at the same time as the change in α during training.

TAB6 shows how the final NMI of NATAC models vary with differing values for λ inital and λ final .

We notice that the value of λ final does not seem to greatly impact the number of clusters or NMI, and that the smaller values of λ inital have very similar NMI scores and number of clusters.

Interestingly, when trained with large λ inital of 10 −3 , the model scores higher in NMI than models with smaller λ inital and also converges to more clusters.

Here, we believe that a larger λ inital forces the model to learn a more uniform mapping to latent space (to minimize the NAT loss) during the warm-up stage of training.
...
However, NATAC does require some hyperparameters to be tuned, namely the dimensionality of the latent space, the length of warm-up training and the values for the loss coefficient λ.
...
Rather than using euclidean coordinates as the latent representation, z is considered a list of angles θ 1 , θ 2 · · · θ n where θ 1 · · · θ n−1 ∈ [0, π] and θ n ∈ [0, 2π].
...
This is so each batch has a representative sample of the targets to reassign in each training step.• The warm-up period is calculated by observing when the auxiliary objective starts to converge.• The final value for λ in training is set so that the NAT loss was approximately 1% of the total loss.

• The initial value for λ is set as approximately 1% of the final value of λ.• The transition phase typically lasts 100 epochs of training.• During the transition phase, the value of α is incrementally increased from 0 to 1.We now explicitly list the hyperparameters used for each experiment:

• A batch size of 100.• A warm-up period of 10 × d epochs, during which λ = 0.001.• A transition period lasts for 250 epochs, where λ is incrementally increased to 0.25, and α is incremented from 0 to 1.

• The ADAM optimizer BID20 ) with a learning rate (α = 10 −4 ) Figure 5 : Architecture of the encoder (left) and decoder (right) used for the MNIST experiments.
...
• A batch size of 100.• A warm-up period of 1, 000 epochs, during which λ = 10 −4 .•

A transition period lasts for 100 epochs, where λ is incrementally increased to 0.01, and α is incremented from 0 to 1.• The ADAM optimizer BID20 ) with a learning rate (α = 10 −5 ).•

Dropout with a keep-probability of 0.95 in the hidden layers of the encoder and decoder.

• A batch size of 100.• A warm-up period of 100 epochs, during which λ = 0.01.• A transition period lasts for 100 epochs, where λ is incrementally increased to 1, and α is incremented from 0 to 1.• The ADAM optimizer BID20 ) with a learning rate (α = 10 −3 ).
