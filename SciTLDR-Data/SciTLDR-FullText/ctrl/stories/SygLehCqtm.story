...
Kočiský et al. BID19 learn bilingual word embeddings jointly with a FastAlign BID20 word alignment model using expectation maximization.
...
In order to calculate the similarity of two amino acid sequences given that each has been encoded into a sequence of vector representations, z 1 ...z n and z 1 ...z m , we develop a soft symmetric alignment mechanism in which the similarity between two sequences is calculated based on their vector embeddings asŝ DISPLAYFORM0 where a ij are entries of the alignment matrix given by DISPLAYFORM1 , and DISPLAYFORM2 and A = n i=1 m j=1 a ij is the length of the alignment.
...
Given parameters θ 1 ...θ 4 and b 1 ...b 4 , the probability that two sequences share similarity greater than or equal to t is defined byp(y ≥ t) = sigmoid(θ tŝ + b t ), with the constraint that θ t ≥ 0 to enforce thatp increases monotonically withŝ.
...
Following common practice in the protein structure field, we define two positions as making contact if the Cα atoms of those residues occur within 8Åin the 3D structure.
...
We define the full multitask objective by DISPLAYFORM1 where λ is a parameter that interpolates between the structural similarity and contact prediction losses.
...
When using the full multitask objective, the contact prediction component uses minibatches of 10 sequences and λ = 0.1.
...
Additional runtime and memory details can be found in Appendix A.3.In the following sections, we refer to the 3-layer biLSTM encoder trained with the full framework as "SSA (full)" and the framework without contact prediction (i.e. λ = 1) as "SSA (no contact prediction)."
...
We measure accuracy, Pearson's correlation (r), Spearman's rank correlation (ρ), and average precision scores for retrieving protein pairs with structural similarity of at least class, fold, superfamily, and family levels.
...
For these baselines, we substitutê DISPLAYFORM0 in for SSA similarity (Equation 1) respectively during model training and prediction.

These models are trained without contact prediction (λ = 1) to compare the alignment component in isolation.
...
We select the resampling probability and λ hyperparameters based on structural similarity prediction accuracy on a validation set held out from the SCOP ASTRAL 2.06 training set (Section 4.1).
...
Based on this result, all models were trained with amino acid resampling of 0.05.Multitask loss weight, λ.

We evaluate three values of λ, interpolating between the structural similarity loss and contact prediction loss, for predicting structural similarity on the validation set.

Prediction accuracy increased progressively as λ decreased and all models trained with contact prediction outperformed those trained without contact prediction (Appendix Table 6 ).

Because it gave the best prediction accuracy on the validation set, λ = 0.1 was selected for training models with our full framework.
