...
Starting from H 0 = X, GCN uses the following rule to iteratively aggregate features from neighborhoods: DISPLAYFORM0 whereÃ = A + I N is the adjacency matrix of the undirected graph with added self connections, I N is the identity matrix,D is a diagonal matrix withD i,i = jÃ ij , and σ is the activation function.

We set σ(x) = ReLU(x) = max(0, x), which is the most common choice in practice.
...
The model could be simplified as: DISPLAYFORM1 whereÂ =D DISPLAYFORM2 Another choice for formingÂ is to normalize the adjacency matrix by rows, leading toÂ =D −1Ã .
...
The Pytorch code normalizes the adjacency matrix by rowÂ = D −1 (A + I), while the TensorFlow code normalizes it symmetricallyÂ = D −1/2 (A + I)D −1/2 .
