...
While deterministic methods such as mean (Little & Rubin, 1987) and median filling (Acuña & Rodriguez, 2004) produced only one possible value, it is desirable to generate samples by considering the data distribution, thus leading to stochastic-based imputation methods.
...
For instance, previous works exploited statistical attributes of observed data, such as mean (Little & Rubin, 1987) and median filling (Acuña & Rodriguez, 2004) , which clearly ignored the temporal relations as well as the correlations among variables.
...
T ×D indicating whether the values are observed or missing, and additionally define a new data representatioñ
...
Given the observations at each time pointx t , we infer z ∈ R k D as the latent representation by making use of the inference network, utilizing the true posterior distribution p φ (z|x t ).

Intuitively, we assume thatx t is generated from some unobserved random variable z by some conditional distribution p θ (x t |z), while z is generated from a prior distribution p θ (z).

Therefore, we define the marginal likelihood as p θ (x t ) = p θ (x t |z)p θ (z)dz.

However, since it is intractable due to involvement of the true posterior p θ (z|x t ), we approximate it with q φ (z|x t ) using a Gaussian distribution N (µ z , σ 2 z ), where the mean and log-variance are obtained such that:

, where E {µ,σ} denotes the inference network with parameter φ.

Furthermore, we apply the reparameterization trick proposed by Kingma & Welling (2014) as z = µ z + σ z z , where z ∼ N (0, I), and denotes the element-wise multiplication, thus, making it possible to be differentiated and trained using standard gradient methods.

Furthermore, given this latent vector z, we estimate p θ (x t |z) by means of the generative network D with parameter θ as :

where µ x and σ 2 x denote the mean and variance of reconstructed data distribution, respectively.

We apply another reparameterization trick in the data space to obtainx t = µ x + σ x x with x ∼ N (0, I).
...
In particular, we set the corresponding uncertainty as zero if the data is observed, indicating that we are confident with full trust to the observation, while set this as a value σ d x,t if the corresponding value is missing as:

As a result of VAE-based imputation network, we obtain the set {X,Ū} denoting the imputed values and its corresponding uncertainty, respectively.
...
Specifically, at each time step t, we produce the uncertainty decay factor υ t in the Eq. (1) using negative exponentional rectifier to guarantee υ t ∈ (0, 1], and further element-wise multiply this withx t to emphasizes the reliable estimates and give penalties to the uncertain ones expressed by the Eq. (2) resulting in
...
Hence, we have a pair of imputed values {x υ t , x r t } corresponding to missing values estimates based on features correlations and temporal relations, respectively.
...
As time delay ∆ t is essential element to capture temporal relations from the data (Che et al., 2018) , we also introduce the temporal decay factor γ t ∈ (0, 1] in the Eq. (6) as

We utilize this factor to control the influence of past observations embedded into hidden states using the form of (h t−1 γ t ).
...
Lastly, to predict the in-hospital mortality as the clinical outcome, we utilize the last hidden state h T to get the predicted labelŷ such that:

Note that W {υ,r,c,γ,h,y} , V h and b {υ,r,c,γ,h,y} } are our learnable parameters in recurrent imputation network.

We specify the composite loss function comprising of the imputation and prediction loss function to tune all model parameters jointly, which are ψ = {θ, φ, W {υ,r,c,γ,h,y} , V h , b {υ,r,c,γ,h,y} }.
...
We add 1 -regularization to introduce the sparsity into the network with λ 1 as the hyperparameter.
...
with α and β are the hyperparameters to represent the ratio between the L vae and L reg , respectively, and λ 2 is the weight decay hyperparameter.
...
We set λ 1 and λ 2 equally with 0.0001.
...
Specifically, we excluded the Eq. (1-2), and replaced x υ t withx t in Eq. (4).
...
As part of the ablation studies, first we investigated the effect of varying the pair {α, β} as the hyperparameters of the imputation on both V-RIN and V-RIN-full model on the in-hospital mortality prediction task.
...
V-RIN achieved its peak with setting {α = 0.25, β = 0.75}, while V-RIN-full with {α = 0.01, β = 0.5}. We interpreted these findings as by emphasizing more on the temporal relations than the features correlations in imputing the missing values, the model are able to obtain its best performance.

However, once we tried to increase the β, we observed that the performance were degraded to some degree.
