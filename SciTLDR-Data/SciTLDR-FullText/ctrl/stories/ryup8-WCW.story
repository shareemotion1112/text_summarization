...
Let θ (D) ∈ R D be a parameter vector in a parameter space of dimension D, let θ (D) 0 be a randomly chosen initial parameter vector, and let θ (D) * be the final parameter vector arrived at via optimization.

Consider a toy optimization problem where D = 1000 and where θ (D) optimized to minimize a squared error cost function that requires the first 100 elements to sum to 1, the second 100 elements to sum to 2, and so on until the vector has been divided into 10 groups with their requisite 10 sums.

We may start from a θ Solutions to this problem are highly redundant.
...
Standard optimization, which we will refer to hereafter as the direct method of training, entails evaluating the gradient of a loss with respect to θ (D) and taking steps directly in the space of θ (D) .

To train in a random subspace, we instead define θ (D) in the following way: DISPLAYFORM0 where P is a randomly generated D × d projection matrix 1 and θ (d) is a parameter vector in a gen- DISPLAYFORM1 and P are randomly generated and frozen (not trained), so the system has only d degrees of freedom.

We initialize θ (d) to a vector of all zeros, so initially DISPLAYFORM2 0 .
...
By this construction P forms an approximately orthonormal basis for a randomly oriented d dimensional subspace of R D , with the origin of the new coordinate system at θ0 .
...
To store this network, one need only store a tuple of three items: (i) the random seed to generate the frozen θ This compression approach differs from other neural network compression methods in the following aspects.
...
σ refers to the parameter perturbation noise used in ES.

Default Adam parameters of β 1 = 0.9, β 2 = 0.999, = 1 × 10 −7 were used.begin to see training runs reach the threshold as early as d = 400, with the median performance steadily increasing with d.
...
Algebraically, we need to left-multiply a vector of parameters v ∈ R d by a random matrix M ∈ R D×d , whose columns are orthonormal, then add an offset vector θ 0 ∈ R D .
