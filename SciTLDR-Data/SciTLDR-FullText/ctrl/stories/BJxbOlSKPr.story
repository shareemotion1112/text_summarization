...
To transform from continuous space to discrete space and back, two major functions are used: 1) a discretization function φ(·) :

D that maps a continuous vector into a K-way D-dimensional discrete code (KD code), and 2) a reverse-discretization function ρ(·) : {1, · · · , K} D → R d that maps the KD code into a continuous embedding vector.

In other words, the general DPQ mapping is T (·) = ρ • φ(·).
...
The discretization function of DPQ computes discrete codes C = φ(Q) where C ∈ {1, · · · , K} n×D is the KD codebook.

To construct the final embedding table for all symbols, the reverse-discretization function of DPQ is applied, i.e. H = ρ(C) where H ∈ R n×d is the final symbol embedding matrix.
...
In below, we specify the discretization function φ(·) and reverse-discretization function ρ(·) via product keys and values.

Product keys for discretization function φ(·).
...
Product values for reverse-discretization function ρ(·).
...
To approximate the arg max, similar to (Chen et al., 2018b; Jang et al., 2016) , we relax the softmax function with temperature τ :
...
The softmax approximated computation defined above is fully differentiable when τ = 0.

However, to compute discrete codes during the forward pass, we have to set τ → 0, which turns the softmax function into a spike concentrated on the C (j)

i -th dimension.

This is equivalent to the arg max operation which does not have gradient.

To enable a pseudo gradient while still be able to output discrete codes, we use a different temperatures during forward and backward pass, i.e. set τ → 0 in forward pass, and τ → 1 in the backward pass.
