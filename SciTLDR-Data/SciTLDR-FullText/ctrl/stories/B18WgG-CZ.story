...
Some recent work has addressed this by learning general-purpose sentence representations BID49 BID17 BID11 BID27 BID20 BID33 BID34 .
...
BID40 and BID4 demonstrate that neural machine translation (NMT) systems appear to capture morphology and some syntactic properties.

BID40 also present evidence that sequence-to-sequence parsers more strongly encode source language syntax.

Similarly, BID0 probe representations extracted by sequence autoencoders, word embedding averages, and skip-thought vectors with a multi-layer perceptron (MLP) classifier to study whether sentence characteristics such as length, word content and word order are encoded.
...
This hypothesis is based on the theoretical work of BID3 .
...
For example, BID29 present an additive and multiplicative linear composition function of the distributed representations of individual words.

BID10 combine symbolic and distributed representations of words using tensor products.

Advances in learning better distributed representations of words BID28 BID37 combined with deep learning have made it possible to learn complex non-linear composition functions of an arbitrary number of word embeddings using convolutional or recurrent neural networks (RNNs).
...
Notably, skip-thought vectors , an extension of the skip-gram model for word embeddings BID28 to sentences, learn re-usable sentence representations from weakly labeled data.

Unfortunately, these models take weeks or often months to train.

BID17 address this by considering faster alternatives such as sequential denoising autoencoders and shallow log-linear models.

BID1 , however, demonstrate that simple word embedding averages are comparable to more complicated models like skip-thoughts.

More recently, BID11 show that a completely supervised approach to learning sentence representations from natural language inference data outperforms all previous approaches on transfer learning benchmarks.
...
BID27 demonstrated that representations learned by state-of-the-art large-scale NMT systems also generalize well to other tasks.
...
BID20 and BID33 demonstrate that discourse-based objectives can also be leveraged to learn good sentence representations.

Our work is most similar to that of BID26 , who train a many-to-many sequenceto-sequence model on a diverse set of weakly related tasks that includes machine translation, constituency parsing, image captioning, sequence autoencoding, and intra-sentence skip-thoughts.

There are two key differences between that work and our own.

First, like BID27 , their use of an attention mechanism prevents learning a fixed-length vector representation for a sentence.
...
We further present a fine-grained analysis of how different tasks contribute to the encoding of different information signals in our representations following work by BID40 and BID0 .

BID16 similarly present a multi-task framework for textual entailment with task supervision at different levels of learning.

"Universal" multi-task models have also been successfully explored in the context of computer vision problems BID23 BID13 .
...
The input x and output y are sequences x 1 , x 2 , . . .

, x m and y 1 , y 2 . . .

, y n .

The encoder produces a fixed length vector representation h x of the input, which the decoder then conditions on to generate an output.

The decoder is auto-regressive and breaks down the joint probability of outputs into a product of conditional probabilities via the chain rule: DISPLAYFORM0 Cho et al. (2014) and use encoders and decoders parameterized as RNN variants such as Long Short-term Memory (LSTMs) BID18 or Gated Recurrent Units (GRUs) BID9 .
...
In this work, as in ; BID17 , we do not employ an attention mechanism.
...
The encoder is a bidirectional GRU while the decoder is a unidirectional conditional GRU whose parameterization is as follows: DISPLAYFORM1 DISPLAYFORM2 DISPLAYFORM3 The encoder representation h x is provided as conditioning information to the reset gate, update gate and hidden state computation in the GRU via the parameters C r , C z and C d to avoid attenuation of information from the encoder.3.1 MULTI-TASK SEQUENCE-TO-SEQUENCE LEARNING BID12 present a simple one-to-many multi-task sequence-to-sequence learning model for NMT that uses a shared encoder for English and task-specific decoders for multiple target languages.

BID26 extend this by also considering many-to-one (many encoders, one decoder) and many-to-many architectures.
...
Our motivation for multi-task training stems from theoretical insights presented in BID3 .
...
Skip-thought vectors Skip-thought vectors are an extension of skip-gram word embedding models to sentences.

The task typically requires a corpus of contiguous sentences, for which we use the BookCorpus BID22 .
...
We also consider training skip-thoughts by predicting only the next sentence given the current, motivated by results in BID44 where it is demonstrated that predicting the next sentence alone leads to comparable performance.
...
We train our representations using multiple target languages motivated by improvements demonstrated by BID12 .
...
We train on 3 million weakly labeled parses obtained by parsing a random subset of the 1-billion word corpus with the Puck GPU parser 1 along with gold parses from sections 0-21 of the WSJ section of Penn Treebank.
...
This is the same classification strategy adopted by BID11 .

We train on a collection of about 1 million sentence pairs from the SNLI BID6 and MultiNLI BID51 corpora.

BID12 use periodic task alternations with equal training ratios for every task.

In contrast, BID26 alter the training ratios for each task based on the size of their respective training sets.

Specifically, the training ratio for a particular task, α i , is the fraction of the number of training examples in that task to the total number of training samples across all tasks.

The authors then perform α i * N parameter updates on task i before selecting a new task at random proportional to the training ratios, where N is a predetermined constant.
...
Let θ denote each model's parameters, α a probability vector (p1 . . .

p k ) denoting the probability of sampling a task such that Σ k i pi = 1, datasets for each task IP1 . . .

IP k and a loss function L. DISPLAYFORM0
...
Table 2 that adding more tasks improves the transfer performance of our model.
...
We observe gains of 1.1-2.3% on the sentiment classification tasks (MR, CR, SUBJ & MPQA) over Infersent and smaller improvement of 0.5% on SST.
...
The addition of constituency parsing improves performance on sentence relatedness (SICK-R) and entailment (SICK-E) consistent with observations made by BID7 .

In Table 4 , we show that simply training an MLP on top of our fixed sentence representations outperforms several strong & complex supervised approaches that use attention mechanisms, even on this fairly large dataset.

For example, we observe a 0.2-0.5% improvement over the decomposable attention model BID35 .

When using only a small fraction of the training data, indicated by the columns 1k-25k, we are able to outperform the Siamese and Multi-Perspective CNN using roughly 6% of the available training set.

We also outperform the Deconv LVM model proposed by BID39 in this low-resource setting.

Unlike BID11 , who use pretrained GloVe word embeddings, we learn our word embeddings from scratch.

Somewhat surprisingly, in TAB4 we observe that the learned word embeddings are competitive with popular methods such as GloVe, word2vec, and fasttext BID5 on the benchmarks presented by BID14 and BID46 .In Table 5 , we probe our sentence representations to determine if certain sentence characteristics and syntactic properties can be inferred following work by BID0 and BID40 .
...
In Appendix TAB9 , we note that our sentence representations outperform skip-thoughts and are on par with Infersent for image-caption retrieval.

We also observe that comparing sentences using across all 10 tasks.
...
Underlines are used for each task to indicate both our best performing model as well as the best performing transfer model that isn't ours.cosine similarities correlates reasonably well with their relatedness on semantic textual similarity benchmarks (Appendix Table 7 ).We also present qualitative analysis of our learned representations by visualizations using dimensionality reduction techniques ( FIG0 ) and nearest neighbor exploration (Appendix Table 8 ).

FIG0 shows t-sne plots of our sentence representations on three different datasets -SUBJ, TREC and DBpedia.

DBpedia is a large corpus of sentences from Wikipedia labeled by category and used by BID53 .

Sentences appear to cluster reasonably well according to their labels.

The clustering also appears better than that demonstrated in FIG1 of on TREC and SUBJ.

Appendix Table 8 contains sentences from the BookCorpus and their nearest neighbors.

Sentences with some lexical overlap and similar discourse structure appear to be clustered together.

BID30 respectively.

We also report QVEC benchmarks BID46 Model Accuracy 1k 5k 10k 25k All (400k) Table 4 : Supervised & low-resource classification accuracies on the Quora duplicate question dataset.

Accuracies are reported corresponding to the number of training examples used.

The first 6 rows are taken from , the next 4 are from BID45 , the next 5 from BID39 and The last 4 rows are our experiments using Infersent BID11 and our models.
...
Table 5 : Evaluation of sentence representations by probing for certain sentence characteristics and syntactic properties.

Sentence length, word content & word order from BID0 and sentence active/passive, tense and top level syntactic sequence (TSS) from BID40 .
...
Having a rich, continuous sentence representation space could allow the application of state-of-the-art generative models of images such as that of BID32 to language.
...
For natural language inference, the same encoder is used to encode both the premise and hypothesis and a concatenation of their representations along with the absolute difference and hadamard product (as described in BID11 ) are given to a single layer MLP with a dropout BID42 ) rate of 0.3.

All models use word embeddings of 512 dimensions and GRUs with either 1500 or 2048 hidden units.

We used minibatches of 48 examples and the Adam BID21 optimizer with a learning rate of 0.002.

Models were trained for 7 days on an Nvidia Tesla P100-SXM2-16GB GPU.

While report close to a month of training, we only train for 7 days, made possible by advancements in GPU hardware and software (cuDNN RNNs).We did not tune any of the architectural details and hyperparameters owing to the fact that we were unable to identify any clear criterion on which to tune them.
...
For example, use the last hidden state while BID11 perform max-pooling across all of the hidden states.
...
We use 113k training images from MSCOCO with 5k images for validation and 5k for testing.
...
In addition to the above tasks which were considered by BID11 , we also evaluate on the recently published Quora duplicate question dataset 5 since it is an order of magnitude larger than the others (approximately 400,000 question pairs).

The task is to correctly identify question pairs that are duplicates of one another, which we formulate as a binary classification problem.
...
We also artificially create a low-resource setting by reducing the number of training examples between 1,000 and 25,000 using the same splits as BID39 .

In an attempt to understand what information is encoded in by sentence representations, we consider six different classification tasks where the objective is to predict sentence characteristics such as length, word content and word order BID0 or syntactic properties such as active/passive, tense and the top syntactic sequence (TSS) from the parse tree of a sentence BID40 .The sentence characteristic tasks are setup in the same way as described in BID0 .

The length task is an 8-way classification problem where sentence lengths are binned into 8 ranges.

The content task is formulated as a binary classification problem that takes a concatenation of a sentence representation u ∈ R k and a word representation w ∈ R d to determine if the word is contained in the sentence.
...
The syntactic properties tasks are setup in the same way as described in BID40 .The passive and tense tasks are characterized as binary classification problems given a sentence's representation.
...
Numbers reported are Pearson Correlations x100.

Skipthought, GloVe average, GloVe TF-IDF, GloVe + WR (U) and all supervised numbers were taken from BID1 and BID49 and Charagram-phrase numbers were taken from BID50 .

Other numbers were obtained from the evaluation suite provided by BID11

<|TLDR|>

@highlight

A large-scale multi-task learning framework with diverse training objectives to learn fixed-length sentence representations

@highlight

This paper is about learning sentence embeddings by combining several training signals: skip-thought, predicting translation, classifying entailment relationships, and predicting the constituent parse.