...
The problem of link prediction in an incomplete undirected hypergraph H involves predicting missing hyperlinks fromĒ = 2 V − E based on the current set of observed hyperlinks E. The number of hypernodes in any given hyperlink e ∈ E can be any integer between 1 and 2 n .
...
However, in practical cases, there is no need to consider all the hyperlinks inĒ as most of them can be easily filtered out .
...
Each e ∈ E is denoted by (t, h) where t ⊆ V is the tail and h ⊆ V is the head with t = Φ, h = Φ. As shown in figure 1, chemical reactions can be modeled by directed hyperlinks with chemical substances forming the set V .
...
The forward model for a simple two-layer GCN takes the following simple form: DISPLAYFORM1 whereĀ =D DISPLAYFORM2 is an input-to-hidden weight matrix for a hidden layer with h hidden units and Θ (1) ∈ R h×r is a hidden-to-output weight matrix.
...
Θ (0) and Θ (1) , are trained using gradient descent.
...
In the experiments, we used a simple 2-layer multilayer perceptron on the concatenated embeddings x 1 ||x 2 i.e.g(x 1 , x 2 ) = MLP Θ x 1 ||x 2 .We train DISPLAYFORM5 , and Θ end-to-end using backpropagation.
...
We compared NHP against the following state-of-the-art baselines for the same E as constructed above.• Spectral hypergraph Clustering (SHC) BID43 : SHC outputs classification scores by f = (I − ξΘ)
