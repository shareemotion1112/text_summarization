...
The intermediate representations for layers λ = 1, ..., L are denoted as h λ = W λ a λ−1 and a λ = f λ (h λ ).

We refer to a λ as the activations, and h λ as the (linear) pre-activations.

The bias terms are absorbed into the W λ by appending a 1 to each a λ .
...
Recent work BID27 BID3 exploits that, for a single data point, the diagonal blocks of these curvature matrices are Kronecker factored: DISPLAYFORM0 where H λ is the Hessian w.r.t.

the weights in layer λ.

Q λ = a λ−1 a T λ−1 denotes the covariance of the incoming activations a λ−1 and H λ = ∂ 2 E ∂h λ ∂h λ the pre-activation Hessian, i.e. the Hessian of the error w.r.t.

the linear pre-activations h λ in a layer.
...
Second, they assume Q λ and H λ to be independent.
...
The main difference between the Kronecker factored second-order optimisers lies in how they efficiently approximate E [H λ ].
...
Specifically, if we approximate the log posterior over the weights of a network given some data D around a MAP estimate θ * , we obtain: DISPLAYFORM0 where DISPLAYFORM1 is the stacked vector of weights andH = E [H] the average Hessian of the negative log posterior 1 .

The first order term is missing because we expand the function around a maximum θ * , where the gradient is zero.

If we exponentiate this equation, it is easy to notice that the right-hand side is of Gaussian functional form for θ, thus we obtain a normal distribution by integrating over it.

The posterior over the weights is then approximated as Gaussian: DISPLAYFORM2 assumingH is p.s.d.

We can then approximate the posterior mean when predicting on unseen data D * by averaging the predictions of T Monte Carlo samples θ (t) from the approximate posterior: DISPLAYFORM3
...
We approximate this by adding a multiple of the identity to each of the Kronecker factors from the log likelihood: DISPLAYFORM0 where τ is the precision of the Gaussian prior on the weights and N the size of the dataset.
...
First, we use the untargeted Fast Gradient Sign method x adv = x − η sgn(∇ x max y log p (M ) (y|x)) suggested in BID8 , which takes the gradient of the class predicted with maximal probability by method M w.r.t.

the input x and reduces this probability with varying step size η.
...
FIG3 shows the average predictive uncertainty and the accuracy on the original class on the MNIST test set as the step size η increases.
...
The updates are of similar form to the untargeted attack, however they increase the probability of the pre-specified class y rather than decreasing the current maximum as x DISPLAYFORM0 We use a step size of η=10 −2 for the targeted attack.
...
a weight W λ a,b is: DISPLAYFORM1 Keeping λ fixed and differentiating again, we find that the per-sample Hessian of that layer is: DISPLAYFORM2 where DISPLAYFORM3 is the pre-activation Hessian.
...
While the square root of Q λ is calculated during the forward pass on all layers, H requires an additional backward pass.
...
If we denote the dimensionality of the input to layer λ as D λ−1 and its output as D λ , the curvature factors correspond to the two precision matrices with DISPLAYFORM0 'parameters' to estimate, since they are symmetric.
...
The residual blocks compute a function of the form res(x) = x + f φ (x), where f φ typically is a sequence of convolutions, batch normalisation and elementwise nonlinearities.
