...
Two metrics are used to check our prediction that some lucky student nodes converge to the teacher:Normalized correlationρ.

We compute normalized correlation (or cosine similarity) ρ between teacher and student activations evaluated on a validation set.

At each layer, we average the best correlation over teacher nodes: ρ = mean j • max j ρ jj • , where ρ jj • is computed for each teacher and student pairs (j, j• )

.ρ ≈ 1 means that most teacher nodes are covered by at least one student.
...
Experiments are summarized in Fig. 5 and FIG3 .ρ indeed grows during training, in particular for low layers that are closer to the input, whereρ moves towards 1.
...
Using BatchNorm accelerates the growth of accuracy, improvesr, but seems not to accelerate the growth ofρ.

The theory also predicts that the top-down modulation β helps the convergence.

For this, we plot β * jj • at different layers during optimization on GAUS.

For better visualization, we align each student node index j with a teacher node j• according to highest ρ.
...
Correlationρ and mean rankr over training on GAUS.ρ steadily grows andr quickly improves over time.
...
We use the following equality extensively: σ(x) = σ (x)x.
...
4 with β * kk • (x) and β kk (x) defined as follows: DISPLAYFORM4 Note that this formulation allows different number of nodes for the teacher and student.
...
DISPLAYFORM8 We can define similar notations for W (which has n l columns/filters), β, D, H and L FIG4

In the following, we will use Eqn.

8 to analyze the dynamics of the multi-layer ReLU networks.

For convenience, we first define the two functions ψ l and ψ d (σ is the ReLU function): DISPLAYFORM0 We assume these two functions have the following property .
...
Due to this property, in the following, for convenience we assume w j 2 = w * j 2 = 1, and the gradientẇ j is always orthogonal to the current weight w j .
...
If n l = m l , L * l = L l = I (e.g., the input of layer l is whitened) and β * l+1 = β l+1 = 11 T (all β entries are 1), then the following theorem shows that weight recovery could follow (we use j as j • ).
...
we can see that due to the fact that h * jj in general becomes larger when w j → w * j (since cos θ 0 can be close to 1), we expect a super-linear convergence near w * j .
...
Intuitively, the projection is needed to remove any ambiguity related to weight scaling, in which the output remains constant if the top-layer weights are multiplied by a constant α, while the low-layer weights are divided by α.
...
See Appendix for the proof (and definition ofλ in Eqn.
...
When V r becomes small, so does β jj for j ∈ [r] or j ∈ [r].
...
For simplicity, the theorem uses a uniform (and conservative) 497 498 499 500 501 502 503 504 505 506 507 508 509 510 511 512 513 514 515 516 517 518 519 520 521 522 523 524 525 526 527 528 529 530 531 532 533 534 535 536 537 538 539 540 541 542 543 544 545 546 547 548 γ throughout the iterations.

In practice, γ is initially small (due to noise introduced by r-set) but will be large after a few iterations when V r vanishes.

Given the same learning rate, this leads to accelerated convergence.

At some point, the learning rate η becomes too large, leading to fluctuation.

In this case, η needs to be reduced.
...
Given a batch with size N , denote pre-batchnorm activations as DISPLAYFORM0 T and gradients as DISPLAYFORM1 T (See FIG0 ).f = (f − µ)/σ is its whitened version, and c 0f + c 1 is the final output of BN.

Here µ = DISPLAYFORM2 2 and c 1 , c 0 are learnable parameters.
...
3. FIG0 , using the property that E x g lin j f lin j = 0 (the expectation is taken over batch) and the weight update ruleẇ jk = E x g lin j f k (over the same batch), we have: DISPLAYFORM4 For FIG0 , note that DISPLAYFORM5 rl j = 0 and conclusion follows.

For simplicity, in the following, we use δw j = w j − w * j .
...
The intuition here is that both the volume of the affected area and the weight difference are proportional to δw j .

l * jj − l jj is their product and thus proportional to δw j 2 .

See FIG0 .

Proof.

First of all, note that δw j = 2 sin θj 2 ≤ 2 sin θ0 2 .

So given θ 0 , we also have a bound for δw j .

When β = β * = 11 T , the matrix form can be written as the following: DISPLAYFORM0 by using P ⊥ wj w j ≡ 0 (and thus h jj doesn't matter).

Since w j is conserved, it suffices to check whether the projected weight vector P ⊥ w * j w j of w j onto the complementary space of the ground truth node w * j , goes to zero: DISPLAYFORM1 Denote θ j = ∠(w j , w * j ) and a simple calculation gives that sin θ j = P ⊥ w * j w j .

First we have: DISPLAYFORM2 From Lemma 2, we knows that DISPLAYFORM3 Note that here we have δw j = 2 sin DISPLAYFORM4 We discuss finite step with very small learning rate η > 0: DISPLAYFORM5 Here DISPLAYFORM6 is an iteration independent constant.

We set γ = cos DISPLAYFORM7 jj and from Lemma 2 we know d * jj ≥d for all j.

Then given the inductive hypothesis that sin θ t j ≤ (1 − ηdγ) t−1 sin θ 0 , we have: DISPLAYFORM8 Therefore, sin θ DISPLAYFORM9 The proof can be decomposed in the following three lemma.
...
For large teacher networks, BatchNorm definitely increases convergence speed and growth ofρ.
...
Visualization of (transpose of) H * and β * matrix before and after optimization (using GAUS).

Student node indices are reordered according to teacher-student node correlations.

After optimization, student node who has high correlation with the teacher node also has high β entries.

Such a behavior is more prominent in H * matrix that combines β * and the activation patterns D * of student nodes (Sec. 5).
