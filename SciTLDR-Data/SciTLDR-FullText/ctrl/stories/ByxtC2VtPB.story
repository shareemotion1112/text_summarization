...
However, as shown in Fig. 1(a) , the equivalent perturbation, i.e., the crafted adversarial perturbation, is still δ and this strategy is easy to be adaptively evaded since the randomness of x 0 w.r.t x 0 is local (Athalye et al., 2018) .
...
In each execution, MI performs a global linear transformation on the inputs, which mixups the input x with a sampled clean example x s , i.e.,x = λx + (1 − λ)x s (detailed in Alg.
...
One is perturbation shrinkage: if the input is adversarial, i.e., x = x 0 + δ, the perturbation δ will shrink by a factor λ after performing MI, which is exactly the mixup ratio of MI according to the similarity between triangles.

Another one is input transfer: after the MI operation, the reduced perturbation λδ acts on random x 0 .
...
Furthermore, the global linearity of the mixup-trained models ensures that the information of x 0 remained inx 0 is proportional to λ, such that the identity of x 0 can be recovered from the statistics ofx 0 .
...
The classifier F makes a correct prediction on x if y =ŷ.

In the adversarial setting, we augment the data pair (x, y) to a triplet (x, y, z) with an extra binary variable z, i.e.,

The variable z is usually considered as hidden in the inference phase, so an input x (either clean or adversarially corrupted) can be generally denoted as x = x 0 + δ · 1 z=1 .

Here x 0 is a clean sample from the data manifold p(x) with label y 0 , 1 z=1 is the indicator function, and δ is a potential perturbation crafted by adversaries.

It is worthy to note that the perturbation δ should not change the true label of the input, i.e., y = y 0 .

For p -norm adversarial attacks (Kurakin et al., 2017; Madry et al., 2018) , we have δ p ≤ , where is a preset threshold.

Based on the assumption that adversarial examples are off the data manifolds, we formally have x 0 + δ / ∈ supp(p(x)) (Pang et al., 2018a) .
...
As an alternative, introduce the mixup training mechanism, which minimizes 1 m m j=1 L(F (x j ),ỹ j ).

Herex j = λx j0 + (1 − λ)x j1 ;ỹ j = λy j0 + (1 − λ)y j1 , the input-label pairs (x j0 , y j0 ) and (x j1 , y j1 ) are randomly sampled from the training dataset, λ ∼ Beta(α, α) and α is a hyperparameter.
...
In the following, we apply colored y,ŷ and y s to visually distinguish different notations.

Consider an input triplet (x, y, z), where z is unknown in advance.

When directly feeding x into the classifier F , we can obtain the predicted labelŷ.
...
Every time we execute MI, we first sample a label y s ∼ p s (y), then we sample x s from p s (x|y s ) and mixup it with x asx = λx + (1 − λ)x s .
...
Here we fix the mixup ratio λ in MI as a hyperparameter, while similar properties hold if λ comes from certain distribution.

Theoretically, with unlimited capability and sufficient clean samples, a well mixup-trained model F can be denoted as a linear function H on the convex combinations of clean examples (Hornik et al., 1989; Guo et al., 2019) , i.e., ∀x i , x j ∼ p(x) and λ ∈ [0, 1], there is
...
Input: The mixup-trained classifier F ; the input x. Hyperparameters: The sample distribution p s ; the mixup ratio λ; the number of execution N .

adversarial, then there should be an extra non-linear part G(δ; x 0 ) of F , since x is off the data manifolds.
...
where ∞ −→ represents the limitation when the execution times N → ∞. Now we separately investigate the y-th andŷ-th (could be the same one) components of F (x) according to Eq. (5), and see how these two components differ from those of F (x).
...
Furthermore, according to Eq. (2), there is 1 y=ŷ = 1 z=0 .

We can represent theŷ-th components as
...
MI with predicted label (MI-PL): In this case, the sampling label y s is the same as the predicted labelŷ, i.e., p s (y) = 1 y=ŷ is a Dirac distribution onŷ.

In this case, the label y s is uniformly sampled from the labels other thanŷ, i.e., p s (y) = Uŷ(y) is a discrete uniform distribution on the set {y ∈ [L]|y =ŷ}.
...
Specifically, in the general-purpose setting where we aim to correctly classify adversarial examples (Madry et al., 2018) , we claim that the MI method improves the robustness if the prediction value on the true label y increases while it on the adversarial labelŷ decreases after performing MI when the input is adversarial (z = 1).

This can be formally denoted as

We refer to this condition in Eq. (10) as robustness improving condition (RIC).

Further, in the detection-purpose setting where we want to detect the hidden variable z and filter out adversarial inputs, we can take the gap of theŷ-th component of predictions before and after the MI operation, i.e., ∆Fŷ(x; p s ) as the detection metric (Pang et al., 2018a) .

To formally measure the detection ability on z, we use the detection gap (DG), denoted as

A higher value of DG indicates that ∆Fŷ(x; p s ) is better as a detection metric.
...
Since an adversarial perturbation usually suppress the predicted confidence on the true label and promote it on the target label (Goodfellow et al., 2015) , there should be Gŷ(δ;x 0 ) > 0 and G y (δ;x 0 ) < 0.
...
The second mechanism is perturbation shrinkage, where the original perturbation δ shrinks by a factor λ.

This equivalently shrinks the perturbation threshold since λδ p = λ δ p ≤ λ , which means that MI generally imposes a tighter upper bound on the potential attack ability for a crafted perturbation.
...
As to MI-OL, when the input is clean (z = 0), there would be a degeneration on the optimal clean prediction as F y (x) = Fŷ(x) = λ, since the sampled x s does not come from the true label y. As compensation, MI-OL can better improve robustness compared to MI-PL when the input is adversarial (z = 1), since the sampled x s also does not come from the adversarial labelŷ in this case.

General-purpose defense: Note that in the MI-OL formulas of Table 1 , there is a term of 1 y=ys .

Since we uniformly select y s from the set [L] \ {ŷ}, there is E(1 y=ys ) = 1 L−1 .
...
The hyperparameter α for mixup and interpolated AT is 1.0 .

All defenses with randomness are executed 30 times to obtain the averaged predictions .

To verify and illustrate our theoretical analyses in Sec. 3, we provide the empirical relationship between the output predictions of MI and the hyperparameter λ in Fig. 2 .
...
Here x * 0 is a randomly perturbed image in the neighborhood of x 0 , i.e.,Ů (x 0 , ), and the finally returned adversarial example is x = x * r = x 0 + δ, following our notations in the main text.
...
Adversarial examples require the perturbation δ to be bounded by a small threshold under p -norm, i.e., δ p ≤ .
...
AT + MI-OL (ablation study) The λOL = 0.8
...
Besides, to better defend white-box adaptive attacks, the mixup ratio λ in MI could also be sampled from certain distribution to put in additional randomness.
...
To improve the performance of MI, a stronger regularization can be imposed, e.g., training with mixup for more epochs, or applying matched λ both in training and inference.
...
Note that generally the λ in MI comes from certain distribution.

For simplicity, we fix λ as a hyperparameter in our implementation.

Therefore, the gradients of the prediction w.r.t.

the input x is:

= E ps ∂F (u) ∂u u=λx+(1−λ)xs ·

∂λx + (1 − λ)x s ∂x

= λE ps ∂F (u) ∂u | u=λx+(1−λ)xs .
...
sign ∂F MI (x) ∂x ≈ sign N A k=1 ∂F (u) ∂u u=λx+(1−λ)x s,k .(24)
