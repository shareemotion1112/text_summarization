...
The policy π lo is trained using a goal-conditioned reward; e.g. the reward of a transition g, s, s is −D(f (s ), g), where D is a distance function.

In this work we adopt a slightly different interpretation of the lower-level policy and its relation to π hi .

Every c steps, the higher-level policy chooses a goal g t based on a state s t .

We interpret this state-goal pair as being mapped to a nonstationary policy π(a|s t+k , k), π ∈ Π, where Π denotes the set of all possible c-step policies acting on M. We use Ψ to denote this mapping from S × G to Π. In other words, on every c th step, we encounter some state s t ∈ S.

We use the higher-level policy to sample a goal g t ∼ π hi (g|s t ) and translate this to a policy π t = Ψ(s t , g t ).

We then use π t to sample actions a t+k ∼ π t (a|s t+k , k) for k ∈ [0, c − 1].

The process is then repeated from s t+c .Although the difference in this interpretation is subtle, the introduction of Ψ is crucial for our subsequent analysis.

The communication of g t is no longer as a goal which π hi desires to reach, but rather more precisely, as an identifier to a low-level behavior which π hi desires to induce or activate.

The mapping Ψ is usually expressed as the result of an RL optimization over Π; e.g., DISPLAYFORM0 where we use P π (s t+k |s t ) to denote the probability of being in state s t+k after following π for k steps starting from s t .

We will consider variations on this low-level objective in later sections.

From Equation 1 it is clear how the choice of representation f affects Ψ (albeit indirectly).We will restrict the environment reward function R to be defined only on states.

We use R max to denote the maximal absolute reward: R max = sup S |R(s)|.

In the previous section, we introduced two-level policies where a higher-level policy π hi chooses goals g, which are translated to lower-level behaviors via Ψ. The introduction of this hierarchy leads to a natural question: How much do we lose by learning π hi which is only able to act on M via Ψ?

The choice of Ψ restricts the type and number of lower-level behaviors that the higher-level policy can induce.

Thus, the optimal policy on M is potentially not expressible by π hi .

Despite the potential lossy-ness of Ψ, can one still learn a hierarchical policy which is near-optimal?To approach this question, we introduce a notion of sub-optimality with respect to the form of Ψ: Let π * hi (g|s, Ψ) be the optimal higher-level policy acting on G and using Ψ as the mapping from G to low-level behaviors.

Let π * hier be the corresponding full hierarchical policy on M. We will compare π * hier to an optimal hierarchical policy π * agnostic to Ψ. To define π * we begin by introducing an optimal higher-level policy π * * hi (π|s) agnostic to Ψ; i.e. every c steps, π * * hi samples a low-level behavior π ∈ Π which is applied to M for the following c steps.
...
We would like to compare π * hier to π * .

A natural and common way to do so is in terms of state values.

Let V π (s) be the future value achieved by a policy π starting at state s. We define the sub-optimality of Ψ as DISPLAYFORM0 The state values V π * hier (s) are determined by the form of Ψ, which is in turn determined by the choice of representation f .
...
In the following section, we derive a series of bounds which establish a more direct relationship between SubOpt(Ψ) and f .

Our main result will show that if one defines Ψ as a slight modification of the traditional objective given in Equation 1, then one may translate sub-optimality of Ψ to a practical representation learning objective for f .

In this section, we provide proxy expressions that bound the sub-optimality induced by a specific choice of Ψ. Our main result is Claim 4, which connects the sub-optimality of Ψ to both goalconditioned policy objectives (i.e., the objective in 1) and representation learning (i.e., an objective for the function f ).
...
In this setting, the class of low-level policies Π may be taken to be simply A, where a ∈ Π corresponds to a policy which always chooses action a. There is no temporal abstraction: The higher-level policy chooses a high-level action g ∈ G at every step, which is translated via Ψ to a low-level action a ∈ A. Our claims are based on quantifying how many of the possible low-level behaviors (i.e., all possible state to state transitions) can be produced by Ψ for different choices of g. To quantify this, we make use of an auxiliary inverse goal model ϕ(s, a), which aims to predict which goal g will cause Ψ to yield an actionã = Ψ(s, g) that induces a next state distribution P (s |s,ã) similar to P (s |s, a).3 We have the following theorem, which bounds the sub-optimality in terms of total variation divergences between P (s |s, a) and P (s |s,ã): DISPLAYFORM0 DISPLAYFORM1 Proof.

See Appendices A and B for all proofs.

Theorem 1 allows us to bound the sub-optimality of Ψ in terms of how recoverable the effect of any action in A is, in terms of transition to the next state.

One way to ensure that effects of actions in A are recoverable is to have an invertible Ψ. That is, if there exists ϕ : S × A → G such that Ψ(s, ϕ(s, a)) = a for all s, a, then the sub-optimality of Ψ is 0.However, in many cases it may not be desirable or feasible to have an invertible Ψ. Looking back at Theorem 1, we emphasize that its statement requires only the effect of any action to be recoverable.

That is, for any s, ∈ S, a ∈ A, we require only that there exist some g ∈ G (given by ϕ(s, a)) which yields a similar next-state distribution.

To this end, we have the following claim, which connects the sub-optimality of Ψ to both representation learning and the form of the low-level objective.

Claim 2.

Let ρ(s) be a prior and f, ϕ be so that, for DISPLAYFORM2 If the low-level objective is defined as DISPLAYFORM3 then the sub-optimality of Ψ is bounded by C .We provide an intuitive explanation of the statement of Claim 2.

First, consider that the distribution K(s |s, a) appearing in Equation 4 may be interpreted as a dynamics model determined by f and ϕ. By bounding the difference between the true dynamics P (s |s, a) and the dynamics K(s |s, a) implied by f and ϕ, Equation FORMULA4 states that the representation f should be chosen in such a way that dynamics in representation space are roughly given by ϕ(s, a).
...
Moving on to Equation 5, we note that the form of Ψ here is only slightly different than the onestep form of the standard goal-conditioned objective in Equation FORMULA0 .
...
Let ρ(s) be a prior over S. Let f, ϕ be such that, DISPLAYFORM2 DISPLAYFORM3 If the low-level objective is defined as DISPLAYFORM4 then the sub-optimality of Ψ is bounded by C .Claim 4 is the main theoretical contribution of our work.
...
We emphasize that Claim 4 applies to any class of low-level policies Π, including either closed-loop or open-loop policies.
...
We begin by elaborating on how we translate Equation 8 into a practical training objective for f and auxiliary ϕ (as well as a practical parameterization of policies π as input to ϕ).
...
In this way, we may learn f and lower-level policy to directly optimize a bound on the sub-optimality of Ψ. A pseudocode of the full algorithm is presented in the Appendix (see Algorithm 1).

Consider a representation function f θ : S → R d and an auxiliary function ϕ θ : S × Π → R d , parameterized by vector θ.

In practice, these are separate neural networks: DISPLAYFORM0 While the form of Equation 8 suggests to optimize a supremum over all s t and π, in practice we only have access to a replay buffer which stores experience s 0 , a 0 , s 1 , a 1 , . . .

sampled from our hierarchical behavior policy.

Therefore, we propose to choose s t sampled uniformly from the replay buffer and use the subsequent c actions a t:t+c−1 as a representation of the policy π, where we use a t:t+c−1 to denote the sequence a t , . . .

, a t+c−1 .

Note that this is equivalent to setting the set of candidate policies Π to A c (i.e., Π is the set of c-step, deterministic, open-loop policies).

This choice additionally simplifies the possible structure of the function approximator used for ϕ θ (a standard neural net which takes in s t and a t:t+c−1 ).

Our proposed representation learning objective is thus, DISPLAYFORM1 where J(θ, s t , a t:t+c−1 ) will correspond to the inner part of the supremum in Equation 8.We now define the inner objective J(θ, s t , a t:t+c−1 ).

To simplify notation, we use DISPLAYFORM2 suggests the following learning objective on each s t , π ≡ a t:t+c−1 : DISPLAYFORM3 DISPLAYFORM4 where B is a constant.

The gradient with respect to θ is then, DISPLAYFORM5 Published as a conference paper at ICLR 2019The first term of Equation 14 is straightforward to estimate using experienced s t+1:t+k .
...
Indeed, one may interpret our objective as maximizing a mutual information M I(s t+k ; s t , π) via an energy function given by E θ (s t+k , s t , π).
...
Equation 9 suggests to optimize a policy π st,g (a|s t+k , k) for every s t , g. This is equivalent to the parameterization π lo (a|s t , g, s t+k , k), which is standard in goal-conditioned hierarchical designs.

Standard RL algorithms may be employed to maximize the low-level reward implied by Equation 9: DISPLAYFORM0 weighted by w k and where π corresponds to π lo when the state s t and goal g are fixed.

While the first term of Equation 16 is straightforward to compute, the log probabilities log ρ(s t+k ), log P π (s t+k |s t ) are in general unknown.

To approach this issue, we take advantage of the representation learning objective for f, ϕ. When f, ϕ are optimized as dictated by Equation 8, we have DISPLAYFORM1 We may therefore approximate the low-level reward as DISPLAYFORM2 As in Section 5.1, we use the sampled actions a t:t+c−1 to represent π as input to ϕ. We approximate the third term of Equation 18 analogously to Equation 15.
...
A PROOF OF THEOREM 3 (GENERALIZATION OF THEOREM 1)Consider the sub-optimality with respect to a specific state s 0 , V DISPLAYFORM0 Recall that π * is the hierarchical result of a policy π *

* hi : S → ∆(Π), and note that π * * hi may be assumed to be deterministic due to the Markovian nature of M. We may use the mapping ϕ to transform π * * hi to a high-level policy π hi on G and using the mapping Ψ: DISPLAYFORM1 Let π hier be the corresponding hierarchical policy.

We will bound the quantity V DISPLAYFORM2 hier (s 0 ).

We follow logic similar to BID1 and begin by bounding the total variation divergence between the γ-discounted state visitation frequencies of the two policies.

Denote the k-step state transition distributions using either π * or π hier as, DISPLAYFORM3 DISPLAYFORM4 for k ∈ [1, c].
...
The network structure for ϕ θ is identical, except using hidden layer dimensions 400 and 300.

We also parameterize ϕ(s, π) := f θ (s) + ϕ θ (s, π).
...
Figure 7: We provide additional results comparing to variants of β-VAE BID10 .
