...
where x ∈ C N is the vectorized unknown image, A ∈ C M ×N is the discretized forward model describing the imaging system, y ∈ C M is a vector of the acquired measurements, and v ∼ N c 0, σ 2 I is a complex-valued Gaussian noise vector.
...
where Q(x) is a suitable regularization term, and λ > 0 is the strength of the regularization.

An alternative, equivalent formulation that directly accounts for the model error due to noise is the constrained problem:

where = σ √ M is the square-root of the expected noise power in the measurements.
...
where ρ > 0 is the ADMM penalty parameter and L2Proj(z, ) is the projection of z onto the 2 -ball of radius .
...
For a fixed CNN R w , the DBP model is a special case of (4):xw ≡ Fw(y; A, ), wherẽ w = (w, ρ) are the network parameters, and the network uses measurements together with knowledge of the system and noise power to return an estimate of the image.
...
We added complex-valued Gaussian noise with standard deviation σ = 0.01 to the noise-free, averaged data.
