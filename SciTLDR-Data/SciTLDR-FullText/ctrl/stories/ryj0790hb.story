...
A binary switching vector α controls the output of the network by switching the controller modules on or off.

α can be determined either manually or via a sub-network ("Dataset Decider") which determines the source domain of the image, switching accordingly between different sets of control parameters.
...
Most contemporary DCNN's follow a common structure: for each input x, the DCNN computes a representation of the input by passing it through a set of l layers φ i , i ∈ 1 . . .
...
We use an architecture where all non-linearities σ are the same function, with no tunable parameters.

Denote by Φ F N (x) the feature part of N .
...
It is also possible to drop the Φ C N term, if the network is fully convolutional, as in BID24 .
...
Each controller module uses the existing weights of the corresponding layer of N to create new convolutional filters adapted to the new task T 2 : for each convolutional layer φ l in N , let F l ∈ R Co×Ci×k×k be the set of filters for that layer, where C o is the number of output features, C l the number of inputs, and k × k the kernel size (assuming a square kernel).
...
This way, we can writẽ DISPLAYFORM3 where W l ∈ R Co×Co is a weight matrix defining linear combinations of the flattened filters of F l , resulting in C o new filters.

UnflatteningF a l to its original shape results in F a l ∈ R Co×Ci×k×k , which we call the adapted filters of layer φ l .
...
The output of layer φ l is computed as follows: let x l be the input of φ l in the adapted network.
...
In this case α is extended to one-hot vector of values determined by another sub-network, allowing each controller network to be switched on or off as needed.
...
layer φ l where D = C i × k × k. A controller module for φ l requires C In the final architecture we use the total number of weights required to adapt the convolutional layers Φ l combined with a new fully-connected layer amounts to about 13% of the original parameters.
...
We show how to discern the domain of an input image and output a proper classification (4.2.1) without manual choice of the control parameters α.
...
With this "dataset-decider", named N dc we augment DAN sketch to set for each input image I from any of the datasets D i the controller scalar α i of DAN sketch→Di to 1 if and only if N dc deemed I to originate from D i and to 0 otherwise.

This produces a network which applies to each input image the correct controllers, classifying it within its own domain.

We Increasing α shifts the network away from the base representation and towards learned tasks -gradually lowering performance on the base task (diamonds) and improving on the learned ones (full circles).
...
The control parameter α can be cast as a real-valued vector, allowing a smooth transition between representations of different tasks.

An example of the effect of such a smooth transition can be seen in FIG2 where α is used to linearly interpolate between the representation of differently learned tasks, allowing one to smoothly control transitions between different behaviors.
