...
An operator sequence π = o 1 , . . . , o k is applicable in s if there exist states s 0 , · · · , s k such that (i) s 0 = s, and (ii) for each 1 ≤ i ≤ k, o i is applicable in s i−1 and s i = s i−1 o i .

We refer to the state s k by s π and call it the end state of π.

An operator sequence π is a plan for a classical planning problem if it is applicable in s I and G ⊆ s I π .

The cost of a plan π is given by C(π) = o∈π C(o); the goal of optimal classical planning is to find a plan with minimal cost.

We refer to a pair of variable v and its value ϑ ∈ dom(v) as a fact and denote it by v, ϑ .

We sometimes abuse notation and treat partial assignments as sets of facts.

An oversubscription planning (OSP) problem is given by Π OSP = V , O, s I , C , u, B , where V , O, s I , and C are as in classical planning, u : ( v, ϑ ) → R 0+ is a non-negative valued utility function over variable assignments (facts), and B is a cost bound for the plan, imposing the additional requirement that only plans π such that C(π) ≤ B are valid.

The utility of a plan π is given by v,ϑ ∈s I π u( v, ϑ ); the objective of OSP problems is to find valid plans with maximal utility.

A multiple cost function (MCF) problem is given by Π MCF = V, O, s I , G, C 0 , C , where V , O, s I , and C 0 are as in classical planning, C 0 is the primary cost function, and C = { C i , B i | 1 ≤ i ≤ n} is a set of secondary cost functions C i : O → R 0+ , and bounds, both non-negative.

Valid plans for MCF planning problems fulfill the condition C i (π) ≤ B i for all secondary cost functions, and optimal plans for MCF planning have minimal primary cost C 0 (π).
...
In the reformulated problem, only the o v,ϑ actions for which ϑ is not the maximum utility value of v have positive primary costs.
...
The primary cost of a plan π for the reformulated problem is then given by v∈V u max (v) − f ∈s π u(f ).Note that this compilation assumes that utilities are defined for single facts.

The more general case, in which utilities are instead defined for logical formulae ϕ, can be handled as in the soft goals compilation by introducing a new variable v ϕ , and two actions that achieve its goal value with cost 0 and precondition ϕ, and cost u(ϕ) and precondition ∅, respectively BID3 ].
...
Second, a new precondition fact that is deleted by the o v,ϑ actions can be added to the original domain actions to ensure that o v,ϑ actions happen only at the end of the plan and are not interleaved with the original domain actions.
...
Definition 2 (Optimal bound-sensitive heuristic) Given an MCF planning problem Π MCF = V, O, s I , G, C 0 , C , the optimal bound-sensitive heuristic h * (s, b) for a state s and budget vector b is given by the minimal primary cost C 0 (π) of a plan π for s such that C i (π) ≤ b i for i = 1, . . .
...
The distance C i between any two abstract states s, s then represents a non-overestimate of the secondary cost of reaching s from s. A bound-sensitive heuristic value for a state s can be computed as the minimum C 0 cost of a path π from s to an abstract goal state s g whose C i cost in the abstract state space does not exceed B i , for any i. The C 0 cost of such such a path can be computed with a modified version of Dijkstra's algorithm that stores secondary cost information for each node and discards nodes for which C i > B i for any i.
...
Since the compilation introduces intermediate states at which some but not all of the o v,ϑ have been applied, we use a further optimization that avoids generating these nodes and applies all of the o v,ϑ actions in a single step, reducing the state space to that of the original OSP task.
