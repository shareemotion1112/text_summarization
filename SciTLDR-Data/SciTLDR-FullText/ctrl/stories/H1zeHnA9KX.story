...
A formal language is a set of strings over a finite alphabet Σ of input symbols.

A Deterministic Finite Automaton (DFA) is a tuple A = (Q, Σ, δ, q 0 , F ) where Q is a finite set of states, Σ is a finite alphabet, δ : Q × Σ → Q is a deterministic transition function, q 0 ∈ Q is a starting state and F ⊂ Q is a set of accepting states.

A reads strings over Σ symbol by symbol, starting from the state q 0 and making state transitions, defined by δ, at each step.
...
A Nondeterministic Finite Automaton (NFA) is similar to a DFA, except that the deterministic transition function δ is now a non-deterministic transition relation δ N F A .

This means that for a state q in the NFA and a ∈ Σ, we have that δ N F A (q, a) is now a subset of NFA states.

For a given regular language L we denote by A n L a Nondeterministic Finite Automaton (NFA) with n states that recognizes a superset of the language L. An abstraction map is a map α : DISPLAYFORM0 DISPLAYFORM1 We define the coarseness of an abstraction A n L , as the number of applications of α on the MDFA required to arrive at A n L .

Intuitively, repeated applications of α create NFAs that accept supersets of the language L recognized by the MDFA, and can hence be seen as coarse-grained versions of the original MDFA.

The coarsest NFA, given by A DISPLAYFORM2 , is a NFA with only one accepting node and it accepts all strings on the alphabet Σ.Given a regular language L, we define R L to be a RNN that is trained to recognize the language L, with a certain threshold accuracy.

Each RNN R L will have a corresponding set of hidden states denoted by H. More details about the RNN are provided in §4.1.

Note that a RNN can also be viewed as a transition system with 5-tuple R = (H, Σ, δ R , h 0 , F R ), where H is a set of possible 'hidden' states (typically H ⊂ R K ), δ R is the transition function of the trained RNN, h 0 is the initial state of the RNN, and F R is the set of accepting RNN states.
...
We can define an abstraction decoding functionf : DISPLAYFORM5 , that is the composition of f with α |n| .

The function α |n| is the function obtained by taking n compositions of α with itself.

Given a dataset of input strings D ⊂ Σ * , we can define the decoding accuracy of a mapf for an abstraction DISPLAYFORM6 where 1(C) is the boolean indicator function that evaluates to 1 if condition C is true and to 0 otherwise, h t+1 = δ R (h t , a t ) and q t+1 = δ 0 (q t , a t ).
...
We then train R L with D on the language recognition task: given an input string x ∈ Σ * , is x ∈ L?
...
We note that this algorithm does not explicitly take into consideration the transitions between states which are essential to evaluating φf (R L , A n L ).
...
Once an abstraction α has been found, we can evaluate whether the learned abstraction decoderf is of high accuracy, and whether the α found is of low coarseness.

Results showing the relationship between high decoding accuracy ρf (R L , A n L ) as a function of coarseness is presented in FIG6 conditioned on the number of nodes in the original MDFA.

As stated in §4.2, as M increases, ρf (R L , A n L ) decreases on the MDFA (i.e. n = 0).
...
Alongside the decoding accuracy, we also present transitional accuracy φf (R L , A n L ) as a function of coarseness FIG6 .
...
4.5 INTERPRETING THE RNN HIDDEN STATE SPACE WITH RESPECT TO THE MINIMAL DFA With an established high accuracyf with low coarseness α reveals a unique interpretation of H with respect to A 0 L .

Using α and f to relate the two, we uncover an interpretation of how R organizes H with respect to A n L ∀ n ∈ [M ].
...
The answer can be seen in the corresponding accuracies ρf (R L , A n L ) in FIG10 .
...
We typically set a threshold for ρf (R L , A n L ) and select the minimum n required to achieve the threshold accuracy.
