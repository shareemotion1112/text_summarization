...
Let π ∈ Π denote a permutation of the items, where each π j ∈ {1, . . .

, n} denotes the index of the item in position j.

Our goal is to predict the output ranking π given the input items x. For instance, given a specific user query, we might want to return an ordered set of music recommendations from a set of candidates that maximizes some measure of user engagement (e.g., number of tracks played).
...
In our case, the conditional p(π j |π <j , x) ∈ ∆ n (a point in the n-dimensional simplex) models the probability of any item being placed at the j'th position in the ranking given the items already placed at previous positions.
...
We employ the pointer-network architecture of BID38 to model the conditional p(π j |π <j , x).
...
Once the next item π j is selected, typically greedily or by sampling (see below), its embedding x πj is fed as input to the next decoder step.

The input of the first decoder step is a learned d-dimensional vector, denoted as go in FIG0 .

Importantly, p θ (π|x) is differentiable for ant fixed permutation π which allows gradient-based learning (see Section 3).
...
We also consider a computationally cheaper single-step decoder with linear cost O(n), which outputs a single vector p 1 , from which we obtain π by sorting the values (similarly to pointwise ranking).
...
Our goal is to learn the parameters θ of p θ (π j |π <j , x) (Eq. (2)) such that permutations π corresponding to "good" rankings are assigned high probabilities.

Various performance measures R(π, y) can be used to evaluate the quality of a permutation π given the labels y, for example, mean average precision (MAP), precision at k, or normalized discounted cumulative gain at k (NDCG@k).
...
One potential approach, which has been applied successfully in related tasks BID3 BID48 , is to use reinforcement learning (RL) to directly optimize for the ranking measure R(π, y).

In this setup, the objective is to maximize the expected ranking metric obtained by sequences sampled from our model: E π∼p θ (.|x) [R(π, y)].

One can use policy gradients and stochastic gradient ascent to optimize θ.

The gradient is formulated using the popular REINFORCE update BID42 and can be approximated via Monte-Carlo sampling as follows: DISPLAYFORM0 where k indexes ranking instances in a batch of size B, π k are permutations drawn from the model p θ and b(x) denotes a baseline function that estimates the expected rewards to reduce the variance of the gradients.
...
Two natural, simple choices for are cross-entropy loss and hinge loss: DISPLAYFORM0 hinge (s, y) = max{0, 1 − min i:yi=1 DISPLAYFORM1 whereŷ i = y i / j y j , and p i is a softmax of s, similar to Eq. (2).
...
In particular, for a fixed permutation π, define the sequence loss: DISPLAYFORM2 where S = {s j } n j=1 , and π<j (s j , y) depends only on the indices in s j and y which are not in the prefix permutation π <j = (π 1 , . . .

, π j−1 ) (see Eq. FORMULA4 ).
...
In this case, the permutation π is not fixed, but rather depends on the scores S. Specifically, we consider two policies for producing a permutation during training, sampling and greedy decoding, and introduce their corresponding losses.

The greedy policy consists of selecting the item that maximizes p θ (·|π <j , x) at every time step j.

The resulting permutation π * then satisfies π * j = argmax i p θ (π j = i|π * <j ) and our loss becomes L π * .

The greedy policy loss is not continuous everywhere since a small change in the scores s may result in a jump between permutations, and therefore L π .
...
Sampling policy The sampling policy consists of drawing each π j from p θ (·|π <j , x).

The corresponding loss E[L] = π p θ (π)L π (θ) is differentiable everywhere since both p θ (π) and L π (θ) are differentiable for any permutation π (See appendix for a direct derivation of E[L] as a function of S).

In this case, the gradient is formulated as: DISPLAYFORM0 which can be approximated by: DISPLAYFORM1 where b(x k ) is a baseline that approximates L π k (θ).
...
To improve generalization, we regularize the model by using dropout with probability of dropping p dropout = 0.1 and L2 regularization with a penalty coefficient λ = 0.0003.
...
We generate an item ranking using the base model, which is then used to generate training data by simulating a user "cascade" model: a user observes each item with decaying probability 1/i η , where i is the base rank of the item and η is a parameter of the generative model.
...
We use η = 0 (no decay, since clicks are sparse anyway due to the diversity term) and q = 0.5.
...
TAB2 , we compare the different training variants outlined in Section 3, namely cross entropy with the greedy or sampling policy, a smooth hinge loss with γ = 1.0, and REINFORCE.
...
For this model we use q = 0.5, and η = 0.3 for Web30k, η = 0.1 for Yahoo, to keep the proportion of positive labels similar.
