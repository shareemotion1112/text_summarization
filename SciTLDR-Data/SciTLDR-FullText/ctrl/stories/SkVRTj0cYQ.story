...
A randomized mechanism M : D → R, with domain D and range R satisfies ( , δ)-differential privacy, if for any two adjacent inputs d, d ∈ D and for any subset of outputs S ⊆ R it holds that DISPLAYFORM0 In this definition, δ accounts for the probability that plain -differential privacy is broken.
...
This sensitivity is defined as the maximum of the absolute distance DISPLAYFORM1 In the following we assume that σ and are fixed and evaluate an inquiry to the GM about a single approximation of f (d).

We can then bound the probability that -dp is broken according to: δ ≤ 5 4 exp(−(σ ) 2 /2) (Theorem 3.22 in BID5 ).

It should be noted that δ is accumulative and grows if the consecutive inquiries to the GM.

Therefore, to protect privacy, an accountant keeps track of δ.

Once a certain threshold for δ is reached, the GM shall not answer any new inquires.
...
For being fixed, a privacy accountant keeps track of δ and stops training once a threshold is reached.
...
DISPLAYFORM1 Gaussian mechanism approximating sum of updatesWhen factorizing 1/m t into the Gaussian mechanism, we notice that the average's distortion is governed by the noise variance S 2 σ 2 /m.
...
Thus, σ and m also define the privacy loss incurred when the randomized mechanism provides an average approximation.
...
Each time the curator allocates a new model, the accountant evaluates δ given , σ and m. Training shall be Step 1: At each communication round t, m t out of total K clients are sampled uniformly at random.
...
stopped once δ reaches a certain threshold, i.e. the likelihood, that a clients contribution is revealed gets too high.

The choice of a threshold for δ depends on the total amount of clients K. To ascertain that privacy for many is not preserved at the expense of revealing total information about a few, we have to ensure that δ 1 K , refer to Dwork & Roth (2014) chapter 2.3 for more details.
...
However, the information leakage through the median is small (Future work will contain such a privacy measure).Choosing σ and m: for fixed S, the ratio r = σ 2 /m governs distortion and privacy loss.

It follows that the higher σ and the lower m, the higher the privacy loss.

The privacy accountant tells us that for fixed r = σ 2 /m, i.e. for the same level of distortion, privacy loss is smaller for σ and m both being small.

An upper bound on the distortion rate r and a lower bound on the number of sub-sampled clientsm would thus lead to a choice of σ.
...
K is the number of participating clients; B is the local mini-batch size, E the number of local epochs, η is the learning rate, {σ} T t=0is the set of variances for the GM.

{m t } T t=0 determines the number of participating clients at each communication round.

defines the dp we aim for.

Q is the threshold for δ, the probability that -dp is broken.

T is the number of communication rounds after which δ surpasses Q. B is a set holding client's data sliced into batches of size B 1: procedure SERVER EXECUTION 2:Initialize: w 0 , Accountant( , K) initialize weights and the priv.

accountant 3:for each round t = 1, 2, ... do

δ ← Accountant(m t , σ t ) Accountant returns priv.
...
For all three scenarios K ∈ {100, 1000, 10000} we performed a cross-validation grid search on the following parameters:• Number of batches per client B• Epochs to run on each client E• Number of clients participating in each round m• The GM parameter σ In accordance to BID0 we fixed to the value of 8.

During training we keep track of privacy loss using the privacy accountant.

Training is stopped once δ reaches e − 3, e − 5, e − 6 for 100, 1000 and 10000 clients, respectively.

In addition, we also analyze the between clients variance over the course of training.

In the cross validation grid search we look for those models that reach the highest accuracy while staying below the respective bound on δ.
...
For differentially private federated optimization, dots at the end of accuracy curves indicate that the δ-threshold was reached and training therefore stopped.
...
When looking at a single early communication round, lowering both m t and σ t in a fashion such that σ 2 t /m t stays constant, has almost no impact on the accuracy gain during that round.
