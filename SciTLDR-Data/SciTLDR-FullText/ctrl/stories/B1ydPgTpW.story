...
The network also uses batch normalization, which has been shown to speed up convergence BID22 .Each recurrent layer is implemented as follows: BatchNorm transformation, andx is the within-mini-batch-standardized version of x. 2 W , U , γ and β are weights learnt by the network through training.
...
α, δ and ν were estimated by linear regression on the training data.
...
The estimated probability distribution of realized price p for a given predicted pricep is DISPLAYFORM0 where [z 1 (p), ..., z 24 (p), µ 1 (p), ..., µ 24 (p), σ 1 (p), ..., σ 24 (p)] is the output vector from a neural network with a single fully-connected layer of 256 neurons with a single inputp.
