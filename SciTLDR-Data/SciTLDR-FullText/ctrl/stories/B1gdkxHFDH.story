...
where Σ ∈ S d×d ++ is a covariance matrix for the ML task at hand.
...
To achieve this we set Σ = I − P ran(A) , i.e. the orthogonal complement projector of ran(A).
...
2) where δ > 0 is a small tolerance for discrepancies in the performance of the ML model on comparable inputs.
...
The function c λ is called the c-transform of .
...
Require: starting pointλ 1 , step sizes α t > 0 1: repeat

It is known that the optimal point of equation 2.1 is the discrete measure

We call T λ an unfair map because it reveals unfairness in the AI system by mapping samples in the audit data to comparable areas of the sample space that the system performs poorly on.

We note that T λ may map samples in the audit data to areas of the sample space that are not represented in the audit data, thereby revealing disparate treatment in the AI system not visible from the audit data alone.

We emphasize that T λ more than reveals disparate treatment in the AI system; it localizes the unfairness to certain areas of the sample space.
...
Comparing equation 2.5 and equation 2.6, we see that the two implications are similar, with in equation 2.6 playing the role of d y in equation 2.5 and 2δ in equation 2.6 playing the part of L equation 2.5.
...
where c λ is defined in equation 2.3.
...
To keep things simple, we assume the hypothesis class is parametrized by θ ∈ Θ ⊂ R d and replace the minimization with respect to H by minimization with respect to θ.
...
Require: starting pointθ 1 , step sizes α t , β t > 0 1: repeat 2:

sample mini-batch (x 1 , y 1 ), . . . , (x B , y B ) ∼ P n 3:

Related work Our approach to fair training is an instance of distributionally robust optimization (DRO), which minimize objectives of the form sup P ∈U E P (Z, θ) , where U is a (data dependent) uncertainty set of probability distributions.
...
Most similar to our work is Hashimoto et al. (2018): they show that DRO with a χ 2 -neighborhood of the training data prevents representation disparity, i.e. minority groups tend to suffer higher losses because the training algorithm ignores them.
...
the feature space X is bounded: D max{diam(X ), diam * (X )} < ∞; (A2) the functions in the loss class L = { (·, θ) : θ ∈ Θ} are non-negative and bounded: 0 ≤ (z, θ) ≤ M for all z ∈ Z and θ ∈ Θ, and L-Lipschitz with respect to d x :
...
λmin(Σ * ) , We see that the error in the transportation cost function vanishes in the large-sample limit as long as Σ is a consistent estimator of Σ * .
...
We see that the discrepancy in the transportation cost function may affect the rate at which the uniform convergence error vanishes: it affects the rate if δ c is ω P (
...
Under the assumptions A1-A3, as long as there isθ ∈ Θ such that sup P :W * (P,P * )≤ E P (Z,θ) ≤ δ * (3.4)

for some δ * > 0,θ ∈ arg min θ∈Θ sup P :W (P,Pn)≤ E P (Z, h) satisfies

where δ n is the uniform convergence error equation 3.1.

Another consequence of uniform convergence is equation 3.3 is close to its empirical counterpart sup P :W (P,Pn)≤ E P (Z, θ) − E Pn (Z, θ) .
...
be the set of features x i ∈ R D of the data except the coordinate for gender is zeroed and where x gi indicates the gender of individual i. For γ > 0,

) + γ w 2 , i.e. w g is the learned hyperplane that classifies gender given by logistic regression.
...
A.1 PROOF OF PROPOSITION 3.1 By the duality result of , for any > 0, sup P :W * (P,P * )≤ E P (Z, θ) − sup

where λ n ∈ arg min λ≥0 λ + E Pn c λ (Z, θ) .
...
This implies has bounded differences, so δ n concentrates sharply around its expectation.
...
It is enough to show sup P :W * (P,P * )≤ E P (Z,θ) ≤ δ * + 2δ n because the loss function is non-negative.

We have

Proof of Proposition 3.3.

The loss function is bounded, so it is possible to bound E P * (Z, θ) − E Pn (Z, θ) by standard uniform convergence results on bounded loss classes.
...
where ϕ(x) : X → R d is a known feature map and Σ ∈ S d×d + is a covariance matrix, from groups of comparable samples.
...
Recall our goal is to obtain Σ so that equation B.2 is small whenever v 1 ≈ v 2 .

One possible choice of Σ is the projection matrix onto the orthogonal complement of ran(A), which we denote by P ran(A) .
...
Let C be a set of classes, A be a binary protected attribute and Y,Ŷ ∈ C be the true class label and the predicted class label.

Then for a ∈ {0, 1} and c ∈ C define TPR a,c = P(Ŷ = c|A = a, Y = c); G where C is composed of the two classes that correspond to whether someone made at least $50k, R refers to race, and G refers to gender.
