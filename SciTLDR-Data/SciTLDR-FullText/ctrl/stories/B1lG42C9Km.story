...
A history of these variables over time is termed a trajectory, τ = {s t ,a t ,r t } T t=0 .

We consider a partially observable setting in which each agent k can only view a portion of the true state, s k t .

Each agent seeks to maximize its own total expected future reward, DISPLAYFORM0 , where γ is a discount factor.

A distributed asynchronous advantage actor-critic approach (A3C) BID14 ) is used to train each agent's independent policy π k .

The policy is learned via REINFORCE with baseline BID33 .

Architecturally, our agents consist of a convolutional layer, fully connected layers, a Long Short Term Memory (LSTM) network BID7 , and linear layers which output π k and the value function V π k (s).

We will refer to the internal LSTM state of agent k at timestep t as u k t .

Social influence intrinsic motivation modifies an agent's reward function so that it becomes R k =αE k +βI k , where E k is the extrinsic or environmental reward, and I k is the causal influence reward.
...
We can also forego the do operator, noting that p(a B t |do(ã A t ),z t )≡p(a B t |ã A t ,z t ) in this case, because z satisfies the back-door criterion BID18 .

Now, consider averaging over several counterfactualsã A t .
...
By sampling N independent trajectories τ n from the environment, where A's actions a DISPLAYFORM1 Thus, in expectation, the social influence reward is the MI between agents' actions.
...
Computing the causal influence reward as introduced in Section 2.1 requires knowing the probability of B's next action given a counterfactual, p(a B t |ã A t ,s B t ), which we previously solved by using a centralised controller that could access other agent's policy networks.
...
In this case, the ablated baseline is a model that has the same structure as in FIG3 , but in which the communication policy π c is trained only with environmental reward.
...
In fact, in the case of Cleanup, we found that α = 0 in the optimal hyperparameter settings, meaning that it was most effective to train the communication head with zero extrinsic or environmental reward (see TAB3 in the Appendix).
...
We found that agents who are more often influenced tend to achieve higher task reward in both Cleanup, ρ = .67, p<0.001, and Harvest, ρ = .34, p<0.001.
...
Thus, the reward function for agent k is R k =E k +η N i=1,i =k E i .

We conducted the same hyperparameter search over the parameters mentioned in Section 6.3.1 varying the weight placed on the collective reward, η ∈[0,2].As expected, we find that agents trained to optimize for collective reward attain higher collective reward in both Cleanup and Harvest, as is shown in FIG7 .

In both games, the optimal value for η = 0.85.
