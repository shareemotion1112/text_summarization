...
After initializing its RNN hidden layer to zeros, the teacher at each timestep evaluatesŷ for all possible symbols and selects the one that maximizesŷ.

The RNN hidden vector induced by the chosen symbol is used in the next timestep.

This is repeated untilŷ becomes bigger than the predefined threshold, or the maximum message length is reached (see Appendix D for algorithm).
...
Although in this work we deterministically selected a character in each timestep, one can, for example, sample characters proportionally toŷ and still use gradient descent for training the agents.
...
Because the teacher stops generating symbols as soon asŷ reaches the threshold, it does not waste any more effort trying to perfect the message.
...
For example, red is always described by either the suffix c or suffix e, but magenta is described by the suffix bb, bd, and sometimes b or bc.ā used for gray objects represents deletion of the prefix a. Note that removing prefixes beyond their length causes the pattern to break.

For example, gray box, gray sphere and gray cylinder use the sameāāa to express the color, but gray capsule and gray ellipsoid use irregular suffixes.
...
We can observe that there is clearly a structure in the communication, although some messages show somewhat weaker patterns compared to when the agents were trained with all object types ( even when we consider the effects ofb andē.
...
It is also notable that the objects described withoutb orē show better performance in general.
...
This, however, results from the grammar (i.e. structure) being not as straightforward as TAB2 , especially with short messages (i.e. frequent use ofb andē).
