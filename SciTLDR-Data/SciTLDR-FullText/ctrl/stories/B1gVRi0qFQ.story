...
An MDP is a tuple S, A, Φ, P, R , where S is the set of states, A is the set of actions, Φ ⊂ S×A is the state-action space (the set of available actions for each state), P (s, a, s ) is the transition function and R(s, a) is the reward function.
...
An MDP homomorphism from M = S, A, Φ, P, R to M = S , A , Φ , P , R is a tuple of surjections f, {g s : s ∈ S} with h(s, a) = (f (s), g s (a)), where f : S → S and g s : DISPLAYFORM0 Computing the optimal state-action value function in the minimized MDP requires fewer computations, but does it help us act in the underlying MDP?
...
Let M = S , A , Φ , P , R be the homomorphic image of the MDP M = S, A, Φ, P, R under the MDP homomorphism h(s, a) DISPLAYFORM1 During MDP minimization, Φ is partitioned and the partition subsequently induces the abstract MDP.

Let B = {b 1 , b 2 , ..., b N } be the partition over Φ. We call B the state-action partition, with b i being a block of state-action pairs.

The state-action partition induces the quotient MDP:Definition 2.

Given a reward respecting SSP partition B of an MDP M = S, A, Φ, P, R , the quotient MDP M/B is the MDP S , A , Φ , P , R , where DISPLAYFORM2 We omit the definition of the reward respecting SSP partition for brevity, please find it in BID5 .
...
An option I, π, β is a temporally extended action; it can be executed from the set of states I, the individual primitive actions are selected with a policy π and each state it encounters has a probability of terminating the option, which is given by β : DISPLAYFORM3
...
Each option is a tuple I, π, β : the set of starting state I contains all starting states of the MDP for the new task, π uses the abstract graph to select actions that lead to a particular state block in the abstract MDP and the terminal condition β terminates the option when the selected state block is reached.

The agent learns the q-values of the options with a Monte Carlo update BID9 ) with a fixed α (the learning rate)-the agent prefers options that make it reach the goal the fastest upon being executed.
