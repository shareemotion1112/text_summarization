...
In our experiments, f trans is implemented by a gated function following PF-GRU (Ma et al., 2019) ) for all i = 1 : K and normalize using η.
...
, we compute the policy p(a | b t ) with a policy network π(b t ).
...
where η is a normalization factor and p(o t | s Resampling.
...
The soft-resampling hyperparameter α is set to be 0.9 for Mountain Hike and 0.5 for the rest of the domains.
...
We follow the default setting provided by and set λ E = 0.1.

The rest of the hyperparameters, including learning rate, gradient clipping value and α in soft-resampling are tuned according to the BeamRider and directly applied to all domains due to the highly expensive experiment setups.

The learning rate for all the networks are searched among the following values: (3 × 10 −5 , 5 × 10 −5 , 1 × 10 −4 , 2 × 10 −4 , 3 × 10 −4 ); the gradient clipping value are searched among {0.5, 1.0}; the soft-resampling α is searched among {0.5, 0.9}. The best performing learning rates were 1 −4 for DPFRL and GRU, and 2 −4 for DVRL; the gradient clipping value for all models was 0.5; the soft-resampling α is set to be 0.9 for Mountain Hike and 0.5 for Atari games.
