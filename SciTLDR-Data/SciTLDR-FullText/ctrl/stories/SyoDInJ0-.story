...
First, let us define meta-time as the time scale for AS: at one meta-time τ corresponds a meta-algorithm decision, i.e. the choice of an algorithm and the generation of a full episode controlled with the policy determined by the chosen algorithm.
...
A trajectory ε τ ∈ E collected at meta-time τ is formalised as a sequence of (observation, action, reward) triplets: ε τ = o τ (t), a τ (t), r τ (t) t∈ 1,|ετ | ∈ E, where |ε τ | is the length of trajectory ε τ .

The objective is, given a discount factor 0 ≤ γ < 1, to generate trajectories with high discounted cumulative reward, also called return, and noted µ(ε τ ) = |ετ | t=1 γ t−1 r τ (t).

Since γ < 1 and R is bounded, the return is also bounded.

The trajectory set at meta-time T is denoted by D T = {ε τ } τ ∈ 1,T ∈ E T .

A sub-trajectory of ε τ until RL-time t is called the history at RL-time t and written ε τ (t) with t ≤ |ε τ |.

The history records what happened in episode ε τ until RL-time t: ε τ (t) = o τ (t ), a τ (t ), r τ (t ) t ∈ 1,t ∈ E.The goal of each RL algorithm α is to find a policy π * : E → A which yields optimal expected returns.

Such an algorithm α is viewed as a black box that takes as an input a trajectory set D ∈ E + , where E + is the ensemble of trajectory sets of undetermined size: DISPLAYFORM0 , and that outputs a policy π α D .

Consequently, a RL algorithm is formalised as follows: α : DISPLAYFORM1 Such a high level definition of the RL algorithms allows to share trajectories between algorithms: a trajectory as a sequence of observations, actions, and rewards can be interpreted by any algorithm in its own decision process and state representation.
...
Pseudo-code 1: Online RL AS setting DISPLAYFORM0 Generate trajectory ε τ with policy π DISPLAYFORM1 The online learning approach is tackled in this article: different algorithms are experienced and evaluated during the data collection.
...
The online AS problem for off-policy RL is novel and we define it as follows: DISPLAYFORM2 is the current trajectory set;• P = {α k } k∈ 1,K is the portfolio of off-policy RL algorithms;• µ : E → R is the objective function, generally set as the RL return.

Pseudo-code 1 formalises the online RL AS setting.

A meta-algorithm is defined as a function from a trajectory set to the selection of an algorithm: σ : E + → P. The meta-algorithm is queried at each meta-time τ = |D τ −1 |+1, with input D τ −1 , and it ouputs algorithm σ (D τ −1 ) = σ(τ ) ∈ P controlling with its policy π σ(τ ) Dτ−1 the generation of the trajectory ε τ in the stochastic environment.

Figure 2 illustrates the algorithm with a diagram flow.

The final goal is to optimise the cumulative expected return.

It is the expectation of the sum of rewards obtained after a run of T trajectories: DISPLAYFORM3 DISPLAYFORM4 Return µ(ε τ ): DISPLAYFORM5 Figure 2: Algorithm selection for reinforcement learning flow diagram expectations.

The outside expectation E σ assumes the meta-algorithm σ fixed and averages over the trajectory set generation and the corresponding algorithms policies.

The inside expectation Eµ assumes the policy fixed and averages over its possible trajectories in the stochastic environment.

Nota bene: there are three levels of decision: meta-algorithm σ selects algorithm α that computes policy π that is in control.
...
First, the optimal expected return Eµ * ∞ is defined as the highest expected return achievable by a policy of an algorithm in portfolio P. Second, for every algorithm α in the portfolio, let us define σ α as its canonical metaalgorithm, i.e. the meta-algorithm that always selects algorithm α: ∀τ , σ α (τ ) = α.
...
The bandit meta-algorithm selects the algorithm controlling the next trajectory ε and the objective function µ(ε) constitutes the reward of the bandit.
...
As a consequence and since the returns are bounded, at each new epoch, the problem can rigorously be cast into an independent stochastic K-armed bandit Ξ, with K = |P|.

Data: D 0 , P, µ: the online RL AS setting DISPLAYFORM0 n kmax + 1 n kmax ← n kmax + 1 and n ← n + 1 end endThe ESBAS meta-algorithm is formally sketched in Pseudo-code 2 embedding UCB1 Auer et al. (2002a) as the stochastic Karmed bandit Ξ. The meta-algorithm takes as an input the set of algorithms in the portfolio.

Meta-time scale is fragmented into epochs of exponential size.

The β th epoch lasts 2 β metatime steps, so that, at meta-time τ = 2 β , epoch β starts.
...
At the beginning of each epoch, a new Ξ instance is reset and run.

During the whole epoch, Ξ selects at each meta-time step the algorithm in control of the next trajectory.

Theoretical analysis -ESBAS intends to minimise the regret for not choosing the algorithm yielding the maximal return at a given meta-time τ .

It is short-sighted: it does not intend to optimise the algorithms learning.

We define the short-sighted pseudo-regret as follows: DISPLAYFORM1 The short-sighted pseudo-regret depends on the gaps ∆ α β : the difference of expected return between the best algorithm during epoch β and algorithm α.

The smallest non null gap at epoch β is noted ∆ † β .

We write its limit when β tends to infinity with ∆ † ∞ .
...
They also provide upper bounds on the ESBAS short-sighted pseudo-regret as a function of the order of magnitude of the gap ∆ † β .
...
In particular if ∆ † β tends to 0, the algorithm selection might prove to be difficult, depending on the order of magnitude of it tending to 0.
...
The meta-time necessary to distinguish through evaluation arms that are ∆ † β apart takes Θ(1/∆ †2 β ) meta-time steps.
...
However, the budget, i.e. the length of epoch β starting at meta-time T = 2 β , equals DISPLAYFORM2 can therefore be considered as the structural limit of distinguishability between the algorithms.
...
At each dialogue, for each option η, players have a private uniformly drawn cost ν Table 2 of the supplementary material.
...
The stochastic bandit is now trained on a sliding window containing the last τ /2 selections.
...
The objective function µ used to optimise the stochastic bandit Ψ is no longer the RL return, but the time spent to collect all the fruits (200 in case of it did not).
...
The reward function is corrupted with a strong Gaussian white noise of variance ζ 2 = 1.

The portfolio is composed of 4 Q-learning algorithms varying from each other by their learning rates: {0.001, 0.01, 0.1, 0.5}. They all have the same linearly annealing τ -greedy exploration.
...
Normal distribution of centre x and variance v 2 Section C.1.1 REFINSIST REFPROP(η), with η being the last proposed option Section C.1.1 REFNEWPROP REFPROP(η), with η being the best option that has not been proposed yet Section C.1.1 ACCEPT ACCEPT(η), with η being the last understood option proposition Section C. DISPLAYFORM0 Non-learning algorithm with average performance µ Section C.1.2 ζ Number of noisy features added to the feature set Section C.1.2
...
The absolute pseudo-regret ρ σ abs (T ) compares the metaalgorithm's expected return with the optimal expected return: DISPLAYFORM0
...
It is verified for any meta-algorithm σ.
...
Its proof is rather complex for such an intuitive result because, in order to control all the possible outcomes, one needs to translate the selections of algorithm α with meta-algorithm σ into the canonical meta-algorithm σ α 's view.

ESBAS intends to minimise the regret for not choosing the best algorithm at a given meta-time τ .

It is short-sighted: it does not intend to optimise the algorithms learning.

Definition 2 (Short-sighted pseudo-regret).

The short-sighted pseudo-regret ρ σ ss (T ) is the difference between the immediate best expected return algorithm and the one selected: DISPLAYFORM0 Theorem 2 (ESBAS short-sighted pseudo-regret).

If the stochastic multi-armed bandit Ξ guarantees a regret of order of magnitude O(log(T )/∆ † β ), then: DISPLAYFORM1 Theorem 2 expresses in order of magnitude an upper bound for the short-sighted pseudo-regret of ESBAS.

But first, let define the gaps: DISPLAYFORM2 .

It is the difference of expected return between the best algorithm during epoch β and algorithm α.

The smallest non null gap at epoch β is noted: DISPLAYFORM3 if there is no non-null gap, the regret is null.

Several upper bounds in order of magnitude on ρ ss (T ) can be easily deduced from Theorem 2, depending on an order of magnitude of ∆ † β .
...
For instance, a poor deterministic algorithm might be optimal at meta-time τ but yield no new information, implying the same situation at meta-time τ + 1, and so on.
...
Under assumption 3, if the stochastic multi-armed bandit Ξ guarantees that the best arm has been selected in the T first episodes at least T /K times, with high probability 1 − δ T , with δ T ∈ O(1/T ), then: DISPLAYFORM1 where meta-algorithm σ * selects exclusively algorithm α * = argmin α∈P ρ σ α abs (T ).

Successive and Median Elimination (Even-Dar et al., 2002) and Upper Confidence Bound BID2 under some conditions BID1 are examples of appropriate Ξ satisfying both conditions stated in Theorems 2 and 3.
...
At each dialogue, for each option η, players have a private uniformly drawn cost ν p η ∼ U[0, 1] to agree on it.

Each player is considered fully empathetic to the other one.

As a result, if the players come to an agreement, the system's immediate reward at the end of the dialogue is R ps (s f ) = 2 − ν ps η − ν pu η , where s f is the state reached by player p s at the end of the dialogue, and η is the agreed option; if the players fail to agree, the final immediate reward is R ps (s f ) = 0, and finally, if one player misunderstands and agrees on a wrong option, the system gets the cost of selecting option η without the reward of successfully reaching an agreement: R ps (s f ) = −ν ps η − ν pu η .

Players act each one in turn, starting randomly by one or the other.

They have four possible actions.

First, REFPROP(η): the player makes a proposition: option η.
...
Third, ACCEPT(η): the player accepts option η that was understood to be proposed by the other player.
...
DISPLAYFORM2 • n-ζ-{simple/fast/simple-2/fast-2}: Versions of previous algorithms with ζ additional features of noise, randomly drawn from the uniform distribution in [0, 1].• constant-µ: the algorithm follows a deterministic policy of average performance µ without exploration nor learning.

Those constant policies are generated with simple-2 learning from a predefined batch of limited size.

In all our experiments, ESBAS has been run with UCB parameter ξ = 1/4.
...
The performance figures plot the curves of algorithms individual performance σ α against the ESBAS portfolio control σ ESBAS in function of the epoch (the scale is therefore logarithmic in meta-time).

The performance is the average return of the reinforcement learning return: it equals γ | | R ps (s f ) in the negotiation game.
...
From Definition 1: DISPLAYFORM1 where sub α (D) is the subset of D with all the trajectories generated with algorithm α, where τ α i is the index of the i th trajectory generated with algorithm α, and where |S| is the cardinality of finite set S. By convention, let us state that Eµ DISPLAYFORM2 To conclude, let us prove by mathematical induction the following inequality: DISPLAYFORM3 is true by vacuity for i = 0: both left and right terms equal Eµ α ∅ .

Now let us assume the property true for i and prove it for i + 1: DISPLAYFORM4 .If |sub α (D σ T )|≥ i + 1, by applying mathematical induction assumption, then by applying Assumption 2 and finally by applying Assumption 1 recursively, we infer that: The mathematical induction proof is complete.

This result leads to the following inequalities: DISPLAYFORM5 DISPLAYFORM6 which leads directly to the result: DISPLAYFORM7 This proof may seem to the reader rather complex for such an intuitive and loose result but algorithm selection σ and the algorithms it selects may act tricky.

For instance selecting algorithm α only when the collected trajectory sets contains misleading examples (i.e. with worse expected return than with an empty trajectory set) implies that the following unintuitive inequality is always true: Eµ DISPLAYFORM8 .

In order to control all the possible outcomes, one needs to translate the selections of algorithm α into σ α 's view.

Theorem 2 (ESBAS short-sighted pseudo-regret).

If the stochastic multi-armed bandit Ξ guarantees a regret of order of magnitude O(log(T )/∆ † β ), then: DISPLAYFORM0 Proof.

By simplification of notation, Eµ DISPLAYFORM1 .
...
Under assumption 3, if the stochastic multi-armed bandit Ξ guarantees that the best arm has been selected in the T first episodes at least T /K times, with high probability 1 − δ T , with δ T ∈ O(1/T ), then: DISPLAYFORM0 where meta-algorithm σ * selects exclusively algorithm α * = argmin α∈P ρ σ α abs (T ).Proof.
...
is to evaluate the size of sub * (D τ −1 ).On the one side, Assumption 3 of fairness states that one algorithm learns as fast as any another over any history.

The asymptotically optimal algorithm(s) when τ → ∞ is(are) therefore the same one(s) whatever the the algorithm selection is.

On the other side, let 1 − δ τ denote the probability, that at time τ , the following inequality is true: DISPLAYFORM0 With probability δ τ , inequality 34 is not guaranteed and nothing can be inferred about Eµ * sub * (Dτ−1) , except it is bounded under by R min /(1 − γ).

Let E DISPLAYFORM1 Let consider E * (α, N ) the set of all sets D such that |sub α (D)|= N and such that last trajectory in D was generated by α.

Since ESBAS, with Ξ, a stochastic bandit with regret in O(log(T )/∆), guarantees that all algorithms will eventually be selected an infinity of times, we know that :(37) ∀α ∈ P, ∀N ∈ N, D ∈E + (α,N ) P(D|σ ESBAS ) = 1.By applying recursively Assumption 2, one demonstrates that: DISPLAYFORM2 D ∈E + (α,N ) DISPLAYFORM3 One also notices the following piece-wisely domination from applying recursively Assumption 1: DISPLAYFORM4 (1 − δ τ )Eµ * ∞ − D ∈E ; the meta-time spent on epoch β τ − 1 is equal to 2 βτ −1; the meta-time spent on epoch β is either below 2 βτ −1 , in which case, the meta-time spent on epoch β τ − 1 is higher than τ 3 , or the meta-time spent on epoch β is over 2 βτ −1 and therefore higher than τ 3 .

Thus, ESBAS is guaranteed to try the best algorithm α * at least τ /3K times with high probability 1 − δ τ and δ τ ∈ O(τ −1 ).

As a result: DISPLAYFORM5 with κ = κ 3 Eµ * ∞ − R min 1 − γ , which proves the theorem.
