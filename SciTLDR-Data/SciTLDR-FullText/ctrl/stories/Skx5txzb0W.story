...
More generally, Boo n N (µ, σ 2 ) can then be expressed as µ + σBoo n (N (0, 1)).
...
DISPLAYFORM1 Expanding the expression for the bivariate Gaussian distribution having marginal test performance with mean µ test , standard deviation σ test , and test-validation correlation ρ as in Appendix A.2 yields a convenient expression DISPLAYFORM2 which can again be used for parametric estimation.
...
For the Gaussian distribution, we can use the standard estimators of the parameters in Equation 6 to get a parametric estimator DISPLAYFORM2 where µ test , ρ and σ test are standard estimators of mean, correlation, and standard deviation respectively.
...
DISPLAYFORM0 where Φ is the c.d.f.
