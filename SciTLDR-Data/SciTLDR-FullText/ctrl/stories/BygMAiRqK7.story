...
Let Y andŶ ∶= G(X) represent observed (i.e. real) and generative (i.e. fake or synthetic) variables, respectively.
...
I.e.average sample likelihoods ≥ − (entropic GAN objective) + constants.(1.2)If (y,ŷ) = y −ŷ 2 , the optimal transport (OT) GAN simplifies to WGAN while if (y,ŷ) = y −ŷ 2 2 , the OT GAN simplifies to the quadratic GAN (or, W2GAN) BID3 ).
...
Let Y ∈ R d represent the real-data random variable with a probability density function f Y (y).GAN's goal is to find a generator function G ∶ R r → R d such thatŶ ∶= G(X) has a similar distribution to Y .
...
In practice, we observe m samples {y 1 , ..., y m } from Y and generate m ′ samples fromŶ , i.e., {ŷ 1 , ...,ŷ m ′ } wherê DISPLAYFORM0 We represent these empirical distributions by P Y and PŶ , respectively.

Note that the number of generative samples m ′ can be arbitrarily large.

GAN computes the optimal generator G * by minimizing a distance between the observed distribution P Y and the generative one PŶ .
...
The Shannon entropy function is defined as H(P Y,Ŷ ) ∶= −E log P Y,Ŷ .

The negative Shannon entropy is a common strongly-convex regularization term.

This leads to the following optimal transport GAN formulation with the entropy regularization, or for simplicity, the entropic GAN formulation: DISPLAYFORM3 where λ is the regularization parameter.
...
Theorem 1 Let the loss function be shift invariant, i.e., (y,ŷ) = h(y −ŷ).
...
Corollary 2 Let G * and P * Y,Ŷ (or, alternatively P * Y,X ) be optimal solutions of the entropic GAN equation 2.3.
...
In this case, optimal primal variables P * Y,Ŷ can be computed according to the following lemma BID27 :Lemma 1 Let D * 1 and D * 2 be the optimal discriminator functions for a given generator function G according to optimization 3.4.

Let DISPLAYFORM6 This lemma is important for our results since it provides an efficient way to compute the optimal coupling between real and generative variables (i.e. P * Y,Ŷ ) using the optimal generator (G * ) and discriminators (D * 1 and D * 2 ) of optimization 3.4.
...
Likelihood lower bounds of Theorem 1 and Corollary 2 hold even for those GANs as long as we obtain the optimal coupling P * Y,Ŷ in addition to the optimal generator G * from GAN's training.

Computation of optimal cou- pling P * Y,Ŷ from the dual formulation of OT GAN can be done when the loss function is quadratic BID3 .

In this case, the gradient of the optimal discriminator provides the optimal coupling between Y andŶ BID29 ) (see Lemma.

2 in Appendix C).For a general GAN architecture, however, the exact computation of optimal coupling P * Y,Ŷ may be difficult.
...
λ = 0.1 was used in all the experiments.
...
DISPLAYFORM2 Optimal coupling P * Y,Ŷ for the W2GAN (quadratic GAN BID3 ) can be computed using the gradient of the optimal discriminator BID29
...
Note that the objective of this optimization differs from that of the entropic GAN optimization 2.3 by a constant term λH(P Y ) + λH(PŶ ).

A sinkhorn distance function is then defined as BID4 : DISPLAYFORM1 W is called the Sinkhorn loss function.

Reference BID4 has shown that as λ → 0, DISPLAYFORM2 For a general λ, we have the following upper and lower bounds:Lemma 3 For a given λ > 0, we havē DISPLAYFORM3 (this can be seen by using an identity coupling as a feasible solution for optimization D.1) and similarly DISPLAYFORM4 Since H(P Y ) + H(PŶ ) is constant in our setup, optimizing the GAN with the Sinkhorn loss is equivalent to optimizing the entropic GAN.
