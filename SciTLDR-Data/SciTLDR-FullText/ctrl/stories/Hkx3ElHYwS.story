...
To construct outputĥ W,Q (x 0 ) of the quantized network, we proceed layer by layer.

We first quantize the weights in layers i = 1, . . .

, L asŵ i = Q w i (w i ), and also quantize the input by settingx 0 = Q a 0 (x 0 ).
...
Note that sinceŵ i andx i−1 are quantized,x i can be computed using integer or fixed point arithmetic.
...
Here ω f , ω q ∈ R are parameters capturing the relative importance of training loss versus distributional loss.
...
, where σ denotes the softmax function, to be the KL divergence between distributions σ(h W ) and σ(ĥ W,Q ) on each input.
...
Weight scheduling for L f and L q Parameters ω f and ω q capture the relative importance of the cross entropy and KL divergence errors during training.

A large ω f ignores the similarity between the floating point and quantized models and may result in a model that is accurate but not quantization-friendly.

Conversely, a large ω q ignores guidance on accuracy from the floating point model and may result in similar but poorly performing floating point and quantized models.

We tested different schemes for weighting L f and L q , and found that using fixed values such as ω f = ω q = 0.5 already yields better results than many current methods, as discussed in Section 4.

However, further experimentation showed that scheduling, i.e. dynamically modifying the values of ω f , ω q during training can produce higher accuracy than using static values.

For example, consider a schedule as shown in Figure 2a , which initially sets ω f = 1, ω q = 0, then alternates between setting ω q = 1 and ω q = 0 several times.
...
Reducing interference between L f and L q The loss function L includes terms in L f and L q , where the former is a function of the floating point parameters W , and the latter involves both W and the quantized versionŴ of W parameterized by θ.
...
Ideally, we would like update W to minimize L f , while independently updatingŴ to minimize L q .

While this is clearly not possible due to the dependency between W andŴ , we found that a heuristic based on this idea helped improve accuracy.
...
In particular, for each layer i, we use one weight quantizer function Q w i,θ w i (·) for all weights in the layer, and one activation quantizer function Q a i,θ a i (·) for all activations.

Here, θ w i and θ a i represent learnable parameters; for expository simplicity we drop a, w and i in the following and denote all parameters by θ.

θ consists of k, the quantization bitwidth, and lb and ub representing the lower and upper quantization boundaries.
...
Alternatingly optimizing W and θ The accuracy of the quantized model depends both on the weights W of the floating point model as well as how these are quantized using the quantizers parameterized by θ.

We found that jointly optimizing W and θ in each iteration resulted in unstable training and poor accuracy.

Performance was substantially improved by alternatingly optimizing W and θ.

That is, in each training epoch we update either the values in W or θ while freezing the values of the other set of parameters, then switch the updated and frozen sets in the next epoch.

The reason alternating optimization improved training is that both W and θ affect the values of quantized weights, so that updating both simultaneously may cause suboptimal changes to quantized weights.
...
Instead, we regard activations in different numerical precision as being from different domains, similar to as in multi-domain transfer learning, and normalize them with separate statistics {µ f , σ f } and {µ q , σ q }:
...
We replaced the BatchNorm layers in these models with SyncBatchNorm, i.e. we used mini-batch statistics µ f , µ q and σ f , σ q computed from all distributed GPUs during training.
...
The weight and activation quantizers each have their own parameters θ = {lb, ub}, which are initialized at the iteration right before the quantization error penalty L q is enabled.
...
Model weights W and quantization parameters θ were optimized with different optimization settings.
...
Quantization parameters θ were optimized using Adam without weight decay, with coefficients for first and second momentums set to β 1 = 0.9 and β 2 = 0.999, and learning rate fixed to 10 −3 during the entire training process.
...
For the vanilla setting, W and θ were optimized jointly at each training step, loss weights were set to ω f = ω q = 0.5 through the entire training process, gradients from L q propagated through both σ(x L ) and σ(x L ), and full precision and quantized activations were normalized using the same set of running statistics.
...
By alternatingly updating model weights W and quantizer parameters θ between training steps, quantization accuracy improved by +4.24% over the vanilla protocol.

This indicates that although gradients ∇ W L and ∇ θ L both guide their respective parameters to minimize training loss, combining them in a training step makes the quantized weightsŴ derived from these parameters suboptimal.

Dynamically adjusting the weights ω f and ω q during different parts of the training process as described in Section 3.2 improved quantization accuracy by +0.23%.
...
A check mark in column "Alt {W, θ}" means alternatingly optimizing W and θ.

A check for "Schedule ω f , ω q " means ω f and ω q were dynamically adjusted, as described in §3.2.

A check for "Detach σ(x L )" means that σ(x L ) is treated as a constant and we do not propagate ∇ σ(x L ) L q during the backward pass.
...
The schedule we used, alternating between short periods of ω q = 0 and longer periods of ω q = 1, suggests we should first allow the floating point model settle into a reasonably accurate state before enforcing its similarity to the quantized model.

Blocking the gradient from L q caused by σ(x L ) further improved quantization accuracy by +0.66%.
