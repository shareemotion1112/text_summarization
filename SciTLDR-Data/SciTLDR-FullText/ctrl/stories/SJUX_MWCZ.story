...
The aim is twofold: firstly, that the classifier is accurate i.e., Y i =Ŷ i ; and secondly, that the classifier is fair with respect to A i.e.,Ŷ does not discriminate unfairly against examples with a particular value of A. Classifiers with fairness constraints provably achieve worse error rates (cf.
...
We include a hyperparameter α to balance accuracy and fairness; there is no "correct" way to weight these.

When we learn such a model, p is a function of X parametrized by θ.
...
These models involve learning two thresholds τ = (t 0 , t 1 ) (see FIG8 ).

We can train with either hard or soft thresholds.

If soft, each threshold t i , i ∈ {0, 1} is associated with a sigmoid function σ i , where DISPLAYFORM0 1+e −x .
...
When training this model, P , I, and N are parametrized functions, with model parameters θ and thresholds τ .
...
The reciprocal of this (π = 1/S) allows high values to be more uncertain, while π = σ(log(1/S)) (where σ is the logistic function) yields uncertainty values in a [0, 1] range.
...
With weights w and variational parameters θ, our variational lower bound L V is then: DISPLAYFORM0 6 LEARNING TO DEFER IDK models come with a consequence: when a model punts, the prediction is made instead by some external, possibly biased decision maker (DM) e.g., a human expert.
...
We introduce a mixing parameter π for each example x, which is the probability of deferral; that is, the probability that the DM makes the final decision on the example x, rather than the model.

Then, 1 − π is the probability that the model's decision becomes the final output of the system.

Let s ∼ Ber(π).

Our mixing parameter π corresponds to our model's uncertainty estimate -I(x) in ordinal regression, σ(log( 1 S )) in the Bayesian neural network.

Let p be the first stage model's predictions andỸ be the DM's predictions.

We can express the joint system's predictionsp aŝ DISPLAYFORM1 In learning this model, we can parametrize p and π by θ = (θ p , θ π ), which may be shared parameters.
...
Each model contains hyperparameters, such as the coefficients (α, γ) for training and/or post-hoc optimization.
...
In the DM-unaware IDK model, punting is penalized at a constant rate, determined by γ.
...
As in Eq. 8, the standard rejection learning objective is DISPLAYFORM5 where the first term encourages a low loss for non-IDK examples and the second term penalizes IDK at a constant rate, with γ punt ≥ 0.

In rejection learning, is usually classification error (cf.

BID9 Chow (1957) ).

Note that this objective has no notion of DM output (Y DM ).If we include a similar γ def er penalty, the deferring loss function is DISPLAYFORM6 Now, if the DM is an oracle, then DISPLAYFORM7 if we set γ def er = γ punt .
...
We do this for several values of α, γ in thresholding, as well as several values of α for the original binary model.
...
We instead chose to calculate a version of the regularizer with squared underlying terms: DISPLAYFORM1 DISPLAYFORM2 Then, we can expandŶ i asŶ DISPLAYFORM3 whereỸ i ∈ [0, 1] and S(x i ) ∈ [0, 1] are the DM and machine predictions respectively.

For brevity we will not show the rest of the calculation, but with some algebra we can obtain a closed form expression for DI sof t (Y, A,Ŷ ) in terms of Y, A,Ỹ and S.F RESULTS: LEARNING TO DEFER, BY DEFERRAL RATE Models which rarely defer behave very differently from those which frequently defer.
