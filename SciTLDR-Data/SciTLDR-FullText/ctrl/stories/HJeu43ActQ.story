...
, y (p) ], with high probability over the randomness of the coefficients, min O * √ n µ log(n) DISPLAYFORM0 The algorithms discussed above implicitly assume that the coefficients can be recovered, after dictionary recovery, via some sparse approximation technique.
...
Specifically, it requires A (0) to be ( 0 , 2)-close to A * for 0 = O * (1/ log(n)), where ( , κ)-closeness is defined as follows.
...
We also use standard notations O(·), Ω(·) ( O(·), Ω(·)) to indicate the asymptotic behavior (ignoring logarithmic factors).
...
Tτ (z) := z · 1 |z|≥τ denotes the hardthresholding operator, where "1" is the indicator function.
...
Finally at the end of this stage, we have estimate DISPLAYFORM5 Learn Stage: Using this estimate of the coefficients, we update the dictionary at t-th iteration A (t) by an approximate gradient descent step (6), using the empirical gradient estimate (5) and the learning rate η A = Θ(m/k); see also A.5.

Finally, we normalize the columns of the dictionary and continue to the next batch.

The running time of each step t of NOODL is therefore O(mnp log(1/δ R )).For a target tolerance of T and δ T , such that A DISPLAYFORM6 NOODL uses an initial HT step and an approximate gradient descent-based strategy as in Arora et al. (2015) .
...
First, the σ(i)A π(i) − A Definition 3.

A matrix A ∈ R n×m with unit-norm columns is µ-incoherent if for all i = j the inner-product between the columns of the matrix follow DISPLAYFORM0 The incoherence parameter measures the degree of closeness of the dictionary elements.

Smaller values (i.e., close to 0) of µ are preferred, since they indicate that the dictionary elements do not resemble each other.

This helps us to effectively tell dictionary elements apart (Donoho and Huo, 2001; Candès and Romberg, 2007) .

We assume that µ = O(log(n)) (Donoho and Huo, 2001).
...
To ensure progress at each iterate t, we show that the expected gradient vector is (Ω(k/m), Ω(m/k), 0)-correlated with the descent direction, defined as follows.

DISPLAYFORM2 This can be viewed as a local descent condition which leads to the true dictionary columns; see also Candès et al. (2015) , Chen and Wainwright (2015) and Arora et al. (2015) .

In convex optimization literature, this condition is implied by the 2ρ − -strong convexity, and 1/2ρ + -smoothness of the objective.

We show that for NOODL, ζ t = 0, which facilitates linear convergence to A * without incurring any bias.
...
4; A.3 The sparsity k satisfies k = O * ( √ n/µ log(n)); A.4 A (0) is ( 0 , 2)-close to A * as per Def.

1, and 0 = O * (1/ log(n)); A.5 The step-size for dictionary update satisfies η A = Θ(m/k);A.6 The step-size and threshold for coefficient estimation satisfies η (r) x < c 1 ( t , µ, n, k) = Ω(k/ √ n) < 1 and τ (r) = c 2 ( t , µ, k, n) = Ω(k 2 /n) for small constants c 1 and c 2 .We are now ready to state our main result.
...
Suppose that assumptions A.1-A.6 hold, and Algorithm 1 is provided with p = Ω(mk 2 ) new samples generated according to model (1) at each iteration t. Then, with DISPLAYFORM4 alg , given R = Ω(log(n)), the coefficient estimate x (t) i at t-th iteration has the correct signed-support and satisfies ( x DISPLAYFORM5 Furthermore, for some 0 < ω < 1/2, the estimate A (t) at (t)-th iteration satisfies DISPLAYFORM6 2 , for all t = 1, 2, . . .
...
In other words, to attain a target tolerance of T and δ T , where DISPLAYFORM7 R is the target decay tolerance for the IHT steps.
...
As a result, since (for NOODL) the error in the coefficients only depends on the error in the dictionary, it can be made arbitrarily small, at a geometric rate, by the choice of T , δ T , and δ R .
...
For the dictionary update, we analyze the interactions between elements of the coefficient vector (introduces by the IHT-based update step) and show that the gradient vector for the dictionary update is (Ω(k/m), Ω(m/k), 0)-correlated with the descent direction.
...
However, the introduction of the IHT-based strategy for coefficient update makes the analysis highly involved as compared to existing results, e.g., the simple HT-based coefficient estimate in Arora et al. (2015) .NOODL has an overall running time of O(mnp log(1/δ R ) max(log(1/ T ), log( √ k/δ T )) to achieve target tolerances T and δ T , with a total sample complexity of p·T = Ω(mk 2 ).

Thus to remove bias, the IHT-based coefficient update introduces a factor of log(1/δ R ) in the computational complexity as compared to Arora et al. (2015) (has a total sample complexity of p · T = Ω(mk)), and also does not have the exponential running time and sample complexity as Barak et al. (2015) ; see TAB0 .
...
The input to the network is a data and step-size pair (y (j) , η x ) to each input node.
...
Also, at this time, the input layer is injected with (y (j) , η x ) to set the step size parameter η x for the IHT phase, as shown in FIG2
...
The process then continues till the time instance = 2R + 1, for R = Ω(log(1/δ R )) to generate the final coefficient estimate x DISPLAYFORM0 for the current batch of data.
...
In this phase, each output layer neuron communicates the final coefficient estimate x (t) (j) = x (R) (j) to the second layer, which evaluates the residual for one last time (with η x = 1), and shares it across all second layer neurons ("Hebbian learning").
...
This completes one outer iteration of Algorithm 1, and the process continues for T iterations to achieve target tolerances T and δ T , with each step receiving a new mini-batch of data.
...
First of these, Arora15(''biased''), is a simple neurally plausible method which incurs a bias and has a sample complexity of Ω(mk).
...
Here, for all experiments we set η x = 0.2 and τ = 0.1.
...
Here, n = 100, k = 3, ηx = 0.2, τ = 0.1, 0 = 2/ log(n), ηA is chosen as per A.5.
...
DISPLAYFORM2 j with probability at least (1 − δ DISPLAYFORM3 1 F x * is the indicator function corresponding to the event that sign(x * ) = sign( x), denoted by F x * , and similarly for the complement F x * B PROOF OF THEOREM 1We now prove our main result.
...
Theorem 1 (Main Result) Suppose that assumptions A.1-A.6 hold, and Algorithm 1 is provided with p = Ω(mk 2 ) new samples generated according to model (1) at each iteration t. Then, with DISPLAYFORM0 i at t-th iteration has the correct signed-support and satisfies DISPLAYFORM1 Furthermore, for some 0 < ω < 1/2, the estimate A (t) at (t)-th iteration satisfies DISPLAYFORM2 alg is some small constant, where δ DISPLAYFORM3

As a first step, we ensure that our coefficient estimate has the correct signed-support (Def.

2).

To this end, we first show that the initialization has the correct signed-support, and then show that the iterative hard-thresholding (IHT)-based update step preserves the correct signed-support for a suitable choice of parameters.• Step I.A: Showing that the initial coefficient estimate has the correct signed-supportGiven an ( 0 , 2)-close estimate A (0) of A * , we first show that for a general sample y the initialization step (3) recovers the correct signed-support with probability at least (1−δ DISPLAYFORM0 ).
...
Step I.B: The iterative IHT-type updates preserve the correct signed support-Next, we show that the IHT-type coefficient update step (4) preserves the correct signed-support for an appropriate choice of step-size parameter η (r)x and threshold τ (r) .

The choice of these parameters arises from the analysis of the IHT-based update step.

Specifically, we show that at each iterate r, the step-size η (r)x should be chosen to ensure that the component corresponding to the true coefficient value is greater than the "interference" introduced by other non-zero coefficient elements.
...
Suppose DISPLAYFORM4 T ), each iterate of the IHT-based coefficient update step shown in (4) has the correct signed-support, if for a constant c DISPLAYFORM5 the step size is chosen as η DISPLAYFORM6 1 , and the threshold τ (r) is chosen as DISPLAYFORM7 for some constants c 1 and c 2 .

Here, DISPLAYFORM8 ) ,and δ DISPLAYFORM9 ).Note that, although we have a dependence on the iterate r in choice of η (r)x and τ (r) , these can be set to some constants independent of r. In practice, this dependence allows for greater flexibility in the choice of these parameters.
...
We show that the effect of the component that depends on the initial coefficient estimate diminishes by a factor of (1 − η x + η x µt √ n ) at each iteration r. Therefore, for a decay parameter δ R , we can choose the number of IHT iterations R, to make this component arbitrarily small.
...
With probability at least (1 − δ DISPLAYFORM2 β ) the i 1 -th element of the coefficient estimate, for each i 1 ∈ supp(x * ), is given by DISPLAYFORM3 Here, ϑ DISPLAYFORM4 ) and δ DISPLAYFORM5 ).We again observe that the error in the coefficient estimate depends on the error in the dictionary via λ
...
Suppose A (t) is ( t , 2) near to A * with t = O * (1/ log(n)), and number of samples used in step t is p = Ω(mk 2 ), then with probability DISPLAYFORM1

Proof of Theorem 1.

From Lemma 7 we have that with probability at least (1 − δ DISPLAYFORM0 j is (Ω(k/m), Ω(m/k), 0)-correlated with A * j .

Further, Lemma 9 ensures that each iterate maintains the closeness property.

Now, applying Lemma 15 we have that, for η A ≤ Θ(m/k), with probability at least (1 − δ DISPLAYFORM1 0 .

where for 0 < ω < 1/2 with ω = Ω(k/m)η A .
...
Consider the (r + 1)-th iterate x (r+1) for the t-th dictionary iterate, where DISPLAYFORM11 ≤ t for all i ∈ [1, m] evaluated as the following by the update step described in Algorithm 1, DISPLAYFORM12 where η(1)x < 1 is the learning rate or the step-size parameter.

Now, using Lemma 1 we know that x (0) (3) has the correct signed-support with probability at least (1 − δ DISPLAYFORM13 can be written as DISPLAYFORM14 we can write the (r + 1)-th iterate of the coefficient update step using (7) as DISPLAYFORM15 Further, the j-th entry of this vector is given by DISPLAYFORM16 We now develop an expression for the j-th element of each of the term in FORMULA79 as follows.
...
Further, by Claim 2 we show that |β DISPLAYFORM25 where ξ DISPLAYFORM26 .

Further, using Claim 1, DISPLAYFORM27 since x (r−1) − x * 1 = O(k).

Therefore, for the (r + 1)-th iteration, we choose the threshold to be DISPLAYFORM28 and the step-size by setting the "noise" component of FORMULA84 to be smaller than the "signal" part, specifically, half the signal component, i.e., DISPLAYFORM29 Also, since we choose the threshold as τ (r) := η DISPLAYFORM30 max , where x (0) min = C/2, we have the following for the (r + 1)-th iteration, DISPLAYFORM31 Therefore, for this step we choose η DISPLAYFORM32 Therefore, η DISPLAYFORM33 can be chosen as DISPLAYFORM34 .

In addition, if we set all η (r) DISPLAYFORM35 Further, since we initialize with the hard-thresholding step, the entries in |x (0) | ≥ C/2.

Here, we define ξ FORMULA0 , we have DISPLAYFORM36 From Claim 2 we have that |β (t) i1 | ≤ t β with probability at least (1 − δ (t) β ).
...
i1 , where we define q= (1 − η DISPLAYFORM0 Here, α DISPLAYFORM1 is defined as DISPLAYFORM2 Our aim now will be to express C ( ) DISPLAYFORM3 . .
...
i1 is upper-bounded as DISPLAYFORM0 Further, since k = O( √ n/µ log(n)), kc x < 1, therefore, we have DISPLAYFORM1 with probability at least (1 − δ DISPLAYFORM2 i1 δ R 0 for an appropriately large R. Therefore, the error in each non-zero coefficient is DISPLAYFORM3 with probability at least (1 − δ (t) β ).

i1 as defined in (9), and recursively substituting for x (r) i1 we have DISPLAYFORM0 where we set all η r x to be η x .

Further, on defining DISPLAYFORM1 where γ DISPLAYFORM2 Note that γ (R) i1 can be made appropriately small by choice of R. Further, by Claim 5 we have DISPLAYFORM3 with probability at least (1 − δ DISPLAYFORM4 Proof of Lemma 5.

From Lemma 4 we have that for each j ∈ S, DISPLAYFORM5 with probability at least (1 − δ DISPLAYFORM6 and let 1 F x * denote the indicator function corresponding to this event.

As we show in Lemma 2, this event occurs with probability at least (1 − δ DISPLAYFORM7 T ).

Using this, we can write the expected gradient vector corresponding to the j-th sample as 1 DISPLAYFORM8 Here, γ := E[(A (t) x − y)sign(x * j )1 F x * ] is small and depends on δ (t)T and δ

β , which in turn drops with t .

Therefore, γ diminishes with t .

Further, since 1 F x * + 1 F x * = 1, and Pr[ DISPLAYFORM0 T ), is very large, DISPLAYFORM1 Therefore, we can write g DISPLAYFORM2 S ] can be made very small by choice of R, we absorb this term in γ.
...
Here, for = |W | the summation DISPLAYFORM3 has the same distribution as Σ j=1 z j , where each z j belongs to a distribution as DISPLAYFORM4 Let w j = z j − E[z], we will now apply the vector Bernstein result shown in Lemma 11.

For this, we require bounds on two parameters for these -L := w j and σ 2 := Σ j E[ w j 2 ] .
...
Bound on the norm w : This bound is evaluated in Claim 8, which states that with probability at least (1 − δ DISPLAYFORM5 Bound on variance parameter E[ w 2 ]: Using Claim 9, we have DISPLAYFORM6 .

Therefore, the bound on the variance parameter σ 2 is given by DISPLAYFORM7 From Claim 2 we have that with probability at least (1 − δ DISPLAYFORM8 Applying vector Bernstein inequality shown in Lemma 11 and using Lemma 14 (Lemma 45 in (Arora et al., 2015) ), choosing = Ω(k 3 ), we conclude DISPLAYFORM9 with probability at least (1 − δ DISPLAYFORM10 Finally, substituting in (21) we have DISPLAYFORM11 with probability at least (1 − δ DISPLAYFORM12 Proof of Lemma 7.
...
To this end, first from Lemma 6 we have that the empirical gradient vector concentrates around its mean, specifically, DISPLAYFORM13 with probability at least (1 − δ DISPLAYFORM14 HW ).
...
Further, using Claim 7 DISPLAYFORM0 Now, since A (t) − A * ≤ 2 A * (the closeness property (Def.1) is maintained at every step using Lemma 9), and further since A * = O( m/n), we have that DISPLAYFORM1 Therefore, we have DISPLAYFORM2 Here, we use the fact that γ drops with decreasing t as argued in Lemma 5.
...
Now, we show that this gradient is (ρ , 1/100ρ , 0) correlated, DISPLAYFORM0 Therefore, for this choice of k, i.e. k = O( √ n), there is no bias in dictionary estimation in comparison to Arora et al. (2015) .

This gain can be attributed to estimating the coefficients simultaneously with the dictionary.

Further, since we choose 4ρ = p j q j , we have that ρ = Θ(k/m), as a result ρ + = 1/100ρ = Ω(m/k).
...
Starting with (A (t) − A * )diag(1 − η A p i q i ), and using the fact that p i = O(1), q i = O(k/m), and DISPLAYFORM6 Using the results derived above, and the the result derived in Lemma 8 which states that with probability at least (1 − δ DISPLAYFORM7

Proof of Claim 1.

We start by looking at the incoherence between the columns of A * , for j = i, DISPLAYFORM0 Claim 2 (Bound on β (t) j : the noise component in coefficient estimate that depends on t ).

With DISPLAYFORM1 ).Proof of Claim 2.

We have the following definition for β (t) j from (11), DISPLAYFORM2 Here, since x * i are independent sub-Gaussian random variables, β (t) j is a sub-Gaussian random variable with the variance parameter evaluated as shown below DISPLAYFORM3

Pr[|β DISPLAYFORM0 Now, we need this for each β (t) j for j ∈ supp(x * ), union bounding over k coefficients DISPLAYFORM1 ).Claim 3 (Error in coefficient estimation for a general iterate (r + 1)).
...
To reveal this structure, let each α DISPLAYFORM9 max for j = i 1 , i 2 , . . .
...
Using (18), the quantity α DISPLAYFORM15

Published as a conference paper at ICLR 2019 Therefore, we are interested in DISPLAYFORM0 Consider the first term which depends on C (0) DISPLAYFORM1 , we have DISPLAYFORM2 where δ R is a small constant, and a parameter which determines the number of iterations R required for the coefficient update step.
...
Therefore, with probability at least (1 − δ DISPLAYFORM6 2 and |β (t)i | = t β with probability at least (1 − δ (t) β ) using Claim 2.Claim 5 (Bound on the noise term in the estimation of a coefficient element in the support).

With probability (1 − δ DISPLAYFORM7 i1 is defined as DISPLAYFORM8 i1 is as defined in FORMULA0 , DISPLAYFORM9 Therefore, we have the following expression for ϑ DISPLAYFORM10

Published as a conference paper at ICLR 2019 DISPLAYFORM0 i1 can be upper-bounded as DISPLAYFORM1 Since from Claim 6 we have DISPLAYFORM2 Further, since 1 − (1 − η x ) r−1 ≤ 1, we have that DISPLAYFORM3 Therefore, DISPLAYFORM4 i1 .

i | = t β with probability at least (1 − δ DISPLAYFORM0 β ) for the t-th iterate, and k = O * ( √ n µ log(n) ), therefore kc x < 1, we have that DISPLAYFORM1 with probability at least (1 − δ DISPLAYFORM2 , we have DISPLAYFORM3 Proof of Claim 6.
...
DISPLAYFORM14 From FORMULA43 we have the following definition for ϑ DISPLAYFORM15 where β (t) j is defined as the following (11) DISPLAYFORM16 S is a vector with each element as defined in (30).

Therefore, the elements of the vector DISPLAYFORM17 Consider the general term of interest DISPLAYFORM18 Further, since DISPLAYFORM19 we have that DISPLAYFORM20 Further, for DISPLAYFORM21 In addition, for s =i ♠ s we have that DISPLAYFORM22 Therefore, using the results for ♣ and DISPLAYFORM23 and for i = j we have DISPLAYFORM24 Here, from Claim 6, for c x = DISPLAYFORM25 Further, due to our assumptions on sparsity, kc x ≤ 1; in addition by Claim 2, and with probability at least (1 − δ DISPLAYFORM26 with probability at least (1 − δ (t) β ).

Combining results from (31), (32) and substituting for the terms in (33) using the analysis above, DISPLAYFORM27 Note that since γ DISPLAYFORM28 i )) can be made small by choice of R. Also, since Pr[i, j ∈ S] = q i,j , we have DISPLAYFORM29 Claim 8 (An intermediate result for concentration results).

With probability (1 − δ DISPLAYFORM30 Proof of Claim 8.

First, using Lemma 4 we have DISPLAYFORM31 i1 .

Therefore, the vector x S , for S ∈ supp(x * ) can be written as DISPLAYFORM32 where x has the correct signed-support with probability at least (1 − δ T ) using Lemma 2.

Using this result, we can write y − A (t) x as DISPLAYFORM33 With x * S being independent and sub-Gaussian, using Lemma 13, which is a result based on the Hanson-Wright result BID2 ) for sub-Gaussian random variables, and since A (t) DISPLAYFORM34 we have that with probability at least (1 − δ DISPLAYFORM35 ).

DISPLAYFORM36 Consider the ♠ term.

Using Claim 5, each ϑ (R) j is bounded by O(t β ).
...
Rearranging the terms we have the following for ♥, DISPLAYFORM43 Therefore, ♥ can be upper-bounded as DISPLAYFORM44 Next, since (1 − λ (t) j ) ≤ 1, we have the following bound for ♥ 2 DISPLAYFORM45 Further, ♥ 3 can be upper-bounded by using bounds for ♥ 1 and ♥ 2 .
...
In this case, letting DISPLAYFORM9 where N ∈ R k×k is a matrix whose each entry N i,j ≤ |ϑ DISPLAYFORM10 .

with probability at least (1 − δ DISPLAYFORM11 Again, using the result on |ϑ DISPLAYFORM12 Combining all the results for ♣, ♠, ♥ and ♦, we have, DISPLAYFORM13
...
Let x ∼ subGaussian(σ 2 ).

Then, for any t > 0, it holds that Pr[|x| > t] ≤ 2 exp t 2 2σ 2 .
...
Then, for all t ≥ 0, DISPLAYFORM0 σ 2 +Rt/3 , where σ 2 := max{ DISPLAYFORM1 Furthermore, E[ k W k ] ≤ 2σ 2 log(n + m) + 1 3 R log(n + m).
