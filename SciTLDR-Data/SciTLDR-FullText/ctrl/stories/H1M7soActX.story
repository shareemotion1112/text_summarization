...
samples, the prediction function f is often parameterized by θ ∈ R D , such as deep neural networks.
...
, N }, and a stochastic gradient is evaluated based on the chosen minibatch,g(θ t ) = i∈Bt ∇ θ (f (x i ; θ t ), y i )/m, which is an unbiased estimator of the full gradient g 0 (θ t ).

Then, the parameters are updated with some learning rate η t as θ t+1 = θ t − η tg (θ t ).

Denote g(θ) = ∇ θ ((f (x; θ), y), the gradient for loss with a single data point (x, y), and assume that the size of minibatch is large enough for the central limit theorem to hold, and thusg(θ t ) follows a Gaussian distribution BID14 , DISPLAYFORM1 (1) Note that the covariance matrix Σ depends on the model architecture, dataset and the current parameter θ t .
...
In this work, we focus on studying the benefits of anisotropic structure of Σ t in SGD helping escape from minima by bridging the covariance matrix with the Hessian of the loss surface, and its implicit regularization effects on generalization, especially in deep learning context.
...
The noise magnitude could be evaluated as the expectation of the squared norm of the noise vector, DISPLAYFORM5 Thus, we introduce the following constraint, DISPLAYFORM6 From the statistical physics point of view, Tr(ησ 2 t Σ t ) characterizes the kinetic energy (Gardiner), thus it is natural to force the energy to be unchanging, otherwise it is trivial that the higher the energy is, the less stable the system is.

For simplicity, we absorb ησ 2 t into Σ t , denoting ησ 2 t Σ t as Σ t .

If not pointed out, the subscript t of matrix Σ t is omitted to emphasize that we are fixing t and discussing the varying structure of Σ.

For a general loss function L(θ) = E X X (θ) (the expectation could be either population or empirical), where X denotes data example and θ denoted parameters to be optimized, under suitable smoothness assumptions, the SDE associated with the gradient variant optimizer as shown in Eq. (4) can be written as follows BID9 BID2 BID8 , with little abuse of notation, DISPLAYFORM0 Let L 0 = L(θ 0 ) be one of the minimal values of L(θ), then for a fixed t small enough (such that DISPLAYFORM1 It is natural to measure the escaping efficiency using E[L t − L 0 ] since it characterizes the increase of the potential, i.e., the increase of the loss L. And also note that L t − L 0 ≥ 0, for any δ > 0, the escaping probability DISPLAYFORM2 where H t denotes the Hessian of L(θ t ) at θ t .We provide the proof in Appendix, and the same for the other propositions.

The escaping efficiency for general processes is hard to analyze due to the intractableness of the integral in Eq. (8).

However, we may consider the second-order approximation locally near the minima θ 0 , where DISPLAYFORM3 .

Without losing generality, we suppose θ 0 = 0.

Further, suppose that H is a positive definite matrix and the diffusion covariance Σ t = Σ is constant for t. Then the SDE FORMULA7 becomes an Ornstein-Uhlenbeck process, DISPLAYFORM4 Proposition 2 (Escaping efficiency of Ornstein-Uhlenbeck process).

For Ornstein-Uhlenbeck process (9), with t small enough, the escaping efficiency from minimum θ 0 = 0 is, DISPLAYFORM5 Inspired by Proposition 1 and Proposition 2, we propose Tr (HΣ) as an empirical indicator measuring the efficiency for a stochastic process escaping from minima.

Now we turn to analysis which kind of noise covariance structure Σ will benefit escaping sharp minima, under the constraint Eq. (6).Firstly, for the isotropic loss surface, i.e., DISPLAYFORM6 Tr Σ, which is invariant under the constraint that Tr Σ is constant (Eq. FORMULA6 ).

Thus it is only nontrivial to study the impact of noise structure when the Hessian of loss surface is anisotropic.

Secondly, H and Σ being semi-positive definite, to achieve the maximum of Tr(HΣ) under constraint (6), Σ should be DISPLAYFORM7 , where λ 1 , u 1 are the maximal eigenvalue and corresponding unit eigenvector of H. Note that the rank-1 matrix Σ * is highly anisotropic.
...
Around the true parameter According to the classic statistical theory (Pawitan, 2001, Chap. 8) , for population loss L(θ) = E X (θ), with being the negative log likelihood, when evaluating at the true parameter θ * , there is the exact equivalence between the Hessian H of the population loss and Fisher information matrix F , DISPLAYFORM0 In practice, with the assumptions that the sample size N is large enough (i.e. indicating asymptotic behavior) and suitable smoothness conditions, when the current parameter θ t is not far from the ground truth, Fisher is close to Hessian.
...
Consider the binary classification problem with data {(x i , y i )} i∈I , y ∈ {0, 1}, and typical (either population or empirical) loss as DISPLAYFORM2 , where f denotes the output of neural network, and φ denotes the cross-entropy loss with softmax, DISPLAYFORM3 If: (1) the neural network f is with one hidden layer and piece-wise linear activation.

And the parameters of output layer are fixed during training; (2) the optimization happens on a set U such that, f (x; θ) ∈ (−C, C), ∀θ ∈ U, ∀x, i.e., the output of the classifier is bounded during optimization.
...
DISPLAYFORM0 σt is adjusted to make σt t share the same expected norm as that of SGD DISPLAYFORM1 The covariance diag(Σt) is the diagonal of the covariance of SGD noise.

DISPLAYFORM2 .

γi, vi are the first k leading eigenvalues and corresponding eigenvalues of the covariance of SGD noise, respectively. (A low rank approximation of Σ sgd t ) GLD Hessian t ∼ N 0,Ht H t is a low rank approximation of the Hessian matrix of loss L(θ) by its the first k leading eigenvalues and corresponding eigenvalues.

GLD 1st eigven (H) t ∼ N 0, λ1u1u DISPLAYFORM3 λ1, u1 are the maximal eigenvalue and its corresponding unit eigenvector of the Hessian matrix of loss L(θt).
...
And we adjust σ t in each dynamics to force their noise to share the same expected squared norm as defined in Eq. (6).
...
The number of hidden nodes for each network varies in {20, 200, 2000}. We plot the empirical indicator Tr (HΣ) in FIG3 .
...
We firstly run the standard gradient decent for 3000 iterations to arrive at the parameters θ * GD near the global minima with near zero training loss and 100% training accuracy, which are typically sharp minima that generalize poorly BID16 .

And then all other compared methods are initialized with θ * GD and run for optimization with the same learning rate η t = 0.07 and same batch size m = 20 (if needed) for fair comparison 2 .Verification of SGD noise satisfying the conditions in Proposition 3 To see whether the noise of SGD in real deep learning circumstance satisfies the two conditions in Proposition 3, we run SGD optimizer initialized from θ * GD , i.e. the sharp minima found by GD.

FIG4 shows the first 400 eigenvalues of Hessian at θ * GD , from which we see that the 140th eigenvalue has already decayed to about 1% of the first eigenvalue.

Note that Hessian H ∈ R D×D , D = 11330, thus H around θ * GD approximately meets the ill-conditioning requirement in Proposition 3.

FIG4 shows the projection coefficient estimated byâ = u T 1 Σu1 Tr H λ1 Tr Σ along the trajectory of SGD.
...
Therefore, Proposition 3 ensures that SGD would escape from minima θ * GD faster than GLD in order of O(D 2d−1 ), as shown in FIG4 (c).

An interesting observation is that in the later stage of SGD optimization, Tr(HΣ) becomes significantly (10 7 times) smaller than in the beginning stage, implying that SGD has already converged to minima being almost impossible to escape from.

This phenomenon demonstrates the reasonability to employ Tr(HΣ) as an empirical indicator for escaping efficiency.

Behaviors of different dynamics escaping from minima and its generalization effects To compare the different dynamics on escaping behaviors and generalization performance, we run dynamics initialized from the sharp minima θ * GD found by GD.

The settings for each compared method are as follows.

The hyperparameter σ 2 for GLD const has already been tuned as optimal (σ = 0.001) by grid search.
...
We adjust σ t in GLD dynamic, GLD Hessian and GLD 1st eigvec(H) to guarantee that they share the same expected squred noise norm defined in Eq. (6) as that of SGD.

And we measure the expected sharpness of different minima as E ν∼N (0,δ 2 I) L(θ + ν) − L(θ), as defined in BID16 , Eq. FORMULA7 ).
...
DISPLAYFORM0 , and δ = 0.01, the expectation is computed by average on 1000 times sampling.
...
Taking expectation with respect to the distribution of θ t , DISPLAYFORM0 for the expectation of Brownian motion is zero.

Thus the solution of EY t is, DISPLAYFORM1

Proof.

Without losing generality, we assume that L 0 = 0.For multivariate Ornstein-Uhlenbeck process, when θ 0 = 0 is an constant, θ t follows a multivariate Gaussian distribution (Øksendal, 2003) .Consider change of variables θ → φ(θ, t) = e Ht θ t .

Here, for symmetric matrix A, DISPLAYFORM0 where λ 1 , . . . , λ n and U are the eigenvalues and eigenvector matrix of A. Note that with this notation, DISPLAYFORM1 Applying Ito's lemma, we have DISPLAYFORM2 which we can integrate form 0 to t to get DISPLAYFORM3 The expectation of θ t is zero.

And by Ito's isometry (Øksendal, 2003) , the covariance of θ t is, DISPLAYFORM4 DISPLAYFORM5 The proof is finished.

Proof.

Firstly compute the gradients and Hessian of φ, DISPLAYFORM0 And note the Gauss-Newton decomposition for functions with the form of L = φ • f , DISPLAYFORM1 Since the output layer parameters for f is fixed and the activation functions are piece-wise linear, f (x; θ) is a piece-wise linear function on its parameters θ.

Therefore ∂ 2 f ∂θ 2 = 0, a.e., and H = E (x,y) DISPLAYFORM2 It is easy to check that e DISPLAYFORM3 .

Thus, DISPLAYFORM4 A.5 PROOF OF PROPOSITION 5Proof.

For simplicity, we define g := ∇ , g 0 := ∇L = E∇ .The gradient covariance and Fisher has the following relationship, DISPLAYFORM5 Hence, DISPLAYFORM6 Therefore, with the condition DISPLAYFORM7 , we have DISPLAYFORM8 Tr F e −2δ , for δ small enough.

On the other hand, Proposition 4 indicates that e −C F H e C F , which means, DISPLAYFORM9 .

Therefore, for λ, u being a positive eigenvalue and the corresponding unit eigenvector of H, we have DISPLAYFORM10 B ADDITIONAL EXPERIMENTS B.1 DOMINANCE OF NOISE OVER GRADIENT FIG6 shows the comparison of gradient mean and the expected norm of noise during training using SGD.
...
The total number of parameters of this network is 9, 750, 922.Training details Learning rates η t = 0.05 are fixed for all optimizers, which is tuned for the best generalization performance of GD.

The batch size of SGD is m = 100.

The noise std of GLD constant is σ = 10 −3 , which is tuned to best.

Due to computational limitation, we only conduct experiments on GD, GLD const, GLD dynamic, GLD diag and SGD.Estimation of Sharpness The sharpness are estimated by DISPLAYFORM0 with M = 100 and δ = 0.01.Experiments Similar experiments are conducted as in main paper for CIFAR-10 and VGG11, as shown in FIG8 .

The observations and conclusions consist with main paper.

These experiments are implemented by PyTorch 0.3.0.

Note that Σ is the inverse of the Hessian of the quadric form generalizeing the sharp minima.
...
• GD: Learning rate η = 0.1.

We tuned the learning rate (in diffusion stage) in a wide range of {0.5, 0.2, 0.15, 0.1, 0.09, 0.08, . . .

, 0.01} and no improvement on generalization.• GLD constant: Learning rate η = 0.07, noise std σ = 10 −3 .

We tuned the noise std in range of {10 −1 , 10 −2 , 10 −3 , 10 −4 , 10 −5 } and no improvement on generalization.• GLD dynamic: Learning rate η = 0.07.• GLD diagnoal: Learning rate η = 0.07.• GLD leading: Learning rate η = 0.07, number of leading eigenvalues k = 20, batchsize m = 20.

We first randomly divide the training set into 60 mini batches containing 20 examples, and then use those minibatches to estimate covariance matrix.• GLD Hessian: Learning rate η = 0.07, number of leading eigenvalues = 20, update frequence f = 10.
...
And to the same reason, we simplily set the coefficent of Hessian noise to Tr H/m Tr Σ, to avoid extensively tuning of hyperparameter.

• GLD 1st eigvec(H): Learning rate η = 0.07, as for GLD Hessian, and we set the coefficient of noise to λ 1 /m Tr Σ, where λ 1 is the first eigenvalue of H. These experiments are implemented by TensorFlow 1.5.0.
