...
We will now attempt to compute the minimum , that we callˆ (x), required before the class assigned to an input, x, changes.

Assuming the network was able to perfectly classify clean images, the adversarial error rate will then be P (ˆ < ).
...
Notationally, we will refer to the output of the network asŷ i (x) and the corresponding logits as h i (x).

The class prediction of the network will then be argmax(ŷ(x)) = argmax(h(x)).
...
We consider the linearized response of the network and find that in the limit of small (see Appendix 6.2.1), DISPLAYFORM4 where J ij = ∂h j /∂x i is the input-logit Jacobian of the network and δ i = ∂L/∂h i is the error of the outputs of the network.

For notational convenience we will define Γ(x) = J T Jδ/||Jδ|| 2 .
...
In FIG1 (b) we plotˆ linear (x) againstˆ (x) evaluated on every MNIST example in the test set.

The white dashed line is the linê (x) =ˆ linear (x).

We see that whenˆ (x) is small theˆ linear (x) concentrate increasingly around thê (x).

Together these results show that the linear response predictions are valid for small adversarial perturbations.

While the linear model outlined above gives excellent agreement in the → 0 limit, the Γ i (x) are themselves complicated objects (being functions of the Jacobian).

This makes the analytic evaluation of Eq. (6) challenging.

We therefore introduce a "mean-field" approximation to Eq. (6) by replacing Γ i (x) by its average over the dataset, Γ i (x) .
...
We therefore assume that this will be the dominant failure mode for neural networks and write down a mean-field estimate forˆ , DISPLAYFORM7 We show in FIG1 (c) that this approximation continues to be strongly correlated withˆ .

Together these results suggest that the adversarial error rate for perturbations of size will be DISPLAYFORM8 where we have defined˜ = ( Γ 2 − Γ 1 ) to be a network-specific rescaling of .
...
Our proposed loss function can be written as: loss = old loss − λ n i=1 p i log p i .

where λ is a hyperparameter, n is the number of classes, and p i are the outputs of the neural network.
...
In Fig. 5 , we show that entropy regularization with a λ = 4.5 increases the adversarial robustness both for regularly trained networks and step l.l.

adversarially trained networks, and both for permutation invariant and regular MNIST.

We note that the same qualitative results hold for other values of λ we tried.
...
such that t β = 1 if β = γ for some γ and t β = 0 otherwise.

We assume our network gets the answer correct so that h γ > h β for all β = γ.

Then we apply the adversarial perturbation, DISPLAYFORM0 Note that we can write DISPLAYFORM1 Where we associate J αβ = ∂h β /∂x α with the input-to-logit Jacobian linking the inputs to the logits and δ = ∂L/∂h β the error of the outputs of the network.
