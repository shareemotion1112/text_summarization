...
By contrast, w i,j , α i,j and η are the structural components of the network, which are optimized by gradient descent between episodes/lifetimes to minimize the expected loss over an episode.
...
The simplest way to introduce neuromodulation of plasticity in this framework is to make the (global) η parameter depend on the output of one or more neurons in the network.

Because η essentially determines the rate of plastic change, placing it under network control allows the network to determine how plastic connections should be at any given time.

Thus, the only modification to the equations above in this simple neuromodulation variant is to replace η in Eq. 2 with the network-computed, time-varying neuromodulatory signal M (t).
...
We simply replace Eq. 2 above with the two equations, DISPLAYFORM0 Here E i,j (t) (the eligibility trace at connection i, j) is a simple exponential average of the Hebbian product of pre-and post-synaptic activity, with trainable decay factor η.
...
The size of the LSTM layers is adjusted to ensure that the total number of trainable parameters remains constant across all experiments; note that this includes all plasticity-related additional parameters, i.e. α i,j as well as the additional parameters related to neuromodulation (see Appendix).
...
Because the number of plastic connections is large, each plastic connection has its own individual η so that their values can be individually tuned by backpropagation.

(3) LSTM with Simple Neuromodulation: here simple neuromodulation is introduced following equation 3.

The η parameters are replaced by the output of a neuron M (t).
...
Note that in this model, plasticity coefficients are attributed "per neuron": there is only one α i for each neuron i (as opposed to one per connection), which is applied to all the Hebbian traces of the connections incoming to this neuron.
...
Each plastic connection has its own individual η (used in equation 2) that is learned through backpropagation.

The plasticity coefficients (α i,j ) are used as shown in equation 1.

As shown in equation 3, for simple neuromodulation, the η is replaced by the output of a network computed neuron M (t).

For neuromodulated LSTMs, individual η for each plastic connection is replaced by the output of a neuron (M (t)) that has a fan-out equal to the number of plastic connections.
...
Plasticity coefficients are attributed "per neuron": rather than having and independent α i,j for each connection, each neuron i has a plasticity coefficient α i that is applied to all its incoming connection (note that Hebbian traces Hebb i,j are still individually maintained for each connection).

This reduces the number of trainable parameters, since α is now a vector of length N rather than a matrix of size N × N (where N is the number of recurrent neurons).We implement simple neuromodulation as described in Equation 3.

A single neuromodulator neuron with tanh nonlinearity receives input from all recurrent neurons.

This neuromodulator input is then passed through a vector of weights, one per neuron, to produce a different η i for each neuron.

In other words, different neurons i have different η i , but these are all fixed multiples of a common value.

This is an intermediate solution between having a single η(t) for the whole network, and independently computing a separate η i for each neuron, each with its own input weights (which would require N × N weights, rather than 2 × N for the current solution).
