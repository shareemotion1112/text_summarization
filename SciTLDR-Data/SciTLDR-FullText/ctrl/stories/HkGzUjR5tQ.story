...
To address this imbalance problem, we impose a weight α on two resources to balance their influences.
...
And transition cliques, on the other hand, reflect the evolution of states between two neighboring elements at time t − 1 and t and we define the transition distribution as θ.
...
To be specific, we prepare an adversarial sample by adding the original sample with a perturbation bounded by a small norm to maximize the loss function as follows: DISPLAYFORM1 where Θ is the current model parameters set.

However, we cannot calculate the value of η exactly in general, because the exact optimization with respect to η is intractable in neural networks.
...
We find such η against the current model parameterized by Θ, at each training step, and construct an adversarial example by x adv = x + η x .
...
2 and define the loss function for adversarial training as: DISPLAYFORM3 where (Θ; x), (Θ; x adv ) represents the loss from an original example and its adversarial counterpart, respectively.
...
For example, for the source data with word embedding w S , the loss for AT can be defined as follows, AT = (Θ; w S ) + (Θ; w S,adv ) with w S,adv = w S + η w S and = GRAD + S .

Similarly, we can compute the perturbations η c for char-embedding and η w T for target word embedding.
...
We set the initial learning rate of β 0 = 0.001 for all experiments.

At each epoch t, learning rate β t is updated using β t = β 0 /(1 + ρ × t), where ρ is decay rate with 0.05.
...
6 We used data ratio ρ = 0.5 for training model and randomly selected 10k testing data for visualization.
...
From the previous results, we know that AT helps enhance the overall performance by adding perturbations to inputs with the limit of = 5, i.e., η 2 ≤ 5.
...
Finally, we analyze the discriminator weight α in GRAD and results are summarized in TAB8 .

From the results, it is interesting to find that α is directly proportional to the data ratio ρ, basically, which means that more target training data requires larger α (i.e., smaller 1 − α to reduce training emphasis on the target domain) to achieve better performance.
