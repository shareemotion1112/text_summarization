...
We will introduce depth into our model, by parameterizing σ using w ∈ R d ≥0 in the following way:
...
Assuming the data is labeled by some σ * ∈ R d ≥0 , we will study the dynamics of this model for general N under a depth-normalized 1 squared loss over Gaussian inputs, which will allow us to derive our analytical solution:

We will assume that our model is initialized uniformly with a tunable scaling factor, such that:

2.2 GRADIENT FLOW ANALYTICAL SOLUTIONS Analyzing our toy model using gradient flow allows us to obtain an analytical solution for the dynamics of σ(t) along with the dynamics of the loss function for a general N .
...
We also assume σ * i > 0 for brevity, however we can derive the solutions for σ * i = 0 as well.
...
Minimizing the toy linear model described in (1) with gradient flow over the depth normalized squared loss (2), with Gaussian inputs and weights initialized as in (3) and assuming σ * i > 0 leads to the following analytical solutions for different values of N :

Proof.

The gradient flow equations for our model are the following:

Given the dynamics of the w parameters, we may use the chain rule to derive the dynamics of the induced model, σ:σ
...
for σ * i ∈ {12, 6, 4, 3} according to the analytical solutions in theorem 1, under different depths and initializations.
...
These dynamics are depicted in figure 2, where we see the dynamics of the different values of σ(t) as learning progresses.
...
While Gidel et al. (2019) focuses on incremental learning in depth-2 models at the limit of σ 0 → 0, we will study the phenomenon for a general depth and for σ 0 > 0.

First, we will define the notion of incremental learning.

Since all values of σ are learned in parallel, we can't expect one value to converge before the other moves at all (which happens for infinitesimal initialization as shown by Gidel et al. (2019) ).

We will need a more relaxed definition for incremental learning in finite initialization scales.

Definition 1.

Given two values σ i , σ j such that σ * i > σ * j > 0 and both are initialized as σ i (0) = σ j (0) = σ 0 < σ * j , and given two scalars s ∈ (0, 1 4 ) and f ∈ ( 3 4 , 1), we call the learning of the values (s, f )-incremental if there exists a t for which:
...
Note that for any N , σ(t) is monotonically increasing and so once σ j (t) = sσ * j , it will not decrease to allow further incremental learning.
...
Rewriting the separable differential equation in (4) to calculate the time until σ(t) = ασ * , we get the following:

The condition for incremental learning is then the requirement that t f (σ i ) ≤ t s (σ j ), resulting in:

We then relax/restrict the above condition to get a necessary/sufficient condition on σ 0 , leading to a lower and upper bound on σ th 0 .
...
Given two values σ i , σ j of a depth-2 toy linear model as in (1), such that
...
Both A and B simplify to r when we take their first order approximation around c = 0, giving us similar bounds and showing that the condition on σ 0 for N = 2 is exponential in gradient descent as well.
...
Where σ i and σ * i are the ith singular values of W and W * , respectively, corresponding to the same singular vector.
...
We see the same dynamical attenuation of small values of σ that is seen in the regression model, caused by the multiplication by σ .

From this, we can expect the same type of incremental learning to occur -weights of σ will be learned incrementally until the dataset can be separated by the current support of σ.
...
We also observe that the initialization scale plays a similar role as before -deep models are less biased towards sparsity when σ 0 is large.
...
According to their results, we should expect to see the same sparsity-bias of diagonal networks in convolutional networks, when looking at the Fourier coefficients of σ.
...
Our strategy will be to define the time t α for which a value reaches a fraction α of it's optimal value, and then require that t f (σ i ) ≤ t s (σ j ).

We begin with recalling the differential equation which determines the dynamics of the model:σ

Since the solution for N ≥ 3 is implicit and difficult to manage in a general form, we will define t α using the integral of the differential equation.

The equation is separable, and under initialization of σ 0 we can describe t α (σ) in the following way:

Incremental learning takes place when σ i (t f ) = f σ * i happens before σ j (t s ) = sσ * j .

We can write this condition in the following way:

Plugging in σ i = rσ j and rearranging, we get the following necessary and sufficient condition for incremental learning:
...
At this point, we cannot solve this equation and isolate σ 0 to obtain a clear threshold condition on it for incremental learning.

Instead, we will relax/restrict the above condition to get a necessary/sufficient condition on σ 0 , leading to a lower and upper bound on the threshold value of σ 0 .

To obtain a sufficient (but not necessary) condition on σ 0 , we may make the condition stricter either by increasing the left-hand side or decreasing the right-hand side.
...
Next, we note that the integration bounds give us a bound on σ for either integral.

This means we can replace 1 − σ σ * j with 1 on the right-hand side, and replace 1 − σ rσ * j with 1 − f on the left-hand side:

We may now solve these integrals for every N and isolate σ 0 , obtaining the lower bound on σ th 0 .

We start with the case where N = 2:

Rearranging to isolate σ 0 , we obtain our result:

For the N ≥ 3 case, we have the following after solving the integrals:

For simplicity we may further restrict the condition by removing the term 1 rf σ * j 1− 2 N .

Solving for σ 0 gives us the following:

To obtain a necessary (but not sufficient) condition on σ 0 , we may relax the condition in (8) either by decreasing the left-hand side or increasing the right-hand side.

We begin by rearranging the equation:

Like before, we may use the integration bounds to bound σ.

Plugging in σ = sσ * j for all integrals decreases the left-hand side and increases the right-hand side, leading us to the following:
...
Rearranging to isolate σ 0 , we get our condition:

Finally, for N ≥ 3, we solve the integrals to give us:

Rearranging to isolate σ 0 , we get our condition:

For a given N , we derived a sufficient condition and a necessary condition on σ 0 for (s, f )-incremental learning.

The necessary and sufficient condition on σ 0 , which is the largest initialization value for which we see incremental learning (denoted σ th 0 ), is between the two derived bounds.
...
We start by deriving the recurrence relation for the values σ(t) for general depth, when t now stands for the iteration.

Remembering that w n i = σ i , we write down the gradient update for w i (t):

Raising w i (t) to the N th power, we get the gradient update for the σ values:
...
Defining r i = σi σ * i and dividing both sides by σ * i , we have:
...
Our strategy will be bounding σ i (t) from below and above, which will give us a lower and upper bound for t α (σ i ).

Once we have these bounds, we will be able to describe either a necessary or a sufficient condition on σ 0 for incremental learning, similar to theorem 2.
...
We may now subtract 1 σ * i from both sides to obtain:

We may now obtain a bound on t α (σ i ) by plugging in σ i (t) = ασ * i and taking the log:
...
Given these bounds, we would like to find the conditions on σ 0 that allows for (s, f )-incremental learning.
...
We can find such a condition by requiring the upper bound of t f (σ i ) to be smaller than the lower bound on t s (σ j ).
...
Now, we will add the very reasonable assumption that σ * j ≥ 2σ 0 , which allows us to replace

Now we can rearrange and isolate σ 0 to get a sufficient condition for incremental learning:

A necessary condition for incremental learning will be one which is possibly more relaxed than the exact condition.

We can find such a condition by requiring the lower bound of t f (σ i ) to be smaller than the upper bound on t s (σ j ).

This becomes the following condition:

log 1−cRj +c 2 R 2 j and rearranging, we get the following:

We may now take the exponent of both sides and rearrange again, remembering σ * i σ * j = r > 1, to get the following condition:

We may now relax the condition further, by removing the r from the denominator of the left-hand side and the σ 0 from the numerator.

This gives us the following:

Finally, rearranging gives us the necessary condition:

While we were able to generalize our result to gradient descent for N = 2, our proof technique relies on the ability to get a non-implicit solution for σ(t) which we discretized and bounded.
...
We start by reminding ourselves of the gradient descent update rule for σ, for a learning rate η = c

To compare two values in the same scales, we will divide both sides by the optimal value σ * i and look at the update step of the ratio r i =
...
Denoting r = σ * i σ * j , we can describe the initialization of r j using that of r i :
...
However, we parameterize σ using w + , w − ∈ R d in the following way:

We can now treat this algorithm as a sparse approximation pursuit algorithm -given a dictionary D ∈ R d×n and an example x ∈ R d , we wish to find the sparsest α for which Dα ≈ x by minimizing the 0 norm of α subject to ||Dα − x|| 2 2 = 0 3 .
...
Where σ i and σ * i are the ith singular values of W and W * , respectively, corresponding to the same singular vectors.

Proof.

We will adapt the proof from Saxe et al. (2013) for multilayer linear networks.

The gradient flow equations for W n , n ∈ [N ] are: (3), U diagonalizes all W n matrices at initialization such that D n = U W n U T = N √ σ 0 I. Making this change of variables for all W n , we get:
...
Denoting σ n,i as the i'th singular value of W n and σ i as the i'th singular value of W , we get the following differential equation:

Since we assume at initialization that ∀n, m, i : σ n,i (0) = σ m,i (0) = N √ σ 0 , the above dynamics are the same for all singular values and we get ∀n, m, i : σ n,i (t) = σ m,i (t) = N σ i (t).
...
Denoting σ −n,i = k =n σ k,i and using the chain rule:
...
Minimizing the quadratic network described and initialized as in (7) with gradient flow over the variance loss defined in (2) with Gaussian inputs leads to the following dynamical equations:σ
...
Now, under our initialization W T 0 W 0 = σ 0 I, we get that W T W and W T * W * are simultaneously diagonalizable at initialization by some matrix U , such that the following is true for diagonal D and D * :
...
in the previous section, we derive the differential equations for the singular values of W T W under the variance loss:σ
...
Figure 6 shows the dynamics of the largest values of σ for different depths of the model.
...
The other panels show the evolution of the absolute values of the top-5 weights of σ for the smallest initialization scale.
...
The other panels show the evolution of the amplitudes of the top-5 frequencies of σ for the smallest initialization scale.
