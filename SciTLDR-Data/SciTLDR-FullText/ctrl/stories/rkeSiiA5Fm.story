...
The 2-sphere is a 2-manifold on which any pointû is a unit vector.

Theû can be parametrized by spherical DISPLAYFORM0 2 has a positive area A = r ds(û), where ds(û) = sin θdθdφ.

A special region on the 2-sphere is called polar cap region R θ0 , around the north pole,η(0, 0, 1), which is azimuthally symmetric and is parameterized by a maximum colatitude angle θ 0 : DISPLAYFORM1 3D Rotations The set of rotations in three dimensions is called "special orthogonal group" SO(3).

SO(3) is a 3-manifold on which any rotation R ∈ SO(3) can be represented as a 3 × 3 matrix.

Each rotation R is associated with three independent parameters, we use the right hand rule zyz-Euler DISPLAYFORM2 If we fix the third rotation angle ω to zero, SO(3) is reduced into a subset A with two independent parameters.

Any rotation R ∈ A can be described as an alt-az rotation 1 : DISPLAYFORM3 An alt-az rotation can be considered as a composition of an altitude rotation R (y) ϑ ∈ SO(2) and a azimuth rotation R (z) ϕ ∈ SO(2).

Rotation operator We define the effect of general rotation on spherical functions as an operator D R (ϕ, ϑ, ω) which corresponds to the rotation matrix R defined in Eqn.

(2).

The effect of D R (ϕ, ϑ, ω) on the spherical image f can be realized through an inverse rotation R −1 of the coordinate system.
...
Given a spherical filter h and spherical image f evaluated at a pointû ∈ S 2 , general anisotropic convolution on S 2 is defined: DISPLAYFORM6 Note that the output function g is not defined on the original S 2 .

Instead, it is a function of three Euler angles (ϕ, ϑ, ω) and is therefore defined on the 3-manifold SO(3) (please see BID5 for detail.)Type II: Azimuthally isotropic convolution: This spherical convolution outputs a function defined on S 2 using an azimuthally symmetric filter h 0 (û) BID7 ; BID6 ): DISPLAYFORM7 Referring to Eqn.
...
Similarly, in the spherical domain, the two degrees of freedom in altaz rotation are the direct analogs of two spatial translations in R 2 ("shifting on the sphere"), and DISPLAYFORM8 ω , emulating the non-rotatable filters in R 2 , can be fixed and treated with data augmentation.
...
Type III: alt-az anisotropic spherical convolution (a 3 SConv): Constraining the rotation of filter within alt-az rotation set A, a filter h spans the altitude change by ϑ and azimuth change by ϕ, and is convolved with the spherical signal f .
...
A local max pooling (LMP) layer replaces a spherical image f in at any pointû 0 (θ 0 , φ 0 ) with the maximum function value in its geodesic disc neighborhood, i.e., DISPLAYFORM0 whereû(θ, φ) is a neighboring point ofû 0 , and |.| denotes the geodesic distance between them.
...
And we compare five modes of experiments: (a) trained with original data without data augmentation (NA), (b) trained with 36 azimuth rotation augmentation (AZ) by sampling ω per 10 degrees, (c) trained with 36 rotation augmentation (SO(2)(x)) by rotating about x-axis per 10 degrees, (d) trained with 72 alt-az rotation augmentation (Alt-AZ) by sampling θ and φ per 30 degrees, and (e) trained with arbitrary 128 rotations (SO FORMULA3 ).
...
For any feature output from a GMP layer, suppose it is activated at pointû 0 (θ 0 , φ 0 ).

It corresponds to a maximum correlation between the learned filter h and the input image f (when h is "alt-az" rotated ontoû o ).

Any SO(3) rotation of f which first rotates the pointû o back to the north poleη(0, 0, 1), followed by an arbitrary alt-az rotation D(φ , θ , 0) to new pointû 1 , will be invariant to the network.

When h is convolved at pointû 1 .

We define this set of rotation as "alt-az shift rotation"(see FIG7 ).

Alt-az shift rotation rotation causes no relative angular change of the geodesic disc centered atû o with respect to the h's convolving.

For a directionn (n is not along z-axis or y-axis), if one rotates f aboutn an arbitrary angle which move the original salient pointû o toû 1 , this rotation is in general not an alt-az shift rotation and will change the correlation between h and f when h is convolved atû 1 (See FIG7 ).

If one rotates f aboutn a full round, the geodesic disc centered atû o , will go through a relative self rotation from 0 to 360 degrees, using the alt-az rotation of h to each corresponding point as the direction references.
