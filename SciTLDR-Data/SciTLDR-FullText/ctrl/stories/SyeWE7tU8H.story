...
To map these anatomical measurements onto our theory, we assume that the synapse counts are proportional to the physiological synaptic weights (for constraints on the net synaptic weight,JK p = αS).

We computed the Laplace approximation for the model evidence (marginal likelihood) under each of the constraints discussed above, marginalizing out the scale factor α relating the anatomical measurements to the modeled synaptic weights [24] .
