...
Despite these improvements, samplers like SGLD are only guaranteed to converge to the correct G t ← minibatch gradient(θ t ) 6:
...
is required, it is sufficient to define an energy function H(z) = − log p(z) + C up to a constant C. As a consequence, we can sample from the posterior distribution p(θ|x) by only evaluating the energy function gradient ∇H(θ) = −∇ log p(x, θ).

The general form of SDEs converging to p(z) for which only the gradient of p(z) is required is as follows (Ma et al., 2015) :

where D(z) is a positive-definite matrix that determines the amount of noise, Q(z) is a skew-symmetric matrix that mixes energy between variables, W t is a Wiener process, and Γ(z) is a correction factor that compensates for dynamics that depend on the current state z. The ATMC sampler that we propose is an instance of (1) for specific definitions of H(z), D(z), and Q(z).

We start by defining the energy function H(z).

The energy function for the model posterior p(θ|x) is defined by the loss function L(θ) = − log p(x, θ).

Because the dataset x is generally large, we would like to only evaluate a mini-batch lossL(θ).
...
where p is the momentum, K(p) defines the momentum distribution, and ξ is a control variate referred to as the temperature.

Both p and ξ have the same dimensionality as θ.

The hyper-parameter m controls the strength of the coupling between ξ and p. The distribution of the control variate p(ξ) depends on the amount of noise B in the stochastic gradient estimateL(θ).
...
where α(ξ) is a non-negative function that determines how the temperature ξ affects the amount of noise added to the momentum update.

We first illustrate the resulting SDE by using a simpler Gaussian momentum distribution

Note that the variance of the momentum Var(p) = m is reused in (2) and (3) to control the strength of the coupling between ξ and p. This will result in a temperature control with a momentum friction term proportional to ξ, unlike previously reported thermostat MCMC methods (Ding et al., 2014; Lu et al., 2016) where the friction term is proportional to ξ/m.

We substitute the dynamics Q(z) and D(z) defined in (3) and energy function H(z) defined in (2) into (1): dθ dp dξ

where we use ∇L(θ)dt = ∇L(θ)dt+ √ BdW t to replace the gradient of the loss with the minibatch estimate.

The momentum p is dampened by a friction term β(ξ) that depends on the choice of α(ξ).
...
Finally, we must choose a function α(ξ) which controls the amount of noise and momentum damping β(ξ).

Previous work uses the Nosé-Hoover thermostat that is defined by α(ξ) = D c where D c is a constant determining the amount of noise added to the momentum update (Ding et al., 2014) .

Although the Nosé-Hoover thermostat is able to correct the stochastic gradient noise B, the correction comes at the cost of slower convergence because additional friction β(ξ) is applied as B increases.

Another drawback of the Nosé-Hoover thermostat is that it causes negative friction when ξ < −D c .

In the negative friction phase β(ξ) < 0, previous gradient terms are amplified rather than dampened.

Although this behavior is mathematically sound we find that it can cause exploding momentum variables.

Our choice of α(ξ) is based on the idea that negative friction should not occur and convergence speed should not be reduced by the stochastic gradient noise.

Based on this intuition, we define the ATMC sampler by α(ξ) = max(D − ξ, 0).

The ATMC sampler is best characterized by the various temperature stages.

For 0 < ξ < D c the total amount of noise added to the momentum is D c and the friction coefficient β(ξ) = D c .

At this stage, the stochastic gradient noise is compensated for by adding less noise to the momentum update.

If B D c the dominant stage will be ξ > D resulting in β(ξ) < D c and zero noise being added to the momentum.

Finally, when ξ < 0 the friction coefficient β(ξ) = D c and the noise added to the momentum is proportional to D c − ξ.

Thus, the momentum always experiences a minimum amount of friction β(ξ) ≥ D c determined by the hyper-parameter D c and the noise added to the momentum update is automatically adjusted based on the amount of noise present in the stochastic gradients.
...
The Gaussian kinetic energy K(p) = p 2 /(2m) is a special case obtained by taking the limit c → ∞. The magnitude of parameter updates ∆θ is determined by the gradient of the momentum:
...
For the hyperbolic distribution, the dynamic friction coefficient β(ξ) will also depend on p.

For the numerical integration of (4) with a hyperbolic momentum distribution we assume β(ξ) to be constant in p.

In this section we construct the numerical integrator required to numerically approximate the ATMC sampler defined in (4).

An efficient numerical integrator can be constructed by splitting the SDE into two terms: dθ dp dξ

Hence, we obtain a linear ODE in part (A) that updates the parameters θ and the thermostats ξ and a linear SDE in part (B) that updates the momentum p.
...
The first operator φ h A is given by

The second operator φ B is an instance of the Ornstein-Uhlenbeck process which can also be computed analytically as follows (Nelson, 1967) :

where η t is isotropic Gaussian noise.

Previous work on higher order integrators for samplers splits the SDE into three parts where the third term is obtained from separating the friction term from the other terms in the momentum update φ B .
...
s is the magnitude of a feature vector θ (i) .
...
Therefore the prior on the direction vector φ

d is chosen such that it is forced to unit length

The prior on the scales p(φ s ) is problem-specific and can for example be chosen to encode a preference for structurally sparse models.
...
For ResNet++ we further use a group Laplace prior p(θ i ) ∝ exp(− θ i /b) with b = 5 to regularize the scales of each linear feature in ResNet++.
...
We also report the results of sampling with a sampler based Nosé-Hoover thermostats (SGNHT) (Ding et al., 2014; Lu et al., 2016) applied to the ResNet++ architecture.

To further analyze the quality of the uncertainty estimates, we group each model's prediction in 8 equally sized bins based on the confidence p(ω i |x i ) whereω i is the maximum probability class for example x i .
...
The posterior predictive is calibrated for the least confident predictions p(ω i |x i ) < 0.9 and shows less bias towards overconfidence compared to the models trained with SGD.
...
Again, the ATMC based posterior predictive produces much better calibrated predictions and is almost perfectly calibrated for low confidence predictions p(ω i |x i ) < 0.9 and shows less bias towards overconfidence compared to the optimization baseline.
