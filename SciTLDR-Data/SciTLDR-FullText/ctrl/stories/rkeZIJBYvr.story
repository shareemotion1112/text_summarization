...
(a) To handle task imbalance (Task Imbal.), class imbalance (Class Imbal.) and outof-distribution tasks (OOD) for each task τ , we introduce task-specific balancing variables γ τ , ω τ and z τ , respectively.

(b) With those variables, we learn to balance between the meta-knowledge θ and task-specific update to handle imbalances and distributional discrepancies.
...
Suppose a task distribution p(τ ) that randomly generates task τ consisting of a training set

Then, the goal of MAML is to meta-learn the initial model parameter θ as a meta-knowledge to generalize over the task distribution p(τ ), such that we can easily obtain the task-specific predictor θ τ in a single (or a few) gradient step from the initial θ.
...
Note that by meta-learning the initial point θ, the task-specific predictor θ

even with D τ which only contains few samples.

We can easily extend the Eq. (1), such that we obtain θ τ with more than one inner-gradient steps from the initial θ.
...
MAML has a fixed number of inner-gradient steps and stepsize α across all tasks, which prevents the model from adaptively deciding how much to use from the meta-knowledge depending on the number of the training examples per task.
...
As shown in Figure 1 for the concepts, we introduce three balancing variables γ τ , ω τ , z τ to tackle each problem mentioned above.

How to compute these variables will be described in Section 4.

In order to learn with realistic scenarios, we assume that the task distribution p(τ ) samples some fixed number of C classes ("way"), and then sample uniform-random number of instances for each class ("shots"), thereby simulating both task and class imbalance at the same time.

Tackling task imbalance.

To control whether to stay close to the initial parameter or deviate far from it, we introduce a clipping function f (·) = max(0, min(·, 1)) and a task-dependent learning-rate decaying factor f (γ τ ), such that the learning rate exponentially decays as

to be large for large tasks, such that they rely more on task-specific updates, while small tasks use small f (γ τ ) to benefit from the meta-knowledge.
...
, C, we introduce a non-negative activation function g(·) = SoftPlus(·) and a set of classspecific non-negative scalars g(ω

is the set of instances and labels for class c.

We expect g(ω τ c ) to be large for tail-classes to consider them more in task-specific gradient updates.

Tackling out-of-distribution tasks.

Lastly, we introduce an additional task-dependent variable z τ with the non-negative activation function g(·) which weights the initial parameter θ according to the usefulness for each task.

We expect the variable g(z τ ) to heavily emphasize the meta-knowledge θ when D τ is similar to the trained dataset, and use less of it when D τ is unfamilar.

This behavior can be implemented with Bayesian modeling on the latent z τ , which we introduce in the next subsection.

A unified framework.

Finally, we assemble all these components together into a single unified framework.

The update rule for the task-specific θ τ is recursively defined as follows:

where the last step θ K corresponds to the task-specific predictor θ τ and α is a multi-dimensional global learning rate vector that is learned such as .

As previously mentioned, we need a Bayesian framework for modeling z τ , since it needs a prior in order to prevent the posterior of z τ from overly utilizing the meta-knowledge θ when the task is out-ofdistribution.

Moreover, for the learning of balancing variables γ τ and ω τ , Bayesian modeling improve the quality of the inference on them, which we empirically verified through extensive experiments.
...
Let φ τ denote the collection of three latent variables, γ τ , ω τ and z τ for uncluttered notation.

Then, the generative process is as follows for each task τ (See Figure 2) :

for the complete data likelihood.

Note that the deterministic θ is shared across all the tasks.

The goal of learning for each task τ is to maximize the log-likelihood of the joint datasetD τ and

.

However, solving it involves the true posterior p(φ τ |D τ ,D τ ), which is intractable.

Thus, we resort to amortized variational inference with a tractable form of approximate posterior q(φ τ |D τ ,D τ ; ψ) parameterized by ψ.

Further, similarly to Ravi & Beatson (2018) , we drop the dependency on the test datasetD τ for the approximate posterior, in order to make the two different pipelines consistent; one for meta-training where we observe the whole test dataset, and the other for meta-testing where the test labels are unknown.

The form of our approximate posterior is now q(φ τ |D τ ; ψ).
...
Also, considering that performing the inner-gradient steps with the training dataset D τ automatically maximizes the training log-likelihood in MAML framework, we slightly modify the objective so that the expected loss term only involves the test examples.

The resultant form of the lower bound that suits for our meta-learning purpose is as follows:

We assume q(φ τ |D τ ; ψ) fully factorizes for each variable and also for each dimension as well:

where we assume that each single dimension of q(φ τ |D τ ; ψ) follows univariate gaussian having trainable mean and variance.

We also let each dimension of prior p(φ τ ) factorize into N (0, 1).

The KL-divergence between two univariate gaussians has a simple closed form (Kingma & Welling, 2014) , thereby we obtain the low-variance estimator for the lower bound L τ θ,ψ .
...
We implicitly assume the reparameterization trick for φ τ to obtain stable and unbiased gradient estimate w.r.t.

ψ (Kingma & Welling, 2014).
...
The main challenge in modeling our variational distribution q(φ τ |D τ ; ψ) is how to refine the training dataset D τ into informative representation capturing the dataset as a distribution, which is not trivial.

This inference network should capture all the necessary statistical information in the dataset D τ to solve both imblanace and out-of-distribution problems.
...
However, for the classification dataset D τ which is the set of (class) sets, we cannot use DeepSets directly as it will completely ignore the label information.
...
for classes c = 1, . . . , C, and X τ c is the collection of class c examples in task t. NN 1 and NN 2 are some appropriate neural networks parameterized by ψ.

The vector v τ finally summarizes the whole classification dataset D τ and our balancing variables γ τ , ω τ and z τ are generated from it with an additional affine transformation.
...
g(z τ ) for handling distributional discrepancy.

g(z τ ) modulates the initial model parameter θ, deciding what and how much to use from the meta-knowledge θ based on the relatedness between θ and the task at hand.

Table 3 shows that by adding in Bayesian g(z τ ) component to Meta-SGD, we can effectively handle out-of-distribution tasks (MNIST) by significant margin.

The histogram in Figure 4 shows that the actual distribution of mask g(z τ ) of OOD tasks is more skewed toward zero than the distribution of ID tasks, which agrees with our expectation.

f (γ τ ) for handling task imbalance.

f (γ τ ), which is a decaying factor for inner gradient steps, handles inter-task imbalance where each task has different number of examples.

Figure 5 shows f (γ τ ) values w.r.t.
...
g(ω τ ) for handling class imbalance.

g(ω τ ) rescales the class-specific gradients to handle class imbalance where the number of instances per class largely varies.
...
Figure 6 shows that g(ω τ ) actually increases the gradient scale of tail-classes (classes with fewer instances), so that we obtain the big improvements on those tail classes.
...
Figure 7 further shows that our balancing variables g(ω τ ) and f (γ τ ), that are responsible for handling class/task imbalance, more sensitively react to the actual imbalance conditions with the Bayesian modeling (Bayesian TAML) than without Bayesian (Deterministic TAML).
...
The metalearning rate β and the total number of iterations are set to 1e-3 and 60000, respectively for all models, and the inner gradient step size α is set to 0.05 for MAML, MT-NET, and ABML and is set to learnable parameters for Meta-SGD and our model.
...
We set the number of query points as 15 and the meta learning rate β is set to 1e-4.
...
A function f operating on a set X ∈ X is a valid set function (i.e. permutation invariant), iff it can be decomposed as f (X) = ρ 2 ( x∈X ρ 1 (x)), where ρ 1 and ρ 2 are appropriate nonlinearities.
...
, s C }) = ρ 2 ( C c=1 ρ 1 (s c )) with appropriate nonlinearities ρ 1 and ρ 2 .
...
) with appropriate nonlinearities ρ 3 and ρ 4 .
...
Let F denote the composite of f and (multiple) g and let NN 2 denote the composite of ρ 1 and ρ 4 .

Further define NN 1 := ρ 3 and NN 3 := ρ 2 .
...
Table 9 : Classification results to θ0 inititalization with g(z).

θ 0 inititalization with g(z).

Applying g(z) to the shared initial parameter θ may set some of the initial parameters to smaller values.

Thus, we further compare our model with applying the linear interpolation between θ and a random initialization vector θ random with coefficient g(z) as θ 0 = g(z)

• θ + (1 − g(z)

• θ random ) to Bayesian TAML (Bayesian g(z) TAML + Interpolation).
