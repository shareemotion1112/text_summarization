...
where A k (n) is the time-varying amplitude of the k-th sinusoidal component and φ k (n) is its instantaneous phase.

The phase φ k (n) is obtained by integrating the instantaneous frequency f k (n):

where φ 0,k is the initial phase that can be randomized, fixed, or learned.
...
Given the original and synthesized audio, we compute their (magnitude) spectrogram S i andŜ i , respectively, with a given FFT size i, and define the loss as the sum of the L1 difference between S i andŜ i as well as the L1 difference between log S i and logŜ i .

where α is a weighting term set to 1.0 in our experiments.
