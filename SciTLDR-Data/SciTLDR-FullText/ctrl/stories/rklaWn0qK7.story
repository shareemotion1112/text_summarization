...
Formally it is a function Ψ : DISPLAYFORM0 where T is a constant update matrix and c is a constant vector.

For each iterator Ψ there may be special vectors u * ∈ R n 2 that satisfy u * = Ψ(u * ).

These vectors are called fixed points.

The iterative solver Ψ should map any initial u 0 ∈ R n 2 to a correct solution of the PDE problem.

This is formalized in the following theorem.

Definition 1 (Valid Iterator).

An iterator Ψ is valid w.r.t.

a PDE problem (A, G, f, b, n) if it satisfies: a) Convergence: There is a unique fixed point u * such that Ψ converges to u * from any initialization: DISPLAYFORM1 The fixed point u * is the solution to the linear system Au = f under boundary condition (G, b).
...
To satisfy Au = f a standard approach is to design Ψ by matrix splitting: BID11 .
...
For Poisson equations, this update rule has the following form,û DISPLAYFORM0 For Poisson equations and any geometry G, the update matrix T = G(I − A) has spectral radius ρ(T ) < 1 (see Appendix B).
...
In this paper, we fix A but vary G, f, b, n, and learn an iterator that solves a class of PDE problems governed by the same A. For a discretized PDE problem (A, G, f, b, n) and given a standard (hand designed) iterative solver Ψ, our goal is to improve upon Ψ and learn a solver Φ that has (1) correct fixed point and (2) fast convergence (on average) on the class of problems of interest.

We will proceed to parameterize a family of Φ that satisfies (1) by design, and achieve (2) by optimization.

In practice, we can only train Φ on a small number of problems (A, f i , G i , b i , n i ).

To be useful, Φ must deliver good performance on every choice of G, f, b, and different grid sizes n.
...
We will use more formal notation Ψ(u; G, f, b, n) as Ψ is a function of u, but also depends on G, f, b, n. Our assumption is that for any choice of G, f, b, n (but fixed PDE class A), Ψ(u; G, f, b, n) is valid.
...
The term GHw can be interpreted as a correction term to Ψ(u; G, f, b, n).

When there is no confusion, we neglect the dependence on G, f, b, n and denote as Ψ(u) and Φ H (u).Φ H should have similar computation complexity as Ψ. Therefore, we choose H to be a convolutional operator, which can be parameterized by a deep linear convolutional network.
...
The correct PDE solution is a fixed point of Φ H by the following lemma: Lemma 1.

For any PDE problem (A, G, f, b, n) and choice of H, if u * is a fixed point of Ψ, it is a fixed point of Φ H in Eq. (10).Proof.

Based on the iterative rule in Eq. (10), if u * satisfies Ψ(u DISPLAYFORM0 Moreover, the space of Φ H subsumes the standard solver Ψ. If H = 0, then Φ H = Ψ. Furthermore, denote Ψ(u) = T u + c, then if H = T , then since GT = T (see Eq. (7) ), DISPLAYFORM1 which is equal to two iterations of Ψ. Computing Ψ requires one convolution T , while computing Φ H requires two convolutions: T and H. Therefore, if we choose H = T , then Φ H computes two iterations of Ψ with two convolutions: it is at least as efficient as the standard solver Ψ.

We train our iterator Φ H (u; G, f, b, n) to converge quickly to the ground truth solution on a set DISPLAYFORM0 of problem instances.
...
To simplify the statement of the theorem, for any linear iterator Φ(u) = T u + c we will refer to the spectral radius (norm) of Φ as the spectral radius (norm) of T .

Theorem 2.

For fixed G, f, b, n, the spectral norm of Φ H (u; G, f, b, n) is a convex function of H, and the set of H such that the spectral norm of Φ H (u; G, f, b, n) < 1 is a convex open set.
...
For fixed A, G, n and fixed H, if for some f 0 , b 0 , Φ H (u; G, f 0 , b 0 , n) is valid for the PDE problem (A, G, f 0 , b 0 , n), then for all f and b, the iterator Φ H (u; G, f, b, n) is valid for the PDE problem (A, G, f, b, n).
...
This is because according to Lemma 1, fixed points of our new iterator is the same as the fixed point of hand designed iterator Ψ.

Therefore if our iterator is convergent, it is valid.

What is H trying to approximate?

In this section we show that we are training our linear function GH to approximate T (I − T ) −1 : if it were able to approximate T (I − T ) −1 perfectly, our iterator Φ H will converge to the correct solution in a single iteration.

Let the original update rule be Ψ(u) = T u + c, and the unknown ground truth solution be u * satisfying u * = T u * + c. Let r = u * − u be the current error, and e = u * − Ψ(u) be the new error after applying one step of Ψ. They are related by DISPLAYFORM0 In addition, let w = Ψ(u) − u be the update Ψ makes.

This is related to the current error r by DISPLAYFORM1 From Eq. (10) we can observe that the linear operator GH takes as input Ψ's update w, and tries to approximate the error e: GHw ≈

e. If the approximation were perfect: GHw = e, the iterator Φ H would converge in a single iteration.

Therefore, we are trying to find some linear operator R, such that Rw = e. In fact, if we combine Eq. FORMULA4 and Eq. FORMULA4 , we can observe that T (I − T ) −1 is (uniquely) the linear operator we are looking for DISPLAYFORM2 where (I − T ) −1 exists because ρ(T ) < 1, so all eigenvalues of I − T must be strictly positive.
...
We select the Jacobi method as our standard solver Ψ.To reemphasize, our goal is to train a model on simple domains where the ground truth solutions can be easily obtained, and then evaluate its performance on different geometries and boundary conditions.
...
As discussed in Section 2.4, the convergence rate of any linear iterator can be determined from the spectral radius ρ(T ), which provides guarantees on convergence and convergence rate.
...
For a linear iterator Ψ(u) = T u + c, Ψ converges to a unique stable fixed point from any initialization if and only if the spectral radius ρ(T ) < 1.Proof.

Suppose ρ(T ) < 1, then (I−T ) −1 must exist because all eigenvalues of I−T must be strictly positive.

Let u * = (I − T ) −1 c; this u * is a stationary point of the iterator Ψ, i.e. u * = T u * + c. For DISPLAYFORM0 Since ρ(T ) < 1, we know BID11 , which means the error e k → 0.

Therefore, Ψ converges to u * from any u 0 .

DISPLAYFORM1 Now suppose ρ(T ) ≥ 1.

Let λ 1 be the largest absolute eigenvalue where ρ(T ) = |λ 1 | ≥ 1, and v 1 be its corresponding eigenvector.

We select initialization u 0 = u * + v 1 , then e 0 = v 1 .

Because |λ 1 | ≥ 1, we have |λ k 1 | ≥ 1, then T k e 0 = λ k 1 v 1 → k→∞ 0 However we know that under a different initializationû 0 = u * , we haveê 0 = 0, so T kê0 = 0.
...
As before, denote Ψ(u) = T u + c. Observe that Φ H (u; G, f, b, n) = T u + c + GH(T u + c − u) = (T + GHT − GH)u + GHc + cThe spectral norm · 2 is convex with respect to its argument, and (T + GHT − GH) is linear in H. Thus, T + GHT − GH 2 is convex in H as well.

Thus, under the condition that T + GHT − GH 2 < 1, the set of H must be convex because it is a sub-level set of the convex function T + GHT − GH 2 .To prove that it is open, observe that · 2 is a continuous function, so T + GHT − GH 2 is a continuous map from H to the spectral radius of Φ H .
...
For fixed A, G, n and fixed H, if for some f 0 , b 0 , Φ H (u; G, f 0 , b 0 , n) is valid for the PDE problem (A, G, f 0 , b 0 , n), then for all f and b, the iterator Φ H (u; G, f, b, n) is valid for the PDE problem (A, G, f, b, n).Proof.

From Theorem 1 and Lemma 1, our iterator is valid if and only if ρ(T + GHT − GH) < 1.
...
We now formally prove that ρ(G(I − A)) < 1 for any G.For any matrix T , the spectral radius is bounded by the spectral norm: ρ(T ) ≤ T 2 , and the equality holds if T is symmetric.

Since (I − A) is a symmetric matrix, ρ(I − A) = I − A 2 .

It has been proven that ρ(I − A) < 1 FIG4 .

Moreover, G 2 = 1.

Finally, matrix norms are sub-multiplicative, so DISPLAYFORM0 ρ(T ) < 1 is true for any G. Thus, the standard Jacobi method is valid for the Poisson equation under any geometry.
