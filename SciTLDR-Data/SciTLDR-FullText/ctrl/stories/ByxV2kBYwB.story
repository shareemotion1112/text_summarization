...
Compressing one cumbersome or several models into a smaller model is a classic idea (Domingos, 1997; Buciluǎ et al., 2006) and has been popularized by the distillation formulation of Hinton et al. (2015) .
...
Each example is fed to an embedding function f θ with learnable parameters θ.

The prototype of a class c is the mean of the output through f θ of examples from c in S train .

We can then feed the examples in S test to the classifier and update the parameters θ.
...
While a naïve approach would be to directly characterize the difference between their model parameters, it turns out to be challenging due to the high dimensionality of the parameter space.
...
σ denotes the softmax function, L CE denotes the cross-entropy loss, e y is the one-hot encoding of y, and γ is a trade-off hyper-parameter that balances the two terms.
...
The embedding function f θ of our cosine classifier has a different architecture depending on the pre-trained features used.
...
We initialize the temperature to 7, the scale factor of the cosine distance to 150, and γ to 5.
