...
DISPLAYFORM0 Forward Propagation:Ŷ = f (X; θ) DISPLAYFORM1 Compute loss function: L(Ŷ , Y )
...
Given a data distribution D with inputs X ∈ R d and corresponding labels Y , a classification/discriminative algorithm models the conditional distribution p(Y |X; θ) by learning the parameters θ.
...
An adversarially robust model should, thus, model p(Y |X, A; θ), where A represents the adversarial inputs.
...
For parameters , α (α = /2), the attack is defined as: DISPLAYFORM1 This method iteratively applies FGSM k times with a step size of β ≥ /k and projects each step perturbation to be bounded by .
...
For CIFAR10, CIFAR100 we used additional weight decay regularization, λ = 5e − 4.
...
Hence, the learning rate for noise (η noise ) was kept 1-2 orders of magnitude lesser than the overall network learning rate (η) throughout the training process.
...
We use mini-batch SGD with momentum of 0.5, learning rate (η=0.1) decayed by 0.1 every 15 epochs and batchsize 64 to learn the network parameters.

We trained 3 ConvNet models independently corresponding to each scenario for 30 epochs.

For the ExL scenarios, we conduct noise modelling with only negative loss gradients (∇LN ≤ 0) with noise learning rate, ηnoise = 0.001, throughout the training process.
...
We use mini-batch SGD with momentum of 0.9, weight decay 5e-4, learning rate (η=0.01) decayed by 0.2 every 10 epochs and batch-size 64 to learn the network parameters.

We trained 4 ConvNet models independently corresponding to each scenario for 30 epochs.

For the ExL scenarios, we conduct noise modelling by backpropagating the corresponding gradient with noise learning rate (ηnoise = 0.001) throughout the training process.
...
We use mini-batch SGD with momentum of 0.9, weight decay 5e-4, learning rate (η=0.1) decayed by 0.1 every 30 epochs and batch-size 64 to learn the network parameters.

We trained 2 ResNet-18 models independently corresponding to each scenario for 60 epochs.

For noise modelling, we use ηnoise = 0.001 decayed by 0.1 every 30 epochs.
...
Instead of using a weighted loss function that controls the relative weight of adversarial/clean examples in the overall loss computation, we use a different learning rate η adv /η (η adv < η) when training with adversarial/clean inputs, respectively, to learn the network parameters.

Accordingly, while performing adversarial training with explainable learning (ExL ens ), the noise modeling learning rate in addition to overall learning rate, η adv /η, for adversarial/clean inputs is also different, η noiseadv /η noise (η noiseadv < η noise ).How to conduct PGD Adversarial Training?
...
Like EnsAdv training, here as well, we use a different learning rate η adv /η (η adv < η) when training with adversarial/clean inputs, respectively, to learn the network parameters.
