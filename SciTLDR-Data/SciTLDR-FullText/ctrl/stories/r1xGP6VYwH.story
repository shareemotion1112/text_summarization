...
We consider a Markov Decision Process (MDP) defined as a tuple (S, A, P, R), where S is the state space, A is the discrete action space, P (·|s, a) is the state-transition distribution, R(·|s, a) is the distribution over rewards and γ ∈ [0, 1) is the discount factor.

The goal of the agent is then to maximise the expected discounted sum of rewards: E[ ∞ t=0 γ t r t |r t ∼ R(·|s t , a t )], in the discounted episodic setting.

A policy π(·|s) is a mapping from states to actions such that it is a valid probability distribution.

Deep Q-Network (DQN) (Mnih et al., 2015) uses a nonlinear function approximator (a deep neural network) to estimate the action-value function, Q(s, a; θ) ≈ Q * (s, a), where θ are the parameters of the network.

Exploration based on intrinsic rewards (e.g., Bellemare et al., 2016) , which uses a DQN agent, additionally augments the observed rewards r t with a bonus β/ N (s t , a t ) based on pseudo-visitation-counts N (s t , a t ).

The DQN parameters θ are trained by gradient descent on the mean squared regression loss L with bootstrapped 'target' y t :
...
To improve stability, DQN uses a target network, parameterised by θ − , which is periodically copied from the regular network and kept fixed for a number of iterations.
...
Our motivation for the form of the bonus in equation 1, C (N (s,a)+1) M , stems from UCB-H (Jin et al., 2018) , where all tabular Q-values are initialised with H and the first update for a state-action pair completely overwrites that value because the learning rate for the update (η 1 ) is 1.
...
In order to then approximate an optimistic initialisation, the scaling parameter C in equation 1 can be chosen to guarantee unseen Q + -values are overestimated, for example, C := H in the undiscounted finite-horizon tabular setting and C := 1/(1 − γ) in the discounted episodic setting (assuming 1 is the maximum reward obtainable at each timestep).
...
For deep RL, we base OPIQ on DQN (Mnih et al., 2015) , which uses a deep neural network with parameters θ as a function approximator Q θ .
...
DQN + PC: we add an intrinsic reward of β/ N (s, a) to the environmental reward based on (Bellemare et al., 2016; Tang et al., 2017) .
...
Our goal is to find a set of policies π t : S → A, π := {π t }, that chooses the agent's actions at time t such that the expected sum of future rewards is maximised.

To this end we define the Q-value at time t of a given policy π as Q π t (s, a) := E r + Q π t+1 (s , π t+1 (s )) | r∼Rt(·|s,a), s ∼Pt(·|s,a) , where Q π t (s, a) = 0, ∀t > H. The agent interacts with the environment for K episodes, T := KH, yielding a total regret:

refers to the starting state and π k to the policy at the beginning of episode k. We are interested in bounding the worst case total regret with probability 1 − p, 0 < p < 1.
...
All Q-values for timesteps t ≤ H are optimistically initialised at H. The learning rate is defined as η N = H+1 H+N , where N := N (s t , a t , t) is the number of times state-action pair (s t , a t ) has been observed at step t and η 1 = 1 at the first encounter of any state-action pair.
...
In particular, for the chain and maze environments we use static hashing (Tang et al., 2017) , which projects a state s to a low-dimensional feature vector φ(s) = sign(Af (s)), where f flattens the state s into a single dimension of length D; A is a k × D matrix whose entries are initialised i.i.d.
...
Given the vector φ(s), we use a counting bloom filter (Fan et al., 2000) to update and retrieve its counts efficiently.

To obtain counts N (s, a) for state-action pairs, we maintain a separate data structure of counts for each action (the same vector φ(s) is used for all actions).
...
In all experiments we set γ = 0.99, use RMSProp with a learning rate of 0.0005 and scale the gradient norms during training to be at most 5.
...
We set β = 0.1 for the scale of the count-based intrinsic motivation.
...
For DQN + PC we consider β ∈ {0.01, 0.1, 1, 10, 100}. For all other methods we set β = 0.1 as it performed best.
...
We scale the squared error by β rnd ∈ {0.001, 0.01, 0.1, 1, 10, 100}:
...
We sweep across the scale of the intrinsic reward β dora ∈ {}.

For DQN + DORA OA we use β dora = and sweep across β dora_action ∈ {}.

For BSP we use the following architecture: We use K = 10 different bootstrapped DQN heads, and sweep over β bsp ∈ {0.1, 1, 3, 10, 30, 100}.
...
For all methods we consider β mmc ∈ {0.005, 0.01, 0.025}.

For DQN + PC we consider β ∈ {0.01, 0.1, 1}.

For OPIQ and its ablations we set M = 2.

We consider: C action ∈ {0.1, 1}, C bootstrap ∈ {0.01, 0.1}, β ∈ {0.01, 0.1}.
...
We scale the squared error by β rnd ∈ {0.001, 0.01, 0.1, 1}:

For BSP we use the same architecture as in (Osband et al., 2018) .

We use K = 10 different bootstrapped DQN heads, and sweep over β bsp ∈ {0.1, 1, 10, 100}.
...
The best hyperparameters we found were: OPIQ without Optimistic Bootstrapping: M= 2, C action = 0.1, β mmc = 0.005.
...
The intrinsic reward used for a state s, is the squared error between the predictor network and the target network β rnd ||predictor(s) − target(s)|| 2 2 .

DQN + DORA:

We train the E-values network using n-step SARSA (same n as the DQN) with γ E = 0.99.
...
The output of each head is Q k + β bsp p k , where p k is a randomly initialised network (of the same architecture as Q k ) which is kept fixed throughout training.

β bsp is a hyperparameter governing the scale of the prior regularisation.
...
This will happen since our Q + -value estimate for the left action will eventually drop below the Q-value estimate for the right action which is a/λ > 0.
...
This then provides a very loose upper bound for R as ( λ a ) 1/M , which then leads to a further underestimation of the probability of failure.

Assume for a contradiction that (1 − a) R < p:

This provides our contradiction as we choose λ s.t M > log(λ/a)/ log(log(p)/ log(1 − a)).

We can always pick such a λ because log(λ/a) can get arbitrarily close to 0.
...
Let Z 0 , ..., Z n be a martingale sequence of random variables such that ∀i∃c i : |Z i − Z i−1 | < c i almost surely, then with probability at least 1 − δ:

(1 − η j ) The following properties hold for η i N :
...
and, with probability at least 1 − δ, the following holds simultaneously for all (s, a, t, k) where N = N (s, a, t) and k 1 , ..., k N < k are the episodes where (s, a) was taken at step t.
...
Let F i be the σ-field generated by all the random variables until episode k i step t.

Then by Azuma's Inequality we have that with probability at least 1 − 2δ/(SAHK)

Then by a Union bound over all τ ∈ [K], we have that with probability at least 1 − 2δ/(SAH):

Since From Lemma 1 we have that

Since inequality equation 9 holds for all fixed τ ∈ [K] uniformly, it also holds for a random variable τ τ = N = N k (s, a, t) ≤ K. Also note that 1[k i ≤ K] = 1 for all i ≤ N .

We can then additionally apply a union bound over all s ∈ S, a ∈ A, t ∈ [H] to give:

which holds with probability 1 − 2δ for all (s, a, t, k) ∈ S × A × [H] × [K].

We then rescale δ to δ/2.
