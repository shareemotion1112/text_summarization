...
λ 1 , λ 2 , λ 3 , and λ 4 are hyperparameters controlling the importance of additional losses.
...
The main module is trained for 600 epochs on the CUB dataset and 120 epochs on the COCO dataset using the Adam optimiser (Kingma & Ba, 2014) with the learning rate 0.0002, and β 1 = 0.5, β 2 = 0.999.
...
The hyperparameters λ 1 , λ 2 , λ 3 , and λ 4 are set to 1, 5, 0.5, and 1 for the CUB dataset, and 15, 5, 0.5, and 1 for COCO, respectively.

First, the visual features v are converted into the same size as the hidden features h last via a convolutional layer F , denotedṽ = F v , whereṽ ∈ R 128×H ×D .
...
Next, to incorporate the visual featuresṽ , we adopt the co-attention module here, donatedã = a W (ṽ ) + b (ṽ ), where W and b are learned weights and bias dependent on visual featuresṽ .

Then, the transformed featuresã are fed into a series of residual blocks followed by a convolutional layer to generate hidden features e.
