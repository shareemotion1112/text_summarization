...
Moreover, the variable τ s in Eq. (1) is introduced by BEGAN to balance the power of D(·) and G(·) during the learning process.

It is dynamically set to be

for each training step s, with τ s ∈ [0, 1].

λ and γ are manually-set hyperparameters.
...
As the harmonization model considers symbolic sequences, we use CREPE to transcribe the generated voices, and Madmom (Böck et al., 2016) to recognize the chord sequence from the accompaniment track.
...
However, existing work is mostly concerned with the generation of single notes of, for example, 4-second long (Défossez et al., 2018; Engel et al., 2019) .
