...
Let the likelihood function be approximated by a ReLU feedforward neural network (final layer is followed by softmax) with weights θ, that is, given an example x, the network produces a set of probabilities

, where M is the number of classes.

For notational simplicity, we denote P θ (y = j|x) as P j θ (·|x).

If the ground truth for x is g, where (1 ≤ g ≤ M ), then we use the shorthand for the predicted likelihood of g as P θ (·|x) := P g θ (·|x).

For any task i, minimizing its specific cross entropy loss L i (θ) achieves the best performance for task i, which can be written as:
...
Bayesian continual learning formulates learning a new task as trying to maximize a posterior p(θ|D 1:i ) given a prior p(θ|D 1:i−1 ).

So, if the weights θ at the end of task i − 1 and i are denoted by θ * 1:i−1 and θ * 1:i respectively, then the prior and the posterior can be thought of as the predicted likelihood distributions represented by the neural network at θ * 1:i−1 and θ * 1:i .

For every task i, the Bayesian formulation tries to minimize L i (θ) as well as the dissimilarity between the prior and the posterior.

EWC uses the KL-divergence of the two predicted likelihood distributions as the dissimilarity metric.

Assuming the difference between θ * 1:i−1 and θ * 1:i is small, the second order Taylor approximation of the KL-divergence produces (with a further diagonal approximation of the Fisher matrix):

For any task i ≥ 2, EWC minimizes the sum of L i (θ) and this approximation (multiplied by a λ ≥ 0).

λ acts as a hyperparameter that controls the weight of the penalty for a specific learned task.

It is typically kept the same for all learned tasks.

After learning a task i, the weights of the network are θ * 1:i .

For simplicity, let θ * 1:i ≡ θ * and that afterwards, at any point in the sequential training process, the weights are at θ * + ∆θ.

Assuming ∆θ is small, we can apply a first order Taylor approximation of the individual predicted likelihood P j θ * +∆θ in the neighbourhood of θ = θ * :

The individual predicted likelihood P j θ on an example x ∈ D i changes by the magnitude |P j θ * +∆θ (·|x) − P j θ * (·|x)|.

The average magnitude change in P j θ over the dataset D i is given by the expectation:
...
Case I. We can preserve the entire set of predicted likelihoods from θ * to θ * + ∆θ, which would penalize changes to any individual predicted likelihood.
...
We can preserve the change in predicted likelihood for the predicted label at θ * , which corresponds to the highest individual probability in {P
...
To achieve this, we can use the expectation over (P j θ * +∆θ (·|x)−P j θ * (·|x))·P j θ * (·|x) rather than the original formulation in (1) and then regularize a sum over j = 1 to M like in Case I. In most cases, this term would evaluate to the difference in the individual predicted likelihoods for the predicted label at θ * (since the probabilities are output by a softmax layer).

Case III.

We can preserve the change in predicted likelihood for the ground truth by computing the expectation for P θ * +∆θ (·|x) − P θ * (·|x) .

Case IV.

We can partially preserve the change in predicted likelihood for the ground truth, that is, penalize the change P θ * (·|x) = 1 → P θ * +∆θ (·|x) = 0 but allow the change P θ * (·|x) = 0 → P θ * +∆θ (·|x) = 1 for the ground truth predicted likelihood.

This applies the penalty only when a correctly classified x at θ * becomes incorrectly classified at θ * + ∆θ.

The expectation is then computed over (P θ * +∆θ (·|x) − P θ * (·|x)) · P θ * (·|x) , similar to Case II.

In all of these cases, we end up with a direct minimization criterion that can be minimized just like in EWC.

In fact, the quadratic loss penalty proposed in Kirkpatrick et al. (2016) , which was later corrected in Huszár (2018) to more appropriately represent Bayesian updating, can be interpreted as the upper-bound of the squared L 2 version of the change in predicted likelihood as described in Case III.

Intuitively, therefore, the quadratic loss penalty works even when not computed as specified in Huszár (2018) because it penalizes the upper bound of the squared L 2 change in likelihood for task i.
...
L 1 regularization is known to produce sparser solutions than L 2 regularization above a critical λ, that is, requiring fewer non zero weights, in the context of plain weight regularization (Moore & DeNero, 2011) .
...
For notational simplicity, let us denote the expectation in (2) as G(θ * 1:k , D k ).

After task i has been learned, the absolute change in predicted likelihood (for the use case) is upper bounded by |θ − θ * 1:i |·G(θ * 1:i , D i ).
...
The upper bound of change in classifier likelihood for a dataset D i is dependent on two terms (see (1)), |∆θ| and the expectation of the absolute gradients.

To minimize the change in classifier likelihood, we may opt to minimize |∆θ| more conventionally, by freezing the most important weights.

This reduces the magnitude of ∆θ and therefore results in a lesser change in likelihood.
...
Training is done on feedforward ReLU networks for each strategy with 2 hidden layers (h = 128, η = 0.0001) for 20 epochs.
...
As can be seen in Table 1 , EWC often requires a high λ to remember previous tasks better.

In contrast, the L 1 methods perform well even with a small λ .

This can be explained by the fact that minimization with an L 2 method contains a (|∆θ|) 2 term instead of (|∆θ|).

This means that the weights (which are typically quite small) are squared in the L 2 methods, which then requires a stronger λ to compensate for the squaring.
...
We speculate that this is because of the λ chosen for S-MNIST and Sim-EMNIST during the hyperparameter search.

During the search, λ is optimized for the best accuracy.

In order for EWC to have the best accuracy for these datasets (S-MNIST, Sim-EMNIST), the required hyperparameter λ is huge (10 4 ), which leads to an over-preservation of past classifier likelihood at the expense of the learning the likelihood for the newest task, while the proposed DM strategies use a normal λ for their corresponding best performance.

In fact, the huge λ leads to sub-optimal performance for the newest task in EWC, but maximizes the average final accuracy.
