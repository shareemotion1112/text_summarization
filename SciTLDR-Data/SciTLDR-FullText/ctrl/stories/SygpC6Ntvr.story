...
There has also been substantial interest in speeding up SpMV computations using specialized hardware such as GPUs (Vazquez et al., 2010; Vázquez et al., 2011) , FPGAs (Zhuo and Prasanna, 2005; Zhang et al., 2009) , and custom hardware (Prasanna and Morris, 2007) .
...
Let F = {f θ : X → R d | θ ∈ Θ} be a class of functions parameterized by θ ∈ Θ, mapping input instances to d-dimensional embeddings.

Typically, for image tasks, the function is chosen to be a suitable CNN (Krizhevsky et al., 2012) .

Suppose X, Y ∼ P, then define the activation probability p j = P(f θ (X) j = 0), and its empirical versionp j =

We now show that sparse embeddings can lead to a quadratic speedup.

Consider a d-dimensional sparse query vector u q = f θ (x q ) ∈ R d and a database of n sparse vectors
...
2 (so that as a regularizer, F(f θ , P) will in turn encourage such a uniform distribution across dimensions).
...
The FLOPs F(f θ , P) being a discontinuous function of model parameters, is hard to optimize, and hence we will instead optimize using a continuous relaxation of it.

Denote by (f θ , D), any metric loss on D for the embedding function f θ .

The goal in this paper is to minimize the loss while controlling the expected FLOPs F(f θ , P) defined in Eqn.

2.

Since the distribution P is unknown, we use the samples to get an estimate of F(f θ , P).

Recall the empirical fraction of non-zero

, which converges in probability to p j .

Therefore, a consistent estimator for F(f θ , P) based on the samples D is given by F(f θ , D) = d j=1p 2 j .
...
for some parameter λ that controls the FLOPs-accuracy tradeoff.

The regularized loss poses a further hurdle, asp j and consequently F(f θ , D) are not continuous due the presence of the indicator functions.

We thus compute the following continuous relaxation.

Define the mean absolute activation a j = E[|f θ (X) j |] and its empirical versionā j = 1 n n i=1 |f θ (x i ) j |, which is the 1 norm of the activations (scaled by 1/n) in contrast to the 0 quasi norm in the FLOPs calculation.

Define the relaxations, F(f θ , P) = .
...
, ρ is the ReLU activation, and where we suppress the dependency of µ j and σ j on P. We experimentally verify that this assumption holds by minimizing the KS distance (Massey Jr, 1951) between the CDF of ρ(X) with X ∼ N (µ, σ 2 ) and the empirical CDF of the activations, with respect to µ, σ.

Figure 2a shows the empirical CDF and the fitted CDF of ρ(X) for two different architectures.

While µ j , σ j cannot be tuned independently for j ∈ [d] due to their dependence on θ, consider a further simplification where the parameters are independent of each other.

Suppose for j ∈ {1, 2}, f θ (X) j = ReLU(X) where X ∼ N (µ j , σ 2 j ), and θ = (µ 1 , µ 2 , σ 1 , σ 2 ).

We analyze how minimizing F(f θ , P) compares to minimizing F(f θ , P).
...
We initialize with θ = (µ 1 , µ 2 , σ 1 , σ 2 ) = (−1/4, −1.3, 1, 1), and minimize the three quantities w.r.t.

θ via gradient descent with infinitesimally small learning rates.

For this contrastive analysis, we do not consider the effect of the metric loss.

Note that while the empirical quantity F(f θ , D) cannot be optimized via gradient descent, it is possible to do so for its population counterpart F(f θ , P) since it is available in closed form when making Gaussian assumptions.
...
We next show that when the embeddings are normalized to have a unit norm, as typically done in metric learning, then minimizing F(f θ , D) is equivalent to promoting orthogonality on the absolute values of the embedding vectors.

Let f θ (x) 2 = 1, ∀x ∈ X , we then have the following:

is minimized when the vectors {|f θ (x i )|} n i=1 are orthogonal.
...
Then p,q∈ [1:d] |v p |, |v q | is minimized when v p = e p , where e p is an one-hot vector with the p th entry equal to 1 and the rest Figure (a) shows that the CDF of the activations (red) closely resembles the CDF of ρ(X) (blue) where X is a Gaussian random variable.
...
This simple example also demonstrates that when the number of classes in the training set is smaller or equal to the number of dimensions d, a trivial embedding that minimizes the metric loss and also achieves a small number of FLOPs is f θ (x) = e y where y is true label for x. This is equivalent to predicting the class of the input instance.
...
More specifically, ReLU(x − λ) can be made more (less) sparse by increasing (decreasing) the components of λ, where λ is the bias parameter of the previous linear layer.
...
In practice, setting a large regularization weight λ from the beginning is harmful for training.

Sparsifying too quickly using a large λ leads to many dead activations (saturated to zero) in the embedding layer and the model getting stuck in a local minima.

Therefore, we use an annealing procedure and gradually increase λ throughout the training using a regularization weight schedule λ(t) : N → R that maps the training step to a real valued regularization weight.

In our experiments we choose a λ(t) that increases quadratically as λ(t) = λ(t/T ) 2 , until step t = T , where T is the threshold step beyond which λ(t) = λ.
...
We also report the sub-optimality ratio R sub = F(f θ , D)/dp 2 computed over the dataset D, wherē p = 1 d d j=1p j is the mean activation probability estimated on the test data.
