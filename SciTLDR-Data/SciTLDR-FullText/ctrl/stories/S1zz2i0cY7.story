...
The corresponding scaled cumulative of a generalized Gaussian with β = 4 used for computing gradients is plotted in cyan, and other choices of β in gray.
...
As a remedy, we replace the gradient instead with DISPLAYFORM7 where DISPLAYFORM8 β , and L is the bit width of w. This function corresponds to a scaled generalized Gaussian probability density with shape parameter β.

In this context, we can think of β as a temperature parameter that makes the function converge to the gradient of the clipping operation as β goes to infinity.

Although this setting permits an annealing schedule, we simply chose β = 4 and obtained good results.

The integral of this function is plotted in figure 2 (left) in cyan, along with other choices of β in gray.
...
After training, we precompute all possible values of p as a function of y i and θ i and form a lookup table, while g is implemented with integer arithmetic.
...
To use integer networks in the encoder or decoder, one can use the equivalent construction as in FORMULA0 Jang et al. FORMULA0 and Ágústsson et al. (2017) are concerned with producing gradients for categorical distributions and vector quantization (VQ), respectively.
...
These two approaches are particularly interesting for image compression, as they scale well: Image compression models are often trained with a rate-distortion objective with a Lagrange parameter λ, equivalent to β in the β-VAE objective BID10 BID1 .
...
We found that when implementing the models described in Ballé (2018) using integer networks throughout, the rate-distortion performance decreased (figure 3, right).
...
The lower bound is given by the cross entropy between the marginal and the prior: DISPLAYFORM0 where |b(y)| is the length of the binary representation of y. If an entropy coding algorithm tolerates error in the values of p(y | θ), this means it must operate under the assumption of identical probability values for a range of values of θ -in other words, discretize the probability values.

Since the cross entropy is minimal only for p(y | θ) = m(y) (for all y), this would impose a new lower bound on |b(y)| given by the cross entropy with the discretized probabilities, which is greater or equal to the cross entropy given above.
