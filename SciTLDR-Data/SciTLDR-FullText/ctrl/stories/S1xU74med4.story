...
Let h 0 = x and ϕ(·) = max(0, ·) be the ReLU activation function.
...
We derived the dynamics of the angle through layers (Table 2) by applying the recurrence relation of the angle TAB0 iteratively and using the fact that, if θ is small, arccos(ψ(θ)) can be well approximated by the linear function, a · θ where a < 1 is constant.
...
We can show that the cosine similarity c l+1 (n, m) is proportional to DISPLAYFORM0 where θ is a parameter we can control by training.

Its plot FIG1 implies that training makes small angles smaller and large angles larger by taking the extreme value of θ like 0 or π.
...
The angle of feature vectors at high layer of the initialized MLP is small, which implies that training doesn't take extreme value of θ and doesn't separate points from different classes.

On the other hand, the skip-connection and the BN preserve the angle between input vectors even at high layer.

Thus, training takes extreme value of θ and separates points from different classes.
