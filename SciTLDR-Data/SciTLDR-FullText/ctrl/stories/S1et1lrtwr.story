...
Our method can also be thought of as automatically acquiring an environment-specific learning procedure for deep neural network policies, somewhat related to data-driven initialization procedures explored in supervised learning (Krähenbühl et al., 2015; Hsu et al., 2018) .
...
To talk about environments on which many tasks might be performed, we consider a controlled Markov process (CMP) -a Markov decision process without a reward function, C = (S, A, P, γ, ρ), with state space S, action space A, transition dynamics P , discount factor γ and initial state distribution ρ.

The CMP along with this reward function r z produces a Markov decision processes M i = (S, A, P, γ, ρ, r z ).

The goal of the learning algorithm f is to learn an optimal policy π * i (a | s) for any reward function r z that is provided with the CMP.
...
In this goal-reaching setting, we consider episodes with finite horizon T and a discount factor of γ = 1.
...
We define ρ T π (s) as the probability that policy π visits state s at time step t = T .

If s g is the true goal, then the event that the policy π reaches s g at the final step of an episode is a Bernoulli random variable with parameter p = ρ T π (s g ).

Thus, the expected hitting time of this goal state is HITTINGTIME π (s g ) = 1 ρ T π (s g ) We now define a meta-learner f π in terms of an exploration policy π.

Before the goal is found, f π uses policy π to explore.

Once the goal s g is found, the meta-learner takes actions to always return to state s g .

Meta-learner f π incurs one unit of regret for each step before it has found the goal, and zero regret afterwards.
...
the marginal distribution ρ T π .

Using the calculus of variations (for more details refer to Appendix C in ), the (exploration) policy for the optimal meta-learner, π * , satisfies:
...
Let π be a policy for which ρ T π (s) is uniform.

Then f π has lowest worst-case regret.
...
where µ denotes a probability distribution over terminal states s T and latent variables z, and P µ is a subset of such distributions.

Observe that this objective contains two competing terms.

is a Dirac distribution, centered at a state s z specified by the latent z. Then any distribution µ that maximizes mutual information I µ (s T ; z)

The key idea is that a joint distribution µ(s T , z) can be defined implicitly via a latent-conditioned policy.

This policy is not a meta-learned model, but rather will become part of the task proposal mechanism.

For a given prior µ(z) and latent-conditioned policy µ(a | s, z), the joint likelihood is µ(τ, z) = µ(z)p(s 1 ) t p(s t+1 | s t , a t )µ(a t | s t , z), and the marginal likelihood is simply given by µ(s T , z) = µ(τ, z)ds 1 a 1 · · · a T −1 .

We define P µ as all distributions µ(s T , z) that can be constructed in such a way using a Markovian policy.

Note that the assumption in Lemma 2 does not always hold, as stochasticity in the environment may mean that µ(s T | z) is never a Dirac distribution, or the existence of unreachable states may mean that µ(s T ) can never be uniform.

Finally, we construct a distribution over tasks from the joint distribution µ. We then define our task proposal distribution by sampling z ∼ p(z) and using the corresponding reward function r z (s T , a T ) log p(s T | z).

This reward function does not depend on the action a T .

In the case when both the prior µ(z) and the marginal µ(s T ) are uniform, this reward function is equivalent to the DIAYN reward function log µ(z | s T ), up to additive constants.
...
Formally, we define a distribution of trajectory-matching tasks by a distribution over goal trajectories, p(τ * ).

For each goal trajectory τ * , the corresponding trajectory-level reward function is
...
However, obtaining such a policy requires knowing the trajectory distribution p(τ ).

In the setting where p(τ ) is unknown, the minimax policy is simply uniform:

Lemma 3.

Let π be a policy for which π(τ ) is uniform.
...
Analogous to Section 3.3, we define a distribution over reward functions as r z (τ ) log p(τ | z).

At optimality, each z corresponds to exactly one trajectory τ z , so the reward function r z (τ ) simply indicates whether τ is equal to τ z .

Since the distribution over trajectories p(τ | z)p(z)dz is uniform at the optimum, the distribution of reward functions r z corresponds to a uniform distribution over trajectories.
...
Let R τ denote the set of trajectory-level reward functions, and R s,a denote the set of all state-action level reward functions.

Bounding the worst-case regret on R τ minimizes an upper bound on the worst-case regret on R s,a :

This inequality holds for all policies π, including the policy that maximizes the LHS.

While we aim to maximize the RHS, we only know how to maximize the LHS, which gives us a lower bound on the RHS.

This inequality holds for all policies π, so it also holds for the policy that maximizes the LHS.
...
Extract corresponding task reward functions rz(s) using D φ (z|s) Update f using MAML with reward rz(s) DIAYN optimizes mutual information by training a discriminator network D φ (z|·) that predicts which z was used to generate the states in a given rollout according to a latent-conditioned policy π(a|s, z).

Our task proposal distribution is thus defined by r z (s, a) = log(D φ (z|s)).

The complete unsupervised meta-learning algorithm follows the recipe in Figure 1 : first, we acquire r z (s, a) by running DIAYN, which learns D φ (z|s) and a latent-conditioned policy π(a|s, z) (which is discarded).
...
In addition to mutual information maximizing task proposals, we will also consider random task proposals, where we also use a discriminator as the reward, according to r(s, z) = log D φrand (z|s), but where the parameters φ rand are chosen randomly (i.e., a random weight initialization for a neural network).
...
Lemma 1 Let π be a policy for which ρ T π (s) is uniform.

Then π has lowest worst-case regret.

Proof of Lemma 1.

To begin, we note that all goal distributions p(s g ) have equal regret for policies where ρ T π (s) = 1/|S| is uniform:

Now, consider a policy π for which ρ T π (s) is not uniform.
...
(11) Thus, a policy π for which ρ T π is non-uniform cannot be minimax, so the optimal policy has a uniform marginal ρ T π .
