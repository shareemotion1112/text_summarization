...
DISPLAYFORM0 whereγ is a K-dimensional vector for the soft mixture-component membership prediction, and p is the output of a multi-layer network parameterized by θ m .
...
DISPLAYFORM1 whereγ i is the membership prediction for the low-dimensional representation z i , andφ k ,μ k ,Σ k are mixture probability, mean, covariance for component k in GMM, respectively.
...
To avoid this issue, we penalize small values on the diagonal entries by DISPLAYFORM2 , where d is the number of dimensions in the low-dimensional representations provided by the compression network.• λ 1 and λ 2 are the meta parameters in DAGMM.

In practice, λ 1 = 0.1 and λ 2 = 0.005 usually render desirable results.
...
For sample x i , the contribution of its compressed representation z i to the energy function can be upper-bounded as follows BID9 ), DISPLAYFORM0 (10) where Q θm (k | z i ) is the estimation network that predicts the membership of z i , KL(·||·) is the Kullback-Leibler divergence between two input distributions, p(k) = φ k is the mixing coefficient to be estimated, and p(k | z i ) is the posterior probability distribution of mixture component k given z i .By minimizing the negative evidence lower bound in Equation (8) , we can make the estimation network approximate the true posterior and tighten the bound of energy function.
...
Moreover, in all the DAGMM instances, we set λ 1 as 0.1 and λ 2 as 0.005.

For readers of interest, we discuss how λ 1 and λ 2 impact DAGMM in Appendix F.For the baseline methods, we conduct exhaustive search to find the optimal meta parameters for them in order to achieve the best performance.
...
Unlike other baselines that only need decision thresholds in the testing phase, OC-SVM needs parameter ν be set in the training phase.

Although ν intuitively means anomaly ratio in training data, it is non-trivial to set a reasonable ν in the case where training data are all normal samples and anomaly ratio in the testing phase could be arbitrary.

In this study, we simply perform exhaustive search to find the optimal ν that renders the highest F 1 score on individual datasets.

In particular, ν is set to be 0.1, 0.02, 0.04, and 0.1 for KDDCUP, Thyroid, Arrhythmia, and KDDCUP-Rev, respectively.
...
The coefficient ratio among the three components can be characterized as 1 : λ 1 : λ 2 .

In terms of λ 1 , a large value could make the loss function of deep autoencoder play little role in optimization so that we are unable to obtain a good reduced representation for input samples, while a small value could lead to ineffective estimation network so that GMM is not well trained.

For λ 2 of a large value, DAGMM tends to find GMM with large covariance, which is less desirable as many samples will have high energy as rare events.

For λ 2 of a small value, the regularization may not be strong enough to counter the singularity effect.
...
For example, when the base is set to 2, λ 1 and λ 2 are adjusted to 0.2 and 0.01, respectively.

TAB6 shows the average precision, recall, and F 1 score after 20 runs of DAGMM on the KDDCUP dataset.

As we vary the base from 1 to 9 with step 2, DAGMM performs in a consistent way, and λ 1 , λ 2 are not sensitive to the changes on the base.
