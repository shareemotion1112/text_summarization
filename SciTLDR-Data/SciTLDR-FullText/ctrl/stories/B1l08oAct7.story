...
A variational approach acknowledges that this posterior generally does not have an analytic form, and introduces a variational distribution q(w; θ) parameterized by θ to approximate p(w|x, y).

The approximation is considered optimal within the variational family for θ * that minimizes the Kullback-Leibler (KL) divergence between q and the true posterior.
...
However, when M is a non-linear neural network, the first term in equation 1 (referred to as the reconstruction term) cannot be computed exactly: this is where MC approximations with finite sample size S are typically employed: DISPLAYFORM1 log p(y|w (s) , x), w (s) ∼ q(w; θ).Our goal in the next section is to develop an explicit and accurate approximation for this expectation, which provides a deterministic, closed-form expectation calculation, stabilizing BNN training by removing all stochasticity due to Monte Carlo sampling.
...
This approximation is improved by considering that (by definition) the residual decays to zero far from the origin in the ( a j , a l ) plane, and so is well modelled by a decaying function exp[−Q( a j , a l , Σ )], where Q is a polynomial in a with a dominant positive even term.
...
Specifically, using dimensionless variables µ i = a i / Σ ii and ρ ij = Σ ij / Σ ii Σ jj , this improved approximation takes the form where the expressions for the dimensionless asymptote A and quadratic Q are given in table table 1 propagate moments all the way through the network to compute the mean and covariances ofq(a L ), our explicit multivariate Gaussian approximation to q(a L ).

Any deep learning framework supporting special functions arcsin and Φ will immediately support backpropagation through the deterministic expressions we have presented.
...
FIG3 shows the comparison between a L distributions reported by our deterministic approximation and MC evaluation using 20k samples from q(w; θ).
...
Regression Case For simplicity we consider scalar y and a Gaussian noise model parameterized by mean m(x; w) and heteroscedastic log-variance log σ 2 y (x) = (x; w).
...
Recall that these parameters themselves are uncertain and the statistics a L and Σ L can be computed following section 3.1.

Inserting the Gaussian forms for p(y|a L ) and q(a L ) into equation 7 and performing the integral (see appendix B.2) gives a closed form expression for the ELBO reconstruction term: DISPLAYFORM1 This heteroscedastic model can be made homoscedastic by setting = Σ = Σ m = 0.
...
For a d-dimensional Gaussian prior p(w) = N (µ p , Σ p ), the KL divergence with the Gaussian variational distribution q = N (µ q , Σ q )

has closed form: DISPLAYFORM0 However, this requires selection of (µ p , Σ p ) for which there is usually little intuition beyond arguing µ p = 0 by symmetry and choosing Σ p to preserve the expected magnitude of the propagated activations (Glorot & Bengio, 2010; He et al., 2015) .
...
We partition the weights into sets {λ} that typically coincide with the layer partitioning 3 , and assign a single element in s to each set: DISPLAYFORM1 for shape α and scale β, and where w λ i is the i th weight in set λ.

Rather than taking the fully Bayesian approach, we adopt an empirical Bayes approach (Type-2 MAP), optimizing s λ , assuming that the integral is dominated by a contribution from this optimal value s λ = s λ * .

We use the data to inform the optimal setting of s λ * to produce the tightest ELBO: DISPLAYFORM2 Writing out the integral for the KL in equation 12, substituting in the forms of the distributions in equation 11 and differentiating to find the optimum gives DISPLAYFORM3 where Ω λ is the number of weights in the set λ.

The influence of the data on the choice of s λ * is made explicit here through dependence on the learned variational parameters Σ q and µ q .

Using s λ * to populate the elements of the diagonal prior variance Σ p , we can evaluate the KL in equation 10 under the empirical Bayes prior.
...
As an alternative to variational Bayes, probabilistic backpropagation (PBP) (Hernández-Lobato & Adams, 2015) applies approximate inference in the form of assumed density filtering (ADF) to refine a Gaussian posterior approximation.

Like in our work, each update to the approximate posterior requires propagating means and variances of activations through the network. (Hernández-Lobato & Adams, 2015) only consider the diagonal propagation case and homoscedastic regression.
...
Additionally, both methods use the same EB prior from equation 13 with a broad inverse Gamma hyperprior (α = 1, β = 10) and an independent s λ for each linear transformation.
...
For Heaviside and ReLU activation functions, closed forms exist for h j in equation 14: DISPLAYFORM1 where SR(x) · · = φ(x) + xΦ(x) is a "soft ReLU", φ and Φ represent the standard Gaussian PDF and CDF, and we have introduced the dimensionless variables µ j = a j / Σ jj .
...
From equation 5, we write: DISPLAYFORM2 where P is the quadratic form: DISPLAYFORM3 Here we have introduced further dimensionless variables η j = α j / Σ jj , η l = α l / Σ ll and ρ jl = Σ jl / Σ jj Σ ll .

We can then rewrite equation 16 in terms of a dimensionless integral I using a scale factor S jl that is 1 for the Heaviside non-linearity or Σ jl /ρ jl for ReLU: DISPLAYFORM4 The normalization constant, Z, is evaluated by integrating over e −P/2 and is explicitly written as Z = 2πρ jl , whereρ jl = 1 − ρ 2 jl .
...
However, for µ j → ∞, vanishing weight appears under the Gaussian in the upper-right quadrant, so we can write down the asymptote of the integral in this limit: DISPLAYFORM0 Here we performed the integral by noticing that the outer integral over η j marginalizes out η j from the bivariate Gaussian, leaving the inner integral as the definition of the Gaussian CDF.

By symmetry, we also have lim µ l →∞ I = Φ(µ j ) and lim µ j,l →−∞ I = 0.

We can then write down the following symmetrized form that satisfies all the limits required to qualify as an asymptote: DISPLAYFORM1 To compute the correction factor we evaluate the derivatives of (I − A) at the origin up to second order to match the moments of e −Q for quadratic Q. Description of this process is found below Zeroth derivative At the origin µ j = µ l = 0, we can diagonalize the quadratic form P : DISPLAYFORM2 .
...
The angle ψ can be found from the coordinate transform between η and ξ as 7 : DISPLAYFORM4 Since A| µ =0 = Φ(0)Φ(0) = 1/4, we can evaluate: DISPLAYFORM5 Here we use the identity cos(2 arctan x) = cos DISPLAYFORM6 First derivative Performing a change of variables x i = η i − µ i , we can write I as: DISPLAYFORM7 where H is the Heaviside function.

Now, using ∂ x H(x) = δ(x), we have: DISPLAYFORM8 In addition, using ∂ x Φ(x) = φ(x), we have: DISPLAYFORM9 By symmetry (I − A) also has zero gradient with respect to µ l at the origin.

Therefore Q has no linear term in µ .Second derivative Taking another derivative in equation 17 gives: DISPLAYFORM10 where we used the identity f (x)∂ x δ(x)dx = − δ(x)∂ x f (x)dx, which holds for arbitrary f .

In addition, we have: DISPLAYFORM11 and the same result holds for the second derivative w.r.t.

µ l .

To complete the Hessian, it is a simple extension of previous results to show that: DISPLAYFORM12 Now that we have obtained derivatives of the residual (I − A) up to second order we propose a correction factor of the form e −Q where Q is truncated at quadratic terms: DISPLAYFORM13 We then find the coefficients {α, β, γ} by matching ( ! =) derivatives at µ = 0: DISPLAYFORM14 This yields the expression seen in table 1 of the main text.

As in the Heaviside case, we begin by computing the asymptote of I by inspecting the limit as µ j → ∞: DISPLAYFORM0 Now, we construct a full 2-dimensional asymptote by symmetrizing equation 18 (using properties SR(x) → x and Φ(x) → 1 as x → ∞ to check that the correct limits are preserved after symmetrizing): DISPLAYFORM1 Next we compute the correction factor e −Q .
...
In section B.1 we justify the intuitive result that expectation of the ELBO reconstruction term over q(w; θ) can be re-written as an expectation overq(a L ).
...
Now we note that the logsumexp function has a simple Hessian ∂ 2 ∂xi∂xj logsumexp(x) = δ ij p i − p i p j , where p = softmax(x).
...
Here we derive the necessary results to add skip connections to our deterministic BNN.We consider a simple layer with skip connections of the following form: DISPLAYFORM0 The moment propagation expressions for this layer are (using the bilinearity of Cov): DISPLAYFORM1 where δ i and Cov(δ i , δ k ) can be computed using analogy to equations 14 and 15.

This just leaves computation of Cov(a i , δ k ) and its transpose, which can be performed analytically using integral results and methods borrowed from appendix A. DISPLAYFORM2 Using this result, we implement a 5-layer, 25-unit network with skip connections.
...
Specifically, we inspect the gradient with respect to the parameters Σ The plots in figure 7 serve to show that rMCVI is not fundamentally different from MCVI, and the performance of one (on either the speed or variance metric) can be transformed into the other by varying the number of samples.
