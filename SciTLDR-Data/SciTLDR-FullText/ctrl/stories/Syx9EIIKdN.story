...
Manifold mixup BID21 implements mixing in the hidden space through convex combinations: DISPLAYFORM2 where λ ∈ [0, 1] (bs,) is sampled from a Uniform(0, 1) distribution and bs denotes the minibatch size.
...
Note that the coefficient λ controls the reconstruction and the coefficient β controls the mixing consistency in the generator.
...
To enforce this constraint, we simply make the conditioning label a convex combinationỹ mix = αy 1 + (1 − α)y 2 as well, using α ∼ Uniform(0, 1).
...
This is achieved through the consistency loss (weighted by β) and the discriminator.

To make this more concrete, the autoencoder and discriminator, in addition to their losses described in Equation 5, try to minimise the following losses: DISPLAYFORM1 label mixes as fake DISPLAYFORM2 Note that for the consistency loss the same coefficient β is used.
...
The mixer function, denoted in this figure as Mix sup , takes h 1 , h 2 and a convex combination of y 1 and y 2 (denotedỹ mix ) and internally produces a Bernoulli mask which is then used to produce an output combinatioñ h mix = mh 1 + (1 − m)h 2 .h mix is then passed to the generator to generatex mix .

In addition to fooling the discriminator usingx mix , the generator also has to make sure the class prediction by the auxiliary classifier is consistent with the mixed classỹ mix .
...
In the case of mixing, one situation which may favour a lower FID is if g(αf ( DISPLAYFORM0 ; in other words, the supposed mix simply decodes into one of the original examples x 1 or x 2 , which clearly lie on the data manifold.

To avoid having the mixed features αx 1 + (1 − α)x 2 being decoded back into samples which lie on the data manifold, we leverage the consistency loss, which is tuned by coefficient β.
...
For Bernoulli mixup, the FID scores appear to be higher than those in the mixup case TAB0 because the sampled Bernoulli mask m is also across the channel axis, i.e., it is of the shape (bs, f ), whereas in mixup α has the shape (bs, ).

Because the mixing is performed on an extra axis, this produces a greater degree of variability in the mixes, and we have observed similar FID scores to the Bernoulli mixup case by evaluating on a variant of mixup where the α has the shape (bs, f ) instead of (bs, ).
...
We consider pairs of examples {(x 1 , y 1 ), (x 2 , y 2 )} (where one example is male and the other female) and produce random convex combinations of the attributesỹ mix = αy 1 + (1 − α)y 2 and decode their resulting mixes Mix sup (f (x 1 ), f (x 2 ),ỹ mix ).
...
We use ADAM for our optimiser with α = 2e −4 , β 1 = 0.0 and β 2 = 0.99.
...
With the mixup loss enabled and λ = 10, we try values of β ∈ {0, 0.1, 10, 100}. After 100 epochs of training, we produce decoded random mixes and plot them over the data distribution, which are shown as orange points (overlaid on top of real samples, shown in blue).

This is shown in FIG3 .As we can see, the lower β is, the more likely interpolated points will lie within the data manifold (i.e. the spiral).

This is because the consistency loss competes with the discriminator loss -as β is decreased, there is a relatively greater incentive for the autoencoder to try and fool the discriminator with interpolations, forcing it to decode interpolated points such that they lie in the spiral.
...
Instead, the discriminator tries to predict what the mixing coefficient α is, and the autoencoder tries to fool it into thinking interpolations have a coefficient of 0.

In FIG5 we compare this to our formulation in which β = 0.

This is shown in FIG5 (right figure) .

It appears that ACAI also prefers to place points in the spiral, although not as strongly as AMR with β = 0 (though this may be because ACAI needs to trained for longer -ACAI and AMR were trained for the same number of epochs).
...
Lastly, in Figure 9 we show some side-by-side comparisons of our model interpolating between faces when β = 50 and β = 0.

We can see that when β = 0 interpolations between faces are not as smooth in terms of colour and lighting.

This somewhat slight discontinuity in the interpolation may be explained by the decoder pushing these interpolated points closer to the data manifold, since there is no consistency loss enforced.(a) Left: AMR with λ = 10, β = 0; right: ACAI with λ = 10 (β = 0 since ACAI does not enforce a consistency loss).

AMR was trained for 200 epochs and ACAI for 300 epochs, since ACAI takes longer to converge.

DISPLAYFORM1 and α ∼ U (0, 1) for randomly sampled {x 1 , x 2 }) over the course of training.

DISPLAYFORM2 Figure 9: Interpolations using AMR {λ = 50, β = 50} and {λ = 50, β = 0}.

To recap, the class mixer in the supervised formulation internally maps from a labelỹ mix to Bernoulli parameters p ∈ [0, 1] K , from which a Bernoulli mask m ∼ Bernoulli(p) is sampled.
