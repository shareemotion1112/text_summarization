...
In supervised deep learning, we typically look for a function f θ : X Ñ Y with parameters θ that maps an input x P X to its target y P Y .

Often the function internally computes one or more intermediate representations z P Z of the data.

In this case, we rewrite f θ as the composition of the encoder h enc θ : X Ñ Z, which maps the input to the corresponding features z P Z, and the decoder h dec θ : Z Ñ Y , which maps features to the output.
...
We define two loss functions: L y pθq, which measures the discrepancy between predictions f θ and the true task y and L s pθ, ψq, which measures the performance on the supplemental task.

The weights of the stethoscope are updated as´∆ψ 9 ∇ ψ L s pθ, ψq to minimise L s pθ, ψq and the weights of the main network as´∆θ 9 ∇ θ L y,s pθ, ψq to minimise the energy DISPLAYFORM0 By choosing different values for the constant λ we obtain three very different use cases:Analytic Stethoscope (λ " 0) Here, the gradients of the stethoscope, which acts as a passive observer, are not used to alter the main model.
...
Auxiliary Stethoscope (λ ą 0) The encoder is trained with respect to the stethoscope objective, hence enforcing correlation between main network and supplemental task.

This setup is related to learning with auxiliary tasks, and helpful if we expect the two tasks to be beneficially related.

Adversarial Stethoscope (λ ă 0) By setting λ ă 0, we train the encoder to maximise the stethoscope loss (which the stethoscope still tries to minimise), thus encouraging independence between main network and supplemental tasks.
...
The dashed lines represent baselines for λ " 0.we consider the setup from FIG3 where the training data only consists of hard scenarios with a baseline performance of 51%.
...
FIG4 shows that auxiliary training substantially improves the performance for weighting parameters λ P r0.5, 16s.

However, for very small values of λ, the contribution of the additional loss term is too small while for large values, performance deteriorates to the level of random chance as a result of the primary task being far out-weighted by the auxiliary task.
...
In FIG6 , we use active stethoscopes (λ ‰ 0) to test this hypothesis.
...
In FIG6 , the results of both adversarial and auxiliary training are compared to the baseline of λ " 0, which is equivalent to the analytic stethoscope setup.
...
The bias is removed more and more with increasing magnitude of the weighting factor λ up to a point where further increasing λ jeopardises the performance on the main task as the encoder puts more focus on confusing the stethoscope than on the main task (in our experiments this happens at λ « 10 1 ).
...
Naturally, the performance of the stethoscope continuously decreases with increasing λ as the encoder puts more and more focus on confusing the stethoscope.
...
With increasing λ and increasing performance of the stethoscope, the network slightly improves its performance on easy scenarios while accuracy deteriorates on hard scenarios.
...
Hence, λ ă 0 denotes adversarial training and λ ą 0 auxiliary training.
...
Let h enc1 θ and h enc2 θ respectively denote image and hint encoders.
...
Hence, a weight close to zero would effectively hide the hint information from the main network, explicitly making the task of the stethoscope h ψ S more difficult.
...
The decoder h dec θ , which acts as a digit classifier, is comprised of a single fully connected layer and receives the image encoding concatenated with the encoded hint vector as input.
...
The bold lines indicate the mean of 100 runs and the shaded areas mark the student-t 1 σ confidence intervals.
...
The bold lines indicate the mean of 20 runs and the shaded areas mark the student-t 1 σ confidence intervals.
...
When using a positive stethoscope loss, which encourages h enc θ to focus more on the artificial hints over the actual digit features, the test accuracy degrades.

This is expected for this toy example as the hints have zero correlation with the digit labels at test time.

In the negative case, h enc θ serves as an adversary to the stethoscope effectively benefiting the main objectives by removing the misleading hint information leading to an improved test accuracy.
