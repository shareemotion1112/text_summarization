...
Input features are represented as embedded vectors {c DISPLAYFORM1 , where d is the embedding size, δ is the vocabulary size, and t represents the 'state' of the embedding after t updates.
...
Once messages have been passed to update input and label nodes, a readout function R is then used on the label nodes to make a binary classification prediction for each label, {ŷ 1 ,ŷ 2 , ...,ŷ L }.
...
Since the autoregressive models only predict the ρ positive labels, they can be faster at training time, whereas the MPED model is predicting the probability for all labels.
...
For the non-autoregressive models, to convert the labels to {0, 1} we chose the best threshold on the validation set from the same set of thresholds used in BID14 .Example-based measures are defined by comparing the target vector y to the prediction vectorŷ.
...
The naïve approach to BR prediction is to predict all labels independently of one another, assuming no dependencies among labels.
...
In other words, only predicting the positive labels (total of ρ for a particular sample) and ignoring all other labels, which we call PCC+.

Similar to PCC, the joint probability of PCC+ can be computed as product of conditional probabilities, but unlike PCC, only ρ < L terms are predicted as positive: DISPLAYFORM2 This can be beneficial when the number of possible labels L is large, reducing the total number of prediction steps.
