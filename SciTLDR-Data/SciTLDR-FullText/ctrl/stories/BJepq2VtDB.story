...
We use much smaller β 2 than in Adam, usually in the range [0.2, 0.5].
...
Algorithm 1 NovoGrad Parameters: Initial learning rate λ 0 , moments β 1 , β 2 , weight decay d, number of steps T Weight initialization: t = 0, Initialize w 0 .

Moment initialization: t = 1, for each layer l set v

end for end while where 0 < β 1 < 1 is the momentum, typically in the same range as in SGD or Adam [0.9 − 0.95].
...
We found that we get much better results when we increase both the learning rate λ and the weight decay d to improve the regularization (see Table 2 ).
...
We will also use the version of SGD with "gradient averaging" (similar to Adam): m t = β · m t−1 + (1 − β) · g t .

For fixed learning rate this SGD version is equivalent to the regular SGD with momentum.

Training trajectories for the baseline (fixed learning rate 0.2, weight decay 0.1, and β 1 = 0.95, β 2 = 0.5.) are shown on the Figure 6 .
