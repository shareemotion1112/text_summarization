...
We define a reward function r(a i , s i ) as DISPLAYFORM0 where E[len(a i , s i )] is the expected conversation length after taking action a i under state s i , E[rel(a i , s i )] is the expected response relevance within the conversation, α = 0.67, and β = 0.33.
...
, v i,n i } is summarized as a context vector c i,j through an attention mechanism: DISPLAYFORM1 where v and W α are parameters, and v i,j−1 is the (j − 1)-th hidden state of the decoder GRU in which v i,j is calculated by DISPLAYFORM2 The generation probability of w i,j is then defined as DISPLAYFORM3 where I(w i,j ) is a vector with only one element 1 indicating the index of w i,j in the vocabulary.
