...
Ha et al. (2017) is a neural network g that is used to generate the weights of another neural network given some context or input vector z. Particularly f (x; θ) becomes f (x; g(z; φ)).
...
We can again write this as f = z T W x where z T W = αI. This is common in methods such as FiLM .
...
While valuable, such a result can also be trivially obtained by adding somewhat exotic activation functions (e.g. Weierstrass function σ(x) = ∞ n=0 0.5 n cos(7 n πx) which is a continuous function but nowhere differentiable (Weierstrass, 1895) ; see appendix for the proof) to the pool of typically used ones.
...
Normally these are then projected to policies π and value functions V that are shared across all tasks.
...
That is, we now have c = relu(MLP(I i )) as a context, and π t , V t = M(h t , c).
...
To show that the inclusion is strict lets consider a Weierstrass function itself f (x) = σ w (x).
...
• Policy = Linear layer or multiplicative interaction followed by a softmax 2 /l 2 ) with fixed σ f = 1.0 and, importantly, a random l ∼ U [0.1, 10.0] for each draw, resulting in a broad distribution of functions.

We also add Gaussian observation noise, i.e. y ∼ N (f, σ 2 n ) and set σ n = 0.02.
