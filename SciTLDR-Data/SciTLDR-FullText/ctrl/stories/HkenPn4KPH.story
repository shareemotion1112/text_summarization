...
In addition, we consider self-supervised losses L ss based on labeled data x → (x,ŷ) that can be derived from inputs x alone.
...
The target labelŷ is the index of the permutation.
...
We follow the method of where the input image x is rotated by an angle θ ∈ {0°, 90°, 180°, 270°} to obtainx and the target labelŷ is the index of the angle.
...
Here φ(x) is an image embedding from a ResNet101 network pretrained on ImageNet.
...
When training with supervised and self-supervised loss, a trade-off term λ between the losses can be used, thus the total loss is L = (1 − λ)L s + λL ss .

We find that simply use λ = 0.5 works the best, except for training on mini-and tiered-ImageNet with jigsaw loss, where we set λ = 0.3.
...
When both jigsaw and rotation losses are used, we set λ = 0.5 and the two self-supervised losses are averaged for L ss .
