...
The loss adjustment parameters α α α c and α α α s is passed to the network Λ which will predict activation normalizers γ α α α and β α α α that normalize activation of main stylizing network T .
...
Models Λ and T are trained jointly by minimizing this weighted sum.

At generation time, values for α α α c and α α α s can be adjusted manually or randomly sampled to generate varied stylizations.
...
G(φ l (p)) denotes the Gram matrix associated with φ l (p).
...
Top row demonstrates that randomizing α α α results to different stylizations however the style features appear in the same spatial position (e.g., look at the swirl effect on the left eye).

Middle row visualizes the effect of adding random noise to the content image in moving these features with fixed α α α.
...
We enable the users to adjust w DISPLAYFORM0 To learn the effect of α α α c and α α α s on the objective, we use a technique called conditional instance normalization (Ulyanov et al.) .

This method transforms the activations of a layer x in the feedforward network T to a normalized activation z which is conditioned on additional inputs α α α = [α α α c , α α α s ]: DISPLAYFORM1 where µ and σ are mean and standard deviation of activations at layer x across spatial axes BID11 and γ α α α , β α α α are the learned mean and standard deviation of this transformation.
...
We implemented Λ as a multilayer fully connected neural network.
...
We trained T and Λ jointly by sampling random values for α α α from U (0, 1).
...
Since there is only one content layer, we fix α α α c = 1.

Our implementation can process 47.5 fps on a NVIDIA GeForce 1080, compared to 52.0 for the base model without Λ sub-network.

The primary goal of introducing the adjustable parameters α α α was to modify the loss of each separate layer manually.
...
As can be seen in , increasing α l s decreases the measured loss corresponding to that parameter.
...
Finally, we verify that modifying the input parameters α α α s generates visually similar stylizations to the retrained base model with different loss weights w l s .
...
To do so, we randomize α α α to generate randomized stylization (top row of FIG1 ).

Changing values of α α α usually do not randomize the position of the "elements" of the style.
...
We train this model with only one style image and to get different stylizations, we add random noise to the style parameters (γ α α α and β α α α parameters of equation 6) at run-time.
...
Convolution 3 1 C SAME ReLU Convolution 3 1 C SAME Linear Add the input and the output Upsampling -C feature maps Nearest-neighbor interpolation, factor 2 Convolution 3 1 C SAME ReLUNormalization Conditional instance normalization after every convolution Optimizer Adam (α = 0.001, β 1 = 0.9, β 2 = 0.999)

Batch size 8 Weight initialization Isotropic gaussian (µ = 0, σ = 0.01) BID15 .
