...
return θ n 14: end procedure function.
...
return θ n 14: end procedure Examined 5 methods find the optimum weights with the same objective: DISPLAYFORM0 where s denotes the number of samples to add at each stage, parameters at each stage determined by same amount of samples for all methods.
...
Definitions: (a) The loss function (θ) with one parameter θ ∈ R is given as in (2) where N is the number of training instances and the loss function for each individual instance i (θ) is given as in (3).

DISPLAYFORM0 (b) The point θ B is a definite local minimum of (θ).
...
If we denote the expected value of (θ) as E[ (θ)], expected values of the loss function in the given points can be ordered as in (4).
...
DISPLAYFORM4 Lemma 1: Probability density function of the derivatives in the θ B point has a skewed distribution with zero mean.

Proof: Derivatives of the loss functions is under the point θ B in FIG0 (b) for more than half of the individual instances.

However i (θ B ) values are high for the above instances.

If we denote the probability density function of the derivatives in the θ B as Pr( i (θ B )), 1) Definition (c) gives that θ B is a local minimum therefore Pr( i (θ B )) has zero mean.2) If κ( (θ)) = 2 as in FIG0 (b) then κ( i (θ)) <= 2 from Assumption (a) and κ( i (θ)) < 2 for more than half of the instances from Assumption (b).

i (θ B ) < 0 for the instances that has κ( i (θ)) < 2.

i (θ B ) < 0 for more than half of the instances therefore Pr( i (θ B )) has a skewed distribution.

Corollary 1: If we take a subset with k instances from the training set we can denote: DISPLAYFORM5 Expected value for the derivative of the loss function of the subset at the point θ B is less than zero.

DISPLAYFORM6 Proof: Probability of being negative for E[ s (θ B )] depends on k and always high from Lemma 1.Theorem 1: If we take a subset with k instances and train with batch gradient descent, we obtain a better local minimum for this subset.

Proof: Optimization in the (θ) surface which starts from θ A will probably end at θ B .

However in the s (θ) surface optimization will be continue to find a better minimum for the current set according to Corollary 1.Theorem 2: To continue the optimization with all samples provides a better local minimum for all training set than stopping at the minimum of the subset.

Proof: Error surface for all instances ( (θ)) and some subsample instances ( sx (θ), sy (θ), sz (θ), sw (θ)) are given in FIG2 .

These examples shows all possible situations for the stopping points by considering θ A , θ B and θ C .

We suppose to start the optimization from θ A .-If we use (θ) it will be stop at θ B .-If we use sx (θ) it will be stop at θ sx in the range θ B < θ sx < θ C .

If we finish the optimization here, error on the surface for all instances will be high (E[ (θ sx )] > E[ (θ B )]).

Therefore optimizing in the (θ) subsequently prevents to stay at a worse point.-If we use (θ sy ) it will be stop at θ sy in the range θ A < θ sy < θ B .

Here E[ (θ sy )]

> E[ (θ B )] therefore optimization for all instances is necessary.-If we use (θ sz ) it will be stop at θ sz in the range θ C < θ sz and it is possible to obtain a better local minimum with optimization on (θ) surface by starting from the local minimum of the subset.-If we use (θ sw ) it will be stop at θ sw in the range θ A > θ sw .

Here the stopping point can be worse for all instances.

Therefore it is possible to avoid from worse point by optimization on the (θ) surface.
...
We started the optimization from θ A , instances under this point are considered as easy and above are difficult.

If we take an easy instance it is possible or not to guide the optimization to a better minimum.

It will be stop at the local minimum θ B in the worst case.
...
It is a better situation to shorten the distance between θ B and θ C in FIG2 to bypass the local minimum.
