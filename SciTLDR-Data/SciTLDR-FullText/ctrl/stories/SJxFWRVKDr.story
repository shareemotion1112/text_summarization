...
Several works have incorporated gradients with respect to input in the form of regularization during the training of deep networks to improve the robustness (Drucker & Le Cun, 1991; Ross & Doshi-Velez, 2018; Sokolić et al., 2017) .
...
An autoencoder consists of an encoder, f θ , and a decoder, g φ as shown in Fig. 2 (a) .

From an input image, x, the latent variable, z, is generated as z = f θ (x) and the reconstructed image is obtained by feeding the latent variable into the decoder, g φ (f θ (x)).

The training is performed by minimizing the loss function, J(x; θ, φ), defined as follows:
...
When the test image, x out , sampled from the test data distribution is given to the autoencoder, it will be reconstructed asx out by the projection, g φ (f θ (x out )).
...
Using the error defined in (1), the gradient of weights, ∂J ∂θ , ∂J ∂φ , can be calculated through the backpropagation.
...
where L (x; W ) is the primal loss for a given task and α is the weight for the gradient constraint, which is the average of cosine similarity calculated for each layer.

We apply this constraint on a subset of all layers.

Also, we set sufficiently small α value to ensure that gradients actively explore the optimal weights until the primal loss becomes small enough.
...
We train CAEs by setting α = 0.03 and Ω(z; θ, φ) = 0 in (4).

This loss utilized for training is utilized as an anomaly score during testing but with the increased α of 0.12.
