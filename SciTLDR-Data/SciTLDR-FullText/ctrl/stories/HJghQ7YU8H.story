...
Given a dataset X = {x} N of data samples, a pre-trained DNN f parameterized by θ such that y = f θ (x) is the prediction of sample x, and an attribution function a which can be any of [1; 4; 5; 6], the attribution score a(n, x, y) measures how much a hidden neuron n contributes to the prediction of sample x into class y.
...
We compute e(n i , x j ) = e ij = a(n i , x j ,ŷ) − 1 |C| c a(n i , x j , c) whereŷ = f θ (x j ) is the predicted class of x j and a can be any of the attritbution function among [1; 4; 5; 6] .
