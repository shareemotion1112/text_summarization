...
The outputs of the network include the class label y and a set of interpretable prototypeŝ P corresponding to a rule list R. The neural network is used to performing accurate classification, under the guidance of prototypes defined by the rule lists.

Formally, the overall objective of PEARL is: DISPLAYFORM2 ( 1) where h(X ; θ 1 ) is the learned representation of X with parameter θ 1 .

h(X ; θ 1 ) is a vector and has the same predefined dimension as p k , R is the learned rule list, and P is the set of learned prototypes.
...
f is a fixed mapping that, given R and learned h(X ; θ 1 ),P are determined without further learning.

More details on f can be found in Section 3.1.2.

Each p k lies in the same space as h(X ; θ 1 ), and should correspond to one or more rules in R. The second term L 2 is the Cross Entropy loss for the final prediction target, where s R (h(X ; θ 1 ); θ 2 ) represents the predicted label for X and y is the ground-truth label.

Here θ 1 represents all the model parameters for data representation learning h(X ; θ 1 ) and θ 2 represents those of classification model s R (·).

We will drop θs for simplicity from now on.
...
Note that relative weights λ 1 and λ 2 values are chosen via hyperparameter tuning.

In general we chose λ 2 > λ 1 to emphasize the classification performance.
...
DISPLAYFORM0 where τ k n is the time difference between consecutive events, such that τ DISPLAYFORM1 for k > 1 and τ 0 n = 0.

By including τ k n as additional features, we incorporate the time information into patient representation learning.
...
Then we collect those prototypes, denoted as K , of which s k is less than a pre-specified threshold η and their corresponding data subjects.
...
After tuning, we set λ 1 = 1 and λ 2 = 1e−3.
...
Data weighting threshold η is set to values between .45 and .55.
...
We then study the empirical effect of data reweighing threshold η in Algorithm 1, where η controls the number of prototypes from which the corresponding subjects are up-weighted.
