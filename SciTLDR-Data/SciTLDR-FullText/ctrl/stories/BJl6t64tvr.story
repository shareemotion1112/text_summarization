...
This really matters in large-scale NLP; a foolproof solution is to use our proposed "ε = 0" variant of AdaGrad.
...
where η t > 0 is a step size (or learning rate) parameter, whose scale and time-varying behavior are typically determined via hyperparameter search.
...
The particular instantiations of the parameters α k , β k are summarized below: Table 1 : Parameter settings for common optimization algorithms in the unified framework of Equation 1.
...
At least one work remarks that the ability to choose a large ε in a secondmoment-based adaptive method might be a feature rather than a shortcoming; the smooth interpolation with SGD may improve the stability of more exotic optimizers.
...
We provide an analysis in the Appendix B, verifying as a sanity check that the standard AdaGrad regret bounds continue to hold when ε is completely removed.
...
We note that it is straightforward to extend AdaGrad to incorporate heavy-ball momentum, where we start withḡ 0 = 0 (and from a certain initialization w 0 ) and iteratively update:

The original definition of the Adam optimizer (Kingma & Ba, 2014) includes a bias correction term, in which the moment estimates are multiplied by the time-varying scalars (1 − β t 1 ) and (1 − β t 2 ).
...
As can be seen from Figure 2 , for the typical values of β 1 = 0.9 and β 2 = 0.999, the effective multiplier on the learning rate essentially resembles an external warmup applied on top of the learning rate.
...
We see that a properly tuned AdaGrad (with ε = 0 and added momentum) outperforms Adam, while SGD with momentum, plain AdaGrad and RMSprop perform much worse on this task.

These results illustrate that adaptivity and momentum are both extremely effective in training these models.

In Section 3.1, we proposed an "ε = 0" variant of AdaGrad.

Here we empirically motivate this modification, by investigating the effect of the parameter ε on the performance of AdaGrad.

We train the Transformer model from above on the en→fr dataset using AdaGrad while varying the value of ε.

The results are given in Fig. 4 .

We see drastic improvement in convergence as we lower the value of ε down to 10 −7 (lower values do not improve convergence further and are thus omitted from the figure).
...
The choice of ε is thus important, and justify our prescription of removing the dependency all-together instead of tuning it as a separate hyper-parameter.
...
We compared several optimization methods on this task as seen in Fig. 5 : the straightforward application of AdaGrad (with a fixed ε and with heavy ball momentum) achieves only a paltry 63.94% test accuracy.
...
Further note that to get down to the same test error as that achieved by AdaGrad, it can be seen that SGD requires at least Ω( √ d) times more steps than AdaGrad.
