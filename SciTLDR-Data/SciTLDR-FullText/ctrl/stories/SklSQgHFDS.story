...
It is formalized by Markov decision processes (MDPs) defined by the tuple S, A, p, r, γ .

At time step t the agent samples an action a ∈ A according to policy π(·|s), which depends on its current state s ∈ S. The agent receives a scalar reward r ∈ R and transits to the next state s ∈ S. The distribution of the corresponding state, action and reward process (S t , A t , R t+1 ) is determined by the distribution of the initial state S 0 , the transition operator p and the policy π.

The goal of the agent is to find a policy that maximizes the expectation of the sum of discounted rewards T k=0 γ k R t+k+1 .
...
Dayan (1993) introduced the idea of representing a state s by the occupancies of all other states from a process starting in s following a fixed policy π, where the occupancies denote the average number of time steps the state process stays in each state per episode.
...
features (SF) (Kulkarni et al., 2016b; Barreto et al., 2017) extend the concept to an arbitrary feature embedding φ : S → R m .

For a fixed policy π and embedding φ the SF is defined by the |m|-dimensional vector

Analogously, the SF represent the average discounted feature activations, when starting in s and following π.
...
A high SFC reward indicates a big change in the future feature activations when π is followed.
...
Contains a shared embedding θ ϕ and two Q-value output heads θ E (extrinsic) and θ I (intrinsic).

• A SF-Net {θ φ , θ ψ }: Contains an embedding θ φ and a successor feature head θ ψ .

θ φ is initialized randomly and kept fixed during training.
...
We learn from K-step targets (K = 5), so each actor at each environment step stores (s t−K , a t−K , K k=1 γ k−1 r t−K+k , s t ) into a shared replay buffer.
...
• A learner: Learns the Q-Net (θ E and θ I are learned with the extrinsic and intrinsic reward respectively) and the SF-Net from samples (Eq.2) from the same shared replay buffer, which contains all experiences collected from following different policies.
...
We note that previous works for learning the deep SF have included an auxiliary task of reconstruction on the features φ Kulkarni et al. (2016a); Zhang et al. (2017) , while in this work we investigate learning ψ without this extra reconstruction stream.

Instead of adapting the features φ while learning the successor features ψ, we fix the randomly initialized φ.

This design follows the intuition that since SF (ψ) estimates the expectation of features (φ) under the transition dynamics and the policy being followed, more stable learning of the SF could be achieved if the features are kept fixed.
...
We scale the extrinsic task reward that is always in {0, 1} with η

, where γ I is the discount rate for the intrinsic reward.

Furthermore, η is a hyperparameter which takes into account that for Q-values from states more distant to the goal the reward is discounted with the discount rate for the extrinsic reward depending on how far away that state is.

In our experiments we set η = 3.
...
For η we tried the values in {0.3, 1, 3, 10}. We also tried to not normalize the rewards and just scale the intrinsic reward.
...
For Fig.1 a) ,b) the successor features were calculated analytically via the formula Ψ = (I − γP ) − 1, where P denotes the one-step transition matrix.

The SF discount factor was set to γ = 0.95.
...
For the purpose of visualization we discretized the map into 85 × 330 grids and position the trained agent SID(M,SFC) at each grid, then computed the successor features ψ for that location for each of the 4 orientations (0°, 90°, 180°, 270°), which resulted in a 4 × 512 matrix.
...
The overall learning objective is to maximize the extrinsic reward E ω(T|St) E π T (At|St) [q T E (S t , A t |A t ∼ π T (·|S t ))] (ω: the macro-policy of the scheduler).
...
The Macro-Q Scheduler is then learned with an additional output head attached to θ ϕ (we also tried θ φ ).
...
It just selects a task based on the current Q-value of the extrinsic head θ e .
