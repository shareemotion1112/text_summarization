...
A discrete autoencoder consists of two parameterized functions: a deterministic encoder function enc φ : X → C with parameters φ that maps from input to code space and a conditional decoder distribution p ψ (x | c) over structures X with parameters ψ.
...
We use the notation,x = arg max x p ψ (x | enc φ (x)) for the (approximate) decoder mode.
...
The method approximates drawing samples from a true distribution c ∼ P r by instead employing a latent variable z and a parameterized deterministic generator functioñ c = g θ (z) to produce samplesc ∼ P g .
...
GAN training utilizes two separate models: a generator g θ (z) maps a latent vector from some easy-to-sample source distribution to a sample and a critic/discriminator f w (c) aims to distinguish real data and generated samples from g θ .

Informally, the generator is trained to fool the critic, and the critic to tell real from generated.

WGAN training uses the following min-max optimization over generator parameters θ and critic parameters w, DISPLAYFORM1 where f w : C → R denotes the critic function,c is obtained from the generator,c = g θ (z), and P r and P g are real and generated distributions.
...
For instance, contractive autoencoders BID30 regularize their loss by the functional smoothness of enc φ .
...
The joint objective regularizes the autoencoder to constrain the discrete encoder to agree in distribution with its continuous counterpart: DISPLAYFORM0 Above W is the Wasserstein-1 distance between P r the distribution of codes from the discrete encoder model (enc φ (x) where x ∼ P(x)) and P g is the distribution of codes from the continuous generator model (g θ (z) for some z, e.g. z ∼ N (0, I)).
...
The model can be used as an autoencoder, where a structure x is encoded and decoded to producex, and as a GAN (ARAE-GAN), where a sample z is passed though a generator g θ to produce a code vector, which is similarly decoded tox.
...
First, we extend the decoder to condition on a transfer variable denoting this attribute y which is known during training, to learn p ψ (x | c, y).

Next, we train the code space to be invariant to this attribute, to force it to be learned fully by the decoder.

Specifically, we further regularize the code space to map similar x with different attribute labels y near enough to fool a code space attribute classifier, i.e.: DISPLAYFORM5 where L class (φ, u) is the loss of a classifier p u (y | c) from code space to labels (in our experiments we always set λ (2) = 1).
...
All three models utilize the same generator architecture, g θ .

The generator architecture uses a low dimensional z with a Gaussian prior p(z) = N (0, I), and maps it to c. Both the critic f w and the generator g θ are parameterized as feed-forward MLPs.
...
Define an RNN as a parameterized recurrent function h j = RNN(x j , h j−1 ; φ) for j = 1 . . .

n (with h 0 = 0) that maps a discrete input structure x to hidden vectors h 1 . . .

h n .

For the encoder, we define enc φ (x) = h n = c. For decoding we feed c as an additional input to the decoder RNN at each time step, i.e.h j = RNN(x j ,h j−1 , c; ψ), and further calculate the distribution over V at each time step via softmax, p ψ (x | c) = n j=1 softmax(Wh j + b) xj where W and b are parameters (part of ψ).
...
For ARAE, we experimented with different λ (1) weighting on the adversarial loss (see section 4) with λ(1) a = 1, λ(1) b = 10.

We generally set λ (2) = 1.
...
After training, an ARAE can also be used as an implicit latent variable model controlled by z and the generator g θ , which we refer to as ARAE-GAN.
...
We experiment with this property by sampling two points z 0 and z 1 from p(z) and constructing intermediary points z λ = λz 1 + (1 − λ)z 0 .

For each we generate the argmax outputx λ .
...
One can interpret the ARAE framework as a dual pathway network mapping two distinct distributions into a similar one; enc φ and g θ both output code vectors that are kept similar in terms of Wasserstein distance as measured by the critic.
...
Let P be a distribution on a compact set χ , and (P n ) n∈N be a sequence of distributions on χ .
...
We decode back into sentence space with the transformed latent vector via sampling from p ψ (g(z + t)).

Some examples of successful transformations are shown in FIG2 (right).

Quantitative evaluation of the success of the vector transformations is given in FIG2 (left).

For each original vector z we sample 100 sentences from p ψ (g(z + t)) over the transformed new latent vector and consider it a match if any of the sentences demonstrate the desired transformation.
