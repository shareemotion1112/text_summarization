...
We propose two embedding objectives: (1) δ-Most Confident Neighbor (MCN δ ), where MCN δ (F, x) = arg max z∈N (x,δ) F (z) ∞ for a point x, a radius parameter δ > 0, and N (x, δ) the δ-neighborhood around x. (2) p-Nearest Confident Neighbor (NCN p ), where NCN p (F, x) = arg min z z − x subject to F (z) ∞ ≥ p, for a point x and a confidence parameter p ∈ (0, 1).

With these, the end to end predictions become F (MCN δ (F, x)) and F (NCN p (F, x)).
...
Specifically, we propose using gradient based optimization, such as Carlini-Wagner attacks BID4 (which, however, are now used for defense) to solve MCN δ or NCN p .
...
It is also natural to consider the following probabilistic version of Definition 4, Definition 5 ((p, q, δ)-separation).

Let D be a data generating distribution, p, q ∈ [0, 1], δ ≥ 0, and d(·, ·) be a distance metric.

F is said to have (p, q, δ)-separation if DISPLAYFORM1 Note that these two definitions have some important differences: (1) Definition 4 is defined over sets of points while Definition 5 is defined point-wise.
...
To start with, their objective function is of the form DISPLAYFORM2 ( 1) where D is the data generating distribution, S is set of allowed perturbations (for example S = {∆ | ∆ ∞ ≤ δ}), and L(θ, x, y) is the loss of θ on (x, y).

BID8 uses cross entropy loss function: DISPLAYFORM3 where F θ is the model instantiated with parameters θ, and 1 y is the indicator vector of label y. DISPLAYFORM4 That is, the probability of an x ∈ x + S that is p-confident on a different label vanishes as p → 1.Proof.

Let E be the event {∃y = y, x ∈ x+S, F θ (x ) y ≥ p}. If E happens, then F θ (x ) y ≤ 1−p, and κ(θ, x, y) ≥ − log(1 − p), therefore by Markov's inequality 1 , DISPLAYFORM5 The proof is complete.

This immediately generates the following corollary, DISPLAYFORM6 This indicates that even if a model is only a somewhat good solution to (1), meaning ρ(θ) ≤ ε for somewhat small ε, (note that in reality D is unknown and a learner can only approximate (1)), then p-confident points will be well separated (in the sense of Definition 5) as soon as p increases.
...
For example, the confidence on the wrong label is only 1 2 + ν for some tiny ν?

Note that by setting p = 1/2 we derive immediately a bound ε (much weaker than −ε/ log(1 − p) for large p) on the probability that bad events happen.

Further, without further assumptions, this bound in tight so that in a worst-case sense that there exists a bad model so that bound ε is achieved.

We have the following proposition.

Proposition 2.

Let F θ be a neural network parameterized by θ, and C F θ be the classification network.

If ρ(θ) ≤ ε, then DISPLAYFORM7 Further the bound is tight.1 Let X be a nonnegative random variable and a > 0, Markov's inequality says that DISPLAYFORM8 (otherwise x will be classified as y), and so κ(θ, x, y) ≥ − log(1/2) = 1.

On the other hand, if E does not happen, then we can lower bound κ(θ, x, y) by 0.
...
Also, cross entropy function is unbounded (so in particular it can happen that ρ(θ) ≥ 1).

Therefore, one may only expect that ρ(θ) is somewhat small.• Nevertheless, even with a somewhat large ρ(θ), Proposition 1 indicates that highconfidence predictions of different classes will be well separated as soon as confidence p increases.
...
Let F be a model, · be a norm, and δ > 0 be a real number.

The δ-Most Confident Neighbor objective (or simply MCN δ objective) computes: DISPLAYFORM0 where N (x, δ) is the δ-ball around x with respect to · , and · ∞ denotes the L ∞ -norm.
...
In short, MCN considers a δ-neighborhood of a point and use the most confident point in the neighborhood for prediction.
...
As a result, a factor model shell for MCN δ objective should compute the final prediction as: DISPLAYFORM4 and accordingly for NCN p objective: DISPLAYFORM5 We note the following:• Exact computations of (4) and (5) provide semantic defenses.
...
Therefore, embedding should be effective in fixing low-confidence errors of good models.• We note that the resulting factor model shells, (F • MCN δ ) and (F • NCN p ) are nondifferentiable as MCN δ (F, x) and NCN p (F, x) are non-differentiable with respect to x (they are argmin or argmax over a ball).

This renders existing attacks fail to work directly to attack F • MCN δ or F • NCN p , since they all require the assumption that the output softmax layer is differentiable with respect to the input feature vector x 2 As a result, this forces attacks to either attack the base model F and then transfer attacks to the model shell, or has to find some differentiable approximations or derivative-free optimization to attack MCN δ and NCN p directly.

Algorithms.

We now give one concrete algorithms to solve (2) and (3).

To start with, we note that the optimization for solving MCN δ (2) is nothing but for each label t ∈ C we try to find DISPLAYFORM6 2 For example, let us examine one objective function used in BID4 , which is one of the strongest attacks known.
...
This thus gives the following factor model shell:Algorithm 1 Solving MCN δ objective.

Input: x a feature vector, δ > 0 a real parameter, a base model F , any gradient-based optimization algorithm O to solve the constrained optimization problem defined in (6).

1: function MCNOracleShell(x, δ, F ) 2:for t ∈ C do 3: DISPLAYFORM8 return z t * where t * = arg max t∈C F (z t ) tWe note that solving (6) is similar to a targeted adversarial attack which tries to modify x so as to increase the confidence on label t.
...
For example, consider the following quadratic penalty function Q(z, α, t DISPLAYFORM12 where [y] − denotes max{y, 0}. Then we can solve the unconstrained optimization min Q(z, α, t) for a series of different α, and terminates once we find a satisfying solution.

The intuition is that (i) solving the unconstrained optimization is easy, (ii) by increasing the coefficient α, we can force the minimizer of the penalty function closer to the feasible region F (z) ≥ p, and (iii) for well chosen α, the minimizer of the penalty function is close enough to that of problem 7.
...
To set the norm bound δ, we note that BID8 uses a norm bound δ = 8.0.

Since we normalize images when applying the CW attack, we set δ = 8 255 .

A perturbation is valid if and only if it both changes the classification and its L ∞ norm is at most δ. (ii) as an embedding procedure for instantiating MCNOracleShell.
...
We first sample a batch of points K. For each p ∈ P, we try to find, for each point (x, y) in K, a point that is predicted as y = y with confidence ≥ p in its δ-neighborhood.
...
From Definition 5 and Proposition 1, our goal is to statistically estimate Pr (x,y)∼D (∃y = y, x ∈ N (x, δ))F (x ) y ≥ p .

As mentioned above, we compare the robust model in BID8 and its natural variant.

Let E b denote the bad event (∃y = y, x ∈ N (x, δ))F (x ) y ≥ p. We randomly sample batches of data points from the test set, and compute frequency that E b happens for each batch.
...
With these statistics we can thus estimate (p, q, δ)-separation (Definition 5) of the models in comparison.
...
In this case, Proposition 1 only indicates very weak separation, (indeed, ε) and so it is expected to be hard to defend.
...
This section gives details of our estimation of (p, q, δ)-separation from statistics in TAB0 .
...
, X t bounded in [0, 1], and X = ( • (Upper bound) With probability at least α, µ is smaller thanμ + ε.• (Lower bound) With probability at least α, µ is bigger thanμ − ε.
