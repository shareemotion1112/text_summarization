...
BID12 proposes a reinforcement learning approach that uses Rènyi divergence to compute utility of sensing actions.
...
BID0 proposed ρPOMDP framework where the reward depends on entropy of the belief.

Spaan et al. (2015) introduced POMDP-IR where the reward depends on accurate prediction about the state.

Satsangi et al. (2018) established an equivalence property between ρPOMDP and POMDP-IR.
...
A pr = {δ ∈ {0, 1} n | |δ| 0 ≤ k} constructs an n-dimensional lattice.

Each component of an action δ ∈ A pr determines whether the corresponding sensor is selected.• k is the maximum number of sensor to be selected.

DISPLAYFORM1 is the probabilistic transition function.• Ω = Ω 1 × Ω 2 × . . .

× Ω n is the partitioned set of observations, where each Ω i corresponds to the set of measurements observable by sensor i.• O : S × A × Ω → [0, 1] is the probabilistic observation function.• R : S × A pl → R is the reward function, and DISPLAYFORM2 At each time step, the environment is in some state s ∈ S. The agent takes an action β ∈ A pl that causes a transition to a state s ∈ S with probability P r(s |s, β) = T (s, β, s ).

At the same time step, the agent also picks k sensors by δ ∈ A pr .

Then it receives an observation ω ∈ Ω with probability P r(ω|s , β, δ) = O(s , β, δ, ω), and a scalar reward R(s, β).
...
, n}, I 1 ∩ I 2 = ∅ : DISPLAYFORM3 Let ζ(δ) = {i|δ(i) = 1} to denote the subset of sensors that are selected by δ.

If Assumption 1 holds, then: DISPLAYFORM4 DISPLAYFORM5 The belief of the agent at each time step, denoted by b t is the posterior probability distribution of states given the history of previous actions and observations, i.e., h t = (a 0 , ω 1 , a 1 , . . . , a t−1 , ω t ).A well-known fact is that due to Markovian property, a sufficient statistics to represent history of actions and observations is belief (Åström, 1965; Smallwood & Sondik, 1973) .

Given the initial belief b 0 , the following update equation holds between previous belief b and the belief b a,ω b after taking action a = (β, δ) and receiving observation ω: DISPLAYFORM6 The goal is to learn a deterministic policy to maximize E[ DISPLAYFORM7 .

A deterministic policy is a mapping from belief to actions π : B → A, where B is the set of belief states.
...
Let α's to denote the corresponding vectors of the hyperplanes and let Γ t to be the set of α vectors at horizon t. Then, DISPLAYFORM9 This fact has motivated approximate point based solvers that try to approximate the value function by updating the hyperplanes over a finite set of belief points.
...
Consider a AP 2 -POMDP P = (S, A, k, T, Ω, O, R, γ) and an initial belief b 0 .

We aim to learn a policy π(b) = (β, δ) such that the expected discounted cumulative reward is maximized, i.e, It is worth noting that the perception actions affect the belief and subsequently the received reward in the objective function.
...
Subsequently, with some algebraic manipulation, it can be shown that the conditional entropy of state given current belief with respect to δ is: DISPLAYFORM4 where ζ(δ) = {i 1 , i 2 , . . .
...
Algorithm 1 Greedy policy for perception action DISPLAYFORM7 ζ ← ζ ∪ {j * } 7: end for 8: return δ corresponding to ζ.
...
, ω n } to represent a set of observations of the state s that conditioned on the state, are mutually independent (Assumption 1 holds).

Then, f (ζ), defined in (8), realizes the following properties: DISPLAYFORM8 2.
...
Let ζ * to denote the optimal subset of observations with regard to objective function f (ζ), and ζ g to denote the output of the greedy algorithm in Algorithm 1.
...
Let the agent's current belief to be b and its planning action to be β.

Consider the optimization problem in (9), and let δ * and δ g to denote the optimal perception action and the perception action obtained by the greedy algorithm, respectively.

It holds that: Proof.

We outline the sketch of the proof and bring the complete proof in the appendix.

First, we show that minimizing conditional entropy of posterior belief is equivalent to maximizing KullbackLeibler (KL-) divergence between current belief and the posterior belief, i.e., D KL (b δ,ω b b).
...
It starts with an initial set of belief points B 0 and their corresponding α vectors.

Then it performs a Bellman backup for each point to update α vectors.

Next, it prunes α vectors to remove dominated ones.
...
To avoid dealing with perception actions in the reachability tree, we apply the greedy scheme to make the choice of δ deterministically dependent on β and previous belief.
...
In point-based solver each witness belief point is associated with an α vector and an action.

Nevertheless, for AP 2 -POMDPs, each witness point is associated with two actions, β and δ.

We compute δ based on greedy maximization of (9) so that given b and β, δ is uniquely determined.

Henceforth, we can rewrite (3) using (4) to obtain: DISPLAYFORM0 whereδ = argmax δ∈A pr f (ζ(δ)) and f is computed atb β b .
...
Inspired by the results in the previous section, we propose the BackUp step detailed in Algorithm 3 to compute the new set of α vectors from the previous ones using Bellman backup operation.
...
This in turn affects computation of Γ b,β,ω t as it represents a different set for each belief point (Lines 9-13).
...
The α vectors are initialized by 1 1−γ min s,a R(s, a).Ones(|S|) (Shani et al., 2013) .
...
There is one camera at each cell that outputs a probability for b ∈ B t do 7:δ = Greedy argmax δ∈A pr f (ζ(δ)) 8: DISPLAYFORM0 for α ∈ Γ t−1 do 11: DISPLAYFORM1 DISPLAYFORM2 DISPLAYFORM3 Figure 3: The robot moves in a grid while communicating with the cameras to localize itself.
...
Then, f (ζ), defined in (8), realizes the following properties: DISPLAYFORM0 2.
...
Notice thatb β b is explicitly present to determine the current distribution of s and it is not a random variable.

Therefore, for simplicity, we omit that in the following proof.

It is clear that DISPLAYFORM1 To prove the monotonicity, consider ζ 1 ⊂ [n] and j ∈ [n]\ζ 1 .
...
Furthermore, from the third line of above proof, we can derive the marginal gain, i.e., the value of adding one sensor, as: DISPLAYFORM3 To prove submodularity, let ζ 1 ⊆ ζ 2 ⊂ [n] and j ∈ [n]\ζ 2 .

Then, DISPLAYFORM4 where (a) is based on the fact that conditioning does not increase entropy, and (b) results from ζ 1 ⊆ ζ 2 .
...
The properties of f stated in Lemma 1 along the theoretical analysis of greedy algorithm by BID17 yields DISPLAYFORM6 Using the definition of f (ζ) and rearranging the terms, we obtain the desired result.
...
This equivalency is due to the definition of mutual information, i.e., I(s; DISPLAYFORM1 and the fact that H(s) is computed atb β b which amounts to a constant value that does not affect selection procedure.
...
An I-sphere with center p and radius ρ is defined as: DISPLAYFORM6 is called the I-projection of p on Λ. Lemma 2.

Instate the definition of p 0 , p g , and p * .

The following inequality holds on expectation: DISPLAYFORM7 Proof.

Consider the set Λ g = {p ∈ ∆ B |H(p) ≥ H(p g ) that contain probability distributions whose entropy is lower-bounded by entropy of p g .

Since entropy is concave over ∆ B , its hypographs are convex.

Consequently Λ g , the projection of a hypograph onto ∆ B , is a convex set.

Furthermore, due to monotonicity of conditional entropy, i.e., expected value of entropy over observations, we know that p 0 ∈ Λ g .

Besides, Due to optimality of ζ * , it holds that DISPLAYFORM8 which in turn yields p * ∈ ∆ B \Λ g .

FIG10 demonstrates these facts for an alphabet of size 3.

p g is the I-projection of p * on Λ g .

Therefore, by exploiting the analogue of Pythagoras' theorem for KL-divergence (Csiszár, 1975), we conclude: DISPLAYFORM9 A direct result of the above lemma, after taking the expectation over i∈[n] ω i , is: DISPLAYFORM10 In the following theorem, we use the stated lemma to bound the expected KL-divergence distance between greedy and optimal selection strategies.
...
Notice that while KL-divergence is not symmetric, the following fact still holds: where (a) follows from the fact that α * is the gradient of the optimal value function, (b) is due toHölder's inequality, and (c) is the result of Theorem 2.

Taking C 2 = C 1 max{|Rmax|,|Rmin|} 1−γ yields the desired result.
...
To that end, consider a fixed set of sampled belief points B. Let Γ to denote the current set of α vectors.

Further, for the simplicity of analysis, assume that the number of possible observations from each information source is |Ω i | =Ω, ∀i ∈ [n].
...
Here the objective function is the conditional entropy whose complexity with a naive approach in the k th iteration is O(Ω k × |S| 2 ).

Therefore, applying Algorithm 3 as the backup step leads to O(|A pl | × |B| × n × k ×Ω k × |S| 2 + |A pl | × |B| ×Ω k × |Γ| × |S| 2 + |B| × |A pl | × |S| ×Ω k ) operations.
