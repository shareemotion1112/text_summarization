...
We design filtering to be a two-stage pseudo-continuous process to simulate discrete cut offs (Π move , Π new ), while keeping reference history.
...
Memory bank creation (Π new ), which creates a new memory bank depending on the current states of entities m i .

If the entropy, or information contained (explained below), of m i is too high, Π new (m i ) will learn to create a new memory bank m i+1 to reduce entropy.2.

Moving entities across banks (Π move ), which determines which entities are relevant to the current question and move such entities to further (higher importance) memory banks.3.

Adding/Updating entities in a bank (Π au ), which adds entities that are not yet encountered to the first memory bank m 0 or if the entity is already in m 0 , the operation updates the entity state.4.

Propagating changes across entities (Π prop ), which updates the entity states in memory banks based on node current states Π prop (M ) and their semantic relationships.

This is to communicate transitive logic.

Both Π new , Π move require a discrete decision (refer to section 4.2.1.), and in particular, for Π new we introduce the notion of entropy.
...
As mentioned earlier, discrete decisions are difficult for neural networks to learn so we designed a specific memory bank controller Π ctrl for binary decision making.

The model takes ideas from the reparameterization trick and uses custom backpropagation to maintain differentiability.

In particular, the adaptive memory module needs to make two discrete decisions on a {0, 1} basis, one in Π new to create a new memory bank and the other in Π move to move nodes to a different memory bank.
...
The operations below will be denoted as Π ctrl and has two instances: one for memory bank creation Π new and one for moving entities across banks Π move .

In equation 9, depending on what Π ctrl is used for, q is a polymorphic function and will take on a different operation and * will be a different input.

Examples of such are given in their respective sections (4.2.2.1, 4.2.2.2).

DISPLAYFORM0 4.2.2 MEMORY BANK OPERATIONS 1.

Memory bank creation Π new : To determine when a new memory bank is created, in other words, if the current memory bank becomes too saturated, the memory bank controller (4.2.1.) will make a discrete decision to create a new memory bank.
...
Moving entities through memory banks: Similar to Π new , individual entities' relevance scores are passed into the bank controller to determine H as the input.
...
Loss is comprised of two parts, answer loss, which is computed from the given annotations, and secondary loss (from Π new , Π move ), which is computed from sentence and story features at each sentence time step l 0...N .
...
Nodes that are not in the connected component containing a are similarly sampled from a Gaussian centered from 0.3 with a variance of 0.1 (capped at 0).p li is initially set to 0.8 and β varies depending on the story length from 0.1 ≤ β ≤ 0.25.
