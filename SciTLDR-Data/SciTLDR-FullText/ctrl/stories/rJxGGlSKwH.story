...
We learn a scoring function h θ which assigns large scores to positive paired samples (x, x + ) while low values for negative pairs (x, x − k ).
...
As in Logeswaran & Lee (2018) the used scoring function is the inner product h θ (u, v) = u T v in order to avoid the situation where the model learns poor representations but compensates with an excellent classifier on the proxy task.
