...
We define a model in terms of a loss function L with parameters θ as a sum over training examples

, where is an example-wise loss (e.g., mean-squared error or cross entropy).

Let θ be the parameters of the trained model, obtained by, e.g., minimizing the loss over this dataset, i.e., θ = arg min θ L(θ).

We write the prediction function given by parameters θ at an input x asŷ(x, θ).
...
Our score has a simple form: it is the norm of the prediction gradient g θ (x ) := ∇ θŷ (x , θ ) multiplied by a matrix of Hessian eigenvectors spanning a subspace of low curvature U m (defined in more detail below).
...
The spectral decomposition of the Hessian H θ plays a key role in our derivation.

Let

where U is a square matrix whose columns are the orthonormal eigenvectors of H θ , written (ξ (1) , · · · , ξ (p) ), and Λ is a square, diagonal matrix with the eigenvalues of H θ , written (λ (1) , · · · , λ (p) ), along its diagonal.
...
In particular, under the assumption that the model has been trained to a local minimum or saddle point, parameter perturbations in flat directions (those corresponding to small eigenvalues λ (j) ) do not change the training loss substantially (see Fig. 1 ).
...
Formally, let m be the eigenvalue index such that the eigenvalues {λ (j) : j > m}, are sufficiently small to be considered "flat".

2 We call the subspace spanned by {ξ (j) : j ∈ σ} the ensemble subspace.

Parameter perturbations in the ensemble subspace generate an ensemble of models with near-identical training loss.

Our score E m (x ) characterizes the variance of test predictions with respect to random parameter perturbations ∆ θ that occur in the ensemble subspace.
...
Let ∆ θ be a random parameter perturbation with mean zero and covariance proportional to the identity · I, projected into the ensemble subspace spanned by {ξ (j) : j > m}. Let P ∆ (x ) be the linearized change in a test prediction at x induced by perturbing the learned parameters θ by ∆ θ :

Proof.

First, we characterize the distribution of the loss-preserving parameter perturbation ∆ θ .

Let U m be the matrix whose columns {ξ (j) : j > m} span the ensemble subspace.

Then U m U m is a projection matrix that projects vectors into the ensemble subspace, so the projected perturbation ∆ θ has covariance · U m U m .
...
For each x , we compare, across the ensemble, the mean value of E m (x ) to the standard deviationŷ(x ).
...
first-order term drops out because we assume θ lies at a local minimum or saddle point.

Perturbations ∆ θ that lie in the subspace spanned by eigenvectors ξ (j) whose corresponding eigenvalues λ (j) are close to zero contribute negligibly to this sum.
...
Specifically, let U m ⊥ be the matrix whose columns are these large-eigenvalue eigenvectors {ξ (j) : j ≤ m}. Then U m ⊥ U m ⊥ is the projection matrix that projects vectors into the "large eigenvalue" subspace, and I − U m ⊥ U m ⊥ projects into its complement: the ensemble subspace.
...
This metric is derived by interpreting the loss function as being equivalent to a Bayesian log-posterior distribution over the model parameters θ, and approximating it with a Gaussian.
...
Additionally, a range of methods exist for related tasks, such as OOD detection (Choi & Jang, 2018; Gal & Ghahramani, 2015; Schölkopf et al., 2001 ) and calibration (Naeini et al., 2015; Guo et al., 2017) .
...
We can make this extrapolation detection task easier by generating several features this way, or make it harder by adding some noise ∼ N (0, σ 2 ) to these features.
...
Firstly, we note the performance of MaxProb and the loss gradient variant of our method are quite correlated, which we hypothesize relates to ∇Ŷ .
...
Our data is generated from y = βx 2 + N (0, 1).
...
We choose 2 random features i, j and a number β ∼ U(0, 1), and generate the new featurex
...
E.3 RELATIONSHIP BETWEEN LOSS GRADIENT AND MaxProb METHOD As discussed in Sec. 5.3, we have the relationship between the loss , predictionŶ , and parameters θ: ∇ θ = ∇Ŷ · ∇ θŶ .

Using min as an aggregation function, we find that min Y ∈{0,1} |∇Ŷ (Y,Ŷ )| has an inverted V-shape (Fig. 15) .

This is a similar shape to 1 − max(Ŷ , 1 −Ŷ ), which is the metric implicitly measured by MaxProb.
...
We then choose a standard deviation σ equal to the average magnitude of the perturbations created through MC-dropout on the corresponding task.
