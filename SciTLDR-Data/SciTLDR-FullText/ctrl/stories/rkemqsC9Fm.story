...
Recently, BID0 exploited the β-VAE loss function of BID8 to explicitly introduce a trade-off between rate and distortion in the latent variable modeling problem, where the notions of rate and distortion have similarities to those used in our article.
...
Lemma 1 For any conditional distribution Q(z|x), p(z), (x|z), and for any x, and α > 0, DISPLAYFORM0 Taking the expectation w.r.t.
...
The twist is that in information theory, these lower bounds apply to (8), where the object being optimized is the test channel Q Z|X whereas in latent variable modeling we want them to apply directly to (1), where the object being optimized is the prior p(z).The beginning point is an elementary result, inspired by the arguments in Theorem 2.5.3 of BID2 , which can be proved using the inequality log 1 u ≥ 1 − u: Lemma 2 For any p(z), and ν(x) > 0 DISPLAYFORM0 Taking the expectation with respect to X in Lemma 2 we obtain DISPLAYFORM1 dz then we obtain DISPLAYFORM2 We now eliminate any dependence on p(z) on the right hand side by taking the sup, and then take the min over p(z) on both sides, obtaining that for any τ (x) > 0, DISPLAYFORM3 The p(z) we eliminated refers to the "optimal" prior, which we don't know (hence why we want to eliminate it).

We now bring the "suboptimal" prior: since the above is true for any τ (x) > 0, set τ (x) = p(x), where p(x) is the latent variable model (1) for a given (p(z), (x|z)).

Note now that the expression on the left is a log likelihood only for α = 1, so choosing this value and assuming that X is distributed uniformly over the training data {x 1 , · · · , x N }, then we obtain Theorem 2 (information theoretic lower bound on negative log likelihood) DISPLAYFORM4 This bound is sharp.
...
We regard p,p as probability measures used to measure sets from the σ-algebra σ(Z).

In the Appendix, we prove that if g(z) is a measurable function g : (Z, σ(Z)) → (Z, σ(Z)) the two following Lebesgue integrals are identical: (x i |g(z))dp(z) = (x i |y)dp(y)The last integral in (15) is identical to ith integral in the left hand side of (14).

Therefore, one can define a newˆ (x|z) = (x|g(z)), and the negative log likelihood of the latent variable model (p(z),ˆ (x|z)) will be identical to that of (p(z), (x|z)).
...
In order to prove this, we claimed that if g(z) is a measurable function g : (Z, σ(Z)) → (Z, σ(Z)) the two following Lebesgue integrals are identical:(x i |g(z))dp(z) = (x i |y)dp(y)This can be seen as follows: for a measurable set DISPLAYFORM0 and therefore setting for any given real η, B = [ ( DISPLAYFORM1 One definition of the Lebesgue integral is through the Riemann integral:(x i |g(z))dp(z) = ([ (x i |Y ) > η])dη = (x i |y)dp(y)This demonstrates (15).
