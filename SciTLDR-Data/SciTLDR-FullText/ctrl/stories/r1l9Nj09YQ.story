...
Furthermore, let j α represent a particular language.

Let p jα (·) represent the language model for the j α th language and w jα represents a word from the j α th language.

Let k jα represent a distributed representation of a specific language along the lines of the PnP argument (Culicover, 1997).

UG, through the lens of statistical language modeling, hypothesizes the existence of a factorization of p jα (·) containing a language agnostic segment.
...
e −1 jα maps our decoded representation back to the token space.

For the purposes of distribution matching we utilize the GAN framework.

Following recent successes we use Wasserstein-1 as our distance function d .Given two languages j α and j β the distribution of the universal representations should be within with respect to the W 1 of each other.
...
Therefore the complete loss function for m languages each containing N documents becomes: DISPLAYFORM3 λ is a scaling factor for the distribution constraint loss.
...
For e jα in equation 1 we use a language specific embedding table followed by a LSTM BID17 jα is non trivial therefore we use another language specific LSTM whose outputs we multiply by the transpose of the embedding table of e jα to obtain token probabilities.
...
The α, βth index in the matrix correspond to the function output of f in calculating DISPLAYFORM0
...
We trained each language model for 14 days on a NVidia Titan X. For each language model we would do a sweep over λ, but in general we have found that λ = 0.1 works sufficiently well for minimizing both perplexity and Wasserstein distance.
...
What commonalities between languages can we learn and are they informative enough to be exploited?We can test out the usefulness of the distribution matching constraint by running an ablation study on the λ hyper-parameter.

We trained UG-WGAN on English, Spanish and Arabic wikidumps following the procedure described above.

We kept all the hyper-parameters consistent apart for augmenting λ from 0 to 10.
...
IMDB ChnSentiCorp SB-10KNMT + Logistic BID32 12.44% 20.12% 22.92% FullUnlabeledBow BID24 11.11% * * NB-SVM TRIGRAM BID27 8.54% 18.20% 19.40% UG-WGAN λ = 0.1 + Logistic (Ours) 8.01% 15.40% 17.32% UG-WGAN λ = 0.0 + Logistic (Ours) 7.80% 53.00% 49.38% Sentiment Neuron BID31 7.70% * * SA-LSTM BID13 7.24% * * We also compare against encodings learned as a by-product of multi-encoder and decoder neural machine translation as a baseline BID22 .
...
One hypothesis might be that as we increase λ the cross lingual generalization gap (difference in test error on a task across languages) will vanish.

To test this hypothesis we conducted the same experiment where UG-WGAN was trained with a λ ranging from 0 to 10.
...
Increasing λ doesn't seem to have a significant impact on the generalization gap but has a large impact on test error.

Our hypothesis is that a large λ doesn't provide the model with enough freedom to learn useful representations since the optimizations focus would largely be on minimizing the Wasserstein distance, while a small λ permits this freedom.
...
We see from Figure 2 that perplexity worsens proportional to λ.

We explore the differences by sampling sentences from an unconstrained language model and λ = 0.1 language model trained towards English and Spanish in Table 3 .
