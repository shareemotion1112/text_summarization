...
For node features, we compute the three dihedral angles of the protein backbone (φ i , ψ i , ω i ) and embed these on the 3-torus as {sin, cos} × (φ i , ψ i , ω i ).

We also consider 'flexible backbone' descriptions of 3D structure based solely on topological binary edge features.

We combine the relative positional encodings with two binary edge features: contacts that indicate when the distance between C α residues at i and j are less than 8 Angstroms and hydrogen bonds which are directed and defined by the electrostatic model of DSSP BID17 .
