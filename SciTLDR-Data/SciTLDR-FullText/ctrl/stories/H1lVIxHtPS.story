...
Actions are selected through a stochastic policy π π π θi : (Silver et al., 2014) with policy parameters θ i , and a new state is generated by the environment according to the transition function T :
...
where T is the time horizon and γ is the discount factor.

We choose the deterministic policy gradient algorithms for all our training.

In this framework, the parameters θ of the policy, µ µ µ θ , are updated such that the objective J(θ) = E s∼p π π π ,a∼µ θ [R(s, a)] and the policy gradient, (see Appendix A), is given by:

Deep Deterministic Policy Gradient (DDPG) is an adaptation of DPG where the policy µ µ µ and critic Q µ µ µ are approximated as neural networks.
...
Each agent's policy is instantiated similar to DDPG, as µ µ µ θi (a i |o i ) conditioned on its local observation o i .
...
An agent's memory is then used to predict the action of the agent, such that policy is given by µ µ µ θ : O i × M i → A i , where M i is the memory of agent i.
...
where ϕ a θ a i is a multi-layer perceptron (MLP) parameterised by θ a i .
...
The policy network µ θi produces actions a i , which is then evaluated by the critic Q µ θ i , which aims to minimize the loss function.

, are the observations when actions {a 1 , a 2 , ..., a N } are performed, the experience replay buffer, D, contains the tuples (x, x , m 1 , m 2 , ..., m N , a 1 , a 2 , ...a N , r 1 , r 2 , ...r N ), and the target Q-value is defined as y. To keep the training stable, delayed updates to the target network Q µ θ i is implemented, such that current parameters of Q µ θ i , are only copied periodically.

The goal of the loss function, L(θ i ) is to minimize the expectation of the difference between the current and the target action-state function.

The gradient of the resulting policy, with communication, to maximize the expectation of the rewards, J(θ i ) = E[R i ], can be written as:
...
In the PG framework, the parameters θ of the policy are directly adjusted to maximize the objective J(θ) = E s∼p π π π ,a∼π π π θ [R], by taking steps in the direction of ∇ θ J(θ), where p π π π , is the state distribution, s is the sampled state and a is the action sampled from the stochastic policy.

Through learning a value function for the state-action pair, Q π π π (s, a), which estimates how good an optimal action a is for an agent in state s, the policy gradient is then written as, (Sutton et al., 2000) :

Several variations of PG have been developed, primarily focused on techniques for estimating Q π π π .

For example, the REINFORCE algorithm (Williams, 1992) uses a rather simplistic method of sample return calculated as a cumulative expected reward for an episode with a discount factor γ, R t = T i=t γ i−t r i .

When temporal-difference learning (Sutton et al., 1998 ) is used, the learned function Q π π π (s, a) is described as the critic, which leads to several different actor-critic algorithms (Sutton et al., 1998), (Konda & Tsitsiklis, 2000) , where the actor could be a stochastic π π π θ or deterministic policy µ µ µ θ for predicting actions.
...
All policies are trained using the Adam optimizer with a learning rate of 5 × 10 −4 , a discount factor, γ = 0.96.

and τ = 0.001, for the soft update of the target network.
...
Exploration noise is implemented through Orhnstein-Uhlenbeck process with θ = 0.15 and σ = 0.3.
