...
Given an image dataset {(x 1 , y 1 ) · · · (x M , y M )} and a classifier F θ that has been trained with this dataset, with parameters θ, the aim of the attack is to produce an adversarial image x adv i such that F θ (x adv i ) = y i , and ||x adv i − x i || is small.
...
In adversarial training, the CNN model is trained on adversarial examples generated from itself (Madry et al., 2017; Kurakin et al., 2016b) or from an ensemble of models (Tramèr et al., 2017) .
...
Transformation-based defenses aim to recover from adversarial perturbations, that is for input transformation T , we want F θ (T (x adv i )) = y i .
...
Let σ x,j , for j = 1, · · · , C, be the j-th component of the softmax vector.
...
Let h x,j be the distribution accumulated from σ

where δ is the kernel width and s ∈ [0, 1] is the support of the softmax output.
...
In Figure 4 , we show more examples of the distributions obtained from clean images (A-H) and their adversarial counterparts (Ã-H) at d=300.

For clean images A-D, voting predicts correctly but on the adversarial counterpartsÃ-D, voting predicts wrongly.

For clean images E-H and the adversarial counterpartsẼ-H, voting predicts wrongly.
...
With the random image transformation, there are similarities in the distribution shapes between the clean and adversarial images, as shown by the groupings and arrows (eg. between E andÃ,Ẽ,F ).
...
In fact, all of the clean (A-H) and adversarial (Ã-H) images (class 6) are classified correctly by the distribution classifier.

Even though the distribution classifier was only trained on distributions from the clean images (A-H), the distribution classifier can recover the correct class for the adversarial images where voting has failed (Ã-H).
...
For PD, d is the number of deflections, w is the window size and σ is the denoising parameter.

, w=20, σ=0 d=100, w=20, σ=0 d=10, w=20, σ=0.08 d=100, w=25, σ=0.08 RRP resize range= [4, 28] resize range= [22, 28] resize range= [23, 28] resize range= [23, 28] not cause significant difference in performance.
