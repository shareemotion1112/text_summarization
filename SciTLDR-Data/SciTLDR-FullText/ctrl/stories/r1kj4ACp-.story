...
Y , the task is to find a good prediction of Y using X. The predictionŶ needs to maximize the conditional entropy H(Ŷ |X) while preserving the same distribution with data (X, Y ).
...
According to the above definition of feature T , feature-based maximum entropy model can be formulated as : DISPLAYFORM0 DISPLAYFORM1 DISPLAYFORM2 DISPLAYFORM3 where λ i (y) and b(y) denote functions of y: λ i (y) is weight for feature T i , and b(y) is the bias term in softmax regression.
...
Since T is a set of random variables only related to X, it is obvious to have DISPLAYFORM1 So the task leaves to prove DISPLAYFORM2 Recall that T is a set of random variables only related to X, then DISPLAYFORM3 We further have T satisfying condition 1: DISPLAYFORM4 Similarly, X → T →Ŷ is Marcov chain, hence we have: DISPLAYFORM5 T is defined feature function on X, so P (X|T ) is a constant.

We further have: DISPLAYFORM6 Note that E P (X,Y ) = E P (X,Ŷ ) indicates that the predicate functions satisfy Eqn.

FORMULA3 in the definition of original ME model, and thus is equivalent to P (X, Y ) = P (X,Ŷ ).With Eqn.
...
Since T is a set of random variables only related to X, we have DISPLAYFORM9 X → T →Ŷ is Marcov chain, we have: DISPLAYFORM10 Additionally , DISPLAYFORM11 So we can derive: DISPLAYFORM12 Lemma 3.
...
We then have: DISPLAYFORM0 where ω denotes variable about y.

We further define b(y) = ∑ i ω i0 and λ i (y) = (ω i1 − ω i0 ), then the solution of Eqn.
...
Note that t i (x) need not to be in range [0, 1] when we use the softmax model, as we can change λ and b to achieve translation and scaling.

where S(T ) denotes the output of softmax model if input is T .Proof.

Since T is only related to X, T i → X → T j is Marcov chain, and DISPLAYFORM3 We can relax the minimization problem to minimize its upper bound instead, so Note thatŶ = S(T ): X → T →Ŷ is M arcov chain Recall thatŶ is solution to the problem P 1 , and P 1 has constraint E P (X,Y ) = E P (X,Ŷ ) .
