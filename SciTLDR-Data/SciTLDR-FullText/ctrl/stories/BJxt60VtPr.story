...
where α is the margin size, and Y is 1 at indices where T corresponds to B, and −1 everywhere else.
...
Then, we generate several 3D rotations of the second tensor, representing "candidate rotations", making a set {F (t) θi |θ i ∈ Θ}, where Θ is the discrete set of 3D rotations considered.

We then use 3D axis-aligned cross-correlations between F

(1) and the F (t)

θi , which yields a cost volume of shape r × w × h × d × e, where e is the total number of spatial positions explored by cross-correlation.
...
We use the Adam optimizer, with β 1 = 0.9, β 2 = 0.999.
...
Method R (rad) t (m) ORB-SLAM2 (Mur-Artal & Tardós, 2017) 0.089 0.038 SfM-Net (Zhou et al., 2017) 0.083 0.086 SfM-Net + GT depth 0.100 0.078 Ours 0.120 0.036 Table 5 : Egomotion error.

Our model is on par with the baselines.

We compare our egomotion module against ORB-SLAM2 (Mur-Artal & Tardós, 2017), a state-ofthe-art geometric SLAM method, and against the SfM-Net (Zhou et al., 2017) architecture, which is a 2D CNN that takes pairs of frames and outputs egomotion.
