...
The objective function of the VAE is as follows: DISPLAYFORM0 where φ, θ are parameters of the encoder and decoder of the VAE, respectively.

The lower bound of DualVAE is as follows: DISPLAYFORM1 where p w (y|z) = Bern(y|σ(Uz)).
...
At the same time, we find the values of θ and φ that maximize the lower bound in Equation 1.We transfer the images on domain i by performing the following operation.

We calculated the following vector w i : DISPLAYFORM2 where λ(∈ R) is a parameter.

Image transfer can be demonstrated by changing λ and decoding w i .

Equation 3 corresponds to moving a latent vector toward a decision boundary.

Require: images (x j ) m j=1 , batch size M , indicator function I ij VAE/encoder optimizers: g, g e , hyper parameter α, and the label matrix Y = (y ij ).

Initialize encoder parameter, decoder parameter and dual vector: θ, φ, U = (u i ) DISPLAYFORM0
...
We calculated the rs and ts values when applying Algorithm 2 on 160 CelebA test images and plotted the figure below when we changed the missing ratio of CelebA's domain labels and the λ in Equation 3.
