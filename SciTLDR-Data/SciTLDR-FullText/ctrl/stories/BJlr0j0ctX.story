...
Let z y denote the logit corresponding to the correct class y. If we add a small perturbation δ to x, then the corresponding change in logits is approximately δ T ∇ x z y under a linear approximation, where ∇ x z y is the gradient of z y with respect to x.

Under a linearity assumption, we can calculate the step-size L needed to move an example from class y to another classȳ.

A classifier is susceptible to adversarial perturbation δ if the perturbed logit of the true class is smaller than the perturbed logit of any other class: DISPLAYFORM0 Assuming a one-step ∞ attack such as FGSM, the perturbation δ can be expressed as DISPLAYFORM1 where L is the ∞ -norm of the perturbation.

Using this choice of δ, Equation 2 becomes Table 1 : Experimental and predicted accuracy of classifiers for MNIST and CIFAR-10.
...
Squash the adversarial gradients: Train a classifier that has small adversarial gradients ∇ x zȳ for any classȳ.

In this case a large perturbation is needed to significantly change zȳ.• Maximize gradient coherence: Produce adversarial gradients ∇ x zȳ that are highly correlated with the gradient for the correct class ∇ x z y .

This will shrink the denominator of equation 4, and produce robustness even if adversarial gradients are large.

In this case, one cannot decrease z y without also decreasing zȳ, and so large perturbations are needed to change the class label.
...
FIG1 shows that adversarial training does indeed squash the logits, although not enough to fully explain the decrease in |z y − zȳ| in FIG0 .
...
A one-hot label vector y hot is smoothed using the formula DISPLAYFORM0 where α ∈ [0, 1] is the smoothing parameter, and N c is the number of classes.

If we pick α = 0 we get a hard decision vector with no smoothing, while α = 1 creates an ambiguous decision by assigning equal certainty to all classes.
...
Logit squeezing relies on the loss function DISPLAYFORM1 where β is the squeezing parameter (i.e., coefficient for the logit-squeezing term) and ||.|| F is the Frobenius norm of the logits for the mini-batch.
...
From the robustness plots in FIG2 , we can see that training with Gaussian noise alone produces a noticeable change in robustness, which seems to be mostly attributable to a widening of the logit gap and slight decrease in the gradient gap ( ∇ x z y − ∇ x zȳ 1 ).
...
See the results on MNIST in Table 2 and the results on CIFAR-10 in TAB3 for various values of σ (standard deviation of Gaussian noise).
...
Larger smoothing parameters α cause the gradient to be both smaller and more aligned.
...
We get more robust models when we use larger smoothing (α) and squeezing (β) parameters, and when Gaussian augmentation is used with standard deviation σ that is greater than the desired (the maximum perturbation size).
...
We see that aggressive logit squeezing with squeezing parameter β = 10 and σ = 20 results in a model that is more robust than the adversarially trained model from BID9 BID9 63.39%* 64.38%* 63.39%* 64.38%* 67.00% 67.25% Table 4 : Black-box attacks on CIFAR-10 models.
...
BID0 suggest these models can be identified by performing "sanity checks" such as attacking them with Figure 5 : The cross-entropy landscape of the first eight images of the validation set for the model trained for 160k iterations with hyper-parameters β = 10 and σ = 30.
...
We run the stronger PGD attacks on a sample model from TAB3 with hyper-parameters k = 160k, β = 10, and σ = 30.
...
As can be seen, Gaussian random augmentation by itself (β = α = 0) is effective in increasing robustness on black-box attacks.

It does not, however, significantly increase robustness on white-box attacks.

Models that are only trained with either logit squeezing or label smoothing without random Gaussian augmentation (σ = 0), can be fooled by an adversary that has knowledge of the model parameters (white-box) with accuracy 100%.

In the black-box setting, they are also not robust.

Increasing the magnitude of the noise (σ) generally increases robustness but degrades the performance on the clean examples.

Keeping the number of iterations k constant, for extremely large σ Table 6 : Accuracy of different models trained on MNIST with a 40 step PGD attack on the crossentropy (X-ent) loss and the Carlini-Wagner (CW) loss under the white-box and black-box threat models.
...
In Table 9 by performing an unbounded PGD attack on a sample logit-squeezed model for the CIFAR-10 (β = 10, σ = 30, and k = 160k), we verify that our attack is not benefiting from obfuscating the gradients.
...
We attack our sample hardened CIFAR-10 model (β = 10, Gaussian augmentation σ = 30 model, and k = 160k iterations), by performing many random restarts.
...
Table 11 : The effect of the number of steps of the white-box PGD attack on the CW loss (worst case based on TAB3 ) for the model trained in 160k steps with logit squeezing parameters β = 10 and σ = 30 on CIFAR-10 dataset.

The model remains resistant against ∞ attacks with = 8 Figure 9 : The cross-entropy landscape of the first eight images of the validation set for the model trained for 160k iteration and hyper-parameters β = 10 and σ = 30.
...
2) with random noise, and logit squeezing (β = 0.5) with random noise.

In all cases, the noise is Gaussian with σ = 0.5.
...
FIG0 : For MNIST, we plot the cumulative sum of activation magnitudes for all neurons in logit layer of a network produced by natural training, adversarial training, natural training with random noise, label smoothing (LS = 0.2) with random noise, and logit squeezing (β = 0.5) with random noise.
