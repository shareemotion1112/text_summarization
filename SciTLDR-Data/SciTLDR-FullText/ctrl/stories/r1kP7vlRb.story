...
Given a dataset of real-world sequences, parameterized generator G θ is trained to produce a sequence Y 1:T = {y 1 , ..., y t , ..., y T }, y t ∈ Y, where Y is the vocabulary of candidate tokens.

G θ is trained to produce a sequence that is similar to real data.

SeqGAN considers this problem as RL, considering G θ to produce action (next token y t ) given state (previously generated tokens Y 1:t−1 ).SeqGAN trains parameterized discriminator D φ as well as generator G θ .

Like GAN training, D φ is trained to discriminate between real data and generated data from G θ .

At the same time, G θ is trained via policy gradient by seeing D φ as a reward function.
...
The objective of RL can be stated as: DISPLAYFORM0 where s is state, a is action, r is reward, d(s) is the probability to encounter the state, π is the policy, and Q(s, a) is the action-state value.
...
To prevent a generator from becoming deterministic, it is common to add an entropy regularizer BID15 to Eq.(2), that is, DISPLAYFORM2 where H(s) = − a π(s, a)logπ(s, a) denotes the entropy, and β is the hyper parameter.
...
A filter of window size h is applied to Y L D i to produce a feature map DISPLAYFORM0 , where σ is a nonlinear activation function, b is a bias vector, and * denotes the convolutional operator.
...
We get as manyĉ h as the number of filters with varying window sizes.

Thosê c h are concatenated to produceĉ = [...,ĉ h , ...], and finally the output DISPLAYFORM1 produced by the fully connected layer.

To enhance the performance, we add the highway architecture BID19 before the final fully connected layer.

We specifically describe howr is produced fromĉ in appendix.
...
Given the partial reward function D i , the action-state value can be derived as: DISPLAYFORM0 where N is the number of roll-out trajectories, α Di is the reward scaler for D i , and γ is the discount value.

The hyperparameter α Di has a role to adjust an importance of a partial reward function with DISPLAYFORM1 Figure 3: Overview of expert-based reward function training.
...
To measure the goodness of edited sequences, we introduce the quality function DISPLAYFORM0 where r (x) ≤ 0 is the pseudo reward function and τ is the temperature parameter.
...
τ controls how fast q(x) decreases as a given sequence is getting edited from an expert.

When τ is small, q(x) rapidly becomes close to 0, and when τ is large, q(x) slowly becomes close to 0.
...
As we mentioned, τ controls how fast q(x) goes to 0 as a sequence is getting edited so τ is expected to determine a smoothness of a learned reward function, because as τ gets larger, a loss for a little edited sequence is eased.

When τ 0, Eq. (8) is the same as conventional BCE shown in Eq.(6) for all edited sequences.
...
We validate the effectiveness of this modification in the experiment by seeing if the generated sequence is better when τ is large.
...
Let G θ and Y denote the generator and the generated completed sequence.
...
We employ L2 regularization to the reward function only when it is trained with adversarial training, and the hyper parameter for the regularization term λ is set to be 0.01.
...
The reward scaler α is set to be always 1.0.
...
At the test, G θ generates 10,000 samples by stochastically choosing tokens according to the generator's output distribution and calculate the average N LL oracle over the samples.
...
In adversarial training, the reward function is first pretrained with G θ trained with MLE.

Then, the reward function is iteratively trained with G θ .
...
Then, the generator G θ is trained with policy gradient until its convergence.
...
We found that the performance of expert-based reward function training depends on a temperature τ .

TAB2 shows the oracle score and τ values in proposed training method.

For short-term partial reward function, τ 0 gives good performance, indicating that the generator prefers strict short-term reward function.

For long-term partial reward func- tion, τ = 1.5 gives good performance, indicating that the generators prefer a smooth reward function, which gives reward to some extent when the partial sequence is different from real data.
...
they will be PG SL exp (α S = 0.3) 0.240 according to the financial times , a new record is expected to go on an announcement on the premiership , an PG SL exp (α S = 1.0) 0.272 according to the report , the survey showed that it will be able to be recycled up for the two companies Table 4 : The result of text generation.
...
In this experiments, we change the scaler parameter for short-term reward α S in PG SL exp.

The scaler parameter for long-term reward α L is set to be always 1.0.
...
The hyper parameter β in Eq. FORMULA2 is first set to be 0.02, and after 15 epochs, β is set to be 0.05.

This is because, when β is high at the beginning of the training of the generator, an optimization w.r.t.
...
By decreasing the short-term reward scaler α S , we can generate more coherent sentences.
...
In our study, this balance is tuned by the hyperparameter α Di .

Unfortunately, the tuning of α Di is difficult.

Even validating the goodness of selected α Di is difficult because there is usually no true metric for the generated sequence (except for the special case such as oracle test).

Therefore, we have to validate the selected α Di by seeing the generated sentences.
...
We describe how to get a feature map containing each filter's outputĉ in section 3.1.

We, then, add highway architecture as below, DISPLAYFORM0 where W T , b T , and W H are highway layer weights, H denotes an affine transform followed by a nonlinear activation function such as ReLU, and τ is the "transform gate" with the same dimensionality as H(ĉ, W H ) andĉ.

We expect the hyperparameter τ to determine the smoothness of the learned reward function.

We describe more specifically how τ determines the smoothness.

When τ is very small, q(x ) 0 for all edited sequences, therefore w(x ) 1.
...
When τ is large, w(x ) decreases for an edited sequence that is little different from an expert one.
