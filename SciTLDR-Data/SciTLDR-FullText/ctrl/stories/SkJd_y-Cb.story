...
R is a feed-forward neural network with parameters (i.e., weights and intercepts)ˇv.

The number of neurons of the input layer is K, equal to the length of the context vectors˛v.

Essentially, we have replaced the per-term embedding vectors v with a per-term neural networkˇv.
...
Iˇv/ to be a three-layer neural network, DISPLAYFORM4 Replacing vectors with neural networks has several implications.
...
Then, for each occurrence of term v in group t we setˇ.`/ v Dˇ.`/ t .
...
To model the observation at position n, we use a neural network specific to that term/tag combination, DISPLAYFORM5 That is, the neural network parameters are combined to form a neural network in which layer`has parametersˇ.`/ t and the other layers have parametersˇ.:`/ v. Thus, we leverage the information about the tag t by replacingˇ.`/ v withˇ.`/ t in layer`, resulting in parameter sharing at that layer.

If the same term v appears at a different position n 0 with a different tag t 0 , at location n 0 we replace the parametersˇ.`/ v of layer`withˇ.`/ t 0 .
...
w ;ˇ.`/ t / > f .˛Iˇ.:`/ v ;ˇ.`/ s / jjf .˛Iˇ.:`/ w ;ˇ.`/ t /jj 2 jjf .˛Iˇ. DISPLAYFORM7
...
In summary, under certain assumptions Bernoulli embeddings are equivalent to with negative sampling, and are related to skip-gram through Jensen's inequality.-Á (negative sampling)First we explain how Bernoulli embeddings and with negative sampling are related.
