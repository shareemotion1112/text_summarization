...
Treat the data as a single-agent trajectory and train a single model: DISPLAYFORM2 Train independent models for each agent: θ = {θ 1 , . . .

, θ K }.As we empirically verify in Section 5, VRNN models using these two approaches have difficulty learning representations of the data that generalize well over long time horizons, and capturing the coordination inherent in multi-agent trajectories.
...
Here, we train an RNN-model to sample macro-intents: DISPLAYFORM4 where ϕ g maps to a distribution over macro-intents and h g,t−1 summarizes the history of macrointents up to time t.
...
We also introduce more stochasticity into the model by periodically updating β randomly.
...
A RNN models the conditional probabilities in Eq.(1) with a hidden state h t that summarizes the information in the first t − 1 timesteps: DISPLAYFORM0 where ϕ maps the hidden state to a probability distribution over states and f is a deterministic function such as LSTMs BID13 or GRUs BID5 .
...
A variational autoencoder (VAE) BID18 ) is a generative model for non-sequential data that injects latent variables z into the joint distribution p θ (x, z) and introduces an inference network parametrized by φ to approximate the posterior q φ (z | x).

The learning objective is to maximize the evidence lower-bound (ELBO) of the log-likelihood with respect to the model parameters θ and φ: DISPLAYFORM1 The first term is known as the reconstruction term and can be approximated with Monte Carlo sampling.
...
The inference model q φ (z | x), generative model p θ (x | z), and prior p θ (z) are often implemented with neural networks.
...
The corresponding ELBO is: DISPLAYFORM0 where p θ (z) is the prior, q φ (z | x ≤T ) is the encoder, and p θ k (x k t | x <t , z) are decoders per agent.

It is intractable to compute the mutual information between z and x ≤T exactly, so we introduce a discriminator q ψ (z | x ≤T ) and use the following variational lower-bound of mutual information: DISPLAYFORM1 We jointly maximize L 1 + λL 2 wrt.

model parameters (θ, φ, ψ), with λ = 1 in our experiments.
