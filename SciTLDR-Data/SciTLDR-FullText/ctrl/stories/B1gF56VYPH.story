...
where θ t and θ sr parameterize the t-net and the sr-block respectively.
...
For the training of all models, we used the Adam optimizer (Kingma & Ba, 2014) with the recommended β's (0.5 and 0.999) for the regression task with a batch size of 8 for 50 epochs.
...
Similar "tricks" have been used in previous works in the form of adding noise to the input of discriminators (Sønderby et al., 2017; Zhang et al., 2019) to make them invariant to noise, and more sensible to structures.
...
The mean square error of the output of the first three max-pooling layers from the pre-trained V GG19 (Simonyan & Zisserman, 2014) , denoted by φ l , was utilized as the perceptual loss function.

To balance the contributions of the l 1 and perceptual losses, a constant α p = 0.01 was introduced.
