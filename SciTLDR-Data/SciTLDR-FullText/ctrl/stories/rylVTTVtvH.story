...
LetÎ ∈ R N ×N ×T be defined facewise asÎ ::t = I, where I is the matrix identity.
...
For a graph with adjacency matrix A and feature matrix X, a GCN layer takes the form Y = σ(ÃXW), wherẽ

is the matrix identity, W is a matrix to be learned when training the NN, and σ is an activation function, e.g., ReLU.

Our approach translates this to a tensor model by utilizing the M-product framework.

We first introduce a tensor activation functionσ which operates in the transformed space.

Definition 4.1.

Let A ∈ R I×J×T be a tensor and σ an elementwise activation function.

We define the activation functionσ asσ(A)

We can now define our proposed dynamic graph embedding.

Let A ∈ R N ×N ×T be a tensor with frontal slices A ::t =Ã (t) , whereÃ (t) is the normalization of A (t) .
...
Let X ∈ R I×J×T and θ ∈ R 1×1×T .

Analogously to the definition of the matrix-scalar product, we define X θ via (X θ) ij:
...
For any ε > 0, there exists an integer K and a set {θ
...
This is precisely our embedding model, with Θ replaced by a learnable parameter tensor W.
...
For training, we use the cross entropy loss function: where f (m, n, t) ∈ R C is a one-hot vector encoding the true class of the edge (m, n) at time t, and α ∈ R C is a vector summing to 1 which contains the weight of each class.

Since the bitcoin and Reddit datasets are so skewed, we weigh the minority class more heavily in the loss function for those datasets, and treat α as a hyperparameter; see Appendix C for details.
...
As mentioned in Section 5, the weight vector α in the loss function (5) is treated as a hyperparameter in the bitcoin and Reddit experiments.

Since these datasets all have two edge classes, let α 0 and α 1 be the weights of the minority (negative) and majority (positive) classes, respectively.

Since these parameters add to 1, we have α 1 = 1 − α 0 .

For all methods, we repeat the bitcoin and Reddit experiments once for each α 0 ∈ {0.75, 0.76, . . .

, 0.95}. For each model and dataset, we then find the best stored performance of the model on the validation data across all α 0 values.
