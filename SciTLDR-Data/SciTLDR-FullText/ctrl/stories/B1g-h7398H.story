...
where y is measured sinogram data,X is the reconstructed attenuation coefficients of the object, A is the x-ray projection matrix subject to the projection geometry, and R(x) is a Total Variation regularization functional, and β serves as a weight to control the strength of the regularization.

The β term is determined empirically.
...
For our experiment on the chest CT dataset, we chose 100 iterations with β = 2e −2 in (1) and the non-negative constraint.
