...
Definition 1 A planning model is a pair Π = D, P rob .
...
This process is called "grounding" and is denoted with: ground(ps, χ) = p where χ ⊆ O is an ordered set of objects.
...
More specifically, for an action sequence φ = a 1 , a 2 , . . . , a n each with a respective time denoted by Dispatch(a i ), we use the definition of plan validity from (Fox and Long 2003) (Definition 15 "Validity of a Simple Plan").

A simple plan is the sequence of actions φ which defines a happening sequence, t i=0...k and a sequence of states, s i=0...k+1 such that s 0 = I and for each i = 0 . . .

k, s i+1 is the result of executing the happening at time t i .

The simple plan φ is valid if s k+1 |= G.Each time window w ∈ W is a tuple w = w lb , w ub , w v where w v is a proposition which becomes true or a numeric effect which acts upon some n ∈ V .
...
Definition 3 An explanation problem is a tuple E = Π, φ, Q , in which Π is a planning model (Definition 1), φ is the plan generated by the planner, and Q is the specific question posed by the user.

The problem is to provide insight that helps the user to answer question Q.In this paper, we assume that the user knows the model Π and the plan φ, so answers such as stating the goal of the problem will not increase their understanding.
...
A compilation of a planning instance where the model is given by Π, and a question is given by Q is shown as Compilation(Π, Q) = Π where: Π = P s , V s, As , arity , Os, I , G , W We call Π the hypothetical model, or HModel.

However, Π can also be used as the input model so that the user can iteratively ask questions about some model, i.e: DISPLAYFORM0 This allows the user to stack questions, further increasing their understanding of the plan through combining compilations.
...
For each iteration of compilation the HPlan is validated against the original model Π.

Given a plan φ, a formal question Q is asked of the form:Why is the operator o with parameters χ used in state s, rather than the operator n with parameters χ ?

where o = n or χ = χ For example, given the example plan in FIG4 the user might ask:"Why is (goto waypoint kenny wp2 wp5) used, rather than (goto waypoint kenny wp2 wp4)?
...
To generate the HPlan, a compilation is formed such that the ground action b = ground(n, χ ) appears in the plan in place of the action a i = ground(o, χ).

Given the example above b = ground(goto waypoint, {kenny, wp2, wp4}), and a i = ground(goto waypoint, {kenny, wp2, wp5}).

Given a plan: φ = a 1 , a 2 , . . .
...
A plan is then generated from this new state with these new time windows for the original goal, which gives us the plan: DISPLAYFORM0 The HPlan is then the initial actions of the original plan φ concatenated with b and the new plan φ : a 1 , a 2 , . . .

, a i−1 , b, a 1 , a 2 , . . .

, a n Specifically, the HModel Π is: Π = P s, V s, As, arity , Os, I , G, W ∪ C where:• I is the final state obtained by executing 1 a 1 , a 2 , . . .
...
This new initial state is then used to plan for the original goals to get the plan φ , which, along with b and φ, gives the HPlan.
...
Given a plan φ, a formal question Q is asked of the form:Why is the operator o with parameters χ not used, rather than being used?1 We use VAL to validate this execution.
...
To generate the HPlan, a compilation is formed such that the action a = ground(o, χ) must be applied for the plan to be valid.
...
To generate the HPlan, the planning model is compiled such that the ground action a = ground(o, χ) can only be used between times lb and ub.

To do this, the original operator o is replaced with two operators o a and o ¬a , which extend o with extra constraints.

Operator o ¬a replaces the original operator o for all other actions ground(o, χ ), where χ = χ.

The action ground(o ¬a , χ) cannot be used (this is enforced using the compilation for forbidding an action described in Section 4.3).

Operator o a acts as the operator o specifically for the action a = ground(o, χ), which has an added constraint that it can only be performed between lb and ub.

Specifically, the HModel Π is: Π = P s , V s, As , arity , Os, I , G , W where:• P s = P s ∪ {can do a, not done a} DISPLAYFORM0 where the new operators o ¬a and o a extend o with the delete effect not done a and the precondition can do a, respectively.

i.e: DISPLAYFORM1 As the proposition ground(can do a, χ) must be true for ground(o a , χ) to be performed, this ensures that the action a can only be performed within the times lb and ub.

Other actions from the same operator can still be applied at any time using the new operator o ¬a .

As in Section 4.3 we make sure the ground action ground(o ¬a , χ) can never appear in the plan.
...
Given a plan φ, a formal question Q is asked of the form:Why is the operator o with parameters χ not used at time lb < t < ub, rather than being used in this time window?
...
The user might ask "why does the robot not refuel between the times x and y (as well as the other times it refuels)?".To generate the HPlan, the planning model is compiled such that the ground action a = ground(o, χ) is forced to be used between times lb and ub, but can also appear at any other time.

This is done using a combination of the compilation in Section 4.2 and a variation of the compilation in Section 4.4.

Simply, the former ensures that new action ground(o a , χ) must appear in the plan, and the latter ensures that the action can only be applied within the time window.
...
This allows the original action a = ground(o, χ) to be applied at other times in the plan.

Given this, the HModel Π is: Π = P s , V s, As , arity , Os, I, G , W where:• P s = P s ∪ {can do a, has done a} • As = {o a } ∪ As • arity (x) = arity(x), ∀x ∈ arity • arity (can do a) = arity (has done a) = arity (o a ) = arity(o) • G = G ∪ {ground(has done a, χ)} • W = W ∪ { lb, ub, ground(can do a, χ) } As wp4 is a dead end there is no valid HPlan following this suggestion.

Given a plan φ, a formal question Q is asked of the form:Why is the operator o with parameters χ used at time t, rather than at least some duration t after/before t?
...
The HModel Π is: Π = P s , V s, As , arity , Os, I , G , W where:• P s = P s ∪ {can do a, not done a, has done a} DISPLAYFORM0 ground(not done a, χ), ground(has done a, χ) }• W = W ∪ bef ore : 0, tReal, ground(can do a, χ) af ter : tReal, inf, ground(can do a, χ) where the new operators o a and o ¬a both extend o.

The latter with the delete effect not done a, while o a extends o with the precondition can do a and add effect has done a; i.e.: DISPLAYFORM1 This ensures that the ground action a = ground(o a , χ) must be present in the plan between the times 0 and tReal, or tReal and inf , depending on the user question, and between those times only.
...
Given a plan φ, a formal question Q is asked of the form:Why is the operator o with parameters χ used before (after) the operator n with parameters χ , rather than after (before)?

where o = n or χ = χFor example, given the example plan in FIG4 the user might ask:"Why is (goto waypoint kenny wp2 wp1) used before (goto waypoint kenny wp2 wp5), rather than after?"A user might ask this because there are more connections from wp5 than wp2.
...
For example the ordering of Q is a ≺ b where a = ground(o, χ) and b = ground(n, χ ).This DAG is then encoded into the model Π to create Π .
...
Specifically, the HModel Π is: Π = P s , V s, As , arity , Os, I , G , W where: DISPLAYFORM0 • As = {o a } ∪ As, ∀a ∈ N • arity (x) = arity(x), ∀x ∈ arity DISPLAYFORM1 where χ and χ are the parameters of a and b, respectively.
...
However, for all b ∈ N , ordered ab is a precondition of o a ; and for each edge (a, b) ∈ E the ground proposition ground(ordered ab , χ, χ ) is added to the initial state to represent that the edge exists in the DAG.

Therefore, the only grounding of the operator that can be performed is the action with parameters χ + χ .
...
For example the question, "Why is the operator o with parameters χ used at time lb < t < ub, rather than not being used in this time window?", can be answered using a variant of Section 4.5.
