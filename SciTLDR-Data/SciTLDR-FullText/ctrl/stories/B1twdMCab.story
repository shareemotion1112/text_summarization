...
DISPLAYFORM0 To finally update the previous embedding e −1 w of word w, we initially maxpool all representations of occurrences matching the lemma of w in every x ∈ X resulting inê w (Eq. 3).

Finally, we combine the context-independent representation e w withê w to form a context-sensitive representation e w via a gated addition which lets the model determine how much to revise the embedding with the newly read information (Eq. 5).
...
p-premise, h-hypothesis, a-assertion,ā-swapped assertion.
