...
Hidden and output bias σ(.), L(ŷ, y) Point-wise non-linear activation function and the loss function 2 RECURRENT NEURAL NETWORK FORMALISM TAB0 summarizes some notations that we use in the paper.
...
, x T −1 , at a time step t RNN performs the following: DISPLAYFORM0 (1) DISPLAYFORM1 whereŷ t is the predicted value at time step t.
...
The learning rate was cross-validated for each of the models from η ∈ {1e−2, 1e−3, 1e−4} and the best results are reported here.

The best learning rate for all the models was found out to be 1e−3 for all the models.

Again if we do not observe a decrease in the validation error after each epoch, we decrease the learning rate by a factor of γ ∈ {1e−1, 2e−1, 3e−1} which is again cross-validated.
...
, x T −1 , let us consider the operation at the hidden layer t of a recurrent neural network: DISPLAYFORM0 By the chain rule, DISPLAYFORM1 where σ is the non-linear activation function and J k+1 = diag(σ (z k+1 )) is the Jacobian matrix of the non-linear activation function.
...
, x T −1 , at a time step t LSTM performs the following: DISPLAYFORM2 DISPLAYFORM3 DISPLAYFORM4 DISPLAYFORM5 DISPLAYFORM6 where σ(.) and τ (.) are the point-wise sigmoid and tanh functions.
...
Unitary evolution RNN (uRNN) proposed to solve the vanishing and exploding gradients through a unitary recurrent matrix, which is for the form: DISPLAYFORM0 Where: DISPLAYFORM1 Diagonal matrices whose diagonal entries are of the from D kk = e iθ k , implies each matrix have N parameters, (θ 0 , . . .

, θ N −1 ).• F and F −1 : Fast Fourier operator and inverse fast Fourier operator respectively.• R 1 , R 2 : Householder reflections.
