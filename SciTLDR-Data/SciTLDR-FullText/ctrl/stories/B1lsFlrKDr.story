...
In inference, based on the parameterized scoring function S Λ with parameter Λ, the parsing problem is formulated as finding the most probable directed spanning tree for a given sentence x: where T * is the highest scoring parse tree and T is the set of all valid trees for the sentence x.
...
whereT represents the candidate parse tree, and s Λ is a function scoring each individual arc.

s Λ (h, m) describes the likelihood of forming an arc from the head h to its modifier m in the tree.
...
This objective can be interpreted as follows: if the training example has a golden tree T with it, then the objective is the log joint probability P θ,ω (T , x); if the golden tree is missing, then the objective is the log marginal probability P θ (x).

The probability of a certain tree is modeled by a tree-CRF in Eq. 1 with parameters ω as P ω (T |x).
...
where s Φ,Θ (h, m) = s Φ (h, m) + log θ mh , with m, x and T all observable.
...
Stochastically update the parameter Λ in the encoder using Adam while fixing the decoder.
...
Update buffer B using the expectation to the power for Obtain Θ globally and analytically based on the buffer B and renew the decoder.
...
Instead, we derive the evidence lower bound (ELBO) of log P Θ,Φ (m|x) as follows, by denoting Q(T ) = P Θ,Φ (T |m, x) as the posterior:
...
In addition, to account for the unambiguity in the posterior, we incorporate entropy regularization (Tu & Honavar, 2012 ) when applying our algorithm, by adding an entropy term − T Q(T ) log Q(T ) with a non-negative factor σ when the input sentence does not have a golden tree.

Adding this regularization term is equivalent as raising the expectation of Q(T ) to the power of 1 1−σ .

We annealed σ from 1 to 0.3 from the beginning of training to the end, as in the beginning, the generative model is well initialized by sentences with golden trees that resolve disambiguity.

In practice, we found the model benefits more by fixing the parameter Φ when the data is unlabeled and optimizing the ELBO w.r.t.

the parameter Θ. We attribute this to the strict convexity of the ELBO w.r.t.

Θ, by sketching the proof in the appendix.
...
Denote the encoder Q is a distribution used to approximate the true posterior distribution P φ (z|x), parameterized by φ such that Q encoding the input into the latent space z.

Proof.

Combining U and L leads to the fact:

In practice, similar as VAE-style models, E z∼Q φ (z|x)

[log P θ (x|z)] is approximated by

, where z j is the jthe sample of N samples sampled from Q φ (z|x).

At prediction stage, we simply use µ z rather than sampling z.

Here we used a mean field approximation (Tanaka, 1999) together with the conditional independence assumption by assuming P θ (z|x) ≈ l t=1 Q φ (z t |x t ).

The generative model P θ (x|z) acting as decoder parameterized by θ tries to regenerate the specific input x t at time step t from the latent space z t , as we assume conditional independence in the generative process among P θ (x t |z t ).
...
We parameterize the encoder Q φ (z t |x t ) in such a way: First a bi-LSTM is used to obtain a non-linear transformation h t of the original x t ; then two separate MLPs are used to compute the mean µ zt and the variance σ 2 zt .

The generative story P θ (x t |z t ) follows such parameterization: we used a MLP of two hidden layers in-between to take z t as the input, and then predict the word (or POS tag) over the vocabulary, such that the reconstruction probability can be measured.

Following traditional VAE training paradigms, we also apply the "re-parameterization" trick (Kingma & Welling, 2014) to circumvent the non-differentiable sampling procedure to sample z t from the Q φ (z t |x t ).

Instead of directly sample from N (µ zt , σ 2 zt ), we form z t = µ zt + σ 2 zt by sampling ∼ N (0, I).
...
Finally, with the inside table α, outside table β and the marginalization Z of all possible latent trees, we can compute the expectation of latent tree in an arc-decomposed manner.
...
the posterior distribution Q(T ) = P Θ,Φ (T |m, x), as we have In this section we derive the strict convexity of ELBO w.r.t.

Θ. Since we only care about the term containing Θ, the KL divergence term degenerates to a constant.

For sentence i, Q(T i ) has been derived in the previous section as matrix P and 1 is the indication function.

Q(1(h → m)) log θ mh Q(1(h → m)) is a Bernoulli distribution, indicating whether the arc (h → m) exists.
