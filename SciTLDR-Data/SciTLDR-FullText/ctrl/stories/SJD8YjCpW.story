...
An MLP consists of repeated applications of fully connected layers: DISPLAYFORM0 at the ith layer of the network, where σ i is an activation function, W i a weight matrix, b i a bias vector, x i the input vector, and y i the output vector.
...
The Dirichlet hash can be given by the following function: DISPLAYFORM2 A high α leads to a balanced distribution (high Shannon entropy), and a low α leads to an unbalanced distribution (low Shannon entropy).

The limiting case of α → ∞ results in a uniform distribution, which has maximum Shannon entropy.

See FIG1 for a visualization of the effects of α on a hash table with 1000 entries.
...
We observe in Figure 3 that on the MNIST dataset, increasing α has a direct positive effect on test accuracy, across different levels of sparsity.

On the CIFAR10 dataset, when the weights are sparse, increasing α has a small positive effect, but at lower levels of sparsity, it has a huge positive effect.
...
Re-plotting the MNIST graph in Figure 3 with the x-axis replaced with Shannon Entropy (equation 7) instead of α in Figure 4 gives us a better sense of scale.
