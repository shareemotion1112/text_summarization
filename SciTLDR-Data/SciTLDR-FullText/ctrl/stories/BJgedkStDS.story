...
∀ p ∈ nodes of layer k, α − β = 1 a p = activations, w pq = weights end for Compute average relevance score per feature map at layer l Relevance score vector at layer l:
...
These relevance scores are then back propagated based on αβ-decomposition rule (Wojciech et al., 2016) with α = 2 and β = 1.
...
, N } by forward propagating x i through DNN Get the binary labels for training data:ỹ i = δ(j − y i ) ∀i ∈ {1, . . .
...
If the RACs across all validation layers predict same class label c, then, we use a pre-defined confidence threshold (δ th ) to decide on early classification as follows:

• If conf idence score (RAC prob ) across all RACs is greater than δ th , we output c as final decision and terminate the inference at the given validation layer without activating any latter layers.

• If conf idence score (RAC prob ) in any of the RACs is lesser than δ th , the input is fed to the latter layers and the final output layer of the DNN is used to make the prediction.
...
The user defined threshold, δ th , can be adjusted to achieve the best trade-off between efficiency and error detection accuracy.
...
The confidence threshold δ th is a user defined hyper-parameter which also influences energy efficiency and detection capability.

The activation module discussed in Section.

3 compares the confidence score produced by RACs to the confidence threshold δ th and selectively classifies the input at RAC stage or at the final classifier.

Thus, we can regulate δ th to modulate the number of inputs being passed to latter layers.

Note that δ th has no contribution to the decision made when the RACs do not output same class.

The confidence threshold also affects the TNR and the FNR.

However, the change in FNR with confidence threshold is negligible (see Fig. 3b ), around 0.1% for 0.1 change in δ th .

Fig. 3c shows the variation in normalized OPS (with respect to baseline DNN) with different δ th for VGG16 network trained on CIFAR10 dataset with RACs appended at layer 7 and 8.

As we increase δ th , TNR increases because higher δ th would qualify more inputs to be verified by the final classifier.

However, beyond a particular δ th , a fraction of inputs which are correctly classified at early stages can be detected as natural errors because of the increase in confusion.

The maximum TNR can be achieved at δ th = 0.9 (in Fig. 3b ) and this point is referred as δ * th .

The number of OPS increases as we increase δ th but the rate of increase is significant beyond δ * th .

In Fig. 3b , we observe that the TNR increases from 39.34% (δ th =0.8) to 43.15% (δ th =0.9) while the normalized Table.
