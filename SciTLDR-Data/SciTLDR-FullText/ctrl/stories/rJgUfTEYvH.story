...
We assume a tractable prior p θ (z) over latent variable z, for eg.
...
Given g = f −1 , we can now generate a samplex from the data distribution, by sampling z ∼ p θ (z) and computingx = g(z).
...
The latent variable z t at timestep t is an invertible transformation of a corresponding frame of video: x t = g θ (z t ).

Furthermore, like in (Dinh et al., 2016; Kingma & Dhariwal, 2018) , we use a multi-scale architecture for g θ (z t ) ( Fig. 1) : the latent variable z t is composed of a stack of multiple levels: where each level l encodes information about frame x t at a particular scale:

We first briefly describe the invertible transformations used in the multi-scale architecture to infer {z

= f θ (x t ) and refer to (Dinh et al., 2016; Kingma & Dhariwal, 2018) for more details.
...
Finally, our multi-scale architecture f θ (x t ) is a composition of the flows at multiple levels from l = 1 . . .
...
As in Equation (1), we need to choose a form of latent prior p θ (z).
...
We specify the conditional prior p θ (z t |z <t ) as having the following factorization:
...
where N N θ (.) is a deep 3-D residual network (He et al., 2015) augmented with dilations and gated activation units and modified to predict the mean and log-scale.
...
| via the sum of the log Jacobian determinants of the invertible transformations mapping the video {x t } T t=1 to {z t } T t=1 ; the latent dynamics model contributes log p θ (z), i.e Equation (5).

We jointly learn the parameters of the multi-scale architecture and latent dynamics model by maximizing this objective.

Note that in our architecture we have chosen to let the prior p θ (z), as described in eq. (5), model temporal dependencies in the data, while constraining the flow g θ to act on separate frames of video.
...
Using 2-D convolutions in our flow f θ with autoregressive priors, allows us to synthesize arbitrarily long sequences without introducing such artifacts.
...
They do not optimize the variational bound on the log-likelihood directly due to the presence of a β = 1 term in their objective and scheduled sampling (Bengio et al., 2015) .
...
In future work, we would address this by incorporating longer memory in our VideoFlow model; for example by parameterizing N N θ () as a recurrent neural network in our autoregressive prior (eq. 8) or using more memory-efficient backpropagation algorithms for invertible neural networks .
