...
Let {µ be the Gaussian component means and covariance matrices of the Gaussian mixture distribution, then the posterior distribution of a class y given the embedded feature f can be expressed as in Eqn.

1 where f = F (x, θ), F : X → R d is the embedding function with parameter θ and d is the dimension of the embedded feature, p(c) is the prior probability of class c and C is the total number of classes.
...
, where ϕ is a non-negative weighting coefficient.

Notice, the distribution parameters {µ are learned automatically from data.
...
We empirically estimate the Gaussian component means {µ
...
Better illustrated in Fig. 1 1 {µ
...
Then mini-batch gradient descent algorithm is adopted for further optimization of the network on 2 Notice, gradient from LP DM does not back-propagate to update {µ .
...
is the convex combination of the source and target error with γ

, f s and f t are the labeling function in the source and target domains respectively, h is a hypothesis in class H,

| measures the domain discrepsancy in the hypothesis space H and λ = s (h * ) + t (h * ) is the combined error in two domains of the joint ideal hypothesis h * = arg min h∈H s (h) + t (h).

Denote the noise ratio of the target labeling function to be ρ, the convex combination of the source and noisy target error as˜ γ (h) = γ t (h) + (1 − γ) s (h), where t (h) is the target error on the noisy target labeling function, then we can bound the target error as follows:

In summary, this bound is decomposed into three parts: the domain discrepancy d H∆H , the error λ of the ideal joint hypothesis and the noise ratio ρ of the pseudo labels.
...
Finally, during training, as we continuously improve the accuracy of the classifier for target data, we get more and more accurate predictions, thus reducing the noise ratio ρ.

We empirically verify that ρ is decreasing in Section 4.2.
...
The three trade-off parameters ϕ, α and β in Eqn.
...
And the noise ratio ρ decreases along the training process, from the initial value of 38.6% decreases to 22.9%, which supports our theoretical analysis in Section 3.5.
...
shows the sensitivity analysis on the hyper-parameters α, β and ϕ with the other hyper-parameters fixed.
