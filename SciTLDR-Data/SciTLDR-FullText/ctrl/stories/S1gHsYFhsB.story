...
We use the output feature maps of the last layer before the final average pooling (dimensionality 512ˆ2ˆ2) as the input to a feature aggregation module which reduces the feature map to a 512-dimensional vector 3 .
...
The network is trained for 5 epochs using the RAdam optimizer (Liu et al., 2019) with learning rate 0.001, β 0 " 0.999, β 1 " 0.9, a batch size of 512 and a weight decay of 0.01.
...
Finally, we train a standard β-VAE (Higgins et al., 2017) on the set of aggregated feature vectors resulting from the previous step.

The encoder network consists of 4 fully-connected layers with 1024 neurons each, followed by two fully-connected layers parametrizing mean and log variance of the factorized Gaussian approximate posterior q pz | xq " N`µ, σ 2w

ith C " 16 latent factors.

The number of latent factors was experimentally determined.

The decoder network consists of 4 fully-connected layers with 1024 neurons each, followed by a fully-connected layer parametrizing the mean of the factorized Gaussian conditional distribution p px | zq " N pμ, Iq.
...
For optimization, we use the RAdam optimizer (Liu et al., 2019 ) with a learning rate of 0.001, β 0 " 0.999, β 1 " 0.9 and a batch size of B " 256.

The VAE is trained for N " 100 epochs by minimizing

where β is a hyperparameter to balance the losses of the MSE reconstruction and the KLD penalty terms.

As the scale of the KLD term depends on the numbers of latent factors C, we normalize it by C such that β can be varied independently of C. It can be harmful to start training with too much weight on the KLD term (Bowman et al., 2015) .

Therefore, we use the following cosine schedule to smoothly anneal β from β start " 0.001 to β end " 0.4 over the course of training:

where βptq is the value for β in training episode t P t0, . . .
