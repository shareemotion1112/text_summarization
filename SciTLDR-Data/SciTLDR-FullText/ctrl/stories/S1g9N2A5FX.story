...
This is a variational Bayesian framework where the posterior of the model parameters θ is learned and updated continually from a sequence of T datasets, {x DISPLAYFORM0 , where t = 1, 2, . . .
...
DISPLAYFORM1 , we approximate the intractable posterior p(θ|D 1:t ) after observing the first t datasets via a tractable variational distribution q t by: DISPLAYFORM2 where q 0 is the prior p, p( DISPLAYFORM3 , and Z t is the normalizing constant.

This framework allows the approximate posterior q t (θ) to be updated continuously from the previous approximate posterior q t−1 (θ) in an online fashion.
...
As such, the relevance of a feature x i 2 is proportionate to the difference between the predictions p(y|x) = θ p(y|θ, x)p(θ) and p(y|x \i ), where x \i refers to the set of all features except x i .
...
Denote by a the area of this relevant rectangle (the superset of the region containing all the salient pixels), and denote byã = max(a, 0.05) a threshold for the area a used to prevent instabilities resulting from very small area values.
...
For the hyperparameter λ of EWC, which controls the overall contribution from previous data, we experiment with two values, λ = 1 and λ = 100.
