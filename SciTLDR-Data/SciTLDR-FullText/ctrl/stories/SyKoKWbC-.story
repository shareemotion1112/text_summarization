...
The objective function for a GAN with generator G and discriminator D is DISPLAYFORM0 where DISPLAYFORM1 maps an observation to the probability that it comes from data distribution P x , and G : DISPLAYFORM2 is a network parametrized by θ G that maps a noise vector z ∈ R l , drawn from a simple distribution P z , to the original data space.
...
, z (B) } from the noise distribution P z , the gradient of G's loss with respect to its parameters θ G is DISPLAYFORM3 where we slightly abuse the notation DISPLAYFORM4 G to denote G's Jacobian matrix.
...
Specifically, we define a neural mean embedding (NME) η, where φ is learned as a neural network: DISPLAYFORM0 In practice, η only has access to P through samples of finite size {x DISPLAYFORM1 ∼ P, thus one effectively uses the empirical estimate DISPLAYFORM2 This distributional encoder forms one pillar of our adversarial learning framework.
...
That is, given an empirical estimate η(X) from a sample X (drawn from the real data P x or the generated distribution P G ), the classifier ψ S outputs 1 to indicate the sample was drawn from P x and 0 otherwise.

Scoring the discriminator's predictions via the logistic loss leads to the following value function for the adversarial game: DISPLAYFORM3 where X and Z are samples from P x and P z respectively, and D S (·) = ψ S • η(·) is the full discriminator (NME and classifier), which we refer to as the sample classifier.

Eq. (3.3) is similar to the original GAN objective (2.1), but differs in a crucial aspect: here, the expectation is inside ψ S , and the classifier is only predicting one label for the entire sample instead of one label for each observation.
...
Concretely, given two NMEs η(P 1 ) and η(P 2 ), the two-sample discriminator ψ 2S uses their absolute difference to output ψ 2S (|η(P 1 ) − η(P 2 )|) ∈ [0, 1], interpreted as the confidence that the two samples were indeed drawn from different distributions.
...
Using distributional adversaries within the context of GANs yields a saddle-point problem analogous to the original GAN (2.1): DISPLAYFORM0 where ξ ∈ {S, 2S} and the objective function V ξ is either (3.3) or (3.4); we refer to these as DAN-ξ.

As with GAN, we can optimize (3.5) via alternating updates on G and D ξ .Although optimizing (3.5) directly yields generators with remarkable performance in simple settings (see empirical results in Section 5.1), when the data distribution is complex, the distributional adversaries proposed here can easily overpower G, particularly early in the training when P G and P x differ substantially.

Thus, we propose to optimize instead a regularized version of (3.5): DISPLAYFORM1 where D is a weaker single-observation discriminator (such as the one in the original GAN), and λ is a parameter that trades off the strength of this regularization.

Note that for λ → ∞ we recover the original GAN objective, while λ = 0 yields the purely distributional loss (3.5).

In between these two extremes, the generator receives both local and global training signals for every generated sample.

We show empirically that DAN training is stable across a reasonable range of λ (Appendix B).During training, all expectations with respect to data and noise distributions are approximated via finite sample averages.
...
In the case of the sample classifier (ξ = S), a derivation as in Section 2 shows that for a sample Z of B points from the noise distribution, the gradient of the loss with respect to G's parameters is DISPLAYFORM0 where we use η B := η({G(z (1) ), . . .
...
This has the effect of sharing information across observations when computing gradients: whether a sample (encoded as a vector η B ) can fool the discriminator or not will have an effect on every observation's gradient.
...
The gradient (3.7) suggests that the true power of this sample-based setting lies in choosing a discriminator ψ S that, through non-linearities, enforces interaction between the points in the sample.
...
Our sample classifier implicitly defines a general notion of discrepancy between distributions: DISPLAYFORM0 for some function classes Ψ and Ξ, a monotone function ρ : R → R, and constant c. This generalized discrepancy includes many existing adversarial objectives as special cases.

For example, if ρ and ψ S are identity functions, Ξ the space of 1-Lipschitz functions and η an embedding into R, we obtain the 1-Wasserstein distance used by WGAN BID12 .Similarly, our two-sample discriminator implicitly defines the following discrepancy: DISPLAYFORM1 where ψ, Ξ are as before.

This form can be thought of as generalizing other discrepancy measures like MMD BID11 BID8 BID27 , defined as: DISPLAYFORM2 where φ(·) is some feature mapping, and k(u, v) = φ(u) φ(v) is the corresponding kernel function.
...
The two distributional training approaches (bottom 2 rows, λ = 0) capture all modes, and are able to recover from a missing mode (second column).
...
For these synthetic distributions, we use the pure distributional objective for DAN (i.e., setting λ = 0 in (3.5)).

Figure 2 displays the results for GAN and our methods; in the appendix, we show results for other models, different λ settings and other synthetic distributions.

Overall, while other methods suffer from mode collapse throughout (GAN) or are sensitive to network architectures and hyperparameters (WGAN, WGAN-GP), our DANs consistently recover all modes of the true distribution and are stable across a reasonable range of λ's.
...
For DAN, we use the the regularized objective (3.5) with λ > 0.
...
For WGAN-GP, λ the weight for the gradient penalty is set to 0.1.
...
All other models are optimized using Adam BID15 with learning rate of 10 −4 and β 1 = 0.5.
...
The discriminator consists of a fully connected For the first set of experiments we optimize using Adam BID15 with learning rate of 10 −4, β 1 = 0.5 and β 2 = 0.9, and train the network for 30, 000 iterations, where each iteration consists of 5 updates for discriminator and 1 for generator for WGAN-GP.

The result is shown in Figure 8 .

All models successfully capture all modes, despite their separation.

However, a slight change in the optimizer's parameters to β 2 = 0.999 (the default value) causes WGAN-GP to fail sometime even if we run for 50, 000 iterations, while DAN still constantly recovers the true distribution (cf.
...
Finally, we show that DAN is stable across a reasonable range of λ, the trade-off parameter for single observation and distributional adversaries.

We set λ = {0, 0.2, 0.5, 1, 2, 5, 10} for DAN and the results are shown in FIG0 and 11.
...
Hypaerparameters We use Adam with learning rate of 0.0005 and β 1 = 0.5, a fixed minibatch size of 256 and 100 epochs of training.
...
DISPLAYFORM0 Under review as a conference paper at ICLR 2018Figure 10: DAN-S with λ set to {0, 0.2, 0.5, 1, 2, 5} (top-down order).
