...
The information from the dth homology group is summarized in a simple complexity measure, the dth Betti number β d , which merely counts the number of d-dimensional features: a circle, for example, has Betti numbers (1, 1), i.e. one connected component and one tunnel, while a filled circle has Betti numbers (1, 0), i.e. one connected component but no tunnel.
...
Since W is typically changing during training, we require a function φ : E → W that maps a specific edge to a weight.
...
This has a computational complexity of O (n · α (n)), where α(·) refers to the extremely slow-growing inverse of the Ackermann function (Cormen et al., 2009, Chapter 22) .
...
For the lower bound, let G − k be a fully-connected layer with |V k | vertices and, given θ ∈ [0, 1], let φ k (e) := θ for every edge e. Since a vertex v is created before its incident edges, the filtration degenerates to a lexicographical ordering of vertices and edges, and all points in D k will be of the form (θ, θ).
...
To show that this term cannot be exceeded by NP(G) for any G, suppose we perturb the weight function φ(e) : DISPLAYFORM3 We can use the upper bound of Theorem 1 to normalize the neural persistence of a layer, making it possible to compare layers (and neural networks) that feature different architectures, i.e. a different number of neurons.
...
As a brief example, we trained a perceptron on the MNIST data set with batch stochastic gradient descent (η = 0.5), achieving a test accuracy of ≈ 0.91.

Moreover, we intentionally 'sabotaged' the training by setting η = 1 × 10 −5 such that SGD is unable to converge properly.
...
Specifically, for m and n input and output neurons we initialize τ = m + n connected components.

When using zero padding, the additional dummy input neurons have to included in m. For all τ tuples in the persistence diagram the creation event c = 1.

Notably, each output neuron shares the same set of edge weights.

Due to this, the destruction events-except for a few special cases-simplify to a list of length τ containing the largest filter values (each value is contained n times) in descending order until the list is filled.
...
We use w to denote the flattened and sorted weight values (in descending order) of the convolutional filter H j , while w c represents the vector of all weights that are located in a corner of H j , whereas wc ,ϕ is the vector of all weights which do not originate from the corner of the filter while still belonging to the first (and thus largest) DISPLAYFORM8 weights in w, which we denote by w 1:⌊
...
This was achieved by sampling the weights from a beta distribution with α = 0.005 and β = 0.5.
