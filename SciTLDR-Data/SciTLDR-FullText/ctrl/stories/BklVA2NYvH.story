...
where θ denotes the parameters of the model, J denotes the loss function and R denotes the regularizer term, and we solve the problem with (stochastic) gradient-descent based methods (Bottou, 2010; Ruder, 2016) .
...
Given a T -layer neural net, we let the dynamical system {f t (x t , θ t ) : t = 0, . . .

, T } represents the network, where x t is the input of t-th layer, θ t is the parameter, and f t :

denotes the t-th layer's transformation, which is usually a non-linear function σ(θ t x t + b t ) for fully-connected layers, convolution layers and batch normalization layers, etc.
...
Specially, if the model is a deep residual network with structure x t+1 = x t + f t (x t , θ t ), we can regard the problem as the forward Euler discretization of the following continuous optimal control problem:

where x(t) is a continuous trajectory from the input to the output logits.

Adversarial examples are usually clean images added by a small calculated perturbation η.

The model predicts correct labels fed with clean inputs x 0 , while the output is completely different when it is fed with perturbed input x 0 + η.
...
For a given dynamical systemẋ = f (x), x(0) = x 0 , x e is an equilibrium, then

• The system is asymptotically stable if it is Lyapunov stable and ∃ δ > 0 such that if x(0) − x e < δ, then lim t→∞ x(t) − x e = 0.

• The system is exponentially stable if it is asymptotically stable and ∃ α > 0, β > 0, δ > 0 such that if x(0) − x e < δ, then x(t) − x e ≤ α x(0) − x e e −βt , for all t ≥ 0.

The definitions can be easily extended to discrete-time systems.

Intuitively, the Lyapunov stability states that for any small perturbation η, the trajectory is still "close enough" to the original one.
...
, where σ is the activation function, e.g., Sigmoid function or ReLU function, it is stable if Re(λ i (A)) ≤ 0, ∀i, where Re denotes the real part, and λ i denotes the i-th eigenvalue.
...
To ensure numerical stability, we require |1 − λ i (A)h| ≤ 1 (Ascher & Petzold, 1998) , where the step size h = 1 in residual networks.
...
For a discrete neural network, i.e., discrete dynamics {f t (x t , θ t ) : t = 0, . . .

, T }, where f t (x t , θ t ) = σ(θ t x t ) (we omit the bias term for simplicity), the network is stable if the ρ(θ t ) ≤ 1, where ρ(A) = max i (|λ i (A)|) is the spectral radius.
...
For dynamics {f t (x t , θ t ) : t = 0, . . .

, T }, assume θ * = θ * 0 , . . .

, θ * T −1 is a solution to the optimal control problem.

Also, we define the Hamiltonian function H :

, where the dot denotes the inner product.

We have the following necessary conditions for θ * .
...
Concretely, after computing the states x and co-states p, the optimization step on layer t is only searching for parameters θ t .
...
Consider a layer in the form of f t (x) = θ t x, where we leave the activation as an individual layer with no parameters for simplicity, we can derive the following optimization problem for Hamiltonian maximization: max

where α θ t 2 2 is the L 2 norm regularizer (weight decay), and θ t is the initial parameter (i.e., θ k t in the algorithm).
...
For continuous deep models, the only constraint is Theorem 1, i.e., Re(λ i (θ t )) ≤ 0.

Anti-symmetric matrices have only imaginary eigenvalues, and hence we can replace θ t with θ t − θ (Goodfellow et al., 2015) 2.34% 77.45% 49.32% PGD-10 (Madry et al., 2017) 0.02% 46.67% 36.33% C&W (Carlini & Wagner, 2017a) Proof.

Recall that ρ(A) ≤ A 2 = λ max (A T A), we have

Hence we can replace ρ(θ t ) ≤ 1 with a positive semi-definite condition, and we turn the Hamiltonian maximization into a new optimization problem, where the target function is quadratic and the constraint is a semi-definite condition.
