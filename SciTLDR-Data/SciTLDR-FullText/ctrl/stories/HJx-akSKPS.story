...
Φ 1 represents Eqs. (1) and (2), Φ 2 represents Eqs. (4) and (5), and two types of gates are Eqs. (3) and (6).

After obtaining a graph representationǦ and a pattern representationP from a sequence model or a graph model where their column vectors are d-dimensional, we feed them as inputs of interaction layers to extract the correlated context between the pattern and the graph.
...
Here M ultiHead is the attention method described in (Vaswani et al., 2017) , σ represents the logistic sigmoid function, s j is the intermediate state of the j th block of memory that summarizes information from the pattern, and s for information from both the pattern and the graph.
...
After getting a graph representationǦ and a pattern representationP from the representation learning modules, we feed them into the following different types of interaction layers for comparison.

SumPool: A simple sum-pooling is applied forǦ andP to obtainǧ andp, and the model sends Concate(ǧ,p,ǧ −p,ǧ p) with the graph size and the pattern size information into the next fully connected (FC) layers.
...
Two hyperparameters control the density of subisomorphisms: (1) α ∈ [0, 1] decides the probability of adding subisomorphisms rather than random edges; (2) β ∈ N + is the parameter of Dirichlet distribution to sample sizes of components.
...
where G is the graph representation in the pattern space, σ is the sigmoid function, f i is the gate that decides to filter the j th vertex or the j th edge, W G ∈ R dp×dg and W F ∈ R 1×dp are trainable.
