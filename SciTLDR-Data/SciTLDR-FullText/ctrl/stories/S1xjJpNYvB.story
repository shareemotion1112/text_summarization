...
To realize this idea, we first build a base network f E (·; θ) shared among all models.
...
In the next step, we build one model per source domain by adding a per-model modulator with a parameter set α i on top of the frozen base network.
...
As depicted in Figure 1 (c), this meta-model parameterized by φ, which we call a model selection network, is trained in order to map a task representation z task for a particular task to an index of the best model in the model pool.
...
The overall training procedure for constructing the pool and training the selection network is summarized in Algorithm 1 in Appendix C. One way of the inference is to predict a class with the best single model chosen by the selection network f S (·; φ) for a given support set (Figure 3(a) ).

Following the method proposed in ProtoNet as shown in Equation 1, a class predictionŷ to a query example x q is made by finding a class whose prototype c y is the closest one to an embedded vector of the given query example.

Specifically, the prototype for the class y is defined as a mean of embedded vectors of all examples in the support set belonging to a class y and squared Euclidean distances d y i to these prototypes are compared between the classes.ŷ
...
Then, our final predictionŷ would be a class to maximize a mean of these probabilities from all M + 1 models.
...
B ARCHITECTURES Figure 2 shows the architecture of the embedding network f E (·; θ, α i ), which processes an input image and produces a 512-dimensional embedding vector.
...
The selection network f S (·; φ) is a two-layered MLP (multi-layer perceptron) network, which receives an embedding vector produced by the embedding network as an input and performs the best model index prediction.
...
2: Learn θ by optimizing f E (x; θ, α 0 ) for the aggregated dataset (α 0 : no modulation).
...
Learn α i by optimizing f E (x; θ, α i ) for (S, Q) while keeping θ fixed.
...
Measure accuracies of M + 1 available embedding models {f E (x; θ, α i )} M i=0 for (S, Q).

Set the best model index y sel to the index of the model with the highest accuracy.

Learn φ by training f S (z task ; φ) so as to predict y sel for (S, Q).

7: end while

We present the experimental results when we do not apply the further adaptation scheme introduced in (Chen et al., 2019) .

Specifically, ProtoNet, FEAT, and our models are tested without additional linear classifiers f c (·; ψ).
...
Equation (3) shows the loss (loss sel ) used to train the selection network f S (·; φ).

Here, acc m is the classification accuracy of the model with the modulator parameterized by α m in the pool.
