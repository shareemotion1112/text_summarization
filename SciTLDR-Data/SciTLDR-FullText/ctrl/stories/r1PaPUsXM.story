...
This joint force endeavour remains active in the new millennium e.g. BID27 ; BID22 ; Nauck & Nürnberger (2013); BID19 ; BID16 .
...
non-linear: in contrast to the standard outer product which is bilinear in each of its entry, the hamming outer product is non-linear since in general x DISPLAYFORM5 where µ ∈ R is a scalar.
...
non-linear: in contrast to the standard outer product which is bilinear in each of its entry, the hamming outer product is non-linear since in general x DISPLAYFORM4 where µ ∈ R is a scalar.
