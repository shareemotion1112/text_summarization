...
This work proposes an approach for text generation using Generative Adversarial Networks with Skip-Thought vectors.
...
This is achieved by training the GAN with sentence embedding vectors produced by SkipThought ), a neural network model for learning fixed length representations of sentences.

Deep neural network architectures have demonstrated strong results on natural language generation tasks BID32 ).

Recurrent neural networks using combinations of shared parameter matrices across time-steps BID30 BID20 ) with different gating mechanisms for easing optimization BID13 ; ) have found success in modeling natural language.
...
Recent text generation models use a wide variety of GANs such as gradient policy based sequence generation framework BID34 ) and an actor-critic conditional GAN to fill missing text conditioned on surrounding text BID6 ) for performing natural language generation tasks.
...
The following equations are iterated over for decoding: Generative Adversarial Networks BID9 ) are deep neural net architectures comprised of two networks, contesting with each other in a zero-sum game framework.
...
DISPLAYFORM1 The two networks -generator and discriminator compete against each other in a zero-sum game.
...
BID9 have formulated the minimax game for a generator G, discriminator D adversarial network with value function V (G, D) as: DISPLAYFORM2
...
The Skip-Thought encoder for the model encodes sentences with length less than 30 words using 2400 GRU units BID4 ) with word vector dimensionality of 620 to produce 4800-dimensional combine-skip vectors. (?.
...
The training process of a GAN is notably difficult as demonstrated by BID28 and several improvement techniques such as batch normalization, feature matching, historical averaging BID28 ) and unrolling GAN (Metz et al.) have been suggested for making the training more stable.
...
4.1 CONDITIONAL GENERATION OF SENTENCES.GANs can be conditioned on data attributes to generate samples BID21 ; Radford et al.) .

In this experiment, both the generator and discriminator are conditioned on Skip-Thought encoded vectors ).

The encoder converts 70000 sentences from the BookCorpus dataset collected in with a training/test/validation split of 5/1/1 into vectors used as real samples for discriminator.
...
TAB0 compares these results for different architectures that have been experimented with in this paper.
...
Table 2 (d) has samples generated from STGAN when using Wasserstein distance f-measure as WGAN in ) and 2(e) contains samples when using a gradient penalty regularizer term as WGAN-GP from BID10 ).

Table 2 : Sample sentences generated from training on CMU-SE Dataset; mode collapse is overcome by using minibatch discrimination.
...
Describes application of generative adversarial networks for modeling textual data with the help of ski-thought vectors and experiments with different flavors of GANs for two different datasets.