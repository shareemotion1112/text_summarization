...
We use the problem formulation of a Markov Decision Process M = (S, A, P, R, ρ) where ρ(s) defines the distribution over initial states, and a robot in state s ∈ S must choose an action a ∈ A, which will give reward r = R(s, a) and proceed to state s with probability P (s |s, a).

We say that a policy π(a|s) will define a policy distribution over actions conditioned on states, and that τ π (s 0 , a 0 , s 1 , a 1 , ...) is the distribution over trajectories created recursively when s 0 ∼ ρ(s), a 0 ∼ π(a|s 0 ), and s 1 ∼ P (s 1 |s 0 , a 0 ).We then define the value function V conditioned on policy π as DISPLAYFORM0 for some discount factor γ ∈ [0, 1].

This can be rewritten recursively as DISPLAYFORM1 We further define Q π and rewrite the value function: DISPLAYFORM2 We define the optimal value function V * (s) = max π V π (s) and a policy π * as the optimal policy if V π * = V * .

The value iteration algorithm can be used to find V * through the following iterative process: DISPLAYFORM3 It is well known that as n → ∞, V n → V * , and an optimal policy π can be inferred as π * (a|s) = 1 argmax a Q∞(s,a) (a).
...
If we assume a reward function R θ parameterized by θ, we can calculate the gradient of the value function with respect to θ after k iterations as: DISPLAYFORM0 where, a * is the optimal action selected by the max Q value in iteration k − 1.
...
θ, we will not be able to remove the sum over actions and corresponding successor states.
