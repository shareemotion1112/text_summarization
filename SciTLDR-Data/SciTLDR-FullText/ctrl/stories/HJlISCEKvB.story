...
To train, a single random Gaussian i out of the mixture is chosen, and then they added µ i to the N ormal(0, 1) noise and multiplied it by σ i , with both µ i and σ i as learnable parameters.
...
Finally, only one component of the noise is trained at a time (a single µ i and σ i is randomly selected for each training batch), while our model learns to model the data over the full collection of generators in each minibatch.
...
To do this, we randomly choose images in each dataset and perform linear interpolation between them with a mixing coefficient α chosen from a U nif orm(0, 1) distribution.
