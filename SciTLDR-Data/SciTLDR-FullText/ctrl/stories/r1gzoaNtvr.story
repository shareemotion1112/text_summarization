...
After two rounds, Q-bot must respond to the task, predicting the requested attribute pairû = U (x Q , h T Q ) as a function of the task and Q-bot's final memory state.
...
both Q-bot and A-bot parameters 9 if e mod E = 0 then 10 Sample replacement set B under policy π and re-initialize all agents in B 11 return all Q-bots and A-bots.
...
Let π be some replacement strategy that returns a subset of the population.

Every E epochs, we call π and reinitialize the parameters and optimizers for the corresponding agents (blue lines 9-10 in Alg.
...
With probability 1−ε replace the A-bot and Q-bot with the lowest validation accuracy.

We use ε = 0.2 in our experiments.
...
Experiments in Gülçehre & Bengio (2016) support this idea by showing that supervision of intermediate representations allows a more complex toy task to be learned.
...
The approach section outlines various replacement strategies (policy π), but does not detail their implementation.
