...
The Laplacian operator on the mesh can be discretized using the cotangent formula: DISPLAYFORM4 where N (i) is the nodes in the neighboring one-ring of i, A i is the area of the dual face corresponding to node i, and α ij and β ij are the two angles opposing edge ij.
...
Denoting the spatial coodinate vector as x, the scalar function as f (x), the scalar function values stored on vertex i as f i , and the piecewise linear "hat" functions as φ i (x), we have: DISPLAYFORM0 the piecewise linear basis function φ i (x) takes the value of 1 on vertex i and takes the value of 0 on all other vertices.

Hence, the gradient of this piecewise linear function can be computed as: DISPLAYFORM1 Due to the linearity of the basis functions φ i , the gradient is constant within each individual triangle.
...
We refer the reader to BID5 for detailed derivations for the gradient operator G. Denoting the area of each face as a (f ) (which can be easily computed using Heron's formula given coordinates of three points), the resulting per-vertex gradient vector can be computed as an average of per-face gradients, weighted by face area: DISPLAYFORM3 denote the per-vertex polar (east-west) and azimuthal (north-south) direction fields asx (v) andŷ (v) .
