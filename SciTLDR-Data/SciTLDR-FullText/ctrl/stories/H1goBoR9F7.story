...
Note that k is determined by the output size and a pre-configured sparsity parameter γ.
...
The generation of each output activation y j requires an inner product operation, as follows: DISPLAYFORM0 where X i is the i-th row in the matrix of input FMs (for the FC layer, there is only one X vector), W j is the j-th column of the weight matrix W , and ϕ(·) is the neuronal transformation (e.g., ReLU function, here we abandon bias).
...
The k value is determined by the activation size and the desired sparsity γ.
