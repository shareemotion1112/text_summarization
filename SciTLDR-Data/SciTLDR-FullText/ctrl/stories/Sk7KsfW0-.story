...
DEN calculates the drift ρ where L is task specific loss function, W t is the parameter for task t, and Ω(W t ) is the regularization (e.g. element-wise 2 norm) to enforce our model W t appropriately.
...
where 1≤l≤L denotes the l th layer of the network, W t l is the network parameter at layer l, and µ is the regularization parameter of the element-wise 1 norm for sparsity on W .
...
Input: Datatset Dt, Threshold τ Perform Algorithm 2 and compute L if L > τ then Add k units h N at all layers Solve for Eq. 5 at all layers for l = L − 1, . . .
...
, t − 1}, and λ is the regularization parameter.
...
After performing Eq. 6, we measure the amount of semantic drift for each hidden unit i, ρ t i , as the 2 -distance between the incoming weights at t-1 and at t.

Then if ρ t i > σ, we consider that the meaning of the feature have significantly changed during training, and split this neuron i into two copies (properly introducing new edges from and to duplicate).
...
Input: Weight W t−1 , Threshold σ Perform Eq. 6 to obtain W t for all hidden unit i do ρ DISPLAYFORM0 Copy i into i (w introduction of edges for i ) Perform Eq. 6 with the initialization of W t to obtain W t Timestamped Inference.
