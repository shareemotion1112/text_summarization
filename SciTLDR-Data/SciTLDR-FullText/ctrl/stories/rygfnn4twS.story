...
The HLC aims to maximize the accumulative extrinsic reward eRd = i j γ i couti+j−1 eRd eRd [Li,Kj ] , where γ eRd ∈ [0, 1) is a decay factor.

The immediate extrinsic reward can be represented by eRd [Li,Kj ]

where N C is the network configuration; HC means the hardware configuration, e.g., memory bandwidth; accuracy(N C) indicates the inference accuracy; lat is the inference latency of the network N C running on the hardware HC; en represents the inference energy of N C running on HC; area is the FPGA area (hardware cost) used by N C on HC; ψ acc , ψ l , ψ e and ψ a are user-defined factors deciding the impact of inference accuracy, latency, energy and FPGA area on the extrinsic reward.

By different values of user-defined factors, AutoQ implements the resource-constrained and accuracy-guaranteed searches.

For resource-constrained applications, e.g., low-power drones, AutoQ sets ψ acc = 1, ψ l = 0, ψ e = 0 and ψ a = 0 to achieve the best accuracy given the maximum amount of hardware resources (latency, energy, and FPGA area).
...
For accuracy-guaranteed applications, e.g., fingerprint locks, AutoQB sets ψ acc = 2, ψ l < 1, ψ e < 1 and ψ a < 1 to obtain the shortest latency, the minimal energy, and the smallest hardware cost with no accuracy loss. [Li,Kj ] , where γ iRd ∈ [0, 1) is a decay factor.
...
where ζ is a user-defined factor dynamically enlarging from 0.1 to 0.8 as the number of training epochs increases.

When ζ is small, the HLC has stronger influence on the LLC.

On the contrary, when ζ = 1, the LLC maximizes only the accumulative extrinsic reward.
...
So the low-level Q-value function Q LLC θ LLC is to minimize the error ε LLC (state [Li,Kj ] , g Li , a [Li,Kj ] , state [Li, Kj+1] ), which is

where µ with Gaussian noises by collecting the actions as N (µ [Li, K0] , g Li , eRd [Li, K0:
...
AutoQ first explores 100 episodes with a constant noise, i.e., δ a [L i ,K j ] = 0.5 for the LLC and δ g [L i ] = 0.5 for the HLC, and then exploits 300 episodes with exponentially decayed noise.
...
We make AutoQ perform the resource-constrained searches by imposing a latency constraint and setting ψ acc = 1, ψ l = 0, ψ e = 0 and ψ a = 0 in the extrinsic reward.
...
We run AutoQ to do the accuracy-guaranteed searches by setting ψ acc = 2, ψ l = 0.5, ψ e = 0 and ψ a = 0 in the extrinsic reward.
...
By setting ψ acc = 2, ψ l = 0.5, ψ e = 0.5 and ψ a = 0.5, AutoQ takes the inference accuracy, latency, energy and FPGA area into consideration during an accuracy-guaranteed search.
