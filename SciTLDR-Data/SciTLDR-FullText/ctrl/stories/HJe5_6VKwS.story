...
For example, Goodfellow et al. (2014) set r = εsign(

δx ).

Similarly, saliency can be computed using the forward derivative

δx (Papernot et al., 2016b; Zhang et al., 2018) .
...
Second, log(f c (Φ(x, f s ))) is included to ensure that the salient pixels suffice to identify the correct class.

Finally, f c (Φ(x, 1 − f s )) is included to ensure that the classifier can no longer recognize the class if the saliency map is removed.

Therefore, our saliency loss function is:

where f c is the softmax probability of the class c, Φ(x, f s ) applies mask f s to image x, and λ i ≥ 0 are hyper-parameters.
...
Using a validation set, we experimented with different drop-out rates between 0.3 and 0.7 and found that the rate δ = 0.6 was optimal.
...
ε is the maximum perturbation allowed and ε i is the maximum perturbation allowed in an iteration.
...
x adv ← x adv + εsign(∇f (x adv , y)) Algorithm 1 provides the white-box attack for JSD.
