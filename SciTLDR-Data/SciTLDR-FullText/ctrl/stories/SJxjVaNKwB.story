...
where µ and σ 2 represents mean and variance, respectively.
...
where [N ] is the set of masked tokens, P tr (i) and P st (i) are two predicted distributions respectively from the teacher and student model on the masked tokens, and α is a hyperparameter in (0, 1).
...
where γ, β ∈ R n and • denotes the Hadamard product.
...
(λ, µ, β, γ) are hyperparameters to balance the different loss terms.

Specifically, we use λ = 1, µ = 100, β = 5000, γ = 5 in our all experiments.
...
In pre-training distillation, the hyperparameter α is used to balance the original masked language modeling loss and the distillation loss.

Following (Kim & Rush, 2016) , we set α to 0.5.
