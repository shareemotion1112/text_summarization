...
An MDP is defined as a tuple M ≡ (S, A, p, r, γ) where S and A are the state and action spaces, p(·|s, a) gives the nextstate distribution upon taking action a in state s, and γ ∈ [0, 1) is a discount factor that gives smaller weights to future rewards.
...
During the reinforcement learning phase the goal of the agent is to find a policy π : S → A that maximizes the expected return G t = ∞ i=0 γ i R t+i , where R t = r(S t , A t , S t+1 ).
...
The action-value function of a policy π is defined as

expected value when following policy π.

Based on Q π we can compute a greedy policy

π is guaranteed to do at least as well as π, that is:

and π are called policy evaluation and policy improvement, respectively; under certain conditions their successive application leads to the optimal value function Q * , from which one can derive an optimal policy using (1).
...
In this case we will use Q π c to refer to the value of π under cumulant c and the associated optimal policies will be referred to as π c , where π c (s) is the greedy policy (1) on Q * c (s, a).
...
Following Barreto et al. (2017; , we assume that there exist features φ(s, a, s ) ∈ R d such that the reward function which specifies a task of interest can be written as

where w ∈ R d are weights that specify how desirable each feature component is, or a 'task vector' for short.

Note that, unless we constrain φ somehow, (2) is not restrictive in any way: for example, by making φ i (s, a, s ) = r(s, a, s ) for some i we can clearly recover the rewards exactly.

Barreto et al. (2017) note that (2) allows one to decompose the value of a policy π as

where φ t = φ(S t , A t , S t+1 ) and ψ π (s, a) are the successor features (SFs) of π.

SFs can be seen as multidimensional value functions in which φ(s, a, s ) play the role of rewards, and as such they can be computed using standard RL algorithms (Szepesvári, 2010) .

One of the benefits provided by SFs is the possibility of quickly evaluating a policy π.

Suppose that during the unsupervised learning phase we have computed ψ π ; then, during the supervised phase, we can find a w ∈ R d by solving a regression problem based on (2) and then compute Q π through (3).

Once we have Q π , we can apply (1) to derive a policy π that will likely outperform π.

Since π was computed without access to the reward, its is not deliberately trying to maximize it.

Thus, the solution π relies on a single step of policy improvement (1) over a policy that is agnostic to the rewards.

It turns out that we can do better than that by extending the strategy above to multiple policies.

Let e : (S → A) → R k be a policy-encoding mapping, that is, a function that turns policies π into vectors in R k .

Borsa et al.'s (2019) universal successor feature (USFs) are defined as ψ(s, a, e(π)) ≡ ψ π (s, a).

Note that, using USFs, we can evaluate any policy π by simply computing

Now that we can compute Q π for any π, we should be able to leverage this information to improve our previous solution based on a single policy.

This is possible through generalized policy improvement (Barreto et al., 2017, GPI) .

Let ψ be USFs, let π 1 , π 2 , ..., π n be arbitrary policies, and let

It can be shown that (5) is a strict generalization of (1), in the sense that Q π (s, a) ≥ Q πi (s, a) for all π i , s, and a. This result can be extended to the case in which (2) holds only approximately and ψ is replaced by a universal successor feature approximator (USFA) ψ θ ≈ ψ(s, a) (Barreto et al., 2017; Borsa et al., 2019) .

The above suggests an approach to leveraging unsupervised pre-training for more dataefficient reinforcement learning.

First, during the unsupervised phase, the agent learns a USFA ψ θ .

Then, the rewards observed at the early stages of the RL phase are used to find an approximate solution w for (2).

Finally, n policies π i are generated and a policy π is derived through (5).

If the approximations used in this process are reasonably accurate, π will be an improvement over π 1 , π 2 , .., π n .

However, in order to actually implement the strategy above we have to answer two fundamental questions: (i) Where do the features φ in (2) come from? (ii) How do we define the policies π i used in (5)?

It turns out that these questions allow for complementary answers, as we discuss next.

Features φ should be defined in such a way that the down-stream task reward is likely to be a simple function of them (see (2)).
...
The objective F(θ) is to find policy parameters θ that maximize the mutual information (I) between some policy-conditioning variable, z, and some function f of the trajectory τ induced by the conditioned policy, where H is the entropy of some variable:
...
Commonly f is assumed to return the final state, but for simplicity we will consider that f samples a single state s uniformly over τ π θ .
...
However, it is not obvious how to optimize the policy parameters θ, as they only affect the loss through the non-differentiable environment.
...
In practice w t is also fed into ψ as an input, which also allows for GPI to be used (see Algorithm 1 in Appendix).

For the random feature baseline, the discriminator q is frozen after initialization, but the same objective is used to train ψ.

Traditionally, the desired product of this optimization was the conditional policy (π).

While the discriminator q could be used for imitating demonstrated behaviors (i.e. by inferring the most likely z for a given τ ), for down-stream RL it was typically discarded in favor of explicit search over all possible z (Eysenbach et al., 2018) .
...
In the SF formulation tasks correspond to linear weightings w of features φ(s).

The reward for a task given by w is r SF (s; w) = φ(s) T w. BMI objectives, on the other hand, define tasks using conditioning vectors z, with the reward for task z given by r BM I (s; z) = log q(z|s).

We propose restricting conditioning vectors z to correspond to task-vectors w of the SFs formulation.

The restriction that z ≡ w, in turn, requires that r SF (s; w) = r BM I (s; w), which implies that the BMI discriminator q must have the form log q(w|s) = φ(s) T w.

One way to satisfy this requirement is by restricting the task vectors w and features φ(s) to be unit length and paremeterizing the discriminator q as the Von Mises-Fisher distribution with a scale parameter of 1.
...
With this variational family for the discriminator, all that is left to complete the base algorithm is to factorize the conditional policy into the policy-conditional successor features (ψ) and the task vector (w).
...
Recall that we are training a USFA ψ(s, a, e(π)) whose encoding function is e(π) = w (that is, π is the policy that tries to maximize the reward in (10) for a particular value of w).
...
Let us call this solution w base , that is, φ(s, a, s ) w base ≈ r(s, a, s ).
...
The main baseline model, RF VISR, removes the BMI objective, instead learning SFs over features given by a random convolutional network (the same architecture as the φ network in the full model).
...
As there are no guarantees that extrinsic rewards will be linear in the learned features (φ), it is not obvious which approach is best in practice.
...
We can minimize L θ with respect to θ q , the parameters of q, using back-propagation.

However, properly adjusting the parameters of π, θ π , is more difficult, as we lack a differentiable model of the environment.
...
Since in this section we will be talking about θ π only (that is, we will not discuss θ q ), we will drop the subscript and refer to the parameters of π as simply θ.

Let τ be a length T trajectory sampled under policy π, and let p θ be the probability of the trajectory τ under the combination of the policy and environment transition probabilities.

We can compute the gradient of p θ with respect to θ as:

This means that we can adjust p θ to make τ more likely under it.

If we interpret p θ as the distribution induced by the policy π, then minimizing (13) corresponds to maximizing the following value function:

We can then use the policy gradient theorem to calculate the gradient of our loss function with respect to the parameters of the policy, θ, for trajectories τ beginning in state s,
...
Figure 3 shows each of the 5 dimension of φ across all states of the grid-world.
...
Figure 4 shows 49 randomly sampled reward functions, generated by sampling a w vector uniformly on the 5-sphere and taking the inner product with φ.

This demonstrates that the space of φ contains many different partitionings of the state-space, which lends credence to our claim that externally defined reward functions are likely to be not far outside of this space, and thus fast-inference can yield substantial benefits.
