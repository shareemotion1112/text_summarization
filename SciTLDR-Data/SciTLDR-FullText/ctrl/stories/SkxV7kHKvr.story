...
GAT (Veličković et al. (2017) ) applies the attention mechanism to the graph network.

DGI (Veličković et al. (2018) ) proposes a framework to learn the unsupervised representations on graph-structured data by maximizing location mutual information.
...
Let L = U ΛU T be the eigendecomposition, where U ∈ R N ×N is composed of orthonormal eigenbasis and Λ = diag(λ 0 , λ 1 , . . .

, λ N −1 ) is a diagonal matrix of eigenvalues which denotes frequencies of graph G , and λ i and u i form an eigenpair.

The convolutional operator * G on the graph signal x is defined by

wheref = U T x andĝ = U T g are regarded as the graph Fourier transform of graph signal x and graph filter g, respectively; f = U (·) is the inverse graph Fourier transform, and is the Hadamard product.

Ĝ = diag(ĝ 0 , · · · ,ĝ N −1 ) behaves as spectral filter coefficients.
...
I denotes the scaled normalized Laplacian for all eigenvalues λ i ∈ [−1, 1] and θ i is trainable parameter.

Graph Convolutional Network (GCN) (Kipf & Welling (2017)) is a variant of ChebNet which only takes first two terms of Equation (3).

By setting the coefficients θ 0 and θ 1 as θ = θ 0 = −θ 1 and with λ max = 2, the convolution operator in convolution layer of GCN is induced as g In graph theory, The definition of the dual varies according to the choice of embedding of the graph G. For planar graphs generally, there may be multiple dual graphs, depending on the choice of planar embedding of the graph.

In this work, we follow the most common definition.

Given a plane graph G = {V, E A}, which is designated as the primal graph, the dual graphĜ = {Ṽ = E,Ẽ,Ã} is a graph that has a vertex (or node) for each edge of G. The dual graphĜ has an edge whenever two edges of G share at least one common vertex.

To be clarified, the vertices (i, j) and (j, i) of dual graphĜ converted from a undirected graph are regarded as the same.
...
Let us take an example, given a dual node (i, j) (corresponds to an edge in primal graph), primal node i has label α and j has label β, then dual node (i, j) is assigned with a label (α, β).
...
σ represents nonlinear activation function, e.g. ReLU.
...
Suppose that P (Y |X) is predictions by primal pipeline and P (Ŷ |X) = P (Ŷ |X) is the derived predictions obtained through an aggregation from the predictions on dual labels by dual pipeline to primal label predictions.

X is derived from X as aforementioned (Section 3.3).

We first calculate the joint probability matrix P (Y,Ŷ ) of two matrices P (Y |X) and P (Ŷ |X)

we further get the marginal probabilities of P (Y ) and P (Ŷ ) from P (Y,Ŷ ).
