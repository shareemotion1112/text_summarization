...
To define constantly curved spaces, we first need to define the notion of sectional curvature K(τ x ) of two linearly independent vectors in the tangent space at a point x ∈ M spanning a two-dimensional plane τ x (Berger, 2012) .
...
A good example of a point that diverges is the origin of the hyperboloid (or a pole of the hypersphere) µ 0 = (1/ |K|, 0, . . . , 0)

T .

In general, we can see that µ 0 2 = 1/K K→0 −−−→ ±∞. Additionally, the distance and the metric tensors of these spaces do not converge to their Euclidean variants as K → 0, hence the spaces themselves do not converge to R d .

This makes both of these spaces unsuitable for trying to learn sign-agnostic curvatures.

Luckily, there exist well-defined non-Euclidean spaces that inherit most properties from the hyperboloid and the hypersphere, yet do not have these properties -namely, the Poincaré ball and the projected sphere, respectively.
...
We first need to define the projection function ρ K .

For a point (ξ; x T ) T ∈ R n+1 and curvature K ∈ R, where ξ ∈ R, x, y ∈ R ; 2y

Under review as a conference paper at ICLR 2020

The formulas correspond to the classical stereographic projections defined for these models (Lee, 1997) .

Note that both of these projections map the point µ 0 = (1/ |K|, 0, . . . , 0) in the original space to µ 0 = 0 in the projected space, and back.
...
2 g E , for all x in the respective manifold (Ganea et al., 2018a) , and g E y = I for all y ∈ E. The conformal factor λ K x is then defined as λ K x = 2/(1 + K x 2 2 ).
...
The n-dimensional Poincaré ball P n K (also called the Poincaré disk when n = 2) for a given curvature K < 0 is defined as
...
The Möbius addition ⊕ K of x, y ∈ M K (for both signs of K) is defined as
...
Since Ganea et al. (2018a) and Tifrea et al. (2019) used the same gyrovector spaces to define an exponential map, its inverse logarithmic map, and parallel transport in the Poincaré ball, we can reuse them for the projected hypersphere by applying the transformations above, as they share the same formalism.

For parallel transport, we additionally need the notion of gyration (Ungar, 2008) gyr [x, y]

Parallel transport in the both the projected hypersphere and the Poincaré ball then is PT
...
We simply sample from a Gaussian distribution in the tangent space at µ 0 with mean 0, and use parallel transport and the exponential map to map the sampled point onto the manifold.
...
A downside of this approach is that vMF only has a single scalar covariance parameter κ, while other approaches can parametrize covariance in different dimensions separately.
...
Mathieu et al. (2019) derive it for the Poincaré ball, and Hauberg (2018) derive the Spherical Normal distribution on the hypersphere.
...
First of all, we need to define an "origin" point on the manifold, which we will denote as µ 0 ∈ M K .

What this point corresponds to is manifold-specific: in the hyperboloid and hypersphere it corresponds to the point µ 0 = (1/ |K|, 0, . . . , 0)

T , and in the Poincaré ball, projected sphere, and Euclidean space it is simply µ 0 = 0, the origin of the coordinate system.
...
As a consequence of the distance function and operations convergence theorems for the Poincaré ball A.5 (analogously for the projected hypersphere), A.17, A.18, and A.20 , we see that the Wrapped Normal distribution converges to the Gaussian distribution as K → 0.
...
Since we have defined the Wrapped Normal family of distributions for all spaces, we can use WN (µ 0 , σ 2 I) as the posterior family, and WN (µ 0 , I) as the prior distribution.

The forms of the distributions depend on the chosen space type.

The mean is parametrized using the exponential map exp In experiments, we sometimes use vMF(µ, κ) for the hypersphere S n K (or a backprojected variant of vMF for D n K ) with the associated hyperspherical uniform distribution U (S n K ) as a prior (Davidson et al., 2018) , or the Riemannian Normal distribution RN (µ, σ 2 ) and the associated prior RN µ 0 , 1 for the Poincare ball P n K (Mathieu et al., 2019) .
...
Same applies for the Poincaré ball and the projected spherical space.
...
Since all the VAE parameters {θ, φ} live in Euclidean manifolds, we can use standard gradient-based optimization methods.

Specifically, we use the Adam (Kingma & Ba, 2015) optimizer with a learning rate of 10 −3 and standard settings for β 1 = 0.9, β 2 = 0.999, and = 10 −8 .
...
Riemannian Normal VAEs did very well on their own -the fixed Poincaré VAE (RN P 2 −1 ) 3 obtains the best score.
...
When paired with another Euclidean and a Riemannian Normal Poincaré disk component, it performed well, but that might be because the RN P −1 component achieved best results across the board on MNIST.
...
That applies mainly to the above mentioned Riemannian Normal Poincaré ball components, but also S 6 both with Wrapped Normal and VMF, as well as most product space VAEs with different curvatures (third section of the table).
...
With a latent dimension of 6, the Riemannian Normal Poincaré ball VAE is still the best performer.
...
Dynamically-binarized Omniglot reconstruction For a latent space of dimension 6 (Table 5) , the best of the baseline models is the Poincaré VAE of (Mathieu et al., 2019) .
...
Summary In conclusion, a very good model seems to be the Riemannian Normal Poincaré ball VAE RN P n .
...
Straight lines are generalized to constant speed curves giving the shortest path between pairs of points x, y ∈ M, so called geodesics, for which it holds that γ * = arg min γ L(γ), such that γ(0) = x, γ(1) = y, and

.

Using this metric, we can go on to define a metric space (M, d M ).

Moving from a point x ∈ M in a given direction v ∈ T x M with constant velocity is formalized by the exponential map: exp x : T x M → M. There exists a unique unit speed geodesic γ such that γ(0) = x and

The corresponding exponential map is then defined as exp x (v) = γ(1).
...
Since points in the n-dimensional projected hypersphere and Poincaré ball models can be represented using a real vector of length n, it enables us to visualize points in these manifolds directly for n = 2 or even n = 3.

On the other hand, optimizing a function over these models is not very well-conditioned.

In the case of the Poincaré ball, a significant amount of points lie close to the boundary of the ball (i.e. with a squared norm of almost 1/K), which causes numerical instabilities even when using 64-bit float precision in computations.
...
Also, we often leverage the "hyperbolic" Pythagorean theorem, in the form cosh 2 (α) − sinh 2 (α) = 1.
...
This is useful for example in the case of orthogonally projecting points from T µ0 H n K onto the manifold.
...
A good example of a point that diverges is the origin of the hyperboloid µ

T .

That makes this model unsuitable for trying to learn sign-agnostic curvatures, similarly to the hypersphere.

and in the case of x := µ 0 = (R, 0, . . . , 0) T :
...
, and the last equality assumes |α| > 1.
...
A special form of parallel transport exists for when the source vector is µ 0 = (R, 0, . . . , 0) T :

Do note, that all the theorems for the projected hypersphere are essentially trivial corollaries of their equivalents in the Poincaré ball (and vice-versa) (Section A.5.2).
...
Distance function The distance function in P n K is (derived from the hyperboloid distance function using the stereographic projection ρ K ):

Theorem A.4 (Distance equivalence in P n K ).

For all K < 0 and for all pairs of points x, y ∈ P n K , the Poincaré distance between them equals the gyrospace distance
...
For any fixed pair of points x, y ∈ P n K , the Poincaré gyrospace distance between them converges to the Euclidean distance in the limit (up to a constant) as K → 0 − :
...
For any fixed pair of points x, y ∈ P n K , the Poincaré distance between them converges to the Euclidean distance in the limit (up to a constant) as
...
In the case of x := µ 0 = (0, . . . , 0) T they simplify to:
...
Unfortunately, on the Poincaré ball, ·, · x has a form that changes with respect to x, unlike in the hyperboloid.
...
Also, we often leverage the Pythagorean theorem, in the form sin 2 (α) + cos 2 (α) = 1.
...
This is useful for example in the case of orthogonally projecting points from T µ0 S n K onto the manifold.
...
A good example of a point that diverges is the north pole of the sphere µ
...
where α = 1 R 2 x, y 2 = K x, y 2 , and the last equality assumes |α| > 1.
...
where α = K x, y 2 , we derive parallel transport in S n K :

A special form of parallel transport exists for when the source vector is µ 0 = (R, 0, . . . , 0) T :

Do note, that all the theorems for the projected hypersphere are essentially trivial corollaries of their equivalents in the Poincaré ball (and vice-versa) (Section A.4.2).
...
We notice that ρ K is not a homeomorphism between the n-dimensional sphere and R n , as it is not defined at −µ 0 = (−R; 0 T ) T .

If we additionally changed compactified the plane by adding a point "at infinity" and set it equal to ρ K (µ 0 ), ρ K would become a homeomorphism.

Proof.

Distance function The distance function in D n K is (derived from the spherical distance function using the stereographic projection ρ K ):
...
Unfortunately, on the projected sphere, ·, · x has a form that changes with respect to x, similarly to the Poincaré ball and unlike in the hypersphere.

Theorem A.14 (Möbius addition converges to Eucl.

vector addition).

Note: This theorem works from both sides, hence applies to the Poincaré ball as well as the projected spherical space.

Observe that the Möbius addition has the same form for both spaces.
...
Using the orthonormal basis (with respect to the Lorentz product) {ξ 1 , . . .

ξ n }, we can compute the determinant by computing the change with respect to each basis vector.

.

Since parallel transport preserves norms and vectors in the orthonormal basis have norm 1, the change is d PT

For computing the determinant of the exponential map Jacobian, we choose the orthonormal basis {ξ 1 = u/ u L , ξ 2 , . . .

, ξ n }, where we just completed the basis based on the first vector.

We again look at the change with respect to each basis vector.

For the basis vector ξ 1 :

where the second equality is due to

For every other basis vector ξ k where k > 1:

where the third equality holds because

where the last equality relies on the fact that the basis is orthogonal, and u is parallel to ξ 1 = u/ u L , hence it is orthogonal to all the other basis vectors.
...
For computing the determinant of the exponential map Jacobian, we choose the orthonormal basis {ξ 1 = u/ u 2 , ξ 2 , . . .
...
where the last equality relies on the fact that the basis is orthogonal, and u is parallel to ξ 1 = u/ u 2 , hence it is orthogonal to all the other basis vectors.
...
Also proven by (Mathieu et al., 2019) in a slightly different form for a scalar scale parameter WN (z; µ, σ 2 I).
...
where p K (r) = 2π sin K (r) denotes the circumference of a circle of radius r in a space of constant curvature K, and
...
An important work in this area is β-VAE, which adds a simple scalar multiplicative constant to the KL divergence term in the ELBO, and has shown to improve both sample quality and (if β > 1) disentanglement of different dimensions in the latent representation.
...
Additionally, Mathieu et al. (2019) also derive the Riemannian Normal, which is a maximum entropy distribution on the Poincaré disk, but in practice performs similar to the Wrapped Normal, especially in higher dimensions.
