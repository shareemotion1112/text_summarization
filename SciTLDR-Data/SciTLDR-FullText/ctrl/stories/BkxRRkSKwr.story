...
Given a phrase p = x i:j in the input sequence x, contextual decomposition (CD) attributes a score φ(p, x) as the contribution solely from p to the model's prediction s(x).

Note that φ(p, x) does not equal to the sum of the scores of each word in the phrase, i.e., φ(p, x) = xi∈p φ(x i , x).

Starting from the input layer, CD iteratively decomposes each hidden state h of the model into the contribution solely made by p, denoted as β, and the contributions involving the words outside the phrase p, denoted as γ, with the relation h = β + γ holds.

Note that the algorithm also keeps the contribution from the bias term, denoted as ζ, temporally before element-wise multiplication.

For a linear layer h = W i x t + b i with input x t , the contribution solely from p to h is defined as β = W i x t when x t is part of the phrase (i.e., x t ∈ p), and the contribution involving other words in the sentences (denoted as x\p) is defined as γ = 0.

The contribution of the bias term ζ is thus b i .

When x t lies outside of the phrase in the sentence (i.e., x t ∈ p), γ is quantified as W i x t and β is 0.
...
When dealing with non-linear activation h = σ(h), CD computes the contribution solely from the phrase p as the average activation differences caused by β supposing γ is present or absent,

Following the three strategies introduced above, CD decomposes all the intermediate outputs starting from the input layer, until reaching the final output of the model h T = β + γ.

The logit score W l β is treated as the contribution of the given phrase p to the final prediction s(x).
...
The decomposition of activation functions is modified as β = σ(β).

For the linear layer h = W h + b with its decomposition h = β + γ, the bias term b is decomposed proportionally and merged into the β term of h , based on β = W β + |W β|/(|W β| + |W γ|) · b.
...
Non-additivity Importance of a phrase φ(p, x) should be quantified by a non-linear function over the importance scores of all the component words
...
Formally, if we are interested in how combining two phrases p 1 and p 2 contribute to a specific prediction for an input x, we expect for input sentencesx where only p 2 is replaced to another phrase, the importance attribution for p 1 remains the same, i.e., φ(p 1 , x) = φ(p 1 ,x).

In the hierarchical explanation setting, we are interested in how combining a phrase and any other contextual words or phrases in the input x changes the prediction for the input x. Therefore, we expect φ(p, x) = φ(p,x) given the phrase p in two different contexts x andx.

Limitations of CD and ACD Unfortunately, while CD and ACD try to construct decomposition so that β terms represent the contributions solely from a given a phrase, the assigned importance scores by these algorithms do not satisfy the context independence property mathematically.

For CD, we see the computation of β involves the γ term of a specific input sentence in Eq. 2 (see Figure 2 (a) for visualization).

Similarly, for ACD, the decomposition of the bias term involves the γ terms of a specific input sentence.

As a result, the β terms computed by both algorithms depend on the context of the phrase p.

Regarding the decomposition of activation functions, the decomposition β = σ(β) in ACD seems plausible, which does not involve γ for computing β terms.

However, in case every activation is decomposed in this way and suppose the bias terms are merged into β, the algorithm is equivalent to feeding only the phrase p into the classifier with all other input masked as zero.
...
Computing Eqs. 3 and 4 are intractable as it requires integrating over a large number of variants of x δ as replacements (i.e., number of variants for x δ is exponential to the size of N ).
...
The language model helps model a smoothed distribution of p(x δ |x −δ ).
...
Specifically, given the output h = s (l) (x) at an intermediate layer l with the decomposition h = β + γ, we decompose the activation value σ(h) into β + γ , with the following definition:

i.e., β is defined as the expected difference between the activation values when the β term is present or absent.

h is computed for different input sequences x with the contexts of the phrase p sampled from the distribution p(x δ |x −δ ).

Eq. 5 is a layer wise application of Eq. 4, where the masking operation is implemented with calculating σ(h − β).
...
For sampling, we mask the words that are not conditioned in p(x δ |x −δ ).
...
Similar to SCD, SOC samples neighboring words x δ from a trained language model p(x δ |x −δ ) and obtain a set of neighboring word replacement S. For each replacement x δ ∈ S, the algorithm computes the model prediction differences after replacing the phrase p with padding tokens.

The importance φ(p, x) is then calculated as the average prediction differences.
...
We follow the quantitative evaluation protocol proposed in CD algorithm (Murdoch et al., 2018) for evaluating word-level explanations, which computes Pearson correlation between the coefficients learned by a linear bag-of-words model and the importance scores attributed by explanation methods, also noted as the word ρ.
...
We generate explanations for each phrase on the parsing tree and evaluate the Pearson correlation between the ground truth scores and the importance scores assigned for phrases, also noted as the phrase ρ.
...
Table 1 shows word ρ and phrase ρ achieved by our algorithms and competitors.

Generally, explanation algorithms that follow our formulations achieve highest word ρ and phrase ρ for all the datasets and models.

SOC and SCD perform robustly on the deep Transformer model, achieving higher word ρ and phrase ρ than input occlusion and contextual decomposition algorithms by a large margin.
...
The statistic based approximation of the context independent impor- tance, which do not employ a trained sampler, yields competitive words ρ, but it is not competitive for phrase ρ, pushing the phrase ρ towards that of the input occlusion algorithm.
...
We also see word ρ and phrase ρ increase as the number of samples K increases.
...
However, the word ρ and the phrase ρ of Direct Feed drop by a large margin, showing a word ρ of -0.38 and 0.09.

SOC and SCD are still robust on the adverse LSTM model, both showing a word ρ and phrase ρ of more than 0.60 and 0.55.
