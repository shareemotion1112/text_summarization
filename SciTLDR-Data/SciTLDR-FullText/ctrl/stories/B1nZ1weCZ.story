...
The actor is a parametric function: π θa (a t |s t ) mapping from the states to the actions according to which the RL agent acts (θ a are the parameters of the policy/actor).

A biased but low-variance sample estimate for the policy gradient is: ∇ θa log π θa (a t |s t )(Q(s t , a t ) − b(s t )) where a t is the action executed in state s t .
...
We can also get an estimate for Q(s t , a t ) as r t+1 + γV (s t+1 ) where γ is the discounting factor.
...
Let ρ i be the game-play score of an MTA in task i, h i be the target score in task i (potentially obtained from other published work).
...
Instead of selecting the next task to train on uniformly at random, our framework maintains for each task T i , an estimate of the MTN's current performance (ρ i ) as well as the target performance (h i ).
...
It computes an estimate of how well the MTN can solve task T i by calculating m i = hi−ρi hi for each of the tasks.

The probability distribution for sampling next tasks (at task decision steps) is then computed as: DISPLAYFORM0 , where τ is a temperature hyper-parameter.
...
We also introduced a tunable hyperparameter β which controls the relative importance of the bonus term and the average reward for a given task.
...
L is the set of worst three tasks, according to 1 − m i = ρi hi (normalized task performance).
...
This was found after hyper-parameter tuning over the set {5, 20}. The discount factor γ for the discounted returns was set to be γ = 0.99.

Entropy-regularization was used to encourage exploration, similar to its use in .

The hyper-parameter which trades-off optimizing for the entropy and the policy improvement is β (introduced in .

β = 0.02 was separately found to give the best performance for all the active sampling methods (A5C, UA4C, EA4C, FA4C) after hyper-parameter tuning over the set {0.003, 0.01, 0.02, 0.03, 0.05}. The best β for BA3C was found to be 0.01.The six task instances (MT1, MT2 and MT3) were trained for 300 million steps.
...
The temperature parameter τ in the softmax function used for the task selection was tuned over the set {0.025, 0.033, 0.05, 0.1}. The best value was found to be 0.05.
...
We hyper-parameter tuned for the discount factor γ used for the meta-decider (tuned over the set {0.8, 0.9, 0.99}) and the scaling factor for the bonus β (tuned over the set {0.125, 0.25, 0.5, 1}).

The best hyper-parameters were found to be γ = 0.99 and β = 0.25.
...
We tuned the β meta for entropy regularization for encouraging exploration in the meta-learner's policy over the set {0, 0.003, 0.01} and found the best value to be β meta = 0.

We also experimented with the γ meta , the discounting factor for the RL problem that the meta-learner is solving.

We tuned it over the set {0.5, 0.8, 0.9} and found the best value to be γ meta = 0.8.
...
τ ← Temperature hyper-parameter of the softmax task-selection non-parametric policy 10:amta ←
