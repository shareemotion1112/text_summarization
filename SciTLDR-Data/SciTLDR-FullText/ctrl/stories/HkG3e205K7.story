...
Our goal is to learn a latent variable generative model p θ (x, z) = p θ (z)p θ (x|z) where x are observed data and z are continuous latent variables.

The marginal likelihood of the observed data, p θ (x) = p θ (x, z) dz, is generally intractable.

Instead, we maximize a variational lower bound on log p θ (x) such as the ELBO DISPLAYFORM0 where q(z|x) is a variational distribution.

Following the influential work by BID20 BID30 , we consider the amortized inference setting where q φ (z|x), referred to as the inference network, is a learnable function parameterized by φ that maps from x to a distribution over z. The tightness of the bound is coupled to the expressiveness of the variational family (i.e., {q φ } φ ).

As a result, limited expressivity of {q φ } φ , can negatively affect the learned model.

BID5 introduced the importance weighted autoencoder (IWAE) bound which alleviates this coupling DISPLAYFORM1 with z 1:K ∼ i q φ (z i |x).

The IWAE bound reduces to the ELBO when K = 1, is non-decreasing as K increases, and converges to log p θ (x) as K → ∞ under mild conditions BID5 .

When q φ is reparameterizable 1 , the standard gradient estimator of the IWAE bound is DISPLAYFORM2 A single sample estimator of this expectation is typically used as the gradient estimator.
...
This does not 1 Meaning that we can express zi as z( i, φ), where z is a deterministic, differentiable function and p( i) does not depend on θ or φ.
...
happen for the model parameters (θ).

Following up on this work, demonstrate that this deteriorates the performance of learned models on practical problems.

Because the IWAE bound converges to log p θ (x) (as K → ∞) regardless of q φ , φ's affect on the bound must diminish as K increases.
...
To derive improved gradient estimators for φ, it is informative to expand the total derivative 2 of the IWAE bound with respect to φ DISPLAYFORM3 Previously, BID31 found that the first term within the parentheses of Eq. 3 can contribute significant variance to the gradient estimator.
...
Because the derivative is a partial derivative ∂ ∂φ , it treats z i = z( i , φ) as a constant, so we can freely change the random variable that the expectation is over to z 1:K .
...
Now, we can use the following well-known equivalence between the REINFORCE gradient and the reparameterization trick gradient (See Appendix 8.1 for a derivation) DISPLAYFORM2 This holds even when f depends on φ.
...
When z i is not reparameterizable (e.g., the models in BID25 ), we can use a control variate (e.g., 1 K ∂ ∂φ log q φ (z i |x)).
...
Computing the gradient of the log marginal likelihood DISPLAYFORM0 requires samples from p θ (z|x), which is generally intractable.

We can approximate the gradient with a self-normalized importance sampling estimator DISPLAYFORM1 where z 1:K ∼ i q φ (z i |x).

Interestingly, this is precisely the same as the IWAE gradient of θ, so the RWS update for θ can be interpreted as maximizing the IWAE lower bound in terms of θ.
...
The wake update approximately minimizes the KL divergence from p θ (z|x) to q φ (z|x).

The gradient of the KL term is DISPLAYFORM2 The wake update of the inference network approximates the intractable expectation by selfnormalized importance sampling DISPLAYFORM3 with z i ∼ q φ (z i |x).
...
Their positive results motivate further exploration of convex combinations of IWAE-DReG and RWS-DReG DISPLAYFORM1 We refer to the algorithm that uses the single sample Monte Carlo estimator of this expression as DReG(α).

When α = 1, this reduces to RWS-DReG, when α = 0, this reduces to IWAE-DReG and when α = 0.5, this reduces STL.
...
In this view, the ELBO can be seen as the MCO corresponding to a single importance sample estimator of the marginal likelihood with q θ as the proposal distribution.
...
We can use a conditional latent variable model p θ (x, z|c) = p θ (x|z, c)p θ (z|c), however, as before, computing the marginal likelihood is generally intractable.
...
We use a similar architecture, but now learn a conditional prior distribution p θ (z|c) where c is the top half of the MNIST digit.
...
Given a function f (z, φ), we have DISPLAYFORM0 0 1000 2000 3000 4000 5000Steps ( for a reparameterizable distribution q φ (z).
...
In the main text, we assumed that θ and φ were disjoint, however, it can be helpful to share parameters between p and q (e.g., BID11 ).
...
In this section, we discuss how to deal with shared parameters in RWS.Suppose that both p and q are parameterized by θ.

If we denote the unshared parameters of q by φ, then we can restrict the RWS wake update to only φ.

Alternatively, with a modified RWS wake update, we can derive a single surrogate objective for each scenario such that taking the gradient with respect to θ results in the proper update.

For clarity, we introduce the following modifier notation for p θ (x, z i ), q θ (z i |x), and w i which are functions of θ and z i = z(θ, i ).

We useX to mean X with stopped gradients with respect to z i ,X to mean X with stopped gradients with respect to θ (but not θ is not stopped in z(θ, i )), andX to mean X with stopped gradients for all variables.
