...
DISPLAYFORM0 In order to fit this model, for a given i ∈ I, one tries to perform Kullback-Leibler divergence minimization between the true distribution P (.|i) and the model of the distribution g θ : DISPLAYFORM1 In the case of multi-class and single-label tasks it is natural to use the softmax formulation.
...
More formally, we now model the output P (.|i) as a n-dimensional (n = card(J)) random vector of Bernoulli random variables, each with a parameter g θ (j|i).

Consequently, for a given set {j 1 , ..., j k } ∈ J k , k n, we have: DISPLAYFORM0 The problem now is to find the right parameter g θ (j|i) of each Bernoulli variable.
...
This means that we model g θ (j|i) with the following PME-Softmax formulation: DISPLAYFORM2 where S i is defined as the true but unknown support set of items j possibly co-occuring with i. Contrary to the softmax formulation, for all items j in S i , each Bernoullis with parameter g θ (j|i) are independent.
...
We denote D = {D α } α∈A as the family of functions, with parameters A ⊂ R p .

The model D α takes an input (i, j) ∈ I ×J and outputs a score D α (i, j) ∈ R. D α is be trained with a binary cross-entropy loss function (BCE), to discriminate between positive data sampled from X and negatives sampled by our generator.

DISPLAYFORM0 where σ is the sigmoid function.

In our sampling model, for a given i ∈ I, we use the modeled softmax conditional distribution g θ (.|i) ∈ P (J) to sample negatives.

It is true that sampling with g θ requires computing the score for every possible continuation, but it is an effective way to sample negatives that are close to the decision boundary.

However, as the model improves during training, the likelihood of sampling positives gets higher.

To fix this, D α is used in a second step, to oversample the samples that are true negatives.

This model called cooperative importance sampling, has the following training loss: DISPLAYFORM1 where V := {j} {j 1 , ..., j n }, j 1 , ..., j n ∼ g θ (.|i).Improved gradient via g and D Intuitively, we want to find true hard negatives that will be informative in terms of gradient.

Ideally, we would only sample negatives that are mutually exclusive with j given the context i. When deriving the sampled softmax loss, we have the following gradient: DISPLAYFORM2 where p DISPLAYFORM3 j∈V exp(G θ (i,j))×Dα(i,j) , a distribution whose support lies in V .

We see that if the negatives sampled are too easily distinguishable from real data, then they will be far from the decision boundary, have low gradients and therefore will not enable G θ to improve itself.
...
The Selfplay method (SP) is defined by sampling the negative samples in the softmax distribution g θ of our model like in our cooperative importance sampling scheme except that we do not correct the weights with the help of the discriminator D α .

Sampling within its own distribution

Require: generator G θ ; discriminator Dα; a generative process X.Initialize G θ , Dα with random weights θ, α. allows us to sample negatives close to the decision boundary.
