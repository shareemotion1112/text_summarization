...
Given a set of training data consisting of image-label pairs, DISPLAYFORM0 nc and a residual neural network (ResNets) BID11 BID18 , our goal is to find network parameters θ ∈ R n and weights of a linear classifier defined by W, µ ∈ R nc such that DISPLAYFORM1 where S is a softmax hypothesis function and y (k) (θ) denotes the output features of the neural network applied to the kth image.
...
The goal in training is to find a θ that transforms the features in a way that simplifies the classification problem.
...
For a generic example y 0 , the jth "time step" of the network reads DISPLAYFORM3 where θ j are the weights associated with the jth step.
...
Then, the operator K(θ) is a four by four block matrix consisting of convolution matrices C parametrized by the different stencils that comprise θ DISPLAYFORM6 Hence, applying a convolution operator going from c in to c out channels requires O(c in · c out ) convolutions.
...
For θ ∈ R 16 , we denote such an operator as DISPLAYFORM1 where ⊗ denotes the Kronecker product, and I is an identity matrix.

Note that M(θ) models a reaction between the features in different channels at a given pixel, but introduces no spatial coupling.
...
DISPLAYFORM2 This is a masked version of the original convolution equation 4, with the mask leaving just the block diagonal and 1 × 1 convolution terms (note that the diagonal part of M(θ (2) ) can be ignored as the same coefficients appear also in K dw (θ (1) )).
