...
To this end, we propose adaptive neural trees (ANTs), which generalise previous work that attempted the same unification (Suárez & Lutsko, 1999; İrsoy et al., 2012; BID32 BID46 BID30 BID57 and address their limitations (see Tab.
...
On the other hand, the best performing methods on the SARCOS multivariate regression dataset are all tree-based, with soft decision trees (SDTs) (Suárez & Lutsko, 1999; BID26 , GBTs (Friedman, 2001) and ANTs achieving the lowest mean squared error.
...
The very first SDT introduced in (Suárez & Lutsko, 1999 ) is a specific case where in our terminology the routers are axis-aligned features, the transformers are identity functions, and the routers are static distributions over classes or linear functions.
...
Architecture growth is a key facet of DTs , and typically performed in a greedy fashion with a termination criteria based on validation set error (Suárez & Lutsko, 1999; Irsoy et al., 2012) .
...
İrsoy et al. (2014) proposes budding trees, which are grown and pruned incrementally based on global optimisation of all existing nodes.
...
Routers, R: each internal node j ∈ N int holds a router module, r θ j : X j → [0, 1] ∈ R, parametrised by θ, which sends samples from the incoming edge to either the left or right child.

Here X j denotes the representation at node j.

We use stochastic routing, where the binary decision (1 for the left and 0 for the right branch) is sampled from Bernoulli distribution with mean r θ j (x j ) for input x j ∈ X j .

As an example, r θ j can be defined as a small convolutional neural network (CNN).

2. Transformers, T : every edge e ∈ E of the tree has one or a composition of multiple transformer module(s).

Each transformer t ψ e ∈ T is a nonlinear function, parametrised by ψ, that transforms samples from the previous module and passes them to the next one.

For example, t ψ e can be a single convolutional layer followed by ReLU BID40 .
...
Solvers, S: each leaf node l ∈ N leaf is assigned to a solver module, s φ l : X l → Y ∈ S, parametrised by φ, which operates on the transformed input data and outputs an estimate for the conditional distribution p(y|x).

For classification tasks, we can define, for example, s φ as a linear classifier on the feature space X l , which outputs a distribution over classes.
...
Lastly, many existing tree-structured models (Suárez & Lutsko, 1999; İrsoy et al., 2012; BID32 BID46 BID30 BID57 are instantiations of ANTs with limitations which we will address with our model (see Sec. 2 for a more detailed discussion).
...
For both phases, we use the negative log-likelihood (NLL) as the common objective function to minimise, which is given by −log p (Y|X, θ, ψ, φ) DISPLAYFORM0 .., y (N ) } denote the training inputs and targets.

As all component modules (routers, transformers and solvers) are differentiable with respect to their parameters Θ = (θ, ψ, φ), we can use gradient-based optimisation.
...
In the first case, the resulting models are equivalent to SDTs (Suárez & Lutsko, 1999) or HMEs (Jordan & Jacobs, 1994) with locally grown architectures, while the second case is equivalent to standard CNNs, grown adaptively layer by layer.
...
Neural Architecture Search: The ANT growing procedure is related to the progressive growing of NNs BID12 BID20 BID58 BID7 Srivastava et al., 2015; BID36 BID6 İrsoy & Alpaydın, 2018) , or more broadly, the field of neural architecture search BID62 BID5 BID8 .
...
In the first case, the resulting models are equivalent to SDTs (Suárez & Lutsko, 1999) or HMEs BID26 with locally grown architectures, while the second case is equivalent to standard NNs, grown adaptively layer by layer.
