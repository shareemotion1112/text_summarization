...
Formally, the detector is a function f from X × R to R. The detection threshold is denoted by the scalar θ ∈ R. Usually there is no single threshold for all terms, and it needs to be adjusted after decoding.
...
We assume that term r, and two instancesx + ∈ X r+ andx − ∈ X r− are drawn from a fixed but unknown probability distribution, and we denote by P{π} and E[π] the probability and the expectation of an event π under this distribution.
...
Assume that the predefined threshold is θ, then the accuracy in the prediction can be measured by DISPLAYFORM1 where ∧ is the logical conjunction symbol.

Hence our goal is to find the parameters of the function f so as to maximize the accuracy Acc θ for a given threshold θ.

Equivalently we find the parameters of function f that minimize the error defined as DISPLAYFORM2 where ∨ is the logical disjunction symbol, and I{π} is the indicator function, that equals 1 if the predicate π holds true and 0 otherwise.
...
Specifically, we replace the last term with the hinge upper bound, DISPLAYFORM3 where [π] + = max{π, 0}. The last upper bound holds true since I{π < 0} ≤ [1 − π] + .

Adding the margin of 1 means that the function f faces a harder problem: not only does it need to have a confidence greater than θ for a positive speech segment and a confidence lower than θ for a negative speech segment -the confidence value must be at least θ + 1 and at most θ − 1 for positive and negative speech segments, respectively.

We now turn to present two algorithmic implementations that are aimed at minimizing the loss function derived from (6), namely, DISPLAYFORM4 Hopefully the minimization of this loss function will lead to the minimization of Err θ in (6).
...
We set the next weight vector w i to be the minimizer of the following optimization problem, DISPLAYFORM7 where C serves as a complexity-accuracy trade-off parameter and ξ is a non-negative slack variable, which indicates the loss of the ith example.

Intuitively, we would like to minimize the loss of the current example, i.e., the slack variable ξ, while keeping the weight vector w as close as possible to our previous weight vector w i−1 .

The constraint makes the projection of the utterance in which the term is uttered onto w greater than θ + 1, and the projection of the utterance in which the term is not uttered onto w less than θ − 1.
...
Denote by ρ : R d × R d → R a measure of similarity between two output vectors of size d. The spoken term detector f u : X * × X * → R is the composition of Siamese networks g u and the similarity function.

Hence an unknown speech segmentx t can be compared to a positive or negative speech segment, as follows: DISPLAYFORM1 The tied parameters u of all the models were found in BID9 BID1 BID13 using the minimization of the ranking loss function DISPLAYFORM2 for different options of the similarity function ρ.

In this work we propose to minimize the loss function in FORMULA5 , which is defined for the weakly supervised case as follows: DISPLAYFORM3 when the margin of γ > 0 is used.

In this case, the parameter γ is not set to 1, since the function f u is not a linear function and hence is not scale invariant to the margin, as in the structured prediction case.
...
We measure performance using the AUC defined in (1) and using the accuracy of a fixed threshold θ denoted Acc θ .
...
The AUC and Acc θ rates found on our validation and test set are presented in Table 1 .

In training PA-ACC we chose arbitrarily θ = 0.

Table 1 : AUC and ACC θ rates of the PA-ACC algorithm.
...
BID0 For all the algorithms we report the AUC and Acc θ in TAB1 .

For the two versions of PA-AUC we selected a single threshold θ that gave the best Acc θ on the validation set.

Similarly we selected the best threshold for the HMM algorithm.

For PA-ACC we arbitrarily selected θ = 0.

It is interesting to see that the AUC of PA-ACC is the same or even higher than that of the PA-AUC.

Since Acc θ is a lower bound to AUC, the AUC can be thought of as Acc θ with the best threshold selected for every term in the set.

Indeed from TAB1 we see that the Acc θ was very close to the AUC but did not reach it.
...
As before we arbitrarily chose the threshold θ = 0.
...
Models were trained using ADADELTA BID22 .We reproduced the results in BID13 by training the Siamese network using the ranking loss in (12) with the cosine similarity as a similarity function ρ.
...
We also train the network using the same similarity function using the Acc θ loss function with θ = 0 as in BID12 .

For the ranking loss we used γ = 0.15 while for the Acc θ loss we used γ = 0.10, because the margin γ is counted twice for Acc θ loss.

These values were chosen by maximizing Acc θ on a validation set over 5 epochs.

The AUC and Acc θ values for training of 5 to 30 epochs are given in TAB3 .

Other training parameters and settings were exactly the same as in BID13 .

We can see in the table that both AUC and Acc θ are higher when the system is trained with the calibrated ranking loss function.
