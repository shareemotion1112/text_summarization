...
In this expression, the deep prior appears as a penalty term weighted by the trade-off parameter λ > 0.
...
by increasing the size of the TV-norm ball) or by increasing the trade-off parameter λ.
...
with η steplength for network weights.
...
As during Elastic-Averaging Stochastic Gradient Descent [10, Chaudhari et al. [11] ], x i 's have room to deviate from each other when λ is not too large.
...
, where A k = ∇F k is the Jacobian and F k (m) is the nonlinear forward operator with m the known smooth background model and δm the unknown perturbation (image).
