...
Consider sampling from a target density π(θ θ θ) that is defined by an energy function: U (θ θ θ), θ θ θ ∈ R D , π(θ θ θ) ∝ exp(−U (θ θ θ)).

In this paper, we focus on this sampling task with a Bayesian modeling set-up, i.e. given observed data D = {o o o n } N n=1 , we define a probabilistic model p(D, θ θ θ) = N n=1 p(o o o n |θ θ θ)p(θ θ θ), and we want samples from the target density defined as posterior distribution π(θ θ θ) = p(θ θ θ|D).

We use Bayesian neural networks as an illustrating example, in this case, o o o n = (x x x n , y y y n ), the prior p(θ θ θ) is a Gaussian N (θ θ θ; 0 0 0, λ −1 I I I), and the energy function is defined as DISPLAYFORM0 log p(y y y n |x x x n , θ θ θ) − log p(θ θ θ) = N n=1 (y y y n , NN θ θ θ (x x x n )) + λ||θ θ θ|| DISPLAYFORM1 with (y y y,ŷ y y) usually defined as the 2 loss for regression or the cross-entropy loss for classification.
...
Some of these methods, e.g. Hamiltonian Monte Carlo (HMC) (Duane et al., 1987; BID27 , further augment the state space as z z z = (θ θ θ, r r r) with auxiliary variables r r r, and sample from the augmented distribution π(z z z) ∝ exp (−H(z z z)), with the Hamiltonian H(z z z) = U (θ θ θ) + g(r r r) such that exp(−g(r r r))dr r r = C. Thus, marginalizing out the auxiliary variable r r r will not affect the stationary distribution π(θ θ θ) ∝ exp(−U (θ θ θ)).For deep learning tasks, the observed dataset D often contains thousands, if not millions, of instances, making MH rejection steps computationally prohibitive.
...
Such a Markov process is called Itô diffusion governed by the continuous-time SDEs: DISPLAYFORM2 with f f f (z z z) the deterministic drift, W W W (t) the Wiener process, and D D D(z z z) the diffusion matrix.

As a simple example, Langevin dynamics considers z z z = θ θ θ, f f f (θ θ θ) = −∇ θ θ θ U (θ θ θ) and D D D(θ θ θ) = I. Then using forward Euler discretization with step-size η the update rule of the parameters is DISPLAYFORM3 Stochastic gradient Langevin dynamics (SGLD, Welling & Teh, 2011) proposed an approximation to (3), by replacing the exact gradient ∇ θ θ θ U (θ θ θ) with an estimate using a mini-batch of datapoints: DISPLAYFORM4 Therefore SGLD can be viewed as a stochastic gradient descent (SGD) that adds in a properly scaled Gaussian noise term.
...
BID25 derived a framework of SG-MCMC samplers using advanced statistical mechanics BID50 BID37 , which explicitly parameterizes the drift f f f (z z z) : DISPLAYFORM5 with Q Q Q(z z z) the curl matrix, D D D(z z z) the diffusion matrix and Γ Γ Γ(z z z) a correction term.

Remarkably BID25 showed the completeness of their framework:1.

π(z z z) ∝ exp(−H(z z z)) is a stationary distribution of the SDE (2)+(5) for any pair of positive semi-definite matrix D D D(z z z) and skew-symmetric matrix Q Q Q(z z z); 2. for any Itô diffusion process that has the unique stationary distribution π(z z z), under mild conditions there exist D D D(z z z) and Q Q Q(z z z) matrices such that the process is governed by (2)+(5).As a consequence, the construction of an SG-MCMC algorithm reduces to defining the state-space z z z and the D D D(z z z), Q Q Q(z z z) matrices.
...
BID25 's completeness result indicates that our proposed method searches SG-MCMC samplers in the biggest subset of all Itô diffusion processes such that each instance is a valid posterior sampler.
...
Thus, the augmented state-space is z z z = (θ θ θ, p p p) (i.e. r r r = p p p), and the Hamiltonian is defined as H(z z z) = U (θ θ θ) + 1 2 p p p T p p p with identity mass matrix.

For neural networks, the dimensionality of θ θ θ can be at least tens of thousands.

Thus, training and applying full D D D(z z z) and Q Q Q(z z z) matrices can cause a huge computational burden, let alone gradient computations required by Γ Γ Γ(z z z).

To address this, we define the preconditioning matrices as follows: DISPLAYFORM0 Here f f f φ D and f f f φ Q are neural network parameterized functions that will be detailed in section 3.2, and c is a small positive constant.
...
From BID25 , our design has the unique stationary distribution π(θ θ θ) ∝ exp(−U (θ θ θ)) if f f f φ D is non-negative for all z z z.

We discuss the role of each precondition matrix for better intuition.

The curl matrix Q Q Q(z z z) in (2) mainly controls the deterministic drift forces introduced by the energy gradient ∇ θ θ θ U (θ θ θ) (as seen in many HMC-like procedures and in eq. FORMULA5 ).
...
This explains the squared term DISPLAYFORM2 The positive scaling constant α is heuristically selected following (Chen et al., 2014; BID25 (see appendix).

Finally, the extra term DISPLAYFORM3 is responsible for compensating the changes introduced by preconditioning matrices Q Q Q(z z z) and D D D(z z z).The discretized dynamics of the state z z z = (θ θ θ, p p p) with step-size η and stochastic gradient ∇ θ θ θŨ (θ θ θ) are DISPLAYFORM4 We use a modified forward Euler discretization BID27 here, and the computation graph of eq. FORMULA10 is visualized in the right part of FIG0 (see appendix for SGHMC discretized updates).Again we see that Q Q Q f (z z z) is responsible for the acceleration of θ θ θ, and from the DISPLAYFORM5 controls the friction introduced to the momentum.

Note that in the big-data setting, the noisy gradient is approximately Gaussian distributed with mean 0 0 0 and variance V V V (θ θ θ).
...
These corrections can be dropped when the discretization step-size η is small, therefore, we do not consider them in our experiments.

We now present detailed functional forms for f f f φ Q and f f f φ D .
...
One useful source of information to identify this is the energy function U (θ θ θ θ θ θ θ θ θ).

1 We also include the momentum p i to the inputs of f f f φ Q , allowing the Q Q Q(z z z) matrix to observe the velocity information of the θ θ θ i .

We further add an offset β to Q Q Q(z z z) to prevent the vanishing of this matrix.

Putting all of them together, we define the i DISPLAYFORM0 The corresponding Γ Γ Γ(z z z) term requires both DISPLAYFORM1 The energy gradient ∂ θi U (θ θ θ) also appears in (7), so it remains to compute ∂ U f f f φ Q , which, along with ∂ pi f φ Q (U (θ θ θ), p i ), can be obtained by automatic differentiation BID0 .Matrix D D D(z z z) is responsible for the friction and the stochastic gradient noise, which are crucial for better exploration around high-density regions.

Therefore, we also add the energy gradient ∂ θi U (θ θ θ) to the inputs, meaning that the i DISPLAYFORM2 By the construction of the D D D(z z z) matrix, the Γ Γ Γ vector only requires ∇ p p p D D D f without computing any higher order information.

In practice, both U (θ θ θ) and ∂ θi U (θ θ θ) are replaced by their stochastic estimatesŨ (θ θ θ) and ∂ θiŨ (θ θ θ), respectively.

To keep the scale of the inputs roughly the same across tasks, we rescale all the inputs using statistics computed by simulating the sampler with randomly initialized f f f φ D and f f f φ Q .

When the computational budget is limited, we replace the exact gradient computation required by Γ Γ Γ(z z z) with finite difference approximations.
...
From now on we consider the discretized dynamics and define q t (θ θ θ|D) as the marginal distribution of the random variable θ θ θ at time t.

Cross-chain loss We introduce cross-chain loss that encourages the sampler to converge faster.

Since the sampler is guaranteed to have the unique stationary distribution π(θ θ θ) ∝ exp(−U (θ θ θ)), fast convergence means that KL[q t ||π] is close to zero when t is small.
...
The issue is mitigated by thinning that collects samples for every τ step (after burn-in), which effectively draws samples from the averaged distributionq(θ θ θ|D) = 1 T /τ T /τ s=1 q sτ (θ θ θ).

The in-chain loss is, therefore, defined as the ELBO evaluated at the averaged distributionq, which is then approximated by Monte Carlo with samples DISPLAYFORM3 obtained by thinning: DISPLAYFORM4 Gradient approximation We leverage the recently proposed Stein gradient estimator to estimate the intractable gradients ∇ φ log q t (θ θ θ) for cross-chain loss and ∇ φ logq(θ θ θ) for in-chain loss.

Precisely, by the chain rule, we have ∇ φ log q t (θ θ θ) = ∇ φ θ θ θ∇ θ θ θ log q t (θ θ θ), so it remains to estimate the gradients G G G = ( extensions to Riemannian Langevin dynamics and HMC BID6 have also been proposed BID30 BID25 .
...
On the other hand, the proposed sampler (with η → 0) is guaranteed to be correct by the complete framework BID25 .
...
For illustration purposes, we also plot in the other two panels the trajectory of samples by simulating NNSGHMC (middle) and SGHMC (right) on a 2D Gaussian for a fixed amount of time ηT .
...
In all tests, the sampler is trained with a 1-hidden layer multi-layer perceptron (MLP) (20 units, ReLU activation) as the underlying model for the target distribution π(θ θ θ).
...
Architecture generalization (NT) The test CNN has two convolutional layers (3 × 3 × 3 × 16 and 3 × 3 × 16 × 16) and one fc layer (100 hidden units), resulting in roughly 4× dimenality of θ θ θ.
...
Here a single datum is a sequence o o o n = {x x x 1 n , ..., x x x T n } and the log-likelihood is defined as log p(o o o n |θ θ θ) = T t=1 log p(x x x n t |x x x n 1 , . . .

, x x x n t−1 , θ θ θ), with each of the conditional densities produced by a gated recurrent unit (GRU) network (Cho et al., 2014) .
...
At test time, we follow Chen et al. FORMULA1 and set the step-size to η = 0.001.
...
Therefore, we further test the meta sampler on JSB without the offset β in f f f φ Q to reduce the acceleration (denoted as NNSGHMC-s).
...
Then the corresponding drift f f f (θ θ θ, p p p) and diffusion matrix D D D are: DISPLAYFORM0 where C C C is a positive definite matrix called friction coefficient.

Thus, HMC's continuous-time dynamics is governed by the following SDE: DISPLAYFORM1 The discretized update rule (with simple Euler discretization) of HMC with step-size η is DISPLAYFORM2 If stochastic gradient ∇Ũ (θ θ θ) is used, we need to replace the covariance matrix of with 2η(C C C −B B B) whereB B B is the variance estimation of the gradients.
...
Also we can rewrite the SGHMC update equations by setting ηp p DISPLAYFORM4 Thus, the discretized SGHMC updates can be viewed as the SGD-M update injected with carefully controlled Gaussian noise.
...
Therefore we further approximate it using delayed estimate: DISPLAYFORM8 Similarly, the Γ Γ Γ p p p term expands as DISPLAYFORM9 We further approximate DISPLAYFORM10 ∂U (θ θ θ) by the following DISPLAYFORM11 This only requires the storage of previous Q Q Q matrix.

However, DISPLAYFORM12 ∂pi requires one further forward pass to obtainf DISPLAYFORM13 Therefore the proposed finite difference method only requires one more forward passes to computê f f f t−1 φ D and instead, save 3 back-propagations.
...
Time complexity figures Every SG-MCMC method (including the meta sampler) requires ∇ θ θ θŨ (θ θ θ).
...
For a distribution q(θ θ θ) that is implicitly defined by a generative procedure, the density q(θ θ θ) is often intractable. derived the Stein gradient estimator that estimates G = (∇ θ θ θ 1 log q(θ θ θ 1 ), · · · ∇ θ θ θ K log q(θ θ θ K )) T on samples θ θ θ 1 , ..., θ θ θ K ∼ q(θ θ θ).
...
DISPLAYFORM0 be a differentiable multivariate test function which maps θ θ θ to a column vector DISPLAYFORM1 T .

One can use integration by parts to show the following Stein's identity when a boundary condition lim ||θ θ θ||→∞ q(θ θ θ)h(θ θ θ) = 0 is assumed for the test function: DISPLAYFORM2 This boundary condition holds for almost any test function if q has sufficiently fast-decaying tails (e.g. Gaussian tails).

proposed the Stein gradient estimator for ∇ θ θ θ log q(θ θ θ) by inverting a Monte Carlo (MC) version of Stein's identity (23): DISPLAYFORM3 Then G is obtained by ridge regression (with || · || F the Frobenius norm of a matrix) DISPLAYFORM4 which has an analytical solutionĜ DISPLAYFORM5 where DISPLAYFORM6 Here θ θ θ k (j) denotes the j th element of vector θ θ θ k .

One can show that the RBF kernel satisfies Stein's identity .

In this case h(θ θ θ) = K(θ θ θ, ·), d = +∞ and by the reproducing kernel property, h(θ θ θ) FORMULA1 also show that the Stein gradient estimator can be obtained by minimizing a Monte Carlo estimate of the kernelized Stein discrepancy (Chwialkowski et al., 2016; .
...
Representative work include deep kernel learning for Gaussian processes BID48 , and adversarial approaches to learn kernel parameters Bińkowski et al., 2018) .

Unfortunately, both approaches do not scale very well to our application as θ θ θ has at least tens of thousands of dimensions.

Furthermore, they both considered kernel learning for observed data, while in our case θ θ θ is a latent variable to be inferred.
...
To mediate this issue, we propose to pre-process the inputs to both f f f φ D and f f f φ Q networks to make it at similar scale as those in training task.

Recall that the energy function is U (θ θ θ)

= − N n=1 log p(y y y n |x x x n , θ θ θ) − log p(θ θ θ) where the prior log p(θ θ θ) is often an isotropic Gaussian distribution.

Thus the energy function scale linearly w.r.t both the dimensionality of θ θ θ and the total number of observations N .

Often the energy function is further approximated using mini-batches of M datapoints.

Putting them together, we propose pre-processing the energy as DISPLAYFORM0 where D train and D test are the dimensionality of θ θ θ in the training task and the test task, respectively.
...
We do not set any offset and additional frictions, i.e. α = 0 and β = 0.

The noise estimation matrixB B B are set to be 0 for both meta sampler and SGHMC.

To mimic stochastic gradient, we manually inject Gaussian noise with zero mean and unit variance into ∇ θ θ θŨ (θ θ θ) = ∇ θ θ θ U (θ θ θ) + , ∼ N (0 0 0, I I I).

The functions f f f φ D and f f f φ Q are represented by 1-hidden-layer MLPs with 40 hidden units.
...
To stabilize the meta sampler we also clamp the output values of f f f φ Q within [−5, 5] .
...
Also, we scale up f f f φ D by 50 to account for sum of stochastic noise.

The offset α is selected as 0.01 η as suggested by Chen et al. (2014) , where η = lr N with lr the per-batch learning rate.

We also turn off the off-set and noise estimation, i.e. β = 0 andB B B = 0.
...
Both f f f φ D and f f f φ Q are parameterized by 1-hidden-layer MLPs with 10 units.
...
We re-initialize θ θ θ and momentum after each epoch.
...
After, we increase the per-batch learning rate to 0.018 with clipped f f f φ Q .
...
Note that even though we use the same per-batch learning rate as before, the discretization step-size is now different due to smaller training dataset, thus, α will be automatically adjusted accordingly.
...
Therefore the dimensionality of θ θ θ is 15, 768.

The training sampler discretization step-size η is 0.0007 50000. and scaling term is α = 0.005 η .

To make it analogous to optimization methods, we call 0.0007 as per-batch learning rate and 0.005 as friction coefficient.

The f f f φ Q and f f f φ D are defined by 2-layer MLPs with 10 hidden units.

We set the offset values to 0 for both Q Q Q and D D D. Further, we scale up the output of D D D f (z z z) by 10 and its gradient input ∇U (θ θ θ) by 100.

We scale up the energy input U (θ θ θ) to both f f f φ Q and f f f φ D by 5.
...
Then we start the next sub-epoch using the last θ θ θ and p p p.

After we finish all sub-epoch, we re-initialize the θ θ θ and p p p using replay techniques with probability 0.15.
...
Thus, the dimensionality of θ θ θ is 61,478, roughly 4 times of the training dimension.
...
We scale the D D D output by 20 and set α =

η , where η is defined in the same way as before.

We use zero offset during training, i.e. β = 0.

We apply input pre-processing for both f f f φ D and f f f φ Q .

To prevent divergence of the meta sampler at early training stage.

We also set the constant of c = 100 to the f φ D .

For dataset generalization, we tune the off-set value based on Piano validation set and transfer the tuned setting β = −1.5 to the other three datasets.

For Piano architecture generalization, we do not tune any hyper-parameters including β and use exactly same settings as training.
...
This potentially makes the generalization much harder due to inconsistent energy scale fed into f f f φ Q and f f f φ D .
...
We also provide additional contour plots (Figure 9) for MNIST experiments to demonstrate the strategy learned by f f f φ D for reference.
