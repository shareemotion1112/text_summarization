...
Let p t be the target distribution which the user is interested in, and it can be expressed as DISPLAYFORM0 where α ∈ [0, 1].
...
The generator function G(z; θ g ) : DISPLAYFORM2 represents a mapping to data space, where G is a differentiable function with parameters θ g .

The discriminator is defined as D (x; θ d ) : R N → [0, 1] that outputs a single scalar.
...
α is the ratio between p g and p d in the mixture distribution.

We used k = 1 and α = 0.8 in experiments.01.
...
In those case studies, α = 0.8 in Eq. FORMULA0 is used.
...
We will first show the optimal discriminator given G and then show that minimizing V (G, D) via G given the optimal discriminator is equivalent to minimizing the Jensen-Shannon divergence between (1 − α)p g + αp d and pd.
...
The assumption, αp d (x) ≤ pd(x) for all x's, in Theorem 1 may be impractical in real applications.
...
Suppose αp d (x) ≥ pd for x ∈ Supp(p d ) and all density functions p d (x), pd(x) and p g (x) are continuous.
...
Given a trained classifier C parameterized by θ and a loss function (x; y; C θ ), adversarial training solves a min-max game, where the first step is to find adversarial examples within -ball for maximizing the loss, and the second step is to train the model for minimizing the loss, given adversarial examples.
...
Instead of relying on PGD, our DSGAN generates adversarial examples directly, which are combined into real training data to fine-tune C θ .
...
In order to have DSGAN generate such samples, we let the samples of pd be the linear combination of those from L and DISPLAYFORM2 tend to match pd while the term −αp d ensures that samples from p g do not belong to p d .
...
The JSD returns the minimal value, which is 0, iff both distributions are the same, namely pd = (1 − α)p g + αp d .

Because p g (x)'s are always non-negative, it should be noted both distributions are the same only if αp d (x) ≤ pd(x) for all x's.
...
By the proof idea of Proposition 2 in BID6 , if f (x) = sup α∈A f α (x) and f α (x) is convex in x for every α, then DISPLAYFORM0 includes the derivative of the function at the point, where the maximum is attained, implying the convergence with sufficiently small updates of p g .

We complete this proof.

We provide a trick to stabilize the training procedure by reformulating the objective function.

Specifically, V (G, D) in (2) is reformulated as: DISPLAYFORM0 Instead of sampling a mini-batch of m samples from p z and p d in Algorithm 1, (1 − α)m and αm samples from both distributions are required, respectively.
...
TAB3 shows our setting of hyperparameters, where β is defined in (8).
