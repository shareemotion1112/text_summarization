...
Hradiš [3] trained a CNN to restore document images.
...
We used 10000 image pairs from Hradiš' [3] training dataset which were created from random patches of documents from the CiteSeerX repository.

We tested our proposed model on Hradiš' [3] test dataset containing unseen document images using unique kernels with different levels of noise degradation.

We compared our results with the baseline network presented by Hradiš, a model which can be duly regarded as a benchmark in the problem space of blind document deblurring.
