...
Given two domains X and Y , presented as cat and dog images, we postulate that these two domains are in part explained by random variables U, V from the shared semantic space and random variables ψ and ξ independent of U, V that explain features specific to X and Y , respectively.
...
Given distributions P X and P Y on the domains X and Y, respectively, we assume that there exist random variables U, V on a measurable space (S, Σ) such that, (A) Given U and V , X and Y are conditionally independent: DISPLAYFORM0 (B) Distributions P U |X and P V |Y are deterministic.

DISPLAYFORM1 We would call (S, Σ) a shared semantics between X and Y .
...
Therefore, we model shared semantics through measurable space (S, Σ), while probabilities of certain modes in Σ by separate variables U and V , as they may differ between source and target variables.
...
Given (possibly empirical) distributions P X , P Y on domains X , Y the learner attempts to find a (possibly) nondeterministic mapping T : X → Y and a shared semantics (S, Σ) such that, DISPLAYFORM2 Although the last assumption is somewhat vague in generality, if we fix sufficiently large S ⊂ R n , the σ-algebras on S can be ordered by inclusion.

We postulate that the quality of transfer depends on the expressiveness of the shared semantics Σ.
...
Therefore, modelling a domain transfer from X to Y can be done with the use of additional variable ξ ∈ Ξ that captures the variability of Y |X, so that there exists a deterministic map G : S × Ξ → Y.
...
In fact a trivial space (S, Σ) = ({0}, {{0}}) satisfies assumptions (A)-(C), while a random transfer from X to Y satisfies covering modes and shared semantics conditions.

Therefore, having a shared semantics space of sufficient capacity is crucial for the quality of the transfer.

Even with large enough space S, learning a mapping 1 we bear a risk of it ignoring the actual source, inferring the Y only from ξ.

This is equivalent to trivial σ-algebra Σ = {∅, S} and again leads to a random transfer.
...
Alternatively, T can be penalized for inferring X from noise ξ, by minimizing I(X; ξ).
...
CycleGAN through its deterministic character assumes that all semantics are shared between domains X and Y, effectively setting (S, Σ) ≡ (Y, σ(Y )).
...
We sample x from the real data distribution P X and fed through E. F (ξ) and E(x) are concatenated and fed through G to get a generated sample y .ables X and Y with joint distribution P X,Y is defined as, DISPLAYFORM1 where P x , P y are marginal densities of X and Y , respectively.
...
That is exactly what BID2 propose with their Mutual Information Neural Estimation (MINE): DISPLAYFORM3 where S θ is the statistics network which is a neural network parameterized by θ.

BID2 provides a proof that I(X; Y ) ≥ I Θ (X; Y ).
...
Feed (x, ξ) through T to get y = T (x, ξ).Note that by searching intoT , we are effectively searching in a s pace of stochastic mappings, namely T = {T Z |T Z = T (·, Z) for T ∈T and Z ∼ N (0, I)}. Specifically, we model T as the composition of networks E, F and G, where• E : X → S is modelled as a convolutional network and can be interpreted as an encoder, • F :
...
As mentioned in Section 2.3, the transfer network can fall into a failure mode where Y is generated using Ξ only.
...
Again, only a GAN loss was necessary to achieve such results.(1) (2) (3) Figure 3 : 64 × 64 Edge to shoes samples generated using (1) TI-GANmaximizingÎ(X; Y ), (2) Only a GAN penalty, (3) Using CycleGAN recommended architecture DISPLAYFORM0 Figure 4: 64 × 64 Edge to shoes samples generated using (1) TI-GANmaximizing I(X; T (X, Ξ)), MNIST to SVHN is a harder task because it requires a geometric transformation.
...
Samples in (2) uses a GAN loss while minimizing I(Ξ; T (X, Ξ)).
...
We see an improvement in accuracy when using TI-GAN when minimizing I(Ξ; Y ).
