...
Here, the "embedding" stands for delay/shift-embedding (i.e., Hankelization) which is a copy/duplication operation of image-patches by sliding window of patch size (τ, τ ).
...
To simplify the complicated "encoder-decoder" structure of ConvNet used in DIP, we consider the following network structure: Embedding H (linear), encoding φ r (non-linear), decoding ψ r (non-linear), and backward embedding H † (linear) (see Fig. 1 ).

Note that its encoder-decoder part (φ r , ψ r ) is just a simple multi-layer perceptron along the filter domain (i.e., manifold learning), and it is sandwitched between forward and backward embedding (H, H † ).
...
When we set the horizontal dimension of hidden tensor L with r, each τ 2 -dimensional fiber in H, which is a vectorization of each (τ, τ )-patch of an input image, is encoded into r-dimensional space.
...
We have r ≤ D. For simplicity, we putted D := n τ n and T := n (I n +τ n −1).

For tensor completion task, F := P Ω is a projection operator onto support set Ω so that the missing elements are set to be zero.
...
We minimize the distance between observation Y and reconstruction X with its support Ω, and all patches in X should be included in some restricted manifold M r .
...
We consider an AE to define the r-dimensional manifold M r in ( n τ n )-dimensional Euclidean space as follows:
...
The properties of the manifold M r are determined by the properties of φ r and ψ r .

By employing multi-layer perceptrons (neural networks) for φ r and ψ r , encoder-decoder may provide a smooth manifold.
...
where we put A r : R D×T → R D×T as an operator which auto-encodes each column of a input matrix with (ψ r ,φ r ), and [g 1 , g 2 , ..., g T ] as a matrix, which has Hankel structure and is transformed by Hankelization of some input tensor Z ∈ R I1×I2×···×I N .

Note that Z is the most compact representation for Hankel matrix [g 1 , g 2 , ..., g T ].

Eq. (4) describes the MMES network shown in Fig. 1 : H,φ r ,ψ r and H † are respectively corresponding to forward embedding, encoding, decoding, and backward embedding, where encoder and decoder can be defined e.g. by multi-layer perceptrons (i.e., repetition of linear transformation and non-linear activation).
...
where we refer respectively the first and second terms by a reconstruction loss and an auto-encoding loss, and λ > 0 is a trade-off parameter for balancing both losses.

Optimization problem (5) consists of two terms: a reconstruction loss, and an auto-encoding loss.

Hyperparameter λ is set to balance both losses.

Basically, λ should be large because auto-encoding loss should be zero.

However, very large λ prohibits minimizing the reconstruction loss, and may lead to local optima.

Therefore, we adjust gradually the value of λ in the optimization process.
...
Adaptation of λ is just an example, and it can be modified appropriately with data.

Here, the trade-off parameter λ is adjusted for keeping L rec > L AE , but for no large gap between both losses.
...
Window size of delay-embedding was τ = 64, the lowest dimension of auto-encoder was r = 3, and additive noise standard deviation was set to σ = 0.05.
...
For MMES, we employed the fixed AE structure as [32τ 2 , r, 32τ 2 ], and parameters as τ = 4, r = 16, and σ = 0.01 for all nine cases.
...
Hankelization of this twodimensional array (matrix) with τ = [2, 2] is given by scanning a matrix with local (2,2)-window column-wise manner, and unfold and stack each local patch left-to-right.
...
Fig. 10 shows the duplication matrix with τ .

For example, our Hankelization with reflection padding of f = [f 1 , f 2 , ..., f 7 ] with τ = 3 is given by Fig. 11 shows an example of our multiway-delay embedding in case of second order tensors.
...
, and trim τ = pad † τ is a trimming operator for removing (τ n −1) elements at start and end of each mode.

Note that H † •H is an identity map, but H • H † is not, that is kind of a projection.

Delay embedding and its pseudo inverse can be implemented by using convolution with all onehot-tensor windows of size (τ 1 , τ 2 , ..., τ N ).

The one-hot-tensor windows can be given by folding a D-dimensional identity matrix I D ∈ R D×D into I D ∈ R τ1×···×τ N ×D .
...
In case that we have I columns, it consists of (I − τ + 1) identity matrices of size (τ, τ ).
...
Finally, we designed an auto-encoder with controlling the dimension r and the standard deviation σ of additive zero-mean Gaussian noise.
...
For the proposed MMES method, we adaptively selected the patch-size τ , and dimension r. Table 2 shows parameter settings of τ = [τ, τ ] and r for MMES.

Noise level of denoising auto-encoder was set as σ = 0.05 for all images.
...
In this experiments, DIP was conducted with the best number of iterations from {1000, 2000, 3000, ..., 9000}. For four times (x4) up-scaling in MMES, we set τ = 6, r = 32, and σ = 0.1.

For eight times (x8) up-scaling in MMES, we set τ = 6, r = 16, and σ = 0.1.

For all images in MMES, the architecture of auto-encoder consists of three hidden layers with sizes of [8τ 2 , r, 8τ 2 ].
...
λ was adapted by Algorithm 1 every 10 iterations.

Fig. 15 shows optimization behaviors of reconstructed image, reconstruction loss L rec , auto-encoding loss L DAE , and trade-off coefficient λ.
...
We evaluate the sensitivity of MMES with three hyper-parameters: r, σ, and τ .

First, we fixed the patch-size as (8, 8) , and dimension r and noise standard deviation σ were varied.

the reconstruction results of a 99% missing image of 'Lena' by the proposed method with different settings of (r, σ).
...
Furthermore, some appropriate noise level (σ = 0.05) provides sharp and clean results.
...
Next, we fixed the noise level as σ = 0.05, and the patch-size were varied with some values of r. Fig. 18 shows the results with various patch-size settings for recovering a 99% missing image.

The patch sizes τ of (8,8) or (10,10) were appropriate for this case.
...
For the proposed method, 3D patch-size was set as τ = [4, 4, 4] , the lowest dimension was r = 6, and noise level was σ = 0.05.
