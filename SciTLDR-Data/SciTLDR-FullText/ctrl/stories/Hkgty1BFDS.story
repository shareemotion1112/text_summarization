...
Assume that there is an unlabeled image set I, and a batch set B with n images: B = {x 1 , x 2 , ..., x n } ⊂ I. Our goal is to get a feature extractor f θ whose representations (i.e., v i = f θ (x i )) are "visually meaningful," a definition we discussed earlier.
...
A temperature parameter (τ < 1) was added to ensure a label distribution with low entropy .
...
Input : Unlabeled image set I, encoder f θ to train, the number of total rounds for training:
...
We select neighborhood set N fromÑ that is in a lower entropy order.
