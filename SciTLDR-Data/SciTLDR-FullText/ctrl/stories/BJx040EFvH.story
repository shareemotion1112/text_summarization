...
This attack was enhanced by adding a randomization step, which was referred to as R+FGSM (Tramèr et al., 2017) .
...
Given a network f θ parameterized by θ, a dataset (x i , y i ), a loss function and a threat model ∆, the learning problem is typically cast as the following robust optimization problem,

A typical choice for a threat model is to take ∆ = {δ : δ ∞ ≤ } for some > 0.

This is the ∞ threat model used by Madry et al. (2017) and is the setting we study in this paper.

The procedure for adversarial training is to use some adversarial attack to approximate the inner maximization over ∆, followed by some variation of gradient descent on the model parameters θ.
...
A better approximation of the inner maximization is to take multiple, smaller FGSM steps of size α instead.

When the iterate leaves the threat model, it is projected back to the set ∆ (for ∞ perturbations.

This is equivalent to clipping δ to the interval [− , ]).
...
A combination of all these techniques is Algorithm 1 PGD adversarial training for T epochs, given some radius , adversarial step size α and N PGD steps and a dataset of size M for a network f θ

Update model weights with some optimizer, e.g. SGD end for end for Algorithm 2 "Free" adversarial training for T epochs, given some radius , N minibatch replays, and a dataset of size M for a network f θ δ = 0 // Iterate T/N times to account for minibatch replays and run for T total epochs for t = 1 . . .
...
This method takes FGSM steps with full step sizes α = followed by updating the model weights for N iterations on the same minibatch (also referred to as "minibatch replays").
...
Note that randomized initialization for FGSM is not a new idea and was previously studied by Tramèr et al. (2017) .

Crucially, Tramèr et al. (2017) use a different, more restricted random initialization and step size, which does not result in models robust to full-strength PGD adversaries.

A more detailed comparison of their approach with ours is in Appendix A.

To test the effect of initialization in FGSM adversarial training, we train several models to be robust at a radius = 8/255 on CIFAR10, starting with the most "pure" form of FGSM, which takes steps of size α = from a zero-initialized perturbation.
...
The adversarial accuracies in Table 1 are calculated using a PGD adversary with 50 iterations, step size α = 2/255, and 10 random restarts.

Specific optimization parameters used for training these models can be found in Appendix B.

FGSM step size Note that an FGSM step with size α = from a non-zero initialization is not guaranteed to lie on the boundary of the ∞ ball, and so this defense could potentially be seen as too weak.

We find that increasing the step size by a factor of 1.25 to α = 10/255 further improved the robustness of the model so that it is on par with the best reported result from free adversarial training.

However, we also found that forcing the resulting perturbation to lie on the boundary with a step size of α = 2 resulted in catastrophic overfitting: it does not produce a model robust to adversarial attacks.
...
All experiments using FGSM adversarial training in this section are carried out with random initial starting points and step size α = 1.25 as described in Section 4.1.
...
For N epochs, we use a cyclic learning rate that increases linearly from 0 to λ over the first N/2 epochs, then decreases linearly from λ to 0 for the remaining epochs, where λ is the maximum learning rate.

For each method, we individually tune λ to be as large as possible without causing the training loss to diverge, which is the recommended learning rate test from Smith & Topin (2018) .
...
Tramèr et al. (2017) and the various changes for the version of FGSM adversarial training done in this paper, over 10 random seeds.

Step size Initialization Robust accuracy R+FGSM (Tramèr et al., 2017) 0 (2017) While a randomized version of FGSM adversarial training was proposed by Tramèr et al. (2017) , it was not shown to be as effective as adversarial training against a PGD adversary.

Here, we note the two main differences between our approach and that of Tramèr et al. (2017) .
...
We use a full step size of α = , whereas Tramèr et al. (2017) use a step size of α = /2.
...
Additionally, note that R+FGSM as done by Tramèr et al. (2017) has high variance in robust performance when done over multiple random seeds, whereas our version of FGSM adversarial training is significantly more consistent and has a very low standard deviation over random seeds.
...
We plot the mean and standard error of the robust accuracy for models trained for 30 epochs over 3 random seeds in Figure 3 , and vary the step size from α = 1/255 to α = 16/255.
...
For example, using a zero initialization or using the random initialization scheme from Tramèr et al. (2017) will result in adversarial examples whose features have been perturbed by {− , 0, }, and so the network learns a decision boundary which is robust only at these perturbation values.
