...
At every step, the agent observes s t , and selects a vector-valued action a t , according to a stochastic policy π(a t |s t ), which gives the probability that an agent executes a particular action from a particular state.
...
A sequence of states, actions and rewards generated in this manner is referred to as a trajectory τ .We define the discounted return from step t within a trajectory to be R DISPLAYFORM0 The objective of the learning agent is to maximize the expected per-trajectory return, given by ρ = DISPLAYFORM1
...
Policy gradient methods directly optimize ρ by performing stochastic gradient ascent on the parameters θ of a family of policies π θ .
...
The agent is given access to a set of options, indexed by ω.

Each option has its own policy: π ω (a t |s t ), an initiation set, representing the states in which the option is available, and a termination function β ω (s t ), which represents the state-dependent probability of terminating the option.

Additionally, the policy over options, π Ω (ω t |s t ) is employed to select from available options once termination of the previous option occurs.
...
The policy over options can be combined with the termination function in order to yield the optionto-option policy function: DISPLAYFORM0 where δ is the Kronecker delta.

To learn options using a policy gradient method we parametrize all aspects of the policy: π Ω,θ denotes the policy over options, parametrized by θ.

π ω,ϑ then denotes the intra-option policy of option ω, parametrized by ϑ. Finally β ω,ξ is the termination function for ω, parametrized by ξ.

We aim to optimize the performance of the agent with respect to a set of policy parameters.

The loss function is identical to that employed by traditional policy gradient methods: we optimize the expected return of trajectories in the MDP sampled using the current policy, DISPLAYFORM0 where E τ denotes expectation over sampled trajectories.
...
In this work, we use a learned parametric approximation of the value function V ν as baseline.

The value function is learned using gradient descent on the mean squared prediction error of Monte-Carlo returns: DISPLAYFORM8 Estimating the value function can also be done using other standard methods such as LSTD or TD(λ).Below, we describe the algorithm for learning options to optimize returns through a series of interactions with the environment.
...
Initialize parameters randomly foreach episode do ω 0 ∼ π Ω (ω|s 0 ) // sample an option from the policy over options at the initial state for t ← 0, . . .

, T do a t ∼ π ωt (s t ) // sample an action according to the current intra-option policy Get next state s t+1 and reward r t from the system ω t+1 ∼π Ω (ω t+1 |ω t , s t+1 ) // sample the next option according to the policy over option end Update ν according to (4), using sampled episode θ, ϑ, and ξ according to (3), using sampled episode end
...
The value function for IOPG and AC methods was represented using a final linear layer of size 1, and for OC, size |Ω|.
...
A moving average of the continuation probability (1 − β ω (s t )) during training indicates that early on, when the options are not well optimized, termination occurs quite frequently.
