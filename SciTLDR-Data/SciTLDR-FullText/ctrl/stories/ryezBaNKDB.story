...
Denote conventional convolution operation as Y = f (X; Θ) where Θ is the shared parameters at different positions.
...
We replace the fixed filters with context-dependent filters y p = f ({x p }; w p ) where the filters w p are in turn obtained as w p = g({x p }; Θ).

The mapping g is the structure of object-in-context patterns and Θ are the learned parameters as we hypothesize.

The entire relation between Y and X can be compactly represented through the higher-order function Y = f (X; g(X; Θ)).
...
Several methods (Marszałek et al., 2009; Sun et al., 2009; Kovashka & Grauman, 2010; Vail et al., 2007; Cao et al., 2015; Chen et al., 2014) exploit contextual information to facilitate action recognition.

Marszałek et al. (2009) exploited the context of natural dynamic scenes for human action recognition in video.
...
In our model Y is derived from X through a second-order relation of the form Y = f (X, g(X; Θ)) -the relation being second order since the function f relating the input and output maps takes a function g as arguments to generate parameters of function f .
...
Representing the entire set of filter parameters as W = {W p,q }, we have W = g(X; Θ) The actual number of parameters required to define the block is the number of components in Θ. We propose two models for g(·) below, with different requirements for the number of parameters.
...
The size of R represents the span from which context is captured to compute any single y p , and must ideally be larger than, or at least no smaller than the size of R. Θ q t are the convolutional filter parameters.

Each Θ q t is a C in × C in matrix.

The complete set of parameters of g() are given by Θ = {Θ q t , q ∈ R, t ∈ R }, with the total number of parameters equal to C 2 in × |R| × |R | where |R| is the number of elements in R.

The shared weights Θ capture the higher-level patterns required to characterize spatio-temporal context.
...
where Θ q are the parameters of the CNN.
...
Following (Qiu et al., 2017b) , we use 3 layers of Pseudo-3D (P3D) convolutions to implement the ConvNet CN N (·, Θ q ) in Equation 6 for obtaining a sufficiently large context field.
