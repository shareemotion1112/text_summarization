...
Throughout the paper, σ denotes a relational vocabulary, consisting of a finite set of predicates or relations R, and a finite set of constants or entities C. The Herbrand Base of σ is the set of all ground atoms, called σ-atoms, of the form R(c 1 , ..., c n ) with R ∈ R and c i ∈ C (that is, all tuples that can be constructed from R and C).

A σ-interpretation is a truth value assignment to all the σ-atoms, often represented as a set containing all the σ-atoms mapped to True.

A relational database can be seen as a σ-interpretation BID0 .

Here, database relations correspond to predicates and database tuples correspond to σ-atoms.

The σ-atoms that are true in the σ-interpretation are those that are listed in the database tables.

In databases (represented as σ-interpretations), query answering can be formalized as follows.
...
, x k /t k ] such that ω |= Q[x 1 /t 1 , x 2 /t 2 , . . . , x k /t k ].

The answer to a query Q w.r.t.

a database ω is denoted as Ans(Q, ω).Example 1 Consider the database consisting of Tables 1, 2 and 3 and the query Q 1 (x) = location(x, mit) ∧ coauthor(x, fred).
...
The rule is understood as defining tuples of a new table R that, in this case, contains all tuples [t] where [z/t] ∈ Ans(∃x, y. location(z, x) ∧ coauthor(z, y), ω).
...
Mathematically, A PDB D, for a vocabulary σ, is a finite set of tuples of the form t, p , where t is a σ-atom and p ∈ [0, 1].

Moreover, if t, p ∈ D and t, q ∈ D, then p = q. Each PDB for the vocabulary σ induces a unique probability distribution over σ-interpretations ω: DISPLAYFORM0 where DISPLAYFORM1 Example 2 TAB0 and 3 form a PDB with 3 relations: coauthor/2, author/2 and location/2.Furthermore, the probability of any Boolean query Q w.r.t.
...
Suppose that we introduce a new probabilistic relation A(x, y) with all possible σ-atoms A(t 1 , t 2 ) having the same probability p and replace the original rule by R(x, y) :-coauthor(x, y), A(x, y).

By changing the parameter p we can now decrease the probabilities of the σ-atoms R(t 1 , t 2 ).We use the following notation as syntactic sugar for probabilistic rules.
...
Since most domains will be sparse, we may assume that a majority of the tuples (over elements of the given domain) will have some very small probability γ target , possibly zero.
