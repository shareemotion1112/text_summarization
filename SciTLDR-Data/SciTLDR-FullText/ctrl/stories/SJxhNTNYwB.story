...
For un-targeted attack, our goal is to find a small perturbation δ such that the classifier predicts the wrong label, i.e. arg max F (x + δ) = y. And for targeted attack, we want the classifier to predicts the target label t, i.e. arg max F (x + δ) = t. The perturbation δ is usually bounded by p norm: δ p ≤ ε, with a small ε > 0.

Adversarial perturbations often have high transferablity across different DNNs.

Given a white-box source DNN F s with known architecture and parameters, we can transfer its white-box adversarial perturbation δ s to a black-box target DNN F t with reasonably good success rate.

It is known that even if x + δ s fails to be an adversarial example, δ s can still act as a good starting point for searching adversarial examples using a score-based attack method.
...
The decoder takes z as the input and outputs an adversarial perturbation δ = ε tanh(D(z)) with dim(δ) = dim(x).

In our new method, we will train the generator G so that δ = ε tanh(G(x)) can fool the source network F s .
...
where t denotes the targeted class and κ is the margin parameter that can be used to adjust transferability of the generator.

A higher value of κ leads to higher transferability to other models (Carlini & Wagner, 2017) .

We focus on ∞ norm in this work.

By adding point-wise tanh function to an unnormalized output D(z), and scaling it with ε, δ = ε tanh(D(z)) is already bounded as δ ∞ < ε.
...
Denote the surrogate loss by L, rather than calculating ∇ δ L(x+δ, y) directly, NES update δ by using

The expectation can be approximated by taking finite samples.

And we could use the following equation to iteratively update δ:

where η is the learning rate, b is the minibatch sample size, ω k is the sample from the gaussian distribution and [−ε,ε] represents a clipping operation, which projects δ onto the ∞ ball.
...
where ν k is the sample from the gaussian distribution N (z t , σ 2 ).

Moreover, z t is updated with stochastic gradient descent.

The detailed procedure is presented in Algorithm 1.

We do not need to do projection explicitly since δ already satisfies δ ∞ < ε.
...
Target Network F t ; Input x and its label y or the target class t; Encoder E; Decoder D; Standard deviation σ; Learning rate η; Sample size b; Iterations T ; Bound for adversarial perturbation ε Output: Adversarial perturbation δ 1: z 0 = E(x) 2: for t = 1 to T do 3:
...
The P-RGF D (λ * ) version was compared.
...
We set ε = 0.03125. ) as the source model to improve transferablity (Liu et al., 2016) for both targeted and un-targeted attack.
...
Varying ε: We also changed ε and performed attack on ε = 0.02 and ε = 0.04.

As shown in Appendix A.5, TREMBA still outperforms the other methods despite using the G trained on ε = 0.03125.

We also show the result of TREMBA for commonly used ε = 0.05.
...
Therefore we set ε = 0.05 and perform un-targeted attack on the API, changing the top1 label to whatever is not on top1 before.
...
V represents VGG16; S represents Squeezenet; G represents Googlenet; R represents Resnet18 A.5 VARYING ε We chose ε = 0.02 and ε = 0.04 and performed targeted attack on ImageNet.

Although TREMBA used the same model that is trained on ε = 0.03125, it still outperformed other methods, which shows that TREMBA can also generalize to different strength of adversarial attack with different ε.

For the commonly used ε = 0.05, TREMBA also performs well.
...
The targeted attack is 0 and ε = 0.03125.
...
All large κ will work well for our method and we chose κ = 200.0.
