...
λ is a hyperparameter that weighs the relevance of the most salient parts of the video segment to the corresponding frame-specific sentence representations.
...
On the DiDeMo dataset, we adopt the mean Intersection-Over-Union (IOU) and Recall@N at IOU threshold = θ.

Recall@N represents the percentage of the test sliding window samples which have a overlap of at least θ with the ground-truth segments.
