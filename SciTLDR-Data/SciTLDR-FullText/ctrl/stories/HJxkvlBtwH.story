...
The assumption is that the attacker can add noise δ to the original signal s so to obtain a perturbed signal s = s + δ.

The measure of signal distortion are decibels (dB) defined as:

Note that the quieter the noise is, the smaller the decibel of perturbation dB s (δ) (it is usually a negative value as it is quieter than the signal).

We assume the attacker can generate noise δ such that dB s (δ) < where is a constant defined by the threat model.

Our goal is to verify whether the neural network classifies s correctly for each small perturbation δ (as constrained above).
...
Filter Bank Log Energy: Ψ = log(ΘΛ) Apply the Mel frequency filter bank to the power spectrum and get the log value of them.

Λ ∈ R N/2×p is the filter bank given the number of filters p, and log is applied entry-wise.

4.

DCT(Discrete Cosine Transformation): X = ΨD Perform DCT on the previous result.
...
We decompose it into affine and square operations which transform signal s (t) into an intermediate representation θ (t) .

Using our novel and optimal abstract transformer for the square operation, formally defined in Section 4 and visualized in Fig. 3b , we obtain linear bounds on θ (t) .
...
To obtain bounds on the output of the logarithm we apply our novel and optimal logarithm transformer, also formally described in Section 4 and visualized in Fig. 3a , and obtain linear upper and lower bounds on the log energy of the filtered power spectrum, ψ (t) .
...
We now apply our σ(x) · tanh(y) and σ(x) · y transformers to get bounds of the cell state c
...
Sigmoid Tanh abstract transformer We define elementwise multiplication between sigmoid and tanh as an assignment z := σ(x) tanh(y), where x, y, z ∈ X .
...
∂y , and Φ, a, r are chosen from Table 1 .
...
We note that Sigmoid Identity transformer is handled in the same manner (we can directly apply the same transformer by replacing f (x, y) with σ(x) · y).
...
Unless z u passes through the point (u x , u y ), the concrete upper bound would be larger than σ(u x ) · tanh(u y ), making it worse than an interval bound(analogously for the lower bound).
...
One of the key differences with our work is the approximation of σ(x) · tanh(y) using linear bounds.
...
The first step is an affine transform and in our example we use:ψ

2 .

Our abstraction is again exact here and additionally computes interval boundsψ

2 ∈ [9, 57].

The final step in this transform is the elementwise logarithm of the input.

We denote the operation as ψ

2 ).

As this is again a non-linear operation, we apply our new log transformer so to obtain: 0.0783ψ
...
The left and right summands are computed using our new binary abstract transformers for σ(x) · y and σ(x) · tanh(y) expressions, respectively.
...
We apply our abstract transformer for the σ(x) · tanh(y) expression, we obtain: 2 .
...
We now apply the same back-substitution technique as Singh et al. (2019a) : For the purpose of demonstration, we stop here and plug in previously obtained interval bounds for ψ (t) .
...
We sample those inputs uniformly from [−2, 2] × [−2, 2] and compared the volume between the curve σ(x) · tanh(y) and bounding planes produced by both DAC and POPQORN.

The volume was computed using 1000 Monde Carlo samples in the set

Since there are two σ(x) · tanh(y) and one σ(x) · y calculation appearing in a single LSTM cell, we run the experiment with such portion of data.

In other words, in 67% of experiments we bound σ(x) · tanh(y), and in 33% we bound σ(x) ·
...
We uniformly sampled the arguments l x , u x , l y , u y that our transformer for σ(x) · tanh(y) and σ(x) · y was invoked with.
...
On the contrary, the resulting value of σ(x) · tanh(y) is within [−1, 1] regardless of the magnitude of arguments which means that the error produced by intervals is bounded.
...
Let z LB ( ) and z U B ( ) be the resulting lower and upper bounds for the final logit z ∈ R d under the perturbation size , with the true label j ∈ {0, · · · , d − 1}. We define the worst-case logitsẑ( ) aŝ

( ) i = j which corresponds to the worst concrete logits under the given input with the predefined perturbation amount.

Recall that we say the input is certified when this worst-case logits satisfy j = arg max iẑi .

Training loss L is a linear combination of the standard cross-entropy loss l(z, e (j) ) and the worst-case cross-entropy loss l(ẑ, e (j) ) where e (j) is target one-hot vector, i.e., L(t ) = κ(t )l(z, e (j) ) + (1 − κ(t ))l(ẑ( (t )), e (j) )

Note that we set up the κ and as the function of t .

As in , gradual increase/decrease of these parameters during the training was essential to get desired performance.

We set these functions with respect to the number of training epochs E as κ(t) = 1 − t/E t < E/2 1/2 otherwise (t) = −70 − (200 − 70) 1−t/E .
