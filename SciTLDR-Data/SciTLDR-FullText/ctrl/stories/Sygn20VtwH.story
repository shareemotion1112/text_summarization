...
Our model is distinctly characterized by recursive parameterization of recurrent gates, i.e., compositional flow is controlled by instances of itself,á la repeatedly and recursively.
...
where σ r is a nonlinear activation such as tanh.
...
To learn α t , β t , we use the following:

where F * (xt) = W x t + b is a simple linear transformation layer applied to sequence X across the temporal dimension.

Intuitively, α, β control the extent of recursion, enabling a soft depth pertaining to the hierarchical parameterization.
...
Finally, we note that it is also possible to assign α ∈ R, β ∈ R to be trainable scalar parameters.
...
These values reflect the α and β values in METAGROSS, signifying how the parameter tree is being constructed during training.
