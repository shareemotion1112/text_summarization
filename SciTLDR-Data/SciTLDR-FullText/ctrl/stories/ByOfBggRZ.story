...
, L. Let φ (·) be the activation function (nonlinearity), and let w y ∈ R p L and b y ∈ R be the coefficients and bias for the final output.
...
[p], we propose to use an average of the relevant feature weights w I as the surrogate for the interaction strength: µ (|w I |), where µ (·) is the averaging function for an interaction that represents the interaction strength due to feature weights.

We provide guidance on how µ should be defined by first considering representative averaging functions from the generalized mean family: maximum value, root mean square, arithmetic mean, geometric mean, harmonic mean, and minimum value BID5 .
...
Let the activation function φ (·) be a 1-Lipschitz function.

Then the output y is z DISPLAYFORM1

We now combine our definitions from Sections 3.1 and 3.2 to obtain the interaction strength ω i (I) of a potential interaction I at the i-th unit in the first hidden layer h DISPLAYFORM0 .(Note that ω i (I) is defined on a single hidden unit, and it is agnostic to scaling ambiguity within a ReLU based neural network.
...
Let R be the set of interactions proposed by Algorithm 1 with µ (·) = min (·), let I ∈ R be a d-way interaction where d ≥ 3, and let S be the set of subset (d − 1)-way interactions of I where |S| = d. Assume that for any hidden unit j which proposed s ∈ S ∩ R, I will also be proposed at the same hidden unit, and DISPLAYFORM2 .
...
By comparing the test performance of MLP-Cutoff and MLP-M with respect to MLP-Cutoff without any interactions (MLP-Cutoff Ø ), we can compute the relative test performance improvement obtained by including detected interactions.
...
The results of this study show DISPLAYFORM2 Figure 6: MLP-Cutoff error with added top-ranked interactions (along x-axis) of F 1 -F 10 TAB2 , where the interaction rankings were generated by the NID framework applied to MLP-M. Red crossmarks indicate ground truth interactions, and Ø denotes MLP-Cutoff without any interactions.

Subset interactions become redundant when their true superset interactions are found.

TAB2 , where the interaction rankings were generated by the NID framework on MLP-M. Ø denotes MLP-Cutoff without any interactions.that a relatively small number of interactions of variable order are highly predictive of their corresponding datasets, as true interactions should.
...
We can provide a finer interaction strength analysis on a bivariate ReLU function: max{α 1 x 1 + α 2 x 2 , 0}, where x 1 , x 2 are two variables and α 1 , α 2 are the weights for this simple network.
...
, β 5 = argmin βi,i=0,...,5 DISPLAYFORM0 Then for the coefficient of interaction {x 1 , x 2 }, β 5 , we have that, DISPLAYFORM1 Note that the choice of the region (−1, 1) × (−1, 1) is arbitrary: for larger region (−c, c) × (−c, c) with c > 1, we found that |β 5 | scales with c −1 .

By the results of Proposition B, the strength of the interaction can be well-modeled by the minimum value between |α 1 | and |α 2 |.

Note that the factor before min{|α 1 |, |α 2 |} in Equation FORMULA13 Proof.

For non-differentiable φ (·) such as the ReLU function, we can replace it with a series of differentiable 1-Lipschitz functions that converges to φ (·) in the limit.

Therefore, without loss of generality, we assume that φ (·) is differentiable with |∂ x φ(x)| ≤ 1.

We can take the partial derivative of the final output with respect to h ( ) i , the i-th unit at the -th hidden layer: DISPLAYFORM2 We can conclude the Lemma by proving the following inequality: DISPLAYFORM3 The left-hand side can be re-written as DISPLAYFORM4 The right-hand side can be re-written as DISPLAYFORM5 We can conclude by noting that |∂ x φ(x)| ≤ 1.

Theorem 4 (Improving the ranking of higher-order interactions).

Let R be the set of interactions proposed by Algorithm 1 with µ (·) = min (·), let I ∈ R be a d-way interaction where d ≥ 3, and let S be the set of subset (d − 1)-way interactions of I where |S| = d. Assume that for any hidden unit j which proposed s ∈ S ∩ R, I will also be proposed at the same hidden unit, and DISPLAYFORM0 .

Then, one of the following must be true: a) ∃s ∈ S ∩ R ranked lower than I, i.e., ω(I) > ω(s), or b) ∃s ∈ S where s / ∈ R.Proof.

Suppose for the purpose of contradiction that S ⊆ R and ∀s ∈ S, ω(s) ≥ ω(I).
...
Specifically, we generate a dataset of n samples and p features { X (i) , y (i) } using the function DISPLAYFORM2 where X (i) ∈ R p is the i th instance of the design matrix X ∈ R p×n , y (i) ∈ R is the i th instance of the response variable y ∈ R n×1 , W ∈ R p×p contains the weights of pairwise interactions, β ∈ R p contains the weights of main effects, (i) is noise, and i = 1, . . .
...
Both a k and β are sparse vectors of 2% nonzero density and are normally distributed with mean 0 and variance 1.
