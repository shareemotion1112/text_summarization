...
Batch normalization (BN) was originally proposed to reduce internal covariate shift by normalizing the feature: DISPLAYFORM0 + β, where y is the input to be normalized and y is the output, γ, β are learnable scale and bias, µ, σ 2 are mean and variance of current mini-batch during training.
...
TAB2 enumerates the number and percentage of parameters in batch normalization layers (after training, µ, σ, γ, β are merged into two parameters).
...
Execute sub-network at current width,ŷ = M (x).

Compute loss, loss = criterion(ŷ, y).
...
In our work, private parameters γ, β, µ, σ 2 of BN are introduced in Switchable Batch Normalization for each sub-network to independently normalize feature y = γ y−µ √ σ 2 + + β, where y is input and y is output, γ, β are learnable scale and bias, µ, σ 2 are moving averaged statistics for testing.

In switchable batch normalization, the private γ, β come for free because after training, they can be merged as y = γ y + β , γ = γ √ σ 2 + , β = β − γ µ. Nevertheless, we present ablation study on how these conditional parameters affect overall performance.
