...
These b-functions are the partial derivative terms [∇ a C, σ (z L ), w l+1 , δ l+1 , σ (z l )] in the backpropagation equation as illustrated in equations 1 and 2.
...
Mathematically, this is equivalent to multiplying equation 1 with DISPLAYFORM1 when σ (z l ) = 0, this is illustrated in equations 3.

DISPLAYFORM2 Within the context of stochastic gradient optimization, replacing the activation function derivative σ (z l ) with the b-function g(z l ) could be interpreted as having a learning rate DISPLAYFORM3 when σ (z l ) = 0 as described in equation 4.

DISPLAYFORM4 β(z l ) is parameterized by z l which infers that the learning rate β(z l ) is an adaptive learning rate with respect to the pre-activations z l .

Similarly, as δ l propagates backwards into δ (l−1) there will be noise = DISPLAYFORM5 propagated backwards into earlier layers.

However, in many cases gradient noise can be beneficial for gradient optimization as described by BID7 .When σ (z l ) = 0, this replaces the zero gradient with a custom credit value.

It's ideal to have g(z l ) = 0 when σ (z l ) = 0 else this can be harmful to the gradient optimization step.
...
A target network f (θ) which is the subject of the experiment, this could be a convolution neural network.

A function approximator which we call the meta-network.

The meta-network g(ω) is a neural network with weights ω and its role is to approximate a b-function.

An optimization component to learn b-functions for the target network f (θ).

Hence, learning b-functions in the backpropagation pass consists of learning weights ω such that J(θ), the cost function for f (θ), is minimized in training.

We formulate the black-box optimization problem as follows: the weights ω of the metanetwork g(ω) serves as the input to the black-box.

The area under the loss curve (AULC) when training the target network f (θ) serves as the evaluation of the black-box function.
