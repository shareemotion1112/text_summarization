...
The adversarial examplex can be written asx = x + δ where δ is the added perturbation.

In our work we constrain δ to be within domain [−∆, ∆] as considered for instance in Google Brain-Kaggle competition [16] , ∆ being the maximum perturbation.
...
With C&W attacks for instance, the loss function includes a norm term δ to minimize perturbation δ but the clipping is still needed as we saw above.
...
α i , β i ] to be valid, α i and β i are respectively the minimum and maximum values that can be taken by the i To find the center of domain definition ofx, three cases are to be considered actually, not four since ∆ i is much smaller than (β i − α i ), as can be seen on FIG2 .
