...
where m and Σ denoting the n-dimensional mean vector and symmetric covariance matrix with rank n, respectively.
...
where λ stands for both the variance and the mean of the distribution.
...
Also, even though the sensitivity of PSNR to Gaussian noise is very high, the metric is unable to present similar performance for different types of perturbation (Avcibas et al., 2002 ) (Horé & Ziou, 2010) .
...
The Structural Similarity between two non-negative image signals x and y, whose means, standard deviations and covariance are denoted by µ x , µ y , σ x , σ y and σ xy respectively, can be expressed as:
