...
We call this feature representation φ(x), where x is the input signal.
...
We can find local maxima within our neural net framework by applying a max pooling function over the feature representation φ(x).

We then find the places where the output of the maxpool equals the original feature representation (i.e., the locations where φ(x) = maxpool (φ(x))).
...
We will refer to this binary fingerprint as ψ (x) where x is the input signal.
...
Once we have such a loss, we can use standard gradient methods to find a perturbation δ that can be added to an audio signal to prevent copyright detection.

To ensure the similarity between perturbed and clean audio, we bound the perturbation δ.

That is, we enforce δ p ≤ .
...
Since the fingerprinting model outputs a binary fingerprint ψ(x), we can simply measure the number of local maxima that the signals x and y share by |ψ(x)·ψ(y)|.
...
We do this by moving the locations of local maxima in φ(x) outside of any neighborhood of the local maxima of φ(y).
...
where α is a smoothing hyper parameter.

As α → ∞, the smoothed max function more accurately approximates the exact max function.

For simplicity, we chose α = 1 for all experiments.
