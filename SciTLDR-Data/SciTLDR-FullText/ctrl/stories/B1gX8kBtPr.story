...
To demonstrate this point, consider the function f : R → R (Figure 2b ) mapping all x ≤ 0 to 1, all x ≥ 1 to 0 and all 0 < x < 1 to 1 − x and two ReLU networks n 1 (Figure 2a ) and n 2 (Figure 2c ) perfectly approximating f , that is n 1 (x) = f (x) = n 2 (x) for all x. For δ = 1 4 , the interval certification that n 1 maps all
...
A fundamentally different approach is randomized smoothing (Li et al., 2019; Lécuyer et al., 2019; Cohen et al., 2019b) , in which probabilistic classification and certification with high confidence is performed.
...
The l p ball around an image is said to be the adversarial ball, and a network is said to be -robust around x if (Figure 3a ) using a ReLU network n = ξ 0 + k n k .
...
The first step in the construction is to deconstruct the function f into slices {f k : Γ → [0,

for all x, where ξ 0 is the minimum of f (Γ).

We approximate each slice f k by a ReLU network δ 2 · n k .

The network n approximating f up to δ will be n(x) := ξ 0 + δ 2 k n k (x).

The construction relies on 2 key insights, (i) the output of δ 2 · n k can be confined to the interval [0, δ 2 ], thus the loss of analysis precision is at most the height of the slice, and (ii) we can construct the networks n k using local bump functions, such that only 4 slices can contribute to the loss of analysis precision, two for the lower interval bound, two for the upper one.
...
Let Γ ⊂ R m be a closed m-dimensional box and let f : Γ → R be continuous.

The N -slicing of f is a set of functions {f k } 0≤k<N defined by

where

To construct a ReLU network satisfying the desired approximation property (Equation (1)) if evaluated on boxes in B(Γ), we need the ReLU network nmin capturing the behavior of min as a building block (similar to He et al. (2018) ).
...
Now, we use the clipping function R [ * ,1] := 1 − R(1 − x) clipping every value exceeding 1 back to 1 (Figure 5 ) to construct the local bumps φ c w.r.t.
...
We define a ReLU neural network φ c :
...
The local bump φ c (x) evaluates to 1 for all x that lie within the convex hull of c, namely conv(c), after which φ c (x) quickly decreases linearly to 0.

φ c has 1 + 2(2d − 1) + 2d ReLUs and 1 + log 2 (2d + 1) + 1 layers.
...
Let Γ ⊂ R m be a closed box and let f : Γ → R be continuous.

For all δ > 0 there exists a set of ReLU networks {n k } 0≤k<N of size N ∈ N approximating the N -slicing of f , {f k } 0≤k<N (ξ k as in Definition 4.1) such that for all boxes B ∈ B(Γ)
...
So we can pick a M ∈ N such that for all x, y ∈ Γ satisfying ||y−x||

Next, we construct for every slice k a set ∆ k of hyperrectangles on the grid G: if a box B ∈ B(Γ) fulfills f (B) ≥ ξ k+1 + δ 2 , then we add a minimal enclosing hyperrectangle c ⊂ G such that B ⊆ conv(c) to ∆ k , where conv(c) denotes the convex hull of c. This implies, using uniform continuity of f and that the grid G is fine enough, that f (conv(c)) ≥ ξ k+1 .
...
where φ c is as in Definition 4.2.

The n k are depicted in Figure 3c .

Now, we see that Equation (2) holds by construction: For all boxes B ∈ B(Γ) such that f ≥ ξ k+1 + δ 2 on B exists c ∈ ∆ k such that B ⊆ conv(c ) which implies, using Lemma 4.3, that φ c (B) = [1, 1], hence

Similarly, if f (B) ≤ ξ k − δ 2 holds, then it holds for all c ∈ ∆ k that B does not intersect N (conv(c)).

Indeed, if a c ∈ ∆ k would violate this, then by construction, f (conv(c)) ≥ ξ k+1 , contradicting f (B) ≤ ξ k −

where l := min f (B) and u := max f (B).

Proof.

Pick N such that the height of each slice is exactly δ 2 , if this is impossible choose a slightly smaller δ.

Let {n k } 0≤k<N be a series of networks as in Lemma 4.4.

Recall that ξ 0 = min f (Γ).
...
where we used in the last step, that ξ 0 + k δ 2 = ξ k .
...
Similarly, from Equation (7) we know, that there is x ∈ B such that f (x) ≥ ξ q = ξ q−1 +

We know further, that if p + 3 ≤ q, than there is an x ∈ B such that f (x) ≥ ξ p+3 = ξ p+2 +
...
Let Γ ⊂ R m be a compact m-dimensional box and let f : Γ → R be continuous.
...
Pick x ∈ Γ and let l ∈ {0, . . .

, N − 1} such that ξ l ≤ f (x) ≤ ξ l+1 .
...
Let N ∈ N such that N ≥ 2 ξmax−ξmin δ where ξ min := min f (Γ) and ξ max := max f (Γ).

For simplicity we assume Γ = [0, 1] m .

Using the Heine-Cantor theorem, we get that f is uniformly continuous, thus there exists a δ > 0 such that ∀x, y ∈ Γ.||y − x|| ∞ < δ ⇒ ||f (y) − f (x)|| <
