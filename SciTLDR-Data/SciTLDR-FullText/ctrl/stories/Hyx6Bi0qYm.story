...
The factor λ that multiplies the AE loss adjusts for the different units and different value ranges of firing rates and EMGs; it equalizes the contributions of the two terms in the loss function so that the learning algorithm does not prioritize one over the other.

The value of λ is updated for each new training iteration; it is computed as the ratio λ = L y L x of the respective losses at the end of the preceding iteration.
...
Both Z 0 and Z k are matrices of dimension l by 8τ , where l is the dimensionality of the latent space and τ is the average time duration of each trial; the factor of 8 arises from concatenating the averaged latent activities for each of the eight targets.
...
The discriminator parameters θ D are initialized with the weights of the BMI AE, trained on the day-0 neural data.
...
The aligner parameters θ A , the weights of the n by n connectivity matrices from input to hidden and from hidden to output, are initialized as the corresponding identity matrices.
...
Given input data X, the discriminator outputsX =X(X, θ D ), with residuals R(X, θ D ) = X −X(X, θ D ) .

Consider the scalar reconstruction losses r obtained by taking the L 1 norm of each column of R. Let ρ 0 and ρ k be the distributions of the scalar losses for day-0 and day-k, respectively, and let µ 0 and µ k be their corresponding means.

We measure the dissimilarity between these two distributions by a lower bound to the Wasserstein distance BID2 , provided by the absolute value of the difference between the means: W (ρ 0 , ρ k ) ≥ |µ 0 − µ k | BID5 .
