...
∀i ∈ S k , y i is the embedding for pixel position i and µ k is the mean embedding of instance k:

Where ρ a denotes the attraction radius within a cluster: we want the embedding to be within ρ a of the centroid.

2ρ r denotes the repulsion radius: we want the centroids of two different clusters to be at least 2ρ r apart.

Therefore, if we set ρ r > 2ρ a , a given pixel embedding of a cluster will be closer to all the pixel embeddings of its cluster than any other pixel embedding.
...
In the ideal case, with a test loss of zero, this will result in perfect clustering if the repulsion radius, ρ r , is twice as large as the attraction radius, ρ a .
...
withÎ s→t the synthesised view of I t from source image I s using the predicted depthD t and the predicted 4 × 4 camera transformationT t→s predicted from the separate pose network.
...
Given the camera intrinsic matrix, K, and the mapping ϕ from image plane to camera coordinate, the corresponding pixel in the source image is provided by:
...
A distance lower than ρ r indicates a match.
...
In the loss function, we set the attraction radius ρ a = 0.5 and repulsion radius ρ r = 1.5.

We weight the losses with attraction and repulsion loss weight λ 1 = λ 2 = 1.0, regularisation loss λ 3 = 0.001 and depth loss λ 4 = 1.0.
...
We evaluate the effect of this noise by clustering with the ground-truth mean for each instance, by thresholding with ρ r around the ground truth instance embedding mean.
