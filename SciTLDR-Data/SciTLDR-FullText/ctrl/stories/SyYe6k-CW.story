...
Other algorithms, like Variational Inference, Black Box α-divergence, and minibatch Markov Chain Monte Carlo approaches, strongly couple their complex representation and uncertainty estimates.
...
We consider distributions π over the space of parameters that completely define a problem instance θ ∈ Θ. For example, θ could encode the reward distributions of a set of arms in the multi-armed bandit scenario, or -more generally-all the parameters of an MDP in reinforcement learning.
...
We consider exact linear posteriors as a baseline; i.e., these formulas compute the posterior when the data was generated according to Y = X T β + where ∼ N (0, σ 2 ), and Y represents the reward.

Importantly, we model the joint distribution of β and σ 2 for each action.

Sequentially estimating the noise level σ 2 for each action allows the algorithm to adaptively improve its understanding of the volume of the hyperellipsoid of plausible β's; in general, this leads to a more aggressive initial exploration phase (in both β and σ 2 ).The posterior at time t for action i, after observing X, Y , is DISPLAYFORM0 , where we assume σ 2 ∼ IG(a t , b t ), and β | σ 2 ∼ N (µ t , σ 2 Σ t ), an Inverse Gamma and Gaussian distribution, respectively.

Their parameters are given by DISPLAYFORM1 DISPLAYFORM2 We set the prior hyperparameters to µ 0 = 0, and Λ 0 = λ Id, while a 0 = b 0 = η > 1.

It follows that initially, for σ We consider two approximations to (1) motivated by function approximators where d is large.

While posterior distributions or confidence ellipsoids should capture dependencies across parameters as shown above (say, a dense Σ t ), in practice, computing the correlations across all pairs of parameters is too expensive, and diagonal covariance approximations are common.
...
Accordingly, we study two linear approximations where Σ t is diagonal.
...
In FIG1 , for d = 2, we see the posterior distribution β t ∼ N (µ t , Σ t ) of a linear model based on (1), in green, together with two diagonal approximations.

Each approximation tries to minimize a different objective.

In blue, the PrecisionDiag posterior approximation finds the diagonalΣ ∈ R d×d minimizing KL(N (µ t ,Σ) || N (µ t , Σ t )), like in mean-field variational inference.

In particular,Σ = Diag(Σ −1 t ) −1 .

On the other hand, in orange, the Diag posterior approximation finds the diagonal matrixΣ minimizing KL(N (µ t , Σ t ) || N (µ t ,Σ)) instead.

In this case, the solution is simplyΣ = Diag(Σ t ).We add linear baselines that do not model the uncertainty in the action noise σ 2 .
...
In particular, in this work, we implement the black-box α-divergence minimization algorithm BID19 , where local parameter sharing is applied to the Power EP energy function.

Note that different values of α ∈ R\{0} correspond to common algorithms: α = 1 to EP, and α → 0 to Variational Bayes.

The optimal α value is problem-dependent BID19 .Dropout Dropout is a training technique where the output of each neuron is independently zeroed out with probability p at each forward pass (Srivastava et al., 2014) .
...
The training updates for the network are unchanged, but when selecting actions, the network weights θ are perturbed with isotropic Gaussian noise.
...
Specifically, the covariance function K(·) of the GP is given by: DISPLAYFORM2 and the task kernel between tasks t and l are DISPLAYFORM3 2 ) where v l indexes the latent vector for task l and r λ (x,x) = |(x λ) − (x λ)|.

The length-scales, λ m and λ l , and amplitude parameters α, β are optimized via the log marginal likelihood.
...
In a static scenario, e.g. supervised learning, we are given a model family Θ (like the set of linear models, trees, or neural networks with specific dimensions), a prior distribution π 0 over Θ, and some observed data D that -importantly-is assumed i.i.d.

Our goal is to return an approximate posterior distribution: DISPLAYFORM0 We define the quality of our approximation by means of some distance d(π, π).On the other hand, in dynamic settings, our estimate at time t, sayπ t , will be used via some mechanism M, in this case Thompson sampling, to collect the next data-point, which is then appended to D t .

In this case, the data-points in D t are no longer independent.

D t will now determine two distributions: the posterior given the data that was actually observed, π t+1 = P(θ | D t ), and our new estimateπ t+1 .

When the goal is to make good sequential decisions in terms of cumulative regret, the distance d(π t , π t ) is in general no longer a definitive proxy for performance.

For instance, a poorly-approximated decision boundary could lead an algorithm, based onπ, to get stuck repeatedly selecting a single sub-optimal action a. After collecting lots of data for that action,π t and π t could start to agree (to their capacity) on the models that explain what was observed for a, while both would stick to something close to the prior regarding the other actions.

At that point, d(π t , π t ) may show relatively little disagreement, but the regret would already be terrible.

Let π * t be the posterior distribution P(θ | D t ) under Thompson Sampling's assumption, that is, data was always collected according to π * j for j < t. We follow the idea thatπ t being close to π * t for all t leads to strong performance.
...
For a given context X ∼ N (µ, Σ), the reward obtained by pulling arm i follows a linear model r i,X = X T β i + with ∼ N (0, σ DISPLAYFORM1 can be exactly computed using the standard Bayesian linear regression formulas presented in Section 3.

We set the contextual dimension d = 20, and the prior to be β ∼ N (0, λ I d ), for λ > 0.In FIG2 , we show the posterior distribution for two dimensions of β i for each arm i after n = 500 pulls.
...
In FIG1 , we see the posterior distribution β ∼ N (µ, Σ) of a linear model in green, together with the two diagonal linear approximations introduced in Section 3: the Diag (in orange) and the PrecisionDiag (in blue) approximations, respectively.

We now assume there are k linear arms, β i ∈ R d for i = 1, . . .
...
Some algorithms truly benefit from hyper-parameter DISPLAYFORM0 Figure 3: Wheel bandits for increasing values of δ ∈ (0, 1).
...
Set d = 2, and δ ∈ (0, 1), the exploration parameter.

Contexts are sampled uniformly at random in the unit circle in R 2 , X ∼ U (D).

There are k = 5 possible actions.

The first action a 1 always offers reward r ∼ N (µ 1 , σ 2 ), independently of the context.

On the other hand, for contexts such that X ≤ δ, i.e. inside the blue circle in Figure 3 , the other four actions are equally distributed and sub-optimal, with r ∼ N (µ 2 , σ 2 ) for µ 2 < µ 1 .

When X > δ, we are outside the blue circle, and only one of the actions a 2 , . . .
...
Non-optimal actions still deliver r ∼ N (µ 2 , σ 2 ) in this region, except a 1 whose mean reward is always µ 1 , while the optimal action provides r ∼ N (µ 3 , σ 2 ), with µ 3 µ 1 .

We set µ 1 = 1.2, µ 2 = 1.0, µ 3 = 50.0, and σ = 0.01.

Note that the probability of a context randomly falling in the high-reward region is 1 − δ 2 (not blue).

The difficulty of the problem increases with δ, and we expect algorithms to get stuck repeatedly selecting action a 1 for large δ.
...
The performance of Black Box α-divergence algorithms was poor.

Because this class of algorithms is similar to BBB (in fact, as α → 0, it converges to the BBB objective), we suspect that partial convergence was also the cause of their poor performance.
...
Given the successes of Black Box α-divergence in other domains BID19 , investigating approaches to sidestep the slow convergence of the uncertainty estimates is a promising direction for future work.
...
Greedy NN approach, fixed learning rate (γ = 0.01).
...
SGFS Burning = 500, learning rate γ = 0.014, EMA decay = 0.9, noise σ = 0.75.

Takes each action at random with equal probability.

BayesByBackprop with noise σ = 0.5. (ts = 100, first 100 times linear decay from ts = 10000).

BayesByBackprop with noise σ = 0.75. (ts = 100, first 100 times linear decay from ts = 10000).

BayesByBackprop with noise σ = 1.0. (ts = 100, first 100 times linear decay from ts = 10000).
...
ConstSGD Burning = 500, EMA decay = 0.9, noise σ = 0.5.

Initial noise σ = 0.01, and level = 0.01.
...
Normalized with respect to the elapsed time required by RMS (which uses t s = 100 and t f = 20).Published as a conference paper at ICLR 2018 Table 9 : Cumulative regret incurred on the Wheel Bandit problem with increasing values of δ.
