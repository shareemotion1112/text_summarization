...
Let us denote this constant by π ks ; by definition, s∈K π ks = 1 for all k ∈ K, and we call π (a) ∈ [0, 1]
...
We use π to denote the collection of confusion matrices {π (a) } a∈ [m] .We assume nr workers w 1,1 , w 1,2 , · · · , w n,r are selected uniformly at random from a pool of m workers with replacement and a batch of r workers are assigned to each of the examples X 1 , X 2 , · · · , X n .
...
Having access to only noisy labels Z (r) by workers w (r) , we compute f as the one which minimizes a suitably modified loss function π, q (f (X), Z (r) , w (r) ).

Where π denote an estimate of confusion matrix π, and q an estimate of q, the prior distribution on Y .

We define π, q in the following section.

Assume that there exists a function f DISPLAYFORM0 .

Under the Dawid-Skene model (described in previous section), the joint likelihood of true labeling function f * (X i ) and observed labels {Z ij } i∈[n],j∈ [r] as a function of confusion matrices of workers π can be written as DISPLAYFORM1 q k 's are the marginal distribution of the true labels Y i '

s. We estimate the worker confusion matrices π and the true labeling function f * by maximizing the likelihood function L(π; f * (X), Z).

Observe that the likelihood function L(π; f * (X), Z) is different than the standard likelihood function of Dawid-Skene model in that we replace each true hidden labels Y i by f * (X i ).

Like the EM algorithm introduced in BID4 , we propose 'Model Bootstrapped EM' (MBEM) to estimate confusion matrices π and the true labeling function f * .
...
In the following two subsections, we motivate and explain our iterative algorithm to estimate the true labeling function f * given a good estimate of worker confusion matrices π and vice-versa.

To begin, we ask, what is the optimal approach to learn the predictor function f when for each worker we have π, a good estimation of the true confusion matrix π, and q, an estimate of the prior?
...
Consider a worker with confusion matrix π, where π y > 1/2 and π −y > 1/2 represent her probability of correctly labeling the examples belonging to class y and −y respectively.

Then their weights are π −y /(π y + π −y − 1) for class y and −(1 − π y )/(π y + π −y − 1) for class −y.

It is evident that their weights become unstably large when the probabilities of correct classification π y and π −y are close to 1/2, limiting the method's usefulness in practice.

As explained below, for the same scenario, our weights would be π y /(1 + π y − π −y ) for class y and (1 − π −y )/(1 + π y − π −y ) for class −y.
...
In particular, we define π, q to be DISPLAYFORM0 If the observed label is uniformly random, then all weights are equal and the loss is identical for all predictor functions f .

Absent noise, we recover the original loss function.

Under the Dawid-Skene model, given the observed noisy labels Z (r) , an estimate of confusion matrices π, and an estimate of prior q, the posterior distribution of the true labels can be computed as follows: DISPLAYFORM1 where I[.] is the indicator function which takes value one if the identity inside it is true, otherwise zero.
...
Given π, q , we learn the predictor function f by minimizing the empirical risk DISPLAYFORM2

The next question is: how do we get a good estimate π of the true confusion matrix π for each worker.
...
For the a-th worker, πks for k, s ∈ K, and q k for k ∈ K, we have, DISPLAYFORM0 The estimate is effective when the hypothesis class F is expressive enough and the learner is robust to noise.
...
MBEM takes data, noisy labels, and the corresponding worker IDs, and returns the best predictor function f in the hypothesis class F.In the first round, we compute the weights of the modified loss function π, q by using the weighted majority vote.

Then we obtain an estimate of the worker confusion matrices π using the maximum likelihood estimator by taking the model predictions as the ground truth labels.
...
, T : number of iterations Output: f : predictor function Initialize posterior distribution using weighted majority vote DISPLAYFORM0 estimate confusion matrices π and prior class distribution q given {t i } i∈[n] π (a) ← Equation FORMULA14 , for a ∈ [m]; q ← Equation (7) estimate label posterior distribution given π, q DISPLAYFORM1
...
We give guarantees on the excess risk of the predictor function f , and accuracy of π estimated in the second round.

For the purpose of analysis, we assume that fresh samples are used in each round for computing function f and estimating π.

In other words, we assume that f and π are each computed using n/4 fresh samples in the first two rounds.

We define α and β to capture the average worker quality.

Here, we give their concise bound for a special case when all the workers are identical, and their confusion matrix is represented by a single parameter, 0 ≤ ρ < 1/2.

Where π kk = 1 − ρ, and π ks = ρ for k = s.

Each worker makes a mistake with probability ρ.

β ≤ (ρ + ) r r u=0 r u (τ u + τ r−u ) −1 , where τ := (ρ + )/(1 − ρ − ).

α for this special case is ρ.

A general definition of α and β for any confusion matrices π is provided in the Appendix.
...
For any hypothesis class F with a finite VC dimension V , and any δ < 1, there exists a universal constant C such that if N is large enough and satisfies DISPLAYFORM0 then for binary classification with 0-1 loss function , f and π returned by Algorithm 1 after T = 2 iterations satisfies DISPLAYFORM1 and DISPLAYFORM2 , with probability at least 1 − δ.

Where := 2 4 γ + 2 8 m log(2 6 mδ)/N , and γ := min f ∈F R ,D (f ) + C( √ V + log(1/δ))/((1 − 2α) N/r).

1 is defined to be with α in it replaced by β .The price we pay in generalization error bound on f is (1 − 2β ).

Note that, when n is large, goes to zero, and β ≤ 2ρ(1 − ρ), for r = 1.If min f ∈F R ,D (f ) is sufficiently small, VC dimension is finite, and ρ is bounded away from 1/2 then for n = O(m log(m)/r), we get 1 to be sufficiently small.
...
Hence, for N large enough, using Equation FORMULA19 and the bound on β , we get that for fixed total annotation budget, the optimal choice of redundancy r is 1 when the worker quality (1 − ρ) is above a threshold.

In particular, if (1 − ρ) ≥ 0.825 then label once is the optimal strategy.
...
BID4 • weighted-EM: Model learned using weighted loss function with weights set by standard EM.• oracle weighted EM: This model is learned by minimizing π , using the true confusion matrices.• oracle correctly labeled: This baseline is trained using the standard loss function but only using those training examples for which at least one of the r workers has given the true label.
...
A worker is a hammer for any class k ∈ K with probability γ.

We sample m confusion matrices {π (a) } a∈ [m] according to the given worker skill distribution for a given γ.
...
In the first plot, we fix redundancy r = 1, and plot generalization error of the model for varying hammer probability γ.
...
We fixed hammer probability γ = 0.2.
...
Under the assumptions of Theorem 4.1, ∞ error in estimated confusion matrices π as computed in Equation (7), using n samples and a predictor function f with risk R ,D ≤ δ, is bounded by DISPLAYFORM0 with probability at least 1 − δ 1 .First we apply Lemma A.1 with P π computed using majority vote.

We get a bound on the risk of function f computed in the first round.

With this f , we apply Lemma A.2.

When n is sufficiently large such that Equation (8) holds, the denominator in Equation FORMULA18 , 1/K − δ − 8 m log(4mK 2 /δ 1 )/(nr) ≥ 1/8.

Therefore, in the first round, the error in confusion matrix estimation is bounded by , which is defined in the Theorem.

For the second round: we apply Lemma A.1 with P π computed as the posterior distribution (5).Where ∞ error in π is bounded by .

This gives the desired bound in (9).

With this f , we apply Lemma A.2 and obtain ∞ error in π bounded by 1 , which is defined in the Theorem.

For the given probability of error δ in the Theorem, we chose δ 1 in both the lemma to be δ/4 such that with union bound we get the desired probability of δ.

DISPLAYFORM1 For ease of notation, we denote D W,π,r by D π .

Similar to R ,D , risk of decision function f with respect to the modified loss function π is characterized by the following quantities: DISPLAYFORM2 2.

Empirical π -risk on samples: R π ,Dπ (f ) : DISPLAYFORM3 i , wi ).

With the above definitions, we have the following, DISPLAYFORM4 DISPLAYFORM5 where (19) follows from Equation FORMULA5 .

FORMULA5 follows from the fact that f is the minimizer of R π ,Dπ as computed in FORMULA12 .
...
FORMULA5 follows from the fact that for 0-1 loss function (f (X), Y ) + (f (X), −Y ) = 1. (24) follows from the definition of β π defined in Equation (12).

When π is computed using weighted majority vote of the workers then (24) holds with β π replaced by α.

α is defined in (14).Following shows the equality used in Equation FORMULA5 .

Using the notations ρ π and τ π , in the following, for any function f ∈ F, we compute the excess risk due to the unbiasedness of the modified loss function π .

where β π (Y ) is defined in (11).

Where (25) follows from the definition of π given in Equation (4).Observe that when π is computed using weighted majority vote of the workers then Equation FORMULA5 holds with β π (Y ) replaced by α(y).

α(y) is defined in (13).A.2 PROOF OF LEMMA A.2Recall that we have DISPLAYFORM0 DISPLAYFORM1 Note that A, B, C, D, E depend upon a ∈ [m], k, s ∈ K. However, for ease of notations, we have not included the subscripts.
