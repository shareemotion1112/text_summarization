...
= 0, thus the resulting gradient estimator has the same variance as the score function estimator.
...
, but their properties are similar (we present some theoretical results of v θ dp in the Appendix.
...
They obtain the transport equation by an analog to the displacement of particles, while we derive the transport equation for reparameterization gradient by rigorous mathematical deduction.
...
Here we verify that v θ ah (z, θ) satisfy the unbiasedness constraint if h(z, θ) is bounded.

Let θ i be any component of θ i , then
