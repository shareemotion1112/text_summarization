...
Kernelized potential functions (Lafferty et al. (2004) ), gaussian energy functions (Krähenbühl & Koltun (2011) ), or semantic-driven potential functions (Lan et al. (2011) ) are developed in varied cases to adapt to specific tasks.

Deep graph neural networks generalizes the message functions to neural networks.

There are a variety of function types developed such as graph convolution operations (Kipf & Welling (2016) ), attention-based functions (Veličković et al. (2017) ), gated functions (Li et al. (2015) ; Deng et al. (2016) ).
...
In this paper, we take a different tack and instead propose to represent the inference process modeled by a neural network P φ :

The neural network P φ represents the distribution over all possible inference trajectories in the graph model.
...
A set of message functions {f k (·, ·; θ k )}, parameterized by θ k , that determine the possible manner in which messages can be passed over edges in the graph.

3.

An automatic reasoning agent A(·; φ), parameterized by φ, that determines which message function would be appropriate to use along an edge in the graph.
...
Given a graph state s, we can output y using a network f o (s; θ o ).
...
Each of the other functions f k (·, ·) is a non-linear function (e.g. MLP) with its own learnable parameters θ k .
...
where π(z ij t+1 |s t ) is a distribution over z ij t+1 , the message function to use for edge (i, j) on step t + 1 given the current graph state.
...
The parameters φ = (θ A , θ B ) for the reasoning agent are shared; however, each edge maintains its own copy of these agents, via different states h ij t .

Action sampling: Given these distributions π, we can now use them to perform inference by sampling actions from these distributions.

In our implementation, the Gumbel softmax trick is used for differentiable sampling.

By executing a set of actions {z t } sampled from each of these distributions π, our model takes selected nodes and corresponding message functions to pass, and the graph state is updated to s t+1 after one reasoning/passing step.

Over a sequence of time steps, this process corresponds to a reasoning trajectory τ = (z 1 , z 2 , . . .
...
However, rather than fixing a reasoning process, we could instead imagine inferring values y by marginalizing over all possible reasoning sequences τ .

The "prior" p(τ |s) could be either a learnable prior or a hand-designed prior.
...
Here we instead adopt a learnable prior π for p(τ |s), where the reasoning process depends on the graph states.

τ represents the latent reasoning sequence.
...
π * is the true posterior distribution over reasoning sequences given the target inference values y and graph state s, which is unknown.

We adopt the standard variational inference method to derive the following equations: log p(y|s) − KL(q(τ |y, s)||π * (τ |y, s)) = E τ ∼q log p(y|τ , s) − KL(q(τ |s, y)||π(τ |s)) (9)
...
To summarize, we learn the parameters of the networks f o and {f k } which specify p(y|τ , s); and A and B which specify π(τ |s).
...
• Attention-based Graph Neural Network (Veličković et al. (2017) ; Wang et al. (2018); Vaswani et al. (2017) ).
