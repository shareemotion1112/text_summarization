...
, C n be formulas representing constraints over X. Consider a formula of the form DISPLAYFORM0 We say the formula ϕ is satisfiable if there exists some assignment to the variables in X which makes ϕ evaluate to TRUE.

Otherwise, it is unsatisfiable.

Note that if ϕ takes the form of equation FORMULA0 , as it does throughout this paper, every clause C i must be TRUE in order for ϕ to evaluate to TRUE.

To implement the temporal constraints within a schedule, the clauses C i are taken from the theory of difference logic (DL), which makes deciding ϕ a satisfiability modulo theory (SMT) problem.

To check satisfiability of problem instances, we use the Microsoft Z3 SMT solver (De Moura and Bjørner 2008).
