...
The agent creates an embedded state representation, S from its input observation, which is then used to compute as output a policy, π (the "actor"), and a baseline value, B (the "critic").

The π consists of logits over the set of possible actions, from which an action to perform is sampled, and the B is an estimate of the statevalue function at the current state.

During learning, the B is used to compute the temporal-difference error, which is used both to optimize π to generate actions with greater returns than B predicts, and to optimize B to more accurately estimate state values.
...
E is a matrix that compiles the entities produced by the visual front-end; g θ is a multilayer perceptron applied in parallel to each row of the output of an MHDPA step, A, and producing updated entities, E.
...
To perform one step of relational reasoning, our architecture computed pairwise interactions between each entity and all others (including itself), denoted p i,j , and updated each entity by accumulating information about all of its interactions,ẽ i ← {p i,j } j=1:N .
...
We included this non-linear transformation (g θ ) to facilitate the computation of more complex relationships between the entities.
...
The relational module's final output,Ẽ, is then used to compute π and B as follows.

TheẼ matrix, with shape N × f , is reduced to an f -dimensional vector by max-pooling over the entity dimension.

This pooled vector is then passed to a small MLP, which returns an (c + 1)-dimensional vector.

This vector is split into a c-dimensional vector of π's logits (where c is the number of discrete actions), and a scalar baseline value estimate, B. The π logits are normalized using a softmax function, and used as probability parameters for a multinomial distribution, from which an action is randomly sampled.
...
Each agents consisted of 100 actors generating trajectories of experience, and a single learner, which learns π and B using the actors' experiences.

The model updates were performed on GPU using mini-batches of 32 trajectories provided by the actors via a queue.

The agents used an entropy cost of 0.005, discount (γ) of 0.99 and unroll length of 40 steps.
...
Policy logits (π, size 4) and baseline function (B, size 1) were produced by a linear projection.
