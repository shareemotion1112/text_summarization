...
In order to satisfy the small memory footprint constraint, we use a GMM which assumes feature independence, i.e., the covariance matrix Σ i of each Gaussian mixture is diagonal.
...
More specifically, the weights update is done by: DISPLAYFORM0 where µ is the momentum factor, α is the learning rate and M is a binary random mask with probability p of being 1 (M is randomly generated throughout the low-shot training).
...
The fully connected layer FC1 is parametrized with weight matrix W ∈ R N ×V , where V is the dimensionality of input feature vector ν and N is the dimensionality of the output feature representation vector v.

As was described in Section 3.4 we want to extend the feature representation v ∈ R N with E additional dimensionsṽ ∈ R E .
...
Let DISPLAYFORM1 be the E indexes of maximum elements of R. We initialize the expansion to FC1 layer W exp in the following manner: DISPLAYFORM2 where w j ∈ R V is the j th row of matrix W , ∼ N (0, std(W )), and α is a weight constant (in our experiments α = 0.25).

This initialization allows the expansion of the feature representationṽ to have non zero responses (after ReLU) with respect to the novel samples.
...
Since v was expanded with additional E featuresṽ, and we want to allow classification of L novel classes, the dimension of expanded W will be (K + L) × (N + E).

As was mentioned in Section 3.4 and illustrated in FIG0 , the hard distillation constraint requires that W will be zero-padded with K × E zeros to avoid influence of the expanded featuresṽ on the output of the base network.

In contrast, the expansion of W which we denote W exp ∈ R L×(N +E) should be encouraged to produce larger responses toṽ to improve learning.

We initialize the expansion of FC2 layer W exp in the following manner: DISPLAYFORM3 where u ∈ {−1, 1} with probability 0.5 and Γ is an amplification parameter.

In our experiments we used Γ = 2.
