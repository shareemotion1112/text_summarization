...
Several other structured constructions were known before BID1 Hinrichs & Vybíral, 2011; BID11 BID15 Choromanski & Sindhwani, 2016; Choromanska et al., 2016; Bojarski et al., 2017) , however these were motivated by computational and space complexity gains and led to weaker accuracy guarantees.
...
Define projections φ t = RF (f t ), η t = RF (h t ), and DISPLAYFORM0 PSRNN model parameters consist of an initial state q 1 and a 3-mode update tensor W .
...
A class of RBF kernels K (RBFs in shorthand) is a family of functions: DISPLAYFORM0 , for z = x−y 2 , where φ : R → R is a fixed positive definite function (not parametrized by n).

An important example is the class of Gaussian kernels.

Every RBF kernel K is shift-invariant, thus in particular its values can be described by an integral via Bochner's Theorem BID6 : DISPLAYFORM1 where µ K ∈ M(R n ) stands for some finite Borel measure.

Some commonly used RBF kernels K together with the corresponding functions φ and probability density functions for measures µ K are given in Table 2 .

The above formula leads straightforwardly to the standard unbiased MonteCarlo estimator of K(x, y) given as: K(x, y) = Φ m,n (x)Φ m,n (y), where a random embedding Φ m,n : R n → R 2m is given as: DISPLAYFORM2 vectors w i ∈ R n are sampled independently from µ K and m stands for the number of random features used.
...
The fact that for RBF kernels the marginal distributions are exactly µ K and thus, kernel estimator is still unbiased, is a direct consequence of the isotropicity of distributions fom which directions of vectors w iid i are sampled.
...
Assume that labeled datapoints (x i , y i ), where x i ∈ R n , y i ∈ R for i = 1, 2, ..., are generated as follows: y i = f * (x i ) + ν i , where f * : R n → R is a function that the model aims to learn, and ν i for i = 1, 2, ... are independent Gaussians with zero mean and standard deviation σ > 0.
...
Assume that we analyze kernel K : R n × R n → R with the corresponding kernel matrix K. It is a well known result (Alaoui & Mahoney, 2015; Bach, 2013) that the empirical risk of f KRR is given by the formula: DISPLAYFORM1 where λ > 0 stands for the regularization parameter and I N ∈ R N ×N is an identity matrix.
...
DISPLAYFORM3 It turns out that one can upper-bound the risk R( f KRR ) for the estimator f KRR in terms of the ∆ parameter if matrix K + λN I N is a ∆-spectral approximation of the matrix K + λN I N , as the next result (Avron et al., 2017) shows:Theorem 1.

Suppose that K 2 ≥ 1 and that matrix K + λN I N obtained with the use of random features is a ∆-spectral approximation of matrix K + λN I N .
...
Our main result is given below and shows that orthogonal random features lead to tighter bounds on ∆ for the spectral approximation of K + λN I N .
...
Let ∆ iid denote the smallest positive number such that K iid + λN I N is a ∆-approximation of K + λN I N , where K iid is an approximate kernel matrix obtained by using unstructured random features.

Then for any a > 0, DISPLAYFORM2 where: p iid N,m is given as: DISPLAYFORM3 2 for some universal constant C > 0, m is the number of random features used, σ min is the smallest singular value of K + λN I N and N is dataset size.
...
Note first that from basic properties of the trigonometric functions we conclude that for any two vectors x, y ∈ R n , the random feature map approximation of the RBF kernel K(x, y) which is of the form K(x, y) = |Φ m,n (x) Φ m,n (y) can be equivalently rewritten as: K(x, y) = 1 m m i=1 cos(w i z) for z = x − y.

This is true for any joint distribution (w 1 , ..., w m ).

Lemma 2.

If mapping Φ m,n is based on the standard mechanism of independent sampling then one can take as function g from Lemma 1 a function given by the following formula: g(x) = e −Cmx 2 for some universal constant C > 0.Proof.
...
Expression A(k, n) is given by the following formula: DISPLAYFORM10 which can be equivalently rewritten (by computing the integrals explicitly) as: DISPLAYFORM11 where: δ(n = 2) = 2 if n = 2 and δ(n = 2) = 1 otherwise and: γ(n = 2) = 1 if n = 2 and γ(n = 2) = 2 otherwise.

In particular, the following is true: DISPLAYFORM12 We will use that later.

Note that v i v j ∼ v z.

Therefore we obtain: DISPLAYFORM13 where DISPLAYFORM14 and λ = i,j∈{1,...,m} n i n j R i R j v i v j .Note that E[(R 2 ) k−1 λ] = 0 since E[(v i v j )] = 0 and furthermore, directions v 1 , ..., v m are chosen independently from lengths R 1 , ..., R m .

Therefore we have: DISPLAYFORM15 where: DISPLAYFORM16 Now let us focus on a single term ρ = E[(R 2 ) k−l λ l ] for some fixed l ≥ 3.Note that the following is true: DISPLAYFORM17 where the maximum is taken over i 1 , j 1 , ..., i l , j l ∈ {1, ..., m} such that i s = j s for s = 1, ..., l.
