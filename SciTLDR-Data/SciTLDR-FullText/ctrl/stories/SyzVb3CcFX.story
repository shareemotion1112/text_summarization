...
G is trained as: DISPLAYFORM0 where E is a measure of prediction error, such as G(c) − x τ 1 .
...
This minimum loss may be viewed as adaptively learning the time offset τ , but in fact, the predictor's task is even simpler since it is not required to provide a timestamp accompanying its prediction.
...
Given input frames c = (x 0 , x last ), fixed-time bidirectional predictors might target, say, the middle frame x τ = x last/2 .
...
In typical fixed-time CVAE predictors targeting time τ , variations in a latent code z input to a generator G(c, z) must capture stochasticity in x τ .

At training time, z is sampled from a posterior distribution q φ (z|x τ ) with parameters φ, represented by a neural network.

At test time, z is sampled from a prior p(z).

The training loss combines a log-likelihood term with a KL-divergence from the prior: DISPLAYFORM0 where we might set p G to a Laplacian distribution such that the second term reduces to a l1- DISPLAYFORM1 In a time-agnostic CVAE, rather than capturing stochasticity at a fixed time τ , z must now capture stochasticity at bottlenecks: e.g., when the agent crosses one of the asterisks in the maze of Fig 1, which pose is it in?

The bottleneck's time index varies and is not known in advance.

For computational reasons (see Appendix B), we pass the entire video X into the inference network q φ , similar to BID0 .
...
We set up L t cgan to use samples z from the prior p(z), while L t cvae-gan instead samples z from the posterior q φ (z|X), and employs separate discriminators {D t }.
...
We set w(t) = β + t/15, so that our preference increases linearly from β to β + 1.

Since w(t) applies multiplicatively, low β corresponds to high disparity in preferences (β = ∞ reduces to MIN, i.e., no time preference).

GENMIN2 is our approach with β = 2 and so on.
...
TAP (various models with varying β) produces an even larger variation in stepsizes than fixed-time methods explicitly targeting the entire video (FIX0.75 and FIX1.0 fall short of producing predictions at 0.75 and 1.0 fraction of the episode length).
...
However, in practice, we use the following slightly different loss: DISPLAYFORM0 where we set l t,t = max(0, 1 − α|t − t |) with α = 0.25.
...
Both the inference network q φ and the generator G would have to process |T| frames.

In comparison, the formulation of Eq 9 allows q φ and G to process just one frame, and only error computation must be done T times, once with each target frame x t for t ∈ T.
...
In our experiments, we set these weights heuristically as follows: w(t) rises linearly from κ = 0.66 at t = 1 to 1.0 at the t = 5, then stay at 1.0 for T − 10 frames.

Then, starting from the (T − 5)-th frame, it would drop linearly to κ once more at t = T .

The only hyperparameter we tuned was κ (search over 2/3 and 1/3).
