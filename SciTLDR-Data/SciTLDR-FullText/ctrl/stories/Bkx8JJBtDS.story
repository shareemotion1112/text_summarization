...
Then, i becomes the continuous variable λ (with Λ denoting the total number of layers), where the discrete N i is approximated by the continuous variable n(λ).
...
From equation 4, we have dλ = dp/n 2 , so equation 5 can be re-written as
...
Assuming that each layer has (1 + α) times the number of channels of the previous layer, i.e., N i+1 = N i × (1 + α), means that for the continuous case we have n(λ) = n 0 e αλ , where n 0 is the number of channels in the first layer.

This allows us to find a closed-form solution for equation 6 (explicit calculations are presented in Appendix A.1),

Figure 1: Z as a function of α and n 0 (left -computed from equation 7) and as a function of Λ and n 0 (rightcomputed from equation 7 and inverting equation 10 from the Appendix A.1).

If we set the number of parameters to P = 35 × 10 6 as in ResNet101 (He et al., 2016) , the maximum value for Z is at α = 0.0178 with the number of channels in the first layer n 0 = 56.5.

This means that the total number of layers is Λ = 167.5 and the optimal size of the last layer is n(Λ) = 1113 ( Fig. 1) .
...
In particular, we will use the empirical relation for the dependency of the optimal learning rate with depth (1/L) and the theoretical result for the optimal variance of the weight initialisation (1 − σ 2 w 1/L).
...
We show in Fig. 3 that the value of Z is a good predictor of the optimal depth Λ for a simple feed forward CNN.
...
A.2 MAXIMISING Z AS A FUNCTIONAL TO SOLVE CONTINUOUS COUNTING A more effective way to maximise Z over all functions n(λ) under the constraint of a fixed number of parameters P relies on the use of calculus of variations.

More specifically, from equation 6, we have

log ((n + δn) 2 × d(n+δn) dp ) (n + δn) 2 dp,

where a maximum point of the function Z implies δZ = 0 for any (small) δn.

By expanding Z +δZ in the first order in δn, discarding higher order terms, and relying on integration by parts, we obtain:

log ((n + δn) 2 d(n+δn) dp )(1 − 2δn/n) n 2 dp P 0 log (n 2 dn dp ) n 2 − 2δn log (n 2 dn dp ) n 3 + 2δn n 3 + dδn dp n 2 dn dp dp P 0     log (n 2 dn dp ) n 2 +     − 2 log (n 2 dn dp ) n 3 + 2 n 3 − d 1 n 2 dn dp dp     δn     dp.

In the last step we integrate by parts and set the boundary conditions for n. To maximise the functional in equation 14, we set boundary conditions with δn(P ) = δn(0) = 0.

Note that the first term of the integral in equation 14 is just Z, as defined in equation 6.

We search for an optimal n such that δZ does not grow (at first order), no matter the choice of δn.

This is equivalent to have δZ = 0 for any δn n(p), which means that for any p the factor inside the integral is zero -that is: − 2 log (n 2 dn dp ) n 3 + 2 n 3 − d 1 n 2 dn dp dp = 0.
...
This differential equation is a first order ordinary differential equation and, given the initial condition for v at n(0), can be integrated numerically to obtain a family of solutions for v as a function of n. Then, n(p) and n(λ) can be integrated numerically.

Fig. 7 shows a family of solutions for n 0 = 56.5.

The thick line is the solution with the largest Z 272, which is larger than 251, obtained in Sec. 2.2 with the exponential assumption (i.e., n(λ) = n 0 e αλ ).

For comparison, we show this exponential function from Sec. 2.2 as a thin black line.

The system predicts a network depth Λ ≈ 174 (the highest point in Fig. 7 right panel) , which is comparable with the value of 167.5 found in Sec. 2.2.
