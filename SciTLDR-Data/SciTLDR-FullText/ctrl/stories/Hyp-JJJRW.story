...
The reconstruction loss weight (α) was set to 0.05, and the optimization method used to train the network was Adam (Kingma & Ba, 2014) with a learning rate η of 0.00001 for 250 epochs.
...
We performed the interpolation by simply taking convex combinations of the two style memories, usinĝ DISPLAYFORM0 where m 1 and m 2 denote the style memories.

The interpolated style memory is denoted bym(λ), where λ ∈ [0, 1] denotes the interpolation coefficient.
...
The top layer is trained using a multi-objective optimization, trying to simultaneously minimize classification error and reconstruction loss.(a) (Figure 11: Image reconstruction with style memory interpolation between digits and letters shown in FIG7 and FIG0 , where λ was increasing from 0.1 to 1.0 with a step of 0.1 from top to bottom.
