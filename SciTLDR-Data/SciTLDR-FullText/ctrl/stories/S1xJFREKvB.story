...
Scalar multiplication for v ∈ R d and β ∈ R is denoted as β · v. f : R d → R denotes the loss function to be minimized and ∇f (x) represents the gradient of f evaluated at x. We denote the unbiased stochastic gradient estimator of ∇f (x) as ∇f i (x) with the random variable i independent of x (e.g., using mini-batch).
...
Nesterov's momentum The original Nesterov's accelerated gradient (with constant step) (Nesterov, 1983; 2013b) has the following scheme 2 (y ∈ R d , η, β ∈ R and y 0 = x 0 ):

where we call β · (y k+1 − y k ) the momentum.
...
It can be verified that schemes (2) and (3) are equivalent to (1) through v k = β −1 ·(x k −y k ) and v pt k = η −1 β −1 ·(y k −x k ), respectively (see Defazio (2018) for other equivalent forms of scheme (1)).
...
SGD and M-SGD In order to study the features of momentum, in this work, we regard momentum as an add-on to plain SGD, which corresponds to fixing the learning rates 4 γ = η.

From the interpretation in Allen-Zhu & Orecchia (2017), η represents the learning rate for the gradient descent "inside" Nesterov's method.
...
Intuitively, Option I is SGD with amortized momentum and Option II applies an m-iterations tail averaging on Option I. 4 Ma & Yarats (2019) observed that when effective learning rates γ = η(1 − β) −1 are fixed, M-SGD and SGD have similar performance.
...
Input: Initial guess x 0 , learning rate η, momentum β, amortization length m, iteration number K.
...
We can improve the efficiency of Algorithm 1 by maintaining a running scaled momentumṽ
...
6 Amortized momentum β·(x + −x) is expected to be much large than Nesterov's momentum β·(y k+1 −y k ).

Table 1 : Detailed data of the curves in Figure 2b .

Best viewed in color.

We observed that when we use schedulers with a large decay factor and the momentum β is too large for the task (e.g., 0.995 for the task of this section), there would be a performance drop after the learning rate reduction.
...
If Option II is used,φ 0 = x 0 . {a running average for the point table φ} 1: for k = 0, . . . , K − 1 do 2:
...
Trading memory for extensibility In expectation, the point table φ stores the most recent m iterations and thus the outputφ K is an m-iterations tail average, which connects to AM1-SGD.
...
where X ⊆ R d is a non-empty closed convex set and h is a proper convex function with its proximal operator prox αh (·) 8 available.

We impose the following assumptions on the regularity of f and the stochastic oracle ∇f i (identical to the ones in Ghadimi & Lan (2012) with µ = 0):
...
For example, (a) covers the cases of f being L-smooth (M = 0) or L 0 -Lipschitz continuous (M = 2L 0 , L = 0) convex functions and if σ = 0 in (c), the assumptions cover several classes of deterministic convex programming problems.

We denote x ∈ X as a solution to problem (4) and x 0 ∈ X as the initial guess.

Unlike its usage in deep learning, the momentum parameter β is always a variable in general convex analysis.

For the simplicity of analysis, we reformulate AM1-SGD (Algorithm 1) and AM2-SGD (Algorithm 2) into the following schemes 10 (z ∈ X, α ∈ R):
...
We show in Appendix B.1 that when h ≡ 0 and β is a constant, the reformulated schemes AM1-SGD and AM2-SGD are equivalent to Algorithm 1 and Algorithm 2 through α s = η(1 − β s ) −1 and Parikh et al. (2014) .
...
(b) If the variance has a "light tail", i.e., E i exp ∇f i (x)−∇f (x) 2 /σ 2 ≤ exp{1}, ∀x ∈ X, and X is compact, denoting D X max x∈X x − x , for any Λ ≥ 0, we have
...
In this case, the convergence rate in Theorem 1a becomes the corresponding Understandings: Theorem 1a gives the expected performance in terms of full-batch loss F (x) − F (x ), from which the trade-off of m is clear: Increasing m improves the dependence on variance σ but deteriorates the O(L/K 2 ) term (i.e., the acceleration).

Based on this trade-off, we can understand the empirical results in Figure 2b : the faster convergence in the early stage could be the result of a better control on σ and the slightly lowered final accuracy is possibly caused by the reduced acceleration effect.
...
Understanding: Comparing the rates, we see that when using the same m, AM2-SGD has slightly better dependence on σ, which is related to the observation in Figure 5 that AM2-SGD is always slightly faster than AM1-SGD.

This difference is suggesting that randomly incorporating past iterates beyond m iterations helps.

If m = O(1), Theorems 1 and 2 establish the optimal O(L/K 2 + (σ + M )/ √ K) rate in the convex setting (see Lan (2012) for optimality), which verifies AM1-SGD and AM2-SGD as variants of the Nesterov's method (Nesterov, 1983; 2013b) .
...
AM1-SGD and AM2-SGD use exactly the same values for (η, β) as M-SGD, which was tuned to optimize the performance of M-SGD.
...
For the choice of (η, β), following Lucas et al. (2019), we chose β = 0.99 and used the scheduler that reduces the learning rate by half when the validation loss has not decreased for 15 epochs.

We swept η from {5, 2.5, 1, 0.1, 0.01} and found that η = 2.5 resulted in the lowest validation perplexity for M-SGD.

We thus ran AM1-SGD and AM2-SGD with this (η, β) and m = 5.
...
Extra results are provided in the appendices for interested readers: the robustness when using large β (Appendix A.2), a CIFAR-100 experiment (Appendix A.6) and comparison with classical momentum (Polyak, 1964)
...
We compare the robustness of M-SGD and AM1-SGD when β is large in Figure 7 & Table 5 .

For fair comparison, AM1-SGD uses Option I. As we can see, the STD error of M-SGD scales up significantly when β is larger and the performance is more affected by a large β compared with AM1-SGD.

CM-SGD with its typical hyper-parameter settings (η 0 = 0.1, β = 0.9) is observed to achieve similar generalization performance as M-SGD.
...
We used the exponential hyper-parameter setting recommended in the original work with the scalefactor a = 0.1 fixed, β (t) = 1 − a t−1 , for t = 1, . . .

, T and choosing T in {2, 3, 4}. We found that T = 2 gave the best performance in this experiment.

As shown in Figure 8 & Table 6 , with the help of passive damping, AggMo is more stable and robust compared with CM-SGD. (2019) introduce the immediate discount factor ν ∈ R for the momentum scheme, which results in the QHM update rules (α ∈ R,

Here we used the recommended hyper-parameter setting for QHM (α 0 = 1.0, β = 0.999, ν = 0.7).
...
Despite of showing worse generalization performance, both QHM and AggMo perform better on reducing the trainbatch loss in this experiment, which is consistent with the results reported in Ma & Yarats (2019) We show in Figure 10 that when β is large for the task, using step learning rate scheduler with decay factor 10, a performance drop is observed after each reduction.
...
If β = 0.9, there is no observable performance drop in this experiment.
...
For all methods, η 0 = 0.1, β = 0.9, run 20 seeds.
...
When h ≡ 0 and β is a constant, we do the reformulations by eliminating the sequence {z k }.
...
which means that we need to set x k+1 ← x k+1 + β · (x s+1 −x s ) (reassign the value of x k+1 ).
...
If α(1 − β) < 1/L, the update scheme (6) satisfies the following recursion:
...
Using Young's inequality with ζ > 0, we obtain

For R 3 , based on the optimality condition of prox αh {z − α · ∇f i (x)} and denoting ∂h(z + ) as a subgradient of h at z + , we have for any u ∈ X,
...
and taking expectation, if α s (1 − β s ) < 1/L, we have
...
It can be verified that with the choices β s = s s+2 and α s = λ1 L(1−βs) , the following holds for s ≥ 0,

Note that since our analysis aims at providing intuition, we do not refine the choice of α s as in (Hu et al., 2009; Ghadimi & Lan, 2012) .
...
Under review as a conference paper at ICLR 2020 where (a) follows from λ 1 ≤ 2 3 and (b) holds because 0 ≤ x → (x + 2) 2 is non-decreasing and thus

, and based on the choice of λ 1 = min
...
, sm + m − 1 and using the choice α s = λ1 L(1−βs)
...
For R 6 , using the choice of α s and λ 1 , we obtain
...
However, in theory, following a similar analysis of Katyusha, we cannot guarantee a reduction of σ in the worst case.

Deriving AM1-SGD from Katyusha Katyusha has the following scheme (non-proximal, in the original notations, σ is the strong convexity parameter, cf.
...
Note that in the parameter setting of Katyusha, we have η = ατ 1 , and thus
...
If we replace the SVRG estimator ∇ k with ∇f i k (x k ), set 1 − τ 1 − τ 2 = 0, which is to eliminate Nesterov's momentum, and use a uniform average for x s+1 , the above scheme becomes exactly AM1-SGD (Algorithm 1).
...
A recent work (Zhou et al., 2018) shows that when 1 − τ 1 − τ 2 = 0, which is to solely use Katyusha momentum, one can still derive optimal rates and the algorithm is greatly simplified.

Their proposed algorithm (i.e., MiG) is structurally more similar to AM1-SGD.

This scheme is equivalent to the PyTorch formulation (scheme (3)) through v Based on this formulation, α is understood as the effective learning rate (i.e., the vector it scales has the same cardinality as a gradient) and the experiments in Ma & Yarats (2019) were conducted with fixed α = 1.
...
In theory, to obtain the optimal convergence rate, the effective learning rate α is set to a very large O(k/L), which can be derived from Theorem 1 or Theorem 2 by setting σ = 0, M = 0, m = 1 (then λ 1 or λ 2 is always 2 3 since the other term is ∞).

If we fix α = 2 3L for both methods, GD has an O(1/K) convergence rate (cf.

Theorem 2.1.13 in Nesterov (2013b)).

For the Nesterov's method, if we use β k = k k+2 , it has the convergence rate (applying Lemma 1): Thus, in this case, both GD and the Nesterov's method yield an O(1/K) rate, and thus we expect them to have similar performance.
...
However, what is special about the Nesterov's method is that it finds a legal way to adopt a large α that breaks the 1/L limitation.

If GD uses the same large α, we would expect it to be unstable and potentially diverge.

In this sense, Nesterov's momentum can be understood as a "stabilizer".

In our basic case study (ResNet34 on CIFAR-10), if we align the effective learning rate and set γ = 1.0 for SGD, the final accuracy is improved but the performance is highly unstable and not robust, which is 2.205% average STD of test accuracy over 5 runs.
...
Our work uses the convergence behavior of SGD as a reference to reflect and to understand the features of our proposed momentum, which is why we set γ = η.
