...
Right: The corresponding hard samples' negative cosine similarities N (t, cos θj) = I(t, cos θj)

cos θj + c after modulation, where c indicates a constant.
...
sT (cos θy i )+ n j=1,j =y i e sN (t,cos θ j ) is the predicted ground truth probability and G(p(x i )) is an indicator function.

T (cos θ yi ) and N (t, cos θ j ) = I(t, cos θ j ) cos θ j + c are the functions to modulate the positive and negative cosine similarities, respectively, where c is a constant, and I(t, cos θ j ) denotes the modulation coefficients of negative cosine similarities.

In margin-based loss function, e.g, ArcFace, G(p(x i )) = 1, T (cos θ yi ) = cos(θ yi + m), and N (t, cos θ j ) = cos θ j .
...
The recent work, SV-Arc-Softmax emphasizes hard samples by increasing I(t, cos θ j ) for hard samples.

That is, G(p(x i )) = 1 and N (t, cos θj ) is formulated as follows:

If a sample is defined to be easy, its negative cosine similarity is kept the same as the original one, cos θ j ; if as a hard sample, its negative cosine similarity becomes t cos θ j + t − 1.

That is, as shown in Fig. 1 , I(·) is a constant and determined by a preset hyper-parameter t. Meanwhile, since t is always larger than 1, t cos θ j + t − 1 > cos θ j always holds true, which means the model always focuses on hard samples, even in the early training stage.
...
N (t, cos θ j ) = cos θj, T (cos θy i ) − cos θj ≥ 0 cos θj(t + cos θj), T (cos θy i ) − cos θj < 0.
...
As shown in Fig. 1 , the modulation coefficient of hard sample negative cosine similarity I(t, θ j ) depends on both the value of t and θ j .
...
Moreover, within the same training stage, I(·) is monotonically decreasing with θ j so that harder sample can be assigned with larger coefficient according to its difficultness.
...
Specifically, let r (k) be the average of the positive cosine similarities of the k-th batch and be formulated as r (k) = i cos θ yi , we have:

where t 0 = 0, α is the momentum parameter and set to 0.99.

As shown in Fig. 2 , the parameter t increases with the model training, thus the gradient modulation coefficients' range of hard sample, M (·) = 2 cos θ j + t, also increases.

Therefore, hard samples are emphasized gradually.

With the EMA, we avoid the hyper-parameter tuning and make the modulation coefficients of hard sample Input: The deep feature of i-th sample xi with its corresponding label yi, last fully-connected layer parameters W , cosine similarity cos θj between two vectors, embedding network parameters Θ, learning rate λ, number of iteration k, parameter t, and margin m k ← 0, t ← 0, m ← 0.5; while not converged do k ← k + 1; if cos(θy i + m) > cos θj then N (t, cos θj) = cos θj; else N (t, cos θj) = (t (k) + cos θj) cos θj ; end T (cos θy i ) = cos(θy i + m); Compute the loss L by Eq. 8; Compute the back-propagation error of xi and Wj by Eq. 9; Update the parameters W and Θ by:

∂Θ (k) ; Update the parameter t by Eq. 7; end Output: W , Θ negative cosine similarities I(·) adaptive to the current training stage.

To sum up, the loss function of our CurricularFace is formulated as follows:

where N (t (k) , cos θ j ) is defined in Eq. 6.

The entire training process is summarized in Algorithm 1.

Figure 2: Illustrations on the adaptive parameter t (red line) and gradient modulation coefficients M (·) = 2 cos θj + t of hard samples (green area).
...
2) The ratio is monotonically increasing with cosθ j , since the larger cosθ j is, the harder the sample is.
...
In the forwarding process, when j = y i , it is the same as the ArcFace, i.e., f j = sT (cos θ yi ), T (cos θ yi ) = cos(θ yi + m).

When j = y i , it has two cases, if x i is an easy sample, it is the the same as the original softmax, i.e., f j = s cos θ j .

Otherwise, it will be modulated as f j = sN (t, cos θ j ), where N (t, cos θ j ) = (t + cos θ j ) cos θ j .
...
Softmax cos θy i = cos θj SphereFace cos(mθy i ) = cos θj CosFace cos θy i − m = cos θj ArcFace cos(θy i + m) = cos θj SV-Arc-Softmax cos(θy i + m) = cos θj (easy) cos(θy i + m) = t cos θj + t − 1 (hard) CurricularFace (Ours) cos(θy i + m) = cos θj (easy) cos(θy i + m) = (t + cos θj ) cos θj (hard)
