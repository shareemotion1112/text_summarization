...
In doing so we introduce \textit{Siamese Capsule Networks}, a new variant that can be used for pairwise learning tasks.

We find that the model improves over baselines in the few-shot learning setting, suggesting that capsule networks are efficient at learning discriminative representations when given few samples.
...
As noted by hinton1985shape, from a psychology perspective of human shape perception, pooling does not account for the coordinate frames imposed on objects when performing mental rotation to identify handedness BID23 ; BID19 BID12 .
...
The Siamese Capsule Network is trained using a contrastive loss with 2 -normalized encoded features and demonstrated on face verification tasks.

BID8 first introduced the idea of using whole vectors to represent internal properties (referred to as instantiation parameters that include pose) of an entity with an associated activation probability where each capsule represents a single instance of an entity within in an image.
...
The original paper by BID24 describes a dynamic routing scheme that represent these internal representations as vectors given a group of designated neurons called capsules, which consist of a pose vector u ∈ R d and activation α ∈ [0, 1].
...
BID7 recently describe matrix capsules that perform routing by agreement using the expectation maximization (EM) algorithm, motivated by computer graphics where pose matrices are used to define rotations and translations of objects to account for viewpoint changes.
...
BID31 formalize the capsule routing strategy as an optimization of a clustering loss and a KL regularization term between the coupling coefficient distribution and its past states.
...
This routing scheme shows significant benefit over the original routing scheme by BID24 as the number of routing iterations increase.
...
BID27 presented a joint identification-verification approach for learning face verification with a contrastive loss and face recognition using cross-entropy loss.
...
BID32 propose a center loss function to improve discriminative feature learning in face recognition.
...
BID21 used an 2 -constraint on the softmax loss for face verification so that the encoded face features lie on the ambit of a hypersphere, showing good improvements in performance.

This work too uses an 2 -constraint on capsule encoded face embeddings.

FaceNet BID26 too uses a triplet network that combines the Inception network BID28 and a 8-layer convolutional model BID33 which learns to align face patches during training to perform face verification, recognition and clustering.
...
The most relevant and notable use of Siamese Networks for face verification is the DeepFace network BID30 .
...
To encode paired images x 1 , x 2 into vector pairs h 1 , h 2 the pose vector of each capsule is vectorized and passed as input to a fully connected layer containing 20 activation units.
...
The logit function learns the dropout rate of the final capsule layer using Concrete Dropout BID4 .

This builds on prior work Kingma et al. FORMULA7 ; BID20 by using a continuous relaxation that is referred to as a concrete distribution, the mask becomes differentiable, unlike the discrete Bernoulli distribution used in Binary dropout.

Equation 2 shows the objective function for updating the concrete distribution.

For a given capsule probability p c in the last capsule layer, the sigmoid computes the relaxationz on the Bernoulli variable z, where u is drawn uniformly between [0,1] and log(u c ) produces log-uniform noise, and t denotes the temperature values (t = 0.1 in our experiments) which forces probabilities at the extremum when small.

The pathwise derivative estimator is used to find a continuous estimation of the dropout mask.z DISPLAYFORM0 Equation FORMULA8 show the dropout loss where the LHS numerator represents the weight regularization term for N weights in the final layer L where γ controls the amount of regularization.

The remaining H(p d L ) is the dropout entropy term used as dropout regularization.

This entropy term H(p d L ) is essential to learn the dropout probability for capsule representations, also note when regularization plays a significant role reducing the contrastive loss, d p → 0.5.

In our experiments, weight regularization λ ω L = 0.1 is chosen based on a grid search during training for λ θ L = {0.001, 0.01, 0.1, 0.2, 0.5}. This is used as an auxiliary regularization term with the contrastive losses discussed in the next section.

DISPLAYFORM1 We also use a straight-through pathwise derivative estimator BID5 to then optimize the dropout probability, following the implementation by BID4 .

BID7 has also been used to maximize the inter-class distance between the target class and the remaining classes for classifying on the smallNORB dataset.

This is given as

where the margin m is increased linearly during training to ensure lower-level capsule stay active throughout training.
...
Equation 4 shows a a pair of images that are passed to the SCN model where DISPLAYFORM0 2 computes the Euclidean distance between encodings and m is the margin.

When using Manhattan distance DISPLAYFORM1 A double margin loss that has been used in prior work by BID17 is also considered to affect matching pairs such that to account for positive pairs that can also have high variance in the distance measure.
...
Equation 5 shows the double-margin contrastive loss where positive margin m p and negative margin m n are used to find better separation between matching and non-matching pairs.
...
DISPLAYFORM2 The original reconstruction loss DISPLAYFORM3 i ) 2 used as regularization is not used in the pairwise learning setting, instead we rely on the dropout for regularization with exception of the SCN model that uses concrete dropout on the final layer.

In the case of using concrete dropout the loss is L conc = L c (ω) + Γ c (p d ) and Γ controls the amount of dropout regularization Optimization Convergence can often be relatively slow for face verification tasks, where few informative batch updates (e.g a sample with significantly different pose for a given class) get large updates but soon after the effect is diminished through gradient exponential averaging (originally introduced to prevent α → 0).

Motivated by recent findings that improve adaptive learning rates we use AMSGrad BID22 .
...
Equation 6 presents the update rule, where diagonal of gradient g t is given as DISPLAYFORM4
...
From the original LFW dataset there has been 2 different versions of the dataset that align the images using funneling BID10 and deep funneling BID11 .

The latter learns to align the images using Restricted Boltzmann Machines with a group sparsity penalty, showing performance improvements for face verification tasks.

The penalty leads to an arrangement of the filters that improved the alignment results.

This overcomes the problems previous CNNs and models alike had in accounting for pose, orientation and problems Capsule Networks look to address.

In contrast, we use the original raw image dataset.

Both datasets allow us to test different challenges in face verification.

The AT&T dataset provides a useful benchmark for the siamese-based models in the few-shot learning setting as it only provides a limited number of instances and unique subjects, while also being limited to grey-pixel images.

In contrast, LFW contains coloured images that are in an unconstrained setting, large in comparison to AT&T and have highly imbalanced classes.

Baselines SCNs are compared against well-established architectures for image recognition and verification tasks, namely AlexNet BID15 , ResNet-34 (He et al., 2016) and InceptionV3 BID29 with 6 inception layers instead of the original network that uses 8 layers which are used many of the aforementioned papers in Section 3.
...
Shaded cells correspond to best performing models over test contrastive loss and test accuracy for each respective dataset and loss (e.g LFW+Double Margin (M)).

The former uses m = 0.4 and the latter uses m = 0.2, while for the double margin contrastive loss m n = 0.2 matching margin and m p = 0.5 negative matching margin is selected.
...
Additionally, the use of a double margin results in better results for the LeNet SCN but a slight drop in performance when used with concrete dropout on the final layer (i.e SDropCapNet).

Figure 1 illustrates the contrastive loss during training 2 -normalized features for each model tested with various distance measures on AT&T and LFW.
...
In batch normalization, (k) and scaled with β (k) so that a (k) = γ (k)x(k) + β (k) .
...
Figure 2 shows the probability density of the positive pair predictions for each model for all distances between encodings with contrastive loss for the LFW dataset.

Accuracy is computed using the aforementioned margins to separate positive and negatives instance pairs, which is set to be constant throughout training since the capsule encoded features are − 2 normalized, similar to the approach taken for FaceNet BID26 .
...
Additionally, varying distances for these matching images were close in variance to nonmatching images.

This motivated the use of the double margin loss considered for the LFW dataset.

DISPLAYFORM0 Finally, the SCN model has between 104-116 % less parameters than Alexnet, 24-27 % Resnet-34 and 127-135% less than the best LeNet baseline for both datasets.
