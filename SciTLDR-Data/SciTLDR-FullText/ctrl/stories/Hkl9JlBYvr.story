...
We define a Markov decision process (MDP) as a tuple M = (S, A, R, T, T 0 , γ, H) with S a set of states, A a set of actions, R(r t+1 |s t , a t , s t+1 ) a reward function, T (s t+1 |s t , a t ) a transition function, T 0 (s 0 ) an initial state distribution, γ a discount factor, and H the horizon.

In the standard RL setting, we want to learn a policy π that maximises J (π) = E T0,T,π H−1 t=0 γ t R(r t+1 |s t , a t , s t+1 ) , the expected return.
...
Since the agent does not have access to the true reward and transition function, it can maintain a belief b t (R, T ) at every timestep, which is the posterior over the MDP given the agent's experience: given a trajectory of states, actions, and rewards, τ :t = {s 0 , a 0 , r 1 , s 1 , a 1 , . . .

, s t }, the prior p(R, T ) can be updated to form a posterior belief b t (R, T ) = p(R, T |τ :t ).
...
• The belief update (computing the posterior p(R, T |τ :t )) is often intractable.
...
i.e., we want to infer the posterior distribution p(m i |τ
...
A trajectory of states, actions and rewards is processed online using an RNN to produce the posterior over task embeddings, q φ (m|τ :t ).
...
Consequently, we need to learn a model of the environment p θ (τ :H + |a :H + −1 ), parameterised by θ, together with an amortised inference network q φ (m|τ :t ), parameterised by φ, which allows fast inference at runtime at each timestep t.

The action-selection policy is not part of the MDP, so an environmental model can only give rise to a distribution of trajectories when conditioned on actions, which we typically draw from our current policy, a ∼ π.

At any given time step t, our model learning objective is thus to maximise

where ρ(M, τ :H + ) is the trajectory distribution induced by our policy and we slightly abuse notation by denoting by τ the state-reward trajectories, excluding the actions.

In the following, we drop the conditioning on a :H + −1 to simplify notation.

Instead of optimising (7), which is intractable, we can optimise a tractable lower bound, defined with a learned approximate posterior q φ (m|τ :t ) which can be estimated by Monte Carlo sampling (for the full derivation see AppendixA):

The term E q [log p(τ :H + |m)] is often referred to as the reconstruction loss, and p(τ :t |m) as the decoder.

The term KL(q(m|τ :t )||p θ (m)) is the KL-divergence between our variational posterior q φ and the prior over the embeddings p θ (m).

For sufficiently expressive decoders, we are free to choose p θ (m).

We set the prior to our previous posterior, q φ (m|τ :t−1 ), with initial prior q φ (m) = N (0, I).

As can be seen in Equation 8 and Figure 2 , when the agent is at timestep t, we encode the past trajectory τ :t to get the current posterior q(m|τ :t ) since this is all the information available to perform inference about the current task.

We then decode the entire trajectory τ :T including the future, i.e., model E q [p(τ :T |m)].
...
The reconstruction term log p(τ :H |m) factorises as log p(τ :H + |m, a :H + −1 ) = log p((s 0 , r 0 , . . .
...
The encoder q φ (m|τ :t ), parameterised by φ;

2.

An approximate transition function T = p θ (s i+1 |s i , a i ; m) and an approximate reward function R = p θ (r i+1 |s t , a t , s i+1 ; m) which are jointly parameterised by θ; and 3.

A policy π ψ (a t |s t , q φ (m|τ :t )) parameterised by ψ and dependent on φ.

The policy is conditioned on both the environment state and the posterior over m, π(a t |s t , q(m|τ :t )).
...
For t = 0, we use the prior q φ (m) = N (0, I).
...
This is necessary since parameters φ are shared between the model and the policy.
