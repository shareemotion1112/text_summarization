...
The first convolution in bottleneck makes channel C drop toC = C/α, where α represents reduction ratio.
...
After pixelshuffle, the output is multiplied by a parameter γ 1 and added to the upsampling result without global reasoning.

The initial value of γ 1 is set to 0.

As the global reasoning module trains, the network will gradually learn to assign values to the γ 1 , thereby fully exploiting global reasoning.
...
In addition, we apply the idea of residual connection in GRRB, which multiplys the information generated via global reasoning by a parameter γ and then add it to the input feature map.

The output is given by:

The initial value of γ is set to 0.

As the training progresses, the network assigns more weight to γ.
...
We set the reduction ratio in bottleneck as α.
...
We use the Adam optimizer to update the parameters of the network with β 1 = 0.9, β 2 = 0.999, and ϵ = 10 −8 .
...
Moreover, we also study the effects of two basic parameters N and α on the performance of the model.

As shown in Table 1 , we observe that larger N and smaller α would lead to higher performance.

Considering that larger N and smaller α will also bring more computation, we set 10 and 8 as the value of N and α respectively.
