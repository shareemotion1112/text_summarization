...
For a G-space acting on X , the map Φ : G × X → X is said to be equivariant if its domain and co-domain are acted on by the same symmetry group (Cohen & Welling, 2016; Cohen et al., 2018a) :

where g 1 ∈ G and g 2 ∈ G. Equivalently Φ(T (g 1 ) x) = T (g 2 ) Φ(x), where T (·) is a linear representation of the group G. Note that T (·) does not have to commute.
...
By a special dynamic routing procedure that uses the activations determined in a previous layer, we arrive at latent capsules that are composed of a set of orientationsq i and new activationsα i .

Thanks to the decoupling of local reference frames,α i is invariant and orientationsq i are equivariant to input rotations.
...
To construct equivariant layers on the group of rotations, we are required to define a left-equivariant averaging operator A that is invariant under permutations of the group elements, as well as a distance metric δ that remains unchanged under the action of the group.

For these, we make the following choices: Definition 7 (Geodesic Distance).

The Riemannian (geodesic) distance in the manifold of rotations lead to the following geodesic distance δ(·) ≡ d quat (·):

Definition 8 (Quaternion Mean µ(·)).
...
Quaternions, the employed mean A(S, w) and geodesic distance δ(·) enjoy the following properties:
...
The transformations g ∈ H 1 preserve the geodesic distance δ(·) given in Dfn.
...
The weights w ← σ(x, y) are inversely propotional to the distance between the vote quaternion and the new quaternion (cluster center).
...
Note that, in practice one is quite free to choose the weighting function σ(·) as long as it is inversely proportional to the geodesic distance and concave (Aftab & Hartley, 2015) .
...
Given points x i and rotations q i , we compute the quaternion average µ i in channel-wise as the initial pose candidates:
...
The network t(·) consists of fully-connected layers that regresses the transformations, similar to common operators for continuous convolutions (Schütt et al., 2017; .
...
Although Riemannian layers of Bécigneul & Ganea (2018) or spherical predictions of Liao et al. (2019) can improve the performance, the simple strategy works reasonably for our case.

After computing the votes, we utilize the input activation α i to iteratively refine the output capsules (weighted average of votes)q i and activationsα i by routing by agreement as shown in Alg.
...
For classification we use the spread loss (Sabour et al., 2018) and the rotation loss is identical to δ(·).
...
RAE between the ground truth and the prediction is computed as the relative angle in degrees: d(q 1 , q 2 )/π.
...
Operator A is invariant under permutations: A({q σ(1) , . . .
...
The transformations g ∈ H 1 preserve the geodesic distance δ(·).
...
Also note that this choice corresponds to tr(qq ) = d+1 i=1 α i = 1.
...
Due to the compactness, we can speak of a ball B(o, ρ) encapsulating all y i .
...
We use a shared network to process two distinct point clouds (X, Y) to arrive at the latent representations (C X , α X ) and (C Y , α Y ) respectively.
