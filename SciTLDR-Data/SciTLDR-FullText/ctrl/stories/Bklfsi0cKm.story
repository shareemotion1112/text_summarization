...
The other activations of the network, from A (2) (X) up to A (L+1) (X), are defined recursively: i,j corresponds to applying the filter to the µth convolutional patch of the channel x j .

DISPLAYFORM2

represents the flattened jth channel of the image that results from applying a convolutional filter to φ(A ( ) (X)).The structure of the pseudo-weight matrices W where it does, as illustrated in Fig. 1 .The outputs of the network are the last activations, A (L+1) (X).
...
, C ( +1) } denote the input and output channels, and ν and µ ∈ {1, . . .

, H ( +1) D ( +1) } denote the location within the input and output channel or feature-maps.

The mean function is thus easy to compute DISPLAYFORM1 i,j,µ,ν have zero mean, and W ( +1) i,j,ν,µ are independent of the activations at the previous layer, φ(A ( ) j,ν (X)).
...
6: end for 7: Output the scalar K DISPLAYFORM8 The weights are independent for different channels: DISPLAYFORM9 can eliminate the sums over j and ν : DISPLAYFORM10 The µth row of W DISPLAYFORM11 is zero for indices ν that do not belong to its convolutional patch, so we can restrict the sum over ν to that region.
...
The elementwise covariance in the right-hand side of Eq. (11) can be computed in closed form for many choices of φ if the activations are Gaussian.

For each element of the activations, one needs to keep track of the 3 distinct entries of the bivariate covariance matrix between the inputs, K DISPLAYFORM0 For example, for the ReLU nonlinearity (φ(x) = max(0, x)), one can adapt BID5 in the same way as Matthews et al. (2018a, section 3 ) to obtain DISPLAYFORM1 where θ ( ) DISPLAYFORM2
...
The kernel hyperparameters are: σ 2 b , σ 2 w ; the number of layers; the convolution stride, filter sizes and edge behaviour; the nonlinearity (we consider the error function and ReLU); and the frequency of residual skip connections (for Residual CNN GPs).
...
The inputs to a classic GP kernel k (e.g. RBF) are preprocessed by applying a feature extractor g (a deep NN) prior to computing the kernel: k deep (X, X ) := k(g(X; θ), g(X , θ)).
...
First, they use the Cramér-Wold device, to reduce the full problem to that of proving convergence of scalar random variables to a Gaussian with specified variance.
...
To extend their proofs to the convolutional case, we begin by defining our networks in a form that is easier to manipulate and as close as possible to Eq. (21-23) in BID21 , DISPLAYFORM0 DISPLAYFORM1 where, DISPLAYFORM2 The first step is to use the Cramér-Wold device (Lemma 6 in BID21 , which indicates that convergence in distribution of a sequence of finite-dimensional vectors is equivalent to convergence on all possible linear projections to the corresponding real-valued random variable.

Mirroring Eq. 25 in BID21 , we consider convergence of random vectors, f DISPLAYFORM3 where L ⊂ X × N × {1, . . .

, H ( ) D ( ) } is a finite set of tuples of data points and channel indicies, i, and indicies of elements within channels/feature maps, µ. The suffix [n] indicates width functions that are instantiated with input, n. Now, we must prove that these projections converge in distribution a Gaussian.

We begin by defining summands, as in Eq. 26 in BID21 , DISPLAYFORM4 such that the projections can be written as a sum of the summands, exactly as in Eq. 27 in BID21 , DISPLAYFORM5 Now we can apply the exchangeable CLT to prove that T ( ) (L, α) [n] converges to the limiting Gaussian implied by the recursions in the main text.

To apply the exchangeable CLT, the first step is to mirror Lemma 8 in BID21 , in showing that for each fixed n and ∈ {2, . . .

, L + 1}, the summands, γ ( ) j (L, α) [n] are exchangeable with respect to the index j.
...
Mirroring Eq. 29 in BID21 , we apply the recursion, k,ξ (x)[n] : k ∈ {1, . . .
...
Lemma 20 states that the eighth moments of f (t)i,µ (x)[n] are bounded by a finite constant independent of n ∈ N. We prove this by induction.

The base case is trivial, as f where g ( −1) j∈{1,...,C ( −1) (n)},ν∈µth patch (x)[n] is the set of post-nonlinearities corresponding to j ∈ {1, . . .

, C ( −1) (n)} and ν ∈ µth patch.
