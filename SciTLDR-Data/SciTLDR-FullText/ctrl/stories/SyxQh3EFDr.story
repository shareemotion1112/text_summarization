...
In COT, the probability of Figure 1: Sorted predicted probabilities (denoted asŷ) from three different training paradigms evaluated on CIFAR-100 dataset using PreAct ResNet-18.

The red bar indicates the probability of the ground-truth (denoted asŷ g ), the green bars are the probabilities of classes in the same parental category as the ground-truth (denoted asŷ G\{g} ), and blue bars are the probabilities of the rest classes (denoted asŷ K\G , see Sec. 3 for detailed notation definition).

Notice the "staircase shape" in (c) showing the significant difference betweenŷ g andŷ G\{g} , and then betweenŷ G\{g} andŷ K\G , which confirms HCOT well captures the label hierarchy.
...
The proposed HCE is defined as the following with θ being the model parameters:
...
Our loss function consists of two terms: the normal cross entropy term (i.e., XE(θ)), and the complement objective term HCE(θ).
