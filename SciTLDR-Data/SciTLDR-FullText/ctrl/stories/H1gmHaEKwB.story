...
However, we show in Theorem 7 and Corollary 8 that we can construct a small coreset C, such that |z −z| ≤ ε for any input x ∈ R d .

Algorithm 1 summarizes the coreset construction for a single neuron with an activation function φ, (our results for common neural activation functions are summarized in Table 1 ).
...
Definition 3 (ε-coreset, multiplicative guarantee).

Let (P, w, X, f ) be a query space, and ε ∈ (0, 1) be an error parameter.

An ε-coreset of (P, w, X, f ) is a weighted set (Q, u) such that for every
...
Let c ≥ 1 be a sufficiently large constant that can be determined from the proof, and let C be a sample (multi-set) of m ≥ ct ε 2 d log t + log 1 δ i.i.d.

points from P , where for every p ∈ P and q ∈ C we have pr(p = q) = s(p)/t.

Then, with probability at least 1 − δ,

Algorithm 2: CORESET PER LAYER(P, w 1 , · · · , w k , m, φ, β)
...
Most of the coresets provide a (1 + ε)-multiplicative factor approximation for every query that is applied on the input set.
...
Let φ : R → [0, ∞) such that φ(b) > 0 if and only if b > 0.

Let α, β > 0, ε ∈ (0, 1) and n ≥ 1 be an integer.

Then there is a set P ⊆ B α (0) of n points such that if a weighted set (C, u) satisfies C ⊆ P and ∀x ∈ B β (0) :
...
Let α, β > 0 and (P, w, B β (0), f ) be a query space of VC-dimension d such that P ⊆ B α (0), the weights w are non-negative, f (p, x) = φ(p T x) and φ : R → [0, ∞) is a nondecreasing function.

Let ε, δ ∈ (0, 1) and

and c is a sufficiently large constant that can be determined from the proof.

Let (C, u) be the output of a call to CORESET(P, w, m, φ, β); see Algorithm 1.

Then, |C| ≤ m and, with probability at least 1 − δ,

The proof is provided in Appendix A.2.

As weights of a neural network can take positive and negative values, and the activation functions φ : R → R may return negative values, we generalize our result to include negative weights and any monotonic (non-decreasing or non-increasing) bounded activation function in the following corollary.
...
Then, |C| ≤ m and, with probability at least 1 − δ, ∀x ∈ B β (0) :
...
Let (C, u 1 , · · · , u k ) be the output of a call to CORESET(P, w 1 , · · · , w k , m, φ, β); see Algorithm 2.
...
where φθ(x) and φ θ (x) are the outputs of the approximated and the original networks respectively.
