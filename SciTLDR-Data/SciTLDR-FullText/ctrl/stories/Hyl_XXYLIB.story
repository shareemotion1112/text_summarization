...
The coefficient of α of this regulator is a hyperparameter of our approach.

We can also gain more control over the amount of sparsity in each layer by choosing a differentα for different layers.
...
However, as we push our sparsity knobs (i.e. the L 1 regulator α and activity threshold θ ) higher, eventually both training and validation accuracy will suffer and the network fails to fit the data properly.
...
We scan over a range of hyperparameters (α, the L 1 weight regulator and ξ , the learning rate) using grid search and note the value of the best validation accuracy across all hyperparameters.
...
We sparsify the picked models using the highest activation thresholdθ such that the model remains within this margin of the best validation accuracy.

We finally pick the hyperparameters which give the highest sparsity among these models.

In this way, we efficiently find the optimal hyperparameters α * (m), θ * (m) and ξ * (m) which afford the highest sparsity model with validation accuracy within m% of the highest validation accuracy among all hyperparameters.
...
For the remaining tasks, we settle on learning rate of 0.002 and L 1 weight regularization α = 10 −7 ,10 −5 ,10 −6 respectively for the first, second and final layers.
