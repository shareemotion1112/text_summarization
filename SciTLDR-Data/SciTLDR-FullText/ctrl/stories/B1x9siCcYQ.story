...
We overview the most related works by categorizing them as follows:Learning vector representation from text: Vector representation of words (Schütze, 1993) has been a long standing research topic.
...
Our goal is to find function f that maximizes: DISPLAYFORM1 Since events N G (v) and N T (φ|v) are independent, (1) can be rewritten as: DISPLAYFORM2 where w v is the number of words in the description of v. Given a word in a node, (2) jointly captures the node neighborhood in the graph and the word neighborhood in text.
...
In this model, each input is a two-tuple (φ, v), i.e., word φ is picked from the description of node v. Then (φ, v) is mapped to two one-hot vectors, w-dimensional word vector φ and n-dimensional vertex vector v, where only the entries corresponding to φ and v are set to 1 and others are set to 0.

Then as in typical fully connected neural network architectures, in the first layer, φ T and v T are multiplied (implemented as look-up for efficiency) by two matrices M w×d and M n×d , respectively.
...
The reason for this decomposition operation is that we use h 1 to represent the probability vector of neighboring words of φ in the description of node v, and h 2 to represent the probability vector of neighboring nodes of v in the graph.
...
At each iteration, we update the model parameters by adding a fraction of −∇L, i.e.,−η∇L = cη∇F G + η∇F T .
...
To do so, we adjust the model parameter update rule −η∇L as DISPLAYFORM2 where β 1 and β 2 are the equivalent learning rates for graph inputs and text inputs, respectively.
