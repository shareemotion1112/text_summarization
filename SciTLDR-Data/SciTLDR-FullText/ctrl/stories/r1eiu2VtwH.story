...
Instead, we propose to use the α-entmax function (Peters et al., 2019) as it is able to learn sparse choices, depending only on a few features, via standard gradient descent.

This function is a generalization of softmax in its variational form:

is Shannon entropy.

We can define α-entmax by replacing H(p) with Tsallis α-entropy 2 .

The choice function is hence replaced by a weighted sum of features, with weights computed as α-entmax (α=1.5) over the learnable feature selection matrix F ∈ R d×n :

Similarly, we relax the Heaviside function 1(f i (x) − b i ) as a two-class entmax, which we denote as

, where b i and τ i are learnable parameters for thresholds and scales respectively.
...
Finally, the output of the NODE layer is composed as a concatenation of the outputs of m individual trees ĥ 1 (x), . . .

,ĥ m (x) .

Multidimensional tree outputs.

In the description above, we assumed that tree outputs are onedimensionalĥ(x) ∈ R. For classification problems, where NODE predicts probabilities of each class, we use multidimensional tree outputsĥ(x) ∈ R |C| , where |C| is a number of classes.
...
Note, in the multi-layer architecture described above, tree outputsĥ(x) from early layers are used as inputs for subsequent layers.

Therefore, we do not restrict the dimensionality ofĥ(x) to be equal to the number of classes, and allow it to have an arbitrary dimensionality l, which correspond to

When averaging the predictions from all layers, only first |C| coordinates ofĥ(x) are used for classification problems and the first one for regression problems.
...
The scales τ i are initialized in such a way that all the samples in the first batch belong to the linear region of σ α , and hence receive nonzero gradients.
...
In all the experiments we set α parameter in the entmax transformation to 1.5.
...
In comparison α parameter was set to 1.5.
...
The results clearly show that Entmax with α=1.5 outperforms the competitors across all experiments.
