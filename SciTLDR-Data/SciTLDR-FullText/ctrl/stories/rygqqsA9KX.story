...
LetX 1∶M denote the generated multimodal data andŶ denote the generated labels, with joint distribution PX 1∶M ,Ŷ = P (X 1∶M ,Ŷ).
...
By construction, F y contributes to the generation ofŶ while {F y , F ai } both contribute to the generation ofX i .

As a result, the joint distribution P (X 1∶M ,Ŷ) can be factorized as follows: DISPLAYFORM0 DISPLAYFORM1 Exact posterior inference in Equation 1 may be analytically intractable due to the integration over Z. We therefore resort to using an approximate inference distribution Q(Z X 1∶M , Y) as detailed in the following subsection.
...
The decoder modules are parametrized according to the factorization of P (X 1∶M ,Ŷ Z) as given by Equation 1 and FIG0 (a).
...
DISPLAYFORM2 1∶M ,Ŷ ) denote the joint-distribution Wasserstein distance over multimodal data under cost function c X i and c Y .
...
The two differences are: (1) we show that P (X 1∶M ,Ŷ Z = z) are Dirac for all z ∈ Z, and (2) we use the fact that c(( DISPLAYFORM4 .
...
A penalty term is added into our objective to find the Q(Z ⋅) ∈ Q that is the closest to prior P Z , thereby approximately enforcing the constraint Q Z = P Z : where λ is a hyper-parameter and MMD is the Maximum Mean Discrepancy (Gretton et al., 2012) as a divergence measure between Q Z and P Z .
...
Formally, let Φ denote the surrogate inference network.
...
We compare with two baselines: 1) a purely generative Seq2Seq model BID8 ) Φ G from observed modalities to missing modalities by optimizing E P X 1∶M (−log P Φ D (X 1 X 2∶M )), and 2) a purely discriminative model Φ D from observed modalities to the label by optimizing DISPLAYFORM3 ).
...
TAB4 shows that MFM with missing modalities outperforms the generative (Φ G ) or discriminative baselines (Φ D ) in terms of modality reconstruction and sentiment prediction.
...
To simplify the proof, we first prove it for the unimodal case by considering the Wasserstein distance between P X,Y and PX ,Ŷ .
...
By definition, the Wasserstein distance under cost function c between P X,Y and PX ,Ŷ is DISPLAYFORM0 where We now introduce two Lemmas to help the proof.

DISPLAYFORM1 DISPLAYFORM2 Since the functions F, G a , G y , D are all deterministic, then P (X,Ŷ Z) are Dirac measures.

◻ Lemma 2.

P P X,Y , PX ,Ŷ = P X,Y,X,Ŷ when P (X,Ŷ Z = z) are Dirac for all z ∈ Z.Proof: WhenX,Ŷ are deterministic functions of Z, for any A in the sigma-algebra induced byX,Ŷ, we have DISPLAYFORM3 Therefore, this implies that (X, Y) ⊥ ⊥ (X,Ŷ) Z which concludes the proof.

A similar argument is made in Lemma 1 of BID41 .◻

Now, we use the fact that P P X,Y , PX ,Ŷ = P X,Y,X,Ŷ (Lemma 1 + Lemma 2), DISPLAYFORM4 Note that in Eq. equation 8, P X,Y,Z = P (X, Y) ∼ P X,Y , Z ∼ P Z and with a proposed Q(Z X), we can rewrite Eq. equation 8 as DISPLAYFORM5 The proof is similar to Proposition 2, and we present a sketch to it.

We can first show P (X 1∶M ,Ŷ Z = z) are Dirac for all z ∈ Z.
...
MFM outperforms β-VAE across these datasets and metrics.
