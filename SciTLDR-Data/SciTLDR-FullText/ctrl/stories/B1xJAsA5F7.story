...
Let N (v) be the set of neighbor nodes of v. There are two hidden vectors ν uv and ν vu for each edge (u, v) representing the message from u to v and vice versa.

These messages are updated iteratively via neural network g 1 (·): DISPLAYFORM0 uv is the message computed in the t-th iteration, initialized with ν (0) uv = 0.
...
Formally, letẼ = {(i 1 , j 1 ), · · · , (i m , j m )} be the edges traversed in a depth first traversal over tree T , where m = 2|E| as each edge is traversed in both directions.

LetẼ t be the first t edges inẼ. At the t-th decoding step, the model visits node i t and receives message vectors h ij from its neighbors.
...
This probability is computed by aggregating the source encodings {x T * } and {x G * } through an attention layer, followed by a feed-forward network (τ (·) stands for ReLU and σ(·) for sigmoid): DISPLAYFORM1 DISPLAYFORM2 Here we use attention(·; U DISPLAYFORM3 Label Prediction If node j t is a new child to be generated from parent i t , we predict its label by DISPLAYFORM4 where q t is a distribution over the label vocabulary and U l att is another set of attention parameters.
...
We first apply a graph message passing network over graph G i to compute atom representations {µ Gi v }.
...
Then we compute the difference vector δ X,Y between molecules X and Y as in Eq.(11).
...
The mean and log variance of Q(·|X, Y ) is computed from δ X,Y with two separate affine layers µ(·) and Σ(·): DISPLAYFORM1 Finally, we combine the latent code z T and z G with source tree and graph vectors: DISPLAYFORM2 wherex T * andx G * are "perturbed" tree and graph vectors of molecule X. The perturbed inputs are then fed into the decoder to synthesize the target molecule Y .
...
Following standard practice in MMPA, we construct training sets by sampling molecular pairs (X, Y ) with significant property improvement and molecular similarity sim(X, Y ) ≥ δ.
...
Following their setup, we experiment with two similarity constraints (δ = 0.4 and 0.6), and we extracted 99K and 79K translation pairs respectively from the ZINC dataset BID45 BID23 for training.
...
We extracted a training set of 88K molecule pairs with similarity constraint δ = 0.4.
...
With similarity constraint δ = 0.4, we derived a training set of 34K molecular pairs from ZINC and the dataset collected by BID36 .
...
Model Configuration Both VSeq2Seq and our models use latent codes of dimension |z| = 8, and we set the KL regularization weight λ KL = 1/|z|.
...
To this end, we define index α v = i if v ∈ C i and α v = j if v ∈ C j \ C i .

The index α v is used to mark the position of the atoms in the junction tree, and to retrieve messages h i,j summarizing the subtree under i along the edge (i, j) obtained by running the tree encoding algorithm.
