...
Given g = (β, v) ∈ ST and an input image x (0) (u, λ) (u ∈ R 2 is the spatial position, and λ is the unstructured channel index, e.g. RGB channels of a color image), the scaling-translation group action

Constructing ST -equivariant CNNs thus amounts to finding an architecture A such that each trained network f ∈ A commutes with the group action D β,v on the input and a similarly defined group action T β,v (to be explained in Section 3.1) on the output.

Inspired by Cheng et al. (2019) and Weiler et al. (2018b) , we consider ST -equivariant CNNs with an extra index α ∈ S for the the scaling group S ∼ = R: for each l ≥ 1, the l-th layer output is denoted as x (l) (u, α, λ), where u ∈ R 2 is the spatial position, α ∈ S is the scale index, and λ ∈ [M l ] := {1, . . .
...
Similar to Cheng et al. (2019) , the group action T β,v on the l-th layer output is defined as a scaling-translation in space as well as a shift in the scale channel:
...
A feedforward neural network with an extra index α ∈ S for layerwise output is ST -equivariant if and only if the layerwise operations are defined as (5) and (6):
...
λ ,λ (u)δ(α), the joint convolution (6) over R 2 × S reduces to only a (multiscale) spatial convolution
...
We consider decomposing the convolutional filters W (l) λ ,λ (u, α) under the product of two function bases, {ψ k (u)} k and {ϕ m (α)} m , which are the eigenfunctions of the Dirichlet Laplacian on, respectively, the unit disk D ⊂ R 2 and [−1, 1], i.e.,

In particular, the spatial basis {ψ k } k satisfying (8) is the Fourier-Bessel (FB) basis (Abramowitz & Stegun, 1965) .
...
λ ,λ (k, m) are the expansion coefficients of the filters.
...
On the other hand, in an ScDCFNet with truncated basis expansion up to K leading coefficients for u and K α coefficients for α, the number of parameters is instead KK α M l−1 M l .

Hence a reduction to a factor of KK α /L 2 L α in trainable parameters is achieved for ScDCFNet via truncated basis decomposition.

In particular, if L = 5, L α = 5, K = 8, and K α = 3, then the number of parameters is reduced to (8 × 3)/(5 2 × 5) = 19.2%.

Computational cost: Suppose the size of the input x (l−1) (u, α, λ) and output x (l) (u, α, λ) at the l-th layer are, respectively, W × W × N α × M l−1 and W × W × N α × M l , where W × W is the spatial dimension, N α is the number of scale channels, and M l−1 (M l ) is the number of the unstructured input (output) channels.

Let the filters W

The following theorem shows that, compared to a regular ST -equivariant CNN, the computational cost in a forward pass of ScDCFNet is reduced again to a factor of KK α /L 2 L α .

e., the number of the output channels is much larger than the size of the convolutional filters in u and α, then the computational cost of an ScDCFNet is reduced to a factor of KK α /L 2 L α when compared to a ST -equivariant CNN without basis decomposition.

Apart from reducing the model size and computational burden, we demonstrate in this section that truncating the filter decomposition has the further benefit of improving the deformation robustness of the equivariant representation, i.e., the equivaraince relation (4) still approximately holds true even if the spatial scaling of the input D β,v x (0) is contaminated by a local deformation.
...
The definition of x (l) is different from that of RotDCFNet (Cheng et al., 2019) , where an L 2 norm is taken for the α index as well.

The reason why we adopt the L ∞ norm for α in (11) is that x (l) is typically not L 2 in α, since the scaling group S, unlike SO(2), has infinite Haar measure.
...
The pointwise nonlinear activation σ : R → R is non-expansive.
...
, where µ k is the k-th eigenvalue of the Dirichlet Laplacian on the unit disk defined in (8).

We next assume that each A l is bounded:

The boundedness of A l is facilitated by truncating the basis decomposition to only low-frequency components (small µ k ), which is one of the key idea of ScDCFNet explained in Section 3.2.
...
Given a C 2 function τ : R 2 → R 2 , the spatial deformation D τ on the feature maps x (l) is defined as
...
Let D τ be a small spatial deformation defined in (14), and let D β,v , T β,v be the group actions corresponding to an arbitrary scaling 2 −β ∈ R + centered at v ∈ R 2 defined in (2) and (3).

In an ScDCFNet satisfying (A1), (A2), and (A3), we have, for any L,

Theorem 3 gauges how approximately equivariant is ScDCFNet if the input undergoes not only a scale change D β,v but also a nonlinear spatial deformation D τ , which is important both in theory and in practice because the scaling of an object is rarely perfect in reality.
...
Figure 2 displays the first-and second-layer feature maps of an original image x (0) and its rescaled version D β,v x (0) using the two comparing architectures.
...
] (rescaled l-th layer feature of the original input, where T β,v is understood as D β,v for a regular CNN due to the lack of a scale index α), and the difference (a) Zero-padding for the scale channel.
...
The error in equivariance is measured in a relative L 2 sense at a particular scale α, i.e.,
...
Since the scaling group S ∼ = R is unbounded, we compute only the feature maps x (l) (u, α, λ) with the α index restricted to the truncated scale interval [−1.6, 0] (2 −1.6 ≈ 0.3), which is discretized uniformly into N α = 9 channels (again, see Appendix B.1 for implementation details.)
...
In particular, M stands for the number of the first-layer (unstructured) output channels, which is doubled after each layer, and K/Kα is the number of basis function in u/α used for filter decomposition.
...
The first two images on the left are the original inputs; Decoder(C) denotes the reconstruction using the (unchanged) image code C; Decoder(D β,v C) and Decoder(T β,v C) denote the reconstructions using the "rescaled" image codes D β,v C and T β,v C respectively according to (2) and (3).
...
Our goal is to demonstrate that the image code produced by the ScDCFNet auto-encoder contains the scale information of the input, i.e., by applying the group action T β,v (3) to the code C of a test image before feeding it to the decoder, we can reconstruct rescaled versions of original input.
...
We also note that the layer-wise operations of a general feedforward neural network with an extra index α ∈ S can be written as
...
Hence for (17) to hold when l = 1, we need ∀ u, α, λ, u , λ , v, β. (26) Keeping u, α, λ, u , λ , β fixed while changing v in (26), we obtain that W (1) (u , λ , u, α, λ) does not depend on the third variable u. Thus

Then, for any given u , λ , u, α, λ, setting β = α in (26) leads to

(28) Hence (18) can be written as (5).

For l > 1, a similar argument leads to

Again, keeping u, α, λ, u , α , λ , β fixed while changing v in (29) leads us to the conclusion that W (l) (u , α , λ , u, α, λ) does not depend on the fourth variable u. Define

After setting β = α in (29), for any given u , α , λ , u, α, λ, we have
...
The summation over λ , adding the bias, and applying the nonlinear activation in (34) requires an additional W 2 N α M l (2 + M l−1 ) flops.
...
On the other hand, in an ScDCFNet with separable basis truncation up to KK α leading coefficients, (6) can be computed via the following steps:

The convolutions in α (36) require
...
In particular, when M l L 2 , L α , the dominating terms in (35) and (39) are, respectively,

Thus the computational cost in an ScDCFNet has been reduced to a factor of KKα L 2 Lα .

Before proving Proposition 1, we need the following two lemmas.

Lemma 1.

Suppose that {ψ k } k are the FB bases, and
...
λ ,λ,m , and b (l) , and let M = M l , M = M l−1 .
...
and ϕ L 2 = 1, and x is a function of three variables
...
where the second equality results from the fact that x(u, α, λ) − x c (u, α, λ) = x 0 (λ) depends only on λ (Proposition 1(b).)

Just like the proof of Proposition 1(a), we take the integral of α first, and define

where |E 1 (u, α, λ) + E 2 (u, α, λ)| 2 du
...
Moreover, for any given u and α,

where the last inequality is again because of (74).

Thus, for any given α,

λ ,λ,m (v, u, α) du dv gradient descent (SGD) with decreasing learning rate from 10 −2 (10 −1 ) to 10 −4 (10 −3 ) is used to train all networks without (with) batch-normalization for 160 epochs.
...
sc(Nα)LxLxM'xM: the first-layer convolution operation (5) in ScDCFNet, where Nα is the number of the uniform grid points to discretize the scale interval [−1.6, 0], and LxL is the spatial kernel size on the largest scale α = 0.

sc(Nα)LxLxLαxM'xM: the l-th layer (l > 1) convolution operation (6) in ScDCFNet, where the extra symbol Lα stands for the filter size in α.
...
The filter expansion in the SDCFNet auto-encoder is truncated to K = 8 and K α = 3.
