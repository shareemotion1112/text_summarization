...
Denoting the temporal dependence by t, the sequential input to the network is {x t ∈ X } T t=1 , and the output is a sequence of class scores vectors DISPLAYFORM0 , where L is the network depth, Θ denotes the parameters of the recurrent network, and T represents the extent of the sequence in time-steps.
...
The output at time t ∈ [T ] of the shallow (depth L = 1) recurrent network with R hidden channels, depicted in fig. 1(a) , is given by: DISPLAYFORM1 DISPLAYFORM2 where h t ∈ R R is the hidden state of the network at time t (h 0 is some initial hidden state), Θ denotes the learned parameters DISPLAYFORM3 , which are the input, hidden and output weights matrices respectively, and g is some non-linear operation.

A bias term is usually added to eq. 1, however, because it bears no effect on our analysis, we omit it for simplicity.

For common RNNs, the non-linearity is given by: DISPLAYFORM4 where σ(·) is typically some point-wise non-linearity such as sigmoid, tanh etc.
...
The output at time t of the depth L recurrent network with R hidden channels in each layer, 1 depicted in fig. 1(b) , is constructed by the following: DISPLAYFORM6 where h t,l ∈ R R is the state of the depth l hidden unit at time t (h 0,l is some initial hidden state per layer), and Θ denotes the learned parameters.
...
The construction of the shallow RAC weights tensor, presented in eq. 7, is referred to as a Tensor Train (TT) decomposition of TT-rank R in the tensor analysis community and is analogously described by a Matrix Product State (MPS) Tensor Network (see Orús (2014) ) in the quantum physics community.
...
See appendix B.1.The above claim establishes an equality between the Start-End separation rank and the rank of the matrix obtained by the corresponding grid tensor matricization, denoted A(y T,1,Θ c ) S,E , with respect to a specific set of template vectors.
...
, x (M ) ∈ X it holds that: DISPLAYFORM12 DISPLAYFORM13 where A(y DISPLAYFORM14 ) is the grid tensor of y T,L,Θ c with respect to the above template vectors.
...
We use a lemma proved in Sharir et al. (2016) , which states that since the entries of A(y T,L,Θ c ) are polynomials in the deep recurrent network's weights, it suffices to find a single example for which the rank of the matricized grid tensor is greater than the desired lower bound.
...
Under the same conditions as in theorem 1, for all values of Θ × h 0,l but a set of Lebesgue measure zero, it holds for any L that: DISPLAYFORM1 We motivate conjecture 1 by investigating the combinatorial nature of the computation performed by a deep RAC.
...
Specifically, we rely on the fact that it is sufficient to find a specific instance of the network parameters Θ × h 0,l for which A(y T,L,Θ c ) S,E achieves a certain rank, in order for this rank to bound the Start-End separation rank of the network from below.
...
The final component in each unit cell is the 3 legged triangle representing the order 3 tensor δ ∈ R R×R×R , referred to as the δ tensor, defined by: DISPLAYFORM0 with ij ∈ [R] ∀j ∈ [3], i.e. its entries are equal to 1 only on the super-diagonal and are zero otherwise.
...
In the first equality, we simply follow the TN prescription and write a summation over all of the contracted indices in the left hand side of FIG5 , in the second equality we use the definition of matrix multiplication, and in the last equality we use the definition of the δ tensor.
...
From the above treatment, it is evident that the restricted δ tensor is in fact the component in the TN that yields the element-wise multiplication property.

After T repetitions of the unit cell calculation with the sequential input {x t } T t=1 , a final multiplication of the hidden state vector h T by the output weights matrix W O yields the output vector y T,1,Θ .The tensor network which represents the order T shallow RAC weights tensor A

, which appears in eqs. 6 and 7, is given by the TN in the upper part of FIG5 .

In FIG5 , we show that by a simple contraction of indices, the TN representing the shallow RAC weights tensor A T,1,Θ c can be drawn in the form of a standard MPS TN.
...
We assume by contradiction that there exists a Tensor Network φ which operates on any vector v ∈ R P and clones it to two separate nodes of degree 1, each equal to v, to form an overall TN representing v ⊗ v. Component wise, this implies that φ upholds ∀v ∈ R P : DISPLAYFORM0 , meaning that ∀α ∈ [P ]: DISPLAYFORM1 By definition of the standard basis elements, the left hand side of eq. 12 takes the form φ αjk while the right hand side equals 1 only if j = k = α, and otherwise 0.

Utilizing the δ-tensor notation presented in eq. 10, in order to successfully clone the standard basis elements, eq. 12 implies that φ must uphold φ αjk = δ αjk .

However, for v = 1, i.e. ∀j ∈ [P ] : vj = 1, a cloning operation does not take place when using this value of φ, since DISPLAYFORM2 Claim 3 seems to pose a hurdle in our pursuit of a TN representing a deep recurrent network.
...
Next, we rely on lemma 1, which states that since the entries of A(y T,L,Θ c ) are polynomials in the deep recurrent network's weights, it suffices to find a single example for which the rank of the matricized grid tensor is greater than the desired lower bound.
...
This is done in similar spirit to the assignment used in the proof of theorem 1, in which W I,2 ij ≡ δi1 (see section B.3).
...
Next, in order to evaluate rank A(y T,L,Θ c ) S,E , we note that graph segments which involve only indices from the "Start" set, will not affect the rank of the matrix under mild conditions on W I,1 , W H,1 .
...
Let φ(T, L, R) be the TN representing the computation performed after T time-steps by an RAC with L layers and R hidden channels per layer.

Then, the number of occurrences in layer L = 1 of the basic unit connecting "Start" and "End" indices (bottom right in FIG9 ), is exactly DISPLAYFORM0 be the function computing the output after T time-steps of an RAC with L layers, R hidden channels per layer and weights denoted by Θ. In order to focus on repetitions in layer L = 1, we assign W I,2 ij ≡ δi1 for which the following upholds 7 :A(y DISPLAYFORM1 , where the constant term in the first line is the contribution of the deeper layers under this assignment, and the tensor V d 1 ...d T/2 , which becomes a vector under the Start-End matricization, reflects the contribution of the "Start" set indices.
...
, x (M ) ∈ X the following equality holds: are column and row vectors, respectively, which we denote by vν and u T ν .
...
According to the presented construction, the shallow RAC weights tensor (eqs. 6 and 7) is represented by a Matrix Product State (MPS) Tensor Network (Orús, 2014) , with the following order-3 tensor building block: DISPLAYFORM0 , where dt ∈ [M ] is the input index and kt−1, kt ∈ [R] are the internal indices (see FIG5 ).
...
Specifically: DISPLAYFORM0 Thus, since it trivially holds that rank A(y DISPLAYFORM1 T/2 (the rank is smaller than the dimension of the matrix), proving that rank A(y DISPLAYFORM2 for all of the values of parameters Θ×h 0,l but a set of Lebesgue measure zero, would satisfy the theorem.
...
In lemma 3 we prove that there exists a value of Ω such that for every sequence of colors d, i.e. a row ofV , the maximal reward over all possible initial states is solely attained at the state q for all values of z but a finite set, we know there exists a value of z for which rank (B) = N , and the theorem follows.
...
Let A ∈ R N ×N be a matrix whose entries are polynomials in x ∈ R. In this case, its determinant may be written as det(A) = σ∈S N sgn(σ)pσ(x), where SN is the symmetric group on N elements and pσ(x) are polynomials defined by pσ(x) ≡ N i=1 A iσ(i) (x), ∀σ ∈ Sn.

Additionally, let there existσ such that deg(pσ(x)) > deg(pσ(x)) ∀σ =σ.
...
Denoting t ≡ deg(pσ(x)), since t > deg(pσ(x)) ∀σ =σ, a monomial of the form c · x t , c ∈ R \ {0} exists in pσ(x) and doesn't exist in any pσ(x), σ =σ.

This implies that det(A) is not the zero polynomial, since its leading term has a non-vanishing coefficient sgn(σ) · c = 0, and the lemma follows from the basic identity: det(A) = 0 ⇐⇒ A is fully ranked.
...
, a (N ) } the following holds for all σ ∈ SN : DISPLAYFORM1 with equality obtained only for σ which upholds σ(i) = j ⇐⇒ a (i) = a (j) .
...
Finally, the hard inequality of the lemma for σ = IN is implied from eq. 15: DISPLAYFORM5 The vector rearrangement inequality in lemma 2, helped us ensure that our matrix of interest denotedŪ upholds the conditions of lemma 1 and is thus fully ranked.

Below, we show an identity that allowed us to make combinatoric sense of a convoluted expression:which is strictly less than the respective contribution in ρ * .
