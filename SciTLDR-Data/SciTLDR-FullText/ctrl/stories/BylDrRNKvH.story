...
(1) , the weights of the second layer by w (2) , and the ReLU function by φ(·).

Then the response function for the input x can be written as f (x) = w (2)T φ( w (1) , x ).
...
To approximate the non-differentiable ReLU φ, we use the softplus activation function φ τ (x), i.e.,

Note φ τ converges to ReLU as τ → ∞ (Glorot et al., 2011) .

Theoretical results with φ τ (x) will hold for any arbitrarily large τ .

For ease of notation, we still use φ(x) to denote the softplus function.

We first analyze the asymptotic prediction error of local minimum with large sample size.

Let the covariance matrix of φ(w

, where x ·k represents k th feature in x, and a k is the attention mask for x ·k .

φ ( w (1)T , x a is the first order derivative with respect to the value in φ(·) and it belongs to

.

Before proceeding, we introduce several necessary assumptions:

(A3) The output y can be specified by the two-layer neural network up to an independent subGaussian error with variance σ 2 , i.e., there exists a set of parameter (a , w (1) , w (2) ), such that

(A4) a 0 ≤ s 0 such that s 0 ≤ p, which represents the sparsity of the attention model, and 0 ≤ a i ≤ 1 for any i = 1, · · · , p.

(A5) λ min (Σ φ ) ≥ C φ , and when the estimation of φ is inaccurate, i.e, E( φ(

, then there exists a feature x ·k and for the t th element of h t (x ·k ), it satisfies sd(h t (x ·k )) = O(1) and cor(u, h t (x ·k )) = O(1) for w (2) in the form of Σ
...
Under (A1) to (A6), for any γ > 0, suppose

where η = C 1 C 2 C x .

Then with probability tending to 1, any stationary point (ã,w (1) ,w (2) ) of the objective function (4) satisfies the following prediction error bound:
...
When s 0 is fixed, we can see up to a log term, prediction error γ is proportion to n −1/2 , which is the optimal rate of convergence in regression.
...
Then with probability tending to 1, any stationary point (ã,w
...
, where η = C 1 C 2 C x , with probability converging to 1, any stationary point (w (1) ,w (2) ) of the objective function equation 5 satisfies that:
...
(A9) The output y can be predicted by the two-layer network (3) with an independent sub-Gaussian error with variance σ 2 , i.e, there exists a set of parameters (a ,
...
Under (A1), (A2), and (A8) to (A10), for any γ > 0, given the sample size:

where η = C 1 C 2 C x , with probability tending to 1, any stationary point (w
...
We will show that this fact also holds for the global attention mechanism, if no constraint on parameter (ã,w (1) ,w (2) ) is imposed.
...
and a minimum θ = (ã,w (1) ,w (2) ) satisfying thatã = 0,w (1) = 0,w (2) = 0.

For any > 0, C(L, θ, ) has an infinite volume, and for any M > 0, we can find a stationary point such that the largest eigenvalue of ∇ 2 L(θ) is larger than M;

(b) Consider the two-layer ReLU neural network with self-attention mechanism as stated in Section 3.2:

and a minimum θ = (

has an infinite volume, and for any M > 0, we can find a stationary point such that the largest eigenvalue of ∇ 2 L(θ) is larger than M.
...
Specifically, (A2) provides an upper bound on the magnitude of (a, w (1) , w (2) ) and (A5) bounds the magnitude of φ( w (1) , x i a ) from below.

These constraints control the parameter space and remove all sharp minima generated in Theorem 4 in which α 1 or α 2 goes to infinity.
...
(A11) The output y can be predicted by the two-layer network with an independent sub-Gaussian error with variance σ 2 , i.e, there exists a set of parameters (w
...
Under (A1),(A2),(A11) to (A13), there exists a sufficient large T , for any γ > 0, suppose

where η = C 1 C 2 C x , such that if there exist stationary point(s), then with probability tending to 1, any stationary point (w (1) ,w (2) ,w f ,w a ) satisfies the following prediction error bound:
...
Firstly, the lower eigenvalue bound assumes φ( w (1) , x i a ) is not degenerated.
...
Secondly, (A5) assumes when φ( w (1) , x i a ) is not well estimated, there exists an 'active feature'.

The correlation between this feature and bias cannot be cancelled by the direction of a specific linear combination of φ( w (1) , x i a ).

Intuitively, this assumption says that the correlation between h t (x ·k ) and E(y|x ·k ) cannot be fully explained by a fixed linear combination if there is some systematic bias in φ( w (1) , x i a ).
...
Considering all active terms in h t (x ·k ), they span a larger space comparing to φ( w (1) , x i a ), considering d is a fixed dimension.
...
, where φ ( w (1) , x i a ) t are O(1) with positive probability otherwise the network always predicts zero, therefore we have sd(h t (x ·k )) = O(1) as long as sd(x ·k ) = O(1) and a k = O(1).
...
By the arbitrary choice of our z, it means that all the directions of z are almost not correlated with u. When p and s 0 are high-dimensional,w (1) (x a)((w (2) t φ ( w (1) , x a ) spans all the directions as p → ∞, thus there must be some direction is correlated with u. Therefore it is reasonable to assume a O(1) correlation occurs.
...
In assumptions, rank(φ ( w (1) , x i a ) i=1,2,.
...
, D has correlation at rate O(1) with bias u(this assumption is parallel to (A5)), we can show the expectation term E x,y (∇R(w (k) )) ≥ O(γ).
...
• (A15) The output y can be predicted by the D-layer self-attention network with an independent sub-Gaussian error with variance σ 2 , i.e, there exists a set of parameters
...
Then for any γ > 0, given the sample size:

where η = C 1 C 2 C x , with probability tending to 1, any stationary point (w (1) ,w (2) ,w Q ,w K ) of the objective function (6) satisfies that:
...
In the proof, we may use o(γ) for vector/matrix case.

In these cases, it means that every element in vector/matrix is o(γ).
...
For the ease of notation, we denote θ = (a, w (1) , w (2) ).

Let Θ = {θ 1 , · · · , θ N } be a corresponding cover with N 3 elements.

Then we can always find Θ such that for any feasible θ, there exists j ∈ [N ] such that max( w
...
Then we consider the case when cov (u, φ( w (1) , x i a )) 2 = o(γ).

In this case, we denote r = cov(E(y|x), φ( w (1) , x a )) ∈ R d , and denote the covariance matrix for φ( w (1) , x a ) as:

, using subtraction and addition, we have: Σ φ w (2) = r + o(γ) With (A5), covariance matrix Σ φ is invertible with smallest eigenvalue lower bounded, we have w (2) = Σ −1 φ r + o(γ).

Next we argue the following term by contradiction:

where φ ( w (1) , x a ) represents the function corresponding to the true parameter set (w (1) , w (2) , a ).

If we have equation 9 violated, we know:

where we use E( φ(

, and we know φ ( w (1) , x a ) corresponding to true parameter set is finite.

Since we have derived w (2) = Σ −1 φ r, with lower bounded eigenvalue assumption, we can derive:

We know if we have cov (u, φ( w (1) ,

.

Therefore we conclude w (2) = w (2) + o(γ).
...
k )) 2 ≥ cγ for some constant c. Suppose E x (∇R(w (2) )) 2 ≥ cγ, and we denote

x , And we know v i 2 2 is bounded as we derived.

Therefore we can use Hoeffding bound on the 2 norm in direction of E x (∇R(w (2) (j) )), since we know the variance on this direction is smaller than σ 2 .
...
ing the bound for φ( w (1) , x i a ) we have derived in last section, with sample size n σ 2 c 2 γ 2 log( s0C1C2Cx cγ )(pd + p + d), conclude that with probability 1 − o p (1):

∇R n (w

Recalling part (a), under the first case that w.h.p ∇R(w (2) ) 2 ≥ cγ 3 for any parameter (a, w

(1) , w (2) ) with w (2) φ( w (1) , X ã ) − E(y|X) 2 ≥ γ.

Combining this with (10), we can conclude that for any positive constant γ > 0, with required sample size, with high probability that ∇R n (w (2) ) 2 > 0, thus they cannot be stationary solution for our loss function.

As we stated, if we are in another case, we have w.h.p ∇R(w

, we can use same techniques to show w.h.p ∇R n (w (2) ) 2 > 0.

In other words, under our assumptions, all the stationary points (ã,w (1) ,w (2) ) in our programming satisfy the prediction error upper bound rate γ w.h.p.
...
Under assumptions, using the same agrument with Theorem 3, we can show there exists a constant c such that either E(∇R(w (2 )) 2 ≥ cγ or there exist k ∈ 1, . . .
...
σ 2 ) Secondly we bound ∇R(w (2) ) − ∇R(w (2) ) j 2 term by subtraction and addition:

recalling that u i(j) and a i(j) are corresponding to the j th epsilon cover.

We choose = cγ 3C 2 1 C 2 2 C 2 7 C 2 x , and combine the above results.
...
Here we define an (α 1 , α 2 ) scale transformation such that:

Then we know the jacobian determinant for T α1,α2 is α

.

Let r > 0 such that B ∞ (r, θ) is in C(L, θ, ) and has empty intersection with (a, w

(1) , w (2) ) = 0.

Since pd > d, we assign α 2 → ∞, such that the jacobian determinant goes to infinity, and the volume of C(L, θ, ) goes to infinity.

For the Hessian matrix, without loss of generality, we assume there is a positive diagonal element δ > 0 in a.

Therefore the Frobenius norm

is lower bounded by α −2 1 δ.

Further we apply the fact that the biggest eigenvalue of a symmetric matrix X is larger than c X F , and pick α 1 < cδ M , then we have the biggest eigenvalue of ∇ 2 L(T α1,α2 (θ)) is larger than M. Therefore there exists a stationary point such that the operator norm for Hessian is arbitrary large.

Thus we finish proving part (a).

Then we consider part (b).

Similar with part (a), we define an α scale transformation such that:

And all the value,query and key matrices remain the same.

Then we know the jacobian determinant for T α = α (pdv−1)d .

Since pd v d ≥ d, as we assign α → ∞, such that the jacobian determinant goes to infinity, and the volume of C(L, θ, ) goes to infinity.

For the Hessian matrix, we still assume a positive diagonal element δ > 0 in w (1) .

Similarly we have the Frobenius norm

is lower bounded by α −2 δ.

When we choose sufficient small α, we have the biggest eigenvalue of ∇ 2 L(T α1,α2 (θ)) is larger than any constant M .
...
Denoting u i = (w (2)T φ( w (1) , x i a ) − y).

The derivatives can be presented as follows:

φ ( w (1) , x i a )))

By assumption, rank(φ( w (1) , x i a ) i=1,...,n ) = n, thus solving the linear system, we must have u i = 0 for any i = 1, 2, ..., n to satisfy that ∇R n (w (2) ) = 0.
...
Same argument follows for the case when E(∇R(w (1 )) k 2 ≥ cγ Then follow the same approach as Theorem 1 and 2, we obtain the sample complexity bound: n σ 2 c 2 γ 2 log(
...
Under (A14) to (A16), if we have E(f (x) − E(y|x)) 2 γ 2 , then:

E(∇R(w kr )) 2 sd(f (w kr ) 2 )sd(u)cor(∇f (w kr ), u) O(γ)

Then similar with Theorem 1 and 2, we construct an -cover over all parameters θ := (w k1 , w k2 , w V , w Q , w K ), and we denote it as {θ 1 , . . .

, θ N } such that for any feasible parameter, there exist j ∈ [N ] such that the maximum 2 distance to θ j is smaller than .

By calculating the number of parameters in all matrices in θ, we have
...
We choose = cγ 3L k , then at least with probability 1−O(N exp(−n c 2 γ 2 σ 2 )), we have ∇R(w (2) ) 2 > cγ 3 .

Therefore we can choose n log(

o(1).

Finally we can conclude that with probability 1 − o n (1), for any (a, w (1) , w (2) ) such that E(w (2) φ( w (1) , x ã ) − E(y|x)) 2 ≥ γ, we have ∇R(w (2) ) 2 > cγ 3 .

Then following the convergence of empirical risk procedure of Theorem 1, we show with probability going to 1 such that ∇R n (w (2) ) 2 > 0 and all parameters with prediction error O(γ) cannot be stationary point as long as n log(
