...
In α-Rank, strategy profiles of N agents are evaluated through an evolutionary process of mutation and selection.
...
With this, α-Rank then simulates a multi-agent game played by randomly sampled learners from each population.
...
We next formalize the process posed by α-Rank, which will lead to its limitations, and also pave the way for our own proposed solution.
...
Finally, given a joint profile π
...
with µ ∈ R + denoting an exploration parameter 1 , π

i representing policies followed by other agents at round k, and α ∈ R + an intensity ranking parameter.
...
Precisely, consider any two joint strategy profiles π [k] joint andπ [k] joint that differ in only one individual strategy for the i th agent, i.e., there exists a unique agent such that π

−i defining the probability that one copy of agent i with strategy π [k] i,a invades the population with all other agents (in that population) playingπ
...
So far, we presented relevant derivations for the (π
...
Now we restrict out attention to variations in joint policies involving more than two individual strategies, i.e., π
...
Summarising the above three cases, we can then write the (π

joint )'s entry of the Markov chain's transition matrix as:

The goal in α-Rank is to establish an ordering in policy profiles dependent on evolutionary stability of each joint strategy.
...
Limitations of α-Rank: Though the work in seeks to determine a solution to the above problem, it is worth mentioning that α-Rank suffers from one major drawback-the scalability-that we remedy in this paper.

We note that the solution methodology in α-Rank is in fact unscalable to settings involving more than a hand-full of agents.
...
In short, α-Rank exhibits an exponential (in terms of the number of agents) complexity for determining a ranking, thus rendering it inapplicable to settings involving more than a small amount of agents.
...
Hence, we commence to propose an efficient evaluation algorithm, i.e., α α -Rank, based on stochastic optimization with suitable complexities and rigorous theoretical guarantees.

At the end, we propose a search heuristic to further scale up our method by introducing oracles and we name it by α α -Oracle.
...
Apart from the above, we can also consider the problem as a standard eigenvalue decomposition task (also what the original α-Rank is implemented according to ) and adopt the method in Coppersmith & Winograd (1990) to compute the stationary distribution.
...
Clearly, these bounds restrict α-Rank to small number of agents N .
...
This makes mirror descent inapplicable for α-Rank when N is large.
...
Introducing logarithmic barrier-functions, with λ > 0 being a penalty parameter, we arrive at
...
6, and λ being a scheduled penalty parameter with λ t+1 = λt /γ for some γ > 1,
...
Letx λ be the output of a gradient algorithm descending in the objective in Eqn.

6, after T iterations, then

where expectation is taken w.r.t.

all randomness of a stochastic gradient implementation, and γ > 1 is a decay-rate for λ, i.e., λ

, penalty parameter λ ≈ O( ), λ decay rate γ > 1, total number of joint strategy profiles n, and a constraint relaxation term δ.
...
Compute the best-response strategy π * i by solving Eqn.

8.

Update strategy pools for each agent i as S

Set k = k + 1 15: Return: Best performing strategy profile π joint, across all agents.
...
Theorem A.2 implies that after T = O( 1 / 2 ) iterations with being a precision parameter, our algorithm outputs a vectorx λ 0 such that
...
In this section, we evaluate the scalability properties of α α -Rank 5 .
...
We note that these sizes are well beyond the capabilities of state-of-the-art methods, e.g., α-Rank ) that considers at maximum four agents with four strategies, or AlphaStar which handles about 600 strategies as detailed in Vinyals et al. (2019) .
...
Fig. 2 shows that, in fact, results generated by α α -Rank, the Phase I of Algorithm 1, are consistent with α-Rank's results.
...
Baselines includes eigenvalue decomposition from Numpy, optimization tools in PyTorch, and α-Rank from OpenSpiel .

For our algorithm we terminated execution with gradient norms being below a predefined threshold of 0.01.

According to Fig. 3 , α α -Rank can achieve three orders of magnitude reduction compared to eigenvalue decomposition in terms of time.
...
Due to the size of the strategy space, we considered both α α -Rank and α α -Oracle.

We set α α -Oracle to run 200 iterations of gradient updates in solving the top-rank strategy profile (Phase I in Algorithm 1).
...
We also note that though such sizes are feasible using α-Rank and the power-method, our results achieve 4 orders of magnitude reduction in total number of iterations.
...
with h j and λ being constant coefficients.

The probability of one set of spin configuration is P (a) = exp(−E(a,h)/τ ) a exp(−E(a,h)/τ ) where τ is the environmental temperature.
...
An interesting phenomenon is the phase change, i.e., the spins will reach an equilibrium in the low temperatures, with the increasing τ , such equilibrium will suddenly break and the system becomes chaotic.

Here we try to observe the phase change through multi-agent evaluation methods.

We assume each spins as an agent, and the reward to be r j = h j a j + λ 2 k =j a j a k , and set α = 1 τ to build the link between Eqn.

1 and P (a).

We consider the top-rank strategy profile from α α -Oracle as the system equilibrium and compare it against the ground truth from MCMC.

We consider a five-by-five 2D model which induces a prohibitively-large strategy space of size 2 25 (tens of millions) to which the existing baselines, including α α -Rank on the single machine, are inapplicable.

Fig. 4(b) illustrates that our method identifies the same phase change as what MCMC suggests.

We show an example of how α α -Oracle's top-ranked profile finds the system equilibrium in Fig. 4 (c) at τ = 1.
...
Indeed, two vertices π [k] ,π [k] ∈ G are connected by the edge if and if only these joint strategy profiles differ in at most one individual strategy, i.e ∃!i ∈ {1, . . .
...
all randomness of a stochastic gradient implementation, and γ > 1 is a decay-rate for λ, i.e., λ Sample i t ∼ [1, . . .

, n] and compute:

The above result implies given precision parameter > 0, after T = O 1 2 iterations, Algorithm 2 outputs vectorx λ 0 such that:
...
For entries j ∈ N t , all entries [x t+1 ] j might be different ( worst case ) and, therefore, update The transitional probability matrix T in α α -Rank is sparse; each row and column in T [k] contains N i=1 s i − N + 1 non-zero elements (see Section 3.2).
...
Our algorithm provides the expected ranking in all three normal-form games shown in Fig. 6 , which is consistent with the results in α-Rank .
...
We also implement infinite α , when calculating transition matrix (or its column), where our noise term is set to be 0.01.

For most of our experiments that involve α α -rank, we set the terminating condition to be, when the gradient norm is less than 10 −9 .
...
For all of the Adam experiments, after the warmup-step we chooses to decay δ and λ by 0.999 for each time steps, where we have δ to always be 0.1.

Similarly, λ starts at the value 0.5.
