...
Among them, Π model and temporal ensembling are widely used (Laine & Aila, 2016) .

They have defined new loss functions for unlabeled data.

The Π model outputs f (x) andf (x) for the same input x by perturbing the input with different random noise and using dropout BID8 , and then minimizes DISPLAYFORM0 2 ) between these output values.
...
I S ← I S ∪ {i} DISPLAYFORM0 retrain f n (·; θ n ), f cl (·; θ c ) and f sel (·; θ s ) for K re epochs using T 19: until stopping criterion is true
...
The output of the backbone network is fed into two sibling fully-connected layers -a classification network f cl (·; θ c ) and a selection network f sel (·; θ s ), where θ c and θ s are learnable parameters for each of them.

In this paper, we define the classification result and the selection score as r i = f cl (f n (x i ; θ n ); θ c ) and DISPLAYFORM0 respectively, where f n (·; θ n ) denotes the backbone network with learnable parameters θ n .
...
Then, we annotate the selected samples with the pseudo-categories evaluated by the classification network and construct a new training dataset T composed of L and U S = {(x i ,ŷ i )|i ∈ I S }.
...
Second, inspired by (Laine & Aila, 2016), we also utilize multiple previous network evaluations of unlabeled instances by updating the ensemble score z i = αz i + (1 − α)s i , where α is a momentum weight for the moving average of ensemble scores.
...
In this case, if the ensemble score z i exceeds 1 − , the pseudo-label of the unlabeled sampleŷ i is set to the classification result r i .

When we construct a new training dataset, we keep the number of samples of each class the same.

The reason is that if one class dominates the others, the classification performance is degraded by the imbalanced distribution (FernáNdez et al., 2013) .
...
The momentum weight for the ensemble of selection scores is set to α = 0.5.
...
The test errors in the SST algorithm were 23.15% and 15.72%, the SST has better performance than Π model but worse than Mean Teacher in 1k test.

In 2k test, the SST has better performance than Π model and similar with Mean Teacher.

Table 6 : Classification error on CIFAR-10 (1k and 2k Labels) with 5 runs using in-class unlabeled data Method CIFAR-10 (1k) CIFAR-10 (2k) supervised (sampled)38.71 ± 0.47% 26.99 ± 0.79% Π model (Laine & Aila, 2016) 27.36 ± 1.20% 18.02 ± 0.60% Mean Teacher BID9 21.55 ± 1.48% 15, 73 ± 0.31%Proposed method (SST) 23.15 ± 0.61% 15.72 ± 0.50%
