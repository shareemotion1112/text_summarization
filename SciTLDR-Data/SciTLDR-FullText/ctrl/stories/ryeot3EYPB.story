...
When using UDA method, we also try the three different annealing strategies introduced in the UDA paper (Xie et al., 2019) , and the λ in (1) is always set as 1.
...
We tune the temperature of µ of the softmax distribution.

As discussed in Xie et al. (2019) , this controls the trade-off between quality and diversity.

When µ = 0, the sampling reduces to the greedy search and produce the best quality samples.

When µ = 1, the sampling produces diverse outputs but loses some semantic information.

The table A.4 illustrates the influence of µ value to the final performance in the English-to-French and English-to-German settings.
