...
We use the following notation: The set describing the pool is denoted by P, the classifier at each AL cycle k is denoted by θ k .
...
Minimal distance sampling using SVM reads as DISPLAYFORM1 where w and b define the separating hyper-plane and φ(·) is a feature map, e.g. induced by a SVM kernel or a neural network.
...
We use a labelled data set (X k , Y k ) to train the classifier θ k .

Then, we use the trained classifier θ k and the generator G to produce uncertain samplesx.
