...
These methods use maximum likelihood to train a parameterized policy π θ : S → A, where S is the state space and A is the action space, such that θ * = arg max θ N log π θ (a i |s i ).
...
Similar to Generative Adversarial Networks BID5 , GAIL has two networks, a policy network π θ : S → A and a discriminator network D ψ : S × A → [0, 1].

GAIL uses a similar min-max objective function as in GANs: DISPLAYFORM1 where π E denotes the expert policy that generated the demonstration trajectories.

This learning objective encourages the policy π θ to have an occupancy measure close to the expert policy π E .In practice, we train π θ with policy gradient methods to maximize the discounted sum of the reward function r gail (s t , a t ) = − log(1 − D ψ (s t , a t )), clipped by a max value of 10.
...
Setting λ to either 0 or 1 reduces this method to the standard RL or GAIL setups.
...
PPO uses a learnable value function V φ to estimate the advantage for policy gradient.

During training, each PPO worker executes the policy for K steps and uses the discounted sum of rewards and the value as an advantage function estimatorÂ t = DISPLAYFORM0 , where γ is the discount factor.
...
Rather than using pixel inputs as the policy network, we take advantage of the low-level physical states (e.g., the position and velocity of the 3D objects and the robot arm) to train the value V φ with a smaller multilayer perceptron.
...
These baselines use the same setup as the full model, except that we set λ = 0 for RL and λ = 1 for GAIL, while our model uses a balanced contribution of the hybrid reward, where λ = 0.5.
...
We then examine the model's sensitivity to the λ values in Eq. 2.

We see in Fig. 5b that, our model works well with a broad range of λ values from 0.3 to 0.7 that provide a balanced mix of the RL and GAIL rewards.
