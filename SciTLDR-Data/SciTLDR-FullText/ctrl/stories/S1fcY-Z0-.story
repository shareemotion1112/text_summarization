...
With this in mind, we propose learning an extremely flexible and powerful posterior, parametrized by a DNN h, which we refer to as a Bayesian hypernetwork in reference to BID17 .A Bayesian hypernetwork (BHN) takes random noise ∼ N (0, I) as input and outputs a sample from the approximate posterior q(θ) for another DNN of interest (the "primary network").

The key insight for building such a model is the use of an invertible hypernet, which enables Monte Carlo estimation of the entropy term − logq(θ) in the variational inference training objective.
...
Broadly speaking, existing methods either 1) use Markov chain Monte Carlo BID39 BID30 , or 2) directly learn an approximate posterior distribution using (stochastic) variational inference BID16 BID9 BID2 , expectation propagation BID19 BID36 , or α-divergences BID27 .
...
This has the advantages of being simple to implement and allowing cheap samples from q(θ).
...
Bayes by Backprop can be viewed as a special instance of a Bayesian hypernet, where the hypernetwork only performs an element-wise scale and shift of the input noise (yielding a factorial Gaussian distribution).More similar is the work of BID28 , who propose and dismiss BHNs due to the issues of scaling BHNs to large primary networks, which we address in Section 3.3.2 Instead, in their work, they use a hypernet to generate scaling factors, z on the means µ of a factorial Gaussian distribution.

Because z follows a complicated distribution, this forms a highly flexible approximate posterior: q(θ) = q(θ|z)q(z)dz.

However, this approach also requires them to introduce an auxiliary inference network to approximate q(z|θ) in order to estimate the entropy term of the variational lower bound, resulting in lower bound on the variational lower bound.
...
In these works, the weights of the primary net are parametrized directly, and the hypernet only outputs scale (γ) and shift (β) parameters for every neuron; this can be viewed as selecting which features are significant (scaling) or present (shifting).
...
Let θ ∈ R D be parameters given the Bayesian treatment as random variables, D a training set of observed data, and q(θ) a learned approximation to the true posterior p(θ|D).

Since the KL divergence is always non-negative, we have, for any q(θ): DISPLAYFORM0 The right hand side of FORMULA0 is the evidence lower bound, or "ELBO".The above derivation applies to any statistical model and any dataset.

In our experiments, we focus on modeling conditional likelihoods p(D) = p(Y|X ).

Using the conditional independence assumption, we further decompose log p(D|θ) := log p(Y|X , θ) as n i=1 log p(y i |x i , θ), and apply stochastic gradient methods for optimization.

Computing the expectation in (2) is generally intractable for deep nets, but can be estimated by Monte Carlo sampling.

For a given value of θ, log p(D|θ) and log p(θ) can be computed and differentiated exactly as in a non-Bayesian DNN, allowing training by backpropagation.

The entropy term E q [− logq(θ)] is also straightforward to evaluate for simple families of approximate posteriors such as Gaussians.
...
Bayesian hypernets (BHNs) express a flexible q(θ) by using a DDGN (section 2.3), h ∈ R D → R D , to transform random noise ∼ N (0, I D ) into independent samples from q(θ).
...
Since BHNs are both trained and evaluated via samples of q(θ), expressing q(θ) as a generative model is a natural strategy.

However, while DDGNs are convenient to sample from, computing the entropy term (E q [− logq(θ)]) of the ELBO additionally requires evaluating the likelihood of generated samples, and most popular DDGNs (such as VAEs and GANs) do not provide a convenient way of doing so.
...
In order to scale BHNs to large primary networks, we use the weight normalization reparametrization 6 : DISPLAYFORM0 where θ j are the input weights associated with a single unit j in the primary network.
...
For this experiment, we trained an over-parametrized linear (primary) network:ŷ = a · b · x on a dataset generated as y = x + , and the BHN learns capture both the modes of a = b = 1 and a = b = −1.
...
We choose an isotropic Gaussian prior for g: p(g) = N (g; 0, λI), which results in an L 2 weight-decay penalty on g, or, equivalently, w = g ≥ E ∼q ( ),g=h φ ( ) [log p(D|g; v, b) + log p(g) − log q( ) + log det DISPLAYFORM0 where v and b are the direction and bias parameters of the primary net, and φ is the parameters of the hypernetwork.

We optimize this bound with respect to {v, b, φ} during training.
