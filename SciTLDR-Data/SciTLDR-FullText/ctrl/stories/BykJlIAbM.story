...
We follow the payoff distribution to define our inadaptive cluster: DISPLAYFORM0 where R(Y, Y * ) denotes the task-level reward (BLEU, CIDEr, METEOR, etc) and R(Y, Y * ) denotes its normalization in the whole output space, τ is the hyper-parameter temperature to control the smoothness of the optimal distribution around correct target Y * .
...
These four systems exhibit different characteristics which are shown in details as below: In a slight abuse of notation, we will denote DISPLAYFORM0 System-A For system-A, we use inadaptive cluster with parallel sampling strategy to train the NMT model, and the forward-backward joint objective functions is defined as: DISPLAYFORM1 Formally, the derivative respect to η and γ are shown as: DISPLAYFORM2 Parallel candidates are sampled from source and target cluster distributions are leveraged by scaled translation scores w(X|Y, Y * ), w(Y |X, X * ) during optimization.
...
During training, ADADELTA (Zeiler, 2012 ) is adopted with = 10 −6 and ρ = 0.95 to separately optimize the NMT's and adaptive cluster's parameters.
...
To prevent too much hyper-parameter tuning in building the inadaptive cluster, we follow to select the best temperature τ = 0.8 in all experiments.
...
In system-A and C, we simplify DISPLAYFORM8 pη(Y |X * ) .
