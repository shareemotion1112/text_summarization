...
Traditionally, graphs are generated based on some network generation model such as the Erdős-Rényi model.
...
The Erdős-Rényi model (Erdős & Rényi, 1960) , the preferential attachment model (Albert & Barabási, 2002) , and the Kronecker graph model (Leskovec et al., 2010) are some typical examples of such models.
...
We define a permutation of the nodes of G as a bijective function π : V → V , under which any graph property of G should be invariant.

We are interested in the topology of a graph which is described by its adjacency matrix A π ∈ R n×n with a specific ordering of the nodes π 1 .
...
where the W and U matrices are trainable weight matrices, σ is the sigmoid function, and r v and z v are the parameters of the reset and update gates for a given node.
...
Formally, given a node ordering π, it considers a graph G as a sequence of vectors:

where S π i = [a 1,i , . . . , a i−1,i ] ∈ {0, 1} i−1 is the adjacency vector between node π(i) and the nodes preceding it ({π(1), . . . , π(i − 1)}).
...
Since a graph can be expressed as a sequence of adjacency vectors (given a node ordering), we can consider instead the distribution p(Ŝ π ; θ), which can be decomposed in an autoregressive manner into the following product:

This product can be parameterized by a neural network.

Specifically, following (You et al., 2018) , we use a hierarchical RNN consisting of two levels: (1) the graph-level RNN which maintains the state of the graph and generates new nodes and thus learns the distribution p(Ŝ π i |Ŝ π k:k<i ) and (2) the edge-level RNN which generates links between each generated node and previously-generated nodes and thus learns the distribution p(â π ji |â π li:l<j ).
...
The resulting value is then squashed by a sigmoid function to produce the probability of existence of an edgeâ j,i .
...
Node ordering It should be mentioned that node ordering π has a large impact on the efficiency of the above generative model.
...
We compare EvoNet against several random graph models: (1) the Erdős-Rényi model (Erdős & Rényi, 1960) , (2) the Stochastic Block model (Holland et al., 1983; Airoldi et al., 2008) , (3) Barabási-Albert model (Albert & Barabási, 2002) , and (4) the Kronecker Graph model (Leskovec et al., 2010) .
...
For instance, in the case of the Erdős-Rényi model, the probability of adding an edge between two nodes is set to some value such that the density of the generated graph is identical to that of the ground-truth graph.
