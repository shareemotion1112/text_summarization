...
a l−1 denotes the activation of the previous layer, B the random binary weight matrix, τ is the temperature used for the concrete distribution, f (·, ·) the linear transformation used in the layer, > 0 a small constant for numerical stability, D the dimensionality of the inner product in f , and γ & β are the parameters for batch normalization.

DISPLAYFORM0 // Max pooling if max pooling required then n ∼ N (0, I); s = µ + σ n; ι = max-pooling-indices(s); µ, σ 2 = select-at-indices(µ, σ 2 , ι); end // Binarization and sampling DISPLAYFORM1 In this section we introduce the probabilistic setting of the PBNet.
...
Specifically, by optimizing the parameters of the weight distributions, we optimize a bound on the actual loss: min DISPLAYFORM2 where B are the binary weights of the network and q θ (B) is a distribution over the binary weights.

For q θ (B) a slight reparametrization of the Bernoulli distribution is used, which we will refer to as the Binary distribution.

This distribution is parameterized by θ ∈ [−1, 1] and is defined by: DISPLAYFORM3 For the properties of this distribution, please refer to Appendix A.We will now consider using the Binary distribution for both the weights and the activations in a Neural Network.
...
Let w ∼ Binary(θ) and h ∼ Binary(φ) be the weight and input random variable for a given layer.
...
Specifically, let a i ∼ N (µ i , σ 2 i ) be a random pre-ctivation obtained using the normal approximation, as introduced in the previous section, then the activation (after binarization) is again given as a Binary random variable".

Interestingly, the Binary probability can be computed in closed form by evaluating the probability density that lies above the binarization threshold: DISPLAYFORM1 where Φ(·|µ, σ 2 ) denotes the CDF of N (µ, σ 2 ).

Applying the binarization function to a random pre-activation has two advantages.

First, the derivatives ∂q i /∂µ i and ∂q i /∂σ i are not zero almost everywhere, in contrast to the derivatives of b det and b stoch when applied to a deterministic input.
...
From the Lyapunov CLT it follows that the approximation to the distribution of the pre-activation is given by: DISPLAYFORM0 where w ∼ Binary(θ) is a random weight.
...
Batch Normalization (BN) BID8 -including an affine transformation -is defined as follows: DISPLAYFORM0 where a i denotes the pre-activation before BN,â the pre-activation after BN, and m & v denote the sample mean and variance of DISPLAYFORM1 , for an M -dimensional pre-activation.
...
Instead, to ensure a standardized distribution over activations, we compute the expected population mean and variance under the pre-activation distribution: DISPLAYFORM2 where M is the total number of activations and a i ∼ N (µ i , σ i ) are the random pre-activations.
...
Instead, we sample one of the input random variables in every spatial region according to the probability of that variable being greater than all other variables, i.e., ρ i = p(a i > z \i ), where z \i = max({a j } j =i ).

ρ i could be obtained by evaluating the CDF of (z \i − a i ) at 0, but to our knowledge this has no analytical form.

Alternatively, we can use Monte-Carlo integration to obtain ρ: DISPLAYFORM0 where one-hot(i) returns a K-dimensional one-hot vector with the ith elements set to one.

The pooling index ι is then sampled from Cat(ρ).

However, more efficiently, we can sample s ∼ p(a 1 , . . . , a K ) and select the index of the maximum in s, which is equivalent sampling from Cat(ρ).
...
For the PBNet the parameters θ for q θ (B) are initialized from a uniform U (−1, 1) distribution.

Although the final parameter distribution more closely follows a Beta(α, α) distribution, for α < 1, we did not observe any significant impact choosing another initialization method for the PBNet.
...
PBNet-map refers to a deterministic PBNet using the map estimate, PBNet-Ternary is a ternary deterministic network obtained from q θ , and PBNet-X refers to an ensemble of X networks, each sampled from the same weight distribution.
