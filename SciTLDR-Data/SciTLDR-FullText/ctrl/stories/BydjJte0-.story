...
kennen-o learns a classifier m θ to map from the order-sensitively concatenated n query outputs, [f (x i )] i=1···n (n × 10 dim for MNIST), to the simultaneous prediction of 12 attributes in f .
...
With the learned parameterθ, m DISPLAYFORM1 gives the prediction of attribute a for the black box g.

In our experiments, we model the classifier m θ via multilayer perceptron (MLP) with two hidden layers with 1000 hidden units.
...
With learned parametersθ and the attribute a for the black box g is predicted by m DISPLAYFORM7 DISPLAYFORM8 .
...
To improve stability against covariate shift, we initialise m θ with kennen-o for 200 epochs.

Afterwards, gradient updates of DISPLAYFORM9 and θ alternate every 50 epochs, for 200 additional epochs.
