...
During metatraining the UnsupervisedUpdate (parameterized by θ) is itself updated by gradient descent on the MetaObjective.
...
Instead at each layer, and for each neuron, a learning signal is injected by a meta-learned MLP parameterized by θ, with hidden state h. Weight updates are again analogous to those in backprop, and depend on the hidden state of the pre-and postsynaptic neurons for each weight.
...
We consider a multilayer perceptron (MLP) with parameters φ t as the base model.
...
The parameters φ t of the base model are then updated iteratively by performing SGD using the gradient ∂l(x,y) ∂φt .

This supervised update rule can be written as φ t+1 = SupervisedUpdate(φ t , x t , y t ; θ), where t denotes the inner-loop iteration or step.

Here θ are the meta-parameters of the optimizer, which consist of hyper-parameters such as learning rate and momentum.

In this work, our learned update is a parametric function which does not depend on label information, φ t+1 = UnsupervisedUpdate(φ t , x t ; θ).

This form of the update rule is general, it encompasses many unsupervised learning algorithms and all methods in Section 2.1.In traditional learning algorithms, expert knowledge or a simple hyper-parameter search determines θ, which consists of a handful of meta-parameters such as learning rate and regularization constants.

In contrast, our update rule will have orders of magnitude more meta-parameters, including the weights of a neural network.

We train these meta-parameters by performing SGD on the sum of the MetaObjective over the course of (inner loop) training in order to find optimal parameters θ * , DISPLAYFORM0 that minimize the meta-objective over a distribution of training tasks.

Note that φ t is a function of θ since θ affects the optimization trajectory.
...
Additionally, code and meta-trained parameters θ for our meta-learned UnsupervisedUpdate is available 1 .
...
In practice, we relax this constraint and incorporate some cross neuron information to decorrelate neurons (see Appendix G.5 for more information).To build these updates, each neuron i in every layer l in the base model has an MLP, referred to as an update network, associated with it, with output h DISPLAYFORM0 ; θ where b indexes the training minibatch.

The inputs to the MLP are the feedforward activations (x l & z l ) defined above, and feedback weights and an error signal (V l and δ l , respectively) which are defined below.

All update networks share meta-parameters θ.
...
As in supervised learning, an error signal δ l bi is then propagated backwards through the network.

Unlike in supervised backprop, however, this error signal is generated by the corresponding update network for each unit.

It is read out by linear projection of the per-neuron hidden state h, δ l bi = lin h l bi , and propogated backward using a set of learned 'backward weights' (V l ) T , rather than the transpose of the forward weights (W l ) T as would be the case in backprop (diagrammed in Figure 1 ).
...
We choose to meta-optimize via SGD as opposed to reinforcement learning or other black box methods, due to the superior convergence properties of SGD in high dimensions, and the high dimensional nature of θ.
...
Training takes ∼8 days, and consists of ∼200 thousand updates to θ with minibatch size 256.
...
For all remaining experimental results, unless otherwise stated, we use meta-parameters, θ, for the UnsupervisedUpdate resulting from 200 hours of meta-training.
...
Each pane consists of first layer filters extracted from φ after 10k applications of UnsupervisedUpdate on MNIST (top) and CIFAR10 (bottom).
...
The base model itself is a fully connected network producing hidden states x l for each layer l. The "backward" pass through the base model uses an error signal from the layer above, δ, which is generated by a meta-learned function.

e.) The weight updates ∆φ are computed using a convolutional network, using δ and x from the pre-and post-synaptic neurons, along with several other terms discussed in the text.
...
When training with truncated backprop the problem shifts from pure optimization to something more like optimizing on a Markov Decision Process where the state space is the base-model weights, φ, and the 'policy' is the learned optimizer.
...
Next, each worker samples k unrolling steps, does k applications of the UnsupervisedUpdate(·; θ), computes the MetaObjective on each new state, computes DISPLAYFORM0 and sends this gradient to a parameter server.

The final base-model state, φ, is then used as the starting point for the next unroll until the specified number of steps is reached.

These gradients from different workers are batched and θ is updated with asynchronous SGD.
...
Training occurs over the course of ∼8 days with ∼200 thousand updates to θ with minibatch size 256.
...
In the following sections, λ name denotes a hyper-parameter for the learned update rule.
...
We then compute weight updates: DISPLAYFORM2 Finally, the next set of φ forward weights (W l ), biases (b l ), and backward weights (V l ), are computed.
...
First, the inputs are converted to embeddings with the base model, DISPLAYFORM4 x L =f (x ; φ) (App.19) Next, we center and normalize the prediction targets.

We show this for y, but y is processed identically.ȳ DISPLAYFORM5 We then solve for the linear regression weights in closed form with features: x L and targets:ŷ.
...
The learned update rule is parameterized by θ.

In the following section, we denote all instances of θ with a subscript to be separate named learnable parameters of the UnsupervisedUpdate.

θ is shared across all instantiations of the unsupervised learning rule (shared across layers).

It is this weight sharing that lets us generalize across different network architectures.

The computation is split into a few main components: first, there is the forward pass, defined in f (x; φ).

Next, there is a "backward" pass, which operates on the hidden states in a reverse order to propagate an error signal δ l back down the network.

In this process, a hidden state h l is created for each layer.

These h are tensors with a batch, neuron and feature index: h l ∈ R B,N l ,λ hdims where λ hdims = 64.
...
Because we are no longer restricted to the form of backprop, we make this quantity a vector for each unit in the network, rather than a scalar, DISPLAYFORM0 In this work we set λ deltadims = 32.

The architecture of TopD is a neural network that operates along every dimension of x L .

The specification can be found in G.7.We structure our error propagation similarly to the structure of the backpropagated error signal in a standard MLP with contributions to the error at every layer in the network, DISPLAYFORM1 where δ l has the same shape as d l , and θ errorP ropW ∈ R λ hdims ,λ deltadims and θ errorP ropB ∈ R λ deltadims .

Note both θ errorP ropW and θ errorP ropB are shared for all layers l.
...
Additionally, we normalize the signal to have fixed second moment,d DISPLAYFORM2 The internal h l ∈ R B,N l ,λ hdims vectors are computed via: DISPLAYFORM3 The architecture of ComputeH is a neural network that operates on every dimension of all the inputs.
...
These terms are then added together with a learnable weight, in θ to form a final update.

The final update is then normalized mixed with the previous weights.

We update both the forward, W l , and the backward, V l , using the same update rule parameters θ.
...
DISPLAYFORM1 Here, Θ, is a placehoder for the parameters of the given readout.

LowRR is defined as: DISPLAYFORM2 where λ gradc = 4 and is the rank of the readout matrix (per batch element).
...
Ŵ DISPLAYFORM1 Additional weight update planes are designed to aid units in remaining decorrelated from each other's activity, and in decorrelating their receptive fields.
...
We only allow updates which do not decrease the weight matrix magnitude, DISPLAYFORM1 whereŴ l is W l scaled to have unit norm, and we normalize the length of the update, DISPLAYFORM2 To compute changes in the biases, we do a readout from h l .
...
DISPLAYFORM3 where θ Breadout ∈ R λ hdims .We then normalize the update via the second moment: DISPLAYFORM4 Finally, we define ComputeDeltaWeight as all layer's forward weight updates, and backward weight updates, and bias updates.
...
They are all separate elements of θ topD .
...
DISPLAYFORM8 We set λ topdeltasize = 64.
...
ComputeH (·; θ computeH ) = m 11 (App.104)We set λ computehsize = 64 which is the inner computation size.
...
We chose this based on the expected distribution of the training step, φ iteration number, across the cluster of workers.
