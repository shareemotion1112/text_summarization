...
That is, for a single dimension of a fully connected layer , weight normalization reparameterizes w as w = g(θ/||θ||), where θ is a vector of the same length as w and g is a scalar.

The network now learns g, θ instead of w. For FGL we apply weight norm on both u and v. Weight norm is applied by treating u as c out different vectors and v as the weights of a fully connected layer.
...
Here,ŷ is the vector of predicted probabilities for each class or label.
...
That is, z|x ∼ N (F x, Σ), for a fixed covariance matrix Σ, and a matrix F of size k × s 2 where k is the number of labels.
...
Additionally, if Σ is an identity matrix, then each dimension of z corresponds to the sum of values of a group of pixels.
