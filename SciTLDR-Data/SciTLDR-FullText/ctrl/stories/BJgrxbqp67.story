...
TRE models the conditional probability by an output distribution over target tokens: DISPLAYFORM1 where h L is the sequence of states after the final layer L, W e is the embedding matrix, and θ are the model parameters that are optimized by stochastic gradient descent.
...
Therefore, we adopt a similar objective: DISPLAYFORM3 where λ is the language model weight, a scalar, weighting the contribution of the language model objective during fine-tuning.
...
Therefore, unless specified otherwise, we used the Adam optimization scheme BID7 with β 1 = 0.9, β 2 = 0.999, a batch size of 8, and a linear learning rate decay schedule with warm-up over 0.2% of training updates.
