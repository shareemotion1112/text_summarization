...
However, it has been noted that the value of the 1 regularizer is proportional to the scaling of parameters (i.e. ||αW || 1 = |α|·||W || 1 ), so it "scales down" all the elements in the weight matrices with the same speed.

This is not efficient in finding sparsity and may sacrifice the flexibility of the trained model.

On the other hand, the 0 regularizer directly reflects the real sparsity of weights and is scale invariant (i.e. ||αW || 0 = ||W || 0 , ∀α = 0), yet the 0 norm cannot provide useful gradients.
...
Moreover, the Hoyer regularizer is scale-invariant, i.e. R(αX) = R(X), because both the 1 norm and the 2 norm are proportional to the scale of X. The gradients of the Hoyer regularizer are purely radial, leading to "rotations" towards the nearest axis.
...
holds for ∀α = 0, so as the 0 norm.
...
α n , α c and β are pre-selected weight decay parameters for the regularizers.
...
Here we assume that w j belongs to group w (ĝ) .
...
The weight decay parameters (αs in Equation (7) and (8)) are picked by hand to reach the best result.
