...
The baseline is formalized as followŝ DISPLAYFORM0 where, as before, f is either the sample mean or the sample median depending on the evaluation metric reported.
...
The probability for a fact DISPLAYFORM0 c g(c|θ)) , where c indexes all possible triples, and θ all learnable parameters of TransE, whose scoring function g is g(d | θ) = ||h + p − t|| 2 .
...
For a given set of facts D that are part of the KB G, the logarithmic loss is defined as DISPLAYFORM1 All parameters θ are learned for minimizing L G with stochastic gradient descent.
...
For numerical attribute a the loss function is given by DISPLAYFORM0 where f ϑ a refers to the regression function for numerical attribute a, ϑ a refers to the learnable parameters of f ϑ a , and λ a is the regularization hyper-parameter.

In this work we use a linear regression model: f ϑ a (e) = e T w a + b a , where w a ∈ d is the weight vector and b a is the corresponding bias term.

At test time, given a query related to a certain numerical attribute a and a certain entity e, the prediction is computed by applying the corresponding linear regression model:n a e = f ϑ a (e).
...
The edge weights of the adjacency matrix represent similarities between the connected entities, which are computed according to a similarity metric ρ -in this work we use a radial basis function kernel 2 .We then compute the transition matrix T by row-wise normalizing the matrix A. Without loss of generality, we arrange labeled and unlabeled data so that T can be decomposed as DISPLAYFORM0 2.
...
Instead we use the learned TransE++ embeddings to train a new regression model L a R for each numerical attribute a ∈ A. This is because of the computational difficulty in tuning hyper parameter λ a for each numerical attribute while learning TransE++, which we found important to obtain good performance.

Note that the hyper parameter space grows exponentially with the number of attributes |A|.

For this reason, we set λ a = λ = 0 in the regression objectives in Eq. (7) for learning TransE++ embeddings (first step).

For the final regression models (second step) we do tune λ a s (independently), which, though suboptimal, facilitates their tuning.
...
After some preliminary experiments, the weight α of TransE++ was fixed to 1.
...
The regularization term λ a is tuned using the values [0, 0.1, 1, 10, 100].For Nap and Nap++ the number of neighbors (k) of the kNN graph is validated among [3, 5, 10, 20] ; and the σ of the RBF kernel is validated among [0.25, 0.5, 1, 10] .All of the above is validated for each numerical predicate and evaluation metric.
...
As we argued in Section 3.2.3 it is not feasible to validate the regularization term λ a for every numerical attribute while learning TransE++.

We speculate that setting λ a = 0 while training TransE++ may explain why Lr++ and Nap++ do not always beat their counterparts.
