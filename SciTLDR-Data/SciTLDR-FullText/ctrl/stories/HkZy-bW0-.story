...
In other words, given a sequence of inputs x 0 ...x t with x 0 = 0 and weights w 1 ...w t :t τ =1 (x τ − x τ −1 ) · w τ = x t · w t unless w t = w 0 ∀t.
...
The Σ function, defined in Equation 2, returns a running sum of the inputs over calls.

So we can write, for example, that when our composition of functions (Σ • ∆) is called with a sequence of input variables x 1 ... DISPLAYFORM0 In general, when we write y t = f (x t ), where f is a function with persistent state, it will be implied that we have previously called f (x τ ) for τ ∈ [1, .., t − 1] in sequence.
...
DISPLAYFORM4 To derive our decoding formula, we can simply solve for x t as x t = DISPLAYFORM5 DISPLAYFORM6 We can expand this recursively to see that this corresponds to a temporal convolution a * κ where κ is a causal exponential kernel DISPLAYFORM7
...
We can show that Q(x t ) = (∆ • R • Σ)(x t ) (see Supplementary Material Section C), where ∆ • R • Σ indicates applying a temporal summation, a rounding, and a temporal difference, in series.
...
Row 2: We efficiently approximate z t by taking the temporal difference, multiplying by w t then temporally integrating, to produceẑ t , as described in Section 2.4.

As the weight w t changes over time, our estimateẑ diverges from the correct value.
...
DISPLAYFORM2 The cost of computingẑ t , on the other hand, depends on the contents of s t .

If the data is temporally redundant, s t ∈ Z din should be sparse, with total magnitude N i |s t,i |.

s t can be decomposed into a sum of one-hot vectors DISPLAYFORM3 where γ in is a length-d in one-hot vector with element (γ in ) in = 1.
...
Assuming the n |s t,n | · d out term dominates, we can say that the relative cost of computingẑ t vs z t is: DISPLAYFORM5 2.5 A NEURAL NETWORKWe can implement this encoding scheme on every layer of a neural network.
...
Middle: Another signal, representing the postsynaptic gradient of the error e = ∂L ∂z l , along with its quantized (ē) and reconstructed (ê) variants.

Bottom:The true weight gradient ∂L ∂wt and the reconstruction gradient ∂L ∂wt .

At the time of the spike inē t , we have two schemes for efficiently computing the weight gradient that will be used to increment weight (see Section 2.6).

The past scheme computes the area underx ·ê since the last spike, and the future scheme computes the total future additional area due to the current spike.parameter update for weight matrix w has the form w ← w − η ∂L ∂w where η is the learning rate.
...
We can start by applying our encoding-quantizing-decoding scheme to our input and error signals asx DISPLAYFORM2 and approximate our true update as ∂L ∂w recon,tx t ⊗ê t wherex t dec(x t ) andê t dec(ē t ).

This does not do any good by itself, because the vectors involved in the outer product,x t andê t , are still not sparse.
...
For a given synapse w i,j , if either the presynaptic neuron spikes (x ti = 0) or the postsynaptic neuron spikes (ē ti = 0), we increment the w i,j by the total area underx τ,iêτ,j since the last spike.

We can do this efficiently because between the current time and the time of the previous spike,x τ,iêτ,j is a geometric sequence.
...
STDP BID10 Using the quantized input signalx and error signalē, and their reconstructionsx t andê t as defined in the last section, we define a causal convolutional kernel DISPLAYFORM0 .
...
In Section D in the supplementary material we show experimentally that while Equations may all result in different updates at different times, the rules are equivalent in that for a given set of pre/post-synaptic spikesx,ē, the cumulative sum of their updates over time converges exactly.
...
φ: The internal state variable of the quantizer Q. enc: An "encoding" operation, which takes a signal and encodes it into a combination of the signal's current value and its change in value since the last time step.
...
∂L ∂ẑ l ∈ R d l Is our approximation to the derivative of the loss of our network with respect toẑ l , which is itself an approximation of the activation z l in a non-spiking implementation of the network.
...
e t ∂L ∂ẑ l,t ∈ R dout is simply a shorthand for "the approximate backpropagated gradient at layer l" (x t andē t ) are the encoded and quantized versions of signals (x t and e t ) (x t andê t ) are the reconstructed versions of signals (x t and e t ), taken from the quantized (x t andē t ) ∂L ∂w recon,tx t ⊗ê t ∈ R din×dout is the approximate gradient of weight matrix w, as calculated by taking the outer product of the (input, error) reconstructions,x,ê. (see Figure 9 ).
...
DISPLAYFORM0 A reparametrization of k p , k d in terms of the memory in our decoder k α and the scaling of our encoded signal (k β ).
...
C SIGMA-DELTA UNWRAPPINGHere we show that Q = ∆ • R • Σ, where Q, ∆, R, Σ are defined in Equations 3, 2, 6, 1, respectively.
...
To simplify our expressions in the update algorithms, we re-parametrize our k p , k d coefficients as DISPLAYFORM0 Equation FORMULA15 shows how we make two approximations when approximating z t = x t · w t withẑ t = (dec • w • Q • enc)(x t ).
...
Instead of directly setting k p , k d as hyperparameters, we fix the ratio k α DISPLAYFORM4 , and adapt the scale k β 1 kp+k d to the magnitude of the signal.

Our update rule for k β is: DISPLAYFORM5 Where η k is the scale-adaptation learning rate, µ t is a rolling average of the L 1 magnitude of signal x t , and k rel β defines how coarse our quantization should be relative to the signal magnitude (higher means coarser).

We can recover k p , k d for use in the encoders and decoders as k p = (1 − k α )/k β and k d = k α /k β .

In our experiments, we choose η k = 0.001, k rel β = 0.91, k alpha = 0.91, and initialize µ 0 = 1.
...
For all experiments, the PDNN started with k α = 0.5, and this was increased to k α = 0.9 after 1 epoch (see Appendix A for the meaning of k α ).
