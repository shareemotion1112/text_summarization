...
are the probability over the next latent state z , given the current observation x and action u, in (SOC2) and (SOC3) (see the paths x t → z t →z t+1 and x t → z t →z t+1 →x t+1 →ẑ t+1 in Figures 1(b)(green) and 1(c)(red)).
...
Let (U * 3 , P * 3 ) be a solution to (SOC3) and (U * 2 , P * 2 ) be a solution to (SOC2) with R 2 ( P ) and and λ 2 defined by (3) and (2).
...
where = ( z , u ) ∼ N (0, δ 2 I), δ > 0 is a tunable parameter that characterizes the "diameter" of latent state-action space in which the latent dynamics model has low curvature.

, where 1/X is the minimum non-zero measure of the sample distribution w.r.t.

X , and 1 − η ∈ [0, 1) is a probability threshold.

Lemma 4 (see Appendix A.5, for proof and discussions on how δ affects LLC performance) shows that a solution of (SOC-LLC) has similar performance to a solution of (SOC1, and thus, (SOC-LLC) is a reasonable optimization problem to learn P , and also the latent space Z.
...
Solving (SOC-LLC) this way is an approximation, in general, but is justified, when the regularization parameter λ LLC is large.

Note that we leave the regularization parameters (λ p , λ c , λ cur ) as hyper-parameters of our algorithm, and do not use those derived in the lemmas of this section.
...
The PCC-Model objective in (PCC-LOSS) introduces the optimization problem min P λ p R 3 ( P ) + λ c R 2 ( P ) + λ cur R LLC ( P ).
...
which holds for any choice of recognition model Q. For simplicity, we assume the recognition model employs bottom-up inference and thus factorizes as Q(z t ,ẑ t+1 |x t , x t+1 , u t ) = Q(ẑ t+1 |x t+1 )Q(z t |ẑ t+1 , x t , u t ).
...
log P (ẑt+1 | zt, ut) , and note that the Entropy H(·) and Kullback-Leibler D KL (· ·) terms are analytically tractable when Q is restricted to a suitably chosen variational family (i.e. in our experiments, Q(ẑ t+1 | x t+1 ) and Q(z t |ẑ t+1 , x t , u t ) are factorized Gaussians).

The derivation is provided in Appendix C.1.

Interestingly, the consistency loss R 2 admits a similar treatment.

We note that the consistency loss seeks to match the distribution ofẑ t+1 | x t , u t with z t+1 | x t+1 , which we represent below as

Here, P (ẑ t+1 | x t , u t ) is intractable due to the marginalization of z t .
...
Intuitively, the amortized curvature loss seeks-for any given (z, u)-to find the best choice of linear approximation induced by A(z, u) and B(z, u) such that the behavior of F µ in the neighborhood of (z, u) is approximately linear.
...
To understand the robustness of each model, we consider both deterministic (σ = 0) and stochastic scenarios.

In the stochastic case, we add noise to the system with different values of σ and evaluate the models' performance under various degree of noise.
...
where z goal is obtained by encoding the goal observation, and Q = κ · I nz , R = I nu .
...
The last inequality is due to change of measures in policy, and the last inequality is due to the facts that (i) π is a deterministic policy, (ii) dU (u t ) is a sampling policy with lebesgue measure 1/U over all control actions, (iii) the following bounds for importance sampling factor holds:
...
Using Jensen's inequality of convex function x log x, for any observation-action pair (x, u) sampled from U τ , one can show that
...
Suppose the nominal latent state-action pair {(z z z t , u u u t )} T −1 t=0 satisfies the condition: (z z z t , u u u t ) ∼ N ((z * 2,t , u * 2,t ), δ 2 I), where {(z * 2,t , u * 2,t } T −1 t=0 is the optimal trajectory of problem (SOC2).

Then with probability 1 − η, we have L(U *
...
To quantify the above condition, one can use Mahalanobis distance (De Maesschalck et al., 2000) to measure the distance of (z z z t , u u u t ) to distribution N ((z * 2,t , u * 2,t ), δ 2 I), i.e., we want to check for the condition:

for any arbitrary error tolerance > 0.

While we cannot verify the condition without knowing the optimal trajectory {(z * 2,t , u * 2,t )} T −1 t=0 , the above condition still offers some insights in choosing the parameter δ based on the trade-off of designing nominal trajectory {(z z z t , u u u t )} T −1 t=0 and optimizing R LLC .

When δ is large, the low-curvature regularization imposed by the R LLC regularizer will cover a large portion of the state-action space.

In the extreme case when δ → ∞, R LLC can be viewed as a regularizer that enforces global linearity.
...
On the other hand, when δ is small the low-curvature regularization in R LLC only covers a smaller region of the latent state-action space, and thus the loss associated with this term is generally lower (which provides a tighter performance bound in Lemma 4).
...
Proof: For simplicity, we will focus on analyzing the noiseless case when the dynamics is deterministic (i.e., Σ w = 0).
...
Using the Lipschitz property of V t+1 , for any (z, u) and (δz, δu), such that (z z z, u u u) = (z − δz, u − δu), one has the following property:

Therefore, at any arbitrary state-action pair (z,ũ), for z z z = z − δz, and u u u =ũ − δu with Gaussian sample (δz, δu) ∼ N (0, δ 2 I), the following inequality on the value function holds w.p.

1 − η:

which further implies

Now letũ * be the optimal control w.r.t.

Bellman operator T t [V t+1 ](z) at any latent statez.

Based on the assumption of this lemma, at each statez the nominal latent state-action pair (z z z, u u u) is generated by perturbing (z,ũ * ) with Gaussian sample (δz, δu) ∼ N (0, δ 2 I) that is in form of z z z =z − δz, u u u =ũ − δu.
...
Notice that here we replace η in the result in (22) with η/T .

In order to prove (23), we utilize (22) for each t ∈ {0, . . .

, T − 1}, and this replacement is the result of applying the Union Probability bound (Murphy, 2012) (to ensure (23) holds with probability 1 − η).
...
where F µ (z t , u t ) is the deterministic dynamics model and F σ F σ is the covariance of the latent dynamics system noise.

Notice that the deterministic dynamics model F µ (z t , u t ) is smooth, and therefore the following Jacobian terms are well-posed:
...
Repeat the above steps with new nominal trajectory Denote by δz t = z t − z z z t and δu t = u t − u u u t the deviations of state and control action at time step t respectively.

Assuming that the nominal next state z z z t+1 is generated by the deterministic transition F µ (z z z t , u u u t ) at the nominal state and action pair (z z z t , u u u t ), the first-order Taylor series approximation of the latent space transition is given by

To find a locally optimal control action sequence u * t = π * δz,t (δz t ) + u u u t , ∀t, that improves the cumulative cost of the trajectory, we compute the locally optimal perturbed policy (policy w.r.t.

perturbed latent state) {π * δz,t (δz t )} T −1 t=0 that minimizes the following second-order Taylor series approximation of Q t around nominal state-action pair (z z z t , u u u t ), ∀t ∈ {0, . . . , T − 1}:
...
Therefore the optimal perturbed policy π * δz,t has the following closed-form solution:
...
log P (x t+1 |x t , u t ) = log zt,ẑt+1

Where (a) holds from the log function concavity, (b) holds by the factorization Q(z t ,ẑ t+1 |x t , x t+1 , u t ) = Q(ẑ t+1 |x t+1 )Q(z t |ẑ t+1 , x t , u t ), and (c) holds by a simple decomposition to the different components.

We derive the bound for the consistency loss Consistency ( P ).

Where (a) holds by the assumption that Q(ẑ t+1 | x t+1 ) = P (z t+1 | x t+1 ), (b) holds from the log function concavity, and (c) holds by a simple decomposition to the different components.
...
Pole is resting down (SwingUp), or randomly sampled in ±π/6 (Balance).

Agent's Objective: pole's angle is within ±π/6 from an upright position.
...
Pole is randomly sampled in ±π/6.

Agent's Objective: pole's angle is within ±π/10 from an upright position.
...
Start State: 1 st pole with angle π, 2 nd pole with angle 2π/3, and 3 rd pole with angle π/3, where angle π is a resting position.

Agent's Objective: the sum of all poles' angles is within ±π/6 from an upright position.
...
• ADAM (Goodfellow et al., 2016) with α = 5 · 10 −4 , β 1 = 0.9, β 2 = 0.999, and = 10 −8 .
...
• λ p was set to 1 across all domains.

• λ c was set to be 7 across all domains, after it was tuned using a parameter sweep in {1, 3, 7, 10} on the Planar system.

• λ cur was set to be 1 across all domains without performing any tuning.

• {z,ū}, for the curvature loss, were generated from {z, u} by adding Gaussian noise N (0, 0.1 2 ), where σ = 0.1 was set across all domains without performing any tuning.
