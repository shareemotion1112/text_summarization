...
Denote by σ(·) the ReLU activation function, σ (z) = max{z, 0}. I{A} is the indicator function, i.e. I{A} = 1 if A is true and 0 otherwise.
...
Our goal is transferring the pretrained W(P ) to learn an accurate model W(Q) for the target distribution Q. When training the model on the pretraining dataset, we initialize the weight as: w r (0) ∼ N (0, κ 2 I), a r ∼unif ({−1, 1}), where ∀r ∈ [m] and κ is a constant.
...
Assume Gram matrices H ∞ P and H ∞ Q are invertible with smallest eigenvalue λ P and λ Q greater than zero.
...
For a two-layer networks with sufficiently large number of hidden unit m defined in Section 6.1, if m ≥ poly(n P , n Q , δ −1 , λ −1
...
For a two-layer networks with m ≥ poly(n P , n Q , δ −1 , λ −1
...
Concretely, suppose the parameter of the center point is θ.

θ i,j denotes the j-th filter of the i-th layer.

Suppose the two unit orthogonal vectors are a and b. Then with filter normalization, a i,j ← ai,j ai,j θ i,j and b i,j ← bi,j bi,j θ i,j .

For each point (i.e. pixel) (p, q) in the plot, the value is evaluated with g(p, q) = L(f (θ + η(pa + qb))), where L denotes the loss function, f denotes the neural networks.

η is a parameter to control the scale of the plot.
...
η is set to 0.001, which is of the same order as 10 times the step size in training.
...
Now following Arora et al. (2019) , the magnitude of these terms can be bounded with probability no less than 1 − δ,
...
For any set of weight vectors w 1 , . . . , w m ∈ R d that satisfy for any r ∈ [m], w r (0) − w r 2 ≤ cδλ0 n 2 R for some small positive constant c, then the matrix H ∈ R n×n defined by
...
With the same condition as lemma 1, with probability no less than 1 − δ,
...
we have with probability no less than 1 − δ,

Also note that E[H P Q,ij (0)] = H ∞ P Q,ij .

By Hoeffding's inequality, we have with probability at least 1 − δ,
...
, since the distribution of w(0) r is Gaussian with mean 0 and covariance matrix κ 2 I. Therefore, taking sum over all i and m and using Markov inequality, with probability at least 1 − δ over the random initialization we have
...
Thus, using lemma 1 and the estimation of |S i |, with probability no less than 1 − δ,
...
Concretely, with probability at least 1 − δ over random initialization,
...
we set the number of hidden nodes m = Ω

, and the learning

then with probability at least 1 − δ over the random initialization we have for k = 0, 1, 2, . . .

The following lemma is a direct corollary of Theorem 3 and lamma 1, and is crucial to analysis to follow.

Lemma 4.

Under the same conditions as Theorem 3, with probability at least 1 − δ over the random initialization we have ∀r ∈ [m], ∀k ≥ 0,
...
, k, for every r ∈ [m], with probability at least 1 − δ, w Q,r (k) − w r (0) 2 ≤ 4 √ n P y P − u P (0) 2 √ mλ P + 4 √ n Q y Q − u Q (P ) 2 √ mλ Q R .
...
since Z Q (k) − Z Q (0) F is bounded, the maximum eigenvalue of H ∞ Q is λ −1
