...
A tensor convention that we will use, however, is the Kronecker delta: δ ij , δ i j , or δ ij .
...
The one quantity that needs to be stored here is η (n,k),j i for k = 1, 2, . . .
...
We now find that we have to store η (n,k),i j values in order to calculate the derivatives.

In DISPLAYFORM9 ∂w 2 , we also end up needing η (r,k),i j for r = n. In a network with n hidden layers, we would then have DISPLAYFORM10 matrices to store.
...
It is still necessary to store η DISPLAYFORM11 , r < n, only actually shows up in one place, and thus it is possible to calculate each η (r,k),i j matrix, use it, and discard it without needing to store it for future calculations.
...
The cubic term makes it possible to use information in the Hessian without requiring convexity, and the weight σ j on that cubic term can have its own update scheme (based on how well m (s j ) approximates f BID7 ).
...
For a general iterative optimization process, this would look like x j+1 = x j + α j g j , where g j is the search direction (which need not be normalized), α j is the learning rate, and the subscript refers to the iteration.

With the search direction fixed, m would then be a cubic function of α at each iteration.

Solving ∂m ∂α = 0 as a quadratic equation in α then yields DISPLAYFORM0 If we assume that g T ∇f < 0 (i.e. g is a descent direction), then α is guaranteed to be real.

Continuing under that assumption, of the two possible α values, we choose the one guaranteed to be positive.

The sampling involved in mini-batch training means that there are a number of possible ways to get a final α j g j result.

One option would be to calculate E [α j g j ].

This would involve calculating an α value with respect to the search direction produced by each sample point and then averaging the product αg over all of the sample points.

Doing this should produce an unbiased estimate of α j g j , but in practice, we found that this approach resulted in a great deal sampling noise and thus was not effective.

The second approach would calculate DISPLAYFORM1 To do this, we would calculate an α value with respect to the search direction produced by each sample point, as in the first option, calculate an average α value, and multiply the overall search direction by that average.

This approach, too, suffered from excessive noise.

In the interest of reducing noise and increasing simplicity, we chose a third option: once the step direction had been determined, we considered that fixed, took the average of g T Hg and g T ∇f over all of the sample points to produce m (α) and then solved for a single α j value.
...
We also used a larger value for σ to account for the increased variability due to those algorithms' adaptive nature.
...
Although the number of matrix multiplications will not change with an increase in width, the cost of naïve matrix multiplication scales cubically with matrix size.
...
For CR, it is necessary to specify σ, but optimization performance is relatively insensitive to this -order of magnitude estimates seem to be sufficient -and varying σ has a stronger affect on the variability of the learning rate than it does on the magnitude (though it does affect both).

If the space is very curved, the choice of σ matters little because the step size determination is dominated by the curvature, and if the space if flat, it bounds the step length.

It is also possible to employ an adaptive approach for updating σ BID7 ), but we did not pursue that here.
...
The σ value used is specified along with the initial learning rate for each network's results.
...
We can define a convolutional layer as DISPLAYFORM0 where x i j is the layer input, σ is the vertical stride, τ is the horizontal stride, A is the activation function, and v s t is the layer output.
