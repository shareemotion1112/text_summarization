...
At each timestep, the task is re-sampled from p(T ) with some probability λ (which we refer to as the hazard rate), or remains the same.

Our goal is to optimize a learning agent to perform well in this setting.

Let p θ (ŷ t | x 1:t , y 1:t−1 ) by the agent's prediction for y t given input x t and the past labeled examples.

We will evaluate the learner's performance through a negative log likelihood loss, and our objective is as follows:

− log p θ (y t | x 1:t , y 1:t−1 ) subject to x t , y t ∼ T t , T t = T t−1 w.p.
...
For a simple nearest-neighbors model, η may simply be the context data.
...
Adopting a Bayesian perspective, we refer to p θ (y | x, η t ) as the posterior predictive distribution.
...
Meta-learning optimizes the parameters θ such that the model performs well across a distribution of tasks, min
...
At time t, run length of r t = τ indicates that the task has switched τ timesteps ago, i.e. D −τ = y t−τ :t are all drawn from a shared task T .
...
Specifically, we extend BOCPD to derive a recursive Bayesian filtering algorithm for run length in the conditional and joint density estimation setting, and leverage a base meta-learning algorithm with parameters θ to provide an underlying predictive model when conditioned on a run length.
...
Importantly, this update requires p θ (x t | η t−1 [r t ]), the base meta-learning algorithm's posterior predictive density over the inputs.
...
Recall that we assume that the task switches with fixed probability λ, the hazard rate.

Thus, for all r t , p(r t+1 = 0 | r t ) = λ, implying
...
We initialize b 1 (r 1 = 0) = 1, and initialize the posterior statistics for η 0 [r 1 = 0] as specified by the parameters θ of the meta learning algorithm.
...
Predict p θ (ŷ t | x 1:t , y 1:t−1 ) according to (7) 9:

Observe y t

Incur NLL loss t = − log p θ (y t | x 1:t , y 1:t−1 )

Compute updated posteriors η t [r t ] for all r t according to (8) 12:
...
Compute ∇ θ k+T t=k t and perform gradient descent update to θ 16: end for according to (4), as well as update the posterior statistics for all the run lengths using the labeled example.

A recursive update rule for η allows these parameters to be computed efficiently using the past values of η
...
ALPaCA (Harrison et al., 2018 ) is a meta-learning approach for which the base learning model is Bayesian linear regression in a learned feature space y | x ∼ N (K T φ(x, w), Σ ) where φ(x, w) is a feed-forward neural network with weights w mapping inputs x to a n φ -dimensional feature space.
...
We fix the prior K ∼ MN (K 0 , Σ , Λ −1 0 ).

In this matrix-normal prior,K 0 ∈ R n φ ×ny is the prior mean and Λ 0 is a n φ × n φ precision matrix (inverse of the covariance).

Given this prior and data model, the posterior may be recursively computed as follows.

First, we define Q t = Λ −1 tKt .

Then, the one step posterior update is

and the posterior predictive distribution is

In summary, ALPaCA is a meta learning model for which the posterior statistics are η t = {Q t , Λ −1 t }, and the recursive update rule h(x, y, η) is given by (9).

The parameters that are meta-learned are the prior statistics, the feature network weights, and the noise covariance: θ = {K 0 , Λ 0 , w, Σ }.
...
In PCOC we process labeled input/class pairs (x t , y t ) by encoding the input through an embedding network z t = φ(x t ; w), and performing Bayesian density estimation for every class.
...
Λ yt,t = Λ yt,t−1 + Σ ,yt where 1 i denotes a one-hot vector with a one at index i. Terms not related to class y t are left unchanged in this recursive update.

Given this set of posterior parameters η t = {α t , q 1:J,t , Λ 1:J,t }, the posterior predictive density in the embedding space can be computed as

where N (z; µ, Σ) denotes the Gaussian pdf with mean µ and covariance Σ evaluated at z. Applying Bayes rule, the posterior predictive on y t+1 given x t+1 is

This generative modeling approach also allows computing p(z t+1 | η t ) by simply marginalizing out y from the joint density of p(z, y),

As this only depends on the input x, we can use this likelihood within MOCA to update the run length belief upon seeing x t and before predictingŷ t .

In summary, PCOC performs Bayesian Gaussian discriminant analysis for online classification, and meta-learns the parameters θ = {α 0 , q 1:J,0 , Λ 1:J,0 , w, Σ ,1:J } for efficient few-shot online classification.
...
Specifically, MOCA meta-trains on an offline timeseries, and keeps the parameters θ fixed online, whereas an online learning algorithm would not have this train/test distinction, and would consider updating θ continuously (Hazan, 2016) .

However, in order to do this with MOCA, we would need to keep a running buffer of all data observed so far and to use as training data to update θ, which may be expensive in real-world domains where large volumes of data (e.g. high definition video from a large collection of cameras on an autonomous vehicle).
...
Thus, we believe it is best to train on the shortest possible sequences, and propose T = 1/λ (where λ is the hazard rate) as a rough rule of thumb.
...
PCOC maps each data point, x t , through an embedding function 1 φ : R nx → R n φ .
...
We will assume that for each class j, z = φ(x) (for x k with y k = j) follows a Gaussian distribution with meanz j and variance Σ j .
...
Compute µ j,k , Λ j,k and posterior Dirichlet concentration parameters for each class j 6:
...
whereφ j,t is the sample mean of the embedded points corresponding to class j. These can also be computed recursively, as outlined in the main paper in equation (11).
...
and we choose to meta-learn the Dirichlet prior α 0 .
...
For example, for small α j,0 's, the model will be highly sensitive to the empirical class counts within one task, whereas for large α j,0 's, the empirical counts within one task will have a relatively small effect.
...
Each task consists of a randomly sampled phase in the range [0, π] and amplitude in [0.1, 5].
