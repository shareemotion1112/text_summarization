...
Then channel normalization performs the following transformation: DISPLAYFORM2 , where mean and var compute the empirical mean and variance, and γ ij and β ij are parameters, learned independently for each channel, and is a fixed small constant.

Learning the parameter γ and β helps the optimization but is not critical.
...
In all of those inverse problems, the goal is to recover an image x from a noisy observation y = f (x) + η.

Here, f is a known forward operator (possibly equal to identity), and η is structured or unstructured noise.
...
We estimate the image asx = G(Ĉ).We use the Adam optimizer for minimizing the loss, but have obtained comparable results with gradient descent.
...
Given a noisy observation y = x + η, where η is additive noise, we estimate an image with the deep decoder by minimizing the least squares loss G(C) − y 2 2 , as described above.
...
In FIG2 we plot the Mean Squared Error (MSE) over the number of iterations of the optimizer for fitting the noisy astronaut image x + η.

Note that to fit the model, we minimize the error G(C) − (x + η) 2 2 , because we are only given the noisy image, but we plot the MSE between the representation and the actual, true image G(C t ) − x 2 2 at iteration t. Here, C t are the parameters of the deep decoder after t iterations of the optimizer.

In FIG2 and (c), we plot the loss or MSE associated with fitting the noiseless astronaut image, x ( G(C t ) − x 2 2 ) and the noise itself, η, ( G(C t ) − η 2 2 ).
...
Let η ∈ R n be zero-mean Gaussian noise with covariance matrix σI, σ > 0.
...
Note that the lower bound does not directly depend on the noise variance σ since both sides of the inequality scale with σ 2 .
