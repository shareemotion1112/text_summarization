...
A discount factor γ = 0.95 is used.
...
Here, the approximate rank is defined as the first k singular values (denoted by σ) that capture more than 99% variance of all singular values, i.e., k i=1 σ 2 i / j σ 2 j ≥ 0.99.
...
Formally, consider an unknown data matrix X ∈ R n×m and a set of observed entries Ω. If the observations are incomplete, it is often assumed that each entry of X is observed independently with probability p ∈ (0, 1].

In addition, the observation could be noisy, where the noise is assumed to be mean zero.

Given such an observed set Ω, the goal of ME is to produce an estimatorM so that ||M − X|| ≈ 0, under an appropriate matrix norm such as the Frobenius norm.

The algorithms in this field are rich.

Theoretically, the essential message is: exact or approximate recovery of the data matrix X is guaranteed if X contains some global structure (Candès & Recht, 2009; Chatterjee et al., 2015; Chen & Chi, 2018) .
...
Early convex optimization methods (Candès & Recht, 2009 ) seek to minimize the nuclear norm, ||M || * , of the estimator.
...
SVP first randomly selects a subset Ω of the state-action pairs.

In particular, each state-action pair in S × A is observed (i.e., included in Ω) independently with probability p.
...
The current iteration then ends by reconstructing the full Q matrix with matrix estimation, from the set of observations in Ω. That is, Q (t+1) = ME {Q(s, a)} (s,a)∈Ω .
...
The dynamics is described by the angle and the angular speed, i.e., s = (θ,θ), and the action a is the torque applied.
...
2 , with respect to the parameter θ.
...
The SV-RL alters the SGD update step as illustrated in Algorithm 1 and sample a set Ω of state-action pairs from S B × A. In particular, each state-action pair in S B × A is observed (i.e., included in Ω) with probability p, independently.

evaluate every state-action pair in Ω viaQ, whereQ is the network that would be used to form the targets {y
...
apply matrix completion to the observed values {Q(s, a)} (s,a)∈Ω to reconstruct Q (t+1) :
...
The physical dynamics of the system is described by the angle and the angular speed, i.e., (θ,θ).

Denote τ as the time interval between decisions, u as the torque input on the pendulum, the dynamics can be written as (Ong, 2015; Sutton & Barto, 2018) :

A reward function that penalizes control effort while favoring an upright pendulum is used:

In the simulation, the state space is (−π, π] for θ and [−10, 10] forθ.

We limit the input torque in [−1, 1] and set τ = 0.3.
...
The physical dynamics of the system is described by the position and the velocity, i.e., (x,ẋ).
...
We follow standard settings to restrict the state space as [−0.07, 0.07] for x and [−1.2, 0.6] forẋ, and limit the input u ∈ [−1, 1].
...
The physical dynamics of the system is described by the position and the velocity (i.e., (x,ẋ)), and can be derived as

Follow Tedrake (2019), we use the quadratic cost formulation to define the reward function, which regulates the brick to

We follow standard settings to restrict the state space as [−3, 3] for both x andẋ, limit the input u ∈ [−1, 1] and set τ = 0.1.
...
The physical dynamics of the system is described by the angle and the angular speed of the pole, and the position and the speed of the cart, i.e., (θ,θ, x,ẋ).

Denote τ as the time interval between decisions, u as the force input on the cart, the dynamics can be written as

x := u + ml θ2 sin θ −θ cos θ
...
A reward function that favors the pole in an upright position, i.e., characterized by keeping the pole in vertical position between |θ| ≤ 12π 180 , is expressed as r(θ) = cos 4 (15θ).

In the simulation, the state space is [− .5] foṙ x.

We limit the input force in [−10, 10] and set τ = 0.1.
...
Since the metric we care is the angle deviation, we here only plot the first two dims (i.e., the (θ,θ) tuple) with fixed x andẋ, to visualize the policy heatmaps.
...
In all experiments, we set the hyper-parameters as follows: learning rate α = 1e −5 , discount coefficient γ = 0.99, and a minibatch size of 32.
...
As described in Sec. 3, the dynamics is described by the angle and the angular speed as s = (θ,θ), and the action a is the torque applied.
