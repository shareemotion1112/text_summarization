...
The set of correct answer strings is S * and our task is to predict a single answer candidateû ∈ S.
...
Each submodel M k returns a score, φ DISPLAYFORM0 s that is fed as input to the submodel at the next level.

φ DISPLAYFORM1 DISPLAYFORM2 DISPLAYFORM3 Using their respective scores, φs , the models M 1 ...M 3 define a distribution over all spans, while M 4 uses φ (4) u to define a distribution over all unique candidates, as follows: DISPLAYFORM4 In training, our total loss is given by an interpolation of losses for each of M 1 , .., M 4 .

However, during inference we make a single prediction, simply computed asû = arg max u∈Su φu .
...
Question Embeddings: Motivated by BID16 we learn a weight δ qi for each of the words in the question using the parameterized function defined below.

This allows the model to learn to focus on the important words in the question.

δ qi is generated with a two-layer feed-forward net with rectified linear unit (ReLU) activations BID17 BID7 , DISPLAYFORM0 where U, V, w, z, a and b are parameters of the feed-forward network.
...
The scores, δ qi are normalized and used to generate an aggregated question vectorq as follows.
...
d g s,G } to be the embeddings of the G words of the sentence that contains s. First, we measure the similarity between every pair of question and sentence embeddings by passing them through a feed-forward net, ffnn att1 and using the resulting hidden layers to compute a similarity score, η.
...
DISPLAYFORM0 The original vector and its corresponding attended vector are concatenated and passed through another feed-forward net, ffnn att2 the final layers from which are summed to obtain a question-aware sentence vectorḡ s , and a sentence context-aware question vector,q.

DISPLAYFORM1 Using these attended vectors as features, along with the hidden layers from level 1 and the questionspan feature, new scores and hidden layers are computed for level 2: DISPLAYFORM2 s ;q;ḡ s ; γ qs ]), φ DISPLAYFORM3 s )
...
DISPLAYFORM0 DISPLAYFORM1 The hidden layer in level 3 is used only for computing the score φu , mentioned here to preserve consistency of notation.
...
Using Equation 1, the overall objective, (U * , V * , w * , z * , a * , b * ) is given by the total negative log likelihood of the correct answer spans under all submodels: DISPLAYFORM0 where λ 1 , .., .λ 4 are hyperparameters, such that 4 i=1 λ i = 1, to weigh the contribution of each loss term.

We briefly discuss the asymptotic complexity of our approach.

For simplicity assume all hidden dimensions and the embedding dimension are ρ and that the complexity of matrix(ρ×ρ)-vector(ρ×1) multiplication is O(ρ 2 ).

Thus, each application of a feed-forward network has O(ρ 2 ) complexity.
...
Building the weighted representation of each question takes O(mρ 2 ) and running the feed forward net to score all the spans requires O(nlρ 2 ), for a total of O(mρ 2 + nlρ 2 ).

Level 2 : Computing the alignment between the question and each sentence in the document takes O(nρ 2 + mρ 2 + nmρ) and then scoring each span requires O(nlρ 2 ).

This gives a total complexity of O(nlρ 2 + nmρ), since we can reasonably assume that m < n.

Thus, the total complexity of our approach is O(nlρ 2 + mnρ).
...
We additionally tuned the following hyperparameters using grid search and indicate the optimal values in parantheses: network size (2-layers, each with 300 neurons), dropout ratio (0.1), learning rate (0.05), context size (1), and loss weights (λ 1 = λ 2 = 0.35, λ 3 = 0.2, λ 4 = 0.1).
