...
Let p θ (x) be a family of probability density (mass) functions on a data space X , indexed by parameters θ.
...
In a latent variable model, p θ (x) is defined via a space of latent variables Z, a family of mixing measures on this latent space with density denoted p θ (z), and a conditional distribution p θ (x | z).

This conditional distribution is sometimes called an "observation model" or a conditional likelihood.

We will take θ to parameterize both p θ (x | z) and p θ (z) in the service of determining the marginal p θ (x) via the mixture integral:
...
The downside of this mixing approach is that the integral may be intractable to compute, making it difficult to evaluate p θ (x)-a quantity often referred to in Bayesian statistics and machine learning as the marginal likelihood or evidence.

Various Monte Carlo techniques have been developed to provide consistent and often unbiased estimators of p θ (x), but it is usually preferable to work with log p θ (x) and unbiased estimation of this quantity has, to our knowledge, not been previously studied.
...
The most common difference measure is the forward Kullback-Leibler (KL) divergence; if p data (x) is the empirical distribution and p θ (x) is a parametric family, then minimizing the KL divergence (D KL ) with respect to θ is equivalent to Since expectations can be estimated in an unbiased manner using Monte Carlo procedures, simple subsampling of the data enables powerful stochastic optimization techniques, with stochastic gradient descent in particular forming the basis for learning the parameters of many nonlinear models.

However, this requires unbiased estimates of ∇ θ log p θ (x), which are not available for latent variable models.

Instead, a stochastic lower bound of log p θ (x) is often used and then differentiated for optimization.
...
If K samples are drawn from q(z; x) then this provides an unbiased estimate of p θ (x) and the biased "importance-weighted autoencoder" estimator IWAE K (x) of log p θ (x) is given by
...
While the IWAE lower bound acts as a useful replacement of log p θ (x) in maximum likelihood training, it may not be suitable for other objectives such as those that involve entropy maximization.

We discuss tasks for which a lower bound estimator would be ill-suited in Section 3.4.

There are two properties of IWAE that will allow us to modify it to produce an unbiased estimator: First, it is consistent in the sense that as the number of samples K increases, the expectation of IWAE K (x) converges to log p θ (x).
...
, the Russian roulette estimator is optimal across a broad family of unbiased randomized truncation estimators if the ∆ g k are statistically independent, in which case it has second moment E||Ĝ||
...
We use φ to denote the parameters of the encoder q φ (z; x).

However, the gradients of SUMO with respect to φ are in expectation zero precisely because SUMO is an unbiased estimator of log p θ (x), regardless of our choice of q φ (z; x).

Nevertheless, we would expect the choice of q φ (z; x) significantly impacts the variance of our estimator.
...
Minimizing log p θ (x).

Some machine learning objectives include terms that seek to increase the entropy of the learned model.

The "reverse KL" objective-often used for training models to perform approximate posterior inferences-minimizes E x∼p θ (x) [log p θ (x) − log π(x)] where π(x) is a target density that may only be known up a normalization constant.
...
Unbiased score function ∇ θ log p θ (x).
...
Of particular note, the REINFORCE gradient estimator (Williams, 1992)-generally applicable for optimizing objectives of the form max θ E x∼p θ (x) [R(x)]-is estimated using the score function.
...
Such estimators are often used for reward maximization in reinforcement learning where p θ (x) is a stochastic policy.
...
A major problem with fitting latent variables models to this objective is the presence of an entropy maximization term, effectively a minimization of log p θ (x).

Estimating this log marginal probability with a lower bound estimator could result in optimizing θ to maximize the bias of the estimator instead of the true objective.
...
We use 20 latent variables, with p(z), p θ (x|z), and q φ (z; x) all being Gaussian distributed.
...
which is now a differentiable function with respect to the parameters θ, whose gradients can be estimated using a combination of the REINFORCE gradient estimator and the SUMO estimator (equation 13).

Furthermore, entropy regularized reinforcement learning-where we maximize R(x) + λH(p θ ) with H(p θ ) being the entropy of p θ (x)-encourages exploration and is inherently related to minimizing a reverse KL objective with the target being an exponentiated reward (Norouzi et al., 2016) .
...
As there exist complex dependencies between the binary variables and optimization of equation 16 requires sampling from the policy distribution p θ (x), a model that is both expressive and allows efficient sampling would be ideal.
...
We note that we rely on ||Y k − µ|| < 1 for this power series to converge.

This condition was implicitly assumed, but not explicitly noted, in (Nowozin, 2018) .

This condition will hold for sufficiently large k so long as the moments of w i exist: one could bound the probability ||Y k −µ|| ≥ 1 by Chebyshev's inequality or by the Central Limit Theorem.
...
For clarity, let C k = Y k − µ be the zero-mean random variable.
...
Assume that ∇ θ SUMO is bounded: it is sufficient that ∇ θ IWAE 1 is bounded and that the sampling probabilities are chosen such that the partial sums of
...
We abbreviate w i := p θ (x,zi) q ψ (zi|x) , and ν i = dwi dλ .

In both λ = ψ and λ = θ cases, it suffices to treat the w i and ν i as i.i.d.

random variables with finite variance and expectation.

Being a likelihood ratio, w i could be ill behaved when the importance sampling distribution q ψ (z i |x) is is particularly mismatched from the true posterior p(z i |x) = p θ (x,zi E z∼p(z) p θ (x,z) .

However, the analysis from IWAE (Burda et al., 2016) requires assuming that the likelihood ratios w i = p θ (x,zi) q ψ (zi|x) are bounded, and we adopt this assumption.
...
In density modeling experiments, all the models are trained using a batch size of 100 and an Amsgrad optimizer (Reddi et al., 2018) with parameters lr = 0.001, β 1 = 0.9, β 2 = 0.999 and = 10 −4 .
