...
The inference network takes in a sequence of images (x i 0 ,...x i T ), concatenated along the channel dimension, and outputs a posterior over the initial state z ∼ q φ (·|x 0 ,...x T ), corresponding to the system's coordinates in phase space at the first frame of the sequence.

We parametrise q φ (z) as a diagonal Gaussian with a unit Gaussian prior p(z) = N (0, I) and optimise it using the usual reparametrisation trick (Kingma & Welling, 2014) .
...
space s 0 , we map samples from the posterior with another function s 0 = f ψ (z) to obtain the system's initial state.
...
The Hamiltonian network is parametrised as a neural network with parameters γ that takes in the inferred abstract state and maps it to a scalar H γ (s t ) ∈ R. We can use this function to do rollouts in the abstract state space using the Hamiltonian equations (Eq. 3), for example by Euler integration:
...
A common idea to address this problem is to start with a simple prior distribution π(u) and then transform it into a more expressive form p(x) through a series of composable invertible transformations f i (u) called normalising flows (Rezende & Mohamed, 2015 ) (see Fig. 3A ).
...
First, we assume that the initial state s 0 is a sample from a simple prior s 0 ∼ π 0 (·).
...
For the two-body problem, we set r ∼ U(0.5,1.5), and we also change the observation noise to σ 2 = 0.05.

For the three-body problem, we set r ∼ U (0.9,1.2), and set the observation noise to σ 2 = 0.2.
...
Fig. 9 demonstrates that NHF can transform a simple soft-uniform prior distribution π(s 0 ;σ,β) into significantly more complex densities with arbitrary numbers of modes.

The soft-uniform density,

2 )), where f is the sigmoid function and β is a constant, was chosen to make it easier to visualise the learned attractors.
...
At each time step t the decoder network d θ takes only the abstract position part q t of the state s t and decodes it back to an output image x t ∈ R 32×32×3 of the same shape as the input images.
...
When optimising the loss, in practice we do not learn the variance of the decoder p θ (x|s) and fix it to 1, which makes the reconstruction objective equivalent to a scaled L2 loss.
...
The encoder network was parametrized as f ψ (p|q) = N (p;µ(q),σ(q)), where µ and σ are ReLU MLPs with size [d,128,128,d] .
...
λ HNN is a Lagrange multiplier, which is set to 0.1, as in the original paper.

L WD is a standard L2 weight decay and its Lagrange multiplier λ WD is set to 1e-5, as in the original paper.
