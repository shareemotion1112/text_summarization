...
DISPLAYFORM0 The value of a state s when following a policy π, v π (s), is defined to be the expected sum of discounted rewards from that state: DISPLAYFORM1 , with γ being the discount factor.

When the transition probability function p and the reward function r are known, we can compute v π (s) recursively by solving the system of equations below BID3 : DISPLAYFORM2 This equation can also be written in matrix form with v π , r ∈ R |S| and P π ∈ R |S|×|S| : DISPLAYFORM3 where P π is the state to state transition probability function induced by π, that is, P π (s, s ) = a π(a|s)p(s |s, a).

Traditional model-based algorithms for RL work by learning estimates of the matrix P π and of the vector r and using them to estimate v π , for example by solving Equation 1.

We useP π andr to denote empirical estimates of P π and r. Formally, DISPLAYFORM4 wherer(i) denotes the i-th entry in the vectorr, n(s, s ) is the number of times the transition s → s was observed, n(s) = s ∈S n(s, s ), and C(s, s ) is the sum of the rewards associated with the n(s, s ) transitions (we drop the action in the discussion to simplify notation).Alternatively, in model-free RL, instead of estimating P π and r we estimate v π (s) directly from samples.

We often use temporal-difference (TD) learning BID25 to update our estimates of DISPLAYFORM5 where α is the step-size parameter.
...
We do so by parametrizingv(s) with a set of weights θ.

We write, given the weights θ,v(s; θ) ≈ v π (s) andq(s, a; θ) ≈ q π (s, a), where q π (s, a) = r(s, a) + γ s p(s |s, a)v π (s ).
...
The successor representation, with respect to a policy π, Ψ π , is defined as DISPLAYFORM6 where we assume the sum is convergent with I denoting the indicator function.

BID5 has shown that this expectation can be estimated from samples through TD learning.

It also corresponds to the Neumann series of γP : DISPLAYFORM7 Notice that the SR is part of the solution when computing a value function: DISPLAYFORM8 We useΨ π to denote the SR computed throughP π , the approximation of P π .The definition of the SR can also be extended to features.
...
For a given 0 ≤ γ < 1, policy π, and for a feature representation φ(s) ∈ R d , the successor features for a state s are: DISPLAYFORM9 Notice that this definition reduces to the SR in the tabular case, where Φ = I.
...
LetP π denote the substochastic matrix induced by the environment's dynamics and by the policy π such thatP π (s |s) = n(s,s ) n(s)+1 .

For a given 0 ≤ γ < 1, the substochastic successor representation,Ψ π , is defined as: DISPLAYFORM0 The theorem below formalizes the idea that the 1 norm of the SSR implicitly counts state visitation.

Theorem 1.

Let n(s) denote the number of times state s has been visited and let χ(s) = (1 + γ) − ||Ψ π (s)|| 1 , whereΨ π is the substochastic SR as in Definition 3.1.

For a given 0 ≤ γ < 1, DISPLAYFORM1 Proof of Theorem 1.

LetP π be the empirical transition matrix.

We first rewriteP π in terms ofP π : DISPLAYFORM2 The expression above can also be written in matrix form:P π = (I − N )P π , where N ∈ R |S|×|S| denotes the diagonal matrix of augmented inverse counts.

ExpandingΨ π we have: DISPLAYFORM3 The top eigenvector of a stochastic matrix is the all-ones vector, e (Meyn & Tweedie, 2012), and it corresponds to the eigenvalue 1.

Using this fact and the definition ofP π with respect toP π we have: DISPLAYFORM4 We can now bound the term γ

π e using the fact that e is also the top eigenvector of the successor representation and has eigenvalue DISPLAYFORM0 Plugging FORMULA14 into the definition of χ we have (notice that Ψ(s)e = ||Ψ(s)|| 1 ): DISPLAYFORM1 When we also use the other bound on the quadratic term we conclude that, for any state s, DISPLAYFORM2 In other words, the SSR, obtained after a slight change to the SR, can be used to recover state visitation counts.
...
This is due to the fact that s P π (s, s ) gets closer to 1 each time state s is visited.

This result can now be used to convert the SSR into a reward function in the tabular case.

We do so by using the SSR to define an exploration bonus, r int , such that the reward being maximized by the agent becomes r(s, a) + βr int (s), where β is a scaling parameter.

Since we want to incentivize agents to visit the least visited states as quickly as possible, we can trivially define r int = −||Ψ π (s)|| 1 , where we penalize the agent by visiting the states that lead to commonly visited states.

Notice that the shift (1 + γ) in χ(s) has no effect as an exploration bonus because it is the same across all states.
...
Learning the SSR becomes even more challenging when the representation, φ(·), is also being learned and so is non-stationary.
...
The distinction between θ and θ − is standard in the field, with θ − denoting the parameters of the target network, which is updated less often for stability purposes BID17 .

As before, we use r int to denote the exploration bonus obtained from the successor features of the internal representation, φ(·), which will be defined below.

Moreover, to ensure all features are in the same range, we normalize the feature vector so that ||φ(·)|| 2 = 1.

In Figure 1 we highlight the layer in which we normalize its output with the symbol φ .
...
The agent might learn to set φ(·) = 0 to achieve zero loss.

We address this problem by not propagating ∇L SR to φ(·) (this is depicted in Figure 1 as an open circle stopping the gradient), and by creating an auxiliary task BID7 to encourage a representation to be learned before a non-zero reward is observed.
...
The loss we minimize for this last part of the network is L Recons = Ŝ t+1 − S t+1 2 .

The last step in describing our algorithm is to define r int (S t ; θ − ), the intrinsic reward we use to encourage exploration.

We choose the exploration bonus to be the inverse of the 2 -norm of the vector of successor features of the current state, that is, DISPLAYFORM0 where ψ(S t ; θ − ) denotes the successor features of state S t parametrized by θ − .

The exploration bonus comes from the same intuition presented in the previous section, but instead of penalizing the agent with the norm of the SR we make r int (S t ; θ − ) into a bonus (we observed in preliminary experiments not discussed here that DQN performs better when dealing with positive rewards).
...
We evaluated our agents in the stochastic setting (sticky actions, ς = 0.25) using a frame skip of 5 with the full action set (|A| = 18).
...
We set β = 0.025 after a rough sweep over values in the game MONTEZUMA'S REVENGE.
...
The discount factor, γ, is set to 0.99 and w TD = 1, w SR = 1000, w Recons = 0.001.
...
n(s, s ) ← 0 ∀s, s ∈ S t(s, a, s ) ← 1 ∀s, s ∈ S, ∀a ∈ Â r(s, a) ← 0 ∀s ∈ S, ∀a ∈ Â P (s, a) ← 1/|S| ∀s ∈ S, ∀a ∈ Ã P (s, s ) ← 0 ∀s, s ∈ S π ← random over A while episode is not over do Observe s ∈ S, take action a ∈ A selected according to π(s), and observe a reward R and a next state s ∈ S n(s, s ) ← n(s, s ) + 1 DISPLAYFORM0 r int ← −Ψe π ← POLICYITERATION(P ,r + βr int ) end while
...
When dropping the auxiliary task, the average performance of this baseline over 4 seeds in MON-TEZUMA'S REVENGE after 100 million frames was 100.0 points (σ 2 = 200.0; min: 0.0, max: 400.0).

As comparison, our algorithm obtains 1778.6 points (σ 2 = 903.6, min: 400.0, max: 2500.0).
...
To do so we dropped the SR module and set β = 0.0 to evaluate whether our exploration bonus was actually improving the agent's performance or whether the auxiliary task was doing it.

The exploration bonus seems to be essential in our algorithm.

When dropping the exploration bonus and the successor representation module, the average performance of this baseline over 4 seeds in MONTEZUMA'S REVENGE after 100 million frames was 398.5 points (σ 2 = 230.1; min: 0.0, max: 400.0).
