...
Denote the input as x. We focus on multi-layered networks with σ(·) as ReLU nonlinearity.

We use the following equality extensively: σ(x) = I[x > 0]x, where I[·] is the indicator function.
...
Let R 0 = {x : ρ(x) > 0} be the infinite training set, where ρ(x) is the input data distribution.
...
Definition 1 (SGD critical point).Ŵ is a SGD critical point if for any batch,
...
In the following, we want to check whether this condition leads to specialization, i.e., whether the teacher's weights are recovered/aligned by the student, i.e., whether for teacher j, there exists a student k at the same layer so that w k = γw j for some γ > 0.

Note that g l (x i ;Ŵ) = 0 might be a strong assumption since in practice the gradient is small but never zero.

A weaker assumption is that g l (x i ;Ŵ) ∞ ≤ or even E t g l (x i ;Ŵ) ∞ ≤ .
...
The intuition is that if the input x takes sufficiently diverse values, ReLU activations σ(w k x) can be proven to be mutually linear independent.

On the other hand, the gradient of each student node k when active, is α k f 1 (x) − b k f 1 (x) = 0, a linear combination of teacher and student nodes (note α k and β k are k-th rows of A 1 and B 1 ).
...
If the boundary of a teacher j and a student k lies in the region, similar logic applies (here α kj is the (k, j) entry of A 1 (x) and is constant in a region R ∈ R 0 ).

Theorem 4 (Student-teacher Alignment, Multiple Layers).

With Assumption 1, at SGD critical points, for any teacher node j at l = 1, if there exists a region R ∈ R and a student observer k so that ∂E * j ∩ E k ∩ R = ∅ and α kj (R) = 0, then node j aligns with at least one student node k .

Note that even with random V 1 (x) (e.g., at initialization), Theorem 4 still holds with high probability (when α kj = 0) and teacher f * 1 (x) can still align with student f 1 (x).
...
Noteθ jj is the angle of two weightsw j andw j .

for any x ∈ R 0 with ≤ 0 , then for any teacher j at l = 1, if there exists a region R ∈ R and a student observer k so that ∂E * j ∩E k ∩R = ∅, and α kj (R) = 0, then j is roughly aligned with a student k :

|α kj | for any δ > 0.

The hidden constants depends on δ, 0 and the size of region ∂E * j ∩E k ∩R.
...
Second, as suggested by Theorem 5, it can take a long time to recover a teacher node k with small v * k (since α kj = v * k v j ).
...
This means that if r is dominated by a teacher j (i.e., v * j is large), thenẇ k would push w k towards w * j .
...
where θ is the angle between w * j and w k .

As a result, for all θ ∈ [0, π], E x f l−1 z k f * j is always between w * j and w k since π − θ and sin θ are always non-negative.
...
With this assumption, we can write the dynamics asẇ k = w k r k , where the time-varying residue r k of node k is defined as the following (ν is a scalar related to ψ ): Figure 3: Student specialization of a 2-layered network with 10 teacher nodes and 1x/2x/5x/10x student nodes.
...
While expected angle between two weights from initialization is π/2, shifting a student node w k from chasing after a strong teacher node w * j to a weaker one w * j could yield a large initial angle (e.g., close to π) between w k and w j .
...
In fact, if there is only one teacher node and θ is the angle between teacher and student, then from Eqn.

8 we arrive atθ ∝ −ψ(θ) sin θ.

Since ψ(θ) sin θ ∼ (π − θ) 2 around θ = π, the time spent from θ = π − to some θ 0 is t 0 ∼ 1 − 1 π−θ0 → +∞ when → 0.
...
We generate the input using N (0, σ 2 I) with σ = 10 and we sample 10k as training and another 10k as evaluation.
...
For node k, y-axis is Ex [β kk (x)], equivalent to the fan-out weight norm v k in 2-layer case, and x-axis is its max correlation to the teachers.
...
For each student node k, the x-axis is its best normalized correlation to teacher nodes, and y-axis is E x [β kk (x)], which is equivalent to v k in 2-layer case.
...
Over-realization boosts student specialization, measured by mean of maximal normalized correlation ρ mean = mean j∈ teacher max j ∈ studentf * jf j at each layer (f j is the normalized activation of node j over N evaluation samples), and improves generalization, evaluated on CIFAR-10 evaluation set.
...
Figure 8: Mean of the max teacher correlation ρmean with student nodes over epochs in CIFAR10.
...
Note that if Ẇ l ∞ ≤ , which is i∈Bj U i ∞ ≤ , then with simple Gaussian elimination for two batches B 1 and B 2 with only two sample difference, we will have for any

Plug things back in and we have |B| U i ∞ ≤ [2(|B| − 1) + 1] , which is U i ∞ ≤ 2 .

If f l−1 (x;Ŵ) has the bias term, then immediately we have g l (x;Ŵ) ∞ ≤ .
...
Consider K ReLU activation functions f j (x) = σ(w j x) for j = 1 . . .
...
Consider K ReLU nodes f j (x) = σ(w j x), j = 1, . . .

, K. w j = 0, w j = γw j for j = j with any γ > 0.
...
Denote θ jj as the angle between the two vectors w j and w j .

Then there exists u j ⊥w j and w j ũ j = sin θ jj .

Proof.

Note that the projection ofw j ontow j is:

It is easy to verify that ũ j = 1 and w j ũ j = sin θ jj .
...
where θ jj is the angle betweenw j andw j , δ > 0, r is the radius of a d − 1 dimensional ball
...
Suppose for any j = j, sin θ jj > KM 1−δ /|c j | or |b j − b j | > M 2 1−2δ /|c j |.

Otherwise the theorem already holds.

From Lemma 4, we know that for any x ∈ ∂E j , if w j x = −q j , with a j ≤ 2q j |c j |/M K 1−δ = Consider a d − 1-dimensional sphere B ⊆ Ω j and its intersection of I j ∩ B for j = j. Suppose the sphere has radius r.
...
In this case, we want to show that for any x ∈ Ω j , |w j x| > q j and thus I j ∩ B = ∅. If this is not the case, then there exists x ∈ Ω j so that |w j x| ≤ q j .
...
This means that there exists x j ∈ B ⊆ Ω j so that x j / ∈ I j ∩ B for any j = j and j in case 1.

That is,

On the other hand, for j in case 2, the above condition holds for entire Ω j , and thus hold for the chosen x j .
...
Then there exists node j = j so that sin θ jj ≤ M K 1−δ /|c j | and |b j − b j | ≤ M 2 1−2δ /|c j |, where r, δ, M, M 2 are defined in Lemma 5 but with r = r − 5 /|c j |.

Proof.

Let q j = 5 /|c j | and Ω j = {x : x ∈ ∂E j ∩ R, B(x, q j ) ⊆ R}. If situation (1) in Lemma 5 happens then the theorem holds.

Otherwise, applying Lemma 5 with R = {x : x ∈ R, B(x, q j ) ⊆ R} and there exists x j ∈ Ω j so that Let two points x ± j = x j ± q jwj ∈ R. In the following we show that the three points x j and x ± j are on the same side of ∂E j for any j = j.
...
4, V 1 (x) = W 1 and V * 1 (x) = W * 1 are independent of input x. Therefore, both A 1 and B 1 are independent of input x. From Assumption 1, since ρ(x) > 0 in R 0 , from Theorem.

1 we know that the SGD critical points gives

Picking node k, the following holds for every node k and every

Here α k is the k-th row of A 1 , A 1 = [α 1 , . . .

, α n1 ] and similarly for β k .
...
Since α kj = 0, we know co-linear(j) = ∅ and there exists at least one student k that is aligned with the teacher j.
...
If for all k ∈ co-linear(k), β k k ≡ v k 2 = 0, then v k = 0 and the proof is complete.
...
Let H R and similarly H * R be the student and teacher nodes whose boundary where θ = 2θ + π and
...
Note that in multi-layer setting, Theorem 3 might not hold since it is for 2-layer so there could be un-specialized student nodes with large β kk (x).

−15, 15] .

For all teacher nodes, the normalized correlations are all close to 1.0 (ρ mean ≥ 0.998 at all layers).
