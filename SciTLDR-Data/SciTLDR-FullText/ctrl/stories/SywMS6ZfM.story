...
Several recently proposed methods aim to encode hypernym relations between words in dense embeddings, such as Gaussian embedding BID45 BID0 , order embedding BID44 , H-feature detector BID33 , HyperScore (Nguyen et al., 2017) , dual tensor BID12 , Poincaré embedding BID28 , and LEAR BID46 .
...
The HyperLex dataset has a continuous score on each candidate word pair, so we adopt Spearman rank coefficient ρ as suggested by the review study of BID47 .
...
By adding slack variables ζ and ξ, the problem could be converted into a linear programming problem: dq a*dq dp a*dq-dp dp-a*dq
