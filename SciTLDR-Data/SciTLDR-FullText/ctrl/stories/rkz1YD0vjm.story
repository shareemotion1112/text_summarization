...
L. The weights of each layer l are denoted by ω l .

We sparsify these weights using a sparsification function, S, which takes as input ω l and a threshold τ l from a vector of thresholds T .

Each weight i of ω l is modified by S as follows: DISPLAYFORM0 Conference on Neural Information Processing Systems (NIPS), CDNNRIA Workshop, 2018, Montréal, Canada.

BID0 The extended version of this paper can be found at: https://arxiv.org/abs/1811.04199 where τ l = T (l) is the threshold used for layer l. Thus, applying a single threshold τ l forces weights within −τ l and +τ l in value to become 0.
...
The flat method defines a constant threshold τ for all layers, irrespective of the distribution of their corresponding weights.
...
The high level work-flow of the sparsification framework is depicted in Figure 1b .Flat Method This method defines a constant threshold τ for all layers, irrespective of the distribution of their corresponding weights.

It is graphically depicted in the top of Figure 1a .

The weights of the layers are profiled to determine the span σ min = min ∀linL (max(ω l ) − min(ω l )).

This span corresponds to the layer k having the smallest range of weights within the pretrained model.

This span is used as an upper-bound value for our flat threshold τ .

Since using σ min as a threshold eliminates all the weights in layer k and is likely to adversely affect the accuracy of the sparsified model, we use a fraction δ, 0 ≤ δ ≤ 1, of the span τ l = σ min × δ, where δ is a parameter of the method that can be varied to achieve different degrees of model sparsity.

The triangular method is defined by two thresholds τ min and τ max for respectively the first convolution layer (i.e, layer 1) and the last fully connected layer (i.e., layer L).
...
Thus, DISPLAYFORM0 where σ conv is the span of the weights in the first convolution layer, defined in a similar way as for the flat method, and it represents an upper bound on τ min .

Thus, δ conv is a fraction that ranges between 0 and 1.

Similarly, σ f c is the span of the weights in the last fully connected layer and it represents an upper bound on τ max .

Thus, δ f c is a fraction that ranges between 0 and 1.

The thresholds of the remaining layers are dictated by the position of these layers in the network.

Relative Method In particular, it uses the δ th percentile of distribution's weight in layer l denoted by δ l .

Thus each element of the vector T (l) = τ l is defined as: DISPLAYFORM1 where 0 ≤ δ l ≤ 1 defines the desired percentile of the δ l of zero weights in each layers.
