...
Variational autoencoders BID14 BID28 consist of an encoder q φ (z|x) and decoder p θ (x|z); oftentimes these can be instantiated as neural networks, E φ (·) and D θ (·) respectively, with learnable parameters, φ and θ.
...
The encoder predicts, µ φ (x) and σ φ (x) for a given input x and a latent sample,ẑ, is drawn from q φ (z|x) as follows: DISPLAYFORM1 .

By choosing a multivariate Gaussian prior, the KL-divergence may be calculated analytically BID14 .

The first term in the loss function is typically approximated by calculating the reconstruction error between many samples of DISPLAYFORM2 New data samples, which are not present in the training data, are synthesised by first drawing latent samples from the prior, z ∼ p(z), and then drawing data samples from p θ (x|z).

This is equivalent to passing the z samples through the decoder, D θ (z).VAEs offer both a generative model, p θ (x|z), and an encoding model, q φ (z|x), which are useful as starting points for image editing in the latent space.
...
GANs consist of two models, a generator, G θ (·), and a discriminator, C χ (·), both of which may be implemented using convolutional neural networks BID27 BID7 .

GAN training involves these two networks engaging in a mini-max game.

The discriminator, C χ , is trained to classify samples from the generator, G θ , as being 'fake' and to classify samples from the data-generating distribution, p(x), as being 'real'.
...
A more interesting approach, for conditional (non-variational and semi-supervised) autoencoders is presented by BID22 , where the encoder outputs both a latent vector,ẑ, and an attribute vector,ŷ.

The encoder is updated to minimize a classification loss between the true label, y, andŷ.
...
We observe that in this naive implementation of conditional VAEs, varying the attribute (or label) vector,ŷ, for a fixedẑ can result in unpredictable changes in synthesized data samples,x.

Consider for example the case where, for a fixedẑ, modifyingŷ does not result in any change in the intended corresponding attribute.

This suggests that information about the attribute one wishes to edit, y, is partially contained inẑ rather than solely in y. Similar problems have been discussed and addressed to some extent in the GAN literature BID24 BID25 , where it has been observed that label information inŷ is often ignored during sample synthesis.

In general, one may think thatẑ andŷ should be independent.

However, if attributes, y, that should be described byŷ remain unchanged for a reconstruction where onlyŷ is changed, this suggests that z contains most of the information that should have been encoded withinŷ.

We propose a process to separate the information about y fromẑ using a mini-max optimization involving y,ẑ, the encoder E φ , and an auxiliary network A ψ .

We refer to our proposed process as 'Adversarial Information Factorization'.

For a given image of a face, x, we would like to describe the face using a latent vector,ẑ, that captures the identity of the person, along with a single unit vector,ŷ ∈ [0, 1], that captures the presence, or absence, of a single desired attribute, y. If a latent encoding,ẑ, contains information about the desired attribute, y, that should instead be encoded within the attribute vector,ŷ, then a classifier should be able to accurately predict y fromẑ.

Ideally,ẑ contains no information about y and so, ideally, a classifier should not be able to predict y fromẑ.

We propose to train an auxiliary network to predict y fromẑ accurately while updating the encoder of the VAE to outputẑ values that cause the auxiliary network to fail.

Ifẑ contains no information about the desired attribute, y, that we wish to edit, then the information can instead be conveyed inŷ sincex must still contain that information in order to minimize reconstruction loss.

We now formalize these ideas.

In what follows, we explain our novel approach to training the encoder of a VAE, to factor (separate) out information about y fromẑ, such that H(y|ẑ) ≈ H(y).
...
Our main contribution is our proposed adversarial method for factorising the label information, y, out of the latent encoding,ẑ.

A schematic of our architecture is presented in Figure 1 .

In addition to the encoder, E φ , decoder, D θ , and discriminator, C χ , we introduce an auxiliary network, A ψ :ẑ →ỹ, whose purpose is described in detail in Section 3.2.

Additionally, the encoder also acts as a classifier, outputting an attribute vector,ŷ, along with a latent vector,ẑ.

The parameters of the decoder, θ, are updated with gradients from the following loss function: et al., 2017) , where y real and y f ake are vectors of ones and zeros respectively and z ∼ p(z).
...
Note that E φ is split in two, E z,φ and E y,φ , to obtain both a latent encoding,ẑ, and the label,ŷ, respectively.

D θ is the decoder and A ψ the auxiliary network.

The pink blocks show how a GAN architecture may be incorporated by placing a discriminator, C χ , after the encoder, E φ , and training C χ to classify decoded samples as "fake" and samples from the dataset as "real".
...
Note that there is no auxiliary network performing information factorization and a label,ŷ, is predicted only for the reconstructed image, rather than for the input image (ŷ).

To factor label information, y, out ofẑ we introduce an additional auxiliary network, A ψ , that is trained to correctly predict y fromẑ.

The encoder, E φ , is simultaneously updated to promote A ψ to make incorrect classifications.

In this way, the encoder is encouraged not to place attribute information, y, inẑ.

This may be described by the following mini-max objective: DISPLAYFORM0 where E z,φ(x) is the latent output of the encoder.

Training is complete when the auxiliary network, A ψ , is maximally confused and cannot predict y fromẑ = E z,φ (x), where y is the true label of x. The encoder loss is therefore given by: DISPLAYFORM1 We call the conditional VAE-GAN trained in this way an Information Factorization cVAE-GAN (IFcVAE-GAN).

The training procedure is presented in Algorithm 1.

To edit an image such that it has a desired attribute, we encode the image to obtain aẑ, the identity representation, append it to our desired attribute label,ŷ ← y, and pass this through the decoder.

We useŷ = 0 andŷ = 1 to synthesize samples in each mode of the desired attribute e.g. 'Smiling' and 'Not Smiling'.
...
We apply the trained classifier to edited images, synthesized usingŷ = 1 and y = 0 to obtain classification scores,Ĉ Smiling andĈ N ot−Smiling respectively.
...
Smaller reconstruction error indicates better reconstruction, and larger classification scores (Ĉ Smiling andĈ N ot−Smiling ) suggest better control over attribute changes.
...
SinceĈ Smiling + C N ot−Smiling ≈ 100%, this strongly suggests that samples are synthesized independently ofŷ and that the synthesized images are the same forŷ = 0 andŷ = 1.(3) Effect of classifying reconstructed samples: We explored the effect of including a classification loss on reconstructed samples, L bce (y,ŷ), whereŷ = E y,φ (D θ (x)).
...
We have proposed a model that learns a representation, {ẑ,ŷ}, for faces such that the identity of the person, encoded inẑ, is factored from a particular facial attribute.

We achieve this by minimizing the mutual information between the identity encoding and the facial attribute encoding to ensure that H(y|ẑ) ≈ H(y), while also training E y,φ as an attribute classifier.

Our training procedure encourages the model to put all label information intoŷ, rather thanẑ.
...
We proceed to use E y,φ directly for facial attribute classification and compare the performance of our model to that of a state of the art classifier proposed by Zhuang et al. (2018) .
...
Briefly, this involves reconstructing an input image, x, for different attribute values,ŷ ∈ {0, 1}.We begin by demonstrating how a cVAE-GAN BID3 ) may fail to edit desired attributes, particularly when it is trained to achieve low reconstruction error.
...
Figure 3 (c,e) shows edited images, settingŷ = 0 for 'Not Smiling' andŷ = 1 for 'Smiling'.
...
We compare the performance of our classifier, E y,φ , to a state of art classifier (Zhuang et al., 2018) .weightings on the KL and GAN loss terms, using α = 0.005 and δ = 0.005 respectively.
...
Under this set-up, we used the following additional hyper-parameter: {ρ = 1.0} in our model.

Figure 3 shows reconstructions when settingŷ = 0 for 'Not Smiling' andŷ = 1 for 'Smiling'.
...
C Smiling andĈ N ot−Smiling denote the proportion of edited samples that have the desired attribute.
...
In this section we apply our proposed method to manipulate other facial attributes where the initial samples, from which theẑ's are obtained, are test samples whose labels are y = 1 indicating the presence of the desired attribute (e.g. 'Blonde Hair').
...
We obtain aẑ, the identity representation, by passing an image, x through the encoder.

We appendẑ with a desired attribute label,ŷ ← y, and pass this through the decoder.

We useŷ = 0 andŷ = 1 to synthesize samples in each mode of the desired attributeWe have presented the novel IFcVAE-GAN model, and (1) demonstrated that our model learns to factor attributes from identity, (2) performed an ablation study to highlight the benefits of using an auxiliary classifier to factorize the representation and (3) shown that our model may be used to achieve competitive scores on a facial attribute classification task.

We now discuss this work in the context of other related approaches.

We have used adversarial training (involving an auxiliary classifier) to factor attribute label information, y, out of the encoded latent representation,ẑ.
...
The β-VAE objective is similar to the information bottleneck BID5 , minimizing mutual information, I(x; z), which forces the model to exploit regularities in the data and learn a disentangled representation.
...
We compare the performance of our classifier, E y,φ , to a linear classifier trained on latent representations extracted from a trained DIP-VAE .
