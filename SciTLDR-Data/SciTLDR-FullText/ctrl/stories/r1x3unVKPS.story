...
We consider an infinite-horizon discounted MDP (S, A, P, r, p 0 , γ), where S is the set of states, A the set of actions, P : S × A × S → [0, 1] the transition probability, r : S × A → R the reward function, p 0 : S → [0, 1] the distribution over initial states, and γ ∈ (0, 1) the discount factor.

Let π be a stochastic policy π :

, and s t+1 ∼ P (·|s t , a t ) for t ≥ 0.

We denote π E the expert policy.

Behavioral Cloning (BC) learns a policy π :
...
where the expectations E π and E π E denote the joint distributions over state-actions of the RL agent and the expert, respectively.
...
Specifically, RED learns the reward parameterθ by minimizing:

where f θ : S × A → R K projects (s, a) from expert demonstrations to some embedding of size K, with randomly initialized θ.
...
Each trajectory is a sequence of stateaction tuples in the form of τ i = {s 1 , a 1 , s 2 , a 2 , ..., s T , a T }.
...
Eq. (4) shows that the adversarial reward only depends on the ratio φ(s, a) =

pπ(s,a) .

Intuitively, r gail incentivizes the RL agent towards under-visited state-actions, where φ(s, a) > 1, and away from over-visited state-actions, where φ(s, a) < 1.

When π E and π match exactly, r gail converges to an indicator function for the support of π E , since φ(s, a) = 1 ∀ (s, a) ∈ supp(π E ) (Goodfellow et al., 2014) .

In practice, the adversarial reward is unlikely to converge, as p π E is estimated from a finite set of expert demonstrations.
...
, Θ function models, initial policy π ω0 , initial discriminator parameters w 0 , learning rate l D .

2: r red = RED(Θ, τ E ) 3: for i = 0, 1, . . .

sample a trajectory τ i ∼ π 5:

π ωi+1 = TRPO(r red · r gail , π ωi ).

Sample θ ∈ Θ 10:θ =MINIMIZE(fθ, f θ , τ ) 11:
...
As r red tends to be very small (ideally zero) for (s, a) ∈ supp(π E ), r sail discourages the agent from exploring those state-actions by masking away the rewards.
...
Formally, let (s, a) ∈ supp(π E ).

Prototypical learning bounds for an estimator of the support r ≥ 0 provide high probability bounds in the form of P(r(s, a) ≤ c log(1/δ)n −α ) > 1 − δ for any confidence δ ∈ (0, 1], with c a constant not depending on δ or the number n of samples (i.e., expert state-actions).

Here, α > 0 represents the learning rate, namely how fast the estimator is converging to the support.

By choosing the reward in Eq. (5), we are leveraging the faster learning rates between α red and α gail , with respect to support estimation.

At the time being, no results are available to characterize the sample complexity of GAIL (loosely speaking, the α and c introduced above).
...
Assume that for any (s, a) ∈ supp(π E ) the rewards for RED and GAIL have the following learning rates in estimating the support

Then, for any δ ∈ (0, 1] and any (s, a) ∈ supp(π E ), the following holds

with probability at least 1 − δ, where R red and R gail are the upper bounds for r red and r gail , respectively.
...
For any (s, a) ∈ supp(π E ) and any δ ∈ (0, 1], we assume that
...
Following the kernel-based version of RED (Wang et al., 2019) , we can borrow previous results from the set learning literature, which guarantee RED to have a rate of α red = 1/2 (De Vito et al., 2014; Rudi et al., 2017) .
...
We present them in the table below, including discriminator learning rate l D , discount factor γ, number of policy steps per iteration n G , and whether the policy has fixed variance.
