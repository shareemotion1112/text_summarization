...
z textual t,l is a scalar value computed as the correlation between word l of the instruction and previous hidden state h t−1 , and α t is the attention weight over features in X at time t. Based on the textual attention distribution, the grounded textual featurex t can be obtained by the weighted sum over the textual featuresx t = α T t X.Visual grounding.
...
The visual attention weight β t can be obtained as: DISPLAYFORM2 where g is a one-layer Multi-Layer Perceptron (MLP), W v are parameters to be learnt.

Similar to Eq. 2, the grounded visual featurev t can be obtained by the weighted sum over the visual featureŝ v t = β T t V .
...
The hidden state output is then concatenated with the attention weights α t on textual-grounding to estimate how close the agent is to the goal 3 .

The output of the progress monitor p pm t , which represents the completeness of instruction-following, is computed as: DISPLAYFORM0 where W h and W pm are the learnt parameters, c t is the cell state of the LSTM, ⊗ denotes the element-wise product, and σ is the sigmoid function.
...
DISPLAYFORM0 where p k,t is the action probability of each navigable direction, λ = 0.5 is the weight balancing the two losses, and y nv t is the ground-truth navigable direction at step t. Inference.
...
We set the maximum length of instruction to be 80, thus the dimension of the attention weights of textual grounding α t is also 80.
