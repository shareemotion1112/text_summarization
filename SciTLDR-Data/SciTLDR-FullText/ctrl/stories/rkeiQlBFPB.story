...
WarpGrad preconditioning is embedded in task-learners f by interleaving warp-layers (ω (1) , ω (2) ) between each task-learner's layers (h (1) , h (2) ).

WarpGrad achieve preconditioning by modulating layer activations in the forward pass and gradients in the backward pass by backpropagating through warp-layers (Dω), which implicitly preconditions gradients by some matrix (P ).

Warp-parameters (φ) are meta-learned over the joint search space induced by task adaptation (E θ [J(φ)]) to form a geometry that facilitates task learning.
...
WarpGrad belongs to the family of optimisation-based meta-learners that parameterise an update rule θ ← U (θ; ξ) with some meta-parameters ξ.

Specifically gradient-based meta-learners define an update rule by relying on the gradient descent, U (θ; ξ) := θ − α∇ L(θ) for some objective L and learning rate α.

A task is defined by a training set D τ train and a test set D τ test , which defines learning objectives L D τ (θ) := E (x,y)∼D τ [ (f (x, θ), y)] over the task-learner f for some loss .

MAML (Finn et al., 2017 ) meta-learns a shared initialisation θ 0 by backpropagating through K steps of gradient descent across a given task distribution p(τ ),

θ 0 θ 0

Figure 2: Gradient-based meta-learning.

Colours denote different tasks (τ ), dashed lines denote backpropagation through the adaptation process, and solid black lines denote optimiser parameter (φ) gradients w.r.t.

one step of task parameter (θ) adaptation.

Left: A meta-learned initialisation compresses trajectory information into a single initial point (θ 0 ).
...
Task adaptation defines an empirical distribution p(τ, θ) over which WarpGrad learns a geometry for adaptation by optimising for steepest descent directions.
...
These methods optimise for meta-parameters ξ = {θ 0 , φ} by backpropagating through the gradient descent process, as per Eq. 1.
...
A preconditioned gradient descent rule, U (θ; φ) := θ − αP (θ; φ)∇ L(θ), defines a geometry via P .

To disentangle the expressive capacity of this geometry from the expressive capacity of the tasklearner f , we take inspiration from T-Nets that embed linear projections T in feed-forward layers, h = σ(T W x+b).
...
WarpGrad meta-learns a warp ω to produce better update directions (magenta; Section 2.4).
...
where D x and D θ denote the Jacobian with respect to input and parameters, respectively.

In the special case where f is feed-forward and each ω a linear projection, we obtain an instance of WarpGrad that is akin to T-Nets since preconditioning is given by D x ω = T .
...
WarpGrad learns to produce warped loss surfaces (illustrated on two tasks τ and τ ) that are smoother and more well-behaved than their respective native loss-surfaces.
...
Let Ω represent the effect of warp-layers by a reparameterisation h (i) (x; Ω(θ; φ)

∀x, i that maps from a space P onto the manifold W with γ = Ω(θ; φ).

We induce a metric G on W by push-forward (Figure 2) :

where

Provided Ω is not degenerate (G is non-singular), G −1 is positivedefinite, hence a valid Riemann metric.
...
Warp parameters φ control the geometry induced by warping, and therefore what task-learners converge to.

By meta-learning φ we can accumulate information that is conducive to task adaptation but that may not be available during that process.
...
by a task-learnerf that is embedded with a shared WarpGrad optimiser, a meta-training objective L τ meta , and a task adaptation objective L τ task .

We use L τ task to adapt task parameters θ and L τ meta to adapt warp parameters φ.
...
To obtain our meta-objective, we recast the canonical objective (Eq. 10) in terms of θ using first-order equivalence of gradient steps (Eq. 9).

Next, we factorise p(τ, θ) into p(θ | τ )p(τ ).

Since p(τ ) is given, it remains to consider a sampling strategy for p(θ | τ ).

For meta-learning of warp-layers, we assume this distribution is given.

We later show how to incorporate meta-learning of a prior p(θ 0 | τ ).

While any sampling strategy is valid, in this paper we exploit that task learning under stochastic gradient descent can be seen as sampling from an empirical prior p(θ | τ ) (Grant et al., 2018) ; in particular, each iterate θ K and sampling such chains defines an empirical distribution p(θ | τ ) around some prior p(θ 0 | τ ), which we will discuss in Section 2.5.

The joint distribution p(τ, θ) defines a joint search space across tasks.

Meta-learning therefore learns a geometry over this space with the steepest expected direction of descent.

This direction is however not with respect to the objective that produced the gradient, L τ task , but with respect to L τ meta ,

Decoupling the task gradient operator ∇L τ task from the geometry learned by L τ meta lets us infuse global knowledge in warp-layers, a promising avenue for future research (Metz et al., 2019; Mendonca et al., 2019) .

As an example of this, in Section 4.3 we meta-learn an update-rule that mitigates catastrophic forgetting by defining L τ meta over current and previous tasks seen at any given point of adaptation.
...
Interestingly, this approximation is a form of multitask learning with respect to φ (Li & Hoiem, 2016; Bilen & Vedaldi, 2017; Rebuffi et al., 2017) that marginalises over task parameters θ τ .

WarpGrad is a method for learning warp layer parameters φ over a joint search space defined by p(τ, θ).

Because WarpGrad takes this distribution as given, we can integrate WarpGrad with methods that define or learn some form of "prior" p(θ 0 | τ ) over θ τ 0 .
...
We use this approach in our Reinforcement Learning experiment (Section 4.3); (b) Meta-learned point-estimate: when task adaptation occurs in batch mode, we can meta-learn a shared initialisation θ 0 .
...
We incorporate such methods by some objective C (potentially vacuous) over θ 0 that we optimise jointly with WarpGrad,

where L can be substituted byL and λ ∈ [0, ∞) is a hyper-parameter.

We train the WarpGrad optimiser via stochastic gradient descent.

We solve Eq. 13 by alternating between sampling task parameters from p(τ, θ) given the current parameter values for φ and taking meta-gradient steps over these samples to update φ.
...
In Algorithm 1 we illustrate a simple online algorithm with constant memory and linear complexity in K, assuming the same holds for C. A drawback of this approach is that it is relatively data inefficient; in Appendix B we detail a more complex offline training algorithm that stores task parameters in a replay buffer for mini-batched training of φ.
...
Require: p(τ ): distribution over tasks Require: α, β, λ: hyper-parameters 1: initialise φ and p(θ 0 | τ ) 2: while not done do

sample mini-batch of task T from p(τ ) 4:
...
end for sample mini-batch of task T from p(τ )
...
Warp parameters are meta-learned to prevent catastrophic forgetting by defining L τ meta to be the average task loss over current and previous sub-tasks, for each sub-task in a task sequence.
...
In our Omniglot experiment, we found offline meta-training to converge faster: in fact, a mini-batch size of 1 (i.e. η = 1 in Algorithm 2 converges rapidly without any instability.
...
It is defined by applying Leap (Flennerhag et al., 2019) to θ 0 (Eq. 16),
...
In our experiment, both L τ task and L τ meta are the advantage actor-critic objective (Wang et al., 2016) ; C is computed on one batch of 30 episodes, whereas L is accumulated over η = 30 such batches, for a total of 900 episodes.
...
Specifically, let E τ = {s 0 , a 1 , r 1 , s 1 , . . .

, s T , a T , r T , s T +1 } denote an episode on task τ , where s denotes state, a action, and r instantaneous reward.

Denote a mini-batch of randomly sampled task episodes by E = {E τ } τ ∼p(τ ) and an ordered set of k consecutive mini-batches by Sample mini-batch of task B from p(τ )
...
g φ ← g φ + ∇L(φ; θ Sample mini-batch of task B from p(τ )

for all τ ∈ B do 6:

g θ ← g θ + ∇C(θ; φ) Sample mini-batch of task B from p(τ )
...
end while 24: end while and C multi (θ;
...
Hence, the initialisation for each task sequence is a fixed random initialisation, (i.e. λC(θ 0 ) = 0).
...
Importantly, because WarpGrad defines task adaptation abstractly by a probability distribution, we can readily implement a continual learning objective by modifying the joint task parameter distribution p(τ, θ) that we use in the meta-objective (Eq. 11).

A task defines a sequence of sub-tasks over which we generate parameter trajectories θ τ .
...
We define the conditional distribution p(θ | τ ) as before by sampling sub-task parameters θ τt from a mini-batch of such task trajectories, keeping track of which sub-task t it belongs to and which sub-tasks came before it in the given task sequence τ .

The meta-objective is constructed, for any sub-task parameterisation θ τt , as
...
where each task f τ is defined by scale and rotation functions g τ that are randomly sampled from a predefined distribution.
...
To visualise the geometry, we use an explicit warp Ω defined by a 2-layer feed-forward network with a hidden-state size of 30 and tanh non-linearities.
...
In our main experiment, each ω i is simply a 3 × 3 convolutional layer with zero padding; in Appendix F we consider both simpler and more sophisticated versions.
...
Both L τ meta and L τ task are defined as the negative log-likelihood loss; importantly, we evaluate them on different batches of task data to ensure warp-layers encourage generalisation.

We found no additional benefit in this experiment from using held-out data to evaluate L τ meta .
...
Unless otherwise noted, we used a batch size of η = 1.

For each mini-batch, we update φ by applying gradient descent under the canonical meta-objective (Eq. 11), where we evaluate L τ meta on a randomly sampled mini-batch of data from the corresponding task.

Consequently, for each meta-batch, we take (up to) 2000 meta-gradient steps on warp parameters φ.
...
That is, in this experiment, we set λC(θ 0 ) = 0.
...
Because task training is a continuous stream of batches of episodes, we accumulating the meta-gradient using the approximate objective (Eq. 12, where L τ task and L τ meta are both the same advantage actor-critic objective) and update warp-parameters on every 30th task parameter update.
...
Each non-linearity is followed by a residual warping block consisting of 2-layer feedforward networks with 100 hidden units and tanh non-linearities, with meta-learned parameters φ which are fixed during the task adaptation process.

Continual learning as task adaptation The task target function g τ is partition into 5 sets of subtask.

The task-learner sees one partition at a time and is given n = 20 gradient steps to adapt, for a total of K = 100 steps of online gradient descent updates for the full task sequence; recall that every such sequence starts from a fixed random initialisation θ 0 .
...
The data distribution changes after each n = 20 steps with inputs x coming from the next sub-interval and targets form the same function g τ (x).
...
Adaptation to each sub-task uses sub-task data only to form task parameter updates θ

We used a constant learning rate α = 0.001.

Warp-parameters φ are fixed across the full task sequence during adaptation and are meta-learned across random samples of task sequences, which we describe next.
...
We define the meta-objective L τ meta as an incremental multitask objective that, for each sub-task!τ t in a given task sequence τ , averages the validation sub-task losses (Eq. 22) for the current and every preceding loss in the task sequence.

The task meta-objective is defined by summing over all sub-tasks in the task sequence.

For some sub-task parameterisation θ τt , we have
