...
The study showed that, writing the discriminator function as D(f , c) := A(g(f , c)), the condition information can be effectively provided to the discriminator using the inner-product of two vectors, c and φ(f ), as follows:

where A(·) denotes an activation function (sigmoid in our case), c denotes condition embedding, φ(f ) denotes output from the inner layer of discriminator and ψ(·) denotes a function that maps input vector to a scalar value (fully-connected layer in our case).
...
Accordingly, we can rewrite the Eq. 2 by providing the condition with the trained speech encoder c = S(s) and substituting φ(·) with a trained face encoder F(·) from the subsection 3.1 as follows:
...
Combining the condition term c T φ(f ) and relGANs loss, g(f , c) can be modified to g rel (f real , f f ake , c) as follows:

Eq. 4 is formulated to produce a face from the paired speech, but it can cause catastrophic forgetting on the trained φ(·) because the discriminator is no longer trained to penalize the mismatched face and voice.
...
To train cGANs, we used Adam optimizer using β 1 and β 1 of 0.9 and 0.9, respectively.
